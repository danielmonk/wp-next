...
get_current_screen()->set_help_sidebar(
    '<p><strong>' .
        ...
        '<p>' .
        __(
            '<a href="https://wordpress.org/support/article/users-screen/">Documentation on Managing Users</a>'
        ) .
        '</p>' .
        '<p>' .
        __(
            '<a href="https://wordpress.org/support/article/roles-and-capabilities/">Descriptions of Roles and Capabilities</a>'
        ) .
        '</p>' .
        '<p>' .
...
switch ($wp_list_table->current_action()) {
    ...
    case 'delete':
        ...
        if ($userids && !$users_have_content) {
            if (
                $wpdb->get_var(
                    "SELECT ID FROM {$wpdb->posts} WHERE post_author IN( " .
                        implode(',', $userids) .
                        ' ) LIMIT 1'
                )
            ) {
                ...
                $wpdb->get_var(
                    "SELECT link_id FROM {$wpdb->links} WHERE link_owner IN( " .
                        implode(',', $userids) .
                        ' ) LIMIT 1'
                )
            ) {
...
<h1><?php _e('Delete Users'); ?></h1>
	...
	<div class="error">
		<p><strong><?php _e('Error:'); ?></strong> <?php _e(
...
<ul>
  ...
  foreach ($all_userids as $id) {
      ...
      if ($id == $current_user->ID) {
          ...
          /* translators: 1: User ID, 2: User login. */
          echo '<li><input type="hidden" name="users[]" value="' .
              esc_attr($id) .
              '" />' .
...
<ul>
    ...
    default:
        ...
        if (isset($_GET['update'])):
            switch ($_GET['update']) {
                ...
                case 'remove':
                    $messages[] =
                        '<div id="message" class="updated notice is-dismissible fade"><p>' .
  ...
  if (current_user_can('create_users')) { ?>
	<a href="<?php echo admin_url(
     'user-new.php'
 ); ?>" class="page-title-action"><?php echo esc_html_x(
    'Add New',
    'user'
); ?></a>
