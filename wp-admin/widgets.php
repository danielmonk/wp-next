...
get_current_screen()->set_help_sidebar(
    '<p><strong>' .
        ...
        '<p>' .
        __(
            '<a href="https://wordpress.org/support/article/appearance-widgets-screen/">Documentation on Widgets</a>'
        ) .
        '</p>' .
        '<p>' .
...
if (!current_theme_supports('widgets')) {
    wp_die(
        __(
            'The theme you are currently using isn&#8217;t widget-aware, meaning that it has no sidebars that you are able to change. For information on making your theme widget-aware, please <a href="https://developer.wordpress.org/themes/functionality/widgets/">follow these instructions</a>.'
        )
    );
}
 ...
 printf(__('Widget %s'), $name); ?>
	...
	<?php foreach ($wp_registered_sidebars as $sbname => $sbvalue) {
     ...
     if (
         ...
         'orphaned_widgets' === substr($sbname, 0, 16)
     ) {
         echo '&nbsp;';
     } else {
         if (
	...
	<div class="widget-control-actions">
		<div class="alignleft">
			<?php if (!isset($_GET['addnew'])): ?>
				<input type="submit" name="removewidget" id="removewidget" class="button-link button-link-delete widget-control-remove" value="<?php _e(
        'Delete'
    ); ?>" />
...
<div id="message" class="updated notice is-dismissible"><p><?php echo $messages[
    $_GET['message']
]; ?></p></div>
<?php } ?>
<?php if (isset($_GET['error']) && isset($errors[$_GET['error']])) { ?>
<div id="message" class="error"><p><?php echo $errors[
    $_GET['error']
]; ?></p></div>
<?php } ?>
...
foreach ($wp_registered_sidebars as $sidebar => $registered_sidebar) {
    if (
        false !== strpos($registered_sidebar['class'], 'inactive-sidebar') ||
