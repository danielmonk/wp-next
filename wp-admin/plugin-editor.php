...
if (empty($plugins)) {
	...
	<div class="wrap">
		...
		<div id="message" class="error"><p><?php _e(
...
get_current_screen()->add_help_tab([
    ...
    'content' =>
        '<p>' .
        __(
            'You can use the plugin editor to make changes to any of your plugins&#8217; individual PHP files. Be aware that if you make changes, plugins updates will overwrite your customizations.'
        ) .
        '</p>' .
        '<p>' .
        __(
            'Choose a plugin to edit from the dropdown menu and click the Select button. Click once on any file name to load it in the editor, and make your changes. Don&#8217;t forget to save your changes (Update File) when you&#8217;re finished.'
        ) .
        '</p>' .
        '<p>' .
        __(
...
get_current_screen()->set_help_sidebar(
    '<p><strong>' .
        ...
        '<p>' .
        __(
            '<a href="https://wordpress.org/support/article/plugins-editor-screen/">Documentation on Editing Plugins</a>'
        ) .
        '</p>' .
        '<p>' .
        __(
...
if ('.php' === substr($real_file, strrpos($real_file, '.'))) {
    ...
    if (!empty($functions)) {
        ...
        $docs_select .=
            '<option value="">' . __('Function Name&hellip;') . '</option>';
...
if (!in_array('plugin_editor_notice', $dismissed_pointers, true)):
    ...
    if (
        $referer &&
        !in_array(
            basename(parse_url($referer, PHP_URL_PATH)),
