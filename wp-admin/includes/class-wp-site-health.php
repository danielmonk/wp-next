...
class WP_Site_Health
{
    ...
    private function prepare_sql_data()
    {
        ...
        $this->mysql_min_version_check = version_compare(
            '5.5',
            $this->mysql_server_version,
            '<='
        );
        $this->mysql_rec_version_check = version_compare(
            ...
            if ($active_theme->parent()) {
                ...
                if ($using_default_theme) {
                    $result['description'] .= sprintf(
                        ...
                        __(
                            'We recommend removing any unused themes to enhance your site&#8217;s security.'
                        )
                    );
                } else {
                    $result['description'] .= sprintf(
    ...
    public function get_test_php_extensions()
    {
        ...
        foreach ($modules as $library => $module) {
            $extension = isset($module['extension'])
                ? $module['extension']
                : null;
        ...
        if (!$this->mysql_rec_version_check) {
            ...
            $result['description'] .= sprintf(
                ...
                sprintf(
                    ...
                    __(
                        'For optimal performance and security reasons, we recommend running %1$s version %2$s or higher. Contact your web hosting company to correct this.'
                    ),
    ...
    public function get_test_utf8mb4_support()
    {
        ...
        if (false !== strpos($mysql_client_version, 'mysqlnd')) {
            $mysql_client_version = preg_replace(
                '/^\D+([\d.]+).*/',
                '$1',
    ...
    public function get_test_is_in_debug_mode()
    {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            if (defined('WP_DEBUG_LOG') && WP_DEBUG_LOG) {
                ...
                $result['status'] =
                    0 === strpos(ini_get('error_log'), ABSPATH)
                        ? 'critical'
                        : 'recommended';
        ...
        if (is_wp_error($r)) {
            ...
            $result['description'] .= sprintf(
                ...
                sprintf(
                    ...
                    __(
                        'The REST API call gave the following unexpected result: (%1$d) %2$s.'
                    ),
    ...
    public static function get_tests()
    {
        ...
        if (function_exists('rest_url')) {
            $tests['direct']['rest_availability'] = [
