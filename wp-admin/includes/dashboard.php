...
function wp_network_dashboard_right_now()
{
 ...
 do_action('wpmuadminresult'); ?>
	...
	<form action="<?php echo network_admin_url('users.php'); ?>" method="get">
		<p>
			<label class="screen-reader-text" for="search-users"><?php _e(
       'Search Users'
   ); ?></label>
	...
	<form action="<?php echo network_admin_url('sites.php'); ?>" method="get">
		<p>
			<label class="screen-reader-text" for="search-sites"><?php _e(
       'Search Sites'
   ); ?></label>
...
function _wp_dashboard_recent_comments_row(&$comment, $show_date = true)
{
    ...
    if (current_user_can('edit_comment', $comment->comment_ID)) {
        ...
        $approve_url = esc_url(
            "comment.php?action=approvecomment&p=$comment->comment_post_ID&c=$comment->comment_ID&$approve_nonce"
        );
        $unapprove_url = esc_url(
            "comment.php?action=unapprovecomment&p=$comment->comment_post_ID&c=$comment->comment_ID&$approve_nonce"
        );
        $spam_url = esc_url(
            "comment.php?action=spamcomment&p=$comment->comment_post_ID&c=$comment->comment_ID&$del_nonce"
        );
        $trash_url = esc_url(
            "comment.php?action=trashcomment&p=$comment->comment_post_ID&c=$comment->comment_ID&$del_nonce"
        );
        $delete_url = esc_url(
            "comment.php?action=deletecomment&p=$comment->comment_post_ID&c=$comment->comment_ID&$del_nonce"
        );
...
function wp_dashboard_cached_rss_widget(
    ...
    $loading =
        ...
        __('Loading&hellip;') .
        '</p><div class="hide-if-js notice notice-error inline"><p>' .
	...
	<div id="community-events" class="community-events" aria-hidden="true">
		<div class="activity-block">
			<p>
				...
				<button class="button-link community-events-toggle-location" aria-label="<?php esc_attr_e(
        'Edit city'
    ); ?>" aria-expanded="false">
...
function wp_dashboard_browser_nag()
{
    ...
    if ($response) {
        ...
        if (!empty($response['img_src'])) {
            ...
            $notice .=
                '<div class="alignright browser-icon"><a href="' .
...
function wp_dashboard_php_nag()
{
    ...
    if (isset($response['is_secure']) && !$response['is_secure']) {
        $msg = sprintf(
            ...
            __(
                'Your site is running an insecure version of PHP (%s), which should be updated.'
            ),
            PHP_VERSION
        );
    } else {
        $msg = sprintf(
            /* translators: %s: The server PHP version. */
            __(
                'Your site is running an outdated version of PHP (%s), which should be updated.'
            ),
            PHP_VERSION
        );
    }
	...
	<p>
      ...
      __(
          'PHP is the programming language used to build and maintain WordPress. Newer versions of PHP are created with increased performance in mind, so you may see a positive effect on your site&#8217;s performance. The minimum recommended version of PHP is %s.'
      ),
...
function wp_welcome_panel()
{
	...
	<div class="welcome-panel-column">
		<?php if (current_user_can('customize')): ?>
			...
			<a class="button button-primary button-hero load-customize hide-if-no-customize" href="<?php echo wp_customize_url(); ?>"><?php _e(
    'Customize Your Site'
); ?></a>
		<?php endif; ?>
		<a class="button button-primary button-hero hide-if-customize" href="<?php echo admin_url(
      'themes.php'
  ); ?>"><?php _e('Customize Your Site'); ?></a>
		<?php if (
      current_user_can('install_themes') ||
      (current_user_can('switch_themes') &&
          count(wp_get_themes(['allowed' => true])) > 1)
  ): ?>
			<?php $themes_link = current_user_can('customize')
       ? add_query_arg('autofocus[panel]', 'themes', admin_url('customize.php'))
       : admin_url('themes.php'); ?>
			<p class="hide-if-no-customize">
				<?php /* translators: %s: URL to Themes panel in Customizer or Themes screen. */
    printf(
	...
	<div class="welcome-panel-column">
      ...
      !get_option('page_for_posts')
			...
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-edit-page">' .
           __('Edit your front page') .
           '</a>',
       get_edit_post_link(get_option('page_on_front'))
   ); ?></li>
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-add-page">' .
       ...
       admin_url('post-new.php?post_type=page')
			...
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-write-blog">' .
           __('Add a blog post') .
           '</a>',
       ...
       admin_url('post-new.php?post_type=page')
			...
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-setup-home">' .
       ...
       current_user_can('customize')
		...
		<?php endif; ?>
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-view-site">' .
           __('View your site') .
           '</a>',
       home_url('/')
   ); ?></li>
	...
	<div class="welcome-panel-column welcome-panel-last">
		...
		<?php if (current_theme_supports('widgets')): ?>
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-widgets">' .
           __('Manage widgets') .
           '</a>',
       admin_url('widgets.php')
		...
		<?php if (current_theme_supports('menus')): ?>
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-menus">' .
           __('Manage menus') .
           '</a>',
       admin_url('nav-menus.php')
		...
		<?php if (current_user_can('manage_options')): ?>
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-comments">' .
       ...
       admin_url('options-discussion.php')
		...
		<?php endif; ?>
			<li><?php printf(
       '<a href="%s" class="welcome-icon welcome-learn-more">' .
           __('Learn more about getting started') .
           '</a>',
       __('https://wordpress.org/support/article/first-steps-with-wordpress/')
   ); ?></li>
		</ul>
	</div>
	</div>
