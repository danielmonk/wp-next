 ...
 */
function list_files($folder = '', $levels = 100, $exclusions = [])
{
    if (empty($folder)) {
        return false;
    }
		...
		<div class="notice inline notice-{{ data.type || 'info' }} {{ data.alt ? 'notice-alt' : '' }} {{ data.dismissible ? 'is-dismissible' : '' }} {{ data.classes || '' }}">
         ...
         __('https://wordpress.org/support/article/changing-file-permissions/')
          ...
          'Update anyway, even though it might break your site?'
			...
			<# } #>
			<# if ( data.dismissible ) { #>
				<button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php _e(
        'Dismiss'
    ); ?></span></button>
			<# } #>
		</div>
...
function download_url($url, $timeout = 300, $signature_verification = false)
{
    ...
    if ($signature_verification) {
        ...
        if (!$signature) {
            ...
            if (
                '.zip' === substr($url_path, -4) ||
                '.tar.gz' === substr($url_path, -7)
            ) {
...
function request_filesystem_credentials(
    ...
    $credentials['hostname'] = defined('FTP_HOST')
        ? FTP_HOST
        : (!empty($submitted_form['hostname'])
            ? $submitted_form['hostname']
            : $credentials['hostname']);
    $credentials['username'] = defined('FTP_USER')
        ? FTP_USER
        : (!empty($submitted_form['username'])
            ? $submitted_form['username']
            : $credentials['username']);
    $credentials['password'] = defined('FTP_PASS')
        ? FTP_PASS
        : (!empty($submitted_form['password'])
            ? $submitted_form['password']
            : '');

    // Check to see if we are setting the public/private keys for ssh.
    $credentials['public_key'] = defined('FTP_PUBKEY')
        ? FTP_PUBKEY
        : (!empty($submitted_form['public_key'])
            ? $submitted_form['public_key']
            : '');
    $credentials['private_key'] = defined('FTP_PRIKEY')
        ? FTP_PRIKEY
        : (!empty($submitted_form['private_key'])
            ? $submitted_form['private_key']
            : '');
