    ...
    foreach ($format_meta_urls as $format_meta_url) {
        ...
        if (isset($post_data[$keyed])) {
            update_post_meta(
                $post_ID,
                $keyed,
                wp_slash(esc_url_raw(wp_unslash($post_data[$keyed])))
            );
        }
    }
        ...
        foreach ((array) $post_data['tax_input'] as $taxonomy => $terms) {
            ...
            if ($tax_object && isset($tax_object->meta_box_sanitize_cb)) {
                $translated['tax_input'][
                    $taxonomy
                ] = call_user_func_array($tax_object->meta_box_sanitize_cb, [
                    $taxonomy,
                    $terms,
                ]);
            }
...
function _fix_attachment_links($post)
{
    ...
    foreach ($link_matches[1] as $key => $value) {
        ...
        if (
            ...
            !$rel_id ||
            $url_id != $rel_id ||
            strpos($url_match[0], $site_url) === false
        ) {
            continue;
        }
...
function wp_edit_posts_query($q = false)
{
    ...
    if (
        isset($q['post_status']) &&
        in_array($q['post_status'], $post_stati, true)
    ) {
...
function get_sample_permalink_html($id, $new_title = null, $new_slug = null)
{
    ...
    if (
        false === strpos($permalink, '%postname%') &&
        ...
        if (
            ...
            !(
                ...
                get_option('page_on_front') == $id
            )
        ) {
            $return .=
                '<span id="change-permalinks"><a href="options-permalink.php" class="button button-small" target="_blank">' .
...
function _admin_notice_post_locked()
{
    ...
    if (
        ...
        $sendback_text = get_post_type_object($post->post_type)->labels
            ->all_items;
    }
...
function the_block_editor_meta_boxes()
{
	...
	<form id="toggle-custom-fields-form" method="post" action="<?php echo esc_attr(
     admin_url('post.php')
 ); ?>">
