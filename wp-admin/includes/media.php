...
function get_image_send_to_editor(
    ...
    if ($url) {
        $html =
            '<a href="' . esc_attr($url) . '"' . $rel . '>' . $html . '</a>';
    }
...
function wp_iframe($content_func, ...$args)
{
 ...
 if (
     (is_array($content_func) &&
         ...
         0 === strpos((string) $content_func[1], 'media')) ||
...
function image_attachment_fields_to_save($post, $attachment)
{
    if ('image' === substr($post['post_mime_type'], 0, 5)) {
        if (strlen(trim($post['post_title'])) == 0) {
            $attachment_url = isset($post['attachment_url'])
                ? $post['attachment_url']
                : $post['guid'];
            $post['post_title'] = preg_replace(
                '/\.\w+$/',
                '',
                wp_basename($attachment_url)
            );
            $post['errors']['post_title']['errors'][] = __(
...
function media_upload_form($errors = null)
{
 ...
 do_action(
	...
	<p id="async-upload-wrap">
		<label class="screen-reader-text" for="async-upload"><?php _e(
      'Upload'
  ); ?></label>
...
function media_upload_library_form($errors)
{
	...
	<p id="media-search" class="search-box">
		<label class="screen-reader-text" for="media-search-input"><?php _e(
      'Search Media'
  ); ?>:</label>
  ...
  if ($month_count && !(1 == $month_count && 0 == $arc_result[0]->mmonth)) { ?>
     ...
     'All dates'
       ...
       echo "<option$default value='" .
           esc_attr($arc_row->yyear . $arc_row->mmonth) .
           "'>";
       echo esc_html(
 ...
 elseif (isset($thumb_url[0])): ?>
	...
	<?php if ('image' === substr($post->post_mime_type, 0, 5)): ?>
       ...
       'Alternative Text'
			...
			<input type="text" class="widefat" name="_wp_attachment_image_alt" id="attachment_alt" aria-describedby="alt-text-description" value="<?php echo esc_attr(
       $alt_text
   ); ?>" />
		</p>
        ...
        if (!empty($data[$version]['comments'])) {
            foreach ($data[$version]['comments'] as $key => $list) {
                if ('length' !== $key && !empty($list)) {
                    ...
                    if (
                        'terms_of_use' === $key &&
                        0 === strpos($metadata[$key], 'yright notice.')
                    ) {
...
function wp_get_media_creation_timestamp($metadata)
{
    ...
    switch ($metadata['fileformat']) {
        ...
        case 'webm':
            if (
                isset($metadata['matroska']['comments']['creation_time']['0'])
            ) {
                $creation_date = strtotime(
                    $metadata['matroska']['comments']['creation_time']['0']
                );
            } elseif (
                isset($metadata['matroska']['info']['0']['DateUTC_unix'])
            ) {
                $creation_date =
