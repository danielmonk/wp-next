...
function list_core_update($update)
{
    ...
    $message = '';
    $submit = $is_development_version
        ? __('Update to latest nightly')
        : __('Update now');
    ...
    if (
        ...
        'en_US' === $update->locale &&
        'en_US' !== get_locale() &&
        (!$update->packages->partial && $wp_version == $update->partial_version)
    ) {
 ...
 foreach ($themes as $stylesheet => $theme) {
     ...
     if (!$compatible_wp && !$compatible_php) {
         $compat .=
             '<br>' .
             __(
                 'This update doesn&#8217;t work with your versions of WordPress and PHP.'
             ) .
             '&nbsp;';
         if (
             current_user_can('update_core') &&
         ...
         $compat .=
             '<br>' .
             __(
                 'This update doesn&#8217;t work with your version of WordPress.'
             ) .
             '&nbsp;';
...
get_current_screen()->set_help_sidebar(
    '<p><strong>' .
        ...
        '<p>' .
        __(
            '<a href="https://wordpress.org/support/article/dashboard-updates-screen/">Documentation on Updating WordPress</a>'
        ) .
        '</p>' .
        $help_sidebar_autoupdates .
        '<p>' .
...
if ('upgrade-core' === $action) {
 ...
 if ($current && isset($current->last_checked)) {
     $last_update_check =
         $current->last_checked + get_option('gmt_offset') * HOUR_IN_SECONDS;
 }
 ...
 printf(
     ...
     date_i18n(__('g:i a'), $last_update_check)
 );
 echo ' <a href="' .
