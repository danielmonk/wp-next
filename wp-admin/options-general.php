...
$options_help .=
    '<p>' .
    __(
        'You can set the language, and the translation files will be automatically downloaded and installed (available if your filesystem is writable).'
    ) .
    '</p>' .
    '<p>' .
...
<p class="description" id="tagline-description"><?php _e(
...
<span>
 ...
 ?>
	<br />
	<?php if (in_array($tzstring, timezone_identifiers_list(), true)) {
...
$date_formats = array_unique(
    apply_filters('date_formats', [__('F j, Y'), 'Y-m-d', 'm/d/Y', 'd/m/Y'])
);
...
for ($day_index = 0; $day_index <= 6; $day_index++):
    $selected =
        get_option('start_of_week') == $day_index ? 'selected="selected"' : '';
    echo "\n\t<option value='" .
        esc_attr($day_index) .
        "' $selected>" .
