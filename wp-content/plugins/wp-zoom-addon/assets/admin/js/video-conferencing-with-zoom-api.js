...
function CopyToClipboard(containerid) {    
    ...
    window.getSelection().removeAllRanges();
    alert("Meeting shortcode has been copied");
}
...
(function ($) { 
    ...
    var ZoomAPIJS = {
        ...
        setupDOM: function () {
            ...
            $dom.zoomDatePicker = $('.datetpicker');
            $dom.reportsDatePicker = $('#reports_date');
            $dom.zoomAccountDatepicker = $(".zoom_account_datepicker");
            $dom.usersTable = $('#zvc_users_list_table');
            $dom.meetingsTable = $('#zvc_meetings_list_table');
        ...
        eventListeners: function () {
            ...
            * End Meeting 
            */
            $("img[name='end_meeting']").on('click', this.endMeeting);

            //FOr the Password Hashing API
            $dom.togglePwd.on('click', this.toggleAPISettings.bind(this));
            $dom.toggleToken.on('click', this.toggleTokenSettings.bind(this));
        ...
        initializeDependencies: function () {
            ...
            **********************************************************/
            ...
            if ($dom.usersTable.length > 0) {
                args.oLanguage = {
                    "sEmptyTable": 'No users found, try Sync Zoom Users and refresh the page again'
                };

                $dom.usersTable.dataTable(args);
            } else if ($dom.meetingsTable.length > 0) {
                args.oLanguage = {
                    "sEmptyTable": 'No meetings found, try Sync Meetings & Webinars and refresh the page again'
                };
                
                $dom.meetingsTable.dataTable(args);
            }
        },
        ...
        datePickers: function () {
            ...
            if ($dom.dateTimePicker.length > 0) {
                ...
                var output = d.getFullYear() + '-' +
                    (month < 10 ? '0' : '') + month + '-' +
                    (day < 10 ? '0' : '') + day + ' ' + time;
                $dom.dateTimePicker.each(function (index, dateTimePicker) {
            ...
            if ($dom.reportsDatePicker.length > 0) {
                $dom.reportsDatePicker.datepicker({
                    ...
                    beforeShow: function (input, inst) {
                        setTimeout(function () {
                            inst.dpDiv.css({
                                top: $(".zoom_account_datepicker").offset().top + 35,
                                left: $(".zoom_account_datepicker").offset().left
                            });
                        }, 0);
                    }
                }).focus(function () {
            ...
            if ($dom.zoomDatePicker.length > 0) {
                $dom.zoomDatePicker.datepicker({ dateFormat: "yy-mm-dd" });
                $dom.zoomDatePicker.datepicker('setDate', $dom.zoomDatePicker.data('existingdate'));
            }
        },
        ...
        endMeeting: function () {
            ...
            if (r == true) {
                $('start-meeting-btn-' + meeting_id).show();
        ...
        toggleTokenSettings: function () {
            var inputID = $('#zoom_api_token');
            if ($dom.toggleToken.html() === "Show") {
                $dom.toggleToken.html('Hide');
                inputID.attr('type', 'text');
            } else {
                $dom.toggleToken.html('Show');
                inputID.attr('type', 'password');
            }
        },
