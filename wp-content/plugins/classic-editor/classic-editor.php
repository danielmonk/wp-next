    ...
    class Classic_Editor
    {
        ...
        public static function init_actions()
        {
            ...
            if (is_multisite()) {
                ...
                add_action('update_wpmu_options', [
                    __CLASS__,
            ...
            if (!$settings['hide-settings-ui']) {
                ...
                if ($settings['allow-users']) {
                    ...
                    add_action('profile_personal_options', [
                        __CLASS__,
                        'user_settings',
                    ]);
                }
            ...
            if ($settings['allow-users']) {
                ...
                add_action('admin_head-edit.php', [
                    __CLASS__,
                    'add_edit_php_inline_style',
                ]);

                add_action('edit_form_top', [
                    __CLASS__,
                    'remember_classic_editor',
                ]);
                add_filter(
                    'block_editor_settings',
                    [__CLASS__, 'remember_block_editor'],
                    10,
                    2
                );
            ...
            if ($gutenberg) {
                ...
                remove_filter(
                    'display_post_states',
                    'gutenberg_add_gutenberg_post_state'
                );
                remove_action(
                    'edit_form_top',
        ...
        public static function remove_gutenberg_hooks($remove = 'all')
        {
            ...
            remove_action(
                'admin_notices',
                'gutenberg_wordpress_version_notice'
            );
            remove_action(
                'rest_api_init',
            ...
            remove_action(
                'admin_print_styles',
                'gutenberg_block_editor_admin_print_styles'
            );
            remove_action(
                'admin_print_scripts',
                'gutenberg_block_editor_admin_print_scripts'
            );
            remove_action(
            ...
            remove_action(
                'admin_footer',
                'gutenberg_block_editor_admin_footer'
            );
            ...
            remove_filter(
                'get_edit_post_link',
                'gutenberg_revisions_link_to_editor'
            );
            remove_filter(
                'wp_prepare_revision_for_js',
                'gutenberg_revisions_restore'
            );
        ...
        private static function get_settings($refresh = 'no')
        {
            ...
            if (is_multisite()) {
                $defaults = [
                    'editor' =>
                        get_network_option(null, 'classic-editor-replace') ===
                        'block'
                            ? 'block'
                            : 'classic',
                    'allow-users' => false,
                ...
                if (
                    get_network_option(null, 'classic-editor-allow-sites') !==
                    'allow'
                ) {
            ...
            if (
                (!isset($GLOBALS['pagenow']) ||
                    $GLOBALS['pagenow'] !== 'options-writing.php') &&
                $allow_users
            ) {
        ...
        public static function save_user_settings($user_id)
        {
            if (
                ...
                $editor = self::validate_option_editor(
		...
		<div class="classic-editor-options">
			<p>
				<input type="radio" name="classic-editor-replace" id="classic-editor-classic" value="classic"<?php if (
        $settings['editor'] === 'classic'
    ) {
        echo ' checked';
    } ?> />
				<label for="classic-editor-classic"><?php _ex(
        'Classic editor',
        'Editor Name',
        'classic-editor'
    ); ?></label>
			...
			<p>
				<input type="radio" name="classic-editor-replace" id="classic-editor-block" value="block"<?php if (
        $settings['editor'] !== 'classic'
    ) {
        echo ' checked';
    } ?> />
				<label for="classic-editor-block"><?php _ex(
        'Block editor',
        'Editor Name',
        'classic-editor'
    ); ?></label>
		...
		<div class="classic-editor-options">
			<p>
				<input type="radio" name="classic-editor-allow-users" id="classic-editor-allow" value="allow"<?php if (
        $settings['allow-users']
    ) {
        echo ' checked';
    } ?> />
			...
			<p>
				<input type="radio" name="classic-editor-allow-users" id="classic-editor-disallow" value="disallow"<?php if (
        !$settings['allow-users']
    ) {
        echo ' checked';
    } ?> />
				<label for="classic-editor-disallow"><?php _e(
        'No',
        'classic-editor'
    ); ?></label>
			</p>
        ...
        public static function network_settings()
        {
            ...
            $is_checked =
                ...
                'allow';
            ?>
		<h2 id="classic-editor-options"><?php _e(
      'Editor Settings',
      'classic-editor'
  ); ?></h2>
		<table class="form-table">
       ...
       'classic-editor-network-settings'
        ...
        'classic-editor'
          ...
          'classic-editor'
			...
			<tr>
				...
				<td>
					<input type="checkbox" name="classic-editor-allow-sites" id="classic-editor-allow-sites" value="allow"<?php if (
         $is_checked
     ) {
         echo ' checked';
     } ?>>
					<label for="classic-editor-allow-sites"><?php _e(
         'Allow site admins to change settings',
         'classic-editor'
     ); ?></label>
					<p class="description"><?php _e(
         'By default the block editor is replaced with the classic editor and users cannot switch editors.',
         'classic-editor'
     ); ?></p>
				</td>
			</tr>
        ...
        public static function save_network_settings()
        {
            if (
                ...
                wp_verify_nonce(
                    $_POST['classic-editor-network-settings'],
                    'allow-site-admin-settings'
                )
            ) {
                if (
                    isset($_POST['classic-editor-replace']) &&
                    $_POST['classic-editor-replace'] === 'block'
                ) {
                ...
                if (
                    isset($_POST['classic-editor-allow-sites']) &&
                    $_POST['classic-editor-allow-sites'] === 'allow'
                ) {
                    update_network_option(
                        null,
        ...
        {
            if (
                get_post_meta($post_id, 'classic-editor-remember', true) !==
                $editor
            ) {
        ...
        public static function choose_editor($use_block_editor, $post)
        {
            ...
            if (empty($post->ID) || $post->post_status === 'auto-draft') {
                if (
                    ($settings['editor'] === 'classic' &&
                        !isset($_GET['classic-editor__forget'])) || // Add New
                    (isset($_GET['classic-editor']) &&
                        isset($_GET['classic-editor__forget'])) // Switch to classic editor when no draft post.
                ) {
            ...
            if ($use_block_editor && !$editors['block_editor']) {
                ...
                !$use_block_editor &&
                !$editors['classic_editor'] &&
                $editors['block_editor']
            ) {
        ...
        public static function redirect_location($location)
        {
            if (
                ...
                (isset($_POST['_wp_http_referer']) &&
                    strpos($_POST['_wp_http_referer'], '&classic-editor') !==
                        false)
            ) {
       ...
       $edit_url
   ); ?>"><?php _e('Switch to block editor', 'classic-editor'); ?></a>
		</p>
		<?php
        }
        ...
        public static function add_settings_link($links, $file)
        {
            ...
            if (
                ...
                if (current_filter() === 'plugin_action_links') {
                    $url = admin_url(
                        'options-writing.php#classic-editor-options'
                    );
                } else {
                    $url = admin_url(
                        '/network/settings.php#classic-editor-options'
                    );
                }
        ...
        public static function add_edit_links($actions, $post)
        {
            ...
            $label = sprintf(
                __(
                    'Edit &#8220;%s&#8221; in the classic editor',
                    'classic-editor'
                ),
                $title
            );
            $edit_classic = sprintf(
                '<a href="%s" aria-label="%s">%s</a>',
                esc_url($url),
                esc_attr($label),
                $text
        ...
        public static function add_post_state($post_states, $post)
        {
            ...
            if (!$editors['classic_editor'] && !$editors['block_editor']) {
                ...
                $state =
                    '<span class="classic-editor-forced-state">' .
