...
class Akismet_Admin
{
    ...
    public static function init_hooks()
    {
        ...
        add_action('manage_comments_nav', [
            'Akismet_Admin',
            'check_for_spam_button',
        ]);
        add_action('admin_action_akismet_recheck_queue', [
            'Akismet_Admin',
            'recheck_queue',
        ]);
        add_action('wp_ajax_akismet_recheck_queue', [
            'Akismet_Admin',
            'recheck_queue',
        ]);
        add_action('wp_ajax_comment_author_deurl', [
            'Akismet_Admin',
            'remove_comment_author_url',
        ]);
        add_action('wp_ajax_comment_author_reurl', [
            'Akismet_Admin',
            'add_comment_author_url',
        ]);
        add_action('jetpack_auto_activate_akismet', [
            'Akismet_Admin',
        ...
        add_filter(
            'plugin_action_links_' .
                plugin_basename(plugin_dir_path(__FILE__) . 'akismet.php'),
    ...
    public static function admin_init()
    {
        if (get_option('Activated_Akismet')) {
            ...
            if (!headers_sent()) {
                wp_redirect(
                    add_query_arg(
                        ['page' => 'akismet-key-config', 'view' => 'start'],
    ...
    public static function load_resources()
    {
        ...
        if (
            in_array(
                $hook_suffix,
                apply_filters('akismet_admin_page_hook_suffixes', [
                    'index.php', # dashboard
                    'edit-comments.php',
                    'comment.php',
                    'post.php',
            ...
            wp_register_style(
                'akismet.css',
                plugin_dir_url(__FILE__) . '_inc/akismet.css',
                [],
                AKISMET_VERSION
            );
    ...
    public static function admin_help()
    {
        ...
        if (current_user_can('manage_options')) {
            if (
                !Akismet::get_api_key() ||
                (isset($_GET['view']) && $_GET['view'] == 'start')
            ) {
                //setup page
                ...
                $current_screen->add_help_tab([
                    ...
                    'content' =>
                        ...
                        sprintf(
                            __(
                                'Sign up for an account on %s to get an API Key.',
                                'akismet'
                            ),
    ...
    public static function enter_api_key()
    {
        ...
        foreach (
            ['akismet_strictness', 'akismet_show_user_comments_approved']
            as $option
        ) {
            update_option(
                $option,
        ...
        if (!empty($_POST['akismet_comment_form_privacy_notice'])) {
            self::set_form_privacy_notice_option(
                $_POST['akismet_comment_form_privacy_notice']
            );
        } else {
            ...
            sprintf(
                ...
                esc_url(
                    add_query_arg(
                        ['page' => 'akismet-admin'],
                        admin_url(
                            isset($submenu['edit-comments.php'])
                                ? 'edit-comments.php'
                                : 'edit.php'
                        )
                    )
    ...
    public static function rightnow_stats()
    {
        if ($count = get_option('akismet_spam_count')) {
            ...
            $intro = sprintf(
                __(
                    '<a href="%s">Akismet</a> blocks spam from getting to your blog. ',
                    'akismet'
                ),
                'https://akismet.com/wordpress/'
            );
    ...
    public static function check_for_spam_button($comment_status)
    {
        ...
        echo '<a
            ...
            '"
				' .
            (!in_array('ajax-disabled', $classes)
        ...
        if (
            !(
                ...
                (isset($_REQUEST['action']) &&
                    'akismet_recheck_queue' == $_REQUEST['action'])
            )
        ) {
            return;
        }
    ...
    {
        $akismet_result = get_comment_meta(
            $comment->comment_ID,
            'akismet_result',
            true
        );
        $akismet_error = get_comment_meta(
            $comment->comment_ID,
            'akismet_error',
            true
        );
        $user_result = get_comment_meta(
            $comment->comment_ID,
            'akismet_user_result',
            true
        );
        ...
        $show_user_comments =
            $show_user_comments === 'false' ? false : $show_user_comments; //option used to be saved as 'false' / 'true'
        ...
        if ($history) {
            foreach ($history as $row) {
                ...
                switch ($row['event']) {
                    ...
                    case 'check-ham':
                        $message = esc_html(
                            __('Akismet cleared this comment.', 'akismet')
                        );
                        break;
                    case 'wp-blacklisted':
                    case 'wp-disallowed':
                        $message = sprintf(
                            ...
                            esc_html(
                                __('Comment was caught by %s.', 'akismet')
                            ),
                    ...
                    case 'report-ham':
                        if (isset($row['user'])) {
                            ...
                            $message = esc_html(
                                __(
                                    'This comment was reported as not spam.',
                                    'akismet'
                                )
                            );
                        }
    ...
    {
        if (
            $file == plugin_basename(plugin_dir_url(__FILE__) . '/akismet.php')
        ) {
            $links[] =
                '<a href="' .
                esc_url(self::get_page_url()) .
    ...
    public static function connect_jetpack_user()
    {
        if ($jetpack_user = self::get_jetpack_user()) {
            if (
                isset($jetpack_user['user_id']) &&
                isset($jetpack_user['api_key'])
            ) {
                ...
                if (is_object($akismet_user)) {
                    ...
                    return in_array($akismet_user->status, [
                        'active',
                        'active-dunning',
                        'no-sub',
                    ]);
    ...
    public static function display_start_page()
    {
        if (isset($_GET['action'])) {
            if ($_GET['action'] == 'delete-key') {
                if (
                    isset($_GET['_wpnonce']) &&
                    wp_verify_nonce($_GET['_wpnonce'], self::NONCE)
                ) {
                    delete_option('wordpress_api_key');
                }
            }
        ...
        if (
            $api_key =
                Akismet::get_api_key() &&
                (empty(self::$notices['status']) ||
                    'existing-key-invalid' != self::$notices['status'])
        ) {
            self::display_configuration_page();
            return;
        }
        ...
        if (
            isset($_GET['token']) &&
            preg_match('/^(\d+)-[0-9a-f]{20}$/', $_GET['token'])
        ) {
            $akismet_user = self::verify_wpcom_key('', '', [
                'token' => $_GET['token'],
            ]);
    ...
    public static function display_configuration_page()
    {
        ...
        if (empty(self::$notices)) {
            if (
                ...
                if ($stat_totals['all']->time_saved > 1800) {
                    $total_in_minutes = round(
                        $stat_totals['all']->time_saved / 60
                    );
                    $total_in_hours = round($total_in_minutes / 60);
                    $total_in_days = round($total_in_hours / 8);
                    $cleaning_up = __(
                        'Cleaning up spam takes time.',
                        'akismet'
                    );
            ...
            if (
                !empty($akismet_user->limit_reached) &&
                in_array($akismet_user->limit_reached, ['yellow', 'red'])
            ) {
                $notices[] = [
                    'type' => 'limit-reached',
		...
		*/
        ...
        Akismet::view(
            'config',
            compact('api_key', 'akismet_user', 'stat_totals', 'notices')
        );
    }
    ...
    public static function display_notice()
    {
        ...
        if (
            ...
            ('plugins.php' === $hook_suffix ||
                'edit-comments.php' === $hook_suffix) &&
            !Akismet::get_api_key()
        ) {
        ...
        if (isset($_GET['akismet_recheck_complete'])) {
            ...
            if ($recheck_count === 0) {
                ...
                if ($spam_count === 0) {
                    $message .= __(
                        'No comments were caught as spam.',
                        'akismet'
                    );
                } else {
                    $message .= sprintf(
     ...
     */
    public static function exclude_commentmeta_from_export(
        $exclude,
        $key,
        $meta
    ) {
