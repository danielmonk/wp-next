...
class Akismet
{
    ...
    private static function init_hooks()
    {
        ...
        add_action('akismet_scheduled_delete', [
            'Akismet',
            'delete_old_comments',
        ]);
        add_action('akismet_scheduled_delete', [
        ...
        add_action('akismet_schedule_cron_recheck', [
            'Akismet',
            'cron_recheck',
        ]);
    ...
    public static function check_key_status($key, $ip = null)
    {
        return self::http_post(
            Akismet::build_query(['key' => $key, 'blog' => get_option('home')]),
            'verify-key',
            $ip
        );
    }
    ...
    public static function auto_check_comment($commentdata)
    {
        ...
        foreach ($_SERVER as $key => $value) {
            ...
            if (
                preg_match(
                    "/^(HTTP_|REMOTE_ADDR|REQUEST_URI|DOCUMENT_URI)/",
                    $key
                )
            ) {
                $comment["$key"] = $value;
            ...
            if (self::matches_last_comment($comment)) {
                ...
                if (self::$last_comment['akismet_result'] == 'true') {
                    ...
                    if (
                        ...
                        if (
                            ...
                            function_exists('wp_blacklist_check') &&
                            wp_blacklist_check(
                                $comment->comment_author,
                                $comment->comment_author_email,
                                $comment->comment_author_url,
                    ...
                    self::update_comment_history(
                        ...
                        [
                            'response' => substr(
                                self::$last_comment['akismet_result'],
                                0,
                                50
                            ),
                        ]
                ...
                if (isset(self::$last_comment['comment_as_submitted'])) {
                    update_comment_meta(
                        ...
                        self::$last_comment['comment_as_submitted']
                    );
                }
    ...
    public static function delete_old_comments()
    {
        ...
        while (
            ...
            $format_string = implode(
                ", ",
                array_fill(0, count($comment_ids), '%s')
            );

            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM {$wpdb->comments} WHERE comment_id IN ( " .
                        $format_string .
                        " )",
                    $comment_ids
                )
    ...
    public static function check_db_comment(
        $id,
    ...
    public static function recheck_comment(
        $id,
        $recheck_reason = 'recheck_queue'
    ) {
        ...
        if (is_wp_error($api_response)) {
            ...
            Akismet::update_comment_history($id, '', 'recheck-error', [
    ...
    public static function transition_comment_status(
        ...
        if (
            ...
            (isset($_POST['action']) &&
                in_array($_POST['action'], ['editedcomment'])) ||
    ...
    public static function cron_recheck()
    {
        ...
        foreach ((array) $comment_errors as $comment_id) {
            ...
            if (!empty($event)) {
                ...
                if (
                    $comment &&
                    'unapproved' == wp_get_comment_status($comment_id)
                ) {
                    if ($status == 'true') {
    ...
    public static function add_comment_nonce($post_id)
    {
        ...
        if (
            $akismet_comment_nonce_option == 'true' ||
            $akismet_comment_nonce_option == ''
        ) {
            echo '<p style="display: none;">';
            wp_nonce_field(
                'akismet_comment_nonce_' . $post_id,
                'akismet_comment_nonce',
                false
            );
            echo '</p>';
    ...
    private static function comments_match($comment1, $comment2)
    {
        ...
        $comments_match =
            ...
            intval($comment1['comment_post_ID']) ==
                intval($comment2['comment_post_ID']) &&
            ...
            (substr($comment1['comment_author'], 0, 248) ==
                substr($comment2['comment_author'], 0, 248) ||
                substr(stripslashes($comment1['comment_author']), 0, 248) ==
                    substr($comment2['comment_author'], 0, 248) ||
                substr($comment1['comment_author'], 0, 248) ==
                    substr(stripslashes($comment2['comment_author']), 0, 248) ||
                // Certain long comment author names will be truncated to nothing, depending on their encoding.
                (!$comment1['comment_author'] &&
                    strlen($comment2['comment_author']) > 248) ||
                (!$comment2['comment_author'] &&
                    strlen($comment1['comment_author']) > 248)) &&
            // The email max length is 100 characters, limited by the VARCHAR(100) column type.
            // Same argument as above for only looking at the first 93 characters.
            (substr($comment1['comment_author_email'], 0, 93) ==
                substr($comment2['comment_author_email'], 0, 93) ||
                substr(
                    stripslashes($comment1['comment_author_email']),
                    0,
                    93
                ) == substr($comment2['comment_author_email'], 0, 93) ||
                substr($comment1['comment_author_email'], 0, 93) ==
                    substr(
                        stripslashes($comment2['comment_author_email']),
                        0,
                        93
                    ) ||
                // Very long emails can be truncated and then stripped if the [0:100] substring isn't a valid address.
                (!$comment1['comment_author_email'] &&
                    strlen($comment2['comment_author_email']) > 100) ||
                (!$comment2['comment_author_email'] &&
                    strlen($comment1['comment_author_email']) > 100));
    ...
    public static function last_comment_status($approved, $comment)
    {
        ...
        if (!self::matches_last_comment($comment)) {
            self::log(
                "comment_is_spam mismatched comment, returning unaltered $approved"
            );
            return $approved;
        }
     ...
     */
    public static function disable_moderation_emails_if_unreachable(
        $emails,
        $comment_id
    ) {
        if (
            !empty(self::$prevent_moderation_email_for_these_comments) &&
            !empty($emails)
        ) {
            $comment = get_comment($comment_id);

            foreach (
                self::$prevent_moderation_email_for_these_comments
                as $possible_match
            ) {
    ...
    public static function http_post($request, $path, $ip = null)
    {
        ...
        if ($ssl && is_wp_error($response)) {
            ...
            if (is_wp_error($response)) {
                ...
                Akismet::log(
                    compact('http_akismet_url', 'http_args', 'response')
                );
            }
        }
    ...
    public static function load_form_js()
    {
        ...
        wp_register_script(
            'akismet-form',
            plugin_dir_url(__FILE__) . '_inc/form.js',
            [],
            AKISMET_VERSION,
            true
        );
    ...
    public static function pre_check_pingback($method)
    {
        ...
        if (!empty($pingback_args[1])) {
            ...
            if (
                ...
                (isset($comment['akismet_result']) &&
                    'true' == $comment['akismet_result']) // It was just a normal spam response.
            ) {
                ...
                $wp_xmlrpc_server->error(
                    new IXR_Error(0, 'Invalid discovery target')
                );
