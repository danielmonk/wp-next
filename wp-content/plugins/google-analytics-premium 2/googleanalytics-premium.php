...
final class MonsterInsights
{
    ...
    public static function get_instance()
    {
        if (
            !isset(self::$instance) &&
            !(self::$instance instanceof MonsterInsights)
        ) {
            ...
            if (
                ...
                (!defined('MONSTERINSIGHTS_FORCE_ACTIVATION') ||
                    !MONSTERINSIGHTS_FORCE_ACTIVATION)
            ) {
                add_action('admin_notices', [
                    self::$instance,
                    'monsterinsights_wp_notice',
                ]);
                return;
            }
            ...
            if (is_admin() || (defined('DOING_CRON') && DOING_CRON)) {
                ...
                if ('' === self::$instance->license->get_license_key()) {
                    ...
                    $connect_license = get_option(
                        'monsterinsights_connect',
                        false
                    );
                    if (!empty($connect_license)) {
                        if (
                            !empty($connect_license['key']) &&
                            !empty($connect_license['time']) &&
                            time() - intval($connect_license['time']) <
                                HOUR_IN_SECONDS / 2
                        ) {
                            include_once ABSPATH .
                                'wp-admin/includes/plugin.php'; // Make sure wp_clean_plugins_cache is available.
                            MonsterInsights()->license_actions->verify_key(
                                $connect_license['key']
                            );
                        }
    ...
    public function __clone()
    {
        _doing_it_wrong(
            __FUNCTION__,
            esc_html__('Cheatin&#8217; huh?', 'ga-premium'),
            '6.0.0'
        );
    }
    ...
    public function load_plugin_textdomain()
    {
        ...
        $mi_pro_mofile1 =
            WP_LANG_DIR . '/google-analytics-premium/' . $mi_pro_mofile;

        // Look in wp-content/languages/plugins/google-analytics-premium/ga-premium-{lang}_{country}.mo
        $mi_pro_mofile2 =
            WP_LANG_DIR . '/plugins/google-analytics-premium/' . $mi_pro_mofile;
        ...
        $mi_pro_mofile4 =
            dirname(plugin_basename(MONSTERINSIGHTS_PLUGIN_FILE)) .
            '/pro/languages/';
        $mi_pro_mofile4 = apply_filters(
            'monsterinsights_pro_languages_directory',
            $mi_pro_mofile4
        );
        ...
        $mi_mofile2 =
            WP_LANG_DIR . '/plugins/google-analytics-premium/' . $mi_mofile;
        ...
        $mi_mofile4 =
            dirname(plugin_basename(MONSTERINSIGHTS_PLUGIN_FILE)) .
            '/languages/';
        $mi_mofile4 = apply_filters(
            'monsterinsights_lite_languages_directory',
            $mi_mofile4
        );
    ...
    public function monsterinsights_wp_notice()
    {
		...
		<div class="error">
			<p><?php echo sprintf(
       esc_html__(
           'Sorry, but your version of WordPress does not meet MonsterInsights\'s required version of %1$s3.8%2$s to run properly. The plugin not been activated. %3$sClick here to return to the Dashboard%4$s.',
           'ga-premium'
       ),
       '<strong>',
       '</strong>',
    ...
    public function monsterinsights_lite_notice()
    {
		...
		<div class="error">
			<p><?php echo sprintf(
       esc_html__(
           'Please %1$suninstall%2$s the MonsterInsights Lite Plugin. Your premium version of MonsterInsights may not work as expected until the Lite version is uninstalled.',
           'ga-premium'
       ),
       '<a href="' . $url . '">',
       '</a>'
    ...
    public function require_files()
    {
        ...
        if (is_admin() || (defined('DOING_CRON') && DOING_CRON)) {
            ...
            require_once MONSTERINSIGHTS_PLUGIN_DIR .
                'pro/includes/admin/licensing/license-actions.php';
            require_once MONSTERINSIGHTS_PLUGIN_DIR .
    ...
    public function require_updater()
    {
        ...
        require_once MONSTERINSIGHTS_PLUGIN_DIR .
            'pro/includes/admin/licensing/updater.php';
    ...
    public function get_tracking_mode()
    {
        if (!isset($this->tracking_mode)) {
            ...
            $this->tracking_mode = monsterinsights_get_option(
                'tracking_mode',
                'gtag'
            );
        }
...
function monsterinsights_activation_hook($network_wide)
{
    ...
    if (
        ...
        wp_die(
            sprintf(
                esc_html__(
                    'Sorry, but your version of WordPress does not meet MonsterInsight\'s required version of %1$s3.8%2$s to run properly. The plugin has not been activated. %3$sClick here to return to the Dashboard%4$s.',
                    'ga-premium'
                ),
                '<strong>',
                '</strong>',
    ...
    if (class_exists('MonsterInsights_Lite')) {
        deactivate_plugins(plugin_basename(__FILE__));
        $lite_file = plugin_basename(
            MonsterInsights_Lite::get_instance()->file
        );
        $deactivate_url = wp_nonce_url(
            admin_url(
                'plugins.php?action=deactivate&plugin=' . urlencode($lite_file)
            ),
            'deactivate-plugin_' . $lite_file
        );
        wp_die(
            sprintf(
                esc_html__(
                    'Please uninstall and remove Google Analytics for WordPress by MonsterInsights before activating MonsterInsights Pro. The Pro version has not been activated.%5$s%5$s %3$sDeactivate the Lite plugin%4$s %1$sReturn to the Plugins list%2$s',
                    'ga-premium'
                ),
                ...
                '<a href="' .
                    $deactivate_url .
                    '" class="button" style="background: #007cba; border-color: #007cba;color: #fff;text-decoration: none;	text-shadow: none;">',
                '</a>',
                '</br>'
            )
        );
