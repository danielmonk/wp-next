...
function sbsw_feed_plugin_init()
{
    ...
    require_once trailingslashit(SBSW_PLUGIN_DIR) .
        'inc/class-sw-display-elements.php';
    ...
    require_once trailingslashit(SBSW_PLUGIN_DIR) .
        'inc/class-sw-cron-updater.php';

    if (is_admin()) {
        require_once trailingslashit(SBSW_PLUGIN_DIR) .
            'inc/admin/class-sw-vars.php';
        require_once trailingslashit(SBSW_PLUGIN_DIR) .
            'inc/admin/admin-functions.php';
        require_once trailingslashit(SBSW_PLUGIN_DIR) .
            'inc/admin/class-sw-admin.php';
        sbsw_admin_init();
    }
}
...
function sbsw_activate($network_wide)
{
    ...
    $cache_cron_interval_selected = isset($settings['cache_cron_interval'])
        ? $settings['cache_cron_interval']
        : '1hour';
    $cache_cron_time = isset($settings['cache_cron_time'])
        ? $settings['cache_cron_time']
        : '12';
    $cache_cron_am_pm = isset($settings['cache_cron_am_pm'])
        ? $settings['cache_cron_am_pm']
        : 'am';
    SW_Cron_Updater::start_cron_job(
        $cache_cron_interval_selected,
        $cache_cron_time,
        $cache_cron_am_pm
    );
...
function sbsw_plugin_updates()
{
    ...
    // setup the updater
    $edd_updater = new EDD_SL_Plugin_Updater(SBSW_STORE_URL, __FILE__, [
