...
class SW_Display_Elements
{
    ...
    public static function get_feed_style($settings)
    {
        ...
        if (
            ...
            if (!empty($settings['itemspacing'])) {
                $styles .=
                    'padding-bottom: ' .
                    (int) $settings['itemspacing'] * 2 .
                    esc_attr($settings['itemspacingunit']) .
                    ';';
            }
            ...
            if (!empty($settings['height'])) {
                $styles .=
                    'height: ' .
                    (int) $settings['height'] .
                    esc_attr($settings['heightunit']) .
                    ';';
            }
            if (
                !empty($settings['contenttextsize']) &&
                $settings['contenttextsize'] !== 'inherit'
            ) {
                $styles .=
                    'font-size: ' .
                    esc_attr($settings['contenttextsize']) .
                    ';';
            }
            //contenttextsize
            $styles .= '"';
    ...
    public static function get_sb_items_style($settings)
    {
        ...
        if (
            !empty($settings['itemspacing']) &&
            $settings['itemspacing'] . $settings['itemspacingunit'] !== '9px'
        ) {
            $styles = ' style="';
            if (
                !empty($settings['itemspacing']) &&
                $settings['itemspacing'] . $settings['itemspacingunit'] !==
                    '9px'
            ) {
                if ($settings['layout'] !== 'list') {
                    $styles .=
                        'padding-right: ' .
                        (int) $settings['itemspacing'] .
                        esc_attr($settings['itemspacingunit']) .
                        ';padding-left: ' .
                        (int) $settings['itemspacing'] .
                        esc_attr($settings['itemspacingunit']) .
        ...
        if (!empty($bg_color)) {
            ...
            if (!empty($bg_color)) {
                $styles .=
                    ...
                    esc_attr(sbsw_hextorgb($bg_color)) .
                    ');';
            }
            $styles .= '"';
        }
    ...
    public static function display_date($post, $plugin, $settings)
    {
        if (
            $plugin === 'facebook' &&
            CustomFacebookFeed\CFF_Parse_Pro::get_status_type($post) === 'event'
        ) {
            return '';
        }

        return SW_Display_Elements::format_date(
            SW_Parse::get_timestamp($post, $plugin),
            $settings
        );
    }
    ...
    {
        return date_i18n(
            SW_Display_Elements::date_format_setting($settings),
            $timestamp + sbsw_get_utc_offset()
        );
    }

    public static function format_date($timestamp, $settings)
    {
        $use_custom = isset($settings['dateformat'])
    ...
    public static function get_media_placeholder(
        $post,
    ...
    public static function get_media_html($post, $settings, $plugin = '')
    {
        ...
        if ($plugin === 'facebook') {
            if (
                CustomFacebookFeed\CFF_Parse_Pro::get_status_type($post) ===
                'shared_story'
            ) {
                return '';
            }
            $media_thumbnail = SW_Display_Elements::get_media_placeholder(
                $post,
                $settings,
                $plugin
            );
    ...
    public static function get_facebook_event_content(
        $post,
        $plugin = '',
        $settings = []
    ) {
        $content = '';
        $title = sbsw_esc_html_with_br(
            CustomFacebookFeed\CFF_Parse_Pro::get_item_title($post)
        );
        ...
        $start_time = CustomFacebookFeed\CFF_Parse_Pro::get_event_start_time(
            $post
        );
        $end_time = CustomFacebookFeed\CFF_Parse_Pro::get_event_end_time($post);
        $location_name = CustomFacebookFeed\CFF_Parse_Pro::get_event_location_name(
            $post
        );
        ...
        $style_options = get_option('cff_style_settings', []);
        $cff_event_date_formatting = isset(
            $style_options['cff_event_date_formatting']
        )
            ? $style_options['cff_event_date_formatting']
            : '';
        $cff_event_date_custom = isset($style_options['cff_event_date_custom'])
            ? $style_options['cff_event_date_custom']
            : '';
        ...
        if (!empty($start_time) || !empty($end_time)) {
            $formatted_start_time = str_replace(
                ['<k>', '</k>'],
                '',
                CustomFacebookFeed\CFF_Utils::cff_eventdate(
                    $start_time,
                    $cff_event_date_formatting,
            ...
            if (!empty($end_time)) {
                $formatted_end_time = str_replace(
                    ['<k>', '</k>'],
                    '',
                    CustomFacebookFeed\CFF_Utils::cff_eventdate(
                        $end_time,
    ...
    public static function get_escaped_bottom_content(
        $post,
        $plugin = '',
        $settings = []
    ) {
        ...
        if ($plugin === 'youtube') {
            ...
            if (!empty($sbsw_caption)) {
                ...
                $content .= sbsw_maybe_shorten_text(
                    sbsw_esc_html_with_br($sbsw_caption),
                    $textlength,
                    true
                );
                $content .= '</p>';
            ...
            $status_type = CustomFacebookFeed\CFF_Parse_Pro::get_status_type(
                $post
            );
            $message = sbsw_esc_html_with_br(
                CustomFacebookFeed\CFF_Parse_Pro::get_message($post)
            );
            ...
            if ($status_type === 'event') {
                $content .= SW_Display_Elements::get_facebook_event_content(
                    $post,
                    $plugin = '',
                    $settings = []
                );
            ...
            if (!empty(trim($content))) {
                $content =
                    '<p class="sbsw-content-text">' .
                    sbsw_maybe_shorten_text($content, $textlength, true) .
                    '</p>';
            }

            if ($status_type === 'shared_story') {
                $attachment_data = CustomFacebookFeed\CFF_Parse_Pro::get_attachments(
                    $post
                );

                $content .= SW_Display_Elements::get_shared_story_html(
                    $attachment_data[0]
                );
            }
        } elseif ($plugin === 'twitter') {
            ...
            $content = sbsw_esc_html_with_br(
                CTF_Parse_Pro::get_tweet_content($post, true, $has_card)
            );
            ...
            if (!empty(CTF_Parse_Pro::get_quoted_user_name($post))) {
                ...
                $content .=
                    '<div class="sbsw-quote-or-card sbsw-quoted-tweet">';
                $content .= '<div class="sbsw-quoted-identity">';
                $content .=
                    '<span class="sbsw-quoted-avatar"><img src="' .
                    CTF_Parse_Pro::get_quoted_avatar($post) .
                    '" alt="@' .
                    esc_attr(CTF_Parse_Pro::get_quoted_user_name($post)) .
                    '"></span>';
                $content .= '<div class="sbsw-quoted-name-date">';
                $content .=
                    '<a href="https://twitter.com/' .
                    CTF_Parse_Pro::get_quoted_user_name($post) .
                    '" target="_blank" rel="noopener noreferrer nofollow" class="sbsw-quoted-screenname">@' .
                    esc_html(CTF_Parse_Pro::get_quoted_user_name($post)) .
                    '</a>';
                $content .=
                    '<span class="sbsw-quoted-date">' .
                    esc_html(
                        SW_Display_Elements::format_date(
                            CTF_Parse_Pro::get_quoted_timestamp($post),
                            $settings
                        )
                    ) .
                    '</span>';
                ...
                $content .= '</div>';
                $content .=
                    '<p class="sbsw-quoted-tweet-text">' .
                    sbsw_esc_html_with_br(
                        CTF_Parse_Pro::get_quoted_text($post)
                    ) .
                    '</p>';
                if (!empty($media_url)) {
                    $quoted_post = CTF_Parse_Pro::get_quoted_post($post);
                    $media_type = SW_Parse::get_media_type(
                        $quoted_post,
                        $plugin
                    );
                    $maybe_play_button_html = SW_Display_Elements::maybe_play_button_html(
                        $media_type
                    );
                    ...
                    $content .=
                        ...
                        esc_url($media_url) .
                        '" alt="' .
                        esc_attr(CTF_Parse_Pro::get_quoted_text($post)) .
                        '">';
                    $lightbox_attribute = SW_Display_Elements::get_lightbox_attributes(
                        [],
                        $quoted_post,
                        $plugin
                    );
                    $content .=
                        ...
                        $lightbox_attribute .
                        '><span class="sbsw-screenreader">' .
                        esc_html(
                            sprintf(
                                __(
                                    'Lightbox link for post with description %s',
                                    'social-wall'
                                ),
                                sbsw_maybe_shorten_text(
                                    CTF_Parse_Pro::get_quoted_text($post),
                                    50
                                )
                            )
                ...
                if ($card_not_empty) {
                    $content .= SW_Display_Elements::get_twitter_card_html(
                        $post
                    );
                }
            } elseif (
                $first_url &&
                CTF_Parse_Pro::should_retrieve_twitter_card($post)
            ) {
                $content .= CTF_Display_Elements_Pro::get_twitter_card_placeholder(
                    $post,
                    $first_url
                );
            }
    ...
    public static function get_twitter_card_html($data)
    {
        ...
        $twitter_card = isset($data['twitter_card'])
            ? $data['twitter_card']
            : $data;
        $image = CTF_Display_Elements_Pro::get_twitter_card_media_html(
            $twitter_card
        );
        $title = CTF_Parse_Pro::get_twitter_card_title($data);
        $description = CTF_Parse_Pro::get_twitter_card_description($data);
        $link_html =
            CTF_Display_Elements_Pro::get_icon('link') .
            CTF_Display_Elements_Pro::get_twitter_card_link_text($data);
    ...
    {
        $url = CustomFacebookFeed\CFF_Parse_Pro::get_attachment_unshimmed_url(
            $data
        );

        $alt = __('Image for shared link', 'social-wall');
        if (
            empty(CustomFacebookFeed\CFF_Parse_Pro::get_sub_attachments($data))
        ) {
            $image_url = CustomFacebookFeed\CFF_Parse_Pro::get_attachment_image(
                $data
            );
            $image =
                '<img src="' .
            ...
            $sub_attachments = CustomFacebookFeed\CFF_Parse_Pro::get_sub_attachments(
                $data
            );
            $image_url = CustomFacebookFeed\CFF_Parse_Pro::get_attachment_image(
                $sub_attachments[0]
        ...
        $description = CustomFacebookFeed\CFF_Parse_Pro::get_attachment_description(
            $data
        );
    ...
    public static function get_boxed_content_html($parts)
    {
        $content =
            '<a href="' .
            esc_url($parts['url']) .
            '" target="_blank" rel="noopener noreferrer nofollow" class="sbsw-twitter-card">';
        $content .= '<div class="sbsw-quote-or-card">';

        $content .= '<div class="sbsw-tc-media">';
        $content .=
            '<span class="sbsw-tc-link-hover">' .
            SW_Display_Elements::get_icon('linkthick') .
            '</span>';
        $content .= $parts['image'];
        $content .= '</div>';
    ...
    public static function get_identity_text($data, $post, $plugin = '')
    {
        ...
        if ($plugin === 'youtube') {
            if (isset($data[$plugin][$identifier])) {
                $name = SBY_Parse_Pro::get_channel_title(
                    $data[$plugin][$identifier]
                );
            }
        } elseif ($plugin === 'instagram') {
            if (isset($data[$plugin][$identifier])) {
                $name = SB_Instagram_Parse_Pro::get_name(
                    $data[$plugin][$identifier]
                );
    ...
    public static function youtube_stats_html(
        $views_count,
        $likes_count,
        $comments_count,
        $settings
    ) {
        $need_counts_flag_class =
            $likes_count === '' &&
            $comments_count === '' &&
            $settings['youtube_stats']
                ? ' sbsw-need-counts'
                : '';

        $html =
            '<span class="sbsw-sby-views sbsw-views' .
            $need_counts_flag_class .
            '">' .
            SW_Display_Elements::get_icon('views') .
            sbsw_format_count($views_count) .
            '</span>';
        if ($settings['youtube_stats']) {
            $html .=
                '<span class="sbsw-sby-likes sbsw-likes">' .
                SW_Display_Elements::get_icon('like') .
                sbsw_format_count($likes_count) .
                '</span>';
            $html .=
                '<span class="sbsw-sby-comments sbsw-comments">' .
                SW_Display_Elements::get_icon('comments') .
                sbsw_format_count($comments_count) .
                '</span>';
        }
    ...
    public static function get_escaped_stats_html(
        $data,
        $post,
        $misc_data,
        $plugin = '',
        ...
        $html = '';
        if ($plugin === 'youtube') {
            $views_count = SBY_Parse_Pro::get_view_count(
                $post,
                $misc_data['youtube']
            );
            $likes_count = SBY_Parse_Pro::get_like_count(
                $post,
                $misc_data['youtube']
            );
            $comments_count = SBY_Parse_Pro::get_comment_count(
                $post,
                $misc_data['youtube']
            );

            $html = SW_Display_Elements::youtube_stats_html(
                $views_count,
                $likes_count,
                $comments_count,
                $settings
            ...
            if (
                SB_Instagram_Parse_Pro::comment_or_like_counts_data_exists(
                    $post
                )
            ) {
                $html =
                    '<span class="sbsw-sbi-likes sbsw-likes">' .
                    SW_Display_Elements::get_icon('heart') .
                    sbsw_format_count($likes_count) .
                    '</span>';
                $html .=
                    '<span class="sbsw-sbi-comments sbsw-comments">' .
                    SW_Display_Elements::get_icon('comments') .
                    sbsw_format_count($comments_count) .
                    '</span>';
            }
            ...
            if ($status_type === 'album') {
                $count_count = CustomFacebookFeed\CFF_Parse_Pro::get_count_count(
                    $post
                );
                $html =
                    '<span class="sbsw-cff-count sbsw-count">' .
                    SW_Display_Elements::get_icon('image') .
                    sbsw_format_count($count_count) .
                    '</span>';
            } elseif ($status_type === 'event') {
                $interested_count = CustomFacebookFeed\CFF_Parse_Pro::get_interested_count(
                    $post
                );
                $attending_count = CustomFacebookFeed\CFF_Parse_Pro::get_attending_count(
                    $post
                );

                $html =
                    '<span class="sbsw-cff-interested sbsw-interested">' .
                    SW_Display_Elements::get_icon('heart') .
                    sbsw_format_count($interested_count) .
                    '</span>';
                $html .=
                    '<span class="sbsw-cff-attending sbsw-attending">' .
                    SW_Display_Elements::get_icon('user') .
                    sbsw_format_count($attending_count) .
                    '</span>';
            } else {
                $likes_count = CustomFacebookFeed\CFF_Parse_Pro::get_likes_count(
                    $post
                );
                $comments_count = CustomFacebookFeed\CFF_Parse_Pro::get_comments_count(
                    $post
                );

                $html =
                    '<span class="sbsw-cff-likes sbsw-likes">' .
                    SW_Display_Elements::get_icon('like') .
                    sbsw_format_count($likes_count) .
                    '</span>';
                $html .=
                    '<span class="sbsw-cff-comments sbsw-comments">' .
                    SW_Display_Elements::get_icon('comments') .
                    sbsw_format_count($comments_count) .
                    '</span>';
            }
            ...
            $html .=
                ...
                '&related=' .
                $screen_name .
                '" class="sbsw-ctf-retweet" target="_blank">' .
                $retweet_icon;
            $html .=
                '<span class="sbsw-ctf-action-count sbsw-ctf-retweet-count">';
            if ($retweet_count > 0) {
                $html .= sbsw_format_count($retweet_count);
            }
            $html .=
                '</span><span class="sbsw-screenreader">Retweet on Twitter ' .
                $id .
                '</span></a>';
            $html .=
                ...
                '&related=' .
                $screen_name .
                '" class="sbsw-ctf-like" target="_blank">' .
                $favorite_icon .
                '';
            $html .=
                '<span class="sbsw-ctf-action-count sbsw-ctf-favorite-count">';
            if ($favorite_count > 0) {
                $html .= sbsw_format_count($favorite_count);
            }
            $html .=
                '</span><span class="sbsw-screenreader">Like on Twitter ' .
                $id .
                '</span></a>';
        }
    ...
    public static function get_escaped_before_identity_html(
        $account_data,
        $post,
        $plugin = ''
    ) {
        ...
        if ($plugin === 'youtube') {
            ...
            if (isset($post['retweeted_status'])) {
                ...
                $content =
                    ...
                    CTF_Parse_Pro::get_retweeter($post) .
                    '" target="_blank" rel="noopener nofollow noreferrer"><span class="sbsw-retweeter-name">' .
                    CTF_Parse_Pro::get_retweeter($post) .
                    ' </span>' .
                    __('Retweeted', 'social-wall') .
                    '</a></div>';
            }
        }
    ...
    public static function get_escaped_share_content(
        $account_data,
        $post,
        $plugin = ''
    ) {
        ...
        $content =
            '<a class="sbsw-share-button" href="JavaScript:void(0);" data-share-data="' .
            esc_attr(sbsw_json_encode($share_data)) .
            '">' .
            SW_Display_Elements::get_icon('share') .
            '<span class="sbsw-screenreader">' .
            __('Share', 'social-wall') .
            ' ' .
            SW_Parse::get_post_id($post, $plugin) .
            '</span></a>';
    ...
    public static function get_lightbox_attributes(
        $account_data,
        $post,
        $plugin = ''
    ) {
        ...
        if ($media_type === 'youtube') {
            ...
            $media_url = SW_Parse::get_media_thumbnail($post, [], $plugin);
        }
        $description = SW_Parse::get_description($post, $plugin);
        $account_link = SW_Parse::get_account_link(
            $account_data,
            $post,
            $plugin
        );
        ...
        $data_array = [
            ...
            'media_url' => $media_url,
            'title' => sbsw_esc_html_with_br(
                sbsw_replace_double_quotes($description)
            ),
        ...
        if (
            ...
            $data_array[
                'title'
            ] = SW_Display_Elements::get_facebook_event_content(
                $post,
                'facebook',
                'settings'
            );
            if (!empty($description)) {
                $data_array['title'] .= sbsw_esc_html_with_br(
                    sbsw_replace_double_quotes($description)
                );
            }
        }
        ...
        $attr =
            ' data-lightbox-info="' . esc_attr(json_encode($data_array)) . '"';

        return $attr;
    }

    public static function get_available_images_attribute(
        $account_data,
        $post,
        $plugin = '',
        $misc_data = []
        ...
        if ($plugin === 'youtube') {
            ...
            $resized = isset($misc_data['instagram']['resized_images'])
                ? $misc_data['instagram']['resized_images']
                : [];
            $data_array = SB_Instagram_Parse_Pro::get_media_src_set(
                $post,
                $resized
            );
        } elseif ($plugin === 'facebook') {
            $data_array = CustomFacebookFeed\CFF_Parse_Pro::get_media_src_set(
                $post
            );
        } elseif ($plugin === 'twitter') {
        ...
        $attr =
            ...
            esc_attr(json_encode($data_array)) .
            '"';
