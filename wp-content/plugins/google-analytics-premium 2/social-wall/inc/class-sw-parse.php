...
class SW_Parse
{
    ...
    public static function get_media_type($post, $settings = [], $plugin = '')
    {
        if (empty($plugin)) {
        ...
        if ($plugin === 'youtube') {
            ...
            $attachments = CustomFacebookFeed\CFF_Parse_Pro::get_attachments(
                $post
            );
            if (!empty($attachments)) {
                $type = CustomFacebookFeed\CFF_Parse_Pro::get_attachment_media_type(
                    $attachments[0]
                );
                if ($type === 'video') {
                    return 'iframe';
                ...
                !empty(CustomFacebookFeed\CFF_Parse_Pro::get_iframe_html($post))
            ) {
                return 'iframe';
            }
            return 'image';
    ...
    public static function get_media_thumbnail(
        $post,
        $settings = [],
        $plugin = ''
    ) {
        ...
        if ($plugin === 'youtube') {
            ...
            $thumbnail_url = SB_Instagram_Parse_Pro::get_media_url(
                $post,
                'lightbox'
            );
        } elseif ($plugin === 'facebook') {
            $thumbnail_url = CustomFacebookFeed\CFF_Parse_Pro::get_media_url(
                $post,
                'lightbox'
            );
        } elseif ($plugin === 'twitter') {
    ...
    public static function get_video_url($post, $plugin)
    {
        ...
        if ($plugin === 'youtube') {
            ...
            $video_url = CustomFacebookFeed\CFF_Parse_Pro::get_media_url(
                $post,
                'lightbox'
            );
        } elseif ($plugin === 'twitter') {
    ...
    public static function get_iframe_url($post, $plugin)
    {
        ...
        if ($plugin === 'youtube') {
            ...
            if (!empty($attachments)) {
                $video_url = CustomFacebookFeed\CFF_Parse_Pro::get_attachment_unshimmed_url(
                    $attachments[0]
                );
                return 'https://www.facebook.com/v2.3/plugins/video.php?href=' .
                    $video_url;
            } else {
                $iframe_html = CustomFacebookFeed\CFF_Parse_Pro::get_iframe_html(
                    $post
                );
    ...
    public static function get_avatar($data, $post, $plugin = '')
    {
        ...
        if ($plugin === 'youtube') {
            if (isset($data[$plugin][$identifier])) {
                $avatar = SBY_Parse_Pro::get_avatar(
                    $data[$plugin][$identifier]
                );
            }
        } elseif ($plugin === 'instagram') {
            if (isset($data[$plugin][$identifier])) {
                $avatar = SB_Instagram_Parse_Pro::get_avatar(
                    $data[$plugin][$identifier]
                );
            }
        } elseif ($plugin === 'facebook') {
        ...
        if ($plugin === 'youtube') {
            ...
            if (!empty($username)) {
                $permalink =
                    ...
                    SB_Instagram_Parse_Pro::get_username($post) .
                    '/';
            } else {
                $permalink =
                    'https://www.instagram.com/' .
                    SB_Instagram_Parse_Pro::get_instagram_url_part(
            ...
            if (
                ...
                $permalink = CustomFacebookFeed\CFF_Parse_Pro::get_permalink(
                    $post
                );
            } else {
                $permalink = CustomFacebookFeed\CFF_Parse_Pro::get_from_link(
                    $post
                );
            }
        } elseif ($plugin === 'twitter') {
