...
{
    $sbsw_settings = get_option('sbsw_settings', []);
    ...
    $capability = current_user_can('manage_options')
        ? 'manage_options'
        : 'manage_social_wall_feed_options';
    $icon =
        'data:image/svg+xml;base64,' .
        base64_encode('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1438 1878" stroke-linejoin="round" stroke-miterlimit="2">
    ...
    $layouts = [
        [
            ...
            'note' => __(
                'Video thumbnails are displayed in columns and play in a lightbox when clicked.',
                $text_domain
            ),
            'options' => [
                [
                    ...
                    'shortcode' => [
                        'example' => '3',
                        'description' => __(
                            'Videos in carousel when 480px screen width or less.',
                            $text_domain
                        ),
                    ],
                ],
                [
                    ...
                    'shortcode' => [
                        'example' => '2',
                        'description' => __(
                            'Columns when 480px screen width or less.',
                            $text_domain
                        ),
                    ],
                ],
                [
                    ...
                    'shortcode' => [
                        'example' => 'false',
                        'description' => __(
                            'Include a row of buttons to filter the feed by social media type at the top of the feed.',
                            $text_domain
                        ),
                    ],
                ],
        ...
        [
            ...
            'image' => 'img/list.png',
            'note' => __(
                'A single columns of videos that play when clicked.',
                $text_domain
            ),
        ],
        [
            ...
            'options' => [
                ...
                [
                    ...
                    'shortcode' => [
                        'example' => '2',
                        'description' => __(
                            'Choose 2 rows to show two posts in a single slide.',
                            $text_domain
                        ),
                    ],
                ],
                [
                    ...
                    'shortcode' => [
                        'example' => 'infinity',
                        'description' => __(
                            'What happens when the last slide is reached.',
                            $text_domain
                        ),
                    ],
                ],
                [
                    ...
                    'shortcode' => [
                        'example' => 'false',
                        'description' => __(
                            'Show arrows on the sides to navigate posts.',
                            $text_domain
                        ),
                    ],
                ],
                [
                    ...
                    'shortcode' => [
                        'example' => 'false',
                        'description' => __(
                            'Show dots below carousel for an ordinal indication of which slide is being shown.',
                            $text_domain
                        ),
                    ],
                ],
                [
                    ...
                    'shortcode' => [
                        'example' => 'true',
                        'description' => __(
                            'Whether or not to change slides automatically on an interval.',
                            $text_domain
                        ),
                    ],
                ],
                [
                    ...
                    'shortcode' => [
                        'example' => '3000',
                        'description' => __(
                            'Duration in milliseconds before the slide changes.',
                            $text_domain
                        ),
                    ],
                ],
    ...
    $display_your_feed_table_headings = [
        [
            'slug' => 'layout',
...
function sbsw_cff_admin_notice()
{
    if (defined('CFFVER') && version_compare(CFFVER, '3.18', '<')) { ?>
     ...
     'An update to the <strong>Custom Facebook Feed</strong> plugin is required to be compatible with the latest version of the <strong>Social Wall</strong> plugin. Please update the plugin on the WordPress  ',
     'social-wall'
 ); ?><a href="<?php echo esc_url(
    admin_url('/plugins.php')
); ?>"><?php echo esc_html__('Plugins page', 'social-wall'); ?></a>.
				</p>
			</div>
		</div>
		<?php }
...
{
    wp_enqueue_style(
        SBSW_SLUG . '_admin_notices_css',
        SBSW_PLUGIN_URL . 'css/sbsw-notices.css',
        [],
        SWVER
    );
    ...
    wp_enqueue_style(
        SBSW_SLUG . '_admin_css',
        SBSW_PLUGIN_URL . 'css/admin.css',
        [],
        SWVER
    );
    wp_enqueue_style(
...
function sbsw_admin_scripts()
{
    ...
    wp_enqueue_script(
        SBSW_SLUG . '_admin_js',
        SBSW_PLUGIN_URL . 'js/admin.js',
        [],
        SWVER
    );
    wp_localize_script(SBSW_SLUG . '_admin_js', 'sbspf', [
...
function sbsw_admin_icon($icon, $class = '')
{
    ...
    if ($icon === 'question-circle') {
        return '<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="question-circle" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-question-circle fa-w-16' .
            $class .
        ...
        return '<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="info-circle" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-info-circle fa-w-16' .
            $class .
            '"><path fill="currentColor" d="M256 8C119.043 8 8 119.083 8 256c0 136.997 111.043 248 248 248s248-111.003 248-248C504 119.083 392.957 8 256 8zm0 110c23.196 0 42 18.804 42 42s-18.804 42-42 42-42-18.804-42-42 18.804-42 42-42zm56 254c0 6.627-5.373 12-12 12h-88c-6.627 0-12-5.373-12-12v-24c0-6.627 5.373-12 12-12h12v-64h-12c-6.627 0-12-5.373-12-12v-24c0-6.627 5.373-12 12-12h64c6.627 0 12 5.373 12 12v100h12c6.627 0 12 5.373 12 12v24z" class=""></path></svg>';
    } elseif ($icon === 'life-ring') {
        return '<svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="life-ring" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-life-ring fa-w-16' .
            $class .
        ...
        return '<svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="envelope" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-envelope fa-w-16' .
            $class .
        ...
        return '<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="chevron-circle-right" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-chevron-circle-right fa-w-16' .
            $class .
            '"><path fill="currentColor" d="M256 8c137 0 248 111 248 248S393 504 256 504 8 393 8 256 119 8 256 8zm113.9 231L234.4 103.5c-9.4-9.4-24.6-9.4-33.9 0l-17 17c-9.4 9.4-9.4 24.6 0 33.9L285.1 256 183.5 357.6c-9.4 9.4-9.4 24.6 0 33.9l17 17c9.4 9.4 24.6 9.4 33.9 0L369.9 273c9.4-9.4 9.4-24.6 0-34z" class=""></path></svg>';
    } elseif ($icon === 'rocket') {
        return '<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="rocket" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-rocket fa-w-16' .
            $class .
            '"><path fill="currentColor" d="M505.05 19.1a15.89 15.89 0 0 0-12.2-12.2C460.65 0 435.46 0 410.36 0c-103.2 0-165.1 55.2-211.29 128H94.87A48 48 0 0 0 52 154.49l-49.42 98.8A24 24 0 0 0 24.07 288h103.77l-22.47 22.47a32 32 0 0 0 0 45.25l50.9 50.91a32 32 0 0 0 45.26 0L224 384.16V488a24 24 0 0 0 34.7 21.49l98.7-49.39a47.91 47.91 0 0 0 26.5-42.9V312.79c72.59-46.3 128-108.4 128-211.09.1-25.2.1-50.4-6.85-82.6zM384 168a40 40 0 1 1 40-40 40 40 0 0 1-40 40z" class=""></path></svg>';
    } elseif ($icon === 'minus-circle') {
        return '<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="minus-circle" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-minus-circle fa-w-16' .
            $class .
            '"><path fill="currentColor" d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8zM124 296c-6.6 0-12-5.4-12-12v-56c0-6.6 5.4-12 12-12h264c6.6 0 12 5.4 12 12v56c0 6.6-5.4 12-12 12H124z" class=""></path></svg>';
    } elseif ($icon === 'times') {
        return '<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="times" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 352 512" class="svg-inline--fa fa-times fa-w-11' .
            $class .
        ...
        return '<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="ellipsis-h" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-ellipsis-h fa-w-16' .
            $class .
            '"><path fill="currentColor" d="M328 256c0 39.8-32.2 72-72 72s-72-32.2-72-72 32.2-72 72-72 72 32.2 72 72zm104-72c-39.8 0-72 32.2-72 72s32.2 72 72 72 72-32.2 72-72-32.2-72-72-72zm-352 0c-39.8 0-72 32.2-72 72s32.2 72 72 72 72-32.2 72-72-32.2-72-72-72z" class=""></path></svg>';
    } else {
        sbsw_icon($icon);
    }
}
...
{
    $sbsw_cache_cron_interval = isset($settings['cache_cron_interval'])
        ? $settings['cache_cron_interval']
        : '';
    $sbsw_cache_cron_time = isset($settings['cache_cron_time'])
        ? $settings['cache_cron_time']
        : '';
    $sbsw_cache_cron_am_pm = isset($settings['cache_cron_am_pm'])
        ? $settings['cache_cron_am_pm']
        : '';

    delete_option('sbsw_cron_report');
    SW_Cron_Updater::start_cron_job(
        $sbsw_cache_cron_interval,
        $sbsw_cache_cron_time,
        $sbsw_cache_cron_am_pm
    );
}
...
function sbsw_activate_license()
{
    ...
    if (isset($_POST['sbsw_license_activate'])) {
        ...
        // Call the custom API.
        $response = wp_remote_get(add_query_arg($api_params, SBSW_STORE_URL), [
            'timeout' => 15,
            'sslverify' => false,
        ]);
...
function sbsw_connect_accounts()
{
    ...
    $current_connected = json_decode(
        str_replace(['\"'], ['"'], $cff_connected_accounts)
    );
    ...
    foreach ($new_connected as $new_connected_account) {
        if (!isset($current_connected->{$new_connected_account['id']})) {
            ...
            foreach ($new_connected_account as $key => $value) {
                $current_connected->{$new_connected_account[
                    'id'
                ]}->{$key} = $value;
            }
        }
...
function sbsw_admin_hide_unrelated_notices()
{
    ...
    foreach (
        ['user_admin_notices', 'admin_notices', 'all_admin_notices']
        as $notices_type
    ) {
        if (
            empty($wp_filter[$notices_type]->callbacks) ||
            !is_array($wp_filter[$notices_type]->callbacks)
        ) {
            continue;
        }
        foreach ($wp_filter[$notices_type]->callbacks as $priority => $hooks) {
            foreach ($hooks as $name => $arr) {
                if (
                    ...
                    unset(
                        $wp_filter[$notices_type]->callbacks[$priority][$name]
                    );
                    continue;
                }
                $class =
