...
if (isset($_POST['sbi_connect_username'])) {
    ...
    $new_account_details = json_decode(
        stripslashes($_POST['sbi_account_json']),
        true
    );
    ...
    if (WPW_SL_ITEM_NAME == 'Custom Facebook Feed WordPress Plugin Smash') {
        ...
        if (isset($cff_ext_options['cff_extensions_multifeed_active'])) {
            $cff_extensions_multifeed_active =
                $cff_ext_options['cff_extensions_multifeed_active'];
        }
        if (isset($cff_ext_options['cff_extensions_date_range_active'])) {
            $cff_extensions_date_range_active =
                $cff_ext_options['cff_extensions_date_range_active'];
        }
        if (isset($cff_ext_options['cff_extensions_featured_post_active'])) {
            $cff_extensions_featured_post_active =
                $cff_ext_options['cff_extensions_featured_post_active'];
        }
        if (isset($cff_ext_options['cff_extensions_album_active'])) {
            $cff_extensions_album_active =
                $cff_ext_options['cff_extensions_album_active'];
        }
        if (isset($cff_ext_options['cff_extensions_lightbox_active'])) {
            $cff_extensions_lightbox_active =
                $cff_ext_options['cff_extensions_lightbox_active'];
        }
        if (isset($cff_ext_options['cff_extensions_masonry_columns_active'])) {
        ...
        if (isset($cff_ext_options['cff_extensions_carousel_active'])) {
            $cff_extensions_carousel_active =
                $cff_ext_options['cff_extensions_carousel_active'];
        }
        if (isset($cff_ext_options['cff_extensions_reviews_active'])) {
            $cff_extensions_reviews_active =
                $cff_ext_options['cff_extensions_reviews_active'];
        }
    }
    is_plugin_active('cff-reviews/cff-reviews.php') ||
    $cff_extensions_reviews_active
        ? ($cff_reviews_active = true)
        : ($cff_reviews_active = false);
    ?>
<div id="cff-admin">
    <input name="cff_page_id" id="cff_page_id" type="hidden" value="<?php echo esc_attr(
        $page_id_val
    ); ?>" data-page-id="<?php echo esc_attr($page_id_val); ?>" />
    <input name="cff_access_token" id="cff_access_token" type="hidden" data-accesstoken="" >
    <input name="cff_connected_accounts" id="cff_connected_accounts" type="hidden" value="<?php echo stripslashes(
        esc_attr($cff_connected_accounts_val)
    ); ?>" />

    <?php cff_after_access_token_retrieved(
        $page_id_val,
        $cff_reviews_active
    ); ?>
</div>
...
if (
    function_exists('sby_attempt_connection') &&
    isset($_GET['sby_access_token'])
) {
...
if (function_exists('cff_admin_modal')) {
    ...
    <div id="cff-admin">
		<?php cff_admin_modal($admin_url_state); ?>
    </div>
	<?php
}
...
<p><?php echo __(
    'Using Social Wall, you can create a wall of multiple different social media feeds. Just use the options below to choose which feeds to add to your wall.',
    'social-wall'
); ?></p><p><?php echo __(
    'Once you\'re done, simply copy and paste the following shortcode block into a page or widget on your site to display your Social Wall. To customize your feed, just click on the "Customize" tab above.',
    'social-wall'
); ?></p>
...
[/social-wall]
        ...
        <div class="sbsw-default-feed-wrap sbsw-instagram" data-plugin="instagram">
                ...
                <?php if (!$empty_connected_accounts) { ?>
                    <div class="sbsw-platform-label">
                        ...
                        <button class="sbsw-add-remove-plugin button"><?php echo __(
                            'Remove',
                            'social-wall'
                        ); ?></button>
                    </div>
                ...
                <h4 class="sbsw-platform-name"><?php echo SW_Display_Elements::get_icon(
                    'instagram'
                ); ?>Instagram</h4>
                ...
                <div class="sbsw-feed-type">
                    ...
                    <select class="sbsw-type-select" name="sbsw_instagram_types">
                        <?php foreach ($available_types as $available_type):
                            $selected =
                                $available_type['shortcode'] ===
                                $default_type_and_terms['type']
                                    ? ' selected'
                                    : ''; ?>
                        <option value="<?php echo esc_attr(
                            $available_type['shortcode']
                        ); ?>"<?php echo $selected; ?>><?php echo esc_html(
    $available_type['label']
); ?></option>
                        <?php
                        endforeach; ?>
                    </select>
                ...
                <div class="sbsw-connected-accounts-wrap">
                    <div class="sbsw-connected-accounts-inner">
                        ...
                        <?php foreach (
                            ...
                            if (
                                $default_type_and_terms['type'] === 'user' ||
                                $default_type_and_terms['type'] === 'tagged'
                            ) {
                                if (
                                    in_array(
                                        $connected_account['username'],
                                        $default_type_and_terms['terms'],
                                        true
                                    ) ||
                                    in_array(
                        ...
                        <div class="sbsw-connected-account<?php echo esc_attr(
                            $wrap_class
                        ); ?>" data-id="<?php echo esc_attr(
    $connected_account['user_id']
); ?>" data-user="<?php echo esc_attr($connected_account['username']); ?>">
                            <button class="sbsw-add-remove-account button<?php echo esc_attr(
                                $button_class
                            ); ?>"><?php echo esc_html(
    $button_text
); ?></button>
	    ...
	    <?php if ($fb_active && $fb_compatible):
         ...
         $if_connected_accounts = $connected_accounts;
         $available_types = $account_and_feed_info['available_types'];
         $empty_connected_accounts = empty($connected_accounts); // var_dump( get_object_vars($connected_accounts) ); // var_dump( $connected_accounts );
         $json_array['facebook'] = [
             'current' => [
                 'type' => $default_type_and_terms['type'],
                 'term' => implode(',', $default_type_and_terms['terms']),
        ...
        <div class="sbsw-default-feed-wrap" data-plugin="facebook">
            ...
            <?php if ($empty_connected_accounts):
                ...
                <div id="cff-admin" style="margin-top: 15px;">
                    <a href="JavaScript:void(0);" class="cff_admin_btn" id="cff_fb_login"><i class="fa fa-facebook-square"></i> <?php _e(
                    ...
                    <div class="sbsw-types-checkbox-wrap">
                        <?php foreach ($available_types as $available_type):
                            $checked = in_array(
                                ...
                                ? ' checked'
                                : ''; ?>
                            <input id="sbsw_fb_types_<?php echo $available_type[
                                'shortcode'
                            ]; ?>" type="radio" value="<?php echo $available_type[
    'shortcode'
]; ?>" name="sbsw_fb_types"<?php echo $checked; ?>><label for="sbsw_fb_types_<?php echo $available_type[
    'shortcode'
]; ?>"><?php echo esc_html($available_type['label']); ?></label><br>
                        <?php
                        endforeach; ?>
                    </div>
                </div>
    ...
    $connected_account->id
); ?>" data-user="<?php echo esc_attr($connected_account->id); ?>">
                                <button class="sbsw-add-remove-account button<?php echo esc_attr(
                                    $button_class
                                ); ?>"><?php echo esc_html(
    $button_text
                                ...
                                <p>
                                    <?php echo esc_html(
                                        urldecode($connected_account->name)
                                    ); ?>
	    ...
	    <?php if ($tw_active && $tw_compatible):
         ...
         ?>
           ...
           $json_array['twitter']['exclude'] = true; ?>
                <div id="ctf-admin">
                <?php CtfAdmin::the_admin_access_token_configure_html(
                    $_GET,
                    'admin.php?page=sbsw',
                    false
                ); ?>
    ...
    $available_type['label']
         ...
         endforeach; ?>
                    ...
                    <div class="sbsw-text-input-wrap">
                        <input type="text" name="sbsw-twitter-text" value="<?php echo esc_attr(
                            $json_array['twitter']['current']['term']
                        ); ?>">
                        <span class="sbsw-text-input-instructions"></span>
                    </div>
	    ...
	    <?php if ($yt_active && $yt_compatible):
         ...
         $available_types = $account_and_feed_info['available_types'];
         $empty_connected_accounts =
             empty($connected_accounts) ||
             isset($connected_accounts['rss_only']);
         $json_array['youtube'] = [
             'current' => [
                 'type' => $default_type_and_terms['type'],
                 'term' => implode(',', $default_type_and_terms['terms']),
           ...
           ?>
                    <div id="sby_admin">
                        <a href="<?php echo $oauth_processor_url .
                            admin_url(
                                'admin.php?page=sbsw'
                            ); ?>" id="sbspf_get_token"><?php echo sprintf(
    __('Connect to %s to Create a Feed', 'youtube-feed'),
    'YouTube'
); ?></a>
                    </div>
                <?php
    ...
    $available_type['label']
         ...
         endforeach; ?>
                    ...
                    <div class="sbsw-text-input-wrap">
                        <input type="text" name="sbsw-youtube-text" value="<?php echo esc_attr(
                            $json_array['youtube']['current']['term']
                        ); ?>">
            ...
            <div class="sbsw-default-feed-wrap sbsw-plugin-missing">
	            ...
	            <?php if ($if_active && !$if_compatible): ?>
                    <div class="sbsw-content">
                        <p><?php echo __(
                            'The version of the Instagram Feed plugin you are using is not compatible with the Social Wall plugin and needs to be updated.',
                            'social-wall'
                        ); ?></p>
                        <a class="button-primary" href="plugins.php"><?php echo __(
                            'Update Instagram Plugin',
                            'social-wall'
                        ); ?></a>
                    </div>
	            <?php else: ?>
                    <div class="sbsw-content">
                        <p><?php echo __(
                            'Add Instagram posts to your Social Wall.',
                            'social-wall'
                        ); ?></p>
                        <a class="button-primary" href="plugin-install.php"><?php echo __(
                            'Install Instagram Plugin',
                            'social-wall'
                        ); ?></a>
                        <p><?php echo __(
                            "Don't have this plugin?",
                            "social-wall"
                        ); ?> <a href="https://smashballoon.com/instagram-feed/" target="_blank"><?php echo __(
     "Get it now",
     "social-wall"
 ); ?></a></p>
                    </div>
        ...
        <?php if (!$fb_active || !$fb_compatible): ?>
            <div class="sbsw-default-feed-wrap sbsw-plugin-missing">
	            ...
	            <?php if ($fb_active && !$fb_compatible): ?>
                    <div class="sbsw-content">
                        <p><?php echo __(
                            'The version of the Custom Facebook Feed plugin you are using is not compatible with the Social Wall plugin and needs to be updated.',
                            'social-wall'
                        ); ?></p>
                        <a class="button-primary" href="plugins.php"><?php echo __(
	            ...
	            <?php else: ?>
                    <div class="sbsw-content">
                        <p><?php echo __(
                            'Add Facebook content to your Social Wall.',
                            'social-wall'
                        ); ?></p>
                        <a class="button-primary" href="plugin-install.php"><?php echo __(
                        ...
                        <p><?php echo __(
                            "Don't have this plugin?",
                            "social-wall"
                        ); ?> <a href="https://smashballoon.com/custom-facebook-feed/" target="_blank"><?php echo __(
     "Get it now",
     "social-wall"
 ); ?></a></p>
                    </div>
        ...
        <?php if (!$tw_active || !$tw_compatible): ?>
            <div class="sbsw-default-feed-wrap sbsw-plugin-missing">
	            ...
	            <?php if ($tw_active && !$tw_compatible): ?>
                    <div class="sbsw-content">
                        <p><?php echo __(
                            'The version of the Custom Twitter Feeds plugin you are using is not compatible with the Social Wall plugin and needs to be updated.',
                            'social-wall'
                        ); ?></p>
                        <a class="button-primary" href="plugins.php"><?php echo __(
	            ...
	            <?php else: ?>
                    <div class="sbsw-content">
                        <p><?php echo __(
                            'Add Tweets to your Social Wall.',
                            'social-wall'
                        ); ?></p>
                        <a class="button-primary" href="plugin-install.php"><?php echo __(
                        ...
                        <p><?php echo __(
                            "Don't have this plugin?",
                            "social-wall"
                        ); ?> <a href="https://smashballoon.com/custom-twitter-feeds/" target="_blank"><?php echo __(
     "Get it now",
     "social-wall"
 ); ?></a></p>
                    </div>
        ...
        <?php if (!$yt_active || !$yt_compatible): ?>
            <div class="sbsw-default-feed-wrap sbsw-plugin-missing">
	            ...
	            <?php if ($yt_active && !$yt_compatible): ?>
                    <div class="sbsw-content">
                        <p><?php echo __(
                            'The version of the Feeds for YouTube plugin you are using is not compatible with the Social Wall plugin and needs to be updated.',
                            'social-wall'
                        ); ?></p>
                        <a class="button-primary" href="plugins.php"><?php echo __(
	            ...
	            <?php else: ?>
                    <div class="sbsw-content">
                        <p><?php echo __(
                            'Add YouTube videos to your Social Wall.',
                            'social-wall'
                        ); ?></p>
                        <a class="button-primary" href="plugin-install.php"><?php echo __(
                        ...
                        <p><?php echo __(
                            "Don't have this plugin?",
                            "social-wall"
                        ); ?> <a href="https://smashballoon.com/youtube-feed/" target="_blank"><?php echo __(
     "Get it now",
     "social-wall"
 ); ?></a></p>
                    </div>
    ...
    <span id="sbsw-account-json" data-json="<?php echo esc_attr(
	...
	<?php foreach ($this->get_sections('configure') as $section): ?>
     ...
     ?>
		<?php if ($section['save_after']): ?>
            <p class="submit"><input class="button-primary" type="submit" name="save" value="<?php esc_attr_e(
                'Save Changes'
            ); ?>" /></p>
		<?php endif; ?>
        <hr>
...
<?php if (
    ...
    isset($_GET['graph_api']) &&
    empty($_POST)
) { ?>
<div id="sbi_admin" class="wrap sbsw-reload" data-reload="<?php echo esc_attr(
    admin_url('admin.php?page=sbsw')
); ?>">
...
<div id="sbi_admin" class="wrap sbsw-reload" data-reload="<?php echo esc_attr(
    ...
    <?php sbi_get_personal_connection_modal(
        $if_connected_accounts,
        'admin.php?page=sbsw'
    ); ?>
</div>
<?php } elseif (isset($_GET['final_response'])) { ?>
    <input type="hidden" id="cff-reloader" class="wrap sbsw-reload" data-reload="<?php echo esc_attr(
        admin_url('admin.php?page=sbsw')
    ); ?>">
<?php }
