...
function sbsw_feed_init($atts, $content = null)
{
    ...
    if (
        empty($atts['includewords']) &&
        !empty($database_settings['includewords'])
    ) {
        $atts['includewords'] = $database_settings['includewords'];
    }
    if (
        empty($atts['excludewords']) &&
        !empty($database_settings['excludewords'])
    ) {
        $atts['excludewords'] = $database_settings['excludewords'];
    }
    if (
        empty($atts['hidephotos']) &&
        !empty($database_settings['hidephotos'])
    ) {
        $atts['hidephotos'] = $database_settings['hidephotos'];
    }

    $plugins_with_atts = sbsw_merge_atts($plugins_with_atts, $atts);
    $plugins_with_atts = sbsw_filter_plugin_with_atts_for_compatibility(
        $plugins_with_atts
    );

    $plugins_types_and_terms = [];
    $plugin_settings = [];
    ...
    if (isset($plugins_with_atts['custom-facebook-feed'])) {
        ...
        $facebook_feed_settings = new CustomFacebookFeed\CFF_Settings_Pro(
            $fb_atts
        );
    ...
    if (isset($plugins_with_atts['youtube-feed'])) {
        ...
        $youtube_feed_settings = new SBY_Settings_Pro(
            $yt_atts,
            $yt_database_settings
        );
    ...
    if (
        isset($plugins_with_atts['custom-twitter-feed']) ||
        isset($plugins_with_atts['custom-twitter-feeds'])
    ) {
        $tw_atts = isset($plugins_with_atts['custom-twitter-feed'])
            ? $plugins_with_atts['custom-twitter-feed']
            : $plugins_with_atts['custom-twitter-feeds'];
    ...
    $social_wall_settings = new SW_Settings(
        $atts,
        $database_settings,
        $plugins_types_and_terms,
        $plugin_settings
    ...
    if (!$social_wall_feed->regular_cache_exists()) {
        ...
        $plugin_num_api_connections = [];
        foreach (
            $plugins_types_and_terms
            as $plugin => $plugins_types_and_term
        ) {
            $num_things = 0;
        ...
        if (isset($plugins_with_atts['instagram-feed'])) {
            ...
            if (
                $instagram_feed->need_posts($num_needed) &&
                $instagram_feed->can_get_more_posts()
            ) {
                while (
                    $instagram_feed->need_posts($num_needed) &&
                    $instagram_feed->can_get_more_posts()
                ) {
                    $instagram_feed->add_remote_posts(
                        $if_settings,
                        $if_feed_type_and_terms,
                        $instagram_feed_settings->get_connected_accounts_in_feed()
                    );
                }
            }
            ...
            $wall_misc_data['instagram'] = [];
            $plugins_index[] = 'instagram-feed';
        }

        if (isset($plugins_with_atts['custom-facebook-feed'])) {
            $facebook_feed = new CustomFacebookFeed\CFF_Feed_Pro(
                $transient_name
            );
            ...
            $fb_settings['apinum'] =
                $settings['num'] * $highest_num_api_connections;
            $fb_settings['showheader'] = true;

            if (
                $facebook_feed->need_posts($num_needed) &&
                $facebook_feed->can_get_more_posts()
            ) {
                while (
                    $facebook_feed->need_posts($num_needed) &&
                    $facebook_feed->can_get_more_posts()
        ...
        if (isset($plugins_with_atts['youtube-feed'])) {
            ...
            if (
                $youtube_feed->need_header(
                    $yt_settings,
                    $yt_feed_type_and_terms
                )
            ) {
                ...
                $first_user = SBY_Parse_Pro::get_channel_id(
                    $first_user_header_data
                );
                $wall_account_data[] = [
            ...
            if (
                $youtube_feed->need_posts($yt_settings['num']) &&
                $youtube_feed->can_get_more_posts()
            ) {
                while (
                    $youtube_feed->need_posts($yt_settings['num']) &&
                    $youtube_feed->can_get_more_posts()
                ) {
                    $youtube_feed->add_remote_posts(
                        $yt_settings,
                        $yt_feed_type_and_terms,
                        $youtube_feed_settings->get_connected_accounts_in_feed()
                    );
            ...
            $wall_misc_data['youtube'] = $youtube_feed->get_misc_data(
                '',
                $post_data
            );
        ...
        if (
            ...
            $twitter_feed->set_remote_header_data(
                $tw_settings,
                $tw_feed_type_and_terms
            );
            $header_data = $twitter_feed->get_header_data();
            if (isset($header_data[0])) {
                $first_user = CTF_Parse_Pro::get_user_name($header_data[0]);
                $wall_account_data['twitter'] = [
                    $first_user => $header_data[0],
                ];
            }
            if (
                $twitter_feed->need_posts($tw_settings['num']) &&
                $twitter_feed->can_get_more_posts()
            ) {
                while (
                    $twitter_feed->need_posts($tw_settings['num']) &&
                    $twitter_feed->can_get_more_posts()
                ) {
                    $twitter_feed->add_remote_posts(
                        $tw_settings,
                        $tw_feed_type_and_terms
                    );
                }
        ...
        $social_wall_feed->cache_feed_data(
            SBSW_CRON_UPDATE_CACHE_TIME,
            $wall_next_pages
        );
    } else {
    ...
    if (!$social_wall_feed->regular_header_cache_exists()) {
            ...
            foreach ($if_feed_type_and_terms as $feed_type => $terms) {
                foreach ($terms as $term) {
                    if ($feed_type === 'users') {
                        ...
                        $user_index++;
                        $instagram_feed->set_remote_header_data(
                            $if_settings,
                            $this_type_and_terms,
                            $instagram_feed_settings->get_connected_accounts_in_feed()
                        );
                        $first_user_header_data = $instagram_feed->get_header_data();
                        $first_user = SB_Instagram_Parse_Pro::get_username(
                            $first_user_header_data
                        );
                        $wall_account_data['instagram'][
                            $first_user
                        ] = $first_user_header_data;
                        ...
                        $wall_account_data['instagram'][
                            $feed_type
                        ] = SB_Instagram_Feed_Pro::get_generic_header_data(
                            $feed_type,
                            $term
                        );
                    }
        ...
        $wall_account_data = isset($wall_account_data)
            ? $wall_account_data
            : [];
        $social_wall_feed->set_header_data($wall_account_data);
        $social_wall_feed->cache_header_data(
            SBSW_CRON_UPDATE_CACHE_TIME,
            $wall_account_data
        );
    } else {
    ...
    $html .= $social_wall_feed->get_the_feed_html(
...
function sbsw_get_next_post_set()
{
    if (
        !isset($_POST['feed_id']) ||
        strpos($_POST['feed_id'], 'sbsw') === false
    ) {
        die('invalid feed ID');
    }

    $feed_id = sanitize_text_field($_POST['feed_id']);
    $atts_raw = isset($_POST['atts'])
        ? json_decode(stripslashes($_POST['atts']), true)
        : [];
    $sanitized_atts = [];
    ...
    if ($social_wall_feed->regular_cache_exists()) {
        ...
        $settings['num'] =
            $settings['num'] * $highest_num_api_connections * ($page + 1);
    }

    if (
        $social_wall_feed->need_posts($settings['num'], $offset, $page) &&
        $social_wall_feed->can_get_more_posts()
    ) {
        $wall_posts = [];
        $wall_next_pages = [];
        $wall_account_data = [];
        ...
        if (isset($plugins_with_atts['instagram-feed'])) {
            ...
            if (
                $instagram_feed->need_posts($num_needed, 0) &&
                $instagram_feed->can_get_more_posts()
            ) {
                while (
                    $instagram_feed->need_posts($num_needed, 0) &&
        ...
        if (isset($plugins_with_atts['youtube-feed'])) {
            ...
            if (
                $youtube_feed->need_posts($num_needed, 0) &&
                $youtube_feed->can_get_more_posts()
            ) {
                while (
                    $youtube_feed->need_posts($num_needed, 0) &&
            ...
            if (!empty($post_data)) {
                ...
                sby_process_post_set_caching(
                    $vid_ids,
                    $youtube_feed_settings->get_transient_name()
                );
            }
            $wall_next_pages[] = $youtube_feed->get_next_pages();
            $wall_misc_data['youtube'] = $youtube_feed->get_misc_data(
        ...
        if (
            ...
            if (
                ...
                $first_user = CTF_Parse_Pro::get_channel_id(
                    $first_user_header_data
                );
                $wall_account_data[] = [
                    $first_user => $first_user_header_data,
            ...
            if (
                $twitter_feed->need_posts($num_needed, 0) &&
                $twitter_feed->can_get_more_posts()
            ) {
                while (
                    $twitter_feed->need_posts($num_needed, 0) &&
    ...
    $should_paginate = $social_wall_feed->should_use_pagination(
        $settings,
        $offset
    );

    $feed_status = ['shouldPaginate' => $should_paginate];
    $social_wall_feed->add_report(
        'Num in cache ' . count($social_wall_feed->get_post_data())
    );
    ...
    $return = [
        'html' => $social_wall_feed->get_the_items_html(
            $post_data,
            $social_wall_feed->get_header_data(),
            $settings,
            $offset
    ...
    if ($filter_type) {
        $return['feedStatus']['filterOutOfPages'] = [];
...
function sbsw_background_processing()
{
    ...
    $return = [];
    if (
        isset($_POST['posts']['youtube']) &&
        function_exists('sby_process_post_set_caching')
    ) {
    ...
    if (
        isset($_POST['posts']['twitter']) &&
        function_exists('ctf_twitter_cards')
    ) {
        $url_item_batch = [];
        ...
        $twitter_card_batch = CTF_Twitter_Card_Manager::process_url_batch(
            $url_item_batch
        );
        ...
        foreach ($twitter_card_batch as $twitter_card_array) {
            ...
            $description = CTF_Parse_Pro::get_twitter_card_description(
                $twitter_card
            );
            $link_html =
                CTF_Display_Elements_Pro::get_icon('link') .
                CTF_Display_Elements_Pro::get_twitter_card_link_text($url);
            ...
            $twitter_return[$twitter_card_array['id']]['cards'] = [
                'html' => $content,
                'url' => $url,
        ...
        $sanitized_ids = [];
        if (
            class_exists('CTF_Resizer') &&
            !empty($_POST['posts']['twitter']['resize'])
        ) {
            ...
            if ($social_wall_feed->regular_cache_exists()) {
                ...
                $posts = SW_Feed::filter_for_plugin(
                    $social_wall_feed->get_post_data(),
                    'twitter'
                );
    ...
    if (
        isset($_POST['posts']['instagram']) &&
        function_exists('sbi_resize_posts_by_id')
    ) {
        $sanitized_ids = [];
    ...
    if (
        isset($_POST['posts']['facebook']) &&
        function_exists('cff_process_submitted_resize_ids')
    ) {
        $sanitized_ids = [];
        ...
        if ($social_wall_feed->regular_cache_exists()) {
            ...
            $resizer = new CustomFacebookFeed\CFF_Resizer(
                $sanitized_ids,
                $feed_id,
                $social_wall_feed->get_post_data(),
                ['disableresize' => false]
...
add_action(
    'wp_ajax_nopriv_sbsw_background_processing',
    'sbsw_background_processing'
);
...
function sbsw_parse_shortcodes($content)
{
    ...
    foreach ($tagnames as $plugin) {
        ...
        if (!empty($parsed_atts)) {
            ...
            $plugins_with_atts[$atts_string[2][$i]] = [];
        }
        $i++;
    }
...
function sbsw_get_not_available_in_shortcode($plugins_with_atts)
{
    ...
    if (isset($plugins_with_atts['instagram-feed'])) {
        if (
            !sbsw_instagram_feed_is_minimum_version() ||
    ...
    if (isset($plugins_with_atts['custom-facebook-feed'])) {
        if (
            !sbsw_facebook_feed_is_minimum_version() ||
    ...
    if (isset($plugins_with_atts['youtube-feed'])) {
        if (
            !sbsw_youtube_feed_is_minimum_version() ||
    ...
    if (
        ...
        if (
            !sbsw_twitter_feed_is_minimum_version() ||
...
{
    $style = !current_user_can('manage_options')
        ? ' style="display:none;"'
        : '';
    ob_start();
    ?>
	<div id="sbsw-mod-error" <?php echo $style; ?>>
      ...
      'social-wall'
		...
		<p><b><?php _e(
      'The following plugins need to be updated or activated to be included in your social wall feed:',
      'instagram-feed'
  ); ?></b>
       ...
       __('Address this on the %splugins%s page'),
       '<a href="' .
...
function sbsw_debug_report($social_wall_feed, $social_wall_settings)
{
	...
	<?php $plugin_type_and_terms = []; ?>
    <p>Type and Terms</p>
    <?php
    foreach ($plugin_type_and_terms as $plugin => $plugin_type_and_term): ?>
    <strong><?php echo esc_html($plugin); ?></strong>
    <ul>
		<?php
  foreach ($plugin_type_and_term as $type => $array_of_terms): ?>
            <li><?php echo esc_html($type); ?>
            <ul>
		<?php foreach ($array_of_terms as $terms_and_params): ?>
            <?php foreach ($terms_and_params as $key => $value): ?>

                <?php echo '<li>' . esc_html($key); ?>: <br>
...
function sbsw_get_resize_data_for_post_set($post_ids, $feed_id = '')
{
    ...
    if (isset($post_ids['facebook'])) {
        $return[
            'facebook'
        ] = CustomFacebookFeed\CFF_Resizer::get_resized_image_data_for_set(
            $post_ids['facebook'],
            $feed_id
        );
    }
    if (isset($post_ids['instagram'])) {
        $return['instagram'] = SB_Instagram_Feed::get_resized_images_source_set(
            $post_ids['instagram'],
            0,
            $feed_id
        );
    ...
    if (isset($post_ids['twitter']) && class_exists('CTF_Resizer')) {
        $return['twitter'] = CTF_Resizer::get_resized_image_data_for_set(
            $post_ids['twitter'],
            0
        );
    }
...
function sbsw_maybe_shorten_text(
    $string,
    $max_characters,
    $with_show_more = false
) {
    ...
    if (!$with_show_more) {
        ...
        $return =
            implode(' ', $first_parts) .
            '<a href="#" class="sbsw-more">...</a><span class="sbsw-remaining">';
...
function sbsw_get_feed_template_part($part, $settings = [])
{
    $file = '';
...
function sbsw_custom_css()
{
    ...
    $css = isset($sbsw_settings['custom_css'])
        ? trim($sbsw_settings['custom_css'])
        : '';
...
function sbsw_custom_js()
{
    ...
    $js = isset($sbsw_settings['custom_js'])
        ? trim($sbsw_settings['custom_js'])
        : '';
...
function sbsw_scripts_enqueue($enqueue = false)
{
    ...
    if (
        isset($sbsw_settings['enqueue_js_in_head']) &&
        $sbsw_settings['enqueue_js_in_head']
    ) {
        wp_enqueue_script(
            'sbsw_scripts',
            trailingslashit(SBSW_PLUGIN_URL) . $js_file,
            ['jquery'],
            SWVER,
            false
        );
    ...
    if (
        isset($sbsw_settings['enqueue_css_in_shortcode']) &&
        $sbsw_settings['enqueue_css_in_shortcode']
    ) {
        wp_register_style(
            'sbsw_styles',
            trailingslashit(SBSW_PLUGIN_URL) . 'css/social-wall.min.css',
            [],
            SWVER
        );
    } else {
    ...
    $data = [
        ...
        'lightboxPlaceholder' =>
            trailingslashit(SBSW_PLUGIN_URL) . 'img/lightbox-placeholder.png',
        'placeholder' =>
            trailingslashit(SBSW_PLUGIN_URL) . 'img/placeholder.png',
    ];

    if (
        method_exists(
            'CustomFacebookFeed\CFF_Utils',
            'cff_get_resized_uploads_url'
        )
    ) {
        $data[
            'cffResizeUrl'
        ] = CustomFacebookFeed\CFF_Utils::cff_get_resized_uploads_url();
    }
