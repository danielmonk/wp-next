...
class SW_Settings
{
    ...
    public function __construct(
        ...
        if (
            empty($atts['colsmobile']) &&
            $this->settings['layout'] === 'masonry'
        ) {
            $this->settings['colsmobile'] =
                $this->settings['masonrycolsmobile'];
        }
        ...
        if (
            ...
            $this->settings['colsmobile'] =
                $this->settings['carouselcolsmobile'];
        }
        $this->settings['num'] = max((int) $this->settings['num'], 0);
        $this->settings['minnum'] = max(
            (int) $this->settings['num'],
            (int) $this->settings['nummobile']
        );
    ...
    public function set_transient_name($transient_name = '')
    {
        if (!empty($transient_name)) {
            ...
            foreach (
                $plugin_type_and_terms
                as $plugin => $feed_type_and_terms
            ) {
                if ($plugin === 'instagram') {
                    ...
                    if (
                        isset($feed_type_and_terms['hashtags_top']) ||
                        isset($feed_type_and_terms['hashtags_recent'])
                    ) {
                        if (isset($feed_type_and_terms['hashtags_recent'])) {
                            $terms_params =
                                $feed_type_and_terms['hashtags_recent'];
                        } else {
                            $terms_params =
                                $feed_type_and_terms['hashtags_top'];
                            $this_term_included_string .= '+';
                        }
                        ...
                        foreach ($terms_params as $term_and_params) {
                            ...
                            $max_length =
                                strlen($full_tag) < 20 ? strlen($full_tag) : 20;
                            $this_term_included_string .= strtoupper(
                                substr($full_tag, 0, $max_length)
                            );
                        }
                    }
                    ...
                    $term_included_string .=
                        strlen($this_term_included_string) > 8
                            ? substr($this_term_included_string, 0, 8)
                            : $this_term_included_string;
                } elseif ($plugin === 'facebook') {
                    $plugins_included_string .= 'f';
                    $this_term_included_string =
                        $feed_type_and_terms['generic'][0]['transient'];
                    $term_included_string .=
                        strlen($this_term_included_string) > 10
                            ? substr($this_term_included_string, 0, 5) .
                                substr($this_term_included_string, -5, 5)
                            : $this_term_included_string;
                    ...
                    if (isset($feed_type_and_terms['channels'])) {
                        foreach (
                            $feed_type_and_terms['channels']
                            as $term_and_params
                        ) {
                            $channel = $term_and_params['term'];
                            $this_term_included_string .= str_replace(
                                'UC-',
                                '',
                                $channel
                            );
                        ...
                        foreach (
                            $feed_type_and_terms['live']
                            as $term_and_params
                        ) {
                            $this_term_included_string .=
                                $term_and_params['term'];
                        }
                    } elseif (isset($feed_type_and_terms['favorites'])) {
                        foreach (
                            $feed_type_and_terms['favorites']
                            as $term_and_params
                        ) {
                    ...
                    if (isset($feed_type_and_terms['usertimeline'])) {
                        foreach (
                            $feed_type_and_terms['usertimeline']
                            as $term_and_params
                        ) {
                            $user = $term_and_params['term'];
                    ...
                    if (isset($feed_type_and_terms['hometimeline'])) {
                        foreach (
                            $feed_type_and_terms['hometimeline']
                            as $term_and_params
                        ) {
                    ...
                    if (isset($feed_type_and_terms['mentionstimeline'])) {
                        foreach (
                            $feed_type_and_terms['mentionstimeline']
                            as $term_and_params
                        ) {
            ...
            foreach ($this->plugin_settings as $plugin => $setting) {
                if (isset($setting['includewords'])) {
                    $includewords = explode(
                        ',',
                        str_replace(' ', '', $setting['includewords'])
                    );
                    ...
                    $excludewords = explode(
                        ',',
                        str_replace(' ', '', $setting['excludewords'])
                    );
                ...
                if (
                    isset($setting['includereplies']) &&
                    $setting['includereplies']
                ) {
                    $settings_string .= 'r';
                }
                if (
                    isset($setting['includeretweets']) &&
                    !$setting['includeretweets']
                ) {
                    $settings_string .= 'n';
                }
            }
