...
class SW_Feed
{
    ...
    public function __construct($transient_name)
    {
        ...
        $this->backup_feed_transient_name =
            SBSW_BACKUP_PREFIX . $transient_name;

        $sbsw_header_transient_name = str_replace(
            'sbsw_',
            'sbsw_header_',
            $transient_name
        );
        $sbsw_header_transient_name = substr(
            $sbsw_header_transient_name,
            0,
            44
        );
        $this->header_transient_name = $sbsw_header_transient_name;

        $sbsw_misc_transient_name = str_replace(
            'sbsw_',
            'sbsw_misc_',
            $transient_name
        );
        ...
        $this->backup_header_transient_name =
            SBSW_BACKUP_PREFIX . $sbsw_header_transient_name;
    ...
    public function append_misc_data($append_misc_data)
    {
        if (isset($this->misc_data['youtube'])) {
            $this->misc_data['youtube']['stats'] = array_merge(
                $this->misc_data['youtube']['stats'],
                $append_misc_data['youtube']['stats']
            );
        }
    }
    ...
    public function cache_feed_data(
        $cache_time,
        $next_pages,
        $save_backup = false
    ) {
        if (!empty($this->post_data)) {
            ...
            set_transient(
                $this->misc_transient_name,
                wp_json_encode($this->misc_data),
                $cache_time
            );
        } else {
    ...
    public function set_post_data_from_cache($atts = [])
    {
        ...
        if ($transient_data) {
            ...
            $this->next_pages = isset($transient_data['pagination'])
                ...
                : [];
            $this->pages_created = isset($transient_data['pages_created'])
                ? $transient_data['pages_created']
                : 0;
            $this->plugins_with_atts = isset(
                $transient_data['plugins_with_atts']
            )
                ? $transient_data['plugins_with_atts']
                : [];
            $this->atts = isset($transient_data['atts'])
                ? $transient_data['atts']
                : [];
            $this->last_retrieve = isset($transient_data['last_retrieve'])
                ? $transient_data['last_retrieve']
                : null;
            $this->last_requested = isset($transient_data['last_requested'])
                ? $transient_data['last_requested']
                : null;
            ...
            $this->add_report(
                'last retrieve: ' .
                    date('Y-m-d H:i:s', $this->last_retrieve) .
                    ', last requested: ' .
                    date('Y-m-d H:i:s', $this->last_requested)
            );
            $this->add_report(
                'pages created: ' .
                    $this->pages_created .
    ...
    public function maybe_set_post_data_from_backup()
    {
        ...
        if ($backup_data) {
            ...
            $post_data = isset($backup_data['data'])
                ? $backup_data['data']
                : [];
            $this->post_data = $post_data;
            $this->next_pages = isset($backup_data['pagination'])
                ? $backup_data['pagination']
                : [];
    ...
    protected function add_other_atts($other_atts, $settings)
    {
        ...
        $colsmobile =
            (int) $settings['colsmobile'] > 0
                ? (int) $settings['colsmobile']
                : 'auto';
        $options_att_arr['colsmobile'] = $colsmobile;

        $layout = $settings['layout'];
        if (!in_array($layout, ['masonry', 'carousel'])) {
            $layout = 'list';
        }
        ...
        if ($layout === 'masonry') {
            ...
            $arrows =
                ...
                $settings['carouselarrows'] == 'on' ||
                $settings['carouselarrows'] == 1 ||
                $settings['carouselarrows'] == '1';
            $pag =
                $settings['carouselpag'] == 'true' ||
                $settings['carouselpag'] == 'on' ||
                $settings['carouselpag'] == 1 ||
                $settings['carouselpag'] == '1';
            $autoplay =
                $settings['carouselautoplay'] == 'true' ||
                $settings['carouselautoplay'] == 'on' ||
                $settings['carouselautoplay'] == 1 ||
                $settings['carouselautoplay'] == '1';
            $time = $autoplay ? (int) $settings['carouseltime'] : false;
            $loop =
                !empty($settings['carouselloop']) &&
                $settings['carouselloop'] !== 'rewind'
                    ? false
                    : true;
            $rows = !empty($settings['carouselrows'])
                ? min((int) $settings['carouselrows'], 2)
                : 1;
            $options_att_arr['carousel'] = [
                $arrows,
                $pag,
                $autoplay,
                $time,
            ...
            $options_att_arr['itemspacing'] =
                (int) $settings['itemspacing'] . $settings['itemspacingunit'];
        } else {
        ...
        if ($moderation_mode) {
            $mod_index = isset($_GET['sbsw_moderation_index'])
                ? sanitize_text_field(
                    substr($_GET['sbsw_moderation_index'], 0, 10)
                )
                : '0';
            $options_att_arr['modindex'] = $mod_index;
            if (!empty($settings['whitelist'])) {
                $white_list_name = $settings['whitelist'];
                $white_list_ids = !empty($settings['whitelist'])
                    ? get_option(
                        'sb_instagram_white_lists_' . $settings['whitelist'],
                        []
                    )
                    : false;
            ...
            $hide_photos = !empty($settings['hidephotos'])
                ? explode(',', str_replace(' ', '', $settings['hidephotos']))
                : [];
        ...
        $other_atts .=
            ' data-options="' .
            esc_attr(wp_json_encode($options_att_arr)) .
            '"';
    ...
    public function get_the_feed_html($posts, $account_data, $settings, $atts)
    {
        ...
        if (
            ...
            $disable_resize = isset($options['cff_disable_resize'])
                ? $options['cff_disable_resize']
                : false;
        ...
        if (
            ...
            if (!class_exists('CTF_Resizer')) {
                ...
                $disable_resize = isset($options['resize'])
                    ? $options['resize'] === 'disable'
                    : false;
        ...
        $settings['showfilter'] =
            $settings['masonryshowfilter'] && $settings['layout'] === 'masonry';
    ...
    public function get_the_items_html(
        $posts,
        $account_data,
        $settings,
        $offset
