...
class SW_Cron_Updater
{
    public static function do_feed_updates()
    {
        ...
        foreach ($feed_caches as $feed_cache) {
            ...
            if ($transient) {
                ...
                $plugins_with_atts = isset($feed_data['plugins_with_atts'])
                    ? $feed_data['plugins_with_atts']
                    : false;
                ...
                $last_retrieve = isset($feed_data['last_retrieve'])
                    ? (int) $feed_data['last_retrieve']
                    : 0;
                $last_requested = isset($feed_data['last_requested'])
                    ? (int) $feed_data['last_requested']
                    : false;
                $report[$feed_id]['last_retrieve'] = date(
                    'Y-m-d H:i:s',
                    $last_retrieve
                );
                if ($plugins_with_atts !== false) {
                    if (
                        !$last_requested ||
                        $last_requested > time() - 60 * 60 * 24 * 60
                    ) {
                        $atts = isset($feed_data['atts'])
                            ? $feed_data['atts']
                            : false;

                        SW_Cron_Updater::do_single_feed_cron_update(
                            $plugins_with_atts,
                            $atts
                        );
                        ...
                        $report[$feed_id]['did_update'] =
                            'no - not recently requested';
                    }
                } else {
    ...
    public static function do_single_feed_cron_update(
        $plugins_with_atts,
        $atts,
        $include_resize = true
    ) {
    ...
    {
        global $wpdb;
        $feed_caches = apply_filters('sbsw_feed_cache_option_names', []);
        ...
        AND `option_name` NOT LIKE ('%\_transient\_sbsw\_misc%');",
            ARRAY_A
        );
    ...
    public static function start_cron_job(
        ...
        if (
            ...
            $add_time =
                $sbi_cache_cron_am_pm === 'pm'
                    ? (int) $sbi_cache_cron_time + 12
                    : (int) $sbi_cache_cron_time;
            $utc_start_time =
                $base_day + ($add_time * 60 * 60 - sbsw_get_utc_offset());
