...
$identity = SW_Display_Elements::get_identity_text(
    $account_data,
    $post,
    $plugin
);
$escaped_before_identity_html = SW_Display_Elements::get_escaped_before_identity_html(
    $account_data,
    $post,
    $plugin
);
...
$follow_button_text = SW_Display_Elements::get_follow_button_text(
    $plugin,
    $settings
);
...
/* Media and light box */
$available_images_attribute = SW_Display_Elements::get_available_images_attribute(
    $account_data,
    $post,
    $plugin,
    $misc_data
...
$escaped_post_bottom_content = SW_Display_Elements::get_escaped_bottom_content(
    $post,
    $plugin,
    $settings
);
...
$escaped_stats_html = SW_Display_Elements::get_escaped_stats_html(
    $account_data,
    $post,
    $misc_data,
    $plugin,
...
$escaped_share_html = SW_Display_Elements::get_escaped_share_content(
    $account_data,
    $post,
    $plugin
);
?>
<div class="sbsw-item sbsw-<?php echo esc_attr(
    $plugin
); ?>-item<?php echo esc_attr($item_classes); ?>" id="sbsw-<?php echo esc_attr(
    $post_id
); ?>"<?php echo $sb_item_style; ?>>
    <div class="sbsw-item-inner"<?php echo $sb_inner_item_style; ?>>
        ...
        <div class="sbsw-item-header<?php echo esc_attr(
            $avatar_class . $date_class
        ); ?>"<?php echo $item_header_style; ?>>
    ...
    __('Account avatar for %s', 'social-wall'),
    esc_attr($identity)
); ?>">
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="sbsw-author">
                        ...
                        <?php if (
                            SW_Parse::get_timestamp($post, $plugin) !== 0 &&
        ...
        <div class="sbsw-item-media"<?php echo $available_images_attribute; ?>>
            ...
            <a href="<?php echo esc_attr(
                $lightbox_image
            ); ?>" class="sbsw-lightbox-hover"<?php echo $lightbox_attribute; ?>><span class="sbsw-screenreader"><?php echo esc_html(
    sprintf(
        __('Lightbox link for post with description %s', 'social-wall'),
        sbsw_maybe_shorten_text($description, 50)
    )
); ?></span></a>
        </div>
        <?php endif; ?>
