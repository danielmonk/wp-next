...
<div class="sb-wall-header">
	<div class="sbsw-filter-bar">
		<div class="sbsw-single-filter sbsw-single-filter-all">
			<a href="javascript:void(0);" data-plugin="all"><?php echo SW_Display_Elements::get_icon(
       'heart'
   ); ?> <span><?php _e('All', 'social-wall'); ?></span></a>
		</div>
	<?php foreach ($plugins_in_feed as $plugin): ?>

		<?php if ($plugin === 'instagram'): ?>
		<div class="sbsw-single-filter sbsw-single-filter-<?php echo esc_attr(
      $plugin
  ); ?>">
			<a href="javascript:void(0);" data-plugin="<?php echo esc_attr(
       $plugin
   ); ?>"><?php echo SW_Display_Elements::get_icon(
    $plugin
