...
$bg_class = empty(str_replace('#', '', $settings['background']))
    ? ''
    : ' sbsw-custom-bg';

do_action('sbsw_before_feed', $posts, $settings);
?>
<div id="sb-wall-<?php echo esc_attr(
    preg_replace("/[^A-Za-z0-9 ]/", '', $feed_id)
); ?>" class="sb-wall sbsw-mobcol-<?php
echo esc_attr($colsmobile_setting);
echo esc_attr($list_class . $bg_class);
?> sbsw-col-<?php
 echo esc_attr($cols_setting);
 echo esc_attr($theme_class);
 ?> cff-light" data-feedid="<?php echo esc_attr(
     $feed_id
 ); ?>" data-shortcode-atts="<?php echo esc_attr(
    $shortcode_atts
); ?>" <?php echo $feed_atts . $feed_style_att; ?>>
    ...
    <div class="sb-wall-items-wrap-outer-wrap">
        <div class="sb-wall-items-wrap sbsw-items-wrap-all sbsw-items-wrap-visible sbsw-items-wrap-current">
			<?php $this->posts_loop($posts, $account_data, $settings); ?>
        </div>
    </div>
	...
	<?php
 $resized_image_data = sbsw_json_encode(
     sbsw_get_resize_data_for_post_set($this->image_ids_post_set, $feed_id)
 );
 $resized_image_data = str_replace('"{', '{', $resized_image_data);
 $resized_image_data = str_replace('}"', '}', $resized_image_data);
 ?>

	<span class="sbsw-resized-image-data" data-feedid="<?php echo esc_attr(
     $feed_id
 ); ?>" data-resized="<?php echo esc_attr($resized_image_data); ?>"></span>
