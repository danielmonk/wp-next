  ...
  function SbYoutubeAdmin(plugin, $adminEl) {
    ...
    this.getModal = function () {
      var modal =
        ...
        '" class="button button-secondary sbspf_dismiss_connect_warning_button" data-action="sby_dismiss_connect_warning_notice">Continue</a>' +
        "                </p>" +
  ...
  SWshortcodeManager.prototype = {
      ...
      $(".sbsw-default-feed-wrap").each(function () {
        ...
        $pluginWrap.find(".sbsw-type-select").change(function () {
          if (typeof manager.state[plugin] !== "undefined") {
            ...
            if (inputType === "connected") {
              ...
              pluginData.current.term = $pluginWrap
                .find(".sbsw-text-input-wrap input")
                .val();
            }
          }
        ...
        $pluginWrap.find(".sbsw-add-remove-account").click(function () {
          if (typeof manager.state[plugin] !== "undefined") {
            ...
            if (
              $(this)
                .closest(".sbsw-connected-account")
                .hasClass("sbsw-selected")
            ) {
              $(this)
                .closest(".sbsw-connected-account")
                .removeClass("sbsw-selected");
              $(this).html(sbspf.add_text);
            } else {
              $(this)
                .closest(".sbsw-connected-account")
                .addClass("sbsw-selected");
              $(this).html(sbspf.remove_text);
            }
        ...
        $pluginWrap.find(".sbsw-types-checkbox-wrap input").change(function () {
          if (typeof manager.state[plugin] !== "undefined") {
            ...
            $pluginWrap
              .find(".sbsw-types-checkbox-wrap input:checked")
              .each(function () {
        ...
        if (typeof manager.state[plugin] !== "undefined") {
          ...
          if (plugin === "instagram") {
            var type = pluginData.current.type,
              inputType = pluginData.available_types[type].input,
              instructions =
                typeof pluginData.available_types[type].instructions !==
                "undefined"
                  ? pluginData.available_types[type].instructions
                  : "";
            ...
            if (inputType === "connected") {
              ...
              $(this)
                .find(".sbsw-text-input-wrap .sbsw-text-input-instructions")
                .html(instructions);
            }
            ...
            if (inputType === "message") {
              ...
              $(this)
                ...
                .html(instructions);
              if (
                pluginData.available_types[type].term_shortcode === "channel" &&
                pluginData.channel_ids_names[pluginData.current.term] !==
                  "undefined"
              ) {
                $(this)
                  .find(".sbsw-text-input-wrap .sbsw-text-input-identity")
                  .html(pluginData.channel_ids_names[pluginData.current.term]);
              } else {
                $(this)
                  .find(".sbsw-text-input-wrap .sbsw-text-input-identity")
                  .html("");
    ...
    updateShortcode: function () {
      ...
      if (
        ...
        (typeof this.state.instagram.exclude === "undefined" ||
          !this.state.instagram.exclude)
      ) {
        var type = this.state.instagram.current.type,
          term = this.state.instagram.current.term,
          shortcodeType = "";
        if (
          typeof this.state.instagram.available_types[type] !== "undefined" &&
          this.state.instagram.current.term !== ""
        ) {
          shortcodeType =
            " " +
            this.state.instagram.settings.type +
            '="' +
            type +
            '"' +
            " " +
            this.state.instagram.available_types[type].term_shortcode +
            '="' +
            this.state.instagram.current.term +
            '"';
        }
      ...
      if (
        ...
        (typeof this.state.facebook.exclude === "undefined" ||
          !this.state.facebook.exclude)
      ) {
        ...
        if (filtered.length > 0 && this.state.facebook.current.term !== "") {
          shortcodeType =
            " " +
            this.state.facebook.settings.type +
            '="' +
            filtered.join(",") +
            '"' +
            " " +
            this.state.facebook.available_types[splitUpType[0]].term_shortcode +
            '="' +
        ...
        $('.sbsw-connected-account[data-id="' + term + '"]')
          .addClass("sbsw-selected")
          .find(".sbsw-add-remove-account")
          .html(sbspf.remove_text);
      }

      if (
        typeof this.state.twitter !== "undefined" &&
        (typeof this.state.twitter.exclude === "undefined" ||
          !this.state.twitter.exclude)
      ) {
        var type = this.state.twitter.current.type,
          term = this.state.twitter.current.term,
          shortcodeType = "";
        if (
          typeof this.state.twitter.available_types[type] !== "undefined" &&
          this.state.twitter.current.term !== ""
        ) {
          var term = this.state.twitter.current.term;

          if (
            this.state.twitter.available_types[type].term_shortcode ===
              "hometimeline" ||
            this.state.twitter.available_types[type].term_shortcode ===
              "mentionstimeline"
          ) {
      ...
      if (
        typeof this.state.youtube !== "undefined" &&
        (typeof this.state.youtube.exclude === "undefined" ||
          !this.state.youtube.exclude)
      ) {
        var type = this.state.youtube.current.type,
          ...
          shortcodeType = "";
        if (
          typeof this.state.youtube.available_types[type] !== "undefined" &&
          this.state.youtube.current.term !== ""
        ) {
          var term = this.state.youtube.current.term;

          if (
            this.state.youtube.available_types[type].term_shortcode ===
              "hometimeline" ||
            this.state.youtube.available_types[type].term_shortcode ===
              "mentionstimeline"
          ) {
          ...
          shortcodeType =
            " " +
            this.state.youtube.settings.type +
            '="' +
            this.state.youtube.available_types[type].term_shortcode +
            '"' +
            " " +
      ...
      $(".sbsw-sc-generator-wrap textarea")
        .val(
          "[social-wall]\n" +
            ifShortcode +
            fbShortcode +
...
jQuery(document).ready(function ($) {
  ...
  window.sbswCASubmit = function (connected_accounts) {
    ...
    $("#sbsw-shortcode-generator").css("opacity", 0.5);
    ...
    var onSuccess = function (data) {
      setTimeout(function () {
        var reloadUrl = $(".sbsw-reload").length
          ? $(".sbsw-reload").attr("data-reload")
