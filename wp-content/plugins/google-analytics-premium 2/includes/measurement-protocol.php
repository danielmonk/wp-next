...
function monsterinsights_mp_api_call($args = [])
{
    $user_agent = '';
    ...
    if (
        ...
        !filter_var($_SERVER['HTTP_CLIENT_IP'], FILTER_VALIDATE_IP) === false
    ) {
    ...
    $user_language = isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])
    ...
    $default_body = [
        ...
        // Required: UA code
        'tid' => monsterinsights_get_ua_to_output(['ecommerce' => $args]),
        ...
        'dh' => str_replace(['http://', 'https://'], '', site_url()),
...
function monsterinsights_mp_track_event_call($args = [])
{
    // Detect if browser request is a prefetch
    if (
        (isset($_SERVER["HTTP_X_PURPOSE"]) &&
            'prefetch' === strtolower($_SERVER["HTTP_X_PURPOSE"])) ||
        (isset($_SERVER["HTTP_X_MOZ"]) &&
            'prefetch' === strtolower($_SERVER["HTTP_X_MOZ"]))
    ) {
        return;
    }
