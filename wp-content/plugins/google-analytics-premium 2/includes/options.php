...
function monsterinsights_get_option($key = '', $default = false)
{
    ...
    $value = !empty($monsterinsights_settings[$key])
        ? $monsterinsights_settings[$key]
        : $default;
    $value = apply_filters(
        'monsterinsights_get_option',
        $value,
        $key,
        $default
    );
    return apply_filters(
        'monsterinsights_get_option_' . $key,
        $value,
        $key,
        $default
    );
...
function monsterinsights_get_ua()
{
    ...
    if (
        ...
        MONSTERINSIGHTS_DISABLE_TRACKING
    ) {
        return '';
    }
    ...
    if (empty($ua)) {
        ...
        if (empty($ua)) {
            ...
            if (empty($ua)) {
                $ua =
                    defined('MONSTERINSIGHTS_GA_UA') && MONSTERINSIGHTS_GA_UA
                        ? monsterinsights_is_valid_ua(MONSTERINSIGHTS_GA_UA)
                        : '';
            }
        }
    }
...
function monsterinsights_get_network_ua()
{
    ...
    if (
        defined('MONSTERINSIGHTS_MS_GA_UA') &&
        monsterinsights_is_valid_ua(MONSTERINSIGHTS_MS_GA_UA)
    ) {
 ...
 */
function monsterinsights_get_ua_to_output($args = [])
{
 ...
 * @return boolean True if removed, false if not.
 */
function monsterinsights_delete_options($keys = [])
{
    // If no keys, exit
...
add_filter(
    ...
    'monsterinsights_force_tracking_mode'
);
...
add_filter(
    'monsterinsights_get_option_events_mode',
    'monsterinsights_force_events_mode'
);
