...
class MonsterInsights_WP_Emails
{
    ...
    public function __construct($template)
    {
        ...
        add_action('monsterinsights_email_send_before', [$this, 'send_before']);
        add_action('monsterinsights_email_send_after', [$this, 'send_after']);
    }
    ...
    public function get_from_name()
    {
        ...
        return apply_filters(
            'monsterinsights_email_from_name',
            monsterinsights_decode_string($this->from_name),
            $this
        );
    }
    ...
    public function get_from_address()
    {
        ...
        return apply_filters(
            'monsterinsights_email_from_address',
            $this->from_address,
            $this
        );
    }
    ...
    public function get_reply_to()
    {
        ...
        return apply_filters(
            'monsterinsights_email_reply_to',
            $this->reply_to,
            $this
        );
    }
    ...
    public function get_content_type()
    {
        if (!$this->content_type && $this->html) {
            $this->content_type = apply_filters(
                'monsterinsights_email_default_content_type',
                'text/html',
                $this
            );
        } elseif (!$this->html) {
        ...
        return apply_filters(
            'monsterinsights_email_content_type',
            $this->content_type,
            $this
        );
    }
    ...
    public function set_initial_args()
    {
        ...
        $args = apply_filters(
            'monsterinsights_emails_templates_set_initial_args',
            $args,
            $this
        );
    ...
    {
        $args = apply_filters(
            'monsterinsights_emails_templates_set_args',
            $args,
            $this
        );
        ...
        foreach ($args as $type => $value) {
            ...
            $this->args[$type] = $merge
                ? array_merge($this->args[$type], $value)
                : $value;
        }
    ...
    {
        if (!empty($type)) {
            return isset($this->args[$type])
                ? apply_filters(
                    'monsterinsights_emails_templates_get_args_' . $type,
                    $this->args[$type],
                    $this
                )
                : [];
        ...
        return apply_filters(
            'monsterinsights_emails_templates_get_args',
            $this->args,
            $this
        );
    }
    ...
    public function build_email($message = null)
    {
        ...
        $email_parts = [];
        $email_parts['header'] = $this->get_template_part(
            'header',
            $this->get_template(),
            true
        );
        ...
        $email_parts['body'] = $this->get_template_part(
            'body',
            $this->get_template(),
            true
        );
        ...
        $email_parts['footer'] = $this->get_template_part(
            'footer',
            $this->get_template(),
            true
        );
     ...
     * @return bool
     */
    public function send($to, $subject, $message = null, $attachments = [])
    {
        if (!did_action('init') && !did_action('admin_init')) {
            _doing_it_wrong(
                __FUNCTION__,
                esc_html__(
                    'You cannot send emails with MI_WP_Emails() until init/admin_init has been reached.',
                    'google-analytics-for-wordpress'
                ),
                null
            );
    ...
    public function is_email_disabled()
    {
        return (bool) apply_filters(
            'monsterinsights_disable_all_emails',
            false,
            $this
        );
    }
    ...
    public function get_template_part($slug, $name = null, $load = true)
    {
        ...
        if (isset($name)) {
            ...
            $html = $this->get_html($template, $this->get_args($slug), true);

            return apply_filters(
                'monsterinsights_emails_templates_get_content_part',
                $html,
                $template,
                $this
            );
     ...
     */
    public static function include_html(
        $template_name,
        $args = [],
        $extract = false
    ) {
        ...
        $located = apply_filters(
            'monsterinsights_helpers_templates_include_html_located',
            self::locate_template($template_name),
            $template_name,
            $args,
            $extract
        );
        $args = apply_filters(
            'monsterinsights_helpers_templates_include_html_args',
            $args,
            $template_name,
            $extract
        );
        ...
        $extract = apply_filters(
            'monsterinsights_helpers_templates_include_html_extract_args',
            $extract,
            $template_name,
            $args
        );
        ...
        if ($extract && is_array($args)) {
            ...
            if (count($args) !== $created_vars_count) {
                throw new RuntimeException(
                    'Extraction failed: variable names are clashing with the existing ones.'
                );
            }
        }
    ...
    public static function locate_template($template_name)
    {
        ...
        if (empty($template_name)) {
            return apply_filters(
                'monsterinsights_helpers_templates_locate',
                '',
                $template_name
            );
        }
    ...
    public static function get_theme_template_paths()
    {
        ...
        $file_paths = [
            ...
            100 =>
                trailingslashit(MONSTERINSIGHTS_PLUGIN_DIR) .
                'includes/emails/templates',
        ];

        $file_paths = apply_filters(
            'monsterinsights_email_template_paths',
            $file_paths
        );
