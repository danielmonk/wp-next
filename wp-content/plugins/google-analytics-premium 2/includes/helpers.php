...
function monsterinsights_track_user($user_id = -1)
{
    ...
    $track_user = true;
    $roles = monsterinsights_get_option('ignore_users', []);
    ...
    $track_super_admin = apply_filters(
        'monsterinsights_track_super_admins',
        false
    );
    if (
        $user_id === -1 &&
        $track_super_admin === false &&
        is_multisite() &&
        is_super_admin()
    ) {
        $track_user = false;
...
function monsterinsights_get_client_id($payment_id = false)
{
    ...
    $saved_cid = !empty($payment_id)
        ? get_post_meta($payment_id, '_yoast_gau_uuid', true)
        : false;
...
function monsterinsights_get_manage_options_roles()
{
    ...
    foreach ($editable_roles as $id => $role) {
        if (
            isset($role['capabilities']['manage_options']) &&
            $role['capabilities']['manage_options']
        ) {
...
function monsterinsights_get_message($type = 'error', $text = '')
{
    ...
    if (
        ...
        return $base->notices->display_inline_notice(
            'monsterinsights_standard_notice',
            '',
            $text,
            $type,
            false,
            ['skip_message_escape' => true]
        );
    } else {
        return '';
    }
...
function monsterinsights_get_shareasale_url(
    ...
    if (empty($custom)) {
        $shareasale_redirect = get_option(
            'monsterinsights_shareasale_redirect_url',
            ''
        );
        $custom = true;
    }
    ...
    $shareasale_url = sprintf(
        'https://www.shareasale.com/r.cfm?B=971799&U=%s&M=69975&urllink=%s',
        $shareasale_id,
        $shareasale_redirect
    );
    $shareasale_url = apply_filters(
        'monsterinsights_shareasale_redirect_entire_url',
        $shareasale_url,
        $shareasale_id,
        $shareasale_redirect
    );
...
function monsterinsights_count_third_party_ua_codes($body)
{
    ...
    if (class_exists('Advanced_Ads')) {
        ...
        $pattern = '/UA-[0-9-]+/';
        if (
            isset($options['ga-UID']) &&
            preg_match($pattern, $options['ga-UID'])
        ) {
            ++$count;
        }
    }
...
function monsterinsights_is_code_installed_frontend()
{
    ...
    $request = wp_remote_request(home_url(), [
        'sslverify' => false,
    ]);
    $errors = [];

    $accepted_http_codes = [200, 503];
    ...
    if (in_array($response_code, $accepted_http_codes, true)) {
        ...
        $cache_error = sprintf(
            esc_html__(
                '%1$sWe noticed you\'re using a caching plugin or caching from your hosting provider.%2$s Be sure to clear the cache to ensure the tracking appears on all pages and posts. %3$s(See this guide on how to clear cache)%4$s.',
                'google-analytics-for-wordpress'
            ),
            '<b>',
            '</b>',
        ...
        $message = esc_html__(
            '%1$sWe have detected multiple tracking codes%2$s! You should remove non-MonsterInsights ones. If you need help finding them please %3$sread this article%4$s.',
            'google-analytics-for-wordpress'
        );
...
function monsterinsights_custom_track_pretty_links_redirect($url)
{
    ...
    if (!empty($referer)) {
        ...
        if (
            !empty($parsed_referer['host']) &&
            !empty($current_site_parsed['host']) &&
            $current_site_parsed['host'] === $parsed_referer['host']
        ) {
    ...
    $ec = 'outbound-link';
    $inbound_paths = monsterinsights_get_option('affiliate_links', []);
    ...
    if (!empty($inbound_paths) && is_array($inbound_paths) && !empty($path)) {
        ...
        foreach ($inbound_paths as $inbound_path) {
            ...
            if (0 === strpos($path, trim($inbound_path['path']))) {
                $label = !empty($inbound_path['label'])
                    ? trim($inbound_path['label'])
                    : 'aff';
                $ec .= '-' . $label;
                $found = true;
                break;
...
add_action(
    'prli_before_redirect',
    'monsterinsights_custom_track_pretty_links_redirect'
);
...
function monsterinsights_tools_copy_url_to_prettylinks()
{
    ...
    $monsterinsights_reference = isset($_GET['monsterinsights_reference'])
        ? $_GET['monsterinsights_reference']
        : '';

    if (
        'post-new.php' === $pagenow &&
        'pretty-link' === $post_type &&
        'url_builder' === $monsterinsights_reference
    ) { ?>
        <script>
...
function monsterinsights_skip_prettylinks_welcome_screen()
{
    ...
    if (
        ...
        if ($onboard == 'welcome' || $onboard == 'update') {
            update_option(
                'monsterinsights_backup_prli_onboard_value',
                $onboard
            );
...
function monsterinsights_restore_prettylinks_onboard_value()
{
    ...
    if ('edit.php' === $pagenow && 'pretty-link' === $post_type) {
        ...
        if (
            class_exists('PrliBaseController') &&
...
add_action(
    'wp_loaded',
    'monsterinsights_restore_prettylinks_onboard_value',
    15
);
...
function monsterinsights_require_upgrader($custom_upgrader = true)
{
    ...
    if (version_compare($wp_version, '5.3', '<')) {
        if ($custom_upgrader) {
            require_once plugin_dir_path($base->file) .
...
function monsterinsights_get_frontend_analytics_script_atts()
{
    ...
    $attributes = apply_filters(
        'monsterinsights_tracking_analytics_script_attributes',
        [
...
function monsterinsights_can_install_plugins()
{
    ...
    if (
        ...
        !wp_is_file_mod_allowed('monsterinsights_can_install')
    ) {
        return false;
    }
