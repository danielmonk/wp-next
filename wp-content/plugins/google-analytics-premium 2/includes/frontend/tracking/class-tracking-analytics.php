...
class MonsterInsights_Tracking_Analytics extends
    MonsterInsights_Tracking_Abstract
{
    ...
    public function frontend_tracking_options()
    {
        ...
        $domain = esc_attr(
            monsterinsights_get_option('subdomain_tracking', 'auto')
        );
        ...
        $create = apply_filters(
            'monsterinsights_frontend_tracking_options_analytics_create',
            $create
        );
        ...
        $options = apply_filters(
            'monsterinsights_frontend_tracking_options_analytics_before_scripts',
            $options
        );
        ...
        if (monsterinsights_get_option('link_attribution', false)) {
            $options['enhanced_link_attribution'] =
        ...
        if (is_array($cross_domains) && !empty($cross_domains)) {
            ...
            if (!empty($cross_domains_strings)) {
                ...
                $options[
                    'cross_domains'
                ] = "'linker:autoLink', [$cross_domains_strings]";
            }
        }
        ...
        $options = apply_filters(
            'monsterinsights_frontend_tracking_options_before_pageview',
            $options,
            $this->name,
            $this->version
        );
        ...
        if (is_404()) {
            if (monsterinsights_get_option('hash_tracking', false)) {
                $options['send'] =
                    "'send','pageview','/404.html?page=' + document.location.pathname + document.location.search + location.hash + '&from=' + document.referrer";
            } else {
                $options['send'] =
                    "'send','pageview','/404.html?page=' + document.location.pathname + document.location.search + '&from=' + document.referrer";
            }
            ...
            if ((int) $wp_query->found_posts === 0) {
                $options['send'] =
                    $pushstr .
                    'no-results:' .
                    rawurlencode($wp_query->query_vars['s']) .
                    "&cat=no-results'";
            } elseif ((int) $wp_query->found_posts === 1) {
                $options['send'] =
                    $pushstr .
                    rawurlencode($wp_query->query_vars['s']) .
                    "&cat=1-result'";
            ...
            $options['send'] =
                "'send','pageview', location.pathname + location.search + location.hash";
        } else {
            $options['send'] = "'send','pageview'";
        }
    ...
    public function frontend_output()
    {
        ...
        $src = apply_filters(
            'monsterinsights_frontend_output_analytics_src',
            '//www.google-analytics.com/analytics.js'
        );
        $compat_mode = monsterinsights_get_option(
            'gatracker_compatibility_mode',
            false
        );
...
<script<?php echo $attr_string; ?>>
	...
	var mi_no_track_reason = <?php echo $reason
     ? "'" . esc_js($reason) . "'"
     : "''"; ?>;
	<?php do_action(
     'monsterinsights_tracking_analytics_frontend_output_after_mi_track_user'
 ); ?>
...
<?php if ($this->should_do_optout()) { ?>
		...
		(function() {
         ...
         __("Not running function", "google-analytics-for-wordpress")
     ); ?> __gaTracker(' + arguments[0] + " ....) <?php echo esc_js(
     __("because you are not being tracked.", 'google-analytics-for-wordpress')
 ); ?> " + mi_no_track_reason );
					return;
				}
				try {
