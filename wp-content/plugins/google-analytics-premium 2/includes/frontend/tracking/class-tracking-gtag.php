...
class MonsterInsights_Tracking_Gtag extends MonsterInsights_Tracking_Abstract
{
    ...
    public function frontend_tracking_options()
    {
        ...
        $options = apply_filters(
            'monsterinsights_frontend_tracking_options_gtag_before_scripts',
            $options
        );
        ...
        if (is_array($cross_domains) && !empty($cross_domains)) {
            ...
            $options['linker'] = json_encode([
                'domains' => $linker_domains,
            ]);
        }

        $options = apply_filters(
            'monsterinsights_frontend_tracking_options_gtag_before_pageview',
            $options
        );
        $options = apply_filters(
        ...
        if (is_404()) {
            if (monsterinsights_get_option('hash_tracking', false)) {
                $options['page_path'] =
                    "'/404.html?page=' + document.location.pathname + document.location.search + location.hash + '&from=' + document.referrer";
            } else {
                $options['page_path'] =
                    "'/404.html?page=' + document.location.pathname + document.location.search + '&from=' + document.referrer";
            }
    ...
    public function frontend_output()
    {
        ...
        $src = apply_filters(
            'monsterinsights_frontend_output_gtag_src',
            '//www.googletagmanager.com/gtag/js?id=' . $ua
        );
        $compat_mode = monsterinsights_get_option(
            'gtagtracker_compatibility_mode',
            true
        );
      ...
      echo $output;
		...
		<?php if ($ua) { ?>
			<script src="<?php echo esc_attr(
       $src
   ); ?>" <?php echo $attr_string; ?>></script>
        ...
        : "''"; ?>;
				<?php do_action(
        'monsterinsights_tracking_gtag_frontend_output_after_mi_track_user'
    ); ?>
     ...
     do_action('monsterinsights_frontend_tracking_gtag_after_pageview'); ?>
					...
					<?php if (
         apply_filters(
             'monsterinsights_tracking_gtag_frontend_gatracker_compatibility',
             true
         )
     ) { ?>
					(
