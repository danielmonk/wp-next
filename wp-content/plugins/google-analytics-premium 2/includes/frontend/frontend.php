...
{
    require_once plugin_dir_path(MONSTERINSIGHTS_PLUGIN_FILE) .
...
function monsterinsights_prevent_loading_frontend_reports()
{
    return !current_user_can('monsterinsights_view_dashboard') ||
        monsterinsights_get_option('hide_admin_bar_reports') ||
        (function_exists('monsterinsights_is_reports_page') &&
            monsterinsights_is_reports_page()) ||
        (function_exists('monsterinsights_is_settings_page') &&
            monsterinsights_is_settings_page());
}
...
function monsterinsights_add_admin_bar_menu()
{
    ...
    $args = [
        ...
        'title' =>
            '<span class="ab-icon dashicons-before dashicons-chart-bar"></span> Insights',
...
function monsterinsights_frontend_admin_bar_scripts()
{
    ...
    $rtl = is_rtl() ? '.rtl' : '';
    $frontend_js_url =
        defined('MONSTERINSIGHTS_LOCAL_FRONTEND_JS_URL') &&
        MONSTERINSIGHTS_LOCAL_FRONTEND_JS_URL
            ? MONSTERINSIGHTS_LOCAL_FRONTEND_JS_URL
            : plugins_url(
                $version_path . '/assets/vue/js/frontend.js',
    ...
    if (!defined('MONSTERINSIGHTS_LOCAL_FRONTEND_JS_URL')) {
        wp_enqueue_style(
            ...
            plugins_url(
                $version_path . '/assets/vue/css/frontend' . $rtl . '.css',
                MONSTERINSIGHTS_PLUGIN_FILE
            ),
            [],
        ...
        wp_enqueue_script(
            ...
            plugins_url(
                $version_path . '/assets/vue/js/chunk-frontend-vendors.js',
                MONSTERINSIGHTS_PLUGIN_FILE
            ),
            [],
            monsterinsights_get_asset_version(),
    ...
    $page_title = is_singular()
        ? get_the_title()
        : monsterinsights_get_page_title();
    ...
    if (
        !wp_script_is('monsterinsights-vue-script') &&
        !wp_script_is('monsterinsights-vue-reports') &&
        !wp_script_is('monsterinsights-vue-widget')
    ) {
        ...
        wp_localize_script('monsterinsights-vue-frontend', 'monsterinsights', [
...
add_action(
    ...
    'monsterinsights_frontend_admin_bar_scripts',
    1005
);
...
function monsterinsights_administrator_tracking_notice()
{
	...
	<div class="monsterinsights-tracking-notice monsterinsights-tracking-notice-hide">
       ...
       plugins_url('assets/images/mascot.png', MONSTERINSIGHTS_PLUGIN_FILE)
   ); ?>"
				 width="40" alt="MonsterInsights Mascot"/>
		</div>
		<div class="monsterinsights-tracking-notice-text">
			<h3><?php esc_html_e(
       'Tracking is Disabled for Administrators',
       'google-analytics-for-wordpress'
   ); ?></h3>
			<p>
				<?php
    $doc_url =
        'https://monsterinsights.com/docs/tracking-disabled-administrators-editors';
    $doc_url = add_query_arg(
        [
            'utm_source' => monsterinsights_is_pro_version()
                ? 'proplugin'
                : 'liteplugin',
    ...
    printf(
        esc_html__(
            'To keep stats accurate, we do not load Google Analytics scripts for admin users. %1$sLearn More &raquo;%2$s',
            'google-analytics-for-wordpress'
        ),
        '<a href="' . esc_url($doc_url) . '" target="_blank">',
        '</a>'
    );
    ?>
			</p>
	...
	<?php if (!wp_script_is('jquery', 'queue')) {
				...
				$( document.querySelector( '.monsterinsights-tracking-notice-close' ) ).on( 'click', function ( e ) {
					...
					$.ajax( {
						...
						data: {
							...
							nonce: '<?php echo esc_js(
           wp_create_nonce('monsterinsights-tracking-notice')
       ); ?>',
						}
					} );
				} );
...
add_action(
    'wp_ajax_monsterinsights_dismiss_tracking_notice',
    'monsterinsights_dismiss_tracking_notice'
);
