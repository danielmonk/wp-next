...
class MonsterInsights_Gtag_Events
{
    ...
    {
        add_action('wp_enqueue_scripts', [$this, 'output_javascript'], 9);
    }
    ...
    public function output_javascript()
    {
        ...
        $download_extensions = explode(
            ',',
            str_replace('.', '', $download_extensions)
        );
        ...
        $hash_tracking = monsterinsights_get_option('hash_tracking', false)
            ? 'true'
            : 'false';
        ...
        if (
            !file_exists(
                MONSTERINSIGHTS_PLUGIN_DIR . 'assets/js/frontend-gtag.min.js'
            )
        ) {
            $suffix = '';
        }
        wp_enqueue_script(
            ...
            plugins_url(
                'assets/js/frontend-gtag' . $suffix . '.js',
