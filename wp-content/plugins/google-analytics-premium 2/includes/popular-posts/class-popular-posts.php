...
class MonsterInsights_Popular_Posts
{
    ...
    public function __construct()
    {
        ...
        $this->ajaxify = monsterinsights_get_option(
            'popular_posts_ajaxify',
            false
        );
    }
    ...
    {
        add_action('wp_enqueue_scripts', [$this, 'load_frontend_styles']);

        add_action('wp_enqueue_scripts', [$this, 'maybe_load_ajaxify_script']);
    ...
    public function register_shortcode()
    {
        if (!empty($this->shortcode_key)) {
            add_shortcode($this->shortcode_key, [$this, 'render_shortcode']);
        }
    }
    ...
    public function load_frontend_styles()
    {
        ...
        if (
            apply_filters(
                'monsterinsights_popular_posts_styles_output',
                'no_styles' === $this->styling,
                $this
            )
        ) {
            return;
        ...
        wp_register_style(
            ...
            plugins_url(
                'assets/css/frontend' . $suffix . '.css',
    ...
    public function maybe_load_ajaxify_script()
    {
        ...
        wp_register_script(
            'monsterinsights-popular-posts-js',
            plugins_url(
                'assets/js/popular-posts' . $suffix . '.js',
                MONSTERINSIGHTS_PLUGIN_FILE
            ),
            [],
            monsterinsights_get_asset_version(),
    ...
    public function get_ajax_json_data($args)
    {
        ...
        $data =
            '<div><script type="application/json" class="monsterinsights-popular-posts-widget-json">';
    ...
    public function get_posts()
    {
        ...
        if (empty($posts)) {
            if (
                isset($posts_args['post__in']) &&
                empty($posts_args['post__in'])
            ) {
                $this->posts = [];
    ...
    private function process_posts($posts)
    {
        ...
        foreach ($posts as $post) {
            ...
            if (!empty($post_thumbnail)) {
                ...
                $post_image_srcset = wp_get_attachment_image_srcset(
                    $post_thumbnail,
                    'small'
                );
            }
    ...
    public function get_theme_props($theme = '')
    {
        ...
        $theme_props = new MonsterInsights_Popular_Posts_Themes(
            $this->type,
            $theme
        );
    ...
    public function get_wrapper_class($atts)
    {
        ...
        $classes = [
            ...
            'no_styles' !== $this->styling
                ? 'monsterinsights-popular-posts-styled'
                : '',
        ];
    ...
    public function is_current_post($id)
    {
        if (defined('DOING_AJAX') && DOING_AJAX) {
            $current_id = isset($_POST['post_id'])
    ...
    public function get_posts_to_display()
    {
        ...
        if (
            apply_filters(
                'monsterinsights_popular_posts_show_duplicates',
                true
            ) &&
            count($posts) > 0 &&
            count($this->shown_posts) > 0 &&
            count($returned_posts) === 0
        ) {
            $this->shown_posts = []; // Reset shown posts.
    ...
    public function get_themes_styles_for_output()
    {
        ...
        $themes =
            !empty($stored_styles[$this->type]) &&
            is_array($stored_styles[$this->type])
                ? $stored_styles[$this->type]
                : [];
