...
class MonsterInsightsHeadlineToolPlugin
{
     ...
     * Class Variables.
     */
    private $emotion_power_words = [];
    private $power_words = [];
    private $common_words = [];
    private $uncommon_words = [];
    ...
    {
        add_action(
            'wp_ajax_monsterinsights_gutenberg_headline_analyzer_get_results',
            [$this, 'get_result']
        );
    }
    ...
    function get_result()
    {
        ...
        if (
            check_ajax_referer(
                'monsterinsights_gutenberg_headline_nonce',
                false,
                false
            ) === false
        ) {
        ...
        if (!empty($result->err)) {
            ...
            wp_send_json_error(['html' => $content, 'analysed' => false]);
        } else {
            // send the response
            wp_send_json_success([
                ...
                'analysed' => !$result->err,
                'sentence' => ucwords(
                    wp_unslash(sanitize_text_field($_REQUEST['q']))
                ),
                'score' =>
                    isset($result->score) && !empty($result->score)
                        ? $result->score
                        : 0,
            ]);
        }
    ...
    function get_headline_scores()
    {
        ...
        if ($result->word_count == 0) {
            $scoret = 0;
        } elseif ($result->word_count >= 2 && $result->word_count <= 4) {
            $scoret += 5;
        ...
        $result->power_words = $this->match_words(
            $result->input,
            $result->input_array,
            $this->power_words()
        );
        $result->power_words_per =
            count($result->power_words) / $result->word_count;
        $result->emotion_words = $this->match_words(
            $result->input,
            $result->input_array,
            $this->emotion_power_words()
        );
        $result->emotion_words_per =
            count($result->emotion_words) / $result->word_count;
        $result->common_words = $this->match_words(
            $result->input,
            $result->input_array,
            $this->common_words()
        );
        $result->common_words_per =
            count($result->common_words) / $result->word_count;
        $result->uncommon_words = $this->match_words(
            $result->input,
            $result->input_array,
            $this->uncommon_words()
        );
        $result->uncommon_words_per =
            count($result->uncommon_words) / $result->word_count;
        $result->word_balance = __(
            'Can Be Improved',
            'google-analytics-for-wordpress'
        );
        $result->word_balance_use = [];

        if ($result->emotion_words_per < 0.1) {
            $result->word_balance_use[] = __(
                'emotion',
                'google-analytics-for-wordpress'
            );
        } else {
        ...
        require_once MONSTERINSIGHTS_HEADLINE_TOOL_DIR_PATH .
            '/phpinsight/autoload.php';
        ...
        if (
            strpos($input, __('how to', 'google-analytics-for-wordpress')) !==
                false ||
            strpos($input, __('howto', 'google-analytics-for-wordpress')) !==
                false
        ) {
    ...
    {
        if (
            isset($this->emotion_power_words) &&
            !empty($this->emotion_power_words)
        ) {
