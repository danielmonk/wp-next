...
final class MonsterInsights_API_Request
{
    ...
    public function request()
    {
        ...
        if ($blocked || is_wp_error($blocked)) {
            if (is_wp_error($blocked)) {
                ...
                return new WP_Error(
                    ...
                    sprintf(
                        __(
                            'The firewall of your server is blocking outbound calls. Please contact your hosting provider to fix this issue. %s',
        ...
        $body['ip'] = !empty($_SERVER['SERVER_ADDR'])
            ? $_SERVER['SERVER_ADDR']
            : '';
        ...
        $headers = [
            ...
            'Pragma' => 'no-cache',
            'Expires' => 0,
            'MIAPI-Referer' => is_network_admin()
                ? network_admin_url()
                : site_url(),
        ...
        $data = [
            ...
            'timeout' => 3000,
            'user-agent' =>
                'MI/' . MONSTERINSIGHTS_VERSION . '; ' . $this->site_url,
            'sslverify' => false,
        ];
        ...
        $response =
            'GET' == $this->method
                ? wp_remote_get(esc_url_raw($this->url) . '?' . $string, $data)
                : wp_remote_post(esc_url_raw($this->url), $data);
        ...
        if (!empty($this->tt)) {
            if (
                empty($response_body['tt']) ||
                !hash_equals($this->tt, $response_body['tt'])
            ) {
    ...
    private function is_blocked($url = '')
    {
        ...
        if (
            defined('AIRMDE_VER') &&
            !empty($Airplane_Mode_Core) &&
            $Airplane_Mode_Core->enabled()
        ) {
            return new WP_Error(
                'api-error',
                __(
                    'Reason: The API was unreachable because the Airplane Mode plugin is active.',
                    'google-analytics-for-wordpress'
                )
            );
        }
            ...
            if (defined('WP_ACCESSIBLE_HOSTS')) {
                ...
                if ($on_blacklist) {
                    return new WP_Error(
                        'api-error',
                        __(
                            'Reason: The API was unreachable because the API url is on the WP HTTP blocklist.',
                            'google-analytics-for-wordpress'
                        )
                    );
                } else {
                    $params = [
                        ...
                        'timeout' => 2,
                        'user-agent' =>
                            'MonsterInsights/' . MONSTERINSIGHTS_VERSION,
                        'body' => '',
                    ];
                    ...
                    if (
                        ...
                        if (is_wp_error($response)) {
                            ...
                            return new WP_Error(
                                'api-error',
                                __(
                                    'Reason: The API was unreachable because the call to Google failed.',
                                    'google-analytics-for-wordpress'
                                )
                            );
                        }
                ...
                return new WP_Error(
                    'api-error',
                    __(
                        'Reason: The API was unreachable because no external hosts are allowed on this site.',
                        'google-analytics-for-wordpress'
                    )
                );
            }
