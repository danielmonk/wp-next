...
class MonsterInsights_Report
{
    ...
    {
        add_filter(
            'monsterinsights_reports_abstract_get_data_pre_cache',
            [$this, 'requirements'],
            10,
            3
        );
    ...
    protected function get_report_html($args = [])
    {
    ...
    public function requirements($error = false, $args = [], $name = '')
    {
        return $error;
    }

    public function show_report($args = [])
    {
        if (!current_user_can('monsterinsights_view_dashboard')) {
            return monsterinsights_get_message(
                'error',
                esc_html__('Access denied', 'google-analytics-for-wordpress')
            );
        }
        ...
        if (monsterinsights_get_option('dashboard_disabled', false)) {
            if (current_user_can('monsterinsights_save_settings')) {
                $url = is_network_admin()
                    ? network_admin_url(
                        'admin.php?page=monsterinsights_settings'
                    )
                ...
                return monsterinsights_get_message(
                    ...
                    sprintf(
                        esc_html__(
                            'Please %1$senable the dashboard%2$s to see report data.',
        ...
        if (monsterinsights_is_pro_version()) {
            if (!MonsterInsights()->license->has_license()) {
                ...
                return monsterinsights_get_message(
                    'error',
                    esc_html__(
                        'You do not have an active license. Please %1$scheck your license configuration.%2$s',
                        'google-analytics-for-wordpress'
                    ),
                    '<a href="' . $url . '">',
                    '</a>'
                );
            } elseif (MonsterInsights()->license->license_has_error()) {
                return monsterinsights_get_message(
                    'error',
                    $this->get_license_error()
                );
            }
        }
        ...
        if (
            ...
            if (current_user_can('monsterinsights_save_settings')) {
                ...
                return monsterinsights_get_message(
                    ...
                    sprintf(
                        esc_html__(
                            'Please %1$sauthenticate %2$swith Google Analytics to allow the plugin to fetch data.',
                            'google-analytics-for-wordpress'
                        ),
                        '<a href="' . $url . '">',
                        '</a>'
        ...
        $error = $this->requirements(false, [], $this->name);
        ...
        if (empty($args['data']) || !is_array($args['data'])) {
            if (monsterinsights_is_pro_version()) {
                ...
                $args = $this->get_data(['default' => true]);
                if (empty($args['data']) || is_array($args['data'])) {
                    return monsterinsights_get_message(
                        'error',
                        __('No data found', 'google-analytics-for-wordpress')
                    );
                }
    ...
    public function get_data($args = [])
    {
        ...
        $start =
            !empty($args['start']) && $this->is_valid_date($args['start'])
                ? $args['start']
                : '';
        $end =
            !empty($args['end']) && $this->is_valid_date($args['end'])
                ? $args['end']
                : '';

        if (
            monsterinsights_is_pro_version() &&
            !MonsterInsights()->license->license_can($this->level)
        ) {
            return [
                'success' => true,
                'upgrade' => true,
        ...
        if ($error) {
            return apply_filters(
                'monsterinsights_reports_handle_error_message',
                [
                    'success' => false,
                    'error' => $error,
                    'data' => [],
        ...
        $check_cache =
            ($start === $this->default_start_date() &&
                $end === $this->default_end_date()) ||
            apply_filters(
                'monsterinsights_report_use_cache',
                false,
                $this->name
            );
        ...
        if ($site_auth || $ms_auth) {
            ...
            $option_name = $site_auth
                ? 'monsterinsights_report_data_' . $this->name
                : 'monsterinsights_network_report_data_' . $this->name;
            $p = $site_auth
                ? MonsterInsights()->auth->get_viewid()
                : MonsterInsights()->auth->get_network_viewid();
            ...
            if (
                ...
                return $this->prepare_report_data([
                    'success' => true,
                    'data' => $data['data'],
                ]);
            }
            ...
            if (is_wp_error($ret)) {
                ...
                if ($check_cache) {
                    ...
                    !$site_auth && $ms_auth
                        ? set_site_transient($transient, $data, $expiration)
    ...
    public function is_valid_date_range($start, $end)
    {
        ...
        if (
            $start > strtotime('now') ||
            $end > strtotime('now') ||
            $start < strtotime('01 January 2005') ||
            $end < strtotime('01 January 2005')
        ) {
            return false;
        }
    ...
    {
        $has_level = monsterinsights_is_pro_version()
        ...
        $message = sprintf(
            __(
                'You currently have a %1$s level license, but this report requires at least a %2$s level license to view the %3$s. Please upgrade to view this report.',
                'google-analytics-for-wordpress'
            ),
            $has_level,
            $this->level,
        ...
        <div class="monsterinsights-upsell-report-container monsterinsights-upsell-report-<?php echo $this->name; ?>-bg">
                    ...
                    <div class="col-lg-10 col-lg-offset-1 align-self-center">
                        <div class="monsterinsights-upsell-card">
                            <img class="monsterinsights-upgrade-mascot"
                                 src="<?php echo trailingslashit(
                                     MONSTERINSIGHTS_PLUGIN_URL
                                 ); ?>assets/css/images/mascot.png"
                                 srcset="<?php echo trailingslashit(
                                     MONSTERINSIGHTS_PLUGIN_URL
                                 ); ?>assets/css/images/mascot@2x.png 2x"
                                 alt="">
                            <div class="monsterinsights-upsell-card-card-content">
                                <span class="monsterinsights-upsell-card-title"><?php esc_html_e(
                                    'Ready to Get Analytics Super-Powers?',
                                    'google-analytics-for-wordpress'
                                ); ?></span>
								...
								<?php if (monsterinsights_is_pro_version()) { ?>
                                    <p>
										<?php // Translators: License level and smiley.
          echo sprintf(
              esc_html__(
                  'Hey there! It looks like you\'ve got the %1$s license installed on your site. That\'s awesome! %s',
                  'google-analytics-for-wordpress'
              ),
              $has_level,
              '<span class="dashicons dashicons-smiley"></span>'
                                    ...
                                    &nbsp;
                                    <p>
										<?php // Translators: Placeholders add the report title and license level.
          echo sprintf(
              esc_html__(
                  'Do you want to access to %1$s reporting right now%2$s in your WordPress Dashboard? That comes with the %3$s level%4$s of our paid packages. You\'ll need to upgrade your license to get instant access.',
                  'google-analytics-for-wordpress'
              ),
              '<strong>' . $this->title,
              '</strong>',
              '<strong><a href="' .
                  monsterinsights_get_url(
                      'reports-page',
                      $this->name . '-report-upsell-license-link',
              ...
              '</a></strong>'
                                    ...
                                    &nbsp;
                                    <p>
										<?php // Translators: Placeholdes add links to the account area and a guide.
          echo sprintf(
              esc_html__(
                  'It\'s easy! To upgrade, navigate to %1$sMy Account%2$s on MonsterInsights.com, go to the licenses tab, and click upgrade. We also have a %3$sstep by step guide%4$s with pictures of this process.',
                  'google-analytics-for-wordpress'
              ),
              '<a href="' .
                  monsterinsights_get_url(
              ...
              '<a href="' .
                  monsterinsights_get_url(
                      'reports-page',
                      $this->name . '-report-upsell-license-link',
                      'https://www.monsterinsights.com/docs/upgrade-monsterinsights-license/'
                  ) .
              ...
              '</a>'
								...
								<?php } else { ?>
                                    <p>
										<?php // Translators: Placeholder adds a smiley face.
          echo sprintf(
              esc_html__(
                  'Hey there! %s It looks like you\'ve got the free version of MonsterInsights installed on your site. That\'s awesome!',
                  'google-analytics-for-wordpress'
              ),
              '<span class="dashicons dashicons-smiley"></span>'
          ); ?>
                                    ...
                                    &nbsp;
                                    <p>
										<?php // Translators: Placeholders make the text bold, add the license level and add a link to upgrade.
          echo sprintf(
              esc_html__(
                  'Do you you want to access to %1$s reporting right now%2$s in your WordPress Dashboard? That comes with %3$s level%4$s of our paid packages. To get instant access, you\'ll want to buy a MonsterInsights license, which also gives you access to powerful addons, expanded reporting (including the ability to use custom date ranges), comprehensive tracking features (like UserID tracking) and access to our world-class support team.',
                  'google-analytics-for-wordpress'
              ),
              '<strong>' . $this->title,
              '</strong>',
          ...
          echo sprintf(
              esc_html__(
                  'Upgrading is easy! To upgrade, navigate to %1$ssour pricing page%2$s, purchase the required license, and then follow the %3$sinstructions in the email receipt%4$s to upgrade. It only takes a few minutes to unlock the most powerful, yet easy to use analytics tracking system for WordPress.',
                  'google-analytics-for-wordpress'
              ),
              '<a href="' .
                  monsterinsights_get_upgrade_link(
              ...
              '<a style="text-decoration:underline !important" href="' .
                  monsterinsights_get_url(
                      'reports-page',
                      $this->name . '-report-go-lite-pro-link',
              ...
              '</a>'
                            ...
                            <div class="monsterinsights-upsell-card-action">
								<?php if (monsterinsights_is_pro_version()) { ?>
                                    <a href="<?php echo monsterinsights_get_upgrade_link(
                                        'reports-page',
                                        $this->name .
                                            '-report-upsell-license-link'
                                    ); ?>"
                                       class="monsterinsights-upsell-card-button"><?php esc_html_e(
                                           'Upgrade Now',
                                           'google-analytics-for-wordpress'
                                       ); ?></a>
								<?php } else { ?>
                                    <a href="<?php echo monsterinsights_get_url(
                                        'reports-page',
                                        $this->name .
                                            '-report-upsell-license-link',
                                        'https://www.monsterinsights.com/docs/upgrade-monsterinsights-license/'
    ...
    function get_ga_report_range($data = [])
    {
        if (
            empty($data['reportcurrentrange']) ||
            empty($data['reportcurrentrange']['startDate']) ||
            empty($data['reportcurrentrange']['endDate'])
        ) {
            return '';
        } else {
            if (
                !empty($data['reportprevrange']) &&
                !empty($data['reportprevrange']['startDate']) &&
                !empty($data['reportprevrange']['endDate'])
            ) {
                return urlencode(
                    '_u.date00=' .
                        str_replace(
                            '-',
                            '',
                            $data['reportcurrentrange']['startDate']
                        ) .
                        '&_u.date01=' .
                        str_replace(
                            '-',
                            '',
                            $data['reportcurrentrange']['endDate']
                        ) .
                        '&_u.date10=' .
                        str_replace(
                            '-',
                            '',
                            $data['reportprevrange']['startDate']
                        ) .
                        '&_u.date11=' .
                        str_replace(
                            '-',
                            '',
                            $data['reportprevrange']['endDate']
                        )
                );
            } else {
                return urlencode(
    ...
    public function get_addons_page_link()
    {
        if (monsterinsights_can_install_plugins()) {
            ...
            $addons_link = esc_html__(
                'Please ask your webmaster to enable this addon.',
                'google-analytics-for-wordpress'
            );
        }
