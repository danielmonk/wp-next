 ...
 * @since 7.12.3
 */
final class MonsterInsights_Notification_To_Setup_Affiliate_Links extends
    MonsterInsights_Notification_Event
{
    ...
    public function prepare_notification_data($notification)
    {
        ...
        if (is_array($affiliate_links) && !empty($affiliate_links)) {
            if (
                2 === count($affiliate_links) &&
                isset($affiliate_links[0]['path']) &&
                isset($affiliate_links[1]['path'])
            ) {
                $no_new_links =
                    "/go/" === $affiliate_links[0]['path'] &&
                    "/recommend/" === $affiliate_links[1]['path']
                        ? true
                        : false;
            }
        }
        ...
        if (
            true === $no_new_links ||
            (is_array($affiliate_links) && empty($affiliate_links))
        ) {
