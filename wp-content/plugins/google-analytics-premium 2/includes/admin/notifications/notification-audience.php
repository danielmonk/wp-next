 ...
 * @since 7.12.3
 */
final class MonsterInsights_Notification_Audience extends
    MonsterInsights_Notification_Event
{
    public $notification_id = 'monsterinsights_notification_audience';
    public $notification_interval = 30; // in days
    public $notification_type = ['basic', 'lite', 'master', 'plus', 'pro'];
        ...
        $notification['btns'] = [
            "view_report" => [
                'url' => $this->get_view_url(
                    'monsterinsights-report-top-countries',
    ...
    public function get_notification_data()
    {
        ...
        $sessions = isset($report['data']['infobox']['sessions']['value'])
            ? $report['data']['infobox']['sessions']['value']
            : 0;
        $countries = isset($report['data']['countries'])
            ? $report['data']['countries']
            : 0;
        ...
        if ($sessions > 0 && is_array($countries) && !empty($countries)) {
            foreach ($countries as $country) {
                ...
                if ($country['sessions'] > 0) {
                    ...
                    $country_session_percentage = round(
                        ($country['sessions'] * 100) / $sessions
                    );
                    ...
                    if (is_array($translations) && !empty($translations)) {
                        $site_iso = isset($translations[$site_language]['iso'])
                            ? $translations[$site_language]['iso']
                            : []; // keep empty array, because site language has no iso setup for en_US language

                        if (
                            is_array($site_iso) &&
                            !in_array($country['iso'], $site_iso)
                        ) {
