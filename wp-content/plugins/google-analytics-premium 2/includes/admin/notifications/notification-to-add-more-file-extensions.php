 ...
 * @since 7.12.3
 */
final class MonsterInsights_Notification_To_Add_More_File_Extensions extends
    MonsterInsights_Notification_Event
{
    ...
    {
        $download_extensions = monsterinsights_get_option(
            'extensions_of_files',
            ''
        );

        if (
            empty($download_extensions) ||
            "doc,pdf,ppt,zip,xls,docx,pptx,xlsx" === $download_extensions
        ) {
            $settings_url = is_network_admin()
                ? $this->get_view_url(
                    'monsterinsights-settings-block-file-downloads',
            ...
            $publishers_report_url = $this->get_view_url(
                'monsterinsights-report-download-links',
