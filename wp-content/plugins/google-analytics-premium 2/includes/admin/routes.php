...
class MonsterInsights_Rest_Routes
{
    ...
    {
        add_action('wp_ajax_monsterinsights_vue_get_license', [
            $this,
            'get_license',
        ]);
        add_action('wp_ajax_monsterinsights_vue_get_profile', [
            $this,
            'get_profile',
        ]);
        add_action('wp_ajax_monsterinsights_vue_get_settings', [
            $this,
            'get_settings',
        ]);
        add_action('wp_ajax_monsterinsights_vue_update_settings', [
            $this,
            'update_settings',
        ]);
        add_action('wp_ajax_monsterinsights_vue_update_settings_bulk', [
            $this,
            'update_settings_bulk',
        ]);
        add_action('wp_ajax_monsterinsights_vue_get_addons', [
            $this,
            'get_addons',
        ]);
        add_action('wp_ajax_monsterinsights_update_manual_ua', [
            $this,
            'update_manual_ua',
        ]);
        add_action('wp_ajax_monsterinsights_vue_get_report_data', [
            $this,
            'get_report_data',
        ]);
        add_action('wp_ajax_monsterinsights_vue_install_plugin', [
            $this,
            'install_plugin',
        ]);
        add_action('wp_ajax_monsterinsights_vue_notice_status', [
            $this,
            'get_notice_status',
        ]);
        add_action('wp_ajax_monsterinsights_vue_notice_dismiss', [
            $this,
            'dismiss_notice',
        ]);
        add_action('wp_ajax_monsterinsights_vue_grab_popular_posts_report', [
            $this,
            'check_popular_posts_report',
        ]);
        add_action(
            'wp_ajax_monsterinsights_vue_popular_posts_update_theme_setting',
            [$this, 'update_popular_posts_theme_setting']
        );

        // TODO: remove function from Google Optimize Addon.
        add_action('wp_ajax_monsterinsights_get_posts', [$this, 'get_posts']);

        // Search for taxonomies.
        add_action('wp_ajax_monsterinsights_get_terms', [
            $this,
            'get_taxonomy_terms',
        ]);

        add_action('wp_ajax_monsterinsights_get_post_types', [
            $this,
            'get_post_types',
        ]);

        add_action('wp_ajax_monsterinsights_handle_settings_import', [
            $this,
            'handle_settings_import',
        ]);

        add_action('admin_notices', [$this, 'hide_old_notices'], 0);

        add_action('wp_ajax_monsterinsights_vue_dismiss_first_time_notice', [
            $this,
    ...
    public function get_license()
    {
        ...
        if (
            !current_user_can('monsterinsights_view_dashboard') ||
    ...
    public function get_settings()
    {
        ...
        $array_fields = ['view_reports', 'save_settings', 'ignore_users'];
        ...
        if (monsterinsights_is_pro_version()) {
            ...
            if (!isset($options['summaries_email_addresses'])) {
                $options['summaries_email_addresses'] = [$default_email];
            }
    ...
    public function update_settings()
    {
        ...
        if (isset($_POST['setting'])) {
            ...
            if (isset($_POST['value'])) {
                ...
                do_action(
                    'monsterinsights_after_update_settings',
                    $setting,
                    $value
                );
            } else {
    ...
    public function update_settings_bulk()
    {
        ...
        if (isset($_POST['settings'])) {
            $settings = json_decode(
                sanitize_text_field(wp_unslash($_POST['settings'])),
                true
            );
    ...
    public function get_addons()
    {
        ...
        // GiveWP.
        $parsed_addons['givewp'] = [
            'active' => function_exists('Give'),
        ];
        // GiveWP Analytics.
        $parsed_addons['givewp_google_analytics'] = [
            'active' => function_exists('Give_Google_Analytics'),
        ];
        // Cookiebot.
        $parsed_addons['cookiebot'] = [
            'active' =>
                function_exists('cookiebot_active') && cookiebot_active(),
        ];
        // Cookie Notice.
        $parsed_addons['cookie_notice'] = [
            'active' => class_exists('Cookie_Notice'),
        ];
        // Fb Instant Articles.
        $parsed_addons['instant_articles'] = [
            'active' =>
                defined('IA_PLUGIN_VERSION') &&
                version_compare(IA_PLUGIN_VERSION, '3.3.4', '>'),
        ];
        // Google AMP.
        $parsed_addons['google_amp'] = [
            'active' => defined('AMP__FILE__'),
        ];
        // WPForms.
        $parsed_addons['wpforms-lite'] = [
            'active' => function_exists('wpforms'),
            'icon' =>
                plugin_dir_url(MONSTERINSIGHTS_PLUGIN_FILE) .
                'assets/images/plugin-wpforms.png',
            'title' => 'WPForms',
            'excerpt' => __(
                'The best drag & drop WordPress form builder. Easily create beautiful contact forms, surveys, payment forms, and more with our 150+ form templates. Trusted by over 4 million websites as the best forms plugin',
        ...
        // AIOSEO.
        $parsed_addons['aioseo'] = [
            'active' => function_exists('aioseo'),
            'icon' =>
                plugin_dir_url(MONSTERINSIGHTS_PLUGIN_FILE) .
                'assets/images/plugin-all-in-one-seo.png',
            'title' => 'AIOSEO',
            'excerpt' => __(
                'The original WordPress SEO plugin and toolkit that improves your website’s search rankings. Comes with all the SEO features like Local SEO, WooCommerce SEO, sitemaps, SEO optimizer, schema, and more.',
                'google-analytics-for-wordpress'
            ),
            'installed' => array_key_exists(
        ...
        $parsed_addons['optinmonster'] = [
            ...
            'title' => 'OptinMonster',
            'excerpt' => __(
                'Instantly get more subscribers, leads, and sales with the #1 conversion optimization toolkit. Create high converting popups, announcement bars, spin a wheel, and more with smart targeting and personalization.',
                'google-analytics-for-wordpress'
            ),
            'installed' => array_key_exists(
        ...
        $parsed_addons['wp-mail-smtp'] = [
            ...
            'title' => 'WP Mail SMTP',
            'excerpt' => __(
                'Improve your WordPress email deliverability and make sure that your website emails reach user’s inbox with the #1 SMTP plugin for WordPress. Over 2 million websites use it to fix WordPress email issues.',
                'google-analytics-for-wordpress'
            ),
            'installed' => array_key_exists(
        ...
        $parsed_addons['coming-soon'] = [
            ...
            'icon' =>
                plugin_dir_url(MONSTERINSIGHTS_PLUGIN_FILE) .
                'assets/images/plugin-seedprod.png',
            'title' => 'SeedProd',
            'excerpt' => __(
                'The fastest drag & drop landing page builder for WordPress. Create custom landing pages without writing code, connect them with your CRM, collect subscribers, and grow your audience. Trusted by 1 million sites.',
                'google-analytics-for-wordpress'
            ),
            'installed' => array_key_exists(
        ...
        $parsed_addons['rafflepress'] = [
            ...
            'icon' =>
                plugin_dir_url(MONSTERINSIGHTS_PLUGIN_FILE) .
                'assets/images/pluign-rafflepress.png',
            'title' => 'RafflePress',
            'excerpt' => __(
                'Turn your website visitors into brand ambassadors! Easily grow your email list, website traffic, and social media followers with the most powerful giveaways & contests plugin for WordPress.',
                'google-analytics-for-wordpress'
            ),
            'installed' => array_key_exists(
        ...
        // TrustPulse
        $parsed_addons['trustpulse-api'] = [
            'active' => class_exists('TPAPI'),
            'icon' =>
                plugin_dir_url(MONSTERINSIGHTS_PLUGIN_FILE) .
                'assets/images/plugin-trust-pulse.png',
            'title' => 'TrustPulse',
            'excerpt' => __(
                'Boost your sales and conversions by up to 15% with real-time social proof notifications. TrustPulse helps you show live user activity and purchases to help convince other users to purchase.',
                'google-analytics-for-wordpress'
            ),
            'installed' => array_key_exists(
        ...
        $parsed_addons['smash-balloon-instagram'] = [
            'active' => class_exists('sb_instagram_feed_init'),
            'icon' =>
                plugin_dir_url(MONSTERINSIGHTS_PLUGIN_FILE) .
                'assets/images/plugin-smash-balloon.png',
            'title' => 'Smash Balloon Instagram Feeds',
            'excerpt' => __(
                'Easily display Instagram content on your WordPress site without writing any code. Comes with multiple templates, ability to show content from multiple accounts, hashtags, and more. Trusted by 1 million websites.',
                'google-analytics-for-wordpress'
            ),
            'installed' => array_key_exists(
        ...
        $parsed_addons['pushengage'] = [
            ...
            'title' => 'PushEngage',
            'excerpt' => __(
                'Connect with your visitors after they leave your website with the leading web push notification software. Over 10,000+ businesses worldwide use PushEngage to send 9 billion notifications each month.',
                'google-analytics-for-wordpress'
            ),
            'installed' => array_key_exists(
                'pushengage/main.php',
    ...
    public function update_manual_ua()
    {
        ...
        $manual_ua_code = isset($_POST['manual_ua_code'])
        ...
        if (
            !empty($_REQUEST['isnetwork']) &&
            sanitize_text_field(wp_unslash($_REQUEST['isnetwork']))
        ) {
            define('WP_NETWORK_ADMIN', true);
        }
        $manual_ua_code_old = is_network_admin()
            ? MonsterInsights()->auth->get_network_manual_ua()
            : MonsterInsights()->auth->get_manual_ua();

        if (
            $manual_ua_code &&
            $manual_ua_code_old &&
            $manual_ua_code_old === $manual_ua_code
        ) {
        ...
        } elseif ($manual_ua_code && empty($manual_ua_code_old)) {
            // Move to manual.
            if (is_network_admin()) {
                MonsterInsights()->auth->set_network_manual_ua($manual_ua_code);
            } else {
        ...
        } elseif (empty($manual_ua_code) && $manual_ua_code_old) {
            // Deleted manual.
            if (is_network_admin()) {
        ...
        } elseif (isset($_POST['manual_ua_code']) && empty($manual_ua_code)) {
            wp_send_json_error([
                'error' => __(
                    'Invalid UA code',
                    'google-analytics-for-wordpress'
    ...
    public function handle_settings_import()
    {
        ...
        $extension = explode(
            '.',
            sanitize_text_field(wp_unslash($_FILES['import_file']['name']))
        );
        ...
        $import_file = sanitize_text_field(
            wp_unslash($_FILES['import_file']['tmp_name'])
        );
    ...
    public function get_report_data()
    {
        ...
        $report_name = isset($_POST['report'])
            ? sanitize_text_field(wp_unslash($_POST['report']))
            : '';
        ...
        $start = !empty($_POST['start'])
            ? sanitize_text_field(wp_unslash($_POST['start']))
            : $report->default_start_date();
        $end = !empty($_POST['end'])
            ? sanitize_text_field(wp_unslash($_POST['end']))
            : $report->default_end_date();
        ...
        if (
            monsterinsights_is_pro_version() &&
            !MonsterInsights()->license->license_can($report->level)
        ) {
            $data = [
                'success' => false,
                'error' => 'license_level',
            ...
            $data = apply_filters(
                'monsterinsights_vue_reports_data',
        ...
        wp_send_json_error([
            'message' => __(
                'We encountered an error when fetching the report data.',
                'google-analytics-for-wordpress'
            ),
        ]);
    }
    ...
    public function install_plugin()
    {
        ...
        if (!monsterinsights_can_install_plugins()) {
            wp_send_json([
                'error' => esc_html__(
                    'You are not allowed to install plugins',
                    'google-analytics-for-wordpress'
                ),
            ]);
        }
        ...
        $installer = new MonsterInsights_Plugin_Upgrader(
            new MonsterInsights_Skin()
        );
    ...
    public function get_notice_status()
    {
        ...
        $notice_id = empty($_POST['notice'])
            ? false
            : sanitize_text_field(wp_unslash($_POST['notice']));
        if (!$notice_id) {
            wp_send_json_error();
        }
    ...
    public function get_posts()
    {
        ...
        $args = [
            's' => isset($_POST['keyword'])
                ? sanitize_text_field(wp_unslash($_POST['keyword']))
                : '',
            'post_type' => $post_type,
            'posts_per_page' => isset($_POST['numberposts'])
                ? sanitize_text_field(wp_unslash($_POST['numberposts']))
                : 10,
            'orderby' => 'relevance',
        ];
        ...
        if (in_array($post_type, ['page', 'any'], true)) {
    ...
    public function get_post_types()
    {
        ...
        foreach ($post_types as $post_type) {
            ...
            if (
                ...
                'product' === $post_type->name
            ) {
                continue;
            }
            $post_types_parsed[$post_type->name] =
                $post_type->labels->singular_name;
        }

        $post_types_parsed = apply_filters(
            'monsterinsights_vue_post_types_editor',
            $post_types_parsed
        );
