...
final class MonsterInsights_Notice_Admin
{
     ...
     */
    public function display_inline_notice(
        $name,
        $title,
        $message,
        $type = 'success',
        ...
        ob_start();
        ?>
        <div class="monsterinsights-notice <?php echo 'monsterinsights-' .
            esc_attr($type) .
            '-notice' .
            $dismissible; ?>" data-notice="<?php echo esc_attr($name); ?>">
            <div class="monsterinsights-notice-icon <?php echo 'monsterinsights-' .
                esc_attr($type) .
                '-notice-icon'; ?>">
            </div>
            <div class="monsterinsights-notice-text <?php echo 'monsterinsights-' .
                esc_attr($type) .
                '-notice-text'; ?>">
                <?php
                // Title
                if (!empty($title)) { ?>
                    <p class="monsterinsights-notice-title"><?php echo esc_html(
                        $title
                    ); ?></p>
                    <?php }
                ...
                if (!empty($message)) {
                    if (empty($args['skip_message_escape'])) { ?>
                        <p class="monsterinsights-notice-message"><?php echo esc_html(
                            $message
                        ); ?></p>
                        <?php } else { ?>
                ...
                if (!empty($args['primary']['text'])) {
                    ...
                    $target = '';
                    if (
                        !empty($args['primary']['target']) &&
                        $args['primary']['target'] === 'blank'
                    ) {
...
?>><?php echo esc_html($text); ?></a>
                ...
                if (!empty($args['secondary']['text'])) {
                    ...
                    $target = '';
                    if (
                        !empty($args['secondary']['target']) &&
                        $args['secondary']['target'] === 'blank'
                    ) {
                        $target = ' target="_blank" rel="noopener noreferrer"';
                    }
