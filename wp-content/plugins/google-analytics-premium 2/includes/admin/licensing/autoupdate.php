    ...
    foreach ($addons_data as $type => $addons) {
        foreach ($addons as $addon) {
            ...
            if (
                'm' === $slug[0] &&
                $slug === $basename &&
                'monsterinsights-ecommerce' === $slug
            ) {
                $basename = monsterinsights_get_plugin_basename_from_slug(
                    'ga-ecommerce'
                );
            }
 ...
 * @return string
 */
function monsterinsights_modify_wordpress_autoupdater_setting(
    $html,
    $plugin_file,
    $plugin_data
) {
    ...
    $is_main_free =
        ...
        'google-analytics-for-wordpress' === $plugin_data['slug'];
    $is_main_pro =
        $is_pro &&
        plugin_basename(MONSTERINSIGHTS_PLUGIN_FILE) === $plugin_file;
...
add_filter(
    ...
    'monsterinsights_modify_wordpress_autoupdater_setting',
    10,
    3
);
...
function monsterinsights_automatic_updates($update, $item)
{
    ...
    $is_free =
        isset($item['slug']) &&
        'google-analytics-for-wordpress' === $item['slug'];
    ...
    if (function_exists('get_current_screen')) {
        ...
        if (
            !empty($screen) &&
            !empty($screen->id) &&
            in_array($screen->id, ['plugins', 'plugins-network'])
        ) {
            $is_pro = monsterinsights_is_pro_version();
            ...
            if ($is_free || $is_main_pro || ($is_addon && $is_pro)) {
                return in_array($automatic_updates, ['all', 'minor']);
    ...
    if (
        (!$is_free && !$is_paid) ||
        ($is_free && !defined('MONSTERINSIGHTS_LITE_VERSION'))
    ) {
        return $update;
    }
