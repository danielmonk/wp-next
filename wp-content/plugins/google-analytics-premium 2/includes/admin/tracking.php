...
class MonsterInsights_Tracking
{
    public function __construct()
    {
        ...
        add_action('monsterinsights_settings_save_general_end', [
            $this,
            'check_for_settings_optin',
        ]);
        add_action('admin_head', [$this, 'check_for_optin']);
        add_action('admin_head', [$this, 'check_for_optout']);
        add_filter('cron_schedules', [$this, 'add_schedules']);
        add_action('monsterinsights_usage_tracking_cron', [
            $this,
            'send_checkin',
        ]);
    }
    ...
    private function get_data()
    {
        ...
        $tracking_mode = monsterinsights_get_option(
            'tracking_mode',
            'analytics'
        );
        ...
        $data['server'] = isset($_SERVER['SERVER_SOFTWARE'])
            ? $_SERVER['SERVER_SOFTWARE']
            : '';
        $data['over_time'] = get_option('monsterinsights_over_time', []);
        ...
        $data['sites'] = $count_b;
        $data['usagetracking'] = get_option(
            'monsterinsights_usage_tracking_config',
            false
        );
        $data['usercount'] = function_exists('get_user_count')
            ? get_user_count()
            : 'Not Set';
        $data['usesauth'] = $usesauth;
        $data['timezoneoffset'] = date('P');
        $data['installed_lite'] = get_option(
            'monsterinsights_installed_lite',
            []
        );
        $data['installed_pro'] = get_option(
            'monsterinsights_installed_pro',
            []
        );
    ...
    public function send_checkin(
        ...
        if (
            is_numeric($last_send) &&
            $last_send > strtotime('-1 week') &&
            !$ignore_last_checkin
        ) {
            return false;
        }

        $request = wp_remote_post('https://miusage.com/v1/checkin/', [
            'method' => 'POST',
            'timeout' => 5,
            'redirection' => 5,
            'httpversion' => '1.1',
            ...
            'body' => $this->get_data(),
            'user-agent' =>
                'MI/' . MONSTERINSIGHTS_VERSION . '; ' . get_bloginfo('url'),
        ]);
    ...
    public function schedule_send()
    {
        if (!wp_next_scheduled('monsterinsights_usage_tracking_cron')) {
            ...
            $tracking['offset'] =
                $tracking['day'] * DAY_IN_SECONDS +
                $tracking['hour'] * HOUR_IN_SECONDS +
                $tracking['minute'] * MINUTE_IN_SECONDS +
                $tracking['second'];
            $tracking['initsend'] =
                strtotime("next sunday") + $tracking['offset'];
    ...
    public function check_for_optin()
    {
        if (
            !(
                !empty($_REQUEST['mi_action']) &&
                'opt_into_tracking' === $_REQUEST['mi_action']
            )
        ) {
            return;
        }
    ...
    public function check_for_optout()
    {
        if (
            !(
                !empty($_REQUEST['mi_action']) &&
                'opt_out_of_tracking' === $_REQUEST['mi_action']
            )
        ) {
            return;
        }
