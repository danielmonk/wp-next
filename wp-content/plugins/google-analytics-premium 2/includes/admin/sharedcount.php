...
class MonsterInsights_SharedCount
{
    ...
    {
        add_action('wp_ajax_monsterinsights_sharedcount_start_indexing', [
            $this,
            'ajax_start_indexing',
        ]);
        add_action('wp_ajax_monsterinsights_sharedcount_get_index_progress', [
            $this,
            'ajax_get_index_progress',
        ]);

        add_action('monsterinsights_sharedcount_get_more_posts', [
            $this,
            'get_more_counts',
        ]);
        ...
        add_action(
            ...
            [$this, 'grab_and_store_bulk_by_id'],
            10,
            2
        );

        add_action($this->cron_key, [$this, 'daily_cron_update']);
    }
    ...
    public function ajax_start_indexing()
    {
        ...
        wp_send_json_error([
            'message' => esc_html__(
                'The SharedCount API key is not set',
    ...
    public function get_api_url($path = '')
    {
        ...
        return apply_filters(
            'monsterinsights_sharedcount_api_url',
            $this->endpoint . $path
        );
    }
    ...
    {
        if (
            isset($parsed_response['Error']) &&
            isset($parsed_response['Type']) &&
            'invalid_api_key' === $parsed_response['Type']
        ) {
            $error = esc_html__(
                'The SharedCount API key is invalid',
                'google-analytics-for-wordpress'
            );
        } elseif (!empty($parsed_response['quota_exceeded'])) {
            $error = $parsed_response['quota_exceeded'];
        } else {
            $error = isset($parsed_response['Error'])
                ? $parsed_response['Error']
                : esc_html__(
                    'There was an error grabbing data from SharedCount, please check the API Key',
                    'google-analytics-for-wordpress'
                );
        }
        $this->error = $error;
    ...
    public function store_bulk_data($bulk_data, $urls_as_keys)
    {
        if (!empty($bulk_data['data']) && is_array($bulk_data['data'])) {
            foreach ($bulk_data['data'] as $url => $values) {
                $post_id = array_key_exists($url, $urls_as_keys)
                    ? $urls_as_keys[$url]
                    : false;
    ...
    public function store_post_counts($post_id, $values)
    {
        ...
        update_post_meta(
            $post_id,
            '_monsterinsights_sharedcount_total',
            $total_count
        );
        update_post_meta(
            $post_id,
            '_monsterinsights_sharedcount_values',
            $values
        );
    }
    ...
    {
        $sort_option = monsterinsights_get_option(
            'popular_posts_inline_sort',
            'comments'
        );
    ...
    public static function get_index_progress_percent()
    {
        ...
        if (
            ...
            isset($progress['max_pages']) &&
            false === $progress['max_pages']
        ) {
            return 100;
        }
    ...
    public static function get_index_progress()
    {
        if (empty(self::$progress)) {
            self::$progress = get_option(self::$progress_key, []);
        }
