...
function monsterinsights_get_addons()
{
    ...
    if (monsterinsights_is_pro_version()) {
        ...
        $type = is_network_admin()
            ? MonsterInsights()->license->get_network_license_type()
            : MonsterInsights()->license->get_site_license_type();
    }
    ...
    foreach ((array) $addons as $i => $addon) {
        ...
        if (
            empty($type) ||
            (in_array('Pro', $addon->categories) &&
                ($type != 'pro' && $type != 'master')) ||
            (in_array('Plus', $addon->categories) &&
                $type != 'plus' &&
                $type != 'pro' &&
                $type != 'master') ||
            (in_array('Basic', $addon->categories) &&
                ($type != 'basic' &&
                    $type != 'plus' &&
                    $type != 'pro' &&
                    $type != 'master'))
...
function monsterinsights_get_addons_data($key)
{
    ...
    if (monsterinsights_is_pro_version() && $key) {
        $addons = MonsterInsights()->license_actions->perform_remote_request(
            'get-addons-data-v600',
            ['tgm-updater-key' => $key]
        );
    } else {
