...
final class MonsterInsights_API_Auth
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_monsterinsights_maybe_authenticate', [
            $this,
            'maybe_authenticate',
        ]);
        add_action('wp_ajax_monsterinsights_maybe_reauthenticate', [
            $this,
            'maybe_reauthenticate',
        ]);
        add_action('wp_ajax_monsterinsights_maybe_verify', [
            $this,
            'maybe_verify',
        ]);
        add_action('wp_ajax_monsterinsights_maybe_delete', [
            $this,
            'maybe_delete',
        ]);
        ...
        add_action('wp_ajax_nopriv_monsterinsights_is_installed', [
            $this,
            'is_installed',
        ]);
        add_action('wp_ajax_nopriv_monsterinsights_rauthenticate', [
            $this,
            'rauthenticate',
        ]);

        add_filter('monsterinsights_maybe_authenticate_siteurl', [
            $this,
            'before_redirect',
        ]);
    }
    ...
    {
        $tt = is_network_admin()
            ? get_site_option('monsterinsights_network_tt', '')
            : get_option('monsterinsights_site_tt', '');
        if (empty($tt)) {
            // if TT is empty, generate a new one, save it and then return it
            $tt = $this->generate_tt();
            $this->is_network_admin()
                ? update_site_option('monsterinsights_network_tt', $tt)
                : update_option('monsterinsights_site_tt', $tt);
        }
        return $tt;
    }
    ...
    public function generate_tt()
    {
        return hash(
            'sha512',
            wp_generate_password(128, true, true) . AUTH_SALT . uniqid("", true)
        );
    }
    ...
    public function maybe_authenticate()
    {
        ...
        if (!current_user_can('monsterinsights_save_settings')) {
            wp_send_json_error([
                'message' => __(
                    "You don't have permission to authenticate MonsterInsights.",
        ...
        if (monsterinsights_is_pro_version()) {
            $valid = is_network_admin()
                ? MonsterInsights()->license->is_network_licensed()
                : MonsterInsights()->license->is_site_licensed();
            if (!$valid) {
                wp_send_json_error([
                    'message' => __(
                        "Cannot authenticate. Please enter a valid, active license key for MonsterInsights Pro into the settings.",
                        'google-analytics-for-wordpress'
                    ),
                ]);
            }
        ...
        if (
            ...
            wp_send_json_error([
                'message' => __(
                    "Cannot authenticate. Please re-authenticate.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        } elseif (
            ...
            wp_send_json_error([
                'message' => __(
                    "Cannot network authenticate. Please re-authenticate on the network settings panel.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        }
        ...
        $siteurl = add_query_arg(
            [
                ...
                'siteurl' => is_network_admin()
                    ? network_admin_url()
                    : site_url(),
                'return' => is_network_admin()
                    ? network_admin_url(
                        'admin.php?page=monsterinsights_network'
                    )
                    : admin_url('admin.php?page=monsterinsights_settings'),
                'testurl' =>
                    'https://' . monsterinsights_get_api_url() . 'test/',
            ],
            $this->get_route(
                'https://' . monsterinsights_get_api_url() . 'auth/new/{type}'
            )
        );
        ...
        $siteurl = apply_filters(
            'monsterinsights_maybe_authenticate_siteurl',
            $siteurl
        );
        wp_send_json_success(['redirect' => $siteurl]);
    }
    ...
    public function authenticate_listener()
    {
        ...
        if (
            empty($_REQUEST['mi-oauth-action']) ||
            $_REQUEST['mi-oauth-action'] !== 'auth'
        ) {
            return;
        }
    ...
    public function maybe_reauthenticate()
    {
        ...
        if (!current_user_can('monsterinsights_save_settings')) {
            wp_send_json_error([
                'message' => __(
                    "You don't have permission to re-authenticate MonsterInsights.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        }
        ...
        if (monsterinsights_is_pro_version()) {
            ...
            if (monsterinsights_is_pro_version() && !$valid) {
                wp_send_json_error([
                    'message' => __(
                        "Cannot re-authenticate. Please enter a valid, active license key for MonsterInsights Pro into the settings.",
                        'google-analytics-for-wordpress'
                    ),
                ]);
            }
        ...
        if (
            ...
            wp_send_json_error([
                'message' => __(
                    "Cannot re-authenticate. Please authenticate.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        } elseif (
            ...
            wp_send_json_error([
                'message' => __(
                    "Cannot re-authenticate the network. Please authenticate on the network settings panel.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        }
        ...
        $siteurl = add_query_arg(
            [
                ...
                'siteurl' => is_network_admin()
                    ...
                    : site_url(),
                'key' => is_network_admin()
                    ? MonsterInsights()->auth->get_network_key()
                    : MonsterInsights()->auth->get_key(),
                'token' => is_network_admin()
                    ? MonsterInsights()->auth->get_network_token()
                    : MonsterInsights()->auth->get_token(),
                'return' => is_network_admin()
                    ? network_admin_url(
                        'admin.php?page=monsterinsights_network'
                    )
    ...
    public function reauthenticate_listener()
    {
        ...
        if (
            empty($_REQUEST['mi-oauth-action']) ||
            $_REQUEST['mi-oauth-action'] !== 'reauth'
        ) {
            return;
        }
        ...
        // we do have a current auth
        $existing = $this->is_network_admin()
        ...
        $url = apply_filters(
            'monsterinsights_reauth_success_redirect_url',
            $url
        );
    ...
    public function maybe_verify()
    {
        ...
        if (!current_user_can('monsterinsights_save_settings')) {
            wp_send_json_error([
                'message' => __(
                    "You don't have permission to verify MonsterInsights.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        }
        ...
        if (
            ...
            wp_send_json_error([
                'message' => __(
                    "Cannot verify. Please authenticate.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        } elseif (
        ...
        if (monsterinsights_is_pro_version()) {
            ...
            if (!$valid) {
                wp_send_json_error([
                    'message' => __(
                        "Cannot verify. Please enter a valid, active license key for MonsterInsights Pro into the settings.",
                        'google-analytics-for-wordpress'
                    ),
                ]);
            }
    ...
    {
        $creds = !empty($credentials)
            ? $credentials
            : ($this->is_network_admin()
                ? MonsterInsights()->auth->get_network_analytics_profile(true)
                : MonsterInsights()->auth->get_analytics_profile(true));
        ...
        $network = !empty($_REQUEST['network'])
            ? $_REQUEST['network'] === 'network'
            : $this->is_network_admin();
        $api = new MonsterInsights_API_Request(
            $this->get_route('auth/verify/{type}/'),
            [
                'network' => $network,
                'tt' => $this->get_tt(),
                'key' => $creds['key'],
    ...
    public function maybe_delete()
    {
        ...
        if (!current_user_can('monsterinsights_save_settings')) {
            wp_send_json_error([
                'message' => __(
                    "You don't have permission to deauthenticate MonsterInsights.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        }
        ...
        if (
            ...
            wp_send_json_error([
                'message' => __(
                    "Cannot deauthenticate. You are not currently authed.",
                    'google-analytics-for-wordpress'
                ),
            ]);
        } elseif (
        ...
        if (monsterinsights_is_pro_version()) {
            ...
            if (!$valid) {
                wp_send_json_error([
                    'message' => __(
                        "Cannot deauthenticate. Please enter a valid, active license key for MonsterInsights Pro into the settings.",
                        'google-analytics-for-wordpress'
                    ),
                ]);
            }
        ...
        $force =
            !empty($_REQUEST['forcedelete']) &&
            $_REQUEST['forcedelete'] === 'true';
    ...
    public function delete_auth($force = false)
    {
        ...
        $api = new MonsterInsights_API_Request(
            $this->get_route('auth/delete/{type}/'),
            [
    ...
    public function before_redirect($url)
    {
        ...
        if (function_exists('bb2_read_settings')) {
            ...
            if (
                empty($bb_settings['offsite_forms']) ||
                false === $bb_settings['offsite_forms']
            ) {
