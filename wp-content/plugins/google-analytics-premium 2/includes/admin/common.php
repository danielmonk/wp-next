...
function monsterinsights_is_settings_page()
{
    ...
    $settings_page = false;
    if (
        !empty($admin_page_hooks['monsterinsights_settings']) &&
        $current_screen->id === $admin_page_hooks['monsterinsights_settings']
    ) {
        $settings_page = true;
    }
    ...
    if (
        ...
        strpos($current_screen->base, 'monsterinsights_network') !== false
    ) {
        $settings_page = true;
    }
...
function monsterinsights_is_reports_page()
{
    ...
    $reports_page = false;
    if (
        !empty($admin_page_hooks['monsterinsights_reports']) &&
        $current_screen->id === $admin_page_hooks['monsterinsights_reports']
    ) {
        $reports_page = true;
    }
...
function monsterinsights_admin_styles()
{
    ...
    wp_register_style(
        'monsterinsights-admin-common-style',
        plugins_url(
            'assets/css/admin-common' . $suffix . '.css',
    ...
    if (
        empty($screen->id) ||
        strpos($screen->id, 'monsterinsights') === false
    ) {
        return;
    }
    ...
    if (monsterinsights_is_settings_page()) {
        if (!defined('MONSTERINSIGHTS_LOCAL_JS_URL')) {
            wp_enqueue_style(
                'monsterinsights-vue-style-vendors',
                plugins_url(
                    $version_path .
                        '/assets/vue/css/chunk-vendors' .
                        $rtl .
                        '.css',
                    MONSTERINSIGHTS_PLUGIN_FILE
                ),
            ...
            wp_enqueue_style(
                'monsterinsights-vue-style-common',
                plugins_url(
                    $version_path .
                        '/assets/vue/css/chunk-common' .
                        $rtl .
                        '.css',
                    MONSTERINSIGHTS_PLUGIN_FILE
                ),
            ...
            wp_enqueue_style(
                'monsterinsights-vue-style',
                plugins_url(
                    $version_path . '/assets/vue/css/settings' . $rtl . '.css',
                    MONSTERINSIGHTS_PLUGIN_FILE
                ),
                [],
                monsterinsights_get_asset_version()
    ...
    if (monsterinsights_is_reports_page()) {
        if (!defined('MONSTERINSIGHTS_LOCAL_REPORTS_JS_URL')) {
            ...
            wp_enqueue_style(
                'monsterinsights-vue-style',
                plugins_url(
                    $version_path . '/assets/vue/css/reports' . $rtl . '.css',
                    MONSTERINSIGHTS_PLUGIN_FILE
                ),
                [],
                monsterinsights_get_asset_version()
...
function monsterinsights_admin_scripts()
{
    ...
    wp_register_script(
        ...
        plugins_url(
            'assets/js/admin-common' . $suffix . '.js',
            MONSTERINSIGHTS_PLUGIN_FILE
        ),
        ['jquery'],
        monsterinsights_get_asset_version()
    ...
    wp_localize_script(
        ...
        [
            ...
            'dismiss_notice_nonce' => wp_create_nonce(
                'monsterinsights-dismiss-notice'
            ),
        ]
    );
    ...
    if (monsterinsights_is_settings_page()) {
        if (!defined('MONSTERINSIGHTS_LOCAL_VENDORS_JS_URL')) {
            wp_enqueue_script(
                ...
                plugins_url(
                    $version_path . '/assets/vue/js/chunk-vendors.js',
                    MONSTERINSIGHTS_PLUGIN_FILE
                ),
                [],
                monsterinsights_get_asset_version(),
            ...
            wp_enqueue_script(
                ...
                plugins_url(
                    $version_path . '/assets/vue/js/chunk-common.js',
                    MONSTERINSIGHTS_PLUGIN_FILE
                ),
                [],
                monsterinsights_get_asset_version(),
            ...
            wp_enqueue_script(
                'monsterinsights-vue-vendors',
                MONSTERINSIGHTS_LOCAL_VENDORS_JS_URL,
                [],
                monsterinsights_get_asset_version(),
                true
            );
            wp_enqueue_script(
                'monsterinsights-vue-common',
                MONSTERINSIGHTS_LOCAL_COMMON_JS_URL,
                [],
                monsterinsights_get_asset_version(),
                true
            );
        ...
        $app_js_url =
            defined('MONSTERINSIGHTS_LOCAL_JS_URL') &&
            MONSTERINSIGHTS_LOCAL_JS_URL
                ? MONSTERINSIGHTS_LOCAL_JS_URL
                : plugins_url(
                    $version_path . '/assets/vue/js/settings.js',
                    MONSTERINSIGHTS_PLUGIN_FILE
                );
        wp_register_script(
        ...
        if (monsterinsights_can_install_plugins()) {
            ...
            if (array_key_exists($amp_key, $plugins)) {
                $install_amp_url = wp_nonce_url(
                    self_admin_url(
                        'plugins.php?action=activate&plugin=' . $amp_key
                    ),
                    'activate-plugin_' . $amp_key
                );
            } else {
                $install_amp_url = wp_nonce_url(
                    self_admin_url(
                        'update.php?action=install-plugin&plugin=amp'
                    ),
                    'install-plugin_amp'
                );
            }
        ...
        if (monsterinsights_can_install_plugins()) {
            ...
            if (array_key_exists($woo_key, $plugins)) {
                $install_woocommerce_url = wp_nonce_url(
                    self_admin_url(
                        'plugins.php?action=activate&plugin=' . $woo_key
                    ),
                    'activate-plugin_' . $woo_key
                );
            } else {
                $install_woocommerce_url = wp_nonce_url(
                    self_admin_url(
                        'update.php?action=install-plugin&plugin=woocommerce'
                    ),
        ...
        if (monsterinsights_can_install_plugins()) {
            ...
            if (array_key_exists($fbia_key, $plugins)) {
                $install_fbia_url = wp_nonce_url(
                    self_admin_url(
                        'plugins.php?action=activate&plugin=' . $fbia_key
                    ),
                    'activate-plugin_' . $fbia_key
                );
            } else {
                $install_fbia_url = wp_nonce_url(
                    self_admin_url(
                        'update.php?action=install-plugin&plugin=fb-instant-articles'
                    ),
                    'install-plugin_fb-instant-articles'
                );
            }
        }
        ...
        wp_localize_script('monsterinsights-vue-script', 'monsterinsights', [
            'ajax' => admin_url('admin-ajax.php'),
            ...
            'assets' => plugins_url(
                $version_path . '/assets/vue',
                MONSTERINSIGHTS_PLUGIN_FILE
            ),
            'roles' => monsterinsights_get_roles(),
            'roles_manage_options' => monsterinsights_get_manage_options_roles(),
            'shareasale_id' => monsterinsights_get_shareasale_id(),
            'shareasale_url' => monsterinsights_get_shareasale_url(
                monsterinsights_get_shareasale_id(),
                ''
            ),
            'addons_url' => is_multisite()
                ? network_admin_url(
                    'admin.php?page=monsterinsights_network#/addons'
                )
                : admin_url('admin.php?page=monsterinsights_settings#/addons'),
            'email_summary_url' => admin_url(
                'admin.php?monsterinsights_email_preview&monsterinsights_email_template=summary'
            ),
            ...
            'first_run_notice' => apply_filters(
                'monsterinsights_settings_first_time_notice_hide',
                monsterinsights_get_option('monsterinsights_first_run_notice')
            ),
            'getting_started_url' => is_network_admin()
                ? network_admin_url(
                    'admin.php?page=monsterinsights_network#/about'
                )
                : admin_url(
                    'admin.php?page=monsterinsights_settings#/about/getting-started'
                ),
            'authed' => $is_authed,
            'new_pretty_link_url' => admin_url(
                'post-new.php?post_type=pretty-link'
            ),
            'wpmailsmtp_admin_url' => admin_url('admin.php?page=wp-mail-smtp'),
            'load_headline_analyzer_settings' => monsterinsights_load_gutenberg_app()
                ? 'true'
                : 'false',
        ]);
    ...
    if (monsterinsights_is_reports_page()) {
        ...
        $app_js_url =
            defined('MONSTERINSIGHTS_LOCAL_REPORTS_JS_URL') &&
            MONSTERINSIGHTS_LOCAL_REPORTS_JS_URL
                ? MONSTERINSIGHTS_LOCAL_REPORTS_JS_URL
                : plugins_url(
                    $version_path . '/assets/vue/js/reports.js',
                    MONSTERINSIGHTS_PLUGIN_FILE
                );
        wp_register_script(
        ...
        wp_localize_script('monsterinsights-vue-reports', 'monsterinsights', [
            'ajax' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mi-admin-nonce'),
            'network' => is_network_admin(),
            'translations' => wp_get_jed_locale_data(
            ...
            'activate_nonce' => wp_create_nonce('monsterinsights-activate'),
            'deactivate_nonce' => wp_create_nonce('monsterinsights-deactivate'),
            'update_settings' => current_user_can(
...
function monsterinsights_remove_conflicting_asset_files()
{
    ...
    foreach ($wp_styles->queue as $handle) {
        if (
            strpos($wp_styles->registered[$handle]->src, 'wp-content') === false
        ) {
            return;
        }
        ...
        foreach ($third_party as $partial) {
            if (
                strpos($wp_styles->registered[$handle]->handle, $partial) !==
                false
            ) {
                ...
                strpos($wp_styles->registered[$handle]->src, $partial) !== false
            ) {
                wp_dequeue_style($handle); // Remove css file from MI screen
                wp_deregister_style($handle);
                break;
    ...
    foreach ($wp_scripts->queue as $handle) {
        if (
            strpos($wp_scripts->registered[$handle]->src, 'wp-content') ===
            false
        ) {
            return;
        }
        ...
        foreach ($third_party as $partial) {
            if (
                strpos($wp_scripts->registered[$handle]->handle, $partial) !==
                false
            ) {
                ...
                strpos($wp_scripts->registered[$handle]->src, $partial) !==
                false
            ) {
                wp_dequeue_script($handle); // Remove JS file from MI screen
                wp_deregister_script($handle);
    ...
    remove_action(
        ...
        'td_on_admin_print_styles_farbtastic'
    );
    remove_action(
        'admin_print_footer_scripts',
    ...
    remove_action(
        'admin_footer',
        'wpbooklist_jre_dismiss_prem_notice_forever_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_dashboard_add_book_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_edit_book_show_form_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_show_book_in_colorbox_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_new_lib_shortcode_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_dashboard_save_library_display_options_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_dashboard_save_post_display_options_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_dashboard_save_page_display_options_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_update_display_options_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_edit_book_pagination_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_edit_book_switch_lib_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_edit_book_search_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_edit_book_actual_action_javascript'
    );
    ...
    remove_action(
        'admin_footer',
        'wpbooklist_upload_new_stylepak_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_upload_new_post_template_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_upload_new_page_template_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_create_db_library_backup_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_restore_db_library_backup_action_javascript'
    );
    remove_action('admin_footer', 'wpbooklist_create_csv_action_javascript');
    remove_action(
        'admin_footer',
        'wpbooklist_amazon_localization_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_delete_book_bulk_action_javascript'
    );
    ...
    remove_action(
        'admin_footer',
        'wpbooklist_storytime_select_category_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_storytime_get_story_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_storytime_expand_browse_action_javascript'
    );
    remove_action(
        'admin_footer',
        'wpbooklist_storytime_save_settings_action_javascript'
    );
...
add_action(
    ...
    'monsterinsights_remove_conflicting_asset_files',
    9999
);
...
function hide_non_monsterinsights_warnings()
{
    ...
    if (
        empty($_REQUEST['page']) ||
        strpos($_REQUEST['page'], 'monsterinsights') === false
    ) {
        return;
    }
    ...
    global $wp_filter;
    if (
        !empty($wp_filter['user_admin_notices']->callbacks) &&
        is_array($wp_filter['user_admin_notices']->callbacks)
    ) {
        foreach (
            ...
            foreach ($hooks as $name => $arr) {
                if (
                    is_object($arr['function']) &&
                    $arr['function'] instanceof Closure
                ) {
                    unset(
                        $wp_filter['user_admin_notices']->callbacks[$priority][
                            $name
                        ]
                    );
                    continue;
                ...
                if (
                    !empty($name) &&
                    strpos($name, 'monsterinsights') === false
                ) {
                    unset(
                        $wp_filter['user_admin_notices']->callbacks[$priority][
                            $name
        ...
        foreach (
            ...
            foreach ($hooks as $name => $arr) {
                if (
                    ...
                    unset(
                        $wp_filter['admin_notices']->callbacks[$priority][$name]
                    );
                    continue;
                }
                if (
    ...
    if (
        !empty($wp_filter['all_admin_notices']->callbacks) &&
        is_array($wp_filter['all_admin_notices']->callbacks)
    ) {
        foreach (
            ...
            foreach ($hooks as $name => $arr) {
                if (
                    ...
                    unset(
                        $wp_filter['all_admin_notices']->callbacks[$priority][
                            $name
                        ]
                    );
                    continue;
...
function monsterinsights_get_upgrade_link(
    ...
    if (!empty($shareasale_id)) {
        ...
        return esc_url(
            monsterinsights_get_shareasale_url($shareasale_id, $url)
        );
    } else {
        return esc_url($url);
    }
...
function monsterinsights_get_url(
    $medium = '',
    $campaign = '',
    $url = '',
    $escape = true
    ...
    $url = !empty($url)
        ? $url
        : 'https://www.monsterinsights.com/' . $default_url;
...
function monsterinsights_ublock_notice()
{
	...
	<div id="monsterinsights-ublock-origin-error" class="error inline" style="display:none;">
		<?php // Translators: Placeholders are for links to fix the issue.
  echo sprintf(
      esc_html__(
      ...
      '<a href="https://monsterinsights.com/docs/monsterinsights-asset-files-blocked/" target="_blank" rel="noopener noreferrer" referrer="no-referrer">',
      '</a>',
      '<a href="https://monsterinsights.com/contact/" target="_blank" rel="noopener noreferrer" referrer="no-referrer">'
  ); ?>
	</div>
...
function monsterinsights_prevent_version_number_removal($src)
{
    ...
    if (
        (false !== strpos($src, 'monsterinsights') ||
            false !== strpos($src, 'google-analytics-for-wordpress') ||
            false !== strpos($src, 'google-analytics-premium')) &&
        false === strpos($src, '?ver')
    ) {
...
add_filter(
    'script_loader_src',
    'monsterinsights_prevent_version_number_removal',
    9999,
    1
);
add_filter(
...
function monsterinsights_get_php_wp_version_warning_data()
{
    ...
    return [
        ...
        'php_version_below_54' => apply_filters(
            'monsterinsights_temporarily_hide_php_under_56_upgrade_warnings',
            version_compare(phpversion(), '5.6', '<')
        ),
        'php_version_below_56' => apply_filters(
            'monsterinsights_temporarily_hide_php_56_upgrade_warnings',
            version_compare(phpversion(), '5.6', '>=') &&
                version_compare(phpversion(), '7', '<')
        ),
        'php_update_link' => monsterinsights_get_url(
            'settings-notice',
            'settings-page',
            'https://www.monsterinsights.com/docs/update-php/'
        ),
        'wp_version' => $wp_version,
        'wp_version_below_46' => version_compare($wp_version, '4.9', '<'),
        'wp_version_below_49' => version_compare($wp_version, '5.3', '<'),
        'wp_update_link' => monsterinsights_get_url(
            'settings-notice',
            'settings-page',
            'https://www.monsterinsights.com/docs/update-wordpress/'
        ),
    ];
}
...
function monsterinsights_maybe_add_wp_php_version_notification()
{
    ...
    if ($needs_php_warning) {
        ...
        $notification['title'] = __(
            'ACTION REQUIRED: Your PHP version is putting your site at risk!',
            'google-analytics-for-wordpress'
        );
        if ($needs_wp_warning) {
            $notification['title'] = __(
                'ACTION REQUIRED: Speed your website up 400% with a single email!',
                'google-analytics-for-wordpress'
            );
        }

        $php_url = monsterinsights_get_url(
            'notifications',
            'upgrade-php',
            'https://www.monsterinsights.com/docs/update-php'
        );

        $notification['type'] = ['basic', 'lite', 'master', 'plus', 'pro'];
        // Translators: Placeholder is for the current PHP version.
        $notification['content'] =
            sprintf(
                esc_html__(
                    'In the next major release of MonsterInsights we are planning to remove support for the version of PHP you are using (%s). This insecure version is no longer supported by WordPress itself, so you are already missing out on the latest features of WordPress along with critical updates for security and performance (modern PHP versions make websites much faster).',
                    'google-analytics-for-wordpress'
                ),
                phpversion()
            ) . "\n\n";
        ...
        $notification['content'] .=
            sprintf(
                ...
                '<a target="_blank" href="' . $php_url . '">',
                '</a>'
            ) . "\n\n";
        $notification['content'] .= esc_html__(
            'Upgrading your PHP version will make sure you are able to continue using WordPress without issues in the future, keep your site secure, and will also make your website up to 400% faster!',
            'google-analytics-for-wordpress'
        );
    ...
    if ($needs_wp_warning) {
        $isitwp_url =
            'https://www.isitwp.com/upgrading-wordpress-is-easier-than-you-think/?utm_source=monsterinsights&utm_medium=notifications&utm_campaign=upgradewp';
        $wpbeginner_url =
            'https://www.wpbeginner.com/beginners-guide/why-you-should-always-use-the-latest-version-of-wordpress/utm_source=monsterinsights&utm_medium=notifications&utm_campaign=upgradewp';

        $notification['id'] = 'upgrade_wp_49_notification';
        $notification['title'] = __(
            'ACTION REQUIRED: Your WordPress version is putting your site at risk!',
            'google-analytics-for-wordpress'
        );
        $notification['type'] = ['basic', 'lite', 'master', 'plus', 'pro'];
        ...
        $notification['content'] =
            sprintf(
                esc_html__(
                    'In the next major release of MonsterInsights we are planning to remove support for the version of WordPress you are using (version %s). This version is several years out of date, and most plugins do not support this version anymore, so you could be missing out on critical updates for performance and security already!',
                    'google-analytics-for-wordpress'
                ),
                $wp_version
            ) . "\n\n";
        ...
        $notification['content'] .= esc_html__(
            'The good news: updating WordPress has never been easier and only takes a few moments.',
            'google-analytics-for-wordpress'
        );
        ...
        $notification['content'] .=
            sprintf(
                esc_html__(
                    'To update, we recommend following this %1$sstep by step guide for updating WordPress%2$s from IsItWP and afterwards check out %3$sWhy You Should Always Use the Latest Version of WordPress%4$s on WPBeginner.',
                    'google-analytics-for-wordpress'
                ),
                '<a target="_blank" href="' . $isitwp_url . '">',
                '</a>',
                '<a target="_blank" href="' . $wpbeginner_url . '">',
                '</a>'
            ) . "\n\n";
...
add_action(
    'admin_init',
    'monsterinsights_maybe_add_wp_php_version_notification'
);
...
function monsterinsights_year_in_review_notification()
{
    ...
    if (monsterinsights_date_is_between('2021-01-01', '2021-01-14')) {
        ...
        $notification['title'] = esc_html__(
            'View 2020 Year in Review report!',
            'google-analytics-for-wordpress'
        );
        $notification['content'] = esc_html__(
        ...
        $notification['btns'] = [
            'learn_more' => [
                'url' => esc_url(
                    admin_url(
                        'admin.php?page=monsterinsights_reports#/year-in-review'
                    )
                ),
                'text' => esc_html__(
                    'Learn More',
