...
class MonsterInsights_Notifications
{
    ...
    {
        add_action('wp_ajax_monsterinsights_notification_dismiss', [
            $this,
            'dismiss',
        ]);

        add_action('wp_ajax_monsterinsights_vue_get_notifications', [
            $this,
            'ajax_get_notifications',
        ]);

        add_action('monsterinsights_admin_notifications_update', [
            $this,
            'update',
        ]);
    }
    ...
    public function has_access()
    {
        ...
        return apply_filters(
            'monsterinsights_admin_notifications_has_access',
            $access
        );
    }
    ...
    public function verify($notifications)
    {
        ...
        foreach ($notifications as $notification) {
            ...
            if (
                ...
                time() < strtotime($notification['start'])
            ) {
                continue;
            }
        ...
        foreach ($notifications as $key => $notification) {
            if (
                ...
                (!empty($notification['end']) &&
                    time() > strtotime($notification['end'])) ||
                (!empty($notification['type']) &&
                    !in_array($license_type, $notification['type']))
            ) {
    ...
    public function get()
    {
        ...
        if (
            empty($option['update']) ||
            time() > $option['update'] + DAY_IN_SECONDS
        ) {
            if (
                false ===
                wp_next_scheduled('monsterinsights_admin_notifications_update')
            ) {
        ...
        $notifications[
            'active'
        ] = $this->get_notifications_with_human_readeable_start_time(
            $notifications['active']
        );
        $notifications[
            'active'
        ] = $this->get_notifications_with_formatted_content(
            $notifications['active']
        );
        $notifications['dismissed'] = !empty($option['dismissed'])
            ? $option['dismissed']
            : [];
        $notifications[
            'dismissed'
    ...
    public function get_notifications_with_formatted_content($notifications)
    {
        ...
        foreach ($notifications as $key => $notification) {
            if (!empty($notification['content'])) {
                $notifications[$key]['content'] = wpautop(
                    $notification['content']
                );
                $notifications[$key]['content'] = apply_filters(
                    'monsterinsights_notification_content_display',
                    $notifications[$key]['content']
                );
            }
        }
     ...
     * @since 7.12.3
     */
    public function get_notifications_with_human_readeable_start_time(
        $notifications
    ) {
        if (!is_array($notifications) || empty($notifications)) {
            return;
        ...
        foreach ($notifications as $key => $notification) {
            ...
            $modified_start_time = sprintf(
                __('%1$s ago', 'google-analytics-for-wordpress'),
                human_time_diff(
    ...
    public function get_active_notifications()
    {
        ...
        return isset($notifications['active']) ? $notifications['active'] : [];
    }
    ...
    public function get_dismissed_notifications()
    {
        ...
        return isset($notifications['dismissed'])
            ? $notifications['dismissed']
            : [];
    }
    ...
    {
        if ($this->get_count() > 0) {
            return '<span class="monsterinsights-menu-notification-indicator update-plugins">' .
                $this->get_count() .
                '</span>';
        }
    ...
    public function ajax_get_notifications()
    {
        ...
        $notifications_data = [
            ...
            'view_url' => $this->get_view_url(
                'monsterinsights-report-overview',
