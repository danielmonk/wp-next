...
add_action(
    'wp_ajax_monsterinsights_install_addon',
...
function monsterinsights_ajax_install_addon()
{
    ...
    if (isset($_POST['plugin'])) {
        ...
        if ($installer->plugin_info()) {
            ...
            echo json_encode(['plugin' => $plugin_basename]);
            wp_die();
        }
    }
...
add_action(
    'wp_ajax_monsterinsights_activate_addon',
    'monsterinsights_ajax_activate_addon'
);
/**
...
function monsterinsights_ajax_activate_addon()
{
    ...
    if (isset($_POST['plugin'])) {
        ...
        if (is_wp_error($activate)) {
            echo json_encode(['error' => $activate->get_error_message()]);
            wp_die();
        }
    }
...
add_action(
    'wp_ajax_monsterinsights_deactivate_addon',
...
function monsterinsights_ajax_deactivate_addon()
{
    ...
    if (!current_user_can('deactivate_plugins')) {
        wp_send_json([
            'error' => esc_html__(
                'You are not allowed to deactivate plugins',
                'google-analytics-for-wordpress'
            ),
        ]);
    }
...
add_action(
    'wp_ajax_monsterinsights_ajax_dismiss_notice',
...
add_action(
    'wp_ajax_monsterinsights_vue_dismiss_semrush_cta',
    'monsterinsights_ajax_dismiss_semrush_cta'
);
...
add_action(
    'wp_ajax_monsterinsights_get_sem_rush_cta_status',
    'monsterinsights_get_sem_rush_cta_status'
);
