...
class MonsterInsights_Install
{
    ...
    public function init()
    {
        ...
        if (!$version) {
            ...
            do_action(
                'monsterinsights_after_existing_upgrade_routine',
                $version
            );
    ...
    public function new_install()
    {
        ...
        do_action(
            'monsterinsights_after_new_install_routine',
            MONSTERINSIGHTS_VERSION
        );
    }
    ...
    public function get_monsterinsights_default_values()
    {
        ...
        return [
            ...
            'popular_posts_inline_post_types' => ['post'],
            'popular_posts_widget_post_types' => ['post'],
        ];
    }
    ...
    public function maybe_import_thirstyaffiliates_options()
    {
        ...
        if (!empty($link_prefix)) {
            ...
            $prefix_set = false;
            foreach (
                $this->new_settings['affiliate_links']
                as $affiliate_link
            ) {
    ...
    public function v700_upgrades()
    {
        ...
        if (
            empty($this->new_settings['events_mode']) ||
            $this->new_settings['events_mode'] !== 'none'
        ) {
        ...
        if (!empty($this->new_settings['manual_ua_code'])) {
            ...
            is_network_admin()
                ? update_site_option('monsterinsights_network_profile', [
                    'manual' => $this->new_settings['manual_ua_code'],
                ])
        ...
        if (!empty($this->new_settings['analytics_profile_code'])) {
            ...
            is_network_admin()
                ? update_site_option('monsterinsights_network_profile', [
                    'manual' => $this->new_settings['analytics_profile_code'],
                ])
                : update_option('monsterinsights_site_profile', [
                    'manual' => $this->new_settings['analytics_profile_code'],
                ]);
        ...
        if (is_multisite()) {
            ...
            if ($ms_license) {
                update_site_option(
                    'monsterinsights_network_license_updates',
                    get_site_option('monsterinsights_license_updates', '')
                );
                update_site_option(
    ...
    public function v740_upgrades()
    {
        ...
        if (!empty($this->new_settings['track_internal_as_outbound'])) {
            $affiliate_old_paths =
                $this->new_settings['track_internal_as_outbound'];
            $affiliate_old_label = isset(
                $this->new_settings['track_internal_as_label']
            )
                ? $this->new_settings['track_internal_as_label']
                : '';
        ...
        if (
            isset($this->new_settings['dashboards_disabled']) &&
            $this->new_settings['dashboards_disabled']
        ) {
    ...
    public function v750_upgrades()
    {
        ...
        $custom_code = isset($this->new_settings['custom_code'])
            ? $this->new_settings['custom_code']
            : '';
    ...
    {
        $cross_domains = isset($this->new_settings['cross_domains'])
            ? $this->new_settings['cross_domains']
            : [];

        if (!empty($cross_domains) && is_array($cross_domains)) {
            $current_domain = wp_parse_url(home_url());
            $current_domain = isset($current_domain['host'])
                ? $current_domain['host']
                : '';
    ...
    public function v780_upgrades()
    {
        if (monsterinsights_get_ua()) {
            ...
            monsterinsights_update_option(
                'monsterinsights_first_run_notice',
                true
            );

            // If they are already tracking when they upgrade, mark connected time as now.
            $over_time = get_option('monsterinsights_over_time', []);
    ...
    public function v790_upgrades()
    {
        ...
        if (monsterinsights_get_ua()) {
            update_option(
                'monsterinsights_frontend_tracking_notice_viewed',
                true
            );
    ...
    public function v7100_upgrades()
    {
        ...
        $current_downloads = isset($this->new_settings['extensions_of_files'])
            ? $this->new_settings['extensions_of_files']
            : [];

        if (!empty($current_downloads)) {
            $extensions_to_remove = ['exe', 'js', 'tgz'];
            $extensions_to_add = ['docx', 'pptx', 'xlsx'];
    ...
    public function v7110_upgrades()
    {
        if (empty($this->new_settings['email_summaries'])) {
            ...
            ] = $admin_email_array; // Not using wp_json_encode for backwards compatibility.
        }
    }
    ...
    public function v7130_upgrades()
    {
        ...
        $notifications = get_option($option_name, []);
        $dismissed_notifications = isset($notifications['dismissed'])
            ? $notifications['dismissed']
            : [];

        if (
            is_array($dismissed_notifications) &&
            !empty($dismissed_notifications)
        ) {
            foreach (
                $dismissed_notifications
                as $key => $dismiss_notification
            ) {
                $title = isset($dismiss_notification['title'])
                    ? $dismiss_notification['title']
                    : '';
                $content = isset($dismiss_notification['content'])
                    ? $dismiss_notification['content']
                    : '';
