...
final class MonsterInsights_Auth
{
    ...
    public function get_network_analytics_profile($force = false)
    {
        if (!empty($this->network) && !$force) {
            ...
            $profile = get_site_option('monsterinsights_network_profile', []);
            $this->network = $profile;
            return $profile;
        }
    }

    public function set_analytics_profile($data = [])
    {
    ...
    public function set_network_analytics_profile($data = [])
    {
    ...
    {
        return !empty($this->profile['ua'])
            ? monsterinsights_is_valid_ua($this->profile['ua'])
            : '';
    }

    public function get_network_ua()
    {
        return !empty($this->network['ua'])
            ? monsterinsights_is_valid_ua($this->network['ua'])
            : '';
    }

    public function get_viewname()
    {
        return !empty($this->profile['viewname'])
    ...
    {
        return !empty($this->network['viewname'])
            ? $this->network['viewname']
            : '';
    }
    ...
    public function get_referral_url()
    {
        ...
        if ($this->is_authed()) {
            $url .=
                ...
                '/';
        } elseif ($this->is_network_authed()) {
            $url .=
                'a' .
                MonsterInsights()->auth->get_network_accountid() .
                'w' .
                MonsterInsights()->auth->get_network_propertyid() .
                'p' .
