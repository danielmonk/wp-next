...
class MonsterInsights_Popular_Posts_Products_Sidebar extends WP_Widget
{
    ...
    {
        $checkboxes = ['meta_price', 'meta_rating', 'meta_image'];
    ...
    public function form($instance)
    {
        ...
        $title_font_sizes = apply_filters(
            'monsterinsights_popular_posts_products_title_sizes',
            range(10, 35)
        );
        $meta_font_sizes = apply_filters(
            'monsterinsights_popular_posts_products_meta_sizes',
            range(8, 24)
        );

        $this->text_input(
            'title',
            _x('Widget Title:', 'Widget', 'google-analytics-premium'),
            $instance
        );
        ?>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('display_title')); ?>">
				<?php echo esc_html(
        _x('Display Widget Title', 'Widget', 'google-analytics-premium')
    ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('post_count')); ?>">
				<?php echo esc_html(
        _x('Number of posts to display:', 'Widget', 'google-analytics-premium')
    ); ?>
			</label>
    ...
    $key
		...
		<div class="monsterinsights-widget-theme-preview">
			...
			<div class="monsterinsights-products-theme-preview-icon monsterinsights-products-theme-preview-icon-<?php echo esc_attr(
       $instance['theme']
   ); ?>"></div>
		</div>
		<?php
  if (!empty($theme['styles']['title']['color'])) {
      $this->color_input(
          'title_color',
          _x('Title Color:', 'Widget', 'google-analytics-premium'),
          $instance
      ); ?>
			<?php
  }
  if (!empty($theme['styles']['title']['size'])) {
      $this->size_input(
          'title_size',
          _x('Title Font Size:', 'Widget', 'google-analytics-premium'),
          $instance,
          $title_font_sizes
      );
  }
  if (!empty($theme['styles']['price']['color'])) {
      $this->color_input(
          'price_color',
          _x('Price Color:', 'Widget', 'google-analytics-premium'),
          $instance
      );
  }
  if (!empty($theme['styles']['price']['size'])) {
      $this->size_input(
          'price_size',
          _x('Price Font Size:', 'Widget', 'google-analytics-premium'),
          $instance,
          $meta_font_sizes
      );
  }
  if (!empty($theme['styles']['rating']['color'])) {
      $this->color_input(
          'rating_color',
          _x('Rating Color:', 'Widget', 'google-analytics-premium'),
          $instance
      );
  }
  if (!empty($theme['styles']['background']['border'])) {
      $this->color_input(
          'background_border',
          _x('Border Color:', 'Widget', 'google-analytics-premium'),
          $instance
      );
  }
  if (!empty($theme['styles']['background']['color'])) {
      $this->color_input(
          'background_color',
          _x('Background Color:', 'Widget', 'google-analytics-premium'),
          $instance
      );
  }
  ?>
		<p>
			...
			<label class="monsterinsights-label-block">
				<?php echo esc_html(
        _x(
            'Only Show Products from These Categories:',
            'Widget',
            'google-analytics-premium'
        )
    ); ?>
		...
		<p>
         ...
         $this->get_field_name('meta_price')
     ); ?>"
			...
			<?php if (!empty($theme['styles']['meta']['rating'])) { ?>
				<label class="monsterinsights-label-block">
					<input type="checkbox" name="<?php echo esc_attr(
         $this->get_field_name('meta_rating')
     ); ?>"
			...
			<?php if (!empty($theme['styles']['meta']['image'])) { ?>
				<label class="monsterinsights-label-block">
					<input type="checkbox" name="<?php echo esc_attr(
         $this->get_field_name('meta_image')
     ); ?>"
    ...
    esc_html_x('%dpx', 'google-analytics-premium'),
    $font_size
); ?></option>
				<?php } ?>
			</select>
