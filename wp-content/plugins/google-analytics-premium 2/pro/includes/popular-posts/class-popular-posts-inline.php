...
class MonsterInsights_Popular_Posts_Inline extends MonsterInsights_Popular_Posts
{
    ...
    public function get_rendered_html($atts)
    {
        ...
        if (
            'curated' === $this->sort &&
            apply_filters(
                'monsterinsights_popular_posts_inline_curated_shuffle',
                true
            )
        ) {
            // Randomize the order.
        ...
        $limit = !empty($theme_styles['posts'])
            ? intval($theme_styles['posts'])
            : 1;

        $label_text = '';
        if (isset($theme_styles['styles']['label'])) {
            $label_text = isset($atts['labelText'])
                ? $atts['labelText']
        ...
        if (!empty($theme_styles['styles']['border'])) {
            $html .=
                '<span class="monsterinsights-inline-popular-posts-border" ' .
                $this->get_element_style($theme, 'border', $atts, 'color') .
                '></span>';
        }
        if (!empty($theme_styles['styles']['border']['color2'])) {
            $html .=
                '<span class="monsterinsights-inline-popular-posts-border-2" ' .
                $this->get_element_style($theme, 'border', $atts, 'color2') .
                '></span>';
        }
        ...
        foreach ($posts as $post) {
            ...
            $html .=
                '<a class="monsterinsights-inline-popular-posts-title" ' .
    ...
    public function build_inline_styles()
    {
        ...
        foreach ($themes as $theme_key => $theme_styles) {
            if (!empty($theme_styles['background'])) {
                $styles .=
                    '.monsterinsights-inline-popular-posts.monsterinsights-popular-posts-styled.monsterinsights-inline-popular-posts-' .
                    $theme_key .
                    ' {';
            ...
            if (!empty($theme_styles['icon'])) {
                $styles .=
                    '.monsterinsights-inline-popular-posts.monsterinsights-popular-posts-styled.monsterinsights-inline-popular-posts-' .
                    $theme_key .
                    ' .monsterinsights-inline-popular-posts-icon path {';
    ...
    public function add_inline_posts_to_content($content)
    {
        ...
        if ($words_count > $after_count) {
            ...
            if (
                $multiple &&
                $words_count > $this->multiple_min_words &&
                $shown_count < $times_to_show
            ) {
                ...
                foreach ($words as $multiple_index => $word) {
                    ...
                    if ($multiple_count > $distance) {
                        ...
                        if (
                            ...
                            $multiple_count = 0;
                            $words[$multiple_index] = substr_replace(
                                $word,
                                $this->shortcode_output([]),
                                $p_index + 4,
                                0
