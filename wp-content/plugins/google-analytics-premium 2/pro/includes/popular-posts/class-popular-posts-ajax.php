...
class MonsterInsights_Popular_Posts_Ajax
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_monsterinsights_popular_posts_empty_cache', [
            $this,
            'empty_cache',
        ]);

        add_action('wp_ajax_monsterinsights_popular_posts_get_widget_output', [
            $this,
            'get_ajax_output',
        ]);
        add_action(
            'wp_ajax_nopriv_monsterinsights_popular_posts_get_widget_output',
            [$this, 'get_ajax_output']
        );

        add_action('wp_ajax_monsterinsights_get_popular_posts_themes', [
            $this,
            'ajax_get_themes',
        ]);
    }
    ...
    public function register_api_endpoints()
    {
        register_rest_route(
            'monsterinsights/v1',
            '/popular-posts/themes/(?P<type>[a-zA-Z0-9-]+)',
            [
                'methods' => 'GET',
                'callback' => [$this, 'get_gutenberg_themes'],
    ...
    {
        $themes_object = new MonsterInsights_Popular_Posts_Themes(
            $type,
            $theme
        );
    ...
    public function get_ajax_output()
    {
        ...
        foreach ($widgets_args as $args) {
            ...
            if (!empty($args['type'])) {
                ...
                $widget_function = function_exists(
                    'MonsterInsights_Popular_Posts_' . $type
                )
                    ? call_user_func('MonsterInsights_Popular_Posts_' . $type)
                    : false;
                if ($widget_function) {
