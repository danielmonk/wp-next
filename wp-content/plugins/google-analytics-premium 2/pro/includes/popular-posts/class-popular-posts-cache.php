...
class MonsterInsights_Popular_Posts_Cache
{
    ...
    public function get_cached_posts($args)
    {
        ...
        $cached_data = get_option($this->get_cache_key(), []);
        $args_key = $this->get_args_key($args); // Generate an unique key based on the instance settings.

        if (
            isset($cached_data[$args_key]) &&
            isset($cached_data[$args_key]['saved_at'])
        ) {
    ...
    {
        $cache_refresh_days = monsterinsights_get_option(
            'popular_posts_caching_refresh',
            7
        );

        if ('custom' === $cache_refresh_days) {
            $cache_refresh_days = monsterinsights_get_option(
                'popular_posts_caching_days',
                7
            );
        }
