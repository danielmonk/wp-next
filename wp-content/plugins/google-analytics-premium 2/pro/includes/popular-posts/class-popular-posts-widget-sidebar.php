...
class MonsterInsights_Popular_Posts_Widget_Sidebar extends WP_Widget
{
    ...
    public function __construct()
    {
        ...
        $this->conditional_options = [
            ...
            'background_border' => ['background', 'border'],
            'meta_color' => ['meta', 'color'],
            'meta_size' => ['meta', 'size'],
            'meta_author' => ['meta', 'author'],
            'meta_date' => ['meta', 'date'],
            'meta_comments' => ['meta', 'comments'],
            'comments_color' => ['comments', 'color'],
        ];
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        $new_instance['display_title'] = isset($new_instance['display_title'])
            ? wp_strip_all_tags($new_instance['display_title'])
            : '';
        $new_instance['post_count'] = absint($new_instance['post_count']);
        if (
            !empty($new_instance['categories']) &&
            is_array($new_instance['categories'])
        ) {
            array_walk($new_instance['categories'], 'absint');
        }

        // Theme-dependant options.
        $themes = new MonsterInsights_Popular_Posts_Themes(
            'widget',
            !empty($old_instance['theme']) ? $old_instance['theme'] : ''
        );
        $theme = $themes->get_theme();

        foreach ($this->conditional_options as $key => $obj) {
            $new_instance = $this->maybe_remove_option(
                !empty($theme['styles'][$obj[0]][$obj[1]]),
                $key,
                $new_instance
            );
        }
    ...
    {
        $checkboxes = ['meta_author', 'meta_date', 'meta_comments'];
    ...
    public function form($instance)
    {
        ...
        $theme_name = empty($instance['theme'])
            ? $this->defaults['theme']
            : $instance['theme'];
        $themes = new MonsterInsights_Popular_Posts_Themes(
            'widget',
        ...
        $title_font_sizes = apply_filters(
            'monsterinsights_popular_posts_widget_title_sizes',
            range(10, 35)
        );
        $meta_font_sizes = apply_filters(
            'monsterinsights_popular_posts_widget_meta_sizes',
            range(8, 24)
        );

        $this->text_input(
            'title',
            _x('Widget Title:', 'Widget', 'google-analytics-for-wordpress'),
            $instance
        );
        ?>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('display_title')); ?>">
				<?php echo esc_html(
        _x('Display Widget Title', 'Widget', 'google-analytics-for-wordpress')
    ); ?>
			</label>
		</p>
		<p>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('theme')); ?>">
				<?php echo esc_html(
        _x('Theme:', 'Widget', 'google-analytics-for-wordpress')
    ); ?>
			</label>
    ...
    $key
		...
		<div class="monsterinsights-widget-theme-preview">
			...
			<div class="monsterinsights-widget-theme-preview-icon monsterinsights-widget-theme-preview-icon-<?php echo esc_attr(
       $instance['theme']
   ); ?>"></div>
		</div>
		<?php
  if (!empty($theme['styles']['title']['color'])) {
      $this->color_input(
          'title_color',
          _x('Title Color:', 'Widget', 'google-analytics-for-wordpress'),
          $instance
      ); ?>
			<?php
  }
  if (!empty($theme['styles']['title']['size'])) {
      $this->size_input(
          'title_size',
          _x('Title Font Size:', 'Widget', 'google-analytics-for-wordpress'),
          $instance,
          $title_font_sizes
      );
  }
  if (!empty($theme['styles']['label']['color'])) {
      $this->color_input(
          'label_color',
          _x('Label Color:', 'Widget', 'google-analytics-for-wordpress'),
          $instance
      );
  }
  if (
      !empty($theme['styles']['label']['editable']) &&
      !empty($theme['styles']['label']['text'])
  ) {
      $this->text_input(
          'label_text',
          _x('Label Text:', 'Widget', 'google-analytics-for-wordpress'),
          $instance
      );
  }
  if (!empty($theme['styles']['meta']['color'])) {
      $this->color_input(
          'meta_color',
          _x('Meta Color:', 'Widget', 'google-analytics-for-wordpress'),
          $instance
      );
  }
  if (!empty($theme['styles']['meta']['size'])) {
      $this->size_input(
          'meta_size',
          _x('Meta Font Size:', 'Widget', 'google-analytics-for-wordpress'),
          $instance,
          $meta_font_sizes
      );
  }
  if (!empty($theme['styles']['background']['border'])) {
      $this->color_input(
          'background_border',
          _x('Border Color:', 'Widget', 'google-analytics-for-wordpress'),
          $instance
      );
  }
  if (!empty($theme['styles']['background']['color'])) {
      $this->color_input(
          'background_color',
          _x('Background Color:', 'Widget', 'google-analytics-for-wordpress'),
          $instance
      );
  }
  ...
  ?>
        ...
        _x(
			...
			<select name="<?php echo esc_attr($this->get_field_name('categories')); ?>[]"
				...
				<?php if (
        is_array($instance['categories']) &&
        !empty($instance['categories'])
    ) {
		...
		<p>
         ...
         $this->get_field_name('meta_author')
     ); ?>"
         ...
         $this->get_field_name('meta_date')
			...
			<?php if (!empty($theme['styles']['meta']['comments'])) { ?>
				<label class="monsterinsights-label-block">
					<input type="checkbox" name="<?php echo esc_attr(
         $this->get_field_name('meta_comments')
     ); ?>"
     ...
     */
    public function size_input($name, $label, $instance, $range = [])
    {
        ?>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id($name)); ?>">
			...
			<select id="<?php echo esc_attr($this->get_field_id($name)); ?>"
				...
				<?php foreach ($range as $font_size) { ?>
					<option value="<?php echo absint($font_size); ?>" <?php selected(
    $instance[$name],
    $font_size
); ?>><?php printf(
    esc_html_x('%dpx', 'google-analytics-for-wordpress'),
    $font_size
); ?></option>
				<?php } ?>
			</select>
    ...
    {
        add_action('admin_enqueue_scripts', [$this, 'load_widget_scripts']);
    }
    ...
    public function load_widget_scripts()
    {
        ...
        wp_enqueue_style(
            'monsterinsights-admin-widget-setting-styles',
            plugins_url(
                'assets/css/admin-widget-settings' . $suffix . '.css',
        ...
        wp_register_script(
            'monsterinsights-admin-widget-settings',
            plugins_url(
                'assets/js/admin-widget-settings' . $suffix . '.js',
                MONSTERINSIGHTS_PLUGIN_FILE
            ),
            ['jquery', 'wp-color-picker', 'monsterinsights-select2'],
            monsterinsights_get_asset_version(),
            true
        );
