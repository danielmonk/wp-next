...
class MonsterInsights_Popular_Posts_GA
{
    ...
    {
        add_action('monsterinsights_popular_posts_grab_ga_data', [
            $this,
            'try_popular_posts_processing',
        ]);
    }
    ...
    public function try_popular_posts_processing()
    {
        ...
        if (
            isset($report_data['error']) &&
            false === $report_data['error'] &&
            isset($report_data['data']['popularposts'])
        ) {
            ...
            foreach ($ga_popular_posts as $popular_post) {
                ...
                $slug = $popular_post['page'];
                $post_data = get_page_by_path($slug, OBJECT, ['post']); // Use array for post type so attachments are excluded.
    ...
    {
        $posts = apply_filters(
            'monsterinsights_popular_posts_ga_processed_before_cache',
            $posts
        );
        $expiration =
            strtotime(' Tomorrow 1am ') -
            get_option('gmt_offset', 0) * HOUR_IN_SECONDS -
            time();
        set_transient(
            'monsterinsights_popular_posts_ga_data',
            $posts,
            $expiration
        );
    }
    ...
    public function dispatch_report_fetching()
    {
        if (
            false ===
            wp_next_scheduled('monsterinsights_popular_posts_grab_ga_data')
        ) {
            wp_schedule_single_event(
                time(),
                'monsterinsights_popular_posts_grab_ga_data'
            );
        }
    }
}
