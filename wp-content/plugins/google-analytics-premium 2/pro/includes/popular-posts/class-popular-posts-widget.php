...
class MonsterInsights_Popular_Posts_Widget extends MonsterInsights_Popular_Posts
{
    ...
    public function get_rendered_html($atts)
    {
        ...
        if (
            'curated' === $this->sort &&
            apply_filters(
                'monsterinsights_popular_posts_widget_curated_shuffle',
                true
            )
        ) {
            // Randomize the order.
        ...
        if (isset($theme_styles['styles']['label'])) {
            $label_text = isset($atts['label_text'])
                ? $atts['label_text']
                : $theme_styles['styles']['label']['text'];
        }
        ...
        if ($show_title) {
            $html .=
                '<h2 class="monsterinsights-widget-popular-posts-widget-title">' .
        ...
        foreach ($posts as $post) {
            ...
            if (!empty($theme_styles['image']) && !empty($post['image'])) {
                $html .=
                    '<div class="monsterinsights-widget-popular-posts-image">';
                $html .=
                    '<img src="' .
                    $post['image'] .
                    '" srcset=" ' .
            ...
            if (isset($theme_styles['styles']['label'])) {
                $html .=
                    '<span class="monsterinsights-widget-popular-posts-label" ' .
                    $this->get_element_style($theme, 'label', $atts) .
                    '>' .
                    $label_text .
                    '</span>';
            }
            $html .=
                '<span class="monsterinsights-widget-popular-posts-title" ' .
                $this->get_element_style($theme, 'title', $atts) .
                '>' .
                $post['title'] .
                '</span>';
            $html .=
                '<div class="monsterinsights-widget-popular-posts-meta" ' .
                $this->get_element_style($theme, 'meta', $atts) .
                '>';
            if ($show_author) {
                // Translators: Placeholder for author name.
                $html .=
                    '<span class="monsterinsights-widget-popular-posts-author">' .
                    sprintf(
                        __('by %s', 'google-analytics-for-wordpress'),
                        $post['author_name']
                    ) .
                    '</span>';
            }
            if (
                !empty($theme_styles['styles']['meta']['separator']) &&
                $show_author &&
                $show_date
            ) {
                $html .=
                    "<span>" .
                    $theme_styles['styles']['meta']['separator'] .
                    "</span>";
            }
            if ($show_date) {
                $html .=
                    '<span class="monsterinsights-widget-popular-posts-date">' .
                    $post['date'] .
                    '</span>';
            }
            $html .= '</div>';
    ...
    public function build_inline_styles()
    {
        ...
        foreach ($themes as $theme_key => $theme_styles) {
            if (!empty($theme_styles['background'])) {
                $styles .=
                    '.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts.monsterinsights-widget-popular-posts-' .
                    $theme_key .
            ...
            if (!empty($theme_styles['meta'])) {
                $styles .=
                    '.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts.monsterinsights-widget-popular-posts-' .
                    $theme_key .
                    ' .monsterinsights-widget-popular-posts-list .monsterinsights-widget-popular-posts-meta span {';
                ...
                if (!empty($theme_styles['meta']['size'])) {
                    $styles .=
                        'font-size:' . $theme_styles['meta']['size'] . 'px;';
                }
            ...
            if (!empty($theme_styles['comments'])) {
                $styles .=
                    '.monsterinsights-widget-popular-posts.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts-' .
                    $theme_key .
                    ' .monsterinsights-widget-popular-posts-list .monsterinsights-widget-popular-posts-comments {';

                if (!empty($theme_styles['comments']['color'])) {
                    $styles .=
                        'color:' . $theme_styles['comments']['color'] . ';';
                }
                ...
                $styles .=
                    '.monsterinsights-widget-popular-posts.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts-' .
                    $theme_key .
                    ' .monsterinsights-widget-popular-posts-list .monsterinsights-widget-popular-posts-comments svg {';

                if (!empty($theme_styles['comments']['color'])) {
                    $styles .=
                        'fill:' . $theme_styles['comments']['color'] . ';';
                }
            ...
            if (!empty($theme_styles['border'])) {
                $styles .=
                    '.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts-' .
                    $theme_key .
            ...
            if (!empty($theme_styles['border']['color2'])) {
                ...
                $styles .=
                    'border-color:' . $theme_styles['border']['color2'] . ';';
    ...
    public function maybe_auto_insert()
    {
        ...
        if (
            ...
            add_filter('the_content', [$this, 'add_inline_posts_to_content']);
        }
    }
