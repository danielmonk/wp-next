 ...
 */
class MonsterInsights_Popular_Posts_Products extends
    MonsterInsights_Popular_Posts
{
    ...
    public function hooks()
    {
        ...
        add_action('wp', [$this, 'maybe_auto_insert']);
    ...
    public function get_rendered_html($atts)
    {
        ...
        if (!$show_image && !empty($theme_styles['styles']['meta']['image'])) {
            $show_image = isset($atts['meta_image'])
                ? 'on' === $atts['meta_image']
                : 'on' === $this->theme_meta_image;
        }

        if (isset($atts['widget_title'])) {
            $show_title = (bool) $atts['widget_title'];
            $title_text = empty($atts['widget_title_text'])
                ? ''
                : $atts['widget_title_text'];
        } else {
        ...
        if ($show_title) {
            $html .=
                '<h2 class="monsterinsights-products-popular-posts-title">' .
        ...
        foreach ($posts as $post) {
            ...
            $html .=
                '<li ' .
                $this->get_element_style($theme, 'background', $atts) .
                '>';
            $html .= '<a href="' . $post['link'] . '">';
            if ($show_image && !empty($post['image'])) {
                $html .=
                    '<div class="monsterinsights-products-popular-posts-image">';
                $html .=
                    '<img src="' .
                    $post['image'] .
                    '" srcset=" ' .
            ...
            $html .=
                '<div class="monsterinsights-products-popular-posts-text">';

            $html .=
                '<span class="monsterinsights-products-popular-posts-title" ' .
                $this->get_element_style($theme, 'title', $atts) .
                '>' .
                $post['title'] .
                '</span>';
            if ($show_price) {
                $html .=
                    '<span class="monsterinsights-products-popular-posts-price" ' .
                    $this->get_element_style($theme, 'price', $atts) .
                    '>' .
                    $post['price'] .
                    '</span>';
            }
            if ($show_rating) {
                $rating_color = !empty($atts['rating_color'])
                    ? $atts['rating_color']
                    : $theme_styles['styles']['rating']['color'];
                $html .=
                    '<span class="monsterinsights-products-popular-posts-rating">' .
                    $this->get_rating_stars($rating_color, $post['rating']) .
                    '</span>';
            }
    ...
    public function get_posts()
    {
        ...
        if (function_exists('wc_get_product')) {
            // Just being extra careful not to trigger a fatal error.
    ...
    public function build_inline_styles()
    {
        ...
        foreach ($themes as $theme_key => $theme_styles) {
            if (!empty($theme_styles['background'])) {
                $styles .=
                    '.monsterinsights-popular-posts-styled.monsterinsights-products-popular-posts.monsterinsights-products-popular-posts-' .
                    $theme_key .
                    ' .monsterinsights-products-popular-posts-list li {';

                if (!empty($theme_styles['background']['color'])) {
                    $styles .=
                        'background-color:' .
                        $theme_styles['background']['color'] .
                        ';';
                }
                if (!empty($theme_styles['background']['border'])) {
                    $styles .=
                        'border-color:' .
                        $theme_styles['background']['border'] .
                        ';';
                }
            ...
            if (!empty($theme_styles['title'])) {
                $styles .=
                    '.monsterinsights-popular-posts-styled.monsterinsights-products-popular-posts.monsterinsights-products-popular-posts-' .
                    $theme_key .
                    ' .monsterinsights-products-popular-posts-list li .monsterinsights-products-popular-posts-title {';
                ...
                if (!empty($theme_styles['title']['size'])) {
                    $styles .=
                        'font-size:' . $theme_styles['title']['size'] . 'px;';
                }
            ...
            if (!empty($theme_styles['price'])) {
                $styles .=
                    '.monsterinsights-popular-posts-styled.monsterinsights-products-popular-posts.monsterinsights-products-popular-posts-' .
                    $theme_key .
                    ' .monsterinsights-products-popular-posts-list li .monsterinsights-products-popular-posts-price {';
                ...
                if (!empty($theme_styles['price']['size'])) {
                    $styles .=
                        'font-size:' . $theme_styles['price']['size'] . 'px;';
                }
