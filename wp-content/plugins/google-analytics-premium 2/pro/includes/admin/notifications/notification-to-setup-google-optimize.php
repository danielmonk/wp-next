 ...
 * @since 7.12.3
 */
final class MonsterInsights_Notification_To_Setup_Google_Optimize extends
    MonsterInsights_Notification_Event
{
    ...
    {
        $goptimize_container = monsterinsights_get_option(
            'goptimize_container',
            ''
        );
        ...
        if (empty($goptimize_container)) {
            $settings_url = is_network_admin()
                ? $this->get_view_url(
                    'monsterinsights-settings-block-google-optimize',
