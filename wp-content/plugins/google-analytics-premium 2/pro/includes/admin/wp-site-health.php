...
class MonsterInsights_WP_Site_Health
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_health-check-monsterinsights-test_connection', [
            $this,
            'test_check_connection',
        ]);

        add_action('wp_ajax_health-check-monsterinsights-test_tracking_code', [
            $this,
    ...
    {
        $tests['direct']['monsterinsights_license'] = [
            'label' => __('MonsterInsights License', 'ga-premium'),
            'test' => [$this, 'test_check_license'],
        ];

        $tests['direct']['monsterinsights_auth'] = [
            'label' => __('MonsterInsights Authentication', 'ga-premium'),
            'test' => [$this, 'test_check_authentication'],
        ];

        if ($this->is_licensed()) {
            $tests['direct']['monsterinsights_automatic_updates'] = [
                'label' => __(
                    'MonsterInsights Automatic Updates',
                    'ga-premium'
                ),
                'test' => [$this, 'test_check_autoupdates'],
            ];
        }

        if ($this->is_ecommerce()) {
            $tests['direct']['monsterinsights_ecommerce'] = [
                'label' => __('MonsterInsights eCommerce', 'ga-premium'),
                'test' => [$this, 'test_check_ecommerce'],
            ];

            $tests['direct']['monsterinsights_excluded_roles'] = [
                'label' => __('MonsterInsights Tracking', 'ga-premium'),
                'test' => [$this, 'test_check_roles_excluded'],
            ];
        }

        if (class_exists('MonsterInsights_Dimensions')) {
            $tests['direct']['monsterinsights_dimensions'] = [
                'label' => __(
                    'MonsterInsights Custom Dimensions',
                    'ga-premium'
                ),
                'test' => [$this, 'test_check_dimensions'],
            ];
        }

        if ($this->uses_amp()) {
            $tests['direct']['monsterinsights_amp'] = [
                'label' => __('MonsterInsights AMP', 'ga-premium'),
                'test' => [$this, 'test_check_amp'],
            ];
        }

        if ($this->uses_fbia()) {
            $tests['direct']['monsterinsights_fbia'] = [
                'label' => __('MonsterInsights FBIA', 'ga-premium'),
                'test' => [$this, 'test_check_fbia'],
            ];
        }

        $tests['async']['monsterinsights_connection'] = [
            'label' => __('MonsterInsights Connection', 'ga-premium'),
            'test' => 'monsterinsights_test_connection',
        ];

        if ($this->is_tracking()) {
            $tests['async']['monsterinsights_tracking_code'] = [
    ...
    public function is_licensed()
    {
        if (!isset($this->is_licensed)) {
            $this->is_licensed = is_network_admin()
    ...
    {
        return class_exists('MonsterInsights_FB_Instant_Articles') ||
            (defined('IA_PLUGIN_VERSION') &&
                version_compare(IA_PLUGIN_VERSION, '3.3.4', '>'));
    }
    ...
    private function is_coming_soon_active()
    {
        if (defined('SEED_CSP4_SHORTNAME')) {
            ...
            return isset($settings['general']['status']) &&
                1 === $settings['general']['status'];
            ...
            return isset($settings['sahu_so_status']) &&
                '1' === $settings['sahu_so_status'];
    ...
    public function test_check_authentication()
    {
        ...
        if (!$this->is_authed) {
            if ('' !== monsterinsights_get_ua()) {
                ...
                $result['label'] = __(
                    'You are using Manual UA code output',
                    'ga-premium'
                );
    ...
    public function test_check_license()
    {
        ...
        if ($this->is_licensed()) {
            $license_type = is_network_admin()
                ? MonsterInsights()->license->get_network_license_type()
                : MonsterInsights()->license->get_license_type();
            $result['label'] = __(
                'Your MonsterInsights license is valid',
                'ga-premium'
            );
            /* Translators: The license type. */
            $result['description'] = sprintf(
                __('Your license key type for this site is %s.', 'ga-premium'),
                '<strong>' . ucfirst($license_type) . '</strong>'
            );
        } else {
            $result['label'] = __(
                'MonsterInsights is not licensed',
                'ga-premium'
            );
            $result['status'] = 'critical';
            /* Translators: The license type. */
            $result['description'] = __(
                'MonsterInsights is not licensed which means you can\'t access reports, automatic updates, and other advanced features',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
    ...
    public function test_check_autoupdates()
    {
        $result = [
            ...
            'description' => __(
                'MonsterInsights automatic updates are enabled and you are getting the latest features, bugfixes, and security updates as they are released.',
                'ga-premium'
            ),
            'test' => 'monsterinsights_automatic_updates',
        ];

        $updates_option = monsterinsights_get_option(
            'automatic_updates',
            false
        );
        ...
        if ('none' === $updates_option) {
            ...
            $result['description'] = __(
                'MonsterInsights automatic updates are disabled. We recommend enabling automatic updates so you can get access to the latest features, bugfixes, and security updates as they are released.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
                add_query_arg(
                    'page',
                    'monsterinsights_settings#/advanced',
                    admin_url('admin.php')
                ),
    ...
    public function test_check_ecommerce()
    {
        $result = [
            'label' => __(
                'You are successfully tracking eCommerce data',
                'ga-premium'
            ),
            'status' => 'good',
            'badge' => [
            ...
            'description' => sprintf(
                __(
                    'MonsterInsights eCommerce addon is properly set up and tracking your %s store',
                    'ga-premium'
                ),
                $this->ecommerce
            ),
        ...
        if (empty($ua)) {
            ...
            $result['description'] = __(
                'Please connect MonsterInsights to Google Analytics to start tracking eCommerce data.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
        ...
        } elseif (!class_exists('MonsterInsights_eCommerce')) {
            $result['status'] = 'recommended';
            $result['label'] = __(
                'eCommerce data is not being tracked',
                'ga-premium'
            ...
            $result['description'] = sprintf(
                __(
                    'You are using %s but the MonsterInsights eCommerce addon is not active, please Install & Activate it to start tracking eCommerce data.',
                    'ga-premium'
                ),
                $this->ecommerce
            );
            ...
            if (in_array($type, ['basic', 'plus'], true)) {
                $result['actions'] = sprintf(
                    '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
                    monsterinsights_get_upgrade_link(
                        'site-health',
                        'ecommerce',
                        'https://monsterinsights.com/my-account'
                    ),
                    __('Upgrade Now', 'ga-premium')
                );
            }
        } elseif (!monsterinsights_get_option('enhanced_ecommerce')) {
            $result['status'] = 'recommended';
            $result['label'] = __(
                'Enhanced eCommerce data is not tracked',
                'ga-premium'
            );
            // Translators: The eCommerce store currently active.
            $result['description'] = sprintf(
                __(
                    'Enhanced eCommerce is available but not enabled in your MonsterInsights settings.',
                    'ga-premium'
                ),
                $this->ecommerce
            );
            $result['actions'] = sprintf(
                ...
                add_query_arg(
                    'page',
                    'monsterinsights_settings#/ecommerce',
                    admin_url('admin.php')
                ),
    ...
    {
        $custom_dimensions = monsterinsights_get_option(
            'custom_dimensions',
            []
        );
        ...
        $result = [
            'label' => __(
                'Custom Dimensions are set up correctly',
                'ga-premium'
            ),
            'status' => 'good',
            'badge' => [
            ...
            'description' => sprintf(
                __(
                    'MonsterInsights Custom Dimensions addon is properly set up and tracking your dimensions',
                    'ga-premium'
                ),
                count($custom_dimensions)
            ),
        ...
        if (empty($custom_dimensions)) {
            $result['status'] = 'recommended';
            $result['label'] = __(
                'No Custom Dimensions are being tracked',
                'ga-premium'
            );
            $result['description'] = __(
                'The MonsterInsights Custom Dimensions addon is installed & activated but no Custom Dimensions have been configured for tracking.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
                add_query_arg(
                    'page',
                    'monsterinsights_settings#/conversions',
                    admin_url('admin.php')
                ),
    ...
    public function test_check_amp()
    {
        $result = [
            ...
            'description' => __(
                'The MonsterInsights AMP addon is active and AMP pages are being tracked.',
                'ga-premium'
            ),
        ...
        if (defined('AMP__FILE__') && !class_exists('MonsterInsights_AMP')) {
            $result['status'] = 'recommended';
            $result['label'] = __(
                'AMP pages are not being tracked',
                'ga-premium'
            );
            $result['description'] = __(
                'Your website has Google AMP-enabled pages set up but they are not tracked by Google Analytics at the moment. You need to Install & Activate the MonsterInsights AMP Addon.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
            ...
            class_exists('MonsterInsights_AMP') &&
            !defined('AMP__FILE__')
        ) {
            $plugins = get_plugins();
            ...
            $result['description'] = __(
                'The MonsterInsights AMP tracking addon is enabled but the official Google-developed AMP plugin for WordPress is not active.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
    ...
    public function test_check_fbia()
    {
        $result = [
            'label' => __(
                'Facebook Instant Articles tracking is set up correctly',
                'ga-premium'
            ),
            'status' => 'good',
            'badge' => [
            ...
            'description' => __(
                'The MonsterInsights Facebook Instant Articles addon is active and Instant Articles pages are being tracked.',
                'ga-premium'
            ),
        ...
        if (
            defined('IA_PLUGIN_VERSION') &&
            version_compare(IA_PLUGIN_VERSION, '3.3.4', '>') &&
            !class_exists('MonsterInsights_FB_Instant_Articles')
        ) {
            $result['status'] = 'recommended';
            $result['label'] = __(
                'Facebook Instant Articles pages are not being tracked',
                'ga-premium'
            );
            $result['description'] = __(
                'Your website has Facebook Instant Articles pages set up but they are not tracked by Google Analytics at the moment. You need to Install & Activate the MonsterInsights Facebook Instant Articles Addon.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
            ...
            $result['status'] = 'critical';
            $result['label'] = __(
                'Facebook Instant Articles plugin is not active',
                'ga-premium'
            );
            $result['description'] = __(
                'The MonsterInsights Facebook Instant Articles tracking addon is enabled but the Instant Articles for WP plugin by Automattic version 3.3.5 or newer is not active.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
            ...
            $result['status'] = 'critical';
            $result['label'] = __(
                'Facebook Instant Articles plugin is outdated',
                'ga-premium'
            );
            $result['description'] = __(
                'The MonsterInsights Facebook Instant Articles tracking addon is enabled but you are using an older, unsupported version of Instant Articles for WP plugin by Automattic.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
    ...
    public function test_check_connection()
    {
        $result = [
            ...
            'description' => __(
                'The MonsterInsights API is reachable and no connection issues have been detected.',
                'ga-premium'
            ),
        ...
        if (
            ...
            $result['description'] = __(
                'Your server is blocking external requests to monsterinsights.com, please check your firewall settings or contact your host for more details.',
                'ga-premium'
            );
    ...
    public function test_check_tracking_code()
    {
        $result = [
            'label' => __(
                'Tracking code is properly being output.',
                'ga-premium'
            ),
            'status' => 'good',
            'badge' => [
            ...
            'description' => __(
                'The Google Analytics tracking code is being output correctly, and no duplicate Google Analytics scripts have been detected.',
                'ga-premium'
            ),
        ...
        if (!empty($errors) && is_array($errors) && !empty($errors[0])) {
            if ($this->is_coming_soon_active()) {
                $result['status'] = 'good';
                $result['label'] = __(
                    'Tracking code disabled: coming soon/maintenance mode plugin present',
                    'ga-premium'
                );
                $result['description'] = __(
                    'MonsterInsights has detected that you have a coming soon or maintenance mode plugin currently activated on your site. This plugin does not allow other plugins (like MonsterInsights) to output Javascript, and thus MonsterInsights is not currently tracking your users (expected). Once the coming soon/maintenance mode plugin is deactivated, tracking will resume automatically.',
                    'ga-premium'
                );
            } else {
                $result['status'] = 'critical';
    ...
    public function test_check_roles_excluded()
    {
        ...
        $mi_roles = monsterinsights_get_option('ignore_users', []);
        $wp_roles = monsterinsights_get_roles();

        if (count($mi_roles) === count($wp_roles)) {
            $result['label'] = __(
                'MonsterInsights is not tracking any logged-in users',
                'ga-premium'
            );
            $result['status'] = 'recommended';
            $result['description'] = __(
                'All the available user roles are excluded from tracking in your MonsterInsights settings. This means none of the logged-in users are being tracked.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
            ...
            $result['label'] = __(
                'MonsterInsights is tracking all logged-in users',
                'ga-premium'
            );
            $result['status'] = 'recommended';
            $result['description'] = __(
                'No user roles are excluded from tracking in your MonsterInsights settings. This means your stats might be inflated by traffic from Administrators or Editors.',
                'ga-premium'
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>',
            ...
            $result['label'] = __(
                'MonsterInsights is properly excluding some logged-in users',
                'ga-premium'
            );
            $result['status'] = 'good';
            $result['description'] = __(
                'Some user roles are excluded from tracking in your MonsterInsights settings.',
                'ga-premium'
            );
        }
