...
final class MonsterInsights_License_Actions
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_monsterinsights_verify_license', [
            $this,
            'ajax_verify_key',
        ]);

        add_action('wp_ajax_monsterinsights_deactivate_license', [
            $this,
            'ajax_deactivate_key',
        ]);

        add_action('wp_ajax_monsterinsights_validate_license', [
            $this,
            'ajax_validate_key',
        ]);
    }
    ...
    public function admin_init()
    {
        ...
        add_action('admin_notices', [$this, 'monsterinsights_notices'], 11);
        add_action(
            'network_admin_notices',
            [$this, 'monsterinsights_notices'],
            11
        );

        // Grab the license key. If it is not set (even after verification), return early.
        $this->key = is_network_admin()
    ...
    {
        if (
            empty($_POST['monsterinsights-license-key']) ||
            strlen($_POST['monsterinsights-license-key']) < 10 ||
            empty($_POST['monsterinsights-verify-submit'])
        ) {
            return;
        }
        ...
        if (
            !wp_verify_nonce(
                $_POST['monsterinsights-key-nonce'],
                'monsterinsights-key-nonce'
            )
        ) {
            return;
        }
    ...
    public function ajax_verify_key()
    {
        ...
        $license = !empty($_POST['license'])
            ? trim(sanitize_text_field(wp_unslash($_POST['license'])))
            : false;
        ...
        if (!current_user_can('monsterinsights_save_settings')) {
            wp_send_json_error([
                'error' => esc_html__(
                    'You are not allowed to verify a license key.',
                    'google-analytics-for-wordpress'
                ),
            ]);
        }
        ...
        $forced =
            isset($_POST['forced']) &&
            'true' === sanitize_text_field(wp_unslash($_POST['forced']));

        $verify = $this->perform_remote_request('verify-key', [
            'tgm-updater-key' => $license,
        ]);
        ...
        $option = $network_admin
            ? MonsterInsights()->license->get_network_license()
            : MonsterInsights()->license->get_site_license();
        $option['key'] = $license;
        $option['type'] = isset($verify->type)
            ? $verify->type
            : $option['type'];
        ...
        $network_admin
            ? MonsterInsights()->license->set_network_license($option)
            : MonsterInsights()->license->set_site_license($option);
        delete_transient('_monsterinsights_addons');
        monsterinsights_get_addons_data($option['key']);

        $message = isset($verify->success)
            ? $verify->success
            : esc_html__(
    ...
    public function verify_key($key = '')
    {
        ...
        $option['is_disabled'] = false;
        $option['is_invalid'] = false;
        $this->success[] = isset($verify->success)
            ? $verify->success
            : esc_html__(
                'Congratulations! This site is now receiving automatic updates.',
                'google-analytics-for-wordpress'
    ...
    {
        $check = is_network_admin()
            ? MonsterInsights()->license->time_to_check_network_license()
            : MonsterInsights()->license->time_to_check_site_license();
        if ($check) {
            $this->validate_key();
        }
    }
    ...
    public function validate_key($forced = false, $network = null)
    {
        ...
        $validate = $this->perform_remote_request('validate-key', [
        ...
        if ((!empty($validate->key) || !empty($this->key)) && $forced) {
            ...
            $this->success[] = esc_html__(
                'Congratulations! Your key has been refreshed successfully.',
                'google-analytics-for-wordpress'
            );
        }
        ...
        $option = $network
            ? MonsterInsights()->license->get_network_license()
            : MonsterInsights()->license->get_site_license();
        $option['type'] = isset($validate->type)
            ? $validate->type
            : $option['type'];
        $option['is_expired'] = false;
        $option['is_disabled'] = false;
    ...
    public function maybe_deactivate_key()
    {
        if (
            empty($_POST['monsterinsights-license-key']) ||
            empty($_POST['monsterinsights-deactivate-submit'])
        ) {
            return;
        }
    ...
    public function ajax_deactivate_key()
    {
        ...
        if (!current_user_can('monsterinsights_save_settings')) {
            wp_send_json_error([
                'error' => esc_html__(
                    'You are not allowed to deactivate the license key.',
                    'google-analytics-for-wordpress'
                ),
            ]);
        }

        $deactivate = $this->perform_remote_request('deactivate-key', [
            'tgm-updater-key' => $license,
        ]);
        ...
        $network_admin
            ? MonsterInsights()->license->delete_network_license()
            : MonsterInsights()->license->delete_site_license();

        wp_send_json_success([
            'message' => isset($deactivate->success)
                ? $deactivate->success
                : esc_html__(
    ...
    public function deactivate_key()
    {
        ...
        $deactivate = $this->perform_remote_request('deactivate-key', [
            'tgm-updater-key' => $_POST['monsterinsights-license-key'],
        ]);
        ...
        $this->success[] = isset($deactivate->success)
            ? $deactivate->success
            : esc_html__(
                'Congratulations! You have deactivated the key from this site successfully.',
                'google-analytics-for-wordpress'
    ...
    public function maybe_refresh_key()
    {
        if (
            empty($_POST['monsterinsights-license-key']) ||
            empty($_POST['monsterinsights-refresh-submit'])
        ) {
            return;
        }
     ...
     */
    public function perform_remote_request(
        $action,
        $body = [],
        $headers = [],
        $return_format = 'json'
