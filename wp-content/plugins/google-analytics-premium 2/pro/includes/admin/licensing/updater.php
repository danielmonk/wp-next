...
class MonsterInsights_Updater
{
    ...
    public function __construct(array $config)
    {
        ...
        add_action(
            'in_plugin_update_message-' . $this->plugin_path,
            [$this, 'maybe_show_license_expired_message'],
            10,
            2
        );
    }
    ...
    public function maybe_show_license_expired_message($plugin_data, $response)
    {
        ...
        if (empty($response->package)) {
            ...
            echo '<br />' .
                sprintf(
                    __(
                        'In order to enable updates, you need to have a valid license key on the %1$ssettings page%2$s. If your license key is expired or you need a new key, then %3$sclick here to purchase MonsterInsights Pro%2$s.',
    ...
    public function update_plugins_filter($value)
    {
        ...
        if (!$this->update) {
            $this->update = $this->perform_remote_request('get-plugin-update', [
        ...
        if (
            isset($this->update->new_version) &&
            version_compare($this->version, $this->update->new_version, '<')
        ) {
    ...
    public function http_request_args($args, $url)
    {
        ...
        if (
            ...
            strpos($url, 'tgm-updater-action=get-plugin-update')
        ) {
    ...
    public function set_plugins_api($default_api)
    {
        ...
        if (!$this->info) {
            $this->info = $this->perform_remote_request('get-plugin-info', [
                'tgm-updater-plugin' => $this->plugin_slug,
            ]);
        ...
        $api->author_profile = isset($this->info->author_profile)
            ? $this->info->author_profile
            : '';
        $api->requires = isset($this->info->requires)
            ? $this->info->requires
            : '';
        $api->tested = isset($this->info->tested) ? $this->info->tested : '';
        $api->last_updated = isset($this->info->last_updated)
            ? $this->info->last_updated
            : '';
        $api->homepage = isset($this->info->homepage)
            ? $this->info->homepage
            : '';

        $changelog = isset($this->info->changelog)
            ? $this->info->changelog
            : '';
        $description = isset($this->info->description)
        ...
        $api->download_link = isset($this->info->download_link)
    ...
    public function premium_update_push($premium_update)
    {
        ...
        $update = $this->set_plugins_api([]);
        if (
            !empty($update) &&
            version_compare(MONSTERINSIGHTS_VERSION, $update->version, '<')
        ) {
