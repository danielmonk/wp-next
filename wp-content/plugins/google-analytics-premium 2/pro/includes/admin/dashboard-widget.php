...
class MonsterInsights_Dashboard_Widget_Pro
{
    ...
    public function __construct()
    {
        ...
        $show_widget = is_multisite()
            ? apply_filters('monsterinsights_show_dashboard_widget', true)
            : true;
        if (!$show_widget) {
            return false;
        }
        ...
        add_action('admin_enqueue_scripts', [$this, 'widget_scripts'], 1000);

        add_action('wp_ajax_monsterinsights_save_widget_state', [
            $this,
            'save_widget_state',
        ]);

        // Reminder notice.
        add_action('admin_footer', [$this, 'load_notice']);

        add_action('wp_ajax_monsterinsights_mark_notice_closed', [
            $this,
            'mark_notice_closed',
        ]);
    }
    ...
    public function register_dashboard_widget()
    {
        ...
        $widget_instance = [
            self::WIDGET_KEY => $normal_dashboard[self::WIDGET_KEY],
        ];
    ...
    public function dashboard_widget_content()
    {
        ...
        if (!$is_authed || $not_licensed) {
            ...
            monsterinsights_settings_error_page(
    ...
    public function widget_scripts()
    {
        ...
        if (isset($screen->id) && 'dashboard' === $screen->id) {
            ...
            if (!defined('MONSTERINSIGHTS_LOCAL_WIDGET_JS_URL')) {
                ...
                wp_enqueue_style(
                    'monsterinsights-vue-widget-style',
                    plugins_url(
                        $version_path .
                            '/assets/vue/css/widget' .
                            $rtl .
                            '.css',
                        MONSTERINSIGHTS_PLUGIN_FILE
                    ),
            ...
            $widget_js_url =
                defined('MONSTERINSIGHTS_LOCAL_WIDGET_JS_URL') &&
                MONSTERINSIGHTS_LOCAL_WIDGET_JS_URL
                    ? MONSTERINSIGHTS_LOCAL_WIDGET_JS_URL
                    : plugins_url(
                        $version_path . '/assets/vue/js/widget.js',
                        MONSTERINSIGHTS_PLUGIN_FILE
                    );
            wp_register_script(
            ...
            if (monsterinsights_can_install_plugins()) {
                ...
                if (array_key_exists($wpforms_key, $plugins)) {
                    $wp_forms_url = wp_nonce_url(
                        self_admin_url(
                            'plugins.php?action=activate&plugin=' . $wpforms_key
                        ),
                        'activate-plugin_' . $wpforms_key
                    );
                    $wpforms_installed = true;
                } else {
                    $wp_forms_url = wp_nonce_url(
                        self_admin_url(
                            'update.php?action=install-plugin&plugin=wpforms-lite'
                        ),
            ...
            wp_localize_script(
                ...
                [
                    ...
                    'addons_url' => is_multisite()
                        ...
                        : admin_url(
                            'admin.php?page=monsterinsights_settings#/addons'
                        ),
                    ...
                    'install_nonce' => wp_create_nonce(
                        'monsterinsights-install'
                    ),
                    'activate_nonce' => wp_create_nonce(
                        'monsterinsights-activate'
                    ),
                    'getting_started_url' => is_multisite()
                        ? network_admin_url(
                            'admin.php?page=monsterinsights_network#/about/getting-started'
                        )
                        : admin_url(
                            'admin.php?page=monsterinsights_settings#/about/getting-started'
                        ),
                    'wizard_url' => admin_url(
                        'index.php?page=monsterinsights-onboarding'
                    ),
                    'timezone' => date('e'),
                ]
            );
    ...
    public function save_widget_state()
    {
        ...
        if (isset($_POST['reports'])) {
            $reports = json_decode(
                sanitize_text_field(wp_unslash($_POST['reports'])),
                true
            );
        }

        $options = [
            'width' => !empty($_POST['width'])
                ? sanitize_text_field(wp_unslash($_POST['width']))
                : $default['width'],
            'interval' => !empty($_POST['interval'])
                ? sanitize_text_field(wp_unslash($_POST['interval']))
                : $default['interval'],
            'compact' => !empty($_POST['compact'])
                ? 'true' === sanitize_text_field(wp_unslash($_POST['compact']))
                : $default['compact'],
            'reports' => $reports,
        ...
        update_user_meta(
            get_current_user_id(),
            'monsterinsights_user_preferences',
            $options
        );
    ...
    public function get_options()
    {
        if (!isset($this->options)) {
            $this->options = self::wp_parse_args_recursive(
                get_user_meta(
                    get_current_user_id(),
                    'monsterinsights_user_preferences',
                    true
        ...
        return apply_filters(
            'monsterinsights_dashboard_widget_options',
            $this->options
        );
    }
    ...
    public function widget_content_no_auth()
    {
        ...
        'ga-premium'
			...
			<a href="<?php echo esc_url($url); ?>" class="mi-dw-btn-large"><?php esc_html_e(
    'Setup Website Analytics',
    'ga-premium'
); ?></a>
			<?php } else { ?>
				<p><?php esc_html_e(
        'To see your website stats, please ask your webmaster to connect MonsterInsights to Google Analytics.',
        'ga-premium'
    ); ?></p>
			<?php } ?>
		</div>
