...
final class MonsterInsights_Report_Forms extends MonsterInsights_Report
{
    ...
    public function requirements($error = false, $args = [], $name = '')
    {
        ...
        if (!class_exists('MonsterInsights_Forms')) {
            ...
            $text = __(
                'Please %s the MonsterInsights Forms addon to view custom dimensions reports.',
                'ga-premium'
            );
        ...
        if (!empty($data['data']['forms'])) {
            foreach ($data['data']['forms'] as $key => $formdata) {
                ...
                if (function_exists('wpforms')) {
                    if (
                        0 === strpos($title, 'wpforms-submit-') ||
                        0 === strpos($title, 'wpforms-form-')
                    ) {
        ...
        if (!empty($data['data'])) {
            $data['data']['galinks'] = [
                'forms' =>
                    ...
                    '%3Fexplorer-table.plotKeys%3D%5B%5D%26_r.drilldown%3Danalytics.eventCategory%3Aform/',
            ];
        }
