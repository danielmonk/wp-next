...
final class MonsterInsights_Report_Dimensions extends MonsterInsights_Report
{
    ...
    public function requirements($error = false, $args = [], $name = '')
    {
        ...
        if (!class_exists('MonsterInsights_Admin_Custom_Dimensions')) {
            add_filter('monsterinsights_reports_handle_error_message', [
                $this,
                'add_error_addon_link',
            ]);

            // Translators: %s will be the action (install/activate) which will be filled depending on the addon state.
            $text = __(
                'Please %s the MonsterInsights Dimensions addon to view custom dimensions reports.',
                'ga-premium'
            );
        ...
        if (empty($dimensions) || !is_array($dimensions)) {
            ...
            return __(
                'Please enable at least 1 dimension to use this report',
                'ga-premium'
            );
        }
    ...
    public function get_active_enabled_custom_dimensions()
    {
        ...
        $custom_dimension_option = monsterinsights_get_option(
            'custom_dimensions',
            []
        );
        ...
        foreach ($custom_dimension_option as $dimension) {
            $dimensions[$dimension['id']] = [
                ...
                'key' => $dimension['type'],
                'name' =>
                    $default_custom_dimensions[$dimension['type']]['title'],
                'label' =>
                    $default_custom_dimensions[$dimension['type']]['label'],
                'enabled' =>
                    $default_custom_dimensions[$dimension['type']]['enabled'],
                'metric' =>
                    $default_custom_dimensions[$dimension['type']]['metric'],
            ];
        }
    ...
    public function prepare_report_data($data)
    {
        ...
        if (class_exists('MonsterInsights_Admin_Custom_Dimensions')) {
            $custom_dimensions_option = monsterinsights_get_option(
                'custom_dimensions',
                []
            );
            ...
            foreach ($custom_dimensions_option as $dimension) {
                if (
                    isset($dimension['type']) &&
                    'user_id' === $dimension['type']
                ) {
            ...
            foreach ($data['data'] as $key => $dimension_data) {
                if (intval($key) > 0) {
                    foreach (
                        $data['data'][$key]['data']
                        as $dimension_key => $dimension_info
                    ) {
                        ...
                        if (
                            ...
                            $title = date(
                                'l, F jS, Y \a\t g:ia ',
                                strtotime($title, current_time('timestamp'))
                            );
                        }
                        ...
                        if (intval($user_id_dimension_index) === intval($key)) {
                            ...
                            $username = isset($user->user_login)
                                ? $user->user_login
                                : $user_id;
                            $title = apply_filters(
                                'monsterinsights_dimensions_user_id_display',
                                $username,
                                $user_id
                            );
                        }

                        $data['data'][$key]['data'][$dimension_key][
                            'label'
                        ] = $title;
                    }
                }
            ...
            $data['data'][
                'dimensions'
            ] = $this->get_active_enabled_custom_dimensions();
        }
        ...
        if (!empty($data['data'])) {
            ...
            foreach ($data['data']['dimensions'] as $key => $dimension_data) {
                $data['data']['galinks'][$key] =
                    'https://analytics.google.com/analytics/web/#/report/visitors-custom-variables/' .
                    MonsterInsights()->auth->get_referral_url() .
                    $this->get_ga_report_range($data['data']) .
                    rawurlencode(
                        '&explorer-segmentExplorer.segmentId=analytics.customVarName' .
                            $key .
    ...
    public function add_dimensions_settings_link($data)
    {
        if (current_user_can('monsterinsights_save_settings')) {
            $data['data']['footer'] = sprintf(
                '<a href="%s">%s</a>',
                admin_url(
                    'admin.php?page=monsterinsights_settings#/conversions'
                ),
