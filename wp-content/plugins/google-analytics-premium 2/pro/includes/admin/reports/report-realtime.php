...
final class MonsterInsights_Report_RealTime extends MonsterInsights_Report
{
    ...
    public function prepare_report_data($data)
    {
        if (!empty($data['data']['realtime'])) {
            foreach (
                $data['data']['realtime']['pageviewsovertime']['labels']
                as $key => $label
            ) {
                // Translators: Number of minutes passed since the view.
                $data['data']['realtime']['pageviewsovertime']['labels'][
                    $key
                ] = sprintf(esc_html__('%s minutes ago', 'ga-premium'), $label);
            }
            ...
            foreach (
                $data['data']['realtime']['countries']
                as $country_key => $country_data
            ) {
                if (!empty($countries[$country_data['country']])) {
                    $data['data']['realtime']['countries'][$country_key][
                        'iso'
            ...
            foreach (
                $data['data']['realtime']['cities']
                as $city_key => $city_data
            ) {
                if (!empty($countries[$city_data['country']])) {
                    $data['data']['realtime']['cities'][$city_key]['iso'] =
                        $countries[$city_data['country']];
                    $data['data']['realtime']['cities'][$city_key]['name'] =
                        $countries_translated[
                            $countries[$city_data['country']]
                        ];
                } else {
        ...
        if (!empty($data['data'])) {
            $data['data']['galinks'] = [
                'toppages' =>
                    'https://analytics.google.com/analytics/web/#/realtime/rt-content/' .
                    MonsterInsights()->auth->get_referral_url(),
                'referrals' =>
                    'https://analytics.google.com/analytics/web/#/realtime/rt-traffic/' .
                    MonsterInsights()->auth->get_referral_url(),
                'countries' =>
                    'https://analytics.google.com/analytics/web/#/realtime/rt-location/' .
                    MonsterInsights()->auth->get_referral_url(),
                'city' =>
                    'https://analytics.google.com/analytics/web/#/realtime/rt-location/' .
                    MonsterInsights()->auth->get_referral_url(),
