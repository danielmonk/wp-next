...
final class MonsterInsights_License
{
    ...
    public function get_site_license($force = false)
    {
        if (!empty($this->site) && !$force) {
            ...
            $site = get_option('monsterinsights_license', []);
            $this->site = $site;
            return $site;
        }
    }
    ...
    public function get_network_license($force = false)
    {
        if (!empty($this->network) && !$force) {
            ...
            $network = get_site_option('monsterinsights_network_license', []);
    ...
    public function get_license_key()
    {
        ...
        $license_key = !empty($license_key)
            ? $license_key
            : MonsterInsights()->license->get_default_license_key();
        return $license_key;
    }
    public function get_site_license_key()
    {
        return !empty($this->site['key']) &&
            is_string($this->site['key']) &&
            strlen($this->site['key']) > 10
            ? $this->site['key']
            : '';
    }
    public function get_network_license_key()
    {
        return !empty($this->network['key']) &&
            is_string($this->network['key']) &&
            strlen($this->network['key']) > 10
            ? $this->network['key']
            : '';
    }
    ...
    public function set_site_license($data = [])
    {
        update_option('monsterinsights_license', $data);
        $this->set_site_license_updates();
        $this->site = $data;
    }
    public function set_network_license($data = [])
    {
    ...
    public function get_license_type()
    {
        ...
        return $this->using_network_license()
            ? $this->get_network_license_type()
            : $this->get_site_license_type();
    }
    public function get_site_license_type()
    {
        return $this->get_site_license_key() &&
            !empty($this->site['type']) &&
            $this->is_valid_license_type($this->site['type'])
            ? $this->site['type']
            : '';
    }
    public function get_network_license_type()
    {
        return $this->get_network_license_key() &&
            !empty($this->network['type']) &&
            $this->is_valid_license_type($this->network['type'])
            ? $this->network['type']
            : '';
    }
    ...
    public function license_has_error()
    {
        ...
        return $this->using_network_license()
            ? $this->network_license_has_error()
            : $this->site_license_has_error();
    }
    ...
    public function license_expired()
    {
        ...
        return $this->using_network_license()
            ? $this->network_license_expired()
            : $this->site_license_expired();
    }
    ...
    public function license_disabled()
    {
        ...
        return $this->using_network_license()
            ? $this->network_license_disabled()
            : $this->site_license_disabled();
    }
    ...
    public function license_invalid()
    {
        ...
        return $this->using_network_license()
            ? $this->network_license_invalid()
            : $this->site_license_invalid();
    }
    ...
    public function get_license_error()
    {
        ...
        return $this->using_network_license()
            ? $this->get_network_license_error()
            : $this->get_site_license_error();
    }
    ...
    public function license_can($requires = 'lite')
    {
        ...
        return $this->using_network_license()
            ? $this->network_license_can($requires)
            : $this->site_license_can($requires);
    }
    public function site_license_can($requires = 'lite')
    {
        ...
        switch ($requires) {
            ...
            case 'pro':
                $can_access =
                    $level === 'master' || $level === 'pro' ? true : false;
                break;

            case 'plus':
                $can_access =
                    $level === 'master' || $level === 'pro' || $level === 'plus'
                        ? true
                        : false;
                break;
    ...
    {
        if (
            defined('MONSTERINSIGHTS_LICENSE_KEY') &&
            is_string(MONSTERINSIGHTS_LICENSE_KEY) &&
            strlen(MONSTERINSIGHTS_LICENSE_KEY) > 10
        ) {
    ...
    public function is_valid_license_type($type = '')
    {
        return !empty($type) &&
            is_string($type) &&
            in_array($type, $this->valid_license_types());
    }

    public function valid_license_types()
    {
        return ['basic', 'plus', 'pro', 'master'];
    }
}
