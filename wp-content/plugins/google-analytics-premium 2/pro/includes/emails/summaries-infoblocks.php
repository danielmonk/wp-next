...
class MonsterInsights_Summaries_InfoBlocks
{
    ...
    protected function verify_fetched($fetched)
    {
        ...
        if (
            empty($fetched['status']) ||
            empty($fetched['summaries']) ||
            empty($fetched['default'])
        ) {
            return $info;
        }
    ...
    protected function filter_by_current_date($data)
    {
        ...
        $blocks_sent = get_option(
            'monsterinsights_email_summaries_infoblocks_sent'
        );
        ...
        $filtered_blocks = array_diff_key(
            $data_by_date,
            array_flip($blocks_sent)
        );
    ...
    public function get_next()
    {
        ...
        if (empty($block)) {
            ...
            $unused_info_blocks = array_diff_key(
                $data,
                array_flip($blocks_sent)
            );
    ...
    {
        $block_id = isset($info_block['id'])
            ? absint($info_block['id'])
            : false;
