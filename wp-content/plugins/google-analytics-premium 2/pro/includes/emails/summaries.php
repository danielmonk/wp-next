...
class MonsterInsights_Email_Summaries
{
    ...
    public function __construct()
    {
        ...
        $options['summaries_carbon_copy'] = monsterinsights_get_option(
            'summaries_carbon_copy',
            'no'
        );
        $options['summaries_email_addresses'] = monsterinsights_get_option(
            'summaries_email_addresses'
        );
        $options['summaries_header_image'] = monsterinsights_get_option(
            'summaries_header_image'
        );
        ...
        if (
            !empty($email_summaries) &&
            'on' !== $email_summaries &&
            wp_next_scheduled('monsterinsights_email_summaries_cron')
        ) {
    ...
    public function hooks()
    {
        ...
        if (
            !empty($this->email_options['email_summaries']) &&
            'on' === $this->email_options['email_summaries']
        ) {
            add_action('init', [$this, 'preview']);
            add_filter('monsterinsights_email_template_paths', [
                $this,
                'add_email_template_path',
            ]);
            add_filter('monsterinsights_emails_templates_set_initial_args', [
                $this,
                'set_template_args',
            ]);
            add_filter('cron_schedules', [$this, 'add_weekly_cron_schedule']);
            add_action('monsterinsights_email_summaries_cron', [$this, 'cron']);
            add_action('wp_ajax_monsterinsights_send_test_email', [
                $this,
                'send_test_email',
            ]);
            add_action(
                'monsterinsights_after_update_settings',
                [$this, 'reset_email_summaries_options'],
                10,
                2
            );
        }
    ...
    protected function is_enabled()
    {
        ...
        return apply_filters(
            'monsterinsights_emails_summaries_is_enabled',
            $this->is_enabled
        );
    }
    ...
    public function is_preview()
    {
        if (
            isset(
                $_GET['monsterinsights_email_preview'],
    ...
    public function get_header_image()
    {
        ...
        $img = [
            'url' => plugins_url(
                "pro/assets/img/emails/summaries/logo-MonsterInsights.png",
                MONSTERINSIGHTS_PLUGIN_FILE
            ),
            '2x' => plugins_url(
                "pro/assets/img/emails/summaries/logo-MonsterInsights@2x.png",
                MONSTERINSIGHTS_PLUGIN_FILE
            ),
        ];
    ...
    {
        $file_paths['1000'] =
            MONSTERINSIGHTS_PLUGIN_DIR . 'pro/includes/emails/templates';
        return $file_paths;
    }
    ...
    {
        $schedules['monsterinsights_email_summaries_weekly'] = [
            'interval' => WEEK_IN_SECONDS,
            'display' => esc_html__(
                'Weekly MonsterInsights Email Summaries',
                'ga-premium'
            ),
        ];
    ...
    public function get_email_subject()
    {
        ...
        $site_url = isset($site_url_parsed['host'])
            ? $site_url_parsed['host']
            : $site_url;

        // Translators: The domain of the site is appended to the subject.
        $subject = sprintf(
            __('MonsterInsights Summary - %s', 'ga-premium'),
            $site_url
        );

        return apply_filters(
            'monsterinsights_emails_summaries_cron_subject',
            $subject
        );
    }
    ...
    public function get_email_addresses()
    {
        ...
        if (!empty($email_addresses) && is_array($email_addresses)) {
            foreach ($email_addresses as $email_address) {
                if (
                    !empty($email_address['email']) &&
                    is_email($email_address['email'])
                ) {
        ...
        return apply_filters(
            'monsterinsights_email_addressses_to_send',
            $emails
        );
    }
    ...
    public function is_enabled_html_template()
    {
        ...
        return apply_filters(
            'monsterinsights_email_html_template',
            $value,
            $this
        );
    }
    ...
    public function set_template_args($args)
    {
        ...
        $args['body']['title'] = esc_html__(
            'Website Traffic Summary',
            'ga-premium'
        );
        $args['body']['description'] = esc_html__(
            'Let’s take a look at how your website traffic performed in the past week.',
            'ga-premium'
        );
        ...
        $args['body']['settings_tab_url'] = esc_url(
            admin_url('admin.php?page=monsterinsights_settings#/advanced')
        );
        $args['footer']['settings_tab_url'] = esc_url(
            admin_url('admin.php?page=monsterinsights_settings#/advanced')
        );

        return apply_filters(
            'monsterinsights_email_summaries_template_args',
            $args
        );
    }
    ...
    public function reset_email_summaries_options($key, $value)
    {
        if (
            ...
            monsterinsights_update_option('summaries_email_addresses', [
                $default_email,
            ]);
