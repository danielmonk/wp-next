<div id="ctf-admin" class="wrap">
    ...
    <h2 class="nav-tab-wrapper">
        <a href="admin.php?page=custom-twitter-feeds&tab=configure" class="nav-tab <?php if (
            $active_tab == 'configure'
        ) {
            echo 'nav-tab-active';
        } ?>"><?php _e('1. Configure', 'ctf'); ?></a>
        <a href="admin.php?page=custom-twitter-feeds&tab=customize" class="nav-tab <?php if (
            $active_tab == 'customize'
        ) {
            echo 'nav-tab-active';
        } ?>"><?php _e('2. Customize', 'ctf'); ?></a>
        <a href="admin.php?page=custom-twitter-feeds&tab=style" class="nav-tab <?php if (
            $active_tab == 'style'
        ) {
            echo 'nav-tab-active';
        } ?>"><?php _e('3. Style', 'ctf'); ?></a>
        <a href="admin.php?page=custom-twitter-feeds&tab=display" class="nav-tab <?php if (
            $active_tab == 'display'
        ) {
            echo 'nav-tab-active';
        } ?>"><?php _e('4. Display Your Feed', 'ctf'); ?></a>
        <a href="admin.php?page=custom-twitter-feeds&tab=support" class="nav-tab <?php if (
            $active_tab == 'support'
        ) {
            echo 'nav-tab-active';
        } ?>"><?php _e('Support', 'ctf'); ?></a>
        <a href="admin.php?page=custom-twitter-feeds-license" class="nav-tab <?php if (
            $active_tab == 'license'
        ) {
            echo 'nav-tab-active';
        } ?>"><?php _e('License', 'ctf'); ?></a>
	    <?php if (!is_plugin_active('social-wall/social-wall.php')) { ?>
            <a href="admin.php?page=custom-twitter-feeds-sw" class="nav-tab"><?php _e(
                'Create a Social Wall'
            ); ?><span class="ctf-alert-bubble">New</span></a>
	    <?php } ?>
    </h2>
    <?php
    function ctf_expiration_notice()
    {
        ...
        if (
            empty($ctf_license) ||
            (!isset($ctf_license) && !$ctf_check_license)
        ) {
            return;
        }
        ...
        if (
            $ctf_interval == 0 ||
            strtotime($ctf_license_expires_date) < strtotime($ctf_todays_date)
        ) {
    ...
    <p><i class="fa fa-life-ring" aria-hidden="true"></i>&nbsp; <?php _e(
        'Need help setting up the plugin? Check out our <a href="https://smashballoon.com/custom-twitter-feeds/docs/" target="_blank">setup directions</a>',
        'custom-twitter-feeds'
    ); ?></p>

    <div class="ctf-quick-start">
        <h3><i class="fa fa-rocket" aria-hidden="true"></i>&nbsp; <?php _e(
            'Display your feed',
            'custom-twitter-feeds'
        ); ?></h3>
        <p><?php _e(
        ...
        <p><?php _e(
            "Find out how to display <a href='?page=custom-twitter-feeds&tab=display'>multiple feeds</a>.",
            "custom-twitter-feeds"
        ); ?></p>
    </div>
    <?php } ?>
