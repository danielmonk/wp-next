...
// Call the custom API.
$ctf_response = wp_remote_get(add_query_arg($ctf_api_params, CTF_STORE_URL), [
    'timeout' => 60,
    'sslverify' => false,
]);
...
<table class="form-table">
    ...
    <tr valign="top">
        ...
        <td>
            <input id="ctf_license_key" name="ctf_license_key" type="text" class="regular-text" value="<?php esc_attr_e(
                $ctf_license
            ); ?>" />
            ...
            <?php if (false !== $ctf_license) { ?>
                ...
                <?php if ($ctf_status !== false && $ctf_status == 'valid') { ?>
                    ...
                    <input type="submit" class="button-secondary" name="ctf_license_deactivate" value="<?php _e(
                        'Deactivate License'
                    ); ?>"/>

                    <?php if ($ctf_license_data['license'] == 'expired') { ?>
                        <span class="ctf_license_status" style="color:red;"><?php _e(
                            'Expired'
                        ); ?></span>
                    <?php } else { ?>
                        <span class="ctf_license_status" style="color:green;"><?php _e(
                            'Active'
                        ); ?></span>
                    <?php } ?>

                <?php } else {wp_nonce_field('ctf_nonce', 'ctf_nonce'); ?>
                    <input type="submit" class="button-secondary" name="ctf_license_activate" value="<?php _e(
                        'Activate License'
                    ); ?>"/>
            ...
            <p style="font-size: 13px;">
                <a href='https://smashballoon.com/checkout/?edd_license_key=<?php echo trim(
                    $ctf_license
                ); ?>&amp;download_id=<?php echo CTF_PRODUCT_ID; ?>' target='_blank'><?php _e(
    "Renew your license"
); ?></a>
                &nbsp;&nbsp;&nbsp;&middot;
    ...
    <div id="ctf-test-license-connection" style="display: none;">
        <?php if (isset($ctf_license_data['item_name'])) {
