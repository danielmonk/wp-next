...
class CtfFeedPro extends CtfFeed
{
    ...
    public static function init(
        ...
        $feed->setStandardBoolOptions(
            ['persistentcache', 'includeretweets', 'autores'],
            true
        );
        ...
        $feed->setStandardTextOptions(['includewords', 'excludewords']);
        $feed->setStandardTextOptions(
            ['includeanyall', 'excludeanyall'],
            'any'
        );
    ...
    {
        $this->feed_options['feed_types_and_terms_display'] = [];
        $this->feed_options['feed_types_and_terms'] = [];
        $this->feed_options['type'] = '';
        $this->feed_options['screenname'] = isset($this->atts['screenname'])
            ? $this->atts['screenname']
        ...
        if (
            isset($this->atts['mentions']) &&
            $this->atts['mentions'] == 'true'
        ) {
            $this->feed_options['feed_types_and_terms'][] = [
                'mentionstimeline',
                '',
        ...
        if (isset($this->atts['search'])) {
            $searches = isset($this->atts['search'])
                ? $this->atts['search']
                : '';
        ...
        if (isset($this->atts['hashtag'])) {
            $hashtags = isset($this->atts['hashtag'])
                ? $this->validateHashtags($this->atts['hashtag'])
                : '';
        ...
        if (sizeof($this->feed_options['feed_types_and_terms']) == 1) {
            $this->feed_options['type'] =
                $this->feed_options['feed_types_and_terms'][0][0];
            $this->feed_options['feed_term'] =
                $this->feed_options['feed_types_and_terms'][0][1];
            $this->feed_options['feed_types_and_terms'] = '';
        }

        if (
            empty($this->feed_options['type']) &&
            empty($this->feed_options['feed_types_and_terms'])
        ) {
            ...
            switch ($this->feed_options['type']) {
                ...
                case 'mentionstimeline':
                    ...
                    break;
                case 'search':
                    $search_str = isset($this->db_options['search_text'])
                    ...
                    break;
                case 'hashtag':
                    $hashtag_str = isset($this->db_options['hashtag_text'])
                        ? $this->db_options['hashtag_text']
                        : '';
                    ...
                    break;
                case 'lists':
                    $lists_str = isset($this->db_options['lists_id'])
                        ? $this->db_options['lists_id']
                        : '';
    ...
    {
        if (
            $this->feed_options['persistentcache'] &&
    ...
    public function setIncludeExcludeOptions()
    {
        ...
        $this->feed_options['tweet_includes'] = isset($this->atts['include'])
            ? explode(',', str_replace(', ', ',', $this->atts['include']))
            : [];
        if (empty($this->feed_options['tweet_includes'][0])) {
            $this->feed_options['tweet_excludes'] = isset(
                $this->atts['exclude']
            )
                ? explode(',', str_replace(', ', ',', $this->atts['exclude']))
                : [];
        }
        if (
            empty($this->feed_options['tweet_excludes'][0]) &&
            empty($this->feed_options['tweet_includes'][0])
        ) {
            $this->feed_options['tweet_includes'][] =
            ...
            $this->feed_options['tweet_includes'][] =
                ...
                $this->db_options['include_author'] == false
                    ...
                    : 'author';
            $this->feed_options['tweet_includes'][] =
                isset($this->db_options['include_logo']) &&
                $this->db_options['include_logo'] == false
                    ? null
                    : 'logo';
            $this->feed_options['tweet_includes'][] =
            ...
            $this->feed_options['tweet_includes'][] =
                ...
                $this->db_options['include_linkbox'] == false
                    ...
                    : 'linkbox';
            $this->feed_options['tweet_includes'][] =
                isset($this->db_options['include_repliedto']) &&
                $this->db_options['include_repliedto'] == false
                    ? null
                    : 'repliedto';
            $this->feed_options['tweet_includes'][] =
                isset($this->db_options['include_media']) &&
                $this->db_options['include_media'] == false
                    ? null
                    : 'media';
            $this->feed_options['tweet_includes'][] =
                isset($this->db_options['include_twittercards']) &&
                $this->db_options['include_twittercards'] == false
                    ? null
                    : 'twittercards';
        }
    }
    ...
    public function setMasonryClasses()
    {
        ...
        if ($this->feed_options['layout'] === 'masonry') {
            ...
            if ($options['masonrycols'] > 3 && $options['masonrycols'] < 7) {
                $classes_to_add .=
                    ' masonry-' . $options['masonrycols'] . '-desktop';
            }
    ...
    public function setTransientName()
    {
        ...
        $includewords = !empty($this->feed_options['includewords'])
            ? substr(
                str_replace(
                    [',', ' '],
                    '',
                    $this->feed_options['includewords']
                ),
                0,
                10
            )
            : '';
        $excludewords = !empty($this->feed_options['excludewords'])
            ? substr(
                str_replace(
                    [',', ' '],
                    '',
                    $this->feed_options['excludewords']
                ),
                0,
                5
            )
        ...
        $remove_by_id_array = explode(
            ',',
            str_replace(' ', '', $this->feed_options['remove_by_id'])
        );
        $remove_by_id_str = '';
        ...
        switch ($this->feed_options['type']) {
            case 'hometimeline':
                ...
                break;
            case 'usertimeline':
                $screenname = isset($this->feed_options['screenname'])
            ...
            case 'mentionstimeline':
                ...
                break;
            case 'lists':
                $list = isset($this->feed_options['feed_term'])
                    ? $this->feed_options['feed_term']
                    : '';
                $this->transient_name = substr(
                    'ctf_' .
    ...
    public function maybeSetTweetsFromCache()
    {
        if (
            ...
            if (is_array($persistent_cache_tweets)) {
                $this->transient_data = array_slice(
                    $persistent_cache_tweets,
                    $this->persistent_index - $this->feed_options['num'] - 1,
                    $this->persistent_index
                );
            } else {
            ...
            if (!is_array($this->transient_data)) {
                $this->transient_data = json_decode(
                    $this->transient_data,
                    $assoc = true
                );
            }
    ...
    {
        if (
            $this->hasTweetTextFilter() ||
            !$this->feed_options['includereplies']
        ) {
    ...
    private function persistentCacheTweets()
    {
        ...
        if (
            ...
            $tweet_set = $this->appendPersistentCacheTweets(
                $existing_cache,
                $tweet_set
            );
            ...
            $api_obj = $this->apiConnectionResponse(
                'search',
                $this->feed_options['feed_term']
            );
            ...
            if ($this->tweet_set) {
                ...
                $cache_list = get_option('ctf_cache_list', []);
    ...
    {
        if (
            !empty($this->feed_options['includewords']) ||
            !empty($this->feed_options['excludewords'])
        ) {
            return true;
        } elseif (
            !empty($this->feed_options['remove_by_id']) ||
            !$this->feed_options['includeretweets']
        ) {
            return true;
        } else {
            return false;
    ...
    {
        if (empty($good_text)) {
            // don't factor in the includewords if there aren't any
            return $default;
        } else {
            $encoded_text =
                ' ' .
    ...
    {
        if (empty($bad_text)) {
            // don't factor in the excludewords if there aren't any
            return $default;
        } else {
            $encoded_text =
                ' ' .
                ...
                foreach ($bad_text as $bad) {
                    $converted_excludeword = trim(
                        str_replace(
                            ...
                            ' ',
                            urlencode(
                                str_replace('#', 'HASHTAG', strtolower($bad))
                            )
                        )
                    );
    ...
    {
        $return = false;
        $good_text = !empty($this->feed_options['includewords'])
            ? explode(
                ',',
                str_replace(' ', '', $this->feed_options['includewords'])
            )
            : '';
        $bad_text = !empty($this->feed_options['excludewords'])
            ? explode(
                ',',
                str_replace(' ', '', $this->feed_options['excludewords'])
            )
            : '';
        ...
        if (!empty($good_text) || !empty($bad_text)) {
            if (
                ...
                if (
                    $this->hasGoodText(
                        $good_text,
                        $includewords_any_all,
                        $tweet_text,
                        true
                    ) &&
                    $this->hasNoBadText($bad_text, 'any', $tweet_text, true)
                ) {
                    $return = false;
                } else {
                    $return = true;
    ...
    {
        $working_tweet_set = isset($tweet_set['statuses'])
            ? $tweet_set['statuses']
            : $tweet_set;
        $usable_tweets = 0;
        if ($limit) {
        ...
        $tweets_to_remove_strip_ctf = str_replace(
            'ctf_',
            '',
            $this->feed_options['remove_by_id']
        );
        $ids_of_tweets_to_remove = !empty($tweets_to_remove_strip_ctf)
            ? explode(',', str_replace(' ', '', $tweets_to_remove_strip_ctf))
            : '';
        $i = 0; // index of working_tweet_set
        $still_setting_filtered_tweets = true;

        while ($still_setting_filtered_tweets) {
            // stays true until the number to display is reached or out of tweets
            if (isset($working_tweet_set[$i])) {
                // if there is another tweet available
                $retweet_id = isset(
                    $working_tweet_set[$i]['retweeted_status']['id_str']
                )
                    ? $working_tweet_set[$i]['retweeted_status']['id_str']
                    : '';
                if (
                    ...
                    $this->feed_options['selfreplies'] &&
                    isset($working_tweet_set[$i]['in_reply_to_screen_name']) &&
                    $working_tweet_set[$i]['in_reply_to_screen_name'] !==
                        $working_tweet_set[$i]['user']['screen_name']
                ) {
                    unset($working_tweet_set[$i]);
                } elseif (
                    !empty($ids_of_tweets_to_remove) &&
                    in_array(
                        $working_tweet_set[$i]['id_str'],
                        $ids_of_tweets_to_remove
                    )
                ) {
                    unset($working_tweet_set[$i]);
    ...
    {
        usort($tweet_set, [$this, 'compareDateCreatedAt']);
        return $tweet_set;
    }
    ...
    private function removeDuplicates($tweet_set, $limit = true)
    {
        ...
        while ($still_setting_filtered_tweets) {
            ...
            if (isset($tweet_set[$i]['retweeted_status']['id_str'])) {
                ...
                $id = isset($tweet_set[$i]['retweeted_status']['id_str'])
                    ? $tweet_set[$i]['retweeted_status']['id_str']
                    : $tweet_set[$i]['id_str'];
    ...
    private function getTweetsSinceID(
        $since_id,
        $end_point = 'search',
        $feed_term,
        $count
        ...
        // actual connection
        $twitter_connect = new CtfOauthConnectPro(
            $request_settings,
            $end_point
        );
    ...
    public function maybeSetTweetsFromTwitter()
    {
        ...
        if (
            empty($this->feed_options['feed_types_and_terms']) ||
            !isset($this->feed_options['feed_types_and_terms'])
        ) {
            $feed_term = isset($this->feed_options['feed_term'])
                ? $this->feed_options['feed_term']
                : '';
            if (
                empty($feed_term) &&
            ...
            if (!isset($working_tweet_set['errors'][0])) {
                ...
                $working_tweet_set = $this->reduceTweetSetData(
                    $working_tweet_set
                );
            ...
            $num_tweets = is_array($working_tweet_set)
                ? count($working_tweet_set)
                : 500;
            // remove the last tweet as it is returned in the next request

            if (
                !isset($working_tweet_set['errors'][0]) &&
                isset($working_tweet_set[0]) &&
                ...
                $tweet_set_to_merge = json_decode(
                    $api_obj->json,
                    $assoc = true
                );

                if (isset($tweet_set_to_merge['statuses'])) {
                    $working_tweet_set = array_merge(
                        $working_tweet_set,
                        $tweet_set_to_merge['statuses']
                    );
            ...
            foreach (
                $this->feed_options['feed_types_and_terms']
                as $feed_type_and_term
            ) {
                $api_obj = $this->apiConnectionResponse(
                    $feed_type_and_term[0],
                    $feed_type_and_term[1]
                );
                $tweet_set_to_merge = json_decode(
                    $api_obj->json,
                    $assoc = true
    ...
    protected function filterTrimmedTweets($trimmed, $tweet)
    {
        if (isset($tweet['in_reply_to_screen_name'])) {
            $trimmed['in_reply_to_screen_name'] =
                $tweet['in_reply_to_screen_name'];
            $trimmed['entities']['user_mentions'][0]['name'] = isset(
                $tweet['entities']['user_mentions'][0]['name']
            )
                ? $tweet['entities']['user_mentions'][0]['name']
                : '';
            $trimmed['in_reply_to_status_id_str'] =
                $tweet['in_reply_to_status_id_str'];
        }

        if (isset($tweet['extended_entities']['media'])) {
            // if there is media, we need to remove the media url from the tweet text
            $text = isset($tweet['full_text'])
                ? $tweet['full_text']
                : $tweet['text'];
            $remove_url_from_tweet = apply_filters(
                'ctf_should_remove_url_from_text',
                true
            );

            if (
                isset($tweet['extended_entities']['media'][0]['url']) &&
                $remove_url_from_tweet
            ) {
                $trimmed['text'] = $this->removeStringFromText(
            ...
            for ($i = 0; $i < $num_media; $i++) {
                $trimmed['extended_entities']['media'][$i]['media_url_https'] =
                    $tweet['extended_entities']['media'][$i]['media_url_https'];
                $trimmed['extended_entities']['media'][$i]['type'] =
                    $tweet['extended_entities']['media'][$i]['type'];
                if (isset($tweet['extended_entities']['media'][$i]['sizes'])) {
                    $trimmed['extended_entities']['media'][$i]['sizes'] =
                        $tweet['extended_entities']['media'][$i]['sizes'];
                }
                if (
                    $tweet['extended_entities']['media'][$i]['type'] ==
                        'video' ||
                    ...
                    foreach (
                        $tweet['extended_entities']['media'][$i]['video_info'][
                            'variants'
                        ]
                        as $variant
                    ) {
                        if (
                            ...
                            $trimmed['extended_entities']['media'][$i][
                                'video_info'
                            ]['variants'][$i]['url'] = $variant['url'];
                        }
                    }
                    if (
                        !isset(
                        ...
                        $trimmed['extended_entities']['media'][$i][
                            ...
                            $tweet['extended_entities']['media'][$i][
                                'video_info'
                            ]['variants'][0]['url'];
                    }
                }
            ...
            if (
                isset($tweet['entities']['media'][0]['url']) &&
                $remove_url_from_tweet
            ) {
                $trimmed['text'] = $this->removeStringFromText(
            ...
            for ($i = 0; $i < $num_media; $i++) {
                $trimmed['entities']['media'][$i]['media_url_https'] =
                    $tweet['entities']['media'][$i]['media_url_https'];
                $trimmed['entities']['media'][$i]['type'] =
                    $tweet['entities']['media'][$i]['type'];
                if (isset($tweet['entities']['media'][$i]['sizes'])) {
                    $trimmed['entities']['media'][$i]['sizes'] =
                        $tweet['entities']['media'][$i]['sizes'];
                }
                if (
                    $tweet['entities']['media'][$i]['type'] == 'video' ||
                    $tweet['entities']['media'][$i]['type'] == 'animated_gif'
                ) {
                    foreach (
                        $tweet['entities']['media'][$i]['video_info'][
                            'variants'
                        ]
                        as $variant
                    ) {
                        if (
                            ...
                            $trimmed['entities']['media'][$i]['video_info'][
                                'variants'
                            ][$i]['url'] = $variant['url'];
                        }
                    }
        ...
        if (isset($tweet['retweeted_status']['extended_entities']['media'])) {
            // if there is media, we need to remove the media url from the tweet text
            $retweeted_text = isset($tweet['retweeted_status']['full_text'])
            ...
            if (
                ...
                $trimmed['retweeted_status'][
                    'text'
                ] = $this->removeStringFromText(
                    $tweet['retweeted_status']['extended_entities']['media'][0][
                        'url'
                    ],
                    $retweeted_text
            ...
            for ($i = 0; $i < $num_media; $i++) {
                $trimmed['retweeted_status']['extended_entities']['media'][$i][
                    'media_url_https'
                ] =
                    $tweet['retweeted_status']['extended_entities']['media'][
                        $i
                ...
                if (
                    ...
                    $trimmed['retweeted_status']['extended_entities']['media'][
                        $i
                    ]['sizes'] =
                        $tweet['retweeted_status']['extended_entities'][
                            'media'
                ...
                if (
                    ...
                    foreach (
                        ...
                        if (
                            ...
                            $trimmed['retweeted_status']['extended_entities'][
                                'media'
                            ][$i]['video_info']['variants'][$i]['url'] =
                                $variant['url'];
                        }
                    }
                    if (
            ...
            $retweeted_text = isset($tweet['retweeted_status']['full_text'])
                ...
                : $tweet['retweeted_status']['text'];
            if (
                isset($tweet['retweeted_status']['entities']['media'][0]['url'])
            ) {
                $trimmed['retweeted_status'][
                    'text'
                ] = $this->removeStringFromText(
                    $tweet['retweeted_status']['entities']['media'][0]['url'],
                    $retweeted_text
                );
            }
            ...
            for ($i = 0; $i < $num_media; $i++) {
                $trimmed['retweeted_status']['entities']['media'][$i][
                    'media_url_https'
                ] =
                    $tweet['retweeted_status']['entities']['media'][$i][
                        'media_url_https'
                    ];
                $trimmed['retweeted_status']['entities']['media'][$i]['type'] =
                    $tweet['retweeted_status']['entities']['media'][$i]['type'];
                if (
                    isset(
                        $tweet['retweeted_status']['entities']['media'][$i][
                            'sizes'
                ...
                if (
                    ...
                    foreach (
                        ...
                        if (
                            ...
                            $trimmed['retweeted_status']['entities']['media'][
                                $i
                            ]['video_info']['variants'][$i]['url'] =
                                $variant['url'];
                        }
                    }
                    if (
            ...
            isset($tweet['quoted_status']['extended_entities']['media'])
        ) {
            // if there is media, we need to remove the media url from the tweet text
            $quoted_text = isset($tweet['quoted_status']['full_text'])
                ? $tweet['quoted_status']['full_text']
                : $tweet['quoted_status']['text'];
            if (
                isset(
                    $tweet['quoted_status']['extended_entities']['media'][0][
                        'url'
                    ]
                )
            ) {
                $trimmed['quoted_status']['text'] = $this->removeStringFromText(
                    $tweet['quoted_status']['extended_entities']['media'][0][
                        'url'
                    ],
                    $quoted_text
            ...
            for ($i = 0; $i < $num_media; $i++) {
                $trimmed['quoted_status']['extended_entities']['media'][$i][
                    'media_url_https'
                ] =
                    $tweet['quoted_status']['extended_entities']['media'][$i][
                        'media_url_https'
                    ];
                $trimmed['quoted_status']['extended_entities']['media'][$i][
                    'type'
                ...
                if (
                    ...
                    foreach (
                        ...
                        if (
                            ...
                            $trimmed['quoted_status']['extended_entities'][
                                'media'
                            ][$i]['video_info']['variants'][$i]['url'] =
                                $variant['url'];
                        }
                    ...
                    if (
                        ...
                        $trimmed['quoted_status']['extended_entities']['media'][
                            $i
                        ]['video_info']['variants'][$i]['url'] =
            ...
            if (isset($tweet['quoted_status']['entities']['media'][0]['url'])) {
                $trimmed['quoted_status']['text'] = $this->removeStringFromText(
                    $tweet['quoted_status']['entities']['media'][0]['url'],
                    $quoted_text
                );
            }
            ...
            for ($i = 0; $i < $num_media; $i++) {
                $trimmed['quoted_status']['entities']['media'][$i][
                    'media_url_https'
                ] =
                    $tweet['quoted_status']['entities']['media'][$i][
                        'media_url_https'
                    ];
                $trimmed['quoted_status']['entities']['media'][$i]['type'] =
                    $tweet['quoted_status']['entities']['media'][$i]['type'];
                if (
                    $tweet['quoted_status']['entities']['media'][$i]['type'] ==
                        'video' ||
                    $tweet['quoted_status']['entities']['media'][$i]['type'] ==
        ...
        if (
            ...
            if (
                ...
                $trimmed['retweeted_status']['quoted_status'][
                    'text'
                ] = $this->removeStringFromText(
                    $tweet['retweeted_status']['quoted_status'][
                        'extended_entities'
            ...
            for ($i = 0; $i < $num_media; $i++) {
                ...
                if (
                    ...
                    $tweet['retweeted_status']['quoted_status'][
                        'extended_entities'
                    ]['media'][$i]['type'] == 'animated_gif'
                ) {
                    foreach (
                        $tweet['retweeted_status']['quoted_status'][
                            'extended_entities'
                        ]['media'][$i]['video_info']['variants']
                        as $variant
                    ) {
                        if (
                            isset($variant['content_type']) &&
                    ...
                    if (
                        !isset(
                            $trimmed['retweeted_status']['quoted_status'][
                                'extended_entities'
                            ]['media'][$i]['video_info']['variants'][$i]['url']
                        )
                    ) {
                        $trimmed['retweeted_status']['quoted_status'][
                            'extended_entities'
                            ...
                            $tweet['retweeted_status']['quoted_status'][
                                'extended_entities'
                            ]['media'][$i]['video_info']['variants'][0]['url'];
                    }
                }
            }
        } elseif (
            isset(
                $tweet['retweeted_status']['quoted_status']['entities']['media']
            )
        ) {
            // if there is media, we need to remove the media url from the tweet text
            $retweeted_text = isset(
            ...
            for ($i = 0; $i < $num_media; $i++) {
                $trimmed['retweeted_status']['quoted_status']['entities'][
                    'media'
                ][$i]['media_url_https'] =
                    $tweet['retweeted_status']['quoted_status']['entities'][
                        'media'
        ...
        if (isset($tweet['quoted_status'])) {
            ...
            if (isset($tweet['entities']['urls'][$maybe_remove_index]['url'])) {
                $text = isset($trimmed['full_text'])
                    ? $trimmed['full_text']
                    : $trimmed['text'];
                $trimmed['text'] = $this->removeStringFromText(
                    $tweet['entities']['urls'][$maybe_remove_index]['url'],
                    $text
                );
            }
        }
        ...
        if (isset($tweet['entities']['urls'][0]['expanded_url'])) {
            $trimmed['entities']['urls'][0]['expanded_url'] =
                $tweet['entities']['urls'][0]['expanded_url'];

            if (ctf_show('twittercards', $this->feed_options)) {
                $twitter_card = CTF_Feed_Pro::maybeGetTwitterCardData(
                    $tweet['entities']['urls'][0]['expanded_url'],
                    $tweet['id_str']
                );
                ...
                if (!empty($twitter_card)) {
                    ...
                    if (
                        isset($tweet['entities']['urls'][0]['url']) &&
                        $remove_url_from_tweet
                    ) {
                        $trimmed['text'] = $this->removeStringFromText(
        ...
        if (
            ...
            $trimmed['retweeted_status']['entities']['urls'][0][
                'expanded_url'
            ] =
                $tweet['retweeted_status']['entities']['urls'][0][
                    'expanded_url'
    ...
    private function carouselDataAtts($options)
    {
        if ($this->feed_options['layout'] === 'carousel') {
            ...
            $custom_breakpoints = apply_filters('ctf_carousel_breakpoints', []);
            if ($options['carouselautoplay']) {
                $data .= ' data-ctf-interval=' . $options['carouseltime'];
            }

            $data .= ' data-ctf-loop="' . $options['carouselloop'] . '"';
            $data .= isset($options['carouselcols'])
                ? ' data-ctf-cols="' . $options['carouselcols'] . '"'
                : '';
            $data .= isset($options['carouselmobilecols'])
                ? ' data-ctf-mobilecols="' .
                    $options['carouselmobilecols'] .
                    '"'
                : '';
            $data .= isset($options['carouselarrows'])
                ? ' data-ctf-arrows="' . $options['carouselarrows'] . '"'
                : '';
            $data .= isset($options['carouselheight'])
                ? ' data-ctf-height="' . $options['carouselheight'] . '"'
                : '';
            $data .= !empty($custom_breakpoints)
                ? ' data-ctf-breakpoints="' .
                    esc_attr(wp_json_encode($custom_breakpoints)) .
                    '"'
                : '';
    ...
    public function getTweetSetHtml($is_pagination = 0)
    {
        ...
        $ctf_data_linktextcolor =
            $feed_options['linktextcolor'] != ''
                ? ' data-ctflinktextcolor="' .
                    ...
                    '"'
                : '';
        $ctf_data_autoscrolldistance = $feed_options['autoscroll']
            ? ' data-ctfscrolloffset="' .
                $feed_options['autoscrolldistance'] .
                '"'
            : '';
        $ctf_data_maxmedia = isset($feed_options['maxmedia'])
            ? ' data-ctfmaxmedia="' . $feed_options['maxmedia'] . '"'
            : '4';
        $ctf_data_imagecols =
            isset($feed_options['imagecols']) != 0
                ? ' data-ctfimagecols="' . $feed_options['imagecols'] . '"'
                : 'auto';
        $ctf_data_other = $this->carouselDataAtts($feed_options);
        $ctf_enable_intents =
            ($feed_options['disableintents'] === false ||
                $feed_options['disableintents'] === 0) &&
            ctf_show('actions', $feed_options)
                ? ' data-ctfintents="1"'
                : '';

        $ctf_data_needed = $this->num_tweets_needed;
        $ctf_feed_classes = CTF_Parse_Pro::get_feed_classes(
            $feed_options,
        ...
        $ctf_main_atts =
            $ctf_data_disablelinks .
            $ctf_data_linktextcolor .
            $ctf_data_autoscrolldistance .
            $ctf_enable_intents .
            $cols_att;
        $ctf_data_atts =
            $ctf_data_other . $ctf_data_maxmedia . $ctf_data_imagecols;
    ...
    public function tweet_loop($tweet_set, $settings, $is_pagination)
    {
        ...
        if ($is_pagination > -1 && isset($tweet_set[1]['id_str'])) {
            while ($i < $len) {
                ...
                $retweeted_status = CTF_Parse_Pro::get_retweet_status(
                    $tweet_set[$i]
                );
                ...
                $twitter_card = CTF_Parse_Pro::get_twitter_card(
                    $twitter_card_post
                );

                $twitter_card_url = CTF_Parse_Pro::get_twitter_card_url(
                    $twitter_card_post
                );
                ...
                if (
                    ...
                    !empty(
                        CTF_Parse_Pro::get_twitter_card_url($twitter_card_post)
                    ) &&
                    !CTF_Parse_Pro::get_media($twitter_card_post) &&
                    !CTF_Parse_Pro::get_quoted_tc($twitter_card_post)
                ):
                    $maybe_twitter_card_placeholder = CTF_Display_Elements_Pro::get_twitter_card_placeholder(
                        $twitter_card_post,
                        $twitter_card_url
                    );
                endif;
                ...
                if (!$media && !empty($twitter_card_url)) {
                    $iframe_data = CTF_Parse_Pro::iframe_data(
                        $twitter_card_url
                    );
                    if ($iframe_data) {
                        $media = [
                ...
                $should_show_twitter_card =
                    !$media &&
                    ctf_show('twittercards', $feed_options) &&
                    isset($twitter_card) &&
                    !empty(
                        $twitter_card_post['twitter_card']['twitter:card']
                    ) &&
                    $twitter_card_post['twitter_card']['twitter:card'] !==
                        'amplify' &&
