...
class CTF_Feed_Locator
{
    ...
    public function add_or_update_entry()
    {
        ...
        if (empty($this->matching_entries)) {
            ...
            foreach ($this->matching_entries as $index => $matching_entry) {
                $details_atts = is_array($this->feed_details['atts'])
                    ? $this->feed_details['atts']
                    : [];
                $matching_atts = json_decode(
                    $matching_entry['shortcode_atts'],
                    true
                );
            ...
            if (false === $matched_location) {
                ...
                if (
                    ...
                    $this->update_entry(
                        $this->matching_entries[$non_unknown_match]['id'],
                        $this->matching_entries[$non_unknown_match][
                            'html_location'
                        ]
                    );
                } else {
                    if (
                        ...
                        $this->update_entry(
                            $this->matching_entries[$unknown_match]['id'],
                            $this->feed_details['location']['html']
                        );
                    } else {
                        $this->insert_entry();
    ...
    public static function should_clear_old_locations()
    {
        ...
        $last_old_feed_check = isset(
            $ctf_statuses_option['feed_locator']['last_check']
        )
            ? $ctf_statuses_option['feed_locator']['last_check']
            : 0;
    ...
    public static function should_do_locating()
    {
        $ctf_statuses_option = get_option('ctf_statuses', []);
        if (
            isset($ctf_statuses_option['feed_locator']['initialized']) &&
        ...
        $should_do_locating = apply_filters(
            'ctf_should_do_locating',
            $should_do_locating
        );
    ...
    public static function create_table()
    {
        ...
        if (
            $wpdb->get_var("show tables like '$feed_locator_table_name'") !=
            $feed_locator_table_name
        ) {
            $sql =
                "CREATE TABLE " .
                $feed_locator_table_name .
        ...
        if (!$had_error) {
            $wpdb->query(
                "ALTER TABLE $feed_locator_table_name ADD INDEX feed_id (feed_id)"
            );
            $wpdb->query(
                "ALTER TABLE $feed_locator_table_name ADD INDEX post_id (post_id)"
            );
        }
    }
