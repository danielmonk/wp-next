...
function ctf_add_mentionstimeline_options($admin)
{
    $admin->create_settings_field([
        ...
        'whatis' =>
            'Select this option to display tweets that @mention your twitter handle', // what is this? text
...
{
    $show_hide_list[] = ['include_replied_to', 'In reply to text'];
    $show_hide_list[] = ['include_media', 'Media (images, videos, gifs)'];
    $show_hide_list[] = ['include_twittercards', 'Twitter Cards'];
    return $show_hide_list;
}

add_action(
    'ctf_admin_style_option',
...
function ctf_add_masonry_autoscroll_options($admin)
{
    ...
    $admin->create_settings_field([
        ...
        'whatis' =>
            'This will replace the default text displayed for "In reply to"',
    ...
    add_settings_section(
        ...
        [$admin, 'general_section_text'], // callback function to explain the section
        'ctf_options_autoscroll' // matches the section name
    );
    ...
    $admin->create_settings_field([
        ...
        'class' => '',
        'whatis' =>
            "This will make every Twitter feed load more Tweets as the user gets to the bottom of the feed",
    ]);
    ...
    $admin->create_settings_field([
        ...
        'class' => 'default-text', // class for the wrapper and input field
        'whatis' =>
            'This is the distance in pixels from the bottom of the page the user must scroll to to trigger the loading of more tweets',
        'default' => '200', // "what is this?" text
    ]);
}

add_action(
    'ctf_admin_customize_option',
...
function ctf_add_filter_options($admin)
{
    ...
    add_settings_field(
        ...
        [
            ...
            'name' => 'remove_by_id',
            'whatis' =>
                'These are the specific ID numbers associated with a tweet. You can find the ID of a Tweet by viewing the Tweet on Twitter and copy/pasting the ID number from the end of the URL',
        ]
    );
...
function ctf_remove_by_id($args)
{
    ...
    <textarea name="<?php echo $args['option'] .
        '[' .
        $args['name'] .
        ']'; ?>" id="ctf_<?php echo $args['name']; ?>" style="width: 70%;" rows="3"><?php esc_attr_e(stripslashes($option_string)); ?></textarea>
    <?php if (isset($args['extra'])): ?><p><?php _e(
    $args['extra'],
    'custom-twitter-feeds'
); ?>
        <a class="ctf-tooltip-link" href="JavaScript:void(0);"><i class="fa fa-question-circle" aria-hidden="true"></i></a>
        <span class="ctf-tooltip ctf-more-info"><?php _e(
            $args['whatis'],
            'custom-twitter-feeds'
        ); ?>.</span>
        </p> <?php endif; ?>
...
{
    ?>
    <input id="ctf-clear-tc-cache" class="button-secondary" style="margin-top: 1px;" type="submit" value="<?php esc_attr_e(
        'Clear Twitter Cards'
    ); ?>" />
    <a class="ctf-tooltip-link" href="JavaScript:void(0);"><i class="fa fa-question-circle" aria-hidden="true"></i></a>
    <p class="ctf-tooltip ctf-more-info"><?php _e(
        'Clicking this button will clear all cached data for your links that have Twitter Cards',
        'custom-twitter-feeds'
    ); ?>.</p>
    <?php
}
...
{
    $options = get_option($args['option']);
    $include_any_all = isset($options['includeanyall'])
        ? esc_attr($options['includeanyall'])
        : 'any';
    $filter_and_or = isset($options['filterandor'])
        ? esc_attr($options['filterandor'])
        : 'and';
    $exclude_any_all = isset($options['excludeanyall'])
        ? esc_attr($options['excludeanyall'])
        : 'any';
    ?>
    <p>Show Tweets that contain
        <select name="<?php echo $args['option'] .
            '[includeanyall]'; ?>" id="ctf_includeanyall">
            <option value="any" <?php if ($include_any_all == "any") {
                echo 'selected="selected"';
            } ?> ><?php _e('any'); ?></option>
            <option value="all" <?php if ($include_any_all == "all") {
                echo 'selected="selected"';
            } ?> ><?php _e('all'); ?></option>
        </select>
        of the "includewords"
        <select name="<?php echo $args['option'] .
            '[filterandor]'; ?>" id="ctf_filterandor">
            <option value="and" <?php if ($filter_and_or == "and") {
                echo 'selected="selected"';
            } ?> ><?php _e('and'); ?></option>
            <option value="or" <?php if ($filter_and_or == "or") {
                echo 'selected="selected"';
            } ?> ><?php _e('or'); ?></option>
        </select>
        do not contain
        <select name="<?php echo $args['option'] .
            '[excludeanyall]'; ?>" id="ctf_excludeanyall">
            <option value="any" <?php if ($exclude_any_all == "any") {
                echo 'selected="selected"';
            } ?> ><?php _e('any'); ?></option>
            <option value="all" <?php if ($exclude_any_all == "all") {
                echo 'selected="selected"';
            } ?> ><?php _e('all'); ?></option>
        </select>
        of the "excludewords"
...
add_action(
    'ctf_admin_add_settings_sections_to_customize',
...
add_filter(
    'ctf_admin_validate_hashtag_text',
    'ctf_validate_hashtag_text',
    10,
    1
);
...
add_filter(
    'ctf_admin_validate_usertimeline_text',
...
add_filter(
    'ctf_admin_validate_include_replies',
...
function ctf_set_include_replies($new_input)
{
    if (isset($new_input) && isset($new_input['type'])) {
        if (
            isset($new_input[$new_input['type'] . '_includereplies']) &&
            $new_input[$new_input['type'] . '_includereplies'] == 'on'
        ) {
            return true;
        } else {
            return false;
...
function ctf_set_include_retweets($new_input)
{
    if (isset($new_input) && isset($new_input['type'])) {
        if (
            isset($new_input[$new_input['type'] . '_includeretweets']) &&
            $new_input[$new_input['type'] . '_includeretweets'] == 'on'
        ) {
            return true;
        } else {
            return false;
...
add_filter(
    ...
    'ctf_customize_checkbox_settings'
);
...
add_filter(
    'ctf_admin_customize_quick_links',
...
function ctf_return_style_quick_links()
{
    return [
        ...
        4 => ['text', 'Tweet Text'],
        5 => ['links', 'Links'],
        6 => ['quoted', 'Retweet Boxes'],
        7 => ['actions', 'Tweet Actions'],
        8 => ['load', 'Load More'],
    ];
}
...
function ctf_admin_hide_unrelated_notices()
{
    ...
    if (
        !isset($_GET['page']) ||
        ($_GET['page'] !== 'custom-twitter-feeds' &&
            $_GET['page'] !== 'custom-twitter-feeds-sw')
    ) {
        return;
    }
...
{
    is_plugin_active('social-wall/social-wall.php')
        ? ($ctf_sw_active = true)
        : ($ctf_sw_active = false); ?>
        ...
        <div class="ctf-sw-info">
            ...
            <a class="ctf-sw-screenshot" href="https://smashballoon.com/social-wall/demo?utm_source=plugin-pro&utm_campaign=ctf&utm_medium=sw-demo" target="_blank">
                ...
                <img src="<?php echo CTF_PLUGIN_URL .
                    'img/sw-screenshot.png'; ?>" alt="Smash Balloon Social Wall plugin screenshot showing Facebook, Instagram, Twitter, and YouTube posts combined into one wall.">
            </a>
        </div>
