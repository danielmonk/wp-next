...
class CTF_Tracking
{
    public function __construct()
    {
        ...
        add_action('ctf_usage_tracking_cron', [$this, 'send_checkin']);
    ...
    private function normalize_and_format($key, $value)
    {
        ...
        $comma_separate_counts_settings = [
            'includewords',
            'excludewords',
            'remove_by_id',
        ];
    ...
    private function get_data()
    {
        ...
        $data['usercount'] = is_array($num_users)
            ? $num_users['total_users']
            : 1;
        $data['timezoneoffset'] = date('P');

        $settings_to_send = [];
        $raw_settings = get_option('ctf_options', []);
        ...
        foreach ($raw_settings as $key => $value) {
            if (
                ...
                if (is_array($raw_settings['connected_accounts'])) {
                    $settings_to_send['connected_accounts'] = count(
                        $raw_settings['connected_accounts']
                    );
                } else {
        ...
        AND `option_name` NOT LIKE ('%\_transient\_ctf\_header%');",
            ARRAY_A
        );
    ...
    public function send_checkin(
        ...
        $usage_tracking = get_option('ctf_usage_tracking', [
            'last_send' => 0,
            'enabled' => ctf_is_pro_version(),
        ]);
        if (
            is_numeric($usage_tracking['last_send']) &&
            $usage_tracking['last_send'] > strtotime('-1 week') &&
            !$ignore_last_checkin
        ) {
            return false;
        }
        ...
        $request = wp_remote_post(
            ...
            [
                ...
                'body' => $this->get_data(),
                'user-agent' =>
                    'MI/' . CTF_VERSION . '; ' . get_bloginfo('url'),
            ]
        );
    ...
    private function tracking_allowed()
    {
        ...
        $tracking_allowed = isset($usage_tracking['enabled'])
    ...
    public function save_setting()
    {
        if (isset($_POST['ctf_usage_tracking_enable'])) {
            ...
            if (
                isset($_POST['ctf_usage_tracking_enable']) &&
                $_POST['ctf_usage_tracking_enable'] === 'on'
            ) {
