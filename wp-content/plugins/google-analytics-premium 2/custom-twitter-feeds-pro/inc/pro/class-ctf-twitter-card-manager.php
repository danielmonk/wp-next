...
class CTF_Twitter_Card_Manager
{
    ...
    {
        $existing_twitter_card_data = get_option('ctf_twitter_cards', []);
        if (!is_array($existing_twitter_card_data)) {
            $existing_twitter_card_data = [];
        }
    ...
    public static function store_card($key, $data)
    {
        ...
        if (
            is_array($existing_twitter_card_data) &&
            count($existing_twitter_card_data) > CTF_TC_LIMIT - 1
        ) {
            reset($existing_twitter_card_data);
            unset(
                $existing_twitter_card_data[key($existing_twitter_card_data)]
            );
        }
    ...
    public static function process_url_batch($items, $max = 5)
    {
        ...
        foreach ($items as $url_or_item_array) {
            if (is_array($url_or_item_array)) {
                ...
                $id = CTF_Twitter_Card_Manager::get_key_for_card(
                    $url_or_item_array
                );
                $url = $url_or_item_array;
            }
            $maybe_stored_card = CTF_Twitter_Card_Manager::maybe_stored_card(
                $id
            );
            ...
            if ($maybe_stored_card !== false) {
                if (!isset($maybe_stored_card['local'])) {
                    $maybe_stored_card[
                        'local'
                    ] = CTF_Twitter_Card_Manager::add_local_image(
                        $maybe_stored_card,
                        $id
                    );
                }
    ...
    public static function clear_all_local()
    {
        ...
        foreach ($existing_twitter_card_data as $key => $single_card) {
            $existing_twitter_card_data[
                $key
            ] = CTF_Twitter_Card_Manager::delete_local($single_card);
        }
