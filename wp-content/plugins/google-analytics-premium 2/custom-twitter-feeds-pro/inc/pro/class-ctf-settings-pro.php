...
class CTF_Settings_Pro
{
    ...
    {
        $this->feed_type_and_terms = [];
        if (!empty($this->settings['feed_types_and_terms'])) {
            foreach (
                $this->settings['feed_types_and_terms']
                as $type_then_term
            ) {
                switch ($type_then_term[0]) {
                    case 'hometimeline':
                        $this->feed_type_and_terms['hometimeline'][] = [
                            'term' => 'hometimeline',
                            'params' => [],
                        ];
                        break;
                    case 'mentionstimeline':
                        $this->feed_type_and_terms['mentionstimeline'][] = [
                            'term' => 'mentionstimeline',
                            'params' => [],
                        ];
                        break;
                    case 'search':
                        $search_str = isset($type_then_term[1])
                            ? $type_then_term[1]
                            : '';
                        $this->feed_type_and_terms['search'][] = [
                            'term' => $search_str,
                            'params' => [],
                        ];
                        break;
                    case 'hashtag':
                        $hashtag_str = isset($type_then_term[1])
                            ? $type_then_term[1]
                            : '';
                        ...
                        break;
                    case 'lists':
                        $list = isset($type_then_term[1])
                            ? $type_then_term[1]
                            : '';
                        $this->feed_type_and_terms['lists'][] = [
                            'term' => $list,
                            'params' => [],
                        ];
                        break;
                    default:
                        $screenname = isset($type_then_term[1])
                            ? $type_then_term[1]
                            : '';

                        $this->feed_type_and_terms['usertimeline'][] = [
                            'term' => $screenname,
                            'params' => [],
                        ];
                        break;
            ...
            if (!empty($this->settings['type'])) {
                switch ($this->settings['type']) {
                    ...
                    case 'mentionstimeline':
                        ...
                        break;
                    case 'search':
                        $search_str = isset($this->settings['search_text'])
                            ? $this->settings['search_text']
                            : '';
                        if (empty($search_str)) {
                            $search_str = $this->settings['feed_term']
                                ? $this->settings['feed_term']
                                : '';
                        }
                        $this->feed_type_and_terms['search'][] = [
                            'term' => $search_str,
                        ...
                        break;
                    case 'hashtag':
                        $hashtag_str = isset($this->settings['hashtag_text'])
                            ? $this->settings['hashtag_text']
                            : '';
                        if (empty($hashtag_str)) {
                            $hashtag_str = $this->settings['feed_term']
                                ? $this->settings['feed_term']
                                : '';
                        }
                        $hashtag_str = str_replace(',', ' OR ', $hashtag_str);
                        ...
                        break;
                    case 'lists':
                        $lists_str = isset($this->settings['lists_id'])
                            ? $this->settings['lists_id']
                            : '';
                        if (empty($lists_str)) {
                            $lists_str = $this->settings['feed_term']
                                ? $this->settings['feed_term']
                                : '';
                        }
                        ...
                        break;
                    default:
                        $screennames_str = isset($this->settings['screenname'])
                            ? $this->settings['screenname']
                            : $this->settings['usertimeline_text'];
