...
class CTF_Resizer
{
    ...
    public function __construct(
        ...
        $this->image_sizes = apply_filters(
            'ctf_resized_image_sizes',
            $image_sizes
        );
    ...
    public function do_resizing()
    {
        ...
        foreach ($this->posts as $post) {
            ...
            $in_resizing_array = in_array(
                $twitter_api_id,
                $this->post_ids_need_resizing,
                true
            );
            if (
                $in_resizing_array &&
                ...
                if (!$single_post->exists_in_posts_table()) {
                    ...
                    $single_post->resize_and_save_image(
                        $this->image_sizes,
                        $this->upload_dir
                    );
    ...
    public function single_resize($url, $new_file_name, $new_size)
    {
        ...
        if (!is_wp_error($image_editor)) {
            ...
            $full_file_name =
                trailingslashit($this->upload_dir) . $new_file_name;
    ...
    public static function get_resized_image_data_for_set(
        $ids_or_feed_id,
        $args = []
    ) {
        global $wpdb;
    ...
    public static function delete_least_used_image()
    {
        ...
        $oldest_posts = $wpdb->get_results(
            "SELECT * FROM $posts_table_name ORDER BY last_requested ASC LIMIT 1",
            ARRAY_A
        );
        ...
        foreach ($oldest_posts as $post) {
            ...
            foreach ($image_sizes as $image_size) {
                ...
                foreach ($image_source_set as $image_file_to_resize) {
                    if ($i < 4) {
                        foreach (
                            $image_file_to_resize
                            as $resolution => $image_url
                        ) {
                            $suffix = $image_size;
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM $posts_table_name WHERE id = %d",
                    $post['id']
                )
            );
            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM $feeds_posts_table_name WHERE record_id = %d",
                    $post['id']
                )
            );
        }
    ...
    public static function max_resizing_per_time_period_reached()
    {
        ...
        $num_new_records = $wpdb->get_var(
            "SELECT COUNT(*) FROM $table_name WHERE created_on > '$fifteen_minutes_ago'"
        );
    ...
    public function image_resizing_disabled()
    {
        ...
        if (!$disable_resizing) {
            $disable_resizing = isset($this->resizing_tables_exist)
                ? !$this->resizing_tables_exist
                : !$this->does_resizing_tables_exist();
        }
    ...
    public function does_resizing_tables_exist()
    {
        ...
        $feeds_posts_table_name = esc_sql(
            $wpdb->prefix . CTF_FEEDS_POSTS_TABLE
        );
