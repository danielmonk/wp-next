...
class CTF_Display_Elements_Pro
{
    ...
    {
        $alt = !empty(CTF_Parse_Pro::get_twitter_card_image_alt($data))
            ? CTF_Parse_Pro::get_twitter_card_image_alt($data)
            : __('Image for twitter card', 'social-wall');
        $available_images = '';
        $local = CTF_Parse_Pro::get_twitter_card_local($data);
        if (
            !empty($local) ||
            CTF_GDPR_Integrations::doing_gdpr(ctf_get_database_settings())
        ) {
            $image = trailingslashit(CTF_PLUGIN_URL) . 'img/placeholder.png';
            $available_images =
                ' data-available-images="' .
                esc_attr(json_encode($local)) .
                '"';
        } else {
    ...
    public static function get_twitter_card_html($parts)
    {
        ...
        $display_url_parts = explode(
            "/",
            preg_replace("(^https?://)", "", $twitter_card_url)
        );
        ...
        if (
            ...
            $card_html .=
                '<a href="' .
                esc_url($twitter_card_url) .
                '" class="ctf-twitter-card ctf-tc-type-' .
                esc_attr($type) .
            ...
            if (!empty($image_url)) {
                ...
                $card_html .=
                    '<div class="ctf-tc-image" data-bg="' .
                    esc_url($image_url) .
                    '">';
                $card_html .= CTF_Display_Elements_Pro::get_twitter_card_media_html(
                    $parts
                );
                $card_html .= '</div>';
            }
            ...
            $card_html .=
                '<p class="ctf-tc-heading">' . esc_html($title) . '</p>';
            $card_html .=
                '<p class="ctf-tc-desc">' .
                esc_html(substr($description, 0, 150));
            ...
            if (isset($image_url)) {
                ...
                $card_html .=
                    '<a href="' .
                    esc_url($twitter_card_url) .
                    '" data-ctf-lightbox="1" data-title="' .
                    esc_attr($description) .
                    '" data-user="" data-name="" data-id="" data-url="" data-avatar="" data-date="" data-video="" data-iframe="' .
                    esc_url($twitter_card_url) .
    ...
    public static function get_twitter_card_placeholder(
        $post,
        $first_url,
        $short_url = ''
    ) {
        return '<span class="ctf-twitter-card-placeholder" data-tweet-id="' .
            esc_attr(CTF_Parse_Pro::get_tweet_id($post)) .
            '" data-tc-url="' .
            esc_attr($first_url) .
            '" data-tc-short-url="' .
            esc_attr($short_url) .
    ...
    public static function media_alt_text($post)
    {
        if (isset($post['text']) && !empty($post['text'])) {
            ...
            $alt_text_for_image =
                __('Image for the Tweet beginning', 'custom-twitter-feeds') .
                ': ' .
                implode(" ", array_splice($alt_text_for_image, 0, 5));
        } else {
    ...
    {
        $ctf_lightbox_image = isset($medium['poster'])
            ? $medium['poster']
            : $medium['url'];

        if ($medium['type'] == 'iframe') {
            $ctf_lightbox_image =
                trailingslashit(CTF_PLUGIN_URL) . 'img/video-lightbox.png';
        }

        return $ctf_lightbox_image;
    }

    public static function media_wrap_classes($medium, $feed_options = [])
    {
        $classes = '';
        if ($medium['type'] == 'iframe') {
            $classes = isset($medium['iframe_type'])
                ? ' ctf-if-type-' . $medium['iframe_type']
                : '';
            $classes .= isset($medium['iframe_class'])
                ? $medium['iframe_class']
                : '';
    ...
    {
        $media_link_classes = $disablelightbox
            ? 'ctf-media-link'
            : 'ctf-lightbox-link';
        if (!$disablelightbox) {
            if (
                ...
                $media_link_classes .=
                    ' ctf-video ctf-video-type-' . $medium['type'];
    ...
    public static function media_link_attributes(
        $medium,
        $post,
        $feed_options,
        $disablelightbox
        ...
        if (!$disablelightbox) {
            $media_link_atts = [
                ...
                'data-date' => ctf_get_formatted_date(
                    $post['created_at'],
                    $feed_options,
    ...
    public static function media_screenreader_text($medium)
    {
        if ($medium['type'] == 'video' || $medium['type'] == 'animated_gif') {
            return '<span class="ctf-screenreader">' .
                esc_html__('Twitter feed video.', 'custom-twitter-feeds') .
                '</span>';
        } elseif ($medium['type'] == 'iframe') {
            return '<span class="ctf-screenreader">' .
                esc_html__('Twitter feed video.', 'custom-twitter-feeds') .
        ...
        return '<span class="ctf-screenreader">' .
            esc_html__('Twitter feed image.', 'custom-twitter-feeds') .
            '</span>';
    }
    ...
    public static function media_element_atts($medium, $feed_options)
    {
        ...
        if ($feed_options['autores'] === true) {
            $sizes_string =
                isset($medium['sizes']) && is_array($medium['sizes'])
                    ? $medium['sizes']['thumb']['w'] .
                        ',' .
                        $medium['sizes']['small']['w'] .
                        ',' .
                        $medium['sizes']['medium']['w'] .
                        ',' .
                        $medium['sizes']['large']['w']
                    : '';
            $data_sizes =
                isset($medium['sizes']) && is_array($medium['sizes'])
                    ? ' data-ctfsizes="' . esc_attr($sizes_string) . '"'
                    : $data_sizes;
        }
    ...
    public static function get_cols_atts($feed_options)
    {
        ...
        if ($feed_options['layout'] === 'masonry') {
            if (
                $feed_options['masonrycols'] > 3 &&
                $feed_options['masonrycols'] < 7
            ) {
    ...
    public static function linkbox_link_attributes(
        $medium,
        $post,
        $feed_options,
        $disablelightbox
    ...
    public static function header_type($feed_options)
    {
        ...
        if (
            ...
            $feed_options['type'] === 'mentionstimeline' ||
            $feed_options['type'] === 'hometimeline'
        ) {
    ...
    public static function media_classes($media)
    {
        ...
        $media_classes .=
            $media && count($media) > 1 ? ' ctf-tweet-media-masonry' : '';
    ...
    public static function load_more_style_att($feed_options)
    {
        $load_more_styles = '';
        if (
            (!empty($feed_options['buttoncolor']) &&
                $feed_options['buttoncolor'] !== '#') ||
            (!empty($feed_options['buttontextcolor']) &&
    ...
    public static function get_ajax_code($feed_options)
    {
        ...
        return '<script> var ctfOptions = ' .
            ctf_json_encode($json_array) .
