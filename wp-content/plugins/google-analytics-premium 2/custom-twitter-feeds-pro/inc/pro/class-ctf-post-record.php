...
class CTF_Post_Record
{
    ...
    public function __construct($api_data, $feed_id)
    {
        ...
        $feed_id_match = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM $table_name WHERE twitter_id = %s LIMIT 1",
                $this->api_post_id
            ),
            ARRAY_A
        );
        ...
        $this->images_done =
            !empty($feed_id_match) && isset($feed_id_match[0]['images_done'])
                ? $feed_id_match[0]['images_done'] === '1'
                : 0;
        ...
        foreach ($image_sizes as $image_size) {
            ...
            foreach ($image_source_set as $image_file_to_resize) {
                if ($i < 4) {
                    ...
                    $largest_resolution_image_file =
                        $image_file_to_resize[$largest_resolution];

                    $suffix = $image_size;

                    $this_image_file_name =
                        $new_file_name . '-' . $i . '-' . $suffix . '.jpg';
     ...
     * @since 2.0/4.0
     */
    public function update_db_data(
        $update_last_requested = true,
        $transient_name = false,
        $image_sizes,
        $upload_dir,
        $upload_url,
        $timestamp_for_update = false
    ) {
        /*
		if ( empty( $this->db_id ) ) {
			return false;
...
*/
    ...
    public function exists_in_feeds_posts_table()
    {
        ...
        $results = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT feed_id FROM $table_name WHERE id = %s AND feed_id = %s LIMIT 1",
                $db_id,
                $this->feed_id
            ),
            ARRAY_A
    ...
    private function get_db_data()
    {
        $db_data = [
            ...
            'time_stamp' => date(
                'Y-m-d H:i:s',
                CTF_Parse_Pro::get_timestamp($this->api_data)
            ),
