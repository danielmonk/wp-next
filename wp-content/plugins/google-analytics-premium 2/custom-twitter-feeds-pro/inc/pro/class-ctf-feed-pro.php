...
class CTF_Feed_Pro
{
    ...
    public function __construct($transient_name)
    {
        ...
        $header_transient_name = str_replace(
            'ctf_',
            'ctf_header_',
            $transient_name
        );
    ...
    public function regular_cache_exists()
    {
        ...
        if (strpos($this->regular_feed_transient_name, 'ctf_!') !== false) {
            ...
            $transient_exists = get_transient(
                $this->regular_feed_transient_name
            );
        }
     ...
     * @since 2.0/5.0
     */
    public function set_post_data_from_cache($atts = [])
    {
        if (strpos($this->regular_feed_transient_name, 'ctf_!') !== false) {
        ...
        if ($transient_data) {
            $post_data = isset($transient_data['data'])
                ? $transient_data['data']
                : [];
            if (empty($post_data)) {
                $post_data = isset($transient_data[0]['id_str'])
                    ? $transient_data
                    : [];
            }
            $this->post_data = $post_data;
            $this->next_pages = isset($transient_data['pagination'])
                ? $transient_data['pagination']
                : [];
    ...
    {
        $num_existing_posts = is_array($this->post_data)
            ? count($this->post_data)
            : 0;
        $num_needed_for_page = (int) $num + (int) $offset;

        $num_existing_posts < $num_needed_for_page
            ? $this->add_report('need more posts')
            : $this->add_report('have enough posts');
    ...
    public function can_get_more_posts()
    {
        ...
        $max_concurrent_api_calls_not_met =
            $this->num_api_calls < $this->max_api_calls;
        $max_concurrent_api_calls_not_met
            ? $this->add_report('max conccurrent requests not met')
            : $this->add_report('max concurrent met');
        $one_type_and_term_has_more_ages
            ? $this->add_report('more pages available')
            : $this->add_report('no next page');
            ...
            foreach ($terms as $term_and_params) {
                ...
                if (isset($raw_tweet_data['errors'][0])) {
                    ...
                    $api_obj->api_error_no =
                        $raw_tweet_data['errors'][0]['code'];
                    $api_obj->api_error_message =
                        $raw_tweet_data['errors'][0]['message'];
                    $this->add_report(
                        'error ' .
                            $api_obj->api_error_no .
                            ' ' .
                    ...
                    if (count($raw_tweet_data) > 0) {
                        if (
                            isset($raw_tweet_data[count($raw_tweet_data) - 1])
                        ) {
                            $last_tweet =
                                $raw_tweet_data[count($raw_tweet_data) - 1];
                        ...
                        $working_tweet_set = CTF_Feed_Pro::reduceTweetSetData(
                            $raw_tweet_data
                        );
                        $working_tweet_set = CTF_Feed_Pro::filterTweetSet(
                            $working_tweet_set,
                            $settings
                        );
                        ...
                        if (count($working_tweet_set) > 1) {
                            $last_tweet =
                                $raw_tweet_data[count($working_tweet_set) - 1];
                        }
                        $next_pages[
                            $term . '_' . $type
                        ] = CTF_Parse_Pro::get_tweet_id($last_tweet);
    ...
    public static function reduceTweetSetData($tweets)
    {
        ...
        foreach ($tweets as $tweet) {
            ...
            if (isset($tweet['retweeted_status'])) {
                $trimmed_tweet['retweeted_status']['user']['name'] =
                    $tweet['retweeted_status']['user']['name'];
                $trimmed_tweet['retweeted_status']['user']['screen_name'] =
                    $tweet['retweeted_status']['user']['screen_name'];
                $trimmed_tweet['retweeted_status']['user']['verified'] =
                    $tweet['retweeted_status']['user']['verified'];
                $trimmed_tweet['retweeted_status']['user'][
                    'profile_image_url_https'
                ] =
                    $tweet['retweeted_status']['user'][
                        'profile_image_url_https'
                    ];
                $trimmed_tweet['retweeted_status']['user']['utc_offset'] =
                    $tweet['retweeted_status']['user']['utc_offset'];
                $trimmed_tweet['retweeted_status']['text'] = isset(
                    $tweet['retweeted_status']['text']
                )
                    ? $tweet['retweeted_status']['text']
                    : $tweet['retweeted_status']['full_text'];
                $trimmed_tweet['retweeted_status']['id_str'] =
                    $tweet['retweeted_status']['id_str'];
                $trimmed_tweet['retweeted_status']['created_at'] =
                    $tweet['retweeted_status']['created_at'];
                $trimmed_tweet['retweeted_status']['retweet_count'] =
                    $tweet['retweeted_status']['retweet_count'];
                $trimmed_tweet['retweeted_status']['favorite_count'] =
                ...
                if (isset($tweet['retweeted_status']['entities']['urls'][0])) {
                    foreach (
                        ...
                        $trimmed_tweet['retweeted_status']['entities'][
                            'urls'
                        ][] = [
                            'url' => $url['url'],
                            'expanded_url' => $url['expanded_url'],
                ...
                if (isset($tweet['retweeted_status']['quoted_status'])) {
                    $trimmed_tweet['retweeted_status']['quoted_status']['user'][
                        'name'
                    ] =
                        $tweet['retweeted_status']['quoted_status']['user'][
                            'name'
                        ];
                    $trimmed_tweet['retweeted_status']['quoted_status']['user'][
                        'screen_name'
                    ...
                    $trimmed_tweet['retweeted_status']['quoted_status'][
                        'text'
                    ] = isset(
                    ...
                    $trimmed_tweet['retweeted_status']['quoted_status'][
                        'id_str'
                    ] = $tweet['retweeted_status']['quoted_status']['id_str'];
                    $trimmed_tweet['retweeted_status']['quoted_status'][
                        'created_at'
                    ] =
                        $tweet['retweeted_status']['quoted_status'][
            ...
            if (isset($tweet['quoted_status'])) {
                $trimmed_tweet['quoted_status']['user']['name'] =
                    $tweet['quoted_status']['user']['name'];
                $trimmed_tweet['quoted_status']['user']['screen_name'] =
                    $tweet['quoted_status']['user']['screen_name'];
                $trimmed_tweet['quoted_status']['user'][
                    'profile_image_url_https'
                ] = $tweet['quoted_status']['user']['profile_image_url_https'];
                $trimmed_tweet['quoted_status']['user']['verified'] =
                    $tweet['quoted_status']['user']['verified'];
                $trimmed_tweet['quoted_status']['text'] = isset(
                    $tweet['quoted_status']['text']
                )
                    ? $tweet['quoted_status']['text']
                    : $tweet['quoted_status']['full_text'];
                $trimmed_tweet['quoted_status']['id_str'] =
                    $tweet['quoted_status']['id_str'];
                $trimmed_tweet['quoted_status']['created_at'] =
                ...
                if (isset($tweet['quoted_status']['entities']['urls'][0])) {
                    foreach (
                        ...
                        $trimmed_tweet['quoted_status']['entities'][
                            'urls'
                        ][] = [
                            'url' => $url['url'],
                            'expanded_url' => $url['expanded_url'],
            ...
            if (isset($tweet['in_reply_to_screen_name'])) {
                $trimmed_tweet['in_reply_to_screen_name'] =
                    $tweet['in_reply_to_screen_name'];
                $trimmed_tweet['entities']['user_mentions'][0]['name'] = isset(
                    $tweet['entities']['user_mentions'][0]['name']
                )
                    ? $tweet['entities']['user_mentions'][0]['name']
                    : '';
                $trimmed_tweet['in_reply_to_status_id_str'] =
            ...
            if (isset($tweet['extended_entities']['media'])) {
                ...
                if (isset($tweet['extended_entities']['media'][0]['url'])) {
                    $trimmed_tweet['text'] = CTF_Feed_Pro::removeStringFromText(
                ...
                for ($i = 0; $i < $num_media; $i++) {
                    $trimmed_tweet['extended_entities']['media'][$i][
                        'media_url_https'
                    ] =
                        $tweet['extended_entities']['media'][$i][
                            'media_url_https'
                        ];
                    $trimmed_tweet['extended_entities']['media'][$i]['type'] =
                        $tweet['extended_entities']['media'][$i]['type'];
                    if (
                        isset($tweet['extended_entities']['media'][$i]['sizes'])
                    ) {
                        $trimmed_tweet['extended_entities']['media'][$i][
                            'sizes'
                    ...
                    if (
                        ...
                        $trimmed_tweet['extended_entities']['media'][$i][
                            'video_info'
                        ]['variants'][0]['url'] = $preferred_variant;
                        $trimmed_tweet['extended_entities']['media'][$i][
                            'video_info'
                        ]['variants'][0]['bitrate'] = $highest_bitrate;
                    }
                }
                ...
                for ($i = 0; $i < $num_media; $i++) {
                    $trimmed_tweet['entities']['media'][$i]['media_url_https'] =
                        $tweet['entities']['media'][$i]['media_url_https'];
                    $trimmed_tweet['entities']['media'][$i]['type'] =
                        $tweet['entities']['media'][$i]['type'];
                    if (isset($tweet['entities']['media'][$i]['sizes'])) {
                        $trimmed_tweet['entities']['media'][$i]['sizes'] =
                    ...
                    if (
                        ...
                        $trimmed_tweet['entities']['media'][$i]['video_info'][
                            'variants'
                        ][0]['url'] = $preferred_variant;
                        $trimmed_tweet['entities']['media'][$i]['video_info'][
                            'variants'
                        ][0]['bitrate'] = $highest_bitrate;
                    }
                }
            }

            if (
                isset($tweet['retweeted_status']['extended_entities']['media'])
            ) {
                // if there is media, we need to remove the media url from the tweet text
                ...
                if (
                    ...
                    $trimmed_tweet['retweeted_status'][
                        'text'
                    ] = CTF_Feed_Pro::removeStringFromText(
                        $tweet['retweeted_status']['extended_entities'][
                            'media'
                        ][0]['url'],
                        $retweeted_text
                ...
                for ($i = 0; $i < $num_media; $i++) {
                    $trimmed_tweet['retweeted_status']['extended_entities'][
                        'media'
                    ][$i]['media_url_https'] =
                        $tweet['retweeted_status']['extended_entities'][
                            'media'
                ...
                for ($i = 0; $i < $num_media; $i++) {
                    $trimmed_tweet['retweeted_status']['entities']['media'][$i][
                        'media_url_https'
                    ] =
                    ...
                    if (
                        ...
                        $trimmed_tweet['retweeted_status']['entities']['media'][
                            $i
                        ]['sizes'] =
                            $tweet['retweeted_status']['entities']['media'][$i][
                                'sizes'
                ...
                for ($i = 0; $i < $num_media; $i++) {
                    $trimmed_tweet['quoted_status']['extended_entities'][
                        'media'
                    ][$i]['media_url_https'] =
                        $tweet['quoted_status']['extended_entities']['media'][
                            $i
                ...
                for ($i = 0; $i < $num_media; $i++) {
                    $trimmed_tweet['quoted_status']['entities']['media'][$i][
                        'media_url_https'
                    ] =
            ...
            if (isset($tweet['quoted_status'])) {
                ...
                if (
                    ...
                    $text = isset($trimmed_tweet['full_text'])
                        ? $trimmed_tweet['full_text']
                        : $trimmed_tweet['text'];
                    $trimmed_tweet['text'] = CTF_Feed_Pro::removeStringFromText(
                        $tweet['entities']['urls'][$maybe_remove_index]['url'],
                        $text
            ...
            if (isset($tweet['entities']['urls'][0]['expanded_url'])) {
                $trimmed_tweet['entities']['urls'][0]['expanded_url'] =
                    $tweet['entities']['urls'][0]['expanded_url'];

                $twitter_card = CTF_Feed_Pro::maybeGetTwitterCardData(
                    $tweet['entities']['urls'][0]['expanded_url'],
                    $trimmed_tweet['id_str']
                );
                ...
                if (!empty($twitter_card)) {
                    ...
                    if (
                        !empty($twitter_card["twitter:card"]) &&
            ...
            if (
                ...
                $trimmed_tweet['retweeted_status']['entities']['urls'][0][
                    'expanded_url'
                ] =
                    $tweet['retweeted_status']['entities']['urls'][0][
                        'expanded_url'
    ...
    public static function filterTweetSet($tweet_set, $settings)
    {
        ...
        foreach ($tweet_set as $tweet) {
            ...
            if (!empty($retweet_id) && !$settings['includeretweets']) {
                ...
                in_array($tweet['id_str'], $ids_of_tweets_to_remove)
            ) {
                $keep = false;
    ...
    {
        $return = false;
        $good_text = !empty($settings['includewords'])
            ? explode(',', str_replace(' ', '', $settings['includewords']))
            : '';
        $bad_text = !empty($settings['excludewords'])
            ? explode(',', str_replace(' ', '', $settings['excludewords']))
            : '';
        ...
        if (!empty($good_text) || !empty($bad_text)) {
            if (
                ...
                if (
                    CTF_Feed_Pro::hasGoodText(
                        $good_text,
                        $includewords_any_all,
                        $tweet_text,
                        true
                    ) &&
                    CTF_Feed_Pro::hasNoBadText(
                        $bad_text,
                        'any',
                        $tweet_text,
                        true
    ...
    public static function hasGoodText(
        $good_text,
        $any_or_all,
        $tweet_text,
        $default
     ...
     */
    public static function hasNoBadText(
        $bad_text,
        $any_or_all,
        $tweet_text,
        $default
    ...
    public static function removeStringFromText(
        $string,
        $text,
        $expanded_url = ''
    ) {
    ...
    public static function maybeGetTwitterCardData($url, $id)
    {
        ...
        $tc_data = get_option('ctf_twitter_cards', []);
        ...
        if ($card && !isset($card['local'])) {
            $card['local'] = CTF_Twitter_Card_Manager::add_local_image(
                $card,
                $id
            );
        }
    ...
    {
        $merged_posts = [];
        $settings['sortby'] = isset($settings['sortby'])
            ? $settings['sortby']
            : 'date';
        ...
        if ($settings['sortby'] === 'alternate') {
            ...
            if (isset($post_sets[1])) {
                ...
                $merged_posts = isset($post_sets[0]) ? $post_sets[0] : [];
            }
    ...
    public function set_remote_header_data($settings, $feed_types_and_terms)
    {
        ...
        $get_fields = $this->setGetFieldsArray(
            $endpoint,
            $settings['screenname'],
            $settings
        );
        ...
        $header_json = isset($request_results->json)
    ...
    public function cache_feed_data($cache_time, $save_backup = true)
    {
        if (!empty($this->post_data) || !empty($this->next_pages)) {
            ...
            set_transient(
                $this->regular_feed_transient_name,
                wp_json_encode($to_cache),
                $cache_time
            );

            if ($save_backup) {
                update_option(
                    $this->backup_feed_transient_name,
                    wp_json_encode($to_cache),
                    false
                );
            }
    ...
    public function cache_header_data($cache_time, $save_backup = true)
    {
        if ($this->header_data) {
            set_transient(
                $this->header_transient_name,
                wp_json_encode($this->header_data),
                $cache_time
            );

            if ($save_backup) {
                update_option(
                    $this->backup_header_transient_name,
                    wp_json_encode($this->header_data),
                    false
                );
            }
    ...
    public function should_use_pagination($settings, $offset = 0)
    {
        ...
        $posts_available =
            count($this->post_data) - ($offset + $settings['minnum']);
        $show_loadmore_button_by_settings =
            ($settings['showbutton'] == 'on' ||
     ...
     * @return object
     */
    public function make_api_connection(
        $connected_account_or_page,
        $type = null,
        $params = null
    ) {
        return new SB_Instagram_API_Connect(
            $connected_account_or_page,
            $type,
            $params
        );
     ...
     * @since 2.0/5.0
     */
    protected function filter_posts($post_set, $settings = [])
    {
