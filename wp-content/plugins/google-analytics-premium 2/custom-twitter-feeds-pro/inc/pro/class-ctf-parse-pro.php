...
class CTF_Parse_Pro
{
    ...
    public static function get_quoted_user_name($data)
    {
        if (isset($data['quoted_status'])) {
            ...
            return $data['retweeted_status']['quoted_status']['user'][
                'screen_name'
            ];
        }
        return '';
    ...
    public static function get_quoted_text(
        $data,
        $replace_urls = true,
        $has_card = false
    ) {
    ...
    public static function get_quoted_timestamp($data)
    {
        ...
        if (isset($data['retweeted_status']['quoted_status'])) {
            return strtotime(
                $data['retweeted_status']['quoted_status']['created_at']
            );
        }
        return '';
    }
    ...
    public static function get_quoted_media_url($data, $resolution = 'lightbox')
    {
        if (isset($data['retweeted_status'])) {
            if (
                isset(
                    $data['retweeted_status']['quoted_status'][
                        'extended_entities'
                    ]['media'][0]['media_url_https']
                )
            ) {
                return $data['retweeted_status']['quoted_status'][
                    'extended_entities'
                ]['media'][0]['media_url_https'];
            }
        } else {
            if (
                ...
                return $data['quoted_status']['extended_entities']['media'][0][
                    'media_url_https'
                ];
            }
        }
    ...
    public static function get_media_type($data, $resolution = 'lightbox')
    {
        ...
        if (isset($working_tweet['extended_entities']['media'])) {
            if (
                $working_tweet['extended_entities']['media'][0]['type'] ==
                    'video' ||
                $working_tweet['extended_entities']['media'][0]['type'] ==
                    'animated_gif'
            ) {
            ...
            if (
                $working_tweet['entities']['media'][0]['type'] == 'video' ||
                $working_tweet['entities']['media'][0]['type'] == 'animated_gif'
            ) {
                $type = 'video';
            }
    ...
    public static function get_media_url($data, $resolution = 'lightbox')
    {
        if (isset($data['retweeted_status'])) {
            if (
                ...
                return $data['retweeted_status']['extended_entities'][
                    'media'
                ][0]['media_url_https'];
            }
        } else {
            if (
                isset($data['extended_entities']['media'][0]['media_url_https'])
            ) {
    ...
    public static function get_video_url($data)
    {
        ...
        $type = isset($working_tweet['extended_entities'])
            ? $working_tweet['extended_entities']['media'][0]['type']
            : $working_tweet['entities']['media'][0]['type'];
        if ($type === 'video' || $type === 'animated_gif') {
            $url = isset($working_tweet['extended_entities'])
                ? $working_tweet['extended_entities']['media'][0]['video_info'][
                    'variants'
                ][0]['url']
                : $working_tweet['entities']['media'][0]['video_info'][
                    'variants'
                ][0]['url'];
        }
    ...
    public static function get_tweet_content(
        $data,
        $replace_urls = true,
        $has_card = false
    ) {
    ...
    public static function get_first_url_in_tweet($data)
    {
        ...
        if (isset($working_tweet['extended_entities']['urls'])) {
            return $working_tweet['extended_entities']['urls'][0][
                'expanded_url'
            ];
    ...
    public static function get_media_classes($data, $feed_options)
    {
        ...
        $media_classes .=
            CTF_Parse_Pro::get_media_count($data) > 1
                ? ' ctf-tweet-media-masonry'
                : '';
    ...
    {
        $data['twitter_card'] = isset($data['twitter_card'])
            ? $data['twitter_card']
            : $data;
    ...
    public static function get_media_src_set($medium_or_api_data)
    {
        if (isset($medium_or_api_data['id_str'])) {
            ...
            if (isset($api_data['quoted_status'])) {
                $quoted_media_items = CTF_Parse_Pro::get_media_items_for_tweet(
                    $api_data['quoted_status']
                );
        ...
        foreach ($media_items as $medium) {
            ...
            if (empty($medium['sizes']) && !empty($this_media_url)) {
    ...
    public static function get_feed_classes(
        $feed_options,
        $check_for_duplicates
    ) {
        $ctf_feed_classes =
            'ctf ctf-type-' . CTF_Parse_Pro::get_feed_type($feed_options);
        ...
        $ctf_feed_classes .= $feed_options['width_mobile_no_fixed']
            ? ' ctf-width-resp'
            : '';
        if ($feed_options['autoscroll']) {
            $ctf_feed_classes .= ' ctf-autoscroll';
        }
        if ($check_for_duplicates) {
            $ctf_feed_classes .= ' ctf-no-duplicates';
        }
        if ($feed_options['persistentcache']) {
            $ctf_feed_classes .= ' ctf-persistent';
        }
        if ($feed_options['font_method'] === 'fontfile') {
            $ctf_feed_classes .= ' ctf-fontfile';
        }
    ...
    public static function get_header_avatar($data, $feed_options = [])
    {
        if (CTF_GDPR_Integrations::doing_gdpr($feed_options)) {
            $avatar = ctf_get_local_avatar(
    ...
    public static function get_user_header_json($data)
    {
        ...
        if (!$header_json) {
            ...
            $CtfFeedPros = new CtfFeedPro([], null, null);

            $get_fields = $CtfFeedPros->setGetFieldsArray(
                $endpoint,
                $data['screenname']
            );
    ...
    public static function get_item_classes($data, $feed_options, $i)
    {
        ...
        $tweet_classes =
            'ctf-item ctf-author-' .
            CTF_Parse_Pro::get_author_screen_name($data) .
            ' ctf-new';
        ...
        if (
            isset($data['in_reply_to_screen_name']) &&
            $data['in_reply_to_screen_name'] !== $data['user']['screen_name']
        ) {
    ...
    public static function get_generic_header_text($data)
    {
        if ($data['type'] === 'search' || $data['type'] === 'hashtag') {
            ...
            $default_header_text = str_replace(
                ' -filter:retweets',
                '',
                $default_header_text
            );
        ...
        if (!empty($data['feed_types_and_terms'])) {
            if ($data['headertext'] != '') {
                ...
                if (
                    $data['feed_types_and_terms'][0][0] === 'search' ||
                    $data['feed_types_and_terms'][0][0] === 'hashtag'
                ) {
    ...
    public static function get_retweeter_name($data, $i)
    {
        ...
        if (isset($data[$i]['retweeted_status'])) {
            $data = $data[$i];
            $twitter_card_retweeted = isset($data['twitter_card'])
                ? $data['twitter_card']
                : false;
            $retweeter = [
                'name' => $data['user']['name'],
    ...
    public static function get_media($data, $num_media = false)
    {
        ...
        if (isset($data['extended_entities']['media'])) {
            ...
            for ($ii = 0; $ii < $num_media; $ii++) {
                if (
                    $data['extended_entities']['media'][$ii]['type'] ==
                        'video' ||
                    $data['extended_entities']['media'][$ii]['type'] ==
                        'animated_gif'
                ) {
                    $media[$ii]['url'] =
                        $data['extended_entities']['media'][$ii]['video_info'][
                            'variants'
                        ][$ii]['url'];
                } else {
                    $media[$ii]['url'] = isset(
                        $data['extended_entities']['media'][$ii][
                            'media_url_https'
                        ]
                    )
                        ? $data['extended_entities']['media'][$ii][
                ...
                $media[$ii]['poster'] = isset(
                    $data['extended_entities']['media'][$ii]['media_url_https']
                )
                    ? $data['extended_entities']['media'][$ii][
                        'media_url_https'
                    ]
                ...
                if ($media[$ii]['type'] == 'photo') {
                    $media[$ii]['sizes'] = isset(
                        $data['extended_entities']['media'][$ii]['sizes']
                    )
                        ? $data['extended_entities']['media'][$ii]['sizes']
                        : '';
                }
            ...
            for ($ii = 0; $ii < $num_media; $ii++) {
                if (
                    $data['entities']['media'][$ii]['type'] == 'video' ||
                    $data['entities']['media'][$ii]['type'] == 'animated_gif'
                ) {
                    $media[$ii]['url'] =
                        $data['entities']['media'][$ii]['video_info'][
                            'variants'
                        ][$ii]['url'];
                } else {
                    $media[$ii]['url'] =
                        $data['entities']['media'][$ii]['media_url_https'];
                }
        ...
        if ($twitter_card_url) {
            ...
            if ($youtube || $youtubeembed || $youtu) {
                if ($youtube || $youtubeembed) {
                    ...
                    if (
                        preg_match("/v=([^&]+)/i", $twitter_card_url, $matches)
                    ) {
                        $id = $matches[1];
                    } elseif (
                        preg_match(
...
<iframe src="https://open.spotify.com/embed/track/5H0kIgpWNZm9HF6VkU4dow?si=KvuO3MAuQmCxq8AI2QdhCw" width="300" height="380" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
            ...
            if ($spotify) {
                $type =
                    strpos($twitter_card_url, 'track/') !== false
                        ? 'track'
                        : 'album';
                $url_pieces =
                    $type === 'track'
                        ? explode('track', $twitter_card_url)
                        : explode('album', $twitter_card_url);
    ...
    {
        $atts = isset($data['video_atts']) ? $data['video_atts'] : [];
        return $atts;
    }
