...
class CTF_Twitter_Card_Generator
{
    ...
    public function __construct($url, $debug_mode = false)
    {
        ...
        $this->twitter_card_data = [];

        $this->open_graph_data = [];
    ...
    public function generate()
    {
        ...
        if (!empty($meta)) {
            $values['twitter:card'] = isset($meta['twitter:card'])
                ? sanitize_text_field($meta['twitter:card'])
                : '';
            $values['twitter:site'] = isset($meta['twitter:site'])
                ? sanitize_text_field($meta['twitter:site'])
                : '';
            $values['twitter:site:id'] = isset($meta['twitter:site:id'])
                ? sanitize_text_field($meta['twitter:site:id'])
                : '';
            $values['twitter:creator'] = isset($meta['twitter:creator'])
                ? sanitize_text_field($meta['twitter:creator'])
                : '';
            $values['twitter:creator:id'] = isset($meta['twitter:creator:id'])
                ? sanitize_text_field($meta['twitter:creator:id'])
                : '';
            $values['twitter:title'] = isset($meta['twitter:title'])
                ? $this->encode_helper($meta['twitter:title'])
                : '';
            $values['twitter:description'] = isset($meta['twitter:description'])
                ? $this->encode_helper($meta['twitter:description'])
                : '';
            $values['twitter:image'] = isset($meta['twitter:image'])
                ? esc_url(urldecode($meta['twitter:image']))
                : '';
            if (
                $values['twitter:image'] === '' &&
                isset($meta['twitter:image:src'])
            ) {
                $values['twitter:image'] = esc_url(
                    urldecode($meta['twitter:image:src'])
                );
            }
            if ($values['twitter:title'] === '' && isset($meta['og:title'])) {
                $values['twitter:title'] = $this->encode_helper(
                    $meta['og:title']
                );
            }
            if (
                ...
                $values['twitter:description'] = $this->encode_helper(
                    $meta['og:description']
                );
            }
            ...
            $values['twitter:image:alt'] = isset($meta['twitter:image:alt'])
                ? sanitize_text_field($meta['twitter:image:alt'])
                : '';

            $parsed_main = parse_url($url);
            if ($values['twitter:image'] !== '') {
                if (strpos($values['twitter:image'], 'http') === false) {
                    $start = !empty($parsed_main['scheme'])
                        ? $parsed_main['scheme']
                        : 'http';
                    $host = !empty($parsed_main['host'])
                        ? $parsed_main['host']
                        : '';
                    $values['twitter:image'] =
                        $start .
                        '://' .
                        trailingslashit($host) .
                        $values['twitter:image'];
                }
            }

            if ($values['twitter:card'] === 'player') {
                $values['twitter:player'] = isset($meta['twitter:player'])
                    ? sanitize_text_field($meta['twitter:player'])
                    : '';
            }
            ...
            if ($values['twitter:card'] === 'amplify') {
                $values['twitter:image:src'] = isset($meta['twitter:image:src'])
                    ? esc_url(urldecode($meta['twitter:image:src']))
                    : '';
                $values['twitter:amplify:teaser_segments_stream'] = isset(
                    $meta['twitter:amplify:teaser_segments_stream']
                )
                    ? sanitize_text_field(
                        $meta['twitter:amplify:teaser_segments_stream']
                    )
                    : '';
                $vmap_url = isset($meta['twitter:amplify:vmap'])
                    ? sanitize_text_field($meta['twitter:amplify:vmap'])
                    : '';
                $media_src = $this->request_amplify_card_video_source(
                    $vmap_url
                );
                $values['twitter:amplify:media:ctfsrc'] = $media_src
                    ? trim($media_src)
                    : '';
            }
            ...
            if (
                $values['twitter:card'] === '' &&
                !empty($values['twitter:image']) &&
                !empty($values['twitter:title'])
            ) {
    ...
    {
        if (
            strpos($url, 'https://www.amazon.com') !== false ||
    ...
    private function get_meta_tags_curl($url)
    {
        ...
        for ($i = 0; $i < $metas->length; $i++) {
            ...
            if (
                in_array($meta->getAttribute('name'), $twitter_card_names, true)
            ) {
    ...
    private function open_graph_data_needed()
    {
        if (!empty($this->twitter_card_data['twitter:card'])) {
            if (
                empty($this->twitter_card_data['twitter:title']) ||
                empty($this->twitter_card_data['twitter:site']) ||
                empty($this->twitter_card_data['twitter:description']) ||
                empty($this->twitter_card_data['twitter:image'])
            ) {
                return true;
            }
        }
    ...
    private function request_open_graph_data()
    {
        ...
        $values['twitter:title'] = isset($graph->title)
            ? sanitize_text_field($graph->title)
            : '';
        $values['twitter:description'] = isset($graph->description)
            ? sanitize_text_field($graph->description)
            : '';
        $values['twitter:image'] = isset($graph->image)
            ? esc_url(urldecode($graph->image))
            : '';
    ...
    private function process_twitter_card_data()
    {
        ...
        if (!empty($tc_data['twitter:image'])) {
            $tc_data['local'] = $this->create_local_image(
        ...
        if (
            $ssl_only &&
            isset($tc_data['twitter:image']) &&
            strpos($tc_data['twitter:image'], 'https') !== 0
        ) {
            $tc_data['twitter:image'] = '';
        }
    ...
    public function create_local_image($url)
    {
        ...
        foreach ($sizes as $size) {
            ...
            $generated = $resizer->single_resize(
                $url,
                $this_image_file_name,
                $size
            );
