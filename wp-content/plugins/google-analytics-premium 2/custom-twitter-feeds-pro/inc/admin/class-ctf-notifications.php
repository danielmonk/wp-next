...
class CTF_Notifications
{
    ...
    public function hooks()
    {
        ...
        add_action('ctf_admin_overview_before_title', [$this, 'output']);

        // on cron. Once a week?
        add_action('ctf_notification_update', [$this, 'update']);

        add_action('wp_ajax_ctf_dashboard_notification_dismiss', [
            $this,
            'dismiss',
        ]);
    }
    ...
    public function get_option($cache = true)
    {
        ...
        $this->option = [
            ...
            'dismissed' => !empty($option['dismissed'])
                ? $option['dismissed']
                : [],
        ];
    ...
    public function fetch_feed()
    {
        ...
        $body = str_replace(['sbi_', 'sbi-'], ['ctf_', 'ctf-'], $body);
    ...
    public function verify($notifications)
    {
        ...
        foreach ($notifications as $notification) {
            ...
            if (
                ...
                ctf_get_current_time() > strtotime($notification['end'])
            ) {
                continue;
            }
            ...
            if (
                ...
                in_array($notification['id'], $option['dismissed'])
            ) {
    ...
    public function get()
    {
        ...
        if (
            empty($option['update']) ||
            ctf_get_current_time() > $option['update'] + DAY_IN_SECONDS
        ) {
            $this->update();
        }

        $events = !empty($option['events'])
            ? $this->verify_active($option['events'])
            : [];
        $feed = !empty($option['feed'])
            ? $this->verify_active($option['feed'])
            : [];
    ...
    public function add($notification)
    {
        ...
        update_option('ctf_notifications', [
    ...
    public function enqueues()
    {
        ...
        wp_localize_script('ctf-admin-notifications', 'ctf_admin', [
    ...
    public function replace_merge_fields($content, $notification)
    {
        $merge_fields = [
            ...
            '{amount}' => isset($notification['amount'])
                ? $notification['amount']
                : '',
            '{platform}' => 'Twitter',
            '{lowerplatform}' => 'twitter',
            '{review-url}' =>
                'https://wordpress.org/support/plugin/custom-twitter-feeds/reviews/',
    ...
    public function output()
    {
        ...
        foreach ($notifications as $notification) {
            ...
            $buttons_html = '';
            if (
                !empty($notification['btns']) &&
                is_array($notification['btns'])
            ) {
                foreach ($notification['btns'] as $btn_type => $btn) {
                    ...
                    $buttons_html .= sprintf(
                        ...
                        !empty($btn['target']) && $btn['target'] === '_blank'
                ...
                $buttons_html = !empty($buttons_html)
            ...
            if (empty($notification['image'])) {
                ...
                if (
                    ...
                    $image_html .=
                        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1438 1878" fill-rule="evenodd" clip-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2">';
                    $image_html .=
                    ...
                    $img_src =
                        SBY_PLUGIN_URL .
                        'img/' .
                        sanitize_text_field($notification['image']);
                    $image_html .=
                        '<img src="' . esc_url($img_src) . '" alt="notice">';
            ...
            $notifications_html .= sprintf(
                ...
                !empty($notification['id'])
                    ? esc_attr(sanitize_text_field($notification['id']))
                    : 0,
                $current_class
            );
        ...
        <div id="ctf-notifications">
            <a class="dismiss" title="<?php echo esc_attr__(
                'Dismiss this message',
                'custom-twitter-feeds'
            ); ?>"><i class="fa fa-times-circle" aria-hidden="true"></i></a>

            <div class="navigation">
                <a class="prev disabled" title="<?php echo esc_attr__(
                    'Previous message',
                    'custom-twitter-feeds'
                ...
                <a class="next disabled" title="<?php echo esc_attr__(
                    'Next message',
                    'custom-twitter-feeds'
    ...
    public function dismiss()
    {
        ...
        if ($id === 'review') {
            ...
            $ctf_statuses_option[
                'rating_notice_dismissed'
            ] = ctf_get_current_time();
            update_option('ctf_statuses', $ctf_statuses_option, false);
        } elseif ($id === 'discount') {
            update_user_meta(
                get_current_user_id(),
                'ctf_ignore_new_user_sale_notice',
                'always'
            );
            ...
            if ($not_early_in_the_year) {
                update_user_meta(
                    get_current_user_id(),
                    'ctf_ignore_bfcm_sale_notice',
                    date('Y', ctf_get_current_time())
                );
            }
        }
