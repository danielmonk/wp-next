 ...
 */
function ctf_get_feed_template_part($part, $settings = [])
{
    $options = get_option('ctf_options', []);

    $file = '';
    $settings['customtemplates'] = isset($options['customtemplates'])
        ? $options['customtemplates'] !== 'false'
        : false;
    /**
    ...
    if ($using_custom_templates_in_theme == true) {
        ...
        $custom_header_generic_template = locate_template(
            'ctf/header-generic.php',
            false,
            false
        );
        ...
        $custom_author_template = locate_template(
            'ctf/author.php',
            false,
            false
        );
        $custom_media_template = locate_template('ctf/media.php', false, false);
        $custom_linkbox_template = locate_template(
            'ctf/linkbox.php',
            false,
            false
        );
...
function ctf_init($atts)
{
    ...
    if (
        !$twitter_feed->tweet_set ||
        $twitter_feed->missing_credentials ||
        !isset($twitter_feed->tweet_set[0]['created_at'])
    ) {
...
{
    $shortcode_data = json_decode(
        str_replace(
            ['\"', "\\'"],
            ['"', "'"],
            sanitize_text_field($_POST['shortcode_data'])
        ),
        true
    ); // necessary to unescape quotes
    ...
    $ids_to_remove = isset($_POST['ids_to_remove'])
        ? $_POST['ids_to_remove']
        : [];
    $is_pagination = empty($last_id_data) ? 0 : 1;
    $persistent_index = isset($_POST['persistent_index'])
        ? sanitize_text_field($_POST['persistent_index'])
        : '';

    include_once CTF_URL . '/inc/CtfFeed.php';
    include_once CTF_URL . '/inc/CtfFeedPro.php';

    $twitter_feed = CtfFeedPro::init(
        $shortcode_data,
        $last_id_data,
        $num_needed,
        $ids_to_remove,
    ...
    $resizer = new CTF_Resizer(
        $twitter_feed->ids_in_set_w_media,
        $twitter_feed->feedID(),
        $twitter_feed->tweet_set,
        $twitter_feed->feed_options
    );
    ...
    $feed_id = isset($_POST['feed_id'])
        ? sanitize_text_field($_POST['feed_id'])
        : 'unknown';
    $location =
        ...
        in_array(
            ...
            : 'unknown';
    $post_id =
        isset($_POST['post_id']) && $_POST['post_id'] !== 'unknown'
            ? (int) $_POST['post_id']
            : 'unknown';
    $feed_details = [
        'feed_id' => $feed_id,
    ...
    echo ctf_add_resized_image_data(
        $twitter_feed->feedID(),
        $twitter_feed->ids_in_set_w_media,
        $twitter_feed->feed_options
    );
...
function ctf_maybe_ajax_theme_html($twitter_feed, $feed_id)
{
    if ($twitter_feed->feed_options['ajax_theme']) {
        echo CTF_Display_Elements_Pro::get_ajax_code(
            $twitter_feed->feed_options
        );
    }
}
...
{
    $tweet_excludes = isset($feed_options['tweet_excludes'])
        ? $feed_options['tweet_excludes']
        : '';
    $tweet_includes = isset($feed_options['tweet_includes'])
        ? $feed_options['tweet_includes']
        : '';
...
{
    $options = get_option('ctf_options');
    $font_method = isset($options['font_method'])
        ? $options['font_method']
        : 'svg';
    ...
    $elems = [
        'fa-arrows-alt' => [
            ...
            'svg' =>
                '<svg class="svg-inline--fa fa-arrows-alt fa-w-16" aria-hidden="true" aria-label="expand" data-fa-processed="" data-prefix="fa" data-icon="arrows-alt" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M352.201 425.775l-79.196 79.196c-9.373 9.373-24.568 9.373-33.941 0l-79.196-79.196c-15.119-15.119-4.411-40.971 16.971-40.97h51.162L228 284H127.196v51.162c0 21.382-25.851 32.09-40.971 16.971L7.029 272.937c-9.373-9.373-9.373-24.569 0-33.941L86.225 159.8c15.119-15.119 40.971-4.411 40.971 16.971V228H228V127.196h-51.23c-21.382 0-32.09-25.851-16.971-40.971l79.196-79.196c9.373-9.373 24.568-9.373 33.941 0l79.196 79.196c15.119 15.119 4.411 40.971-16.971 40.971h-51.162V228h100.804v-51.162c0-21.382 25.851-32.09 40.97-16.971l79.196 79.196c9.373 9.373 9.373 24.569 0 33.941L425.773 352.2c-15.119 15.119-40.971 4.411-40.97-16.971V284H284v100.804h51.23c21.382 0 32.09 25.851 16.971 40.971z"></path></svg>',
        ],
        'fa-check-circle' => [
    ...
    if (!empty($feed_options['datecustom'])) {
        ...
        switch ($feed_options['dateformat']) {
            ...
            default:
                ...
                $ctf_minute = !empty($feed_options['mtime'])
                    ? $feed_options['mtime']
                    : 'm';
                $ctf_hour = !empty($feed_options['htime'])
                    ? $feed_options['htime']
                    : 'h';
                $ctf_now_str = !empty($feed_options['nowtime'])
                    ? $feed_options['nowtime']
                    : 'now';
                ...
                if ($difference < 60) {
                    ...
                    $one_year_from_date = new CtfDateTime(
                        $raw_date,
...
{
    $limit = is_array($feed_settings)
        ? $feed_settings['textlength']
        : $feed_settings;
    ...
    $return =
        implode(' ', $first_parts) .
        '<a href="#" class="ctf_more">...</a><span class="ctf_remaining">';
...
function ctf_background_processing()
{
    ...
    foreach ($twitter_card_batch as $twitter_card_array) {
        ...
        $parts = CTF_Display_Elements_Pro::get_twitter_card_parts(
            $url,
            $twitter_card
        );
        ...
        $twitter_return[$twitter_card_array['id']] = [
            'html' => $content,
            'url' => $url,
    ...
    // resizing
    $images_need_resizing_raw = isset($_POST['needs_resizing'])
        ? $_POST['needs_resizing']
        : [];
    ...
    $results = CTF_Resizer::get_resized_image_data_for_set(
        $images_need_resizing
    );
    $return = [];
...
add_action(
    'wp_ajax_nopriv_ctf_background_processing',
...
function ctf_do_locator()
{
    if (
        !isset($_POST['feed_id']) ||
        strpos($_POST['feed_id'], 'ctf') === false
    ) {
        die('invalid feed ID');
    }
...
function ctf_add_resized_image_data($feed_id, $ids, $settings, $page = 1)
{
    ...
    $disable_resizing = isset($settings['resizing'])
        ? $settings['resizing'] === 'disabled'
        : false;
    ...
    if ($cache) {
        ...
        $resized_data = CTF_Resizer::get_resized_image_data_for_set(
            $ids,
            $args
        );
        set_transient(
            $transient_name,
            ctf_json_encode($resized_data),
            $settings['cache_time']
        );
    ...
    $return =
        '<span class="ctf-resized-image-data" data-feedid="' .
        esc_attr($feed_id) .
        '" data-resized="' .
        esc_attr($att) .
        '">';
...
{
    $local_avatars = get_option('ctf_local_avatars', []);
    ...
    if (!$maybe_url) {
        $transient =
            $feed_options['type'] === 'usertimeline'
                ? 'ctf_header_' . $feed_options['screenname']
                : 'ctf_hometimeline_header';
        ...
        if (!$header_json) {
            ...
            if ($endpoint === 'accountlookup') {
                ...
                set_transient(
                    'ctf_header_' . $screenname,
                    $header_json,
                    60 * 60
                );
            }
        }
        $header_info = isset($header_json)
            ? json_decode($header_json, true)
            : [];
...
function ctf_create_local_avatar($screenname, $file_name)
{
    ...
    if (!is_wp_error($image_editor)) {
        ...
        $full_file_name =
            ...
            trailingslashit(CTF_UPLOADS_NAME) .
            $screenname .
            '.jpg';
...
function ctf_auto_save_tokens()
{
    if (current_user_can('edit_posts')) {
        ...
        $options['access_token_secret'] = sanitize_text_field(
...
add_action(
    'wp_ajax_ctf_clear_twitter_card_cache',
    'ctf_clear_twitter_card_cache'
);
...
function ctf_admin_database_warning()
{
    if (
        isset($_GET['page']) &&
        in_array($_GET['page'], ['custom-twitter-feeds', ''])
    ) {
        ...
        'custom-twitter-feeds'
            ...
            <p>
				<?php echo sprintf(
        __(
            'For a solution, please follow the directions %shere%s.',
            'custom-twitter-feeds'
        ),
        '<a href="https://smashballoon.com/why-does-my-database-have-a-lot-of-twitter-feed-caches/" target="_blank" rel="noopener noreferrer">',
        '</a>'
    ); ?>
            </p>
        </div>
...
function ctf_retrieve_lists_by_owner()
{
    if (current_user_can('edit_posts')) {
        $options = get_option('ctf_options');
        $consumer_key =
            !empty($options['consumer_key']) && $options['have_own_tokens']
                ? $options['consumer_key']
                : 'FPYSYWIdyUIQ76Yz5hdYo5r7y';
        $consumer_secret =
            !empty($options['consumer_secret']) && $options['have_own_tokens']
...
add_action(
    'wp_ajax_ctf_retrieve_lists_by_owner',
    'ctf_retrieve_lists_by_owner'
);
...
{
    $return = [];
    $twitter_feed_settings = new CTF_Settings_Pro([]);
    $twitter_feed_settings->set_feed_type_and_terms();
    $tw_settings = $twitter_feed_settings->get_settings();
    $tw_feed_type_and_terms = $twitter_feed_settings->get_feed_type_and_terms();
    ...
    if (
        !empty($tw_settings['access_token']) &&
        $tw_settings['access_token'] !== 'missing'
    ) {
        $connected_accounts = [
            'access_token' => true,
        ];
...
function ctf_check_for_db_updates()
{
    ...
    if (version_compare($db_ver, '1.2', '<')) {
        ...
        include_once trailingslashit(CTF_PLUGIN_DIR) .
            'inc/class-ctf-gdpr-integrations.php';
        $disable_resizing = isset($ctf_options['resizing'])
            ? $ctf_options['resizing'] === 'disabled'
            : false;

        $ctf_statuses_option = get_option('ctf_statuses', []);

        if (
            $disable_resizing ||
            !CTF_GDPR_Integrations::gdpr_tests_successful(true)
        ) {
...
{
    $options = get_option('ctf_options');
    $ctf_custom_css = isset($options['custom_css'])
        ? $options['custom_css']
        : '';

    //Show CSS if an admin (so can see Hide Tweet link), if including Custom CSS
    current_user_can('edit_posts') || !empty($ctf_custom_css)
        ? ($ctf_show_css = true)
        : ($ctf_show_css = false);
...
{
    require_once trailingslashit(CTF_PLUGIN_DIR) .
    ...
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/pro/class-ctf-display-elements-pro.php';
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/class-ctf-feed-locator.php';
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/class-ctf-gdpr-integrations.php';
    ...
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/pro/class-ctf-feed-pro.php';
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/pro/class-ctf-parse-pro.php';
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/pro/class-ctf-settings-pro.php';
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/pro/class-ctf-twitter-card-generator.php';
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/pro/class-ctf-twitter-card-manager.php';
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/pro/class-ctf-resizer.php';
    include_once trailingslashit(CTF_PLUGIN_DIR) .
        'inc/pro/class-ctf-post-record.php';
