...
class CTF_GDPR_Integrations
{
    ...
    public static function gdpr_tests_successful($retest = false)
    {
        ...
        if (!isset($ctf_statuses_option['gdpr']['image_editor']) || $retest) {
            ...
            if (!is_wp_error($image_editor)) {
                ...
                $image_editor = wp_get_image_editor(
                    'http://plugin.smashballoon.com/editor-test.png'
                );
                if (!is_wp_error($image_editor)) {
                    $ctf_statuses_option['gdpr']['image_editor'] = true;
                }
        ...
        if (
            !$ctf_statuses_option['gdpr']['upload_dir'] ||
    ...
    public static function gdpr_tests_error_message()
    {
        ...
        if (!$ctf_statuses_option['gdpr']['upload_dir']) {
            $errors[] = __(
                'A folder for storing resized images was not successfully created.'
            );
        }
        if (!$ctf_statuses_option['gdpr']['tables']) {
            $errors[] = __(
                'Tables used for storing information about resized images were not successfully created.'
            );
        }
        if (!$ctf_statuses_option['gdpr']['image_editor']) {
            $errors[] = __(
                'An image editor is not available on your server. Twitter Feed is unable to create local resized images.'
            );
        }

        if (isset($_GET['tab']) && $_GET['tab'] !== 'support') {
            $errors[] =
                '<a href="?page=custom-twitter-feeds&amp;tab=customize&amp;retest=1" class="button button-secondary">' .
                __('Retest', 'custom-twitter-feeds') .
                '</a>';
        }
