jQuery(document).ready(function ($) {
    ...
    if (!$ctfHaveOwnTokens.is(":checked")) {
      $.ajax({
        ...
        success: function (data) {
          $("#ctf_access_token").after(
            '<span class="ctf-success"><i class="fa fa-check-circle"></i> saved</span>'
          );
          $("#ctf_access_token_secret").after(
            '<span class="ctf-success"><i class="fa fa-check-circle"></i> saved</span>'
          );
        },
      });
  ...
  function toggleAccessInputs() {
    if ($ctfHaveOwnTokens.is(":checked")) {
      ...
      if (
        $ctfAccessFields.find("#ctf_access_token").val() == "" &&
        $ctfAccessFields.find("#ctf_access_token_secret").val() == ""
      ) {
  ...
  $("#ctf_search_radio")
    .siblings(".ctf-tooltip-link")
    .text("How to build a search feed");
  ...
  $("#ctf-admin .ctf-tooltip-link").on("click", function () {
    $(this)
      .closest("tr, h3, p, .ctf-tooltip-wrap")
      .find(".ctf-tooltip")
      .slideToggle();
  });
  $("#ctf-admin .ctf-multiple-tooltip-link").on("click", function () {
    $(this)
      .closest("tr, h3, .ctf-tooltip-wrap")
      .find(".ctf-multiple-tooltip")
      .slideToggle();
  });
  ...
  $("#ctf_hometimeline_radio")
    .closest("tr")
    .find(".ctf-tooltip-link")
    .css("margin-left", 1)
    .end()
    .next("tr")
    .find(".ctf-tooltip-link")
  ...
  function toggleIncludeReplies() {
    $(".ctf_include_replies_toggle").each(function () {
      if (
        $(this).closest("td").find(".ctf-feed-settings-radio").is(":checked")
      ) {
        $(this).slideDown();
      } else {
        $(this).slideUp();
    ...
    switch (e.type) {
      case "mouseenter":
        if ($(this).siblings(".ctf_shortcode").length > 0) {
          $(this)
            .attr("title", "Click for shortcode option")
            .append('<code class="ctf_shortcode_symbol">[]</code>');
        }
        break;
      case "mouseleave":
  ...
  if (typeof ctfWidth !== "undefined") {
    ...
    if (ctfWidth.length > 1 && !(ctfWidth == "100" && ctfWidthUnit == "%"))
      $ctfWidthOptions.show();
      ...
      success: function (data) {
        ...
        if (!data === false) {
          $ctfClearCacheButton.after(
            '<i id="ctf-clear-cache-success" class="fa fa-check-circle ctf-success"></i>'
          );
        } else {
  ...
  var $ctfClearPersistentCacheButton = $(
    "#ctf-admin #ctf-clear-persistent-cache"
  );
          ...
          if (data != "0") {
            ...
            for (var key in lists) {
              $(".ctf-lists-list").append(
                '<span class="ctf_list_id_item"><span class="ctf_list_id" data-ctf-listid="' +
                  lists[key].id +
                  '">+</span> ' +
                  lists[key].name +
  ...
  $("body").on("click", ".ctf_list_id", function () {
    ...
    if ($(this).text() == " -") {
      ...
      $ctfListsID.val(
        $ctfListsID.val().replace("," + $(this).attr("data-ctf-listid"), "")
      );
      $ctfListsID.val(
        $ctfListsID.val().replace($(this).attr("data-ctf-listid") + ",", "")
      );
      ...
      $ctfListsID.val(
        $ctfListsID.val() + separator + $(this).attr("data-ctf-listid")
      );
    }
  ...
  function ctfUpdateLayoutTypeOptionsDisplay() {
    setTimeout(function () {
      ...
      jQuery(
        ".ctf_layout_settings.ctf_layout_type_" +
          jQuery(".ctf_layout_type:checked").val()
      ).show();
    }, 1);
  }
