...
$header_no_bio =
    !$feed_options['showbio'] || empty($header_info['description'])
        ? ($header_no_bio = ' ctf-no-bio')
        : ($header_no_bio = "");
...
$verified_account =
    $header_info['verified'] == 1 ? ctf_get_fa_el('fa-check-circle') : "";
$tweet_count = CTF_Parse_Pro::get_header_tweet_count($header_info);
$follower_count = CTF_Parse_Pro::get_follower_count($header_info);
$header_styles =
    $feed_options['headerbgcolor'] . $feed_options['headertextcolor'];
$follow_button_text = __('Follow', 'custom-twitter-feeds');
?> 

<div class="ctf-header<?php echo esc_attr(
    $header_no_bio
); ?>" style="<?php echo $header_styles; ?>">
    <a href="<?php echo esc_url(
        'https://twitter.com/' . $username . '/'
    ); ?>" target="_blank" rel="noopener noreferrer" title="@<?php echo esc_attr(
    $username
); ?>" class="ctf-header-link">
        <div class="ctf-header-text">
            <p class="ctf-header-user">
                <span class="ctf-header-name"><?php echo esc_html(
                    $header_text
                ); ?></span>
                <span class="ctf-verified"><?php echo $verified_account; ?></span>
                <span class="ctf-header-follow"><?php echo $follow_button_text; ?></span>
                <span class="ctf-header-counts">
                    <span class="ctf-header-tweets-count" title="<?php echo $tweet_count; ?> Tweets"><?php echo ctf_get_fa_el(
     'fa-twitter'
 ) . $tweet_count; ?></span>
                    <span class="ctf-header-followers"  title="<?php echo $follower_count .
                        ' ' .
                        __(
                            'Followers',
                            'registrations-for-the-events-calendar'
                        ); ?>"><?php echo ctf_get_fa_el('fa-user') .
    $follower_count; ?></span>
                </span>
            </p>
            <?php if (
                $feed_options['showbio'] &&
                !empty($header_description)
            ): ?>
        ...
        <div class="ctf-header-img">
            <div class="ctf-header-img-hover"><?php echo ctf_get_fa_el(
                'fa-twitter'
            ); ?></div>
            <img src="<?php echo esc_url($avatar); ?>" alt="<?php echo esc_attr(
    $username
); ?>" width="48" height="48">
        </div>
    </a>
