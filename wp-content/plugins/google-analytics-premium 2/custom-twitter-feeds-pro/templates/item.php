...
$icon_size = !empty($feed_options['iconsize'])
    ? 'font-size: ' .
        floor(
            0.8 *
                str_replace(
...
if ($feed_options['linktexttotwitter']) {
    $tweet_text_start =
        '<a class="ctf-tweet-text-link" href="' .
        esc_url('https://twitter.com/' . $author . '/status/' . $post_id) .
...
<div class="<?php echo esc_attr(
    $tweet_classes
); ?>" id="ctf_<?php echo esc_attr($id_post_id); ?>" style="<?php echo esc_attr(
    ...
    <?php if (
        ctf_show('text', $feed_options) ||
        ctf_show('media', $feed_options) ||
        ctf_show('twittercards', $feed_options) ||
        ctf_show('linkbox', $feed_options)
    ): ?>
    <div class="ctf-tweet-content<?php echo esc_attr(
        $disable_lightbox_class
    ); ?>">
        <?php if (ctf_show('text', $feed_options)): ?>
	        <?php echo $tweet_text_start; ?><p class="ctf-tweet-text" style="<?php echo esc_attr(
    $tweet_text_styles
); ?>"><?php echo nl2br($post_text); ?></p><?php echo $tweet_text_end; ?>
        <?php endif; ?>
        ...
        <?php if (ctf_show('linkbox', $feed_options) && isset($quoted)): ?>
            <?php include ctf_get_feed_template_part(
                'linkbox',
                $feed_options
            ); ?>
        <?php endif; ?>
    ...
    <?php if (ctf_show('actions', $feed_options)): ?>
        <a href="<?php echo esc_url(
            'https://twitter.com/intent/tweet?in_reply_to=' .
                $tweet_action_end_url
        ); ?>" class="ctf-reply" target="_blank" rel="noopener noreferrer" style="<?php echo esc_attr(
    $tweet_action_styles
); ?>">
            <?php echo ctf_get_fa_el('fa-reply'); ?>
            <span class="ctf-screenreader"><?php _e(
                'Reply on Twitter',
                'custom-twitter-feeds'
            ); ?> <?php echo esc_attr($post_id); ?></span>
        </a>

        <a href="<?php echo esc_url(
            'https://twitter.com/intent/retweet?tweet_id=' .
                $tweet_action_end_url
        ); ?>" class="ctf-retweet" target="_blank" rel="noopener noreferrer" style="<?php echo esc_attr(
    $tweet_action_styles
); ?>">
        ...
        <a href="<?php echo esc_url(
            'https://twitter.com/intent/like?tweet_id=' . $tweet_action_end_url
        ); ?>" class="ctf-like" target="_blank" rel="noopener noreferrer" style="<?php echo esc_attr(
    $tweet_action_styles
); ?>">
    ...
    <?php if (ctf_show('twitterlink', $feed_options)): ?>
        <a href="<?php echo esc_url(
            'https://twitter.com/' . $author . '/status/' . $post_id
        ); ?>" class="ctf-twitterlink" style="<?php echo esc_attr(
    $twitter_link_styles
); ?>" target="_blank">
            <?php echo esc_html($feed_options['twitterlinktext']); ?>
            <span class="ctf-screenreader"><?php echo esc_attr(
                $post_id
            ); ?></span>
        </a>
    <?php endif; ?>
