...
if (!defined('CTF_URL')) {
    ...
    define(
        'CTF_JS_URL',
        plugins_url('/js/ctf-scripts-1-10.min.js?ver=' . CTF_VERSION, __FILE__)
    );
    define(
        'OAUTH_PROCESSOR_URL',
        'https://api.smashballoon.com/twitter-login.php?return_uri='
    );
...
if (function_exists('ctf_init')) {
    wp_die(
        "Please deactivate the free version of the Custom Twitter Feeds plugin before activating this version.<br /><br />Back to the WordPress <a href='" .
...
function ctf_activate($network_wide)
{
    ...
    if (!empty($existing_deprecated_options) && empty($existing_pro_options)) {
        ...
        $merged_options = array_merge(
            $merged_options,
            get_option('ctf_customize', [])
        );
        $merged_options = array_merge(
            $merged_options,
            get_option('ctf_style', [])
        );
...
function ctf_create_database_table()
{
    ...
    if (
        $wpdb->get_var("show tables like '$feeds_posts_table_name'") !=
        $feeds_posts_table_name
    ) {
        $sql =
            "CREATE TABLE " .
            $feeds_posts_table_name .
...
function sb_ctf_plugin_updater()
{
    ...
    // Setup the updater
    $edd_updater = new EDD_SL_Plugin_Updater(CTF_STORE_URL, __FILE__, [
...
function ctf_scripts_and_styles_pro($enqueue = false)
{
    wp_enqueue_style(
        'ctf_styles',
        plugins_url('/css/ctf-styles.min.css', __FILE__),
        [],
        CTF_VERSION
    );
    ...
    if (isset($ctf_options['headenqueue']) && $ctf_options['headenqueue']) {
        wp_enqueue_script(
            'ctf_scripts',
            plugins_url('/js/ctf-scripts-1-10.min.js', __FILE__),
            ['jquery'],
            CTF_VERSION,
            false
        );
    ...
    wp_localize_script('ctf_scripts', 'ctfOptions', [
...
function ctf_get_resized_uploads_url()
{
    ...
    $resize_url = apply_filters(
        'ctf_resize_url',
        trailingslashit($base_url) . trailingslashit(CTF_UPLOADS_NAME)
    );
...
function ctf_admin_scripts_and_styles_pro()
{
    wp_enqueue_style(
        'ctf_admin_styles',
        plugins_url('/css/ctf-admin-styles.css', __FILE__),
        [],
        CTF_VERSION
    );
    wp_enqueue_style(
    ...
    wp_enqueue_script(
        'ctf_admin_scripts',
        plugins_url('/js/ctf-admin-scripts.js', __FILE__),
        ['jquery'],
        CTF_VERSION,
        false
    );
    wp_localize_script('ctf_admin_scripts', 'ctf', [
        'ajax_url' => admin_url('admin-ajax.php'),
...
add_filter(
    "plugin_action_links_{$ctf_plugin_file}",
    'ctf_add_settings_link_pro',
    10,
    2
);
function ctf_add_settings_link_pro($links, $file)
{
    $ctf_settings_link =
        '<a href="' .
        admin_url('admin.php?page=custom-twitter-feeds') .
        '">' .
