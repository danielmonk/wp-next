...
class GFSelectColumns
{
    ...
    public static function select_columns_page()
    {
		...
		<head>
		<?php
  wp_print_styles(['wp-admin', 'colors-fresh']);
  ...
  ?>
      ...
      'Drag & drop to order and select which columns are displayed in the entries table.',
      'gravityforms'
  ); ?></div>
		<div class="gcolumn_wrapper">
			<div class="gcolumn_container_left">
				<div class="gform_select_column_heading"><?php esc_html_e(
        'Active Columns',
        'gravityforms'
    ); ?></div>
				<ul id="sortable_selected" class="sortable_connected">
				<?php foreach ($columns as $field_id => $field_info) { ?>
					<li id="<?php echo esc_attr($field_id); ?>"><?php echo esc_html(
    $field_info['label']
); ?></li>
				<?php } ?>
				</ul>
        ...
        if (
            ...
            !in_array($field->id, $field_ids)
						...
						<li id="<?php echo esc_attr($field->id); ?>"><?php echo esc_html(
    GFCommon::get_label($field)
        ...
        $display = !in_array($input_type, ['list', 'repeater']);
                    ...
                    !(
                        ...
                        in_array($input['id'], [
                            ...
                            floatval("{$field->id}.5"),
								...
								<li id="<?php echo esc_attr($input['id']); ?>"><?php echo esc_html(
    GFCommon::get_label($field, $input['id'])
            ...
            $display
						...
						<li id="<?php echo $field->id; ?>"><?php echo esc_html(
    GFCommon::get_label($field)
); ?></li>
					<?php }
    } ?>
