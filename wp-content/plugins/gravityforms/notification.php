...
class GFNotification
{
    ...
    public static function notification_edit_page($form_id, $notification_id)
    {
        ...
        $form = gf_apply_filters(
            ['gform_form_notification_page', $form_id],
            $form,
            $notification_id
        );
        ...
        $notification = !$notification_id
            ? []
            : self::get_notification($form, $notification_id);
        ...
        if (!empty($_POST)) {
            check_admin_referer(
                'gforms_save_notification',
                'gforms_save_notification'
            );

            // Clear out notification because it could have legacy data populated
            $notification = [
                'isActive' => isset($notification['isActive'])
                    ? rgar($notification, 'isActive')
                    : true,
            ];

            $notification['name'] = sanitize_text_field(
                rgpost('gform_notification_name')
            );
            $notification['service'] = sanitize_text_field(
                rgpost('gform_notification_service')
            );
            $notification['event'] = sanitize_text_field(
                rgpost('gform_notification_event')
            );
            $notification['to'] =
                rgpost('gform_notification_to_type') == 'field'
                    ? rgpost('gform_notification_to_field')
                    : rgpost('gform_notification_to_email');
            $to_type = rgpost('gform_notification_to_type');
            if (!in_array($to_type, ['email', 'field', 'routing', 'hidden'])) {
                $to_type = 'email';
            }
            ...
            $notification['subject'] = sanitize_text_field(
                rgpost('gform_notification_subject')
            );

            $notification['message'] = rgpost('gform_notification_message');

            $notification['from'] = sanitize_text_field(
                rgpost('gform_notification_from')
            );
            $notification['fromName'] = sanitize_text_field(
                rgpost('gform_notification_from_name')
            );

            $notification['replyTo'] = rgpost('gform_notification_reply_to');
            $routing = !rgempty('gform_routing_meta')
                ? GFCommon::json_decode(rgpost('gform_routing_meta'), true)
                : null;
            if (!empty($routing)) {
                $routing_logic = ['rules' => $routing];
                $routing_logic = GFFormsModel::sanitize_conditional_logic(
                    $routing_logic
                );
            ...
            $conditional_logic = !rgempty('gform_conditional_logic_meta')
                ? GFCommon::json_decode(
                    rgpost('gform_conditional_logic_meta'),
                    true
                )
                : null;
            ...
            $notification[
                'conditionalLogic'
            ] = GFFormsModel::sanitize_conditional_logic($conditional_logic);

            $notification['disableAutoformat'] = (bool) rgpost(
                'gform_notification_disable_autoformat'
            );

            $notification['enableAttachments'] = (bool) rgpost(
                'gform_notification_attachments'
            );
            ...
            if (rgpost('save')) {
                ...
                $notification = gf_apply_filters(
                    ['gform_pre_notification_save', $form_id],
                    $notification,
                    $form,
                    $is_new_notification
                );
                 ...
                 *
                 */
                $is_valid = gf_apply_filters(
                    ['gform_notification_validation', $form_id],
                    $is_valid,
                    $notification,
                    $form
                );
                ...
                if ($is_valid) {
                    ...
                    // Trim values
                    $notification = GFFormsModel::trim_conditional_logic_values_from_element(
                        $notification,
                        $form
                    );
                    ...
                    RGFormsModel::save_form_notifications(
                        $form_id,
                        $form['notifications']
                    );
                }
        ...
        if ($is_update && $is_valid) {
            ...
            GFCommon::add_message(
                sprintf(
                    esc_html__(
                        'Notification saved successfully. %sBack to notifications.%s',
                        'gravityforms'
                    ),
                    '<a href="' . esc_url($url) . '">',
                    '</a>'
             ...
             */
            gf_do_action(
                ['gform_post_notification_save', $form_id],
                $notification,
                $form,
                $is_new_notification
            );
        } elseif ($is_update && !$is_valid) {
            GFCommon::add_error_message(
                esc_html__(
                    'Notification could not be updated. Please enter all required information below.',
                    'gravityforms'
                )
            );
        }
        ...
        GFFormSettings::page_header(
            esc_html__('Notifications', 'gravityforms')
        );

        $notification_ui_settings = self::get_notification_ui_settings(
            $notification,
            $is_valid
        );
        ...
        esc_html__('Save & Continue Link', 'gravityforms')
    ); ?> });
				mergeTags["other"].tags.push({ tag: '{save_token}', label: <?php echo json_encode(
        esc_html__('Save & Continue Token', 'gravityforms')
    ); ?> });
			}
  ...
  ?>
		...
		function CreateRouting(routings) {
			...
			for (var i = 0; i < routings.length; i++) {
				...
				str += "<div style='width:99%'>" + <?php echo json_encode(
        esc_html__('Send to', 'gravityforms')
    ); ?> + " <input type='text' id='routing_email_" + i + "' value='" + email + "' class='gfield_routing_email' />";
				str += " " + <?php echo json_encode(
        esc_html__('if', 'gravityforms')
				...
				str += "<option value='is' " + isSelected + ">" + <?php echo json_encode(
        esc_html__('is', 'gravityforms')
    ); ?> + "</option>";
				str += "<option value='isnot' " + isNotSelected + ">" + <?php echo json_encode(
        esc_html__('is not', 'gravityforms')
    ); ?> + "</option>";
				str += "<option value='>' " + greaterThanSelected + ">" + <?php echo json_encode(
        esc_html__('greater than', 'gravityforms')
    ); ?> + "</option>";
				str += "<option value='<' " + lessThanSelected + ">" + <?php echo json_encode(
        esc_html__('less than', 'gravityforms')
    ); ?> + "</option>";
				str += "<option value='contains' " + containsSelected + ">" + <?php echo json_encode(
        esc_html__('contains', 'gravityforms')
    ); ?> + "</option>";
				str += "<option value='starts_with' " + startsWithSelected + ">" + <?php echo json_encode(
        esc_html__('starts with', 'gravityforms')
    ); ?> + "</option>";
				str += "<option value='ends_with' " + endsWithSelected + ">" + <?php echo json_encode(
        esc_html__('ends with', 'gravityforms')
    ); ?> + "</option>";
				str += "</select>&nbsp;";
		...
		function IsNotificationConditionalLogicField(field) {
			...
			var supported_fields = <?php echo json_encode(
       self::get_routing_field_types()
   ); ?>;
		...
		function GetFieldValues(index, fieldId, selectedValue, labelMaxCharacters) {
			...
			if (field["type"] == 'post_category' && field["displayAllCategories"]) {
				...
				if (dropdown.length > 0) {
					...
					str = "<select id='gfield_ajax_placeholder_" + index + "' class='gfield_routing_select'><option>" + <?php json_encode(
         esc_html__('Loading...', 'gravityforms')
     ); ?> + "</option></select>";
				}
			}
				...
				str = "<input type='text' placeholder='" + <?php echo json_encode(
		...
		<form method="post" id="gform_notification_form" onsubmit="gform_has_unsaved_changes = false; SaveJSMeta();">
			...
			<?php if (rgar($notification, 'isDefault')) {
       echo '<input type="hidden" id="gform_is_default" name="gform_is_default" value="1"/>';
			...
			<input type="hidden" id="gform_notification_id" name="gform_notification_id" value="<?php echo esc_attr(
       $notification_id
   ); ?>" />

			<table class="form-table gform_nofification_edit">
				<?php array_map(['GFFormSettings', 'output'], $notification_ui_settings); ?>
			</table>

			<p class="submit">
				<?php
    $button_label = $is_new_notification
        ? __('Save Notification', 'gravityforms')
    ...
    public static function notification_list_page($form_id)
    {
        ...
        $add_new_url = add_query_arg(['nid' => 0]);
        ?>

		<h3><span><i class="fa fa-envelope-o"></i> <?php esc_html_e(
      'Notifications',
      'gravityforms'
  ); ?>
				<a id="add-new-confirmation" class="add-new-h2" href="<?php echo esc_url(
        $add_new_url
    ); ?>"><?php esc_html_e('Add New', 'gravityforms'); ?></a></span>
		</h3>
		...
		<script type="text/javascript">
			function ToggleActive(img, notification_id) {
				...
				if (is_active) {
					...
					jQuery(img).attr('title', <?php echo json_encode(
         esc_html__('Inactive', 'gravityforms')
     ); ?>).attr('alt', <?php echo json_encode(esc_html__('Inactive', 'gravityforms')); ?>);
				}
				else {
					img.src = img.src.replace("active0.png", "active1.png");
					jQuery(img).attr('title', <?php echo json_encode(
         esc_html__('Active', 'gravityforms')
     ); ?>).attr('alt', <?php echo json_encode(esc_html__('Active', 'gravityforms')); ?>);
				}
				...
				mysack.setVar("rg_update_notification_active", "<?php echo wp_create_nonce(
        'rg_update_notification_active'
         ...
         esc_html__('Ajax error while updating notification', 'gravityforms')
     ); ?>)
				};
				mysack.runAJAX();
  ...
  ?>
		...
		<form id="notification_list_form" method="post">
			...
			<?php wp_nonce_field(
       'gform_notification_list_action',
       'gform_notification_list_action'
   ); ?>
    ...
    public static function maybe_process_notification_list_action()
    {
        ...
        switch ($action) {
            case 'delete':
                $notification_deleted = GFNotification::delete_notification(
                    $object_id,
                    rgget('id')
                );
                if ($notification_deleted) {
                    ...
                    GFCommon::add_error_message(
                        esc_html__(
                            'There was an issue deleting this notification.',
                            'gravityforms'
                        )
                    );
                }
                break;
            case 'duplicate':
                $notification_duplicated = GFNotification::duplicate_notification(
                    $object_id,
                    rgget('id')
                );
                if ($notification_duplicated) {
                    GFCommon::add_message(
                        esc_html__('Notification duplicated.', 'gravityforms')
                    );
                } else {
                    GFCommon::add_error_message(
                        esc_html__(
                            'There was an issue duplicating this notification.',
                            'gravityforms'
                        )
                    );
                }
     ...
     */
    private static function get_notification_ui_settings(
        $notification,
        $is_valid = true
    ) {
		...
		<tr valign="top" <?php echo rgar($notification, 'isDefault')
      ? "style='display:none'"
      : ''; ?> >
			<th scope="row">
			...
			<td>
				<input type="text" class="fieldwidth-2" name="gform_notification_name" id="gform_notification_name" value="<?php echo esc_attr(
        rgget('name', $notification)
    ); ?>" />
			</td>
		</tr> <!-- / name -->
		<?php
  ...
  $notification_service = !rgempty('gform_notification_service')
      ? rgpost('gform_notification_service')
      : rgar($notification, 'service');
  ...
  ?>
		...
		<tr valign="top" <?php echo $service_style; ?>>
			...
			<td>
				...
				<div id="gform-notification-service-<?php echo esc_attr(
        $service_name
    ); ?>" class="gform-notification-service<?php echo rgar(
    $service,
    'disabled'
) && rgar($service, 'disabled_message')
    ? ' gf_tooltip'
    : ''; ?>" <?php echo rgar($service, 'disabled') &&
rgar($service, 'disabled_message')
    ? 'title="' . esc_attr($service['disabled_message']) . '"'
    : ''; ?>>
					<input type="radio" id="gform_notification_service_<?php echo esc_attr(
         $service_name
     ); ?>" name="gform_notification_service" <?php checked(
    $service_name,
    $notification_service
); ?> value="<?php echo esc_attr(
     $service_name
 ); ?>" onclick="jQuery(this).parents('form').submit();" onkeypress="jQuery(this).parents('form').submit();" <?php echo rgar(
    $service,
    'disabled'
)
    ? 'disabled="disabled"'
    : ''; ?> />
					<label for="gform_notification_service_<?php echo esc_attr(
         $service_name
     ); ?>" class="inline">
						<span><img src="<?php echo esc_attr(
          rgar($service, 'image')
      ); ?>" /><br /><?php echo esc_html(rgar($service, 'label')); ?></span>
					</label>
				</div>
				<?php } ?>
			</td>
		...
		<?php
  $ui_settings['notification_service'] = ob_get_contents();
  ob_clean();
  ?>
  ...
  $event_style =
      count($notification_events) == 1 || rgar($notification, 'isDefault')
          ? "style='display:none'"
          : '';
  ?>
		<tr valign="top" <?php echo $event_style; ?>>
			...
			<td>
				<select name="gform_notification_event" id="gform_notification_event">
					<?php foreach ($notification_events as $code => $label) { ?>
						<option value="<?php echo esc_attr($code); ?>" <?php selected(
    rgar($notification, 'event'),
    $code
  ...
  ?>
		...
		<?php
  $notification_to_type = !rgempty('gform_notification_to_type')
      ? rgpost('gform_notification_to_type')
      : rgar($notification, 'toType');
  ...
  ?>
		<tr valign="top" class='<?php echo esc_attr(
      $send_to_class
  ); ?>' <?php echo $notification_to_type == 'hidden'
    ? 'style="display:none;"'
    : ''; ?>>
			<th scope="row">
         ...
         'gravityforms'
			...
			<td>
				<input type="radio" id="gform_notification_to_type_email" name="gform_notification_to_type" <?php checked(
        'email',
        $notification_to_type
				...
				&nbsp;&nbsp;
				<input type="radio" id="gform_notification_to_type_field" name="gform_notification_to_type" <?php checked(
        'field',
        $notification_to_type
    ); ?> value="field" onclick="jQuery('.notification_to_container').hide(); jQuery('#gform_notification_to_field_container').show('slow');" onkeypress="jQuery('.notification_to_container').hide(); jQuery('#gform_notification_to_field_container').show('slow');" />
				<label for="gform_notification_to_type_field" class="inline">
					<?php esc_html_e('Select a Field', 'gravityforms'); ?>
				</label>
				&nbsp;&nbsp;
				<input type="radio" id="gform_notification_to_type_routing" name="gform_notification_to_type" <?php checked(
        'routing',
        $notification_to_type
    ); ?> value="routing" onclick="jQuery('.notification_to_container').hide(); jQuery('#gform_notification_to_routing_container').show('slow');" onkeypress="jQuery('.notification_to_container').hide(); jQuery('#gform_notification_to_routing_container').show('slow');" />
  ...
  if ($notification_to_type == 'hidden') {
      $ui_settings['notification_to_email_type'] =
          '<input type="hidden" name="gform_notification_to_type" value="hidden" />';
  }
  ?>

		<tr id="gform_notification_to_email_container" class="notification_to_container <?php echo esc_attr(
      $send_to_class
  ); ?>" <?php echo $notification_to_type != 'email'
    ? "style='display:none';"
    : ''; ?>>
			...
			<td>
				<?php $to_email =
        rgget('toType', $notification) == 'email'
            ? rgget('to', $notification)
            : ''; ?>
				<input type="text" name="gform_notification_to_email" id="gform_notification_to_email" value="<?php echo esc_attr(
        $to_email
    ); ?>" class="fieldwidth-1" />

				<?php if (
        rgpost('gform_notification_to_type') == 'email' &&
        $is_invalid_email_to
    ) { ?>
					<span class="validation_message"><?php esc_html_e(
  ...
  ?>
		...
		<?php $email_fields = gf_apply_filters(
      ['gform_email_fields_notification_admin', $form['id']],
      GFCommon::get_email_fields($form),
      $form
  ); ?>
		<tr id="gform_notification_to_field_container" class="notification_to_container <?php echo esc_attr(
      $send_to_class
  ); ?>" <?php echo $notification_to_type != 'field'
    ? "style='display:none';"
    : ''; ?>>
			<?php echo $subsetting_open; ?>
          ...
          'gravityforms'
						...
						<?php
      $to_field =
          rgget('toType', $notification) == 'field'
              ? rgget('to', $notification)
              : '';
      foreach ($email_fields as $field) { ?>
							<option value="<?php echo esc_attr($field->id); ?>" <?php echo selected(
    $field->id,
    $to_field
); ?>><?php echo GFCommon::get_label($field); ?></option>
						<?php }
      ?>
					</select>
				<?php } else { ?>
  ...
  ?>
		...
		<tr id="gform_notification_to_routing_container" class="notification_to_container <?php echo esc_attr(
      $send_to_class
  ); ?>" <?php echo $notification_to_type != 'routing'
    ? "style='display:none';"
    : ''; ?>>
			<?php echo $subsetting_open; ?>
			<td colspan="2">
     ...
     if (empty($routing_fields)) { ?>
						<div class="gold_notice">
							<p><?php esc_html_e(
           'To use notification routing, your form must have a field supported by conditional logic.',
           'gravityforms'
       ); ?></p>
						</div>
					<?php } else {
         ...
         for ($i = 0; $i < $count; $i++) {
             ...
             $is_invalid_rule =
                 !$is_valid &&
                 $_POST['gform_notification_to_type'] == 'routing' &&
                 !self::is_valid_notification_email(rgar($routing, 'email'));
             $class = $is_invalid_rule ? "class='grouting_rule_error'" : '';
             ?>
							<div style='width:99%' <?php echo $class; ?>>
								<?php esc_html_e('Send to', 'gravityforms'); ?>
								<input type="text" id="routing_email_<?php echo $i; ?>" value="<?php echo esc_attr(
    rgar($routing, 'email')
); ?>" class='gfield_routing_email' />
								<?php esc_html_e('if', 'gravityforms'); ?>
								<select id="routing_field_id_<?php echo $i; ?>" class='gfield_routing_select' onchange='jQuery("#routing_value_<?php echo $i; ?>").replaceWith(GetRoutingValues(<?php echo $i; ?>, jQuery(this).val())); SetRouting(<?php echo $i; ?>); '><?php echo self::get_routing_fields(
    $form,
    rgar($routing, 'fieldId')
); ?></select>
								<select id="routing_operator_<?php echo $i; ?>" onchange="SetRouting(<?php echo $i; ?>)" class="gform_routing_operator">
									<option value="is" <?php echo rgar($routing, 'operator') == 'is'
             ? "selected='selected'"
             : ''; ?>><?php esc_html_e('is', 'gravityforms'); ?></option>
									<option value="isnot" <?php echo rgar($routing, 'operator') == 'isnot'
             ? "selected='selected'"
             : ''; ?>><?php esc_html_e('is not', 'gravityforms'); ?></option>
									<option value=">" <?php echo rgar($routing, 'operator') == '>'
             ? "selected='selected'"
             : ''; ?>><?php esc_html_e(
    'greater than',
    'gravityforms'
									...
									<option value="<" <?php echo rgar($routing, 'operator') == '<'
             ? "selected='selected'"
             : ''; ?>><?php esc_html_e('less than', 'gravityforms'); ?></option>
									<option value="contains" <?php echo rgar($routing, 'operator') == 'contains'
             ? "selected='selected'"
             : ''; ?>><?php esc_html_e('contains', 'gravityforms'); ?></option>
									<option value="starts_with" <?php echo rgar($routing, 'operator') ==
         'starts_with'
             ? "selected='selected'"
             : ''; ?>><?php esc_html_e(
    'starts with',
    'gravityforms'
									...
									<option value="ends_with" <?php echo rgar($routing, 'operator') == 'ends_with'
             ? "selected='selected'"
             : ''; ?>><?php esc_html_e('ends with', 'gravityforms'); ?></option>
								</select>
								<?php echo self::get_field_values(
            $i,
            $form,
            rgar($routing, 'fieldId'),
            rgar($routing, 'value')
								...
								<a class='gf_insert_field_choice' title='add another rule' onclick='SetRouting(<?php echo $i; ?>); InsertRouting(<?php echo $i +
    1; ?>);' onkeypress='SetRouting(<?php echo $i; ?>); InsertRouting(<?php echo $i +
    1; ?>);'><i class='gficon-add'></i></a>
  ...
  ?>
		...
		<tr valign="top">
			...
			<td>
				<input type="text" class="fieldwidth-2 merge-tag-support mt-position-right mt-hide_all_fields" name="gform_notification_from_name" id="gform_notification_from_name" value="<?php echo esc_attr(
        rgget('fromName', $notification)
    ); ?>" />
			</td>
		</tr> <!-- / from name -->
		<?php
  ...
  $is_invalid_from_email =
      !$is_valid &&
      $from_email_value &&
      !self::is_valid_notification_email($from_email_value);
  ...
  ?>
		<tr valign="top" <?php echo $class; ?>>
			...
			<td>
				<input type="text" class="fieldwidth-2 mt-position-right mt-hide_all_fields" name="gform_notification_from" id="gform_notification_from" value="<?php echo rgempty(
        'from',
        $notification
    )
        ? '{admin_email}'
        : esc_attr(rgget('from', $notification)); ?>" />
				<?php
    /**
    ...
    if (
        !$disable_from_warning &&
        rgar($notification, 'service') === 'wordpress' &&
        !$is_invalid_from_email &&
        !self::is_site_domain_in_from($from_email_value)
    ) {
        ...
        echo sprintf(
            ...
            '<a href="' . esc_url($doc_page) . '" target="_blank" >',
            '</a>'
        );
        echo '</div>';
    }
    ...
    if ($is_invalid_from_email) { ?>
					<br><span class="validation_message"><?php esc_html_e(
  ...
  $is_invalid_reply_to =
      !$is_valid &&
      $reply_to_value &&
      !self::is_valid_notification_email($reply_to_value);
  ...
  ?>
		<tr valign="top" <?php echo $class; ?>>
			...
			<td>
				<input type="text" name="gform_notification_reply_to" id="gform_notification_reply_to" class="merge-tag-support mt-hide_all_fields fieldwidth-2" value="<?php echo esc_attr(
        $reply_to_value
    ); ?>" />
				<?php if ($is_invalid_reply_to) { ?>
					<br><span class="validation_message"><?php esc_html_e(
         'Please enter a valid email address or merge tag in the Reply To field.',
         'gravityforms'
     ); ?></span><?php } ?>
			</td>
		</tr> <!-- / reply to -->
  ...
  $enable_cc = gf_apply_filters(
      ['gform_notification_enable_cc', $form['id'], rgar($notification, 'id')],
      false,
      $notification,
      $form
  );
  ...
  $is_invalid_cc =
      !$is_valid && $cc_value && !self::is_valid_notification_email($cc_value);
  ...
  ?>
		<tr valign="top" <?php echo $class; ?>>
			...
			<td>
				<input type="text" name="gform_notification_cc" id="gform_notification_cc" value="<?php echo esc_attr(
        $cc_value
    ); ?>" class="merge-tag-support mt-hide_all_fields fieldwidth-2" />
				<?php if ($is_invalid_cc) { ?>
					<br><span class="validation_message"><?php esc_html_e(
         'Please enter a valid email address or merge tag in the CC field.',
         'gravityforms'
     ); ?></span><?php } ?>
			</td>
		</tr> <!-- / cc -->
  ...
  $is_invalid_bcc =
      !$is_valid &&
      $bcc_value &&
      !self::is_valid_notification_email($bcc_value);
  ...
  ?>
		<tr valign="top" <?php echo $class; ?>>
			...
			<td>
				<input type="text" name="gform_notification_bcc" id="gform_notification_bcc" value="<?php echo esc_attr(
        $bcc_value
    ); ?>" class="merge-tag-support mt-hide_all_fields fieldwidth-2" />
				<?php if ($is_invalid_bcc) { ?>
					<br><span class="validation_message"><?php esc_html_e(
         'Please enter a valid email address or merge tag in the BCC field.',
         'gravityforms'
     ); ?></span><?php } ?>
			</td>
		</tr> <!-- / bcc -->
  ...
  ?>
		...
		<?php
  $is_invalid_subject =
      !$is_valid && empty($_POST['gform_notification_subject']);
  ...
  ?>
		<tr valign="top" <?php echo $subject_class; ?>>
         ...
         'gravityforms'
			...
			<td>
				<input type="text" name="gform_notification_subject" id="gform_notification_subject" class="fieldwidth-1 merge-tag-support mt-hide_all_fields mt-position-right" value="<?php echo esc_attr(
        rgar($notification, 'subject')
    ); ?>" />
				<?php if ($is_invalid_subject) { ?>
					<span class="validation_message"><?php esc_html_e(
         'Please enter a subject for the notification email',
         'gravityforms'
     ); ?></span><?php } ?>
			</td>
		</tr> <!-- / subject -->
  ...
  ?>
		...
		<?php
  $is_invalid_message =
      !$is_valid && empty($_POST['gform_notification_message']);
  ...
  ?>
		<tr valign="top" <?php echo $message_class; ?>>
         ...
         'gravityforms'
			...
			<td>
				...
				<?php
    wp_editor(rgar($notification, 'message'), 'gform_notification_message', [
        'autop' => false,
        'editor_class' =>
    ...
    if ($is_invalid_message) { ?>
					<span class="validation_message"><?php esc_html_e(
         'Please enter a message for the notification email',
         'gravityforms'
     ); ?></span><?php }
    ?>
			</td>
  ...
  ?>
        ...
        <?php
        $upload_fields = GFCommon::get_fields_by_type($form, ['fileupload']);
        if ($upload_fields) { ?>
        <tr valign="top">
            <th scope="row">
            ...
            <td>
                <input type="checkbox" name="gform_notification_attachments" id="gform_notification_attachments" value="1" <?php checked(
                    '1',
                    rgar($notification, 'enableAttachments')
                ); ?>/>
        ...
        <?php
        $ui_settings['notification_attachments'] = ob_get_contents();
        ob_clean();
        }
        ?>
        ...
        <tr valign="top">
			...
			<td>
				<input type="checkbox" name="gform_notification_disable_autoformat" id="gform_notification_disable_autoformat" value="1" <?php echo empty(
        $notification['disableAutoformat']
    )
        ? ''
        : "checked='checked'"; ?>/>
  ...
  ?>
     ...
     ?>
			...
			<td>
				<input type="checkbox" id="notification_conditional_logic" onclick="SetConditionalLogic(this.checked); ToggleConditionalLogic(false, 'notification');" onkeypress="SetConditionalLogic(this.checked); ToggleConditionalLogic(false, 'notification');" <?php checked(
        is_array(rgar($notification, 'conditionalLogic')),
        true
    ); ?> />
  ...
  $ui_settings = gf_apply_filters(
      ['gform_notification_ui_settings', $form_id],
      $ui_settings,
      $notification,
      $form,
      $is_valid
  ...
  return $ui_settings;
    ...
    public static function get_notification_services()
    {
         ...
         */
        return gf_apply_filters(['gform_notification_services'], $services);
    }
    ...
    {
        $notification_events = [
            'form_submission' => esc_html__(
                'Form is submitted',
                'gravityforms'
            ),
        ];
        if (rgars($form, 'save/enabled')) {
            $notification_events['form_saved'] = esc_html__(
                'Form is saved',
                'gravityforms'
            );
            $notification_events['form_save_email_requested'] = esc_html__(
                'Save and continue email is requested',
                'gravityforms'
            );
        }
    ...
    {
        $is_valid =
            self::is_valid_notification_to() &&
            !rgempty('gform_notification_subject') &&
            !rgempty('gform_notification_message');
        ...
        if (
            !empty($reply_to) &&
            !self::is_valid_notification_email($reply_to)
        ) {
            $is_valid = false;
        }
        ...
        if (
            !empty($from_email) &&
            !self::is_valid_notification_email($from_email)
        ) {
            $is_valid = false;
        }
    ...
    {
        $routing = !empty($_POST['gform_routing_meta'])
            ? GFCommon::json_decode(
                stripslashes($_POST['gform_routing_meta']),
                true
            )
            : null;
        if (empty($routing)) {
    ...
    private static function is_valid_notification_to()
    {
        ...
        $is_valid =
            (rgpost('gform_notification_to_type') == 'routing' &&
                self::is_valid_routing()) ||
            (rgpost('gform_notification_to_type') == 'email' &&
                self::is_valid_notification_email($notification_to_email)) ||
            (rgpost('gform_notification_to_type') == 'field' &&
                !rgempty('gform_notification_to_field')) ||
     ...
     */
    private static function get_field_values(
        $i,
        $form,
        $field_id,
        $selected_value,
        ...
        if (
            ...
            $str .= wp_dropdown_categories([
                'class' =>
                    'gfield_routing_select gfield_category_dropdown gfield_routing_value_dropdown',
                'orderby' => 'name',
                'id' => 'routing_value_' . $i,
            ...
            $str .=
                "<select id='routing_value_" .
                $i .
                "' class='gfield_routing_select gfield_routing_value_dropdown'>";
            ...
            $str =
                ...
                esc_html__('Enter value', 'gravityforms') .
                "' class='gfield_routing_select' id='routing_value_" .
                $i .
                "' value='" .
                esc_attr($selected_value) .
                "' onchange='SetRouting(" .
    ...
    public static function get_post_category_values()
    {
        ...
        $dropdown = wp_dropdown_categories([
            'class' =>
                'gfield_routing_select gfield_routing_value_dropdown gfield_category_dropdown',
            'orderby' => 'name',
            'id' => $id,
            'selected' => $selected,
            'hierarchical' => true,
    ...
    public static function delete_notification($notification_id, $form_id)
    {
        ...
        return RGFormsModel::save_form_notifications(
            $form['id'],
            $form['notifications']
        );
    }
...
class GFNotificationTable extends WP_List_Table
{
    ...
    {
        $this->form = $form;
        $this->notification_events = GFNotification::get_notification_events(
            $form
        );
    ...
    function display()
    {
		...
		<table class="wp-list-table <?php echo esc_attr(
    ...
    {
        $edit_url = add_query_arg(['nid' => $item['id']]);
        /**
         ...
         */
        $actions = apply_filters('gform_notification_actions', [
            'edit' =>
                '<a href="' .
                esc_url($edit_url) .
                '">' .
                esc_html__('Edit', 'gravityforms') .
                '</a>',
            'duplicate' =>
                '<a href="javascript:void(0);" onclick="javascript: DuplicateNotification(\'' .
                esc_js($item['id']) .
                '\');" onkeypress="javascript: DuplicateNotification(\'' .
                esc_js($item['id']) .
                '\');" style="cursor:pointer;">' .
                esc_html__('Duplicate', 'gravityforms') .
                '</a>',
            'delete' =>
                '<a href="javascript:void(0);" class="submitdelete" onclick="javascript: if(confirm(\'' .
                esc_js(
                    esc_html__(
                        'WARNING: You are about to delete this notification.',
                        'gravityforms'
                    )
                ) .
                esc_js(
                ...
                esc_js($item['id']) .
                '\'); }" style="cursor:pointer;">' .
                esc_html__('Delete', 'gravityforms') .
                '</a>',
        ]);
    ...
    function column_service($notification)
    {
        ...
        } elseif (rgar($services, $notification['service'])) {
    ...
    function no_items()
    {
        $url = add_query_arg(['nid' => 0]);
        printf(
            esc_html__(
                "This form doesn't have any notifications. Let's go %screate one%s.",
                'gravityforms'
            ),
            "<a href='" . esc_url($url) . "'>",
            '</a>'
