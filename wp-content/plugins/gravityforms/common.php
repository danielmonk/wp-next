...
class GFCommon
{
     ...
     */
    public static $dismissible_messages = [];
    ...
    public static function is_numeric($value, $number_format = '')
    {
        if ($number_format == 'currency') {
            $number_format = self::is_currency_decimal_dot()
                ? 'decimal_dot'
                : 'decimal_comma';
...
<IfModule headers_module>
  Header set X-Robots-Tag "noindex"
    ...
    public static function clean_number($number, $number_format = '')
    {
        ...
        for ($i = 0, $count = sizeof($array); $i < $count; $i++) {
            ...
            if ($char >= '0' && $char <= '9') {
                ...
                empty($decimal_char) &&
                ($char == '.' || $char == ',') &&
                strlen($clean_number) - $i <= 3
            ) {
                $float_number .= '.';
            } elseif ($decimal_char == $char) {
                $float_number .= '.';
            }
        }
    ...
    public static function truncate_middle($text, $max_length)
    {
        ...
        return self::safe_substr($text, 0, $middle) .
            '...' .
            self::safe_substr($text, strlen($text) - $middle, $middle);
    }
    ...
    public static function get_label(
        ...
        $input_id = 0,
        $input_only = false,
        $allow_admin_label = true
    ) {
        return RGFormsModel::get_label(
            $field,
            $input_id,
            $input_only,
            $allow_admin_label
    ...
    public static function insert_variables(
        $fields,
        $element_id,
      ...
      $element_id
  ); ?>_variable_select" onchange="<?php echo $onchange; ?>" class="<?php echo esc_attr(
    $class
); ?>">
       ...
       ?>
				...
				<?php
    foreach ($tags as $tag) { ?>
					<option value="<?php esc_attr_e($tag['tag']); ?>"><?php esc_html_e(
    $tag['label']
); ?></option>
					<?php }
    if ($group_label) { ?>
    ...
    public static function get_merge_tags(
        ...
        $required_fields = $optional_fields = $pricing_fields = [];
        $ungrouped = $required_group = $optional_group = $pricing_group = $other_group = [];
        ...
        foreach ($fields as $field) {
            ...
            if (
                is_array($exclude_field_types) &&
                in_array($input_type, $exclude_field_types)
            ) {
                continue;
            }
                ...
                switch ($input_type) {
                    case 'name':
                        if ($field->nameFormat == 'extended') {
                            $prefix = GFCommon::get_input(
                                $field,
                                $field->id . '.2'
                            );
                            $suffix = GFCommon::get_input(
                                $field,
                                $field->id . '.8'
                            );
                            $optional_field = $field;
                            $optional_field['inputs'] = [$prefix, $suffix];
        ...
        if (!empty($required_fields)) {
            foreach ($required_fields as $field) {
                $required_group = array_merge(
                    $required_group,
                    self::get_field_merge_tags($field, $option)
                );
            }
        }
        ...
        $other_group[] = [
            'tag' => '{ip}',
            'label' => esc_html__('User IP Address', 'gravityforms'),
        ];
        $other_group[] = [
            'tag' => '{date_mdy}',
        ...
        $other_group[] = [
            'tag' => '{embed_url}',
            'label' => esc_html__('Embed URL', 'gravityforms'),
        ];
        $other_group[] = [
            'tag' => '{entry_id}',
        ...
        $other_group[] = [
            'tag' => '{entry_url}',
            'label' => esc_html__('Entry URL', 'gravityforms'),
        ];
        $other_group[] = [
            'tag' => '{form_id}',
            'label' => esc_html__('Form Id', 'gravityforms'),
        ];
        $other_group[] = [
            'tag' => '{form_title}',
        ...
        $other_group[] = [
            'tag' => '{user_agent}',
            'label' => esc_html__('HTTP User Agent', 'gravityforms'),
        ];
        $other_group[] = [
            'tag' => '{referer}',
            'label' => esc_html__('HTTP Referer URL', 'gravityforms'),
        ];
        ...
        $other_group[] = [
            'tag' => '{user:user_email}',
            'label' => esc_html__('User Email', 'gravityforms'),
        ];
        $other_group[] = [
            'tag' => '{user:user_login}',
            'label' => esc_html__('User Login', 'gravityforms'),
        ];

        $form_id = isset($fields[0]) ? $fields[0]->formId : rgget('id');
        $form_id = absint($form_id);

        $custom_group = apply_filters(
    ...
    public static function get_field_merge_tags($field, $option = '')
    {
        $merge_tags = [];
        $tag_args =
            RGFormsModel::get_input_type($field) == 'list' ? ":{$option}" : ''; //args currently only supported by list field
        ...
        if (is_array($inputs)) {
            if (RGFormsModel::get_input_type($field) == 'checkbox') {
                $value =
                    '{' .
                    esc_html(GFCommon::get_label($field, $field->id)) .
                    ':' .
                    $field->id .
                    "{$tag_args}}";
                $merge_tags[] = [
                    'tag' => $value,
                    'label' => esc_html(
                        GFCommon::get_label($field, $field->id)
                    ),
                ];
            }
            ...
            foreach ($field->inputs as $input) {
                if (RGFormsModel::get_input_type($field) == 'creditcard') {
                    ...
                    $value =
                        '{' .
                        esc_html(GFCommon::get_label($field, $input['id'])) .
                        ':' .
                        $input['id'] .
                        "{$tag_args}}";
                    $merge_tags[] = [
    ...
    public static function insert_field_variable(
        $field,
        $max_label_size = 40,
        $args = ''
    ) {
        $tag_args =
            RGFormsModel::get_input_type($field) == 'list' ? ":{$args}" : ''; //args currently only supported by list field
    ...
    public static function insert_post_content_variables(
        $fields,
        $element_id,
        $callback,
        $max_label_size = 25
        ...
        $insert_variables_onchange = sprintf(
            "InsertPostContentVariable('%s', '%s');",
            esc_js($element_id),
            esc_js($callback)
        );
        ...
        ?>
		&nbsp;&nbsp;
		<select id="<?php echo $element_id; ?>_image_size_select" onchange="InsertPostImageVariable('<?php echo esc_js(
    $element_id
); ?>', '<?php echo esc_js(
    $element_id
); ?>'); SetCustomFieldTemplate();" style="display:none;">
			<option value=""><?php esc_html_e(
       'Select image size',
       'gravityforms'
   ); ?></option>
			<option value="thumbnail"><?php esc_html_e(
       'Thumbnail',
       'gravityforms'
   ); ?></option>
			<option value="thumbnail:left"><?php esc_html_e(
       'Thumbnail - Left Aligned',
       'gravityforms'
   ); ?></option>
			<option value="thumbnail:center"><?php esc_html_e(
       'Thumbnail - Centered',
       'gravityforms'
   ); ?></option>
			<option value="thumbnail:right"><?php esc_html_e(
       'Thumbnail - Right Aligned',
       'gravityforms'
   ); ?></option>

			<option value="medium"><?php esc_html_e('Medium', 'gravityforms'); ?></option>
			<option value="medium:left"><?php esc_html_e(
       'Medium - Left Aligned',
       'gravityforms'
   ); ?></option>
			<option value="medium:center"><?php esc_html_e(
       'Medium - Centered',
       'gravityforms'
   ); ?></option>
			<option value="medium:right"><?php esc_html_e(
       'Medium - Right Aligned',
       'gravityforms'
   ); ?></option>

			<option value="large"><?php esc_html_e('Large', 'gravityforms'); ?></option>
			<option value="large:left"><?php esc_html_e(
       'Large - Left Aligned',
       'gravityforms'
   ); ?></option>
			<option value="large:center"><?php esc_html_e(
       'Large - Centered',
       'gravityforms'
   ); ?></option>
			<option value="large:right"><?php esc_html_e(
       'Large - Right Aligned',
       'gravityforms'
   ); ?></option>

			<option value="full"><?php esc_html_e('Full Size', 'gravityforms'); ?></option>
			<option value="full:left"><?php esc_html_e(
       'Full Size - Left Aligned',
       'gravityforms'
   ); ?></option>
			<option value="full:center"><?php esc_html_e(
       'Full Size - Centered',
       'gravityforms'
   ); ?></option>
			<option value="full:right"><?php esc_html_e(
       'Full Size - Right Aligned',
       'gravityforms'
   ); ?></option>
		</select>
		<?php
    ...
    public static function insert_calculation_variables(
        $fields,
        $element_id,
        $onchange = '',
        $callback = '',
   ...
   $custom_merge_tags = apply_filters(
       'gform_custom_merge_tags',
       [],
       $form_id,
       $fields,
   ...
   if (is_array($custom_merge_tags) && !empty($custom_merge_tags)) { ?>
				...
				<optgroup label="<?php esc_attr_e('Custom', 'gravityforms'); ?>">
					...
					<?php foreach ($custom_merge_tags as $custom_merge_tag) { ?>
						...
						<option value='<?php echo esc_attr(
          rgar($custom_merge_tag, 'tag')
      ); ?>'><?php echo esc_html(rgar($custom_merge_tag, 'label')); ?></option>
    ...
    private static function get_post_image_variable(
        $media_id,
        $arg1,
        $arg2,
        $is_url = false
        ...
        switch ($arg1) {
            ...
            default:
                $img = wp_get_attachment_image($media_id, $arg1, false, [
                    'class' => "size-{$arg1} align{$arg2} wp-image-{$media_id}",
                ]);
    ...
    public static function replace_variables_post_image(
        $text,
        $post_images,
        $lead
    ) {
        ...
        if (is_array($matches)) {
            foreach ($matches as $match) {
                ...
                $value = is_wp_error($media_id)
                    ? ''
                    : self::get_post_image_variable(
                        $media_id,
                        $size_meta,
                        $align,
                        $is_url
    ...
    public static function format_variable_value(
        $value,
        $url_encode,
        $esc_html,
        $format,
    ...
    public static function replace_variables(
        $text,
        $form,
        $lead,
        $url_encode = false,
        ...
        $data = array_merge(['entry' => $lead], $aux_data);
        ...
        if (is_array($matches)) {
            foreach ($matches as $match) {
                ...
                $text = self::replace_field_variable(
                    $text,
                    $form,
                    $lead,
                    $url_encode,
        ...
        foreach ($pricing_matches as $match) {
            ...
            if (strpos($text, $match[0]) !== false) {
                $pricing_fields = self::get_submitted_pricing_fields(
                    $form,
                    $lead,
                    $format,
                    !$use_value,
        ...
        $text = str_replace(
            '{form_id}',
            $url_encode ? urlencode(rgar($form, 'id')) : rgar($form, 'id'),
            $text
        );
        ...
        $text = str_replace(
            '{entry_id}',
            $url_encode ? urlencode(rgar($lead, 'id')) : rgar($lead, 'id'),
            $text
        );
        ...
        $entry_url =
            get_bloginfo('wpurl') .
            '/wp-admin/admin.php?page=gf_entries&view=entry&id=' .
            rgar($form, 'id') .
            '&lid=' .
            rgar($lead, 'id');
         ...
         */
        $entry_url = esc_url(
            apply_filters('gform_entry_detail_url', $entry_url, $form, $lead)
        );
        $text = str_replace(
            '{entry_url}',
            $url_encode ? urlencode($entry_url) : $entry_url,
            $text
        );
        ...
        $text = str_replace(
            '{admin_email}',
            $url_encode ? urlencode($wp_email) : $wp_email,
            $text
        );
        ...
        $text = str_replace(
            '{admin_url}',
            $url_encode ? urlencode(admin_url()) : admin_url(),
            $text
        );
        ...
        $text = str_replace(
            '{logout_url}',
            $url_encode ? urlencode(wp_logout_url()) : wp_logout_url(),
            $text
        );
        ...
        $text = str_replace(
            '{post_edit_url}',
            $url_encode ? urlencode($post_url) : $post_url,
            $text
        );

        $text = self::replace_variables_prepopulate(
            $text,
            $url_encode,
            $lead,
            $esc_html,
    ...
    {
        $description_class =
            rgar($form, 'descriptionPlacement') == 'above'
                ? 'description_above'
                : 'description_below';
        $sublabel_class =
            rgar($form, 'subLabelPlacement') == 'above'
                ? 'form_sublabel_above'
                : 'form_sublabel_below';
        $label_class = rgempty('labelPlacement', $form)
            ? 'top_label'
            : rgar($form, 'labelPlacement');

        $css_class = preg_replace(
            '/\s+/',
            ' ',
            "gform_fields {$label_class} {$sublabel_class} {$description_class}"
        ); //removing extra spaces

        return $css_class;
    }

    public static function replace_variables_prepopulate(
        $text,
        $url_encode = false,
        $entry = false,
        $esc_html = false,
        ...
        if (strpos($text, '{') !== false) {
            //embed url
            $current_page_url = empty($entry)
                ? RGFormsModel::get_current_page_url()
            ...
            $text = str_replace(
                '{date_mdy}',
                $url_encode ? urlencode($local_date_mdy) : $local_date_mdy,
                $text
            );
            ...
            $text = str_replace(
                '{date_dmy}',
                $url_encode ? urlencode($local_date_dmy) : $local_date_dmy,
                $text
            );

            // ip
            $request_ip = rgars($form, 'personalData/preventIP')
                ? ''
                : GFFormsModel::get_ip();
            ...
            $text = str_replace(
                '{ip}',
                $url_encode ? urlencode($ip) : $ip,
                $text
            );

            //user agent
            $user_agent = isset($entry['user_agent'])
                ? $entry['user_agent']
                : sanitize_text_field(rgar($_SERVER, 'HTTP_USER_AGENT'));
            $text = str_replace(
                '{user_agent}',
                self::format_variable_value(
                    $user_agent,
                    $url_encode,
                    $esc_html,
                    $format,
            ...
            if (!empty($ep_matches) || !empty($cf_matches)) {
                ...
                foreach ($ep_matches as $match) {
                    ...
                    $text = str_replace(
                        $full_tag,
                        $url_encode ? urlencode($value) : $value,
                        $text
                    );
                }
                ...
                foreach ($cf_matches as $match) {
                    ...
                    $custom_field_value =
                        $is_singular && !empty($post_array['ID'])
                            ? get_post_meta(
                                $post_array['ID'],
                                $custom_field_name,
                                true
    ...
    public static function get_submitted_fields(
        ...
        foreach ($form['fields'] as $field) {
            ...
            $field_label =
                $format == 'text'
                    ? sanitize_text_field(self::get_label($field))
                    : esc_html(self::get_label($field));
            ...
            switch ($field->type) {
                ...
                case 'section':
                    if (
                        GFFormsModel::is_field_hidden($form, $field, [], $lead)
                    ) {
                        break;
                    }

                    if (
                        (!GFCommon::is_section_empty($field, $form, $lead) ||
                            $display_empty) &&
                        !$field->is_administrative()
                    ) {
                        switch ($format) {
                            case 'text':
                                $field_value = "--------------------------------\n{$field_label}\n\n";
                ...
                default:
                    if (self::is_product_field($field->type)) {
                        ...
                        $display_product_summary = apply_filters(
                    ...
                    $raw_field_value = RGFormsModel::get_lead_field_value(
                        $lead,
                        $field
                    );
                    $field_value = GFCommon::get_lead_field_display(
                        $field,
                        $raw_field_value,
                        rgar($lead, 'currency'),
                        $use_text,
        ...
        if ($display_product_summary) {
            $field_data .= self::get_submitted_pricing_fields(
                $form,
                $lead,
                $format,
                $use_text,
    ...
    public static function get_submitted_pricing_fields(
        $form,
        $lead,
        $format,
        $use_text = true,
        ...
        $form_id = $form['id'];
        $order_label = gf_apply_filters(
            ['gform_order_label', $form_id],
            esc_html__('Order', 'gravityforms'),
            $form['id']
        );
        ...
        switch ($format) {
            ...
            default:
                if (!empty($products['products'])) {
                     ...
                     */
                    $gform_product = gf_apply_filters(
                        ['gform_product', $form_id],
                        esc_html__('Product', 'gravityforms'),
                        $form_id
                    );
                    /**
                     ...
                     * @var int $form_id The ID of the form being processed.
                     */
                    $gform_product_qty = gf_apply_filters(
                        ['gform_product_qty', $form_id],
                        esc_html__('Qty', 'gravityforms'),
                        $form_id
                    );
                    /**
                     ...
                     * @var int $form_id The ID of the form being processed.
                     */
                    $gform_product_unitprice = gf_apply_filters(
                        ['gform_product_unitprice', $form_id],
                        esc_html__('Unit Price', 'gravityforms'),
                        $form_id
                    );
                    /**
                     ...
                     * @var int $form_id The ID of the form being processed.
                     */
                    $gform_product_price = gf_apply_filters(
                        ['gform_product_price', $form_id],
                        esc_html__('Price', 'gravityforms'),
                        $form_id
                    );
                    ...
                    $field_data =
                        '<tr bgcolor="#EAF2FA">
                            <td colspan="2">
                                <font style="font-family: sans-serif; font-size:12px;"><strong>' .
                        $order_label .
                        '</strong></font>
                            </td>
                       </tr>
                       <tr bgcolor="#FFFFFF">
                            ...
                            <td>
                                ...
                                <thead>
                                    <tr>
	                                    <th style="background-color:#F4F4F4; border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; font-family: sans-serif; font-size:12px; text-align:left">' .
                        $gform_product .
                        '</th>
	                                    <th style="background-color:#F4F4F4; border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; width:50px; font-family: sans-serif; font-size:12px; text-align:center">' .
                        $gform_product_qty .
                        '</th>
	                                    <th style="background-color:#F4F4F4; border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; width:155px; font-family: sans-serif; font-size:12px; text-align:left">' .
                    ...
                    foreach ($products['products'] as $product) {
                        $field_data .=
                            '<tr>
                                                        <td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; font-family: sans-serif; font-size:11px;" >
                                                            <strong style="color:#BF461E; font-size:12px; margin-bottom:5px">' .
                            $product['name'] .
                            '</strong>
                                                            <ul style="margin:0">';
                        ...
                        $field_data .=
                            '</ul>
                                                        ...
                                                        <td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; text-align:center; width:50px; font-family: sans-serif; font-size:11px;" >' .
                            $product['quantity'] .
                            '</td>
                        ...
                        $field_data .=
                            '
                                    <tr>
                                        ...
                                        <td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; text-align:right; width:155px; font-family: sans-serif;"><strong style="font-size:12px;">' .
                            $products['shipping']['name'] .
                            '</strong></td>
                                        <td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; width:155px; font-family: sans-serif;"><strong style="font-size:12px;">' .
                            self::to_money(
                                $products['shipping']['price'],
                                $lead['currency']
                            ) .
    ...
    public static function send_user_notification(
        $form,
        $lead,
        ...
        $notification = self::prepare_user_notification(
            $form,
            $lead,
            $override_options
        );
    ...
    public static function send_admin_notification(
        ...
        $notification = self::prepare_admin_notification(
            $form,
            $lead,
            $override_options
        );
    ...
    private static function prepare_user_notification(
        $form,
        $lead,
        $override_options = false
    ) {
        ...
        $to_field = isset($form['autoResponder']['toField'])
            ? rgget($form['autoResponder']['toField'], $lead)
            : '';
        $to = gf_apply_filters(
            ['gform_autoresponder_email', $form_id],
            $to_field,
            $form
        );
        $subject = GFCommon::replace_variables(
            rgget('subject', $form['autoResponder']),
            $form,
            $lead,
            false,
            false
        ...
        $message_format = gf_apply_filters(
            ['gform_notification_format', $form_id],
            'html',
            'user',
            $form,
            $lead
        ...
        $message = GFCommon::replace_variables(
            rgget('message', $form['autoResponder']),
            $form,
            $lead,
            false,
            false,
            !rgget('disableAutoformat', $form['autoResponder']),
            $message_format
        );
        ...
        $from = GFCommon::replace_variables(
            rgget('from', $form['autoResponder']),
            $form,
            $lead,
            false,
            false
        );
        $bcc = GFCommon::replace_variables(
            rgget('bcc', $form['autoResponder']),
            $form,
            $lead,
            false,
            false
        );
        $reply_to = GFCommon::replace_variables(
            rgget('replyTo', $form['autoResponder']),
            $form,
            $lead,
            false,
            false
        );
        $from_name = GFCommon::replace_variables(
            rgget('fromName', $form['autoResponder']),
            $form,
            $lead,
            false,
            false
        ...
        $attachments = gf_apply_filters(
            ['gform_user_notification_attachments', $form_id],
            [],
            $lead,
            $form
        );
     ...
     * @return array
     */
    private static function prepare_admin_notification(
        $form,
        $lead,
        $override_options = false
    ) {
        ...
        $subject = GFCommon::replace_variables(
            rgget('subject', $form['notification']),
            $form,
            $lead,
            false,
            false
        ...
        $message = GFCommon::replace_variables(
            rgget('message', $form['notification']),
            $form,
            $lead,
            false,
            false,
            !rgget('disableAutoformat', $form['notification']),
            $message_format
        );
        ...
        $email_to = GFCommon::replace_variables(
            $email_to,
            $form,
            $lead,
            false,
        ...
        $to = gf_apply_filters(
            ['gform_notification_email', $form_id],
            $email_to,
            $lead
        );
        ...
        $attachments = gf_apply_filters(
            ['gform_admin_notification_attachments', $form_id],
            [],
            $lead,
            $form
        );
    ...
    public static function send_notification(
        ...
        GFCommon::log_debug(
            "GFCommon::send_notification(): Starting to process notification (#{$notification['id']} - {$notification['name']})."
        );

        $notification = gf_apply_filters(
            ['gform_notification', $form['id']],
            $notification,
            $form,
            $lead
        );
        ...
        if (
            empty($email_to) &&
            rgar($notification, 'toType') == 'routing' &&
            !empty($notification['routing'])
        ) {
            $email_to = [];
            foreach ($notification['routing'] as $routing) {
                ...
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): Evaluating Routing - rule => ' .
                        print_r($routing, 1)
                );

                $source_field = RGFormsModel::get_field(
                    $form,
                    rgar($routing, 'fieldId')
                );
                $field_value = RGFormsModel::get_lead_field_value(
                    $lead,
                    $source_field
                ...
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): Evaluating Routing - field value => ' .
                        print_r($field_value, 1)
                );
                $is_value_match = $is_value_match ? 'Yes' : 'No';
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): Evaluating Routing - is value match? ' .
                        $is_value_match
                );
            }
            ...
            $email_to = RGFormsModel::get_lead_field_value(
                $lead,
                $source_field
            );
        }
        ...
        $replyTo = GFCommon::replace_variables(
         ...
         */
        $enable_cc = gf_apply_filters(
            ['gform_notification_enable_cc', $form['id'], $notification['id']],
            false,
            $notification,
            $form
        );
        ...
        $message_format = rgempty('message_format', $notification)
            ? 'html'
            : rgar($notification, 'message_format');

        $merge_tag_format =
            $message_format === 'multipart' ? 'html' : $message_format;

        $message = GFCommon::replace_variables(
            rgar($notification, 'message'),
            $form,
            $lead,
            false,
            false,
            !rgar($notification, 'disableAutoformat'),
            $merge_tag_format,
            $data
        );
        ...
        if (rgar($notification, 'enableAttachments', false)) {
            ...
            $upload_fields = GFCommon::get_fields_by_type($form, [
                'fileupload',
            ]);
            $upload_root = GFFormsModel::get_upload_root();
            ...
            foreach ($upload_fields as $upload_field) {
                ...
                // Convert to array.
                $attachment_urls = $upload_field->multipleFiles
                    ? json_decode($attachment_urls, true)
                    : [$attachment_urls];

                self::log_debug(
                    __METHOD__ .
                        '(): Attaching file(s) for field #' .
                        $upload_field->id .
                        '. ' .
                        print_r($attachment_urls, true)
                );
        ...
        if ($message_format === 'multipart') {
            ...
            $text_message = GFCommon::replace_variables(
                rgar($notification, 'message'),
                $form,
                $lead,
                false,
    ...
    public static function send_notifications(
        ...
        if (!is_array($notification_ids) || empty($notification_ids)) {
            GFCommon::log_debug(
                __METHOD__ .
                    "(): Aborting. No notifications to process for {$event} event for entry #{$entry_id}."
            );
        ...
        GFCommon::log_debug(
            __METHOD__ .
                "(): Processing notifications for {$event} event for entry #{$entry_id}: " .
                print_r($notification_ids, true) .
        ...
        foreach ($notification_ids as $notification_id) {
            ...
            if (
                isset($form['notifications'][$notification_id]['isActive']) &&
                !$form['notifications'][$notification_id]['isActive']
            ) {
                GFCommon::log_debug(
                    __METHOD__ .
                        "(): Notification is inactive, not processing notification (#{$notification_id} - {$form['notifications'][$notification_id]['name']}) for entry #{$entry_id}."
                );
                continue;
            }
            ...
            if (
                $do_conditional_logic &&
                !GFCommon::evaluate_conditional_logic(
                    rgar($notification, 'conditionalLogic'),
                    $form,
                    $lead
                )
            ) {
                GFCommon::log_debug(
                    __METHOD__ .
                        "(): Notification conditional logic not met, not processing notification (#{$notification_id} - {$notification['name']}) for entry #{$entry_id}."
                );
                continue;
            }
            ...
            if (rgar($notification, 'type') == 'user') {
                ...
                $legacy_notification = GFCommon::prepare_user_notification(
                    $form,
                    $lead
                );
                $notification = self::merge_legacy_notification(
                    $notification,
                    $legacy_notification
                );
            } elseif (rgar($notification, 'type') == 'admin') {
                //Getting admin notification from legacy structure (for backwards compatibility)
                $legacy_notification = GFCommon::prepare_admin_notification(
                    $form,
                    $lead
                );
                $notification = self::merge_legacy_notification(
    ...
    private static function merge_legacy_notification(
        $notification,
        $notification_data
    ) {
        $keys = [
    ...
    public static function get_notifications($event, $form)
    {
        ...
        foreach ($form['notifications'] as $notification) {
            ...
            $omit_from_resend = ['form_saved', 'form_save_email_requested'];
            if (
                $notification_event == $event ||
                ($event == 'resend_notifications' &&
                    !in_array($notification_event, $omit_from_resend))
            ) {
    ...
    {
        return (!empty($form['notification']['to']) ||
            !empty($form['notification']['routing'])) &&
            (!empty($form['notification']['subject']) ||
                !empty($form['notification']['message']));
    }

    public static function has_user_notification($form)
    {
        return !empty($form['autoResponder']['toField']) &&
            (!empty($form['autoResponder']['subject']) ||
                !empty($form['autoResponder']['message']));
    }
    ...
    public static function send_email(
        ...
        if (!GFCommon::is_valid_email_list($to)) {
            $error_info = esc_html__(
                'Cannot send email because the TO address is invalid.',
                'gravityforms'
            );
            GFFormsModel::add_notification_note(
                $entry_id,
                false,
                $notification,
                $error_info,
        ...
        if (is_wp_error($error)) {
            GFCommon::log_error(
                __METHOD__ . '(): ' . $error->get_error_message()
            );
            GFCommon::log_error(
                print_r(compact('to', 'subject', 'message'), true)
            );
        ...
        $message = self::format_email_message(
            $message,
            $message_format,
            $subject
        );
        ...
        $headers = [];
        $headers['From'] =
            'From: "' . wp_strip_all_tags($name, true) . '" <' . $from . '>';
        ...
        if (!$abort_email) {
            GFCommon::log_debug(
                __METHOD__ . '(): Sending email via wp_mail().'
            );
            GFCommon::log_debug(
                print_r(
                    compact(
            ...
            add_filter('wp_mail_content_type', [
                'GFCommon',
                'set_content_type_boundary',
            ]);
            add_filter('wp_mail_charset', ['GFCommon', 'set_mail_charset']);
            ...
            remove_filter('wp_mail_charset', ['GFCommon', 'set_mail_charset']);

            $result = is_wp_error($is_success)
                ? $is_success->get_error_message()
                : $is_success;
            ...
            GFCommon::log_debug(
                __METHOD__ . "(): Result from wp_mail(): {$result}"
            );

            if (!is_wp_error($is_success) && $is_success) {
                GFCommon::log_debug(
                    sprintf(
                        '%s(): WordPress successfully passed the notification email (#%s - %s)%s to the sending server.',
                        __METHOD__,
                        $notification['id'],
                        $notification['name'],
                        $entry_info
                ...
                GFCommon::log_error(
                    sprintf(
                        '%s(): WordPress was unable to send the notification email (#%s - %s)%s to the sending server.',
                        __METHOD__,
                        $notification['id'],
                        $notification['name'],
                        $entry_info
            ...
            if (has_filter('phpmailer_init')) {
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): The WordPress phpmailer_init hook has been detected, usually used by SMTP plugins. It can alter the email setup/content or sending server, and impact the notification deliverability.'
                );
            }
            ...
            if (!empty($error_info)) {
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): PHPMailer class returned an error message: ' .
                        $error_info
                );
            }
        } else {
            GFCommon::log_debug(
                sprintf(
                    '%s(): Aborting notification (#%s - %s)%s. The gform_pre_send_email hook was used to set the abort_email parameter to true.',
                    __METHOD__,
                    $notification['id'],
                    $notification['name'],
                    $entry_info
     ...
     * @return string
     */
    private static function format_email_message(
        $message,
        $message_format,
        $subject
    ) {
        switch (strtolower($message_format)) {
            ...
            case 'multipart':
                $html_message = self::format_html_message(
                    $message['html'],
                    $subject
                );
...
--{$boundary}--";
     ...
     */
    public static function current_user_can_uninstall(
        $caps = 'gravityforms_uninstall',
    ...
    {
        $types = ['creditcard', 'donation'];

        return in_array($field_type, $types, true) ||
            self::is_product_field($field_type);
    }
    ...
    public static function is_product_field($field_type)
    {
         ...
         */
        $product_fields = apply_filters('gform_product_field_types', [
            'option',
            'quantity',
            'product',
            'total',
    ...
    public static function has_update($use_cache = true)
    {
        ...
        return empty($version)
            ? false
            : version_compare(GFCommon::$version, $version, '<');
    }

    public static function get_key_info($key)
    {
        $options = ['method' => 'POST', 'timeout' => 3];
        $options['headers'] = [
            'Content-Type' =>
        ...
        $raw_response = self::post_to_manager(
            'api.php',
            "op=get_key&key={$key}",
            $options
        );
    ...
    public static function get_version_info($cache = true)
    {
        ...
        if (!$cache) {
            ...
            $cache_timestamp =
                $version_info && isset($version_info['timestamp'])
        ...
        if (!$version_info) {
            ...
            $options = ['method' => 'POST', 'timeout' => 20];
            $options['headers'] = [
                'Content-Type' =>
                    'application/x-www-form-urlencoded; charset=' .
                    get_option('blog_charset'),
            ...
            if (
                is_wp_error($raw_response) ||
                rgars($raw_response, 'response/code') != 200
            ) {
                $version_info = [
                    'is_valid_key' => '1',
                    'version' => '',
    ...
    public static function get_remote_request_params()
    {
        ...
        return sprintf(
            'of=GravityForms&key=%s&v=%s&wp=%s&php=%s&mysql=%s&version=2',
            urlencode(self::get_key()),
    ...
    public static function ensure_wp_version()
    {
        if (!GF_SUPPORTED_WP_VERSION) {
            echo "<div class='error' style='padding:10px;'>" .
                sprintf(
                    esc_html__(
                        'Gravity Forms requires WordPress %s or greater. You must upgrade WordPress in order to use Gravity Forms',
                        'gravityforms'
                    ),
                    GF_MIN_WP_VERSION
                ) .
    ...
    public static function check_update($option, $cache = true)
    {
        ...
        $plugin_path =
            plugin_basename(GFCommon::get_base_path()) . '/gravityforms.php';
    ...
    public static function cache_remote_message()
    {
        ...
        $body = "key=$key";
        $options = ['method' => 'POST', 'timeout' => 3, 'body' => $body];
        $options['headers'] = [
            'Content-Type' =>
                'application/x-www-form-urlencoded; charset=' .
                get_option('blog_charset'),
        ...
        $raw_response = self::post_to_manager(
            'message.php',
            GFCommon::get_remote_request_params(),
            $options
        );

        if (
            is_wp_error($raw_response) ||
            200 != $raw_response['response']['code']
        ) {
            $message = '';
        } else {
        ...
        if ($is_human) {
            ...
            if ($time_diff > 0 && $time_diff < 24 * 60 * 60) {
                $date_display = sprintf(
                    esc_html__('%s ago', 'gravityforms'),
                    human_time_diff($lead_gmt_time)
                );
            } else {
                $date_display = $include_time
                    ? sprintf(
                        esc_html__('%1$s at %2$s', 'gravityforms'),
                        date_i18n($date_format, $lead_local_time, true),
                        date_i18n(
            ...
            $date_display = $include_time
                ? sprintf(
                    esc_html__('%1$s at %2$s', 'gravityforms'),
                    date_i18n($date_format, $lead_local_time, true),
                    date_i18n(get_option('time_format'), $lead_local_time, true)
                )
                : date_i18n($date_format, $lead_local_time, true);
        }
    ...
    public static function selection_display(
        $value,
        $field,
        $currency = '',
        $use_text = false
    ...
    {
        $placeholder_value = GFCommon::replace_variables_prepopulate(
            $field->placeholder
        );

        return !empty($placeholder_value)
            ? sprintf("placeholder='%s'", esc_attr($placeholder_value))
            : '';
    }
    ...
    public static function get_input_placeholder_value($input)
    {
        ...
        return empty($placeholder)
            ? ''
            : GFCommon::replace_variables_prepopulate($placeholder);
    }

    public static function get_tabindex()
    {
        return GFCommon::$tab_index > 0
    ...
    public static function get_checkbox_choices($field, $value, $disabled_text)
    {
        _deprecated_function(
            ...
            '1.9',
            'GF_Field_Checkbox::get_checkbox_choices'
        );
     ...
     * @return mixed
     */
    public static function get_radio_choices(
        $field,
        $value = '',
        $disabled_text
    ) {
        _deprecated_function(
            'get_radio_choices',
            '1.9',
            'GF_Field_Checkbox::get_radio_choices'
        );
    ...
    public static function get_select_choices(
        $field,
        $value = '',
        $support_placeholders = true
    ) {
        $choices = '';
        $placeholder = '';

        if ($support_placeholders && !rgblank($field->placeholder)) {
            $placeholder = self::replace_variables_prepopulate(
                $field->placeholder
            );
        }
        ...
        if (is_array($field->choices)) {
            if (!rgblank($placeholder)) {
                ...
                $choices .= sprintf(
                    "<option value='' %s class='gf_placeholder'>%s</option>",
                    $selected,
            ...
            foreach ($field->choices as $choice) {
                ...
                $field_value =
                    ...
                    $field->type == 'post_category'
                        ? $choice['value']
                        : $choice['text'];
                if ($field->enablePrice) {
                    $price = rgempty('price', $choice)
                        ? 0
                        : GFCommon::to_number(rgar($choice, 'price'));
                    $field_value .= '|' . $price;
                }
                ...
                if (
                    ...
                    $selected = rgar($choice, 'isSelected')
                        ? "selected='selected'"
                        : '';
                } else {
                    if (is_array($value)) {
                        ...
                        $selected = RGFormsModel::choice_value_match(
                            $field,
                            $choice,
                            $value
                        )
    ...
    public static function get_section_fields($form, $section_field_id)
    {
        ...
        $in_section = false;
        foreach ($form['fields'] as $field) {
            if (in_array($field->type, ['section', 'page']) && $in_section) {
                return $fields;
            }
    ...
    {
        return sizeof(GFAPI::get_fields_by_type($form, ['page'])) > 0;
    }

    public static function get_product_fields_by_type(
        $form,
        $types,
        $product_id
    ) {
    ...
    {
        $editable_class = GFCommon::current_user_can_any(
            'gravityforms_edit_forms'
        )
    ...
    {
        _deprecated_function(
            'has_field_calculation',
            '1.7',
            'GF_Field::has_calculation'
        );
     ...
     */
    public static function get_field_input(
        $field,
        $value = '',
        $lead_id = 0,
        $form_id = 0,
        ...
        $field_input = gf_apply_filters(
            ['gform_field_input', $form_id, $field->id],
            '',
            $field,
            $value,
            $lead_id,
        ...
        switch ($type) {
            ...
            case 'adminonly_hidden':
                ...
                if (!is_array($inputs)) {
                    ...
                    return sprintf(
                        "<input name='input_%d' id='%s' class='gform_hidden' type='hidden' value='%s'/>",
                        $id,
                        esc_attr($field_id),
                        esc_attr($value)
                    );
                ...
                foreach ($inputs as $input) {
                    $fields .= sprintf(
                        "<input name='input_%s' class='gform_hidden' type='hidden' value='%s'/>",
                        $input['id'],
    ...
    public static function is_ssl()
    {
        ...
        $has_https_plugin =
            class_exists('WordPressHTTPS') && isset($wordpress_https);
        $has_is_ssl_method =
            $has_https_plugin && method_exists('WordPressHTTPS', 'is_ssl');
        $has_isSsl_method =
            $has_https_plugin && method_exists('WordPressHTTPS', 'isSsl');

        //Use the WordPress HTTPs plugin if installed
        if ($has_https_plugin && $has_is_ssl_method) {
            $is_ssl = $wordpress_https->is_ssl();
        } elseif ($has_https_plugin && $has_isSsl_method) {
    ...
    public static function clean_extensions($extensions)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            $extensions[$i] = str_replace(
                '.',
                '',
                str_replace(' ', '', $extensions[$i])
            );
        }
    ...
    public static function get_disallowed_file_extensions()
    {
        ...
        $extensions = apply_filters(
            'gform_disallowed_file_extensions',
            $extensions
        );
    ...
    {
        return self::match_file_extension(
            $file_name,
            self::get_disallowed_file_extensions()
        ) || strpos(strtolower($file_name), '.php.') !== false;
    }
    ...
    public static function check_type_and_ext($file, $file_name = '')
    {
        ...
        if (
            $proper_filename &&
            strtolower($proper_filename) !== strtolower($file_name)
        ) {
            return new WP_Error(
                'invalid_file',
                esc_html__('There was an problem while verifying your file.')
            );
        }
        ...
        if (!$ext) {
            return new WP_Error(
                'illegal_extension',
                esc_html__(
                    'Sorry, this file extension is not permitted for security reasons.'
                )
            );
        }
        if (!$type) {
    ...
    {
        _deprecated_function(
            'GFCommon::get_simple_captcha',
            '1.9',
            'GFField_CAPTCHA::get_simple_captcha'
        );
    ...
    {
        _deprecated_function(
            'GFCommon::get_captcha',
            '1.9',
            'GFField_CAPTCHA::get_captcha'
        );
    ...
    {
        _deprecated_function(
            'GFCommon::get_math_captcha',
            '1.9',
            'GFField_CAPTCHA::get_math_captcha'
        );
    ...
    public static function get_product_fields(
        ...
        if (!rgempty('id', $lead)) {
            $product_info = gform_get_meta(
                rgar($lead, 'id'),
                "gform_product_info_{$use_choice_text}_{$use_admin_label}"
            );
        }
        ...
        if (!$product_info) {
            foreach ($form['fields'] as $field) {
                ...
                $quantity_field = self::get_product_fields_by_type(
                    $form,
                    ['quantity'],
                    $id
                );
                $quantity =
                    sizeof($quantity_field) > 0 &&
                    !RGFormsModel::is_field_hidden(
                        $form,
                        $quantity_field[0],
                        [],
                ...
                switch ($field->type) {
                    case 'product':
                        ...
                        $is_hidden = RGFormsModel::is_field_hidden(
                            $form,
                            $field,
                            [],
                            $lead
                        ...
                        if (is_array($lead_value)) {
                            ...
                            $products[$id]['name'] =
                                $use_admin_label && !empty($field->adminLabel)
                                    ? $field->adminLabel
                                    : rgar($lead_value, $id . '.1');
                            $products[$id]['price'] = rgar(
                                $lead_value,
                                $id . '.2'
                            );
                            ...
                            if ($field->inputType == 'price') {
                                ...
                                if ($use_choice_text) {
                                    $name = RGFormsModel::get_choice_text(
                                        $field,
                                        $name
                                    );
                                }
                                 ...
                                 */
                                $include_field_label = apply_filters(
                        ...
                        if (isset($products[$id])) {
                            $option_fields = self::get_product_fields_by_type(
                                $form,
                                ['option'],
                                $id
                            );
                            foreach ($option_fields as $option_field) {
                                $option_value = RGFormsModel::get_lead_field_value(
                                    $lead,
                                    $option_field
                                );
                                $option_label =
                                    $use_admin_label &&
                                    !empty($option_field->adminLabel)
                                        ? $option_field->adminLabel
                                        : $option_field->label;
                                if (is_array($option_value)) {
                                    foreach ($option_value as $value) {
                                        $option_info = self::get_option_info(
                                            $value,
                                            $option_field,
                                            $use_choice_text
                                        );
                            ...
                            if (
                                empty($products[$id]['options']) &&
                                empty($products[$id]['name']) &&
                                rgblank($products[$id]['price'])
                            ) {
                                self::log_debug(
                                    __METHOD__ .
                                        "(): Product field #{$id} has no options, name, or price; removing."
                                );
                                unset($products[$id]);
                            }
                        }
            ...
            $shipping_fields = GFAPI::get_fields_by_type($form, ['shipping']);
            $shipping_price = $shipping_name = $shipping_field_id = '';

            if (
                !empty($shipping_fields) &&
                !RGFormsModel::is_field_hidden(
                    $form,
                    $shipping_fields[0],
                    [],
                ...
                $shipping_price = RGFormsModel::get_lead_field_value(
                    $lead,
                    $shipping_fields[0]
                );
                $shipping_name =
                    $use_admin_label && !empty($shipping_fields[0]->adminLabel)
                        ? $shipping_fields[0]->adminLabel
                        : $shipping_fields[0]->label;
                $shipping_field_id = $shipping_fields[0]->id;
                if (
                    $shipping_fields[0]->inputType != 'singleshipping' &&
                    !empty($shipping_price)
                ) {
                    list($shipping_method, $shipping_price) = explode(
                        '|',
                        $shipping_price
                    );
                    if ($use_choice_text) {
                        $shipping_method = RGFormsModel::get_choice_text(
                            $shipping_fields[0],
                            $shipping_method
                        );
                    }
             ...
             * @param array $lead         The current entry object.
             */
            $product_info = gf_apply_filters(
                ['gform_product_info', $form['id']],
                $product_info,
                $form,
                $lead
            );
    ...
    public static function gform_do_shortcode($content)
    {
        ...
        foreach ($forms as $form) {
             ...
             */
            $disable_print_form_script = apply_filters(
    ...
    public static function is_spam_entry($entry, $form)
    {
        ...
        if (self::akismet_enabled($form_id)) {
            ...
            self::log_debug(
                __METHOD__ . '(): Result from Akismet: ' . json_encode($is_spam)
            );
        }

        if (
            has_filter('gform_entry_is_spam') ||
            has_filter("gform_entry_is_spam_{$form_id}")
        ) {
            /**
            ...
            self::log_debug(
                __METHOD__ .
                    '(): Result from gform_entry_is_spam filter: ' .
                    json_encode($is_spam)
            );
        }
        ...
        self::log_debug(
            __METHOD__ . "(): Is submission considered spam? {$log_is_spam}."
        );
    ...
    public static function akismet_enabled($form_id)
    {
        ...
        $enabled =
            get_option('rg_gforms_enable_akismet') === false
                ? true
                : get_option('rg_gforms_enable_akismet') == true;
     ...
     * @return string
     */
    private static function get_akismet_fields(
        $form,
        $entry,
        $action = 'submit'
    ) {
        ...
        $akismet_fields['comment_author'] = self::get_akismet_field(
            'name',
            $form,
            $entry
        );
        $akismet_fields['comment_author_email'] = self::get_akismet_field(
            'email',
            $form,
            $entry
        );
        $akismet_fields['comment_author_url'] = self::get_akismet_field(
            'website',
            $form,
            $entry
        );
        $akismet_fields['comment_content'] = self::get_akismet_field(
            'textarea',
            $form,
            $entry
        );
        ...
        $akismet_fields['user_ip'] = preg_replace(
            '/[^0-9., ]/',
            '',
            rgar($entry, 'ip')
        );
        $akismet_fields['user_agent'] = rgar($entry, 'user_agent');
        $akismet_fields['referrer'] = $is_admin
            ? ''
            : rgar($_SERVER, 'HTTP_REFERER');
         ...
         * @param string $action         The action triggering the Akismet request: submit, spam, or ham.
         */
        $akismet_fields = gf_apply_filters(
            ['gform_akismet_fields', $form['id']],
            $akismet_fields,
            $form,
            $entry,
            $action
    ...
    {
        $disable_post = gf_apply_filters(
            ['gform_disable_post_creation', $form['id']],
            false,
            $form,
            $lead
        );
    ...
    public static function evaluate_conditional_logic($logic, $form, $lead)
    {
        ...
        $entry_meta_keys = array_keys(
            GFFormsModel::get_entry_meta($form['id'])
        );
        $match_count = 0;
        if (is_array($logic['rules'])) {
            foreach ($logic['rules'] as $rule) {
                if (in_array($rule['fieldId'], $entry_meta_keys)) {
                    ...
                    $source_field = GFFormsModel::get_field(
                        $form,
                        $rule['fieldId']
                    );
                    $field_value = empty($lead)
                        ? GFFormsModel::get_field_value($source_field, [])
                        : GFFormsModel::get_lead_field_value(
                            $lead,
                            $source_field
                        );
                    $is_value_match = GFFormsModel::is_value_match(
        ...
        $do_action =
            ($logic['logicType'] == 'all' &&
                $match_count == sizeof($logic['rules'])) ||
            ($logic['logicType'] == 'any' && $match_count > 0);
    ...
    public static function get_card_type($number)
    {
        ...
        if (
            $matched_card &&
            $matched_card['checksum'] &&
            !self::is_valid_card_checksum($number)
        ) {
            $matched_card = false;
        }
    ...
    public static function is_logging_plugin_active()
    {
        ...
        return is_plugin_active('gravityformslogging/logging.php') &&
            file_exists(
    ...
    public static function add_categories_as_choices($field, $value)
    {
        ...
        if ($has_placeholder) {
            $choices[] = [
                'text' => $field->categoryInitialItem,
                'value' => '',
                'isSelected' => true,
            ];
        }
        ...
        $args = gf_apply_filters(
            ['gform_post_category_args', $field->id],
            $args,
            $field
        );
        ...
        $categories = $walker->walk($terms_copy, 0, [0]); // 3rd parameter prevents notices triggered by $walker::display_element() function which checks $args[0]
         ...
         */
        $field->choices = gf_apply_filters(
            ['gform_post_category_choices', $form_id, $field->id],
            $field->choices,
            $field,
            $form_id
        );
    ...
    public static function prepare_post_category_value(
        $value,
        $field,
        $mode = 'entry_detail'
    ) {
            ...
            case 'entry_detail':
                $value =
                    RGFormsModel::get_input_type($field) == 'checkbox'
                        ? $cat_names
                        : self::implode_non_blank(', ', $cat_names);
                break;
    ...
    public static function calculate($field, $form, $lead)
    {
        ...
        if (empty($number_format)) {
            ...
            $number_format = self::is_currency_decimal_dot($currency)
                ? 'decimal_dot'
                : 'decimal_comma';
        }

        $formula = (string) apply_filters(
        ...
        if (is_array($matches)) {
            foreach ($matches as $match) {
                ...
                $value = self::get_calculation_value(
                    $input_id,
                    $form,
                    $lead,
                    $number_format
        ...
        if (!$result || !is_numeric($result) || !is_finite($result)) {
            GFCommon::log_debug(
                __METHOD__ .
                    '(): No result or non-numeric result. Returning zero instead.'
            );
            $result = 0;
        }
    ...
    public static function get_calculation_value(
        $field_id,
        $form,
        $lead,
        $number_format = ''
        ...
        foreach ($filters as $filter) {
            ...
            $replaced_value = GFCommon::replace_variables(
                "{:{$field_id}:$filter}",
                $form,
                $lead
            );
        ...
        if (!$value || !is_numeric($value)) {
            GFCommon::log_debug(
                "GFCommon::get_calculation_value(): No value or non-numeric value available for field #{$field_id}. Returning zero instead."
            );
            $value = 0;
        }
    ...
    {
        $has_currency =
            self::is_pricing_field($field->type) ||
            rgobj($field, 'numberFormat') == 'currency';
    ...
    public static function conditional_shortcode($attributes, $content = null)
    {
        extract(
            shortcode_atts(
                ...
                $attributes,
                'gravityforms_conditional'
            )
        );

        return RGFormsModel::matches_operation($merge_tag, $value, $condition)
    ...
    public static function is_valid_for_calcuation($field)
    {
        ...
        $unsupported_field_types = ['category'];
        $input_type = RGFormsModel::get_input_type($field);

        return in_array($input_type, $supported_input_types) &&
            !in_array($input_type, $unsupported_field_types);
    }
    ...
    {
        _deprecated_function(
            'GFCommon::echo_if() is deprecated',
            '1.9.9',
            'Use checked() or selected() instead.'
        );
    ...
    {
        $gf_global = [];
        $gf_global['gf_currency_config'] = RGCurrency::get_currency(
            GFCommon::get_currency()
        );
        $gf_global['base_url'] = GFCommon::get_base_url();
        $gf_global['number_formats'] = [];
        $gf_global['spinnerUrl'] =
            GFCommon::get_base_url() . '/images/spinner.gif';
    ...
    public static function gf_vars($echo = true)
    {
        ...
        $gf_vars['selectFormat'] = esc_html__(
            'Select a format',
            'gravityforms'
        );
        $gf_vars['editToViewAll'] = esc_html__(
            '5 of %d items shown. Edit field to view all',
            'gravityforms'
        );
        $gf_vars['enterValue'] = esc_html__('Enter a value', 'gravityforms');
        $gf_vars['formTitle'] = esc_html__('Untitled Form', 'gravityforms');
        $gf_vars['formDescription'] = esc_html__(
            'We would love to hear from you! Please fill out this form and we will get in touch with you shortly.',
            'gravityforms'
        );
        $gf_vars['formConfirmationMessage'] = esc_html__(
        ...
        $gf_vars['thisSectionIf'] = esc_html__(
            'this section if',
            'gravityforms'
        );
        $gf_vars['thisPage'] = esc_html__('this page', 'gravityforms');
        $gf_vars['thisFormButton'] = esc_html__(
            'this form button if',
            'gravityforms'
        );
        $gf_vars['show'] = esc_html__('Show', 'gravityforms');
        $gf_vars['hide'] = esc_html__('Hide', 'gravityforms');
        $gf_vars['all'] = esc_html(
            _x('All', 'Conditional Logic', 'gravityforms')
        );
        $gf_vars['any'] = esc_html(
            _x('Any', 'Conditional Logic', 'gravityforms')
        );
        $gf_vars['ofTheFollowingMatch'] = esc_html__(
        ...
        $gf_vars['emptyChoice'] = wp_strip_all_tags(
            __('Empty (no choices selected)', 'gravityforms')
        );

        $gf_vars['thisConfirmation'] = esc_html__(
            'Use this confirmation if',
            'gravityforms'
        );
        $gf_vars['thisNotification'] = esc_html__(
            'Send this notification if',
            'gravityforms'
        );
        $gf_vars['confirmationSave'] = esc_html__('Save', 'gravityforms');
        $gf_vars['confirmationSaving'] = esc_html__(
            'Saving...',
            'gravityforms'
        );
        $gf_vars['confirmationAreYouSure'] = __(
            'Are you sure you wish to cancel these changes?',
            'gravityforms'
        );
        $gf_vars['confirmationIssueSaving'] = __(
            'There was an issue saving this confirmation.',
            'gravityforms'
        );
        $gf_vars['confirmationConfirmDelete'] = __(
            'Are you sure you wish to delete this confirmation?',
            'gravityforms'
        );
        $gf_vars['confirmationIssueDeleting'] = __(
            'There was an issue deleting this confirmation.',
            'gravityforms'
        );
        $gf_vars['confirmationConfirmDiscard'] = __(
            'There are unsaved changes to the current confirmation. Would you like to discard these changes?',
            'gravityforms'
        );
        $gf_vars['confirmationDefaultName'] = __(
            'Untitled Confirmation',
            'gravityforms'
        );
        $gf_vars['confirmationDefaultMessage'] = __(
            'Thanks for contacting us! We will get in touch with you shortly.',
            'gravityforms'
        );
        $gf_vars['confirmationInvalidPageSelection'] = __(
            'Please select a page.',
            'gravityforms'
        );
        $gf_vars['confirmationInvalidRedirect'] = __(
            'Please enter a URL.',
            'gravityforms'
        );
        $gf_vars['confirmationInvalidName'] = __(
            'Please enter a confirmation name.',
            'gravityforms'
        );
        $gf_vars['confirmationDeleteField'] = __(
            "Warning! Deleting this field will also delete all entry data associated with it. 'Cancel' to stop. 'OK' to delete.",
            'gravityforms'
        );

        $gf_vars['conditionalLogicDependency'] = __(
            "Warning! This form contains conditional logic dependent upon this field. Deleting this field will deactivate those conditional logic rules and also delete all entry data associated with the field. 'OK' to delete, 'Cancel' to abort.",
            'gravityforms'
        );
        $gf_vars['conditionalLogicDependencyChoice'] = __(
            "This form contains conditional logic dependent upon this choice. Are you sure you want to delete this choice? 'OK' to delete, 'Cancel' to abort.",
            'gravityforms'
        );
        $gf_vars['conditionalLogicDependencyChoiceEdit'] = __(
        ...
        $gf_vars['conditionalLogicDependencyAdminOnly'] = __(
            "This form contains conditional logic dependent upon this field. Are you sure you want to mark this field as Admin Only? 'OK' to confirm, 'Cancel' to abort.",
            'gravityforms'
        );

        $gf_vars['mergeTagsTooltip'] =
            '<h6>' .
            esc_html__('Merge Tags', 'gravityforms') .
            '</h6>' .
            esc_html__(
                'Merge tags allow you to dynamically populate submitted field values in your form content wherever this merge tag icon is present.',
                'gravityforms'
            );

        $gf_vars['baseUrl'] = GFCommon::get_base_url();
        $gf_vars['gf_currency_config'] = RGCurrency::get_currency(
            GFCommon::get_currency()
        );
        ...
        $gf_vars['addFieldFilter'] = esc_html__(
            'Add a condition',
            'gravityforms'
        );
        $gf_vars['removeFieldFilter'] = esc_html__(
            'Remove a condition',
            'gravityforms'
        );
        $gf_vars['filterAndAny'] = esc_html__(
            'Include results if {0} match:',
            'gravityforms'
        );

        $gf_vars['customChoices'] = esc_html__(
            'Custom Choices',
            'gravityforms'
        );
        $gf_vars['predefinedChoices'] = esc_html__(
            'Predefined Choices',
            'gravityforms'
        );
        ...
        if (is_admin() && rgget('id')) {
            ...
            $gf_vars['mergeTags'] = GFCommon::get_merge_tags(
                $form['fields'],
                '',
                false
            );
            ...
            $gf_vars['addressTypes'] = $address_field->get_address_types(
                $form['id']
            );
            $gf_vars[
                'defaultAddressType'
            ] = $address_field->get_default_address_type($form['id']);
        }
        ...
        $gf_vars['nameFieldDefaultPrefixes'] = [];
        foreach ($prefixes as $prefix) {
     ...
     * @since 2.0
     */
    public static function add_dismissible_message(
        $text,
        $key,
        $type = 'warning',
        $capabilities = false,
        ...
        if ($sticky) {
            $sticky_messages = get_option('gform_sticky_admin_messages', []);
     ...
     * @since 2.0
     */
    public static function display_dismissible_message(
        $messages = false,
        $page = null
    ) {
        if (!$messages) {
        ...
        if (!empty($messages)) {
            foreach ($messages as $message) {
                ...
                if (
                    isset($sticky_messages[$message['key']]) &&
                    isset($message['page']) &&
                    $message['page'] &&
                    $page !== $message['page']
                ) {
                ...
                if (
                    empty($message['key']) ||
                    self::is_message_dismissed($message['key'])
                ) {
                    continue;
                }

                if (
                    isset($message['capabilities']) &&
                    $message['capabilities'] &&
                    !GFCommon::current_user_can_any($message['capabilities'])
                ) {
                    continue;
                }

                $class = in_array($message['type'], [
                    'warning',
                    'error',
                    'updated',
                    'success',
				...
				<div class="notice below-h1 notice-<?php echo $class; ?> is-dismissible"
				     ...
				     data-gf_dismissible_nonce="<?php echo wp_create_nonce(
             'gf_dismissible_nonce'
         ); ?>">
					<p>
    ...
    public static function get_field_filter_settings($form)
    {
        ...
        foreach ($form['fields'] as $field) {
            ...
            if ($input_type == 'likert') {
                ...
                if (
                    $field->gsurveyLikertEnableMultipleRows &&
                    !isset($filter_settings['group'])
                ) {
                    $sub_filters = [];
                    ...
                    foreach ($rows as $row) {
                        $sub_filter = [];
                        $sub_filter['key'] =
                            $filter_settings['key'] . '|' . rgar($row, 'value');
                    ...
                    unset(
                        $filter_settings['values'],
                        $filter_settings['preventMultiple'],
                        $filter_settings['operators']
                    );
                }
            }
    ...
    public static function get_entry_payment_statuses()
    {
         ...
         */
        $payment_statuses = apply_filters(
    ...
    public static function get_entry_meta_filter_settings($form_id)
    {
        ...
        foreach ($entry_meta as $key => $meta) {
            if (isset($meta['filter'])) {
                $filter = [];
                $filter['key'] = $key;
                $filter['preventMultiple'] = isset(
                    $meta['filter']['preventMultiple']
                )
                    ? $meta['filter']['preventMultiple']
                    : false;
                $filter['text'] = rgar($meta, 'label');
                $filter['operators'] = isset($meta['filter']['operators'])
        ...
        if (is_array($filter_fields)) {
            ...
            for ($i = 0; $i < count($filter_fields); $i++) {
                ...
                if ($field) {
                    ...
                    if (
                        $field->type == 'product' &&
                        in_array($input_type, ['radio', 'select'])
                    ) {
                        $operator = 'contains';
                    }
                }
    ...
    {
        $fileupload_fields = GFAPI::get_fields_by_type($form, [
            'fileupload',
            'post_custom_field',
        ]);
    ...
    {
        wp_localize_script('gform_gravityforms', 'gform_gravityforms', [
            'strings' => [
                'invalid_file_extension' => wp_strip_all_tags(
                    __(
                        'This type of file is not allowed. Must be one of the following: ',
                        'gravityforms'
                    )
                ),
                'delete_file' => wp_strip_all_tags(
                    __('Delete this file', 'gravityforms')
                ),
                'in_progress' => wp_strip_all_tags(
                    __('in progress', 'gravityforms')
                ),
                'file_exceeds_limit' => wp_strip_all_tags(
                    __('File exceeds size limit', 'gravityforms')
                ),
                'illegal_extension' => wp_strip_all_tags(
                    __('This type of file is not allowed.', 'gravityforms')
                ),
                'max_reached' => wp_strip_all_tags(
                    __('Maximum number of files reached', 'gravityforms')
                ),
                ...
                'currently_uploading' => wp_strip_all_tags(
                    __(
                        'Please wait for the uploading to complete',
                        'gravityforms'
                    )
                ),
                'cancel' => wp_strip_all_tags(__('Cancel', 'gravityforms')),
                'cancel_upload' => wp_strip_all_tags(
                    __('Cancel this upload', 'gravityforms')
                ),
                'cancelled' => wp_strip_all_tags(
                    __('Cancelled', 'gravityforms')
                ),
            ],
            'vars' => [
    ...
    public static function send_resume_link(
        $message,
        $subject,
        $email,
        $embed_url,
        ...
        $resume_url = add_query_arg(['gf_token' => $resume_token], $embed_url);
    ...
    public static function trim_deep($array)
    {
        ...
        return array_map(['GFCommon', 'trim_deep'], $array);
    }
    ...
    public static function encrypt(
        $text,
        $key = null,
        $mcrypt_cipher_name = false
    ) {
        _deprecated_function(
            'GFCommon::encrypt()',
            '2.3',
            'GFCommon::openssl_encrypt()'
        );

        $use_mcrypt = apply_filters(
            'gform_use_mcrypt',
        ...
        if ($use_mcrypt) {
            ...
            $key = !is_null($key)
                ? $key
                : substr(md5(wp_salt('nonce')), 0, $iv_size);

            $encrypted_value = trim(
                base64_encode(
                    mcrypt_encrypt(
                        $mcrypt_cipher_name,
                        $key,
    ...
    public static function openssl_encrypt(
        ...
        if (function_exists('openssl_encrypt')) {
            ...
            if (empty($encryption_key)) {
                $encryption_key =
                    'gravityforms_encryption_key' . wp_salt('nonce');
            }
    ...
    {
        $is_entry_detail =
            GFForms::get_page() == 'entry_detail_edit' ||
            GFForms::get_page() == 'entry_detail';
     ...
     */
    public static function load_gf_text_domain(
        $domain = 'gravityforms',
        $basename = ''
    ) {
    ...
    public static function replace_field_variable(
        $text,
        $form,
        $lead,
        $url_encode,
        ...
        $value = self::format_variable_value(
            $value,
            $url_encode,
            $esc_html,
            $format,
        ...
        $value = $field->get_value_merge_tag(
            $value,
            $input_id,
            $lead,
            $form,
        ...
        if (!in_array($field->type, ['html', 'section', 'signature'])) {
        ...
        if (in_array('label', $modifiers)) {
            if (empty($value)) {
                ...
                $field->set_context_property(
                    'use_admin_label',
                    in_array('admin', $modifiers)
                );
                $value =
                    $format == 'text'
                        ? sanitize_text_field(self::get_label($field))
     ...
     * @return string
     */
    public static function maybe_wp_kses(
        $html,
        $allowed_html = 'post',
        $allowed_protocols = []
    ) {
     ...
     * @return string
     */
    public static function maybe_sanitize_confirmation_message(
         ...
         */
        $sanitize_confirmation_nessage = apply_filters(
            'gform_sanitize_confirmation_message',
            $sanitize_confirmation_nessage
        );
    ...
    {
        $key =
            absint($form_id) . ':' . absint($field_id) . ':' . urlencode($file);
    ...
    public static function get_visibility_options()
    {
        $options = [
            [
                ...
                'value' => 'visible',
                'description' => __(
                    'Default option. The field is visible when viewing the form.',
                    'gravityforms'
                ),
            ],
            [
                ...
                'value' => 'hidden',
                'description' => __(
                    'The field is hidden when viewing the form. Useful when you require the functionality of this field but do not want the user to be able to see this field.',
                    'gravityforms'
                ),
            ],
            [
                ...
                'description' => __(
                    'The field is only visible when administering submitted entries. The field is not visible or functional when viewing the form.',
                    'gravityforms'
                ),
            ],
        ];
    ...
    public static function get_visibility_tooltip()
    {
        ...
        $markup = sprintf(
            '<ul><li>%s</li></ul>',
            implode('</li><li>', $markup)
        );

        return sprintf(
            '<h6>%s</h6> %s<br><br>%s',
            __('Visibility', 'gravityforms'),
            __('Select the visibility for this field.', 'gravityforms'),
            $markup
        );
    }
    ...
    public static function update_site_registration($new_key, $is_md5 = false)
    {
        ...
        if (empty($new_key)) {
            ...
            $is_valid_new =
                $version_info['is_valid_key'] && !$is_site_registered;
            $is_valid_registered =
                $version_info['is_valid_key'] && $is_site_registered;
            ...
            if ($is_valid_new) {
                ...
                $result = new WP_Error(
                    'invalid_license',
                    'Invalid license. Site cannot be registered'
                );
                GFCommon::log_error(
                    'Invalid license. Site cannot be registered'
                );
        ...
        if (is_wp_error($result)) {
            GFCommon::log_error(
                'Failed to update site registration with Gravity Manager. ' .
                    print_r($result, true)
            );
        }
    ...
    {
        GFCommon::log_debug(
            __METHOD__ . '(): Email address: ' . $email_address
        );

        if (!is_email($email_address)) {
            GFCommon::log_debug(
                __METHOD__ . '(): Email address failed is_email() validation.'
            );
            return false;
        }
        ...
        $domain_matches =
            strpos($domain, array_pop($email_domain)) !== false ? true : false;
        GFCommon::log_debug(
            __METHOD__ .
                '(): Domain matches? ' .
                var_export($domain_matches, true)
        );
    ...
    public static function random_str($length = 64)
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            $rand = function_exists('random_int')
                ? random_int(0, $max)
                : rand(0, $max);
...
class GFCategoryWalker extends Walker
{
     ...
     * @var array
     */
    var $db_fields = ['parent' => 'parent', 'id' => 'term_id'];
...
class GFCache
{
    ...
    public static function get($key, &$found = null, $is_persistent = true)
    {
        ...
        if (false === $data) {
            ...
            self::$_cache[$key] = ['data' => $data, 'is_persistent' => true];
            $found = true;
...
class EncryptDB extends wpdb
{
    ...
    public static function encrypt($text, $key)
    {
        ...
        $encrypted = base64_encode(
            $db->get_var(
                $db->prepare('SELECT AES_ENCRYPT(%s, %s) AS data', $text, $key)
            )
        );
    ...
    public static function decrypt($text, $key)
    {
        ...
        $decrypted = $db->get_var(
            $db->prepare(
                'SELECT AES_DECRYPT(%s, %s) AS data',
                base64_decode($text),
                wp_salt('nonce')
            )
        );
