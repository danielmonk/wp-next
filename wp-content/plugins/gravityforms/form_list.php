...
class GFFormList
{
    public static function form_list_page()
    {
        ...
        add_action(
            ...
            [__CLASS__, 'output_form_list_script_block'],
            20
        );
		...
		<div id="gf_new_form_modal" style="display:none;">
			<form class="gf_new_form_modal_container" onsubmit="saveNewForm();return false;">
				...
				<div class="setting-row">
					<label for="new_form_description"><?php esc_html_e(
         'Form Description',
         'gravityforms'
     ); ?></label><br />
      ...
      */
     echo apply_filters(
         'gform_new_form_button',
         '<input id="save_new_form" type="submit" class="button button-large button-primary" value="' .
             esc_html__('Create Form', 'gravityforms') .
             '" tabindex="9002" />'
     ); ?>
        ...
        __(
            'WARNING: You are about to delete this form and ALL entries associated with it. ',
            'gravityforms'
        ) . esc_html__('Cancel to stop, OK to delete.', 'gravityforms')
    ); ?>) ){
					DeleteForm(form_id);
				}
			}
				...
				jQuery.ajax(
					{
						...
						data: {
							...
							rg_update_form_active: '<?php echo wp_create_nonce(
           ...
           __('Ajax error while updating form', 'gravityforms')
       ); ?>' );
						}
					}
				);
     ...
     esc_attr__('Active', 'gravityforms')
			...
			function gfConfirmBulkAction(element_id) {
				...
				if (jQuery(element).val() == 'delete')
					return confirm(<?php echo json_encode(
         __(
             'WARNING: You are about to delete these forms and ALL entries associated with them. ',
             'gravityforms'
         ...
         __(
             ...
             'gravityforms'
         ) . __("'Cancel' to stop, 'OK' to reset.", 'gravityforms')
     ); ?>);
				else if (jQuery(element).val() == 'delete_entries')
					return confirm(<?php echo json_encode(
         __(
             'WARNING: You are about to delete ALL entries associated with the selected forms. ',
             'gravityforms'
         ) . __("'Cancel' to stop, 'OK' to delete.", 'gravityforms')
     ); ?>);
  ...
  ); ?>/css/admin<?php echo $min; ?>.css?ver=<?php echo GFForms::$version; ?>"/>
		<div class="wrap <?php echo sanitize_html_class(
      GFCommon::get_browser_class()
  ); ?>">
   ...
   if (GFCommon::current_user_can_any('gravityforms_create_form')) {
       echo '<a class="add-new-h2" href="" onclick="return loadNewFormModal();" onkeypress="return loadNewFormModal();">' .
  ...
  ?>
    ...
    public static function save_new_form()
    {
        ...
        if (empty($form['title'])) {
            $result = [
                'error' => __('Please enter a form title.', 'gravityforms'),
            ];
        ...
        switch (rgar($result, 'status')) {
            ...
            default:
                ...
                die(
                    json_encode([
                        'redirect' => admin_url(
                            "admin.php?page=gf_edit_forms&id={$form_id}&isnew=1"
                        ),
                    ])
                );
        }
    ...
    public static function output_form_list_script_block()
    {
        ...
        esc_html__('Create a New Form', 'gravityforms')
			...
			function saveNewForm() {
				...
				createButton.val(<?php echo json_encode(
        esc_html__('Creating Form...', 'gravityforms')
    ); ?>);
				...
				jQuery.post(ajaxurl, {
					...
					gf_save_new_form: <?php echo json_encode(
         wp_create_nonce('gf_save_new_form')
     ); ?>
				}, function(response){
					...
					if(respData['error']) {
						...
						createButton.val(<?php echo json_encode(
          esc_html__('Saved! Redirecting...', 'gravityforms')
      ); ?>);
					}
...
class GF_Form_List_Table extends WP_List_Table
{
    ...
    function get_sortable_columns()
    {
        return [
            'title' => ['title', false],
            'id' => ['id', false],
            'entry_count' => ['entry_count', false],
            'view_count' => ['view_count', false],
    ...
    function get_views()
    {
        ...
        $views = [
            'all' =>
                '<a class="' .
                $all_class .
                '" href="?page=gf_edit_forms">' .
                esc_html(_x('All', 'Form List', 'gravityforms')) .
                ' <span class="count">(<span id="all_count">' .
                $form_count['total'] .
                '</span>)</span></a>',
            'active' =>
                '<a class="' .
                $active_class .
                '" href="?page=gf_edit_forms&filter=active">' .
                esc_html(_x('Active', 'Form List', 'gravityforms')) .
                ' <span class="count">(<span id="active_count">' .
                $form_count['active'] .
                '</span>)</span></a>',
            'inactive' =>
                '<a class="' .
                $inactive_class .
                '" href="?page=gf_edit_forms&filter=inactive">' .
                esc_html(_x('Inactive', 'Form List', 'gravityforms')) .
                ' <span class="count">(<span id="inactive_count">' .
                $form_count['inactive'] .
                '</span>)</span></a>',
            'trash' =>
                '<a class="' .
                $trash_class .
                '" href="?page=gf_edit_forms&filter=trash">' .
                esc_html(_x('Trash', 'Form List', 'gravityforms')) .
                ' <span class="count">(<span id="trash_count">' .
                $form_count['trash'] .
                '</span>)</span></a>',
        ];
        return $views;
    ...
    function prepare_items()
    {
        ...
        $sort_direction = empty($_GET['order'])
            ? 'ASC'
        ...
        if (rgblank($search_query)) {
            ...
            $forms = GFFormsModel::search_forms(
                $search_query,
                $active,
                $sort_column,
                $sort_direction,
        ...
        if (
            in_array($sort_column, ['view_count', 'entry_count', 'conversion'])
        ) {
            usort($forms, [
                $this,
                'compare_' . $sort_column . '_' . $sort_direction,
            ]);
        }
    ...
    function single_row_columns($item)
    {
        ...
        foreach ($columns as $column_name => $column_display_name) {
            ...
            $data =
                'data-colname="' .
                wp_strip_all_tags($column_display_name) .
                '"';
    ...
    function _column_is_active($form, $classes, $data, $primary)
    {
        ...
        if ($this->filter !== 'trash') { ?>
			<img class="gform_active_icon" src="<?php echo esc_url(
       GFCommon::get_base_url()
   ); ?>/images/active<?php echo intval(
    $form->is_active
); ?>.png" style="cursor: pointer;" alt="<?php echo $form->is_active
    ? esc_attr__('Active', 'gravityforms')
    : esc_attr__(
        'Inactive',
        'gravityforms'
    ); ?>" title="<?php echo $form->is_active
    ? esc_attr__('Active', 'gravityforms')
    : esc_attr__(
        'Inactive',
        'gravityforms'
    ); ?>" onclick="ToggleActive(this, <?php echo absint(
    $form->id
    ...
    {
        echo '<strong><a href="?page=gf_edit_forms&id=' .
            absint($form->id) .
            '">' .
            esc_html($form->title) .
            '</a></strong>';
    ...
    {
        echo '<a href="?page=gf_edit_forms&id=' .
            absint($form->id) .
            '">' .
            absint($form->id) .
            '</a>';
    ...
    {
        echo '<a href="?page=gf_entries&id=' .
            absint($form->id) .
            '">' .
            absint($form->entry_count) .
            '</a>';
    }
    ...
    function column_conversion($form)
    {
        ...
        if ($form->view_count > 0) {
            $conversion =
                (float) number_format(
                    $form->entry_count / $form->view_count,
                    3
                ) *
                    100 .
                '%';
    ...
    function column_cb($form)
    {
      ...
      $form_id
  ); ?>"><?php _e('Select form'); ?></label>
		<input type="checkbox" class="gform_list_checkbox" name="form[]" value="<?php echo esc_attr(
      $form_id
  ); ?>" />
   ...
   if ($this->filter == 'trash') {
       ...
       $form_actions['duplicate'] = [
           ...
           'url' => '#',
           'onclick' =>
               'DuplicateForm(' . absint($form->id) . ');return false;',
           'onkeypress' =>
               'DuplicateForm(' . absint($form->id) . ');return false;',
   ...
   ?>
		...
		<?php return $column_name === $primary
    ...
    function no_items()
    {
        if (rgget('s')) {
            printf(
                esc_html__(
                    "No forms were found for your search query. %sView all forms%s.",
                    'gravityforms'
                ),
    ...
    function process_action()
    {
        ...
        if ($single_action) {
            ...
            switch ($single_action) {
                case 'trash':
                    if (
                        ...
                        $message = is_wp_error($trashed)
                            ? $trashed->get_error_message()
                            : __('Form moved to the trash.', 'gravityforms');
                        $message_class = is_wp_error($trashed)
                            ? 'error'
                            : 'updated';
                    } else {
                        $message = __(
                            "You don't have adequate permission to trash forms.",
                            'gravityforms'
                        );
                        $message_class = 'error';
                    }
                ...
                case 'restore':
                    if (
                        ...
                        $message = is_wp_error($restored)
                            ? $restored->get_error_message()
                            : __('Form restored.', 'gravityforms');
                        $message_class = is_wp_error($restored)
                            ? 'error'
                            : 'updated';
                    } else {
                        $message = __(
                            "You don't have adequate permission to restore forms.",
                            'gravityforms'
                        );
                        $message_class = 'error';
                    }
                ...
                case 'delete':
                    if (
                        ...
                        $message = is_wp_error($deleted)
                            ? $deleted->get_error_message()
                            : __('Form deleted.', 'gravityforms');
                        $message_class = is_wp_error($deleted)
                            ? 'error'
                            : 'updated';
                    } else {
                        $message = __(
                            "You don't have adequate permission to delete forms.",
                            'gravityforms'
                        );
                        $message_class = 'error';
                    }
                ...
                case 'duplicate':
                    if (
                        ...
                        $message = is_wp_error($duplicated)
                            ? $duplicated->get_error_message()
                            : __('Form duplicated.', 'gravityforms');
                        $message_class = is_wp_error($duplicated)
                            ? 'error'
                            : 'updated';
                    } else {
                        $message = __(
                            "You don't have adequate permission to duplicate forms.",
                            'gravityforms'
                        );
                        $message_class = 'error';
                    }
            ...
            $form_ids = is_array(rgpost('form')) ? rgpost('form') : [];
            ...
            switch ($bulk_action) {
                case 'trash':
                    if (
                        ...
                        $message = _n(
                            '%s form moved to the trash.',
                            '%s forms moved to the trash.',
                            $form_count,
                            'gravityforms'
                        );
                    } else {
                        $message = __(
                            "You don't have adequate permissions to trash forms.",
                            'gravityforms'
                        );
                    }
                    break;
                case 'restore':
                    if (
                        ...
                        $message = __(
                            "You don't have adequate permissions to restore forms.",
                            'gravityforms'
                        );
                    }
                    break;
                case 'delete':
                    if (
                        ...
                        $message = __(
                            "You don't have adequate permissions to delete forms.",
                            'gravityforms'
                        );
                    }
                    break;
                ...
                case 'delete_entries':
                    if (
                        ...
                        $message = _n(
                            'Entries for %s form have been deleted.',
                            'Entries for %s forms have been deleted.',
                            $form_count,
                            'gravityforms'
                        );
                    } else {
                ...
                case 'activate':
                    ...
                    $message = _n(
                        '%s form has been marked as active.',
                        '%s forms have been marked as active.',
                        $form_count,
                        'gravityforms'
                    );
                    break;
                case 'deactivate':
                    ...
                    $message = _n(
                        '%s form has been marked as inactive.',
                        '%s forms have been marked as inactive.',
                        $form_count,
                        'gravityforms'
                    );
                    break;
        ...
        if (!empty($message)) {
            echo '<div id="message" class="' .
                (isset($message_class) ? $message_class : 'updated') .
    ...
    {
        echo '<tr class="' .
            $this->locking_info->list_row_class($form->id, false) .
            '">';
    ...
    {
        $a_conversion =
            $a->view_count > 0 ? $a->entry_count / $a->view_count : 0;
        $b_conversion =
            $b->view_count > 0 ? $b->entry_count / $b->view_count : 0;
