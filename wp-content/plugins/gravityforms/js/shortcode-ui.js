...
(function (gfShortCodeUI, $) {
  ...
  sui.models.Shortcode = Backbone.Model.extend({
    ...
    set: function (attributes, options) {
      if (
        ...
        !(attributes.attrs instanceof sui.models.ShortcodeAttributes)
      ) {
    ...
    toJSON: function (options) {
      ...
      if (
        options.attrs !== undefined &&
        options.attrs instanceof sui.models.ShortcodeAttributes
      ) {
    ...
    formatShortcode: function () {
      ...
      this.get("attrs").each(function (attr) {
        ...
        if (
          ((!val || val.length < 1) && type != "checkbox") ||
          (type == "checkbox" && def != "true" && !val)
        ) {
          return;
        }
  ...
  sui.views.editShortcodeForm = wp.Backbone.View.extend({
    ...
    dispose: function () {
      this.remove();
      $("#gform-shortcode-ui-wrap").append(
  ...
  sui.views.editAttributeField = Backbone.View.extend({
    ...
    render: function () {
      this.template = wp.media.template(
        "gf-shortcode-ui-field-" + this.model.get("type")
      );
    ...
    updateAction: function (e) {
      ...
      var newShortcodeModel = sui.shortcodes.findWhere({
      ...
      GformShortcodeUI = new sui.views.editShortcodeForm({
  ...
  sui.utils.shortcodeViewConstructor = {
    ...
    getShortcodeModel: function (options) {
      var actionTag =
        typeof options.attrs.named.action != "undefined"
          ? options.attrs.named.action
          : "";
    ...
    fetch: function () {
      ...
      if (!this.fetching) {
        ...
        $.post(ajaxurl, data)
          ...
          .fail(function () {
            self.content =
              '<span class="gf_shortcode_ui_error">' +
              gfShortcodeUIData.strings.errorLoadingPreview +
              "</span>";
          })
          .always(function () {
    ...
    View: {
      ...
      initialize: function (options) {
        var actionTag =
          typeof options.shortcode.attrs.named.action != "undefined"
            ? options.shortcode.attrs.named.action
            : "";
        ...
        shortcode.get("attrs").each(function (attr) {
          if (attr.get("attr") in options.shortcode.attrs.named) {
            attr.set("value", options.shortcode.attrs.named[attr.get("attr")]);
          }

          if (
            attr.get("attr") === "content" &&
      ...
      setIframes: function (body) {
        ...
        this.getNodes(function (editor, node, content) {
          ...
          if (!wp.mce.views.sandboxStyles) {
            tinymce.each(
              dom.$('link[rel="stylesheet"]', editor.getDoc().head),
              function (link) {
                if (
                  link.href &&
    ...
    edit: function (shortcodeString) {
      ...
      if ("object" === typeof shortcodeString) {
        shortcodeString = decodeURIComponent(
          jQuery(shortcodeString).attr("data-wpview-text")
        );
      }
      ...
      if (currentShortcode) {
        var action = currentShortcode.shortcode.attrs.named.action
          ? currentShortcode.shortcode.attrs.named.action
          : "";
        ...
        var idAttr = currentShortcodeModel
          .get("attrs")
          .findWhere({ attr: "id" });
        ...
        tb_show(
          "Edit Gravity Form",
          "#TB_inline?inlineId=select_gravity_form&width=753&height=686",
          ""
        );
      }
    },
  ...
  $(document).ready(function () {
    ...
    sui.shortcodes = new sui.collections.Shortcodes(
      gfShortcodeUIData.shortcodes
    );
    ...
    $(document).on("click", ".gform_media_link", function () {
      ...
      var shortcode = sui.shortcodes.findWhere({
        shortcode_tag: "gravityform",
        action_tag: "",
      });
      GformShortcodeUI = new sui.views.editShortcodeForm({
