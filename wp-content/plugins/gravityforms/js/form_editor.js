...
jQuery(document).ready(function () {
  ...
  jQuery(".field_type input").each(function () {
    ...
    if (typeof type == "undefined") {
      ...
      if (window.console) {
        console.log(
          "Deprecated button for the " +
            this.value +
            ' field. Since v1.9 the field type must be specified in the "type" data attribute.'
        );
      }
    }
    if (
  ...
  jQuery("#gform_fields").sortable({
    ...
    beforeStop: function (event, ui) {
      ...
      var li =
        "<li id='gform_adding_field_spinner'><i class='gficon-gravityforms-spinner-icon gficon-spin'></i></li>";
  ...
  if (
    typeof gf_global["view"] == "undefined" ||
    gf_global["view"] != "settings"
  )
    InitializeForm(form);
    ...
    if (data) {
      ...
      for (var e in data.events) {
        if (jQuery.inArray(e, deprecatedEvents) !== -1) {
          console.log(
            'Gravity Forms API warning: The jQuery event "' +
              e +
              '" is deprecated on this page since version 1.7'
          );
        }
      }
    }
  ...
  jQuery(document).on(
    "focus",
    "#field_choices input.field-choice-text, #field_choices input.field-choice-value",
    function () {
...
function InitializeFieldSettings() {
  ...
  jQuery(".field_input_choice_values_enabled").on(
    "click keypress",
    function () {
      var $container = jQuery(this)
        .parent()
        .siblings(".gfield_settings_input_choices_container");
  ...
  jQuery("#field_rich_text_editor").on("click keypress", function () {
    ...
    if (this.checked) {
      ...
      if (disablePlaceHolder) {
        jQuery("#field_placeholder, #field_placeholder_textarea").prop(
          "disabled",
          true
        );
  ...
  jQuery(
    ...
    .on("click keypress", ".input_active_icon", function () {
      var inputId = jQuery(this)
        .closest(".field_custom_input_row")
        .data("input_id");
      ToggleInputHidden(this, inputId);
    })
    .on(
      "input propertychange",
      ".field_custom_input_default_label",
      function () {
        var inputId = jQuery(this)
          .closest(".field_custom_input_row")
          .data("input_id");
  ...
  jQuery(".default_input_values_setting")
    .on("input propertychange", ".default_input_value", function () {
      var inputId = jQuery(this)
        .closest(".default_input_value_row")
        .data("input_id");
  ...
  jQuery(".choices_setting, .columns_setting").on(
    "input propertychange",
    ".field-choice-input",
    function (e) {
      var $this = jQuery(this);
      ...
      if (
        $this.hasClass("field-choice-text") ||
        $this.hasClass("field-choice-value")
      ) {
  ...
  jQuery("#field_copy_values_option_label").on(
    "input propertychange",
    function () {
  ...
  jQuery("#next_button_text_input, #next_button_image_url").on(
    "input propertychange",
    function () {
      SetPageButton("next");
    }
  ...
  jQuery("#previous_button_image_url, #previous_button_text_input").on(
    "input propertychange",
    function () {
  ...
  jQuery("#field_custom_field_name_text").on(
    "input propertychange",
    function () {
  ...
  jQuery("#field_customfield_content_template").on(
    "input propertychange",
    function () {
...
function InitializeForm(form) {
  ...
  jQuery("#last_page_button_text_input").val(
    form.lastPageButton ? form.lastPageButton.text : gf_vars["previousLabel"]
  );
  jQuery("#last_page_button_image_url").val(
    form.lastPageButton ? form.lastPageButton.imageUrl : ""
  );
  TogglePageButton("last_page", true);

  if (form.postStatus) jQuery("#field_post_status").val(form.postStatus);

  if (form.postAuthor) jQuery("#field_post_author").val(form.postAuthor);

  //default to checked
  if (form.useCurrentUserAsAuthor === undefined)
    form.useCurrentUserAsAuthor = true;

  jQuery("#gfield_current_user_as_author").prop(
    "checked",
    form.useCurrentUserAsAuthor ? true : false
  );

  if (form.postCategory) jQuery("#field_post_category").val(form.postCategory);

  if (form.postFormat) jQuery("#field_post_format").val(form.postFormat);
  ...
  jQuery("#gform_last_page_settings").click(function () {
    FieldClick(this);
  });
  jQuery("#gform_pagination").click(function () {
    FieldClick(this);
  });
  ...
  var paginationType =
    form["pagination"] && form["pagination"]["type"]
  ...
  if (paginationSteps) jQuery("#pagination_type_steps").prop("checked", true);
  ...
  jQuery("#field_settings, #last_page_settings, #pagination_settings").tabs({
    selected: 0,
  });
...
function LoadFieldSettings() {
  ...
  jQuery("#field_required").prop(
    "checked",
    field.isRequired == true ? true : false
  );
  jQuery("#field_margins").prop(
    "checked",
    field.disableMargins == true ? true : false
  );
  jQuery("#field_no_duplicates").prop(
    "checked",
    field.noDuplicates == true ? true : false
  );
  jQuery("#field_default_value").val(
    field.defaultValue == undefined ? "" : field.defaultValue
  );
  jQuery("#field_default_value_textarea").val(
    field.defaultValue == undefined ? "" : field.defaultValue
  );
  jQuery("#field_description").val(
    field.description == undefined ? "" : field.description
  );
  jQuery("#field_description").attr(
    "placeholder",
    field.descriptionPlaceholder == undefined
      ? ""
      : field.descriptionPlaceholder
  );
  jQuery("#field_checkbox_label").val(
    field.checkboxLabel == undefined ? "" : field.checkboxLabel
  );
  jQuery("#field_css_class").val(
    field.cssClass == undefined ? "" : field.cssClass
  );
  jQuery("#field_range_min").val(
    field.rangeMin == undefined || field.rangeMin === false
      ? ""
      : field.rangeMin
  );
  jQuery("#field_range_max").val(
    field.rangeMax == undefined || field.rangeMax === false
      ? ""
      : field.rangeMax
  );
  jQuery("#field_name_format").val(field.nameFormat);
  jQuery("#field_force_ssl").prop("checked", field.forceSSL ? true : false);
  jQuery("#credit_card_style").val(
    field.creditCardStyle ? field.creditCardStyle : "style1"
  );
  ...
  if (
    field.labelPlacement == "left_label" ||
    field.labelPlacement == "right_label" ||
    (field.labelPlacement == "" && form.labelPlacement != "top_label")
  ) {
  ...
  jQuery("#field_placeholder, #field_placeholder_textarea").val(
    field.placeholder
  );

  jQuery("#field_file_extension").val(
    field.allowedExtensions == undefined ? "" : field.allowedExtensions
  );
  jQuery("#field_multiple_files").prop(
    "checked",
    field.multipleFiles ? true : false
  );
  jQuery("#field_max_files").val(field.maxFiles ? field.maxFiles : "");
  jQuery("#field_max_file_size").val(
    field.maxFileSize ? field.maxFileSize + "MB" : ""
  );
  ToggleMultiFile(true);

  jQuery("#field_phone_format").val(field.phoneFormat);
  jQuery("#field_error_message").val(field.errorMessage);
  jQuery("#field_select_all_choices").prop(
    "checked",
    field.enableSelectAll ? true : false
  );
  jQuery("#field_other_choice").prop(
    "checked",
    field.enableOtherChoice ? true : false
  );
  jQuery("#field_add_icon_url").val(field.addIconUrl ? field.addIconUrl : "");
  jQuery("#field_delete_icon_url").val(
    field.deleteIconUrl ? field.deleteIconUrl : ""
  );
  jQuery("#gfield_enable_enhanced_ui").prop(
    "checked",
    field.enableEnhancedUI ? true : false
  );

  jQuery("#gfield_password_strength_enabled").prop(
    "checked",
    field.passwordStrengthEnabled == true ? true : false
  );
  jQuery("#gfield_password_visibility_enabled").prop(
    "checked",
    field.passwordVisibilityEnabled == true ? true : false
  );
  TogglePasswordVisibility(true);
  jQuery("#gfield_min_strength").val(
    field.minPasswordStrength == undefined ? "" : field.minPasswordStrength
  );
  TogglePasswordStrength(true);

  jQuery("#gfield_email_confirm_enabled").prop(
    "checked",
    field.emailConfirmEnabled == true ? true : false
  );
  ...
  if (!field.numberFormat) {
    if (jQuery("#field_number_format #field_number_format_blank").length == 0) {
      jQuery("#field_number_format").prepend(
        "<option id='field_number_format_blank' value=''>" +
          gf_vars["selectFormat"] +
          "</option>"
      );
    }
  } else jQuery("#field_number_format_blank").remove();

  jQuery("#field_number_format").val(
    field.numberFormat ? field.numberFormat : ""
  );
  ...
  jQuery("#field_enable_calculation").prop(
    "checked",
    field.enableCalculation ? true : false
  );
  ToggleCalculationOptions(field.enableCalculation, field);

  jQuery("#field_calculation_formula").val(field.calculationFormula);
  var rounding = gformIsNumber(field.calculationRounding)
  ...
  if (
    ...
    field["inputType"] == "calculation"
  ) {
    var basePrice = field.basePrice == undefined ? "" : field.basePrice;
    jQuery("#field_base_price").val(
      field.basePrice == undefined ? "" : field.basePrice
    );
  ...
  jQuery("#field_disable_quantity").prop(
    "checked",
    field.disableQuantity == true ? true : false
  );
  ...
  jQuery("#field_maxlen").val(
    typeof field.maxLength == "undefined" ? "" : field.maxLength
  );
  jQuery("#field_maxrows").val(
    typeof field.maxRows == "undefined" ? "" : field.maxRows
  );

  var addressType =
    field.addressType == undefined ? "international" : field.addressType;
  ...
  var defaultProvince =
    field.defaultProvince == undefined ? "" : field.defaultProvince; //for backwards compatibility
  var defaultStateProvince =
    addressType == "canadian" && defaultState == ""
      ? defaultProvince
      : defaultState;

  jQuery("#field_address_default_state_" + addressType).val(
    defaultStateProvince
  );
  jQuery("#field_address_default_country_" + addressType).val(
    field.defaultCountry == undefined ? "" : field.defaultCountry
  );

  SetAddressType(true);

  jQuery("#gfield_display_title").prop(
    "checked",
    field.displayTitle == true ? true : false
  );
  jQuery("#gfield_display_caption").prop(
    "checked",
    field.displayCaption == true ? true : false
  );
  jQuery("#gfield_display_description").prop(
    "checked",
    field.displayDescription == true ? true : false
  );
  ...
  else jQuery("#field_custom_field_name_text").val(field.postCustomFieldName);

  if (customFieldExists) jQuery("#field_custom_existing").prop("checked", true);
  else jQuery("#field_custom_new").prop("checked", true);

  ToggleCustomField(true);

  jQuery("#gfield_customfield_content_enabled").prop(
    "checked",
    field.customFieldTemplateEnabled ? true : false
  );
  jQuery("#field_customfield_content_template").val(
    field.customFieldTemplateEnabled ? field.customFieldTemplate : ""
  );
  ToggleCustomFieldTemplate(true);

  if (field.displayAllCategories)
    jQuery("#gfield_category_all").prop("checked", true);
  else jQuery("#gfield_category_select").prop("checked", true);

  ToggleCategory(true);

  jQuery("#gfield_post_category_initial_item_enabled").prop(
    "checked",
    field.categoryInitialItemEnabled ? true : false
  );
  jQuery("#field_post_category_initial_item").val(
    field.categoryInitialItemEnabled ? field.categoryInitialItem : ""
  );
  ...
  if (inputType == "creditcard") {
    ...
    for (i in field.creditCards) {
      if (!field.creditCards.hasOwnProperty(i)) continue;

      jQuery("#field_credit_card_" + field.creditCards[i]).prop(
        "checked",
        true
      );
    }
  ...
  jQuery("#gfield_calendar_icon_url").val(
    field["calendarIconUrl"] == undefined ? "" : field["calendarIconUrl"]
  );
  jQuery("#field_date_format").val(
    field["dateFormat"] == "dmy" ? "dmy" : field["dateFormat"]
  );
  ...
  jQuery("#field_prepopulate").prop(
    "checked",
    field.allowsPrepopulate ? true : false
  );

  jQuery("#field_rich_text_editor").prop(
    "checked",
    field.useRichTextEditor ? true : false
  );
  ...
  jQuery("#field_enable_copy_values_option").prop(
    "checked",
    field.enableCopyValuesOption == true ? true : false
  );
  jQuery("#field_copy_values_option_default").prop(
    "checked",
    field.copyValuesOptionDefault == true ? true : false
  );
  var copyValueOptions = GetCopyValuesFieldsOptions(
    field.copyValuesOptionField,
    field
  );
  ...
  if (field.nextButton) {
    ...
    else jQuery("#next_button_text").prop("checked", true);
  ...
  if (field.previousButton) {
    ...
    else jQuery("#previous_button_text").prop("checked", true);
  ...
  else jQuery("#field_type, #field_multiple_files").prop("disabled", false);
  ...
  var fg =
    field.simpleCaptchaFontColor == undefined
      ? ""
      : field.simpleCaptchaFontColor;
  jQuery("#field_captcha_fg").val(fg);
  SetColorPickerColor("field_captcha_fg", fg);

  var bg =
    field.simpleCaptchaBackgroundColor == undefined
      ? ""
      : field.simpleCaptchaBackgroundColor;
  jQuery("#field_captcha_bg").val(bg);
  SetColorPickerColor("field_captcha_bg", bg);

  jQuery("#field_captcha_type").val(
    field.captchaType == undefined ? "captcha" : field.captchaType
  );
  jQuery("#field_captcha_badge").val(
    field.captchaBadge == undefined ? "bottomright" : field.captchaBadge
  );
  jQuery("#field_captcha_size").val(
    field.simpleCaptchaSize == undefined ? "medium" : field.simpleCaptchaSize
  );

  //controlling settings based on captcha type
  if (field["type"] == "captcha") {
    var recaptcha_settings =
      ".captcha_language_setting, .captcha_theme_setting";
    var simple_captcha_settings =
      ".captcha_size_setting, .captcha_fg_setting, .captcha_bg_setting";
    ...
    var theme =
      field.captchaTheme == undefined ||
      ["blackglass", "dark"].indexOf(field.captchaTheme) < 0
        ? "light"
        : "dark";
    ...
    var lang =
      field.captchaLanguage == undefined ? "en" : field.captchaLanguage;
    ...
    if (jQuery('#field_captcha_type option[value="captcha"]').length < 1) {
      jQuery("#field_captcha_type").prepend(
  ...
  if (
    ...
    (field["inputType"] == "textarea" || field["inputType"] == "text")
  ) {
  ...
  if (field["type"] == "name") {
    if (
      typeof field["nameFormat"] == "undefined" ||
  ...
  if (field.type === "password" || field.inputType === "password") {
    ...
    jQuery(
      "#field_password_fields_container table tr:eq(1) td:eq(0) img"
    ).remove();

    // Show/Hide Size setting.
    var confirmEnabled =
      field.inputs[1].isHidden == "undefined"
        ? true
        : !field.inputs[1].isHidden;
    if (confirmEnabled) {
      jQuery(".size_setting").hide();
    }

    // Hide Password sub-label.
    jQuery(".password_setting .custom_inputs_setting ").on(
      "click keypress",
      ".input_active_icon",
      function () {
        var field = GetSelectedField(),
  ...
  var $appearanceSettings = jQuery("#gform_tab_3 li.field_setting").filter(
    function () {
      return (
        jQuery(this).is(":hidden") && jQuery(this).css("display") != "none"
      );
    }
  );
  ...
  var $advancedSettings = jQuery("#gform_tab_2 li.field_setting").filter(
    function () {
      return (
        jQuery(this).is(":hidden") && jQuery(this).css("display") != "none"
      );
...
function SetUpAdvancedNameField() {
  ...
  jQuery(".name_setting .custom_inputs_setting").on(
    "click",
    ".input_active_icon",
    function () {
...
function GetCopyValuesFieldsOptions(selectedFieldId, currentField) {
  var options = [],
    ...
    field,
    option,
    currentType = GetInputType(currentField),
    selected;
  ...
  for (var i = 0; i < form.fields.length; i++) {
    ...
    if (
      ...
      GetInputType(field) == currentType &&
...
function SetDisableQuantity(isChecked) {
  ...
  if (isChecked)
    jQuery(
      ".field_selected .ginput_quantity_label, .field_selected .ginput_quantity"
    ).hide();
  else
    jQuery(
      ".field_selected .ginput_quantity_label, .field_selected .ginput_quantity"
...
function SetBasePrice(number) {
  ...
  jQuery(
    ".field_selected .ginput_product_price, .field_selected .ginput_shipping_price"
  ).html(price);
...
function UpdateAddressFields() {
  ...
  jQuery("#field_custom_input_default_label_" + field.id + "_5").text(
    zip_label
  );
  jQuery("#field_custom_input_label_" + field.id + "\\.5").attr(
    "placeholder",
    zip_label
  );
  if (!zipInput.customLabel) {
    jQuery(".field_selected #input_" + field["id"] + "_5_label").html(
      zip_label
    );
  }
  ...
  jQuery("#field_custom_input_default_label_" + field.id + "_4").text(
    state_label
  );
  jQuery("#field_custom_input_label_" + field.id + "\\.4").attr(
    "placeholder",
    state_label
  );
  if (!stateInput.customLabel) {
    jQuery(".field_selected #input_" + field["id"] + "_4_label").html(
      state_label
    );
  }
  ...
  var hide_country =
    jQuery("#field_address_country_" + addressType).val() != "";
  ...
  if (hide_country) {
    ...
    jQuery(".field_selected #input_" + field.id + "_6").val(
      jQuery("#field_address_default_country_" + addressType).val()
    );
    jQuery(".field_selected #input_" + field.id + "_6_container").show();
    jQuery(
      ".field_selected .field_custom_input_row_input_" + field.id + "_6"
    ).show();
  }

  var has_state_drop_down =
    jQuery("#field_address_has_states_" + addressType).val() != "";
  if (has_state_drop_down) {
    jQuery(".field_selected .state_text").hide();
    var selected_state = jQuery(
      "#field_address_default_state_" + addressType
    ).val();
    var state_dropdown = jQuery(".field_selected .state_dropdown");
    state_dropdown.append(
      jQuery("<option></option>").val(selected_state).html(selected_state)
    );
...
function MaybeUpgradeNameField(field) {
  if (
    ...
    (field.nameFormat == "simple" && !has_entry(field.id))
  ) {
...
function UpgradeNameField(field, prefixHiddex, middleHidden, suffixHidden) {
  field.nameFormat = "advanced";
  field.inputs = MergeInputArrays(
    GetAdvancedNameFieldInputs(field, prefixHiddex, middleHidden, suffixHidden),
    field.inputs
  );
...
function UpgradeDateField(field) {
  ...
  if (
    typeof field.dateType != "undefined" &&
    field.dateType != "datepicker" &&
    !field.inputs
  ) {
...
function TogglePasswordVisibility(isInit) {
  if (jQuery("#gfield_password_visibility_enabled").is(":checked")) {
    jQuery(
      ".gfield.field_selected .ginput_container_password span button"
    ).show();
  } else {
    jQuery(
      ".gfield.field_selected .ginput_container_password span button"
...
function SetCustomFieldTemplate() {
  ...
  SetFieldProperty(
    "customFieldTemplate",
    enabled ? jQuery("#field_customfield_content_template").val() : null
  );
  SetFieldProperty("customFieldTemplateEnabled", enabled);
}

function SetCategoryInitialItem() {
  var enabled = jQuery("#gfield_post_category_initial_item_enabled").is(
    ":checked"
  );
  SetFieldProperty(
    "categoryInitialItem",
    enabled ? jQuery("#field_post_category_initial_item").val() : null
  );
...
function TogglePostTitleTemplate(isInit) {
  ...
  if (jQuery("#gfield_post_title_enabled").is(":checked")) {
    ...
    if (!isInit) PopulateContentTemplate("field_post_title_template");
  } else {
...
function ToggleChoiceValue(isInit) {
  ...
  container.removeClass(
    "choice_with_price choice_with_value choice_with_value_and_price"
  );
...
function SetPageButton(button_name) {
  field = GetSelectedField();
  var buttonType = jQuery("#" + button_name + "_button_image").is(":checked")
    ? "image"
    : "text";
  field[button_name + "Button"]["type"] = buttonType;
  if (buttonType == "image") {
    field[button_name + "Button"]["text"] = "";
    field[button_name + "Button"]["imageUrl"] = jQuery(
      "#" + button_name + "_button_image_url"
    ).val();
  } else {
    field[button_name + "Button"]["text"] = jQuery(
      "#" + button_name + "_button_text_input"
    ).val();
...
function ToggleCustomField(isInit) {
  ...
  show_element = isExisting
    ? "#field_custom_field_name_select"
    : "#field_custom_field_name_text";
  hide_element = isExisting
    ? "#field_custom_field_name_text"
    : "#field_custom_field_name_select";
...
function ToggleInputMaskOptions(isInit) {
  var isStandard = jQuery("#field_mask_standard").is(":checked"),
    show_element = isStandard
      ? "#field_mask_select"
      : "#field_mask_text, .mask_text_description",
    hide_element = isStandard
      ? "#field_mask_text, .mask_text_description"
      : "#field_mask_select",
    speed = isInit ? "" : "";
...
function ToggleAutoresponder() {
  ...
  else jQuery("#form_autoresponder_container").hide("slow");
}
...
function HasPostContentField() {
  for (var i = 0; i < form.fields.length; i++) {
    ...
    if (type == "post_content") return true;
  }
  return false;
}
...
function HasPostTitleField() {
  for (var i = 0; i < form.fields.length; i++) {
    var type = form.fields[i].type;
    if (type == "post_title") return true;
  }
  return false;
}
...
function HasCustomField() {
  for (var i = 0; i < form.fields.length; i++) {
    var type = form.fields[i].type;
    if (type == "post_custom_field") return true;
  }
  return false;
}
...
function UpdateFormObject() {
  ...
  if (HasPostField()) {
    form.postAuthor = jQuery("#field_post_author").val()
      ? jQuery("#field_post_author").val()
      : "";
    form.useCurrentUserAsAuthor = jQuery("#gfield_current_user_as_author").is(
      ":checked"
    );
    form.postCategory = jQuery("#field_post_category").val();
    form.postFormat =
      jQuery("#field_post_format").length != 0
        ? jQuery("#field_post_format").val()
        : 0;
    form.postStatus = jQuery("#field_post_status").val();
  }

  if (
    jQuery("#gfield_post_content_enabled").is(":checked") &&
    HasPostContentField()
  ) {
  ...
  if (
    jQuery("#gfield_post_title_enabled").is(":checked") &&
    HasPostTitleField()
  ) {
  ...
  if (jQuery("#gform_last_page_settings").is(":visible")) {
    ...
    form.lastPageButton.type = jQuery("#last_page_button_text").is(":checked")
      ? "text"
      : "image";
    if (form.lastPageButton.type == "image") {
      form.lastPageButton.text = "";
      form.lastPageButton.imageUrl = jQuery(
        "#last_page_button_image_url"
      ).val();
    } else {
  ...
  if (jQuery("#gform_pagination").is(":visible")) {
    ...
    if (type == "percentage") {
      ...
      form["pagination"]["backgroundColor"] =
        form["pagination"]["style"] == "custom"
          ? jQuery("#percentage_style_custom_bgcolor").val()
          : null;
      form["pagination"]["color"] =
        form["pagination"]["style"] == "custom"
          ? jQuery("#percentage_style_custom_color").val()
          : null;
      form["pagination"]["display_progressbar_on_confirmation"] = jQuery(
        "#percentage_confirmation_display"
      ).is(":checked");
      form["pagination"]["progressbar_completion_text"] = jQuery(
        "#percentage_confirmation_display"
      ).is(":checked")
  ...
  if (window["gform_before_update"]) {
    ...
    if (window.console)
      console.log(
        '"gform_before_update" is deprecated since version 1.7! Use the "gform_pre_form_editor_save" filter instead.'
      );
  }
...
function EndUpdateForm(formId) {
  ...
  if (formId) jQuery("#after_update_dialog").slideDown();
  else jQuery("#after_update_error_dialog").slideDown();
...
function DeleteField(element) {
  ...
  if (
    !HasConditionalLogicDependency(fieldId) &&
    !confirm(gf_vars.confirmationDeleteField)
  ) {
    return;
  }
  ...
  if (
    HasConditionalLogicDependency(fieldId) &&
    !confirm(gf_vars.conditionalLogicDependency)
  ) {
    return;
  }
...
function HasConditionalLogicDependencyLegwork(fieldId, value) {
  ...
  if (
    form.button &&
    ObjectHasConditionalLogicDependency(form.button, fieldId, value)
  )
    return true;

  // check confirmations conditional logic
  for (i in form.confirmations) {
    if (!form.confirmations.hasOwnProperty(i)) continue;

    if (
      ObjectHasConditionalLogicDependency(form.confirmations[i], fieldId, value)
    )
      return true;
  }
  ...
  for (i in form.notifications) {
    if (!form.notifications.hasOwnProperty(i)) continue;

    if (
      ObjectHasConditionalLogicDependency(form.notifications[i], fieldId, value)
    )
      return true;
  }
  ...
  for (i in form.fields) {
    if (!form.fields.hasOwnProperty(i)) continue;

    var field = form.fields[i];

    if (ObjectHasConditionalLogicDependency(field, fieldId, value)) return true;

    // if this is a page field, check the next button conditional logic as well
    if (
      GetInputType(field) == "page" &&
      ObjectHasConditionalLogicDependency(field.nextButton, fieldId, value)
    )
      return true;
  }
...
function HasConditionalLogicDependency(fieldId, value) {
  ...
  return gform.applyFilters(
    "gform_has_conditional_logic_dependency",
    result,
    fieldId,
    value
  );
...
function ObjectHasConditionalLogicDependency(object, fieldId, value) {
  if (!object.conditionalLogic) return false;
  ...
  for (i in rules) {
    ...
    if (value !== false && rule.value != value) continue;
...
function StartDuplicateField(element) {
  ...
  for (fieldIndex in form.fields) {
    if (!form.fields.hasOwnProperty(fieldIndex)) continue;
    ...
    if (form.fields[fieldIndex].id == sourcefieldId) {
      ...
      if (field.inputs != null) {
        for (inputIndex in field.inputs) {
          if (!field.inputs.hasOwnProperty(inputIndex)) continue;

          var id = field.inputs[inputIndex]["id"] + "",
            newId =
              id == sourcefieldId
                ? field.id
                : id.replace(/(\d+\.)/, field.id + ".");
      ...
      field = gform.applyFilters(
        "gform_duplicate_field_{0}".format(GetInputType(field)),
        field,
        form
      );
...
function StartChangeDonationType(type) {
  field = GetSelectedField();
  if (type != "donation") field["enablePrice"] = true;
  else field["enablePrice"] = null;
...
function StartChangePostCustomFieldType(type) {
  if (
    jQuery.inArray(type, ["radio", "select", "checkbox", "multiselect"]) === -1
  ) {
    field.choices = null;
  }
  return StartChangeInputType(type, field);
...
function EndChangeInputType(params) {
  var fieldId = params.id,
    ...
    fieldString = params.fieldString;
...
function InitializeFields() {
  ...
  jQuery(
    "#field_settings, #form_settings, #last_page_settings, #pagination_settings, .captcha_message, .form_delete_icon, .all-merge-tags"
...
function CustomFieldExists(name) {
  ...
  for (var i = 0; i < options.length; i++) {
    if (options[i].value == name) return true;
  }
  return false;
}
...
function LoadFieldChoices(field) {
  //loading ui
  jQuery("#field_choice_values_enabled").prop(
    "checked",
    field.enableChoiceValue ? true : false
  );
  ToggleChoiceValue();
  var container_name =
    GetInputType(field) == "list" ? "field_columns" : "field_choices";
...
function LoadInputChoices($ul, input) {
  ...
  $container
    .find(".field_input_choice_values_enabled")
    .prop("checked", input.enableChoiceValue ? true : false);
...
function LoadBulkChoices(field) {
  ...
  for (var i = 0; i < field.choices.length; i++) {
    choice =
      field.choices[i].text == field.choices[i].value
...
function LoadCustomChoices() {
  ...
  if (!IsEmpty(gform_custom_choices)) {
    var str =
      "<li class='choice_section_header'>" + gf_vars.customChoices + "</li>";
    for (key in gform_custom_choices) {
      if (!gform_custom_choices.hasOwnProperty(key)) continue;

      var selectChoiceAction =
        'SelectCustomChoice( jQuery(this).data("key") );';

      str +=
        "<li class='bulk_custom_choice'><a href='javascript:void(0);' data-key='" +
        escapeAttr(key) +
        "' onclick='" +
        selectChoiceAction +
        "' onkeypress='" +
        selectChoiceAction +
        "' class='bulk-choice bulk_custom_choice'>" +
        escapeHtml(key) +
        "</a></li>";
    }
    str +=
      "<li class='choice_section_header'>" +
...
function SelectPredefinedChoice(name) {
  jQuery("#gfield_bulk_add_input").val(
    gform_predefined_choices[name].join("\n")
  );
...
function InsertBulkChoices(choices) {
  ...
  for (var i = 0; i < choices.length; i++) {
    ...
    field.choices.push(
      new Choice(
        jQuery.trim(text_value[0]),
        jQuery.trim(text_value[text_value.length - 1]),
        jQuery.trim(price)
      )
    );

    if (text_value.length > 1) enableValue = true;
  }
...
function IsEmpty(array) {
  var key;
  for (key in array) {
    if (array.hasOwnProperty(key)) return false;
  }
  return true;
}
...
function SetFieldChoice(inputType, index) {
  ...
  jQuery("#field_choices :radio, #field_choices :checkbox").each(function (
    index
  ) {
...
function UpdateFieldChoices(fieldType) {
  ...
  if (fieldType == "checkbox") field.inputs = new Array();
  ...
  switch (GetInputType(field)) {
    case "select":
      for (var i = 0; i < field.choices.length; i++) {
        ...
        var choiceValue = field.choices[i].value
          ? field.choices[i].value
          : field.choices[i].text;
        choices +=
          "<option value='" +
          choiceValue.replace(/'/g, "&#039;") +
          "' " +
          selected +
          ">" +
          field.choices[i].text +
    ...
    case "radio":
      ...
      choices += field.enableOtherChoice
        ? "<li><input type='" +
          fieldType +
          "' " +
          checked +
    ...
    case "list":
      ...
      choices +=
        "<td><img src='" +
        gf_vars["baseUrl"] +
        "/images/add.png' class='add_list_item' style='cursor:pointer; margin:0 3px;' /></td>";
      choices += "</tr></tbody>";
...
function UpdateInputChoices(input) {
  ...
  for (var i = 0; i < input.choices.length; i++) {
    ...
    var choiceValue = input.choices[i].value
      ? input.choices[i].value
      : input.choices[i].text;
    choices +=
      "<option value='" +
      choiceValue.replace(/'/g, "&#039;") +
      "' " +
...
function DeleteFieldChoice(index) {
  field = GetSelectedField();
  var value = jQuery(
    "#" + GetInputType(field) + "_choice_value_" + index
  ).val();

  if (HasConditionalLogicDependency(field.id, value)) {
    if (!confirm(gf_vars.conditionalLogicDependencyChoice)) return;
  }
...
function LoadDateInputs() {
  ...
  if (type == "datefield") {
    switch (position) {
      case "ymd":
        jQuery(".field_selected #gfield_input_date_month")
          .remove()
          .insertBefore(".field_selected #gfield_input_date_day");
        jQuery(".field_selected #gfield_input_date_year")
          .remove()
          .insertBefore(".field_selected #gfield_input_date_month");
        break;

      case "mdy":
        jQuery(".field_selected #gfield_input_date_day")
          .remove()
          .insertBefore(".field_selected #gfield_input_date_year");
        jQuery(".field_selected #gfield_input_date_month")
          .remove()
          .insertBefore(".field_selected #gfield_input_date_day");
        break;
    ...
    switch (position) {
      case "ymd":
        jQuery(".field_selected #gfield_dropdown_date_month")
          .remove()
          .insertBefore(".field_selected #gfield_dropdown_date_day");
        jQuery(".field_selected #gfield_dropdown_date_year")
          .remove()
          .insertBefore(".field_selected #gfield_dropdown_date_month");
        break;

      case "mdy":
        jQuery(".field_selected #gfield_dropdown_date_day")
          .remove()
          .insertBefore(".field_selected #gfield_dropdown_date_year");
        jQuery(".field_selected #gfield_dropdown_date_month")
          .remove()
          .insertBefore(".field_selected #gfield_dropdown_date_day");
        break;
    ...
    else jQuery(".field_selected #gfield_input_datepicker_icon").hide();
  }
}

function SetCalendarIconType(iconType, isInit) {
  field = GetSelectedField();
  if (GetInputType(field) != "date") return;

  if (iconType == undefined) iconType = "none";

  if (iconType == "none") jQuery("#gsetting_icon_none").prop("checked", true);
...
function SetPostImageMeta() {
  var displayTitle = jQuery(".field_selected #gfield_display_title").is(
    ":checked"
  );
  var displayCaption = jQuery(".field_selected #gfield_display_caption").is(
    ":checked"
  );
  var displayDescription = jQuery(
    ".field_selected #gfield_display_description"
  ).is(":checked");
  ...
  //updating UI
  jQuery(".field_selected .ginput_post_image_title").css(
    "display",
    displayTitle ? "block" : "none"
  );
  jQuery(".field_selected .ginput_post_image_caption").css(
    "display",
    displayCaption ? "block" : "none"
  );
  jQuery(".field_selected .ginput_post_image_description").css(
    "display",
    displayDescription ? "block" : "none"
  );
  jQuery(".field_selected .ginput_post_image_file").css(
    "display",
    displayLabel ? "block" : "none"
  );
}
...
function ResetInputPlaceholders(field) {
  ...
  jQuery(field.inputs).each(function () {
    var placeholder =
      typeof this.placeholder != "undefined" ? this.placeholder : "";
  ...
  for (var i = 0; i < field["inputs"].length; i++) {
    ...
    if (input.id == inputId) {
      if (value == "") {
        ...
        label =
          typeof input.defaultLabel != "undefined"
            ? input.defaultLabel
            : input.label;
      } else {
...
function SetSelectedCategories() {
  ...
  jQuery(".gfield_category_checkbox").each(function () {
    if (this.checked) field["choices"].push(new Choice(this.name, this.value));
  });
...
function SetFieldLabel(label) {
  ...
  jQuery(".field_selected .gfield_label, .field_selected .gsection_title")
    .text(label)
    .append(requiredElement);
...
function SetCaptchaSize(size) {
  ...
  RedrawCaptcha();
  jQuery(".field_selected .gfield_captcha_input_container")
    .removeClass(type + "_small")
    .removeClass(type + "_medium")
    .removeClass(type + "_large")
    .addClass(type + "_" + size);
}
...
function SetFieldSize(size) {
  jQuery(
    ".field_selected .small, .field_selected .medium, .field_selected .large"
  )
    .removeClass("small")
    .removeClass("medium")
    .removeClass("large")
...
function SetFieldLabelPlacement(labelPlacement) {
  var labelPlacementClass = labelPlacement
    ? labelPlacement
    : form.labelPlacement;
  SetFieldProperty("labelPlacement", labelPlacement);
  jQuery(".field_selected")
    .removeClass("top_label")
    .removeClass("right_label")
    .removeClass("left_label")
    .removeClass("hidden_label")
    .addClass(labelPlacementClass);
...
function SetFieldDescriptionPlacement(descriptionPlacement) {
  var isDescriptionAbove =
    descriptionPlacement == "above" ||
    (descriptionPlacement == "" && form.descriptionPlacement == "above)");
...
function SetFieldVisibility(visibility, handleInputs, isInit) {
  if (
    ...
    HasConditionalLogicDependency(field.id)
  ) {
...
function SetFieldDefaultValue(defaultValue) {
  jQuery(
    ".field_selected > div > input:visible, .field_selected > div > textarea:visible, .field_selected > div > select:visible"
  ).val(defaultValue);
...
function ToggleEmailSettings(field) {
  var isConfirmEnabled =
    typeof field.emailConfirmEnabled != "undefined" &&
    field.emailConfirmEnabled == true;
...
function SetCardType(elem, value) {
  var cards = GetSelectedField()["creditCards"]
    ? GetSelectedField()["creditCards"]
    : new Array();
...
function SetMaxLength(input) {
  ...
  for (i in characters) {
    if (!characters.hasOwnProperty(i)) continue;

    if (!patt.test(characters[i])) cleanValue += characters[i];
  }
...
function ValidateKeyPress(event, patt, matchPositive) {
  var matchPositive =
    typeof matchPositive == "undefined" ? true : matchPositive;
...
function ToggleCalculationOptions(isEnabled, field) {
  if (isEnabled) {
    ...
    if (field.type != "product") jQuery("li.range_setting").gfSlide("up");
  } else {
    jQuery("#calculation_options").gfSlide("up");
    if (field.type != "product") jQuery("li.range_setting").gfSlide("down");
...
function FormulaContentCallback() {
  SetFieldProperty(
    "calculationFormula",
    jQuery("#field_calculation_formula").val().trim()
  );
}
  ...
  for (i = 0; i < hxs.length; i++) {
    ...
    for (j = 0; j < tblChilds.length; j++) {
      ...
      for (k = 0; k < tblCells.length; k++) {
        jQuery(tblChilds[j].childNodes[k])
          .unbind()
          ...
          .click(function () {
            ...
            if (callback) window[callback](aaa);
            jQuery(this);
          });
      }
    }
...
function SetColorPickerColor(field_name, color, callback) {
  ...
  if (callback) window[callback](color);
}
 ...
 */
gform.addFilter(
  "gform_is_conditional_logic_field",
