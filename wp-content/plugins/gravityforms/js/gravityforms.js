...
function Currency(currency) {
  ...
  this.toMoney = function (number, isNumeric) {
    ...
    money =
      negative +
      this.htmlDecode(symbol_left) +
      money +
      this.htmlDecode(symbol_right);
...
function gformCleanNumber(text, symbol_right, symbol_left, decimal_separator) {
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    if (
      (parseInt(digit, 10) >= 0 && parseInt(digit, 10) <= 9) ||
      digit == decimal_separator
    )
      clean_number += digit;
...
function gformDeleteUploadedFile(formId, fieldId, deleteButton) {
  ...
  if (filesJson) {
    ...
    if (files) {
      ...
      var $multfile = parent.find(
        "#gform_multifile_upload_" + formId + "_" + fieldId
      );
...
function gformIsHidden(element) {
  return (
    element.parents(".gfield").not(".gfield_hidden_product").css("display") ==
    "none"
  );
}

function gformCalculateTotalPrice(formId) {
  if (!_gformPriceFields[formId]) return;
...
function gformGetShippingPrice(formId) {
  ...
  if (shippingField.length == 1 && !gformIsHidden(shippingField)) {
    if (
      shippingField.attr("type") &&
      shippingField.attr("type").toLowerCase() == "hidden"
    )
...
function gformGetProductQuantity(formId, productFieldId) {
  ...
  if (quantityInput.length > 0) {
    ...
    quantityInput = jQuery(
      ".gfield_quantity_" + formId + "_" + productFieldId + " :input"
    );
    quantity = 1;
...
function gformGetOptionLabel(
  ...
  var price_label =
    element[0].tagName.toLowerCase() == "option"
      ? " " + diff
      : "<span class='ginput_price'>" + diff + "</span>";
  var label = original_label + price_label;

  //calling hook to allow for custom option formatting
  if (window["gform_format_option_label"])
    label = gform_format_option_label(
      label,
      original_label,
      price_label,
      current_price,
...
function gformGetProductIds(parent_class, element) {
  ...
  for (var i = 0; i < classes.length; i++) {
    if (
      classes[i].substr(0, parent_class.length) == parent_class &&
      classes[i] != parent_class
    )
      return {
...
function gformRoundPrice(price) {
  var currency = new Currency(gf_global.gf_currency_config);
  var roundedPrice = currency.numberFormat(
    price,
...
function gformShowPasswordStrength(fieldId) {
  var password = document.getElementById(fieldId).value,
    confirm = document.getElementById(fieldId + "_2")
      ? document.getElementById(fieldId + "_2").value
      : "";
...
function gformToggleCheckboxes(toggleCheckbox) {
  ...
  $checkboxes.each(function () {
    ...
    jQuery('input[type="checkbox"]', this)
      .prop("checked", toggleCheckbox.checked)
      .trigger("change");
...
function gformAdjustClasses($container) {
  ...
  $groups.each(function (i) {
    var $group = jQuery(this),
      oddEvenClass =
        (i + 1) % 2 == 0 ? "gfield_list_row_even" : "gfield_list_row_odd";
...
function gformToggleIcons($container, max) {
  ...
  $container
    ...
    .css("visibility", groupCount == 1 ? "hidden" : "visible");
        ...
        $c.children(".gfield_repeater_items")
          ...
          .each(function (i) {
            ...
            $children.each(function () {
              var $subSubContainer = jQuery(this)
        ...
        jQuery(this)
          ...
          .each(function () {
            ...
            var newName = name.replace(parts[0], newNameIndex),
              newNameIsChecked = jQuery('input[name="' + newName + '"]').is(
                ":checked"
              );
...
function gformToggleRepeaterButtons($container) {
  ...
  if (max > 0 && groupCount >= max) {
    ...
    $addButtons.data(
      "title",
      $buttonsContainer.children(".add_repeater_item").attr("title")
    );
    $addButtons.addClass("gfield_icon_disabled").attr("title", "");
  } else if (max > 0) {
    $addButtons.removeClass("gfield_icon_disabled");
...
function gformFindCardType(value) {
  ...
  for (type in rules) {
    ...
    for (i in rules[type]) {
      ...
      if (
        rules[type][i].indexOf(value.substring(0, rules[type][i].length)) === 0
      ) {
...
function gformInitChosenFields(fieldList, noResultsText) {
  return jQuery(fieldList).each(function () {
    ...
    if (
      element.is(":visible") &&
      element.siblings(".chosen-container").length == 0
    ) {
      var options = gform.applyFilters(
...
var GFMergeTag = function () {
  ...
  GFMergeTag.getMergeTagValue = function (formId, inputId, modifier) {
    ...
    var isVisible = window["gf_check_field_rule"]
        ? gf_check_field_rule(formId, fieldId, true, "") == "show"
        : true,
      val;
    ...
    var value = gform.applyFilters(
      "gform_value_merge_tag_" + formId + "_" + fieldId,
      false,
      input,
      modifier
    );
  ...
  GFMergeTag.replaceMergeTags = function (formId, text) {
    ...
    for (i in mergeTags) {
      ...
      var fieldId = parseInt(inputId, 10);
      var modifier =
        mergeTags[i][3] == undefined ? "" : mergeTags[i][3].replace(":", "");
  ...
  GFMergeTag.formatValue = function (value, modifier) {
    ...
    if (value.length > 1) {
      val =
        modifier === "price" || modifier === "currency"
          ? gformToNumber(value[1])
          : value[0];
    } else {
      val = value[0];
    }
...
var GFCalc = function (formId, formulaFields) {
  ...
  this.runCalc = function (formulaField, formId) {
    ...
    var numberFormat = gf_get_field_number_format(
  ...
  this.bindCalcEvent = function (inputId, formulaField, formId, delay) {
    ...
    calcObj.isCalculating[formulaFieldId][inputId] = window.setTimeout(
      function () {
  ...
  this.replaceFieldTags = function (formId, expr, formulaField) {
    ...
    for (i in matches) {
      ...
      if (matches[i][3]) {
        ...
        var is_product_radio = jQuery(
          ".gfield_price input[name=input_" + fieldId + "]"
        ).is("input[type=radio]");
        var is_product_dropdown =
          jQuery(".gfield_price select[name=input_" + fieldId + "]").length > 0;
        var is_option_checkbox = jQuery(
      ...
      var value = isVisible
        ? GFMergeTag.getMergeTagValue(formId, inputId, modifier)
        : 0;
...
function gf_get_field_number_format(fieldId, formId, context) {
  var fieldNumberFormats = rgars(
      window,
      "gf_global/number_formats/{0}/{1}".format(formId, fieldId)
    ),
    format = false;
  ...
  if (typeof context == "undefined") {
    format =
      fieldNumberFormats.price !== false
...
function gformValidateFileSize(field, max_file_size) {
  ...
  if (jQuery(field).closest("div").siblings(".validation_message").length > 0) {
    validation_element = jQuery(field)
      .closest("div")
  ...
  if (file && file.size > max_file_size) {
    ...
    validation_element.text(
      file.name + " - " + gform_gravityforms.strings.file_exceeds_limit
    );
  } else {
...
(function (gfMultiFileUploader, $) {
  ...
  var strings =
    typeof gform_gravityforms != "undefined" ? gform_gravityforms.strings : {};
  var imagesUrl =
    typeof gform_gravityforms != "undefined"
      ? gform_gravityforms.vars.images_url
      : "";

  $(document).bind("gform_post_render", function (e, formID) {
    $("form#gform_" + formID + " .gform_fileupload_multifile").each(
      function () {
        setup(this);
      }
    );
  ...
  $(document).bind(
    ...
    function (e, formID, fields, isInit) {
      if (!isInit) {
        $.each(gfMultiFileUploader.uploaders, function (i, uploader) {
          uploader.refresh();
        });
  ...
  $(document).ready(function () {
    if (
      ...
      typeof adminpage !== "undefined" &&
      adminpage.indexOf("_page_gf_entries") > -1
    ) {
  ...
  function setup(uploadElement) {
    ...
    uploader.bind("Init", function (up, params) {
      if (!up.features.dragdrop) $(".gform_drop_instructions").hide();
    ...
    gfMultiFileUploader.toggleDisabled = function (settings, disabled) {
      var button =
        typeof settings.browse_button == "string"
      ...
      $.each(files, function (i, file) {
        ...
        if ($.inArray(extension, disallowed) > -1) {
          addMessage(
            ...
            file.name + " - " + strings.illegal_extension
          );
          up.removeFile(file);
          return;
        }
    ...
    uploader.bind("Error", function (up, err) {
      if (err.code === plupload.FILE_EXTENSION_ERROR) {
        ...
        addMessage(
          ...
          err.file.name +
            " - " +
            strings.invalid_file_extension +
            " " +
            extensions
        );
        ...
        addMessage(
          up.settings.gf_vars.message_id,
          err.file.name + " - " + strings.file_exceeds_limit
        );
      } else {
        var m =
          "Error: " +
    ...
    uploader.bind("FileUploaded", function (up, file, result) {
      ...
      if (file.percent == 100) {
        if (response.status && response.status == "ok") {
          ...
          addMessage(
            up.settings.gf_vars.message_id,
            strings.unknown_error + ": " + file.name
          );
        }
      }
    });
    ...
    function getAllFiles() {
      ...
      files =
        typeof files === "undefined" || files === "" ? {} : $.parseJSON(files);
...
function gf_raw_input_change(event, elem) {
  ...
  var isChangeElem =
      $input.is(":checkbox") || $input.is(":radio") || $input.is("select"),
...
jQuery(document).on(
  ...
  function (event) {
    var formWrapper = jQuery(this).closest(".gform_wrapper"),
      ...
      sourcePage = parseInt(
        formWrapper.find('input[name^="gform_source_page_number_"]').val(),
        10
      ),
      targetPage = parseInt(
        formWrapper.find('input[name^="gform_target_page_number_"]').val(),
        10
      ),
    ...
    if (hasPages) {
      ...
      submitButton = visiblePage.find(
        '.gform_page_footer [id^="gform_' + buttonType + '_button_"]'
      );
    } else {
    ...
    if (abortSubmission) {
      ...
      if ($reCaptcha.length !== 0 && $reCaptcha.data("size") === "invisible") {
        ...
        var $reCaptchaResponse = formWrapper.find(
