...
gform.addAction(
  ...
  function (elem, formId, fieldId) {
    ...
    var dependentFieldIds = rgars(
      gf_form_conditional_logic,
      [formId, "fields", gformExtractFieldId(fieldId)].join("/")
    );
...
function gf_apply_rules(formId, fields, isInit) {
  var rule_applied = 0;
  jQuery(document).trigger("gform_pre_conditional_logic", [
    formId,
    fields,
    isInit,
  ]);
...
function gf_get_field_logic(formId, fieldId) {
  var formConditionalLogic = rgars(
    window,
    "gf_form_conditional_logic/" + formId
  );
...
function gf_apply_field_rule(formId, fieldId, isInit, callback) {
  ...
  var conditionalLogic =
    window["gf_form_conditional_logic"][formId]["logic"][fieldId];
...
function gf_get_field_action(formId, conditionalLogic) {
  if (!conditionalLogic) return "show";
  ...
  for (var i = 0; i < conditionalLogic["rules"].length; i++) {
     ...
     * @since 2.4.22
     */
    var rule = gform.applyFilters(
      "gform_rule_pre_evaluation",
      jQuery.extend({}, conditionalLogic["rules"][i]),
      formId,
      conditionalLogic
    );
    if (gf_is_match(formId, rule)) matches++;
  }

  var action;
  if (
    (conditionalLogic["logicType"] == "all" &&
      matches == conditionalLogic["rules"].length) ||
    (conditionalLogic["logicType"] == "any" && matches > 0)
  )
    action = conditionalLogic["actionType"];
  else action = conditionalLogic["actionType"] == "show" ? "hide" : "show";
...
function gf_is_match(formId, rule) {
  ...
  var isCheckable =
    ...
    isMatch = isCheckable
      ? gf_is_match_checkable($inputs, rule, formId, fieldId)
      : gf_is_match_default($inputs.eq(0), rule, formId, fieldId);
...
function gf_is_match_default($input, rule, formId, fieldId) {
  ...
  var isMatch =
    rule.operator == "isnot" ? matchCount == valuesLength : matchCount > 0;
...
function gf_do_field_action(formId, action, fieldId, isInit, callback) {
  ...
  for (var i = 0; i < dependent_fields.length; i++) {
    var targetId =
      fieldId == 0
        ? "#gform_submit_button_" + formId
        : "#field_" + formId + "_" + dependent_fields[i];
...
function gf_reset_to_default(targetId, defaultValue) {
  var dateFields = jQuery(targetId).find(
  ...
  if (dateFields.length > 0) {
    dateFields.each(function () {
      ...
      if (defaultValue) {
        var key = "d";
        if (
          element.parents().hasClass("gfield_date_month") ||
          element.parents().hasClass("gfield_date_dropdown_month")
        ) {
          key = "m";
        } else if (
          element.parents().hasClass("gfield_date_year") ||
          element.parents().hasClass("gfield_date_dropdown_year")
        ) {
          key = "y";
        }
      ...
      if (element.val() != val) element.val(val).trigger("change");
  ...
  var target = jQuery(targetId).find(
  ...
  if (
    defaultValue &&
    target.parents(".ginput_list").length > 0 &&
    target.length < defaultValue.length
  ) {
    while (target.length < defaultValue.length) {
      gformAddListItem(target.eq(0), 0);
      target = jQuery(targetId).find(
  ...
  target.each(function () {
    ...
    if (element.val() != val) {
      ...
      if (gf_is_hidden_pricing_input(element)) {
        ...
        jQuery("#input_" + ids[0] + "_" + ids[1]).text(
          gformFormatMoney(element.val())
        );
      }
    } else {
      element.val(val);
  ...
  var elements = jQuery(targetId).find(
  ...
  elements.each(function () {
    ...
    var doCheck = defaultValue
      ? jQuery.inArray(jQuery(this).attr("id"), defaultValue) > -1
      : false;
...
function gf_is_hidden_pricing_input(element) {
  ...
  if (
    element.attr("id") &&
    element.attr("id").indexOf("ginput_base_price") === 0
  ) {
    return true;
  }
