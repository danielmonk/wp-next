...
function ToggleConditionalLogic(isInit, objectType) {
  ...
  if (jQuery("#" + objectType + "_conditional_logic").is(":checked")) {
    ...
    SetConditionalProperty(
      objectType,
      "actionType",
      jQuery("#" + objectType + "_action_type").val()
    );
    SetConditionalProperty(
      objectType,
      "logicType",
      jQuery("#" + objectType + "_logic_type").val()
    );
...
function CreateConditionalLogic(objectType, obj) {
  if (!obj.conditionalLogic) obj.conditionalLogic = new ConditionalLogic();

  var hideSelected =
    obj.conditionalLogic.actionType == "hide" ? "selected='selected'" : "";
  var showSelected =
    obj.conditionalLogic.actionType == "show" ? "selected='selected'" : "";
  var allSelected =
    obj.conditionalLogic.logicType == "all" ? "selected='selected'" : "";
  var anySelected =
    obj.conditionalLogic.logicType == "any" ? "selected='selected'" : "";

  var objText;
  if (obj["type"] == "section") objText = gf_vars.thisSectionIf;
  else if (objectType == "field") objText = gf_vars.thisFieldIf;
  else if (objectType == "page") objText = gf_vars.thisPage;
  else if (objectType == "confirmation") objText = gf_vars.thisConfirmation;
  else if (objectType == "notification") objText = gf_vars.thisNotification;
  else objText = gf_vars.thisFormButton;

  var descPieces = {};
  descPieces.actionType =
    "<select id='" +
    objectType +
    "_action_type' onchange='SetConditionalProperty(\"" +
    objectType +
    "\", \"actionType\", jQuery(this).val());'><option value='show' " +
    showSelected +
    ">" +
    gf_vars.show +
  ...
  descPieces.logicType =
    "<select id='" +
    objectType +
    "_logic_type' onchange='SetConditionalProperty(\"" +
    objectType +
    "\", \"logicType\", jQuery(this).val());'><option value='all' " +
    allSelected +
    ">" +
    gf_vars.all +
  ...
  for (i = 0; i < obj.conditionalLogic.rules.length; i++) {
    ...
    str +=
      "<a class='add_field_choice' title='add another rule' onclick=\"InsertRule('" +
      objectType +
      "', " +
      (i + 1) +
      ');" onkeypress="InsertRule(\'' +
      objectType +
      "', " +
      (i + 1) +
    ...
    if (obj.conditionalLogic.rules.length > 1)
      str +=
        "<a class='delete_field_choice' title='remove this rule' onclick=\"DeleteRule('" +
        objectType +
        "', " +
        i +
        ');" onkeypress="DeleteRule(\'' +
        objectType +
        "', " +
        i +
        ");\" ><i class='gficon-subtract'></i></a></li>";
...
function GetRuleOperators(objectType, i, fieldId, selectedOperator) {
  ...
  str =
    ...
    "_rule_operator_" +
    i +
    "' class='gfield_rule_select' onchange='SetRuleProperty(\"" +
    objectType +
    '", ' +
    i +
    ', "operator", jQuery(this).val());var valueSelector="#' +
    objectType +
    "_rule_value_" +
    i +
    '"; jQuery(valueSelector).replaceWith(GetRuleValues("' +
    objectType +
    '", ' +
    i +
    ',"' +
  ...
  operators = IsEntryMeta(fieldId)
    ? GetOperatorsForMeta(supportedOperators, fieldId)
    : supportedOperators;

  operators = gform.applyFilters(
...
function GetOperatorsForMeta(supportedOperators, key) {
  ...
  if (
    entry_meta[key] &&
    entry_meta[key].filter &&
    entry_meta[key].filter.operators
  ) {
...
function GetRuleFields(objectType, ruleIndex, selectedFieldId) {
  var str =
    ...
    "_rule_field_" +
    ruleIndex +
    "' class='gfield_rule_select' onchange='jQuery(\"#" +
    objectType +
    "_rule_operator_" +
    ruleIndex +
    '").replaceWith(GetRuleOperators("' +
    objectType +
    '", ' +
    ruleIndex +
    ', jQuery(this).val()));jQuery("#' +
    objectType +
    "_rule_value_" +
    ruleIndex +
    '").replaceWith(GetRuleValues("' +
    objectType +
    '", ' +
    ruleIndex +
    ', jQuery(this).val())); SetRule("' +
    objectType +
    '", ' +
    ruleIndex +
    "); '>";
  ...
  for (var i = 0; i < form.fields.length; i++) {
    ...
    if (IsConditionalLogicField(field)) {
      ...
      if (
        field.inputs &&
        jQuery.inArray(GetInputType(field), ["checkbox", "email", "consent"]) ==
          -1
      ) {
...
function GetRuleFieldsOptions(options, selectedFieldId) {
  ...
  for (var i = 0; i < options.length; i++) {
    ...
    if (typeof option.options !== "undefined") {
      ...
      var selected =
        option.value == selectedFieldId ? "selected='selected'" : "";
...
function GetEntryMetaFields(selectedFieldId) {
  ...
  if (typeof entry_meta == "undefined") return options;

  jQuery.each(entry_meta, function (key, meta) {
    if (typeof meta.filter == "undefined") return;
...
function IsConditionalLogicField(field) {
  ...
  isConditionalLogicField = gform.applyFilters(
...
function IsEntryMeta(key) {
  return (
    typeof entry_meta != "undefined" && typeof entry_meta[key] != "undefined"
  );
}
...
function GetRuleValues(
  ...
  if (!inputName) inputName = false;

  var dropdownId =
    inputName == false ? objectType + "_rule_value_" + ruleIndex : inputName;

  if (selectedFieldId == 0) selectedFieldId = GetFirstRuleField();
  ...
  if (
    ...
    if (dropdown.length > 0) {
      ...
      str =
        "<select id='" +
        dropdownId +
        "' class='gfield_rule_select gfield_rule_value_dropdown gfield_category_dropdown'>" +
        options +
        "</select>";
    } else {
      var placeholderName =
        inputName == false
          ? "gfield_ajax_placeholder_" + ruleIndex
          : inputName + "_placeholder";
      ...
      str =
        "<select id='" +
        placeholderName +
        "' class='gfield_rule_select'><option>" +
        gf_vars["loading"] +
        "</option></select>";
    }
  } else if (
    field &&
    field.choices &&
    jQuery.inArray(operator, ["is", "isnot"]) > -1
  ) {
    var emptyChoice, ruleChoices;
    ...
    isEntryMeta &&
    entry_meta &&
    entry_meta[selectedFieldId] &&
    entry_meta[selectedFieldId].filter &&
    typeof entry_meta[selectedFieldId].filter.choices != "undefined"
  ) {
    str = GetRuleValuesDropDown(
      entry_meta[selectedFieldId].filter.choices,
      objectType,
      ruleIndex,
      selectedValue,
      inputName
    ...
    str =
      ...
      gf_vars["enterValue"] +
      "' class='gfield_rule_select gfield_rule_input' id='" +
      dropdownId +
      "' name='" +
      dropdownId +
      "' value='" +
      selectedValue.replace(/'/g, "&#039;") +
      "' onchange='SetRuleProperty(\"" +
      objectType +
      '", ' +
      ruleIndex +
      ', "value", jQuery(this).val());\' onkeyup=\'SetRuleProperty("' +
      objectType +
      '", ' +
      ruleIndex +
...
function IsAddressSelect(inputId, field) {
  ...
  var addressType = field.addressType
    ? field.addressType
    : gf_vars.defaultAddressType;
  ...
  return (
    (isCountryInput && addressType == "international") ||
    (isStateInput && typeof addressTypeObj.states == "object")
  );
}

function GetFirstRuleField() {
  for (var i = 0; i < form.fields.length; i++) {
    if (IsConditionalLogicField(form.fields[i])) return form.fields[i].id;
  }
...
function GetRuleValuesDropDown(
  ...
  var str =
    "<select class='gfield_rule_select gfield_rule_value_dropdown' id='" +
    dropdown_id +
    "' name='" +
    dropdown_id +
    "'>";
  ...
  for (var i = 0; i < choices.length; i++) {
    var choiceValue =
      typeof choices[i].value == "undefined" || choices[i].value == null
        ? choices[i].text + ""
        : choices[i].value + "";
    ...
    var choiceText =
      jQuery.trim(jQuery("<div>" + choices[i].text + "</div>").text()) === ""
        ? choiceValue
        : choices[i].text;
    str +=
      "<option value='" +
...
function SetRule(objectType, ruleIndex) {
  SetRuleProperty(
    ...
    ruleIndex,
    "fieldId",
    jQuery("#" + objectType + "_rule_field_" + ruleIndex).val()
  );
  SetRuleProperty(
    objectType,
    ruleIndex,
    "operator",
    jQuery("#" + objectType + "_rule_operator_" + ruleIndex).val()
  );
  SetRuleProperty(
    objectType,
    ruleIndex,
    "value",
    jQuery("#" + objectType + "_rule_value_" + ruleIndex).val()
  );
}
...
function InsertVariable(element_id, callback, variable) {
  if (!variable) variable = jQuery("#" + element_id + "_variable_select").val();
  ...
  if (variableSelect.length > 0) variableSelect[0].selectedIndex = 0;
...
function GetInput(field, id) {
  if (
    typeof field["inputs"] != "undefined" &&
    jQuery.isArray(field["inputs"])
  ) {
    for (i in field["inputs"]) {
      if (!field["inputs"].hasOwnProperty(i)) continue;
...
function GetLabel(field, inputId, inputOnly) {
  if (typeof inputId == "undefined") inputId = 0;

  if (typeof inputOnly == "undefined") inputOnly = false;
...
function SetConfirmationConditionalLogic() {
  confirmation["conditionalLogic"] = jQuery("#conditional_logic").val()
    ? jQuery.parseJSON(jQuery("#conditional_logic").val())
...
function ToggleConfirmation() {
  ...
  if (isRedirect) {
    ...
    hideElement =
      "#form_confirmation_message_container, .form_confirmation_page_container";
    ClearConfirmationSettings(["text", "page"]);
  } else if (isPage) {
    showElement = ".form_confirmation_page_container";
    hideElement =
      "#form_confirmation_message_container, .form_confirmation_redirect_container";
    ClearConfirmationSettings(["text", "redirect"]);
  } else {
    showElement = "#form_confirmation_message_container";
    hideElement =
      ".form_confirmation_page_container, .form_confirmation_redirect_container";
...
(function (gaddon, $, undefined) {
  ...
  function isSet($var) {
    if (typeof $var != "undefined") return true;
    return false;
  }

  function rgar(array, name) {
    if (typeof array[name] != "undefined") return array[name];
    return "";
  }
...
function Copy(variable) {
  ...
  else if (typeof variable != "object") return variable;

  variable = jQuery.isArray(variable)
    ? variable.slice()
    : jQuery.extend({}, variable);
...
var gfMergeTagsObj = function (form, element) {
  ...
  self.bindKeyDown = function () {
    self.elem.on("keydown.gravityforms", function (event) {
      var menuActive =
        self.elem.data("autocomplete") && self.elem.data("autocomplete").menu
          ? self.elem.data("autocomplete").menu.active
          : false;
  ...
  self.initAutocomplete = function () {
    self.elem.autocomplete({
      ...
      source: function (request, response) {
        ...
        var tags = jQuery.map(
          self.getAutoCompleteMergeTags(self.elem),
          function (item) {
  ...
  self.addMergeTagIcon = function () {
    ...
    self.mergeTagIcon = jQuery(
      '<span class="all-merge-tags ' +
        positionClass +
        " " +
        inputType +
        '"><a class="open-list tooltip-merge-tag" title="' +
  ...
  this.getMergeTags = function (
    ...
    if (typeof excludeFieldTypes == "undefined") excludeFieldTypes = [];
    ...
    if (!hideAllFields)
      ungrouped.push({
        tag: "{all_fields}",
        label: this.getMergeTagLabel("{all_fields}"),
      });
    ...
    if (!isPrepop) {
      ...
      for (i in fields) {
        ...
        if (jQuery.inArray(inputType, excludeFieldTypes) != -1) continue;
      ...
      if (requiredFields.length > 0) {
        for (i in requiredFields) {
          ...
          requiredGroup = requiredGroup.concat(
            this.getFieldMergeTags(requiredFields[i], option)
          );
        }
      }
      ...
      if (optionalFields.length > 0) {
        for (i in optionalFields) {
          if (!optionalFields.hasOwnProperty(i)) continue;

          optionalGroup = optionalGroup.concat(
            this.getFieldMergeTags(optionalFields[i], option)
          );
        }
      }
      ...
      if (pricingFields.length > 0) {
        if (!hideAllFields)
          pricingGroup.push({
            tag: "{pricing_fields}",
            label: this.getMergeTagLabel("{pricing_fields}"),
          });

        for (i in pricingFields) {
          if (!pricingFields.hasOwnProperty(i)) continue;
    ...
    if (!HasPostField() || isPrepop) {
      // TODO: consider adding support for passing form object or fields array
      otherTags.splice(otherTags.indexOf("post_id"), 1);
      otherTags.splice(otherTags.indexOf("post_edit_url"), 1);
    }

    for (var i in otherTags) {
      if (jQuery.inArray(otherTags[i], excludeFieldTypes) != -1) continue;

      otherGroup.push({
        tag: "{" + otherTags[i] + "}",
        label: this.getMergeTagLabel("{" + otherTags[i] + "}"),
      });
    }
    ...
    if (customMergeTags.tags.length > 0) {
      for (i in customMergeTags.tags) {
        if (!customMergeTags.tags.hasOwnProperty(i)) continue;
  ...
  this.getMergeTagLabel = function (tag) {
    for (groupName in gf_vars.mergeTags) {
      if (!gf_vars.mergeTags.hasOwnProperty(groupName)) continue;

      var tags = gf_vars.mergeTags[groupName].tags;
      for (i in tags) {
        if (!tags.hasOwnProperty(i)) continue;

        if (tags[i].tag == tag) return tags[i].label;
      }
    }
    ...
    if (
      ...
      for (i in field.inputs) {
        ...
        if (
          ...
          jQuery.inArray(parseFloat(input.id), [
            parseFloat(field.id + ".2"),
            parseFloat(field.id + ".3"),
            parseFloat(field.id + ".5"),
          ]) > -1
        )
          continue;
  ...
  this.getAutoCompleteMergeTags = function (elem) {
    ...
    var mergeTags = this.getMergeTags(
      fields,
      elementId,
      hideAllFields,
      excludeFieldTypes,
    ...
    for (group in mergeTags) {
      if (!mergeTags.hasOwnProperty(group)) continue;
  ...
  this.hasMultipleGroups = function (mergeTags) {
    ...
    for (group in mergeTags) {
      ...
      if (mergeTags[group].tags.length > 0) count++;
    }
    return count > 1;
  };
...
var FeedConditionObj = function (args) {
  ...
  this.init = function () {
    ...
    gform.addFilter(
      ...
      "FeedConditionConditionalObject"
    );
    gform.addFilter(
      "gform_conditional_logic_description",
      "FeedConditionConditionalDescription"
    );
    ...
    jQuery("input#feed_condition_conditional_logic")
      .parents("form")
      .on("submit", function () {
        jQuery("input#feed_condition_conditional_logic_object").val(
          JSON.stringify(fcobj.logicObject)
        );
      });
  };
...
function SimpleConditionObject(object, objectType) {
  if (objectType.indexOf("simple_condition") < 0) return object;
...
function FeedConditionConditionalObject(object, objectType) {
  if (objectType != "feed_condition") return object;

  return feedCondition.logicObject;
}

function FeedConditionConditionalDescription(
  description,
  descPieces,
  objectType,
  obj
) {
  if (objectType != "feed_condition") return description;

  descPieces.actionType = descPieces.actionType.replace(
    "<select",
