(function (gfFieldFilterUI, $) {
  ...
  function getFilterFields() {
    ...
    select.push("</select>");
    select.push(
      "<input type='hidden' class='gform-filter-type' name='t[]' value='' >"
    );
    return select.join("");
  }
  ...
  function getOptions(setting, depth) {
    ...
    if (setting.group) {
      ...
      for (j = 0; j < numRows; j++) {
        ...
        options.push(
          '<option {0} value="{1}">{2}</option>'.format(disabled, val, label)
        );
      }
      ...
      if (setting.isNestable) {
        ...
        select.push(
          '<optgroup label="{0}"></optgroup>{1}'.format(
            groupLabel,
            options.join("")
          )
        );
        ...
        select.push(
          '<optgroup label="{0}">{1}</optgroup>'.format(
            groupLabel,
            options.join("")
          )
        );
      ...
      disabled =
        setting.preventMultiple && isFieldSelected(key)
          ? "disabled='disabled'"
          : "";
      label = setting.text;
      select.push(
        '<option {0} value="{1}">{2}</option>'.format(disabled, key, label)
      );
    }
    return select.join("");
  }
  ...
  function changeOperator(operatorSelect) {
    ...
    if (filter) {
      $select
        .siblings(".gform-filter-value")
        .replaceWith(getFilterValues(filter, operatorSelect.value));
    }
    setDisabledFields();
  ...
  function changeField(fieldSelect) {
    ...
    if (filter) {
      ...
      $select
        .siblings(".gform-filter-value")
        .replaceWith(getFilterValues(filter));
      $select.siblings(".gform-filter-type").val(filter.type);
      $select
        .siblings(".gform-filter-operator")
        .replaceWith(getFilterOperators(filter));
  ...
  function getFilterOperators(filter) {
    ...
    if (filter) {
      for (i = 0; i < filter.operators.length; i++) {
        ...
        str += '<option value="{0}">{1}</option>'.format(
          operator,
          gf_vars[operatorStrings[operator]]
        );
      }
    }
    str += "</select>";
  ...
  function getFilterValues(filter, selectedOperator) {
    ...
    if (filter && filter.values && selectedOperator != "contains") {
      ...
      for (i = 0; i < filter.values.length; i++) {
        ...
        if (
          filter.values[i].operators &&
          $.inArray(selectedOperator, filter.values[i].operators) === -1
        ) {
          continue;
        }
        options += '<option value="{0}">{1}</option>'.format(val, text);
      }
      str = "<select name='v[]' class='{0}'>{1}</select>".format(
        cssClass,
        options
      );
    } else {
      placeholder =
        filter && typeof filter.placeholder != "undefined"
          ? "placeholder='{0}'".format(filter.placeholder)
          : "";

      str = "<input type='text' value='' name='v[]' class='{0}' {1}/>".format(
        cssClass,
        placeholder
      );
    }
  ...
  function getAddRemoveButtons() {
    var str = "";
    if (!allowMultiple) return str;

    str += "<img class='gform-add' src='{0}/add.png' alt='{1}' title='{2}'>".format(
      imagesURL,
      gf_vars.addFieldFilter,
      gf_vars.addFieldFilter
    );
    str +=
      "<img class='gform-remove' src='" +
      imagesURL +
      "/remove.png' alt='" +
      gf_vars.removeFieldFilter +
      "' title='" +
      gf_vars.removeFieldFilter +
      "'>";
    return str;
  ...
  function maybeMakeResizable() {
    ...
    var makeResizable =
      $filterBox.get(0).scrollHeight > $container.height() ||
  ...
  function displayNoFiltersMessage() {
    ...
    str += "<img class='gform-add' src='{0}/add.png' alt='{1}' title='{2}'></div>".format(
      imagesURL,
      gf_vars.addFieldFilter,
      gf_vars.addFieldFilter
    );
  ...
  function setDisabledFields() {
    ...
    $("select.gform-filter-field").each(function (i) {
      ...
      if (
        ...
        filter.preventMultiple &&
        isFieldSelected(this.value)
      ) {
        $(
          "select.gform-filter-field option[value='" +
            this.value +
            "']:not(:selected)"
  ...
  function getFilterMode(mode) {
    var html;
    html = '<select name="mode"><option value="all" {0}>{1}</option><option value="any" {2}>{3}</option></select>'.format(
      selected("all", mode),
      gf_vars.all,
      selected("any", mode),
      gf_vars.any
  ...
  function addNewFieldFilter(el) {
    var $el, $filterRow;
    $el = $(el);
    if ($el.is("img")) $filterRow = $el.parent();
    else $filterRow = $el;

    $filterRow.after(getNewFilterRow());
    $filterRow
      .next("div")
      .find(".gform-filter-field")
      .change()
      .find(".gform-filter-operator")
      .change();
  ...
  function removeFieldFilter(img) {
    $(img).parent().remove();
    if ($(".gform-field-filter").length == 0) displayNoFiltersMessage();
    setDisabledFields();
    maybeMakeResizable();
  }
