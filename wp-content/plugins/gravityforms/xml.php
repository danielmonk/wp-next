...
class RGXML
{
    ...
    public function serialize($parent_node_name, $data, $path = "")
    {
        ...
        if (!is_array($data)) {
            ...
            return strlen($data) == 0 && !rgar($option, "allow_empty")
                ? ""
                : "$padding<$parent_node_name>" .
                        $this->xml_value($parent_node_name, $data) .
                        "</$parent_node_name>";
        }
        ...
        // Opening parent node
        $version =
            $path == $parent_node_name && isset($this->options["version"])
        ...
        $child_node_name = isset($this->options[$path]["array_tag"])
            ? $this->options[$path]["array_tag"]
            : $this->to_singular($parent_node_name);
    ...
    {
        $level = isset($values[$parent_index]["level"])
            ? $values[$parent_index]["level"] + 1
            : false;
        $nodes = [];
        for (
            $i = $parent_index + 1, $count = sizeof($values);
            $i < $count;
            $i++
        ) {
            $current = $values[$i];
            ...
            if (
                ...
                $current["level"] == $level &&
                ($current["type"] == "open" || $current["type"] == "complete")
            ) {
                $nodes[] = ["tag" => $current["tag"], "index" => $i];
    ...
    private function to_singular($str)
    {
        ...
        if ($last3 == "ies" && in_array($fourth, ["a", "e", "i", "o", "u"])) {
