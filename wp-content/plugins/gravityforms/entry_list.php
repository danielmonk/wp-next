...
class GFEntryList
{
    public static function all_entries_page()
    {
        ...
        if (rgget('trashed_entry')) {
            ...
            GFCommon::add_dismissible_message(
                sprintf(
                    esc_html__(
                        '1 entry moved to the Trash. %sUndo%s',
                        'gravityforms'
                    ),
        ...
        if (sizeof($forms) == 0) { ?>
			<div style="margin:50px 0 0 10px;">
				<?php echo sprintf(
        esc_html__(
            "You don't have any active forms. Let's go %screate one%s",
            'gravityforms'
        ),
        '<a href="?page=gf_new_form">',
        '</a>'
    ); ?>
			</div>
			<?php } else {if (empty($form_id)) {
                $form_id = $forms[0]->id;
            }
    ...
    public static function get_screen_options_markup($status, $args)
    {
        ...
        foreach ($filters as $filter) {
            ...
            $radios_arr[] = sprintf(
                '<input type="radio" name="gform_default_filter" value="%s" id="gform_default_filter_%s" %s /><label for="gform_default_filter_%s">%s</label>',
                $id,
                $id,
                $checked,
                $id,
        ...
        $entries_label = esc_html__(
            'Number of entries per page:',
            'gravityforms'
        );

        $button = get_submit_button(
            esc_html__('Apply', 'gravityforms'),
   ...
   ?>
			...
			<?php GFForms::top_toolbar(); ?>
				...
				<div id="entry_search_container">
					...
					<a style="" class="button" id="entry_search_button"
					   href="javascript:Search('<?php echo esc_js(
            $table->get_orderby()
        ); ?>', '<?php echo esc_js($table->get_order()); ?>', <?php echo absint($form_id); ?>, jQuery('.gform-filter-value').val(), '<?php echo esc_js($table->get_filter()); ?>', jQuery('.gform-filter-field').val(), jQuery('.gform-filter-operator').val());"><?php esc_html_e('Search', 'gravityforms'); ?></a>
    ...
    public static function get_filter_links($form, $include_counts = true)
    {
        ...
        $summary = $include_counts
            ? GFFormsModel::get_form_counts($form_id)
            : [];
        ...
        $filter_links = [
            ...
            [
                'id' => 'unread',
                'field_filters' => [['key' => 'is_read', 'value' => false]],
                'count' => $unread_count,
                'label' => esc_html_x('Unread', 'Entry List', 'gravityforms'),
            ],
            [
                'id' => 'star',
                'field_filters' => [['key' => 'is_starred', 'value' => true]],
                'count' => $starred_count,
...
final class GF_Entry_List_Table extends WP_List_Table
{
    ...
    function get_views()
    {
        ...
        foreach ($filter_links as $filter_link_index => $filter_link) {
            ...
            $link =
                '<a class="' .
                $selected .
                '" href="?page=gf_entries&view=entries&id=' .
                $form_id .
                esc_attr($filter_arg) .
                '">' .
                esc_html($filter_link['label']) .
                '<span class="count"> (<span	id="' .
                esc_attr($filter_link['id']) .
                '_count">' .
    ...
    function prepare_items()
    {
        ...
        $page_size = isset($screen_options['per_page'])
            ? absint($screen_options['per_page'])
            : 20;

        $page_size = gf_apply_filters(
            ['gform_entry_page_size', $form_id],
            $page_size,
            $form_id
        );
        ...
        $paging = ['offset' => $first_item_index, 'page_size' => $page_size];
        $total_count = 0;
        ...
        $args = gf_apply_filters(
            ['gform_get_entries_args_entry_list', $form_id],
            compact('form_id', 'search_criteria', 'sorting', 'paging')
        );
    ...
    function get_search_criteria()
    {
        ...
        $status = in_array($this->filter, ['trash', 'spam'])
            ? $this->filter
            : 'active';
        ...
        if (isset($_GET['field_id']) && $_GET['field_id'] !== '') {
            ...
            $filter_operator = empty($search_operator)
                ? 'is'
                : $search_operator;
            $form = $this->get_form();
    ...
    function get_columns()
    {
        ...
        if (!in_array($this->filter, ['trash', 'spam'])) {
        ...
        if (empty($columns)) {
            $table_columns['field_id-id'] = esc_html__(
                'Entry Id',
                'gravityforms'
            );
        }

        $column_selector_url = add_query_arg(
            [
        ...
        $table_columns['column_selector'] =
            '<a aria-label="' .
            esc_attr__('click to select columns to display', 'gravityforms') .
            '" href="' .
            esc_url($column_selector_url) .
            '" class="thickbox entries_edit_icon"><i aria-hidden="true" class="fa fa-cog" title="' .
            esc_attr__('click to select columns to display', 'gravityforms') .
            '"></i></a>';
        ...
        return apply_filters(
            'gform_entry_list_columns_' . $form_id,
            $table_columns,
            $form_id
        );
    }
    ...
    function get_sortable_columns()
    {
        ...
        $table_columns = [];
        foreach ($columns as $key => $column_info) {
            $table_columns['field_id-' . (string) $key] = [
                (string) $key,
                false,
            ];
        }
    ...
    {
        $entry_id = $entry['id']; ?>
		<label class="screen-reader-text" for="cb-select-<?php echo esc_attr(
      $entry_id
  ); ?>"><?php _e('Select entry'); ?></label>
		<input type="checkbox" class="gform_list_checkbox" name="entry[]" value="<?php echo esc_attr(
      $entry_id
  ); ?>" />
		<?php $this->locking_info->lock_indicator();
    }
    ...
    function _column_is_starred($entry, $classes, $data, $primary)
    {
        ...
        if ($this->filter !== 'trash') { ?>
			<img id="star_image_<?php echo esc_attr(
       $entry['id']
   ); ?>" src="<?php echo GFCommon::get_base_url(); ?>/images/star<?php echo intval(
    $entry['is_starred']
); ?>.png" onclick="ToggleStar(this, '<?php echo esc_js(
    $entry['id']
); ?>','<?php echo esc_js($this->filter); ?>');" />
			<?php }
        echo '</th>';
    }
    ...
    function column_default($entry, $column_id)
    {
        ...
        if (!empty($field) && $field->type == 'post_category') {
            $value = GFCommon::prepare_post_category_value(
                $value,
                $field,
                'entry_list'
            );
        ...
        switch ($field_id) {
            case 'source_url':
                $value =
                    "<a href='" .
                    esc_attr($entry['source_url']) .
                    "' target='_blank' alt='" .
                    esc_attr($entry['source_url']) .
                    "'>.../" .
                    esc_attr(GFCommon::truncate_url($entry['source_url'])) .
                    '</a>';
                break;
            ...
            case 'payment_status':
                $value = GFCommon::get_entry_payment_status_text(
            ...
            default:
                if ($field !== null) {
                    $value = $field->get_value_entry_list(
                        $value,
                        $entry,
                        $field_id,
                        $columns,
        ...
        if ($column_id == $primary) {
            ...
            echo '<a aria-label="' .
                esc_attr__('View this entry', 'gravityforms') .
                '" href="' .
                $edit_url .
                '">' .
                $value .
    ...
    public function single_row($entry)
    {
        ...
        $class .= in_array($this->filter, ['trash', 'spam'])
            ? ' entry_spam_trash'
            : '';
        echo sprintf(
            '<tr id="entry_row_%d" class="%s" data-id="%d">',
            $entry['id'],
            $class,
            $entry['id']
        );
        ...
        switch ($this->filter) {
            ...
            case 'star':
                $message = isset($_GET['field_id'])
                    ...
                    : esc_html__(
                        'This form does not have any starred entries.',
                        'gravityforms'
                    );
                break;
            ...
            case 'spam':
                $message = esc_html__(
                    'This form does not have any spam.',
                    'gravityforms'
                );
                break;
            ...
            case 'trash':
                $message = isset($_GET['field_id'])
                    ? esc_html__(
                        'This form does not have any entries in the trash matching the search criteria.',
                        'gravityforms'
                    )
                    : esc_html__(
                        'This form does not have any entries in the trash.',
                        'gravityforms'
                    );
                break;
            ...
            default:
                $message = isset($_GET['field_id'])
                    ? esc_html__(
                        'This form does not have any entries matching the search criteria.',
                        'gravityforms'
                    )
                    : esc_html__(
   ...
   switch ($this->filter) { case 'trash': ?>
     ...
     if (GFCommon::current_user_can_any('gravityforms_delete_entries')) { ?>
						<span class="edit">
							<a data-wp-lists='delete:the-list:entry_row_<?php echo esc_attr(
           $entry['id']
       ); ?>::status=restore&entry=<?php echo esc_attr(
    $entry['id']
); ?>' href="<?php echo wp_nonce_url(
    '?page=gf_entries',
    'gf_delete_entry'
); ?>"><?php esc_html_e('Restore', 'gravityforms'); ?></a>
							|
						</span>
						...
						<span class="delete">
                            <?php
                            $delete_link =
                                '<a data-wp-lists="delete:the-list:entry_row_' .
                                esc_attr($entry['id']) .
                                '::status=delete&entry=' .
                                esc_attr($entry['id']) .
                                '" href="' .
     ...
     case 'spam': ?>
					...
					<span class="unspam">
                        <a data-wp-lists='delete:the-list:entry_row_<?php echo esc_attr(
                            $entry['id']
                        ); ?>::status=unspam&entry=<?php echo esc_attr(
    $entry['id']
); ?>' aria-label="<?php esc_attr_e(
    'Mark this entry as not spam',
    'gravityforms'
); ?>" href="<?php echo wp_nonce_url(
    '?page=gf_entries',
    'gf_delete_entry'
); ?>"><?php esc_html_e('Not Spam', 'gravityforms'); ?></a>
						<?php echo GFCommon::current_user_can_any('gravityforms_delete_entries')
          ? '|'
          : ''; ?>
                    </span>
     ...
     default: ?>
					...
					<span class="edit">
                        <a id="mark_read_<?php echo esc_attr(
                            $entry['id']
                        ); ?>" aria-label="Mark this entry as read" href="javascript:ToggleRead('<?php echo esc_js(
    $entry['id']
); ?>', '<?php echo esc_js(
    $this->filter
); ?>');" style="display:<?php echo $entry['is_read']
    ? 'none'
    : 'inline'; ?>;"><?php esc_html_e(
    'Mark read',
    'gravityforms'
); ?></a><a id="mark_unread_<?php echo absint(
    $entry['id']
); ?>" aria-label="<?php esc_attr_e(
    'Mark this entry as unread',
    'gravityforms'
); ?>" href="javascript:ToggleRead('<?php echo esc_js(
    $entry['id']
); ?>', '<?php echo esc_js(
    $this->filter
); ?>');" style="display:<?php echo $entry['is_read']
    ? 'inline'
    : 'none'; ?>;"><?php esc_html_e('Mark unread', 'gravityforms'); ?></a>
						<?php echo GFCommon::current_user_can_any('gravityforms_delete_entries') ||
      GFCommon::akismet_enabled($form_id)
          ? '|'
          : ''; ?>
                    </span>
					<?php
     if (GFCommon::spam_enabled($form_id)) { ?>
						<span class="spam">
                            <a data-wp-lists='delete:the-list:entry_row_<?php echo esc_attr(
                                $entry['id']
                            ); ?>::status=spam&entry=<?php echo esc_attr(
    $entry['id']
); ?>' aria-label="<?php esc_attr_e(
    'Mark this entry as spam',
    'gravityforms'
); ?>" href="<?php echo wp_nonce_url(
    '?page=gf_entries',
    'gf_delete_entry'
); ?>"><?php esc_html_e('Spam', 'gravityforms'); ?></a>
							<?php echo GFCommon::current_user_can_any('gravityforms_delete_entries')
           ? '|'
           : ''; ?>
                        </span>
     ...
     if (GFCommon::current_user_can_any('gravityforms_delete_entries')) { ?>
						<span class="trash">
	                        <a data-wp-lists='delete:the-list:entry_row_<?php echo esc_attr(
                             $entry['id']
                         ); ?>::status=trash&entry=<?php echo esc_attr(
    $entry['id']
); ?>' aria-label="<?php esc_attr_e(
    ...
    '?page=gf_entries',
    'gf_delete_entry'
); ?>"><?php esc_html_e('Trash', 'gravityforms'); ?></a>
	                    </span>
						<?php }
     break;
     }
  ...
  return '<button type="button" class="toggle-row"><span class="screen-reader-text">' .
    ...
    function get_primary_column_name()
    {
        if (!isset($this->primary_column_name)) {
            ...
            $column_index = in_array($this->filter, ['trash', 'spam']) ? 1 : 2;
            $primary = isset($column_keys[$column_index])
    ...
    function get_bulk_actions()
    {
        ...
        switch ($this->filter) {
            ...
            default:
                $actions['mark_read'] = esc_html__(
                    'Mark as Read',
                    'gravityforms'
                );
                $actions['mark_unread'] = esc_html__(
                    'Mark as Unread',
                    'gravityforms'
                );
                $actions['add_star'] = esc_html__('Add Star', 'gravityforms');
                $actions['remove_star'] = esc_html__(
                    'Remove Star',
                    'gravityforms'
                );
                $actions['resend_notifications'] = esc_html__(
        ...
        return gf_apply_filters(
            ['gform_entry_list_bulk_actions', $form_id],
            $actions,
            $form_id
        );
    }
    ...
    function bulk_actions($which = '')
    {
        ...
        if (in_array($filter, ['trash', 'spam'])) {

            $message =
                $filter == 'trash'
                    ? esc_html__(
                        "WARNING! This operation cannot be undone. Empty trash? 'Ok' to empty trash. 'Cancel' to abort.",
                        'gravityforms'
                    )
                    : esc_html__(
                        "WARNING! This operation cannot be undone. Permanently delete all spam? 'Ok' to delete. 'Cancel' to abort.",
                        'gravityforms'
                    );
            $button_label =
                $filter == 'trash'
        ...
        if ($single_action) {
            ...
            switch ($single_action) {
                case 'delete':
                    ...
                    break;
                case 'change_columns':
                    $columns = GFCommon::json_decode(
                        stripslashes($_POST['grid_columns']),
                        true
                    );
             ...
             */
            gf_do_action(
                ['gform_entry_list_action', $single_action, $form_id],
                $single_action,
                [$entry_id],
                $form_id
            );
            ...
            $entries = empty($select_all)
                ? $_POST['entry']
                : GFAPI::get_entry_ids($form_id, $search_criteria);
            ...
            switch ($bulk_action) {
                ...
                case 'trash':
                    if (
                        ...
                        GFFormsModel::update_entries_property(
                            $entries,
                            'status',
                            'trash'
                        );
                        $message = sprintf(
                            esc_html__('%s moved to Trash.', 'gravityforms'),
                            $entry_count
                        );
                    } else {
                        $message = esc_html__(
                            "You don't have adequate permissions to trash entries.",
                            'gravityforms'
                        );
                        $message_class = 'error';
                    }
                ...
                case 'restore':
                    if (
                        ...
                        $message = esc_html__(
                            "You don't have adequate permissions to restore entries.",
                            'gravityforms'
                        );
                        $message_class = 'error';
                    }
             ...
             */
            gf_do_action(
                ['gform_entry_list_action', $bulk_action, $form_id],
                $bulk_action,
                $entries,
                $form_id
            );
    ...
    function output_scripts()
    {
        ...
        $init_field_operator = empty($search_operator)
            ? 'contains'
            : $search_operator;
        $init_filter_vars = [
            'mode' => 'off',
		...
		<script type="text/javascript">
			...
			function UpdateEntryProperty(entry_id, name, value) {
				...
				mysack.setVar("rg_update_lead_property", "<?php echo wp_create_nonce(
        'rg_update_lead_property'
         ...
         __('Ajax error while setting lead property', 'gravityforms')
     ); ?>)
				};
				mysack.runAJAX();
			...
			function handleBulkApply(actionElement) {
				...
				if (leadIds.length == 0) {
					alert(<?php echo json_encode(
         __('Please select at least one entry.', 'gravityforms')
     ); ?>);
					return false;
				}
				...
				switch (action) {
					...
					case 'resend_notifications':
						...
						tb_show(<?php echo json_encode(
          esc_html__('Resend Notifications', 'gravityforms')
					...
					case 'print':
						resetPrintUI();
						tb_show(<?php echo json_encode(
          esc_html__('Print Entries', 'gravityforms')
          ...
          'gf_resend_notifications'
					...
					function (response) {
						...
						if (response) {
							...
							var message = <?php echo json_encode(
           __('Notifications for %s were resent successfully.', 'gravityforms')
       ); ?>;
							var c = leadIds == 0 ? gformVars.countAllEntries : leadIds.length;
							displayMessage(message.replace('%s', c + ' ' + getPlural(c, <?php echo json_encode(
           __('entry', 'gravityforms')
       ); ?>, <?php echo json_encode(
    __('entries', 'gravityforms')
); ?>)), "updated", "#entry_list_form");
							closeModal(true);
						}
			...
			function BulkPrint() {
				...
				var url = '<?php echo trailingslashit(
        site_url()
			...
			var gformStrings = {
				"allEntriesOnPageAreSelected": <?php echo json_encode(
        sprintf(
            esc_html__(
                'All %s{0}%s entries on this page are selected.',
                'gravityforms'
            ),
            '<strong>',
            '</strong>'
				...
				"selectAll"                  : <?php echo json_encode(
        sprintf(
            esc_html__('Select all %s{0}%s entries.', 'gravityforms'),
            '<strong>',
            '</strong>'
        )
    ); ?>,
				"allEntriesSelected"         : <?php echo json_encode(
        sprintf(
            esc_html__(
                'All %s{0}%s entries have been selected.',
                'gravityforms'
            ),
            '<strong>',
            '</strong>'
				...
				"clearSelection"             : <?php echo json_encode(
        __('Clear selection', 'gravityforms')
    ); ?>
			};

			var gformVars = {
				"countAllEntries": <?php echo intval(
        $this->get_pagination_arg('total_items')
    ); ?>,
			...
			jQuery(document).ready(function () {
				...
				var list = jQuery("#the-list").wpList({ delAfter: afterAjaxDelete, alt: <?php echo json_encode(
        esc_html__('Entry List', 'gravityforms')
    ); ?>});
				list.bind('wpListDelEnd', function (e, s, list) {
					var currentStatus = <?php echo json_encode(
         $filter == 'trash' || $filter == 'spam' ? $filter : 'active'
     ); ?>;
				...
				jQuery("#entry_filters").on("keypress", ".gform-filter-value", (function (event) {
					if (event.keyCode == 13) {
						Search(<?php echo json_encode($orderby); ?>, <?php echo json_encode(
    $order
); ?>, <?php echo absint(
    $form_id
); ?>, jQuery('.gform-filter-value').val(), <?php echo json_encode(
    $filter
    ...
    public function modals()
    {
			...
			<div id="notifications_container">
				...
				<div id="post_tag" class="tagsdiv">
					<div id="resend_notifications_options">
						...
						<?php
      $notifications = GFCommon::get_notifications(
      ...
      if (!is_array($notifications) || count($form['notifications']) <= 0) { ?>
           ...
           'You cannot resend notifications for these entries because this form does not currently have any notifications configured.',
           'gravityforms'
       ); ?></p>

							<a href="<?php echo esc_url(
           admin_url(
               "admin.php?page=gf_edit_forms&view=settings&subview=notification&id={$form['id']}"
           )
    ...
    'gravityforms'
							...
							<?php } else { ?>
							<p class="description"><?php esc_html_e(
           'Specify which notifications you would like to resend for the selected entries.',
           'gravityforms'
       ); ?></p>
    ...
    $notification['id']
            ...
            $notification['id']
							...
							<div id="notifications_override_settings" style="display:none;">
								...
								<p class="description" style="padding-top:0; margin-top:0;">
									<?php esc_html_e(
             'You may override the default notification settings by entering a comma delimited list of emails to which the selected notifications should be sent.',
             'gravityforms'
         ); ?>
								</p>
     ...
     'notification_override_email'
							...
							<input type="button" name="notification_resend" id="notification_resend" value="<?php esc_attr_e(
           'Resend Notifications',
           'gravityforms'
      ...
      ?>
					...
					<div id="resend_notifications_close" style="display:none;margin:10px 0 0;">
						<input type="button" name="resend_notifications_close_button" value="<?php esc_attr_e(
          'Close Window',
          'gravityforms'
			...
			<div id="print_container">
				...
				<div class="tagsdiv">
					<div id="print_options">
						...
						<p class="description"><?php esc_html_e(
          'Print all of the selected entries at once.',
          'gravityforms'
      ); ?></p>
						...
						<?php if (GFCommon::current_user_can_any('gravityforms_view_entry_notes')) { ?>
							...
							<label for="gform_print_notes"><?php esc_html_e(
           'Include notes',
           'gravityforms'
       ); ?></label>
							<br /><br />
						...
						<label for="gform_print_page_break"><?php esc_html_e(
          'Add page break between entries',
          'gravityforms'
      ); ?></label>
						<br /><br />

						<input type="button" value="<?php esc_attr_e(
          'Print',
          'gravityforms'
    ...
    function get_grid_columns()
    {
        if (!isset($this->_grid_columns)) {
            $this->_grid_columns = GFFormsModel::get_grid_columns(
                $this->get_form_id(),
                true
            );
        }
