...
class GFFormDisplay
{
    ...
    {
        GFCommon::log_debug(
            "GFFormDisplay::process_form(): Starting to process form (#{$form_id}) submission."
        );
         ...
         */
        $filtered_form = gf_apply_filters(
            ['gform_pre_process', $form['id']],
            $form
        );
         ...
         */
        $require_login = gf_apply_filters(
            ['gform_require_login', $form['id']],
            rgar($form, 'requireLogin'),
            $form
        );
        ...
        GFCommon::log_debug(
            "GFFormDisplay::process_form(): Source page number: {$source_page_number}. Target page number: {$target_page}."
        );
        ...
        if (
            !$saving_for_later &&
            (empty($target_page) || $target_page >= $page_number)
        ) {
        ...
        GFCommon::log_debug(
            "GFFormDisplay::process_form(): After validation. Is submission valid? {$log_is_valid}."
        );
        ...
        if ($target_page > $page_number || $target_page == 0) {
            if (!empty($_FILES) && !$saving_for_later) {
                ...
                GFCommon::log_debug(
                    'GFFormDisplay::process_form(): Uploading files...'
                );
        ...
        if (($is_valid && $page_number == 0) || $saving_for_later) {
            ...
            if (
                GFFormsModel::is_submit_button_hidden($form) &&
                !$saving_for_later
            ) {
                // Ignore submission.
                return;
            ...
            $failed_honeypot =
                rgar($form, 'enableHoneypot') &&
                !self::validate_honeypot($form);

            if ($failed_honeypot) {
                GFCommon::log_debug(
                    'GFFormDisplay::process_form(): Failed Honeypot validation. Displaying confirmation and aborting.'
                );
                ...
                GFCommon::log_debug(
                    'GFFormDisplay::process_form(): Submission is valid. Moving forward.'
                );
                ...
                $form = gf_apply_filters(
                    ['gform_pre_submission_filter', $form_id],
                    $form
                );
                ...
                if (
                    has_filter('gform_after_submission') ||
                    has_filter("gform_after_submission_{$form['id']}")
                ) {
                    GFCommon::log_debug(
                        __METHOD__ .
                            '(): Executing functions hooked to gform_after_submission.'
                    );
                }
                /**
                 ...
                 */
                gf_do_action(
                    ['gform_after_submission', $form['id']],
                    $lead,
                    $form
                );
            } elseif ($saving_for_later) {
                GFCommon::log_debug(
                    'GFFormDisplay::process_form(): Saving for later.'
                );
                ...
                $nl2br = rgar($form['confirmation'], 'disableAutoformat')
                    ? false
                    : true;
                $confirmation = GFCommon::replace_variables(
                    $confirmation,
                    $form,
                    $lead,
                    false,
                ...
                $ip = rgars($form, 'personalData/preventIP')
                    ? ''
                    : GFFormsModel::get_ip();
                ...
                $notifications_to_send = GFCommon::get_notifications_to_send(
                    'form_saved',
                    $form,
                    $lead
                );

                $log_notification_event = empty($notifications_to_send)
                    ? 'No notifications to process'
                    : 'Processing notifications';
                GFCommon::log_debug(
                    "GFFormDisplay::process_form(): {$log_notification_event} for form_saved event."
                );
                ...
                foreach ($notifications_to_send as $notification) {
                    if (
                        ...
                        GFCommon::log_debug(
                            "GFFormDisplay::process_form(): Notification is inactive, not processing notification (#{$notification['id']} - {$notification['name']})."
                        );
                        continue;
                    }
                    $notification['message'] = self::replace_save_variables(
                ...
                self::set_submission_if_null(
                    $form_id,
                    'resume_token',
                    $resume_token
                );
                GFCommon::log_debug(
                    'GFFormDisplay::process_form(): Saved incomplete submission.'
                );
            }
             ...
             */
            $suppress_redirect = apply_filters(
    ...
    public static function maybe_add_review_page($form, $partial_entry = [])
    {
        ...
        if (
            has_filter('gform_review_page') ||
            has_filter("gform_review_page_{$form['id']}")
        ) {
             ...
             */
            $review_page = gf_apply_filters(
                ['gform_review_page', $form['id']],
                $review_page,
                $form,
                $partial_entry
            );
            ...
            if (!rgempty('button_text', $review_page)) {
                $review_page['nextButton']['text'] =
                    $review_page['button_text'];
            }
        }
    ...
    private static function upload_files($form, $files)
    {
        ...
        GFCommon::log_debug(
            "GFFormDisplay::upload_files(): Upload path {$form_upload_path}"
        );
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (
                !in_array($input_type, ['fileupload', 'post_image']) ||
                $field->multipleFiles
            ) {
                continue;
            }
            ...
            if ($field->failed_validation) {
                GFCommon::log_debug(
                    "GFFormDisplay::upload_files(): Skipping field because it failed validation: {$field->label}({$field->id} - {$field->type})."
                );
                continue;
            }
            ...
            if (empty($_FILES[$input_name]['name'])) {
                GFCommon::log_debug(
                    "GFFormDisplay::upload_files(): Skipping field because a file could not be found: {$field->label}({$field->id} - {$field->type})."
                );
                continue;
            }
            ...
            if (GFCommon::file_name_has_disallowed_extension($file_name)) {
                GFCommon::log_debug(
                    __METHOD__ . "(): Illegal file extension: {$file_name}"
                );
                continue;
            }

            $allowed_extensions = !empty($field->allowedExtensions)
                ? GFCommon::clean_extensions(
                    explode(',', strtolower($field->allowedExtensions))
                )
                : [];

            if (!empty($allowed_extensions)) {
                if (
                    !GFCommon::match_file_extension(
                        $file_name,
                        $allowed_extensions
                    )
                ) {
                    GFCommon::log_debug(
                        __METHOD__ .
                            "(): The uploaded file type is not allowed: {$file_name}"
                    );
                    continue;
                }
            }
             ...
             */
            $whitelisting_disabled = apply_filters(
            ...
            if (empty($allowed_extensions) && !$whitelisting_disabled) {
                ...
                $valid_file_name = GFCommon::check_type_and_ext(
                    $_FILES[$input_name],
                    $file_name
                );
            ...
            $file_info = RGFormsModel::get_temp_filename(
                $form['id'],
                $input_name
            );
            GFCommon::log_debug(
                'GFFormDisplay::upload_files(): Temp file info: ' .
            ...
            if (
                ...
                move_uploaded_file(
                    ...
                    $target_path . $file_info['temp_filename']
                )
            ) {
                GFFormsModel::set_permissions(
                    $target_path . $file_info['temp_filename']
                );
                $files[$input_name] = $file_info['uploaded_filename'];
                GFCommon::log_debug(
                    "GFFormDisplay::upload_files(): File uploaded successfully: {$file_info['uploaded_filename']}"
                );
            } else {
                GFCommon::log_error(
                    "GFFormDisplay::upload_files(): File could not be uploaded: tmp_name: {$_FILES[$input_name]['tmp_name']} - target location: " .
                        $target_path .
    ...
    public static function get_state($form, $field_values)
    {
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (
                ...
                $value = RGFormsModel::get_field_value(
                    $field,
                    $field_values,
                    false
                );
                ...
                switch ($field->inputType) {
                    ...
                    case 'hiddenproduct':
                        ...
                        $product_name =
                            !is_array($value) ||
                            empty($value[$field->id . '.1'])
                                ? $field->label
                                : $value[$field->id . '.1'];

                        $fields[$field->id . '.1'] = wp_hash(
                    ...
                    case 'singleshipping':
                        ...
                        $price = !empty($price)
                            ? GFCommon::to_number($price)
                            : 0;

                        $fields[$field->id] = wp_hash(
                            GFFormsModel::maybe_trim_input(
                                $price,
                                $form['id'],
                                $field
                    ...
                    case 'consent':
                        ...
                        $description = GFFormsModel::get_latest_form_revisions_id(
                            $form['id']
                        );
        ...
        return base64_encode(json_encode([$hash, $checksum]));
    }
    ...
    private static function has_enhanced_dropdown($form)
    {
        ...
        foreach ($form['fields'] as $field) {
            if (
                in_array(RGFormsModel::get_input_type($field), [
                    'select',
                    'multiselect',
                ]) &&
    ...
    private static function has_password_visibility($form)
    {
        ...
        foreach ($form['fields'] as $field) {
            if (
                $field->type == 'password' &&
                $field->passwordVisibilityEnabled
            ) {
                return true;
            }
        }
    ...
    public static function get_target_page($form, $current_page, $field_values)
    {
        ...
        while (
            RGFormsModel::is_page_hidden($form, $page_number, $field_values)
        ) {
    ...
    {
        $page_number = isset(self::$submission[$form_id])
            ? intval(self::$submission[$form_id]['page_number'])
            : 1;
    ...
    public static function is_last_page($form, $mode = 'submit')
    {
        ...
        if ($mode == 'render') {
            ...
            $is_last_page =
                $is_valid && $target_page == self::get_max_page_number($form);
        } else {
        ...
        switch ($period) {
            ...
            case 'month':
                ...
                return [
                    ...
                    'end_date' => gmdate(
                        'Y-m-d H:i:s',
                        strtotime("{$month_start} +1 month -1 second")
                    ),
                ];
                break;
    ...
    public static function get_form(
        ...
        if (rgpost('gform_send_resume_link') == $form_id) {
            $save_email_confirmation = self::handle_save_email_confirmation(
                $form,
                $ajax
            );
            if (is_wp_error($save_email_confirmation)) {
                ...
                $resume_token = rgpost('gform_resume_token');
                $resume_token = sanitize_key($resume_token);
                $incomplete_submission_info = GFFormsModel::get_draft_submission_values(
                    $resume_token
                );
                if ($incomplete_submission_info['form_id'] == $form_id) {
                    $submission_details_json =
                        $incomplete_submission_info['submission'];
                    $submission_details = json_decode(
                        $submission_details_json,
                        true
                    );
                    $partial_entry = $submission_details['partial_entry'];
                    $form = self::update_confirmation(
                        $form,
                        $partial_entry,
                        'form_saved'
                    );
                    $confirmation_message = rgar(
                        $form['confirmation'],
                        'message'
                    );
                    $nl2br = rgar($form['confirmation'], 'disableAutoformat')
                    ...
                    return self::handle_save_confirmation(
                        $form,
                        $resume_token,
                        $confirmation_message,
                        $ajax
        ...
        $submission_info = isset(self::$submission[$form_id])
            ? self::$submission[$form_id]
            : false;
        ...
        if (isset($_GET['gf_token'])) {
            ...
            if (rgar($incomplete_submission_info, 'form_id') == $form_id) {
                ...
                GFFormsModel::$unique_ids[$form_id] =
                    $submission_details['gform_unique_id'];
                GFFormsModel::$uploaded_files[$form_id] =
                    $submission_details['files'];
                self::set_submission_if_null(
                    $form_id,
                    'resuming_incomplete_submission',
                    true
                );
        ...
        if (!is_array($partial_entry)) {
             ...
             */
            $view_counter_disabled = gf_apply_filters(
                ['gform_disable_view_counter', $form_id],
                false
            );
            ...
            if ($submission_info) {
                $is_postback = true;
                $is_valid =
                    rgar($submission_info, 'is_valid') ||
                    rgar($submission_info, 'is_confirmation');
                $form = $submission_info['form'];
                $lead = $submission_info['lead'];
                $confirmation_message = rgget(
                    'confirmation_message',
                    $submission_info
                );
                ...
                if (
                    $is_valid &&
                    !RGForms::get('is_confirmation', $submission_info)
                ) {
                    if ($submission_info['page_number'] == 0) {
                         ...
                         */
                        gf_do_action(
                            ['gform_post_paging', $form['id']],
                            $form,
                            $submission_info['source_page_number'],
                            $submission_info['page_number']
                        );
                    }
                }
            } elseif (
                !current_user_can('administrator') &&
                !$view_counter_disabled
            ) {
        ...
        GFCommon::$tab_index = gf_apply_filters(
            ['gform_tabindex', $form_id],
            $tabindex,
            $form
        );
        ...
        if (!$force_display && !$is_postback) {
            ...
            if (rgar($form, 'requireLogin')) {
                if (!is_user_logged_in()) {
                    return empty($form['requireLoginMessage'])
                        ? '<p>' .
                                esc_html__(
        ...
        if (empty($confirmation_message)) {
            $wrapper_css_class =
                GFCommon::get_browser_class() . ' gform_wrapper';
            ...
            $style = self::has_conditional_logic($form)
                ? "style='display:none'"
                : '';
            ...
            if (!empty($form_css_class)) {
                ...
                $custom_wrapper_css_class =
                    ' ' . implode(' ', $form_css_classes);
            }

            $form_string .=
                "
                <div class='{$wrapper_css_class}{$custom_wrapper_css_class}' id='gform_wrapper_$form_id' " .
                $style .
                '>';
            ...
            $form_css_class = !empty($form['cssClass'])
                ? "class='{$form_css_class}'"
                : '';

            $action = esc_url($action);
            $form_string .= gf_apply_filters(
                ['gform_form_tag', $form_id],
                "<form method='post' enctype='multipart/form-data' {$target} id='gform_{$form_id}' {$form_css_class} action='{$action}'>",
                $form
            );
            ...
            if (
                isset($_GET['gf_token']) &&
                !is_array($incomplete_submission_info)
            ) {
                /**
                 ...
                 * @param array  $form    The current Form object.
                 */
                $savecontinue_expired_message = gf_apply_filters(
                    ['gform_savecontinue_expired_message', $form['id']],
                    esc_html__(
            ...
            if ($has_pages && $is_postback && !$is_valid) {
                self::set_current_page(
                    $form_id,
                    GFFormDisplay::get_first_page_with_error($form)
                );
            }
            ...
            if ($has_pages && !$is_admin) {
                if ($form['pagination']['type'] == 'percentage') {
                    $form_string .= self::get_progress_bar(
                        $form,
                        $current_page,
                        $confirmation_message
                    );
                } elseif ($form['pagination']['type'] == 'steps') {
                    $form_string .= self::get_progress_steps(
                        $form,
                        $current_page
                    );
                }
            ...
            if ($is_postback && !$is_valid) {
                if (gf_upgrade()->get_submissions_block()) {
                    $validation_message =
                        "<div class='gf_submission_limit_message'><p>" .
                        esc_html__(
                            'Your form was not submitted. Please try again in a few minutes.',
                            'gravityforms'
                        ) .
                        '</p></div>';
                } else {
                ...
                $form_string .= gf_apply_filters(
                    ['gform_validation_message', $form_id],
                    $validation_message,
                    $form
                );
            }
            ...
            if ($has_pages) {
                $style = self::is_page_active($form_id, 1)
                    ? ''
                    : "style='display:none;'";
                $class = !empty($form['firstPageCssClass'])
            ...
            $form_string .=
                "<ul id='gform_fields_{$form_id}' class='" .
                GFCommon::get_ul_classes($form) .
                "'>";

            if (is_array($form['fields'])) {
                foreach ($form['fields'] as $field) {
                    /* @var GF_Field $field */
                    $field->conditionalLogicFields = self::get_conditional_logic_fields(
                        $form,
                        $field->id
                    );
                    ...
                    if (is_array($submitted_values)) {
                        ...
                        if (
                            $field->type === 'consent' &&
                            ($field_value[$field->id . '.3'] !=
                                GFFormsModel::get_latest_form_revisions_id(
                                    $form['id']
                                ) ||
                                $field_value[$field->id . '.2'] !=
                                    $field->checkboxLabel)
                        ) {
                            $field_value = GFFormsModel::get_field_value(
                                $field,
                                $field_values
                            );
                        }
            ...
            if ($has_pages) {
                $previous_button_alt = rgempty(
                    'imageAlt',
                    $form['lastPageButton']
                )
                    ? __('Previous Page', 'gravityforms')
                    : $form['lastPageButton']['imageAlt'];
                $previous_button = self::get_form_button(
                    $form['id'],
                    "gform_previous_button_{$form['id']}",
                    $form['lastPageButton'],
                    __('Previous', 'gravityforms'),
                    'gform_previous_button',
                    $previous_button_alt,
                    self::get_current_page($form_id) - 1
                );
                 ...
                 */
                $previous_button = gf_apply_filters(
                    ['gform_previous_button', $form_id],
                    $previous_button,
                    $form
                );
                $form_string .=
                    ...
                    self::gform_footer(
                        $form,
                        'gform_page_footer ' . $form['labelPlacement'],
                        $ajax,
                        $field_values,
                        $previous_button,
                        $display_title,
            ...
            if (!$has_pages) {
                $form_string .= self::gform_footer(
                    $form,
                    'gform_footer ' . $form['labelPlacement'],
                    $ajax,
                    $field_values,
                    '',
                    $display_title,
            ...
            if ($ajax && !$is_postback) {
                $spinner_url = gf_apply_filters(
                    ['gform_ajax_spinner_url', $form_id],
                    GFCommon::get_base_url() . '/images/spinner.gif',
                    $form
                );
                $scroll_position = ['default' => '', 'confirmation' => ''];

                if ($anchor['scroll'] !== false) {
                    $scroll_position['default'] = is_numeric($anchor['scroll'])
                        ? 'jQuery(document).scrollTop(' .
                            intval($anchor['scroll']) .
                            ');'
                        : "jQuery(document).scrollTop(jQuery('#gform_wrapper_{$form_id}').offset().top - mt);";
                    $scroll_position['confirmation'] = is_numeric(
                        $anchor['scroll']
                    )
                        ? 'jQuery(document).scrollTop(' .
                            intval($anchor['scroll']) .
                            ');'
                        : "jQuery(document).scrollTop(jQuery('{$anchor['id']}').offset().top - mt);";
                }

                // Accessibility enhancements to properly handle the iframe title and content.
                $iframe_content = esc_html__(
                ...
                if (defined('GF_DEBUG') && GF_DEBUG) {
                    ...
                    $iframe_style =
                        'display:block;width:600px;height:300px;border:1px solid #eee;';
                } else {
                ...
                $form_string .=
                    "
                <iframe style='{$iframe_style}' src='about:blank' name='gform_ajax_frame_{$form_id}' id='gform_ajax_frame_{$form_id}'" .
                    $iframe_title .
                    ">" .
                    $iframe_content .
                    "</iframe>
            ...
            if (!$ajax || $is_first_load) {
                ...
                if (apply_filters('gform_init_scripts_footer', false)) {
                    $callback = [
                        new GF_Late_Static_Binding(['form_id' => $form['id']]),
                        'GFFormDisplay_footer_init_scripts',
                    ];
                    ...
                    $form_string .=
                        "<script type='text/javascript'>" .
                        apply_filters('gform_cdata_open', '') .
                        " jQuery(document).ready(function(){jQuery(document).trigger('gform_post_render', [{$form_id}, {$current_page}]) } ); " .
                        apply_filters('gform_cdata_close', '') .
                        '</script>';
                }
            }
            ...
            $start_at_zero = rgars(
                $form,
                'pagination/display_progressbar_on_confirmation'
            );
            $start_at_zero = apply_filters(
            ...
            if (
                ...
                ($form['confirmation']['type'] == 'message' &&
                    $form['pagination']['type'] == 'percentage')
            ) {
                $progress_confirmation = self::get_progress_bar(
                    $form,
                    0,
                    $confirmation_message
                );
                if ($ajax) {
                    $progress_confirmation = self::get_ajax_postback_html(
                        $progress_confirmation
                    );
                }
            } else {
    ...
    public static function footer_init_scripts($form_id)
    {
         ...
         */
        $form_string = gf_apply_filters(
            ['gform_footer_init_scripts_filter', $form_id],
            $form_string,
            $form,
            $current_page
        );
    ...
    public static function add_init_script(
        $form_id,
        $script_name,
        $location,
        $script
    ...
    public static function get_form_button(
        $form_id,
        $button_input_id,
        $button,
        $default_text,
        ...
        if (
            rgar($button, 'type') == 'text' ||
            rgar($button, 'type') == 'link' ||
            empty($button['imageUrl'])
        ) {
            ...
            if (rgar($button, 'type') == 'link') {
                ...
                $class .= ' button';
                $button_input =
                    "<input type='{$input_type}' id='{$button_input_id}' class='{$class}' value='" .
    ...
    public static function gform_footer(
        $form,
        $class,
        $ajax,
        $field_values,
        ...
        <div class='" .
            esc_attr($class) .
            "'>";
        $button_input = self::get_form_button(
            $form['id'],
            "gform_submit_button_{$form['id']}",
            $form['button'],
            __('Submit', 'gravityforms'),
            'gform_button',
            __('Submit', 'gravityforms'),
            0
        );
        $button_input = gf_apply_filters(
            ['gform_submit_button', $form_id],
            $button_input,
            $form
        );

        $save_button = rgars($form, 'save/enabled')
            ? self::get_form_button(
                $form_id,
                "gform_save_{$form_id}_footer",
                $form['save']['button'],
                rgars($form, 'save/button/text'),
                'gform_save_link',
                rgars($form, 'save/button/text'),
                0,
                "jQuery(\"#gform_save_{$form_id}\").val(1);"
            )
            : '';
        ...
        $save_button = apply_filters(
            "gform_savecontinue_link_{$form_id}",
            $save_button,
            $form
        );
        ...
        if ($ajax) {
            $footer .=
                "<input type='hidden' name='gform_ajax' value='" .
                esc_attr(
                    "form_id={$form_id}&amp;title={$display_title}&amp;description={$display_description}&amp;tabindex={$tabindex}"
                ) .
                "' />";
        }
        ...
        $next_page =
            $next_page > self::get_max_page_number($form) ? 0 : $next_page;
        $field_values_str = is_array($field_values)
            ? http_build_query($field_values)
            : $field_values;
        $files_input = '';
        if (
            GFCommon::has_multifile_fileupload_field($form) ||
            !empty(RGFormsModel::$uploaded_files[$form_id])
        ) {
            $files = !empty(RGFormsModel::$uploaded_files[$form_id])
                ? json_encode(
                    RGFormsModel::$uploaded_files[$form_id],
                    JSON_UNESCAPED_UNICODE
                )
                : '';
            $files_input =
                "<input type='hidden' name='gform_uploaded_files' id='gform_uploaded_files_{$form_id}' value='" .
                str_replace("'", '&#039;', $files) .
                "' />";
        }
        $save_inputs = '';
        if (rgars($form, 'save/enabled')) {
            $resume_token = isset($_POST['gform_resume_token'])
                ? $_POST['gform_resume_token']
                : rgget('gf_token');
            $resume_token = sanitize_key($resume_token);
        ...
        $unique_id =
            isset(self::$submission[$form_id]) &&
            rgar(
                self::$submission[$form_id],
                'resuming_incomplete_submission'
            ) == true
                ? rgar(GFFormsModel::$unique_ids, $form_id)
        ...
        $footer .=
            ...
            esc_attr($unique_id) .
            "' />
            <input type='hidden' class='gform_hidden' name='state_{$form_id}' value='" .
            self::get_state($form, $field_values) .
            "' />
            <input type='hidden' class='gform_hidden' name='gform_target_page_number_{$form_id}' id='gform_target_page_number_{$form_id}' value='" .
            esc_attr($next_page) .
            "' />
            <input type='hidden' class='gform_hidden' name='gform_source_page_number_{$form_id}' id='gform_source_page_number_{$form_id}' value='" .
            esc_attr($current_page) .
            "' />
            <input type='hidden' name='gform_field_values' value='" .
            esc_attr($field_values_str) .
            "' />
            {$files_input}
        </div>";
    ...
    {
        $honeypot_labels = ['Name', 'Email', 'Phone', 'Comments'];
    ...
    public static function handle_submission($form, &$lead, $ajax = false)
    {
        $lead_id = gf_apply_filters(
            ['gform_entry_id_pre_save_lead', $form['id']],
            null,
            $form
        );
        ...
        if ($is_spam) {
            ...
            GFFormsModel::update_entry_property(
                $lead['id'],
                'status',
                'spam',
                false,
        ...
        $lead = gf_apply_filters(
            ['gform_entry_post_save', $form['id']],
            $lead,
            $form
        );
    ...
    public static function handle_confirmation(
        ...
        GFCommon::log_debug(
            'GFFormDisplay::handle_confirmation(): Sending confirmation.'
        );
        ...
        if ($form['confirmation']['type'] == 'message') {
            ...
            if (!empty($form['confirmation']['pageId'])) {
                ...
                $url = GFCommon::replace_variables(
                    trim($form['confirmation']['url']),
                    $form,
                    $lead,
                    false,
                    false,
            ...
            $dynamic_query = GFCommon::replace_variables(
                trim($form['confirmation']['queryString']),
                $form,
                $lead,
                true,
                false,
            ...
            $dynamic_query = str_replace(["\r", "\n"], '', $dynamic_query);
            $query_string .=
                rgempty('query', $url_info) || empty($dynamic_query)
                    ? $dynamic_query
                    : '&' . $dynamic_query;
        ...
        if (
            has_filter('gform_confirmation') ||
            has_filter("gform_confirmation_{$form['id']}")
        ) {
            GFCommon::log_debug(
                __METHOD__ .
        ...
        $confirmation = gf_apply_filters(
            ['gform_confirmation', $form['id']],
            $confirmation,
            $form,
            $lead,
            $ajax
        ...
        if (!is_array($confirmation)) {
            ...
            //Perform client side redirect for AJAX forms, of if headers have already been sent
            $confirmation = self::get_js_redirect_confirmation(
        ...
        GFCommon::log_debug(
            'GFFormDisplay::handle_confirmation(): Confirmation => ' .
    ...
    public static function get_confirmation_message(
        ...
        $message = empty($confirmation['message'])
            ? "{$anchor} "
            : "{$anchor}<div id='gform_confirmation_wrapper_{$form['id']}' class='gform_confirmation_wrapper {$css_class}'><div id='gform_confirmation_message_{$form['id']}' class='gform_confirmation_message_{$form['id']} gform_confirmation_message'>" .
                $message .
                '</div></div>';
     ...
     * @return string
     */
    private static function maybe_sanitize_confirmation_message(
        $confirmation_message
    ) {
        return GFCommon::maybe_sanitize_confirmation_message(
            $confirmation_message
        );
    }
    ...
    private static function get_js_redirect_confirmation($url, $ajax)
    {
        ...
        $url = defined('JSON_HEX_TAG')
            ? json_encode($url, JSON_HEX_TAG)
            : json_encode($url);
        $confirmation =
            "<script type=\"text/javascript\">" .
            apply_filters('gform_cdata_open', '') .
    ...
    public static function send_emails($form, $lead)
    {
        _deprecated_function(
            'send_emails',
            '1.7',
            'GFCommon::send_form_submission_notifications'
        );
        GFCommon::send_form_submission_notifications($form, $lead);
    }
    ...
    public static function validate(
        ...
        $is_valid = true;
        $is_last_page =
            self::get_target_page($form, $page_number, $field_values) == '0';
        foreach ($form['fields'] as &$field) {
            /* @var GF_Field $field */
            ...
            if ($field->isRequired && self::is_empty($field, $form['id'])) {
                ...
                $field->validation_message = empty($field->errorMessage)
                    ? __('This field is required.', 'gravityforms')
                    : $field->errorMessage;
            }
            //display error if field does not allow duplicates and the submitted value already exists
            elseif (
                $field->noDuplicates &&
                RGFormsModel::is_duplicate($form['id'], $field, $value)
            ) {
                $field->failed_validation = true;
                ...
                switch ($input_type) {
                    case 'date':
                        $default_message = __(
                            'This date has already been taken. Please select a new date.',
                            'gravityforms'
                        );
                        break;

                    default:
                        $default_message = is_array($value)
                            ? __(
                                'This field requires a unique entry and the values you entered have already been used.',
                                'gravityforms'
                            )
                            : sprintf(
                                __(
                ...
                $field->validation_message = gf_apply_filters(
                    ['gform_duplicate_message', $form['id']],
                    $default_message,
                    $form,
                    $field,
                    $value
                ...
                if (
                    self::failed_state_validation($form['id'], $field, $value)
                ) {
                    $field->failed_validation = true;
                    $field->validation_message = in_array($field->inputType, [
                        'singleproduct',
                        'singleshipping',
                        'hiddenproduct',
                        'consent',
                    ])
                        ? __('Please enter a valid value.', 'gravityforms')
                        : __(
                            'Invalid selection. Please select one of the available choices.',
                            'gravityforms'
                        );
                } else {
            ...
            $custom_validation_result = gf_apply_filters(
                ['gform_field_validation', $form['id'], $field->id],
                [
            ...
            $field->failed_validation = rgar(
                $custom_validation_result,
                'is_valid'
            )
                ? false
                : true;
            $field->validation_message = rgar(
                $custom_validation_result,
                'message'
            );
        ...
        $validation_result = gf_apply_filters(
            ['gform_validation', $form['id']],
            [
                'is_valid' => $is_valid,
                'form' => $form,
     ...
     * @return bool
     */
    public static function is_field_validation_supported(
        $field,
        $type_check_only = false
    ) {
        $is_valid_type = !in_array($field->type, ['html', 'page', 'section']);
        ...
        return !(
            $field->is_administrative() || $field->visibility === 'hidden'
        );
    }
    ...
    public static function is_form_empty($form)
    {
        foreach ($form['fields'] as $field) {
            if (
                self::is_field_validation_supported($field, true) &&
                !$field->is_field_hidden &&
                !self::is_empty($field, $form['id'])
            ) {
                return false;
            }
        }
    ...
    public static function failed_state_validation($form_id, $field, $value)
    {
        ...
        if (!isset($_gf_state)) {
            $state = json_decode(
                base64_decode($_POST["state_{$form_id}"]),
                true
            );
        ...
        foreach ($value as $key => $input_value) {
            ...
            $fails_hash =
                ...
                $state !== false &&
                ((is_array($state) && !in_array($hash, $state)) ||
                    (!is_array($state) && $hash != $state));
            $fails_sanitized_hash =
                strlen($sanitized_input_value) > 0 &&
                $state !== false &&
                ((is_array($state) && !in_array($sanitized_hash, $state)) ||
     ...
     * @param array  $found_blocks An array of found GF blocks.
     */
    public static function parse_forms(
        $post_content,
        &$found_forms,
        &$found_blocks
    ) {
     ...
     * @param array $found_blocks An array of found GF blocks.
     */
    public static function parse_forms_from_blocks(
        $blocks,
        &$found_forms,
        &$found_blocks
    ) {
        ...
        foreach ($blocks as $block) {
            ...
            if ($block['blockName'] === 'core/block') {
                self::parse_forms_from_reusable_block(
                    $block,
                    $found_forms,
                    $found_blocks
                );
            ...
            if (
                !$supported_type ||
                ($supported_type && empty($block['attrs']['formId']))
            ) {
                continue;
            }
            ...
            $ajax = isset($block['attrs']['ajax'])
                ? (bool) $block['attrs']['ajax']
                : false;
     ...
     * @param array $found_blocks An array of found GF blocks.
     */
    public static function parse_forms_from_reusable_block(
        $block,
        &$found_forms,
        &$found_blocks
    ) {
        ...
        if (
            empty($reusable_block) ||
            $reusable_block->post_type !== 'wp_block'
        ) {
            return;
        }
     ...
     * @param array  $found_forms  An array of found forms using the form ID as the key to the ajax status.
     */
    public static function parse_forms_from_shortcode(
        $post_content,
        &$found_forms
    ) {
        if (empty($post_content)) {
        ...
        if (
            ...
            foreach ($matches as $match) {
                ...
                $ajax =
                    isset($attr['ajax']) &&
                    strtolower(substr($attr['ajax'], 0, 4)) == 'true';
    ...
    {
        return !isset($found_forms[$form_id]) ||
            (isset($found_forms[$form_id]) &&
                true === $ajax &&
                false === $found_forms[$form_id]);
    }
    ...
    public static function enqueue_form_scripts($form, $ajax = false)
    {
        ...
        if (
            $ajax ||
            self::has_price_field($form) ||
            self::has_password_strength($form) ||
            self::has_password_visibility($form) ||
            GFCommon::has_list_field($form) ||
            GFCommon::has_credit_card_field($form) ||
            self::has_conditional_logic($form) ||
            self::has_currency_format_number_field($form) ||
            self::has_calculation_field($form) ||
            self::has_recaptcha_field($form) ||
            self::has_checkbox_field($form, true) ||
            self::has_js_merge_tag($form) ||
            GFCommon::has_repeater_field($form)
        ) {
    ...
    public static function print_form_scripts($form, $ajax)
    {
        if (!get_option('rg_gforms_disable_css')) {
            if (!wp_style_is('gforms_css')) {
                ...
                wp_enqueue_style('gforms_reset_css');
                wp_print_styles(['gforms_reset_css']);

                wp_enqueue_style('gforms_formsmain_css');
                wp_print_styles(['gforms_formsmain_css']);

                wp_enqueue_style('gforms_ready_class_css');
                wp_print_styles(['gforms_ready_class_css']);

                wp_enqueue_style('gforms_browsers_css');
                wp_print_styles(['gforms_browsers_css']);
                ...
                if (is_rtl()) {
                    wp_enqueue_style('gforms_rtl_css');
                    wp_print_styles(['gforms_rtl_css']);
                }
            }
        }
        ...
        if (
            (($ajax ||
                self::has_enhanced_dropdown($form) ||
                self::has_price_field($form) ||
                self::has_password_strength($form) ||
                self::has_password_visibility($form) ||
                self::has_pages($form) ||
                ...
                GFCommon::has_credit_card_field($form) ||
                self::has_calculation_field($form)) &&
                !wp_script_is('gform_gravityforms')) ||
            self::has_js_merge_tag($form)
        ) {
        ...
        if (
            self::has_conditional_logic($form) &&
            !wp_script_is('gform_conditional_logic')
        ) {
            $scripts[] = 'gform_conditional_logic';
        }

        if (
            self::has_datepicker_field($form) &&
            !wp_script_is('gform_datepicker_init')
        ) {
        ...
        if (
            self::has_character_counter($form) &&
            !wp_script_is('gform_textarea_counter')
        ) {
            $scripts[] = 'gform_textarea_counter';
        }

        if (
            self::has_input_mask($form) &&
            !wp_script_is('gform_masked_input')
        ) {
        ...
        if (is_array(rgar($form, 'fields'))) {
            foreach (rgar($form, 'fields') as $field) {
                if (
                    ...
                    self::has_conditional_logic_legwork([
                        'fields' => $field->fields,
                    ])
                ) {
                    return true;
     ...
     * @return string
     */
    private static function get_conditional_logic($form, $field_values = [])
    {
        $logics = '';
        $dependents = '';
        $fields_with_logic = [];
        ...
        foreach ($form['fields'] as $field) {
            ...
            $field_dependents[$field->id] = !empty($field_deps)
                ? $field_deps
                : [];

            //use section's logic if one exists
            $section = RGFormsModel::get_section($form, $field->id);
            $section_logic = !empty($section)
                ? $section->conditionalLogic
                : null;

            $field_logic =
                $field->type != 'page' ? $field->conditionalLogic : null; //page break conditional logic will be handled during the next button click

            $next_button_logic =
                !empty($field->nextButton) &&
                !empty($field->nextButton['conditionalLogic'])
            ...
            if (!empty($field_logic) || !empty($next_button_logic)) {
                $field_section_logic = [
                    'field' => $field_logic,
                    'nextButton' => $next_button_logic,
                    'section' => $section_logic,
                ];

                $logics .=
                    $field->id .
                    ': ' .
                    GFCommon::json_encode($field_section_logic) .
                    ',';
                ...
                $peers =
                    $field->type == 'section'
                        ? GFCommon::get_section_fields($form, $field->id)
                        : [$field];
                $peer_ids = [];
                ...
                $dependents .=
                    $field->id . ': ' . GFCommon::json_encode($peer_ids) . ',';
            }
            ...
            if ($field->allowsPrepopulate) {
                if ($input_type == 'checkbox' || $input_type == 'multiselect') {
                    $field_val = RGFormsModel::get_parameter_value(
                        $field->inputName,
                        $field_values,
                        $field
                    );
                    ...
                    foreach ($inputs as $input) {
                        $field_val[
                            $input['id']
                        ] = RGFormsModel::get_parameter_value(
                            rgar($input, 'name'),
                            $field_values,
                            $field
                        );
                    ...
                    $parameter_val = RGFormsModel::get_parameter_value(
                        $field->inputName,
                        $field_values,
                        $field
                    );
                    ...
                    $field_val = is_array($parameter_val)
                        ? $parameter_val
                        : explode(',', str_replace('|', ',', $parameter_val));
            ...
            if (is_array($field->choices) && $input_type != 'list') {
                ...
                foreach ($field->choices as $choice) {
                    ...
                    $is_prepopulated = is_array($field_val)
                        ? in_array($choice['value'], $field_val)
                        : $choice['value'] == $field_val;
                    $is_choice_selected =
                        rgar($choice, 'isSelected') || $is_prepopulated;
                    ...
                    if ($is_choice_selected) {
                        ...
                        if ($input_type == 'select') {
                            ...
                            $val =
                                $is_pricing_field && $field->type != 'quantity'
                                    ? $choice['value'] . '|' . $price
                                    : $choice['value'];
                            $default_values[$field->id] = $val;
                        } else {
                            if (!isset($default_values[$field->id])) {
                                $default_values[$field->id] = [];
                            }
                            // Multiselect
                            ...
                            // Checkboxes & Radio Buttons
                                ...
                                $default_values[
                                    $field->id
                                ][] = "choice_{$form['id']}_{$field->id}_{$choice_index}";
                            }
                        }
                    }
                    $choice_index++;
                ...
                switch ($input_type) {
                    case 'date':
                        ...
                        if (
                            ...
                            (!is_array($field_val) || !isset($field_val['m']))
                        ) {
                            $format = empty($field->dateFormat)
                                ? 'mdy'
                                : esc_attr($field->dateFormat);
                            $date_info = GFcommon::parse_date(
                                $field_val,
                                $format
                            );

                            // converts date to array( 'm' => 1, 'd' => '13', 'y' => '1987' )
                            $field_val = $field->get_date_array_by_format([
                                $date_info['month'],
                                $date_info['day'],
                                $date_info['year'],
                            ]);
                    ...
                    case 'time':
                        if (is_array($field_val)) {
                            $ampm_key = key(
                                array_slice($field_val, -1, 1, true)
                            );
                            $field_val[$ampm_key] = strtolower(
                                $field_val[$ampm_key]
                            );
                        }
                        break;
                    case 'address':
                        ...
                        if (
                            isset($field_val[$state_input_id]) &&
                            !$field_val[$state_input_id]
                        ) {
                        ...
                        if (
                            isset($field_val[$country_input_id]) &&
                            !$field_val[$country_input_id]
                        ) {
                            $field_val[$country_input_id] =
        ...
        if (isset($form['button']['conditionalLogic'])) {
            $logics .=
                '0: ' .
                GFCommon::json_encode([
                    'field' => $form['button']['conditionalLogic'],
                    'section' => null,
                ]) .
                ',';
            $dependents .= '0: ' . GFCommon::json_encode([0]) . ',';
            $fields_with_logic[] = 0;
        }
        ...
        $str =
            ...
            "window['gf_form_conditional_logic'][{$form['id']}] = { logic: { {$logics} }, dependents: { {$dependents} }, animation: {$animation}, defaults: " .
            json_encode($default_values) .
            ", fields: " .
            json_encode($field_dependents) .
            " }; " .
            "if(!window['gf_number_format'])" .
            "window['gf_number_format'] = '" .
            $number_format .
            "';" .
            'jQuery(document).ready(function(){' .
            "gf_apply_rules({$form['id']}, " .
            json_encode($fields_with_logic) .
            ', true);' .
     ...
     * @param bool  $is_ajax
     */
    public static function register_form_init_scripts(
        $form,
        $field_values = [],
        $is_ajax = false
    ) {
        ...
        if (self::has_conditional_logic($form)) {
            self::add_init_script(
                ...
                'number_formats',
                self::ON_PAGE_RENDER,
                self::get_number_formats_script($form)
            );
            self::add_init_script(
                $form['id'],
                'conditional_logic',
                self::ON_PAGE_RENDER,
                self::get_conditional_logic($form, $field_values)
            );
        }
        ...
        if (self::has_price_field($form)) {
            ...
            self::add_init_script(
                ...
                'pricing',
                self::ON_PAGE_RENDER,
                self::get_pricing_init_script($form)
            );
        }
        ...
        if (self::has_character_counter($form)) {
            self::add_init_script(
                ...
                'character_counter',
                self::ON_PAGE_RENDER,
                self::get_counter_init_script($form)
            );
        }
        ...
        if (self::has_input_mask($form)) {
            self::add_init_script(
                ...
                'input_mask',
                self::ON_PAGE_RENDER,
                self::get_input_mask_init_script($form)
            );
        }
        ...
        if (self::has_calculation_field($form)) {
            ...
            self::add_init_script(
                ...
                'calculation',
                self::ON_PAGE_RENDER,
                self::get_calculations_init_script($form)
            );
        }
        ...
        if (self::has_currency_format_number_field($form)) {
            self::add_init_script(
                ...
                'currency_format',
                self::ON_PAGE_RENDER,
                self::get_currency_format_init_script($form)
            );
        }
        ...
        if (self::has_currency_copy_values_option($form)) {
            self::add_init_script(
                ...
                'copy_values',
                self::ON_PAGE_RENDER,
                self::get_copy_values_init_script($form)
            );
        }
        ...
        if (self::has_placeholder($form)) {
            self::add_init_script(
                ...
                'placeholders',
                self::ON_PAGE_RENDER,
                self::get_placeholders_init_script($form)
            );
        }
         ...
         */
        gf_do_action(
            ['gform_register_init_scripts', $form['id']],
            $form,
            $field_values,
            $is_ajax
        );
    ...
    public static function get_chosen_init_script($form)
    {
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (
                $field->enableEnhancedUI &&
                in_array($input_type, ['select', 'multiselect'])
            ) {
                $chosen_fields[] = "#input_{$form['id']}_{$field->id}";
            }
        }

        return "gformInitChosenFields('" .
            implode(',', $chosen_fields) .
            "','" .
            esc_attr(
                gf_apply_filters(
                    ['gform_dropdown_no_results_text', $form['id']],
                    __('No results matched', 'gravityforms'),
                    $form['id']
                )
            ) .
            "');";
    ...
    public static function get_currency_format_init_script($form)
    {
        ...
        return "gformInitCurrencyFormatFields('" .
            implode(',', $currency_fields) .
            "');";
    }
    ...
    public static function get_counter_init_script($form)
    {
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (!empty($max_length) && !$field->is_administrative()) {
                $rte_enabled =
                    $field instanceof GF_Field_Textarea &&
                    $field->is_rich_edit_enabled();
                ...
                $field_script =
                    ...
                    "    'displayFormat' : '#input " .
                    esc_js(__('of', 'gravityforms')) .
                    ' #max ' .
                    esc_js(__('max characters', 'gravityforms')) .
                    "'" .
                    "    });" .
                    "jQuery('{$input_id}').next('.ginput_counter').attr('aria-live','polite');}";

                $script .= gf_apply_filters(
                    ['gform_counter_script', $form['id']],
                    $field_script,
                    $form['id'],
                    $input_id,
                    $max_length,
    ...
    {
        $field_script =
            "if(!window['gf_text']){window['gf_text'] = new Array();} window['gf_text']['password_blank'] = '" .
            esc_js(__('Strength indicator', 'gravityforms')) .
            "'; window['gf_text']['password_mismatch'] = '" .
            esc_js(__('Mismatch', 'gravityforms')) .
            "';window['gf_text']['password_unknown'] = '" .
            esc_js(__('Password strength unknown', 'gravityforms')) .
            "';window['gf_text']['password_bad'] = '" .
            esc_js(__('Weak', 'gravityforms')) .
            "'; window['gf_text']['password_short'] = '" .
            esc_js(__('Very weak', 'gravityforms')) .
            "'; window['gf_text']['password_good'] = '" .
            esc_js(__('Medium', 'gravityforms')) .
            "'; window['gf_text']['password_strong'] = '" .
            esc_js(__('Strong', 'gravityforms')) .
            "';";
    ...
    public static function get_input_mask_init_script($form)
    {
        ...
        foreach ($form['fields'] as $field) {
            ...
            $script =
                "jQuery('#input_{$form['id']}_{$field->id}').mask('" .
                esc_js($mask) .
                "').bind('keypress', function(e){if(e.which == 13){jQuery(this).blur();} } );";

            $script_str .= gf_apply_filters(
                ['gform_input_mask_script', $form['id']],
                $script,
                $form['id'],
                $field->id,
                $mask
    ...
    public static function get_calculations_init_script($form)
    {
        ...
        $script =
            'if( typeof window.gf_global["gfcalc"] == "undefined" ) { window.gf_global["gfcalc"] = {}; } window.gf_global["gfcalc"][' .
            $form['id'] .
            '] = new GFCalc(' .
            $form['id'] .
            ', ' .
            GFCommon::json_encode($formula_fields) .
            ');';
    ...
    public static function get_number_formats_script($form)
    {
        ...
        foreach ($form['fields'] as $field) {
            ...
            switch (GFFormsModel::get_input_type($field)) {
                case 'number':
                    $value_format = $field->numberFormat
                        ? $field->numberFormat
                        : 'decimal_dot';
                    break;
                case 'singleproduct':
            ...
            $number_formats[$field->id] = [
                'price' => $price_format,
                'value' => $value_format,
            ];
        }

        return 'gf_global["number_formats"][' .
            $form['id'] .
            '] = ' .
            json_encode($number_formats) .
            ';';
    }
        ...
        if (is_array($form['fields'])) {
            foreach ($form['fields'] as $field) {
                if (isset($field->fields) && is_array($field->fields)) {
                    return self::has_datepicker_field([
                        'fields' => $field->fields,
                    ]);
                }

                if (
                    RGFormsModel::get_input_type($field) == 'date' &&
     ...
     * @return bool
     */
    private static function has_checkbox_field(
        $form,
        ...
        if (is_array($form['fields'])) {
            foreach ($form['fields'] as $field) {
                if (
                    $field->get_input_type() == 'checkbox' &&
                    (!$select_all_enabled ||
                        ($select_all_enabled && $field->enableSelectAll))
                ) {
                    return true;
                }
            }
    ...
    private static function has_fileupload_field($form)
    {
        if (is_array($form['fields'])) {
            foreach ($form['fields'] as $field) {
                $input_type = RGFormsModel::get_input_type($field);
                if (in_array($input_type, ['fileupload', 'post_image'])) {
                    return true;
                }
            }
        }
    ...
    private static function has_recaptcha_field($form)
    {
        if (is_array($form['fields'])) {
            foreach ($form['fields'] as $field) {
                if (
                    ($field->type == 'captcha' ||
                        $field->inputType == 'captcha') &&
                    !in_array($field->captchaType, ['simple_captcha', 'math'])
                ) {
                    return true;
                }
            }
    ...
    public static function has_js_merge_tag($form)
    {
         ...
         * @param array $form The current Form Object
         */
        $has_js_merge_tags = gf_apply_filters(
            ['gform_has_js_merge_tag', $form['id']],
            false,
            $form
        );
    ...
    public static function get_conditional_logic_fields($form, $fieldId)
    {
        ...
        foreach ($form['fields'] as $field) {
                ...
                foreach ($field->conditionalLogic['rules'] as $rule) {
                    if (intval($rule['fieldId']) == $fieldId) {
                        ...
                        if ($field->type == 'section') {
                            $section_fields = GFCommon::get_section_fields(
                                $form,
                                $field->id
                            );
            ...
            if (!empty($field->nextButton['conditionalLogic'])) {
                foreach (
                    $field->nextButton['conditionalLogic']['rules']
                    as $rule
                ) {
                    if (
                        intval($rule['fieldId']) == $fieldId &&
                        !in_array($fieldId, $fields)
                    ) {
                        $fields[] = floatval($field->id);
                        break;
                    }
    ...
    public static function get_field(
        ...
        if ($field->type == 'page') {
            if ($is_entry_detail) {
                ...
                $previous_button_alt = rgempty(
                    'imageAlt',
                    $field->previousButton
                )
                    ? __('Previous Page', 'gravityforms')
                    : $field->previousButton['imageAlt'];
                $previous_button =
                    $field->pageNumber == 2
                        ? ''
                        : self::get_form_button(
                            $form_id,
                            "gform_previous_button_{$form_id}_{$field->id}",
                            $field->previousButton,
                            __('Previous', 'gravityforms'),
                            'gform_previous_button',
                            $previous_button_alt,
                ...
                $next_button_alt = rgempty('imageAlt', $field->nextButton)
                    ? __('Next Page', 'gravityforms')
                    : $field->nextButton['imageAlt'];
                $next_button = self::get_form_button(
                    $form_id,
                    "gform_next_button_{$form_id}_{$field->id}",
                    $field->nextButton,
                ...
                $next_button = gf_apply_filters(
                    ['gform_next_button', $form_id],
                    $next_button,
                    $form
                );
                ...
                $save_button = rgars($form, 'save/enabled')
                    ? self::get_form_button(
                        $form_id,
                        "gform_save_{$form_id}_{$field->pageNumber}",
                        $form['save']['button'],
                        rgars($form, 'save/button/text'),
                        'gform_save_link',
                        rgars($form, 'save/button/text'),
                ...
                $style = self::is_page_active($form_id, $field->pageNumber)
                    ? ''
                    : "style='display:none;'";
                ...
                <div id='gform_page_{$form['id']}_{$field->pageNumber}' class='gform_page{$custom_class}' {$style}>
                    <div class='gform_page_fields'>
                        <ul id='gform_fields_{$form_id}_{$field->pageNumber}' class='" .
                    GFCommon::get_ul_classes($form) .
                    "'>";
        ...
        $admin_only_class =
            $field->visibility == 'administrative' ? 'field_admin_only' : ''; // maintain for backwards compat
        $visibility_class = sprintf(
            'gfield_visibility_%s',
            $field->visibility ? $field->visibility : 'visible'
        );
        $selectable_class = $is_admin ? 'selectable' : '';
        $hidden_class = in_array($input_type, ['hidden', 'hiddenproduct'])
            ? 'gform_hidden'
            : '';
        ...
        $html_formatted_class =
            $field->type == 'html' && !$is_admin && !$field->disableMargins
                ? 'gfield_html_formatted'
                : '';
        $html_no_follows_desc_class =
            $field->type == 'html' &&
            !$is_admin &&
            !self::prev_field_has_description($form, $field->id)
                ? 'gfield_no_follows_desc'
                : '';

        $calculation_class =
            $input_type == 'calculation' ||
            ($input_type == 'number' && $field->has_calculation())
                ? 'gfield_calculation'
                : '';
        ...
        $option_class =
            $field->type == 'option'
                ? "gfield_price gfield_price{$product_suffix} gfield_option{$product_suffix}"
                : '';
        $quantity_class =
            $field->type == 'quantity'
                ? "gfield_price gfield_price{$product_suffix} gfield_quantity gfield_quantity{$product_suffix}"
                : '';
        $total_class =
            $field->type == 'total'
                ? "gfield_price gfield_price{$product_suffix} gfield_total gfield_total{$product_suffix}"
                : '';
        $shipping_class =
            $field->type == 'shipping'
                ? "gfield_price gfield_shipping gfield_shipping_{$form_id}"
                : '';
        $product_class =
            $field->type == 'product'
                ? "gfield_price gfield_price_{$form_id}_{$field->id} gfield_product_{$form_id}_{$field->id}"
                : '';
        $hidden_product_class =
            $input_type == 'hiddenproduct' ? 'gfield_hidden_product' : '';
        $donation_class =
            $field->type == 'donation'
                ? "gfield_price gfield_price_{$form_id}_{$field->id} gfield_donation_{$form_id}_{$field->id}"
                : '';
        $required_class = $field->isRequired ? 'gfield_contains_required' : '';
        $creditcard_warning_class =
            $input_type == 'creditcard' && !GFCommon::is_ssl()
                ? 'gfield_creditcard_warning'
                : '';

        $form_sublabel_setting = rgempty('subLabelPlacement', $form)
            ? 'below'
            : $form['subLabelPlacement'];
        $sublabel_setting =
            !isset($field->subLabelPlacement) ||
            empty($field->subLabelPlacement)
                ? $form_sublabel_setting
                : $field->subLabelPlacement;
        $sublabel_class = "field_sublabel_{$sublabel_setting}";

        $form_description_setting = rgempty('descriptionPlacement', $form)
            ? 'below'
            : $form['descriptionPlacement'];
        $description_setting =
            !isset($field->descriptionPlacement) ||
            empty($field->descriptionPlacement)
                ? $form_description_setting
        ...
        $label_placement = empty($field_setting_label_placement)
            ? ''
            : $field_setting_label_placement;
        ...
        $css_class = gf_apply_filters(
            ['gform_field_css_class', $form_id],
            trim($css_class),
            $field,
            $form
        );
        ...
        $field_content = self::get_field_content(
            $field,
            $value,
            $force_frontend_label,
            $form_id,
            $form
        );
        ...
        $field_container = gf_apply_filters(
            ['gform_field_container', $form_id, $field->id],
            $field_container,
            $field,
            $form,
            $css_class,
     ...
     * @return string
     */
    public static function get_field_content(
        $field,
        $value = '',
        $force_frontend_label = false,
        $form_id = 0,
        ...
        if ($input_type == 'adminonly_hidden') {
            $field_content = !$is_admin
                ? '{FIELD}'
                : sprintf(
                    "%s<label class='gfield_label' >%s</label>{FIELD}",
                    $admin_buttons,
                    esc_html($field_label)
                );
        } else {
            $field_content = $field->get_field_content(
                $value,
                $force_frontend_label,
                $form
            );
        ...
        if ($input_type == 'creditcard' && !GFCommon::is_ssl() && !$is_admin) {
            $field_content =
                "<div class='gfield_creditcard_warning_message'><span>" .
                esc_html__(
        ...
        $field_content = str_replace(
            '{FIELD}',
            GFCommon::get_field_input($field, $value, 0, $form_id, $form),
            $field_content
        );

        $field_content = gf_apply_filters(
            ['gform_field_content', $form_id, $field->id],
            $field_content,
            $field,
            $value,
            0,
    ...
    public static function get_progress_bar(
        $form,
        $page,
        $confirmation_message = ''
    ) {
        ...
        $page_name = rgars(
            $form['pagination'],
            sprintf('pages/%d', $current_page - 1)
        );
        $page_name = !empty($page_name) ? ' - ' . $page_name : '';
        $style = $form['pagination']['style'];
        $color =
            $style == 'custom' ? " color:{$form['pagination']['color']};" : '';
        $bgcolor =
            $style == 'custom'
                ? " background-color:{$form['pagination']['backgroundColor']};"
                : '';
        ...
        $progressbar_page_count = $start_at_zero
            ? $current_page - 1
            : $current_page;
        $percent = !$progress_complete
            ? floor(($progressbar_page_count / $page_count) * 100) . '%'
            : '100%';
        $percent_number = !$progress_complete
            ? floor(($progressbar_page_count / $page_count) * 100) . ''
            : '100';
        ...
        $progress_bar .= !$progress_complete
            ? esc_html__('Step', 'gravityforms') .
                " {$current_page} " .
                esc_html__('of', 'gravityforms') .
    ...
    public static function get_progress_steps($form, $page)
    {
        ...
        $pages = isset($form['pagination']['pages'])
        ...
        for ($i = 0, $count = sizeof($pages); $i < $count; $i++) {
            ...
            $previous_class =
                $step_number + 1 == $page ? ' gf_step_previous' : '';
    ...
    public static function validate_entry_limit($form)
    {
        ...
        if (rgar($form, 'limitEntries')) {
            ...
            if ($entry_count >= $form['limitEntriesCount']) {
                return empty($form['limitEntriesMessage'])
                    ? "<div class='gf_submission_limit_message'><p>" .
                            esc_html__(
        ...
        if (rgar($form, 'scheduleForm')) {
            ...
            if (!empty($form['scheduleStart']) && $now < $timestamp_start) {
                return empty($form['schedulePendingMessage'])
                    ...
                    : '<p>' .
                            GFCommon::gform_do_shortcode(
                                $form['schedulePendingMessage']
                            ) .
                            '</p>';
            } elseif (!empty($form['scheduleEnd']) && $now > $timestamp_end) {
                return empty($form['scheduleMessage'])
                    ? '<p>' .
                            esc_html__(
     ...
     * @return array
     */
    public static function update_confirmation(
        $form,
        $entry = null,
        $event = ''
    ) {
        if (
            (is_array($entry) &&
                (empty($entry) || rgar($entry, 'status') === 'spam')) ||
            empty($form['confirmations']) ||
            !is_array($form['confirmations'])
        ) {
        ...
        if (!empty($event)) {
            $confirmations = wp_filter_object_list($form['confirmations'], [
                'event' => $event,
            ]);
        } else {
        ...
        $filtered_list = wp_filter_object_list($form['confirmations'], [
            'isDefault' => true,
        ]);
    ...
    public static function process_send_resume_link()
    {
        ...
        if (rgar($form, 'requireLogin')) {
            ...
            check_admin_referer(
                'gform_send_resume_link',
                '_gform_send_resume_link_nonce'
            );
        }

        $draft_submission = GFFormsModel::get_draft_submission_values(
            $resume_token
        );
        ...
        $log_notification_event = empty($notifications_to_send)
            ...
            : 'Processing notifications';
        GFCommon::log_debug(
            "GFFormDisplay::process_send_resume_link(): {$log_notification_event} for form_save_email_requested event."
        );
        ...
        foreach ($notifications_to_send as $notification) {
            if (
                ...
                GFCommon::log_debug(
                    "GFFormDisplay::process_send_resume_link(): Notification is inactive, not processing notification (#{$notification['id']} - {$notification['name']})."
                );
                continue;
            }
    ...
    public static function replace_save_variables(
        $text,
        $form,
        $resume_token,
        $email = null
         ...
         */
        $resume_url = apply_filters(
            'gform_save_and_continue_resume_url',
            add_query_arg(
                ['gf_token' => $resume_token],
        ...
        $resume_email_validation_message = esc_html__(
        ...
        $validation_message =
            !is_null($email) && !GFCommon::is_valid_email($email)
                ? sprintf(
                    '<div class="validation_message">%s</div>',
                    $resume_email_validation_message
                )
                : '';
        ...
        if ($ajax) {
            $ajax_fields =
                "<input type='hidden' name='gform_ajax' value='" .
                esc_attr(
                    "form_id={$form_id}&amp;title=1&amp;description=1&amp;tabindex=1"
                ) .
                "' />";
            $ajax_fields .=
                "<input type='hidden' name='gform_field_values' value='' />";
        ...
        $ajax_submit = $ajax
            ? "onclick='jQuery(\"#gform_{$form_id}\").trigger(\"submit\",[true]);'"
            : '';
    ...
    public static function handle_save_email_confirmation($form, $ajax)
    {
        ...
        if (!GFCommon::is_valid_email($resume_email)) {
            GFCommon::log_debug(
                'GFFormDisplay::handle_save_email_confirmation(): Invalid email address: ' .
                    $resume_email
            );
        ...
        $submission_details = GFFormsModel::get_draft_submission_values(
            $resume_token
        );
        ...
        $confirmation =
            '<div class="form_saved_message_sent"><span>' .
            $confirmation_message .
            '</span></div>';
        $nl2br = rgar($form['confirmation'], 'disableAutoformat')
            ? false
            : true;
        $save_email_confirmation = self::replace_save_variables(
            $confirmation,
            $form,
            $resume_token,
            $resume_email
        );
        $save_email_confirmation = GFCommon::replace_variables(
        ...
        $save_email_confirmation = GFCommon::gform_do_shortcode(
            $save_email_confirmation
        );
        $save_email_confirmation = self::maybe_sanitize_confirmation_message(
            $save_email_confirmation
        );
        ...
        if ($ajax) {
            $save_email_confirmation = self::get_ajax_postback_html(
                $save_email_confirmation
            );
        }

        GFCommon::log_debug(
            'GFFormDisplay::handle_save_email_confirmation(): Confirmation => ' .
                print_r($save_email_confirmation, true)
        );
    ...
    public static function handle_save_confirmation(
        ...
        $resume_email = isset($_POST['gform_resume_email'])
            ? $_POST['gform_resume_email']
            : null;

        $confirmation_message = self::maybe_sanitize_confirmation_message(
            $confirmation_message
        );
        $confirmation_message = self::replace_save_variables(
            $confirmation_message,
            $form,
            $resume_token,
            $resume_email
        );
        $confirmation_message = GFCommon::gform_do_shortcode(
            $confirmation_message
        );
        $confirmation_message =
            "<div class='form_saved_message'><span>" .
            $confirmation_message .
            '</span></div>';
        ...
        $confirmation_message =
            "<div class='{$wrapper_css_class}' id='gform_wrapper_{$form_id}'>" .
            $confirmation_message .
            '</div>';

        if ($ajax) {
            $confirmation_message = self::get_ajax_postback_html(
                $confirmation_message
            );
        }

        GFCommon::log_debug(
            'GFFormDisplay::handle_save_confirmation(): Confirmation => ' .
                print_r($confirmation_message, true)
        );
    ...
    public static function insert_review_page($form, $review_page)
    {
        ...
        $review_page_break->cssClass =
            'gform_review_page ' . rgar($review_page, 'cssClass', '');
    ...
    public static function get_anchor($form, $ajax)
    {
         ...
         * @param array    $form   The current Form object.
         */
        $anchor = gf_apply_filters(
            ['gform_confirmation_anchor', $form_id],
            $anchor,
            $form
        );
        ...
        return [
            ...
            'tag' =>
                $anchor !== false
                    ? "<div id='gf_{$form_id}' class='gform_anchor' tabindex='-1'></div>"
                    : '',
