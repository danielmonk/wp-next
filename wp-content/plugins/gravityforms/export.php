...
class GFExport
{
        ...
        if (isset($_POST['export_forms'])) {
            ...
            if (empty($selected_forms)) {
                GFCommon::add_error_message(
                    __('Please select the forms to be exported', 'gravityforms')
                );
    ...
    public static function export_forms($form_ids)
    {
        ...
        $filename =
            apply_filters(
                ...
                'gravityforms-export-' . date('Y-m-d'),
                $form_ids
            ) . '.json';
    ...
    public static function import_file($filepath, &$forms = null)
    {
        ...
        if (
            GFCommon::safe_substr($file_contents, 0, 38) ==
    ...
    public static function import_json($forms_json, &$forms = null)
    {
        ...
        if (!$forms) {
            GFCommon::log_debug(
                __METHOD__ . '(): Import Failed. Invalid form objects.'
            );

            return 0;
        } elseif (
            !rgar($forms, 'version') ||
            version_compare($forms['version'], self::$min_import_version, '<')
        ) {
            GFCommon::log_debug(
                __METHOD__ .
                    '(): Import Failed. The JSON version is not compatible with the current Gravity Forms version.'
            );
        ...
        if (is_wp_error($form_ids)) {
            GFCommon::log_debug(
                __METHOD__ . '(): Import Failed => ' . print_r($form_ids, 1)
            );
            $form_ids = [];
        } else {
    ...
    public static function import_xml($xmlstr, &$forms = null)
    {
        ...
        $options = [
            ...
            'routin' => ['unserialize_as_array' => true], //routin is for backwards compatibility
        ...
        foreach ($forms as $key => &$form) {
            ...
            if (isset($form['confirmations'])) {
                $form['confirmations'] = self::set_property_as_key(
                    $form['confirmations'],
                    'id'
                );
                $form[
                    'confirmations'
                ] = GFFormsModel::trim_conditional_logic_values(
                    $form['confirmations'],
                    $form
                );
            ...
            if (isset($form['notifications'])) {
                $form['notifications'] = self::set_property_as_key(
                    $form['notifications'],
                    'id'
                );
                $form[
        ...
        foreach ($forms as &$form) {
            ...
            foreach ($form['fields'] as &$field) {
                ...
                if (
                    ...
                    $adjust_by = 0;
                    for (
                        $i = 1, $count = sizeof($field->choices);
                        $i <= $count;
                        $i++
                    ) {
                        if (!$field->enableChoiceValue) {
                            $field->choices[$i - 1]['value'] =
                                $field->choices[$i - 1]['text'];
                        }
    ...
    public static function import_form_page()
    {
        ...
        if (isset($_POST['import_forms'])) {
            ...
            if (!empty($_FILES['gf_import_file']['tmp_name'][0])) {
                ...
                foreach (
                    $_FILES['gf_import_file']['tmp_name']
                    as $import_file
                ) {
                ...
                if ($count == 0) {
                    GFCommon::add_error_message(
                        __(
                            'Forms could not be imported. Please make sure your export file is in the correct format.',
                            'gravityforms'
                        )
                    );
                } elseif ($count == '-1') {
                    GFCommon::add_error_message(
                        __(
                            'Forms could not be imported. Your export file is not compatible with your current version of Gravity Forms.',
                            'gravityforms'
                        )
                    );
                } else {
                    ...
                    $edit_link =
                        $count == 1
                            ? "<a href='admin.php?page=gf_edit_forms&id={$forms[0]['id']}'>" .
                                __('Edit Form', 'gravityforms') .
                                '</a>'
                            : '';
                    GFCommon::add_message(
                        sprintf(
                            __(
                                "Gravity Forms imported %d %s successfully",
                                'gravityforms'
                            ),
                            $count,
                            $form_text
		...
		<p class="textleft">
			<?php esc_html_e(
       'Select the Gravity Forms export files you would like to import. When you click the import button below, Gravity Forms will import the forms.',
       'gravityforms'
   ); ?>
		</p>
		...
		<form method="post" enctype="multipart/form-data" style="margin-top:10px;">
			...
			<table class="form-table">
				<tr valign="top">
					...
					<th scope="row">
						<label for="gf_import_file"><?php esc_html_e(
          'Select Files',
          'gravityforms'
      ); ?></label> <?php gform_tooltip('import_select_file'); ?>
					</th>
    ...
    public static function export_form_page()
    {
		...
		<p class="textleft"><?php esc_html_e(
      ...
      'gravityforms'
		...
		<form id="gform_export" method="post" style="margin-top:10px;">
			...
			<table class="form-table">
				<tr valign="top">
					<th scope="row">
						<label for="export_fields"><?php esc_html_e(
          'Select Forms',
          'gravityforms'
      ); ?></label> <?php gform_tooltip('export_select_forms'); ?>
					</th>
					<td>
						<ul id="export_form_list">
							<li>
								<input type="checkbox" id="gf_export_forms_all" />
								<label for="gf_export_forms_all" data-deselect="<?php esc_attr_e(
            'Deselect All',
            'gravityforms'
        ); ?>" data-select="<?php esc_attr_e(
    'Select All',
    'gravityforms'
    ...
    'gravityforms'
       ...
       foreach ($forms as $form) { ?>
								<li>
									<input type="checkbox" name="gf_form_id[]" id="gf_form_id_<?php echo absint(
             $form->id
         ); ?>" value="<?php echo absint($form->id); ?>" />
									<label for="gf_form_id_<?php echo absint($form->id); ?>"><?php echo esc_html(
    $form->title
); ?></label>
								</li>
							<?php }
       ...
       'gravityforms'
   ); ?>" name="export_forms" class="button button-large button-primary" />
		</form>
    ...
    public static function export_lead_page()
    {
		...
		<script type="text/javascript">
			...
			function SelectExportForm(formId) {
				...
				mysack.setVar("rg_select_export_form", "<?php echo wp_create_nonce(
        'rg_select_export_form'
    ); ?>");
				mysack.setVar("form_id", formId);
				mysack.onError = function () {
					alert(<?php echo json_encode(
         __('Ajax error while selecting a form', 'gravityforms')
     ); ?>)
				};
				mysack.runAJAX();
			...
			function EndSelectExportForm(aryFields, filterSettings) {
				...
				var fieldList = "<li><input id='select_all' type='checkbox' onclick=\"jQuery('.gform_export_field').prop('checked', this.checked); jQuery('#gform_export_check_all').html(this.checked ? '<strong><?php echo esc_js(
        __('Deselect All', 'gravityforms')
    ); ?></strong>' : '<strong><?php echo esc_js(
    __('Select All', 'gravityforms')
); ?></strong>'); \" onkeypress=\"jQuery('.gform_export_field').prop('checked', this.checked); jQuery('#gform_export_check_all').html(this.checked ? '<strong><?php echo esc_js(
    __('Deselect All', 'gravityforms')
); ?></strong>' : '<strong><?php echo esc_js(
    __('Select All', 'gravityforms')
); ?></strong>'); \"> <label id='gform_export_check_all' for='select_all'><strong><?php esc_html_e(
    'Select All',
    'gravityforms'
); ?></strong></label></li>";
				...
				gf_vars.filterAndAny = <?php echo json_encode(
        esc_html__(
            'Export entries if {0} of the following match:',
            'gravityforms'
        )
    ); ?>;
				...
				$(document).ready(function() {
					$("#submit_button").click(function () {
						if ($(".gform_export_field:checked").length == 0) {
							alert(<?php echo json_encode(
           __('Please select the fields to be exported', 'gravityforms')
       ); ?>);
							return false;
						}
					...
					$.ajax({
						...
						dataType: 'json'
							...
							if ( response.status == 'in_progress' ) {
								...
								var url = ajaxurl + '?action=gf_download_export&_wpnonce=<?php echo wp_create_nonce(
            'gform_download_export'
        ); ?>&export-id=' + response.exportId + '&form-id=' + formId;
		...
		<p class="textleft"><?php esc_html_e(
      'Select a form below to export entries. Once you have selected a form you may select the fields you would like to export and then define optional filters for field values and the date range. When you click the download button below, Gravity Forms will create a CSV file for you to save to your computer.',
      'gravityforms'
  ); ?></p>
		<div class="hr-divider"></div>
		<form id="gform_export" method="post" style="margin-top:10px;">
			...
			<table class="form-table">
				<tr valign="top">
					...
					<th scope="row">
						<label for="export_form"><?php esc_html_e(
          'Select A Form',
          'gravityforms'
      ); ?></label> <?php gform_tooltip('export_select_form'); ?>
					</th>
					<td>
       ...
       foreach ($forms as $form) { ?>
								<option value="<?php echo absint($form->id); ?>" <?php selected(
    rgget('id'),
    $form->id
); ?>><?php echo esc_html($form->title); ?></option>
								<?php }
       ?>
						</select>
          ...
          'Select Fields',
          'gravityforms'
      ); ?></label> <?php gform_tooltip('export_select_fields'); ?>
					</th>
					<td>
          ...
          'Conditional Logic',
          'gravityforms'
      ); ?></label> <?php gform_tooltip('export_conditional_logic'); ?>
					</th>
					<td>
				...
				<tr id="export_date_container" valign="top" style="display: none;">
					<th scope="row">
						<label for="export_date"><?php esc_html_e(
          'Select Date Range',
          'gravityforms'
      ); ?></label> <?php gform_tooltip('export_date_range'); ?>
					</th>
					<td>
						<div>
                            <span style="width:150px; float:left; ">
                                <input type="text" id="export_date_start" name="export_date_start" style="width:90%" />
                                <strong><label for="export_date_start" style="display:block;"><?php esc_html_e(
                                    'Start',
                                    'gravityforms'
                                ); ?></label></strong>
                            </span>
                            ...
                            <span style="width:150px; float:left;">
                                ...
                                <strong><label for="export_date_end" style="display:block;"><?php esc_html_e(
                                    'End',
                                    'gravityforms'
                                ); ?></label></strong>
                            </span>
           ...
           'gravityforms'
			...
			<ul>
				<li id="export_submit_container" style="display:none; clear:both;">
					<br /><br />
					<button id="submit_button" class="button button-large button-primary"><?php esc_attr_e(
         'Download Export File',
         'gravityforms'
     ); ?></button>
    ...
    public static function get_field_row_count(
        $form,
        $exported_field_ids,
        $entry_count
    ) {
        $list_fields = GFAPI::get_fields_by_type($form, ['list'], true);

        //only getting fields that have been exported
        $field_ids = '';
        foreach ($list_fields as $field) {
            if (
                in_array($field->id, $exported_field_ids) &&
                $field->enableColumns
            ) {
                $field_ids .= $field->id . ',';
            }
        ...
        while ($go_to_next_page) {
            ...
            foreach ($results as $result) {
                ...
                $current_row_count = isset($row_counts[$result['field_id']])
                    ? intval($row_counts[$result['field_id']])
                    : 0;
    ...
    public static function get_gmt_timestamp($local_timestamp)
    {
        _deprecated_function(
            'GFExport::get_gmt_timestamp',
            '2.0.7',
            'GFCommon::get_gmt_timestamp'
        );
    ...
    public static function start_export($form, $offset = 0, $export_id = '')
    {
         ...
         */
        $max_execution_time = apply_filters(
        ...
        $remaining_entry_count =
            $offset == 0 ? $total_entry_count : $total_entry_count - $offset;

        $lines = '';

        // Set the separator
        $separator = gf_apply_filters(
            ['gform_export_separator', $form_id],
            ',',
            $form_id
        );

        $field_rows = self::get_field_row_count(
            $form,
            $fields,
            $remaining_entry_count
        );
        ...
        if ($offset == 0) {
            GFCommon::log_debug(
                __METHOD__ . '(): Processing request for form #' . $form_id
            );
            ...
            foreach ($fields as $field_id) {
                ...
                GFCommon::log_debug(
                    "GFExport::start_export(): Header for field ID {$field_id}: {$value}"
                );
                ...
                $subrow_count = isset($field_rows[$field_id])
                    ? intval($field_rows[$field_id])
                    : 0;
                if ($subrow_count == 0) {
            ...
            GFCommon::log_debug(
                __METHOD__ . '(): sorting: ' . print_r($sorting, true)
            );
        }
        ...
        while ($remaining_entry_count > 0) {
            ...
            GFCommon::log_debug(
                __METHOD__ . '(): paging: ' . print_r($paging, true)
            );
            ...
            $leads = gf_apply_filters(
                ['gform_leads_before_export', $form_id],
                $leads,
                $form,
                $paging
            );
            ...
            foreach ($leads as $lead) {
                $line = self::get_entry_export_line(
                    $lead,
                    $form,
                    $fields,
                    $field_rows,
        ...
        $status = [
            ...
            'progress' =>
                $remaining_entry_count > 0
                    ? intval(
                            100 -
                                ($remaining_entry_count / $total_entry_count) *
                                    100
                        ) . '%'
                    : '',
        ];
     ...
     * @return string
     */
    public static function get_entry_export_line(
        $entry,
        $form,
        $fields,
        $field_rows,
        ...
        GFCommon::log_debug(
            __METHOD__ . '(): Processing entry #' . $entry['id']
        );
            ...
            switch ($field_id) {
                ...
                case 'payment_date':
                    ...
                    if ($value) {
                        ...
                        $lead_local_time = GFCommon::get_local_timestamp(
                            $lead_gmt_time
                        );
                        $value = date_i18n(
                            'Y-m-d H:i:s',
                ...
                default:
                    ...
                    $value = is_object($field)
                        ? $field->get_value_export(
                            $entry,
                            $field_id,
                            false,
                            true
    ...
    public static function add_default_export_fields($form)
    {
        ...
        array_push($form['fields'], [
            'id' => 'date_created',
            'label' => __('Entry Date', 'gravityforms'),
        ]);
        array_push($form['fields'], [
            'id' => 'source_url',
            'label' => __('Source Url', 'gravityforms'),
        ]);
        array_push($form['fields'], [
            'id' => 'transaction_id',
    ...
    public static function write_file($lines, $export_id)
    {
        ...
        $file =
            $export_folder .
            sanitize_file_name('export-' . $export_id . '.csv');

        GFCommon::log_debug(__METHOD__ . '(): Writing to file.');
        $result = file_put_contents($file, $lines, FILE_APPEND);
        if ($result === false) {
            GFCommon::log_error(
                __METHOD__ . '(): An issue occurred whilst writing to the file.'
            );
        } else {
            GFCommon::log_debug(
                __METHOD__ .
...
deny from all';
    ...
    public static function ajax_download_export()
    {
        ...
        if (!function_exists('readfile')) {
            GFCommon::log_error(
                __METHOD__ .
                    '(): Aborting. The PHP readfile function is not available.'
            );
            die(
                esc_html__(
                    'The PHP readfile function is not available, please contact the web host.',
                    'gravityforms'
                )
            );
        }
        ...
        $filename =
            ...
            '-' .
            gmdate('Y-m-d', GFCommon::get_local_timestamp(time())) .
            '.csv';

        GFCommon::log_debug(
            __METHOD__ . '(): Starting download of file: ' . $filename
        );
        ...
        if (has_filter('sanitize_file_name')) {
            GFCommon::log_debug(
                __METHOD__ .
                    '(): The WordPress sanitize_file_name filter has been detected.'
            );
        }
        ...
        if ($result === false) {
            GFCommon::log_error(
                __METHOD__ . '(): An issue occurred whilst reading the file.'
            );
        } else {
            @unlink($file);
            GFCommon::log_debug(
    ...
    public static function prepare_forms_for_export($forms)
    {
        ...
        foreach ($forms as &$form) {
            foreach ($form['fields'] as &$field) {
                ...
                if (isset($form['confirmations'])) {
                    $form['confirmations'] = array_values(
                        $form['confirmations']
                    );
                }

                if (isset($form['notifications'])) {
                    $form['notifications'] = array_values(
                        $form['notifications']
                    );
                }
            }
