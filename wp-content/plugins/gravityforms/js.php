...
if (!class_exists('GFForms')) {
	...
	function DeleteCustomChoice() {
      ...
      __(
          "Delete this custom choice list? 'OK' to delete, 'Cancel' to abort.",
          'gravityforms'
      )
  ); ?>))
			return;
		...
		jQuery.post( ajaxurl, {action: "gf_delete_custom_choice", name: gform_selected_custom_choice, gf_delete_custom_choice: "<?php echo wp_create_nonce(
      'gf_delete_custom_choice'
		...
		InitBulkCustomPanel();
		LoadCustomChoices();
		DisplayCustomMessage(<?php echo json_encode(
      esc_html__('Item has been deleted.', 'gravityforms')
  ); ?>);
	}
	...
	function SaveCustomChoices() {
       ...
       esc_html__('Please enter name.', 'gravityforms')
   ); ?>);
			return;
		}
			...
			alert(<?php echo json_encode(
       esc_html__(
           'This custom choice name is already in use. Please enter another name.',
           'gravityforms'
       )
   ); ?>);
			return;
		...
		//Sending AJAX request
		jQuery.post(ajaxurl, {action: "gf_save_custom_choice", previous_name: gform_selected_custom_choice, new_name: name, choices: jQuery.toJSON(choices), gf_save_custom_choice: "<?php echo wp_create_nonce(
      'gf_save_custom_choice'
		...
		DisplayCustomMessage(<?php echo json_encode(
      esc_html__('Item has been saved.', 'gravityforms')
  ); ?>);
	}
	...
	function InitializeFormConditionalLogic() {
		...
		if (canHaveConditionalLogic) {
			...
			jQuery("#form_button_conditional_logic_container").show().html("<span class='instruction' style='margin-left:0'>" + <?php echo json_encode(
       esc_html__(
           'To use conditional logic, please create a field that supports conditional logic.',
           'gravityforms'
       )
   ); ?> + "</span>");
		}
	...
	function InitPaginationOptions(isInit) {
		...
		for (var i = 0; i < pages.length; i++) {
			...
			str += "<li><label class='inline' for='gform_pagename_" + i + "' >" + <?php echo json_encode(
       esc_html__('Page', 'gravityforms')
   ); ?> + " " + (i + 1) + "</label> <input type='text' class='fieldwidth-4' id='gform_pagename_" + i + "' value='" + pageName + "' /></li>";
		}
		str += "</ul>";
		...
		if (jQuery("#pagination_type_none").is(":checked")) {
			...
			var completion_text = form["pagination"] && form["pagination"]["display_progressbar_on_confirmation"] ? form["pagination"]["progressbar_completion_text"] : <?php echo json_encode(
       esc_html__('Completed', 'gravityforms')
   ); ?>;
	...
	function TogglePostCategoryInitialItem(isInit) {
		...
		if (jQuery("#gfield_post_category_initial_item_enabled").is(":checked")) {
			...
			if (!isInit) {
				jQuery("#field_post_category_initial_item").val(<?php echo json_encode(
        esc_html__('Select a category', 'gravityforms')
    ); ?>);
			}
		}
		else {
	...
	function CreateInputNames(field) {
		...
		if (!inputs || GetInputType(field) == "checkbox") {
			field_str = "<label for='field_input_name' class='inline'>" + <?php echo json_encode(
       esc_html__('Parameter Name:', 'gravityforms')
   ); ?> + "&nbsp;</label>";
			field_str += "<input type='text' value='" + field["inputName"] + "' id='field_input_name' />";
		}
		else {
			var priceId = field['id'] + 0.2;
			field_str = "<table><tr><td><strong>Field</strong></td><td><strong>" + <?php echo json_encode(
       esc_html__('Parameter Name', 'gravityforms')
   ); ?> + "</strong></td></tr>";
	...
	function CreateDefaultValuesUI(field) {
		...
		if (!field['inputs']) {
			field_str = "<label for='field_single_default_value' class='inline'>" + <?php echo json_encode(
       esc_html__('Default Value:', 'gravityforms')
   ); ?> + "&nbsp;</label>";
			...
			field_str = "<table class='default_input_values'><tr><td><strong>Field</strong></td><td><strong>" + <?php echo json_encode(
       esc_html__('Default Value', 'gravityforms')
   ); ?> + "</strong></td></tr>";
			for (var i = 0; i < field["inputs"].length; i++) {
				id = field["inputs"][i]["id"];
	...
	function CreatePlaceholdersUI(field) {
		...
		if (!field["inputs"]) {
			field_str = "<label for='field_single_placeholder' class='inline'>" + <?php echo json_encode(
       esc_html__('Placeholder:', 'gravityforms')
   ); ?> + "&nbsp;</label>";
			...
			field_str = "<table class='input_placeholders'><tr><td><strong>Field</strong></td><td><strong>" + <?php echo json_encode(
       esc_html__('Placeholder', 'gravityforms')
   ); ?> + "</strong></td></tr>";
			for (var i = 0; i < field["inputs"].length; i++) {
				id = field["inputs"][i]["id"];
	...
	function GetCustomizeInputsUI(field, showInputSwitches) {
		...
		if (!field['inputs']) {
			html = "<label for='field_single_input' class='inline'>" + <?php echo json_encode(
       esc_html__('Sub-Label:', 'gravityforms')
   ); ?> + "&nbsp;</label>";
			...
			if (showInputSwitches) {
				html += "<td><strong>" + <?php echo json_encode(
        esc_html__('Show', 'gravityforms')
    ); ?>+ "</strong></td>";
			}
			html += "<td><strong><?php esc_html_e(
       'Field',
       'gravityforms'
   ); ?></strong></td><td><strong>" + <?php echo json_encode(
    esc_html__('Custom Sub-Label', 'gravityforms')
); ?> + "</strong></td></tr>";
			for (var i = 0; i < field["inputs"].length; i++) {
				...
				title = isHidden ? <?php echo json_encode(
        esc_html__('Inactive', 'gravityforms')
    ); ?> : <?php echo json_encode(esc_html__('Active', 'gravityforms')); ?>;
	...
	function SetCopyValuesOptionProperties(isEnabled) {
		var defaultLabel = <?php echo json_encode(
      esc_html__('Same as previous', 'gravityforms')
  ); ?>;
	...
	function SetProductField(field) {
		...
		if (productFields.length < 1) {
			jQuery("#product_field").hide().after("<div id='gform_no_product_field_message'>" + <?php echo json_encode(
       esc_html__(
           'This field is not associated with a product. Please add a Product Field to the form.',
           'gravityforms'
       )
   ); ?> + "</div>");
		}
			...
			if (!is_selected && field["productField"] != "") {
				product_field.append("<option value='" + field["productField"] + "' selected='selected'>[" + <?php echo json_encode(
        esc_html__('Deleted Field', 'gravityforms')
    ); ?> + "]</option>");
			}
	...
	function LoadFieldConditionalLogic(isEnabled, objectType) {
		...
		if (isEnabled) {
			...
			jQuery("#" + objectType + "_conditional_logic_container").show().html("<span class='instruction' style='margin-left:0'>" + <?php echo json_encode(
       esc_html__(
           'To use conditional logic, please create a field that supports conditional logic.',
           'gravityforms'
       )
	...
	function GetCurrentCurrency() {
		<?php $current_currency = RGCurrency::get_currency(GFCommon::get_currency()); ?>
		var currency = new Currency(<?php echo GFCommon::json_encode(
      $current_currency
  ); ?>);
		return currency;
	}
	...
	function ToggleColumns(isInit) {
		...
		if (jQuery('#field_columns_enabled').is(":checked")) {
			...
			if (!field.choices)
				field.choices = new Array(new Choice(<?php echo json_encode(
        esc_html__('Column 1', 'gravityforms')
    ); ?>), new Choice(<?php echo json_encode(
    esc_html__('Column 2', 'gravityforms')
); ?>), new Choice(<?php echo json_encode(
    esc_html__('Column 3', 'gravityforms')
); ?>));
	...
	function ValidateForm() {
		...
		if (jQuery.trim(form.title).length == 0) {
			error = <?php echo json_encode(
       esc_html__(
           'Please enter a Title for this form. When adding the form to a page or post, you will have the option to hide the title.',
           'gravityforms'
       )
   ); ?>;
		}
				...
				switch (field["type"]) {
					case "page" :
						if (i == last_page_break + 1 || i == form["fields"].length - 1)
							error = <?php echo json_encode(
           esc_html__(
               'Your form currently has one or more pages without any fields in it. Blank pages are a result of Page Breaks that are positioned as the first or last field in the form or right after each other. Please adjust your Page Breaks and try again.',
               'gravityforms'
           )
       ); ?>;
	...
	function SetDefaultValues( field, index ) {
		...
		switch (inputType) {
			...
			case "post_category" :
				field.label = <?php echo json_encode(
        esc_html__('Post Category', 'gravityforms')
    ); ?>;
				field.inputs = null;
			...
			case "section" :
				field.label = <?php echo json_encode(
        esc_html__('Section Break', 'gravityforms')
    ); ?>;
				field.inputs = null;
			...
			case "page" :
				...
				field["nextButton"]["text"] = <?php echo json_encode(
        esc_html__('Next', 'gravityforms')
    ); ?>;
				field["previousButton"] = new Button();
				field["previousButton"]["text"] = <?php echo json_encode(
        esc_html__('Previous', 'gravityforms')
    ); ?>;
				break;

			case "html" :
				field.label = <?php echo json_encode(
        esc_html__('HTML Block', 'gravityforms')
    ); ?>;
				field.inputs = null;
				field["displayOnly"] = true;
				break;
			...
			case "checkbox" :
				if (!field.label)
					field.label = <?php echo json_encode(
         esc_html__('Untitled', 'gravityforms')
     ); ?>;

				if (!field.choices)
					field.choices = new Array(new Choice(<?php echo json_encode(
         esc_html__('First Choice', 'gravityforms')
     ); ?>), new Choice(<?php echo json_encode(
    esc_html__('Second Choice', 'gravityforms')
); ?>), new Choice(<?php echo json_encode(
    esc_html__('Third Choice', 'gravityforms')
); ?>));
			...
			case "radio" :
				...
				field.inputs = null;
				if (!field.choices) {
					field.choices = field["enablePrice"] ? new Array(new Choice(<?php echo json_encode(
         esc_html__('First Choice', 'gravityforms')
     ); ?>, "", "0.00"), new Choice(<?php echo json_encode(
    esc_html__('Second Choice', 'gravityforms')
); ?>, "", "0.00"), new Choice(<?php echo json_encode(
    esc_html__('Third Choice', 'gravityforms')
); ?>, "", "0.00"))
						: new Array(new Choice(<?php echo json_encode(
          esc_html__('First Choice', 'gravityforms')
      ); ?>), new Choice(<?php echo json_encode(
    esc_html__('Second Choice', 'gravityforms')
); ?>), new Choice(<?php echo json_encode(
    ...
    esc_html__('Third Choice', 'gravityforms')
			...
			case "address" :
				...
				field.addressType = <?php echo json_encode(
        GF_Fields::get('address')->get_default_address_type(rgget('id'))
    ); ?>;
				field.inputs = [new Input(field.id + 0.1, <?php echo json_encode(
        gf_apply_filters(
            ['gform_address_street', rgget('id')],
            esc_html__('Street Address', 'gravityforms'),
            rgget('id')
        )
    ); ?>), new Input(field.id + 0.2, <?php echo json_encode(
    gf_apply_filters(
        ['gform_address_street2', rgget('id')],
        esc_html__('Address Line 2', 'gravityforms'),
        rgget('id')
    )
); ?>), new Input(field.id + 0.3, <?php echo json_encode(
    gf_apply_filters(
        ['gform_address_city', rgget('id')],
        esc_html__('City', 'gravityforms'),
        rgget('id')
    )
); ?>),
					new Input(field.id + 0.4, <?php echo json_encode(
         gf_apply_filters(
             ['gform_address_state', rgget('id')],
             __('State / Province', 'gravityforms'),
             rgget('id')
         )
     ); ?>), new Input(field.id + 0.5, <?php echo json_encode(
    gf_apply_filters(
        ['gform_address_zip', rgget('id')],
        esc_html__('ZIP / Postal Code', 'gravityforms'),
        rgget('id')
    )
); ?>), new Input(field.id + 0.6, <?php echo json_encode(
    gf_apply_filters(
        ['gform_address_country', rgget('id')],
        esc_html__('Country', 'gravityforms'),
        rgget('id')
    )
); ?>)];
				break;
			case "creditcard" :
				...
				if (!field.label)
					field.label = <?php echo json_encode(
         esc_html__('Credit Card', 'gravityforms')
     ); ?>;
				var ccNumber, ccExpirationMonth, ccExpirationYear, ccSecruityCode, ccCardType, ccName;

				ccNumber = new Input(field.id + ".1", <?php echo json_encode(
        gf_apply_filters(
            ['gform_card_number', rgget('id')],
            esc_html__('Card Number', 'gravityforms'),
            rgget('id')
        )
    ); ?>);
				ccExpirationMonth = new Input(field.id + ".2_month", <?php echo json_encode(
        gf_apply_filters(
            ['gform_card_expiration', rgget('id')],
            esc_html__('Expiration Month', 'gravityforms'),
            rgget('id')
        )
    ); ?>);
				ccExpirationMonth.defaultLabel = <?php echo json_encode(
        esc_html__('Expiration Date', 'gravityforms')
    ); ?>;
				ccExpirationYear = new Input(field.id + ".2_year", <?php echo json_encode(
        gf_apply_filters(
            ['gform_card_expiration', rgget('id')],
            esc_html__('Expiration Year', 'gravityforms'),
            rgget('id')
        )
    ); ?>);
				ccSecruityCode = new Input(field.id + ".3", <?php echo json_encode(
        gf_apply_filters(
            ['gform_card_security_code', rgget('id')],
            esc_html__('Security Code', 'gravityforms'),
            rgget('id')
        )
    ); ?>);
				ccCardType = new Input(field.id + ".4", <?php echo json_encode(
        gf_apply_filters(
            ['gform_card_type', rgget('id')],
            __('Card Type', 'gravityforms'),
            rgget('id')
        )
    ); ?>);
				ccName = new Input(field.id + ".5", <?php echo json_encode(
        gf_apply_filters(
            ['gform_card_name', rgget('id')],
            esc_html__('Cardholder Name', 'gravityforms'),
            rgget('id')
        )
    ); ?>);
			...
			case "password" :
				...
				if (!field.label)
					field.label = <?php echo json_encode(
         esc_html__('Password', 'gravityforms')
     ); ?>;
				break;
			case "fileupload" :
				field.inputs = null;
			...
			case "hidden" :
				...
				if (!field.label)
					field.label = <?php echo json_encode(
         esc_html__('Hidden Field', 'gravityforms')
     ); ?>;
				break;
			case "post_title" :
				field.inputs = null;
				field.label = <?php echo json_encode(
        esc_html__('Post Title', 'gravityforms')
    ); ?>;
				break;
			case "post_content" :
				field.inputs = null;
				field.label = <?php echo json_encode(
        esc_html__('Post Body', 'gravityforms')
    ); ?>;
				break;
			case "post_excerpt" :
				field.inputs = null;
				field.label = <?php echo json_encode(
        esc_html__('Post Excerpt', 'gravityforms')
    ); ?>;
				field.size = "small";
				break;
			case "post_tags" :
				field.inputs = null;
				field.label = <?php echo json_encode(
        esc_html__('Post Tags', 'gravityforms')
    ); ?>;
				field.size = "large";
				break;
			case "post_custom_field" :
				...
				field.label = <?php echo json_encode(
        esc_html__('Post Custom Field', 'gravityforms')
    ); ?>;
				break;
			case "post_image" :
				field.label = <?php echo json_encode(
        esc_html__('Post Image', 'gravityforms')
    ); ?>;
				field.inputs = null;
			...
			case "hiddenproduct" :
				field.label = <?php echo json_encode(
        esc_html__('Product Name', 'gravityforms')
    ); ?>;
				field.inputs = null;
				...
				if (field.inputType == "singleproduct" || field.inputType == "hiddenproduct" || field.inputType == "calculation") {
					...
					field.inputs = [new Input(field_id + 0.1, <?php echo json_encode(
         esc_html__('Name', 'gravityforms')
     ); ?>), new Input(field_id + 0.2, <?php echo json_encode(
    esc_html__('Price', 'gravityforms')
); ?>), new Input(field_id + 0.3, <?php echo json_encode(
    esc_html__('Quantity', 'gravityforms')
			...
			case "shipping" :
				field.label = <?php echo json_encode(
        esc_html__('Shipping', 'gravityforms')
    ); ?>;
				field.inputs = null;
			...
			case "option" :
				...
				if (!field.choices) {
					field.choices = new Array(new Choice(<?php echo json_encode(
         esc_html__('First Option', 'gravityforms')
     ); ?>, "", "0.00"), new Choice(<?php echo json_encode(
    esc_html__('Second Option', 'gravityforms')
); ?>, "", "0.00"), new Choice(<?php echo json_encode(
    esc_html__('Third Option', 'gravityforms')
); ?>, "", "0.00"));
				}
			...
			case "donation" :
				...
				field.label = <?php echo json_encode(
        esc_html__('Donation', 'gravityforms')
    ); ?>;
        ...
        esc_html__('Quantity', 'gravityforms')
			...
			case 'consent':
				...
				field.inputs = [new Input(field.id + ".1", <?php echo json_encode(
        esc_html__('Consent', 'gravityforms')
    ); ?>), new Input(field.id + ".2", <?php echo json_encode(
    esc_html__('Text', 'gravityforms')
); ?>), new Input(field.id + ".3", <?php echo json_encode(
    esc_html__('Description', 'gravityforms')
); ?>)];
				// Hide the description from select columns.
				field.inputs[1].isHidden = true;
				field.inputs[2].isHidden = true;
				field.checkboxLabel = <?php echo json_encode(
        esc_html__('I agree to the privacy policy.', 'gravityforms')
    ); ?>;
				field.descriptionPlaceholder = <?php echo json_encode(
        esc_html__(
            'Enter consent agreement text here.  The Consent Field will store this agreement text with the form entry in order to track what the user has consented to.',
            'gravityforms'
        )
    ); ?>;
				if (!field.inputType)
				...
				if (!field.choices)
					field.choices = new Array(new Choice(<?php echo json_encode(
         esc_html__('Checked', 'gravityforms')
     ); ?>, '1'));
				break;
	...
	function GetAdvancedNameFieldInputs(field, prefixHidden, middleHidden, suffixHidden) {
		var prefixInput = new Input(field.id + '.2', <?php echo json_encode(
      gf_apply_filters(
          ['gform_name_prefix', rgget('id')],
          esc_html__('Prefix', 'gravityforms'),
          rgget('id')
      )
  ); ?>);
		...
		var firstInput = new Input(field.id + '.3', <?php echo json_encode(
      gf_apply_filters(
          ['gform_name_first', rgget('id')],
          esc_html__('First', 'gravityforms'),
          rgget('id')
      )
  ); ?>);
		 ...
		 */
		var middleInput = new Input(field.id + '.4', <?php echo json_encode(
      gf_apply_filters(
          ['gform_name_middle', rgget('id')],
          esc_html__('Middle', 'gravityforms'),
          rgget('id')
      )
  ); ?>);
		middleInput.isHidden = middleHidden;

		var lastInput = new Input(field.id + '.6', <?php echo json_encode(
      gf_apply_filters(
          ['gform_name_last', rgget('id')],
          esc_html__('Last', 'gravityforms'),
          rgget('id')
      )
  ); ?>);
		var suffixInput = new Input(field.id + '.8', <?php echo json_encode(
      gf_apply_filters(
          ['gform_name_suffix', rgget('id')],
          esc_html__('Suffix', 'gravityforms'),
          rgget('id')
      )
  ); ?>);
	...
	function GetDateFieldInputs(field) {
		...
		switch (field.dateType) {
			case 'datefield' :
				month = new Input(field.id + '.1', <?php echo json_encode(
        _x('MM', 'Abbreviation: Month', 'gravityforms')
    ); ?>);
				day = new Input(field.id + '.2', <?php echo json_encode(
        esc_html__('DD', 'gravityforms')
    ); ?>);
				year = new Input(field.id + '.3', <?php echo json_encode(
        esc_html__('YYYY', 'gravityforms')
    ); ?>);
				break;
			case 'datedropdown' :
				month = new Input(field.id + '.1', <?php echo json_encode(
        esc_html__('Month', 'gravityforms')
    ); ?>);
				month.placeholder = <?php echo json_encode(
        esc_html__('Month', 'gravityforms')
    ); ?>;
				day = new Input(field.id + '.2', <?php echo json_encode(
        esc_html__('Day', 'gravityforms')
    ); ?>);
				day.placeholder = <?php echo json_encode(esc_html__('Day', 'gravityforms')); ?>;
				year = new Input(field.id + '.3', <?php echo json_encode(
        esc_html__('Year', 'gravityforms')
    ); ?>);
				year.placeholder = <?php echo json_encode(
        esc_html__('Year', 'gravityforms')
    ); ?>;
				break;
			default:
	...
	function GetTimeFieldInputs(field) {
		...
		hour = new Input(field.id + '.1', <?php echo json_encode(
      esc_html__('HH', 'gravityforms')
  ); ?>);
		min = new Input(field.id + '.2', <?php echo json_encode(
      _x('MM', 'Abbreviation: Minutes', 'gravityforms')
  ); ?>);
		ampm = new Input(field.id + '.3', <?php echo json_encode(
      esc_html__('AM/PM', 'gravityforms')
  ); ?>);
	...
	function GetEmailFieldInputs(field) {
		...
		email = new Input(field.id, <?php echo json_encode(
      esc_html__('Enter Email', 'gravityforms')
  ); ?>);
		confirmation = new Input(field.id + '.2', <?php echo json_encode(
      esc_html__('Confirm Email', 'gravityforms')
  ); ?>);
	...
	function GetPasswordFieldInputs(field) {
		...
		password = new Input(field.id, <?php echo json_encode(
      esc_html__('Enter Password', 'gravityforms')
  ); ?>);
		confirmation = new Input(field.id + '.2', <?php echo json_encode(
      esc_html__('Confirm Password', 'gravityforms')
  ); ?>);
	...
	function UpgradeCreditCardField(field) {
		...
		if (legacyExpirationInput) {
			var monthInput = new Input(field.id + ".2_month", <?php echo json_encode(
       gf_apply_filters(
           ['gform_card_expiration', rgget('id')],
           esc_html__('Expiration Month', 'gravityforms'),
           rgget('id')
			...
			monthInput.defaultLabel = <?php echo json_encode(
       esc_html__('Expiration Date', 'gravityforms')
   ); ?>;
			var yearInput = new Input(field.id + ".2_year", <?php echo json_encode(
       esc_html__('Expiration Year', 'gravityforms')
   ); ?>);
			field.inputs.splice(1, 1, monthInput, yearInput);
			var nameInput = GetInput(field, field.id + ".5");
			nameInput.label = <?php echo json_encode(
       gf_apply_filters(
           ['gform_card_name', rgget('id')],
           __('Cardholder Name', 'gravityforms'),
           rgget('id')
       )
   ); ?>;
		}
   ...
   if (
       class_exists('ReallySimpleCaptcha') &&
   ...
   ?>
		...
		switch (type) {
			case "captcha" :
				if (GetFieldsByType(["captcha"]).length > 0) {
					alert(<?php echo json_encode(
         esc_html__(
             'Only one reCAPTCHA field can be added to the form',
             'gravityforms'
         )
     ); ?>);
					return false;
			...
			case "shipping" :
				if (GetFieldsByType(["shipping"]).length > 0) {
					alert(<?php echo json_encode(
         esc_html__(
             'Only one Shipping field can be added to the form',
             'gravityforms'
         )
     ); ?>);
					return false;
			...
			case "post_content" :
				if (GetFieldsByType(["post_content"]).length > 0) {
					alert(<?php echo json_encode(
         esc_html__(
             'Only one Post Content field can be added to the form',
             'gravityforms'
         )
     ); ?>);
					return false;
			...
			case "post_title" :
				if (GetFieldsByType(["post_title"]).length > 0) {
					alert(<?php echo json_encode(
         esc_html__(
             'Only one Post Title field can be added to the form',
             'gravityforms'
         )
     ); ?>);
					return false;
			...
			case "post_excerpt" :
				if (GetFieldsByType(["post_excerpt"]).length > 0) {
					alert(<?php echo json_encode(
         esc_html__(
             'Only one Post Excerpt field can be added to the form',
             'gravityforms'
         )
     ); ?>);
					return false;
			...
			case "creditcard" :
				if (GetFieldsByType(["creditcard"]).length > 0) {
					alert(<?php echo json_encode(
         esc_html__(
             'Only one credit card field can be added to the form',
             'gravityforms'
         )
     ); ?>);
					return false;
			...
			case "option" :
				if (GetFieldsByType(["product"]).length <= 0) {
					alert(<?php echo json_encode(
         esc_html__(
             'You must add a product field to the form first',
             'gravityforms'
         )
     ); ?>);
					return false;
	...
	function StartAddField(type, index) {
		...
		mysack.onError = function () {
			alert(<?php echo json_encode(
       esc_html__('Ajax error while adding field', 'gravityforms')
   ); ?>)
		};
		mysack.runAJAX();
	...
	function RefreshSelectedFieldPreview(callback) {
		...
		var fieldId = field.id,
			data = {'action': 'rg_refresh_field_preview', 'rg_refresh_field_preview': '<?php echo wp_create_nonce(
       'rg_refresh_field_preview'
   ); ?>', 'field': jQuery.toJSON(field), 'formId': form.id};
	...
	function StartChangeInputType(type, field) {
		...
		mysack.setVar("rg_change_input_type", "<?php echo wp_create_nonce(
      'rg_change_input_type'
		...
		mysack.onError = function () {
			alert(<?php echo json_encode(
       esc_html__('Ajax error while changing input type', 'gravityforms')
   ); ?>)
		};
		mysack.runAJAX();
	...
	function GetFieldChoices(field) {
		...
		for (var i = 0; i < field.choices.length; i++) {
			...
			str += "<button class='gf_insert_field_choice' onclick=\"InsertFieldChoice(" + (i + 1) + ");\" aria-label='<?php esc_attr_e(
       'Add choice',
       'gravityforms'
   ); ?>'><i class='gficon-add' aria-hidden='true'></i></button>";

			if (field.choices.length > 1) {
				str += "<button class='gf_delete_field_choice' onclick=\"DeleteFieldChoice(" + i + ");\" aria-label='<?php esc_attr_e(
        'Delete choice',
        'gravityforms'
    ); ?>'><i class='gficon-subtract' aria-hidden='true'></i></button>";
			}
	...
	function GetInputChoices(input) {
		...
		for (var i = 0; i < input.choices.length; i++) {
			...
			str += "<button class='gf_insert_field_choice field-input-insert-choice' onclick=\"InsertFieldChoice(" + (i + 1) + ");\" aria-label='<?php esc_attr_e(
       'Add choice',
       'gravityforms'
   ); ?>'><i class='gficon-add' aria-hidden='true'></i></button>";

			if (input.choices.length > 1) {
				str += "<button class='gf_delete_field_choice field-input-delete-choice' onclick=\"DeleteFieldChoice(" + i + ");\" aria-label='<?php esc_attr_e(
        'Delete choice',
        'gravityforms'
    ); ?>'><i class='gficon-subtract' aria-hidden='true'></i></button>";
			}
	...
	function GetCaptchaUrl(pos) {
      ...
      'admin-ajax.php?action=rg_captcha_image'
  ); ?>" + "&type=" + field.captchaType + "&pos=" + pos + "&size=" + size + "&fg=" + fg.replace("#", "%23") + "&bg=" + bg.replace("#", "%23");
		return url;
	}

	function SetFieldPhoneFormat(phoneFormat) {
		var instruction = phoneFormat == "standard" ? <?php echo json_encode(
      esc_html__('Phone format:', 'gravityforms')
  ); ?> + " (###) ###-####" : "";
	...
	function LoadMessageVariables() {
		var options = "<option>" + <?php echo json_encode(
      esc_html__('Select a field', 'gravityforms')
  ); ?> + "</option><option value='{form_title}'>" + <?php echo json_encode(
     esc_html__('Form Title', 'gravityforms')
 ); ?> + "</option><option value='{date_mdy}'>" + <?php echo json_encode(
     esc_html__('Date', 'gravityforms')
 ); ?> + " (mm/dd/yyyy)</option><option value='{date_dmy}'>" + <?php echo json_encode(
     esc_html__('Date', 'gravityforms')
 ); ?> + " (dd/mm/yyyy)</option><option value='{ip}'>" + <?php echo json_encode(
     esc_html__('User IP Address', 'gravityforms')
 ); ?> + "</option><option value='{all_fields}'>" + <?php echo json_encode(
     esc_html__('All Submitted Fields', 'gravityforms')
 ); ?> + "</option>";
...
<?php wp_print_scripts(['gform_form_editor']); ?>
