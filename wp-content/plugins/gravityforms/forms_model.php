...
class GFFormsModel
{
    ...
    private static $_batch_field_updates = [];
    private static $_batch_field_inserts = [];
    private static $_batch_field_deletes = [];
    ...
    public static function get_forms(
        ...
        $order_by = !empty($sort_column)
            ? "ORDER BY $sort_column $sort_keyword"
            : '';
    ...
    {
        return ['id', 'title', 'date_created', 'is_active', 'is_trash'];
    }
    ...
    {
        global $wpdb;
        $entry_table_name = version_compare(
            ...
            if (is_array($lead_date_results)) {
                foreach ($lead_date_results as $entry_date_result) {
                    if ($entry_date_result['form_id'] == $forms[$i]['id']) {
                        $forms[$i]['last_entry_date'] =
                            $entry_date_result['last_entry_date'];
                        $forms[$i]['total_entries'] =
                            $entry_date_result['total_entries'];
                        break;
                    }
                }
            }
    ...
    public static function get_form($form_id, $allow_trash = false)
    {
        ...
        $results = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM $table_name WHERE id=%d {$trash_clause}",
                $form_id
            )
        );
    ...
    public static function get_form_meta($form_id)
    {
        ...
        $form_row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT display_meta, notifications FROM {$table_name} WHERE form_id=%d",
                $form_id
            ),
            ARRAY_A
        );
    ...
    public static function get_next_field_id($fields, $next_field_id = 1)
    {
        foreach ($fields as $field) {
            if (is_array($field->fields)) {
                $next_field_id = self::get_next_field_id(
                    $field->fields,
                    $next_field_id
                );
            }
        ...
        if (is_array(rgar($form, 'fields'))) {
            foreach ($form['fields'] as &$field) {
                ...
                if (!isset($field['visibility'])) {
                    $field['visibility'] =
                        isset($field['adminOnly']) && $field['adminOnly']
                            ? 'administrative'
                            : 'visible';
                ...
                if (is_array($field->fields)) {
                    self::convert_sub_field_objects(
                        $field,
                        $form['id'],
                        $page_number
                    );
     ...
     */
    private static function convert_sub_field_objects(
        &$field,
        $form_id,
        $page_number,
        $nesting_level = 0
    ...
    public static function get_form_meta_by_id($ids)
    {
        ...
        foreach ($results as &$result) {
            ...
            $form['confirmations'] = self::unserialize(
                $result['confirmations']
            );
            $form['notifications'] = self::unserialize(
    ...
    private static function load_notifications_to_legacy($form)
    {
        ...
        foreach ($form['notifications'] as $notification) {
            if (!in_array(rgar($notification, 'type'), ['user', 'admin'])) {
                continue;
            }
    ...
    private static function load_notifications_from_legacy($form)
    {
        ...
        if (GFCommon::has_admin_notification($form)) {
            ...
            $admin_notification = self::convert_property_to_merge_tag(
                $form,
                $admin_notification,
                'from',
                'fromField'
            ...
            $admin_notification['toType'] = !rgempty(
                'routing',
                $admin_notification
            )
                ? 'routing'
            ...
            $admin_notification['name'] = esc_html__(
                'Admin Notification',
                'gravityforms'
            );
        ...
        if (GFCommon::has_user_notification($form)) {
            ...
            $user_notification['name'] = esc_html__(
                'User Notification',
                'gravityforms'
            );
     ...
     */
    private static function convert_property_to_merge_tag(
        $form,
        $array,
        $target_property,
        $source_property
    ) {
        $merge_tag = self::get_field_merge_tag(
            $form,
            rgar($array, $source_property)
        );
        if ($merge_tag) {
    ...
    private static function get_field_merge_tag($form, $field_id)
    {
        ...
        return '{' .
            GFCommon::get_label($field, $field_id) .
            ':' .
            $field_id .
            '}';
    }
    ...
    public static function get_grid_column_meta($form_id)
    {
        ...
        return maybe_unserialize(
            $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT entries_grid_meta FROM $table_name WHERE form_id=%d",
                    $form_id
                )
            )
        );
    ...
    public static function update_grid_column_meta($form_id, $columns)
    {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE $table_name SET entries_grid_meta=%s WHERE form_id=%d",
                $meta,
                $form_id
            )
        );
    ...
    public static function get_lead_detail_id(
        $current_fields,
        $field_number,
        $item_index = ''
    ) {
        if (
            ...
            return GF_Forms_Model_Legacy::get_lead_detail_id(
                $current_fields,
                $field_number
            );
        }
    ...
    public static function update_form_active($form_id, $is_active)
    {
        ...
        $sql = $wpdb->prepare(
            "UPDATE $form_table SET is_active=%d WHERE id=%d",
            $is_active,
            $form_id
        );
        $wpdb->query($sql);
    ...
    public static function update_notification_active(
        ...
        if (!isset($form['notifications'][$notification_id])) {
            return new WP_Error(
                'not_found',
                __('Notification not found', 'gravityforms')
            );
        }
        ...
        $result = GFFormsModel::update_form_meta(
            $form_id,
            $form['notifications'],
            'notifications'
        );
    ...
    public static function update_confirmation_active(
        ...
        $form['confirmations'][$confirmation_id][
            'isActive'
        ] = (bool) $is_active;
    ...
    public static function update_leads_property(
        $leads,
        $property_name,
        $property_value
    ) {
    ...
    public static function update_lead_property(
        ...
        return self::update_entry_property(
            $lead_id,
            $property_name,
            $property_value,
            $update_akismet,
    ...
    public static function update_entries_property(
        $leads,
        $property_name,
        $property_value
    ) {
    ...
    public static function update_entry_property(
        $lead_id,
        $property_name,
        $property_value,
        $update_akismet = true,
        ...
        if (
            ...
            return GF_Forms_Model_Legacy::update_lead_property(
                $lead_id,
                $property_name,
                $property_value,
                $update_akismet,
        ...
        if (
            $update_akismet &&
            GFCommon::akismet_enabled($lead['form_id']) &&
            $property_name == 'status' &&
            in_array($property_value, ['active', 'spam'])
        ) {
        ...
        if (
            $property_name == 'status' &&
            $property_value == 'trash' &&
            !empty($current_user->user_login)
        ) {
            GFCommon::log_debug(
                __METHOD__ .
                    "(): User ID {$current_user->ID} requested moving of entry #{$lead_id} to trash."
            );
        }
        ...
        if (!$disable_hook) {
            ...
            if ($previous_value != $property_value) {
                ...
                if (
                    ...
                    $lead['post_id'] = GFCommon::create_post(
                        isset($form)
                            ? $form
                            : GFAPI::get_form($lead['form_id']),
                        $lead
                    );
                }
    ...
    public static function delete_entries_by_form($form_id, $status = '')
    {
        ...
        GFCommon::log_debug(
            __METHOD__ . "(): Deleting entries for form #{$form_id}."
        );
        ...
        if (!empty($entry_ids)) {
            ...
            if (!empty($current_user->user_login)) {
                GFCommon::log_debug(
                    __METHOD__ .
                        "(): User ID {$current_user->ID} requested deletion of entries: " .
                        json_encode($entry_ids)
                );
            }
        ...
        // Delete from entry
        $sql = $wpdb->prepare(
            "DELETE FROM $entry_table WHERE form_id=%d {$status_filter}",
            $form_id
        );
        $wpdb->query($sql);
    }
    ...
    public static function delete_views($form_id)
    {
        if (gf_upgrade()->get_submissions_block()) {
            return new WP_Error(
                'submissions_blocked',
                __(
                    'Submissions are currently blocked due to an upgrade in progress',
                    'gravityforms'
                )
            );
        }
        ...
        if (!empty($current_user->user_login)) {
            GFCommon::log_debug(
                __METHOD__ .
                    "(): User ID {$current_user->ID} requested deletion of views for form #{$form_id}."
            );
        }
        ...
        //Delete form view
        $sql = $wpdb->prepare(
            "DELETE FROM $form_view_table WHERE form_id=%d",
            $form_id
        );
        $wpdb->query($sql);
    ...
    public static function delete_form($form_id)
    {
        ...
        if (!empty($current_user->user_login)) {
            GFCommon::log_debug(
                __METHOD__ .
                    "(): User ID {$current_user->ID} requested deletion of form #{$form_id}."
            );
        }
        ...
        //Delete form meta
        $sql = $wpdb->prepare(
            "DELETE FROM $form_meta_table WHERE form_id=%d",
            $form_id
        );
        $wpdb->query($sql);
        ...
        $sql = $wpdb->prepare(
            "DELETE FROM $form_revisions_table WHERE form_id=%d",
            $form_id
        );
        $wpdb->query($sql);
    ...
    public static function trash_form($form_id)
    {
        ...
        if (!empty($current_user->user_login)) {
            GFCommon::log_debug(
                __METHOD__ .
                    "(): User ID {$current_user->ID} requested moving of form #{$form_id} to trash."
            );
        }
        $form_table_name = self::get_form_table_name();
        $sql = $wpdb->prepare(
            "UPDATE $form_table_name SET is_trash=1 WHERE id=%d",
            $form_id
        );
    ...
    public static function restore_form($form_id)
    {
        ...
        $form_table_name = self::get_form_table_name();
        $sql = $wpdb->prepare(
            "UPDATE $form_table_name SET is_trash=0 WHERE id=%d",
            $form_id
        );
        $result = $wpdb->query($sql);
    ...
    public static function is_unique_title($title, $form_id = 0)
    {
        ...
        foreach ($forms as $form) {
            if (
                strtolower($form->title) === strtolower($title) &&
                $form->id !== $form_id
            ) {
                return false;
            }
    ...
    public static function ensure_tables_exist()
    {
        ...
        if ($wpdb->last_error) {
            GFCommon::log_debug(
                'GFFormsModel::ensure_tables_exist(): Blog ' .
                    get_current_blog_id() .
                    ' - Form database table does not exist. Forcing database setup.'
            );
    ...
    public static function insert_form($form_title)
    {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "INSERT INTO $form_table_name(title, date_created) VALUES(%s, utc_timestamp())",
                $form_title
            )
        );
    ...
    public static function update_form_meta(
        ...
        $form_meta = gf_apply_filters(
            ['gform_form_update_meta', $form_id],
            $form_meta,
            $form_id,
            $meta_name
        );
        ...
        if (
            intval(
                $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT count(0) FROM $meta_table_name WHERE form_id=%d",
                        $form_id
                    )
                )
            ) > 0
            ...
            $result = $wpdb->query(
                $wpdb->prepare(
                    "UPDATE $meta_table_name SET $meta_name=%s WHERE form_id=%d",
                    $form_meta,
                    $form_id
                )
            );
            ...
            $result = $wpdb->query(
                $wpdb->prepare(
                    "INSERT INTO $meta_table_name(form_id, $meta_name) VALUES(%d, %s)",
                    $form_id,
                    $form_meta
                )
            );
         ...
         */
        gf_do_action(
            ['gform_post_update_form_meta', $form_id],
            $form_meta,
            $form_id,
            $meta_name
        );
     ...
     * @param int    $form_id Form ID.
     */
    public static function maybe_create_form_revision(
        $new_display_meta,
        $form_id
    ) {
        global $wpdb;
        ...
        if (GFCommon::has_consent_field($new_display_meta)) {
            ...
            if (
                intval(
                    $wpdb->get_var(
                        $wpdb->prepare(
                            "SELECT count(0) FROM $revisions_table_name WHERE form_id=%d",
                            $form_id
                        )
                    )
                ) === 0
                ...
                $wpdb->query(
                    $wpdb->prepare(
                        "INSERT INTO $revisions_table_name(form_id, display_meta, date_created) VALUES(%d, %s, utc_timestamp())",
                        $form_id,
                        json_encode($new_display_meta)
                    )
                );
            ...
            foreach ($new_display_meta['fields'] as $field) {
                if ($field['type'] === 'consent') {
                    if (
                        $field['description'] !==
                        rgar($old_consent_fields, $field['id'])
                    ) {
    ...
    public static function get_latest_form_revisions_id($form_id)
    {
        ...
        $value = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT id FROM $revisions_table_name WHERE form_id=%d ORDER BY date_created DESC, id DESC LIMIT 1",
                $form_id
            )
        );
    ...
    public static function delete_files_by_form($form_id, $status = '')
    {
        ...
        $results = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id FROM {$entry_table_name} WHERE form_id=%d {$status_filter}",
                $form_id
            ),
            ARRAY_A
        );
    ...
    public static function get_delete_file_field_types($form)
    {
        ...
        return gf_apply_filters(
            ['gform_field_types_delete_files', $form['id']],
            $field_types,
            $form
        );
    }
    ...
    public static function delete_field_files($form_id, $field_id)
    {
        ...
        $values = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT meta_value FROM {$entry_meta_table_name} WHERE form_id=%d AND meta_key=%s",
                $form_id,
                $field_id
            )
        );
    ...
    public static function delete_file($entry_id, $field_id, $file_index = 0)
    {
        ...
        if (version_compare(self::get_database_version(), '2.3-dev-1', '<')) {
            GF_Forms_Model_Legacy::delete_file(
                $entry_id,
                $field_id,
                $file_index
            );
        ...
        $entry_meta_table_name = self::get_entry_meta_table_name();
        $sql = $wpdb->prepare(
            "SELECT id FROM {$entry_meta_table_name} WHERE entry_id=%d AND meta_key = %s",
            $entry_id,
            $field_id
        );
        $entry_meta_id = $wpdb->get_var($sql);

        self::update_entry_field_value(
            $form,
            $entry,
            $field,
            $entry_meta_id,
    ...
    public static function get_physical_file_path($url)
    {
        ...
        if (is_multisite() && get_site_option('ms_files_rewriting')) {
            $file_path = preg_replace(
                "|^(.*?)/files/gravity_forms/|",
                BLOGUPLOADDIR . 'gravity_forms/',
                $url
            );
        } else {
            $file_path = str_replace(
                self::get_upload_url_root(),
    ...
    public static function delete_field(
        ...
        $form = is_numeric($form_or_id)
            ? self::get_form_meta($form_or_id)
            : $form_or_id;
        ...
        for ($i = $count - 1; $i >= 0; $i--) {
            ...
            if (!empty($field->conditionalLogic)) {
                $field->conditionalLogic = self::delete_field_from_conditional_logic(
            ...
            if (
                $field->type === 'page' &&
                !empty($field->nextButton['conditionalLogic'])
            ) {
                $field->nextButton[
                    'conditionalLogic'
                ] = self::delete_field_from_conditional_logic(
                    $field->nextButton['conditionalLogic'],
                    $field_id
                );
            }
        ...
        if (
            empty($field_type) &&
            ($deleted_field = self::get_field($form_id, $field_id))
        ) {
    ...
    public static function delete_field_from_confirmations($form, $field_id)
    {
        ...
        foreach ($form['confirmations'] as &$confirmation) {
            if (!empty($confirmation['conditionalLogic'])) {
                $processed = self::delete_field_from_conditional_logic(
    ...
    public static function delete_field_from_notifications($form, $field_id)
    {
        ...
        foreach ($form['notifications'] as &$notification) {
            if (!empty($notification['routing'])) {
                ...
                if ($dirty) {
                    $notification['routing'] = empty($notification['routing'])
                        ? null
                        : array_values($notification['routing']);
                    $save = true;
                }
            }
     ...
     * @return null|array
     */
    public static function delete_field_from_conditional_logic(
        $logic,
        $field_id
    ) {
    ...
    public static function delete_field_values($form_id, $field_id)
    {
        ...
        // Delete from entry meta
        $sql = $wpdb->prepare(
            "DELETE FROM $entry_meta_table WHERE form_id=%d AND meta_key = %s",
            $form_id,
            $field_id
        );
        if (is_numeric($field_id)) {
            $sql .= $wpdb->prepare(
                " OR form_id=%d AND meta_key LIKE %s",
                $form_id,
                sprintf('%d.%%', $field_id)
            );
        }
    ...
    public static function delete_entry($entry_id)
    {
        ...
        if (!empty($current_user->user_login)) {
            GFCommon::log_debug(
                __METHOD__ .
                    "(): User ID {$current_user->ID} requested deletion of entry #{$entry_id}"
            );
        }
        ...
        // Delete from entry meta
        $sql = $wpdb->prepare(
            "DELETE FROM $entry_meta_table_name WHERE entry_id=%d",
            $entry_id
        );
        $wpdb->query($sql);
        ...
        $sql = $wpdb->prepare(
            "DELETE FROM $entry_notes_table WHERE entry_id=%d",
            $entry_id
        );
        $wpdb->query($sql);
        ...
        $sql = $wpdb->prepare(
            "DELETE FROM $entry_table WHERE id=%d",
            $entry_id
        );
        $wpdb->query($sql);
    }
    ...
    public static function add_note(
        ...
        if (
            ...
            GF_Forms_Model_Legacy::add_note(
                $entry_id,
                $user_id,
                $user_name,
                $note,
        ...
        $sql = $wpdb->prepare(
            "INSERT INTO $table_name(entry_id, user_id, user_name, value, note_type, sub_type, date_created) values(%d, %d, %s, %s, %s, %s, utc_timestamp())",
            $entry_id,
            $user_id,
            $user_name,
            $note,
    ...
    public static function delete_note($note_id)
    {
        ...
        $lead_id = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT entry_id FROM $table_name WHERE id = %d",
                $note_id
            )
        );
     ...
     */
    public static function add_notification_note(
        $entry_id,
        $result,
        $notification,
        $error_info = '',
        ...
        if (!empty($note_args['text'])) {
            ...
            self::add_note(
                ...
                0,
                sprintf(
                    esc_html__('%1$s (ID: %2$s)', 'gravityforms'),
                    $notification['name'],
                    $notification['id']
                ),
                $note_args['text'],
    ...
    public static function save_entry($form, &$entry)
    {
        ...
        if (
            ...
            wp_die(
                esc_html__(
                    "You don't have adequate permission to edit entries.",
                    'gravityforms'
                )
            );
        }
        ...
        if (
            ...
            GFCommon::log_debug(
                __METHOD__ .
                    "(): User ID {$current_user->ID} requested update of entry #{$entry['id']}."
            );
        }
        ...
        if ($is_new_lead) {
            ...
            $user_id =
                $current_user && $current_user->ID ? $current_user->ID : null;

            $user_agent = self::truncate(
                rgar($_SERVER, 'HTTP_USER_AGENT'),
                250
            );
             ...
             *
             */
            $currency = gf_apply_filters(
                ['gform_currency_pre_save_entry', $form['id']],
            ...
            GFCommon::log_debug(
                __METHOD__ .
                    "(): Entry record created in the database. ID: {$lead_id}."
            );
        } else {
            GFCommon::log_debug(
                __METHOD__ . "(): Updating existing entry. ID: {$entry['id']}."
            );
        ...
        $current_fields = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id, meta_key, item_index FROM $entry_meta_table WHERE entry_id=%d",
                $entry['id']
            )
        );
        ...
        foreach ($form['fields'] as $field) {
             ...
             */
            $read_value_from_post = gf_apply_filters(
                [
                    'gform_use_post_value_for_conditional_logic_save_entry',
                    $form['id'],
                ],
                $is_new_lead || !isset($entry['date_created']),
                $form,
                $entry
            );
            ...
            if (
                $is_entry_detail ||
                !GFFormsModel::is_field_hidden(
                    $form,
                    $field,
                    [],
                    $read_value_from_post ? null : $entry
                )
            ) {
        ...
        if ($is_new_lead && is_wp_error($results['inserts'])) {
            ...
            GFCommon::log_error(
                __METHOD__ .
                    '(): Error while saving field values for new entry. ' .
        ...
        if (!empty($calculation_fields)) {
            ...
            if ($is_new_lead && is_wp_error($results['inserts'])) {
                ...
                GFCommon::log_error(
                    __METHOD__ .
                        '(): Error while saving calculation field values for new entry. ' .
                        $error->get_error_message()
                );
                wp_die($die_message);
            }
        ...
        if (!empty($total_fields)) {
            ...
            if ($is_new_lead && is_wp_error($results['inserts'])) {
                ...
                GFCommon::log_error(
                    __METHOD__ .
                        '(): Error while saving total field values for new entry. ' .
                        $error->get_error_message()
                );
                wp_die($die_message);
            }
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (is_array($inputs)) {
                foreach ($inputs as $input) {
                    $entry[(string) $input['id']] = gf_apply_filters(
                        [
                ...
                if (
                    GFFormsModel::is_openssl_encrypted_field(
                        $entry['id'],
                        $field->id
                    )
                ) {
                ...
                $entry[(string) $field->id] = gf_apply_filters(
                    ['gform_get_input_value', $form['id'], $field->id],
                    $value,
                    $entry,
                    $field,
                    ''
    ...
    public static function add_properties_to_entry(&$entry)
    {
        ...
        $entry_table = GFFormsModel::get_entry_table_name();
        $sql = $wpdb->prepare(
            "SELECT * FROM $entry_table WHERE id=%d",
            $entry['id']
        );
    ...
    public static function create_lead($form)
    {
        ...
        $lead['ip'] = rgars($form, 'personalData/preventIP')
            ? ''
            : self::get_ip();
        $source_url = self::truncate(self::get_current_page_url(), 200);
         ...
         *
         */
        $lead['currency'] = gf_apply_filters(
            ['gform_currency_pre_save_entry', $form['id']],
            GFCommon::get_currency(),
            $form
        );
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (!RGFormsModel::is_field_hidden($form, $field, [])) {
                ...
                if (is_array($inputs)) {
                    ...
                    $lead[$field->id] = self::get_prepared_input_value(
                        $form,
                        $field,
                        $lead,
                        $field->id
        ...
        if (!empty($total_fields)) {
            foreach ($total_fields as $total_field) {
                $lead[$total_field->id] = self::get_prepared_input_value(
                    $form,
                    $total_field,
                    $lead,
                    $total_field->id
    ...
    public static function get_prepared_input_value(
        $form,
        $field,
        $lead,
        $input_id
        ...
        if (
            $field->enableCopyValuesOption &&
            rgpost('input_' . $field->id . '_copy_values_activated')
        ) {
            $source_field_id = $field->copyValuesOptionField;
            $source_input_name = str_replace(
                'input_' . $field->id,
                'input_' . $source_field_id,
                $input_name
            );
        ...
        switch (self::get_input_type($field)) {
            case 'post_image':
                ...
                if (!empty($file_info)) {
                    $file_path = self::get_file_upload_path(
                        $form['id'],
                        $file_info['uploaded_filename']
                    );
                    $url = $file_path['url'];

                    $image_title = isset($_POST["{$input_name}_1"])
                        ? strip_tags($_POST["{$input_name}_1"])
                        : '';
                    $image_caption = isset($_POST["{$input_name}_4"])
                        ? strip_tags($_POST["{$input_name}_4"])
                        : '';
                    $image_description = isset($_POST["{$input_name}_7"])
                        ? strip_tags($_POST["{$input_name}_7"])
                        : '';
            ...
            case 'fileupload':
                if ($field->multipleFiles) {
                    ...
                    $file_info = self::get_temp_filename(
                        $form['id'],
                        $input_name
                    );
                    if (!empty($file_info)) {
        ...
        return gf_apply_filters(
            ['gform_save_field_value', $form['id'], $field->id],
            $value,
            $lead,
            $field,
            $form,
     ...
     * @return mixed
     */
    public static function is_field_hidden(
        $form,
        $field,
        $field_values,
        $lead = null
        ...
        $cache_key =
            'GFFormsModel::is_field_hidden_' . $form['id'] . '_' . $field->id;
        ...
        $section_display = self::get_field_display(
            $form,
            $section,
            $field_values,
            $lead
        ...
        if ($section_display == 'hide') {
            ...
            $display = self::get_field_display(
                $form,
                $field,
                $field_values,
                $lead
    ...
    public static function is_submit_button_hidden($form)
    {
        ...
        $is_visible = self::evaluate_conditional_logic(
            $form,
            $form['button']['conditionalLogic'],
            []
        );
    ...
    public static function is_page_hidden(
        $form,
        $page_number,
        $field_values,
        $lead = null
     ...
     * @return bool
     */
    public static function is_value_match(
        $field_value,
        $target_value,
        $operation = 'is',
        $source_field = null,
        ...
        if ($source_field && is_subclass_of($source_field, 'GF_Field')) {
            if ($source_field->type == 'post_category') {
                $field_value = GFCommon::prepare_post_category_value(
                    $field_value,
                    $source_field,
                    'conditional_logic'
                );
            } elseif (
                $source_field instanceof GF_Field_MultiSelect &&
                !empty($field_value) &&
                ...
                $source_field->get_input_type() != 'checkbox' &&
                is_array($field_value) &&
                $source_field->id != $rule['fieldId'] &&
                is_array($source_field->get_entry_inputs())
            ) {
        ...
        $form_id =
            $source_field instanceof GF_Field ? $source_field->formId : 0;

        $target_value = GFFormsModel::maybe_trim_input(
            $target_value,
            $form_id,
            $source_field
        );
        ...
        if (is_array($field_value)) {
            ...
            foreach ($field_value as $val) {
                $val = GFFormsModel::maybe_trim_input(
                    GFCommon::get_selection_value($val),
                    $form_id,
                    $source_field
                );
            ...
            $is_match =
                $operation == 'isnot'
                    ? $match_count == count($field_value)
                    : $match_count > 0;
        } elseif (
            self::matches_operation(
                GFFormsModel::maybe_trim_input(
                    GFCommon::get_selection_value($field_value),
                    $form_id,
                    $source_field
                ),
                $target_value,
     ...
     * @return string
     */
    private static function get_field_display(
        $form,
        $field,
        $field_values,
        $lead = null
     ...
     */
    public static function get_field_value(
        &$field,
        $field_values = [],
        $get_from_post = true
    ) {
        ...
        if (
            $field->get_input_type() == 'list' &&
            $field->enableColumns &&
            $get_from_post &&
            rgpost('is_submit_' . $field->formId)
        ) {
     ...
     * @return false|int
     */
    public static function purge_expired_incomplete_submissions(
        $expiration_days = 30
    ) {
     ...
     * @return false|int
     */
    public static function purge_expired_draft_submissions(
        $expiration_days = 30
    ) {
        global $wpdb;
         ...
         */
        $expiration_days = apply_filters(
            'gform_incomplete_submissions_expiration_days',
            $expiration_days
        );
        $expiration_date = gmdate(
            'Y-m-d H:i:s',
            time() - $expiration_days * 24 * 60 * 60
        );

        $table = version_compare(self::get_database_version(), '2.3-dev-1', '<')
    ...
    {
        _deprecated_function(
            'GFFormsModel::delete_incomplete_submission',
            '2.4',
            'GFFormsModel::delete_draft_submission'
        );
    ...
    public static function delete_draft_submission($token)
    {
        ...
        $table = version_compare(self::get_database_version(), '2.3-dev-1', '<')
            ...
            : self::get_draft_submissions_table_name();
        $result = $wpdb->query(
            $wpdb->prepare("DELETE FROM $table WHERE uuid = %s", $token)
        );
     ...
     */
    public static function save_incomplete_submission(
        $form,
        $entry,
        $field_values,
        $page_number,
        ...
        _deprecated_function(
            'GFFormsModel::save_incomplete_submission',
            '2.4',
            'GFFormsModel::save_draft_submission'
        );
        return self::save_draft_submission(
            $form,
            $entry,
            $field_values,
            $page_number,
     ...
     * @return bool|false|int|string
     */
    public static function save_draft_submission(
        $form,
        $entry,
        $field_values,
        $page_number,
        ...
        foreach ($form['fields'] as $field) {
            ...
            $submitted_values[$field->id] = RGFormsModel::get_field_value(
                $field,
                $field_values
            );
        }
         ...
         * @param array $form             The Form object
         */
        $submitted_values = apply_filters(
        ...
        $submission_json = self::filter_draft_submission_pre_save(
            $submission_json,
            $resume_token,
            $form
        );
     ...
     * @return string
     */
    private static function filter_draft_submission_pre_save(
        $submission_json,
        $resume_token,
        $form
    ) {
     ...
     * @return bool|false|int|string
     */
    public static function update_draft_submission(
        $resume_token,
        $form,
        $date_created,
        $ip,
    ...
    {
        _deprecated_function(
            'GFFormsModel::get_incomplete_submission_values',
            '2.4',
            'GFFormsModel::get_draft_submission_values'
        );
    ...
    public static function get_draft_submission_values($resume_token)
    {
        ...
        $table = version_compare(self::get_database_version(), '2.3-dev-1', '<')
            ...
            : self::get_draft_submissions_table_name();
        $sql = $wpdb->prepare(
            "SELECT date_created, form_id, submission, source_url FROM {$table} WHERE uuid = %s",
            $resume_token
        );
        ...
        if (!empty($row)) {
            ...
            $row['submission'] = self::filter_draft_submission_post_get(
                $row['submission'],
                $resume_token,
                $form
            );
     ...
     * @return string
     */
    private static function filter_draft_submission_post_get(
        $submission_json,
        $resume_token,
        $form
    ) {
    ...
    {
        _deprecated_function(
            'GFFormsModel::add_email_to_incomplete_sumbmission',
            '2.4',
            'GFFormsModel::add_email_to_draft_sumbmission'
        );
    ...
    public static function add_email_to_draft_sumbmission($token, $email)
    {
        ...
        $table = version_compare(self::get_database_version(), '2.3-dev-1', '<')
            ...
            : self::get_draft_submissions_table_name();
        $sql = $wpdb->prepare(
            "UPDATE $table SET email = %s WHERE uuid = %s",
            $email,
            $token
        );
        $result = $wpdb->query($sql);
    ...
    public static function get_parameter_value($name, $field_values, $field)
    {
        ...
        if (!empty($value) && RGFormsModel::get_input_type($field) == 'list') {
            ...
            $column_count = is_array($field->choices)
                ? count($field->choices)
                : 0;
            ...
            if (!empty($rows)) {
                ...
                foreach ($rows as $row) {
                    ...
                    $ary_rows = array_merge(
                        $ary_rows,
                        rgexplode($delimiter, $row, $column_count)
                    );
                }
                ...
                if (!IS_ADMIN) {
                    if (is_array($value)) {
                        foreach ($value as &$_value) {
                            $_value = GFCommon::replace_variables_prepopulate(
                                $_value
                            );
                        }
                    } else {
                        $value = GFCommon::replace_variables_prepopulate(
                            $value
                        );
                    }
                }
    ...
    private static function get_post_field_value($field, $lead)
    {
        if (is_array($field->get_entry_inputs())) {
            ...
            foreach ($field->inputs as $input) {
                $val = isset($lead[strval($input['id'])])
                    ? $lead[strval($input['id'])]
                    : '';
                if (!empty($val)) {
    ...
    {
        $post_data = [];
        $post_data['post_custom_fields'] = [];
        $post_data['tags_input'] = [];
        $categories = [];
        $images = [];

        foreach ($form['fields'] as $field) {
            if (self::is_field_hidden($form, $field, [], $lead)) {
                continue;
            }
            ...
            switch ($field->type) {
                ...
                case 'post_content':
                    ...
                    $post_data[$field->type] = GFCommon::encode_shortcodes(
                        $value
                    );
                    break;
                ...
                case 'post_tags':
                    ...
                    if (is_array($tags) && sizeof($tags) > 0) {
                        $post_data['tags_input'] = array_merge(
                            $post_data['tags_input'],
                            $tags
                        );
                    }
                ...
                case 'post_custom_field':
                    ...
                    if (!isset($post_data['post_custom_fields'][$meta_name])) {
                        ...
                        !is_array($post_data['post_custom_fields'][$meta_name])
                    ) {
                        $post_data['post_custom_fields'][$meta_name] = [
                            $post_data['post_custom_fields'][$meta_name],
                            $value,
                        ];
                    } else {
        ...
        $post_data['post_category'] = !empty($categories)
            ? $categories
            : [rgar($form, 'postCategory')];
        $post_data['images'] = $images;

        //setting current user as author depending on settings
        $post_data['post_author'] =
            $form['useCurrentUserAsAuthor'] && !empty($lead['created_by'])
                ? $lead['created_by']
                : rgar($form, 'postAuthor');
    ...
    public static function get_custom_field_names()
    {
         ...
         */
        $disable_query = gf_apply_filters(
            ['gform_disable_custom_field_names_query', $form_id],
            false
        );
        ...
        $sql = "SELECT DISTINCT meta_key
			...
			ORDER BY meta_key";
        $keys = $wpdb->get_col(
            $wpdb->prepare($sql, $wpdb->esc_like('_') . '%')
        );
    ...
    private static function get_default_post_title()
    {
        ...
        $titles = $wpdb->get_col(
            "SELECT post_title FROM $wpdb->posts WHERE post_title like '%Untitled%'"
        );
     ...
     * @return mixed
     */
    public static function prepare_value(
        $form,
        $field,
        $value,
        $input_name,
        ...
        $value = $field->get_value_save_entry(
            $value,
            $form,
            $input_name,
            $lead_id,
        ...
        if ($field->type == 'post_category') {
            ...
            foreach ($value as $cat_id) {
                ...
                $full_values[] =
                    !is_wp_error($cat) && is_object($cat)
                        ? $cat->name . ':' . $cat_id
                        : '';
            }

            $value = $is_multiselect
                ? $field->to_string($full_values)
                : implode(',', $full_values);
        }
    ...
    public static function is_checkbox_checked(
        $field_id,
        $field_label,
        $lead,
        $form
        ...
        foreach ($lead_field_keys as $input_id) {
            ...
            if (
                is_numeric($input_id) &&
                absint($input_id) == absint($field_id)
            ) {
    ...
    {
        _deprecated_function(
            'GFFormsModel::get_fileupload_value',
            '1.9',
            'GF_Field_Fileupload::get_fileupload_value'
        );
        ...
        if (empty($_gf_uploaded_files)) {
            GFCommon::log_debug(
                'GFFormsModel::get_fileupload_value(): No files uploaded. Exiting.'
            );
            $_gf_uploaded_files = [];
        }
        ...
        if (!isset($_gf_uploaded_files[$input_name])) {
            ...
            $temp_filepath =
                self::get_upload_path($form_id) .
                '/tmp/' .
                $file_info['temp_filename'];
            GFCommon::log_debug(
                'GFFormsModel::get_fileupload_value(): Temp file path: ' .
                    $temp_filepath
            );
            if ($file_info && file_exists($temp_filepath)) {
                GFCommon::log_debug(
                    'GFFormsModel::get_fileupload_value(): Moving temp file: ' .
                        $temp_filepath
                );
                $_gf_uploaded_files[$input_name] = self::move_temp_file(
                    $form_id,
                    $file_info
                );
            } elseif (!empty($_FILES[$input_name]['name'])) {
                GFCommon::log_debug(
                    'GFFormsModel::get_fileupload_value(): Uploading file: ' .
                        $_FILES[$input_name]['name']
                );
                $_gf_uploaded_files[$input_name] = self::upload_file(
                    $form_id,
                    $_FILES[$input_name]
                );
            }
    ...
    public static function get_temp_filename($form_id, $input_name)
    {
        ...
        if (
            empty($uploaded_filename) &&
            isset(self::$uploaded_files[$form_id])
        ) {
            $uploaded_filename = rgget(
                $input_name,
                self::$uploaded_files[$form_id]
            );
        }
    ...
    public static function get_choice_text($field, $value, $input_id = 0)
    {
        ...
        foreach ($field->choices as $choice) {
            if (
                is_array($value) &&
                self::choice_value_match($field, $choice, $value[$input_id])
            ) {
                return $choice['text'];
            } elseif (
                !is_array($value) &&
                self::choice_value_match($field, $choice, $value)
            ) {
                return $choice['text'];
            }
        }
    ...
    {
        $choice_value = GFFormsModel::maybe_trim_input(
            $choice['value'],
            $field->formId,
            $field
        );
        ...
        elseif (RGFormsModel::get_input_type($field) == 'multiselect') {
            ...
            if (
                in_array($choice_value, $values) ||
                in_array($choice_value, $sanitized_values)
            ) {
                return true;
            }
        }
    ...
    public static function create_post($form, &$lead)
    {
        ...
        foreach ($form['fields'] as $field) {
            $is_hidden = self::is_field_hidden($form, $field, [], $lead);
            if (
                !$is_hidden &&
                in_array($field->type, [
                    'post_category',
        ...
        if (!$has_post_field) {
            GFCommon::log_debug(
                "GFFormsModel::create_post(): Stopping. The form doesn't have any post fields."
            );
        ...
        if (
            empty($post_data['post_title']) &&
            empty($post_data['post_content']) &&
            empty($post_data['post_excerpt'])
        ) {
        ...
        // inserting post
        GFCommon::log_debug(
            'GFFormsModel::create_post(): Inserting post via wp_insert_post().'
        );
        $post_id = wp_insert_post($post_data, true);
        GFCommon::log_debug(
            'GFFormsModel::create_post(): Result from wp_insert_post(): ' .
                print_r($post_id, 1)
        );

        if (is_wp_error($post_id)) {
            GFCommon::log_debug(
                __METHOD__ . '(): $post_data => ' . print_r($post_data, 1)
            );
        ...
        if (!empty($post_data['images'])) {
            ...
            GFCommon::log_debug(
                'GFFormsModel::create_post(): Processing post images.'
            );
            ...
            foreach ($post_data['images'] as $image) {
                ...
                $media_id = self::media_handle_upload(
                    $image['url'],
                    $post_id,
                    $image_meta
                );
                ...
                if ($media_id) {
                    ...
                    if ($field->postFeaturedImage) {
                        ...
                        GFCommon::log_debug(
                            __METHOD__ .
                                '(): Setting the featured image. Result from set_post_thumbnail(): ' .
                                var_export($result, 1)
                        );
                    }
                }
        ...
        GFCommon::log_debug(
            'GFFormsModel::create_post(): Adding custom fields.'
        );
        foreach (
            $post_data['post_custom_fields']
            as $meta_name => $meta_value
        ) {
            ...
            foreach ($meta_value as $value) {
                GFCommon::log_debug(
                    'GFFormsModel::create_post(): Getting custom field: ' .
                        $meta_name
                );
                ...
                if (
                    $custom_field &&
                    $custom_field->customFieldTemplateEnabled
                ) {
                    $value = self::process_post_template(
                        $custom_field->customFieldTemplate,
                        'post_custom_field',
                        $post_images,
                        $post_data,
                        $form,
                ...
                switch (RGFormsModel::get_input_type($custom_field)) {
                    ...
                    case 'date':
                        $value = GFCommon::date_display(
                            $value,
                            rgar($custom_field, 'dateFormat')
                        );
                        if (!rgblank($value)) {
        ...
        $has_content_field =
            sizeof(self::get_fields_by_type($form, ['post_content'])) > 0;
        $has_title_field =
            sizeof(self::get_fields_by_type($form, ['post_title'])) > 0;
        $post = false;

        //if a post field was configured with a content or title template, process template
        if (
            (rgar($form, 'postContentTemplateEnabled') && $has_content_field) ||
            (rgar($form, 'postTitleTemplateEnabled') && $has_title_field)
        ) {
            $post = get_post($post_id);

            if (
                rgar($form, 'postContentTemplateEnabled') &&
                $has_content_field
            ) {
            ...
            if (rgar($form, 'postTitleTemplateEnabled') && $has_title_field) {
                $post_title = self::process_post_template(
                    $form['postTitleTemplate'],
                    'post_title',
                    $post_images,
                    $post_data,
        ...
        GFCommon::log_debug(
            'GFFormsModel::create_post(): Updating entry with post id.'
        );
         ...
         */
        gf_do_action(
            ['gform_after_create_post', $form['id']],
            $post_id,
            $lead,
            $form
        );
     ...
     * @return string
     */
    public static function process_post_template(
        $template,
        $field_type,
        $post_images,
        $post_data,
        ...
        GFCommon::log_debug(
            __METHOD__ . "(): Processing {$field_type} template."
        );

        //replacing post image variables
        $template = GFCommon::replace_variables_post_image(
            $template,
            $post_images,
            $entry
        );
        ...
        $template = GFCommon::replace_variables(
            $template,
            $form,
            $entry,
            false,
        ...
        if ($field_type != 'post_content') {
             ...
             * @since 2.0.0.4
             */
            $process_template_shortcodes = apply_filters(
                'gform_process_template_shortcodes_pre_create_post',
                $process_template_shortcodes,
                $field_type,
                $post_data,
                $form,
                $entry
            ...
            $process_template_shortcodes = apply_filters(
                'gform_process_template_shortcodes_pre_create_post_' .
                    $form['id'],
                $process_template_shortcodes,
                $field_type,
                $post_data,
    ...
    {
        $custom_fields = self::get_fields_by_type($form, ['post_custom_field']);
    ...
    private static function copy_post_image($url, $post_id)
    {
        ...
        $upload_root_info = ['path' => $target_root, 'url' => $target_root_url];
        $upload_root_info = gf_apply_filters(
            'gform_upload_path',
            $form_id,
            $upload_root_info,
            $form_id
        ...
        $path = str_replace(
            $upload_root_info['url'],
            $upload_root_info['path'],
            $url
        );
        ...
        return ['file' => $new_file, 'url' => $url, 'type' => $type['type']];
    }
    ...
    public static function media_handle_upload($url, $post_id, $post_data = [])
    {
        ...
        if (!$file) {
            GFCommon::log_debug(
                __METHOD__ .
                    '(): Image could not be copied to the media directory.'
            );
        ...
        GFCommon::log_debug(
            __METHOD__ .
                '(): Image copied to the media directory. Result from wp_insert_attachment(): ' .
                print_r($id, 1)
        );
    ...
    public static function save_input(
        $form,
        $field,
        &$lead,
        $current_fields,
        ...
        if (isset($field->fields) && is_array($field->fields)) {
            foreach ($field->fields as $sub_field) {
                ...
                foreach ($current_fields as $current_field) {
                    if (
                        intval($current_field->meta_key) == $sub_field->id &&
                        !isset($current_field->update)
                    ) {
                        ...
                        $result = self::queue_batch_field_operation(
                            ...
                            $sub_field,
                            $current_field->id,
                            $current_field->meta_key,
                            '',
                            $current_field->item_index
                        );
                        GFCommon::log_debug(
                            __METHOD__ .
                                "(): Deleting: {$field->label}(#{$sub_field->id}{$current_field->item_index} - {$field->type}). Result: " .
                                var_export($result, 1)
                        );
                    }
                }
        ...
        if (
            rgget('view') == 'entry' &&
            $type == 'fileupload' &&
            ((!$multiple_files && empty($_FILES[$input_name]['name'])) ||
                ($multiple_files &&
                    !isset($uploaded_files[$form_id][$input_name])))
        ) {
            return;
        } elseif (
            rgget('view') == 'entry' &&
     ...
     */
    public static function queue_save_input_value(
        $value,
        $form,
        $field,
        &$lead,
        ...
        if (
            is_array($value) &&
            !($field->is_value_submission_array() && !is_array($value[0]))
        ) {
            foreach ($value as $i => $v) {
                $new_item_index = $item_index . '_' . $i;
                if (
                    is_array($v) &&
                    !($field->is_value_submission_array() && !is_array($v[0]))
                ) {
                    self::queue_save_input_value(
                        $v,
                        $form,
                ...
                $lead_detail_id = self::get_lead_detail_id(
                    $current_fields,
                    $input_id,
                    $new_item_index
                );
                ...
                GFCommon::log_debug(
                    __METHOD__ .
                        "(): Saving: {$field->label}(#{$input_id}{$item_index} - {$field->type}). Result: " .
                        var_export($result, 1)
                );
                foreach ($current_fields as $current_field) {
                    if (
                        $current_field->meta_key == $input_id &&
                        $current_field->item_index == $new_item_index
                    ) {
            ...
            if (
                $lead != null &&
                isset($lead[$input_id]) &&
                $value === rgget((string) $input_id, $lead)
            ) {
                return;
            }
            ...
            GFCommon::log_debug(
                __METHOD__ .
                    "(): Queued field operation: {$field->label}(#{$input_id} - {$field->type})."
            );
        }
    }
     ...
     * @return bool
     */
    public static function update_lead_field_value(
        $form,
        $lead,
        $field,
        $lead_detail_id,
        ...
        return self::update_entry_field_value(
            $form,
            $lead,
            $field,
            $lead_detail_id,
     ...
     * @return bool
     */
    public static function update_entry_field_value(
        $form,
        $entry,
        $field,
        $entry_meta_id,
        ...
        if (version_compare(self::get_database_version(), '2.3-dev-1', '<')) {
            return GF_Forms_Model_Legacy::update_lead_field_value(
                $form,
                $entry,
                $field,
                $entry_meta_id,
        ...
        $value = apply_filters(
            "gform_save_field_value_{$form['id']}",
            $value,
            $entry,
            $field,
            $form,
                ...
                if (is_array($field->get_entry_inputs())) {
                    $_input_id =
                        false === strpos($input_id, '.')
                            ...
                            : $input_id;
                    $sql = $wpdb->prepare(
                        "DELETE FROM $entry_meta_table_name WHERE entry_id=%d AND meta_key LIKE %s ",
                        $entry_id,
                        $_input_id
                    );
                } else {
                    $sql = $wpdb->prepare(
                        "DELETE FROM $entry_meta_table_name WHERE entry_id=%d AND meta_key = %s ",
                        $entry_id,
                        $input_id
                    );
                }
     ...
     * @return bool
     */
    public static function queue_batch_field_operation(
        $form,
        &$entry,
        $field,
        $entry_meta_id,
        ...
        if (!rgblank($value)) {
            if ($entry_meta_id > 0) {
                self::$_batch_field_updates[] = [
                    'meta_value' => $value,
                    'id' => $entry_meta_id,
                ];
            } else {
                self::$_batch_field_inserts[] = [
                    'entry_id' => $entry_id,
                    'form_id' => $form_id,
                    'meta_key' => $input_id,
                    'meta_value' => $value,
            ...
            $entry_meta_id > 0 &&
            !in_array($input_id, GFFormsModel::get_lead_db_columns())
        ) {
    ...
    {
        return !empty(self::$_batch_field_updates) ||
            !empty(self::$_batch_field_inserts) ||
            !empty(self::$_batch_field_deletes);
    }
    ...
    public static function commit_batch_field_operations()
    {
        ...
        if (!empty(self::$_batch_field_deletes)) {
            $in_str_arr = array_fill(
                0,
                count(self::$_batch_field_deletes),
                '%d'
            );
            $in_str = join(',', $in_str_arr);
            $ids = array_map('absint', self::$_batch_field_deletes);
            $delete_sql = $wpdb->prepare(
                "DELETE FROM {$meta_table} WHERE id IN ( {$in_str} )",
                $ids
            );
    ...
    private static function move_temp_file($form_id, $tempfile_info)
    {
        _deprecated_function(
            'move_temp_file',
            '1.9',
            'GF_Field_Fileupload::move_temp_file'
        );

        $target = self::get_file_upload_path(
            $form_id,
    ...
    public static function upload_file($form_id, $file)
    {
        _deprecated_function(
            'upload_file',
            '1.9',
            'GF_Field_Fileupload::upload_file'
        );
        $target = self::get_file_upload_path($form_id, $file['name']);
        if (!$target) {
            GFCommon::log_debug(
                'GFFormsModel::upload_file(): FAILED (Upload folder could not be created.)'
            );
        ...
        if (move_uploaded_file($file['tmp_name'], $target['path'])) {
            GFCommon::log_debug(
                'GFFormsModel::upload_file(): Setting permissions on ' .
                    $target['path']
            );
            self::set_permissions($target['path']);
            ...
            GFCommon::log_debug(
                'GFFormsModel::upload_file(): FAILED (Temporary file could not be copied.)'
            );
    ...
    public static function get_file_upload_path($form_id, $file_name)
    {
        ...
        if (!is_dir($target_root)) {
            ...
            if (
                ...
                !file_exists($target_root . 'index.html')
            ) {
                ...
                !file_exists(self::get_upload_path($form_id) . '/index.html')
            ) {
                GFCommon::recursive_add_index_file(
                    self::get_upload_path($form_id)
                );
            } elseif (
                !file_exists(self::get_upload_path($form_id) . "/$y/index.html")
            ) {
                GFCommon::recursive_add_index_file(
                    self::get_upload_path($form_id) . "/$y"
                );
            } else {
                GFCommon::recursive_add_index_file(
                    self::get_upload_path($form_id) . "/$y/$m"
                );
            }
        }
        ...
        return ['path' => $target_path, 'url' => $target_url];
    }
    ...
    public static function drop_tables()
    {
        ...
        add_filter('query', ['GFForms', 'filter_query']);
    }
    ...
    public static function insert_form_view($form_id, $deprecated = null)
    {
        ...
        $sql = $wpdb->prepare(
            " SELECT id FROM $table_name
				WHERE form_id=%d
				AND date_created BETWEEN DATE_SUB(utc_timestamp(), INTERVAL 1 DAY) AND utc_timestamp()",
            $form_id
        );
        ...
        if (empty($id)) {
            $wpdb->query(
                $wpdb->prepare(
                    "INSERT INTO $table_name(form_id, date_created, ip) values(%d, utc_timestamp(), %s)",
                    $form_id,
                    ''
                )
            );
    ...
    public static function is_duplicate($form_id, $field, $value)
    {
        ...
        if (version_compare(self::get_database_version(), '2.3-dev-1', '<')) {
            return GF_Forms_Model_Legacy::is_duplicate(
                $form_id,
                $field,
                $value
            );
        ...
        switch (GFFormsModel::get_input_type($field)) {
            ...
            case 'phone':
                ...
                $sql_comparison =
                    'replace( replace( replace( replace( ld.meta_value, ")", "" ), "(", "" ), "-", "" ), " ", "" ) = %s';
                break;
            case 'email':
    ...
    public static function get_notes($search_criteria = [], $sorting = null)
    {
        ...
        if (rgar($search_criteria, 'start_date')) {
            if (
                !is_numeric($search_criteria['start_date']) ||
                (int) $search_criteria['start_date'] !=
                    $search_criteria['start_date']
            ) {
                $search_criteria['start_date'] = strtotime(
                    $search_criteria['start_date']
                );
            }
            $valid_timestamp = gmdate(
                'Y-m-d H:i:s',
                $search_criteria['start_date']
            );
            $where[] = $wpdb->prepare(
                'timestampdiff(SECOND, %s, date_created) >= 0',
                $valid_timestamp
            );
        }
        ...
        if (rgar($search_criteria, 'end_date')) {
            if (
                !is_numeric($search_criteria['end_date']) ||
                (int) $search_criteria['end_date'] !=
                    $search_criteria['end_date']
            ) {
                $search_criteria['end_date'] = strtotime(
                    $search_criteria['end_date']
                );
            }
            $valid_timestamp = gmdate(
            ...
            if ('00:00:00' == substr($valid_timestamp, -8)) {
                $valid_timestamp =
                    gmdate('Y-m-d', $search_criteria['end_date']) . ' 23:59:59';
            }
            $where[] = $wpdb->prepare(
                'timestampdiff(SECOND, %s, date_created) <= 0',
                $valid_timestamp
        ...
        if (is_array($sorting) && !empty($sorting)) {
            ...
            if (
                !isset($sorting['key']) ||
                !in_array($sorting['key'], $sorting_options)
            ) {
                $sorting['key'] = 'n.id';
            }
            ...
            if (
                ...
                !in_array($sorting['direction'], $direction_options)
            ) {
                $sorting['direction'] = 'ASC';
            }
            $orderby =
                'ORDER BY ' . $sorting['key'] . ' ' . $sorting['direction'];
        } else {
    ...
    public static function refresh_lead_field_value($lead_id, $field_id)
    {
        if (version_compare(GFForms::$version, '2.3-dev', '>=')) {
            _deprecated_function(
                'GFFormsModel::refresh_lead_field_value',
                '2.3'
            );
        }
        $cache_key =
            'GFFormsModel::get_lead_field_value_' . $lead_id . '_' . $field_id;
     ...
     */
    public static function get_field_value_long(
        $lead,
        $field_number,
        $form,
        $apply_filter = true
        ...
        if ($apply_filter) {
            ...
            $input_id =
                (string) $field_number == (string) $field->id
                    ? ''
                    : $field_number;
            $val = gf_apply_filters(
                [
                    'gform_get_input_value',
                    $field->formId,
                    $field->id,
    ...
    {
        if (version_compare(self::get_database_version(), '2.3-dev-1', '<')) {
            return GF_Forms_Model_Legacy::get_leads_by_meta(
                $meta_key,
                $meta_value
            );
        }
    ...
    public static function get_leads(
        ...
        if (!empty($search)) {
            $search_criteria['field_filters'][] = ['value' => $search];
        }

        if (!is_null($star)) {
            $search_criteria['field_filters'][] = ['is_starred' => $star];
        }

        if (!is_null($read)) {
            $search_criteria['field_filters'][] = ['is_read' => $read];
        }

        if ($payment_status) {
            $search_criteria['field_filters'][] = ['payment_status' => $read];
        }
     ...
     * @return null|string
     */
    public static function get_lead_count(
        $form_id,
        $search,
        $star = null,
        $read = null,
        ...
        if (version_compare(self::get_database_version(), '2.3-dev-1', '<')) {
            return GF_Forms_Model_Legacy::get_lead_count(
                $form_id,
                $search,
                $star,
                $read,
        ...
        $where = self::get_entries_where_sql(
            compact(
                'form_id',
                'search',
                'status',
    ...
    {
        _doing_it_wrong(
            'GFFormsModel::get_entries_where_sql',
            'Use GFAPI::count_entries instead',
            '2.3.0.1'
        );
        ...
        if ($star !== null && $status == 'active') {
            $where[] = $wpdb->prepare(
                "is_starred = %d AND status = 'active'",
                $star
            );
        }
     ...
     * @return array|string
     */
    public static function get_lead_ids(
        $form_id,
        $search,
        $star = null,
        $read = null,
        ...
        if (version_compare(self::get_database_version(), '2.3-dev-1', '<')) {
            return GF_Forms_Model_Legacy::get_lead_ids(
                $form_id,
                $search,
                $star,
                $read,
        ...
        foreach ($field_ids as $field_id) {
            switch ($field_id) {
                ...
                case 'created_by':
                    ...
                    break;
                case (is_string($field_id) || is_int($field_id)) &&
                    array_key_exists($field_id, $entry_meta):
                    $columns[$field_id] = [
                ...
                default:
                    ...
                    if ($field) {
                        if ($field->type === 'consent') {
                            ...
                            $input_label_only = apply_filters(
    ...
    public static function get_label(
        ...
        if (self::get_input_type($field) == 'checkbox' && $input != null) {
            ...
            if (
                self::get_input_type($field) === 'consent' &&
                (RG_CURRENT_PAGE == 'select_columns.php' ||
                    RG_CURRENT_PAGE == 'print-entry.php' ||
                    rgget('gf_page', $_GET) == 'select_columns' ||
                    rgget('gf_page', $_GET) == 'print-entry' ||
    ...
    public static function get_current_page_url($force_ssl = false)
    {
        ...
        $pageURL .=
            RGForms::get('HTTP_HOST', $_SERVER) .
            rgget('REQUEST_URI', $_SERVER);
    ...
    public static function get_submitted_fields($form_id)
    {
        ...
        $fields = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT DISTINCT meta_key FROM $entry_meta_table_name WHERE form_id=%d",
                $form_id
            )
        );
        foreach ($fields as $field) {
        ...
        switch ($event) {
            case 'form_saved':
                return [
                    ...
                    'type' => 'message',
                    'message' => __(
                        '<p>Please use the following link to return and complete this form from any computer.</p><p class="resume_form_link_wrapper"> {save_link} </p><p> Note: This link will expire after 30 days.<br />Enter your email address if you would like to receive the link via email.</p></p> {save_email_input}</p>',
                        'gravityforms'
                    ),
                    'url' => '',
                    'pageId' => '',
            ...
            case 'form_save_email_sent':
                return [
                    ...
                    'type' => 'message',
                    'message' => __(
                        '<span class="saved_message_success">Success!</span>The link was sent to the following email address: <span class="saved_message_email">{save_email}</span>',
                        'gravityforms'
                    ),
                    'url' => '',
                    'pageId' => '',
    ...
    public static function get_form_confirmations($form_id)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT confirmations FROM $tablename WHERE form_id = %d",
            $form_id
        );
        $results = $wpdb->get_results($sql, ARRAY_A);
        ...
        if (!empty($confirmations)) {
            $confirmations = self::remove_corrupt_confirmations(
                self::unserialize($confirmations)
            );
        }
    ...
    public static function get_form_ids($active = true, $trash = false)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT id from $table where is_active = %d and is_trash = %d",
            (bool) $active,
            (bool) $trash
        );
    ...
    public static function get_entry_meta($form_ids)
    {
        ...
        foreach ($form_ids as $form_id) {
            if (!isset($_entry_meta[$form_id])) {
                $_entry_meta = [];
                $_entry_meta[$form_id] = apply_filters(
                    'gform_entry_meta',
                    [],
                    $form_id
                );
    ...
    public static function set_entry_meta($lead, $form)
    {
        ...
        foreach ($keys as $key) {
            if (isset($entry_meta[$key]['update_entry_meta_callback'])) {
                ...
                $value = call_user_func_array($callback, [$key, $lead, $form]);
     ...
     * @return array
     */
    public static function search_leads(
        $form_id,
        $search_criteria = [],
        $sorting = null,
        $paging = null
    ...
    public static function search_lead_ids($form_id, $search_criteria = [])
    {
    ...
    public static function trim_form_meta_values($form, &$form_updated = false)
    {
        ...
        if (isset($form['fields']) && is_array($form['fields'])) {
            ...
            $form['fields'] = GFFormsModel::trim_conditional_logic_values(
                $form['fields'],
                $form,
                $form_updated
            );
        ...
        if ($form_updated) {
            GFCommon::log_debug(
                'GFFormsModel::trim_form_meta_values(): Form values trimmed.'
            );
        }
     ...
     */
    public static function trim_conditional_logic_values(
        $meta_array,
        $form,
        &$updated = false
    ) {
        GFCommon::log_debug(
            'GFFormsModel::trim_conditional_logic_values(): Starting.'
        );
        if (is_array($meta_array)) {
            foreach ($meta_array as &$meta) {
                $meta = self::trim_conditional_logic_values_from_element(
                    $meta,
                    $form,
                    $updated
                );
        ...
        if ($updated) {
            GFCommon::log_debug(
                'GFFormsModel::trim_conditional_logic_values(): Conditional logic values trimmed.'
            );
        }
     ...
     */
    public static function trim_conditional_logic_values_from_element(
        $element,
        $form = [],
        &$updated = false
    ) {
        if ($element instanceof GF_Field) {
            ...
            if (
                is_array($element->conditionalLogic) &&
                isset($element->conditionalLogic['rules']) &&
                is_array($element->conditionalLogic['rules'])
            ) {
                foreach ($element->conditionalLogic['rules'] as &$rule) {
                    $value = (string) $rule['value'];
                    if ($value !== trim($value)) {
                        $field = isset($form['fields'])
                            ? GFFormsModel::get_field($form, $rule['fieldId'])
                            : [];
                        $trim_value = apply_filters(
                            'gform_trim_input_value',
                            true,
            ...
            if (
                isset($element['conditionalLogic']) &&
                is_array($element['conditionalLogic']) &&
                isset($element['conditionalLogic']['rules']) &&
                is_array($element['conditionalLogic']['rules'])
            ) {
    ...
    {
        _deprecated_function(
            'GFCommon:get_encrypted_fields',
            '2.3',
            'GFCommon:get_openssl_encrypted_fields'
        );
    ...
    {
        _deprecated_function(
            'GFCommon:set_encrypted_fields',
            '2.3',
            'GFCommon:set_openssl_encrypted_fields'
        );
        ...
        $encrypted_fields = array_merge(
            self::get_encrypted_fields($entry_id),
            $field_ids
        );
    ...
    {
        _deprecated_function(
            'GFCommon:is_encrypted_field',
            '2.3',
            'GFCommon:is_openssl_encrypted_field'
        );
    ...
    {
        $encrypted_fields = gform_get_meta(
            $entry_id,
            '_openssl_encrypted_fields'
        );
    ...
    public static function set_openssl_encrypted_fields($entry_id, $field_ids)
    {
        ...
        $encrypted_fields = array_merge(
            self::get_openssl_encrypted_fields($entry_id),
            $field_ids
        );
    ...
    public static function maybe_sanitize_form_settings($form)
    {
        if (
            isset($form['version']) &&
            version_compare($form['version'], '1.9.6.10', '>=')
        ) {
    ...
    public static function sanitize_settings($form)
    {
        ...
        if (isset($form['labelPlacement'])) {
            $form['labelPlacement'] = GFCommon::whitelist(
                $form['labelPlacement'],
                ['top_label', 'left_label', 'right_label']
            );
        }
        if (isset($form['descriptionPlacement'])) {
            $form['descriptionPlacement'] = GFCommon::whitelist(
        ...
        if (isset($form['subLabelPlacement'])) {
            $form['subLabelPlacement'] = GFCommon::whitelist(
        ...
        if (isset($form['button'])) {
            $form['button']['type'] = GFCommon::whitelist(
                $form['button']['type'],
                ['text', 'image']
            );
            $form['button']['text'] =
                $form['button']['type'] == 'text'
                    ? sanitize_text_field($form['button']['text'])
                    : '';
            $form['button']['imageUrl'] =
                $form['button']['type'] == 'image'
                    ? sanitize_text_field($form['button']['imageUrl'])
                    : '';
        }
        if (isset($form['button']['conditionalLogic'])) {
            $form['button'][
                'conditionalLogic'
            ] = self::sanitize_conditional_logic(
                $form['button']['conditionalLogic']
            );
        }
        ...
        if (isset($form['save'])) {
            ...
            $form['save']['button']['text'] = sanitize_text_field(
        ...
        if (isset($form['limitEntries'])) {
            ...
            $form['limitEntriesCount'] = $form['limitEntries']
                ? absint($form['limitEntriesCount'])
                : '';
            $form['limitEntriesPeriod'] = $form['limitEntries']
                ? GFCommon::whitelist($form['limitEntriesPeriod'], [
                    '',
                    'day',
                    'week',
                    'month',
                ...
                : '';
            $form['limitEntriesMessage'] = $form['limitEntries']
                ? self::maybe_wp_kses($form['limitEntriesMessage'])
                : '';
        }
        ...
        if (isset($form['scheduleForm'])) {
            ...
            $form['scheduleStart'] = $form['scheduleForm']
                ? wp_strip_all_tags($form['scheduleStart'])
                : '';
            $form['scheduleStartHour'] = $form['scheduleForm']
                ? GFCommon::int_range($form['scheduleStartHour'], 1, 12)
                : '';
            $form['scheduleStartMinute'] = $form['scheduleForm']
                ? GFCommon::int_range($form['scheduleStartMinute'], 1, 60)
                : '';
            $form['scheduleStartAmpm'] = $form['scheduleForm']
                ? GFCommon::whitelist($form['scheduleStartAmpm'], ['am', 'pm'])
                : '';
            $form['scheduleEnd'] = $form['scheduleForm']
                ? wp_strip_all_tags($form['scheduleEnd'])
                : '';
            $form['scheduleEndHour'] = $form['scheduleForm']
                ? GFCommon::int_range($form['scheduleEndHour'], 1, 12)
                : '';
            $form['scheduleEndMinute'] = $form['scheduleForm']
                ? GFCommon::int_range($form['scheduleEndMinute'], 1, 60)
                : '';
            $form['scheduleEndAmpm'] = $form['scheduleForm']
                ? GFCommon::whitelist($form['scheduleEndAmpm'], ['am', 'pm'])
                : '';
            $form['schedulePendingMessage'] = $form['scheduleForm']
                ? self::maybe_wp_kses($form['schedulePendingMessage'])
                : '';
            $form['scheduleMessage'] = $form['scheduleForm']
                ? self::maybe_wp_kses($form['scheduleMessage'])
                : '';
        }
        ...
        if (isset($form['requireLogin'])) {
            ...
            $form['requireLoginMessage'] = $form['requireLogin']
                ? self::maybe_wp_kses($form['requireLoginMessage'])
                : '';
        }
    ...
    public static function sanitize_conditional_logic($logic)
    {
        ...
        if (
            isset($logic['actionType']) &&
            !in_array($logic['actionType'], ['show', 'hide'])
        ) {
            $logic['actionType'] = 'show';
        }
        if (
            isset($logic['logicType']) &&
            !in_array($logic['logicType'], ['all', 'any'])
        ) {
            $logic['logicType'] = 'all';
        }
        ...
        if (isset($logic['rules']) && is_array($logic['rules'])) {
            foreach ($logic['rules'] as &$rule) {
                ...
                if (isset($rule['operator'])) {
                    $is_valid_operator = self::is_valid_operator(
                        $rule['operator']
                    );
                    $rule['operator'] = $is_valid_operator
                        ? $rule['operator']
                        : 'is';
                }
                ...
                if (isset($rule['value'])) {
                    ...
                    $rule['value'] = wp_kses_no_null($rule['value'], [
                        'slash_zero' => 'keep',
                    ]);
                    $rule['value'] = wp_kses_hook(
                        $rule['value'],
                        'post',
                        $allowed_protocols
                    );
                    $rule['value'] = wp_kses_split(
                        $rule['value'],
                        'post',
                        $allowed_protocols
                    );
    ...
    private static function maybe_wp_kses(
        $html,
        $allowed_html = 'post',
        $allowed_protocols = []
    ) {
        return GFCommon::maybe_wp_kses(
            $html,
            $allowed_html,
            $allowed_protocols
        );
     ...
     * @return GF_Field[]
     */
    public static function get_fields_by_type(
        $form,
        $types,
     ...
     */
    private static function evaluate_conditional_logic(
        $form,
        $logic,
        $field_values,
        $entry = null
        ...
        foreach ($logic['rules'] as $rule) {
            try {
                ...
                GFCommon::log_error(
                    __METHOD__ .
                        '(): Error from function hooked to gform_rule_pre_evaluation. ' .
                        $e->getMessage()
                );
            }
            $source_field = RGFormsModel::get_field($form, $rule['fieldId']);
            $field_value = empty($entry)
                ? self::get_field_value($source_field, $field_values)
                : self::get_lead_field_value($entry, $source_field);
            $is_value_match = self::is_value_match(
                $field_value,
                $rule['value'],
                $rule['operator'],
                $source_field,
        ...
        $is_hidden =
            ($do_action && $logic['actionType'] == 'hide') ||
            (!$do_action && $logic['actionType'] == 'show');
    ...
    {
        $files = GFCommon::json_decode(
            stripslashes(GFForms::post('gform_uploaded_files'))
        );
        if (!is_array($files)) {
            $files = [];
        }
                ...
                if (isset($upload_field[0]) && is_array($upload_field[0])) {
                    foreach ($upload_field as &$upload) {
                        if (isset($upload['temp_filename'])) {
                            $upload['temp_filename'] = sanitize_file_name(
                                wp_basename($upload['temp_filename'])
                            );
                        }
                        if (isset($upload['uploaded_filename'])) {
                            $upload['uploaded_filename'] = sanitize_file_name(
                                wp_basename($upload['uploaded_filename'])
                            );
                        }
                    }
                }
    ...
    public static function entry_exists($entry_id)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT count(id) FROM {$entry_table_name} WHERE id = %d",
            $entry_id
        );
...
{
    if (
        version_compare(GFFormsModel::get_database_version(), '2.3-dev-1', '<')
    ) {
...
function gform_get_meta_values_for_entries($entry_ids, $meta_keys)
{
    ...
    if (
        ...
        return GF_Forms_Model_Legacy::gform_get_meta_values_for_entries(
            $entry_ids,
            $meta_keys
        );
    }
    ...
    foreach ($results as $result) {
        foreach ($meta_keys as $meta_key) {
            $result->$meta_key =
                $result->$meta_key === null
                    ? false
                    : maybe_unserialize($result->$meta_key);
        }
    }
...
function gform_update_meta($entry_id, $meta_key, $meta_value, $form_id = null)
{
    ...
    if (
        ...
        GF_Forms_Model_Legacy::gform_update_meta(
            $entry_id,
            $meta_key,
            $meta_value,
            $form_id
    ...
    if ($meta_exists) {
        ...
        if (empty($form_id)) {
            ...
            $form_id = $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT form_id from $entry_table_name WHERE id=%d",
                    $entry_id
                )
            );
        } else {
...
function gform_add_meta($entry_id, $meta_key, $meta_value, $form_id = null)
{
    ...
    if (
        ...
        GF_Forms_Model_Legacy::gform_add_meta(
            $entry_id,
            $meta_key,
            $meta_value,
            $form_id
...
function gform_delete_meta($entry_id, $meta_key = '')
{
    ...
    $table_name = RGFormsModel::get_entry_meta_table_name();
    $meta_filter = empty($meta_key)
        ? ''
        : $wpdb->prepare('AND meta_key=%s', $meta_key);

    $wpdb->query(
        $wpdb->prepare(
            "DELETE FROM {$table_name} WHERE entry_id=%d {$meta_filter}",
            $entry_id
        )
    );
