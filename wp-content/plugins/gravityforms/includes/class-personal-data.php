...
class GF_Personal_Data
{
    ...
    public static function form_settings($form_id)
    {
        ...
        $action_url = admin_url(
            sprintf(
                'admin.php?page=gf_edit_forms&view=settings&subview=personal-data&id=%d',
                $form_id
            )
        );

        $enabled = (bool) rgars(
            $form_personal_data_settings,
            'exportingAndErasing/enabled'
        );

        $prevent_ip = (bool) rgar($form_personal_data_settings, 'preventIP');

        $retention_policy = rgars(
            $form_personal_data_settings,
            'retention/policy'
        );
        ...
        $retention_days = rgars(
            $form_personal_data_settings,
            'retention/retain_entries_days'
        );
        ...
        ?>
		<h3><span><i class="fa fa-lock"></i> <?php esc_html_e(
      'Personal Data',
      'gravityforms'
  ); ?>
		</h3>
		<div class="gform_panel gform_panel_form_settings" id="gf_personal_data_settings">
			<form action="<?php esc_url($action_url); ?>" method="POST">
				...
				<table class="gforms_form_settings" cellspacing="0" cellpadding="0">
					<tr>
						<td colspan="2">
							<h4 class="gf_settings_subgroup_title"><?php esc_html_e(
           'General Settings',
           'gravityforms'
       ); ?></h4>
						</td>
					...
					<tr>
						...
						<td>
							<label for="gf_personal_data_prevent_ip">
								...
								/>
								<?php esc_html_e(
            'Prevent the storage of IP addresses during form submission.',
            'gravityforms'
        ); ?>
							</label>
						</td>
					...
					<tr>
						...
						<td>
							...
							<div id="gf_personal_data_retain_entries_days_container" style="<?php echo $retention_policy !==
       'retain'
           ? ''
           : 'display:none'; ?>">
								<label for="gf_personal_data_retain_entries_days_container">
									<?php esc_html_e(
             'Number of days to retain entries before trashing/deleting:',
             'gravityforms'
         ); ?>
									<input
           ...
           'gravityforms'
					...
					<?php
     $identification_field_choices = [];

     $email_fields = GFAPI::get_fields_by_type($form, 'email');

     foreach ($email_fields as $email_field) {
         $identification_field_choices[(string) $email_field->id] =
             $email_field->label;
     }
      ...
      */
     $identification_field_choices = gf_apply_filters(
         ['gform_personal_data_identification_fields', $form['id']],
         $identification_field_choices,
         $form
     );
     ...
     $lookup_field = rgars(
         $form,
         'personalData/exportingAndErasing/identificationField'
     );
     ...
     if ($no_id) {
         if ($lookup_field == 'created_by') {
             ...
             $selected_field = GFFormsModel::get_field($form, $lookup_field)
         ) {
             $no_id = false;
     ...
     ?>
					<tr>
						...
						<td>
							<label for="gf_personal_data_enable">
								...
								/>
								<?php esc_html_e(
            'Enable integration with the WordPress tools for exporting and erasing personal data.',
            'gravityforms'
        ); ?>
							</label>
						</td>
					...
					<?php if (!$no_id) { ?>
						<tr class="gf_personal_data_settings" style="<?php echo $enabled
          ? ''
          : 'display:none;'; ?>">
							<th>
          ...
          : 'display:none;'; ?>">
						...
						<td>
							<table id="gf_personal_data_field_settings">
								<thead>
								<tr>
									<th class="gf_personal_data_field_label_title"><?php esc_html_e(
             'Fields',
             'gravityforms'
         ); ?></th>
									<th class="gf_personal_data_cb_title"><?php esc_html_e(
             'Export',
             'gravityforms'
         ); ?></th>
									<th class="gf_personal_data_cb_title"><?php esc_html_e(
        ...
        $all_column_settings = rgars(
            $form_personal_data_settings,
            'exportingAndErasing/columns'
        );
        ...
        if (!empty($custom_items)) { ?>
									...
									<?php
         $custom_items_settings = rgars(
             $form_personal_data_settings,
             'exportingAndErasing/custom'
         );
         ...
         foreach ($custom_items as $key => $custom_item_details) {
             ...
             ?>
										<tr>
											...
											<td class="gf_personal_data_cb_cell">
												<?php if (
                isset($custom_item_details['exporter_callback']) &&
                is_callable($custom_item_details['exporter_callback'])
            ) { ?>
													<input
														class="gf_personal_data_cb_export"
														type="checkbox"
											...
											<td class="gf_personal_data_cb_cell">
												<?php if (
                isset($custom_item_details['eraser_callback']) &&
                is_callable($custom_item_details['eraser_callback'])
            ) { ?>
													<input
														class="gf_personal_data_cb_erase"
														type="checkbox"
						...
						<tr>
							...
							<td>
								<div class="alert_red" style="padding:15px;">
									<?php esc_html_e(
             'You must add an email address field to the form in order to enable this setting.',
             'gravityforms'
         ); ?>
								</div>
							</td>
		...
		<script>
          ...
          __(
              'Warning: this will affect all entries that are older than the number of days specified.',
              'gravityforms'
          )
      ); ?> );
    ...
    public static function process_form_settings($form_id)
    {
        ...
        $retention_policy = in_array($retention_policy, [
            'retain',
            'trash',
            'delete',
        ])
        ...
        $form_personal_data_settings['retention'] = [
            'policy' => $retention_policy,
            'retain_entries_days' => $retain_entries_days,
        ];

        if (!isset($form_personal_data_settings['exportingAndErasing'])) {
            $form_personal_data_settings['exportingAndErasing'] = [];
        }

        $exporting_and_erasing =
            $form_personal_data_settings['exportingAndErasing'];

        if (!isset($exporting_and_erasing['columns'])) {
            $exporting_and_erasing['columns'] = [];
        }

        $exporting_and_erasing['enabled'] = (bool) rgpost(
            'exporting_and_erasing_enabled'
        );

        if (!$exporting_and_erasing['enabled']) {
            $exporting_and_erasing['identificationField'] = '';
        } else {
            $exporting_and_erasing['identificationField'] = sanitize_text_field(
                rgpost('identification_field')
            );
        }
        ...
        foreach ($fields as &$field) {
            $field->personalDataExport = (bool) rgar(
                $posted_export_fields,
                $field->id
            );
            $field->personalDataErase = (bool) rgar(
                $posted_erase_fields,
                $field->id
            );
        }
        ...
        if (!empty($custom_items)) {
            $custom_items_settings = rgar($exporting_and_erasing, 'custom')
                ? $exporting_and_erasing['custom']
                : [];
		...
		<div class="updated below-h2" id="after_update_dialog">
			<p>
				<strong><?php _e(
        'Personal data settings updated successfully.',
        'gravityforms'
    ); ?></strong>
			</p>
		</div>
    ...
    public static function get_entries($email_address, $page = 1, $limit = 50)
    {
        ...
        $all_conditions = call_user_func_array(
    ...
    public static function data_exporter($email_address, $page = 1)
    {
        ...
        if ($page == 1) {
            $export_data = self::get_draft_submissions_export_items(
                $email_address
            );
        }
        ...
        foreach ($entries as $entry) {
            ...
            $columns_settings = rgars(
                $forms,
                $form_id . '/personalData/exportingAndErasing/columns'
            );
            ...
            if (!empty($custom_items)) {
                $all_custom_settings = rgars(
                    $forms,
                    $form_id . '/personalData/exportingAndErasing/custom'
                );
                foreach (
                    $custom_items
                    as $custom_item_key => $custom_item_details
                ) {
                    $custom_settings = rgar(
                        $all_custom_settings,
                        $custom_item_key
                    );
                    if (
                        rgars($custom_settings, 'export') &&
                        isset($custom_item_details['exporter_callback']) &&
                        is_callable($custom_item_details['exporter_callback'])
                    ) {
                        $data[] = call_user_func(
                            $custom_item_details['exporter_callback'],
                            $form,
                            $entry
                        );
                    }
    ...
    public static function get_draft_submissions_export_items($email_address)
    {
        ...
        foreach ($draft_submissions as $i => $draft_submission) {
            ...
            $group_label = __(
                'Draft Forms (Save and Continue Later)',
                'gravityforms'
            );
            ...
            if (is_array($columns_settings)) {
                foreach ($columns_settings as $column_key => $column_settings) {
                    if (
                        rgar($column_settings, 'export') &&
                        isset($draft_submission[$column_key])
                    ) {
                        $data[] = [
                            'name' => $columns[$column_key],
    ...
    public static function data_eraser($email_address, $page = 1)
    {
        ...
        $items_removed =
            $page == 1
                ? self::erase_draft_submissions_data($email_address)
                : false;
        ...
        foreach ($entries as $entry) {
            ...
            if (!empty($custom_items)) {
                ...
                foreach (
                    ...
                    if (
                        rgars($custom_settings, 'erase') &&
                        isset($custom_item_details['eraser_callback']) &&
                        is_callable($custom_item_details['eraser_callback'])
                    ) {
                        call_user_func(
                            $custom_item_details['eraser_callback'],
                            $form,
                            $entry
                        );
                        $items_removed = true;
    ...
    public static function erase_field_values(
        $value,
        $entry_id,
        $input_id,
        $item_index = ''
        ...
        foreach ($draft_submissions as $i => $draft_submission) {
            ...
            if (
                ($field &&
                    ...
                    $entry[(string) $identification_field] ===
                        $email_address) ||
    ...
    public static function erase_draft_submissions_data($email_address)
    {
        ...
        foreach ($draft_entries as $draft_entry) {
            ...
            if ($entry_dirty) {
                ...
                GFFormsModel::update_draft_submission(
                    $resume_token,
                    $form,
                    $date_created,
                    $draft_entry['ip'],
                    $draft_entry['source_url'],
                    $submission_json
                );
                $items_removed = true;
            }
    ...
    public static function cron_task()
    {
        ...
        foreach ($forms as $form) {
            ...
            $retention_days = rgars(
                $form,
                'personalData/retention/retain_entries_days'
            );
        ...
        if (!empty($trash_conditions)) {
            ...
            $all_trash_conditions = [];

            $all_trash_conditions[] = call_user_func_array(
                ['GF_Query_Condition', '_or'],
                $trash_conditions
            );
            ...
            $entry_ids = $query
                ->from($trash_form_ids)
                ->where($all_trash_conditions)
                ->get_ids();

            self::log_debug(
                __METHOD__ . '(): trashing entries: ' . join(', ', $entry_ids)
            );
        ...
        if (!empty($delete_conditions)) {
            ...
            $all_delete_conditions = call_user_func_array(
                ['GF_Query_Condition', '_or'],
                $delete_conditions
            );

            $entry_ids = $query
                ->from($delete_form_ids)
                ->where($all_delete_conditions)
                ->get_ids();

            self::log_debug(
                __METHOD__ . '(): deleting entries: ' . join(', ', $entry_ids)
            );
