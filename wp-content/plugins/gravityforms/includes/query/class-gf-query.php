...
class GF_Query
{
    ...
    public function parse(
        ...
        $sort_dir = isset($sorting['direction'])
        ...
        $this->limit($page_size)->offset($offset);
        ...
        if (!empty($start_date)) {
            try {
                ...
                GFAPI::log_error(
                    __METHOD__ . '(): Invalid start_date; ' . $e->getMessage()
                );
            }
        }
        ...
        if (!empty($end_date)) {
            try {
                ...
                GFAPI::log_error(
                    __METHOD__ . '(): Invalid end_date; ' . $e->getMessage()
                );
            }
        }
        ...
        if (!empty($property_conditions)) {
            if (count($property_conditions) > 1) {
                $properties_condition = call_user_func_array(
        ...
        if (!empty($search_criteria['field_filters'])) {
            ...
            foreach ($field_filters as $filter) {
                ...
                if (empty($key)) {
                    $global_condition = $this->get_global_condition(
                        $filter,
                        $form_id
                    );
                ...
                if (
                    $field &&
                    $operator != GF_Query_Condition::LIKE &&
                    ($field->get_input_type() == 'number' ||
                        rgar($filter, 'is_numeric'))
                ) {
                    if (!is_numeric($value)) {
                ...
                if (is_array($value)) {
                    foreach ($value as &$v) {
                        $v =
                            $field && $field->storageType == 'json'
                                ? new GF_Query_JSON_Literal((string) $v)
                                : new GF_Query_Literal($v);
                    }
                ...
                if (
                    ...
                    if (!empty($value)) {
                        try {
                            ...
                            $date_created_start =
                                $search_date_str . ' 00:00:00';
                            $date_create_start_utc = get_gmt_from_date(
                                $date_created_start
                            );
                            $date_created_end = $search_date_str . ' 23:59:59';
                            $date_created_end_utc = get_gmt_from_date(
                                $date_created_end
                            );

                            $column =
                                count($from) > 1
                                    ? new GF_Query_Column($key)
                                    : new GF_Query_Column($key, $form_id);
                ...
                $literal =
                    $field && $field->storageType == 'json'
                        ? new GF_Query_JSON_Literal((string) $value)
            ...
            $condition_mode =
                strtolower($search_mode) == 'any' ? '_or' : '_and';
            if (count($filters) > 1) {
                $filters_condition = call_user_func_array(
                    ['GF_Query_Condition', $condition_mode],
                    $filters
                );
            } elseif ($filters) {
        ...
        if (!empty($properties_condition) && !empty($filters_condition)) {
            $where = call_user_func_array(
                ['GF_Query_Condition', '_and'],
                [$properties_condition, $filters_condition]
            );
        } else {
            $where = !empty($properties_condition)
    ...
    private function get_global_condition($filter, $form_ids)
    {
        ...
        switch ($original_operator) {
            ...
            case 'IN':
                ...
                break;
            default:
                $operator = empty($original_operator)
                    ? GF_Query_Condition::EQ
                    : $original_operator;
        }
        ...
        if (count($choice_filters) > 1) {
            $combine_operator = in_array($operator, [
            ...
            $condition = call_user_func_array(
                ['GF_Query_Condition', $combine_operator],
                $choice_filters
            );
        } else {
    ...
    {
        if (
            $on_column instanceof GF_Query_Column &&
            $to_column instanceof GF_Query_Column
        ) {
    ...
    private function query()
    {
        if (count($this->queries)) {
            GFCommon::log_debug(
                'Reusing GF_Query is undefined behavior. Create a new instance instead.'
            );
        }
        ...
        $from = sprintf(
            ...
            $this->_alias(null, reset($this->from))
        );
        ...
        if (
            !empty($this->order) &&
            ($orders = $this->_order_generate($this->order))
        ) {
        ...
        $join = count($joins)
            ? 'LEFT JOIN ' . implode(' LEFT JOIN ', $this->_prime_joins($joins))
            : '';
        ...
        $select = sprintf(
            ...
            implode(', ', $this->_select_infer($this->joins))
        );
        ...
        $sql = apply_filters(
            'gform_gf_query_sql',
            compact('select', 'from', 'join', 'where', 'order', 'paginate')
        );
    ...
    public function _select_infer($joins)
    {
        ...
        foreach ($joins as $join) {
            list($on, $column) = $join;
            if (
                !$on instanceof GF_Query_Column ||
                !$column instanceof GF_Query_Column
            ) {
                continue;
            }
            ...
            $alias = $this->_alias(
                $on->is_entry_column() ? null : $on->field_id,
                $on->source
            );
            $select[] = sprintf(
                '`%s`.`%s` AS `%s_id`',
                $alias,
                $on->is_entry_column() ? 'id' : 'entry_id',
                $alias
            );
        }
    ...
    public function _join_generate($joins)
    {
        ...
        foreach ($joins as $join) {
            ...
            if ($column->is_entry_column()) {
                 ...
                 */
                $_joins = array_merge(
                    $this->_join_infer($meta_condition),
                    $_joins
                );
            }
    ...
    public function _join_infer($condition)
    {
        ...
        foreach ($condition->columns as $column) {
            ...
            if (
                !$column->is_meta_column() &&
                $column->field_id != GF_Query_Column::META
            ) {
                if (
                    $column->field_id == intval($column->field_id) &&
                    ($field = GFFormsModel::get_field(
                        GFAPI::get_form(
                            $column->source
                                ? $column->source
                                : reset($this->from)
                     ...
                     */
                    $literal = new GF_Query_Literal(
                        sprintf('%d.%%', $column->field_id)
                    );
    ...
    public function _alias($field_id, $source = null, $prefix = 't')
    {
        if (
            't' == $prefix &&
            (!$source || empty($this->from) || in_array($source, $this->from))
        ) {
        ...
        if (!isset($this->aliases[$source_id])) {
            $this->aliases[$source_id] = sprintf(
                '%s%d',
                $prefix,
                count($this->aliases) + 1
            );
        }
    ...
    public function _where_unwrap($condition)
    {
        ...
        if (
            ...
            $conditions = array_filter(
                array_map([$this, __FUNCTION__], $condition->expressions),
                'strlen'
            );
            ...
            if (count($conditions)) {
                return count($conditions) == 1
                    ? current($conditions)
                    : '(' .
                            implode(" {$condition->operator} ", $conditions) .
                            ')';
            }
    ...
    public function get_entries($entry_ids)
    {
        ...
        $sql = sprintf(
            "SELECT * from $entry_table WHERE id IN(%s)",
        ...
        foreach ($entryset as $entry) {
            ...
            if (isset($cache['form_entry_meta'][$form_id])) {
                ...
                $entry_meta = $cache['form_entry_meta'][
                    $form_id
                ] = RGFormsModel::get_entry_meta($form_id);
            }
            if (!empty($entry_meta)) {
                $entry_meta_placeholders = implode(
                    ',',
                    array_fill(0, count($entry_meta), '%s')
                );
                $sql = sprintf(
                    '( form_id = %d AND meta_key IN (%s) )',
                    $form_id,
                    $entry_meta_placeholders
                );
                if (!isset($meta_clauses[$form_id])) {
                    $meta_clauses[$form_id] = $wpdb->prepare(
                        $sql,
                        array_keys($entry_meta)
                    );
                }
            }
        }

        $meta_clauses_str = empty($meta_clauses)
            ? ''
            : sprintf('OR (%s)', join(' OR ', $meta_clauses));
...
",
        ...
        foreach ($metaset as $meta) {
            $entries[$meta['entry_id']][
                $meta['meta_key'] . $meta['item_index']
            ] = $meta['meta_value'];
        }
        ...
        foreach ($entryset as $entry) {
            if (isset($cache['form_meta'][$entry['form_id']])) {
                ...
                $form = $cache['form_meta'][
                    $entry['form_id']
                ] = RGFormsModel::get_form_meta($entry['form_id']);
            }

            $openssl_encrypted_fields = GFFormsModel::get_openssl_encrypted_fields(
                $entry['id']
            );
                ...
                if (is_array($inputs)) {
                    foreach ($inputs as $input) {
                        $entries[$entry['id']][
                            ...
                            rgar($entries[$entry['id']], (string) $input['id']),
                            $entry,
                            $field,
                            $input['id']
                        );
                    ...
                    if (
                        in_array((string) $field->id, $openssl_encrypted_fields)
                    ) {
            ...
            foreach (array_keys($entry_meta) as $meta_key) {
                if (isset($entries[$entry['id']][$meta_key])) {
                    $entries[$entry['id']][$meta_key] = maybe_unserialize(
                        $entries[$entry['id']][$meta_key]
                    );
                } else {
    ...
    private function set_sub_field_values(
        $field,
        $db_values,
        $sub_field_values,
        $form,
