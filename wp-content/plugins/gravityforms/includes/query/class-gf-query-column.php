...
class GF_Query_Column
{
    ...
    public function sql($query)
    {
        ...
        if ($this->is_entry_column()) {
            return sprintf(
                ...
                $this->alias
                    ? $this->alias
                    : $query->_alias(null, $this->source),
                $this->field_id
            );
        } elseif ($this->is_meta_column()) {
            return sprintf(
                '`%s`.`%s`',
                $this->alias
                    ? $this->alias
                    : $query->_alias($this->field_id, $this->source, 'm'),
                $this->field_id
            );
        }
    ...
    public function is_entry_column()
    {
        ...
        static $entry_columns = [];
        if (empty($entry_columns)) {
            global $wpdb;
            $entry_columns = wp_list_pluck(
                $wpdb->get_results(
                    'SHOW COLUMNS FROM ' . GFFormsModel::get_entry_table_name(),
                    ARRAY_A
                ),
                'Field'
            );
    ...
    public function is_nullable_entry_column()
    {
        ...
        static $nullable_entry_columns = [];
        if (empty($nullable_entry_columns)) {
            global $wpdb;
            $nullable_entry_columns = wp_list_pluck(
                $wpdb->get_results(
                    'SHOW COLUMNS FROM ' .
    ...
    public function is_meta_column()
    {
        ...
        return in_array($this->field_id, ['meta_key', 'meta_value']);
    }
