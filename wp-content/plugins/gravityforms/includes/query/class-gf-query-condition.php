...
class GF_Query_Condition
{
    ...
    public function __construct($left = null, $operator = null, $right = null)
    {
         ...
         */
        if (
            self::is_valid_expression_type($left) &&
            !$left instanceof GF_Query_Series
        ) {
            $this->_left = $left;
        }
    ...
    public function sql($query)
    {
        ...
        if ($this->operator && $this->left && $this->right) {
             ...
             */
            if (
                $this->left instanceof GF_Query_Column &&
                (!$this->left->is_entry_column() &&
                    !$this->left->is_meta_column()) &&
                $this->left->field_id
            ) {
                /**
                 ...
                 */
                if (
                    is_numeric($this->left->field_id) &&
                    intval($this->left->field_id) == $this->left->field_id
                ) {
                    if (
                        ($field = GFFormsModel::get_field(
                            GFAPI::get_form($this->left->source),
                            $this->left->field_id
                        )) &&
                         ...
                         */
                        if (
                            in_array($this->operator, [self::EQ, self::NEQ]) &&
                            $this->right instanceof GF_Query_Series
                        ) {
                            $compare_conditions = [];
                            foreach ($this->right->values as $literal) {
                                $subquery = $wpdb->prepare(
                                    sprintf(
                                        "SELECT 1 FROM `%s` WHERE `meta_key` LIKE %%s AND `meta_value` = %s AND `entry_id` = `%s`.`id`",
                                ...
                                $compare_conditions[] = $compare_condition->sql(
                                    $query
                                );
                            }
                            ...
                            $subquery = $wpdb->prepare(
                                sprintf(
                                    "SELECT COUNT(1) FROM `%s` WHERE `meta_key` LIKE %%s AND `entry_id` = `%s`.`id`",
                                    GFFormsModel::get_entry_meta_table_name(),
                             ...
                             */
                            $compare_conditions[] = sprintf(
                                "(%s) %s %d",
                                $subquery,
                                $this->operator == self::NEQ ? '!=' : '=',
                                count($compare_conditions)
                            );
                            return sprintf(
                                "(%s)",
                                implode(
                            ...
                            in_array($this->operator, [self::IN, self::NIN]) &&
                            $this->right instanceof GF_Query_Series
                        ) {
                            $subquery = $wpdb->prepare(
                                sprintf(
                                    "SELECT 1 FROM `%s` WHERE `meta_key` LIKE %%s AND `meta_value` IN (%s) AND `entry_id` = `%s`.`id`",
                                    GFFormsModel::get_entry_meta_table_name(),
                                    str_replace(
                                        '%',
                                        '%%',
                            ...
                            $subquery = $wpdb->prepare(
                                sprintf(
                                    "SELECT 1 FROM `%s` WHERE `meta_key` LIKE %%s AND `meta_value` %s %s AND `entry_id` = `%s`.`id`",
                                    GFFormsModel::get_entry_meta_table_name(),
                                    $operator,
                                    str_replace(
                                        '%',
                ...
                if (
                    (in_array($this->operator, [self::NIN, self::NBETWEEN]) &&
                        !in_array(
                    ...
                    $subquery = $wpdb->prepare(
                        sprintf(
                            "SELECT 1 FROM `%s` WHERE `meta_key` = %%s AND `entry_id` = `%s`.`id`",
                            GFFormsModel::get_entry_meta_table_name(),
                            $query->_alias(null, $this->left->source)
                        ),
                        $this->left->field_id
                    ...
                    $not_exists = new self(
                        new GF_Query_Call('NOT EXISTS', [$subquery])
                    );
                    return $query->_where_unwrap(
                        self::_or($not_exists, $compare_condition)
                    );
                }
            ...
            if (
                ($left = $this->left_sql($query)) &&
                ($right = $this->right_sql($query))
            ) {
                if (
                    in_array($this->operator, [self::NBETWEEN, self::BETWEEN])
                ) {
                ...
                if (
                    $this->left instanceof GF_Query_Column &&
                    $this->left->is_nullable_entry_column()
                ) {
                    if (
                        ($this->operator == self::EQ &&
                            empty($this->right->value)) ||
                        ($this->operator == self::NEQ &&
                            !empty($this->right->value))
                    ) {
                ...
                if (
                    ...
                    if (
                        $query->is_multisource() &&
                        $this->left->field_id != 'form_id'
                    ) {
        ...
        if (
            $this->left &&
            ($this->left instanceof GF_Query_Call ||
                $this->left instanceof GF_Query_Column)
        ) {
    ...
    {
        return $expression instanceof GF_Query_Literal ||
    ...
    private function right_sql($query)
    {
         ...
         */
        if (in_array($this->operator, [self::IN, self::NIN])) {
             ...
             * BETWEEN
             */
        } elseif (in_array($this->operator, [self::BETWEEN, self::NBETWEEN])) {
            if (!$this->right instanceof GF_Query_Series) {
                return '';
            }

            return $this->right->sql($query, ' AND ');
        } elseif (in_array($this->operator, [self::IS, self::ISNOT])) {
    ...
    private function left_sql($query)
    {
        if ($this->left instanceof GF_Query_Call) {
            ...
            if ($columns) {
                ...
                return implode(
                    ' AND ',
                    array_merge($meta_key_conditions, [
                        $this->left->sql($query),
                    ])
                );
            }
