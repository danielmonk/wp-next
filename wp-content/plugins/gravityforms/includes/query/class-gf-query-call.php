...
class GF_Query_Call
{
    ...
    public function sql($query)
    {
        if (method_exists($this, strtolower($this->function_name) . '_sql')) {
            return call_user_func(
                [$this, strtolower($this->function_name) . '_sql'],
                $query
            );
        }
        return sprintf(
    ...
    public static function CAST($column, $type)
    {
        ...
        return new self('CAST', [$column, $type]);
    }
    ...
    private function cast_sql($query)
    {
        ...
        if (
            !in_array($type, [
                GF_Query::TYPE_SIGNED,
                GF_Query::TYPE_UNSIGNED,
                GF_Query::TYPE_DECIMAL,
            ])
        ) {
            return '';
        }
        ...
        return sprintf(
            'CAST(`%s`.`%s` AS %s)',
            $query->_alias($column->field_id, $column->source, 'c'),
            $id,
            $type
        );
    }
