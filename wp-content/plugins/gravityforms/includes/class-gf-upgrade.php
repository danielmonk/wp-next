...
class GF_Upgrade
{
     ...
     * @var array
     */
    private $manual_upgrade_versions = [];
    ...
    public function maybe_display_wizard()
    {
        ...
        if ($this->requires_install_wizard()) {
            ...
            require_once GFCommon::get_base_path() .
                '/includes/wizard/class-gf-upgrade-wizard.php';
    ...
    public function maybe_upgrade()
    {
        ...
        if ($this->requires_install()) {
            ...
            $this->requires_upgrade() &&
            !$this->requires_upgrade_wizard()
        ) {
    ...
    {
        $versions = $this->get_versions();
        $is_downgrading = version_compare(
            $versions['version'],
        ...
        if ($is_downgrading) {
            ...
            $versions['current_version'] = $this->get_wp_option(
                'rg_form_version'
            );
    ...
    public function upgrade($from_db_version = null, $force_upgrade = false)
    {
        ...
        if ($from_db_version === null) {
            $from_db_version = empty($versions['current_db_version'])
                ? $versions['current_version']
    ...
    public function is_upgrading()
    {
        ...
        $is_upgrading = $wpdb->get_var(
            "SELECT option_value FROM {$wpdb->options} WHERE option_name='gf_upgrade_lock'"
        );

        return $is_upgrading ? true : false;
    }

    private function set_upgrade_started(
        $from_db_version = null,
        $force_upgrade = false
    ) {
        global $wpdb;
        ...
        if ($lock_params && !$force_upgrade) {
            GFCommon::log_debug(
                __METHOD__ . '(): Upgrade in process. Aborting'
            );
        ...
        if ($insert) {
            ...
            $sql = $wpdb->prepare(
                "INSERT INTO {$wpdb->options}(option_name, option_value) VALUES('gf_upgrade_lock', %s) ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`)",
                $lock_params_serialized
            );
            ...
            $sql = $wpdb->prepare(
                "UPDATE {$wpdb->options} SET option_value=%s WHERE option_name='gf_upgrade_lock'",
                $lock_params_serialized
            );
        ...
        $system_status_link_open = sprintf(
            '<a href="%s">',
            admin_url('admin.php?page=gf_system_status')
        );
        ...
        $message = sprintf(
            esc_html__(
                'Gravity Forms is currently upgrading the database to version %1$s. For sites with a large number of entries this may take a long time. Check the %2$sSystem Status%3$s page for further details.',
                'gravityforms'
            ),
            GFForms::$version,
            $system_status_link_open,
            $system_status_link_close
        );
    ...
    public function check_table_schema($table_name)
    {
        ...
        if (empty($to_update)) {
            ...
            GFCommon::log_debug(
                __METHOD__ . '(): Failed queries: ' . print_r($to_update, true)
            );
    ...
    public function dbDelta($sql, $execute = true)
    {
        ...
        add_filter('dbdelta_create_queries', [$this, 'dbdelta_fix_case']);

        $result = dbDelta($sql, $execute);

        remove_filter('dbdelta_create_queries', [$this, 'dbdelta_fix_case']);
    ...
    protected function post_upgrade_schema($from_db_version, $force_upgrade)
    {
        ...
        $current_db_version = empty($from_db_version)
            ? $versions['current_version']
            : $from_db_version;

        //-- Data Upgrade Process Start --//

        $this->update_upgrade_status(
            esc_html__('Queued for upgrade.', 'gravityforms')
        );
        ...
        if (version_compare($current_db_version, '2.3-dev-1', '<')) {
            GFForms::$background_upgrader->push_to_queue([
                $this,
		...
		*/
        ...
        if (GFForms::$background_upgrader->get_data()) {
            ...
            GFCommon::log_debug(
                __METHOD__ .
                    '(): Background upgrade not necessary. Setting new version.'
            );
            $this->update_db_version();
    ...
    public function gf_upgrade_230_migrate_forms()
    {
        global $wpdb;
        $this->update_upgrade_status(
            esc_html__('Migrating forms.', 'gravityforms')
        );
...
WHERE lfv.id NOT IN
      ( SELECT id
        ...
        $this->update_upgrade_status(
            esc_html__('Forms migrated.', 'gravityforms')
        );
        return false;
    }
        ...
        do {
            ...
            if ($wpdb->last_error) {
                ...
                $this->update_upgrade_status(
                    sprintf(
                        esc_html__(
                            'Error Migrating Entry Headers: %s',
                            'gravityforms'
                        ),
                        $wpdb->last_error
                    )
...
WHERE l2.id NOT IN ( SELECT e.id FROM {$entry_table} e )";
                        ...
                        if ($remaining > 0) {
                            $this->update_upgrade_status(
                                sprintf(
                                    esc_html__(
                                        'Migrating leads. Step 1/3 Migrating entry headers. %d rows remaining.',
                                        'gravityforms'
                                    ),
                                    $remaining
                                )
        ...
        do {
            ...
            if ($wpdb->last_error) {
                ...
                $this->update_upgrade_status(
                    sprintf(
                        esc_html__(
                            'Error Migrating Entry Details: %s',
                            'gravityforms'
                        ),
                        $wpdb->last_error
                    )
...
WHERE ld.id NOT IN ( SELECT em.id FROM {$entry_meta_table} em )";
                    ...
                    if ($remaining > 0) {
                        $this->update_upgrade_status(
                            sprintf(
                                esc_html__(
                                    'Migrating leads. Step 2/3 Migrating entry details. %d rows remaining.',
                                    'gravityforms'
                                ),
                                $remaining
                            )
...
WHERE NOT EXISTS
      (SELECT * FROM {$entry_meta_table} em WHERE em.entry_id = lm.lead_id AND CONVERT(em.meta_key USING {$charset_db}) = CONVERT(lm.meta_key USING {$charset_db}) {$collate})";
                    ...
                    if ($remaining > 0) {
                        $this->update_upgrade_status(
                            sprintf(
                                esc_html__(
                                    'Migrating leads. Step 3/3 Migrating entry meta. %d rows remaining.',
                                    'gravityforms'
                                ),
                                $remaining
                            )
        ...
        $this->update_upgrade_status(
            esc_html__('Entry details migrated.', 'gravityforms')
        );
        return false;
    }
    ...
    public function gf_upgrade_230_migrate_incomplete_submissions()
    {
        ...
        $this->update_upgrade_status(
            esc_html__('Migrating incomplete submissions.', 'gravityforms')
        );
...
WHERE CONVERT(insub.uuid USING {$charset_db}) {$collate} NOT IN
      ( SELECT uuid FROM {$draft_submissions_table} )";
        ...
        if ($wpdb->last_error) {
            ...
            $this->update_upgrade_status(
                sprintf(
                    esc_html__(
                        'Error Migrating incomplete submissions: %s',
                        'gravityforms'
                    ),
                    $wpdb->last_error
                )
    ...
    public function gf_upgrade_230_migrate_lead_notes()
    {
        ...
        $this->update_upgrade_status(
            esc_html__('Migrating entry notes.', 'gravityforms')
        );
...
WHERE ln.id NOT IN
    ...
    protected function maybe_populate_keys()
    {
        global $gf_license_key;
        $license_key =
            defined('GF_LICENSE_KEY') && empty($gf_license_key)
                ? GF_LICENSE_KEY
                : $gf_license_key;
        ...
        $private_key =
            defined('GF_RECAPTCHA_PRIVATE_KEY') &&
            empty($gf_recaptcha_private_key)
        ...
        $public_key =
            defined('GF_RECAPTCHA_PUBLIC_KEY') &&
            empty($gf_recaptcha_public_key)
        ...
        if (defined('GF_THEME_IMPORT_FILE')) {
            ...
            if (!isset($themes[$theme])) {
                ...
                GFExport::import_file(
                    get_stylesheet_directory() . '/' . GF_THEME_IMPORT_FILE
                );
    ...
    private function fix_leading_and_trailing_spaces()
    {
        ...
        $result = $wpdb->query(
            "UPDATE {$lead_details_table} SET value = TRIM(value)"
        );

        $results = $wpdb->get_results(
            "SELECT form_id, display_meta, confirmations, notifications FROM {$meta_table_name}",
            ARRAY_A
        );
        ...
        foreach ($results as &$result) {
            ...
            $confirmations = GFFormsModel::unserialize(
                $result['confirmations']
            );
            $confirmations_updated = false;
            $confirmations = GFFormsModel::trim_conditional_logic_values(
                $confirmations,
                $form,
            ...
            $notifications = GFFormsModel::unserialize(
                $result['notifications']
            );
            $notifications_updated = false;
            $notifications = GFFormsModel::trim_conditional_logic_values(
                $notifications,
                $form,
    ...
    public function drop_index($table, $index)
    {
        ...
        if (
            !GFFormsModel::is_valid_table($table) ||
            !GFFormsModel::is_valid_index($index)
        ) {
            return;
        }
...
SET d.value = l.value");
        ...
        GFCommon::log_debug(
            __METHOD__ . '(): result: ' . print_r($result, true)
        );
    }
    ...
    {
        if (
            rgar($hook_extra, 'plugin') == 'gravityforms/gravityforms.php' &&
            !$this->has_database_permission($error)
        ) {
    ...
    public function has_database_permission(&$error)
    {
        ...
        if ($has_permission) {
            $sql =
                "ALTER TABLE {$wpdb->prefix}rg_test ADD COLUMN a" .
                uniqid() .
                ' int';
            $wpdb->query($sql);
            $error =
                'Current database user does not have necessary permissions to modify (ALTER) tables. Gravity Forms requires that the database user has CREATE and ALTER permissions. If you need assistance in changing database user permissions, contact your hosting provider.';
            if (!empty($wpdb->last_error)) {
                $has_permission = false;
            }
    ...
    private function can_upgrade_longtext()
    {
        ...
        $is_longtext = $this->check_column(
            $lead_detail_table_name,
            'value',
            'longtext'
        );

        $first_entry_value = $wpdb->get_results(
            "SELECT value FROM $lead_detail_table_name LIMIT 1"
        );
        ...
        if (!$is_longtext) {
            if ($col_type == '252' || $col_type == 'blob') {
                ...
                $result = $wpdb->query(
                    "ALTER TABLE {$lead_detail_table} MODIFY `value` LONGTEXT;"
                );
                if (empty($wpdb->last_error)) {
                    $is_longtext = true;
                } else {
        ...
        if (!$is_longtext) {
            ...
            GFCommon::log_debug(
                __METHOD__ . '(): lead detail value column issue'
            );
        ...
        if (
            version_compare($current_version, '2.0-beta-3.2', '<') || // No upgrades have been attempted.
...
HAVING count(*) > 1;");
            ...
            if (count($results) == 0) {
                ...
                GFCommon::add_dismissible_message(
                    esc_html__(
                        'There appears to be an issue with the data in the Gravity Forms database tables. Please get in touch with support.',
                        'gravityforms'
                    ),
                    'gform_long_table_upgrade',
                    'error',
    ...
    public function get_versions()
    {
        ...
        $this->versions = [
            ...
            'previous_db_version' => empty($previous_db_version)
                ? '0'
                : $previous_db_version,
        ];
    ...
    public function requires_install_wizard()
    {
        ...
        $pending_installation =
            get_option('gform_pending_installation') ||
            isset($_GET['gform_installation_wizard']);
        ...
        $install_wizard_required =
            $this->requires_install() || $pending_installation;
    ...
    public function get_upgrade_lock()
    {
        ...
        $lock_params_serialized = $wpdb->get_var(
            "SELECT option_value FROM {$wpdb->options} WHERE option_name='gf_upgrade_lock'"
        );
    ...
    public function get_submissions_block()
    {
        ...
        $timestamp = $wpdb->get_var(
            "SELECT option_value FROM {$wpdb->options} WHERE option_name='gf_submissions_block'"
        );
    ...
    public function add_post_upgrade_admin_notices()
    {
        ...
        if ($number_outdated == 1) {
            ...
            $message = sprintf(
                esc_html__(
                    'The %s is not compatible with this version of Gravity Forms. See the plugins list for further details.',
                    'gravityforms'
                ),
                $outdated[0]
            );
            ...
            $message = sprintf(
                esc_html__(
                    'There are %d add-ons installed that are not compatible with this version of Gravity Forms. See the plugins list for further details.',
                    'gravityforms'
                ),
                $number_outdated
            );
    ...
    public function get_min_addon_requirements()
    {
        return [
            ...
            'gravityformspaypalpro/paypalpro.php' => [
