...
if (!class_exists('GF_Background_Process')) {
    ...
    abstract class GF_Background_Process extends WP_Async_Request
    {
        ...
        {
            GFCommon::log_debug(
                sprintf('%s(): Running for %s.', __METHOD__, $this->action)
            );

            if ($this->is_queue_empty()) {
                $this->clear_scheduled_event();

                $dispatched = new WP_Error(
                    'queue_empty',
            ...
            if (is_wp_error($dispatched)) {
                GFCommon::log_debug(
                    sprintf(
                        '%s(): Unable to dispatch tasks to Admin Ajax: %s',
                        __METHOD__,
        ...
        {
            $running = false;
            $lock_timestamp = get_site_option(
        ...
        {
            GFCommon::log_debug(
                sprintf('%s(): Running for blog #%s.', __METHOD__, $blog_id)
            );
        ...
        protected function time_exceeded()
        {
            $finish =
                $this->start_time +
                apply_filters($this->identifier . '_default_time_limit', 20); // 20 seconds
            $return = false;
        ...
        public function schedule_cron_healthcheck($schedules)
        {
            ...
            $schedules[$this->identifier . '_cron_interval'] = [
                ...
                'display' => sprintf(
                    __('Every %d Minutes', 'gravityforms'),
                    $interval
                ),
            ];
