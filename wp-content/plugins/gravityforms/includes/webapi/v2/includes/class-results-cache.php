...
class GF_Results_Cache
{
    ...
    private function schedule_results_cron($form_id, $search_criteria, $args)
    {
        ...
        $cron_args = [$form_id, $search_criteria, $args];
    ...
    public function calculate(
        ...
        $count_search_entries = GFAPI::count_entries(
            $form_id,
            $search_criteria
        );
        ...
        while ($entries_left > 0) {
            ...
            $search_entries_time =
                $search_entries_time_end - $search_entries_time_start;
            ...
            foreach ($entries as $entry) {
                ...
                foreach ($fields as $field) {
                    ...
                    if (
                        ...
                        $field_data[$field->id][
                            'sum_of_scores'
                        ] += $this->get_likert_score($field, $entry);
                    }
                }
                $entries_processed++;
                $entry_time_end = microtime(true);
                $total_execution_time =
                    $entry_time_end - $search_entries_time_start;
                $entry_execution_time = $entry_time_end - $entry_time_start;
                if (
                    $total_execution_time + $entry_execution_time >
                    $max_execution_time
                ) {
                    break;
                }
            ...
            if (
                isset($callbacks['calculation']) &&
                is_callable($callbacks['calculation'])
            ) {
                $data = call_user_func(
            ...
            if (
                $entries_left > 0 &&
                $execution_time + $search_entries_time > $max_execution_time
            ) {
                $data['status'] = 'incomplete';
                $data['offset'] = $offset;
                $progress =
                    $data['entry_count'] > 0
                        ? round(($data['offset'] / $data['entry_count']) * 100)
                        : 0;
    ...
    private function get_labels($form, $args)
    {
        ...
        foreach ($fields as $field) {
            ...
            if (is_array($field->choices)) {
                ...
                if (
                    $field instanceof GF_Field_Likert &&
                    $field->gsurveyLikertEnableMultipleRows
                ) {
