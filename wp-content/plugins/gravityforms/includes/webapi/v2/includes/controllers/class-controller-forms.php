...
class GF_REST_Forms_Controller extends GF_REST_Controller
{
    ...
    public function delete_item($request)
    {
        ...
        if (empty($form)) {
            return new WP_Error(
                'gf_form_invalid_id',
                __('Invalid form id.', 'gravityforms'),
                ['status' => 404]
            );
        }
        ...
        if ($force) {
            ...
            if (rgar($form, 'is_trash')) {
                $message = __(
                    'The form has already been deleted.',
                    'gravityforms'
                );
        ...
        if (!$form_json) {
            ...
            if (empty($form_json) || is_array($form_json)) {
                return new WP_Error(
                    'missing_form',
                    __(
                        'The Form object must be sent as a JSON string in the request body with the content-type header set to application/json.',
                        'gravityforms'
                    )
                );
            }
        }
        $form = is_string($form_json)
            ? json_decode($form_json, true)
            : $form_json;
