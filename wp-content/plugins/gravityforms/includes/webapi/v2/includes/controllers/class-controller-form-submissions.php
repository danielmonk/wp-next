...
class GF_REST_Form_Submissions_Controller extends GF_REST_Controller
{
    ...
    public function create_item($request)
    {
        ...
        if (empty($params)) {
            ...
            $field_values = isset($input_values['field_values'])
                ? $input_values['field_values']
                : [];
            $target_page = isset($input_values['target_page'])
                ? $input_values['target_page']
                : 0;
            $source_page = isset($input_values['source_page'])
                ? $input_values['source_page']
                : 1;
            $input_values = []; // The input values are already in $_POST
        } else {
            $input_values = $params;
            $field_values = isset($params['field_values'])
                ? $params['field_values']
                : [];
            $target_page = isset($params['target_page'])
                ? $params['target_page']
                : 0;
            $source_page = isset($params['source_page'])
                ? $params['source_page']
                : 1;
        }
        ...
        if (
            isset($result['confirmation_type']) &&
