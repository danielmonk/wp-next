...
class GF_REST_Form_Feeds_Controller extends GF_REST_Controller
{
    ...
    public function create_item($request)
    {
        ...
        $response->header(
            'Location',
            rest_url(sprintf('%s/%s/%d', $this->namespace, $base, $feed_id))
        );
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        if (empty($feed)) {
            return new WP_Error(
                'missing_feed',
                __('Missing feed JSON', 'gravityforms')
            );
        }
        ...
        if (empty($addon_slug)) {
            return new WP_Error(
                'missing_addon_slug',
                __('Missing add-on slug', 'gravityforms')
            );
        }
        ...
        if (empty($feed['meta'])) {
            return new WP_Error(
                'missing_feed_meta',
                __('Missing feed meta', 'gravityforms')
            );
        }
    ...
    public function get_item_schema()
    {
        $schema = [
            ...
            'properties' => [
                ...
                'meta' => [
                    'description' => __(
                        'The JSON string containing the feed meta.',
                        'gravityforms'
                    ),
                    'type' => 'object',
                    'readonly' => false,
                ...
                'addon_slug' => [
                    'description' => __(
                        'The add-on the feed belongs to.',
                        'gravityforms'
                    ),
                    'type' => 'integer',
                    'readonly' => true,
