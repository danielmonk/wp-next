...
abstract class GF_REST_Controller extends WP_REST_Controller
{
    ...
    public function parse_entry_search_params($request)
    {
        ...
        $sort_key =
            isset($sorting_param['key']) && !empty($sorting_param['key'])
                ? $sorting_param['key']
                : 'id';
        $sort_dir =
            isset($sorting_param['direction']) &&
        ...
        $page_size = isset($paging_param['page_size'])
            ? intval($paging_param['page_size'])
            : 10;
        if (isset($paging_param['current_page'])) {
            $current_page = intval($paging_param['current_page']);
            $offset = $page_size * ($current_page - 1);
        } else {
            $offset = isset($paging_param['offset'])
                ? intval($paging_param['offset'])
                : 0;
        }
    ...
    public function is_field_value_json($field)
    {
        ...
        if (in_array($input_type, ['multiselect', 'list'])) {
            return true;
        }
        ...
        if (!empty($form['fields']) && is_array($form['fields'])) {
            foreach ($form['fields'] as $field) {
                ...
                if (
                    ...
                    $new_list_value = self::maybe_decode_json(
                        $entry[$field->id]
                    );
    ...
    public function maybe_json_encode_applicable_fields($entry)
    {
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (
                $field->get_input_type() === 'fileupload' &&
                $field->multipleFiles
            ) {
    ...
    public function current_user_can_any($capability, $request)
    {
        ...
        if (!$this->_validate_caps_logged) {
            $this->log_debug(
                sprintf(
                    '%s(): method: %s; route: %s; capability: %s; result: %s.',
                    __METHOD__,
                    $request->get_method(),
                    $request->get_route(),
