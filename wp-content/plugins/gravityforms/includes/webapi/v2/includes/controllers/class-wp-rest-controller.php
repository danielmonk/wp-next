...
abstract class WP_REST_Controller
{
    ...
    public function prepare_response_for_collection($response)
    {
        ...
        if (method_exists($server, 'get_compact_response_links')) {
            $links = call_user_func(
                [$server, 'get_compact_response_links'],
                $response
            );
        } else {
    ...
    public function filter_response_by_context($data, $context)
    {
        ...
        foreach ($data as $key => $value) {
            if (
                empty($schema['properties'][$key]) ||
                empty($schema['properties'][$key]['context'])
            ) {
                continue;
            }
            ...
            if (
                'object' === $schema['properties'][$key]['type'] &&
                !empty($schema['properties'][$key]['properties'])
            ) {
                foreach (
    ...
    public function get_endpoint_args_for_item_schema(
        ...
        foreach ($schema_properties as $field_id => $params) {
            ...
            if (isset($params['description'])) {
                $endpoint_args[$field_id]['description'] =
                    $params['description'];
            }
            ...
            foreach (['type', 'format', 'enum'] as $schema_prop) {
                if (isset($params[$schema_prop])) {
                    $endpoint_args[$field_id][$schema_prop] =
                        $params[$schema_prop];
                }
            }
...
if (!function_exists('rest_validate_request_arg')) {
    ...
    function rest_validate_request_arg($value, $request, $param)
    {
        ...
        if ('integer' === $args['type'] && !is_numeric($value)) {
            return new WP_Error(
                'rest_invalid_param',
                sprintf(__('%s is not of type %s'), $param, 'integer')
            );
        }
        ...
        if ('string' === $args['type'] && !is_string($value)) {
            return new WP_Error(
                'rest_invalid_param',
                sprintf(__('%s is not of type %s'), $param, 'string')
            );
        }
            ...
            switch ($args['format']) {
                case 'date-time':
                    if (!rest_parse_date($value)) {
                        return new WP_Error(
                            'rest_invalid_date',
                            __('The date you provided is invalid.')
                        );
                    }
                    break;
                ...
                case 'email':
                    if (!is_email($value)) {
                        return new WP_Error(
                            'rest_invalid_email',
                            __('The email address you provided is invalid.')
                        );
                    }
                    break;
            }
        ...
        if (
            in_array($args['type'], ['numeric', 'integer']) &&
            ...
            if (isset($args['minimum']) && !isset($args['maximum'])) {
                if (
                    ...
                    return new WP_Error(
                        'rest_invalid_param',
                        sprintf(
                            __('%s must be greater than %d (exclusive)'),
                            $param,
                            $args['minimum']
                        )
                    );
                    ...
                    return new WP_Error(
                        'rest_invalid_param',
                        sprintf(
                            __('%s must be greater than %d (inclusive)'),
                            $param,
                            $args['minimum']
                        )
                    );
                ...
                if (
                    ...
                    return new WP_Error(
                        'rest_invalid_param',
                        sprintf(
                            __('%s must be less than %d (exclusive)'),
                            $param,
                            $args['maximum']
                        )
                    );
                    ...
                    return new WP_Error(
                        'rest_invalid_param',
                        sprintf(
                            __('%s must be less than %d (inclusive)'),
                            $param,
                            $args['maximum']
                        )
                    );
                ...
                if (
                    ...
                    if (
                        ...
                        return new WP_Error(
                            ...
                            sprintf(
                                __(
                                    '%s must be between %d (exclusive) and %d (exclusive)'
                                ),
                                $param,
                                $args['minimum'],
                                $args['maximum']
                    ...
                    if (
                        ...
                        return new WP_Error(
                            ...
                            sprintf(
                                __(
                                    '%s must be between %d (exclusive) and %d (inclusive)'
                                ),
                                $param,
                                $args['minimum'],
                                $args['maximum']
