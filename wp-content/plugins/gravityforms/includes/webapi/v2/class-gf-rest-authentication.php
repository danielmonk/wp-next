...
class GF_REST_Authentication
{
    ...
    public function init()
    {
        ...
        add_filter(
            ...
            [$this, 'check_authentication_error'],
            99
        );
        add_filter(
            'rest_pre_dispatch',
        ...
        add_filter(
            'rest_post_dispatch',
            [$this, 'send_unauthorized_headers'],
            50
        );
    }
    ...
    public function override_rest_authentication_errors($errors)
    {
        _deprecated_function(
            __METHOD__,
            '2.4.22',
            'GF_REST_Authentication::check_authentication_error'
        );
    ...
    protected function is_request_to_rest_api()
    {
        ...
        $is_gf_endpoint =
            strpos($_SERVER['REQUEST_URI'], $rest_prefix . 'gf/') !== false;

        // Allow third party plugins use our authentication methods.
        $third_party =
            false !== strpos($_SERVER['REQUEST_URI'], $rest_prefix . 'gf-');

        if (has_filter('gform_is_request_to_rest_api')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_is_request_to_rest_api.'
            );
        }
        ...
        return apply_filters(
            'gform_is_request_to_rest_api',
            $is_gf_endpoint || $third_party
        );
    }
    ...
    public function authenticate($user_id)
    {
        ...
        if (!empty($user_id)) {
            $this->log_debug(
                __METHOD__ .
                    sprintf('(): User #%d already authenticated.', $user_id)
            );
    ...
    private function perform_application_password_authentication()
    {
        ...
        if (empty($user_id)) {
            ...
            if (is_wp_error($wp_rest_application_password_status)) {
                $this->set_error(
                    new WP_Error(
                        'gform_rest_authentication_error',
                        $wp_rest_application_password_status->get_error_message(),
                        ['status' => 401]
                    )
                );
            }
        ...
        return $this->set_user((object) ['user_id' => $user_id]);
    }
    ...
    private function perform_basic_authentication()
    {
        ...
        if (!$consumer_key || !$consumer_secret) {
            $this->log_error(
                __METHOD__ . '(): Aborting; credentials not found.'
            );
        ...
        if (!hash_equals($user->consumer_secret, $consumer_secret)) {
            $this->set_error(
                new WP_Error(
                    'gform_rest_authentication_error',
                    __('Consumer secret is invalid.', 'gravityforms'),
                    ['status' => 401]
                )
            );
    ...
    public function get_oauth_parameters()
    {
        ...
        if (!empty($errors)) {
            ...
            $this->set_error(
                new WP_Error(
                    'gform_rest_authentication_missing_parameter',
                    $message,
                    ['status' => 401]
                )
            );
    ...
    private function perform_oauth_authentication()
    {
        ...
        if (empty($params)) {
            $this->log_error(
                __METHOD__ . '(): Aborting; OAuth parameters not found.'
            );
        ...
        $user = $this->get_user_data_by_consumer_key(
        ...
        if (empty($user)) {
            $this->set_error(
                new WP_Error(
                    'gform_rest_authentication_error',
                    __('Consumer key is invalid.', 'gravityforms'),
                    ['status' => 401]
                )
            );
        ...
        $timestamp_and_nonce = $this->check_oauth_timestamp_and_nonce(
            $user,
    ...
    private function check_oauth_signature($user, $params)
    {
        ...
        $base_request_uri = rawurlencode(
            get_home_url(null, $request_path, is_ssl() ? 'https' : 'http')
        );

        // Get the signature provided by the consumer and remove it from the parameters prior to checking the signature.
        $consumer_signature = rawurldecode(
            str_replace(' ', '+', $params['oauth_signature'])
        );
        ...
        $string_to_sign =
            $http_method . '&' . $base_request_uri . '&' . $query_string;

        if (
            'HMAC-SHA1' !== $params['oauth_signature_method'] &&
            'HMAC-SHA256' !== $params['oauth_signature_method']
        ) {
            return new WP_Error(
                'gform_rest_authentication_error',
                __(
        ...
        $hash_algorithm = strtolower(
            str_replace('HMAC-', '', $params['oauth_signature_method'])
        );
        $secret = $user->consumer_secret . '&';
        $signature = base64_encode(
            hash_hmac($hash_algorithm, $string_to_sign, $secret, true)
        );

        if (!hash_equals($signature, $consumer_signature)) {
            $this->log_debug(
                __METHOD__ . '(): Signature base: ' . $string_to_sign
            );
     ...
     */
    private function join_with_equals_sign(
        $params,
        $query_params = [],
        $key = ''
    ) {
        foreach ($params as $param_key => $param_value) {
            ...
            if (is_array($param_value)) {
                $query_params = $this->join_with_equals_sign(
                    $param_value,
                    $query_params,
                    $param_key
                );
    ...
    private function check_oauth_timestamp_and_nonce($user, $timestamp, $nonce)
    {
        ...
        if (
            $timestamp < time() - $valid_window ||
            $timestamp > time() + $valid_window
        ) {
            return new WP_Error(
                'gform_rest_authentication_error',
                __('Invalid timestamp.', 'gravityforms'),
                ['status' => 401]
            );
        }
    ...
    private function check_permissions($method)
    {
        ...
        switch ($method) {
            ...
            default:
                return new WP_Error(
                    'gform_rest_authentication_error',
                    __('Unknown request method.', 'gravityforms'),
                    ['status' => 401]
                );
        }
    ...
    {
        if (
            is_wp_error($this->get_error()) &&
            'basic_auth' === $this->auth_method
        ) {
            $auth_message = __(
                'Gravity Forms API. Use a consumer key in the username field and a consumer secret in the password field.',
                'gravityforms'
            );
            $response->header(
                'WWW-Authenticate',
    ...
    {
        if (is_array($value)) {
            return array_map([$this, 'urlencode_rfc3986'], $value);
        } else {
    ...
    {
        return in_array($this->auth_method, ['basic_auth', 'oauth1']);
    }
}
