...
if (class_exists('GFForms')) {
    ...
    class GFWebAPI extends GFAddOn
    {
        ...
        public function __construct()
        {
            ...
            if (defined('DOING_CRON') && DOING_CRON) {
                add_action(
                    'gravityforms_results_cron_' . $this->_slug,
                    [$this, 'results_cron'],
                    10,
                    3
                );
            ...
            $is_v2_enabled =
                $this->is_v2_enabled($this->get_plugin_settings()) ||
                $this->is_v2_enabled();
            if ($is_v2_enabled) {
                ...
                if (!is_admin()) {
                    require_once plugin_dir_path(__FILE__) .
                        'v2/class-gf-rest-authentication.php';
                }
            }
  ...
  KEY consumer_secret (consumer_secret)
        ...
        public function requires_schema_upgrade()
        {
            ...
            $upgrade_required = version_compare(
                GFForms::$version,
                $rest_api_db_version,
                '>'
            );
            ...
            if ($upgrade_required) {
                ...
                $rest_api_db_version = gf_upgrade()->get_wp_option(
                    'gf_rest_api_db_version'
                );
        ...
        {
            parent::init_ajax();
            add_action('wp_ajax_gfwebapi_qrcode', [$this, 'ajax_qrcode']);

            add_action('wp_ajax_delete_key', [$this, 'ajax_delete_key']);
        }
        ...
        public function init_admin()
        {
            ...
            if (
                GFForms::get_page() == 'settings' &&
                rgget('subview') == $this->_slug
            ) {
                require_once plugin_dir_path(__FILE__) .
                    'includes/class-gf-api-keys-table.php';
            }
        ...
        public function init_v1()
        {
            add_rewrite_rule(
                GFWEBAPI_SLUG . '/(.*)',
                'index.php?' . GFWEBAPI_ROUTE_VAR . '=$matches[1]',
                $after = 'top'
            );
            ...
            add_action('template_redirect', [$this, 'handle_page_request']);
            ...
            $scripts = [
                ...
                [
                    ...
                    'src' =>
                        ...
                        '/includes/webapi/js/hmac-sha1.min.js',
                    'enqueue' => [['admin_page' => ['plugin_settings']]],
                ],
                [
                    ...
                    'src' =>
                        GFCommon::get_base_url() .
                        '/includes/webapi/js/enc-base64-min.js',
                    'deps' => ['gfwebapi_hmac_sha1'],
                ...
                [
                    ...
                    'src' =>
                        GFCommon::get_base_url() .
                        "/includes/webapi/js/gfwebapi_settings{$min}.js",
        ...
        public function styles()
        {
            ...
            $styles = [
                [
                    ...
                    'src' =>
                        GFCommon::get_base_url() .
                        "/includes/webapi/css/gfwebapi_settings{$min}.css",
                    'version' => $this->_version,
                    'deps' => ['thickbox'],
                    'enqueue' => [['admin_page' => ['plugin_settings']]],
                ],
        ...
        public function display_api_key_confirmation($api_key)
        {
			...
			<table class="form-table gforms_form_settings">
				<tbody><tr id="gaddon-setting-row-public_key">
					...
					<td>
						<input type="text" name="consumer_key" id="consumer_key" value="<?php echo $api_key[
          'consumer_key'
				...
				<tr>
					...
					<td>
						<input type="text" name="consumer_secret" id="consumer_secret" value="<?php echo $api_key[
          'consumer_secret'
      ]; ?>" class="medium gaddon-setting gaddon-text">
					</td>
				</tr>
				<tr>
					<th colspan="2">
						<div class="alert_yellow" style="padding:15px;">
							<?php esc_html_e(
           'Make sure you have copied the consumer key and secret above. They will not be available once you leave this page',
           'gravityforms'
       ); ?>
						</div>
					</th>
				...
				<tr>
					<th colspan="2" class="padding-top:10px;">
						<a href="?page=gf_settings&subview=gravityformswebapi" class="button"><?php esc_html_e(
        ...
        public function display_api_key_edit($key_id, $has_updated = false)
        {
            ...
            if ($has_updated) { ?>
				<div class="updated below-h2" style="padding:10px;"><?php esc_html_e(
        ...
        'gravityforms'
			...
			<table class="form-table gforms_form_settings">
				<tbody><tr id="gaddon-setting-row-public_key">
					...
					<td>
						<input type="text" name="key_description" value="<?php echo rgobj(
          $key,
          'description'
      ); ?>" class="medium gaddon-setting gaddon-text">
					</td>
       ...
       foreach ($users as $user) {
           $selected =
               rgobj($key, 'user_id') == $user['value'] ? ' selected' : ''; ?>
								<option value="<?php echo $user[
            'value'
        ]; ?>" <?php echo $selected; ?>><?php echo $user['label']; ?></option>
								<?php
       }
       ?>
						</select>
				...
				<tr>
					...
					<td>
						<select name="key_permission" class="gaddon-setting gaddon-select">
							<?php
       $permissions = [
           ['value' => 'read', 'text' => __('Read', 'gravityforms')],
           ['value' => 'write', 'text' => __('Write', 'gravityfroms')],
           [
               'value' => 'read_write',
               'text' => __('Read/Write', 'gravityforms'),
           ],
       ];
       foreach ($permissions as $permission) {
           $selected =
               rgobj($key, 'permissions') === $permission['value']
                   ? ' selected'
                   : ''; ?>
    ...
    if ($key_id != 0) {
        $last_access =
            rgobj($key, 'last_access') == ''
                ? __('Never Accessed', 'gravityforms')
                : GFCommon::format_date(
					...
					<tr>
						<th style="padding-top:18px;"><?php esc_html_e(
          'Last Access',
          'gravityforms'
      ); ?></th>
    ...
    $button_label =
        $key_id == 0
            ? __('Add Key', 'gravityforms')
            : __('Update', 'gravityforms');
    $link_label = $has_updated
        ? __('Back to API Settings', 'gravityforms')
        : __('Cancel', 'gravityforms');
    ?>
				<tr>
					<td colspan="2" style="padding-top:10px;">
						...
						<a href="?page=gf_settings&subview=gravityformswebapi" class="button" style="margin-top:10px;"><?php echo esc_html(
          $link_label
      ); ?></a>
					</td>
        ...
        {
            $args = apply_filters('gform_webapi_get_users_settings_page', [
                'number' => 3000,
            ]);
        ...
        public function plugin_settings_fields()
        {
            ...
            return [
                ...
                [
                    ...
                    'description' => sprintf(
                        __(
                            'Create an API Key below to use the REST API version 2. Alternatively, you can use cookie authentication which is supported for logged in users. %sVisit our documentation pages%s for more information.',
                            'gravityforms'
                        ),
                        '<a href="https://docs.gravityforms.com/rest-api-v2/" target="_blank">',
                        '</a>'
                    ),
                    'dependency' => [$this, 'is_v2_enabled'],
                    'fields' => [
                        [
                            'type' => 'api_keys',
                ...
                [
                    ...
                    'description' => sprintf(
                        __(
                            'Configure your API Key below to use the REST API version 1. Alternatively, you can use cookie authentication which is supported for logged in users. %sVisit our documentation pages%s for more information.',
                            'gravityforms'
                        ),
                        '<a href="https://docs.gravityforms.com/web-api/" target="_blank">',
                        '</a>'
                    ),
                    'dependency' => [$this, 'is_v1_enabled'],
                    'fields' => [
                        [
                            'name' => 'public_key',
                            'label' => esc_html__(
                        ...
                        [
                            ...
                            'type' => 'text',
                            'default_value' => substr(
                                wp_hash(get_bloginfo('admin_email')),
                                0,
                                15
                            ),
                            'class' => 'medium',
        ...
        public function is_v1_enabled($settings = null)
        {
             ...
             */
            $is_v1_enabled = apply_filters(
                'gform_is_rest_api_v1_enabled',
                true
            );
        ...
        {
            return $this->get_setting('enabled', '', $settings) &&
                !is_callable('gf_rest_api');
        }
        ...
        public function settings_requirements_check()
        {
			...
			<span class="gf_invalid">
					<?php esc_html_e(
         'Permalinks are not in the correct format.',
         'gravityforms'
     ); ?>
				</span>
			<br/>
			<span class='gf_settings_description'>
				<?php printf(
        esc_html__(
            'Change the %sWordPress Permalink Settings%s from default to any of the other options to get started.',
            'gravityforms'
        ),
        '<a href="' . esc_url($permalinks_url) . '">',
        '</a>'
    ); ?>
			</span>
			<?php
        ...
        public function settings_qrcode()
        {
			...
			<button class="button-secondary"
			        id="gfwebapi-qrbutton"><?php esc_html_e(
               'Show/hide QR Code',
               'gravityforms'
           ); ?></button>
        ...
        public function handle_page_request()
        {
            ...
            if (empty($settings) || !$settings['enabled']) {
                $this->log_debug(
                    __METHOD__ . '(): API not enabled, permission denied.'
                );
            ...
            $offset = isset($_GET['paging']['offset'])
                ? strtolower($_GET['paging']['offset'])
                : 0;
            $page_size = isset($_GET['paging']['page_size'])
                ? strtolower($_GET['paging']['page_size'])
                : 10;

            $method = strtoupper($_SERVER['REQUEST_METHOD']);
            $args = compact('offset', 'page_size', 'schema');

            $endpoint = empty($collection2)
                ? strtolower($method) . '_' . $collection
                : strtolower($method) . '_' . $collection . '_' . $collection2;
             ...
             */
            $authentication_required = apply_filters(
                'gform_webapi_authentication_required_' . $endpoint,
            ...
            if ($authentication_required) {
                ...
                $this->log_debug(
                    __METHOD__ . '(): Authentication not required.'
                );
            }
            ...
            $this->log_debug(
                __METHOD__ . '(): HTTP_RAW_POST_DATA = ' . $HTTP_RAW_POST_DATA
            );
        ...
        public function authorize($caps = [])
        {
        ...
        public function get_feeds($feed_ids, $form_id = null)
        {
            ...
            $capability = apply_filters(
                'gform_web_api_capability_get_feeds',
            ...
            $response =
                false === empty($feed_ids) &&
                false === is_array($feed_ids) &&
                is_array($output)
                    ? array_shift($output)
                    : '';
        ...
        public function delete_feeds($feed_ids, $form_id = null)
        {
             ...
             */
            $capability = apply_filters(
                'gform_web_api_capability_delete_feeds',
                'gravityforms_edit_forms'
            );
            $this->authorize($capability);
            ...
            if (isset($result) && is_wp_error($result)) {
                ...
                $status = 200;
                $response = sprintf(
                    __('Feeds deleted successfully: %d', 'gravityforms'),
                    $count
                );
            }
        ...
        public function put_feeds($feed_data, $feed_id = null)
        {
             ...
             */
            $capability = apply_filters(
                'gform_web_api_capability_put_feeds',
                'gravityforms_edit_forms'
            );
            $this->authorize($capability);
            ...
            if (isset($results) && is_wp_error($result)) {
                ...
                $status = 200;
                $response = sprintf(
                    __('Feeds updated: %d', 'gravityforms'),
                    $count
                );
            }
        ...
        public function post_feeds($feeds, $form_id = null)
        {
             ...
             */
            $capability = apply_filters(
                'gform_web_api_capability_post_feeds',
                'gravityforms_edit_forms'
            );
            $this->authorize($capability);
            ...
            $result = [];
            foreach ($feeds as $feed) {
                $addon_slug = isset($feed['addon_slug'])
                    ? $feed['addon_slug']
                    : rgget('addon');
                ...
                if (empty($f_id)) {
                    $result = new WP_Error(
                        'missing_form_id',
                        __('Missing form id', 'gravityforms')
                    );
                    break;
                }
        ...
        public function submit_form($data, $id)
        {
            ...
            $field_values = isset($data['field_values'])
                ? $data['field_values']
                : [];
            $target_page = isset($data['target_page'])
                ? $data['target_page']
                : 0;
            $source_page = isset($data['source_page'])
                ? $data['source_page']
                : 1;
        ...
        public function delete_forms($form_ids)
        {
            ...
            if (isset($result) && is_wp_error($result)) {
                ...
                $status = 200;
                $response = sprintf(
                    __('Forms deleted successfully: %d', 'gravityforms'),
                    $count
                );
            }
        ...
        public function post_entries($data, $form_id = null)
        {
            ...
            foreach ($data as $entry) {
                $entries[] = $this->maybe_serialize_list_fields(
                    $entry,
                    $form_id
                );
            }
        ...
        public function put_entries($data, $entry_id = null)
        {
            ...
            if (is_wp_error($result)) {
                ...
                $status = 200;
                $response = empty($entry_id)
                    ? __('Entries updated successfully', 'gravityforms')
                    : __('Entry updated successfully', 'gravityforms');
            }
        ...
        public function put_forms_properties($property_values, $form_id)
        {
            ...
            foreach ($property_values as $key => $property_value) {
                $result = GFAPI::update_form_property(
                    $form_id,
                    $key,
                    $property_value
                );
        ...
        public function put_entry_properties($property_values, $entry_id)
        {
            ...
            if (is_array($property_values)) {
                foreach ($property_values as $key => $property_value) {
                    $result = GFAPI::update_entry_property(
                        $entry_id,
                        $key,
                        $property_value
                    );
                ...
                if (empty($property_values)) {
                    $response = __(
                        'No property values were found in the request body',
                        'gravityforms'
                    );
                } else {
                    $response = __(
                        'Property values should be sent as an array',
                        'gravityforms'
                    );
                }
            }
        ...
        public function put_forms($data, $form_id = null)
        {
            ...
            if (is_wp_error($result)) {
                ...
                $status = 200;
                $response = empty($form_id)
                    ? __('Forms updated successfully', 'gravityforms')
                    : __('Form updated successfully', 'gravityforms');
            }
        ...
        public function delete_entries($entry_ids)
        {
            ...
            if (is_array($entry_ids)) {
                foreach ($entry_ids as $entry_id) {
                    $this->log_debug(
                        __METHOD__ . '(): Deleting entry id ' . $entry_id
                    );
            ...
            if (isset($result) && is_wp_error($result)) {
                ...
                $status = 200;
                $response = sprintf(
                    __('Entries deleted successfully: %d', 'gravityforms'),
                    $count
                );
            }
            ...
            if ($entry_ids) {
                if (is_array($entry_ids)) {
                    foreach ($entry_ids as $entry_id) {
                        ...
                        if (!is_wp_error($result)) {
                            $result = $this->maybe_json_encode_list_fields(
                                $result
                            );
                            $response[$entry_id] = $result;
                            if (
                                !empty($field_ids) &&
                                !empty($response[$entry_id])
                            ) {
                                $response[
                                    $entry_id
                    ...
                    if (!is_wp_error($result)) {
                        ...
                        if (!empty($field_ids) && !empty($response)) {
                            $response = $this->filter_entry_object(
                                $response,
                                $field_ids
                            );
                        }
                ...
                // Sorting parameters
                $sort_key =
                    isset($_GET['sorting']['key']) &&
                    !empty($_GET['sorting']['key'])
                        ? $_GET['sorting']['key']
                        : 'id';
                $sort_dir =
                    isset($_GET['sorting']['direction']) &&
                    !empty($_GET['sorting']['direction'])
                        ? $_GET['sorting']['direction']
                        : 'DESC';
                $sorting = ['key' => $sort_key, 'direction' => $sort_dir];
                ...
                if (!is_wp_error($result)) {
                    ...
                    if ($schema == 'mtd') {
                        $response = $this->mtd_transform_entries_data(
                            $response,
                            $form_ids
                        );
                    }
        ...
        public static function filter_entry_object($entry, $field_ids)
        {
            ...
            foreach ($entry as $key => $val) {
                if (
                    in_array($key, $field_ids) ||
                    (is_numeric($key) && in_array(intval($key), $field_ids))
                ) {
                    $new_entry[$key] = $val;
                }
            }
        ...
        public function maybe_serialize_list_fields($entry, $form_id = null)
        {
            ...
            if (!empty($form['fields']) && is_array($form['fields'])) {
                foreach ($form['fields'] as $field) {
                    /* @var GF_Field $field */
                    if ($field->get_input_type() == 'list') {
                        $new_list_value = $this->maybe_decode_json(
                            $entry[$field->id]
                        );
        ...
        public function get_results_cache_key(
            $form_id,
            $fields,
            $search_criteria
        ) {
            ...
            $key .= wp_hash(
                json_encode($fields) . json_encode($search_criteria)
            );
        ...
        public function update_results_cache_meta(
            $form_id,
            $fields,
            $expiry = null
        ) {
        ...
        {
            $form_id = $form['id'];
            $key = $this->get_results_cache_key(
                $form_id,
                $fields,
                $search_criteria
            );
            ...
            if (!empty($state)) {
                if (!class_exists('GFResults')) {
                    require_once GFCommon::get_base_path() .
                        '/includes/addon/class-gf-results.php';
                }
                $gf_results = new GFResults($this->_slug, []);
                $results = $gf_results->get_results_data(
                    $form,
                    $fields,
                    $search_criteria,
                    $state
        ...
        public function get_results($form_id)
        {
            ...
            $search_criteria = false === empty($s) && is_array($s) ? $s : [];
            ...
            $cache_timestamp = isset($data['timestamp'])
                ? $data['timestamp']
                : 0;
            $cache_expired = $cache_expiry
                ? $cache_expiry > $cache_timestamp
                : false;

            // check for valid cached results first
            if (
                !empty($data) &&
                'complete' == rgar($data, 'status') &&
                !$cache_expired
            ) {
                $results = $data;
                $status = 200;
                ...
                if (
                    empty($state) ||
                    ('complete' == rgar($data, 'status') && $cache_expired)
                ) {
                    if (!class_exists('GFResults')) {
                        require_once GFCommon::get_base_path() .
                            '/includes/addon/class-gf-results.php';
                    ...
                    if (
                        $state_age > 180 &&
                        !$this->results_cron_is_scheduled(
                            $form,
                            $fields,
                            $search_criteria
                        )
            ...
            if (!empty($fields)) {
                ...
                $results['field_data'] = $this->results_data_add_labels(
                    $form,
                    $fields
                );
            }
        ...
        public function schedule_results_cron(
            $form,
            $fields,
            $search_criteria,
            $delay_in_seconds = 10
            ...
            $args = [$form, $fields, $search_criteria];

            wp_schedule_single_event(
                time() + $delay_in_seconds,
                $this->get_results_cron_hook(),
                $args
            );
        }

        public function results_cron_is_scheduled(
            $form,
            $fields,
            $search_criteria
        ) {
                ...
                if (is_array($choice_counts)) {
                    ...
                    foreach ($choice_counts as $choice_value => $choice_count) {
                        if (
                            ...
                            $row_text = GFSurvey::get_likert_row_text(
                                $field,
                                $i++
                            );
                            $counts_for_row = [];
                            foreach ($choice_count as $col_val => $col_count) {
                                $text = GFSurvey::get_likert_column_text(
                                    $field,
                                    $choice_value . ':' . $col_val
                                );
                                $counts_for_row[$col_val] = [
                                    'text' => $text,
                                    'data' => $col_count,
                                ];
                            }
                            $counts_for_row[$choice_value][
                                'data'
                            ] = $counts_for_row;
                            $fields[$field_id][$choice_value] = [
                                'text' => $row_text,
                                'value' => "$choice_value",
                                'count' => $counts_for_row,
                            ];
                        } else {
                            $text = GFFormsModel::get_choice_text(
                                $field,
                                $choice_value
                            );
                            $fields[$field_id][$choice_value] = [
        ...
        private function authenticate()
        {
            ...
            if (isset($_REQUEST['_gf_json_nonce']) && is_user_logged_in()) {
                $this->log_debug(
                    __METHOD__ . '(): Using WP cookie authentication.'
                );
            ...
            if ($authenticated) {
                ...
                if (empty($settings) || !$settings['enabled']) {
                    ...
                    $this->log_debug(
                        __METHOD__ . '(): Switching to impersonation account.'
                    );
            ...
            if (!$authenticated) {
                $this->log_debug(
                    __METHOD__ .
                        '(): Could not authenticate, permission denied.'
                );
                $this->die_permission_denied();
            }
        }
        ...
        private function check_signature()
        {
            ...
            $is_valid =
                $signature == $calculated_sig ||
                $signature == rawurlencode($calculated_sig);
            $this->log_debug(
                __METHOD__ . '(): result = ' . var_export($is_valid, 1)
            );
        ...
        public function get_error_status($wp_error)
        {
            ...
            $http_code = isset($mappings[$error_code])
                ? $mappings[$error_code]
                : 400;
        ...
        public static function mtd_transform_entry_data($entry)
        {
            ...
            foreach ($fields as $field) {
                ...
                if (is_array($value) && isset($field->choices)) {
                    ...
                    $element = [];
                    $element['type'] = 'string';
                    $element['caption'] = GFFormsModel::get_choice_text(
                        $field,
                        $value
                    );
