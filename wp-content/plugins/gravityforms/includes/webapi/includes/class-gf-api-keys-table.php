...
class GF_API_Keys_Table extends WP_List_Table
{
    ...
    {
        return ['widefat', 'striped', 'feeds', 'api_key_table'];
    }
    ...
    function column_description($item)
    {
        ...
        $confirm =
            "javascript: if( ! confirm('WARNING: You are about to revoke this API Key. \'Cancel\' to stop, \'OK\' to revoke.')){ event.stopPropagation(); return false } ";
        $nonce_url = wp_nonce_url(
            '?page=gf_settings&subview=gravityformswebapi',
            'gf_revoke_key'
        );
        ...
        $actions = [
            'edit' =>
                '<a href="' .
                $this->get_edit_url($item['key_id']) .
                '">' .
                esc_html__('Edit', 'gravityforms') .
                '</a>',
            'delete' => sprintf(
                '<a data-wp-lists="delete:the-list:key_row_%d::status=delete&action=delete_key&key=%d" onclick="%s" href="%s" class="submitdelete">Revoke</a>',
                absint($item['key_id']),
                absint($item['key_id']),
                $confirm,
                $nonce_url
    ...
    {
        return sprintf(
            '?page=gf_settings&subview=gravityformswebapi&action=edit&key_id=%s',
            absint($key_id)
        );
    }
    ...
    {
        return empty($item['last_access'])
            ? __('Never Accessed', 'gravityforms')
            : GFCommon::format_date($item['last_access'], true, '', true);
    }
    ...
    function no_items()
    {
        echo '<div style="padding:10px;">' .
            sprintf(
                esc_html__(
                    'You don\'t have any API keys. Let\'s go %1$screate one%2$s!',
                    'gravityforms'
                ),
                '<a href="' . $this->get_edit_url(0) . '">',
                '</a>'
            ) .
            '</div>';
    }
    ...
    public function display()
    {
      ...
      $this->get_table_classes()
       ...
       echo " data-wp-lists='list:$singular'";
		...
		<div>
			<a class="button-secondary gfbutton gaddon-setting" id="add_setting_button" href="<?php echo $this->get_edit_url(
       0
   ); ?>">Add Key</a>
		</div>
		<?php
