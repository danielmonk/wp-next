...
class GF_Block_Form extends GF_Block
{
    ...
    public function scripts()
    {
        ...
        return [
            [
                ...
                'version' => $min
                    ? GFForms::$version
                    : filemtime(GFCommon::get_base_path() . '/js/blocks.js'),
    ...
    {
        wp_localize_script($script['handle'], 'gform_block_form', [
        ...
        if (function_exists('wp_set_script_translations')) {
            wp_set_script_translations(
                $script['handle'],
                'gravityforms',
                GFCommon::get_base_path() . '/languages'
            );
        }
    }
        ...
        return [
            [
                ...
                'version' =>
                    defined('SCRIPT_DEBUG') && SCRIPT_DEBUG
                        ? filemtime(
                            GFCommon::get_base_path() . '/css/blocks.min.css'
                        )
                        : GFForms::$version,
            ],
        ];
    ...
    public function render_block($attributes = [])
    {
        ...
        $tabindex = isset($attributes['tabindex'])
            ? intval($attributes['tabindex'])
            : 0;
        ...
        return sprintf(
            '[gravityforms id="%d" title="%s" description="%s" ajax="%s" tabindex="%d" field_values="%s"]',
            $form_id,
            $title ? 'true' : 'false',
    ...
    public function get_forms()
    {
        ...
        foreach ($form_objects as $form) {
            $forms[] = [
                'id' => $form['id'],
                'title' => $form['title'],
                'hasConditionalLogic' => GFFormDisplay::has_conditional_logic(
                    $form
                ),
            ];
        }
...
if (
    true !==
        ($registered = GF_Blocks::register(GF_Block_Form::get_instance())) &&
    is_wp_error($registered)
) {
    // Log that block could not be registered.
    GFCommon::log_error(
        'Unable to register block; ' . $registered->get_error_message()
    );
}
