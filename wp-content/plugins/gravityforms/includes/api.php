...
class GFAPI
{
    ...
    public static function delete_form($form_id)
    {
        ...
        if (empty($form)) {
            return new WP_Error(
                'not_found',
                sprintf(
                    __('Form with id: %s not found', 'gravityforms'),
                    $form_id
                ),
                $form_id
            );
        }
        self::delete_forms([$form_id]);
    ...
    public static function update_form($form, $form_id = null)
    {
        ...
        if (!$form) {
            return new WP_Error(
                'invalid',
                __('Invalid form object', 'gravityforms')
            );
        }
        ...
        if (isset($form['fields'])) {
            ...
            $form['fields'] = self::add_missing_ids(
                $form['fields'],
                $next_field_id
            );
        }
        ...
        if (
            ...
            return new WP_Error(
                'not_found',
                __('Form not found', 'gravityforms')
            );
        }
        ...
        if (false === $result) {
            return new WP_Error(
                'error_updating_form',
                __('Error updating form', 'gravityforms'),
                $wpdb->last_error
            );
        }

        if (isset($form['confirmations']) && is_array($form['confirmations'])) {
            $confirmations = self::set_property_as_key(
                $form['confirmations'],
                'id'
            );
            ...
            if (false === $result) {
                return new WP_Error(
                    'error_updating_confirmations',
                    __('Error updating form confirmations', 'gravityforms'),
                    $wpdb->last_error
                );
            }
        }
        ...
        if (isset($form['notifications']) && is_array($form['notifications'])) {
            $notifications = self::set_property_as_key(
                $form['notifications'],
                'id'
            );
            $result = GFFormsModel::update_form_meta(
            ...
            if (false === $result) {
                return new WP_Error(
                    ...
                    __('Error updating form notifications', 'gravityforms'),
                    $wpdb->last_error
                );
            }
        }
        ...
        $result = $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$form_table_name} SET title=%s, is_active=%s WHERE id=%d",
                $form['title'],
                $is_active,
                $form['id']
            )
        ...
        if (false === $result) {
            return new WP_Error(
                ...
                __('Error updating title', 'gravityforms'),
                $wpdb->last_error
            );
        }
    ...
    private static function add_missing_ids($fields, $next_field_id)
    {
        foreach ($fields as &$field) {
            ...
            if (is_array($field->fields)) {
                $field->fields = self::add_missing_ids(
                    $field->fields,
                    $next_field_id
                );
            }
     ...
     */
    public static function update_forms_property(
        $form_ids,
        $property_key,
        $value
    ) {
        ...
        if (!in_array(strtolower($property_key), $db_columns)) {
            return new WP_Error(
                'property_key_incorrect',
                __('Property key incorrect', 'gravityforms')
            );
        }
    ...
    public static function update_form_property($form_id, $property_key, $value)
    {
        ...
        return self::update_forms_property([$form_id], $property_key, $value);
    }
    ...
    public static function add_forms($forms)
    {
        ...
        if (!$forms || !is_array($forms)) {
            return new WP_Error(
                'invalid',
                __('Invalid form objects', 'gravityforms')
            );
        }
        $form_ids = [];
        foreach ($forms as $form) {
    ...
    public static function add_form($form_meta)
    {
        ...
        if (rgar($form_meta, 'title') == '') {
            return new WP_Error(
                'missing_title',
                __('The form title is missing', 'gravityforms')
            );
        }
        ...
        if (!isset($form_meta['fields']) || !is_array($form_meta['fields'])) {
            return new WP_Error(
                'missing_fields',
                __('The form fields are missing', 'gravityforms')
            );
        }
        ...
        if (isset($form_meta['confirmations'])) {
            $form_meta['confirmations'] = self::set_property_as_key(
                $form_meta['confirmations'],
                'id'
            );
        ...
        if (isset($form_meta['notifications'])) {
            $form_meta['notifications'] = self::set_property_as_key(
        ...
        $form_meta['fields'] = self::add_missing_ids(
            $form_meta['fields'],
            $next_field_id
        );
        ...
        if (false === $result) {
            return new WP_Error(
                'insert_form_error',
                __(
                    'There was a problem while inserting the form',
                    'gravityforms'
                ),
                $wpdb->last_error
            );
    ...
    public static function get_entries(
        ...
        if (
            ...
            $entries = GF_Forms_Model_Legacy::search_leads(
                $form_ids,
                $search_criteria,
                $sorting,
                $paging
    ...
    public static function get_entry_ids(
        ...
        if (
            ...
            $entry_ids = GF_Forms_Model_Legacy::search_lead_ids(
                $form_id,
                $search_criteria
            );
            return $entry_ids;
    ...
    public static function count_entries($form_ids, $search_criteria = [])
    {
        if (
            ...
            return GF_Forms_Model_Legacy::count_search_leads(
                $form_ids,
                $search_criteria
            );
        }
        ...
        if (
            ...
            if (empty($entries)) {
                return new WP_Error(
                    'not_found',
                    sprintf(
                        __('Entry with id %s not found', 'gravityforms'),
                        $entry_id
                    ),
                    $entry_id
                );
    ...
    public static function update_entry($entry, $entry_id = null)
    {
        ...
        if (!isset($entry['ip'])) {
            $entry['ip'] = rgars($form, 'personalData/preventIP')
                ? ''
                : GFFormsModel::get_ip();
        }
        $ip = $entry['ip'];
        ...
        $entry['user_agent'] = isset($entry['user_agent'])
        ...
        $payment_status = !empty($entry['payment_status'])
        ...
        $payment_date = $payment_date
            ? sprintf("'%s'", esc_sql(gmdate('Y-m-d H:i:s', $payment_date)))
            : 'NULL';

        if (!isset($entry['payment_amount'])) {
            $entry['payment_amount'] = null;
        }
        $payment_amount = !empty($entry['payment_amount'])
        ...
        $transaction_id = !empty($entry['transaction_id'])
        ...
        $is_fulfilled = !empty($entry['is_fulfilled'])
        ...
        $transaction_type = !empty($entry['transaction_type'])
        ...
        self::purge_missing_entry_values(
            $form,
            $entry,
            $current_entry,
            $current_fields
     ...
     */
    private static function purge_missing_entry_values(
        $form,
        &$entry,
        $current_entry,
        $current_fields,
        ...
        if ($current_entry !== null) {
            foreach ($current_entry as $k => $v) {
                ...
                if ($field instanceof GF_Field_Repeater && !empty($v)) {
                    ...
                    $result = GFFormsModel::queue_batch_field_operation(
                        $form,
                        $entry,
                        $field,
                        $lead_detail_id,
    ...
    private static function queue_batch_field_operation(
        $form,
        $entry,
        $field,
        $item_index = '',
        ...
        if (is_array($inputs)) {
            foreach ($field->inputs as $input) {
                ...
                $input_value = isset($entry[(string) $input_id])
                    ? $entry[(string) $input_id]
                    : '';
                $current_value = isset($current_entry[(string) $input_id])
                    ? $current_entry[(string) $input_id]
                    : '';
                if (empty($current_entry) || $input_value != $current_value) {
                    $lead_detail_id = $current_fields
                    ...
                    foreach ($current_fields as $current_field) {
                        if (
                            ...
                            $current_field->item_index == $item_index
                        ) {
            ...
            $field_value = isset($entry[(string) $field_id])
                ...
                : '';
            $current_value = isset($current_entry[(string) $field_id])
                ? $current_entry[(string) $field_id]
                : '';
            if (empty($current_entry) || $field_value != $current_value) {
                ...
                foreach ($current_fields as $current_field) {
                    if (
                        $current_field->meta_key == $field_id &&
                        $current_field->item_index == $item_index
                    ) {
                        $current_field->update = true;
                    }
    ...
    public static function add_entry($entry)
    {
        ...
        $date_created =
            isset($entry['date_created']) && $entry['date_created'] != ''
                ...
                : 'utc_timestamp()';
        $date_updated =
            isset($entry['date_updated']) && $entry['date_updated'] != ''
                ? sprintf("'%s'", esc_sql($entry['date_updated']))
                : 'utc_timestamp()';
    ...
    public static function delete_entry($entry_id)
    {
        ...
        if (empty($entry)) {
            return new WP_Error(
                'invalid_entry_id',
                sprintf(__('Invalid entry id: %s', 'gravityforms'), $entry_id),
                $entry_id
            );
        }
    ...
    public static function update_entry_property($entry_id, $property, $value)
    {
        ...
        return GFFormsModel::update_entry_property(
            $entry_id,
            $property,
            $value
        );
    ...
    public static function update_entry_field(
        ...
        if (
            ...
            return GF_Forms_Model_Legacy::update_entry_field(
                $entry_id,
                $input_id,
                $value
            );
        ...
        if (
            ...
            if (isset($entry[$input_id])) {
                ...
                $result = GFFormsModel::update_entry_field_value(
                    $form,
                    $entry,
                    $field,
                    0,
            ...
            foreach ($value as $i => $sub_values) {
                ...
                foreach ($sub_values as $key => $sub_value) {
                    $result = self::update_entry_field(
                        $entry_id,
                        $key,
                        $sub_value,
                        $new_item_index
    ...
    {
        $note = GFFormsModel::get_notes(['id' => $note_id]);

        if (empty($note)) {
            return new WP_Error(
                'note_not_found',
                __('Note not found', 'gravityforms')
            );
        }
    ...
    public static function add_note(
        ...
        if (empty($note) || !is_string($note)) {
            return new WP_Error(
                'invalid_note',
                __('Invalid or empty note', 'gravityforms'),
                $entry_id
            );
        }

        $new_note = GFFormsModel::add_note(
            intval($entry_id),
            $user_id,
            $user_name,
            wp_kses_post($note),
            sanitize_text_field($note_type),
            sanitize_text_field($sub_type)
        );
    ...
    public static function update_note($note, $note_id = '')
    {
        ...
        if (!is_array($note) || empty($note)) {
            return new WP_Error(
                'invalid_note_format',
                __('Invalid note format', 'gravityforms')
            );
        }
        ...
        if (empty($note_id)) {
            return new WP_Error(
                'missing_note_id',
                __('Missing note id', 'gravityforms')
            );
        }
        ...
        $result = GFFormsModel::update_note(
            $note['id'],
            $note['entry_id'],
            $note['user_id'],
            $note['user_name'],
    ...
    public static function submit_form(
        ...
        if (empty($form) || !$form['is_active'] || $form['is_trash']) {
            return new WP_Error(
                'form_not_found',
                __('Your form could not be found', 'gravityforms')
            );
        }

        $input_values['is_submit_' . $form_id] = true;
        $input_values['gform_submit'] = $form_id;
        $input_values['gform_target_page_number_' . $form_id] = absint(
            $target_page
        );
        $input_values['gform_source_page_number_' . $form_id] = absint(
            $source_page
        );
        ...
        add_filter(
            ...
            ['GFAPI', 'submit_form_filter_gform_pre_validation'],
            50
        );
        ...
        $result['source_page_number'] =
            $submission_details['source_page_number'];
        ...
        if ($submission_details['is_valid']) {
            ...
            if (is_array($confirmation_message)) {
                if (isset($confirmation_message['redirect'])) {
                    ...
                    $result['confirmation_redirect'] =
                        $confirmation_message['redirect'];
    ...
    public static function get_feeds(
        ...
        if (empty($results)) {
            return new WP_Error(
                'not_found',
                __('Feed not found', 'gravityforms')
            );
        }
    ...
    public static function delete_feed($feed_id)
    {
        ...
        if (false === $results) {
            return new WP_Error(
                ...
                sprintf(
                    __(
                        'There was an error while deleting feed id %s',
                        'gravityforms'
                    ),
                    $feed_id
                ),
        ...
        if (0 === $results) {
            return new WP_Error(
                'not_found',
                sprintf(__('Feed id %s not found', 'gravityforms'), $feed_id)
            );
        }
    ...
    public static function update_feed($feed_id, $feed_meta, $form_id = null)
    {
        ...
        if (empty($form_id)) {
            $sql = $wpdb->prepare(
                "UPDATE {$table} SET meta= %s WHERE id=%d",
                $feed_meta_json,
                $feed_id
            );
        } else {
            $sql = $wpdb->prepare(
                "UPDATE {$table} SET form_id = %d, meta= %s WHERE id=%d",
                $form_id,
                $feed_meta_json,
                $feed_id
            );
        ...
        if (false === $results) {
            return new WP_Error(
                ...
                sprintf(
                    __(
                        'There was an error while updating feed id %s',
                        'gravityforms'
                    ),
                    $feed_id
                ),
    ...
    public static function add_feed($form_id, $feed_meta, $addon_slug)
    {
        ...
        $feed_meta_json = json_encode($feed_meta);
        $sql = $wpdb->prepare(
            "INSERT INTO {$table} (form_id, meta, addon_slug) VALUES (%d, %s, %s)",
            $form_id,
            $feed_meta_json,
            $addon_slug
        );
        ...
        if (false === $results) {
            return new WP_Error(
                'error_inserting',
                __('There was an error while inserting a feed', 'gravityforms'),
                $wpdb->last_error
            );
        }
    ...
    private static function get_missing_table_wp_error($table)
    {
        return new WP_Error(
            'missing_table',
            sprintf(__('The %s table does not exist.', 'gravityforms'), $table)
        );
    }
    ...
    public static function send_notifications(
        ...
        $entry_id = rgar($entry, 'id');
        GFCommon::log_debug(
            "GFAPI::send_notifications(): Gathering notifications for {$event} event for entry #{$entry_id}."
        );
        ...
        foreach ($form['notifications'] as $notification) {
            ...
            if ($event == 'form_submission') {
                 ...
                 */
                if (
                    rgar($notification, 'type') == 'user' &&
                    gf_apply_filters(
                        ['gform_disable_user_notification', $form['id']],
                        false,
                        $form,
                        $entry
                    )
                    ...
                    GFCommon::log_debug(
                        "GFAPI::send_notifications(): Notification is disabled by gform_disable_user_notification hook, not including notification (#{$notification['id']} - {$notification['name']})."
                    );
                    ...
                    rgar($notification, 'type') == 'admin' &&
                    gf_apply_filters(
                        ['gform_disable_admin_notification', $form['id']],
                        false,
                        $form,
                        $entry
                    )
                    ...
                    GFCommon::log_debug(
                        "GFAPI::send_notifications(): Notification is disabled by gform_disable_admin_notification hook, not including notification (#{$notification['id']} - {$notification['name']})."
                    );
            ...
            if (
                gf_apply_filters(
                    ['gform_disable_notification', $form['id']],
                    false,
                    $notification,
                    $form,
                    $entry,
                ...
                GFCommon::log_debug(
                    "GFAPI::send_notifications(): Notification is disabled by gform_disable_notification hook, not including notification (#{$notification['id']} - {$notification['name']})."
                );
