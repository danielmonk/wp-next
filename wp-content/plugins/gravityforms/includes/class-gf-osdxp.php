...
class GF_OSDXP
{
    ...
    private function __construct()
    {
        ...
        add_filter('osdxp_license_key_gravityforms', [$this, 'license_key']);
        add_filter(
            'osdxp_dashboard_license_submit_response',
            [$this, 'process_license_key_submit'],
            10,
            3
        );
        add_filter(
            'osdxp_dashboard_license_deletion_response',
            [$this, 'process_license_key_deletion'],
            10,
            2
        );
        add_filter('osdxp_add_module_settings_page', [$this, 'settings_page']);
        add_filter('parent_file', [$this, 'settings_page_highlight']);
        add_action('in_admin_header', [$this, 'nav_tabs']);
    }
     ...
     */
    public static function process_license_key_submit(
        $response,
        $plugin_slug,
        $license_key
    ) {
        ...
        if (!rgempty('is_error', $version_info)) {
            ...
            $success_message = esc_html__(
                'Valid Key : Your license key has been successfully validated.',
                'gravityforms'
            );

            if (
                empty($response['success_messages']) ||
                !is_array($response['success_messages'])
            ) {
            ...
            rgar($version_info, 'reason') == 'requires_enterprise_license'
        ) {
        ...
        if (!empty($error_message)) {
            if (
                empty($response['error_messages']) ||
                !is_array($response['error_messages'])
            ) {
    ...
    public static function process_license_key_deletion($response, $plugin_slug)
    {
        ...
        $response['success_messages'][] = esc_html__(
    ...
    public function settings_page($pages)
    {
        ...
        // System status page.
        $pages[] = [
            'function' => ['GFForms', 'system_status'],
    ...
    public function nav_tabs()
    {
        ...
        if ($this->is_setting_page($plugin_page)) { ?>
            <ul class="osdxp-nav-tabs">
				<?php foreach ($this->slugs as $path => $name) { ?>
                    <li <?php echo $path === $plugin_page
                        ? 'class="active"'
                        : ''; ?>>
