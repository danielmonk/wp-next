...
class GF_Forms_Model_Legacy
{
    ...
    public static function delete_lead($lead_id)
    {
        ...
        $sql = $wpdb->prepare(
            "DELETE FROM $lead_detail_table_name WHERE lead_id=%d",
            $lead_id
        );
        $wpdb->query($sql);
    ...
    public static function delete_leads_by_form($form_id, $status = '')
    {
        ...
        //Delete from lead
        $sql = $wpdb->prepare(
            "DELETE FROM $lead_table WHERE form_id=%d {$status_filter}",
            $form_id
        );
        $wpdb->query($sql);
    }
    ...
    public static function delete_files_by_form($form_id, $status = '')
    {
         ...
         */
        $field_types = gf_apply_filters(
            ['gform_field_types_delete_files', $form_id],
            $field_types,
            $form
        );
        ...
        $results = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id FROM {$wpdb->prefix}rg_lead WHERE form_id=%d {$status_filter}",
                $form_id
            ),
            ARRAY_A
        );
    ...
    public static function delete_file($entry_id, $field_id, $file_index = 0)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT id FROM $lead_detail_table WHERE lead_id=%d AND meta_key = %s",
            $entry_id,
            $field_id
        );
    ...
    public static function delete_field_values($form_id, $field_id)
    {
        ...
        // Delete from lead details
        $sql = $wpdb->prepare(
            "DELETE FROM $lead_detail_table WHERE form_id=%d AND field_number >= %d AND field_number < %d",
            $form_id,
            $field_id,
            $field_id + 1
        );
    ...
    public static function save_lead($form, &$lead)
    {
        ...
        if ($is_new_lead) {
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "INSERT INTO $lead_table(form_id, ip, source_url, date_created, user_agent, currency, created_by) VALUES(%d, %s, %s, utc_timestamp(), %s, %s, {$user_id})",
                    $form['id'],
                    GFFormsModel::get_ip(),
                    $source_url,
                    $user_agent,
        ...
        $current_fields = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id, field_number FROM $lead_detail_table WHERE lead_id=%d",
                $lead['id']
            )
        );
        ...
        foreach ($form['fields'] as $field) {
            ...
            $read_value_from_post =
                $is_new_lead || !isset($lead['date_created']);
            ...
            if (
                ...
                !GFFormsModel::is_field_hidden(
                    ...
                    $field,
                    [],
                    $read_value_from_post ? null : $lead
                )
            ) {
        ...
        if (!empty($calculation_fields)) {
            ...
            GFFormsModel::refresh_product_cache(
                $form,
    ...
    public static function save_input(
        ...
        $value = GFFormsModel::prepare_value(
            $form,
            $field,
            $value,
            $input_name,
        ...
        $lead_detail_id = GFFormsModel::get_lead_detail_id(
            $current_fields,
            $input_id
        );
        $result = GFFormsModel::update_lead_field_value(
            $form,
            $lead,
            $field,
            $lead_detail_id,
        ...
        GFCommon::log_debug(
            __METHOD__ .
                "(): Saving: {$field->label}(#{$input_id} - {$field->type}). Result: " .
                var_export($result, 1)
        );
    }
    ...
    public static function is_duplicate($form_id, $field, $value)
    {
        ...
        switch (GFFormsModel::get_input_type($field)) {
            ...
            case 'phone':
                ...
                $sql_comparison =
                    'replace( replace( replace( replace( ld.value, ")", "" ), "(", "" ), "-", "" ), " ", "" ) = %s';
                break;
            case 'email':
    ...
    public static function get_leads(
        ...
        if (is_numeric($sort_field_number)) {
            $sql = self::sort_by_custom_field_query(
                $form_id,
                $sort_field_number,
                $sort_direction,
                $search,
            ...
            $sql = self::sort_by_default_field_query(
                $form_id,
                $sort_field_number,
                $sort_direction,
                $search,
    ...
    private static function sort_by_custom_field_query(
        $form_id,
        $sort_field_number = 0,
        $sort_direction = 'DESC',
        $search = '',
        ...
        if (
            !is_numeric($form_id) ||
            !is_numeric($sort_field_number) ||
            !is_numeric($offset) ||
            !is_numeric($page_size)
        ) {
            return '';
        }
        ...
        $sort_direction = in_array(strtolower($sort_direction), [
            'desc',
            'asc',
            'rand',
        ])
        ...
        $orderby = $is_numeric_sort
            ? "ORDER BY query, (value+0) $sort_direction"
            : "ORDER BY query, value $sort_direction";
        $is_default = false;

        $search_sql = self::get_leads_where_sql(
            compact(
                'form_id',
                'search',
                'status',
    ...
    private static function sort_by_default_field_query(
        $form_id,
        $sort_field,
        $sort_direction = 'DESC',
        $search = '',
        ...
        $where = self::get_leads_where_sql(
            compact(
                'form_id',
                'search',
                'status',
        ...
        $entry_meta_sql_join = '';
        if (
            false === empty($entry_meta) &&
            array_key_exists($sort_field, $entry_meta)
        ) {
        ...
        if (
            ...
            false === array_key_exists($sort_field, $grid_columns)
        ) {
            $sort_field = 'date_created';
        }
        $orderby = $is_numeric_sort
            ? "ORDER BY ($sort_field+0) $sort_direction"
    ...
    public static function get_submitted_fields($form_id)
    {
        ...
        $fields = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT DISTINCT field_number FROM $lead_detail_table_name WHERE form_id=%d",
                $form_id
            )
        );
    ...
    public static function build_lead_array($results)
    {
        ...
        if (is_array($results) && sizeof($results) > 0) {
            ...
            $lead = [
                ...
                'is_read' => intval($results[0]->is_read),
                'ip' => $results[0]->ip,
                ...
                'is_fulfilled' => $results[0]->is_fulfilled,
                'created_by' => $results[0]->created_by,
                'transaction_type' => $results[0]->transaction_type,
            ...
            foreach ($results as $result) {
                if ($prev_lead_id != $result->id && $prev_lead_id > 0) {
                    ...
                    $lead = [
                        ...
                        'is_starred' => intval($result->is_starred),
        ...
        foreach ($leads as &$lead) {
            foreach ($form['fields'] as $field) {
                ...
                if (is_array($inputs)) {
                    foreach ($inputs as $input) {
                        $lead[(string) $input['id']] = gf_apply_filters(
                            [
                    ...
                    $lead[$field->id] = gf_apply_filters(
        ...
        $entry_meta_data_rows = gform_get_meta_values_for_entries(
            $entry_ids,
            $meta_keys
        );
        foreach ($leads as &$lead) {
    ...
    private static function get_form_id_where($form_id)
    {
        ...
        if (is_array($form_id)) {
            ...
            $form_id_where =
                $form_id > 0 ? $wpdb->prepare('l.form_id=%d', $form_id) : '';
        }

        return $form_id_where;
    }

    private static function sort_by_field_query(
        $form_id,
        $search_criteria,
        $sorting,
        $paging
        ...
        $sort_direction = isset($sorting['direction'])
            ? $sorting['direction']
            : 'DESC';

        $is_numeric_sort = isset($sorting['is_numeric'])
            ? $sorting['is_numeric']
            : false;
    ...
    private static function sort_by_column_query(
        $form_id,
        $search_criteria,
        $sorting,
        $paging
        ...
        if (
            ...
            if (
                $sort_field != 'date_created' &&
                false === in_array($sort_field, $db_columns)
            ) {
                $sort_field = 'date_created';
            }
        }
        ...
        if ($sort_field_is_entry_meta) {
            ...
            $orderby = $is_numeric_sort
                ? "ORDER BY (l.$sort_field+0) $sort_direction"
    ...
    private static function get_search_where($form_id, $search_criteria)
    {
        ...
        $field_filters_where = self::get_field_filters_where(
            $form_id,
            $search_criteria
        );
        ...
        $where = empty($where_arr)
            ? ''
            : '(' . join(" $search_operator ", $where_arr) . ')';

        $date_range_where = self::get_date_range_where($search_criteria);

        $where_and_clause_arr = [];
        ...
        $status_where = isset($search_criteria['status'])
            ? $wpdb->prepare('l.status = %s', $search_criteria['status'])
            : '';
    ...
    private static function get_field_filters_where($form_id, $search_criteria)
    {
        ...
        if (is_array($form_id)) {
            $in_str_arr = array_fill(0, count($form_id), '%d');
            $in_str = join(',', $in_str_arr);
            $form_id_where = $wpdb->prepare(
                "AND form_id IN ($in_str)",
                $form_id
            );
        } else {
            $form_id_where =
                $form_id > 0 ? $wpdb->prepare('AND form_id=%d', $form_id) : '';
        }
        ...
        foreach ($field_filters as $search) {
            ...
            $operator = self::is_valid_operator(rgar($search, 'operator'))
                ? strtolower($search['operator'])
                : '=';
            ...
            switch ($search_type) {
                case 'field':
                    ...
                    $upper_field_number_limit =
                        (string) (int) $key === (string) $key
                            ? (float) $key + 0.9999
                            : (float) $key + 0.0001;
                    ...
                    if (is_array($search_term)) {
                        ...
                        $search_terms = array_fill(
                            0,
                            count($search_term),
                            '%s'
                        );
                        $search_terms_in = $wpdb->prepare(
                        ...
                        $search_term_placeholder =
                            rgar($search, 'is_numeric') || $is_number_field
                                ? '%f'
                                : '%s';
                        /* doesn't support "<>" for checkboxes */
                        $field_query = $wpdb->prepare(
                ...
                case 'global':
                    ...
                    $choice_texts_clauses = [];
                    foreach ($forms as $form) {
                        if (isset($form['fields'])) {
                            $choice_texts_clauses_for_form = [];
                            foreach ($form['fields'] as $field) {
                                /* @var GF_Field $field */
                                $choice_texts_clauses_for_field = [];
                                if (is_array($field->choices)) {
                                    foreach ($field->choices as $choice) {
                                        if (
                                            ($operator == '=' &&
                                                strtolower($choice['text']) ==
                                                    strtolower($val)) ||
                                            ($operator == 'like' &&
                                                !empty($val) &&
                                                strpos(
                                            ...
                                            $choice_texts_clauses_for_field[] = $wpdb->prepare(
                                                "(field_number BETWEEN %s AND %s AND value {$choice_search_operator} %s)",
                                                (float) $field->id - 0.0001,
                                                (float) $field->id + 0.9999,
                                                $choice_value
                                            );
                                        }
                                    }
                                ...
                                if (!empty($choice_texts_clauses_for_field)) {
                                    $choice_texts_clauses_for_form[] = join(
                                        ' OR ',
                                        $choice_texts_clauses_for_field
                                    );
                                }
                        ...
                        if (!empty($choice_texts_clauses_for_form)) {
                            $choice_texts_clauses[] =
                                ...
                                $form['id'] .
                                ' AND (' .
                                join(' OR ', $choice_texts_clauses_for_form) .
                                ' ))';
                        }
                    }
                    $choice_texts_clause = '';
                    if (!empty($choice_texts_clauses)) {
                        $choice_texts_clause = join(
                            ' OR ',
                            $choice_texts_clauses
                        );
                        $choice_texts_clause = "
                    ...
                    $choice_value_clause = $wpdb->prepare(
                        "value {$operator} %s",
                        $search_term
                    );
                    $sql_array[] =
                        '(' . $choice_texts_clause . $choice_value_clause . ')';
                    break;
                case 'meta':
        ...
        $sql = empty($sql_array)
            ? ''
            : join(' ' . $search_operator . ' ', $sql_array);
    ...
    private static function get_date_range_where($search_criteria)
    {
        ...
        $sql = empty($where_array)
            ? ''
            : '(' . join(' AND ', $where_array) . ')';
    ...
    private static function get_search_operator($search_criteria)
    {
        ...
        $search_mode = isset($field_filters['mode'])
            ? strtolower($field_filters['mode'])
            : 'all';
    ...
    private static function get_info_search_where($search_criteria)
    {
        ...
        foreach ($field_filters as $filter) {
            ...
            $operator = GFFormsModel::is_valid_operator(
                rgar($filter, 'operator')
            )
                ? strtolower($filter['operator'])
                : '=';
            ...
            if ('date_created' == $key && '=' === $operator) {
                ...
                $where_array[] = $wpdb->prepare(
                    ...
                    $date_create_start_utc,
                    $date_created_end_utc
                );
            } elseif (in_array($key, $int_columns)) {
                $where_array[] = $wpdb->prepare(
                    "l.{$key} $operator %d",
                    $search_term
                );
        ...
        $sql = empty($where_array)
            ? ''
            : join(" $search_operator ", $where_array);
    ...
    public static function gform_get_meta_values_for_entries(
        $entry_ids,
        $meta_keys
    ) {
        global $wpdb;
     ...
     */
    public static function gform_update_meta(
        $entry_id,
        $meta_key,
        $meta_value,
        $form_id = null
        ...
        if ($meta_exists) {
            $wpdb->update(
                ...
                ['lead_id' => $entry_id, 'meta_key' => $meta_key],
                ['%s'],
                ['%d', '%s']
            );
        } else {
            if (empty($form_id)) {
                ...
                $form_id = $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT form_id from $lead_table_name WHERE id=%d",
                        $entry_id
                    )
                );
            } else {
     ...
     * @param int|null $form_id The form ID of the entry (optional, saves extra query if passed when creating the metadata)
     */
    public static function gform_add_meta(
        $entry_id,
        $meta_key,
        $meta_value,
        $form_id = null
    ...
    public static function gform_delete_meta($entry_id, $meta_key = '')
    {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$table_name} WHERE lead_id=%d {$meta_filter}",
                $entry_id
            )
        );
    ...
    public static function update_entry($entry, $entry_id = null)
    {
        ...
        if (empty($entry_id)) {
            return new WP_Error(
                'missing_entry_id',
                __('Missing entry id', 'gravityforms')
            );
        }
        ...
        if (!$current_entry) {
            return new WP_Error(
                'not_found',
                __('Entry not found', 'gravityforms'),
                $entry_id
            );
        }
        ...
        if (false === self::form_id_exists($form_id)) {
            return new WP_Error(
                'invalid_form_id',
                __('The form for this entry does not exist', 'gravityforms')
            );
        }
        ...
        $post_id = isset($entry['post_id'])
            ? intval($entry['post_id'])
            : 'NULL';
        $date_created = isset($entry['date_created'])
            ? sprintf("'%s'", esc_sql($entry['date_created']))
            : 'utc_timestamp()';
        ...
        $source_url = isset($entry['source_url'])
            ? $entry['source_url']
            : GFFormsModel::get_current_page_url();
        $user_agent = isset($entry['user_agent'])
            ? $entry['user_agent']
            : 'API';
        $currency = isset($entry['currency'])
            ? $entry['currency']
            : GFCommon::get_currency();
        $payment_status = isset($entry['payment_status'])
            ? sprintf("'%s'", esc_sql($entry['payment_status']))
            : 'NULL';
        $payment_date = strtotime(rgar($entry, 'payment_date'))
            ? "'" .
                gmdate('Y-m-d H:i:s', strtotime("{$entry['payment_date']}")) .
                "'"
            : 'NULL';
        $payment_amount = isset($entry['payment_amount'])
            ? (float) $entry['payment_amount']
            : 'NULL';
        $payment_method = isset($entry['payment_method'])
            ? $entry['payment_method']
            : '';
        $transaction_id = isset($entry['transaction_id'])
            ? sprintf("'%s'", esc_sql($entry['transaction_id']))
            : 'NULL';
        $is_fulfilled = isset($entry['is_fulfilled'])
            ? intval($entry['is_fulfilled'])
            : 'NULL';
        $status = isset($entry['status']) ? $entry['status'] : 'active';

        global $current_user;
        $user_id = isset($entry['created_by'])
            ? absint($entry['created_by'])
            : '';
        if (empty($user_id)) {
            $user_id =
        ...
        $transaction_type = isset($entry['transaction_type'])
            ? intval($entry['transaction_type'])
            : 'NULL';
        ...
        if (false === $result) {
            return new WP_Error(
                'update_entry_properties_failed',
                __(
                    'There was a problem while updating the entry properties',
                    'gravityforms'
                ),
                $wpdb->last_error
            );
        ...
        $form = gf_apply_filters(
            ['gform_form_pre_update_entry', $form_id],
            $form,
            $entry,
            $entry_id
        );
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (is_array($inputs)) {
                ...
                $field_id = $field->id;
                $field_value = isset($entry[(string) $field_id])
                    ? $entry[(string) $field_id]
                    : '';
                if ($field_value != $current_entry[$field_id]) {
                    ...
                    if (false === $result) {
                        return new WP_Error(
                            'update_field_values_failed',
                            __(
                                'There was a problem while updating the field values',
                                'gravityforms'
                            ),
                            $wpdb->last_error
                        );
         ...
         */
        gf_do_action(
            ['gform_post_update_entry', $form_id],
            $entry,
            $original_entry
        );
    ...
    public static function add_entry($entry)
    {
        ...
        if (empty($form_id)) {
            return new WP_Error(
                'empty_form_id',
                __('The form id must be specified', 'gravityforms')
            );
        }
        ...
        if (false === $result) {
            return new WP_Error(
                'insert_entry_properties_failed',
                __(
                    'There was a problem while inserting the entry properties',
                    'gravityforms'
                ),
                $wpdb->last_error
            );
        ...
        foreach ($form['fields'] as $field) {
            /* @var GF_Field $field */
            if (in_array($field->type, ['html', 'page', 'section'])) {
                continue;
            }
            $inputs = $field->get_entry_inputs();
            if (is_array($inputs)) {
                ...
                if (false === $result) {
                    return new WP_Error(
                        'insert_field_values_failed',
                        __(
                            'There was a problem while inserting the field values',
                            'gravityforms'
                        ),
                        $wpdb->last_error
                    );
    ...
    public static function update_entry_field($entry_id, $input_id, $value)
    {
        ...
        $lead_detail_id = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT id FROM {$lead_details_table_name} WHERE lead_id=%d AND field_number BETWEEN %s AND %s",
                $entry_id,
                $input_id_min,
                $input_id_max
            )
    ...
    public static function update_lead_field_value(
        ...
        if (!rgblank($value)) {
            ...
            $sql = $wpdb->prepare(
                "DELETE FROM $lead_detail_table WHERE lead_id=%d AND field_number BETWEEN %s AND %s ",
                $lead_id,
                doubleval($input_id) - 0.0001,
                doubleval($input_id) + 0.0001
            );
            $result = $wpdb->query($sql);
            if (false === $result) {
    ...
    public static function add_note(
        ...
        $sql = $wpdb->prepare(
            "INSERT INTO $table_name(lead_id, user_id, user_name, value, note_type, date_created) values(%d, %d, %s, %s, %s, utc_timestamp())",
            $lead_id,
            $user_id,
            $user_name,
            $note,
    ...
    public static function delete_note($note_id)
    {
        ...
        $lead_id = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT lead_id FROM $table_name WHERE id = %d",
                $note_id
            )
        );
