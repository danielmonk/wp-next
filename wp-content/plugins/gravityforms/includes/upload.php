...
class GFAsyncUpload
{
    public static function upload()
    {
        ...
        if (!isset($_REQUEST['form_id'])) {
            GFCommon::log_debug(
                'GFAsyncUpload::upload(): File upload aborted because the form_id was not found. The file may have been bigger than the max post size ini setting.'
            );
        ...
        if (!is_dir($target_dir)) {
            if (!wp_mkdir_p($target_dir)) {
                GFCommon::log_debug(
                    "GFAsyncUpload::upload(): Couldn't create the tmp folder: " .
                        $target_dir
                );
                self::die_error(
                    500,
        ...
        if (!file_exists(GFFormsModel::get_upload_root() . '/index.html')) {
            ...
            !file_exists(
                GFFormsModel::get_upload_path($form_id) . '/index.html'
            )
        ) {
            ...
            !file_exists(
                GFFormsModel::get_upload_path($form_id) . "/$y/index.html"
            )
        ) {
            GFCommon::recursive_add_index_file(
                GFFormsModel::get_upload_path($form_id) . "/$y"
            );
        } else {
            GFCommon::recursive_add_index_file(
                GFFormsModel::get_upload_path($form_id) . "/$y/$m"
            );
        }
         ...
         */
        $field = gf_apply_filters(
            ['gform_multifile_upload_field', $form['id'], $field_id],
            GFFormsModel::get_field($form, $field_id),
            $form,
            $field_id
        );
        ...
        if (
            empty($field) ||
            GFFormsModel::get_input_type($field) != 'fileupload'
        ) {
            die();
        }
        ...
        $max_upload_size_in_bytes =
            $field->maxFileSize > 0
                ? $field->maxFileSize * 1048576
                : wp_max_upload_size();
        $max_upload_size_in_mb = $max_upload_size_in_bytes / 1048576;

        if (
            $_FILES['file']['size'] > 0 &&
            $_FILES['file']['size'] > $max_upload_size_in_bytes
        ) {
            self::die_error(
                104,
                sprintf(
        ...
        if (
            GFCommon::file_name_has_disallowed_extension($file_name) ||
            GFCommon::file_name_has_disallowed_extension($uploaded_filename)
        ) {
            GFCommon::log_debug(
                "GFAsyncUpload::upload(): Illegal file extension: {$file_name}"
            );
            self::die_error(
                104,
                __('The uploaded file type is not allowed.', 'gravityforms')
            );
        }
        ...
        if (!empty($allowed_extensions)) {
            if (
                ...
                GFCommon::log_debug(
                    "GFAsyncUpload::upload(): The uploaded file type is not allowed: {$file_name}"
                );
                self::die_error(
                    104,
                    sprintf(
                        ...
                        strtolower($field['allowedExtensions'])
                    )
                );
            }
        }
        ...
        if (!$whitelisting_disabled) {
            ...
            $valid_uploaded_filename = GFCommon::check_type_and_ext(
                $_FILES['file'],
                $uploaded_filename
            );
            ...
            if (is_wp_error($valid_uploaded_filename)) {
                GFCommon::log_debug(
                    sprintf(
                        ...
                        __METHOD__,
                        $uploaded_filename,
                        $valid_uploaded_filename->get_error_code(),
                        $valid_uploaded_filename->get_error_message()
                    )
                );
                self::die_error(
                    $valid_uploaded_filename->get_error_code(),
            ...
            if (is_wp_error($valid_file_name)) {
                GFCommon::log_debug(
                    sprintf(
                        ...
                        __METHOD__,
                        $file_name,
                        $valid_file_name->get_error_code(),
                        $valid_file_name->get_error_message()
                    )
                );
                self::die_error(
                    $valid_file_name->get_error_code(),
            ...
            if (is_dir($target_dir) && ($dir = opendir($target_dir))) {
                while (($file = readdir($dir)) !== false) {
                    ...
                    if (
                        ...
                        GFCommon::log_debug(
                            'GFAsyncUpload::upload(): Deleting file: ' .
                                $tmp_file_path
                        );
                ...
                GFCommon::log_debug(
                    'GFAsyncUpload::upload(): Failed to open temp directory: ' .
                        $target_dir
                );
                self::die_error(
                    100,
                    __('Failed to open temp directory.', 'gravityforms')
                );
            }
        }
            ...
            if (
                ...
                if ($out) {
                    ...
                    if ($in) {
                        ...
                        self::die_error(
                            101,
                            __('Failed to open input stream.', 'gravityforms')
                        );
                    }
                    ...
                    self::die_error(
                        102,
                        __('Failed to open output stream.', 'gravityforms')
                    );
                }
            } else {
                self::die_error(
                    103,
                    __('Failed to move uploaded file.', 'gravityforms')
                );
            }
        } else {
            // Open temp file
        ...
        if (!$chunks || $chunk == $chunks - 1) {
            ...
            if (file_exists($file_path)) {
                ...
                self::die_error(
                    105,
                    __('Upload unsuccessful', 'gravityforms') .
                        ' ' .
                        $uploaded_filename
                );
            }
            ...
            gf_do_action(
                ['gform_post_multifile_upload', $form['id']],
                $form,
                $field,
                $uploaded_filename,
                $tmp_file_name,
            ...
            GFCommon::log_debug(
                sprintf(
                    'GFAsyncUpload::upload(): File upload complete. temp_filename: %s  uploaded_filename: %s ',
                    $tmp_file_name,
                    $uploaded_filename
                )
            );
            ...
            GFCommon::log_debug(
                sprintf(
                    'GFAsyncUpload::upload(): Chunk upload complete. temp_filename: %s  uploaded_filename: %s chunk: %d',
                    $tmp_file_name,
                    $uploaded_filename,
                    $chunk
                )
        ...
        $output = [
            ...
            'data' => [
                ...
                'uploaded_filename' => str_replace(
                    "\\'",
                    "'",
                    urldecode($uploaded_filename)
