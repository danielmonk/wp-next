...
if (!class_exists('Gravity_Api')) {
    ...
    class Gravity_Api
    {
        ...
        public function register_current_site($license_key, $is_md5 = false)
        {
            ...
            $result = $this->request('sites', $body, 'POST', [
                'headers' => $this->get_license_auth_header($license_key),
            ]);
            ...
            GFCommon::log_debug(
                __METHOD__ .
                    '(): site registration successful. Site Key: ' .
                    $result->key
            );
        ...
        public function update_current_site($new_license_key_md5)
        {
            ...
            $result = $this->request('sites/' . $site_key, $body, 'PUT', [
                'headers' => $this->get_site_auth_header(
                    $site_key,
                    $site_secret
                ),
            ]);
            ...
            if (is_wp_error($result)) {
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): error updating site registration. ' .
                        print_r($result, true)
                );
                return $result;
            }
        ...
        private function get_site_auth_header($site_key, $site_secret)
        {
            ...
            return ['Authorization' => 'GravityAPI ' . $auth];
        }
        ...
        public function prepare_response_body($raw_response)
        {
            if (is_wp_error($raw_response)) {
                ...
                return new WP_Error(
                    'server_error',
                    'Error from server: ' . $raw_response['response']['message']
                );
            }
            ...
            if ($response_body === null) {
                return new WP_Error(
                    'invalid_response',
                    'Invalid response from server: ' . $raw_response['body']
                );
            }
        ...
        public function request(
            ...
            $options = wp_parse_args($options, [
                'method' => $method,
                'timeout' => 10,
                'body' => in_array($method, ['GET', 'DELETE'])
                    ? null
                    : json_encode($body),
                'headers' => [],
                'sslverify' => false,
            ...
            $options['headers'] = wp_parse_args($options['headers'], [
                'Content-Type' =>
