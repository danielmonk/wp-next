...
abstract class GFAddOn
{
    ...
    private $_no_conflict_scripts = [];
    private $_no_conflict_styles = [];
    private $_preview_styles = [];
    private $_print_styles = [];
    private static $_registered_addons = ['active' => [], 'inactive' => []];
    ...
    function __construct()
    {
        ...
        if ($this->_enable_rg_autoupgrade) {
            ...
            $is_gravityforms_supported = $this->is_gravityforms_supported(
    ...
    public static function init_addons()
    {
        ...
        foreach ($active_addons as $addon) {
            call_user_func([$addon, 'get_instance']);
        }
    }
    ...
    public function init()
    {
        ...
        add_filter('gform_logging_supported', [$this, 'set_logging_supported']);

        add_action(
            'gform_post_upgrade',
            [$this, 'post_gravityforms_upgrade'],
            10,
            3
        );
    ...
    public function init_admin()
    {
        ...
        if (
            isset($this->_min_gravityforms_version) &&
            RG_CURRENT_PAGE == 'plugins.php'
        ) {
            add_action(
                'after_plugin_row_' . $this->get_path(),
                [$this, 'plugin_row'],
                10,
                2
            );
        ...
        if (
            isset($this->_min_gravityforms_version) &&
            !$this->is_gravityforms_supported($this->_min_gravityforms_version)
        ) {
            return;
        }
        ...
        if (self::has_app_menu()) {
            if ($this->current_user_can_any($this->_capabilities_app_menu)) {
                add_action('admin_menu', [$this, 'create_app_menu']);
            }
        }
        ...
        if ($this->has_members_plugin()) {
            add_action(
                ...
                [$this, 'members_register_cap_group'],
                11
            );
            add_action(
                'members_register_caps',
                [$this, 'members_register_caps'],
                11
            );
        }
        ...
        add_filter(
            ...
            [$this, 'filter_ure_capabilities_groups_tree'],
            11
        );
        add_filter(
            'ure_custom_capability_groups',
            [$this, 'filter_ure_custom_capability_groups'],
            10,
            2
        );
        ...
        if ($this->method_is_overridden('get_results_page_config')) {
            ...
            if (
                $results_page_config &&
                $this->current_user_can_any($results_capabilities)
            ) {
        ...
        // No conflict scripts
        add_filter('gform_noconflict_scripts', [
            $this,
            'register_noconflict_scripts',
        ]);
        add_filter('gform_noconflict_styles', [
            $this,
    ...
    public function init_ajax()
    {
        if (rgpost('view') == 'gf_results_' . $this->_slug) {
            ...
            $gf_results = new GFResults(
                $this->_slug,
                $this->get_results_page_config()
            );
            add_action(
                'wp_ajax_gresults_get_results_gf_results_' . $this->_slug,
                [$gf_results, 'ajax_get_results']
            );
            add_action(
                'wp_ajax_gresults_get_more_results_gf_results_' . $this->_slug,
                [$gf_results, 'ajax_get_more_results']
            );
    ...
    public function meets_minimum_requirements()
    {
        ...
        // Prepare response.
        $meets_requirements = ['meets_requirements' => true, 'errors' => []];
        ...
        foreach ($requirements as $requirement_type => $requirement) {
            ...
            if (is_callable($requirement)) {
                $meets_requirements = call_user_func(
                    $requirement,
                    $meets_requirements
                );
                continue;
            ...
            switch ($requirement_type) {
                case 'add-ons':
                    ...
                    foreach (
                        self::$_registered_addons['active']
                        as $active_addon
                    ) {
                        // Get Add-On instance.
                        $active_addon = call_user_func([
                            $active_addon,
                            'get_instance',
                        ]);

                        // Add to active Add-Ons array.
                        $active_addons[$active_addon->get_slug()] = [
                    ...
                    foreach (
                        $requirement
                        as $addon_slug => $addon_requirements
                    ) {
                        ...
                        if (!isset($active_addons[$addon_slug])) {
                            // Get Add-On name.
                            $addon_name = rgar($addon_requirements, 'name')
                                ? $addon_requirements['name']
                                : $addon_slug;

                            $meets_requirements['meets_requirements'] = false;
                            $meets_requirements['errors'][] = sprintf(
                                esc_html__(
                                    'Required Gravity Forms Add-On is missing: %s.',
                                    'gravityforms'
                                ),
                                $addon_name
                            );
                        ...
                        if (
                            rgar($addon_requirements, 'version') &&
                            !version_compare(
                                $active_addons[$addon_slug]['version'],
                                $addon_requirements['version'],
                                '>='
                            )
                        ) {
                            $meets_requirements['meets_requirements'] = false;
                            $meets_requirements['errors'][] = sprintf(
                                esc_html__(
                                    'Required Gravity Forms Add-On "%s" does not meet minimum version requirement: %s.',
                                    'gravityforms'
                                ),
                                $active_addons[$addon_slug]['title'],
                                $addon_requirements['version']
                            );
                            continue;
                        }
                ...
                case 'plugins':
                    ...
                    foreach ($requirement as $plugin_path => $plugin_name) {
                        ...
                        if (!is_plugin_active($plugin_path)) {
                            ...
                            $meets_requirements['errors'][] = sprintf(
                                esc_html__(
                                    'Required WordPress plugin is missing: %s.',
                                    'gravityforms'
                                ),
                                $plugin_name
                            );
                ...
                case 'php':
                    ...
                    if (
                        ...
                        $meets_requirements['errors'][] = sprintf(
                            esc_html__(
                                'Current PHP version (%s) does not meet minimum PHP version requirement (%s).',
                                'gravityforms'
                            ),
                            PHP_VERSION,
                            $requirement['version']
                    ...
                    if (rgar($requirement, 'extensions')) {
                        ...
                        foreach (
                            ...
                            as $extension => $extension_requirements
                        ) {
                            ...
                            if (
                                rgar($extension_requirements, 'version') &&
                                !version_compare(
                                    phpversion($extension),
                                    $extension_requirements['version'],
                                    '>='
                                )
                            ) {
                                $meets_requirements[
                                ...
                                $meets_requirements['errors'][] = sprintf(
                                    esc_html__(
                                        'Required PHP extension "%s" does not meet minimum version requirement: %s.',
                                        'gravityforms'
                                    ),
                                    $extension,
                                    $extension_requirements['version']
                    ...
                    if (rgar($requirement, 'functions')) {
                        ...
                        foreach ($requirement['functions'] as $function) {
                            if (!function_exists($function)) {
                                ...
                                $meets_requirements['errors'][] = sprintf(
                                    esc_html__(
                                        'Required PHP function missing: %s',
                                        'gravityforms'
                                    ),
                                    $function
                                );
                ...
                case 'wordpress':
                    ...
                    if (
                        ...
                        $meets_requirements['errors'][] = sprintf(
                            esc_html__(
                                'Current WordPress version (%s) does not meet minimum WordPress version requirement (%s).',
                                'gravityforms'
                            ),
                            get_bloginfo('version'),
                            $requirement['version']
    ...
    public function failed_requirements_init()
    {
        ...
        $error_message = sprintf(
            ...
            sprintf(
                esc_html__(
                    '%s is not able to run because your WordPress environment has not met the minimum requirements.',
                    'gravityforms'
                ),
                $this->_title
            ),
            sprintf(
                esc_html__(
                    'Please resolve the following issues to use %s:',
                    'gravityforms'
                ),
        ...
        if (
            ...
            GFForms::get_page() === 'system_status'
        ) {
    ...
    public function setup()
    {
        ...
        $installed_version = get_option(
            'gravityformsaddon_' . $this->_slug . '_version'
        );
        ...
        if ($installed_version != $this->_version) {
            $installed_version = GFForms::get_wp_option(
                'gravityformsaddon_' . $this->_slug . '_version'
            );
        }
     ...
     */
    public function post_gravityforms_upgrade(
        $db_version,
        $previous_db_version,
        $force_upgrade
    ) {
    ...
    public function styles()
    {
        ...
        return [
            ...
            [
                ...
                'src' =>
                    GFAddOn::get_gfaddon_base_url() .
                    "/css/gaddon_results{$min}.css",
                'version' => GFCommon::$version,
                'enqueue' => [['admin_page' => ['results']]],
            ],
        ];
    }
    ...
    public function scripts()
    {
        ...
        return [
            ...
            [
                ...
                'callback' => ['GFResults', 'localize_results_scripts'],
                'enqueue' => [['admin_page' => ['results']]],
            ],
            [
                'handle' => 'gaddon_repeater',
                'src' =>
                    GFAddOn::get_gfaddon_base_url() . "/js/repeater{$min}.js",
                'version' => GFCommon::$version,
                'deps' => ['jquery'],
                'enqueue' => [
                    [
            ...
            [
                ...
                'src' =>
                    GFAddOn::get_gfaddon_base_url() .
                    "/js/gaddon_fieldmap{$min}.js",
                'version' => GFCommon::$version,
                'deps' => ['jquery', 'gaddon_repeater'],
                'enqueue' => [['admin_page' => ['form_settings']]],
            ],
            [
    ...
    public function enqueue_scripts($form = '', $is_ajax = false)
    {
        ...
        foreach ($scripts as $script) {
            ...
            $version = array_key_exists('version', $script)
                ? $script['version']
                : false;
            ...
            if (
                isset($script['enqueue']) &&
                $this->_can_enqueue_script($script['enqueue'], $form, $is_ajax)
            ) {
                $this->add_no_conflict_scripts([$script['handle']]);
                wp_enqueue_script($script['handle']);
                if (isset($script['strings'])) {
                    wp_localize_script(
                        $script['handle'],
                        $script['handle'] . '_strings',
                        $script['strings']
                    );
                }
                if (
                    isset($script['callback']) &&
                    is_callable($script['callback'])
                ) {
        ...
        foreach ($styles as $style) {
            ...
            $version = array_key_exists('version', $style)
                ? $style['version']
                : false;
            $media = isset($style['media']) ? $style['media'] : 'all';
            wp_register_style($style['handle'], $src, $deps, $version, $media);
            if (
                $this->_can_enqueue_script($style['enqueue'], $form, $is_ajax)
            ) {
                $this->add_no_conflict_styles([$style['handle']]);
                if ($this->is_preview()) {
    ...
    {
        $this->_no_conflict_scripts = array_merge(
            $scripts,
            $this->_no_conflict_scripts
        );
    }
    ...
    {
        $this->_no_conflict_styles = array_merge(
            $styles,
            $this->_no_conflict_styles
        );
    }

    private function _can_enqueue_script(
        $enqueue_conditions,
        $form = [],
        $is_ajax = false
    ) {
        ...
        foreach ($enqueue_conditions as $condition) {
            if (is_callable($condition)) {
                ...
                $query_matches = isset($condition['query'])
                    ? $this->_request_condition_matches(
                        $_GET,
                        $condition['query']
                    )
                    : true;
                $post_matches = isset($condition['post'])
                    ? $this->_request_condition_matches(
                        $_POST,
                        $condition['post']
                    )
                    : true;
                $admin_page_matches = isset($condition['admin_page'])
                    ? $this->_page_condition_matches(
                        $condition['admin_page'],
                        rgar($condition, 'tab')
                    )
                    : true;
                $field_type_matches = isset($condition['field_types'])
                    ? $this->_field_condition_matches(
                        $condition['field_types'],
                        $form
                    )
                    : true;
    ...
    public function results_page_init($results_page_config)
    {
        ...
        if (isset($results_page_config['callbacks']['filters'])) {
            add_filter(
                ...
                $results_page_config['callbacks']['filters'],
                10,
                2
            );
        }
        ...
        if (isset($results_page_config['callbacks']['filter_ui'])) {
            add_filter(
                'gform_filter_ui',
                $results_page_config['callbacks']['filter_ui'],
                10,
                5
            );
        }
    ...
    {
        members_register_cap_group('gravityforms_addons', [
    ...
    public function get_members_caps()
    {
        ...
        // Add capabilities.
        if (
            !empty($this->_capabilities_form_settings) &&
            is_string($this->_capabilities_form_settings)
        ) {
            $caps[$this->_capabilities_form_settings] = esc_html__(
                'Form Settings',
                'gravityforms'
            );
        }
        if (
            !empty($this->_capabilities_uninstall) &&
            is_string($this->_capabilities_uninstall)
        ) {
            $caps[$this->_capabilities_uninstall] = esc_html__(
                'Uninstall',
                'gravityforms'
            );
        }
        if (
            !empty($this->_capabilities_plugin_page) &&
            is_string($this->_capabilities_plugin_page)
        ) {
            $caps[$this->_capabilities_plugin_page] = esc_html__(
                'Add-On Page',
                'gravityforms'
            );
        }
        if (
            !empty($this->_capabilities_settings_page) &&
            is_string($this->_capabilities_settings_page)
        ) {
            $caps[$this->_capabilities_settings_page] = esc_html__(
                'Add-On Settings',
                'gravityforms'
            );
        }
        ...
        if (
            !empty($results_cap) &&
            $results_cap !== 'gravityforms_view_entries' &&
            !isset($caps[$results_cap])
        ) {
     ...
     * @return array
     */
    public function filter_ure_custom_capability_groups(
        $groups = [],
        $cap_id = ''
    ) {
    ...
    public function render_settings($sections)
    {
		...
		<form id="gform-settings" action="" method="post">
			<?php wp_nonce_field(
       $this->_slug . '_save_settings',
       '_' . $this->_slug . '_save_settings_nonce'
   ); ?>
    ...
    public function single_section($section, $is_first = false)
    {
        ...
        if (is_callable([$this, "single_setting_row_{$field['type']}"])) {
            call_user_func(
                [$this, "single_setting_row_{$field['type']}"],
                $field
            );
        } else {
    ...
    {
        $display =
            rgar($field, 'hidden') || rgar($field, 'type') == 'hidden'
                ? 'style="display:none;"'
                : '';
		...
		<tr id="gaddon-setting-row-<?php echo $field[
      'name'
  ]; ?>" <?php echo $display; ?>>
			<th>
    ...
    public function single_setting($field)
    {
        ...
        } elseif (is_callable([$this, "settings_{$field['type']}"])) {
            call_user_func([$this, "settings_{$field['type']}"], $field);
        } else {
            printf(
                esc_html__(
                    "Field type '%s' has not been implemented",
                    'gravityforms'
                ),
                esc_html($field['type'])
            );
        ...
        if (count($_POST) > 0) {
            foreach ($_POST as $key => $value) {
                if (preg_match('|_gaddon_setting_(.*)|', $key, $matches)) {
                    $_gaddon_posted_settings[
                        $matches[1]
                    ] = self::maybe_decode_json(stripslashes_deep($value));
                }
            }
        }
    ...
    public static function is_json($value)
    {
        if (
            ...
            is_array(json_decode($value, ARRAY_A))
        ) {
            return true;
        }
    ...
    {
        $sections[count($sections) - 1]['fields'][] = ['type' => 'save'];
    ...
    public function get_save_success_message($sections)
    {
        ...
        return isset($save_button['messages']['success'])
            ? $save_button['messages']['success']
            : sprintf(
                esc_html__('%s settings updated.', 'gravityforms'),
                $this->get_short_title()
            );
    }
    ...
    public function get_save_error_message($sections)
    {
        ...
        return isset($save_button['messages']['error'])
    ...
    public function settings_text($field, $echo = true)
    {
        ...
        $field['input_type'] = rgar($field, 'input_type')
            ? rgar($field, 'input_type')
            : 'text';
        $attributes = $this->get_field_attributes($field);
        $default_value = rgar($field, 'value')
            ? rgar($field, 'value')
            : rgar($field, 'default_value');
        ...
        if (is_callable($feedback_callback)) {
            $is_valid = call_user_func_array($feedback_callback, [
                $value,
                $field,
            ]);
            $icon = '';
    ...
    public function settings_textarea($field, $echo = true)
    {
        ...
        if (rgar($field, 'use_editor')) {
            $html .=
                '<span class="mt-gaddon-editor mt-_gaddon_setting_' .
                $field['name'] .
                '"></span>';

            ob_start();

            wp_editor($value, '_gaddon_setting_' . $field['name'], [
                'autop' => false,
                'editor_class' =>
                    'merge-tag-support mt-wp_editor mt-manual_position mt-position-right',
            ]);
    ...
    public function settings_checkbox($field, $echo = true)
    {
        ...
        $horizontal =
            rgar($field, 'horizontal') || $have_icon
        ...
        $default_choice_attributes = [
            'onclick' =>
                'jQuery(this).siblings("input[type=hidden]").val(jQuery(this).prop("checked") ? 1 : 0);',
            'onkeypress' =>
                'jQuery(this).siblings("input[type=hidden]").val(jQuery(this).prop("checked") ? 1 : 0);',
        ];
        ...
        if (is_array($field['choices'])) {
            foreach ($field['choices'] as $choice) {
                ...
                $choice_attributes = $this->get_choice_attributes(
                    $choice,
                    $field_attributes,
                    $default_choice_attributes
                );
                ...
                $error_icon = '';
                if ($is_first_choice) {
                    $error_icon = $this->field_failed_validation($field)
                        ? $this->get_error_icon($field)
                        : '';
                }

                // Add icon to choice if choices have icon
                if ($have_icon) {
                    $choice['icon'] = rgar($choice, 'icon')
                        ? $choice['icon']
                        : 'fa-cog';
                }

                $html .= $this->checkbox_item(
                    $choice,
                    $horizontal,
                    $choice_attributes,
                    $value,
    ...
    public function checkbox_item(
        ...
        $icon_class = rgar($choice, 'icon')
            ? ' gaddon-setting-choice-visual'
            : '';

        $checkbox_item =
            '<div id="gaddon-setting-checkbox-choice-' .
            $choice['id'] .
            '" class="gaddon-setting-checkbox' .
            $horizontal_class .
            $icon_class .
            '">';
        $checkbox_item .=
            '<input type=hidden name="_gaddon_setting_' .
            esc_attr($choice['name']) .
            '" value="' .
            $hidden_field_value .
            '" />';

        if (is_callable([$this, "checkbox_input_{$choice['name']}"])) {
            $markup = call_user_func(
                [$this, "checkbox_input_{$choice['name']}"],
                $choice,
                $attributes,
                $value,
                $tooltip
    ...
    public function settings_radio($field, $echo = true)
    {
        ...
        $selected_value = $this->get_setting(
            $field['name'],
            rgar($field, 'default_value')
        );
        $field_attributes = $this->get_field_attributes($field);
        ...
        if (is_array($field['choices'])) {
            foreach ($field['choices'] as $i => $choice) {
                ...
                $tooltip = $this->maybe_get_tooltip($choice);
                $radio_value = isset($choice['value'])
                    ? $choice['value']
                    : $choice['label'];
                ...
                if ($have_icon) {
                    ...
                    $html .=
                        '<div id="gaddon-setting-radio-choice-' .
                        $choice['id'] .
                        '" class="gaddon-setting-radio gaddon-setting-choice-visual' .
                        $horizontal .
                        '">';
                    $html .=
                        '<input type="radio" name="_gaddon_setting_' .
                        esc_attr($field['name']) .
                        '" ' .
                        'value="' .
                        $radio_value .
    ...
    public function settings_select($field, $echo = true)
    {
        ...
        if (
            (empty($field['choices']) || !rgar($field, 'choices')) &&
            rgar($field, 'no_choices')
        ) {
    ...
    public function settings_select_custom($field, $echo = true)
    {
        ...
        $select_field = $field;
        $select_field_value = $this->get_setting(
            $select_field['name'],
            rgar($select_field, 'default_value')
        );
        $select_field['onchange'] = '';
        $select_field['class'] = isset($select_field['class'])
            ? $select_field['class'] . 'gaddon-setting-select-custom'
            : 'gaddon-setting-select-custom';
        ...
        foreach ($select_field['choices'] as $choice) {
            if (
                rgar($choice, 'name') == 'gf_custom' ||
                rgar($choice, 'value') == 'gf_custom'
            ) {
                $has_gf_custom = true;
            }
            ...
            if (rgar($choice, 'choices')) {
                foreach ($choice['choices'] as $subchoice) {
                    if (
                        rgar($subchoice, 'name') == 'gf_custom' ||
                        rgar($subchoice, 'value') == 'gf_custom'
                    ) {
                        $has_gf_custom = true;
                    }
                }
        ...
        if (
            $select_field_value == 'gf_custom' ||
            (count($select_field['choices']) == 1 &&
                $select_field['choices'][0]['value'] == 'gf_custom')
        ) {
        ...
        /* Add input field */
        $html .=
            '<div class="gaddon-setting-select-custom-container"' .
            $input_field_display .
            '>';
        $html .=
            count($select_field['choices']) > 1
    ...
    public function get_select_option($choice, $selected_value)
    {
        if (is_array($selected_value)) {
            $selected = in_array($choice['value'], $selected_value)
                ? "selected='selected'"
                : '';
        } else {
    ...
    public function settings_generic_map($field, $echo = true)
    {
        ...
        foreach (['key', 'value'] as $type) {
            if (isset($field[$type . '_field_title'])) {
                $field[$type . '_field']['title'] =
                    $field[$type . '_field_title'];
        ...
        $key_field['name'] .= '_key';
        $key_field['choices'] = rgar($field, 'key_choices')
            ? $field['key_choices']
            : rgars($field, 'key_field/choices');
        ...
        $custom_key_field['placeholder'] = rgars(
            $field,
            'key_field/placeholder'
        )
            ? $field['key_field']['placeholder']
            : esc_html__('Custom Key', 'gravityforms');

        // Define value field properties.
        $value_field['name'] .= '_custom_value';
        $value_field['choices'] = rgar($field, 'value_choices')
            ? $field['value_choices']
            : rgars($field, 'value_field/choices');
        ...
        $custom_value_field['placeholder'] = rgars(
            $field,
            'value_field/placeholder'
        )
            ? $field['value_field']['placeholder']
            : esc_html__('Custom Value', 'gravityforms');

        // Get key/field column titles.
        $key_field_title = rgars($field, 'key_field/title')
            ? $field['key_field']['title']
            : esc_html__('Key', 'gravityforms');
        $value_field_title = rgars($field, 'value_field/title')
            ? $field['value_field']['title']
            : esc_html__('Value', 'gravityforms');
        ...
        $html .=
            ...
            "',
					'mergeTags':      " .
            var_export($field['merge_tags'], true) .
            ",
					'limit':          '" .
            $limit .
            "'
    ...
    public function settings_field_map($field, $echo = true)
    {
        ...
        foreach ($field['field_map'] as $child_field) {
            ...
            if (
                !$this->setting_dependency_met(rgar($child_field, 'dependency'))
            ) {
                continue;
            }

            // Get field map choice name, tooltip and required indicator.
            $child_field['name'] = $this->get_mapped_field_name(
                $field,
                $child_field['name']
            );
            $tooltip = $this->maybe_get_tooltip($child_field);
            $required = rgar($child_field, 'required')
                ? ' ' . $this->get_required_indicator($child_field)
                : '';
    ...
    public function settings_dynamic_field_map($field, $echo = true)
    {
        ...
        // Define key field properties.
        $key_field['name'] .= '_key';
        $key_field['choices'] = isset($field['field_map'])
            ? $field['field_map']
            : null;
        $key_field['class'] = 'key key_{i}';
        ...
        unset(
            $field['field_map'],
            $value_field['field_map'],
            $key_field['field_map'],
            $custom_key_field['field_map']
        );
    ...
    public function settings_field_map_select($field, $form_id)
    {
        ...
        $field_type = rgempty('field_type', $field)
            ? null
            : $field['field_type'];

        // Get field types to exclude.
        $exclude_field_types = rgempty('exclude_field_types', $field)
            ? null
            : $field['exclude_field_types'];

        // Get form field choices based on field type inclusions/exclusions.
        $field['choices'] = $this->get_field_map_choices(
            $form_id,
            $field_type,
            $exclude_field_types
        );
        ...
        if (
            ...
            (count($field['choices']) == 1 &&
                rgblank($field['choices'][0]['value']))
        ) {
            if (
                (!is_array($field_type) && !rgblank($field_type)) ||
                (is_array($field_type) && count($field_type) == 1)
            ) {
                $type = is_array($field_type) ? $field_type[0] : $field_type;
                $type = ucfirst(
                    GF_Fields::get($type)->get_form_editor_field_title()
                );

                return sprintf(
                    __('Please add a %s field to your form.', 'gravityforms'),
                    $type
                );
            }
        }
    ...
    public function get_mapping_field($type, $select_field, $text_field)
    {
        ...
        if (
            isset($select_field['choices']) &&
            !is_array($select_field['choices']) &&
            'form_fields' === strtolower($select_field['choices'])
        ) {
            // Set choices to form fields.
            $select_field['choices'] = $this->get_field_map_choices(
                rgget('id')
            );
        }
        ...
        if (empty($select_field['choices'])) {
            ...
            if (!$has_gf_custom) {
                if ($type == 'key') {
                    $enable_custom = rgars(
                        $select_field,
                        'key_field/custom_value'
                    )
                        ? (bool) $select_field['key_field']['custom_value']
                        : !(bool) rgar($select_field, 'disable_custom');
                    $enable_custom = isset($select_field['enable_custom_key'])
                    ...
                    $enable_custom = rgars(
                        $select_field,
                        'value_field/custom_value'
                    )
                        ? (bool) $select_field['value_field']['custom_value']
                        : (bool) rgars($select_field, 'enable_custom_value');
    ...
    public function field_map_table_header()
    {
        return '<thead>
            ...
            '</th>
						<th>' .
            esc_html__('Form Field', 'gravityforms') .
            '</th>
					</tr>
				</thead>';
    }
     ...
     * @return array
     */
    public static function get_field_map_choices(
        $form_id,
        $field_type = null,
        $exclude_field_types = null
    ) {
        ...
        if (
            ...
            $first_choice_label = sprintf(
                __('Select a %s Field', 'gravityforms'),
                $type
            );
        }

        $fields[] = ['value' => '', 'label' => $first_choice_label];
        ...
        if (is_array($form['fields'])) {
            foreach ($form['fields'] as $field) {
                ...
                $field_is_valid_type =
                    empty($field_type) ||
                    (is_array($field_type) &&
                        in_array($input_type, $field_type)) ||
                    (!empty($field_type) && $input_type == $field_type);
        ...
        if (function_exists('get_called_class')) {
            $callable = [get_called_class(), 'get_instance'];
            if (is_callable($callable)) {
                $add_on = call_user_func($callable);
                $slug = $add_on->get_slug();

                $fields = apply_filters(
                    "gform_{$slug}_field_map_choices",
                    $fields,
                    $form_id,
                    $field_type,
                    $exclude_field_types
    ...
    public static function get_dynamic_field_map_fields($feed, $field_name)
    {
        ...
        if (!empty($dynamic_fields)) {
            ...
            foreach ($dynamic_fields as $dynamic_field) {
                ...
                $field_key =
                    'gf_custom' === $dynamic_field['key']
                        ? $dynamic_field['custom_key']
                        : $dynamic_field['key'];
     ...
     * @return array
     */
    public function get_generic_map_fields(
        $feed,
        $field_name,
        $form = [],
        $entry = []
        ...
        $generic_fields = rgar($feed, 'meta')
            ? rgars($feed, 'meta/' . $field_name)
            : rgar($feed, $field_name);
        ...
        if (!empty($generic_fields)) {
            ...
            foreach ($generic_fields as $generic_field) {
                // Get mapped key or replace with custom value.
                $field_key =
                    'gf_custom' === $generic_field['key']
                        ? $generic_field['custom_key']
                        : $generic_field['key'];
                ...
                if ('gf_custom' === $generic_field['value']) {
                    ...
                    $field_value = empty($form)
                        ? $generic_field['custom_value']
    ...
    public function prepare_field_select_field($field)
    {
        ...
        if (!$args['disable_first_choice']) {
            // Setup first choice
            if (
                empty($args['input_types']) ||
                (is_array($args['input_types']) &&
                    count($args['input_types']) > 1)
            ) {
                $first_choice_label = __('Select a Field', 'gravityforms');
            } else {
                $type = is_array($args['input_types'])
                    ? $args['input_types'][0]
                    : $args['input_types'];
                $type = ucfirst(
                    GF_Fields::get($type)->get_form_editor_field_title()
        ...
        $field['choices'] = array_merge(
            $field['choices'],
            $this->get_form_fields_as_choices($this->get_current_form(), $args)
        );
    ...
    public function get_default_field_select_field($field)
    {
        ...
        if (rgar($field, 'auto_mapping') !== false) {
            ...
            $default_value_choices = [$field_label];

            // Define global aliases to help with the common case mappings.
            $global_aliases = [
                __('First Name', 'gravityforms') => [
                    __('Name (First)', 'gravityforms'),
                ],
                __('Last Name', 'gravityforms') => [
                    __('Name (Last)', 'gravityforms'),
                ],
                __('Address', 'gravityforms') => [
                    __('Address (Street Address)', 'gravityforms'),
                ],
                __('Address 2', 'gravityforms') => [
                    __('Address (Address Line 2)', 'gravityforms'),
                ],
                __('City', 'gravityforms') => [
                    __('Address (City)', 'gravityforms'),
                ],
                __('State', 'gravityforms') => [
                    __('Address (State / Province)', 'gravityforms'),
                ],
                __('Zip', 'gravityforms') => [
                    __('Address (Zip / Postal Code)', 'gravityforms'),
                ],
                __('Country', 'gravityforms') => [
                    __('Address (Country)', 'gravityforms'),
                ],
            ];
            ...
            if (isset($global_aliases[$field_label])) {
                $default_value_choices = array_merge(
                    $default_value_choices,
                    $global_aliases[$field_label]
                );
            }
            ...
            if (rgars($field, 'default_value/aliases')) {
                $default_value_choices = array_merge(
                    $default_value_choices,
                    $field['default_value']['aliases']
                );
            }

            // Convert all auto-population choices to lowercase.
            $default_value_choices = array_map(
                'strtolower',
                $default_value_choices
            );
    ...
    public function get_form_fields_as_choices($form, $args = [])
    {
        ...
        foreach ($form['fields'] as $field) {
            if (
                !empty($args['field_types']) &&
                !in_array($field->type, $args['field_types'])
            ) {
                continue;
            }
            ...
            $is_applicable_input_type =
                empty($args['input_types']) ||
                in_array($input_type, $args['input_types']);

            if (is_callable($args['callback'])) {
                $is_applicable_input_type = call_user_func(
                    $args['callback'],
                    $is_applicable_input_type,
                    $field,
                    $form
            ...
            if (
                ...
                (!isset($field->{$args['property']}) ||
                    $field->{$args['property']} != $args['property_value'])
            ) {
                continue;
            }
    ...
    public function get_field_attributes($field, $default = [])
    {
        ...
        $props['id'] = rgempty('id', $props)
            ? rgar($props, 'name')
            : rgar($props, 'id');
        $props['class'] = trim(
            "{$props['class']} gaddon-setting gaddon-{$props['type']}"
        );
    ...
    public function get_choice_attributes(
        ...
        foreach ($choice as $prop => $val) {
            ...
            if (
                in_array($prop, $no_output_choice_attributes) ||
                is_array($val)
            ) {
        ...
        foreach (
            $default_choice_attributes
            as $default_attr_name => $default_attr_value
        ) {
            if (isset($choice_attributes[$default_attr_name])) {
                $choice_attributes[
                    $default_attr_name
                ] = $this->prepend_attribute(
                    $default_attr_name,
                    $default_attr_value,
                    $choice_attributes[$default_attr_name]
                );
            } else {
                $choice_attributes[$default_attr_name] =
                    "{$default_attr_name}='" .
    ...
    public function validate_settings($fields, $settings)
    {
        foreach ($fields as $section) {
            ...
            foreach ($section['fields'] as $field) {
                if (
                    !$this->setting_dependency_met(rgar($field, 'dependency'))
                ) {
                    continue;
                }
                ...
                if (is_callable(rgar($field, 'validation_callback'))) {
                    call_user_func(
                        rgar($field, 'validation_callback'),
                        $field,
                        $field_setting
                    );
                    continue;
                ...
                if (
                    is_callable([
                        $this,
                        'validate_' . $field['type'] . '_settings',
                    ])
                ) {
                    call_user_func(
                        [$this, 'validate_' . $field['type'] . '_settings'],
                        $field,
                        $settings
                    );
                    continue;
    ...
    public function validate_text_settings($field, $settings)
    {
        ...
        if ($field_setting !== $field_setting_safe) {
            ...
            $script = sprintf(
                'jQuery("input[name=\"_gaddon_setting_%s\"]").val(jQuery(this).data("safe"));',
                $field['name']
            );
            $double_encoded_safe_value = htmlspecialchars(
                htmlspecialchars($field_setting_safe, ENT_QUOTES),
                ENT_QUOTES
            );
            $message .= sprintf(
    ...
    public function validate_textarea_settings($field, $settings)
    {
        ...
        if ($field_setting !== $field_setting_safe) {
            ...
            $script = sprintf(
                'jQuery("textarea[name=\"_gaddon_setting_%s\"]").val(jQuery(this).data("safe"));',
                $field['name']
            );
            $double_encoded_safe_value = htmlspecialchars(
                htmlspecialchars($field_setting_safe, ENT_QUOTES),
    ...
    public function validate_radio_settings($field, $settings)
    {
        ...
        $this->set_field_error(
            $field,
            esc_html__('Invalid value', 'gravityforms')
        );
    }
    ...
    public function validate_select_custom_settings($field, $settings)
    {
        ...
        if (
            rgar($field, 'required') &&
            $select_value == 'gf_custom' &&
            rgblank($custom_value)
        ) {
            $custom_field = $field;
    ...
    public function validate_field_map_settings($field, $settings)
    {
        ...
        foreach ($field_map as $child_field) {
            ...
            if (rgar($child_field, 'required') && rgblank($setting_value)) {
                ...
                rgar($child_field, 'validation_callback') &&
                is_callable(rgar($child_field, 'validation_callback'))
            ) {
                call_user_func(
                    rgar($child_field, 'validation_callback'),
                    $child_field,
    ...
    {
        $choice_value = isset($choice['value'])
            ? $choice['value']
            : $choice['label'];

        return is_array($value)
            ? in_array($choice_value, $value)
            : $choice_value == $value;
    }
    ...
    public function get_field_errors($field = false)
    {
        ...
        return isset($this->_setting_field_errors[$field['name']])
            ? $this->_setting_field_errors[$field['name']]
            : [];
    }
    ...
    public function get_error_icon($field)
    {
        ...
        return '<span
            ...
            title="<h6>' .
            esc_html__('Validation Error', 'gravityforms') .
            '</h6>' .
            $error .
            '"
        ...
        foreach ($fields as $section) {
            ...
            foreach ($section['fields'] as $field) {
                ...
                if (is_callable(rgar($field, 'save_callback'))) {
                    $settings[$field['name']] = call_user_func(
                        rgar($field, 'save_callback'),
                        $field,
                        $field_setting
                    );
                    continue;
    ...
    public function simple_condition($setting_name_root)
    {
        ...
        $value_input = esc_js(
            '_gaddon_setting_' . esc_attr($setting_name_root) . '_value'
        );
        ...
        $str = $this->settings_select(
            [
                ...
                'onchange' =>
                    "jQuery('#" .
                    esc_js($setting_name_root) .
                    "_container').html(GetRuleValues('{$object_type}', 0, jQuery(this).val(), '', '{$value_input}'));",
            ],
            false
        );
        ...
        $str .= $this->settings_select(
            [
                ...
                'type' => 'select',
                'onchange' =>
                    "SetRuleProperty('{$object_type}', 0, 'operator', jQuery(this).val()); jQuery('#" .
                    esc_js($setting_name_root) .
                    "_container').html(GetRuleValues('{$object_type}', 0, jQuery('#{$setting_name_root}_field_id').val(), '', '{$value_input}'));",
                'choices' => [
                    [
                        'value' => 'is',
        ...
        $str .= sprintf(
            "<span id='%s_container'></span>",
            esc_attr($setting_name_root)
        );
        ...
        $field_id_attribute = !empty($field_id)
            ? $field_id
            : 'jQuery("#' . esc_attr($setting_name_root) . '_field_id").val()';

        $str .=
            "<script type='text/javascript'>
			var " .
            esc_attr($setting_name_root) .
            "_object = {'conditionalLogic':{'rules':[{'fieldId':'{$field_id}','operator':'{$operator}','value':'" .
            esc_attr($value) .
            "'}]}};
            ...
            esc_attr($setting_name_root) .
            "_container').html(
											GetRuleValues('{$object_type}', 0, {$field_id_attribute}, '" .
            esc_attr($value) .
            "', '_gaddon_setting_" .
            esc_attr($setting_name_root) .
            "_value'));
     ...
     * @return bool
     */
    public function is_simple_condition_met(
        $setting_name_root,
        $form,
        $entry,
        $feed = []
        ...
        $logic = [
            ...
            'rules' => [
                [
                    'fieldId' => rgar(
                        $settings,
                        $setting_name_root . '_field_id'
                    ),
                    'operator' => rgar(
                        $settings,
                        $setting_name_root . '_operator'
                    ),
    ...
    public function form_settings_init()
    {
        ...
        if (
            ...
            $view == 'settings' &&
            $subview == $this->_slug &&
            $this->current_user_can_any($this->_capabilities_form_settings)
        ) {
            require_once GFCommon::get_base_path() . '/tooltips.php';
            add_action('gform_form_settings_page_' . $this->_slug, [
                $this,
                'form_settings_page',
            ]);
        }
    ...
    public function create_plugin_page_menu($menus)
    {
        $menus[] = [
            ...
            'callback' => [$this, 'plugin_page_container'],
            'permission' => $this->_capabilities_plugin_page,
        ];
   ...
   if ($this->method_is_overridden('form_settings')) {
       ...
       ?>
				...
				<?php //rendering settings based on fields and current settings
   ...
   ?>
        ...
        if ($this->is_save_postback()) {
            ...
            if ($result) {
                GFCommon::add_message(
                    $this->get_save_success_message($sections)
                );
            } else {
                GFCommon::add_error_message(
                    $this->get_save_error_message($sections)
                );
            }
    ...
    {
        return $this->method_is_overridden('form_settings_fields') ||
            $this->method_is_overridden('form_settings');
    }
   ...
   ?>
    ...
    {
        return $this->has_app_settings() ||
            $this->method_is_overridden('get_app_menu_items');
    }
    ...
    public function create_app_menu()
    {
        ...
        $min_cap = GFCommon::current_user_can_which(
            $this->_capabilities_app_menu
        );
        if (empty($min_cap)) {
         ...
         */
        $addon_menus = apply_filters(
            'gform_addon_app_navigation_' . $this->_slug,
            $addon_menus
        );
        ...
        $callback = isset($parent_menu['callback'])
        ...
        $menu_position = apply_filters(
            'gform_app_menu_position_' . $this->_slug,
            $menu_position
        );
        ...
        if (method_exists($this, 'load_screen_options')) {
            add_action("load-$this->app_hook_suffix", [
                $this,
                'load_screen_options',
            ]);
        }
        ...
        foreach ($menu_items as $menu_item) {
            $callback = isset($menu_item['callback'])
                ? $menu_item['callback']
                : [$this, 'app_tab_page'];
            add_submenu_page(
                $parent_menu['name'],
                $menu_item['label'],
                $menu_item['label'],
                $has_full_access || empty($menu_item['permission'])
                    ? 'gform_full_access'
        ...
        if ($this->has_app_settings()) {
            add_submenu_page(
                ...
                $has_full_access
                    ? 'gform_full_access'
                    : $this->_capabilities_app_settings,
                $this->_slug . '_settings',
                [$this, 'app_tab_page']
            );
        }
    }
    ...
    private function get_parent_menu($menu_items, $addon_menus)
    {
        ...
        if (GFCommon::current_user_can_any($this->_capabilities_app_menu)) {
            ...
            $this->has_app_settings() &&
            $this->current_user_can_any($this->_capabilities_app_settings)
        ) {
            $parent = [
                'name' => $this->_slug . '_settings',
                'callback' => [$this, 'app_settings'],
            ];
        }
    ...
    public function app_tab_page()
    {
        ...
        if (empty($current_tab)) {
            foreach ($tabs as $tab) {
                if (
                    !isset($tab['permission']) ||
                    $this->current_user_can_any($tab['permission'])
                ) {
        ...
        if (empty($current_tab)) {
            wp_die(
                esc_html__(
                    "You don't have adequate permission to view this page",
                    'gravityforms'
                )
            );
        }
    ...
    public function plugin_settings_init()
    {
        ...
        RGForms::add_settings_page([
            ...
            'handler' => [$this, 'plugin_settings_page'],
        ]);
        if (
    ...
    public function plugin_settings_page()
    {
		...
		<div class="push-alert-red" style="border-left: 1px solid #E6DB55; border-right: 1px solid #E6DB55;">
			<?php esc_html_e(
       'This add-on needs to be updated. Please contact the developer.',
       'gravityforms'
   ); ?>
		</div>
		<?php endif; ?>

		<?php if ($this->method_is_overridden('plugin_settings')) {
      ...
      $this->plugin_settings();
			...
			<div class="push-alert-gold" style="border-left: 1px solid #E6DB55; border-right: 1px solid #E6DB55;">
				<?php printf(
        esc_html__(
            '%s has been successfully uninstalled. It can be re-activated from the %splugins page%s.',
            'gravityforms'
        ),
        $this->_title,
        "<a href='plugins.php'>",
		...
		<?php } else {//saves settings page if save button was pressed
    ...
    {
        return sprintf(
            esc_html__("%s Settings", "gravityforms"),
            $this->get_short_title()
        );
    }
    ...
    {
        return $this->method_is_overridden('plugin_settings_fields') ||
            $this->method_is_overridden('plugin_settings_page') ||
            $this->method_is_overridden('plugin_settings');
    }
    ...
    public function get_plugin_setting($setting_name)
    {
        ...
        return isset($settings[$setting_name])
    ...
    public function maybe_save_plugin_settings()
    {
        if ($this->is_save_postback()) {
            ...
            if (
                ...
                GFCommon::add_error_message(
                    esc_html__(
                        "You don't have sufficient permissions to update the settings.",
                        'gravityforms'
                    )
                );
                return false;
    ...
    public function get_app_settings_tabs()
    {
        ...
        $setting_tabs = [
            [
                ...
                'callback' => [$this, 'app_settings_tab'],
            ],
        ];
        ...
        $setting_tabs = apply_filters(
            'gform_addon_app_settings_menu_' . $this->_slug,
            $setting_tabs
        );
        ...
        if ($this->current_user_can_uninstall()) {
            $setting_tabs[] = [
                'name' => 'uninstall',
                'label' => esc_html__('Uninstall', 'gravityforms'),
                'callback' => [$this, 'app_settings_uninstall_tab'],
            ];
        }
    ...
    public function app_settings_uninstall_tab()
    {
		...
		<?php } else {if ($this->current_user_can_uninstall()) { ?>
			<form action="" method="post">
				<?php wp_nonce_field('uninstall', 'gf_addon_uninstall'); ?>
				<?php  ?>
					<h3>
						<span><i class="fa fa-times"></i> <?php printf(
          esc_html__('Uninstall %s', 'gravityforms'),
          $this->get_short_title()
      ); ?></span>
					</h3>
					...
					<div class="delete-alert alert_red">
						...
						<h3>
							<i class="fa fa-exclamation-triangle gf_invalid"></i> <?php esc_html_e(
           'Warning',
           'gravityforms'
       ); ?>
						</h3>
						...
						<?php
      $uninstall_button =
          '<input type="submit" name="uninstall" value="' .
          sprintf(
              esc_attr__('Uninstall %s', 'gravityforms'),
              $this->get_short_title()
          ) .
          '" class="button" onclick="return confirm(\'' .
          esc_js($this->uninstall_confirm_message()) .
     ...
     */
    public function app_tab_page_header(
        $tabs,
        $current_tab,
        $title,
        $message = ''
        ...
        // Print admin styles
        wp_print_styles(['jquery-ui-styles', 'gform_admin']); ?>
       ...
       ?>
				<li <?php echo urlencode($current_tab) == $tab['name']
        ? "class='active'"
        : ''; ?>>
					<a href="<?php echo esc_url(
         add_query_arg(['view' => $tab['name']])
    ...
    public function app_settings_tab()
    {
		...
		<?php if ($this->method_is_overridden('app_settings')) {
      //enables plugins to override settings page by implementing a plugin_settings() function
      $this->app_settings();
  } elseif ($this->maybe_uninstall()) { ?>
			<div class="push-alert-gold" style="border-left: 1px solid #E6DB55; border-right: 1px solid #E6DB55;">
    ...
    {
        return $this->method_is_overridden('app_settings_fields') ||
            $this->method_is_overridden('app_settings');
    }
    ...
    {
        return get_option(
            'gravityformsaddon_' . $this->_slug . '_app_settings'
        );
    }
    ...
    public function settings_fields_only($settings_type = 'plugin')
    {
        ...
        if (!is_callable([$this, "{$settings_type}_settings_fields"])) {
            return $fields;
        }

        $sections = call_user_func([$this, "{$settings_type}_settings_fields"]);
    ...
    public function render_uninstall()
    {
		...
		<form action="" method="post">
			...
			<?php if ($this->current_user_can_uninstall()) { ?>
				...
				<h3><span><i class="fa fa-times"></i> <?php printf(
        esc_html__('Uninstall %s Add-On', 'gravityforms'),
        $this->get_short_title()
    ); ?></span></h3>
				<div class="delete-alert alert_red">
					<h3><i class="fa fa-exclamation-triangle gf_invalid"></i> <?php printf(
         esc_html__('Warning', 'gravityforms')
     ); ?></h3>
					<div class="gf_delete_notice">
						<?php echo $this->uninstall_warning_message(); ?>
					</div>
					<input type="submit" name="uninstall" value="<?php esc_attr_e(
         'Uninstall  Add-On',
         'gravityforms'
     ); ?>" class="button" onclick="return confirm('<?php echo esc_js(
    $this->uninstall_confirm_message()
); ?>');" onkeypress="return confirm('<?php echo esc_js(
    $this->uninstall_confirm_message()
); ?>');">
				</div>
    ...
    public function uninstall_warning_message()
    {
        return sprintf(
            esc_html__(
                '%sThis operation deletes ALL %s settings%s. If you continue, you will NOT be able to retrieve these settings.',
                'gravityforms'
            ),
            '<strong>',
    ...
    public function uninstall_confirm_message()
    {
        return sprintf(
            __(
                "Warning! ALL %s settings will be deleted. This cannot be undone. 'OK' to delete, 'Cancel' to stop",
                'gravityforms'
            ),
            __($this->get_short_title())
        );
    }
    /**
     * Not intended to be overridden or called directly by Add-Ons.
    ...
    public function uninstall_addon()
    {
        if (!$this->current_user_can_uninstall()) {
            die(
                esc_html__(
                    "You don't have adequate permission to uninstall this add-on: " .
                        $this->_title,
                    'gravityforms'
                )
            );
        ...
        foreach ($forms as $form) {
            ...
            $entry_meta = $this->get_entry_meta([], $form->id);
            if (is_array($entry_meta)) {
                foreach (array_keys($entry_meta) as $meta_key) {
                    $sql = $wpdb->prepare(
                        "DELETE from $meta_table WHERE meta_key=%s",
                        $meta_key
                    );
                    $wpdb->query($sql);
                }
        ...
        if (!empty($all_form_ids)) {
            ...
            foreach ($form_metas as $form_meta) {
                if (isset($form_meta[$this->_slug])) {
                    ...
                    GFFormDetail::save_form_info(
                        $form_meta['id'],
                        addslashes($form_json)
                    );
                }
            }
        }
    ...
    {
        if (
            false === $this->_enable_rg_autoupgrade &&
            !self::is_gravityforms_supported($this->_min_gravityforms_version)
        ) {
            $message = $this->plugin_message();
            self::display_plugin_message($message, true);
        }

        GFForms::maybe_display_update_notification(
            $plugin_name,
            $plugin_data,
            $this->get_slug(),
            $this->_version
     ...
     * @return string
     */
    public function get_mapped_field_value(
        $setting_name,
        $form,
        $entry,
        $settings = false
    ...
    public function get_field_value($form, $entry, $field_id)
    {
            ...
            default:
                ...
                if (is_object($field)) {
                    ...
                    if ($is_integer && $input_type == 'address') {
                        $field_value = $this->get_full_address(
                            $entry,
                            $field_id
                        );
                        ...
                        is_callable([$this, "get_{$input_type}_field_value"])
                    ) {
                        $field_value = call_user_func(
                            [$this, "get_{$input_type}_field_value"],
                            $entry,
                            $field_id,
                            $field
                        );
                    } else {
                        $field_value = $field->get_value_export(
                            $entry,
                            $field_id
                        );
                    }
        ...
        $field_value = gf_apply_filters(
            ['gform_addon_field_value', $form['id'], $field_id],
            $field_value,
            $form,
            $entry,
            $field_id,
        ...
        return $this->maybe_override_field_value(
            $field_value,
            $form,
            $entry,
            $field_id
     ...
     * @return string
     */
    public function maybe_override_field_value(
        $field_value,
        $form,
        $entry,
        $field_id
        ...
        return gf_apply_filters(
            ["gform_{$slug}_field_value", $form['id'], $field_id],
            $field_value,
            $form,
            $entry,
            $field_id
     ...
     * @return string
     */
    public function get_first_field_by_type(
        $field_type,
        $subfield_id = null,
        $form_id = null,
    ...
    {
        if (
            $note->user_name == $this->_short_title &&
            empty($note->user_id) &&
            $this->method_is_overridden('note_avatar', 'GFAddOn')
        ) {
            $new_avatar = $this->note_avatar();
        }

        return empty($new_avatar)
            ? $avatar
            : "<img alt='{$this->_short_title}' src='{$new_avatar}' class='avatar avatar-48' height='48' width='48' />";
    }
    ...
    final protected function method_is_overridden(
        $method_name,
        $base_class = 'GFAddOn'
    ) {
    ...
    public function get_plugin_settings_url()
    {
        return add_query_arg(
            ['page' => 'gf_settings', 'subview' => $this->_slug],
            admin_url('admin.php')
        );
    }
    ...
    {
        return rgempty('id', $_GET)
            ? false
            : GFFormsModel::get_form_meta(rgget('id'));
    }
    ...
    {
        $is_form_settings =
            rgget('page') == 'gf_edit_forms' && rgget('view') == 'settings';
    ...
    private function _tab_matches($tabs)
    {
        ...
        $current_tab = rgempty('subview', $_GET)
            ? 'settings'
            : rgget('subview');
    ...
    public function is_entry_view()
    {
        if (
            ...
            rgget('view') == 'entry' &&
            (!isset($_POST['screen_mode']) || rgpost('screen_mode') == 'view')
        ) {
            return true;
        }
    ...
    public function is_entry_list()
    {
        if (
            rgget('page') == 'gf_entries' &&
            (rgget('view') == 'entries' || rgempty('view', $_GET))
        ) {
            return true;
        }
    ...
    public static function get_all_deprecated_protected_methods(
        $add_on_class_name = ''
    ) {
        $deprecated = [];
        $deprecated = array_merge(
            $deprecated,
            self::get_deprecated_protected_methods_for_base_class(
                'GFAddOn',
                $add_on_class_name
            )
        );
    ...
    public static function get_deprecated_protected_methods_for_base_class(
        $base_class_name,
        $add_on_class_name = ''
    ) {
        $deprecated = [];
        ...
        $base_class_names = ['GFAddOn', 'GFFeedAddOn', 'GFPaymentAddOn'];

        $base_class = new ReflectionClass($base_class_name);

        $classes = empty($add_on_class_name)
            ? get_declared_classes()
            : [$add_on_class_name];

        foreach ($classes as $class) {
            if (
                !is_subclass_of($class, $base_class_name) ||
                in_array($class, $base_class_names)
            ) {
                continue;
            }

            $add_on_class = new ReflectionClass($class);
            $add_on_methods = $add_on_class->getMethods(
                ReflectionMethod::IS_PROTECTED
            );
            foreach ($add_on_methods as $method) {
                $method_name = $method->getName();
                $base_has_method = $base_class->hasMethod($method_name);
                $is_declared_by_base_class =
                    $base_has_method &&
                    $base_class
    ...
    public function update_path()
    {
        ...
        if ($path_dirname !== '.') {
            ...
            if ($path_dirname !== $full_path_dirname) {
                $this->_path =
                    trailingslashit($full_path_dirname) .
                    basename($this->_path);
            }
        }
    }
    ...
    public function get_capabilities($capability = '')
    {
        ...
        return isset($this->{'_capabilities_' . $capability})
            ? $this->{'_capabilities_' . $capability}
            : [];
    }
    ...
    public function load_text_domain()
    {
        GFCommon::load_gf_text_domain(
            $this->_slug,
            plugin_basename(dirname($this->_full_path))
        );
    }
    ...
    {
        return GFCommon::current_user_can_uninstall(
