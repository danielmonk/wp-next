...
abstract class GFFeedAddOn extends GFAddOn
{
     ...
     */
    private static $_frontend_feeds = [];
    ...
    public function init()
    {
        ...
        add_filter(
            ...
            [$this, 'maybe_process_feed'],
            10,
            2
        );
        add_action('gform_after_delete_form', [$this, 'delete_feeds']);

        // Register GFFrontendFeeds.
        if (
            $this->_supports_frontend_feeds &&
            !has_action('gform_register_init_scripts', [
                __CLASS__,
                'register_frontend_feeds_init_script',
            ])
        ) {
            // Use priority 20 so other add-ons that support frontend feeds can all load their scripts first.
            add_action(
                'gform_register_init_scripts',
                [__CLASS__, 'register_frontend_feeds_init_script'],
                20
            );
        }
    }
    ...
    public function init_ajax()
    {
        ...
        add_action("wp_ajax_gf_feed_is_active_{$this->_slug}", [
            $this,
            'ajax_toggle_is_active',
        ]);
        add_action('wp_ajax_gf_save_feed_order', [
            $this,
    ...
    public function scripts()
    {
        ...
        if ($this->_supports_frontend_feeds) {
            $scripts[] = [
                ...
                'src' =>
                    $this->get_gfaddon_base_url() .
                    "/js/gaddon_frontend{$min}.js",
                'deps' => ['jquery', 'gform_conditional_logic'],
                'version' => GFCommon::$version,
                'enqueue' => [[$this, 'has_frontend_feeds']],
            ];
        }
    ...
    public function uninstall()
    {
        global $wpdb;
        $sql = $wpdb->prepare(
            "DELETE FROM {$wpdb->prefix}gf_addon_feed WHERE addon_slug=%s",
            $this->_slug
        );
        $wpdb->query($sql);
    }
    ...
    public function maybe_process_feed($entry, $form)
    {
        if ('spam' === $entry['status']) {
            $this->log_debug(
                "GFFeedAddOn::maybe_process_feed(): Entry #{$entry['id']} is marked as spam; not processing feeds for {$this->_slug}."
            );
        ...
        $this->log_debug(
            __METHOD__ .
                "(): Checking for feeds to process for entry #{$entry['id']} for {$this->_slug}."
        );
        ...
        if (empty($feeds)) {
            $this->log_debug(
                __METHOD__ .
                    "(): No feeds to process for entry #{$entry['id']}."
            );
            return $entry;
        }
        ...
        foreach ($feeds as $feed) {
            ...
            if (!$feed['is_active']) {
                $this->log_debug(
                    "GFFeedAddOn::maybe_process_feed(): Feed is inactive, not processing feed (#{$feed['id']} - {$feed_name}) for entry #{$entry['id']}."
                );
                continue;
            }
            ...
            if (!$this->is_feed_condition_met($feed, $form, $entry)) {
                $this->log_debug(
                    "GFFeedAddOn::maybe_process_feed(): Feed condition not met, not processing feed (#{$feed['id']} - {$feed_name}) for entry #{$entry['id']}."
                );
                continue;
            }
            ...
            if (!$is_delayed) {
                ...
                if ($this->is_asynchronous($feed, $entry, $form)) {
                    ...
                    $this->log_debug(
                        "GFFeedAddOn::maybe_process_feed(): Adding feed (#{$feed['id']} - {$feed_name}) for entry #{$entry['id']} for {$this->_slug} to the processing queue."
                    );
                    ...
                    $this->log_debug(
                        "GFFeedAddOn::maybe_process_feed(): Starting to process feed (#{$feed['id']} - {$feed_name}) for entry #{$entry['id']} for {$this->_slug}"
                    );
                    ...
                    if (
                        is_array($returned_entry) &&
                        rgar($returned_entry, 'id')
                    ) {
                    ...
                    do_action(
                        "gform_{$this->_slug}_post_process_feed",
                        $feed,
                        $entry,
                        $form,
                        $this
                    ...
                    $this->log_debug(
                        'GFFeedAddOn::maybe_process_feed(): Marking entry #' .
                            $entry['id'] .
                            ' as fulfilled for ' .
                            $this->_slug
                    );
                ...
                // Log that feed processing is being delayed.
                $this->log_debug(
                    'GFFeedAddOn::maybe_process_feed(): Feed processing is delayed, not processing feed for entry #' .
                        $entry['id'] .
                        ' for ' .
                        $this->_slug
                );
    ...
    public function maybe_delay_feed($entry, $form)
    {
        ...
        $is_delayed = apply_filters(
            'gform_is_delayed_pre_process_feed_' . $form['id'],
            $is_delayed,
            $form,
            $entry,
            $slug
    ...
    public function is_asynchronous($feed, $entry, $form)
    {
        ...
        $is_asynchronous = gf_apply_filters(
            ['gform_is_feed_asynchronous', $form['id'], $feed['id']],
            $this->_async_feed_processing,
            $feed,
            $entry,
            $form
        );
    ...
    public function is_feed_condition_met($feed, $form, $entry)
    {
        ...
        $is_condition_enabled =
            rgar($feed_meta, 'feed_condition_conditional_logic') == true;
        $logic = rgars(
            $feed_meta,
    ...
    public function get_active_feeds($form_id = null)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}gf_addon_feed
                               WHERE addon_slug=%s AND is_active=1 {$form_filter} ORDER BY `feed_order`, `id` ASC",
            $this->_slug
        );
    ...
    public function get_single_submission_feed($entry = false, $form = false)
    {
        ...
        if (
            !empty($this->_single_submission_feed) &&
            (!$form || $this->_single_submission_feed['form_id'] == $form['id'])
        ) {
            ...
            if (empty($feeds)) {
                $feed = $this->get_single_submission_feed_by_form(
                    $form,
                    $entry
                );
            } else {
    ...
    public function get_single_submission_feed_by_form($form, $entry)
    {
        if ($form) {
            ...
            foreach ($feeds as $_feed) {
                if (
                    $_feed['is_active'] &&
                    $this->is_feed_condition_met($_feed, $form, $entry)
                ) {
                    return $_feed;
                }
            }
    ...
    public function pre_process_feeds($feeds, $entry, $form)
    {
        ...
        $feeds = apply_filters(
            "gform_addon_pre_process_feeds_{$form['id']}",
            $feeds,
            $entry,
            $form
        );
        $feeds = apply_filters(
            "gform_{$this->_slug}_pre_process_feeds",
            $feeds,
            $entry,
            $form
        );
        $feeds = apply_filters(
            "gform_{$this->_slug}_pre_process_feeds_{$form['id']}",
            $feeds,
            $entry,
            $form
        );
    ...
    public function get_default_feed_name()
    {
        ...
        if ($feeds_to_filter) {
            ...
            foreach ($feeds_to_filter as $check) {
                ...
                $name = rgars($check, 'meta/feed_name')
                    ? rgars($check, 'meta/feed_name')
                    : rgars($check, 'meta/feedName');
                $name = trim($name);
    ...
    public function delete_feed($id)
    {
        ...
        do_action(
            "gform_{$this->get_short_slug()}_pre_delete_feed",
            $id,
            $this
        );

        $wpdb->delete("{$wpdb->prefix}gf_addon_feed", ['id' => $id], ['%d']);
    }
    ...
    public function delete_feeds($form_id = null)
    {
        ...
        if (!empty($feed_ids)) {
            array_map([$this, 'delete_feed'], $feed_ids);
        }
    }
    ...
    public function duplicate_feed($id, $new_form_id = false)
    {
        ...
        // Get feed name key.
        $feed_name_key = rgars($original_feed, 'meta/feed_name')
            ? 'feed_name'
            : 'feedName';
        ...
        $count = 2;
        $feed_name =
            rgars($original_feed, 'meta/' . $feed_name_key) .
            ' - ' .
            esc_html__('Copy 1', 'gravityforms');
        while (
            !$this->is_unique_feed_name($feed_name, $original_feed['form_id'])
        ) {
            $feed_name =
                rgars($original_feed, 'meta/' . $feed_name_key) .
                ' - ' .
                sprintf(esc_html__('Copy %d', 'gravityforms'), $count);
            $count++;
        }
        ...
        // Create the new feed.
        return $this->insert_feed(
            $new_form_id,
            $original_feed['is_active'],
            $meta
        );
    }
    ...
    public function save_feed_order($feed_order)
    {
        ...
        $feed_order = array_combine(
            range(1, count($feed_order)),
            array_values($feed_order)
        );
    ...
    public function ajax_save_feed_order()
    {
        ...
        $feed_order = rgpost('feed_order') ? rgpost('feed_order') : [];
    ...
    public function form_settings($form)
    {
        if (!$this->_multiple_feeds || $this->is_detail_page()) {
            // feed edit page
            $feed_id = $this->_multiple_feeds
                ? $this->get_current_feed_id()
                : $this->get_default_feed_id($form['id']);
    ...
    {
        return sprintf(
            esc_html__('%s Feeds', 'gravityforms'),
  ...
  $this->render_settings($this->get_feed_settings_fields($form));
    ...
    public function settings($sections)
    {
		...
		<input type="hidden" name="gf_feed_id" value="<?php echo esc_attr(
      $this->get_current_feed_id()
  ); ?>" />
		<?php
    }
   ...
   if ($this->_supports_feed_ordering) {
       ...
       echo 'jQuery( document ).ready( function() {
						window.GFFeedOrderObj = new GFFeedOrder( ' .
           json_encode($feed_order_options) .
           ');
					} );';
   }?>
		</script>
    ...
    public function get_feed_table($form)
    {
        ...
        return new GFAddOnFeedsTable(
            $feeds,
            $this->_slug,
            $columns,
            $bulk_actions,
            $action_links,
            $column_value_callback,
            $no_item_callback,
            $message_callback,
            $this
        );
    ...
    public function trim_conditional_logic_vales($settings, $form_id)
    {
        ...
        if (
            isset($settings['feed_condition_conditional_logic_object']) &&
    ...
    public function get_save_success_message($sections)
    {
        ...
        return isset($save_button['messages']['success'])
            ...
            : esc_html__('Feed updated successfully.', 'gravityforms');
    }
    ...
    public function get_feed_settings_fields()
    {
         ...
         * @return array
         */
        $feed_settings_fields = apply_filters(
            'gform_addon_feed_settings_fields',
            $this->feed_settings_fields(),
            $this
        );
        $feed_settings_fields = apply_filters(
            "gform_{$this->_slug}_feed_settings_fields",
            $feed_settings_fields,
            $this
        );

        $this->_feed_settings_fields = $this->add_default_feed_settings_fields_props(
            $feed_settings_fields
        );
    ...
    {
        $feed_id = '_id_';
        $edit_url = add_query_arg(['fid' => $feed_id]);
        $links = [
            'edit' =>
                '<a href="' .
                esc_url($edit_url) .
                ...
                '</a>',
            'duplicate' =>
                '<a href="#" onclick="gaddon.duplicateFeed(\'' .
                esc_js($feed_id) .
                '\');" onkeypress="gaddon.duplicateFeed(\'' .
                esc_js($feed_id) .
                '\');">' .
    ...
    public function feed_list_no_item_message()
    {
        ...
        return sprintf(
            esc_html__(
                "You don't have any feeds configured. Let's go %screate one%s!",
                'gravityforms'
            ),
            "<a href='" . esc_url($url) . "'>",
            '</a>'
    ...
    public function configure_addon_message()
    {
        ...
        return sprintf(
            __('To get started, please configure your %s.', 'gravityforms'),
            $settings_link
        );
    }
    ...
    {
        if (is_callable([$this, "get_column_value_{$column}"])) {
            return call_user_func([$this, "get_column_value_{$column}"], $item);
    ...
    public function get_default_feed_id($form_id)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}gf_addon_feed WHERE addon_slug=%s AND form_id = %d LIMIT 0,1",
            $this->_slug,
            $form_id
        );
    ...
    public function settings_feed_condition($field, $echo = true)
    {
        ...
        $instructions = isset($field['instructions'])
            ? $field['instructions']
            : esc_html__('Process this feed if', 'gravityforms');
        $html = $this->settings_checkbox($checkbox_field, false);
        $html .= $this->settings_hidden($hidden_field, false);
        $html .=
            '<div id="feed_condition_conditional_logic_container"><!-- dynamically populated --></div>';
        $html .=
            '<script type="text/javascript"> var feedCondition = new FeedConditionObj({' .
            'strings: { objectDescription: "' .
    ...
    {
        $checkbox_label = isset($field['checkbox_label'])
            ? $field['checkbox_label']
            : esc_html__('Enable Condition', 'gravityforms');
    ...
    {
        $conditional_logic_object = $this->get_setting(
            'feed_condition_conditional_logic_object'
        );
    ...
    public function validate_feed_condition_settings($field, $settings)
    {
        ...
        $conditional_logic_safe = GFFormsModel::sanitize_conditional_logic(
            $conditional_logic
        );
        if (
            serialize($conditional_logic) != serialize($conditional_logic_safe)
        ) {
    ...
    public function add_post_payment_actions($feed_settings_fields, $addon)
    {
        ...
        if ($this->has_feed($form_id)) {
            $addon_label = rgar(
                $this->delayed_payment_integration,
                'option_label'
            );
            $choice = [
                'label' => $addon_label
            ...
            if (!$field) {
                ...
                if (rgar($config, 'position') === 'before') {
                    $feed_settings_fields = $this->add_field_before(
                        $setting,
                        $fields,
                        $feed_settings_fields
                    );
                ...
                $feed_settings_fields = $this->replace_field(
                    $field_name,
                    $field,
                    $feed_settings_fields
                );
     ...
     */
    public function paypal_fulfillment(
        $entry,
        $paypal_config,
        $transaction_id,
        $amount
     ...
     */
    public function action_trigger_payment_delayed_feeds(
        $transaction_id,
        $payment_feed,
        $entry,
        $form = null
        ...
        $this->log_debug(
            __METHOD__ .
                '(): Checking fulfillment for transaction ' .
                $transaction_id .
                ' for ' .
                $payment_feed['addon_slug']
        );

        $is_fulfilled = gform_get_meta(
            $entry['id'],
            "{$this->_slug}_is_fulfilled"
        );
        if ($is_fulfilled || !$this->is_delayed($payment_feed)) {
            $this->log_debug(
                __METHOD__ .
                    '(): Entry ' .
                    $entry['id'] .
                    ' is already fulfilled or feeds are not delayed. No action necessary.'
            );
    ...
    public function has_paypal_payment($feed, $form, $entry)
    {
        ...
        $payment_field =
            $feed['meta']['transactionType'] === 'product'
                ? $feed['meta']['paymentAmount']
                : $feed['meta']['recurringAmount'];
    ...
    public function is_delayed_payment($entry, $form, $is_delayed)
    {
        ...
        $has_payment =
            self::get_paypal_payment_amount($form, $entry, $paypal_feed) > 0;

        return rgar($paypal_feed['meta'], "delay_{$this->_slug}") &&
            $has_payment &&
            !$is_delayed;
    }

    public static function get_paypal_payment_amount(
        $form,
        $entry,
        $paypal_config
    ) {
        ...
        foreach ($products['products'] as $id => $product) {
            if (
                $paypal_config['meta']['type'] != 'subscription' ||
                $recurring_field == $id ||
                $recurring_field == 'all'
            ) {
    ...
    public function get_frontend_feeds($form)
    {
        ...
        foreach ($feeds as $feed) {
            $_feed = [
                ...
                'conditionalLogic' =>
                    rgars($feed, 'meta/feed_condition_conditional_logic') ===
                    '0'
                        ? false
                        : rgars(
                            $feed,
            ...
            $_feed = apply_filters(
                'gform_addon_frontend_feed',
                $_feed,
                $form,
                $feed
            );
            $_feed = apply_filters(
                "gform_addon_frontend_feed_{$form['id']}",
                $_feed,
                $form,
                $feed
            );
            $_feed = apply_filters(
                "gform_{$this->_slug}_frontend_feed",
                $_feed,
                $form,
                $feed
            );
            $_feed = apply_filters(
                "gform_{$this->_slug}_frontend_feed_{$form['id']}",
                $_feed,
                $form,
                $feed
            );
...
class GFAddOnFeedsTable extends WP_List_Table
{
    ...
    private $_no_items_callback = [];
    private $_message_callback = [];
    ...
    {
        $this->items = isset($this->_feeds) ? $this->_feeds : [];
    }
    ...
    function display_rows_or_placeholder()
    {
		...
		<?php } else {parent::display_rows_or_placeholder();}
    }
    ...
    function column_cb($item)
    {
        ...
        return sprintf(
            '<input type="checkbox" name="feed_ids[]" value="%s" />',
            esc_attr($feed_id)
        );
    }
    ...
    function _column_is_active($item, $classes, $data, $primary)
    {
        ...
        // Get image title tag.
        $title = $is_active
            ? esc_attr__('Active', 'gravityforms')
            : esc_attr__('Inactive', 'gravityforms');

        // Display feed active button.
        echo sprintf(
            '<img src="%s" title="%s" onclick="gaddon.toggleFeedActive(this, \'%s\', \'%s\');" onkeypress="gaddon.toggleFeedActive(this, \'%s\', \'%s\');" style="cursor:pointer";/>',
            $src,
            $title,
            esc_js($this->_slug),
            esc_js($item['id']),
