...
class GFAutoUpgrade
{
    ...
    public function init()
    {
        if (is_admin()) {
            ...
            add_action('gform_after_check_update', [
                $this,
                'flush_version_info',
            ]);
            add_action('gform_updates', [$this, 'display_updates']);

            add_filter('gform_updates_list', [$this, 'get_update_info']);

            if (RG_CURRENT_PAGE == 'plugins.php') {
                add_action('after_plugin_row_' . $this->_path, [
                    $this,
                    'rg_plugin_row',
                ]);
            } elseif (in_array(RG_CURRENT_PAGE, ['admin-ajax.php'])) {
        ...
        add_filter('mwp_premium_perform_update', [$this, 'premium_update']);
    }
    ...
    public function premium_update_push($premium_update)
    {
        ...
        if (
            ...
            version_compare($this->_version, $update['version'], '<')
        ) {
            $plugin_data = get_plugin_data($this->_full_path);
            $plugin_data['type'] = 'plugin';
            $plugin_data['slug'] = $this->_path;
            $plugin_data['new_version'] = isset($update['version'])
                ? $update['version']
                : false;
    ...
    public function premium_update($premium_update)
    {
        ...
        if (
            ...
            $plugin_data['slug'] = $this->_path;
            $plugin_data['type'] = 'plugin';
            $plugin_data['url'] = isset($update['url'])
                ? $update['url']
                : false; // OR provide your own callback function for managing the update
    ...
    public function check_update($option)
    {
        ...
        if (
            ...
            version_compare($this->_version, $version_info['version'], '>=')
        ) {
    ...
    private function get_changelog()
    {
        ...
        $raw_response = GFCommon::post_to_manager(
            'changelog.php',
            $this->get_remote_request_params(
                $this->_slug,
                $key,
                $this->_version
            ),
    ...
    private function get_version_info($offering, $use_cache = true)
    {
        ...
        $is_valid_key =
            rgar($version_info, 'is_valid_key') &&
            rgars($version_info, "offerings/{$offering}/is_available");
    ...
    private function get_remote_request_params($offering, $key, $version)
    {
        ...
        return sprintf(
            'of=%s&key=%s&v=%s&wp=%s&php=%s&mysql=%s',
            urlencode($offering),
            urlencode($key),
            urlencode($version),
    ...
    public function get_update_info($updates)
    {
        ...
        if (!rgar($version_info, 'is_valid_key')) {
            ...
            $version_message = sprintf(
                ...
                sprintf(
                    esc_html(
                        '%sRegister%s your copy of Gravity Forms to receive access to automatic updates and support. Need a license key? %sPurchase one now%s.',
                        'gravityforms'
                    ),
            ...
            $details_url = self_admin_url(
                'plugin-install.php?tab=plugin-information&plugin=' .
                    urlencode($this->_slug) .
                    '&section=changelog&TB_iframe=true&width=600&height=800'
            );
            $message_link_text = sprintf(
                esc_html__('View version %s details', 'gravityforms'),
                $version_info['version']
            );
            $message_link = sprintf(
                '<a href="%s" class="thickbox" title="%s">%s</a>',
                esc_url($details_url),
                esc_attr($this->_title),
                $message_link_text
            );
            $message = sprintf(
                esc_html__(
                    'There is a new version of %1$s available. %s.',
                    'gravityforms'
                ),
                $this->_title,
                $message_link
            ...
            $version_message = sprintf(
                esc_html__('Your version of %s is up to date.', 'gravityforms'),
                $this->_title
            );
        }
   ...
   if (!rgar($version_info, 'is_valid_key')) { ?>
       ...
       if (rgar($version_info, 'is_valid_key')) {
          ...
          sprintf(
              esc_html__(
                  'You can update to the latest version automatically or download the update and install it manually. %sUpdate Automatically%s %sDownload Update%s',
                  'gravityforms'
              ),
              "</p><a class='button-primary' href='{$upgrade_url}'>",
              '</a>',
              "&nbsp;<a class='button' href='{$version_info['url']}'>",
              '</a>'
          ); ?>
					</div>
				<?php
