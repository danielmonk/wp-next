...
class GFAddonLocking extends GFLocking
{
    ...
    {
        $this->_addon = $addon;
        $capabilities = isset($config['capabilities'])
            ? $config['capabilities']
            : [];
        $redirect_url = isset($config['redirect_url'])
            ? $config['redirect_url']
            : '';
        $edit_url = isset($config['edit_url']) ? $config['edit_url'] : '';
        $object_type = isset($config['object_type'])
            ? $config['object_type']
            : '';
        $this->_strings = isset($config['strings']) ? $config['strings'] : [];
        parent::__construct(
            $object_type,
            $redirect_url,
            $edit_url,
