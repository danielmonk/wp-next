...
function cancel_subscription(entryId) {
  if (!confirm(gaddon_payment_strings.subscriptionCancelWarning)) return;
  ...
  jQuery.post(
    ajaxurl,
    {
      ...
      gaddon_cancel_subscription:
        gaddon_payment_strings.subscriptionCancelNonce,
    },
    function (response) {
      jQuery("#subscription_cancel_spinner").hide();
      if (response.success === true) {
        jQuery("#gform_payment_status").html(
          gaddon_payment_strings.subscriptionCanceled
        );
