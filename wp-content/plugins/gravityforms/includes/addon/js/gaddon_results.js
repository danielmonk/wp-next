...
var gresults = {
    ...
    jQuery.ajax({
      ...
      success: function (response) {
        if (response === -1) {
          ...
          if (response.html) jQuery(container).append(response.html);
  ...
  clearFilterForm: function () {
    jQuery("#gresults-results-field-filters-container")
      .off("click", ".gform-add")
      .gfFilterUI(gresultsFilterSettings, [], true);
    jQuery("#gresults-results-filter-form")
      .find("input, select")
      .each(function () {
        switch (this.type) {
          case "text":
  ...
  recordFormState: function () {
    jQuery("#gresults-results-filter-form input[type='radio']").each(
      function () {
        if (this.checked) {
    ...
    jQuery("#gresults-results-filter-form input[type='checkbox']").each(
      function () {
        if (this.checked) {
          jQuery(this).prop("defaultChecked", true);
        } else {
    ...
    jQuery("#gresults-results-filter-form input[type='text']").each(
      function () {
  ...
  setCustomFilter: function (key, value) {
    ...
    if (jQuery("#" + elementId).length == 0)
      jQuery("#gresults-results-filter-form").append(
    ...
    else jQuery("#" + elementId).val(value);
  },
};
...
jQuery(window).on("load", function () {
  if (jQuery("#gresults-results").length > 0) {
    jQuery("#gresults-results-field-filters-container").gfFilterUI(
      gresultsFilterSettings,
      gresultsInitVars,
      true
    );
    ...
    window.onpopstate = function (e) {
      if (e.state) gresults.renderStateData(e.state);
    };

    jQuery(
      "#gresults-results-filter-date-start, #gresults-results-filter-date-end"
    ).datepicker({
      dateFormat: "yy-mm-dd",
      changeMonth: true,
      changeYear: true,
    ...
    if (window["gform_initialize_tooltips"]) gform_initialize_tooltips();
  }
});
