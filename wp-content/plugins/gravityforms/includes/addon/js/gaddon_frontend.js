var GFFrontendFeeds = function (args) {
  ...
  self.bindEvents = function () {
    gform.addAction("gform_input_change", function (elem, formId, inputId) {
      ...
      var isTriggeredInput =
        $.inArray(inputId, self.triggerInputIds) !== -1 ||
        $.inArray(fieldId, self.triggerInputIds) !== -1;
  ...
  self.evaluateFeeds = function () {
    ...
    for (i = 0; i < self.options.feeds.length; i++) {
      ...
      if (!isMatch && isActivated !== null) {
        ...
        !isActivated &&
        (!feed.isSingleFeed ||
          (feed.isSingleFeed && self.hasPriority(feed.feedId, feed.addonSlug)))
      ) {
    ...
    gform.doAction(
      "gform_frontend_feeds_evaluated_{0}".format(self.options.formId),
      self.options.feeds,
      self.options.formId,
      self
    );
    gform.doAction(
      "gform_{0}_frontend_feeds_evaluated".format(feed.addonSlug),
      self.options.feeds,
      self.options.formId,
      self
    );
  ...
  self.activateFeed = function (feeds) {
    ...
    for (var i = 0; i < feeds.length; i++) {
      ...
      gform.doAction(
        "gform_frontend_feed_activated_{0}".format(self.options.formId),
        feed,
        self.options.formId
      );
      gform.doAction(
        "gform_{0}_frontend_feed_activated".format(feed.addonSlug),
        feed,
        self.options.formId
      );
      gform.doAction(
  ...
  self.deactivateFeed = function (feeds) {
    ...
    for (var i = 0; i < feeds.length; i++) {
      ...
      gform.doAction(
        "gform_frontend_feed_deactivated_{0}".format(self.options.formId),
        feed,
        self.options.formId
      );
      gform.doAction(
        "gform_{0}_frontend_feed_deactivated".format(feed.addonSlug),
        feed,
        self.options.formId
      );
      gform.doAction(
