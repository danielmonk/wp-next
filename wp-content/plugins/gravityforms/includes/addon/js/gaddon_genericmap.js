var GFGenericMap = function (options) {
  ...
  self.bindEvents = function () {
    self.UI.on(
      "change",
      'select[name="_gaddon_setting_' + self.options.keyFieldName + '"]',
      function () {
        var $select = jQuery(this),
          $selectElm = $select.data("chosen")
            ? $select.siblings(".chosen-container")
            : $select.data("select2")
    ...
    self.UI.on(
      "change",
      'select[name="_gaddon_setting_' + self.options.valueFieldName + '"]',
      function () {
        var $select = jQuery(this),
          $selectElm = $select.data("chosen")
            ? $select.siblings(".chosen-container")
    ...
    self.UI.closest("form").on("submit", function (event) {
      jQuery('[name^="_gaddon_setting_' + self.options.fieldName + '_"]').each(
        function (i) {
      ...
      callbacks: {
        add: function (obj, $elem, item) {
          ...
          if (self.options.mergeTags) {
            new gfMergeTagsObj(
              form,
              $elem.find(".custom-value-container input")
            );
            $elem
              .find(".custom-value-container")
              .addClass("supports-merge-tags");
          }
