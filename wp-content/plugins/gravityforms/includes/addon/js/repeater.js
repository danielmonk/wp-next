...
jQuery.fn.repeater = function (options) {
  ...
  self.bindEvents = function () {
    ...
    self.elem.on(
      self.options.saveEvents,
      self.options.saveElements,
      function () {
        self.save();
      }
    );
  ...
  self.stashTemplate = function () {
    ...
    if (!self._template) self._template = self.elem.html();
  ...
  self.getItemMarkup = function (item, index) {
    ...
    for (var property in item) {
      ...
      itemMarkup = itemMarkup.replace(
        "{buttons}",
        self.getRepeaterButtonsMarkup(index)
      );
      itemMarkup = itemMarkup.replace(
        new RegExp("{" + property + "}", "g"),
        escapeAttr(item[property])
      );
    }
  ...
  self.getRepeaterButtonsMarkup = function (index) {
    ...
    if (!buttonsMarkup) buttonsMarkup = self.getDefaultButtonsMarkup(index);
  ...
  self.getDefaultButtonsMarkup = function (index) {
    var cssClass =
        self.items.length >= self.options.limit && self.options.limit !== 0
          ? "inactive"
          : "",
      buttons =
        '<a class="add-item ' +
        ...
        index +
        '">' +
        self.options.addButtonMarkup +
        "</a>";

    if (self.items.length > self.options.minItemCount)
      buttons +=
        '<a class="remove-item" data-index="' +
        index +
        '">' +
        self.options.removeButtonMarkup +
        "</a>";
  ...
  self.populateSelects = function (item, index) {
    ...
    for (var property in item) {
      ...
      if (jQuery.isArray(item[property])) {
        ...
        input
          .find('option[value="' + item[property] + '"]')
          .prop("selected", true);
      }
    }
  };
    ...
    var isElem = self.isElement(elemOrItem),
      index = parseInt(
        typeof index !== "undefined"
          ? index
          : isElem
          ? parseInt(jQuery(elemOrItem).attr("data-index"), 10) + 1
          : self.items.length,
        10
      ),
  ...
  self.removeItem = function (elemOrIndex) {
    var index = self.isElement(elemOrIndex)
      ? jQuery(elemOrIndex).attr("data-index")
      : elemOrIndex;
  ...
  self.save = function () {
    ...
    for (var i = 0; i < self.items.length; i++) {
      if (typeof self.items[i] == "undefined") continue;
