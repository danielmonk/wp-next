...
class GF_Feed_Processor extends GF_Background_Process
{
    ...
    protected function task($item)
    {
        ...
        if (!$addon instanceof GFFeedAddOn) {
            GFCommon::log_error(
                __METHOD__ .
                    "(): attempted feed (#{$feed['id']} - {$feed_name}) for entry #{$item['entry_id']} for {$feed['addon_slug']} but add-on could not be found. Bailing."
            );
        ...
        if (is_wp_error($entry)) {
            call_user_func(
                [$addon, 'log_debug'],
                __METHOD__ .
                    "(): attempted feed (#{$feed['id']} - {$feed_name}) for entry #{$item['entry_id']} for {$addon_slug} but entry could not be found. Bailing."
            );
        ...
        if (
            is_array($processed_feeds) &&
            in_array($feed['id'], $processed_feeds)
        ) {
            call_user_func(
                [$addon, 'log_debug'],
                __METHOD__ .
                    "(): already processed feed (#{$feed['id']} - {$feed_name}) for entry #{$entry['id']} for {$addon_slug}. Bailing."
            );
        ...
        if ($item['attempts'] > $max_attempts) {
            call_user_func(
                [$addon, 'log_debug'],
                __METHOD__ .
                    "(): attempted feed (#{$feed['id']} - {$feed_name}) for entry #{$entry['id']} for {$addon->get_slug()} too many times. Bailing."
            );
        ...
        call_user_func(
            [$addon, 'log_debug'],
            __METHOD__ .
                "(): Starting to process feed (#{$feed['id']} - {$feed_name}) for entry #{$entry['id']} for {$addon->get_slug()}. Attempt number: " .
                $item['attempts']
        );
        ...
        try {
            ...
            call_user_func(
                [$addon, 'log_error'],
                __METHOD__ .
                    "(): Unable to process feed due to error: {$e->getMessage()}"
            );
        ...
        if (is_wp_error($returned_entry)) {
            ...
            call_user_func(
                [$addon, 'log_error'],
                __METHOD__ .
                    "(): Unable to process feed due to error: {$returned_entry->get_error_message()}"
            );
        ...
        do_action(
            "gform_{$feed['addon_slug']}_post_process_feed",
            $feed,
            $entry,
            $form,
            $addon
     ...
     * @return false
     */
    public function custom_error_handler(
        $number,
        $string,
        $file,
        $line,
        ...
        if (
            in_array($number, [E_USER_ERROR, E_RECOVERABLE_ERROR]) &&
            $error_is_enabled
        ) {
    ...
    protected function increment_attempts($item)
    {
        ...
        foreach ($batch->data as $key => $task) {
            ...
            if (
                $item_feed['id'] === $task_feed['id'] &&
                $item_entry_id === $task_entry_id
            ) {
                $batch->data[$key]['attempts'] = isset(
                    $batch->data[$key]['attempts']
                )
                    ? $batch->data[$key]['attempts'] + 1
                    : 1;
