...
abstract class GFPaymentAddOn extends GFFeedAddOn
{
    ...
    public function pre_init()
    {
        ...
        add_action('parse_request', [$this, 'maybe_process_callback']);
    ...
    public function init()
    {
        ...
        add_filter('gform_confirmation', [$this, 'confirmation'], 20, 4);

        add_filter('gform_validation', [$this, 'maybe_validate'], 20);
        add_filter('gform_entry_post_save', [$this, 'entry_post_save'], 10, 2);

        if ($this->_requires_credit_card) {
            add_filter(
                'gform_register_init_scripts',
                [$this, 'register_creditcard_token_script'],
                10,
                3
            );
            add_filter(
                'gform_field_content',
                [$this, 'add_creditcard_token_input'],
                10,
                5
            );
            add_filter(
                'gform_form_args',
                [$this, 'force_ajax_for_creditcard_tokens'],
                10,
                1
            );
        }
        ...
        add_filter(
            ...
            [$this, 'maybe_delay_feed_processing'],
            20,
            4
        );
    }
    ...
    public function init_admin()
    {
        ...
        add_filter('gform_delete_lead', [$this, 'entry_deleted']);
        add_action(
            'gform_before_delete_field',
            [$this, 'before_delete_field'],
            10,
            2
        );

        if (rgget('page') == 'gf_entries') {
            add_action('gform_payment_details', [$this, 'entry_info'], 10, 2);
        }
    }
    ...
    public function init_ajax()
    {
        ...
        add_action('wp_ajax_gaddon_cancel_subscription', [
            $this,
    ...
    public function post_gravityforms_upgrade(
        ...
        parent::post_gravityforms_upgrade(
            $db_version,
            $previous_db_version,
            $force_upgrade
        );
     ...
     * @return bool
     */
    public function maybe_delay_feed_processing(
        $is_delayed,
        $form,
        $entry,
        $slug
     ...
     * @param array  $form           The form currently being processed.
     */
    public function trigger_payment_delayed_feeds(
        $transaction_id,
        $payment_feed,
        $entry,
        $form
        ...
        if (has_filter('gform_trigger_payment_delayed_feeds')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_trigger_payment_delayed_feeds.'
            );
    ...
    public function confirmation($confirmation, $form, $entry, $ajax)
    {
        ...
        $confirmation = ['redirect' => $this->redirect_url];
    ...
    public function validation($validation_result)
    {
        ...
        if ($gf_payment_gateway && $gf_payment_gateway !== $this->get_slug()) {
            $this->log_debug(
                __METHOD__ .
                    '() Aborting. Submission already processed by ' .
                    $gf_payment_gateway
            );
        ...
        if (
            ...
            $this->log_debug(
                __METHOD__ .
                    '(): Aborting. Payment amount not valid for processing.'
            );
        ...
        if (GFCommon::is_spam_entry($entry, $form)) {
            $this->log_debug(
                __METHOD__ . '() Aborting. Submission flagged as spam.'
            );
        ...
        if (
            $this->payment_method_is_overridden('authorize') &&
            !$is_subscription
        ) {
            ...
            $this->payment_method_is_overridden('subscribe') &&
            $is_subscription
        ) {
            ...
            $this->authorization['is_authorized'] = rgar(
                $subscription,
                'is_success'
            );
            $this->authorization['error_message'] = rgar(
                $subscription,
                'error_message'
            );
        ...
        if ($performed_authorization) {
            $this->log_debug(
                __METHOD__ .
                    "(): Authorization result for form #{$form['id']} submission => " .
                    print_r($this->authorization, 1)
            );
        }

        if (
            $performed_authorization &&
            !rgar($this->authorization, 'is_authorized')
        ) {
            $validation_result = $this->get_validation_result(
                $validation_result,
                $this->authorization
            );
            ...
            $current_page = intval(
                rgar($validation_result, 'credit_card_page')
            );
     ...
     * @return bool
     */
    public function is_valid_payment_amount(
        $submission_data,
        $feed,
        $form,
        $entry
        ...
        $tag = sprintf(
            'gform_%s_is_valid_payment_amount',
            $this->get_short_slug()
        );
        $form_id = absint($form['id']);
        $tag_args = [$tag, $form_id];

        if (gf_has_filters($tag_args)) {
            $this->log_debug(
                sprintf(
                    '%s(): Executing functions hooked to %s.',
                    __METHOD__,
                    $tag
                )
            );
             ...
             */
            $is_valid = (bool) gf_apply_filters(
                $tag_args,
                $is_valid,
                $submission_data,
                $feed,
        ...
        foreach ($validation_result['form']['fields'] as &$field) {
            if ($field->type == 'creditcard') {
                ...
                $field->validation_message =
                    $authorization_result['error_message'];
    ...
    public function entry_post_save($entry, $form)
    {
        ...
        if (!empty($this->authorization)) {
            ...
            if ($feed['meta']['transactionType'] == 'subscription') {
                $entry = $this->process_subscription(
                    $this->authorization,
                    $feed,
                    $this->current_submission_data,
                    $form,
                    $entry
                );
            } else {
                if (
                    $this->payment_method_is_overridden('capture') &&
                    rgempty('captured_payment', $this->authorization)
                ) {
                    $this->authorization['captured_payment'] = $this->capture(
                        $this->authorization,
                        $feed,
                        $this->current_submission_data,
                        $form,
            ...
            $entry['transaction_type'] =
                rgars($feed, 'meta/transactionType') == 'subscription' ? 2 : 1;
    ...
    public function process_capture(
        ...
        $this->log_debug(
            __METHOD__ .
                "(): Updating entry #{$entry['id']} with result => " .
                print_r($payment, 1)
        );
        ...
        if ($payment['is_success']) {
            ...
            $payment['note'] = sprintf(
                esc_html__(
                    'Payment failed to be captured. Reason: %s',
                    'gravityforms'
                ),
    ...
    public function process_subscription(
        ...
        if ($payment && $payment['is_success']) {
            ...
            $amount_formatted = GFCommon::to_money(
                $payment['amount'],
                $entry['currency']
            );
            $note = sprintf(
                esc_html__(
                    '%s has been captured successfully. Amount: %s. Transaction Id: %s',
                    'gravityforms'
                ),
                $payment_name,
                $amount_formatted,
        ...
        if ($subscription['is_success']) {
            ...
            $this->add_note(
                ...
                sprintf(
                    esc_html__(
                        'Subscription failed to be created. Reason: %s',
                        'gravityforms'
                    ),
    ...
    public function insert_transaction(
        ...
        $payment_count = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT count(id) FROM {$wpdb->prefix}gf_addon_payment_transaction WHERE lead_id=%d",
                $entry_id
            )
        );
        $is_recurring =
            $payment_count > 0 && $transaction_type == 'payment' ? 1 : 0;
        ...
        $sql = $wpdb->prepare(
            " INSERT INTO {$wpdb->prefix}gf_addon_payment_transaction (lead_id, transaction_type, transaction_id, amount, is_recurring, date_created, subscription_id)
                                values(%d, %s, %s, %f, %d, utc_timestamp(), %s)",
            $entry_id,
            $transaction_type,
            $transaction_id,
            $amount,
        ...
        if (has_filter('gform_post_payment_transaction')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_post_payment_transaction.'
            );
        }
    ...
    public function get_payment_feed($entry, $form = false)
    {
        ...
        if ($entry['id']) {
            ...
            $submission_feed = empty($feeds)
                ? false
                : $this->get_feed($feeds[0]);
        } elseif ($form) {
            // Getting all feeds.
            ...
            foreach ($feeds as $feed) {
                if (
                    $feed['is_active'] &&
                    $this->is_feed_condition_met($feed, $form, $entry)
                ) {
    ...
    public function get_submission_data($feed, $form, $entry)
    {
        ...
        foreach ($billing_fields as $billing_field) {
            ...
            $submission_data[$field_name] = $this->get_field_value(
                $form,
                $entry,
                $input_id
            );
        ...
        if ($card_field) {
            $submission_data[
                'card_number'
            ] = $this->remove_spaces_from_card_number(
                rgpost("input_{$card_field->id}_1")
            );
            $submission_data['card_expiration_date'] = rgpost(
                "input_{$card_field->id}_2"
            );
            $submission_data['card_security_code'] = rgpost(
                "input_{$card_field->id}_3"
            );
        ...
        return gf_apply_filters(
            ['gform_submission_data_pre_process_payment', $form['id']],
            $submission_data,
            $feed,
            $form,
            $entry
    ...
    {
        $fields = GFAPI::get_fields_by_type($form, ['creditcard']);
    ...
    public function get_order_data($feed, $form, $entry)
    {
        ...
        $setup_fee_field = rgar($feed['meta'], 'setupFee_enabled')
            ? $feed['meta']['setupFee_product']
            : false;
        $trial_field = rgar($feed['meta'], 'trial_enabled')
        ...
        foreach ($products['products'] as $field_id => $product) {
            ...
            $product_price = GFCommon::to_number(
                $product['price'],
                $entry['currency']
            );
            ...
            if (!empty($trial_field) && $trial_field == $field_id) {
                ...
                !empty($setup_fee_field) &&
                $setup_fee_field == $field_id
            ) {
            ...
            if (!empty($options)) {
                $description =
                    esc_html__('options: ', 'gravityforms') .
                    ' ' .
                    implode(', ', $options);
            }
        ...
        if ($trial_field == 'enter_amount') {
            $trial_amount = rgar($feed['meta'], 'trial_amount')
                ? GFCommon::to_number(
                    rgar($feed['meta'], 'trial_amount'),
                    $entry['currency']
                )
                : 0;
        }
        ...
        if (
            !empty($products['shipping']['name']) &&
            !is_numeric($payment_field)
        ) {
            $line_items[] = [
    ...
    {
        $key =
            rgars($feed, 'meta/transactionType') === 'subscription'
                ? 'recurringAmount'
                : 'paymentAmount';
    ...
    public function maybe_process_callback()
    {
        ...
        $this->log_debug(
            __METHOD__ .
                '(): Initializing callback processing for: ' .
                $this->_slug
        );
        ...
        $this->log_debug(
            __METHOD__ .
                '(): Result from gateway callback => ' .
                print_r($callback_action, true)
        );
        ...
        if (is_wp_error($callback_action)) {
            ...
            $callback_action &&
            is_array($callback_action) &&
            rgar($callback_action, 'type') &&
            !rgar($callback_action, 'abort_callback')
        ) {
            $result = $this->process_callback_action($callback_action);

            $this->log_debug(
                __METHOD__ .
                    '(): Result of callback action => ' .
                    print_r($result, true)
            );
    ...
    private function display_callback_error($error)
    {
        ...
        $status = !rgempty('status_header', $data)
            ? $data['status_header']
            : 200;
    ...
    private function process_callback_action($action)
    {
        ...
        if (
            rgar($action, 'id') &&
            $this->is_duplicate_callback($action['id'])
        ) {
            return new WP_Error(
                'duplicate',
                sprintf(
                    esc_html__(
                        'This webhook has already been processed (Event Id: %s)',
                        'gravityforms'
                    ),
                    $action['id']
                )
        ...
        $action = $this->maybe_add_action_amount_formatted(
            $action,
            $entry['currency']
        );
        ...
        if (has_filter('gform_action_pre_payment_callback')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_action_pre_payment_callback.'
            );
        }
        ...
        switch ($action['type']) {
            ...
            case 'create_subscription':
                ...
                $result =
                    rgar($result, 'payment_status') == 'Active' &&
                    rgar($result, 'transaction_id') ==
            ...
            default:
                ...
                if (is_callable([$this, rgar($action, 'callback')])) {
        ...
        if (has_filter('gform_post_payment_callback')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_post_payment_callback.'
            );
        }
    ...
    public function register_callback($callback_id, $entry_id)
    {
        ...
        $wpdb->insert("{$wpdb->prefix}gf_addon_payment_callback", [
    ...
    public function is_duplicate_callback($callback_id)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}gf_addon_payment_callback WHERE addon_slug=%s AND callback_id=%s",
            $this->_slug,
            $callback_id
        );
        if ($wpdb->get_var($sql)) {
    ...
    public function add_pending_payment($entry, $action)
    {
        ...
        if (empty($action['note'])) {
            $action['note'] = sprintf(
                esc_html__(
                    'Payment is pending. Amount: %s. Transaction Id: %s.',
                    'gravityforms'
                ),
    ...
    public function complete_authorization(&$entry, $action)
    {
        ...
        if (!rgar($action, 'note')) {
            $action['note'] = sprintf(
                esc_html__(
                    'Payment has been authorized. Amount: %s. Transaction Id: %s.',
                    'gravityforms'
                ),
                $action['amount_formatted'],
                $action['transaction_id']
    ...
    public function complete_payment(&$entry, $action)
    {
        ...
        if (!rgar($action, 'note')) {
            $action['note'] = sprintf(
                esc_html__(
                    'Payment has been completed. Amount: %s. Transaction Id: %s.',
                    'gravityforms'
                ),
                $action['amount_formatted'],
                $action['transaction_id']
        ...
        if (has_filter('gform_post_payment_completed')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_post_payment_completed.'
            );
        }
        $this->post_payment_action($entry, $action);
    ...
    public function refund_payment($entry, $action)
    {
        ...
        if (empty($action['note'])) {
            $action['note'] = sprintf(
                esc_html__(
                    'Payment has been refunded. Amount: %s. Transaction Id: %s.',
                    'gravityforms'
                ),
                $action['amount_formatted'],
                $action['transaction_id']
        ...
        if (has_filter('gform_post_payment_refunded')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_post_payment_refunded.'
            );
        }
        $this->post_payment_action($entry, $action);
    ...
    public function fail_payment($entry, $action)
    {
        ...
        if (empty($action['note'])) {
            $action['note'] = sprintf(
                esc_html__('Payment has failed. Amount: %s.', 'gravityforms'),
                $action['amount_formatted']
            );
        }
    ...
    public function void_authorization($entry, $action)
    {
        ...
        if (empty($action['note'])) {
            $action['note'] = sprintf(
                esc_html__(
                    'Authorization has been voided. Transaction Id: %s',
                    'gravityforms'
                ),
                $action['transaction_id']
            );
    ...
    public function start_subscription($entry, $subscription)
    {
        ...
        if (!$this->has_subscription($entry)) {
            ...
            $this->add_note(
                ...
                sprintf(
                    esc_html__(
                        'Subscription has been created. Subscription Id: %s.',
                        'gravityforms'
                    ),
            ...
            $subscription = $this->maybe_add_action_amount_formatted(
                $subscription,
                $entry['currency']
            );
            ...
            if (has_filter('gform_post_subscription_started')) {
                $this->log_debug(
                    __METHOD__ .
                        '(): Executing functions hooked to gform_post_subscription_started.'
                );
            }
    ...
    public function add_subscription_payment($entry, $action)
    {
        ...
        if (empty($action['note'])) {
            $action['note'] = sprintf(
                esc_html__(
                    'Subscription has been paid. Amount: %s. Subscription Id: %s',
                    'gravityforms'
                ),
                $action['amount_formatted'],
        ...
        $transaction_id = !empty($action['transaction_id'])
            ? $action['transaction_id']
            : $action['subscription_id'];
        ...
        if (has_filter('gform_post_add_subscription_payment')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_post_add_subscription_payment.'
            );
        }
        $this->post_payment_action($entry, $action);
    ...
    public function fail_subscription_payment($entry, $action)
    {
        ...
        if (empty($action['note'])) {
            $action['note'] = sprintf(
                esc_html__(
                    'Subscription payment has failed. Amount: %s. Subscription Id: %s.',
                    'gravityforms'
                ),
                $action['amount_formatted'],
                $action['subscription_id']
        ...
        if (has_filter('gform_subscription_payment_failed')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_subscription_payment_failed.'
            );
        }
        /**
        ...
        if (has_filter('gform_post_fail_subscription_payment')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_post_fail_subscription_payment.'
            );
        }
        $this->post_payment_action($entry, $action);
    ...
    public function cancel_subscription($entry, $feed, $note = null)
    {
        ...
        if (!$note) {
            $note = sprintf(
                esc_html__(
                    'Subscription has been cancelled. Subscription Id: %s.',
                    'gravityforms'
                ),
        ...
        if (strtolower($entry['payment_status']) == 'cancelled') {
            $this->log_debug(
                __METHOD__ . '(): Subscription is already canceled.'
            );
        ...
        if (
            has_filter('gform_subscription_canceled') ||
            has_filter('gform_subscription_cancelled')
        ) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_subscription_canceled.'
            );
        }
    ...
    public function expire_subscription($entry, $action)
    {
        ...
        if (empty($action['note'])) {
            $action['note'] = sprintf(
                esc_html__(
                    'Subscription has expired. Subscriber Id: %s',
                    'gravityforms'
                ),
                $action['subscription_id']
            );
    ...
    {
        if (
            rgar($entry, 'transaction_type') == 2 &&
            !rgempty('transaction_id', $entry)
        ) {
            return true;
        } else {
            return false;
    ...
    public function get_entry_by_transaction_id($transaction_id)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT id FROM {$entry_table_name} WHERE transaction_id = %s",
            $transaction_id
        );
        ...
        if (!$entry_id) {
            $sql = $wpdb->prepare(
                "SELECT lead_id FROM {$wpdb->prefix}gf_addon_payment_transaction WHERE transaction_id = %s",
                $transaction_id
            );
            $entry_id = $wpdb->get_var($sql);
        }
    ...
    public function post_payment_action($entry, $action)
    {
        ...
        if (has_filter('gform_post_payment_action')) {
            $this->log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_post_payment_action.'
            );
        }
        ...
        if (!empty($supported_events)) {
            if (!empty($action['payment_status'])) {
                $action[
                    'payment_status'
                ] = GFCommon::get_entry_payment_status_text(
                    $action['payment_status']
                );
            }
    ...
    public function setup_cron()
    {
        ...
        add_action($cron_name, [$this, 'check_status']);
    ...
    {
        if (
            $this->_requires_credit_card &&
            !$this->has_credit_card_field($this->get_current_form())
        ) {
    ...
    public function requires_credit_card_message()
    {
        ...
        return sprintf(
            esc_html__(
                "You must add a Credit Card field to your form before creating a feed. Let's go %sadd one%s!",
                'gravityforms'
            ),
            "<a href='" . esc_url($url) . "'>",
            '</a>'
                ...
                'fields' => [
                    [
                        ...
                        'tooltip' =>
                            ...
                            '</h6>' .
                            esc_html__(
                                "Select which field determines the recurring payment amount, or select 'Form Total' to use the total of all pricing fields as the recurring amount.",
                                'gravityforms'
                            ),
                    ],
                    [
                        ...
                        'tooltip' =>
                            ...
                            '</h6>' .
                            esc_html__(
                                'Select your billing cycle.  This determines how often the recurring payment should occur.',
                                'gravityforms'
                            ),
                    ],
                    [
                        ...
                        'tooltip' =>
                            ...
                            '</h6>' .
                            esc_html__(
                                'Select how many times the recurring payment should be made.  The default is to bill the customer until the subscription is canceled.',
                                'gravityforms'
                            ),
                    ],
                    [
                    ...
                    [
                        ...
                        'tooltip' =>
                            ...
                            '</h6>' .
                            esc_html__(
                                'Enable a trial period.  The user\'s recurring payment will not begin until after this trial period.',
                                'gravityforms'
                            ),
                    ],
                ],
    ...
    public function settings_billing_cycle($field, $echo = true)
    {
        ...
        foreach ($intervals as $unit => $interval) {
            if (!empty($interval)) {
                $choices[] = ['value' => $unit, 'label' => $interval['label']];
            }
        }
        ...
        $unit_field = [
            ...
            'type' => 'select',
            'onchange' =>
                "loadBillingLength('" . esc_attr($field['name']) . "')",
            'choices' => $choices,
        ];
    ...
    public function settings_trial($field, $echo = true)
    {
        ...
        $form = $this->get_current_form();
        $payment_choices = array_merge($this->get_payment_choices($form), [
            [
        ...
        $product_field = [
            'name' => $field['name'] . '_product',
            'type' => 'select',
            'class' => $this->get_setting("{$field['name']}_enabled")
                ? ''
                : 'hidden',
        ...
        $amount_field = [
            ...
            'class' =>
                $this->get_setting("{$field['name']}_enabled") &&
                $this->get_setting("{$field['name']}_product") == 'enter_amount'
                    ? 'gform_currency'
                    : 'hidden gform_currency',
        ];
    ...
    public function billing_info_fields()
    {
        $fields = [
            ...
            [
                'name' => 'address2',
                'label' => esc_html__('Address 2', 'gravityforms'),
                'required' => false,
            ],
            [
                'name' => 'city',
            ...
            [
                'name' => 'state',
                'label' => esc_html__('State', 'gravityforms'),
                'required' => false,
            ],
            [
                'name' => 'zip',
                'label' => esc_html__('Zip', 'gravityforms'),
                'required' => false,
            ],
            [
                'name' => 'country',
    ...
    {
        $fields = GFAPI::get_fields_by_type($form, ['product']);
        $choices = [
            [
                'label' => esc_html__('Select a product field', 'gravityforms'),
                'value' => '',
            ],
        ];
        ...
        foreach ($fields as $field) {
            ...
            $choices[] = ['value' => $field_id, 'label' => $field_label];
        }
    ...
    public function get_results_page_config()
    {
        return [
            ...
            'capabilities' => ['gravityforms_view_entries'],
            'callbacks' => [
                'fields' => [$this, 'results_fields'],
                'data' => [$this, 'results_data'],
                'markup' => [$this, 'results_markup'],
                'filter_ui' => [$this, 'results_filter_ui'],
            ],
        ];
    }
    ...
    {
        $html =
            "<table width='100%' id='gaddon-results-summary'>
                    <tr>
                        <td class='gaddon-results-summary-label'>" .
            esc_html__('Today', 'gravityforms') .
            "</td>
                        <td class='gaddon-results-summary-label'>" .
            esc_html__('Yesterday', 'gravityforms') .
            "</td>
                        <td class='gaddon-results-summary-label'>" .
            esc_html__('Last 30 Days', 'gravityforms') .
            "</td>
                        <td class='gaddon-results-summary-label'>" .
                    ...
                    <tr>
                        <td class='gaddon-results-summary-data'>
                            <div class='gaddon-results-summary-data-box'>
                                ...
                                <div class='gaddon-results-summary-secondary'>{$data['summary']['today']['subscriptions']} " .
            esc_html__('subscriptions', 'gravityforms') .
            "</div>
                                <div class='gaddon-results-summary-secondary'>{$data['summary']['today']['orders']} " .
            esc_html__('orders', 'gravityforms') .
            "</div>
                            </div>
                        </td>
                        <td class='gaddon-results-summary-data'>
                            <div class='gaddon-results-summary-data-box'>
                                ...
                                <div class='gaddon-results-summary-secondary'>{$data['summary']['yesterday']['subscriptions']} " .
            esc_html__('subscriptions', 'gravityforms') .
            "</div>
                                <div class='gaddon-results-summary-secondary'>{$data['summary']['yesterday']['orders']} " .
            esc_html__('orders', 'gravityforms') .
            "</div>
                            </div>
                        </td>
                        ...
                        <td class='gaddon-results-summary-data'>
                            <div class='gaddon-results-summary-data-box'>
                                ...
                                <div class='gaddon-results-summary-secondary'>{$data['summary']['last30']['subscriptions']} " .
            esc_html__('subscriptions', 'gravityforms') .
            "</div>
                                <div class='gaddon-results-summary-secondary'>{$data['summary']['last30']['orders']} " .
            esc_html__('orders', 'gravityforms') .
            "</div>
                            </div>
                        </td>
                        <td class='gaddon-results-summary-data'>
                            <div class='gaddon-results-summary-data-box'>
                                ...
                                <div class='gaddon-results-summary-secondary'>{$data['summary']['total']['subscriptions']} " .
            esc_html__('subscriptions', 'gravityforms') .
            "</div>
                                <div class='gaddon-results-summary-secondary'>{$data['summary']['total']['orders']} " .
            esc_html__('orders', 'gravityforms') .
            '</div>
                            </div>
                        </td>
        ...
        if ($data['row_count'] == '0') {
            $html .=
                "<div class='updated' style='padding:20px; margin-top:40px;'>" .
                esc_html__(
            ...
            $sales_table = new GFPaymentStatsTable(
                $data['table']['header'],
                $data['data'],
                $data['row_count'],
                $data['page_size']
    ...
    public static function get_sales_chart($sales_data)
    {
        ...
        $data_table = [];
        $data_table[] = [
            $sales_data['hAxis_title'],
            $sales_data['vAxis_title'],
        ];

        foreach ($sales_data['data'] as $key => $value) {
            $data_table[] = [(string) $key, $value];
        }
    ...
    public function get_sales_data($form_id, $search, $state)
    {
        ...
        $data = [
            ...
            'table' => [
                'header' => [
                    ...
                    'subscriptions' => esc_html__(
                        'Subscriptions',
                        'gravityforms'
                    ),
                    'recurring_payments' => esc_html__(
                        'Recurring Payments',
                        'gravityforms'
                    ),
        ...
        switch ($group) {
            case 'weekly':
                $select =
                    "concat(left(transaction.week,4), ' - ', right(transaction.week,2)) as week";
                $select_inner1 =
                    "yearweek(CONVERT_TZ(payment_date, '+00:00', '" .
                    $tz_offset .
                    "')) week";
                $select_inner2 =
                    "yearweek(CONVERT_TZ(t.date_created, '+00:00', '" .
                    $tz_offset .
                    "')) week";
                $group_by = 'week';
                $order_by = 'week desc';
                ...
                $data['chart']['hAxis']['label'] = esc_html__(
                    'Week',
                    'gravityforms'
                );
                $data['table']['header'] = array_merge(
                    ['week' => esc_html__('Week', 'gravityforms')],
                    $data['table']['header']
                );
            ...
            case 'monthly':
                $select =
                    "date_format(transaction.inner_month, '%%Y') as year, date_format(transaction.inner_month, '%%c') as month, '' as month_abbrev, '' as month_year";
                $select_inner1 =
                    "date_format(CONVERT_TZ(payment_date, '+00:00', '" .
                    $tz_offset .
                    "'), '%%Y-%%m-01') inner_month";
                $select_inner2 =
                    "date_format(CONVERT_TZ(t.date_created, '+00:00', '" .
                    $tz_offset .
                    "'), '%%Y-%%m-01') inner_month";
                ...
                $data['table']['header'] = array_merge(
                    ['month_year' => esc_html__('Month', 'gravityforms')],
                    $data['table']['header']
                );
            ...
            default:
                //daily
                $select =
                    "transaction.date, date_format(transaction.date, '%%c') as month, day(transaction.date) as day, dayname(transaction.date) as day_of_week, '' as month_day";
                $select_inner1 =
                    "date(CONVERT_TZ(payment_date, '+00:00', '" .
                    $tz_offset .
                    "')) as date";
                $select_inner2 =
                    "date(CONVERT_TZ(t.date_created, '+00:00', '" .
                    $tz_offset .
                    "')) as date";
                $group_by = 'date';
                $order_by = 'date desc';
        ...
        if (isset($search['start_date'])) {
            $lead_date_filter = $wpdb->prepare(
                " AND timestampdiff(SECOND, %s, CONVERT_TZ(l.payment_date, '+00:00', '" .
                    $tz_offset .
                    "')) >= 0",
                $search['start_date']
            );
            $transaction_date_filter = $wpdb->prepare(
                " AND timestampdiff(SECOND, %s, CONVERT_TZ(t.date_created, '+00:00', '" .
                    $tz_offset .
                    "')) >= 0",
                $search['start_date']
            );
        ...
        if (!empty($payment_method)) {
            $payment_method_filter = $wpdb->prepare(
                ' AND l.payment_method=%s',
                $payment_method
            );
        }
        ...
        if (isset($search['start_date']) || isset($search['end_date'])) {
            foreach ($results as &$result) {
                ...
                $result['recurring_payments'] = intval(
                    $result['recurring_payments']
                );
            ...
            for ($i = 1; $i <= 10; $i++) {
                ...
                $display_result['subscriptions'] = intval(
                    $display_result['subscriptions']
                );
                $display_result['refunds'] = intval($display_result['refunds']);
                $display_result['recurring_payments'] = intval(
                    $display_result['recurring_payments']
                );
                $display_result['revenue'] = floatval(
                ...
                $decremented_date =
                    $current_date . ' ' . $i * -1 . ' ' . $decrement_period;

                $current_period_timestamp = strtotime($decremented_date);

                $current_period = date(
                    $current_period_format,
    ...
    public function format_chart_h_axis($result)
    {
        ...
        if (isset($result['month_abbrev'])) {
            ...
            $result['month_year'] =
                $months[intval($result['month']) - 1] . ', ' . $result['year'];

            return $result;
        } elseif (isset($result['month_day'])) {
            $result['month_day'] =
                $months[intval($result['month']) - 1] . ' ' . $result['day'];
    ...
    public function get_sales_summary($form_id)
    {
        ...
        $summary = $wpdb->get_results(
            $wpdb->prepare(
                "
                    ...
                    FROM (
                       SELECT  date( CONVERT_TZ(payment_date, '+00:00', '" .
                    $tz_offset .
                    "') ) as date,
                               sum( if(transaction_type = 1,1,0) ) as orders,
                               sum( if(transaction_type = 2,1,0) ) as subscriptions
                       FROM {$entry_table_name}
                       WHERE status='active' AND form_id = %d AND datediff(now(), CONVERT_TZ(payment_date, '+00:00', '" .
                    $tz_offset .
                    "') ) <= 30
                       GROUP BY date
                     ) AS leads
                     ...
                     RIGHT OUTER JOIN(
                       SELECT  date( CONVERT_TZ(t.date_created, '+00:00', '" .
                    $tz_offset .
                    "') ) as date,
                               sum( if(t.transaction_type = 'refund', abs(t.amount) * -1, t.amount) ) as revenue
                       FROM {$wpdb->prefix}gf_addon_payment_transaction t
        ...
        $total_revenue = $wpdb->get_var(
            $wpdb->prepare(
                "
                    ...
                    FROM {$wpdb->prefix}gf_addon_payment_transaction t
                    INNER JOIN {$entry_table_name} l ON l.id = t.lead_id
                    WHERE l.form_id=%d AND status='active'",
                $form_id
            )
        );
        ...
        $result = [
            ...
            'last30' => ['revenue' => 0, 'orders' => 0, 'subscriptions' => 0],
            'total' => [
        ...
        foreach ($summary as $day) {
            ...
            $is_within_30_days =
                strtotime($day['date']) >= strtotime('-30 days', $local_time);
            if ($is_within_30_days) {
                $result['last30']['revenue'] += floatval($day['revenue']);
                $result['last30']['orders'] += floatval($day['orders']);
                $result['last30']['subscriptions'] += floatval(
                    $day['subscriptions']
                );
            }
        }

        $result['last30']['revenue'] = GFCommon::to_money(
            $result['last30']['revenue']
        );

        return $result;
    }

    public function results_filter_ui(
        $filter_ui,
        $form_id,
        $page_title,
        $gf_page,
        ...
        $view_markup =
            "<div>
                    <select id='gaddon-sales-group' name='group'>
                        <option value='daily' " .
            selected('daily', rgget('group'), false) .
            '>' .
            esc_html__('Daily', 'gravityforms') .
            "</option>
                        <option value='weekly' " .
            selected('weekly', rgget('group'), false) .
            '>' .
            esc_html__('Weekly', 'gravityforms') .
            "</option>
                        <option value='monthly' " .
            selected('monthly', rgget('group'), false) .
            '>' .
            esc_html__('Monthly', 'gravityforms') .
            '</option>
                    </select>
                  </div>';
        $view_filter = [
            'view' => [
                ...
                'tooltip' =>
                    ...
                    '</h6>' .
                    esc_html__(
                        'Select how you would like the sales data to be displayed.',
                        'gravityforms'
                    ),
                'markup' => $view_markup,
            ],
            ...
            "
                <div>
                    <select id='gaddon-sales-group' name='payment_method'>
                        <option value=''>" .
            esc_html__(
                _x('Any', 'regarding a payment method', 'gravityforms')
            ) .
            '</option>';
        ...
        foreach ($payment_methods as $payment_method) {
            $payment_method_markup .=
                ...
                "' " .
                selected($payment_method, rgget('payment_method'), false) .
                '>' .
                $payment_method .
                '</option>';
        }
    ...
    public function get_payment_methods($form_id)
    {
        ...
        $payment_methods = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT DISTINCT payment_method FROM {$entry_table_name} WHERE form_id=%d",
                $form_id
            )
        );
    ...
    public static function get_entry_table_name()
    {
        return version_compare(
            self::get_gravityforms_db_version(),
            '2.3-dev-1',
            '<'
        )
    ...
    {
        return method_exists('GFFormsModel', 'get_database_version')
    ...
    public function uninstall()
    {
        ...
        $sql = $wpdb->prepare(
            "DELETE FROM {$wpdb->prefix}gf_addon_payment_transaction
                                WHERE lead_id IN
                                   (SELECT lead_id FROM {$entry_meta_table_name} WHERE meta_key='payment_gateway' AND meta_value=%s)",
            $this->_slug
        );
        $wpdb->query($sql);
        ...
        if ($this->_supports_callbacks) {
            ...
            $sql = $wpdb->prepare(
                "DELETE FROM {$wpdb->prefix}gf_addon_payment_callback WHERE addon_slug=%s",
                $this->_slug
            );
            $wpdb->query($sql);
        }
    ...
    public function scripts()
    {
        ...
        $scripts = [
            [
                ...
                'src' =>
                    $this->get_gfaddon_base_url() .
                    "/js/gaddon_payment{$min}.js",
                'version' => GFCommon::$version,
                'strings' => [
                    'subscriptionCancelWarning' => __(
                        "Warning! This subscription will be canceled. This cannot be undone. 'OK' to cancel subscription, 'Cancel' to stop",
                        'gravityforms'
                    ),
                    'subscriptionCancelNonce' => wp_create_nonce(
                        'gaddon_cancel_subscription'
                    ),
                    'subscriptionCanceled' => __('Canceled', 'gravityforms'),
                    'subscriptionError' => __(
                        'The subscription could not be canceled. Please try again later.',
                        'gravityforms'
                    ),
                ],
                'enqueue' => [
            ...
            [
                'handle' => 'gaddon_token',
                'src' =>
                    $this->get_gfaddon_base_url() . "/js/gaddon_token{$min}.js",
                'version' => GFCommon::$version,
                'deps' => ['jquery'],
                'in_footer' => false,
                'enqueue' => [[$this, 'enqueue_creditcard_token_script']],
            ],
            [
                'handle' => 'gform_form_admin',
                'enqueue' => [['admin_page' => ['entry_edit']]],
            ],
        ];
     ...
     * @return string
     */
    public function add_creditcard_token_input(
        $content,
        $field,
        $value,
        $entry_id,
        ...
        if (
            !$this->has_feed($form_id) ||
            GFFormsModel::get_input_type($field) != 'creditcard'
        ) {
            return $content;
        }
    ...
    public function force_ajax_for_creditcard_tokens($args)
    {
        ...
        $args['ajax'] = $this->enqueue_creditcard_token_script($form)
            ? true
            : $args['ajax'];
    ...
    {
        return $form &&
            $this->has_feed($form['id']) &&
            $this->creditcard_token_info($form);
    }
     ...
     * @return void
     */
    public function register_creditcard_token_script(
        $form,
        $field_values,
        $is_ajax
    ) {
        ...
        $gftoken = [
            ...
            'responseField' =>
                '#gf_' .
                str_replace('gravityforms', '', $this->_slug) .
                '_response',
        ];

        /* Get needed fields. */
        $gftoken['fields'] = $this->get_creditcard_token_entry_fields(
            $gftoken['feeds']
        );
     ...
     * @return array
     */
    public function maybe_add_action_amount_formatted(
        $action,
        $currency_code = ''
    ) {
        if (!empty($action['amount']) && empty($action['amount_formatted'])) {
            $action['amount_formatted'] = GFCommon::to_money(
                $action['amount'],
                $currency_code
            );
        }
    ...
    public function ajax_cancel_subscription()
    {
        ...
        $this->log_debug(
            __METHOD__ . '(): Processing request for entry #' . $entry_id
        );
        ...
        if (
            $feed &&
            $this->_slug === $feed['addon_slug'] &&
            !GFCommon::current_user_can_any($this->_capabilities_settings_page)
        ) {
            wp_send_json_error([
                'message' => esc_html__('Access denied.', 'gravityforms'),
            ]);
        }
        ...
        if (empty($feed)) {
            $this->log_debug(
                __METHOD__ . '(): Aborting. Entry does not have a feed.'
            );
            return;
        }
        ...
        if ($this->cancel($entry, $feed)) {
            ...
            $this->log_debug(
                __METHOD__ . '(): Aborting. Unable to cancel subscription.'
            );
            wp_send_json_error();
        }
    }
    ...
    private function payment_method_is_overridden(
        $method_name,
        $base_class = 'GFPaymentAddOn'
    ) {
...
class GFPaymentStatsTable extends WP_List_Table
{
    ...
    {
        esc_html_e(
            "There hasn't been any sales in the specified date range.",
            'gravityforms'
        );
    }
    ...
    function pagination($which)
    {
        ...
        $output =
            ...
            sprintf(
                _n('1 item', '%s items', $total_items, 'gravityforms'),
        ...
        $page_links[] =
            ...
            sprintf(
                esc_html_x('%1$s of %2$s', 'paging', 'gravityforms'),
                $html_current_page,
                $html_total_pages
            ) .
            '</span>';
