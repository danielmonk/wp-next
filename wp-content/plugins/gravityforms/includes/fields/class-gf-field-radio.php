...
class GF_Field_Radio extends GF_Field
{
    ...
    public function validate($value, $form)
    {
        if (
            $this->isRequired &&
            $this->enableOtherChoice &&
            rgpost("input_{$this->id}") == 'gf_other_choice'
        ) {
            if (
                empty($value) ||
                strtolower($value) ==
                    strtolower(GFCommon::get_other_choice_value($this))
            ) {
                $this->failed_validation = true;
                $this->validation_message = empty($this->errorMessage)
                    ? esc_html__('This field is required.', 'gravityforms')
                    : $this->errorMessage;
            }
        }
    }
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        return sprintf(
            "<div class='ginput_container ginput_container_radio'><ul class='gfield_radio' id='%s'>%s</ul></div>",
            $field_id,
            $this->get_radio_choices($value, $disabled_text, $form_id)
        );
    }

    public function get_radio_choices(
        $value = '',
        $disabled_text = '',
        $form_id = 0
    ) {
        ...
        if (is_array($this->choices)) {
            ...
            if ($is_form_editor && $count < $total) {
                $choices .=
                    ...
                    sprintf(
                        esc_html__(
                            '%d of %d items shown. Edit field to view all',
                            'gravityforms'
                        ),
                        $count,
                        $total
        ...
        return gf_apply_filters(
            ['gform_field_choices', $this->formId],
            $choices,
            $this
        );
    }
     ...
     * @return string
     */
    public function get_choice_html(
        $choice,
        &$choice_id,
        $value,
        $disabled_text,
        ...
        $field_value =
            !empty($choice['value']) || $this->enableChoiceValue
                ? $choice['value']
                : $choice['text'];
        ...
        if (rgblank($value) && rgget('view') != 'entry') {
            ...
            $checked = GFFormsModel::choice_value_match($this, $choice, $value)
                ? "checked='checked'"
                : '';
        }
        ...
        $label = sprintf(
            "<label for='choice_%s' id='label_%s'>%s</label>",
            $id,
            $id,
            $choice['text']
        );
        ...
        if ($this->enableOtherChoice && rgar($choice, 'isOtherChoice')) {
            $other_default_value = empty($choice['text'])
                ? GFCommon::get_other_choice_value($this)
                : $choice['text'];

            $onfocus = !$is_admin
                ? 'jQuery(this).prev("input")[0].click(); if(jQuery(this).val() == "' .
                    $other_default_value .
                    '") { jQuery(this).val(""); }'
                : '';
            $onblur = !$is_admin
                ? 'if(jQuery(this).val().replace(" ", "") == "") { jQuery(this).val("' .
                    $other_default_value .
                    '"); }'
                : '';

            $input_focus = !$is_admin
                ? "onfocus=\"jQuery(this).next('input').focus();\""
                : '';
            $value_exists = GFFormsModel::choices_value_match(
                $this,
                $this->choices,
                $value
            );
            ...
            $label =
                "<input id='input_{$this->formId}_{$this->id}_other' name='input_{$this->id}_other' type='text' value='" .
                esc_attr($other_value) .
                "' aria-label='" .
                esc_attr__('Other', 'gravityforms') .
                "' onfocus='$onfocus' onblur='$onblur' $tabindex $disabled_text />";
        }

        $choice_markup = sprintf(
            "<li class='gchoice_$id'><input name='input_%d' type='radio' value='%s' %s id='choice_%s' $tabindex %s %s />%s</li>",
            $this->id,
         ...
         */
        return gf_apply_filters(
            ['gform_field_choice_markup_pre_render', $this->formId, $this->id],
            $choice_markup,
            $choice,
            $this,
            $value
    ...
    public function get_value_entry_list(
        ...
        return wp_kses_post(
            GFCommon::selection_display($value, $this, $entry['currency'])
        );
    }
    ...
    public function get_value_entry_detail(
        ...
        return wp_kses_post(
            GFCommon::selection_display($value, $this, $currency, $use_text)
        );
    }
    ...
    public function get_value_merge_tag(
        ...
        $use_price =
            $format_currency || (!$use_value && in_array('price', $modifiers));

        if (is_array($raw_value) && (string) intval($input_id) != $input_id) {
            $items = [$input_id => $value]; // Float input Ids. (i.e. 4.1 ). Used when targeting specific checkbox items.
        } elseif (is_array($raw_value)) {
            $items = $raw_value;
        } else {
            $items = [$input_id => $raw_value];
        }
        ...
        foreach ($items as $input_id => $item) {
            if ($use_value) {
                ...
                $val = RGFormsModel::is_field_hidden($form, $this, [], $entry)
                    ? ''
                    : $item_value;
            } else {
                $val = RGFormsModel::is_field_hidden($form, $this, [], $entry)
            ...
            $ary[] = GFCommon::format_variable_value(
                $val,
                $url_encode,
                $esc_html,
                $format
    ...
    public function sanitize_entry_value($value, $form_id)
    {
        ...
        $value = wp_kses_no_null($value, ['slash_zero' => 'keep']);
