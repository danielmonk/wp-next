...
class GF_Field_Number extends GF_Field
{
    ...
    public function validate($value, $form)
    {
        ...
        $raw_value =
            isset($_POST['input_' . $this->id]) &&
            !is_array($_POST['input_' . $this->id])
                ? GFCommon::maybe_add_leading_zero(rgpost('input_' . $this->id))
                : $value;

        $requires_valid_number =
            !rgblank($raw_value) && !$this->has_calculation();
        $is_valid_number =
            $this->validate_range($value) &&
            GFCommon::is_numeric($raw_value, $this->numberFormat);

        if ($requires_valid_number && !$is_valid_number) {
            $this->failed_validation = true;
            $this->validation_message = empty($this->errorMessage)
                ? $this->get_range_message()
                : $this->errorMessage;
        } elseif ($this->type == 'quantity') {
            if (intval($value) != $value) {
                $this->failed_validation = true;
                $this->validation_message = empty($field['errorMessage'])
                    ? esc_html__(
                        'Please enter a valid quantity. Quantity cannot contain decimals.',
                        'gravityforms'
                    )
                    : $field['errorMessage'];
            } elseif (
    ...
    private function validate_range($value)
    {
        ...
        $numeric_min =
            $this->numberFormat == 'decimal_comma'
                ? GFCommon::clean_number($this->rangeMin, 'decimal_comma')
                : $this->rangeMin;
        $numeric_max =
            $this->numberFormat == 'decimal_comma'
                ? GFCommon::clean_number($this->rangeMax, 'decimal_comma')
                : $this->rangeMax;

        if (
            (is_numeric($numeric_min) && $value < $numeric_min) ||
    ...
    public function get_range_message()
    {
        ...
        if ($this->numberFormat == 'decimal_comma') {
            $numeric_min = empty($min)
                ? ''
                : GFCommon::clean_number($min, 'decimal_comma', '');
            $numeric_max = empty($max)
                ? ''
                : GFCommon::clean_number($max, 'decimal_comma', '');
        }
        ...
        if (is_numeric($numeric_min) && is_numeric($numeric_max)) {
            ...
            $message = sprintf(
                esc_html__(
                    'Please enter a number greater than or equal to %s.',
                    'gravityforms'
                ),
                "<strong>$min</strong>"
            );
            ...
            $message = sprintf(
                esc_html__(
                    'Please enter a number less than or equal to %s.',
                    'gravityforms'
                ),
                "<strong>$max</strong>"
            );
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        if (!$is_entry_detail && !$is_form_editor) {
            if ($this->has_calculation()) {
                ...
                $validation_class = $this->failed_validation
                    ? 'validation_message'
                    : '';
                ...
                if (
                    ...
                    $instruction =
                        "<div class='instruction $validation_class'>" .
                        $message .
                        '</div>';
                }
            }
        } elseif (rgget('view') == 'entry') {
            $value = GFCommon::format_number(
                $value,
                $this->numberFormat,
                rgar($entry, 'currency')
            );
        ...
        $html_input_type =
            $is_html5 &&
            !$this->has_calculation() &&
            ($this->numberFormat != 'currency' &&
                $this->numberFormat != 'decimal_comma')
                ? 'number'
                : 'text'; // chrome does not allow number fields to have commas, calculations and currency values display numbers with commas
        ...
        $include_thousands_sep = apply_filters(
        ...
        $input = sprintf(
            "<div class='ginput_container ginput_container_number'><input name='input_%d' id='%s' type='{$html_input_type}' {$step_attr} {$min_attr} {$max_attr} value='%s' class='%s' {$tabindex} {$read_only} %s %s %s %s %s/>%s</div>",
            $id,
            $field_id,
            esc_attr($value),
            esc_attr($class),
    ...
    public function get_value_merge_tag(
        ...
        $formatted_value = GFCommon::format_number(
            $value,
            $this->numberFormat,
            rgar($entry, 'currency'),
            $include_thousands_sep
    ...
    public function get_value_save_entry(
        ...
        if ($this->has_calculation()) {
            ...
            if ($this->numberFormat !== 'currency') {
                $value = GFCommon::round_number(
                    $value,
                    $this->calculationRounding
                );
            }
            ...
            $value = $this->clean_number(
                GFCommon::maybe_add_leading_zero($value)
            );
        }
    ...
    public function sanitize_settings()
    {
        ...
        if (
            !in_array($this->numberFormat, [
                'currency',
                'decimal_comma',
                'decimal_dot',
            ])
        ) {
            $this->numberFormat = GFCommon::is_currency_decimal_dot()
                ? 'decimal_dot'
                : 'decimal_comma';
        }
        ...
        if ($this->numberFormat == 'decimal_comma') {
            $this->rangeMin = GFCommon::format_number(
                $this->rangeMin,
                'decimal_comma'
            );
            $this->rangeMax = GFCommon::format_number(
                $this->rangeMax,
                'decimal_comma'
            );
        }
