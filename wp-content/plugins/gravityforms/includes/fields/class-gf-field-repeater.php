...
class GF_Field_Repeater extends GF_Field
{
            ...
            foreach ($fields as $field) {
                ...
                if (is_array($inputs)) {
                    ...
                    foreach ($field_keys as $input_id) {
                        if (
                            is_numeric($input_id) &&
                            absint($input_id) == absint($field->id)
                        ) {
                            $field_value[$input_id] = $item[$input_id];
                        }
                    }
                } else {
                    $field_value = isset($item[$field->id])
                        ? $item[$field->id]
                        : '';
                }
                ...
                if (
                    $field->isRequired &&
                    $field->is_value_empty($field_value)
                ) {
    ...
    public function get_value_submission(
        ...
        $submission_values = $this->get_value_submission_recursive(
            $field_values,
     ...
     * @return array
     */
    public function get_value_submission_recursive(
        $field_values,
        $get_from_post_global_var
    ) {
        $items = [];
        ...
        if (isset($this->fields) && is_array($this->fields)) {
            foreach ($this->fields as $sub_field) {
                ...
                if (isset($sub_field->fields) && is_array($sub_field->fields)) {
                    ...
                    $field_items = $sub_field->get_value_submission_recursive(
                        $field_values,
                        $get_from_post_global_var
                    );
                } else {
                    $values = $sub_field->get_value_submission(
                        $field_values,
                        $get_from_post_global_var
                    );
                    ...
                    $field_items = $this->flatten(
                        $values,
                        $prefix,
                        $sub_field->is_value_submission_array()
                    );
                }
                $items = array_merge($items, $field_items);
            }
        } else {
            $values = $this->get_value_submission(
                $field_values,
                $get_from_post_global_var
            );
            $field_items = $this->flatten(
                $values,
                $this->id . '_',
                $this->is_value_submission_array()
            );
            $items = array_merge($items, $field_items);
        }
    ...
    public function get_field_input($form, $values = '', $entry = null)
    {
        ...
        return sprintf(
            "<div class='gfield_repeater_wrapper' data-max_items='{$max_items}'>%s</div>",
            $html
        );
    }
    ...
    public function get_input_items($values, $entry)
    {
        ...
        foreach ($values as $value) {
            ...
            foreach ($fields as $field) {
                ...
                $field_input = $this->get_sub_field_input(
                    $field,
                    $form,
                    $field_value,
                    $entry,
    ...
    public function get_buttons($values)
    {
        ...
        $add_events = $is_form_editor
            ? ''
            : "onclick='gformAddRepeaterItem(this)' onkeypress='gformAddRepeaterItem(this)'";
        $delete_events = $is_form_editor
            ? ''
            : sprintf(
                "onclick='if(confirm(\"%s\")){gformDeleteRepeaterItem(this)};' onkeypress='gformDeleteRepeaterItem(this)'",
                esc_js(
                    __(
        ...
        $disabled_icon_class =
            !empty($this->maxItems) && count($values) >= intval($this->maxItems)
                ? 'gfield_icon_disabled'
                : '';

        $add_button_text = $this->addButtonText
            ? $this->addButtonText
            : '&#43;';
        $remove_button_text = $this->removeButtonText
            ? $this->removeButtonText
            : '&#45;';

        $add_button_class = $this->addButtonText
            ? 'add_repeater_item_text'
            : 'add_repeater_item_plus';
        $remove_button_class = $this->removeButtonText
            ? 'remove_repeater_item_text'
            : 'remove_repeater_item_minus';
        $html =
            "<button type='button' class='add_repeater_item {$disabled_icon_class} {$add_button_class}' {$add_events}>" .
            $add_button_text .
            "</button>" .
            "<button type='button' class='remove_repeater_item {$remove_button_class}' {$delete_events} style='{$delete_display}'>" .
            $remove_button_text .
            "</button>";
    ...
    public function get_value_merge_tag(
        ...
        if ($format == 'html') {
            ...
            $merge_tag = $this->get_value_entry_detail(
                $raw_value,
                $entry['currency'],
                $use_text,
                $format,
            ...
            $merge_tag = $this->get_value_export_recursive(
                $entry,
                $input_id,
                $use_text,
                false,
    ...
    public function get_value_entry_list(
        ...
        $display_value = is_array($value)
            ? sprintf(esc_html__('Number of items: %d'), count($value))
            : '';
    ...
    public function get_value_entry_detail(
        ...
        if ($format == 'text') {
            return $this->get_value_export_recursive(
                [$this->id => $item_values],
                $this->id,
                $use_text,
                false,
        ...
        $repeater_style =
            $media == 'email'
                ? "style='padding: 5px 0 0 15px;font-size: 14px'"
                : '';
        $label_style =
            $media == 'email'
                ? "style='color: rgba(35, 40, 45, 1.000);font-weight:600; padding-top:10px;font-size: 14px'"
                : '';
        $sub_field_label_style =
            $media == 'email'
                ? "style='color:rgb(155, 154, 154);padding-top:8px;font-size: 14px;'"
                : '';
        ...
        foreach ($item_values as $item_value) {
            ...
            foreach ($fields as $sub_field) {
                if ($sub_field->fields) {
                    ...
                    $sub_field_value = $this->get_field_value(
                        $sub_field,
                        $item_value
                    );
                }
                $label = $sub_field->get_field_label(true, $item_values);
                $label = empty($sub_field->fields)
                    ? "<div class='gfield_repeater_label' {$sub_field_label_style}>{$label}</div>"
                    : '';
                $value = $sub_field->get_value_entry_detail(
                    $sub_field_value,
                    $currency,
                    $use_text,
                    'html',
        ...
        if ($field->fields) {
            ...
            if (is_array($value)) {
                if (is_array($inputs)) {
                    ...
                    $field_value = isset($value[$field->id])
                        ? $value[$field->id]
                        : '';
                }
            } else {
     ...
     * @return mixed
     */
    public function get_sub_field_input(
        $field,
        $form,
        $field_value,
        $entry,
        ...
        $field_content = $this->get_sub_field_content(
            $field,
            $field_value,
            $form,
            $entry
        ...
        $validate =
            ...
            rgpost('is_submit_' . $this->formId) &&
            ($target_page == 0 || $target_page > $source_page);
        ...
        $validation_message =
            $field->failed_validation && !empty($field->validation_message)
                ? sprintf(
                    "<div class='gfield_description validation_message'>%s</div>",
     ...
     * @return array
     */
    public function get_repeater_items(
        &$entry,
        $repeater_field = null,
        $index = ''
    ) {
            ...
            for ($i = 0; $i <= $max_indexes[$field->id]; $i++) {
                ...
                if (is_array($inputs)) {
                    foreach ($inputs as $input) {
                        ...
                        if (
                            $field->isRequired ||
                            $max_indexes[$field->id] > 0 ||
                            ($max_indexes[$field->id] === 0 && $value !== '')
                        ) {
    ...
    public function get_value_export(
        ...
        $export = $this->get_value_export_recursive(
            $entry,
            $input_id,
            $use_text,
            $is_csv
     ...
     * @return string|array
     */
    public function get_value_export_recursive(
        $entry,
        $input_id = '',
        $use_text = false,
        $is_csv = false,
        ...
        foreach ($items as $item) {
            foreach ($fields as $field) {
                ...
                $label =
                    str_repeat($padding, $depth) .
                    GFFormsModel::get_label($field);
                ...
                if (is_array($field->fields)) {
                    ...
                    $line =
                        $label .
                        "\n" .
                        $field->get_value_export_recursive(
                            $field_value,
                            $field->id,
                            $use_text,
                            $is_csv,
                    ...
                    if (
                        'list' === $field->get_input_type() &&
                        !empty($field_value[$field->id])
                    ) {
                        $list_rows = maybe_unserialize(
