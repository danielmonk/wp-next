...
class GF_Field_CreditCard extends GF_Field
{
    ...
    {
        return []; // this button is conditionally added in the form detail page
    }
    ...
    public function validate($value, $form)
    {
        ...
        if (
            ...
            (empty($card_number) ||
                empty($security_code) ||
                empty($expiration_date[0]) ||
                empty($expiration_date[1]))
        ) {
            ...
            if (empty($security_code)) {
                ...
                $this->validation_message =
                    ...
                    ' ' .
                    esc_html__(
                        'is not supported. Please enter one of the supported credit cards.',
                        'gravityforms'
                    );
            }
        }
    ...
    public function is_card_supported($card_slug)
    {
        ...
        $default_cards = ['amex', 'discover', 'mastercard', 'visa'];

        if (
            !empty($supported_cards) &&
            in_array($card_slug, $supported_cards)
        ) {
            return true;
        } elseif (
            empty($supported_cards) &&
            in_array($card_slug, $default_cards)
        ) {
            return true;
        }
    ...
    public function get_value_submission(
        ...
        if ($get_from_post_global_var) {
            $value[$this->id . '.1'] = $this->get_input_value_submission(
                'input_' . $this->id . '_1',
                rgar($this->inputs[0], 'name'),
                $field_values,
                true
            );
            $value[$this->id . '.2'] = $this->get_input_value_submission(
                'input_' . $this->id . '_2',
                rgar($this->inputs[1], 'name'),
                $field_values,
                true
            );
            $value[$this->id . '.3'] = $this->get_input_value_submission(
                'input_' . $this->id . '_3',
                rgar($this->inputs[3], 'name'),
                $field_values,
                true
            );
            $value[$this->id . '.4'] = $this->get_input_value_submission(
                'input_' . $this->id . '_4',
                rgar($this->inputs[4], 'name'),
                $field_values,
                true
            );
            $value[$this->id . '.5'] = $this->get_input_value_submission(
                'input_' . $this->id . '_5',
                rgar($this->inputs[5], 'name'),
                $field_values,
                true
            );
        } else {
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        $action = !($is_entry_detail || $is_form_editor)
            ? "gformMatchCard(\"{$field_id}_1\");"
            : '';
        ...
        $card_style = $this->creditCardStyle
            ? $this->creditCardStyle
            : 'style1';
        ...
        if (is_array($payment_methods)) {
            foreach ($payment_methods as $payment_method) {
                $checked =
                    rgpost('gform_payment_method') == $payment_method['key']
                        ? "checked='checked'"
                        : '';
        ...
        $checked =
            rgpost('gform_payment_method') == 'creditcard' ||
            rgempty('gform_payment_method')
                ? "checked='checked'"
                : '';
        $card_radio_button = empty($payment_options)
            ? ''
            : "<input type='radio' name='gform_payment_method' id='gform_payment_method_creditcard' value='creditcard' onclick='gformToggleCreditCard();' onkeypress='gformToggleCreditCard();' {$checked}/>";
        ...
        $card_number_field_input = GFFormsModel::get_input(
            $this,
            $this->id . '.1'
        );
        $html5_output =
            !is_admin() && GFFormsModel::is_html5_enabled()
                ? "pattern='[0-9]*' title='" .
                    esc_attr__('Only digits are allowed', 'gravityforms') .
                    "'"
                : '';
        $card_number_label =
            rgar($card_number_field_input, 'customLabel') != ''
                ? $card_number_field_input['customLabel']
                : esc_html__('Card Number', 'gravityforms');
        $card_number_label = gf_apply_filters(
            ['gform_card_number', $form_id],
            $card_number_label,
            $form_id
        );

        $card_number_placeholder = $this->get_input_placeholder_attribute(
        ...
        $expiration_month_input = GFFormsModel::get_input(
            $this,
            $this->id . '.2_month'
        );
        $expiration_month_placeholder = $this->get_input_placeholder_value(
            $expiration_month_input
        );
        $expiration_year_input = GFFormsModel::get_input(
            $this,
            $this->id . '.2_year'
        );
        $expiration_year_placeholder = $this->get_input_placeholder_value(
            $expiration_year_input
        );
        $expiration_months = $this->get_expiration_months(
            $expiration_month,
            $expiration_month_placeholder
        );
        $expiration_years = $this->get_expiration_years(
            $expiration_year,
            $expiration_year_placeholder
        );
        $expiration_label =
            rgar($expiration_month_input, 'customLabel') != ''
                ? $expiration_month_input['customLabel']
                : esc_html__('Expiration Date', 'gravityforms');
        $expiration_label = gf_apply_filters(
            ['gform_card_expiration', $form_id],
            $expiration_label,
            $form_id
        );
        if ($is_sub_label_above) {
        ...
        //security code field
        $tabindex = $this->get_tabindex();
        $security_code_field_input = GFFormsModel::get_input(
            $this,
            $this->id . '.3'
        );
        $security_code_label =
            rgar($security_code_field_input, 'customLabel') != ''
                ? $security_code_field_input['customLabel']
                : esc_html__('Security Code', 'gravityforms');
        $security_code_label = gf_apply_filters(
            ['gform_card_security_code', $form_id],
            $security_code_label,
            $form_id
        );
        $html5_output = GFFormsModel::is_html5_enabled()
            ? "pattern='[0-9]*' title='" .
                esc_attr__('Only digits are allowed', 'gravityforms') .
                "'"
            : '';
        $security_code_placeholder = $this->get_input_placeholder_attribute(
        ...
        $tabindex = $this->get_tabindex();
        $card_name_field_input = GFFormsModel::get_input(
            $this,
            $this->id . '.5'
        );
        $card_name_label =
            rgar($card_name_field_input, 'customLabel') != ''
                ? $card_name_field_input['customLabel']
                : esc_html__('Cardholder Name', 'gravityforms');
        $card_name_label = gf_apply_filters(
            ['gform_card_name', $form_id],
            $card_name_label,
            $form_id
        );

        $card_name_placeholder = $this->get_input_placeholder_attribute(
            $card_name_field_input
        );
        if ($is_sub_label_above) {
        ...
        return "<div class='ginput_complex{$class_suffix} ginput_container ginput_container_creditcard' id='{$field_id}'>" .
            $card_field .
            $expiration_field .
            $security_field .
            $card_name_field .
    ...
    private function get_expiration_months($selected_month, $placeholder)
    {
        ...
        for ($i = 1; $i < 13; $i++) {
            $selected =
                intval($selected_month) == $i ? "selected='selected'" : '';
    ...
    private function get_expiration_years($selected_year, $placeholder)
    {
        ...
        for ($i = $year; $i < $year + 20; $i++) {
            $selected =
                intval($selected_year) == $i ? "selected='selected'" : '';
    ...
    public function get_value_entry_detail(
        ...
        if (is_array($value)) {
            ...
            return empty($card_number)
                ? ''
                : $card_type . $separator . $card_number;
        } else {
            return '';
        }
    }
    ...
    public function get_form_inline_script_on_page_render($form)
    {
        ...
        if ($this->forceSSL && !GFCommon::is_ssl() && !GFCommon::is_preview()) {
            $script =
                ...
                esc_js(RGFormsModel::get_current_page_url(true)) .
                "';";
        } else {
        ...
        $script =
            "if(!window['gf_cc_rules']){window['gf_cc_rules'] = new Array(); } window['gf_cc_rules'] = " .
            GFCommon::json_encode($card_rules) .
            "; $script";
    ...
    public function get_entry_inputs()
    {
        ...
        foreach ($this->inputs as $input) {
            if (in_array($input['id'], [$this->id . '.1', $this->id . '.4'])) {
                $inputs[] = $input;
            }
        }
    ...
    public function get_value_save_entry(
        ...
        list($input_token, $field_id_token, $input_id) = rgexplode(
            '_',
            $input_name,
            3
        );
