...
class GF_Field extends stdClass implements ArrayAccess
{
    ...
    private function maybe_fire_array_access_deprecation_notice($offset)
    {
        ...
        if (!self::$deprecation_notice_fired) {
            _deprecated_function(
                "Array access to the field object is now deprecated. Further notices will be suppressed. \$field['" .
                    $offset .
                    "']",
                '2.0',
                'the object operator e.g. $field->' . $offset
            );
    ...
    public function __set($key, $value)
    {
        switch ($key) {
            case '_context_properties':
                _doing_it_wrong(
                    '$field->_context_properties',
                    'Use $field->get_context_property() instead.',
                    '2.3'
                );
                break;
            case 'adminOnly':
    ...
    public function &__get($key)
    {
        switch ($key) {
            ...
            case 'size':
                ...
                if (isset($this->size)) {
                    $value = GFCommon::whitelist($this->size, [
                        'small',
                        'medium',
                        'large',
                    ]);
    ...
    {
        return isset($this->_context_properties[$property_key])
            ? $this->_context_properties[$property_key]
            : null;
    }
    ...
    public function get_field_content($value, $force_frontend_label, $form)
    {
        ...
        $validation_message_id =
            'validation_message_' . $form_id . '_' . $this->id;
        $validation_message =
            $this->failed_validation && !empty($this->validation_message)
                ? sprintf(
                    "<div id='%s' class='gfield_description validation_message' aria-live='polite'>%s</div>",
                    $validation_message_id,
        ...
        $required_div =
            $is_admin || $this->isRequired
                ? sprintf(
                    "<span class='gfield_required'>%s</span>",
                    $this->isRequired ? '*' : ''
                )
                : '';
        ...
        $for_attribute = empty($target_input_id)
            ? ''
            : "for='{$target_input_id}'";
        ...
        if ($this->is_description_above($form)) {
            ...
            $field_content = sprintf(
                "%s<label class='%s' $for_attribute >%s%s</label>%s{FIELD}%s$clear",
                $admin_buttons,
            ...
            $field_content = sprintf(
                "%s<label class='%s' $for_attribute >%s%s</label>{FIELD}%s%s",
                $admin_buttons,
                esc_attr($this->get_field_label_class()),
                esc_html($field_label),
                $required_div,
    ...
    public function is_value_submission_empty($form_id)
    {
        ...
        if (is_array($this->inputs)) {
            ...
            if (is_array($value)) {
                ...
                $is_empty =
                    strlen(trim($value)) <= 0 ||
                    ($this->type == 'post_category' && $value < 0);
        ...
        if (is_array($inputs)) {
            ...
            foreach ($inputs as $input) {
                $value[
                    ...
                    'input_' . str_replace('.', '_', strval($input['id'])),
                    RGForms::get('name', $input),
                    $field_values,
                    $get_from_post_global_var
                );
            }
        } else {
            $value = $this->get_input_value_submission(
                'input_' . $this->id,
                $this->inputName,
                $field_values,
                $get_from_post_global_var
     ...
     * @return array|string
     */
    public function get_input_value_submission(
        $standard_name,
        $custom_name = '',
        $field_values = [],
        $get_from_post_global_var = true
        ...
        $form_id = $this->formId;
        if (
            !empty($_POST['is_submit_' . $form_id]) &&
            $get_from_post_global_var
        ) {
            ...
            return GFFormsModel::get_parameter_value(
                $custom_name,
                $field_values,
                $this
            );
     ...
     * @return string
     */
    public function get_value_merge_tag(
        $value,
        $input_id,
        $entry,
        $form,
    ...
    public function get_value_entry_detail(
        ...
        if (is_array($value)) {
            _doing_it_wrong(
                __METHOD__,
                'Override this method to handle array values',
                '2.0'
            );
            return $value;
        }
    ...
    public function get_conditional_logic_event($event)
    {
        ...
        if (
            empty($this->conditionalLogicFields) ||
        ...
        switch ($event) {
            case 'keyup':
                return "onchange='gf_apply_rules(" .
                    $this->formId .
                    ',' .
                    GFCommon::json_encode($this->conditionalLogicFields) .
                    ");' onkeyup='clearTimeout(__gf_timeout_handle); __gf_timeout_handle = setTimeout(\"gf_apply_rules(" .
                    $this->formId .
                    ',' .
                    GFCommon::json_encode($this->conditionalLogicFields) .
                    ")\", 300);'";
    ...
    public function get_field_placeholder_attribute()
    {
        ...
        return !rgblank($placeholder_value)
    ...
    {
        return isset($this->_is_entry_detail)
    ...
    public function is_description_above($form)
    {
        ...
        $is_description_above =
            $field_description_placement == 'above' &&
            ($field_label_placement == 'top_label' ||
                $field_label_placement == 'hidden_label' ||
                (empty($field_label_placement) &&
                    $form_label_placement == 'top_label'));
    ...
    public function get_admin_buttons()
    {
        ...
        $duplicate_field_link = !in_array($this->type, $duplicate_disabled)
            ? "<a class='field_duplicate_icon' id='gfield_duplicate_{$this->id}' title='" .
                esc_attr__('click to duplicate this field', 'gravityforms') .
                "' href='#' onclick='StartDuplicateField(this); return false;' onkeypress='StartDuplicateField(this); return false;'><i class='fa fa-files-o fa-lg'></i></a>"
            : '';
         ...
         */
        $duplicate_field_link = apply_filters(
            'gform_duplicate_field_link',
            $duplicate_field_link
        );

        $delete_field_link =
            "<a class='field_delete_icon' id='gfield_delete_{$this->id}' title='" .
            esc_attr__('click to delete this field', 'gravityforms') .
            "' href='#' onclick='DeleteField(this); return false;' onkeypress='DeleteField(this); return false;'><i class='fa fa-times fa-lg'></i></a>";
         ...
         */
        $delete_field_link = apply_filters(
            'gform_delete_field_link',
            $delete_field_link
        );
        $field_type_title = esc_html(
            GFCommon::get_field_type_title($this->type)
        );
        ...
        $admin_buttons = $is_admin
            ? "<div class='gfield_admin_icons'><div class='gfield_admin_header_title'>{$field_type_title} : " .
                esc_html__('Field ID', 'gravityforms') .
                " {$this->id}</div>" .
                $delete_field_link .
                $duplicate_field_link .
                "<a href='javascript:void(0);' class='field_edit_icon edit_icon_collapsed' aria-expanded='false' title='" .
                esc_attr__(
                    'click to expand and edit the options for this field',
                    'gravityforms'
                ) .
                "'><i class='fa fa-caret-down fa-lg'></i></a></div>"
            : '';
    ...
    {
        $field_label = $force_frontend_label
            ? $this->label
            : GFCommon::get_label($this);
        if (
            ($this->inputType == 'singleproduct' ||
                $this->inputType == 'calculation') &&
            !rgempty($this->id . '.1', $value)
        ) {
    ...
    public function get_description($description, $css_class)
    {
        ...
        return $is_admin || !empty($description)
            ? "<div class='$css_class' id='$id'>" . $description . '</div>'
            : '';
    }
    ...
    public function get_value_default()
    {
        if (is_array($this->inputs)) {
            $value = [];
            foreach ($this->inputs as $input) {
                $value[strval($input['id'])] = $this->is_form_editor()
            ...
            $value = $this->is_form_editor()
                ? $this->defaultValue
                : GFCommon::replace_variables_prepopulate($this->defaultValue);
        }
    ...
    public function register_form_init_scripts($form)
    {
        GFFormDisplay::add_init_script(
            ...
            $this->get_form_inline_script_on_page_render($form)
        );
    }
    ...
    public function sanitize_settings()
    {
        ...
        if ($this->size) {
            $this->size = GFCommon::whitelist(
                $this->size,
                $this->get_size_choices(true)
            );
        }
        ...
        if ($this->descriptionPlacement) {
            $this->descriptionPlacement = wp_strip_all_tags(
                $this->descriptionPlacement
            );
        }

        if ($this->subLabelPlacement) {
            $this->subLabelPlacement = wp_strip_all_tags(
                $this->subLabelPlacement
            );
        }
        ...
        if (is_array($this->inputs)) {
            foreach ($this->inputs as &$input) {
                ...
                if (isset($input['customLabel'])) {
                    $input['customLabel'] = $this->maybe_wp_kses(
                        $input['customLabel']
                    );
                }
                ...
                if (isset($input['placeholder'])) {
                    $input['placeholder'] = sanitize_text_field(
                        $input['placeholder']
                    );
                }

                if (isset($input['defaultValue'])) {
                    $input['defaultValue'] = wp_strip_all_tags(
                        $input['defaultValue']
                    );
                }
            }
    ...
    public function sanitize_settings_choices($choices = null)
    {
        ...
        foreach ($choices as &$choice) {
            ...
            if (isset($choice['value'])) {
                ...
                $choice['value'] = wp_kses_no_null($choice['value'], [
                    'slash_zero' => 'keep',
                ]);
                $choice['value'] = wp_kses_hook(
                    $choice['value'],
                    'post',
                    $allowed_protocols
                );
                $choice['value'] = wp_kses_split(
                    $choice['value'],
                    'post',
                    $allowed_protocols
                );
    ...
    {
        $choices = [
            ['value' => 'small', 'text' => __('Small', 'gravityforms')],
            ['value' => 'medium', 'text' => __('Medium', 'gravityforms')],
            ['value' => 'large', 'text' => __('Large', 'gravityforms')],
        ];
