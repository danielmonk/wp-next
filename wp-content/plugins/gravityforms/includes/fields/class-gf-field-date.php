...
class GF_Field_Date extends GF_Field
{
    ...
    {
        return in_array($this->dateType, ['datefield', 'datedropdown']);
    }
    ...
    public function validate($value, $form)
    {
        ...
        if (!empty($value)) {
            ...
            if (
                empty($date) ||
                !$this->checkdate($date['month'], $date['day'], $date['year'])
            ) {
                ...
                $message =
                    $this->dateType == 'datepicker'
                        ? sprintf(
                            esc_html__(
                                'Please enter a valid date in the format (%s).',
                                'gravityforms'
                            ),
                            $format_name
                        )
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        $format = empty($this->dateFormat)
            ? 'mdy'
            : esc_attr($this->dateFormat);
        ...
        $month_sub_label =
            rgar($month_input, 'customLabel') != ''
                ? $month_input['customLabel']
                : esc_html(_x('MM', 'Abbreviation: Month', 'gravityforms'));
        $day_sub_label =
            rgar($day_input, 'customLabel') != ''
                ? $day_input['customLabel']
                : esc_html__('DD', 'gravityforms');
        $year_sub_label =
            rgar($year_input, 'customLabel') != ''
                ? $year_input['customLabel']
                : esc_html__('YYYY', 'gravityforms');

        $month_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $month_input
        );
        $day_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $day_input
        );
        $year_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $year_input
        );

        $month_placeholder_value = GFCommon::get_input_placeholder_value(
            $month_input
        );
        $day_placeholder_value = GFCommon::get_input_placeholder_value(
            $day_input
        );
        $year_placeholder_value = GFCommon::get_input_placeholder_value(
            $year_input
        );
        ...
        $year_min_attribute =
            $is_html5 && is_numeric($year_min) ? "min='{$year_min}'" : '';
        $year_max_attribute =
            $is_html5 && is_numeric($year_max) ? "max='{$year_max}'" : '';
        $year_step_attribute = $is_html5 ? "step='1'" : '';

        $field_position = substr($format, 0, 3);
        if ($is_form_editor) {
            $datepicker_display = in_array($this->dateType, [
                'datefield',
                'datedropdown',
            ])
                ? 'none'
                : 'inline';
            $datefield_display =
                $this->dateType == 'datefield' ? 'inline' : 'none';
            $dropdown_display =
                $this->dateType == 'datedropdown' ? 'inline' : 'none';
            $icon_display =
                $this->calendarIconType == 'calendar' ? 'inline' : 'none';
            ...
            $month_dropdown =
                "<div class='gfield_date_dropdown_month ginput_date_dropdown' id='gfield_dropdown_date_month' style='display:$dropdown_display'>" .
                $this->get_month_dropdown(
                    '',
                    "{$field_id}_1",
                    rgar($date_info, 'month'),
                    '',
                    $disabled_text,
                    $month_placeholder_value
                ) .
                '</div>';
            $day_dropdown =
                "<div class='gfield_date_dropdown_day ginput_date_dropdown' id='gfield_dropdown_date_day' style='display:$dropdown_display'>" .
                $this->get_day_dropdown(
                    ...
                    '',
                    $disabled_text,
                    $day_placeholder_value
                ) .
                '</div>';
            $year_dropdown =
                "<div class='gfield_date_dropdown_year ginput_date_dropdown' id='gfield_dropdown_date_year' style='display:$dropdown_display'>" .
                $this->get_year_dropdown(
                    ...
                    '',
                    $disabled_text,
                    $year_placeholder_value,
                    $form
                ) .
                '</div>';

            $field_string =
                "<div class='ginput_container ginput_container_date' id='gfield_input_datepicker' style='display:$datepicker_display'><input name='ginput_datepicker' type='text' {$date_picker_placeholder} {$disabled_text} value='{$picker_value}'/><img src='" .
                GFCommon::get_base_url() .
                "/images/calendar.png' id='gfield_input_datepicker_icon' style='display:$icon_display'/></div>";
            ...
            if (in_array($date_type, ['datefield', 'datedropdown'])) {
                switch ($field_position) {
                    case 'dmy':
                        ...
                        if ($date_type == 'datedropdown') {
                            $field_str =
                                "<div class='clear-multi'><div class='gfield_date_dropdown_day ginput_container ginput_container_date' id='{$field_id}_2_container'>" .
                                $this->get_day_dropdown(
                                    "input_{$id}[]",
                                    "{$field_id}_2",
                                    rgar($date_info, 'day'),
                            ...
                            $field_str .=
                                "<div class='gfield_date_dropdown_month ginput_container ginput_container_date' id='{$field_id}_1_container'>" .
                                $this->get_month_dropdown(
                                    "input_{$id}[]",
                                    "{$field_id}_1",
                                    rgar($date_info, 'month'),
                            ...
                            $field_str .=
                                "<div class='gfield_date_dropdown_year ginput_container ginput_container_date' id='{$field_id}_3_container'>" .
                                $this->get_year_dropdown(
                                    "input_{$id}[]",
                                    "{$field_id}_3",
                                    rgar($date_info, 'year'),
                    ...
                    case 'ymd':
                        ...
                        if ($date_type == 'datedropdown') {
                            $field_str =
                                "<div class='clear-multi'><div class='gfield_date_dropdown_year ginput_container ginput_container_date' id='{$field_id}_3_container'>" .
                                $this->get_year_dropdown(
                                    "input_{$id}[]",
                                    "{$field_id}_3",
                                    rgar($date_info, 'year'),
                            ...
                            $field_str .=
                                "<div class='gfield_date_dropdown_day ginput_container ginput_container_date' id='{$field_id}_2_container'>" .
                                $this->get_day_dropdown(
                                    "input_{$id}[]",
                                    "{$field_id}_2",
                                    rgar($date_info, 'day'),
                    ...
                    default:
                        ...
                        if ($date_type == 'datedropdown') {
                            $field_str =
                                "<div class='clear-multi'><div class='gfield_date_dropdown_month ginput_container ginput_container_date' id='{$field_id}_1_container'>" .
                                $this->get_month_dropdown(
                                    "input_{$id}[]",
                                    "{$field_id}_1",
                                    rgar($date_info, 'month'),
                ...
                $picker_value = esc_attr(
                    GFCommon::date_display($picker_value, $format)
                );
                $icon_class =
                    $this->calendarIconType == 'none'
                        ? 'datepicker_no_icon'
                        : 'datepicker_with_icon';
                $icon_url = empty($this->calendarIconUrl)
                    ? GFCommon::get_base_url() . '/images/calendar.png'
    ...
    {
        return $this->dateType == 'datefield'
    ...
    public function get_date_array_by_format($value)
    {
        ...
        $date = array_combine(['m', 'd', 'y'], $value); // takes our numerical array and converts it to an associative array
    ...
    public function get_value_entry_detail(
        ...
        return GFCommon::date_display(
            $value,
            $this->dateFormat,
            $this->get_output_date_format()
        );
    }
    ...
    private function get_month_dropdown(
        $name = '',
        $id = '',
        $selected_value = '',
        $tabindex = '',
        ...
        return $this->get_number_dropdown(
            $name,
            $id,
            $selected_value,
            $tabindex,
    ...
    private function get_day_dropdown(
        $name = '',
        $id = '',
        $selected_value = '',
        $tabindex = '',
    ...
    private function get_year_dropdown(
        $name = '',
        $id = '',
        $selected_value = '',
        $tabindex = '',
    ...
    private function get_number_dropdown(
        $name,
        $id,
        $selected_value,
        $tabindex,
        ...
        for (
            $i = $start_number;
            $i != $end_number + $increment;
            $i += $increment
        ) {
            $selected =
                intval($i) == intval($selected_value)
                    ? "selected='selected'"
                    : '';
    ...
    public function get_first_input_id($form)
    {
        return in_array($this->dateType, ['datefield', 'datedropdown'])
            ? ''
            : parent::get_first_input_id($form);
    }
    ...
    public function get_filter_settings()
    {
        ...
        $filter_settings['placeholder'] = esc_html__(
            'yyyy-mm-dd',
            'gravityforms'
        );
