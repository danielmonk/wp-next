...
class GF_Field_Consent extends GF_Field
{
    ...
    public function __construct($data = [])
    {
         ...
         */
        $this->checked_indicator_url = apply_filters(
            'gform_consent_checked_indicator',
            GFCommon::get_base_url() . '/images/tick.png'
        );
         ...
         */
        $this->checked_indicator_markup = apply_filters(
            'gform_consent_checked_indicator_markup',
            '<img src="' . esc_url($this->checked_indicator_url) . '" />'
        );
    }
    ...
    public function get_field_input($form, $value = [], $entry = null)
    {
        ...
        $required_div =
            $this->labelPlacement === 'hidden_label' &&
        ...
        if ($is_admin && !GFCommon::is_entry_detail_edit()) {
            $checkbox_label =
                !is_array($value) || empty($value[$id . '.2'])
                    ? $this->checkboxLabel
                    : $value[$id . '.2'];
            $revision_id =
                !is_array($value) || empty($value[$id . '.3'])
                    ? GFFormsModel::get_latest_form_revisions_id($form['id'])
                    : $value[$id . '.3'];
            $value =
                !is_array($value) || empty($value[$id . '.1'])
                    ? '0'
                    : esc_attr($value[$id . '.1']);
        } else {
            ...
            $current_description = $this->get_field_description_from_revision(
                $revision_id
            );
            $submitted_description = $this->get_field_description_from_revision(
                $value[$id . '.3']
            );
        ...
        $aria_describedby = '';
        $description = $is_entry_detail
            ? $this->get_field_description_from_revision($revision_id)
            : $this->description;
        ...
        $input .=
            "<input type='hidden' name='input_{$id}.2' value='" .
            esc_attr($checkbox_label) .
            "' class='gform_hidden' />";
        $input .=
            "<input type='hidden' name='input_{$id}.3' value='" .
            esc_attr($revision_id) .
            "' class='gform_hidden' />";

        if ($is_entry_detail) {
            $input .= $this->get_description(
                $this->get_field_description_from_revision($revision_id),
                ''
            );
        }
        ...
        return sprintf(
            "<div class='ginput_container ginput_container_consent'>%s</div>",
            $input
        );
    }
    ...
    public function get_description($description, $css_class)
    {
        if (!empty($description)) {
            ...
            return "<div class='$css_class' id='$id'>" .
                nl2br($description) .
                '</div>';
        }
    ...
    public function get_value_merge_tag(
        ...
        switch ($input_id) {
            case '1':
                $value = !rgblank($value)
                    ? $this->checked_indicator_markup
                    : '';
                break;
            case '3':
                $value = !rgblank($value)
                    ? $this->get_field_description_from_revision($value)
                    : '';
                if ($value !== '' && $nl2br) {
                    $value = nl2br($value);
                }
    ...
    public function get_value_entry_detail(
        ...
        if (is_array($value) && !empty($value)) {
            ...
            if (!rgblank($consent)) {
                ...
                // checking revisions.
                $description = $this->get_field_description_from_revision(
                    $revision_id
                );

                if (!empty($description)) {
                    $return .=
                        '<br /><div class="gfield_consent_description">' .
                        nl2br($description) .
                        '</div>';
                }
            }
        ...
        switch ($input_id) {
            case '1':
                $value = !rgblank($value)
                    ...
                    : esc_html__('Not Checked', 'gravityforms');
                break;
            case '3':
                $value = !rgblank($value)
                    ? $this->get_field_description_from_revision($value)
    ...
    public function get_field_description_from_revision($revision_id)
    {
        ...
        $display_meta = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT display_meta FROM $revisions_table_name WHERE form_id=%d AND id=%d",
                $this->formId,
                $revision_id
            )
        );
