...
class GF_Field_Time extends GF_Field
{
    ...
    public function validate($value, $form)
    {
        ...
        if (
            ...
            $this->validation_message = empty($this->errorMessage)
                ? esc_html__('Please enter a valid time.', 'gravityforms')
                : $this->errorMessage;
        }
    }
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        if (!is_array($value) && !empty($value)) {
            ...
            $am_selected =
                strpos($the_rest, 'am') > -1 ? "selected='selected'" : '';
            $pm_selected =
                strpos($the_rest, 'pm') > -1 ? "selected='selected'" : '';
        } elseif (is_array($value)) {
            $value = array_values($value);
            $hour = esc_attr($value[0]);
            $minute = esc_attr($value[1]);
            $am_selected =
                strtolower(rgar($value, 2)) == 'am'
                    ? "selected='selected'"
                    : '';
            $pm_selected =
                strtolower(rgar($value, 2)) == 'pm'
                    ? "selected='selected'"
                    : '';
        }
        ...
        $hour_placeholder_attribute = $this->get_input_placeholder_attribute(
            $hour_input
        );
        $minute_placeholder_attribute = $this->get_input_placeholder_attribute(
            $minute_input
        );
        ...
        $max_hour = $this->timeFormat == '24' ? 24 : 12;
        $hour_html5_attributes = $is_html5
            ? "min='0' max='{$max_hour}' step='1'"
            : '';
        $minute_html5_attributes = $is_html5 ? "min='0' max='59' step='1'" : '';

        $ampm_field_style =
            $is_form_editor && $this->timeFormat == '24'
                ? "style='display:none;'"
                : '';
        ...
        $hour_label =
            rgar($hour_input, 'customLabel') != ''
                ? $hour_input['customLabel']
                : esc_html__('HH', 'gravityforms');
        $minute_label =
            rgar($minute_input, 'customLabel') != ''
                ? $minute_input['customLabel']
                : esc_html(_x('MM', 'Abbreviation: Minutes', 'gravityforms'));
    ...
    public function sanitize_settings()
    {
        ...
        if (!$this->timeFormat || !in_array($this->timeFormat, [12, 24])) {
            $this->timeFormat = '12';
        }
    }
}
