...
class GF_Field_Phone extends GF_Field
{
    ...
    public function validate($value, $form)
    {
        ...
        if (
            rgar($phone_format, 'regex') &&
            $value !== '' &&
            $value !== 0 &&
            !preg_match($phone_format['regex'], $value)
        ) {
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        if ($this->failed_validation) {
            ...
            if (rgar($phone_format, 'instruction')) {
                $instruction_div = sprintf(
                    "<div class='instruction validation_message'>%s %s</div>",
                    esc_html__('Phone format:', 'gravityforms'),
                    $phone_format['instruction']
                );
            }
        }
        ...
        return sprintf(
            "<div class='ginput_container ginput_container_phone'><input name='input_%d' id='%s' type='{$html_input_type}' value='%s' class='%s' {$tabindex} {$placeholder_attribute} {$required_attribute} {$invalid_attribute} {$aria_describedby} %s/>{$instruction_div}</div>",
            $id,
            $field_id,
            esc_attr($value),
            esc_attr($class),
