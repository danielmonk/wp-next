...
class GF_Field_Checkbox extends GF_Field
{
    ...
    public function get_value_submission(
        ...
        $parameter_values = GFFormsModel::get_parameter_value(
            $this->inputName,
            $field_values,
            $this
        );
        ...
        foreach ($this->inputs as $input) {
            if (
                !empty($_POST['is_submit_' . $this->formId]) &&
                ...
                if (is_array($parameter_values)) {
                    foreach ($parameter_values as $item) {
                        ...
                        if (
                            GFFormsModel::choice_value_match(
                                $this,
                                $this->choices[$choice_index],
                                $item
                            )
                        ) {
                            $value[$input['id'] . ''] = $item;
        ...
        if (absint($field_id) == $field_id) {
            ...
            foreach ($lead_field_keys as $input_id) {
                if (is_numeric($input_id) && absint($input_id) == $field_id) {
                    $items[] = GFCommon::selection_display(
                        rgar($entry, $input_id),
                        null,
                        $entry['currency'],
                        false
    ...
    public function get_value_merge_tag(
        ...
        $use_price = in_array($modifier, ['price', 'currency']);
        ...
        foreach ($items as $input_id => $item) {
            ...
            if ($use_value) {
                ...
                $val = GFFormsModel::is_field_hidden($form, $this, [], $entry)
                    ? ''
                    : $item_value;
        ...
        if (empty($input_id) || absint($input_id) == $input_id) {
            ...
            foreach ($this->inputs as $input) {
                ...
                if (!rgempty($index, $entry)) {
                    $selected[] = GFCommon::selection_display(
                        rgar($entry, $index),
                        $this,
                        rgar($entry, 'currency'),
                        $use_text
            ...
            $value = $this->is_checkbox_checked(
                $input_id,
                GFCommon::get_label($this, $input_id),
                $entry
            );
    ...
    public function get_checkbox_choices($value, $disabled_text, $form_id = 0)
    {
        ...
        if (is_array($this->choices)) {
            ...
            if ($this->enableSelectAll) {
                 ...
                 */
                $select_label = gf_apply_filters(
                    [
                        'gform_checkbox_select_all_label',
                        $this->formId,
                        $this->id,
                    ],
                    esc_html__('Select All', 'gravityforms'),
                    $this
                );
                 ...
                 * @param object $field          The field currently being processed.
                 */
                $deselect_label = gf_apply_filters(
                    [
                        'gform_checkbox_deselect_all_label',
                        $this->formId,
                        $this->id,
                    ],
                    esc_html__('Deselect All', 'gravityforms'),
                    $this
                );
                ...
                $choice_markup =
                    "<li class='gchoice_select_all'>
						...
						<label for='{$id}' id='label_" .
                    $this->id .
                    "_select_all' data-label-select='{$select_label}' data-label-deselect='{$deselect_label}'>{$select_label}</label>
					</li>";
            ...
            foreach ($this->choices as $choice) {
                ...
                if (
                    ($is_form_editor ||
                        (!isset($_GET['gf_token']) && empty($_POST))) &&
                    ...
                    GFFormsModel::choice_value_match(
                        $this,
                        $choice,
                        rgget($input_id, $value)
                    )
                ) {
                    $checked = "checked='checked'";
                } elseif (
                    !is_array($value) &&
                    GFFormsModel::choice_value_match($this, $choice, $value)
                ) {
                    $checked = "checked='checked'";
                } else {
                    $checked = '';
        ...
        return gf_apply_filters(
            ['gform_field_choices', $this->formId, $this->id],
            $choices,
            $this
        );
    }
