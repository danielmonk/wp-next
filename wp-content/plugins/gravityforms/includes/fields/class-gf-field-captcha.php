...
class GF_Field_CAPTCHA extends GF_Field
{
    ...
    public function validate($value, $form)
    {
        switch ($this->captchaType) {
            ...
            case 'math':
                $prefixes = explode(
                    ',',
                    $_POST["input_captcha_prefix_{$this->id}"]
                );
                $captcha_obj = $this->get_simple_captcha();
    ...
    public function verify_recaptcha_response($response, $secret_key = null)
    {
        ...
        if (!is_wp_error($response)) {
            ...
            GFCommon::log_debug(
                __METHOD__ .
                    '(): Validating the reCAPTCHA response has failed due to the following: ' .
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        switch ($this->captchaType) {
            case 'simple_captcha':
                $size = empty($this->simpleCaptchaSize)
                    ? 'medium'
                    : esc_attr($this->simpleCaptchaSize);
                $captcha = $this->get_captcha();

                $tabindex = $this->get_tabindex();

                $dimensions =
                    $is_entry_detail || $is_form_editor
                        ? ''
                        : "width='" .
                            esc_attr(rgar($captcha, 'width')) .
                            "' height='" .
                            esc_attr(rgar($captcha, 'height')) .
                            "'";

                return "<div class='gfield_captcha_container'><img class='gfield_captcha' src='" .
                    esc_url(rgar($captcha, 'url')) .
                    "' alt='' {$dimensions} /><div class='gfield_captcha_input_container simple_captcha_{$size}'><input type='text' name='input_{$id}' id='{$field_id}' {$tabindex}/><input type='hidden' name='input_captcha_prefix_{$id}' value='" .
                    esc_attr(rgar($captcha, 'prefix')) .
                    "' /></div></div>";
                break;
            ...
            case 'math':
                ...
                $dimensions =
                    $is_entry_detail || $is_form_editor
                        ? ''
                        : "width='" .
                            esc_attr(rgar($captcha_1, 'width')) .
                            "' height='" .
                            esc_attr(rgar($captcha_1, 'height')) .
                            "'";
                $prefix_value =
                    rgar($captcha_1, 'prefix') .
                    ',' .
                    rgar($captcha_2, 'prefix') .
                    ',' .
                    rgar($captcha_3, 'prefix');

                return "<div class='gfield_captcha_container'><img class='gfield_captcha' src='" .
                    esc_url(rgar($captcha_1, 'url')) .
                    "' alt='' {$dimensions} /><img class='gfield_captcha' src='" .
                    esc_url(rgar($captcha_2, 'url')) .
                    "' alt='' {$dimensions} /><img class='gfield_captcha' src='" .
                    esc_url(rgar($captcha_3, 'url')) .
                    "' alt='' {$dimensions} /><div class='gfield_captcha_input_container math_{$size}'><input type='text' name='input_{$id}' id='{$field_id}' {$tabindex}/><input type='hidden' name='input_captcha_prefix_{$id}' value='" .
                    esc_attr($prefix_value) .
                    "' /></div></div>";
                break;
            ...
            default:
                ...
                $secret_key = get_option('rg_gforms_captcha_private_key');
                $theme = in_array($this->captchaTheme, ['blackglass', 'dark'])
                    ? 'dark'
                    : 'light';
                ...
                if ($is_entry_detail || $is_form_editor) {
                    ...
                    if (empty($site_key) || empty($secret_key)) {
                        return "<div class='captcha_message'>" .
                            __(
                                'To use the reCAPTCHA field you must do the following:',
                                'gravityforms'
                            ) .
                            "</div><div class='captcha_message'>1 - <a href='https://www.google.com/recaptcha/admin' target='_blank'>" .
                            sprintf(
                                __(
                                    'Sign up%s for an API key pair for your site.',
                                    'gravityforms'
                                ),
                                '</a>'
                            ) .
                            "</div><div class='captcha_message'>2 - " .
                            sprintf(
                                __(
                                    'Enter your reCAPTCHA site and secret keys in the reCAPTCHA Settings section of the %sSettings page%s',
                                    'gravityforms'
                                ),
                                "<a href='?page=gf_settings' target='_blank'>",
                                '</a>'
                            ) .
                            '</div>';
                    } else {
                        ...
                        return "<div class='ginput_container'><img class='gfield_captcha' src='" .
                            GFCommon::get_base_url() .
                            "/images/captcha_{$type_suffix}{$theme}.jpg' alt='{$alt}' /></div>";
                    }
                } else {
                    $language = empty($this->captchaLanguage)
                        ? 'en'
                        : $this->captchaLanguage;
                    ...
                    add_action('wp_footer', [$this, 'ensure_recaptcha_js'], 21);
                    add_action('gform_preview_footer', [
                        $this,
                        'ensure_recaptcha_js',
                    ]);
                    ...
                    if ($this->use_stoken()) {
                        ...
                        $secure_token = self::create_recaptcha_secure_token(
                            $secret_key
                        );
                        $stoken = sprintf(
                            'data-stoken=\'%s\'',
                            esc_attr($secure_token)
                        );
                    }
                    ...
                    if ($type == 'invisible') {
                        ...
                        $badge = $this->captchaBadge
                            ? $this->captchaBadge
                            : 'bottomright';
                        $tabindex = -1;
                    } else {
                        $tabindex =
                            GFCommon::$tab_index > 0
                                ? GFCommon::$tab_index++
                                : 0;
                    }
                    ...
                    $output =
                        "<div id='" .
                        esc_attr($field_id) .
                        "' class='ginput_container ginput_recaptcha' data-sitekey='" .
                        esc_attr($site_key) .
                        "' {$stoken} data-theme='" .
                        esc_attr($theme) .
                        "' data-tabindex='{$tabindex}' {$size} data-badge='{$badge}'></div>";

                    $recaptcha_response = sanitize_text_field(
                        rgpost('g-recaptcha-response')
                    );
                    $current_page = GFFormDisplay::get_current_page(
                        $form['id']
                    );
                    ...
                    if (
                        ...
                        $hash = sanitize_text_field(
                            rgpost('gf-recaptcha-response-hash')
                        );
                        if (!$hash) {
    ...
    public function get_captcha()
    {
        ...
        return [
            'path' => $path,
            'url' => $url,
            'height' => $captcha->img_size[1],
            'width' => $captcha->img_size[0],
            'prefix' => $prefix,
        ];
    }
    ...
    public function create_recaptcha_secure_token($secret_key)
    {
        ...
        //create json string
        $params = ['session_id' => $session_id, 'ts_ms' => $ts_ms];
