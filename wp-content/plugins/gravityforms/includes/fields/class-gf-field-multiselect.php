...
class GF_Field_MultiSelect extends GF_Field
{
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
         ...
         */
        $placeholder = gf_apply_filters(
            ['gform_multiselect_placeholder', $form_id, $this->id],
            __('Click to select...', 'gravityforms'),
            $form_id,
            $this
        );
        $placeholder = $this->enableEnhancedUI
        ...
        return sprintf(
            "<div class='ginput_container ginput_container_multiselect'><select multiple='multiple' {$placeholder} size='{$size}' name='input_%d[]' id='%s' class='%s' $tabindex %s>%s</select></div>",
            $id,
            esc_attr($field_id),
            $css_class,
            $disabled_text,
    ...
    public function get_value_entry_detail(
        ...
        if (
            empty($value) ||
            ($format == 'text' && $this->storageType !== 'json')
        ) {
            return $value;
        }
        ...
        return "<ul class='bulleted'><li>" .
            GFCommon::implode_non_blank('</li><li>', $items) .
            '</li></ul>';
    }
        ...
        if ($this->type == 'post_category') {
            if (is_array($items)) {
                ...
                foreach ($items as &$item) {
                    ...
                    $item = GFCommon::format_variable_value(
                        $cat,
                        $url_encode,
                        $esc_html,
                        $format
            ...
            foreach ($items as &$item) {
                $item = GFCommon::selection_display(
                    $item,
                    $this,
                    rgar($entry, 'currency'),
                    true
    ...
    public function sanitize_settings()
    {
        ...
        $this->storageType =
            empty($this->storageType) || $this->storageType === 'json'
                ? $this->storageType
                : 'json';
