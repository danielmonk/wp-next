...
class GF_Field_FileUpload extends GF_Field
{
    ...
    public function validate($value, $form)
    {
        ...
        $allowed_extensions = !empty($this->allowedExtensions)
            ? GFCommon::clean_extensions(
                explode(',', strtolower($this->allowedExtensions))
            )
            : [];
        if ($this->multipleFiles) {
            $file_names = isset(
                GFFormsModel::$uploaded_files[$form['id']][$input_name]
            )
                ? GFFormsModel::$uploaded_files[$form['id']][$input_name]
                : [];
        } else {
            $max_upload_size_in_bytes =
                isset($this->maxFileSize) && $this->maxFileSize > 0
                ...
                if (empty($uploaded_file_name)) {
                    ...
                    switch ($_FILES[$input_name]['error']) {
                        ...
                        case UPLOAD_ERR_FORM_SIZE:
                            GFCommon::log_debug(
                                __METHOD__ .
                                    ...
                                    ' exceeds size limit. Maximum file size: ' .
                                    $max_upload_size_in_mb .
                                    'MB'
                            );
                            $fileupload_validation_message = sprintf(
                                esc_html__(
                                    'File exceeds size limit. Maximum file size: %dMB',
                                    'gravityforms'
                                ),
                                $max_upload_size_in_mb
                            );
                        ...
                        default:
                            GFCommon::log_debug(
                                __METHOD__ .
                                    '(): The following error occurred while uploading - ' .
                                    $_FILES[$input_name]['error']
                            );
                            $fileupload_validation_message = sprintf(
                                esc_html__(
                                    'There was an error while uploading the file. Error code: %d',
                                    'gravityforms'
                                ),
                                $_FILES[$input_name]['error']
                            );
                    ...
                    $this->validation_message = empty($this->errorMessage)
                        ? $fileupload_validation_message
                        : $this->errorMessage;
                    return;
                }
            } elseif (
                $_FILES[$input_name]['size'] > 0 &&
                $_FILES[$input_name]['size'] > $max_upload_size_in_bytes
            ) {
                $this->failed_validation = true;
                GFCommon::log_debug(
                    __METHOD__ .
            ...
            $file_names = [['uploaded_filename' => $single_file_name]];
        }

        foreach ($file_names as $file_name) {
            GFCommon::log_debug(
                __METHOD__ .
                    '(): Validating file upload for ' .
            ...
            if (empty($allowed_extensions)) {
                if (
                    GFCommon::file_name_has_disallowed_extension(
                        rgar($file_name, 'uploaded_filename')
                    )
                ) {
                    GFCommon::log_debug(
                        __METHOD__ .
                            '(): The file has a disallowed extension, failing validation.'
                    );
                    $this->failed_validation = true;
                    $this->validation_message = empty($this->errorMessage)
                        ? esc_html__(
                ...
                if (
                    ...
                    GFCommon::log_debug(
                        __METHOD__ .
                            '(): The file is of a type that cannot be uploaded, failing validation.'
                    );
                    $this->failed_validation = true;
                    $this->validation_message = empty($this->errorMessage)
                        ? sprintf(
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        $allowed_extensions = !empty($this->allowedExtensions)
            ...
            : [];
        if (!empty($allowed_extensions)) {
            $extensions_message = esc_attr(
                sprintf(
                    __('Accepted file types: %s.', 'gravityforms'),
        ...
        if ($multiple_files) {
            ...
            if (
                defined('DOING_AJAX') &&
                DOING_AJAX &&
                'rg_change_input_type' === rgpost('action')
            ) {
                $plupload_init = [];
            } else {
                $plupload_init = [
                    ...
                    'unique_names' => true,
                    'file_data_name' => 'file', // chunking doesn't currently have very good cross-browser support
                    /*'chunk_size' => '10mb',*/ 'url' => $upload_action_url,
                ...
                if (rgar($form, 'requireLogin')) {
                    $plupload_init['multipart_params'][
                        '_gform_file_upload_nonce_' . $form_id
                    ] = wp_create_nonce(
                        'gform_file_upload_' . $form_id,
                        '_gform_file_upload_nonce_' . $form_id
                    );
                }
            ...
            $plupload_init = gf_apply_filters(
                ['gform_plupload_settings', $form_id],
                $plupload_init,
                $form_id,
                $this
            );

            $drop_files_here_text = esc_html__(
                'Drop files here or',
                'gravityforms'
            );
            $select_files_text = esc_attr__('Select files', 'gravityforms');

            $plupload_init_json = htmlspecialchars(
                json_encode($plupload_init),
                ENT_QUOTES,
                'UTF-8'
            );
            ...
            if ($is_entry_detail) {
                $upload .= sprintf(
                    '<input type="hidden" name="input_%d" value=\'%s\' />',
                    $id,
                    esc_attr($value)
                );
            }
            ...
            $validation_message_id =
                'validation_message_' . $form_id . '_' . $id;
            $live_validation_message_id =
                'live_validation_message_' . $form_id . '_' . $id;
            $upload .= sprintf(
                "<input name='input_%d' id='%s' type='file' class='%s' aria-describedby='%s %s %s' onchange='javascript:gformValidateFileSize( this, %s );' {$tabindex} %s/>",
                $id,
                $field_id,
                esc_attr($class),
                $validation_message_id,
        ...
        if ($is_entry_detail && !empty($value)) {
            // edit entry
            $file_urls = $multiple_files ? json_decode($value) : [$value];
            ...
            foreach ($file_urls as $file_index => $file_url) {
                ...
                if (
                    GFCommon::is_ssl() &&
                    strpos($file_url, 'http:') !== false &&
                    $field_ssl === true
                ) {
                    $file_url = str_replace('http:', 'https:', $file_url);
                }
                $download_file_text = esc_attr__(
                    'Download file',
                    'gravityforms'
                );
                ...
                $download_button_url =
                    GFCommon::get_base_url() . '/images/download.png';
                $delete_button_url =
                    GFCommon::get_base_url() . '/images/delete.png';
            ...
            $uploaded_files = isset(
                GFFormsModel::$uploaded_files[$form_id][$input_name]
            )
                ? GFFormsModel::$uploaded_files[$form_id][$input_name]
                : [];
            $file_infos = $multiple_files
                ? $uploaded_files
                : RGFormsModel::get_temp_filename($form_id, $input_name);

            if (!empty($file_infos)) {
                $preview = sprintf("<div id='%s'>", $file_list_id);
                $file_infos = $multiple_files ? $uploaded_files : [$file_infos];
                foreach ($file_infos as $file_info) {
                    $file_upload_markup = apply_filters(
                        'gform_file_upload_markup',
                        "<img alt='" .
                            esc_attr__('Delete file', 'gravityforms') .
                            "' class='gform_delete' src='" .
                            GFCommon::get_base_url() .
                            "/images/delete.png' onclick='gformDeleteUploadedFile({$form_id}, {$id}, this);' onkeypress='gformDeleteUploadedFile({$form_id}, {$id}, this);' /> <strong>" .
                ...
                return "<div class='ginput_container ginput_container_fileupload'>" .
                    $upload .
                    " {$preview}</div>";
            } else {
                $preview = $multiple_files
                    ? sprintf("<div id='%s'></div>", $file_list_id)
                    : '';

                return "<div class='ginput_container ginput_container_fileupload'>$upload</div>" .
                    $preview;
            }
        }
    }
        ...
        if (isset($_gf_uploaded_files[$input_name])) {
            ...
            if (isset(GFFormsModel::$uploaded_files[$form_id][$input_name])) {
                ...
                foreach ($uploaded_temp_files as $i => $file_info) {
                    $temp_filepath =
                        GFFormsModel::get_upload_path($form_id) .
                        '/tmp/' .
                        wp_basename($file_info['temp_filename']);
                    if ($file_info && file_exists($temp_filepath)) {
                        $uploaded_files[$i] = $this->move_temp_file(
                            $form_id,
                            $file_info
                        );
                    }
                ...
                GFCommon::log_debug(
                    __METHOD__ . '(): No files uploaded. Exiting.'
                );
    ...
    public function get_single_file_value($form_id, $input_name)
    {
        ...
        if (!isset($_gf_uploaded_files[$input_name])) {
            ...
            if ($file_info && file_exists($temp_filepath)) {
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): File already uploaded to tmp folder, moving.'
                );
                $_gf_uploaded_files[$input_name] = $this->move_temp_file(
                    $form_id,
                    $file_info
                );
            } elseif (!empty($_FILES[$input_name]['name'])) {
                GFCommon::log_debug(__METHOD__ . '(): calling upload_file');
                $_gf_uploaded_files[$input_name] = $this->upload_file(
                    $form_id,
                    $_FILES[$input_name]
                );
            } else {
                GFCommon::log_debug(
                    __METHOD__ . '(): No file uploaded. Exiting.'
                );
            }
        }
    ...
    {
        GFCommon::log_debug(
            __METHOD__ . '(): Uploading file: ' . $file['name']
        );

        $target = GFFormsModel::get_file_upload_path($form_id, $file['name']);
        if (!$target) {
            GFCommon::log_debug(
                __METHOD__ . '(): FAILED (Upload folder could not be created.)'
            );

            return 'FAILED (Upload folder could not be created.)';
        }
        GFCommon::log_debug(
            __METHOD__ . '(): Upload folder is ' . print_r($target, true)
        );
    ...
    public function get_value_entry_list(
        ...
        if ($this->multipleFiles) {
            $uploaded_files_arr = empty($value)
                ? []
                : json_decode($value, true);
            $file_count = count($uploaded_files_arr);
            if ($file_count > 1) {
                $value = empty($uploaded_files_arr)
                    ? ''
                    : sprintf(
                        esc_html__('%d files', 'gravityforms'),
                        count($uploaded_files_arr)
                    );
                return $value;
            } elseif ($file_count == 1) {
        ...
        if (!empty($file_path)) {
            ...
            $value =
                "<a href='$file_path' target='_blank' aria-label='" .
                esc_attr__('View the image', 'gravityforms') .
                "'><img src='$thumb' alt='' /></a>";
        }
        return $value;
    }
    ...
    public function get_value_entry_detail(
        ...
        $output =
            empty($output) || $format == 'text'
                ? $output
                : sprintf('<ul>%s</ul>', $output);
    ...
    public function get_value_merge_tag(
        ...
        if ($this->multipleFiles) {
            ...
            $value =
                $format == 'html' ? join('<br />', $files) : join(', ', $files);
        } else {
    ...
    {
        $target = GFFormsModel::get_file_upload_path(
            $form_id,
        ...
        $source =
            GFFormsModel::get_upload_path($form_id) .
            '/tmp/' .
            wp_basename($tempfile_info['temp_filename']);

        GFCommon::log_debug(
            __METHOD__ . '(): Moving temp file from: ' . $source
        );
        ...
        if (rename($source, $target['path'])) {
            ...
            GFCommon::log_debug(
                __METHOD__ . '(): FAILED (Temporary file could not be moved.)'
            );
    ...
    {
        GFCommon::log_debug(
            __METHOD__ . '(): Setting permissions on: ' . $path
        );
    ...
    public function sanitize_settings()
    {
        ...
        $this->allowedExtensions = sanitize_text_field(
    ...
    public function get_download_url($file, $force_download = false)
    {
         ...
         */
        $secure_download_location = apply_filters(
        ...
        $secure_download_location = apply_filters(
            'gform_secure_file_download_location_' . $this->formId,
            $secure_download_location,
            $file,
            $this
        );
        ...
        if (strpos($file, $upload_root) !== false) {
            ...
            $args = [
                ...
                'form-id' => $this->formId,
                'field-id' => $this->id,
                'hash' => GFCommon::generate_download_hash(
                    $this->formId,
                    $this->id,
                    $file
                ),
