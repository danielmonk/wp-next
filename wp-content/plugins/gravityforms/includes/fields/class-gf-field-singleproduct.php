...
class GF_Field_SingleProduct extends GF_Field
{
    ...
    public function validate($value, $form)
    {
        ...
        if (
            ...
            !empty($quantity) &&
            (!is_numeric($quantity) ||
                intval($quantity) != floatval($quantity) ||
                intval($quantity) < 0)
        ) {
            $this->failed_validation = true;
    ...
    public function get_value_default()
    {
        ...
        if (is_array($this->inputs)) {
            foreach ($this->inputs as $index => $input) {
                $input_value = $this->is_form_editor()
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        $product_name =
            !is_array($value) || empty($value[$this->id . '.1'])
                ? esc_attr($this->label)
                : esc_attr($value[$this->id . '.1']);
        $price =
            !is_array($value) || empty($value[$this->id . '.2'])
                ? $this->basePrice
                : esc_attr($value[$this->id . '.2']);
        ...
        $has_quantity =
            sizeof(
                GFCommon::get_product_fields_by_type(
                    $form,
                    ['quantity'],
                    $this->id
                )
        ...
        $currency =
            $is_entry_detail && !empty($entry) ? $entry['currency'] : '';
        ...
        $product_quantity_sub_label = gf_apply_filters(
            ['gform_product_quantity', $form_id, $this->id],
        ...
        if ($is_entry_detail || $is_form_editor) {
            ...
            $quantity_field .=
                " <span class='ginput_quantity_label'>" .
                $product_quantity_sub_label .
                "</span> <input type='{$qty_input_type}' name='input_{$id}.3' value='{$quantity}' id='ginput_quantity_{$form_id}_{$this->id}' class='ginput_quantity' size='10' {$qty_min_attr} {$tabindex} {$disabled_text}/>";
        } else {
        ...
        return "<div class='ginput_container ginput_container_singleproduct'>
					...
					<span class='ginput_product_price_label'>" .
            gf_apply_filters(
                ['gform_product_price', $form_id, $this->id],
                esc_html__('Price', 'gravityforms'),
                $form_id
            ) .
            ":</span> <span class='ginput_product_price' id='{$field_id}'>" .
            esc_html(GFCommon::to_money($price, $currency)) .
            "</span>
					<input type='hidden' name='input_{$id}.2' id='ginput_base_price_{$form_id}_{$this->id}' class='gform_hidden' value='" .
            esc_attr($price) .
            "'/>
					{$quantity_field}
				</div>";
    ...
    public function get_value_entry_detail(
        ...
        if (is_array($value) && !empty($value)) {
            ...
            if (!rgblank($quantity)) {
                $product_details .=
                    ', ' . esc_html__('Qty: ', 'gravityforms') . $quantity;
            }
            ...
            if (!rgblank($price)) {
                $product_details .=
                    ', ' .
                    esc_html__('Price: ', 'gravityforms') .
                    GFCommon::format_number($price, 'currency', $currency);
            }
