...
class GF_Field_List extends GF_Field
{
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        $columns = $has_columns ? $this->choices : [[]];
        ...
        if ($has_columns) {
            ...
            for ($colnum = 1; $colnum <= count($columns) + 1; $colnum++) {
                ...
                $list .= sprintf(
                    "<col id='gfield_list_%d_col_%d' class='gfield_list_col_%s' />",
                    $this->id,
                    $colnum,
                    $odd_even
                );
            ...
            foreach ($columns as $column) {
                // a11y: scope="col"
                $list .=
                    '<th scope="col">' . esc_html($column['text']) . '</th>';
            }
        ...
        $delete_display =
            count($value) == 1 ? 'style="visibility:hidden;"' : '';
        $maxRow = intval($this->maxRows);
        $disabled_icon_class =
            !empty($maxRow) && count($value) >= $maxRow
                ? 'gfield_icon_disabled'
                : '';

        $add_icon = !empty($this->addIconUrl)
            ? $this->addIconUrl
            : GFCommon::get_base_url() . '/images/list-add.svg';
        $delete_icon = !empty($this->deleteIconUrl)
            ? $this->deleteIconUrl
            : GFCommon::get_base_url() . '/images/list-remove.svg';

        $add_events = $is_form_editor
            ? ''
            : "onclick='gformAddListItem(this, {$maxRow})' onkeypress='gformAddListItem(this, {$maxRow})'";
        $delete_events = $is_form_editor
            ? ''
            : "onclick='gformDeleteListItem(this, {$maxRow})' onkeypress='gformDeleteListItem(this, {$maxRow})'";
        ...
        foreach ($value as $item) {
            ...
            foreach ($columns as $column) {
                ...
                if (is_array($item)) {
                    if ($has_columns) {
                        ...
                        $data_label =
                            "data-label='" . esc_attr($column['text']) . "'";
                    } else {
                ...
                $list .=
                    "<td class='gfield_list_cell gfield_list_{$this->id}_cell{$colnum}' {$data_label}>" .
                    $this->get_list_input(
                        $has_columns,
                        $column,
                        $val,
            ...
            if ($this->maxRows != 1) {
                ...
                $list .=
                    "   <a href='javascript:void(0);' class='add_list_item {$disabled_icon_class}' aria-label='" .
                    esc_attr__('Add another row', 'gravityforms') .
                    "' {$add_events}><img src='{$add_icon}' alt='' title='" .
                    esc_attr__('Add a new row', 'gravityforms') .
                    "' /></a>" .
                    "   <a href='javascript:void(0);' class='delete_list_item' aria-label='" .
                    esc_attr__('Remove this row', 'gravityforms') .
                    "' {$delete_events} {$delete_display}><img src='{$delete_icon}' alt='' title='" .
                    esc_attr__('Remove this row', 'gravityforms') .
                    "' /></a>";
                $list .= '</td>';
            }
    ...
    public function get_list_input($has_columns, $column, $value, $form_id)
    {
         ...
         * @param int    $form_id        The form ID.
         */
        $input_info = gf_apply_filters(
            ['gform_column_input', $form_id, $this->id, $column_index],
            $input_info,
            $this,
            $column_text,
            $value,
        ...
        switch ($input_info['type']) {
            case 'select':
                ...
                if (!is_array($input_info['choices'])) {
                    $input_info['choices'] = array_map(
                        'trim',
                        explode(',', $input_info['choices'])
                    );
                }
                ...
                foreach ($input_info['choices'] as $choice) {
                    if (is_array($choice)) {
                        ...
                        $choice_selected = array_key_exists(
                            'isSelected',
                            $choice
                        )
                            ? $choice['isSelected']
            ...
            default:
                ...
                $input =
                    "<input aria-label='" .
                    esc_attr($aria_label) .
                    "' type='text' name='input_{$this->id}[]' value='" .
                    esc_attr($value) .
                    "' {$tabindex} {$disabled}/>";
                break;
        }
        ...
        return gf_apply_filters(
            ['gform_column_input_content', $form_id, $this->id, $column_index],
            $input,
            $input_info,
            $this,
            rgar($column, 'text'),
    ...
    public function get_value_merge_tag(
        ...
        $allowed_modifiers = ['text', 'html', 'url'];

        if (
            $found_modifiers = array_intersect($modifiers, $allowed_modifiers)
        ) {
