...
class GF_Field_Textarea extends GF_Field
{
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        $maxlength_attribute = is_numeric($this->maxLength)
        ...
        if (
            !$is_admin &&
            $this->is_rich_edit_enabled() &&
            (!$current_screen ||
                ($current_screen && !rgobj($current_screen, 'is_block_editor')))
        ) {
            ...
            add_filter('mce_buttons_2', [$this, 'filter_mce_buttons'], 10, 2);
            add_filter('mce_buttons_3', [$this, 'filter_mce_buttons'], 10, 2);
            add_filter('mce_buttons_4', [$this, 'filter_mce_buttons'], 10, 2);
            ...
            $editor_settings = apply_filters(
                ...
                [
                    ...
                    'editor_class' => $class,
                    'editor_height' => rgar(
                        ['small' => 110, 'medium' => 180, 'large' => 280],
            ...
            $editor_settings = apply_filters(
                sprintf('gform_rich_text_editor_options_%d', $form['id']),
                $editor_settings,
                $this,
                $form,
                $entry
            ...
            $editor_settings = apply_filters(
                sprintf(
                    'gform_rich_text_editor_options_%d_%d',
                    $form['id'],
                    $this->id
                ),
                $editor_settings,
            ...
            if (
                !has_action('wp_tiny_mce_init', [
                    __CLASS__,
                    'start_wp_tiny_mce_init_buffer',
                ])
            ) {
                add_action('wp_tiny_mce_init', [
                    __CLASS__,
                    'start_wp_tiny_mce_init_buffer',
                ]);
            }
            ...
            remove_filter('mce_buttons_2', [$this, 'filter_mce_buttons'], 10);
            remove_filter('mce_buttons_3', [$this, 'filter_mce_buttons'], 10);
            remove_filter('mce_buttons_4', [$this, 'filter_mce_buttons'], 10);
        } else {
            $input = '';
            $input_style = '';
            ...
            if ($this->is_form_editor()) {
                ...
                $input_style = $this->useRichTextEditor
                    ? 'style="display:none;"'
                    : '';
                $size = $this->size ? $this->size : 'medium';
                $input = sprintf(
                    '<div id="%s_rte_preview" class="gform-rte-preview %s" style="display:%s"></div>',
                    $field_id,
                    $size,
                    $display
                );
        ...
        return sprintf(
            "<div class='ginput_container ginput_container_textarea'>%s</div>",
            $input
        );
    }
    ...
    public static function start_wp_tiny_mce_init_buffer()
    {
        ...
        add_action(
            'after_wp_tiny_mce',
            [__CLASS__, 'end_wp_tiny_mce_init_buffer'],
            1
        );
    }
    ...
    public function filter_mce_buttons($mce_buttons, $editor_id)
    {
         ...
         * Additional filters for specific form and fields IDs.
         */
        $mce_buttons = gf_apply_filters(
            [
                'gform_rich_text_editor_buttons' . $mce_filter,
                $this->formId,
                $this->id,
            ],
            $mce_buttons,
    ...
    public function is_rich_edit_enabled()
    {
        ...
        add_filter('get_user_option_rich_editing', [
            $this,
            'filter_user_option_rich_editing',
        ]);
        $user_can_rich_edit = user_can_richedit();
        remove_filter('get_user_option_rich_editing', [
            $this,
            'filter_user_option_rich_editing',
        ]);
