...
class GF_Field_Name extends GF_Field
{
    ...
    function validate($value, $form)
    {
        if ($this->isRequired && $this->nameFormat != 'simple') {
            ...
            if (
                (empty($first) &&
                    !$this->get_input_property('3', 'isHidden')) ||
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        $first_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $first_input
        );
        $middle_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $middle_input
        );
        $last_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $last_input
        );
        $suffix_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $suffix_input
        );
        ...
        switch ($this->nameFormat) {
            ...
            case 'extended':
                ...
                $prefix_sub_label =
                    rgar($prefix_input, 'customLabel') != ''
                        ? $prefix_input['customLabel']
                        : gf_apply_filters(
                            ['gform_name_prefix', $form_id],
                            esc_html__('Prefix', 'gravityforms'),
                            $form_id
                        );
                $first_name_sub_label =
                    rgar($first_input, 'customLabel') != ''
                        ? $first_input['customLabel']
                        : gf_apply_filters(
                            ['gform_name_first', $form_id],
                            esc_html__('First', 'gravityforms'),
                            $form_id
                        );
                $middle_name_sub_label =
                    rgar($middle_input, 'customLabel') != ''
                        ? $middle_input['customLabel']
                        : gf_apply_filters(
                            ['gform_name_middle', $form_id],
                            esc_html__('Middle', 'gravityforms'),
                            $form_id
                        );
                $last_name_sub_label =
                    rgar($last_input, 'customLabel') != ''
                        ? $last_input['customLabel']
                        : gf_apply_filters(
                            ['gform_name_last', $form_id],
                            esc_html__('Last', 'gravityforms'),
                            $form_id
                        );
                $suffix_sub_label =
                    rgar($suffix_input, 'customLabel') != ''
                        ? $suffix_input['customLabel']
                        : gf_apply_filters(
                            ['gform_name_suffix', $form_id],
                ...
                if ($is_sub_label_above) {
                    $style =
                        $is_admin && rgar($prefix_input, 'isHidden')
                            ? "style='display:none;'"
                            : '';
                    if ($is_admin || !rgar($prefix_input, 'isHidden')) {
                        $prefix_select_class =
                            isset($prefix_input['choices']) &&
                            is_array($prefix_input['choices'])
                                ? 'name_prefix_select'
                                : '';
                        $prefix_markup = self::get_name_prefix_field(
                            $prefix_input,
                            $id,
                            $field_id,
                            $prefix,
                    ...
                    $style =
                        $is_admin && rgar($first_input, 'isHidden')
                            ? "style='display:none;'"
                            : '';
                    ...
                    $style =
                        $is_admin &&
                        (!isset($middle_input['isHidden']) ||
                            rgar($middle_input, 'isHidden'))
                            ? "style='display:none;'"
                            : '';
                    if (
                        $is_admin ||
                        (isset($middle_input['isHidden']) &&
                            $middle_input['isHidden'] == false)
                    ) {
                    ...
                    $style =
                        $is_admin && rgar($last_input, 'isHidden')
                            ? "style='display:none;'"
                            : '';
                    ...
                    $style =
                        $is_admin && rgar($suffix_input, 'isHidden')
                            ? "style='display:none;'"
                            : '';
                    if ($is_admin || !rgar($suffix_input, 'isHidden')) {
                        $suffix_select_class =
                            isset($suffix_input['choices']) &&
                            is_array($suffix_input['choices'])
                                ? 'name_suffix_select'
                                : '';
            ...
            case 'simple':
                ...
                $placeholder_attribute = GFCommon::get_field_placeholder_attribute(
                    $this
                );
            ...
            default:
                ...
                if ($is_sub_label_above) {
                    ...
                    if ($is_admin || !rgar($last_input, 'isHidden')) {
                        $last_markup =
                            "<span id='{$field_id}_6_container' class='name_last' {$style}>
                                                <label for='{$field_id}_6' {$sub_label_class_attribute}>" .
                            $last_name_sub_label .
                            "</label>
                                                <input type='text' name='input_{$id}.6' id='{$field_id}_6' value='{$last}' aria-label='{$last_name_aria_label}' {$last_tabindex} {$disabled_text} {$required_attribute} {$invalid_attribute} {$last_placeholder_attribute}/>
                                            </span>";
     ...
     */
    public static function get_name_prefix_field(
        $input,
        $id,
        $field_id,
        $value,
        ...
        if (isset($input['choices']) && is_array($input['choices'])) {
            ...
            foreach ($input['choices'] as $choice) {
                $choice_value = $value_enabled
                    ? $choice['value']
                    : $choice['text'];
                $is_selected_by_default = rgar($choice, 'isSelected');
                $is_this_choice_selected = empty($value)
                    ? $is_selected_by_default
                    : strtolower($choice_value) == strtolower($value);
                $selected = $is_this_choice_selected
                    ? "selected='selected'"
                    : '';
            ...
            $placeholder_attribute = GFCommon::get_input_placeholder_attribute(
                $input
            );
    ...
    {
        $input = GFFormsModel::get_input(
            $this,
            $this->id . '.' . (string) $input_id
        );
    ...
    public function sanitize_settings()
    {
        ...
        if (is_array($this->inputs)) {
            foreach ($this->inputs as &$input) {
                if (isset($input['choices']) && is_array($input['choices'])) {
                    $input['choices'] = $this->sanitize_settings_choices(
                        $input['choices']
                    );
                }
            }
