...
class GF_Field_Password extends GF_Field
{
    ...
    public function validate($value, $form)
    {
        ...
        if ($this->is_confirm_input_enabled() && $password != $confirm) {
            ...
            $this->passwordStrengthEnabled &&
            !empty($this->minPasswordStrength) &&
            !empty($password)
        ) {
            ...
            $levels = ['short' => 1, 'bad' => 2, 'good' => 3, 'strong' => 4];
            if (
                rgar($levels, $strength, 0) <
                $levels[$this->minPasswordStrength]
            ) {
                $this->failed_validation = true;
                $this->validation_message = empty($this->errorMessage)
                    ? sprintf(
                        esc_html__(
                            'Your password does not meet the required strength. %sHint: To make it stronger, use upper and lower case letters, numbers and symbols like ! " ? $ %% ^ & ).',
                            'gravityforms'
                        ),
                        '<br />'
                    )
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        $strength_style = !$this->passwordStrengthEnabled
            ? "style='display:none;'"
            : '';
        $strength_indicator_label = esc_html__(
            'Strength indicator',
            'gravityforms'
        );
        $strength =
            $this->passwordStrengthEnabled || $is_admin
                ? "<div id='{$field_id}_strength_indicator' class='gfield_password_strength' {$strength_style}>
        ...
        $onchange = $this->passwordStrengthEnabled
            ? "onchange='{$action}'"
            : '';
        ...
        $enter_password_field_input = GFFormsModel::get_input(
            $this,
            $this->id . ''
        );
        $confirm_password_field_input = GFFormsModel::get_input(
            $this,
            $this->id . '.2'
        );

        $enter_password_label =
            rgar($enter_password_field_input, 'customLabel') != ''
                ? $enter_password_field_input['customLabel']
                : esc_html__('Enter Password', 'gravityforms');
        $enter_password_label = gf_apply_filters(
            ['gform_password', $form_id],
            $enter_password_label,
            $form_id
        );

        $confirm_password_label =
            rgar($confirm_password_field_input, 'customLabel') != ''
                ? $confirm_password_field_input['customLabel']
                : esc_html__('Confirm Password', 'gravityforms');
        $confirm_password_label = gf_apply_filters(
            ['gform_password_confirm', $form_id],
            $confirm_password_label,
            $form_id
        );
        ...
        $enter_password_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $enter_password_field_input
        );
        $confirm_password_placeholder_attribute = GFCommon::get_input_placeholder_attribute(
            $confirm_password_field_input
        );

        $visibility_toggle_style = !$this->passwordVisibilityEnabled
            ? " style='display:none;'"
            : '';
        $enter_password_toggle =
            $this->passwordVisibilityEnabled || $is_admin
                ? "<button type='button' onclick='javascript:gformToggleShowPassword(\"{$field_id}\");' label='" .
                    esc_attr__('Show Password', 'gravityforms') .
                    "' data-label-show='" .
                    esc_attr__('Show Password', 'gravityforms') .
                    "' data-label-hide='" .
                    esc_attr__('Hide Password', 'gravityforms') .
                    "'{$visibility_toggle_style}><span class='dashicons dashicons-hidden' aria-hidden='true'></span></button>"
                : "";
        $confirm_password_toggle =
            $this->passwordVisibilityEnabled || $is_admin
                ? "<button type='button' onclick='javascript:gformToggleShowPassword(\"{$field_id}_2\");' label='" .
                    esc_attr__('Show Password', 'gravityforms') .
                    "' data-label-show='" .
                    esc_attr__('Show Password', 'gravityforms') .
                    "' data-label-hide='" .
        ...
        if ($is_form_editor) {
            $confirm_style = $this->is_confirm_input_enabled()
                ? ''
                : "style='display:none;'";
    ...
    public function get_value_save_entry(
         ...
         */
        $encrypt_password = apply_filters(
    ...
    public static function delete_passwords($entry, $form)
    {
        ...
        $encrypted_fields = GFFormsModel::get_openssl_encrypted_fields(
            $entry['id']
        );
    ...
    private function is_confirm_input_enabled()
    {
        ...
        return isset($confirm_input['isHidden'])
            ? !$confirm_input['isHidden']
            : true;
    }
    ...
    public static function stash_passwords($form)
    {
        foreach ($form['fields'] as $field) {
            ...
            if ($field->get_input_type() == 'password') {
                self::$passwords[$field->id] = $field->get_value_submission(
