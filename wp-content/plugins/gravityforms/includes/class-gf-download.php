...
class GF_Download
{
    ...
    public static function maybe_process()
    {
        if (isset($_GET['gf-download'])) {
            ...
            GFCommon::log_debug(
                __METHOD__ .
                    "(): Starting file download process. file: {$file}, hash: {$hash}."
            );

            $permission_granted = self::validate_download(
                $form_id,
                $field_id,
                $file,
                $hash
            ...
            if (has_filter('gform_permission_granted_pre_download')) {
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): Executing functions hooked to gform_permission_granted_pre_download.'
                );
            }
             ...
             */
            $permission_granted = apply_filters(
            ...
            if ($permission_granted) {
                GFCommon::log_debug(
                    __METHOD__ . '(): Download validated. Proceeding.'
                );
                self::deliver($form_id, $file);
            } else {
                GFCommon::log_debug(
                    __METHOD__ .
                        '(): Download validation failed. Aborting with 401.'
                );
                self::die_401();
            }
        }
    ...
    private static function validate_download($form_id, $field_id, $file, $hash)
    {
        ...
        if (has_filter('gform_require_login_pre_download')) {
            GFCommon::log_debug(
                __METHOD__ .
                    '(): Executing functions hooked to gform_require_login_pre_download.'
            );
        }
        ...
        $hash_check = GFCommon::generate_download_hash(
            $form_id,
            $field_id,
            $file
        );
    ...
    private static function deliver($form_id, $file)
    {
        ...
        GFCommon::log_debug(
            __METHOD__ . "(): Checking if file exists: {$file_path}."
        );

        if (file_exists($file_path)) {
            GFCommon::log_debug(
                __METHOD__ . '(): File exists. Starting delivery.'
            );
            ...
            GFCommon::log_debug(
                __METHOD__ . '(): File does not exist. Aborting with 404.'
            );
            self::die_404();
        }
    }
