(function (gflocking, $) {
  ...
  function initUI() {
    $("#gform-lock-request-button").click(function () {
      ...
      $.getJSON(ajaxurl, {
        action: "gf_lock_request_" + objectType,
        object_id: objectID,
      })
        .done(function (json) {
  ...
  function initHeartbeat() {
    ...
    $(document).on("heartbeat-send." + refreshLockKey, function (e, data) {
      ...
      if (!objectID || !$("#gform-lock-dialog").length) return;
    ...
    $(document).on("heartbeat-send." + requestLockKey, function (e, data) {
      ...
      if (!lockRequestInProgress) return data;
    ...
    $(document).on("heartbeat-tick." + refreshLockKey, function (e, data) {
      ...
      if (data[refreshLockKey]) {
        ...
        if (received.lock_error || received.lock_request) {
          details = received.lock_error
            ? received.lock_error
            : received.lock_request;
          wrap = $("#gform-lock-dialog");
          if (!wrap.length) return;
          if (!wrap.is(":visible")) {
            if (details.avatar_src) {
              avatar = $(
                '<img class="avatar avatar-64 photo" width="64" height="64" />'
              ).attr("src", details.avatar_src.replace(/&amp;/g, "&"));
