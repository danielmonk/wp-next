...
abstract class GFLocking
{
    ...
    public function init_ajax()
    {
        add_filter(
            'heartbeat_received',
            [$this, 'heartbeat_refresh_nonces'],
            10,
            3
        );
        add_filter(
            'heartbeat_received',
            [$this, 'heartbeat_check_locked_objects'],
            10,
            3
        );
        add_filter(
            'heartbeat_received',
            [$this, 'heartbeat_refresh_lock'],
            10,
            3
        );
        add_filter(
            'heartbeat_received',
            [$this, 'heartbeat_request_lock'],
            10,
            3
        );
        add_filter('wp_ajax_gf_lock_request_' . $this->_object_type, [
            $this,
            'ajax_lock_request',
        ]);
        add_filter('wp_ajax_gf_reject_lock_request_' . $this->_object_type, [
            $this,
    ...
    {
        add_action('admin_enqueue_scripts', [$this, 'enqueue_list_scripts']);
    }
    ...
    public function register_scripts()
    {
        ...
        wp_register_script(
            'gforms_locking',
            $locking_path . "js/locking{$min}.js",
            ['jquery', 'heartbeat'],
            GFCommon::$version
        );
        wp_register_script(
            'gforms_locking_view',
            $locking_path . "js/locking-view{$min}.js",
            ['jquery', 'heartbeat'],
            GFCommon::$version
        );
        wp_register_script(
            'gforms_locking_list',
            $locking_path . "js/locking-list{$min}.js",
            ['jquery', 'heartbeat'],
            GFCommon::$version
        );
        wp_register_style(
            'gforms_locking_css',
            $locking_path . "css/locking{$min}.css",
            [],
            GFCommon::$version
        );
        wp_register_style(
            'gforms_locking_list_css',
            $locking_path . "css/locking-list{$min}.css",
            [],
            GFCommon::$version
        );
    ...
    {
        $locking_styles = ['gforms_locking_css', 'gforms_locking_list_css'];
    ...
    {
        $strings = [
            'currently_locked' => wp_strip_all_tags(
                __(
                    'This page is currently locked. Click on the "Request Control" button to let %s know you\'d like to take over.',
                    'gravityforms'
                )
            ),
            'accept' => wp_strip_all_tags(__('Accept', 'gravityforms')),
            'cancel' => wp_strip_all_tags(__('Cancel', 'gravityforms')),
            'currently_editing' => wp_strip_all_tags(
                __('%s is currently editing', 'gravityforms')
            ),
            'taken_over' => wp_strip_all_tags(
                __(
            ...
            'lock_requested' => wp_strip_all_tags(
                __(
                    '%s has requested permission to take over control.',
                    'gravityforms'
                )
            ),
            'gained_control' => wp_strip_all_tags(
                __('You now have control', 'gravityforms')
            ),
            'request_pending' => wp_strip_all_tags(
                __('Pending', 'gravityforms')
            ),
            'no_response' => wp_strip_all_tags(
                __('No response', 'gravityforms')
            ),
            'request_again' => wp_strip_all_tags(
                __('Request again', 'gravityforms')
            ),
            'request_error' => wp_strip_all_tags(__('Error', 'gravityforms')),
            'request_rejected' => wp_strip_all_tags(
                __('Your request was rejected', 'gravityforms')
            ),
        ];
    ...
    protected function request_lock($object_id)
    {
        ...
        if (!$lock_holder_user_id) {
            ...
            $result['html'] = sprintf(
                __('Your request has been sent to %s.', 'gravityforms'),
                $user->display_name
            );
    ...
    {
        return GFCache::get(
            self::PREFIX_EDIT_LOCK . $this->_object_type . '_' . $object_id
        );
    }
    ...
    public function maybe_lock_object($is_edit_page)
    {
        if (isset($_GET['get-edit-lock'])) {
            ...
            if (
                $is_edit_page &&
                !($user_id = $this->check_lock($this->_object_id))
            ) {
                $this->set_lock($this->_object_id);
            }
        }
    ...
    public function heartbeat_check_locked_objects($response, $data, $screen_id)
    {
        ...
        if (
            array_key_exists($heartbeat_key, $data) &&
            is_array($data[$heartbeat_key])
        ) {
            foreach ($data[$heartbeat_key] as $object_id) {
                if (
                    ...
                    $send = [
                        'text' => sprintf(
                            __($this->get_string('currently_editing')),
                            $user->display_name
                        ),
                    ];
        ...
        if (array_key_exists($heartbeat_key, $data)) {
            ...
            if (
                ...
                if ($new_lock = $this->set_lock($object_id)) {
                    ...
                    if (
                        ($lock_requester = $this->check_lock_request(
                            $object_id
                        )) &&
                        ($user = get_userdata($lock_requester))
                    ) {
                        $lock_request = [
                            'text' => sprintf(
                                __($this->get_string('lock_requested')),
                                $user->display_name
                            ),
                        ];
    ...
    public function heartbeat_refresh_nonces($response, $data, $screen_id)
    {
        if (array_key_exists('gform-refresh-nonces', $data)) {
            ...
            if (
                !GFCommon::current_user_can_any($this->_capabilities) ||
            ...
            if (
                2 ===
                wp_verify_nonce(
                    $received['object_nonce'],
                    'update-contact_' . $object_id
                )
            ) {
                $response['gform-refresh-nonces'] = [
                    'replace' => [
                        '_wpnonce' => wp_create_nonce(
                            'update-object_' . $object_id
                        ),
                    ],
        ...
        if ($locked) {
            $message =
                '<div class="gform-locked-message">
                            <div class="gform-locked-avatar">' .
                get_avatar($user->ID, 64) .
                '</div>
                            <p class="currently-editing" tabindex="0">' .
                sprintf(
                    $this->get_string('currently_locked'),
                    $user->display_name
                ) .
                '</p>
                            <p>

                                <a id="gform-take-over-button" style="display:none" class="button button-primary wp-tab-first" href="' .
                esc_url(add_query_arg('get-edit-lock', '1', $edit_url)) .
                '">' .
                __('Take Over', 'gravityforms') .
                '</a>
                                <button id="gform-lock-request-button" class="button button-primary wp-tab-last">' .
                __('Request Control', 'gravityforms') .
                '</button>
                                <a class="button" href="' .
                esc_url($this->_redirect_url) .
                '">' .
            ...
            $message =
                '<div class="gform-taken-over">
                            <div class="gform-locked-avatar"></div>
                            <p class="wp-tab-first" tabindex="0">
                                <span class="currently-editing"></span><br>
                            </p>
                            <p>
                                <a id="gform-release-lock-button" class="button button-primary wp-tab-last"  href="' .
                esc_url(add_query_arg('release-edit-lock', '1', $edit_url)) .
                '">' .
                $this->get_string('accept') .
                '</a>
                                <button id="gform-reject-lock-request-button" style="display:none"  class="button button-primary wp-tab-last">' .
                __('Reject Request', 'gravityforms') .
                '</button>
                            </p>
                        </div>';
        }
        $html =
            '<div id="gform-lock-dialog" class="notification-dialog-wrap' .
            $hidden .
            '">
    ...
    public function lock_info($object_id, $echo = true)
    {
        ...
        if ($user_id && ($user = get_userdata($user_id))) {
            ...
            $locked_text = esc_html(
                sprintf(
                    $this->get_string('currently_editing'),
                    $user->display_name
                )
            );
        } else {
        ...
        $locked_info =
            '<div class="locked-info"><span class="locked-avatar">' .
            $locked_avatar .
