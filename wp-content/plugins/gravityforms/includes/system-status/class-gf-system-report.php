...
class GF_System_Report
{
    ...
    public static function system_report()
    {
       ...
       'gravityforms'
			...
			<p class="inline"><a href="#" class="button-primary" id="gf_copy_report" data-clipboard-target="#gf_system_report"><?php _e(
       'Copy System Report',
       'gravityforms'
   ); ?></a></p>
    ...
    esc_html_e('Report generated!', 'gravityforms');
    ...
    ?></p>
        ...
        'gravityforms'
			...
			<textarea id="gf_system_report" readonly="readonly" ><?php echo esc_html(
       $system_report_text
   ); ?></textarea>
		</div>
  ...
  if (!gapi()->is_site_registered()) { ?>
			<div id="gform_register_site">
				...
				<div>
					<p>
					<?php esc_html_e(
         'To register your site, enter your license key below.',
         'gravityforms'
     ); ?>
					</p>
					<input type="text" id="gform_license_key" name="gform_license_key" placeholder="<?php esc_html_e(
  ...
  GF_System_Status::page_footer();
        ...
        foreach ($sections as $section) {
            ...
            foreach ($section['tables'] as $table) {
                ...
                foreach ($table['items'] as $item) {
                    ...
                    $system_report_text .=
                        self::get_export($item, 'label') .
                        ': ' .
                        self::prepare_item_value($item, true) .
                        "\n";
                }
            }
        }

        $system_report_text = str_replace(
            ['()', '../'],
            ['', '[DT]'],
            $system_report_text
        );
    ...
    {
        // Get value.
        $value = isset($array["{$item}_export"])
            ? $array["{$item}_export"]
            : $array[$item];
    ...
    private static function maybe_process_action()
    {
        ...
        switch (rgpost('gf_action')) {
            case 'upgrade_database':
                ...
                if (
                    version_compare($previous_db_version, '2.3-beta-1', '<') &&
                    GFCommon::table_exists($wpdb->prefix . 'rg_form')
                ) {
                    ...
                    $percent_label = sprintf(
                        esc_html__('complete.', 'gravityforms'),
                        $percent
                    );

                    $status = sprintf(
                        '<span id="gf-upgrade-status">%s</span> <span id="gf-upgrade-precent">%s</span>%% %s',
                        $status,
                        $percent,
                        $percent_label
                    );
                    ...
                    $message = sprintf(
                        esc_html__('Current status: %s', 'gravityforms'),
                        $status
                    );
                    ...
                    $warning = esc_html__(
                        'Do not close or navigate away from this page until the upgrade is 100% complete.',
                        'gravityfroms'
                    );
            ...
            case 'register_site':
                ...
                if (!empty($new_key) && !gapi()->is_site_registered()) {
                    ...
                    if ($new_key_md5 != $previous_key_md5) {
                        ...
                        GFSettings::update_site_registration(
                            $previous_key_md5,
                            $new_key_md5
                        );
                    }
    ...
    public static function get_system_report()
    {
        ...
        if (is_wp_error($response)) {
            ...
            if ($response_code == 200) {
                $background_validation_message = esc_html__(
                    'Unexpected content in the response.',
                    'gravityforms'
                );
            } else {
                $background_validation_message = sprintf(
                    esc_html__('Response code: %s', 'gravityforms'),
                    $response_code
                );
            }
        }
                    ...
                    [
                        ...
                        'items' => [
                                ...
                                'versions' => [
                                    'support' => [
                                        ...
                                        'validation_message' => sprintf(
                                            esc_html__(
                                                'The Gravity Forms support agreement requires WordPress %s or greater. This site must be upgraded in order to be eligible for support.',
                                                'gravityforms'
                                            ),
                            ...
                            [
                                ...
                                'value' => !$wp_cron_disabled
                                    ? __('Yes', 'gravityforms')
                                    : __('No', 'gravityforms'),
                                'value_export' => !$wp_cron_disabled
                                    ? 'Yes'
                                    : 'No',
                            ],
                            [
                                ...
                                'value' => $alternate_wp_cron
                                    ? __('Yes', 'gravityforms')
                                    : __('No', 'gravityforms'),
                                'value_export' => $alternate_wp_cron
                                    ? 'Yes'
                                    : 'No',
                            ],
                            [
                    ...
                    [
                        ...
                        'items' => [
                            [
                                ...
                                'version_compare' => '>=',
                                'minimum_version' => '7.3',
                                'validation_message' => esc_html__(
                                    'Recommended: PHP 7.3 or higher.',
                                    'gravityforms'
                                ),
                            ],
                            [
                                'label' =>
                                    esc_html__('Memory Limit', 'gravityforms') .
                                    ' (memory_limit)',
                            ...
                            [
                                ...
                                'value' => function_exists('curl_init')
                                    ? __('Yes', 'gravityforms') .
                                        ' (' .
                                        __('version', 'gravityforms') .
                                        ' ' .
                                        rgar(curl_version(), 'version') .
                                        ')'
                                    : __('No', 'gravityforms'),
                                'value_export' => function_exists('curl_init')
                                    ? 'Yes' .
                                        ' (' .
                                        __('version', 'gravityforms') .
                                        ' ' .
                            ...
                            [
                                ...
                                'value' => defined('OPENSSL_VERSION_TEXT')
                                    ? OPENSSL_VERSION_TEXT .
                                        ' (' .
                                        OPENSSL_VERSION_NUMBER .
                                        ')'
                                    : __('No', 'gravityforms'),
                                'value_export' => defined(
                                    'OPENSSL_VERSION_TEXT'
                                )
                                    ? OPENSSL_VERSION_TEXT .
                                        ' (' .
                    ...
                    [
                        ...
                        'items' => [
                            [
                                ...
                                'validation_message' => esc_html__(
                                    'Gravity Forms requires MySQL 5 or above.',
                                    'gravityforms'
                                ),
                            ],
                            [
    ...
    public static function prepare_item_value($item, $is_export = false)
    {
        ...
        switch ($type) {
            ...
            case 'version_check':
                ...
                $valid_version = version_compare(
                    $item['value'],
                    $item['minimum_version'],
                    $item['version_compare']
                );

                // Display value based on valid version check.
                if ($valid_version) {
                    return $is_export
                        ? self::get_export($item, 'value') . ' ✔'
                        : $item['value'] .
                    ...
                    $html =
                        self::get_export($item, 'value') .
                        ' ✘ ' .
                        self::get_export($item, 'validation_message');
            ...
            case 'wordpress_version_check':
                // Run version checks.
                $version_check_support = version_compare(
                    $item['value'],
                    $item['versions']['support']['minimum_version'],
                    $item['versions']['support']['version_compare']
                );
                $version_check_min = version_compare(
                    $item['value'],
                    $item['versions']['minimum']['minimum_version'],
                    $item['versions']['minimum']['version_compare']
                );
                ...
                if ($version_check_support) {
                    ...
                    $validation_message = $version_check_min
                        ? self::get_export(
                            $item['versions']['support'],
                            'validation_message'
                        )
                        : self::get_export(
                            $item['versions']['minimum'],
                            'validation_message'
                        );

                    return self::get_export($item, 'value') .
                        ' ✘ ' .
                        $validation_message;
                } else {
                    $validation_message = $version_check_min
                        ? $item['versions']['support']['validation_message']
                        : $item['versions']['minimum']['validation_message'];
            ...
            default:
                ...
                if (isset($item['action']) && !$is_export) {
                    $url = add_query_arg(['action' => $item['action']['code']]);
                    $value .=
                        "&nbsp;<a href='#' onclick='gfDoAction(\"{$item['action']['code']}\", \"" .
    ...
    public static function get_gravityforms()
    {
        ...
        if ($is_registered) {
            ...
            $validation_message = sprintf(
                esc_html__(
                    'There was an error registering your site. Please check that the licence key entered is valid and not expired. If the problem persists, please contact support. %1$sRegister Site%2$s.',
                    'gravityforms'
                ),
                '<a class="thickbox" href="#TB_inline?width=400&inlineId=gform_register_site">',
                '</a>'
            );
        } else {
            $validation_message = sprintf(
                esc_html__(
                    'This site has not been registered. %1$sPlease register your site%2$s.',
                    'gravityforms'
                ),
                '<a class="thickbox" href="#TB_inline?width=400&inlineId=gform_register_site">',
                '</a>'
        ...
        $gravityforms = [
            ...
            [
                ...
                'value' => $is_writable
                    ? __('Writable', 'gravityforms')
                    : __('Not writable', 'gravityforms'),
                'value_export' => $is_writable ? 'Writable' : 'Not writable',
                'is_valid' => $is_writable,
                'validation_message' => $is_writable
                    ? ''
                    : esc_html__(
                        'File uploads, entry exports, and logging will not function properly.',
                        'gravityforms'
                    ),
            ],
            [
            ...
            [
                ...
                'value' => $is_registered
                    ? esc_html__('Site registered ', 'gravityforms') .
                        ' ( ' .
                        $site_key .
                        ' ) '
                    : '',
                'is_valid' => $is_registered,
                'value_export' => $is_registered
                    ? 'Site registered ( ' . $site_key . ' ) '
                    : 'Site not registered',
    ...
    public static function get_database()
    {
        ...
        foreach ($gf_tables as $i => $table_name) {
            ...
            if (!GFCommon::table_exists($table_name)) {
                ...
                $value = false;
                $validation_message = __(
                    'Table has not been upgraded successfully.',
                    'gravityforms'
                );
                $validation_message_export =
                    'Table has not been upgraded successfully.';
        ...
        $warning_message = __(
            "WARNING! Re-running the upgrade process is only recommended if you are currently experiencing issues with your database. This process may take several minutes to complete. 'OK' to upgrade. 'Cancel' to abort.",
            'gravityforms'
        );
        ...
        if (
            ...
            if (gf_upgrade()->is_upgrading()) {
                ...
                $status = empty($status)
                    ? ''
                    : sprintf(
                        __('Current Status: %s', 'gravityforms'),
                        $status
                    );
                $percent = self::get_upgrade_percent_complete();
                $percent_label = sprintf(
                    esc_html__('%s%% complete.', 'gravityforms'),
                    $percent
                );
                $status .= ' ' . $percent_label;
                if (
                    defined('GFORM_AUTO_DB_MIGRATION_DISABLED') &&
                    GFORM_AUTO_DB_MIGRATION_DISABLED
                ) {
                    $message = sprintf(
                        __(
                            'Automatic background migration is disabled but the database needs to be upgraded to version %s. %s',
                            'gravityforms'
                        ),
                        GFForms::$version,
                        $status
                    ...
                    $action_label = __(
                        'Force the migration manually',
                        'gravityforms'
                    );
                } else {
                    $message = sprintf(
                        __(
                            'The database is currently being upgraded to version %s. %s',
                            'gravityforms'
                        ),
                        GFForms::$version,
                        $status
                    ...
                    if (!self::$background_tasks) {
                        $message .=
                            ' ' .
                            __(
                                "As this site doesn't support background tasks the upgrade process will take longer than usual and the status will change infrequently.",
                                'gravityforms'
                            );
                    }
                ...
                $tables[0] = array_merge($tables[0], [
                    ...
                    'is_valid' => false,
                    'message' => __(
                        'Your database version is out of date.',
                        'gravityforms'
                    ),
            ...
            $tables[0] = array_merge($tables[0], [
                ...
                'is_valid' => false,
                'message' =>
                    'upgrade_database' == rgpost('gf_action')
                        ? __('Database upgrade failed.', 'gravityforms')
                        : __(
                            'There are issues with your database.',
                            'gravityforms'
                        ),
                'message_export' =>
                    'upgrade_database' == rgpost('gf_action')
            ...
            $tables[0] = array_merge($tables[0], [
                ...
                'message' =>
                    'upgrade_database' == rgpost('gf_action')
                        ? __('Database upgraded successfully.', 'gravityforms')
                        : __('Your database is up-to-date.', 'gravityforms') .
                            ' ' .
                            __(
                                'Warning: downgrading Gravity Forms is not recommended.',
                                'gravityforms'
                            ),
                'message_export' =>
                    'upgrade_database' == rgpost('gf_action')
    ...
    public static function get_available_logs()
    {
        ...
        foreach ($supported_plugins as $plugin_slug => $plugin_name) {
            ...
            $logs[] = [
                'label' =>
                    '<a href="' .
                    gf_logging()->get_log_file_url($plugin_slug) .
                    '">' .
    ...
    public static function get_active_plugins(
        $include_gravity_forms = true,
        $include_gf_addons = true,
        $include_non_gf_addons = true
    ) {
        ...
        foreach (get_plugins() as $plugin_path => $plugin) {
            ...
            if (
                'gravityforms/gravityforms.php' === $plugin_path &&
            ...
            if ($is_addon) {
                ...
                if (
                    isset($version_info['offerings'][$slug]) &&
                    version_compare(
                        $plugin['Version'],
                        $version_info['offerings'][$slug]['version'],
                        '<'
                    )
                ) {
                    $is_valid = false;
                    $validation_message = sprintf(
                        __('New version %s available.', 'gravityforms'),
                        $version_info['offerings'][$slug]['version']
                    );
                    ...
                    $validation_message = sprintf(
                        __(
                            'Your system does not meet the minimum requirements for this Add-On (%d errors).',
                            'gravityforms'
                        ),
                        count($errors)
                    );
                    $validation_message_export = sprintf(
                        'Your system does not meet the minimum requirements for this Add-On (%1$d errors). %2$s',
                        count($errors),
                        implode('. ', $errors)
                    );
                }
            ...
            $plugin_name = $is_addon
                ? str_replace(
                    ' Add-On',
                    '',
                    str_replace('Gravity Forms ', '', $plugin['Name'])
                )
                : $plugin['Name'];
    ...
    public static function get_network_active_plugins()
    {
        ...
        $network_active_plugins = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT meta_value FROM {$wpdb->sitemeta} WHERE meta_key=%s",
        ...
        foreach ($network_active_plugins as $plugin_path => &$plugin) {
            ...
            $plugin_data = get_plugin_data(
                WP_CONTENT_DIR . '/plugins/' . $plugin_path
            );
    ...
    public static function get_gf_addon($path)
    {
        ...
        foreach ($gf_addons as $gf_addon) {
            ...
            if (!is_callable([$gf_addon, 'get_instance'])) {
                continue;
            }

            // Get Add-On instance.
            $addon = call_user_func([$gf_addon, 'get_instance']);
    ...
    public static function get_theme()
    {
        ...
        $update_themes_versions = !empty($update_themes->checked)
            ? $update_themes->checked
            : [];
        ...
        $theme_details = [
            [
                ...
                'is_valid' => version_compare(
                    $theme_version,
                    rgar(
                        $update_themes_versions,
        ...
        if (is_child_theme()) {
            ...
            $theme_details[] = [
                'label' => sprintf(
                    '%s (%s)',
                    $parent_name,
                    esc_html__('Parent', 'gravityforms')
                ),
    ...
    public static function get_upgrade_percent_complete()
    {
        ...
        $legacy_count =
            $c->legacy_form_count +
            $c->legacy_form_meta_count +
            $c->legacy_form_view_count +
            $c->lead_count +
            $c->lead_detail_count +
            $c->lead_meta_count +
            $c->lead_notes_count;
