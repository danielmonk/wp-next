...
class GFLogging extends GFAddOn
{
     ...
     */
    protected $_capabilities = ['gravityforms_logging'];
    ...
    public function plugin_settings_page()
    {
        ...
        if ($plugin_slug) {
            ...
            if (isset($supported_plugins[$plugin_slug])) {
                if (
                    wp_verify_nonce(
                        rgget($this->_nonce_action),
                        $this->_nonce_action
                    ) &&
                    $this->delete_log_file($plugin_slug)
                ) {
                ...
                GFCommon::add_error_message(
                    esc_html__('Invalid log file.', 'gravityforms')
                );
            }
        }
        ...
        if ('1' === rgget('deleted')) {
            GFCommon::add_message(
                esc_html__('Log file was successfully deleted.', 'gravityforms')
            );
        }
    ...
    public function plugin_settings_fields()
    {
        ...
        $plugin_fields[] = [
            ...
            'messages' => [
                'success' => esc_html__(
                    'Plugin logging settings have been updated.',
                    'gravityforms'
                ),
            ],
        ];
        ...
        foreach ($supported_plugins as $plugin_slug => $plugin_name) {
            ...
            if ($this->log_file_exists($plugin_slug)) {
                $delete_url = add_query_arg(
                    [
                        'delete_log' => $plugin_slug,
                        $this->_nonce_action => $nonce,
                    ],
                    admin_url(
                ...
                $after_select .=
                    ...
                    esc_attr($this->get_log_file_url($plugin_slug)) .
                    '" target="_blank">' .
                    esc_html__('view log', 'gravityforms') .
                    '</a>';
                $after_select .=
                    '&nbsp;&nbsp;<a href="' .
                    $delete_url .
                    '">' .
                    esc_html__('delete log', 'gravityforms') .
                    '</a>';
                $after_select .=
                    '&nbsp;&nbsp;(' .
                    $this->get_log_file_size($plugin_slug) .
                    ')</span>';
            }
    ...
    public static function log_message(
        $plugin,
        $message = null,
        $message_type = KLogger::DEBUG
    ) {
        // If message is empty, exit.
        if (
            rgblank($message) ||
            !class_exists('GFForms') ||
            !get_option('gform_enable_logging')
        ) {
            return;
        }
    ...
    public function get_log_file_size($plugin_name)
    {
        ...
        $size =
            sprintf('%.2f', $size / pow(1024, $factor)) . ' ' . $units[$factor];
    ...
    private function maybe_reset_logs($file_path, $gmt_offset)
    {
        ...
        if (
            ...
            filesize($file_path) > $this->max_file_size
        ) {
            $adjusted_date = gmdate(
                $this->date_format_log_file,
                time() + $gmt_offset
            );
            $new_file_name =
                $file_base . '_' . $adjusted_date . '.' . $file_ext;
        ...
        if (false !== $similar_files && $file_count > $this->max_file_count) {
            ...
            usort($similar_files, [$this, 'filemtime_diff']);
    ...
    public function upgrade($previous_version)
    {
        ...
        $previous_is_pre_ui =
            !empty($previous_version) &&
            version_compare($previous_version, '1.3', '<');
    ...
    public function upgrade_from_pre_ui_update()
    {
        ...
        foreach ($settings as $plugin_slug => &$plugin_setting) {
            ...
            $plugin_setting['enable'] =
                $plugin_setting['log_level'] == KLogger::OFF ? '0' : '1';
        }
    ...
    {
        $supported_plugins = apply_filters('gform_logging_supported', []);
