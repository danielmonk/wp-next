...
class GFEntryDetail
{
    ...
    public static function add_meta_boxes()
    {
        ...
        $meta_boxes = [
            'submitdiv' => [
                ...
                'callback' => ['GFEntryDetail', 'meta_box_entry_info'],
                'context' => 'side',
            ],
        ];

        if (GFCommon::current_user_can_any('gravityforms_edit_entry_notes')) {
            $meta_boxes['notifications'] = [
                'title' => esc_html__('Notifications', 'gravityforms'),
                'callback' => ['GFEntryDetail', 'meta_box_notifications'],
                'context' => 'side',
            ];
        }
        ...
        if (GFCommon::current_user_can_any('gravityforms_view_entry_notes')) {
            $meta_boxes['notes'] = [
                ...
                'callback' => ['GFEntryDetail', 'meta_box_notes'],
                'context' => 'normal',
            ];
        }
            ...
            $meta_boxes['payment'] = [
                'title' =>
                    $entry['transaction_type'] == 2
                        ? esc_html__('Subscription Details', 'gravityforms')
                        : esc_html__('Payment Details', 'gravityforms'),
        ...
        foreach ($meta_boxes as $id => $meta_box) {
            ...
            add_meta_box(
                ...
                $screen->id,
                $meta_box['context'],
                isset($meta_box['priority'])
                    ? $meta_box['priority']
                    : 'default',
                isset($meta_box['callback_args'])
    ...
    public static function get_current_entry()
    {
        ...
        $form = self::get_current_form();
        $form_id = absint($form['id']);
        $lead_id = rgpost('entry_id')
            ? absint(rgpost('entry_id'))
            : absint(rgget('lid'));
        ...
        if (isset($_GET['field_id']) && $_GET['field_id'] !== '') {
            ...
            $search_criteria['field_filters'][] = [
                'key' => $key,
                'operator' => rgempty('operator', $_GET)
                    ? 'is'
                    : rgget('operator'),
                'value' => $val,
            ];
        ...
        $search_criteria = gf_apply_filters(
            ['gform_search_criteria_entry_list', $form_id],
            $search_criteria,
            $form_id
        );
    ...
    public static function lead_detail_page()
    {
        ...
        if (is_wp_error($lead) || !$lead) {
            esc_html_e(
                "Oops! We couldn't find your entry. Please try again",
                'gravityforms'
            );
         ...
         * @since 2.3.3.9
         */
        gf_do_action(['gform_pre_entry_detail', $form['id']], $form, $lead);

        $total_count = self::get_total_count();
        $position = rgget('pos') ? rgget('pos') : 0;
        $prev_pos =
            !rgblank($position) && $position > 0 ? $position - 1 : false;
        $next_pos =
            !rgblank($position) && $position < self::$_total_count - 1
                ? $position + 1
                : false;
        ...
        switch (RGForms::post('action')) {
            case 'update':
                 ...
                 */
                gf_do_action(
                    ['gform_after_update_entry', $form['id']],
                    $form,
                    $lead['id'],
                    $original_entry
                );
                ...
                if (GFCommon::has_consent_field($form)) {
                    ...
                    foreach ($form['fields'] as $field) {
                        if ($field['type'] === 'consent') {
                            $field_obj = GFFormsModel::get_field(
                                $form,
                                $field['id']
                            );
                            $revision_id = GFFormsModel::get_latest_form_revisions_id(
                                $form['id']
                            );
                            $current_description = $field_obj->get_field_description_from_revision(
                                $revision_id
                            );
                            $submitted_description = $field_obj->get_field_description_from_revision(
                                $original_entry[$field['id'] . '.3']
                            );

                            if (
                                $lead[$field['id'] . '.1'] !==
                                    $original_entry[$field['id'] . '.1'] ||
                                $field['checkboxLabel'] !==
                                    $original_entry[$field['id'] . '.2'] ||
                                $current_description !== $submitted_description
                            ) {
                                if (!empty($consent_update_note)) {
                                    $consent_update_note .= "\n";
                                }
                                $consent_update_note .= empty(
                                    $lead[$field['id'] . '.1']
                                )
                                    ? sprintf(
                                        esc_html__(
            ...
            case 'add_note':
                ...
                GFFormsModel::add_note(
                    ...
                    $current_user->ID,
                    $user_data->display_name,
                    stripslashes($_POST['new_note'])
                );

                //emailing notes if configured
                if (rgpost('gentry_email_notes_to')) {
                    GFCommon::log_debug(
                        'GFEntryDetail::lead_detail_page(): Preparing to email entry notes.'
                    );
                    $email_to = $_POST['gentry_email_notes_to'];
                    $email_from = $current_user->user_email;
                    $email_subject = stripslashes(
                        $_POST['gentry_email_subject']
                    );
                    $body = stripslashes($_POST['new_note']);

                    $headers = "From: \"$email_from\" <$email_from> \r\n";
                    GFCommon::log_debug(
                        "GFEntryDetail::lead_detail_page(): Emailing notes - TO: $email_to SUBJECT: $email_subject BODY: $body HEADERS: $headers"
                    );
                    $is_success = wp_mail(
                        $email_to,
                        $email_subject,
                    ...
                    $result = is_wp_error($is_success)
                        ...
                        : $is_success;
                    GFCommon::log_debug(
                        "GFEntryDetail::lead_detail_page(): Result from wp_mail(): {$result}"
                    );
                    if (!is_wp_error($is_success) && $is_success) {
                        GFCommon::log_debug(
                            'GFEntryDetail::lead_detail_page(): Mail was passed from WordPress to the mail server.'
                        );
                    } else {
                        GFCommon::log_error(
                            'GFEntryDetail::lead_detail_page(): The mail message was passed off to WordPress for processing, but WordPress was unable to send the message.'
                        );
                    }
                    ...
                    if (has_filter('phpmailer_init')) {
                        GFCommon::log_debug(
                            __METHOD__ .
                                '(): The WordPress phpmailer_init hook has been detected, usually used by SMTP plugins, it can impact mail delivery.'
                        );
                    }
            ...
            case 'bulk':
                ...
                if ($_POST['bulk_action'] == 'delete') {
                    if (
                        ...
                        wp_die(
                            esc_html__(
                                "You don't have adequate permission to delete notes.",
                                'gravityforms'
                            )
                        );
                    }
            ...
            case 'trash':
                ...
                if (
                    ...
                    wp_die(
                        esc_html__(
                            "You don't have adequate permission to trash entries.",
                            'gravityforms'
                        )
                    );
                }
            ...
            case 'restore':
                ...
                if (
                    ...
                    wp_die(
                        esc_html__(
                            "You don't have adequate permission to restore entries.",
                            'gravityforms'
                        )
                    );
                }
            ...
            case 'delete':
                ...
                if (
                    ...
                    wp_die(
                        esc_html__(
                            "You don't have adequate permission to delete entries.",
                            'gravityforms'
                        )
                    );
                }
        ...
        __(
            ...
            'gravityforms'
					...
					mysack.setVar("rg_delete_file", "<?php echo wp_create_nonce(
         'rg_delete_file'
     ); ?>");
          ...
          __('Ajax error while deleting field.', 'gravityforms')
      ); ?>)
					};
					mysack.runAJAX();
			...
			function ResendNotifications() {
				...
				if (selectedNotifications.length <= 0) {
					displayMessage(<?php echo json_encode(
         __(
             'You must select at least one type of notification to resend.',
             'gravityforms'
         )
     ); ?>, 'error', '#notifications');
					return;
				...
				jQuery.post(ajaxurl, {
						...
						gf_resend_notifications: '<?php echo wp_create_nonce(
          'gf_resend_notifications'
					...
					function (response) {
						if (response) {
							...
							displayMessage(<?php echo json_encode(
           esc_html__('Notifications were resent successfully.', 'gravityforms')
		...
		<?php $editable_class = GFCommon::current_user_can_any(
      ...
      : ''; ?>
		<form method="post" id="entry_form" enctype='multipart/form-data'>
			...
			<input type="hidden" name="screen_mode" id="screen_mode" value="<?php echo esc_attr(
       rgpost('screen_mode')
   ); ?>" />

			<input type="hidden" name="entry_id" id="entry_id" value="<?php echo absint(
       $lead['id']
   ); ?>" />

			<div class="wrap gf_entry_wrap">
				<h2 class="gf_admin_page_title">
					<span id='gform_settings_page_title' class='gform_settings_page_title<?php echo $editable_class; ?>' onclick='GF_ShowEditTitle()'><?php echo esc_html(
    rgar($form, 'title')
); ?></span>
						...
						<div class="gf_entry_detail_pagination">
							<ul>
								<li class="gf_entry_count">
									<span>entry <strong><?php echo $position +
             1; ?></strong> of <strong><?php echo $total_count; ?></strong></span>
								</li>
								<li class="gf_entry_prev gf_entry_pagination"><?php echo GFEntryDetail::entry_detail_pagination_link(
            $prev_pos,
            'Previous Entry',
            'gf_entry_prev_link',
            'fa fa-arrow-circle-o-left'
        ); ?></li>
								<li class="gf_entry_next gf_entry_pagination"><?php echo GFEntryDetail::entry_detail_pagination_link(
            $next_pos,
            'Next Entry',
            'gf_entry_next_link',
     ...
     ?>
       ...
       do_action('gform_entry_detail_sidebar_middle', $form, $lead); ?>
							...
							<div class="detail-view-print">
								<a href="javascript:;" onclick="var notes_qs = jQuery('#gform_print_notes').is(':checked') ? '&notes=1' : ''; var url='<?php echo trailingslashit(
            site_url()
        ); ?>?gf_page=print-entry&fid=<?php echo absint(
    $form['id']
); ?>&lid=<?php echo absint(
    $lead['id']
); ?>' + notes_qs; window.open (url,'printwindow');" class="button"><?php esc_html_e(
    'Print',
    'gravityforms'
); ?></a>
								<?php if (GFCommon::current_user_can_any('gravityforms_view_entry_notes')) { ?>
									...
									<label for="print_notes"><?php esc_html_e(
             'include notes',
             'gravityforms'
         ); ?></label>
								<?php } ?>
		...
		<div class="postbox">
         ...
         switch ($field->get_input_type()) {
								...
								<tr valign="top">
									<td class="detail-view">
										<div style="margin-bottom:10px; border-bottom:1px dotted #ccc;">
											<h2 class="detail_gsection_title">' .
             ...
             default:
                 ...
                 $content =
                     "<tr valign='top'><td class='detail-view' id='{$td_id}'>" .
                     $field_label .
          ...
          */
         $content = gf_apply_filters(
             ['gform_field_content', $form['id'], $field->id],
             $content,
             $field,
             $value,
             $lead['id'],
         ...
         echo $content;
				...
				<div class="gform_footer">
					...
					<input type="hidden" name="gform_uploaded_files" id='gform_uploaded_files_<?php echo absint(
         $form_id
     ); ?>' value="" />
				</div>
			</div>
    ...
    public static function notes_grid(
        $notes,
        $is_editable,
        $emails = null,
        $subject = ''
        ...
        if (
            sizeof($notes) > 0 &&
            $is_editable &&
            GFCommon::current_user_can_any('gravityforms_edit_entry_notes')
        ) { ?>
			<div class="alignleft actions" style="padding:3px 0;">
				<label class="hidden" for="bulk_action"><?php esc_html_e(
        ' Bulk action',
        'gravityforms'
    ); ?></label>
    ...
    $apply_button =
        ...
        esc_attr__('Apply', 'gravityforms') .
        '" onclick="jQuery(\'#action\').val(\'bulk\');" style="width: 50px;" />';
    /**
   ...
   foreach ($notes as $note) {
       ...
       ?>
				<tr valign="top" data-id="<?php echo esc_attr(
        $note->id
    ); ?>" data-type="<?php echo esc_attr(
    $note->note_type
); ?>"<?php echo rgobj($note, 'sub_type')
    ? ' data-sub-type="' . esc_attr($note->sub_type) . '"'
    : ''; ?>>
					<?php if (
         $is_editable &&
         GFCommon::current_user_can_any('gravityforms_edit_entry_notes')
        ...
        ?>
							...
							<div class="note-meta">
								...
								<?php if ($note->user_email): ?><a href="mailto:<?php echo esc_attr(
    $note->user_email
); ?>" class="note-email"><?php echo esc_html(
    $note->user_email
); ?></a><?php endif; ?><br />
								<time class="note-date"><?php esc_html_e(
            'added',
            'gravityforms'
        ); ?> <?php echo esc_html(
     GFCommon::format_date($note->date_created, true)
 ); ?></time>
							</div>
						</div>
						<div class="detail-note-content <?php echo implode(
          ' ',
          $classes
      ); ?>"><?php echo nl2br(esc_html($note->value)); ?></div>
					</td>
   ...
   if (
       ...
       GFCommon::current_user_can_any('gravityforms_edit_entry_notes')
				...
				<tr>
					<td colspan="3" style="padding:10px;" class="lastrow">
						...
						<?php
      $note_button =
          '<input type="submit" name="add_note" value="' .
          esc_attr__('Add Note', 'gravityforms') .
          '" class="button" style="width:auto;padding-bottom:2px;" onclick="jQuery(\'#action\').val(\'add_note\');"/>';
    ...
    $email
                                ...
                                <span id='gentry_email_subject_container' style="display:none;">
                                    <label for="gentry_email_subject"><?php esc_html_e(
                                        'Subject:',
                                        'gravityforms'
                                    ); ?></label>
    ...
    public static function lead_detail_grid(
        $form,
        $lead,
        $allow_display_empty_fields = false
    ) {
        $form_id = absint($form['id']);
        ...
        $display_empty_fields = self::maybe_display_empty_fields(
            $allow_display_empty_fields,
            $form,
            $lead
        );
        ?>
     ...
     ?>
				...
				<th style="width:auto; font-size:10px; text-align: right;">
					<?php if ($allow_display_empty_fields) { ?>
						<input type="checkbox" id="gentry_display_empty_fields" <?php echo $display_empty_fields
          ? "checked='checked'"
          : ''; ?> onclick="ToggleShowEmptyFields();" />&nbsp;&nbsp;
						<label for="gentry_display_empty_fields"><?php esc_html_e(
          'show empty fields',
          'gravityforms'
      ); ?></label>
						<?php } ?>
   ...
   foreach ($form['fields'] as $field) {
       ...
       switch ($field->get_input_type()) {
           case 'section':
               if (
                   ...
                   $content =
                       '
                                <tr>
                                    <td colspan="2" class="entry-view-section-break' .
                       $is_last .
                       '">' .
                       esc_html(GFCommon::get_label($field)) .
                       '</td>
           ...
           default:
               ...
               $display_value = GFCommon::get_lead_field_display(
                   $field,
                   $value,
                   $lead['currency']
               );
               ...
               if (
                   ...
                   $count++;
                   $is_last =
                       $count >= $field_count && !$has_product_fields
                           ? true
                           : false;
                   $last_row = $is_last ? ' lastrow' : '';

                   $display_value =
                       empty($display_value) && $display_value !== '0'
                           ? '&nbsp;'
                           : $display_value;
   ...
   if ($has_product_fields) {
       ...
       if (!empty($products['products'])) {
           ob_start(); ?>
					<tr>
						<td colspan="2" class="entry-view-field-name"><?php echo esc_html(
          gf_apply_filters(
              ['gform_order_label', $form_id],
              __('Order', 'gravityforms'),
              $form_id
          )
      ); ?></td>
					</tr>
					<tr>
						<td colspan="2" class="entry-view-field-value lastrow">
							<table class="entry-products" cellspacing="0" width="97%">
								...
								<thead>
								<th scope="col"><?php echo gf_apply_filters(
            ['gform_product', $form_id],
            __('Product', 'gravityforms'),
            $form_id
        ); ?></th>
								<th scope="col" class="textcenter"><?php echo esc_html(
            gf_apply_filters(
                ...
                $form_id
								...
								<th scope="col"><?php echo esc_html(
            gf_apply_filters(
                ['gform_product_unitprice', $form_id],
                __('Unit Price', 'gravityforms'),
                $form_id
            )
        ); ?></th>
								<th scope="col"><?php echo esc_html(
        ...
        ?>
								...
								<tfoot>
								<?php if (!empty($products['shipping']['name'])) { ?>
									<tr>
										<td colspan="2" rowspan="2" class="emptycell">&nbsp;</td>
										<td class="textright shipping"><?php echo esc_html(
              $products['shipping']['name']
          ); ?></td>
										<td class="shipping_amount"><?php echo GFCommon::to_money(
              ...
              $lead['currency']
								...
								<tr>
									<?php if (empty($products['shipping']['name'])) { ?>
										...
										<?php } ?>
									<td class="textright grandtotal"><?php esc_html_e(
             'Total',
             'gravityforms'
         ); ?></td>
									<td class="grandtotal_amount"><?php echo GFCommon::to_money(
             $total,
             $lead['currency']
         ); ?></td>
								</tr>
      ...
      */
     $order_summary = gf_apply_filters(
    ...
    public static function entry_detail_pagination_link(
        $pos,
        $label = '',
        $class = '',
        $icon = ''
    ) {
        $url = add_query_arg(['pos' => $pos], remove_query_arg(['pos', 'lid']));
        ...
        return '<a ' .
            ...
            '" aria-label="' .
            esc_attr($label) .
            '"><i aria-hidden="true" class="fa-lg ' .
            esc_attr($icon) .
            '" title="' .
            esc_attr($label) .
            '"></i></a>';
    ...
    public static function payment_details_box($entry, $form)
    {
        _deprecated_function(
            __FUNCTION__,
            '2.0',
            'Use add_meta_box() with GFEntryDetail::meta_box_payment_details as the "callback" parameter.'
        ); ?>
		<!-- PAYMENT BOX -->
		<div id="submitdiv" class="stuffbox">

			<h3 class="hndle" style="cursor:default;">
				<span><?php echo $entry['transaction_type'] == 2
        ? esc_html__('Subscription Details', 'gravityforms')
        : esc_html__('Payment Details', 'gravityforms'); ?></span>
			</h3>
    ...
    public static function meta_box_payment_details($args)
    {
		...
		<div id="submitcomment" class="submitbox">
			<div id="minor-publishing">
				<?php
    $payment_status = apply_filters(
        'gform_payment_status',
        GFCommon::get_entry_payment_status_text($entry['payment_status']),
        $form,
        $entry
    );
    if (!empty($payment_status)) { ?>
					<div id="gf_payment_status" class="gf_payment_detail">
						...
						<span id="gform_payment_status"><?php echo $payment_status;
        // May contain HTML
        ?></span>
					</div>
     ...
     if (!empty($payment_date)) { ?>
						<div id="gf_payment_date" class="gf_payment_detail">
							<?php echo $entry['transaction_type'] == 2
           ? esc_html__('Start Date', 'gravityforms')
           : esc_html__('Date', 'gravityforms'); ?>:
							<span id='gform_payment_date'><?php echo $payment_date;
         // May contain HTML
         ?></span>
						</div>
						<?php }
     ...
     if (!empty($transaction_id)) { ?>
						<div id="gf_payment_transaction_id" class="gf_payment_detail">
							<?php echo $entry['transaction_type'] == 2
           ? esc_html__('Subscription Id', 'gravityforms')
           : esc_html__('Transaction Id', 'gravityforms'); ?>:
							<span id='gform_payment_transaction_id'><?php echo $transaction_id;
         // May contain HTML
         ?></span>
						</div>
						<?php }
     ...
     if (!rgblank($payment_amount)) { ?>
           ...
           : esc_html__('Amount', 'gravityforms'); ?>:
							<span id='gform_payment_amount'><?php echo $payment_amount;
         // May contain HTML
         ?></span>
						</div>
						<?php }
    ...
    public static function meta_box_entry_info($args, $metabox)
    {
        ...
        'Entry Id',
        'gravityforms'
    ); ?>: <?php echo absint($entry['id']); ?><br /><br />
				<?php esc_html_e(
        'Submitted on',
        'gravityforms'
    ); ?>: <?php echo esc_html(GFCommon::format_date($entry['date_created'], false, 'Y/m/d')); ?>
				<br /><br />
				<?php
    if (
        !empty($entry['date_updated']) &&
        $entry['date_updated'] != $entry['date_created']
    ) {
        esc_html_e('Updated', 'gravityforms'); ?>: <?php
echo esc_html(GFCommon::format_date($entry['date_updated'], false, 'Y/m/d'));
echo '<br /><br />';

    }

    if (!empty($entry['ip'])) {
        esc_html_e('User IP', 'gravityforms'); ?>: <?php
...
echo '<br /><br />';
    ...
    if (
        ...
        ($usermeta = get_userdata($entry['created_by']))
    ) { ?>
					<?php esc_html_e('User', 'gravityforms'); ?>:
					<a href="user-edit.php?user_id=<?php echo absint(
         $entry['created_by']
     ); ?>"><?php echo esc_html($usermeta->user_login); ?></a>
					<br /><br />
					<?php }
    ...
    ?>:
        ...
        $entry['source_url']
    ); ?>" target="_blank">.../<?php echo esc_html(GFCommon::truncate_url($entry['source_url'])); ?></a>
				<br /><br />
				<?php
    if (!empty($entry['post_id'])) {
        $post = get_post($entry['post_id']); ?>
					<?php esc_html_e('Edit Post', 'gravityforms'); ?>:
					<a href="post.php?action=edit&post=<?php echo absint(
         $post->ID
    ...
    ?>
			...
			<div id="major-publishing-actions">
				<div id="delete-action">
					<?php switch ($entry['status']) { case 'spam': ?>
							<a onclick="jQuery('#action').val('unspam'); jQuery('#entry_form').submit()" href="#"><?php esc_html_e(
           'Not Spam',
           'gravityforms'
       ); ?></a>
							<?php
       ...
       if (GFCommon::current_user_can_any('gravityforms_delete_entries')) { ?>
								<a class="submitdelete deletion" onclick="if ( confirm('<?php echo esc_js(
            __(
                "You are about to delete this entry. 'Cancel' to stop, 'OK' to delete.",
                'gravityforms'
            )
        ); ?>') ) {jQuery('#action').val('delete'); jQuery('#entry_form').submit(); return true;} return false;" href="#"><?php esc_html_e(
    'Delete Permanently',
    'gravityforms'
); ?></a>
								<?php }
       ...
       case 'trash':
             if (
                 GFCommon::current_user_can_any('gravityforms_delete_entries')
             ) { ?>
								<a onclick="jQuery('#action').val('restore'); jQuery('#entry_form').submit()" href="#"><?php esc_html_e(
            'Restore',
            'gravityforms'
        ); ?></a>
								|
    ...
    'gravityforms'
         ...
         default:
             if (
                 GFCommon::current_user_can_any('gravityforms_delete_entries')
								...
								<a class="submitdelete deletion" onclick="jQuery('#action').val('trash'); jQuery('#entry_form').submit()" href="#"><?php esc_html_e(
            'Move to Trash',
            'gravityforms'
        ); ?></a>
								<?php echo GFCommon::spam_enabled($form['id']) ? '|' : '';}
             if (GFCommon::spam_enabled($form['id'])) { ?>
								<a class="submitdelete deletion" onclick="jQuery('#action').val('spam'); jQuery('#entry_form').submit()" href="#"><?php esc_html_e(
            'Mark as Spam',
            'gravityforms'
        ); ?></a>
								<?php }
         ...
         $button_click =
             $mode == 'view'
                 ? "jQuery('#screen_mode').val('edit');"
                 : "jQuery('#action').val('update'); jQuery('#screen_mode').val('view');";
         $update_button =
             '<input id="' .
             $update_button_id .
             '" ' .
             $disabled .
             ' class="button button-large button-primary" type="submit" tabindex="4" value="' .
         ...
         if ($mode == 'edit') {
             echo '&nbsp;&nbsp;<input class="button button-large" type="submit" tabindex="5" value="' .
                 esc_attr__('Cancel', 'gravityforms') .
                 '" name="cancel" onclick="jQuery(\'#screen_mode\').val(\'view\');"/>';
         }
     } ?>
				</div>
				<div class="clear"></div>
   ...
   if (!is_array($notifications) || count($form['notifications']) <= 0) { ?>
        ...
        'You cannot resend notifications for this entry because this form does not currently have any notifications configured.',
        'gravityforms'
    ); ?></p>

				<a href="<?php echo admin_url(
        "admin.php?page=gf_edit_forms&view=settings&subview=notification&id={$form_id}"
    ); ?>" class="button"><?php esc_html_e(
    'Configure Notifications',
    'gravityforms'
); ?></a>
				<?php } else {foreach ($notifications as $notification) { ?>
					<input type="checkbox" class="gform_notifications" value="<?php echo esc_attr(
         $notification['id']
     ); ?>" id="notification_<?php echo esc_attr(
    $notification['id']
); ?>" onclick="toggleNotificationOverride();" />
					<label for="notification_<?php echo esc_attr(
         $notification['id']
     ); ?>"><?php echo esc_html($notification['name']); ?></label>
					<br /><br />
					<?php } ?>
				...
				<div id="notifications_override_settings" style="display:none;">
					...
					<label for="notification_override_email"><?php esc_html_e(
         'Send To',
         'gravityforms'
     ); ?> <?php gform_tooltip('notification_override_email'); ?></label><br />
					<input type="text" name="notification_override_email" id="notification_override_email" style="width:99%;" />
					<br /><br />

				</div>

				<input type="button" name="notification_resend" value="<?php esc_attr_e(
        'Resend Notifications',
        'gravityforms'
    ); ?>" class="button" style="" onclick="ResendNotifications();" />
				<span id="please_wait_container" style="display:none; margin-left: 5px;">
							<i class='gficon-gravityforms-spinner-icon gficon-spin'></i> <?php esc_html_e(
           'Resending...',
           'gravityforms'
       ); ?>
                        </span>
     ...
     * @return bool
     */
    public static function maybe_display_empty_fields(
        $allow_display_empty_fields,
        $form,
        $lead = false
    ) {
        ...
        if ($allow_display_empty_fields) {
            $display_empty_fields = (bool) rgget(
                'gf_display_empty_fields',
                $_COOKIE
            );
        }
