...
class GFSettings
{
    ...
    public static function settings_uninstall_page()
    {
        ...
        if (isset($_POST['uninstall'])) {
            ...
            if (!GFCommon::current_user_can_uninstall()) {
                die(
                    esc_html__(
                        "You don't have adequate permission to uninstall Gravity Forms.",
                        'gravityforms'
                    )
                );
            }
            ...
            $processors = [GFForms::$background_upgrader, gf_feed_processor()];
            ...
            update_option(
                'recently_activated',
                [$plugin => time()] + (array) get_option('recently_activated')
            );
            ?>
			<div class="updated fade" style="padding:20px;"><?php echo sprintf(
       esc_html__(
           'Gravity Forms has been successfully uninstalled. It can be re-activated from the %splugins page%s.',
           'gravityforms'
       ),
       "<a href='plugins.php'>",
       '</a>'
       ...
       wp_nonce_field('gform_uninstall', 'gform_uninstall_nonce'); ?>
				<h3><span><i class="fa fa-times"></i> <?php esc_html_e(
        ...
        'gravityforms'
          ...
          'gravityforms'
					...
					<div class="gf_delete_notice"><strong><?php esc_html_e(
         'This operation deletes ALL Gravity Forms data.',
         'gravityforms'
     ); ?></strong> <?php esc_html_e(
    'If you continue, you will not be able to retrieve or restore your forms or entries.',
    'gravityforms'
); ?>
				</div>
    ...
    $uninstall_button =
        ...
        esc_js(
            __(
                "Warning! ALL Gravity Forms data, including form entries will be deleted. This cannot be undone. 'OK' to delete, 'Cancel' to stop",
                'gravityforms'
            )
        ) .
    ...
    public static function gravityforms_settings_page()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!GFCommon::current_user_can_any('gravityforms_edit_settings')) {
                die(
                    esc_html__(
                        "You don't have adequate permission to edit settings.",
                        'gravityforms'
                    )
                );
            }
            ...
            update_option(
                ...
                (bool) rgpost('gforms_disable_css')
            );
            update_option(
                'rg_gforms_enable_html5',
                (bool) rgpost('gforms_enable_html5')
            );
            update_option(
                'gform_enable_noconflict',
                (bool) rgpost('gform_enable_noconflict')
            );
            update_option(
                'gform_enable_background_updates',
                (bool) rgpost('gform_enable_background_updates')
            );
            update_option(
                'gform_enable_toolbar_menu',
                (bool) rgpost('gform_enable_toolbar_menu')
            );
            update_option(
                'rg_gforms_enable_akismet',
                self::get_posted_akismet_setting()
            ); // do not cast to bool, option is enabled by default; need a "1" or a "0"
            update_option(
                'rg_gforms_captcha_public_key',
                sanitize_text_field(rgpost('gforms_captcha_public_key'))
            );
            update_option(
                'rg_gforms_captcha_private_key',
                sanitize_text_field(rgpost('gforms_captcha_private_key'))
            );
            update_option(
                'rg_gforms_captcha_type',
                sanitize_text_field(rgpost('gforms_captcha_type'))
            );
            ...
            if (rgpost('gform_recaptcha_reset')) {
                ...
                if ($site && $secret && $response) {
                    ...
                    $recaptcha_response = $recaptcha->verify_recaptcha_response(
                        $response,
                        $secret
                    );
                    GFCommon::log_debug(
            ...
            if (
                !rgempty('gforms_currency') &&
                in_array(
                    rgpost('gforms_currency'),
                    array_keys(RGCurrency::get_currencies())
                )
            ) {
       ...
       'gravityforms'
			...
			<table class="form-table">
				<tr valign="top">
					<th scope="row">
						<label for="gforms_key"><?php esc_html_e(
          'Support License Key',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_license_key'); ?>
					</th>
					<td>
						<?php
      $key = GFCommon::get_key();
      $key_field =
          '<input type="password" name="gforms_key" id="gforms_key" style="width:350px;" value="' .
          $key .
          '" autocomplete="off" />';
      if (!rgempty('is_error', $version_info)) {
          $key_field .=
              ...
              "/images/exclamation.png' class='gf_keystatus_error gf_tooltip' alt='There was an error validating your key' title='<h6>" .
              esc_attr__('Validation Error', 'gravityforms') .
              '</h6>' .
              esc_attr__(
                  'There was an error while validating your license key. Gravity Forms will continue to work, but automatic upgrades will not be available. Please contact support to resolve this issue.',
                  'gravityforms'
              ) .
              "'/>";
      } elseif (rgar($version_info, 'is_valid_key')) {
          $key_field .=
              "&nbsp;<i class='fa fa-check gf_keystatus_valid'></i> <span class='gf_keystatus_valid_text'>" .
              esc_html__(
                  'Valid Key : Your license key has been successfully validated.',
                  'gravityforms'
              ) .
              '</span>';
      } elseif (!empty($key)) {
          $key_field .=
              "&nbsp;<i class='fa fa-times gf_keystatus_invalid'></i> <span class='gf_keystatus_invalid_text'>" .
              esc_html__(
      ...
      ?>
						<br />
						<span class="gf_settings_description"><?php esc_html_e(
          ...
          'gravityforms'
				...
				<tr valign="top">
					<th scope="row">
						<label for="gforms_disable_css"><?php esc_html_e(
          'Output CSS',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_output_css'); ?>
					</th>
					<td>
						<input type="radio" name="gforms_disable_css" value="0" id="gforms_css_output_enabled" <?php echo get_option(
          'rg_gforms_disable_css'
      ) == 1
          ? ''
          : "checked='checked'"; ?> /> <?php esc_html_e(
     'Yes',
     'gravityforms'
 ); ?>&nbsp;&nbsp;
						<input type="radio" name="gforms_disable_css" value="1" id="gforms_css_output_disabled" <?php echo get_option(
          'rg_gforms_disable_css'
      ) == 1
          ? "checked='checked'"
          : ''; ?> /> <?php esc_html_e('No', 'gravityforms'); ?>
						<br />
						<span class="gf_settings_description"><?php esc_html_e(
          'Set this to No if you would like to disable the plugin from outputting the form CSS.',
          'gravityforms'
      ); ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row">
						<label for="gforms_enable_html5"><?php esc_html_e(
          'Output HTML5',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_html5'); ?>
					</th>
					<td>
						<input type="radio" name="gforms_enable_html5" value="1" <?php echo get_option(
          'rg_gforms_enable_html5'
      ) == 1
          ? "checked='checked'"
          : ''; ?> id="gforms_enable_html5" /> <?php esc_html_e(
     'Yes',
     'gravityforms'
 ); ?>&nbsp;&nbsp;
						<input type="radio" name="gforms_enable_html5" value="0" <?php echo get_option(
          'rg_gforms_enable_html5'
      ) == 1
          ? ''
          : "checked='checked'"; ?> /> <?php esc_html_e(
     ...
     'gravityforms'
						...
						<br />
						<span class="gf_settings_description"><?php esc_html_e(
          'Set this to No if you would like to disable the plugin from outputting HTML5 form fields.',
          'gravityforms'
      ); ?></span>
					</td>
				</tr>
				...
				<tr valign="top">
					<th scope="row">
						<label for="gform_enable_noconflict"><?php esc_html_e(
          'No-Conflict Mode',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_noconflict'); ?>
					</th>
					<td>
						<input type="radio" name="gform_enable_noconflict" value="1" <?php echo get_option(
          'gform_enable_noconflict'
      ) == 1
          ? "checked='checked'"
          : ''; ?> id="gform_enable_noconflict" /> <?php esc_html_e(
     'On',
     'gravityforms'
 ); ?>&nbsp;&nbsp;
						<input type="radio" name="gform_enable_noconflict" value="0" <?php echo get_option(
          'gform_enable_noconflict'
      ) == 1
          ? ''
          : "checked='checked'"; ?> id="gform_disable_noconflict" /> <?php esc_html_e(
     'Off',
     'gravityforms'
 ); ?>
						<br />
          ...
          'gravityforms'
				...
				<?php if (GFCommon::has_akismet()) { ?>
					<tr valign="top">
						<th scope="row">
							<label for="gforms_enable_akismet"><?php esc_html_e(
           'Akismet Integration',
           'gravityforms'
       ); ?></label>  <?php gform_tooltip('settings_akismet'); ?>
						</th>
						<td>
							<?php
       $akismet_setting = get_option('rg_gforms_enable_akismet');
       $is_akismet_enabled =
           $akismet_setting === false || !empty($akismet_setting);

        //Akismet is enabled by default.
        ?>
							<input type="radio" name="gforms_enable_akismet" value="1" <?php checked(
           $is_akismet_enabled,
           true
       ); ?> id="gforms_enable_akismet" /> <?php esc_html_e(
     'Yes',
     'gravityforms'
 ); ?>&nbsp;&nbsp;
							<input type="radio" name="gforms_enable_akismet" value="0" <?php checked(
           $is_akismet_enabled,
           false
       ); ?> /> <?php esc_html_e('No', 'gravityforms'); ?>
							<br />
           ...
           'gravityforms'
				...
				<tr valign="top">
					<th scope="row">
						<label for="gforms_currency"><?php esc_html_e(
          'Currency',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_currency'); ?>
					</th>
					<td>
						<?php $disabled = apply_filters('gform_currency_disabled', false)
          ? "disabled='disabled'"
          : ''; ?>
       ...
       foreach (RGCurrency::get_currencies() as $code => $currency) { ?>
								<option value="<?php echo esc_attr($code); ?>" <?php echo $current_currency ==
$code
    ? "selected='selected'"
    : ''; ?>><?php echo esc_html($currency['name']); ?></option>
       ...
       ?>
				...
				<tr valign="top">
					<th scope="row">
						<label for="gform_enable_background_updates"><?php esc_html_e(
          'Background updates',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_background_updates'); ?>
					</th>
					<td>
						<input type="radio" name="gform_enable_background_updates" value="1" <?php echo get_option(
          'gform_enable_background_updates'
      ) == 1
          ? "checked='checked'"
          : ''; ?> id="gform_enable_background_updates" /> <?php esc_html_e(
     'On',
     'gravityforms'
 ); ?>&nbsp;&nbsp;
						<input type="radio" name="gform_enable_background_updates" value="0" <?php echo get_option(
          'gform_enable_background_updates'
      ) == 1
          ? ''
          : "checked='checked'"; ?> id="gform_disable_background_updates" /> <?php esc_html_e(
     'Off',
     'gravityforms'
 ); ?>
						<br />
          ...
          'gravityforms'
				...
				<tr valign="top">
					<th scope="row">
						<label for="gform_toolbar_menu"><?php esc_html_e(
          'Toolbar Menu',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_toolbar_menu'); ?>
					</th>
					<td>
						<input type="radio" name="gform_enable_toolbar_menu" value="1" <?php echo get_option(
          'gform_enable_toolbar_menu'
      ) == 1
          ? "checked='checked'"
          : ''; ?> id="gform_enable_toolbar_menu" /> <?php esc_html_e(
     'On',
     'gravityforms'
 ); ?>&nbsp;&nbsp;
						<input type="radio" name="gform_enable_toolbar_menu" value="0" <?php echo get_option(
          'gform_enable_toolbar_menu'
      ) == 1
          ? ''
          : "checked='checked'"; ?> id="gform_disable_toolbar_menu" /> <?php esc_html_e(
     'Off',
     'gravityforms'
 ); ?>
						<br />
          ...
          'gravityforms'
				...
				<?php if (!GFCommon::is_logging_plugin_active()) { ?>
					<tr valign="top">
						<th scope="row">
							<label for="gform_toolbar_menu"><?php esc_html_e(
           'Logging',
           'gravityforms'
       ); ?></label> <?php gform_tooltip('settings_logging'); ?>
						</th>
						<td>
							<input type="radio" name="gform_enable_logging" value="1" <?php echo get_option(
           'gform_enable_logging'
       ) == 1
           ? "checked='checked'"
           : ''; ?> id="gform_enable_logging" /> <?php esc_html_e(
     'On',
     'gravityforms'
 ); ?>&nbsp;&nbsp;
							<input type="radio" name="gform_enable_logging" value="0" <?php echo get_option(
           'gform_enable_logging'
       ) == 1
           ? ''
           : "checked='checked'"; ?> id="gform_disable_logging" /> <?php esc_html_e(
     'Off',
     'gravityforms'
 ); ?>
							<br />
       ...
       'gravityforms'
			...
			<p style="text-align: left;">
				<?php esc_html_e(
        'Gravity Forms integrates with reCAPTCHA, a free CAPTCHA service that uses an advanced risk analysis engine and adaptive challenges to keep automated software from engaging in abusive activities on your site. ',
        'gravityforms'
    ); ?>
				<strong><?php esc_html_e(
    ...
    'gravityforms'
				...
				<a href="http://www.google.com/recaptcha/" target="_blank"><?php esc_html_e(
        ...
        'gravityforms'
			...
			<table class="form-table">
				...
				<tr valign="top">
					<th scope="row">
						<label for="gforms_captcha_public_key"><?php esc_html_e(
          'Site Key',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_recaptcha_public'); ?>
					</th>
					<td>
						<input type="text" name="gforms_captcha_public_key" style="width:350px;" value="<?php echo esc_attr(
          get_option('rg_gforms_captcha_public_key')
      ); ?>" onchange="loadRecaptcha();" />
				...
				<tr valign="top">
					<th scope="row">
						<label for="gforms_captcha_private_key"><?php esc_html_e(
          'Secret Key',
          'gravityforms'
      ); ?></label>  <?php gform_tooltip('settings_recaptcha_private'); ?>
					</th>
					<td>
						<?php $recaptcha_private_key = get_option('rg_gforms_captcha_private_key'); ?>
						<input type="text" name="gforms_captcha_private_key" style="width:350px;" value="<?php esc_attr_e(
          $recaptcha_private_key
				...
				<tr valign="top">
					<th scope="row">
						<?php esc_html_e('Type', 'gravityforms'); ?>  <?php gform_tooltip(
      ...
      ?>
						...
						/>
						<label for="gforms_captcha_type_checkbox"><?php esc_html_e(
          'Checkbox',
          'gravityforms'
      ); ?></label>&nbsp;&nbsp;
						<input
						...
						/><label for="gforms_captcha_type_invisible"><?php esc_html_e(
          'Invisible',
          'gravityforms'
      ); ?></label>
					</td>
				...
				<tr valign="top" id="gforms_confirm_recaptcha" style="display:none;">
					<th scope="row">
						<label id="gforms_validate_recaptcha_label" for="gforms_validate_recaptcha"><?php esc_html_e(
          'Validate Keys',
          'gravityforms'
      ); ?></label> <?php gform_tooltip('gforms_validate_recaptcha'); ?>
					</th>
					<td>

						<p id="gforms_checkbox_recaptcha_message" style="margin-bottom:10px;"><?php esc_html_e(
     ...
     $save_button =
         ...
         esc_html__('Save Settings', 'gravityforms') .
     ...
     ?>
					<span id="gform_spinner" style="display:none;margin-left:10px;"><img src="<?php echo GFCommon::get_base_url() .
    ...
    public static function upgrade_license()
    {
        ...
        $raw_response = GFCommon::post_to_manager(
            'api.php',
            'op=upgrade_message&key=' . GFCommon::get_key(),
            $options
        );
    ...
    public static function page_header($title = '', $message = '')
    {
		...
		<div class="wrap <?php echo GFCommon::get_browser_class(); ?> gforms_settings_wrap">
         ...
         $url = add_query_arg(
             ['subview' => $tab['name']],
             admin_url('admin.php?page=gf_settings')
         );
         ?>
        ...
        if (function_exists('gf_logging')) {
            ...
            self::add_settings_page(
                [
                    ...
                    'handler' => [gf_logging(), 'plugin_settings_page'],
                ],
                null,
                null
            );
