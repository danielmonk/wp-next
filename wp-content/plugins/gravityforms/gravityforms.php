...
define(
    ...
    version_compare(get_bloginfo('version'), GF_MIN_WP_VERSION, '>=')
);
...
add_action('init', ['GFForms', 'init']);
add_action('wp', ['GFForms', 'maybe_process_form'], 9);
add_action('admin_init', ['GFForms', 'maybe_process_form'], 9);
add_action('wp', ['GFForms', 'process_exterior_pages']);
add_action('admin_init', ['GFForms', 'process_exterior_pages']);
add_action('wp_ajax_update_auto_update_setting', [
    'GFForms',
    'update_auto_update_setting',
]);
add_filter('upgrader_pre_install', ['GFForms', 'validate_upgrade'], 10, 2);
add_filter('tiny_mce_before_init', ['GFForms', 'modify_tiny_mce_4'], 20);
add_filter('user_has_cap', ['RGForms', 'user_has_cap'], 10, 3);
add_filter('query', ['GFForms', 'filter_query']);
add_filter(
    'plugin_auto_update_setting_html',
    ['GFForms', 'auto_update_message'],
    10,
    3
);
add_filter(
    'plugin_auto_update_debug_string',
    ['GFForms', 'auto_update_debug_message'],
    10,
    4
);
...
if (
    is_admin() &&
    (GFForms::is_gravity_page() || GFForms::is_gravity_ajax_action())
) {
    add_action(
        'wp_print_scripts',
        ['GFForms', 'no_conflict_mode_script'],
        1000
    );
    add_action(
    ...
    add_action('wp_print_styles', ['GFForms', 'no_conflict_mode_style'], 1000);
    add_action('admin_print_styles', ['GFForms', 'no_conflict_mode_style'], 1);
    add_action(
        'admin_print_footer_scripts',
        ['GFForms', 'no_conflict_mode_style'],
        1
    );
    add_action('admin_footer', ['GFForms', 'no_conflict_mode_style'], 1);
}

add_action('plugins_loaded', ['GFForms', 'loaded']);

register_activation_hook(__FILE__, ['GFForms', 'activation_hook']);
register_deactivation_hook(__FILE__, ['GFForms', 'deactivation_hook']);
...
class GFForms
{
    ...
    public static function has_members_plugin($deprecated = null)
    {
        return function_exists('members_register_cap_group') &&
            function_exists('members_register_cap');
    }
    ...
    public static function init()
    {
        ...
        add_action('gravityforms_cron', ['GFForms', 'cron']);
        ...
        add_filter('gform_logging_supported', [
            'GFForms',
            'set_logging_supported',
        ]);
        add_action('admin_head', ['GFCommon', 'maybe_output_gf_vars']);
        add_action('admin_head', ['GFForms', 'load_admin_bar_styles']);
        add_action('wp_head', ['GFForms', 'load_admin_bar_styles']);
        ...
        if (function_exists('register_block_type')) {
            ...
            if (!class_exists('GF_Blocks')) {
                require_once plugin_dir_path(__FILE__) .
                    'includes/blocks/class-gf-blocks.php';
            }

            // Load included Blocks.
            GFCommon::glob_require_once(
                '/includes/blocks/class-gf-block-*.php'
            );
        }

        // Plugin update actions
        add_filter('transient_update_plugins', ['GFForms', 'check_update']);
        ...
        if (IS_ADMIN) {
            ...
            add_filter(
                ...
                ['GFForms', 'filter_ure_custom_capability_groups'],
                10,
                2
            );
            ...
            if (GFCommon::current_user_can_any(GFCommon::all_caps())) {
                ...
                add_action('admin_menu', ['GFForms', 'create_menu']);
                ...
                if (GF_SUPPORTED_WP_VERSION) {
                    ...
                    require_once GFCommon::get_base_path() .
                        '/includes/locking/locking.php';
                    ...
                    if (self::is_gravity_page()) {
                        ...
                        in_array(RG_CURRENT_PAGE, [
                            'admin.php',
                            'admin-ajax.php',
                        ])
                    ) {
                        add_action('wp_ajax_rg_save_form', [
                            'GFForms',
                            'save_form',
                        ]);
                        add_action('wp_ajax_rg_change_input_type', [
                            'GFForms',
                            'change_input_type',
                        ]);
                        add_action('wp_ajax_rg_refresh_field_preview', [
                            'GFForms',
                            'refresh_field_preview',
                        ]);
                        add_action('wp_ajax_rg_add_field', [
                            'GFForms',
                            'add_field',
                        ]);
                        add_action('wp_ajax_rg_duplicate_field', [
                            'GFForms',
                            'duplicate_field',
                        ]);
                        ...
                        add_action('wp_ajax_rg_select_export_form', [
                            'GFForms',
                            'select_export_form',
                        ]);
                        add_action('wp_ajax_rg_start_export', [
                            'GFForms',
                            'start_export',
                        ]);
                        add_action('wp_ajax_gf_upgrade_license', [
                            'GFForms',
                            'upgrade_license',
                        ]);
                        add_action('wp_ajax_gf_delete_custom_choice', [
                            'GFForms',
                        ...
                        add_action('wp_ajax_gf_get_post_categories', [
                            'GFForms',
                            'get_post_category_values',
                        ]);
                        add_action(
                            'wp_ajax_gf_get_address_rule_values_select',
                            ['GFForms', 'get_address_rule_values_select']
                        );
                        add_action(
                            'wp_ajax_gf_get_notification_post_categories',
                            ['GFForms', 'get_notification_post_category_values']
                        );
                        //add_action( 'wp_ajax_gf_save_confirmation', array( 'GFForms', 'save_confirmation' ) );
                        add_action('wp_ajax_gf_delete_confirmation', [
                            'GFForms',
                            'delete_confirmation',
                        ]);
                        add_action('wp_ajax_gf_save_new_form', [
                            'GFForms',
                            'save_new_form',
                        ]);
                        add_action('wp_ajax_gf_save_title', [
                            'GFForms',
                            'save_form_title',
                        ]);

                        //entry list ajax operations
                        add_action('wp_ajax_rg_update_lead_property', [
                            'GFForms',
                            'update_lead_property',
                        ]);
                        add_action('wp_ajax_delete-gf_entry', [
                            'GFForms',
                            'update_lead_status',
                        ]);

                        //form list ajax operations
                        add_action('wp_ajax_rg_update_form_active', [
                            'GFForms',
                            'update_form_active',
                        ]);

                        //notification list ajax operations
                        add_action('wp_ajax_rg_update_notification_active', [
                            'GFForms',
                            'update_notification_active',
                        ]);

                        //confirmation list ajax operations
                        add_action('wp_ajax_rg_update_confirmation_active', [
                            'GFForms',
                            'update_confirmation_active',
                        ]);

                        //dynamic captcha image
                        add_action('wp_ajax_rg_captcha_image', [
                            'GFForms',
                            'captcha_image',
                        ]);

                        //dashboard message "dismiss upgrade" link
                        add_action('wp_ajax_rg_dismiss_upgrade', [
                            'GFForms',
                            'dashboard_dismiss_upgrade',
                        ]);

                        // entry detail: resend notifications
                        add_action('wp_ajax_gf_resend_notifications', [
                            'GFForms',
                            'resend_notifications',
                        ]);

                        // Shortcode UI
                        add_action('wp_ajax_gf_do_shortcode', [
                            'GFForms',
                            'handle_ajax_do_shortcode',
                        ]);

                        // Export
                        add_filter('wp_ajax_gf_process_export', [
                            'GFForms',
                            'ajax_process_export',
                        ]);
                        add_filter('wp_ajax_gf_download_export', [
                            'GFForms',
                            'ajax_download_export',
                        ]);

                        // Dismiss message
                        add_action('wp_ajax_gf_dismiss_message', [
                            'GFForms',
                            'ajax_dismiss_message',
                        ]);

                        // Check background tasks for the system report
                        add_action('wp_ajax_gf_check_background_tasks', [
                            'GFForms',
                            'check_background_tasks',
                        ]);

                        // Check status of upgrade
                        add_action('wp_ajax_gf_force_upgrade', [
                            'GFForms',
                            'ajax_force_upgrade',
                        ]);

                        // Disable logging.
                        add_action('wp_ajax_gf_disable_logging', [
                            'GFForms',
                            'ajax_disable_logging',
                        ]);

                        // Get change log.
                        add_action('wp_ajax_gf_get_changelog', [
                            'GFForms',
                    ...
                    add_action(
                        'in_plugin_update_message-gravityforms/gravityforms.php',
                        ['GFForms', 'in_plugin_update_message'],
                        10,
                        2
                    );
                    add_action(
                    ...
                    add_filter(
                        'plugin_action_links',
                        ['GFForms', 'plugin_settings_link'],
                        10,
                        2
                    );
                }
            }
            add_action('admin_init', ['GFForms', 'ajax_parse_request'], 10);
            ...
            if ($gf_page == 'entry_list' && !isset($_GET['filter'])) {
                ...
                if ($default_filter !== 'all') {
                    $url = add_query_arg(['filter' => $default_filter]);
                    $url = esc_url_raw($url);
            ...
            if ($gf_page == 'entry_list') {
                ...
                add_filter(
                    'set-screen-option',
                    ['GFForms', 'set_screen_options'],
                    10,
                    3
                );
                // For WP 5.4.2+.
                add_filter(
                    'set_screen_option_gform_entries_screen_options',
                    ['GFForms', 'set_screen_options'],
                    10,
                    3
                );
            ...
            if ($gf_page == 'form_list') {
                ...
                // For WP 5.4.2+.
                add_filter(
                    'set_screen_option_gform_forms_per_page',
                    ['GFForms', 'set_screen_options'],
                    10,
                    3
                );
            ...
            add_filter(
                ...
                ['GFForms', 'register_data_exporter'],
                10
            );
            add_filter(
                'wp_privacy_personal_data_erasers',
                ['GFForms', 'register_data_eraser'],
                10
            );
        } else {
            add_action(
            ...
            add_action('wp', ['GFForms', 'ajax_parse_request'], 10);
        }

        // Add admin bar items
        add_action('wp_before_admin_bar_render', ['GFForms', 'admin_bar']);

        add_shortcode('gravityform', ['GFForms', 'parse_shortcode']);
        add_shortcode('gravityforms', ['GFForms', 'parse_shortcode']);

        // ManageWP premium update filters
        add_filter('mwp_premium_update_notification', [
            'GFForms',
            'premium_update_push',
        ]);
        add_filter('mwp_premium_perform_update', ['GFForms', 'premium_update']);

        // Push Gravity Forms to the top of the list of plugins to make sure it's loaded before any add-ons
        add_action('activated_plugin', ['GFForms', 'load_first']);
        ...
        if (GFForms::page_supports_add_form_button()) {
            ...
            add_action('media_buttons', ['GFForms', 'add_form_button'], 20);
            // Adding the modal
    ...
    public static function load_first()
    {
        ...
        $active_plugins = array_values(
            maybe_unserialize(self::get_wp_option('active_plugins'))
        );
    ...
    public static function process_exterior_pages()
    {
        ...
        $is_legacy_upload_page =
            $_SERVER['REQUEST_METHOD'] == 'POST' && $page == 'upload';

        if ($is_legacy_upload_page && (!defined('DOING_AJAX') || !DOING_AJAX)) {
            _doing_it_wrong(
                'gf_page=upload',
                'gf_page=upload is now deprecated. Use GFCommon::get_upload_page_slug() instead',
                '1.9.6.13'
            );
        }
        ...
        $is_upload_page =
            ...
            $page == GFCommon::get_upload_page_slug();
    ...
    private static function rename_suspicious_files_recursive($dir)
    {
        ...
        while (false !== ($file = readdir($dir_handle))) {
            if (
                ...
                GFCommon::file_name_has_disallowed_extension($file) &&
                !GFCommon::match_file_extension($file, [
                    'htaccess',
                    'bak',
                    'html',
                ])
    ...
    private static function heal_wp_upload_dir()
    {
        ...
        foreach (
            glob($wp_upload_path . DIRECTORY_SEPARATOR . '*_input_*.php')
            as $filename
        ) {
    ...
    public static function no_conflict_mode_style()
    {
        ...
        $gf_required_styles = [
            'common' => ['gform_tooltip', 'gform_font_awesome', 'gform_admin'],
            'gf_edit_forms' => [
                'thickbox',
                'editor-buttons',
                'wp-jquery-ui-dialog',
            ...
            'gf_new_form' => ['thickbox'],
            'gf_entries' => ['thickbox', 'gform_chosen'],
            'gf_settings' => [],
            'gf_export' => [],
            'gf_help' => [],
            'gf_system_status' => ['thickbox'],
        ];

        self::no_conflict_mode(
            $wp_styles,
            $wp_required_styles,
            $gf_required_styles,
            'styles'
    ...
    public static function no_conflict_mode_script()
    {
        ...
        $gf_required_scripts = [
            'common' => ['gform_tooltip_init', 'sack'],
            'gf_edit_forms' => [
                'backbone',
                'editor',
                'gform_floatmenu',
     ...
     */
    private static function no_conflict_mode(
        &$wp_objects,
        $wp_required_objects,
        $gf_required_objects,
        $type = 'scripts'
        ...
        $page_objects = isset($gf_required_objects[$current_page . '_' . $view])
            ? $gf_required_objects[$current_page . '_' . $view]
            : rgar($gf_required_objects, $current_page);
        ...
        $required_objects = array_merge(
            $wp_required_objects,
            $gf_required_objects['common'],
            $page_objects
        );

        //allowing addons or other products to change the list of no conflict scripts
        $required_objects = apply_filters(
            "gform_noconflict_{$type}",
            $required_objects
        );
        ...
        $required_objects = self::add_script_dependencies(
            $wp_objects->registered,
            $required_objects
        );
        ...
        foreach (
            $wp_objects->registered
            as $script_name => $script_registration
        ) {
    ...
    private static function add_script_dependencies($registered, $scripts)
    {
        ...
        do {
            ...
            foreach ($scripts as $script) {
                $deps =
                    isset($registered[$script]) &&
                    is_array($registered[$script]->deps)
                        ? $registered[$script]->deps
                        : [];
                foreach ($deps as $dep) {
                    if (
                        !in_array($dep, $scripts) &&
                        !in_array($dep, $dependents)
                    ) {
                        $dependents[] = $dep;
                    }
                }
    ...
    public static function premium_update_push($premium_update)
    {
        ...
        if (
            rgar($update, 'is_valid_key') == true &&
            version_compare(GFCommon::$version, $update['version'], '<')
        ) {
            $gforms = get_plugin_data(__FILE__);
            $gforms['type'] = 'plugin';
            $gforms['slug'] = 'gravityforms/gravityforms.php';
            $gforms['new_version'] = !rgempty('version', $update)
                ? $update['version']
                : false;
    ...
    {
        members_register_cap_group('gravityforms', [
    ...
    {
        $caps = [
            'gravityforms_create_form' => esc_html__(
                'Create Forms',
                'gravityforms'
            ),
            'gravityforms_delete_forms' => esc_html__(
                'Delete Forms',
                'gravityforms'
            ),
            'gravityforms_edit_forms' => esc_html__(
                'Edit Forms',
                'gravityforms'
            ),
            'gravityforms_preview_forms' => esc_html__(
                'Preview Forms',
                'gravityforms'
            ),
            'gravityforms_view_entries' => esc_html__(
                'View Entries',
                'gravityforms'
            ),
            'gravityforms_edit_entries' => esc_html__(
                'Edit Entries',
                'gravityforms'
            ),
            'gravityforms_delete_entries' => esc_html__(
                'Delete Entries',
                'gravityforms'
            ),
            'gravityforms_view_entry_notes' => esc_html__(
                'View Entry Notes',
                'gravityforms'
            ),
            'gravityforms_edit_entry_notes' => esc_html__(
                'Edit Entry Notes',
                'gravityforms'
            ),
            'gravityforms_export_entries' => esc_html__(
                'Import/Export',
                'gravityforms'
            ),
            'gravityforms_view_settings' => esc_html__(
                'View Plugin Settings',
                'gravityforms'
            ),
            'gravityforms_edit_settings' => esc_html__(
                'Edit Plugin Settings',
                'gravityforms'
            ),
            'gravityforms_view_updates' => esc_html__(
                'Manage Updates',
                'gravityforms'
            ),
            'gravityforms_view_addons' => esc_html__(
                'Manage Add-Ons',
                'gravityforms'
            ),
            'gravityforms_system_status' => esc_html__(
                'View System Status',
                'gravityforms'
            ),
            'gravityforms_uninstall' => esc_html__(
                'Uninstall Gravity Forms',
                'gravityforms'
            ),
            'gravityforms_logging' => esc_html__(
                'Logging Settings',
                'gravityforms'
            ),
            'gravityforms_api_settings' => esc_html__(
                'REST API Settings',
                'gravityforms'
            ),
        ];
     ...
     * @return array
     */
    public static function filter_ure_custom_capability_groups(
        $groups = [],
        $cap_id = ''
    ) {
    ...
    public static function is_gravity_ajax_action()
    {
        ...
        if (
            defined('DOING_AJAX') &&
            DOING_AJAX &&
            in_array($current_action, $gf_ajax_actions)
        ) {
            return true;
        }
    ...
    public static function create_menu()
    {
        ...
        $update_icon =
            GFCommon::has_update() && current_user_can('install_plugins')
                ? "<span title='" .
                    esc_attr(__('Update Available', 'gravityforms')) .
                    "' class='update-plugins count-1'><span class='update-count'>&#49;</span></span>"
                : '';

        $admin_icon = self::get_admin_icon_b64(
            GFForms::is_gravity_page() ? '#fff' : false
        );

        $forms_hook_suffix = add_menu_page(
            __('Forms', 'gravityforms'),
            __('Forms', 'gravityforms') . $update_icon,
        ...
        add_action('load-' . $forms_hook_suffix, [
            'GFForms',
            'load_screen_options',
        ]);
        ...
        add_submenu_page(
            $parent_menu['name'],
            __('New Form', 'gravityforms'),
            __('New Form', 'gravityforms'),
            $has_full_access ? 'gform_full_access' : 'gravityforms_create_form',
            'gf_new_form',
            ['GFForms', 'new_form']
        );

        $entries_hook_suffix = add_submenu_page(
            $parent_menu['name'],
            __('Entries', 'gravityforms'),
            __('Entries', 'gravityforms'),
            $has_full_access
                ? 'gform_full_access'
                : 'gravityforms_view_entries',
            'gf_entries',
            ['GFForms', 'all_leads_page']
        );

        add_action('load-' . $entries_hook_suffix, [
            'GFForms',
            'load_screen_options',
        ]);
        ...
        if (is_array($addon_menus)) {
            foreach ($addon_menus as $addon_menu) {
                add_submenu_page(
                    esc_html($parent_menu['name']),
                    esc_html($addon_menu['label']),
                    esc_html($addon_menu['label']),
                    $has_full_access
                        ? 'gform_full_access'
                        : $addon_menu['permission'],
                    esc_html($addon_menu['name']),
                    $addon_menu['callback']
                );
            }
        }
        ...
        add_submenu_page(
            ...
            'gf_settings',
            ['GFForms', 'settings_page']
        );

        add_submenu_page(
            $parent_menu['name'],
            __('Import/Export', 'gravityforms'),
            __('Import/Export', 'gravityforms'),
            $has_full_access
                ? 'gform_full_access'
        ...
        add_submenu_page(
            $parent_menu['name'],
            __('System Status', 'gravityforms'),
            __('System Status', 'gravityforms'),
            $has_full_access
                ? 'gform_full_access'
                : 'gravityforms_system_status',
            'gf_system_status',
            ['GFForms', 'system_status']
        );
    ...
    public static function get_admin_icon_b64($color = false)
    {
        ...
        if ($color) {
            $svg_xml =
                ...
                self::get_admin_icon_svg($color);
            $icon = sprintf(
                'data:image/svg+xml;base64,%s',
                base64_encode(sprintf($svg_xml, $color))
            );
        } else {
            $svg_b64 =
    ...
    public static function get_parent_menu($addon_menus)
    {
        if (GFCommon::current_user_can_any('gravityforms_edit_forms')) {
            $parent = [
                'name' => 'gf_edit_forms',
                'callback' => ['GFForms', 'forms'],
            ];
        } elseif (GFCommon::current_user_can_any('gravityforms_create_form')) {
            $parent = [
                'name' => 'gf_new_form',
                'callback' => ['GFForms', 'new_form'],
            ];
        } elseif (GFCommon::current_user_can_any('gravityforms_view_entries')) {
            $parent = [
                'name' => 'gf_entries',
                'callback' => ['GFForms', 'all_leads_page'],
            ];
        } elseif (is_array($addon_menus) && sizeof($addon_menus) > 0) {
            foreach ($addon_menus as $addon_menu) {
                if (GFCommon::current_user_can_any($addon_menu['permission'])) {
                    $parent = [
                        'name' => $addon_menu['name'],
                        'callback' => $addon_menu['callback'],
                    ];
                    break;
                }
            }
            ...
            $parent = [
                'name' => 'gf_settings',
                'callback' => ['GFForms', 'settings_page'],
            ];
        } elseif (
            ...
            $parent = [
                'name' => 'gf_export',
                'callback' => ['GFForms', 'export_page'],
            ];
        } elseif (GFCommon::current_user_can_any('gravityforms_view_addons')) {
            $parent = [
                'name' => 'gf_addons',
                'callback' => ['GFForms', 'addons_page'],
            ];
        } elseif (
            GFCommon::current_user_can_any('gravityforms_system_status')
        ) {
            $parent = [
                'name' => 'gf_system_status',
                'callback' => ['GFForms', 'system_status_page'],
            ];
        } elseif (GFCommon::current_user_can_any(GFCommon::all_caps())) {
            $parent = [
                'name' => 'gf_help',
                'callback' => ['GFForms', 'help_page'],
            ];
        }
    ...
    public static function modify_admin_title($admin_title, $title)
    {
        ...
        if ($page_title) {
            $admin_title = sprintf(
                __('%1$s &lsaquo; %2$s &#8212; WordPress', 'gravityforms'),
                esc_html($page_title),
                $admin_title
            );
        }
    ...
    public static function parse_shortcode($attributes, $content = null)
    {
        ...
        switch ($action) {
            case 'conditional':
                $shortcode_string = GFCommon::conditional_shortcode(
                    $attributes,
                    $content
                );
                break;
            ...
            default:
                ...
                $description =
                    strtolower($description) == 'false' ? false : true;
                $field_values = htmlspecialchars_decode($field_values);
                $field_values = str_replace(
                    ['&#038;', '&#091;', '&#093;'],
                    ['&', '[', ']'],
                    $field_values
                );
                ...
                $shortcode_string = self::get_form(
                    $id,
                    $title,
                    $description,
                    false,
         ...
         */
        $shortcode_string = apply_filters(
            "gform_shortcode_{$action}",
            $shortcode_string,
            $attributes,
            $content
        );
    ...
    {
        require_once GFCommon::get_base_path() .
            '/includes/class-gravity-api.php';
    }
    ...
    public static function get_ajax_form_response()
    {
        ...
        if (GFFormDisplay::is_submit_form_id_valid($form_id)) {
            $display_title =
                !isset($args['title']) || !empty($args['title']) ? true : false;
            $display_description =
                !isset($args['description']) || !empty($args['description'])
                    ? true
                    : false;
            $tabindex = isset($args['tabindex'])
                ? absint($args['tabindex'])
                : 0;

            parse_str(rgpost('gform_field_values'), $field_values);

            $result = GFFormDisplay::get_form(
                $form_id,
                $display_title,
                $display_description,
                false,
            ...
            $result = GFFormDisplay::get_ajax_postback_html(
    ...
    {
        $is_post_edit_page = in_array(RG_CURRENT_PAGE, [
            'post.php',
            'page.php',
            'page-new.php',
            'post-new.php',
        ...
        $display_add_form_button = apply_filters(
    ...
    public static function add_form_button()
    {
        ...
        echo '<style>.gform_media_icon{
              ...
              <a href="#" class="button gform_media_link" id="add_gform" aria-label="' .
            esc_attr__('Add Gravity Form', 'gravityforms') .
            '"><div class="gform_media_icon svg" style="background-image: url(\'' .
            self::get_admin_icon_b64() .
            '\')"><br /></div><div style="padding-left: 20px;">' .
    ...
    public static function plugin_row($plugin_name, $plugin_data)
    {
        ...
        if (isset($add_ons[$plugin_name])) {
            ...
            if (version_compare($current_version, $min_version, '<')) {
                ...
                $message = esc_html__(
                    'This version of the %1$s is not compatible with the version of Gravity Forms that is installed. Upgrade this add-on to version %2$s or greater to avoid compatibility issues and potential loss of data.',
                    'gravityforms'
                );
                echo '</tr><tr class="plugin-update-tr"><td colspan="3" style="border-left: 4px solid #dc3232;"><div class="update-message">' .
     ...
     */
    public static function maybe_display_update_notification(
        $plugin_name,
        $plugin_data,
        $slug = '',
        $version = ''
        ...
        if (
            is_multisite() &&
            !is_network_admin() &&
            version_compare($version, rgar($version_info, 'version'), '<')
        ) {
            $changelog_url = wp_nonce_url(
                self_admin_url(
                    'admin-ajax.php?action=gf_get_changelog&plugin=' .
                        $slug .
                        '&TB_iframe=true&width=640&height=808'
                )
            );
                ...
                $upgrade_url = wp_nonce_url(
                    self_admin_url(
                        'update.php?action=upgrade-plugin&amp;plugin=' .
                            urlencode($plugin_name)
                    ),
                    'upgrade-plugin_' . $plugin_name
                );
        ...
        if (!$valid_key) {
            $message .= sprintf(
                esc_html__(
                    '%sRegister%s your copy of Gravity Forms to receive access to automatic upgrades and support. Need a license key? %sPurchase one now%s.',
                    'gravityforms'
                ),
        ...
        if (!empty($message)) {
            if (is_network_admin()) {
                $active_class = is_plugin_active_for_network($plugin_name)
                    ? ' active'
                    : '';
            } else {
    ...
    public static function in_plugin_update_message($args, $response)
    {
        ...
        if (
            version_compare($args['new_version'], '2.3', '>=') &&
            version_compare(GFForms::$version, '2.3', '<')
        ) {
            $message = esc_html__(
                'IMPORTANT: As this is a major update, we strongly recommend creating a backup of your site before updating.',
                'gravityforms'
            );

            require_once GFCommon::get_base_path() .
                '/includes/system-status/class-gf-update.php';

            $updates = GF_Update::available_updates();

            $addons_requiring_updates = [];
            ...
            foreach ($updates as $update) {
                ...
                if ($update_available) {
                    $addons_requiring_updates[] =
                        $update['name'] . ' ' . $update['installed_version'];
                }
            }
            ...
            if (count($addons_requiring_updates) > 0) {
                ...
                $message .=
                    ...
                    sprintf(
                        esc_html__(
                            "The versions of the following add-ons you're running haven't been tested with Gravity Forms %s. Please update them or confirm compatibility before updating Gravity Forms, or you may experience issues:",
                            'gravityforms'
                        ),
                        $args['new_version']
                    );
            ...
            echo sprintf(
                '<br /><br /><span style="display:inline-block;background-color: #d54e21; padding: 10px; color: #f9f9f9;">%s</span><br /><br />',
                $message
            );
        }
    }
    ...
    public static function get_changelog()
    {
        ...
        $raw_response = GFCommon::post_to_manager(
            'changelog.php',
        ...
        if (
            ...
            $page_text = sprintf(
                esc_html__(
                    'Oops!! Something went wrong. %sPlease try again or %scontact us%s.',
                    'gravityforms'
                ),
                '<br/>',
    ...
    public static function dashboard_setup()
    {
        ...
        wp_add_dashboard_widget('rg_forms_dashboard', $dashboard_title, [
            'GFForms',
            'dashboard',
        ]);
    }
        ...
        if (!empty($form['total_entries'])) { ?>
						<tr class='author-self status-inherit' valign="top">
							<td class="gf_dashboard_form_title column-title" style="padding:8px 18px;">
								<a <?php echo $form['unread_count'] > 0
            ? "class='form_title_unread' style='font-weight:bold;'"
            : ''; ?> href="admin.php?page=gf_entries&view=entries&id=<?php echo absint(
     $form['id']
 ); ?>"><?php echo esc_html($form['title']); ?></a>
							</td>
							<td class="gf_dashboard_entries_unread column-date" style="padding:8px 18px; text-align:center;">
								<a <?php echo $form['unread_count'] > 0
            ? "class='form_entries_unread' style='font-weight:bold;'"
            : ''; ?> href="admin.php?page=gf_entries&view=entries&filter=unread&id=<?php echo absint(
     $form['id']
 ); ?>" aria-label="<?php printf(
    esc_attr__('Last Entry: %s', 'gravityforms'),
    $date_display
); ?>"><?php echo absint($form['unread_count']); ?></a>
							</td>
							<td class="gf_dashboard_entries_total column-date" style="padding:8px 18px; text-align:center;">
								<a href="admin.php?page=gf_entries&view=entries&id=<?php echo absint(
            $form['id']
        ); ?>" aria-label="<?php esc_attr_e(
    'View All Entries',
    'gravityforms'
); ?>"><?php echo absint($form['total_entries']); ?></a>
							</td>
						</tr>
						<?php }
    } ?>
			...
			<?php if (GFCommon::current_user_can_any('gravityforms_edit_forms')): ?>
				<p class="textright">
				<a class="gf_dashboard_button button" href="admin.php?page=gf_edit_forms"><?php esc_html_e(
        'View All Forms',
        'gravityforms'
    ); ?></a>
			<?php endif; ?>
			...
			<div class="gf_dashboard_noforms_notice">
				<?php echo sprintf(
        esc_html__(
            "You don't have any forms. Let's go %screate one %s!",
            'gravityforms'
        ),
        '<a href="admin.php?page=gf_new_form">',
        '</a>'
    ); ?>
			</div>
			<?php }
        ...
        if (
            GFCommon::current_user_can_any('gravityforms_view_updates') &&
    ...
    public static function dashboard_update_message()
    {
        ...
        $is_dismissed =
            !empty($ary_dismissed) &&
            in_array($version_info['version'], $ary_dismissed);
        ...
        if (
            ...
            $message = sprintf(
                esc_html__(
                    'There is an update available for Gravity Forms. %sView Details%s',
                    'gravityforms'
                ),
                "<a href='admin.php?page=gf_system_status&subview=updates'>",
                '</a>'
            ); ?>
			<div class='updated' style='padding:15px; position:relative;' id='gf_dashboard_message'><?php echo $message; ?>
				<a href="javascript:void(0);" onclick="GFDismissUpgrade();" onkeypress="GFDismissUpgrade();" style='float:right;'><?php esc_html_e(
        'Dismiss',
        'gravityforms'
    ); ?></a>
			</div>
    ...
    public static function register_scripts()
    {
        ...
        wp_register_script(
            'gform_chosen',
            $base_url . "/js/chosen.jquery.min.js",
            ['jquery'],
            $version
        );
        wp_register_script(
        ...
        wp_register_script(
            ...
            $base_url . "/js/datepicker{$min}.js",
            ['jquery', 'jquery-ui-datepicker', 'gform_gravityforms'],
            $version,
            true
        );
        wp_register_script(
            'gform_floatmenu',
            $base_url . "/js/floatmenu_init{$min}.js",
            ['jquery'],
            $version
        );
        wp_register_script(
            'gform_form_admin',
            $base_url . "/js/form_admin{$min}.js",
            [
                'jquery',
                'jquery-ui-autocomplete',
                'gform_placeholder',
        ...
        wp_register_script(
            'gform_form_editor',
            $base_url . "/js/form_editor{$min}.js",
            ['jquery', 'gform_json', 'gform_placeholder'],
            $version,
            true
        );
        wp_register_script(
            'gform_forms',
            $base_url . "/js/forms{$min}.js",
            ['jquery'],
            $version
        );
        wp_register_script(
            'gform_gravityforms',
            $base_url . "/js/gravityforms{$min}.js",
            ['jquery', 'gform_json'],
            $version
        );
        wp_register_script(
            'gform_json',
            $base_url . "/js/jquery.json{$min}.js",
            ['jquery'],
            $version,
            true
        );
        wp_register_script(
            'gform_masked_input',
            $base_url . "/js/jquery.maskedinput{$min}.js",
            ['jquery'],
            $version
        );
        wp_register_script(
            'gform_menu',
            $base_url . "/js/menu{$min}.js",
            ['jquery'],
            $version
        );
        wp_register_script(
            'gform_placeholder',
            $base_url . '/js/placeholders.jquery.min.js',
            ['jquery'],
            $version
        );
        wp_register_script(
            'gform_tooltip_init',
            $base_url . "/js/tooltip_init{$min}.js",
            ['jquery-ui-tooltip'],
            $version
        );
        wp_register_script(
            'gform_textarea_counter',
            $base_url . "/js/jquery.textareaCounter.plugin{$min}.js",
            ['jquery'],
            $version
        );
        wp_register_script(
            'gform_field_filter',
            $base_url . "/js/gf_field_filter{$min}.js",
            ['jquery', 'gform_datepicker_init'],
            $version
        );
        wp_register_script(
            'gform_shortcode_ui',
            $base_url . "/js/shortcode-ui{$min}.js",
            ['jquery', 'wp-backbone'],
            $version,
            true
        );
        wp_register_script(
            'gform_system_report_clipboard',
            $base_url . '/includes/system-status/js/clipboard.min.js',
            ['jquery'],
            $version,
            true
        );
        ...
        wp_register_script(
            'gform_auto_updates',
            $base_url . "/js/auto_update{$min}.js",
            ['jquery'],
            $version,
            true
        );
        ...
        wp_register_style(
            'gform_admin',
            $base_url . "/css/admin{$min}.css",
            [],
            $version
        );
        wp_register_style(
            'gform_chosen',
            $base_url . "/css/chosen{$min}.css",
            [],
            $version
        );
        wp_register_style(
            'gform_shortcode_ui',
            $base_url . "/css/shortcode-ui{$min}.css",
            [],
            $version
        );
        wp_register_style(
        ...
        wp_register_style(
            'gform_tooltip',
            $base_url . "/css/tooltip{$min}.css",
            ['gform_font_awesome'],
            $version
        );
        wp_register_style(
            'gform_dashicons',
            $base_url . "/css/dashicons{$min}.css",
            [],
            $version
        );
        ...
        wp_register_style(
            'gforms_reset_css',
            $base_url . "/css/formreset{$min}.css",
            null,
            $version
        );
        wp_register_style(
            'gforms_datepicker_css',
            $base_url . "/css/datepicker{$min}.css",
            null,
            $version
        );
        wp_register_style(
            'gforms_formsmain_css',
            $base_url . "/css/formsmain{$min}.css",
            null,
            $version
        );
        wp_register_style(
            'gforms_ready_class_css',
            $base_url . "/css/readyclass{$min}.css",
            null,
            $version
        );
        wp_register_style(
            'gforms_browsers_css',
            $base_url . "/css/browsers{$min}.css",
            null,
            $version
        );
        wp_register_style(
            'gforms_rtl_css',
            $base_url . "/css/rtl{$min}.css",
            null,
            $version
        );
    }
    ...
    public static function enqueue_admin_scripts($hook)
    {
        ...
        switch ($page) {
            ...
            case 'entry_detail':
                $scripts = ['gform_json', 'sack', 'postbox'];
                break;
        ...
        if ('plugins.php' === $hook) {
            ...
            wp_localize_script('gform_auto_updates', 'gf_update_ajax', [
        ...
        if (self::page_supports_add_form_button()) {
            ...
            wp_localize_script('gform_shortcode_ui', 'gfShortcodeUIData', [
                 ...
                 */
                'previewDisabled' => apply_filters(
                    'gform_shortcode_preview_disabled',
                    true
                ),
                'strings' => [
                    'pleaseSelectAForm' => wp_strip_all_tags(
                        __('Please select a form.', 'gravityforms')
                    ),
                    'errorLoadingPreview' => wp_strip_all_tags(
                        __(
                            'Failed to load the preview for this form.',
                            'gravityforms'
                        )
                    ),
                ],
    ...
    public static function get_page()
    {
        ...
        if (
            rgget('page') == 'gf_edit_forms' &&
            rgget('view') == 'settings' &&
            (!rgget('subview') || rgget('subview') == 'settings')
        ) {
        ...
        if (
            ...
            (rgget('view') == 'export_entry' || !isset($_GET['view']))
        ) {
            return 'export_entry';
        }
        ...
        if (
            ...
            DOING_AJAX &&
            ((isset($_POST['form_id']) &&
                rgpost('action') === 'rg_select_export_form') ||
                (isset($_POST['export_form']) &&
                    rgpost('action') === 'gf_process_export'))
        ) {
    ...
    {
        require_once GFCommon::get_base_path() .
            '/includes/system-status/class-gf-system-status.php';
        require_once GFCommon::get_base_path() .
    ...
    public static function ajax_disable_logging()
    {
        ...
        if ($disabled) {
            wp_send_json_success(
                esc_html__('Logging disabled.', 'gravityforms')
            );
        } else {
            wp_send_json_error(
                esc_html__('Unable to disable logging.', 'gravityforms')
            );
        }
    }
    ...
    public static function addons_page()
    {
        ...
        foreach ($plugins as $key => $plugin) {
            ...
            $installed_plugin['activation_url'] = $is_active
                ? ''
                : wp_nonce_url(
                    "plugins.php?action=activate&plugin={$key}",
                    "activate-plugin_{$key}"
                );
            $installed_plugin['deactivation_url'] = !$is_active
                ? ''
                : wp_nonce_url(
                    "plugins.php?action=deactivate&plugin={$key}",
                    "deactivate-plugin_{$key}"
                );
        ...
        $raw_response = GFCommon::post_to_manager(
            'api.php',
            "op=plugin_browser&{$_SERVER['QUERY_STRING']}",
            $options
        );
        ...
        if (
            ...
            echo "<div class='error' style='margin-top:50px; padding:20px;'>" .
                esc_html__(
                    'Add-On browser is currently unavailable. Please try again later.',
                    'gravityforms'
                ) .
                '</div>';
        } else {
    ...
    public static function get_addon_info($api, $action, $args)
    {
        if (
            ...
            empty($api) &&
            (!rgempty('rg', $_GET) || $args->slug == 'gravityforms')
        ) {
            $key = GFCommon::get_key();
            $raw_response = GFCommon::post_to_manager(
                'api.php',
                "op=get_plugin&slug={$args->slug}&key={$key}",
                []
            );
    ...
    public static function get_addon_nonces()
    {
        ...
        foreach ($addons as $addon) {
            $nonces[$addon['key']] = wp_create_nonce(
                "install-plugin_{$addon['key']}"
            );
        }
    ...
    public static function get_address_rule_values_select()
    {
        ...
        $type_obj =
            $address_type && isset($address_types[$address_type])
        ...
        $markup = sprintf(
            '<select id="%1$s" name="%1$s" class="gfield_rule_select gfield_rule_value_dropdown">%2$s</select>',
            esc_attr($id),
            $address_field->get_state_dropdown($items, $value)
        );
    ...
    public static function all_leads_page()
    {
        ...
        } elseif ($view == 'entries' || empty($view)) {
            require_once GFCommon::get_base_path() . '/entry_list.php';
    ...
    {
        if (isset($_POST[$name])) {
            return $do_stripslashes
                ? stripslashes_deep($_POST[$name])
                : $_POST[$name];
        }
    ...
    public static function resend_notifications()
    {
        ...
        if (0 == $leads) {
            ...
            if (isset($_POST['fieldId']) && $_POST['fieldId'] !== '') {
                ...
                $search_criteria['field_filters'][] = [
                    'key' => $key,
                    'operator' => rgempty('operator', $_POST)
                        ? 'is'
                        : rgpost('operator'),
                    'value' => $val,
                ];
        ...
        $form = gf_apply_filters(
            ['gform_before_resend_notifications', $form_id],
            RGFormsModel::get_form_meta($form_id),
            $leads
        );

        if (empty($leads) || empty($form)) {
            esc_html_e(
                'There was an error while resending the notifications.',
                'gravityforms'
            );
            die();
        }
        ...
        if (
            !rgempty('sendTo', $_POST) &&
            !GFCommon::is_valid_email_list(rgpost('sendTo'))
        ) {
            die(
                sprintf(
                    esc_html__(
                        'The %sSend To%s email address provided is not valid.',
                        'gravityforms'
                    ),
                    '<strong>',
                    '</strong>'
    ...
    public static function update_form_active()
    {
        ...
        if (GFCommon::current_user_can_any('gravityforms_edit_forms')) {
            GFFormsModel::update_form_active(
                $_POST['form_id'],
                $_POST['is_active']
            );
        } else {
    ...
    public static function update_notification_active()
    {
        ...
        if (GFCommon::current_user_can_any('gravityforms_edit_forms')) {
            GFFormsModel::update_notification_active(
                $_POST['form_id'],
    ...
    public static function edit_form_title($form)
    {
		...
		<div id="edit-title-container" class="add_field_button_container">
			...
			<div class="add-buttons">
				<input type="text" id='edit-title-input' value='<?php echo esc_attr(
        $form['title']
    ); ?>' />

				<div class="edit-form-footer">
					<input type="button" value="<?php esc_html_e(
         'Update',
         'gravityforms'
     ); ?>" class="button-primary" onclick="GF_SaveTitle();" />
				...
				jQuery.post(ajaxurl, {
					...
					.done(function (data) {
						...
						if ( !isValid ) {
							...
							var errorMessage = result ? result.message : '<?php esc_attr_e(
    ...
    public static function form_switcher()
    {
         ...
         */
        $all_forms = apply_filters(
            'gform_form_switcher_forms',
            GFFormsModel::get_forms(null, 'title')
        );

        // Sort forms by active state.
        $forms = ['active' => [], 'inactive' => []];
		...
		<div id="form_switcher_container">
			<select data-placeholder="<?php esc_attr_e(
       'Switch Form',
       'gravityforms'
   ); ?>" name="form_switcher" id="form_switcher" onchange="GF_SwitchForm(jQuery(this).val());">
				<option></option>
				<?php foreach ($all_forms as $form_info) {
        $title = $form_info->title; ?>
					<option value="<?php echo absint($form_info->id); ?>"><?php echo esc_html(
    $title
); ?></option>
					<?php
    } ?>
    ...
    $menu_items = apply_filters(
        'gform_toolbar_menu',
        self::get_toolbar_menu_items($id),
        $id
    );
     ...
     */
    public static function format_toolbar_menu_items(
        $menu_items,
        $compact = false
    ) {
        if (empty($menu_items)) {
            ...
            if (is_array($menu_item)) {
                if (
                    ...
                    if (is_array($sub_menu_items)) {
                        foreach ($sub_menu_items as $k => $val) {
                            if (
                                false ===
                                GFCommon::current_user_can_any(
                                    rgar($sub_menu_items[$k], 'capabilities')
                                )
                            ) {
                    ...
                    if ($count_sub_menu_items == 1) {
                        $label = $compact
                            ? rgar($menu_item, 'label')
                            : rgar($sub_menu_items[0], 'label');
                        $menu_item = $sub_menu_items[0];
                    } else {
                        $label = rgar($menu_item, 'label');
                        $sub_menu_str = self::toolbar_sub_menu_items(
                            $sub_menu_items,
                            $compact
                        );
                    }
                    ...
                    $link =
                        "<a class='{$link_class}' onclick='{$onclick}' onkeypress='{$onclick}' {$aria_label} href='{$url}' target='{$target}'>{$icon} {$label}</a>" .
                        $sub_menu_str;
                    if ($compact) {
                        if ($key == 'delete') {
                            /**
                ...
                $divider = $key == $last_key ? '' : ' | ';
                $output .=
                    '<span class="edit">' . $menu_item . $divider . '</span>';
            }
        }
    ...
    public static function get_toolbar_menu_items($form_id, $compact = false)
    {
        ...
        $menu_items['settings'] = [
            ...
            'url' => $is_mobile
                ? '#'
                : '?page=gf_edit_forms&view=settings&id=' . $form_id,
        ...
        $menu_items['entries'] = [
            ...
            'aria-label' => __(
                'View entries generated by this form',
                'gravityforms'
            ),
        ...
        $menu_items['preview'] = [
            ...
            'url' =>
                trailingslashit(site_url()) . '?gf_page=preview&id=' . $form_id,
    ...
    public static function toolbar_sub_menu_items($menu_items, $compact = false)
    {
        ...
        foreach ($menu_items as $menu_item) {
            if (
                GFCommon::current_user_can_any(rgar($menu_item, 'capabilities'))
            ) {
    ...
    public static function get_form_settings_sub_menu_items($form_id)
    {
        ...
        foreach ($tabs as $tab) {
            ...
            $sub_menu_items[] = [
                'url' => admin_url(
                    "admin.php?page=gf_edit_forms&view=settings&subview={$tab['name']}&id={$form_id}"
                ),
                'label' => $tab['label'],
                'capabilities' => isset($tab['capabilities'])
                    ? $tab['capabilities']
                    : ['gravityforms_edit_forms'],
            ];
        }
    ...
    private static function toolbar_class($item)
    {
        switch ($item) {
            case 'editor':
                if (
                    in_array(rgget('page'), ['gf_edit_forms', 'gf_new_form']) &&
                    rgempty('view', $_GET)
                ) {
    ...
    public static function admin_bar()
    {
        ...
        $args = [
            'id' => 'gform-forms',
            'title' =>
                '<div class="ab-item gforms-menu-icon svg" style="background-image: url(\'' .
                self::get_admin_icon_b64('#888888') .
                '\');"></div><span class="ab-label">' .
                ...
                foreach ($forms as $form) {
                    if ($form['id'] == $recent_form_id) {
                        $wp_admin_bar->add_node([
                            ...
                            'href' => GFCommon::current_user_can_any(
                                'gravityforms_edit_forms'
                            )
                                ? admin_url(
                        ...
                        if (
                            ...
                            $wp_admin_bar->add_node([
                                'id' =>
                                    'gform-form-' . $recent_form_id . '-edit',
                        ...
                        if (
                            ...
                            $wp_admin_bar->add_node([
                                ...
                                'href' => admin_url(
                                    'admin.php?page=gf_edit_forms&view=settings&subview=settings&id=' .
                                        $recent_form_id
                                ),
                            ]);
                        }
    ...
    public static function maybe_auto_update($update, $item)
    {
        if (isset($item->slug) && $item->slug == 'gravityforms') {
            GFCommon::log_debug(
                'GFForms::maybe_auto_update() - Starting auto-update for gravityforms.'
            );

            $auto_update_disabled = self::is_auto_update_disabled();
            GFCommon::log_debug(
                'GFForms::maybe_auto_update() - $auto_update_disabled: ' .
                    var_export($auto_update_disabled, true)
            );

            if (
                $auto_update_disabled ||
                version_compare(GFForms::$version, $item->new_version, '=>')
            ) {
                GFCommon::log_debug(
                    'GFForms::maybe_auto_update() - Aborting update.'
                );

                return false;
            }

            $current_major = implode(
                '.',
                array_slice(preg_split('/[.-]/', GFForms::$version), 0, 1)
            );
            $new_major = implode(
                '.',
                array_slice(preg_split('/[.-]/', $item->new_version), 0, 1)
            );

            $current_branch = implode(
                '.',
                array_slice(preg_split('/[.-]/', GFForms::$version), 0, 2)
            );
            $new_branch = implode(
                '.',
                array_slice(preg_split('/[.-]/', $item->new_version), 0, 2)
            );
            ...
            if (
                ...
                GFCommon::log_debug(
                    __METHOD__ .
                        '() - Aborting update. Not on the same major version.'
                );
    ...
    public static function is_auto_update_disabled()
    {
         ...
         */
        $wp_updates_disabled = apply_filters(
        ...
        if ($wp_updates_disabled) {
            GFCommon::log_debug(
                __METHOD__ .
                    '() - Background updates are disabled in WordPress.'
            );
        ...
        GFCommon::log_debug(
            'GFForms::is_auto_update_disabled() - $enabled: ' .
        ...
        GFCommon::log_debug(
            'GFForms::is_auto_update_disabled() - $disabled: ' .
                var_export($disabled, true)
        );

        if (!$disabled) {
            $disabled =
                defined('GFORM_DISABLE_AUTO_UPDATE') &&
                GFORM_DISABLE_AUTO_UPDATE;
            GFCommon::log_debug(
                'GFForms::is_auto_update_disabled() - GFORM_DISABLE_AUTO_UPDATE: ' .
                    var_export($disabled, true)
            );
        }
     ...
     * @return string|void
     */
    public static function auto_update_message(
        $html,
        $plugin_file,
        $plugin_data
    ) {
        ...
        $update = GFCommon::get_version_info();
        $no_update_message = esc_html__(
            'Auto-updates unavailable.',
            'gravityforms'
        );
        ...
        if (
            'gravityforms.php' !==
            substr($plugin_file, strrpos($plugin_file, '/') + 1)
        ) {
     ...
     * @return string|void
     */
    public static function auto_update_debug_message(
        $auto_updates_string,
        $plugin_path,
        $plugin,
        $enabled
        ...
        if (
            'gravityforms.php' !==
            substr($plugin_path, strrpos($plugin_path, '/') + 1)
        ) {
            if (!rgar($update, 'is_valid_key')) {
                return $no_update_message;
            } else {
    ...
    public static function update_auto_update_setting()
    {
        ...
        $acceptable_tasks = ['enable-gf-updates', 'disable-gf-updates'];
        if (
            !rgar($_POST, 'task') ||
            !in_array($_POST['task'], $acceptable_tasks)
        ) {
            wp_send_json_error(
                __(
                    'Error processing request.  Please refresh and try again.',
                    'gravityforms'
                )
            );
        }
    ...
    public static function handle_ajax_do_shortcode()
    {
        $shortcode = !empty($_POST['shortcode'])
            ? sanitize_text_field(stripslashes($_POST['shortcode']))
            : null;
        $post_id = !empty($_POST['post_id']) ? intval($_POST['post_id']) : null;

        if (
            !current_user_can('edit_post', $post_id) ||
            !wp_verify_nonce($_POST['nonce'], 'gf-shortcode-ui-preview')
        ) {
    ...
    public static function modify_tiny_mce_4($init)
    {
        ...
        $editor_styles =
            $base_url . "/css/shortcode-ui-editor-styles{$min}.css,";
        ...
        $init['content_css'] =
            $init['content_css'] . $editor_styles . $form_styles;
    ...
    public static function get_shortcodes()
    {
        ...
        $default_attrs = [
            [
                'label' => __(
                    'Select a form below to add it to your post or page.',
                    'gravityforms'
                ),
                'tooltip' => __(
                    'Select a form from the list to add it to your post or page.',
                    'gravityforms'
                ),
                'attr' => 'id',
                'type' => 'select',
            ...
            [
                ...
                'type' => 'checkbox',
                'tooltip' => __(
                    'Whether or not to display the form title.',
                    'gravityforms'
                ),
            ],
            [
                ...
                'type' => 'checkbox',
                'tooltip' => __(
                    'Whether or not to display the form description.',
                    'gravityforms'
                ),
            ],
            [
         ...
         */
        $add_on_actions = apply_filters('gform_shortcode_builder_actions', []);

        if (!empty($add_on_actions)) {
            $action_options = ['' => __('Select an action', 'gravityforms')];
    ...
    public static function enqueue_form_scripts($form_id, $is_ajax = false)
    {
        ...
        foreach ($addons as $addon) {
            $a = call_user_func([$addon, 'get_instance']);
    ...
    {
        $notice_disabled =
            defined('GF_LOGGING_DISABLE_NOTICE') && GF_LOGGING_DISABLE_NOTICE;
        $logging_enabled =
            get_option('gform_enable_logging', false) ||
            is_plugin_active('gravityformslogging/logging.php');

        // If logging is disabled, return.
        if (
            $notice_disabled ||
            !$logging_enabled ||
            !GFCommon::current_user_can_any('gravityforms_edit_settings')
        ) {
            return;
        }
        ...
        $message = sprintf(
            ...
            sprintf(
                esc_html__(
                    "Gravity Forms logging is currently enabled. Log files can contain sensitive information so ensure that logging is only enabled for as long as is required for troubleshooting. %sClick here to disable logging.%s",
                    'gravityforms'
                ),
                '<a href="' .
            ...
            esc_html__(
                "If you're currently receiving support, do not disable logging until the issue is resolved.",
                'gravityforms'
            )
        );
        ...
        printf(
            '<div class="notice notice-error" id="gform_disable_logging_notice" data-nonce="%s">%s</div>%s',
            wp_create_nonce('gf_disable_logging_nonce'),
            $message,
            $script
        );
    }
    ...
    public static function set_screen_options($status, $option, $value)
    {
        ...
        if ($option == 'gform_entries_screen_options') {
            $return = [];
            $return['default_filter'] = sanitize_key(
                rgpost('gform_default_filter')
            );
    ...
    public static function load_screen_options()
    {
        ...
        } elseif (in_array($page, ['entry_detail', 'entry_detail_edit'])) {
            require_once GFCommon::get_base_path() . '/entry_detail.php';
    ...
    public static function delete_old_export_files()
    {
        ...
        foreach (
            GFCommon::glob('*.csv', $export_folder . DIRECTORY_SEPARATOR)
            as $filename
        ) {
            ...
            if ($timestamp < time() - DAY_IN_SECONDS) {
                ...
                GFCommon::log_debug(
                    __METHOD__ . '(): Proceeding to delete ' . $filename
                );
    ...
    public static function delete_old_log_files()
    {
        ...
        foreach (
            GFCommon::glob('*.txt', $logs_folder . DIRECTORY_SEPARATOR)
            as $filename
        ) {
            $timestamp = filemtime($filename);
    ...
    public static function delete_orphaned_entries()
    {
        ...
        if (
            ...
            GFFormsModel::has_batch_field_operations()
        ) {
            return;
        }

        GFCommon::log_debug(
            __METHOD__ . '(): Starting to delete orphaned entries'
        );
        ...
        GFCommon::log_debug(
            __METHOD__ . '(): Delete result: ' . print_r($result, true)
        );
    }
    ...
    public static function drop_index($table, $index)
    {
        _deprecated_function(
            'This function has been deprecated. Use gf_upgrade()->drop_index() instead',
            '2.2',
            'gf_upgrade()->drop_index()'
        );
    ...
    {
        _deprecated_function(
            'This function has been deprecated. Use gf_upgrade()->maybe_upgrade() or gf_upgrade()->upgrade() instead',
            '2.2',
            'gf_upgrade()->upgrade() or gf_upgrade()->maybe_upgrade()'
        );
    ...
    {
        _deprecated_function(
            'This function has been deprecated. Use gf_upgrade()->upgrade_schema()',
            '2.2',
    ...
    public static function filter_query($query)
    {
        ...
        if (
            preg_match(
                "/$wpdb->prefix(rg_lead_detail|rg_lead_meta|rg_lead_notes|rg_lead|rg_form_meta|rg_form_view|rg_form|rg_incomplete_submissions)/",
                $query,
                $matches
            )
        ) {
            if (
                ...
                $message = esc_html__(
                    'An outdated add-on or custom code is attempting to access the %1$s table which is not valid in this version of Gravity Forms. Update your add-ons and custom code to prevent loss of form data. Further details: %2$s',
                    'gravityforms'
                );
    ...
    {
        $exporters['gravityforms'] = [
            'exporter_friendly_name' => __('Gravity Forms Exporter'),
            'callback' => ['GFForms', 'data_exporter'],
        ];
        return $exporters;
    }
...
if (!function_exists('gf_apply_filters')) {
    ...
    function gf_apply_filters($filter, $value)
    {
        ...
        if (is_array($filter)) {
            ...
            $modifiers = !is_array($value) ? [$value] : $value;
            $value = $args[2];
...
if (!function_exists('gf_do_action')) {
    ...
    function gf_do_action($action)
    {
        ...
        if (is_array($action)) {
            ...
            $modifiers = !is_array($args[1]) ? [$args[1]] : $args[1];
            $args = array_slice($args, 2);
        }
