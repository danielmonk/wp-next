...
class Check_Changes_Handler
{
    ...
    {
        \add_action('admin_action_duplicate_post_check_changes', [
            $this,
            'check_changes_action_handler',
        ]);
    }
    ...
    public function check_changes_action_handler()
    {
        ...
        if (
            !(
                ...
                (isset($_REQUEST['action']) &&
                    'duplicate_post_check_changes' === $_REQUEST['action'])
            )
        ) {
            // Input var okay.
            \wp_die(
       ...
       Utils::get_edit_or_view_link($this->original) // phpcs:ignore WordPress.Security.EscapeOutput
			...
			<a href="<?php echo \esc_url($post_edit_link); ?>"><?php \esc_html_e(
    ...
    'default'
      ...
      foreach ($fields as $field => $name) {
          ...
          if (!$diff && 'post_title' === $field) {
              ...
              $diff .=
                  '<td>' .
                  \esc_html($this->original->post_title) .
                  '</td><td></td><td>' .
          ...
          if ($diff) { ?>
								...
								<?php echo $diff;} // phpcs:ignore WordPress.Security.EscapeOutput
      }
      ?>
		...
		<?php $this->require_wordpress_footer();
    }
