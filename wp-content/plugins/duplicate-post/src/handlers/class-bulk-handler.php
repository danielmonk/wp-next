...
class Bulk_Handler
{
    ...
    public function add_bulk_handlers()
    {
        ...
        foreach (
            ...
            as $duplicate_post_type_enabled
        ) {
            \add_filter(
                "handle_bulk_actions-edit-{$duplicate_post_type_enabled}",
    ...
    {
        $redirect_to = $this->clone_bulk_action_handler(
            $redirect_to,
            $doaction,
            $post_ids
        );
        return $this->rewrite_bulk_action_handler(
            $redirect_to,
            $doaction,
            $post_ids
        );
     ...
     * @return string The URL to redirect to.
     */
    public function rewrite_bulk_action_handler(
        $redirect_to,
        $doaction,
        $post_ids
    ) {
        ...
        if (\is_array($post_ids)) {
            foreach ($post_ids as $post_id) {
                ...
                if (
                    ...
                    $new_post_id = $this->post_duplicator->create_duplicate_for_rewrite_and_republish(
                        $post
                    );
     ...
     * @return string The URL to redirect to.
     */
    public function clone_bulk_action_handler(
        $redirect_to,
        $doaction,
        $post_ids
    ) {
        ...
        if (\is_array($post_ids)) {
            foreach ($post_ids as $post_id) {
                ...
                if (
                    ...
                    if (
                        \intval(
                            \get_option('duplicate_post_copychildren') !== 1
                        ) ||
