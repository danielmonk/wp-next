...
class Link_Handler
{
    ...
    {
        \add_action('admin_action_duplicate_post_rewrite', [
            $this,
            'rewrite_link_action_handler',
        ]);
        \add_action('admin_action_duplicate_post_clone', [
            $this,
            'clone_link_action_handler',
        ]);
        \add_action('admin_action_duplicate_post_new_draft', [
            $this,
            'new_draft_link_action_handler',
        ]);
    }
    ...
    public function new_draft_link_action_handler()
    {
        ...
        if (
            !(
                isset($_GET['post']) ||
                isset($_POST['post']) || // Input var okay.
                (isset($_REQUEST['action']) &&
                    'duplicate_post_new_draft' === $_REQUEST['action'])
            )
        ) {
            // Input var okay.
            \wp_die(
        ...
        $id = isset($_GET['post'])
            ? \intval(\wp_unslash($_GET['post']))
            : \intval(\wp_unslash($_POST['post'])); // Input var okay.
        ...
        if (!$post) {
            \wp_die(
                \esc_html(
                    \__(
                        'Copy creation failed, could not find original:',
                        'duplicate-post'
                    ) .
                        ' ' .
                        $id
    ...
    public function clone_link_action_handler()
    {
        ...
        if (
            !(
                ...
                isset($_POST['post']) || // Input var okay.
                (isset($_REQUEST['action']) &&
                    'duplicate_post_clone' === $_REQUEST['action'])
            )
        ) {
            // Input var okay.
            \wp_die(
        ...
        if (
            ...
            $sendback = \remove_query_arg(
                ['trashed', 'untrashed', 'deleted', 'cloned', 'ids'],
                $sendback
            );
        }
    ...
    public function rewrite_link_action_handler()
    {
        ...
        if (
            !(
                ...
                isset($_POST['post']) || // Input var okay.
                (isset($_REQUEST['action']) &&
                    'duplicate_post_rewrite' === $_REQUEST['action'])
            )
        ) {
            // Input var okay.
            \wp_die(
        ...
        $new_id = $this->post_duplicator->create_duplicate_for_rewrite_and_republish(
            $post
        );
