...
class Column
{
        ...
        if (\intval(\get_option('duplicate_post_show_original_column')) === 1) {
            ...
            if (\count($enabled_post_types)) {
                foreach ($enabled_post_types as $enabled_post_type) {
                    \add_filter("manage_{$enabled_post_type}_posts_columns", [
                        $this,
                        'add_original_column',
                    ]);
                    \add_action(
                        "manage_{$enabled_post_type}_posts_custom_column",
                        [$this, 'show_original_item'],
                        10,
                        2
                    );
                }
                \add_action(
                    ...
                    [$this, 'quick_edit_remove_original'],
                    10,
                    2
                );
    ...
    public function add_original_column($post_columns)
    {
        if (\is_array($post_columns)) {
            $post_columns['duplicate_post_original_item'] = \__(
                'Original item',
                'duplicate-post'
            );
        }
    ...
    public function show_original_item($column_name, $post_id)
    {
        if ('duplicate_post_original_item' === $column_name) {
            ...
            if ($original_item) {
                ...
                $is_rewrite_and_republish_copy = \is_null($post)
                    ? false
                    : $this->permissions_helper->is_rewrite_and_republish_copy(
                        $post
                    );

                $data_attr = $is_rewrite_and_republish_copy
                    ? ' data-copy-is-for-rewrite-and-republish="1"'
                    : '';
    ...
    public function quick_edit_remove_original($column_name)
    {
        ...
        \printf(
            ...
            \esc_html__('Delete reference to original item.', 'duplicate-post'),
            \wp_kses(
                \__(
