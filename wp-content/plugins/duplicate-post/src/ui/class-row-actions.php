...
class Row_Actions
{
    ...
    {
        if (
            \intval(Utils::get_option('duplicate_post_show_link_in', 'row')) ===
            0
        ) {
            return;
        }
        ...
        if (
            \intval(Utils::get_option('duplicate_post_show_link', 'clone')) ===
            1
        ) {
            \add_filter(
                'post_row_actions',
                [$this, 'add_clone_action_link'],
                10,
                2
            );
            \add_filter(
        ...
        if (
            ...
            \add_filter(
                'post_row_actions',
                [$this, 'add_new_draft_action_link'],
                10,
                2
            );
            \add_filter(
        ...
        if (
            ...
            \add_filter(
                'post_row_actions',
                [$this, 'add_rewrite_and_republish_action_link'],
                10,
                2
            );
            \add_filter(
    ...
    public function add_clone_action_link($actions, $post)
    {
        ...
        $actions['clone'] =
            '<a href="' .
            $this->link_builder->build_clone_link($post->ID) .
            '" aria-label="' .
            \esc_attr(
                /* translators: %s: Post title. */
                \sprintf(
                    \__('Clone &#8220;%s&#8221;', 'duplicate-post'),
                    $title
                )
            ) .
            '">' .
            \esc_html_x('Clone', 'verb', 'duplicate-post') .
            '</a>';
    ...
    public function add_new_draft_action_link($actions, $post)
    {
        ...
        $actions['edit_as_new_draft'] =
            '<a href="' .
            $this->link_builder->build_new_draft_link($post->ID) .
            '" aria-label="' .
            \esc_attr(
                /* translators: %s: Post title. */
                \sprintf(
                    \__('New draft of &#8220;%s&#8221;', 'duplicate-post'),
                    $title
                )
            ) .
            '">' .
    ...
    public function add_rewrite_and_republish_action_link($actions, $post)
    {
        ...
        $actions['rewrite'] =
            '<a href="' .
            $this->link_builder->build_rewrite_and_republish_link($post->ID) .
            '" aria-label="' .
            \esc_attr(
                /* translators: %s: Post title. */
                \sprintf(
            ...
            '">' .
            \esc_html_x('Rewrite & Republish', 'verb', 'duplicate-post') .
            '</a>';
