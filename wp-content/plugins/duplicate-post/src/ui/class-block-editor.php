...
class Block_Editor
{
    ...
    public function register_hooks()
    {
        ...
        \add_action(
            ...
            [$this, 'enqueue_elementor_script'],
            9
        );
        \add_action(
            'admin_enqueue_scripts',
            [$this, 'should_previously_used_keyword_assessment_run'],
            9
        );
        ...
        \add_filter(
            ...
            [$this, 'remove_original_from_wpseo_link_suggestions'],
            10,
            3
        );
    }
    ...
    {
        if (
            $this->permissions_helper->is_edit_post_screen() ||
            $this->permissions_helper->is_new_post_screen()
        ) {
            $post = \get_post();

            if (
                $post instanceof WP_Post &&
                ($this->permissions_helper->is_rewrite_and_republish_copy(
                    $post
                ) ||
                    $this->permissions_helper->has_rewrite_and_republish_copy(
                        $post
                    ))
            ) {
                \add_filter(
    ...
    {
        $is_rewrite_and_republish_copy = $this->permissions_helper->is_rewrite_and_republish_copy(
            $post
        );
     ...
     */
    public function remove_original_from_wpseo_link_suggestions(
        $suggestions,
        $object_id,
        $object_type
    ) {
        ...
        return \array_filter($suggestions, function ($suggestion) use (
            $original_post_id
        ) {
