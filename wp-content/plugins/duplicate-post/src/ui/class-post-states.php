...
class Post_States
{
    ...
    public function register_hooks()
    {
        \add_filter(
            'display_post_states',
            [$this, 'show_original_in_post_states'],
            10,
            2
        );
    }
    ...
    public function show_original_in_post_states($post_states, $post)
    {
        ...
        if ($this->permissions_helper->is_rewrite_and_republish_copy($post)) {
            ...
            $post_states['duplicate_post_original_item'] = \sprintf(
                \esc_html__('Rewrite & Republish of %s', 'duplicate-post'),
                Utils::get_edit_or_view_link($original_item)
            );
            return $post_states;
        }
        ...
        if (
            \intval(
                \get_option('duplicate_post_show_original_in_post_states')
            ) === 1
        ) {
            /* translators: %s: Original item link (to view or edit) or title. */
            $post_states['duplicate_post_original_item'] = \sprintf(
                \__('Original: %s', 'duplicate-post'),
                Utils::get_edit_or_view_link($original_item)
            );
        }
