...
class Classic_Editor
{
    ...
    public function register_hooks()
    {
        \add_action(
            ...
            [$this, 'add_check_changes_link'],
            90
        );

        if (
            \intval(
                Utils::get_option('duplicate_post_show_link_in', 'submitbox')
            ) === 1
        ) {
            if (
                \intval(
                    Utils::get_option('duplicate_post_show_link', 'new_draft')
                ) === 1
            ) {
                \add_action('post_submitbox_start', [
                    $this,
                    'add_new_draft_post_button',
                ]);
            }
            ...
            if (
                ...
                \add_action('post_submitbox_start', [
                    $this,
                    'add_rewrite_and_republish_post_button',
                ]);
            }
        }
        ...
        \add_filter(
            'gettext',
            [$this, 'change_republish_strings_classic_editor'],
            10,
            2
        );
        \add_filter(
            'gettext_with_context',
            [$this, 'change_schedule_strings_classic_editor'],
            10,
            3
        );
        \add_filter(
            'post_updated_messages',
            [$this, 'change_scheduled_notice_classic_editor'],
            10,
            1
        );
        ...
        \add_action('admin_enqueue_scripts', [
            $this,
            'enqueue_classic_editor_scripts',
        ]);
        \add_action('admin_enqueue_scripts', [
            $this,
            'enqueue_classic_editor_styles',
        ]);
        ...
        \add_filter(
            ...
            [$this, 'remove_sample_permalink_slug_editor'],
            10,
            5
        );
    }
    ...
    public function add_new_draft_post_button($post = null)
    {
        ...
        if (
         ...
         $this->link_builder->build_new_draft_link($post)
     ); ?>"><?php \esc_html_e('Copy to a new draft', 'duplicate-post'); ?>
				</a>
			</div>
			<?php }
    }
    ...
    public function add_rewrite_and_republish_post_button($post = null)
    {
        ...
        if (
            $post instanceof WP_Post &&
            $this->permissions_helper->should_rewrite_and_republish_be_allowed(
                $post
            ) &&
            $this->permissions_helper->should_links_be_displayed($post)
        ) { ?>
			<div id="rewrite-republish-action">
				<a class="submitduplicate duplication" href="<?php echo \esc_url(
        $this->link_builder->build_rewrite_and_republish_link($post)
    ); ?>"><?php \esc_html_e('Rewrite & Republish', 'duplicate-post'); ?>
				</a>
			</div>
			<?php }
    }
    ...
    public function add_check_changes_link($post = null)
    {
        ...
        if (
         ...
         $this->link_builder->build_check_link($post)
     ); ?>>
