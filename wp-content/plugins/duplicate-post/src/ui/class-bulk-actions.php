...
class Bulk_Actions
{
    ...
    public function register_hooks()
    {
        if (
            \intval(
                Utils::get_option('duplicate_post_show_link_in', 'bulkactions')
            ) === 0
        ) {
            return;
        }
    ...
    public function add_bulk_filters()
    {
        ...
        foreach (
            ...
            \add_filter("bulk_actions-edit-{$duplicate_post_type_enabled}", [
                $this,
    ...
    public function register_bulk_action($bulk_actions)
    {
        ...
        $is_draft_or_trash =
            ...
            in_array($_REQUEST['post_status'], ['draft', 'trash'], true);

        if (
            \intval(Utils::get_option('duplicate_post_show_link', 'clone')) ===
            1
        ) {
            $bulk_actions['duplicate_post_bulk_clone'] = \esc_html__(
                'Clone',
                'duplicate-post'
            );
        }
        ...
        if (
            ...
            ] = \esc_html__('Rewrite & Republish', 'duplicate-post');
        }
