...
class Post_Duplicator
{
    ...
    public function create_duplicate(\WP_Post $post, array $options = [])
    {
        ...
        if (
            !empty($options['increase_menu_order_by']) &&
            \is_numeric($options['increase_menu_order_by'])
        ) {
        ...
        $new_post = [
            ...
            'post_content' => $options['copy_content']
                ? $post->post_content
                : '',
            'post_content_filtered' => $options['copy_content']
                ? $post->post_content_filtered
                : '',
            'post_title' => $title,
            'post_excerpt' => $options['copy_excerpt']
                ? $post->post_excerpt
                : '',
            ...
            'post_password' => $options['copy_password']
                ? $post->post_password
                : '',
            'post_name' => $options['copy_name'] ? $post->post_name : '',
            'post_parent' => empty($options['parent_id'])
                ? $post->post_parent
                : $options['parent_id'],
    ...
    public function copy_post_taxonomies($new_id, $post, $options)
    {
        ...
        if ($options['use_filters']) {
             ...
             * @return array
             */
            $taxonomies_excludelist = \apply_filters(
                'duplicate_post_taxonomies_excludelist_filter',
                $taxonomies_excludelist
            );
        }

        $post_taxonomies = \array_diff(
            $post_taxonomies,
            $taxonomies_excludelist
        );
    ...
    public function copy_post_meta_info($new_id, $post, $options)
    {
        ...
        $meta_excludelist = \array_merge(
            $meta_excludelist,
            Utils::get_default_filtered_meta_names()
        );
        ...
        if ($options['use_filters']) {
             ...
             * @return array
             */
            $meta_excludelist = \apply_filters(
        ...
        if (\strpos($meta_excludelist_string, '*') !== false) {
            $meta_excludelist_string = \str_replace(
                ['*'],
                ['[a-zA-Z0-9_]*'],
                $meta_excludelist_string
            );
        ...
        foreach ($meta_keys as $meta_key) {
            ...
            foreach ($meta_values as $meta_value) {
                ...
                \update_post_meta(
                    $new_id,
                    $meta_key,
                    Utils::recursively_slash_strings($meta_value)
                );
            }
        }
    }
