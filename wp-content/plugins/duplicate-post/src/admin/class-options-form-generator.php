...
class Options_Form_Generator
{
    ...
    public function generate_options_input(array $options, $parent_option = '')
    {
        ...
        foreach ($options as $option => $option_values) {
            ...
            if (
                \array_key_exists('version', $option_values) &&
                \version_compare(
                    \get_bloginfo('version'),
                    $option_values['version']
                ) < 0
            ) {
                continue;
            }
            ...
            $id = \array_key_exists('id', $option_values)
                ? $option_values['id']
                : $this->prepare_input_id($option);

            if ($parent_option !== '') {
                $id = \sprintf(
                    '%s-%s',
                    $this->prepare_input_id($parent_option),
                    $id
                );
            ...
            switch ($option_values['type']) {
                case 'checkbox':
                    ...
                    break;
                case 'text':
                    $output .= $this->options_inputs->text(
                        $option,
                        $option_values['value'],
                        $id
                    );
                    break;
                case 'number':
                    $output .= $this->options_inputs->number(
                        $option,
                        $option_values['value'],
                        $id
                    );
    ...
    public function extract_description($description, $id)
    {
        ...
        return \sprintf(
            '<p id="%s-description">%s</p>',
            $id,
            \implode('<br />', \array_map('\esc_html', $description))
        );
    }
            ...
            if (
                ...
                $output .= $this->generate_options_input([
                    'duplicate_post_roles[]' => [
                        'type' => 'checkbox',
                        'id' =>
                            'duplicate-post-' . $this->prepare_input_id($name),
                        'value' => $name,
                        'checked' => $role->has_cap('copy_posts'),
                        'label' => \esc_html(
                            \translate_user_role($display_name)
                        ),
                    ],
                ]);
            }
        ...
        foreach ($post_types as $post_type_object) {
            ...
            $output .= $this->generate_options_input([
                'duplicate_post_types_enabled[]' => [
                    ...
                    'id' => 'duplicate-post-' . $this->prepare_input_id($name),
                    'value' => $name,
                    'checked' => $this->is_post_type_enabled(
    ...
    public function is_checked($option, $option_values, $parent_option = '')
    {
        ...
        $saved_option = !empty($parent_option)
        ...
        $cleaned_option = \trim(
            \str_replace($parent_option, '', $option),
            '[]'
        );

        return \array_key_exists($cleaned_option, $saved_option) &&
            (int) $saved_option[$cleaned_option] === 1;
    }
    ...
    {
        $duplicate_post_types_enabled = \get_option(
            'duplicate_post_types_enabled',
            ['post', 'page']
        );
    ...
    {
        $hidden_post_types = ['attachment', 'wp_block'];
