...
<div class="wrap">
	...
	<form id="duplicate_post_settings_form" method="post" action="options.php" style="clear: both">
		...
		<header role="tablist" aria-label="<?php \esc_attr_e(
      'Settings sections',
      'duplicate-post'
  ); ?>"
		...
		<section
			...
			<h2 class="hide-if-js"><?php \esc_html_e(
       'What to copy',
       'duplicate-post'
   ); ?></h2>
         ...
         'duplicate-post'
           ...
           'duplicate-post'
							...
							<?php // phpcs:ignore WordPress.Security.EscapeOutput -- Already escapes correctly.
       echo $this->generate_tab_inputs('what-to-copy', 'elements-to-copy'); ?>
				...
				<tr>
					<th scope="row">
						<label for="duplicate-post-title-prefix"><?php \esc_html_e(
          'Title prefix',
          'duplicate-post'
      ); ?></label>
					</th>
      ...
      echo $this->generate_input('duplicate_post_title_prefix'); ?>
				...
				<tr>
					<th scope="row">
						<label for="duplicate-post-title-suffix"><?php \esc_html_e(
          'Title suffix',
          'duplicate-post'
      ); ?></label>
					</th>
      ...
      echo $this->generate_input('duplicate_post_title_suffix'); ?>
				...
				<tr>
					<th scope="row">
						<label for="duplicate-post-increase-menu-order-by"><?php \esc_html_e(
      ...
      echo $this->generate_input('duplicate_post_increase_menu_order_by'); ?>
				...
				<tr>
					<th scope="row">
						<label for="duplicate-post-blacklist"><?php \esc_html_e(
      ...
      echo $this->generate_input('duplicate_post_blacklist'); ?>
       ...
       echo $this->generate_input('duplicate_post_types_enabled'); ?>
							<p>
								<?php \esc_html_e(
            'Select the post types you want the plugin to be enabled for.',
            'duplicate-post'
        ); ?>
								<br/>
								<?php \esc_html_e(
       ...
       echo $this->generate_tab_inputs('display', 'show-links-in'); ?>
           ...
           '<a href="' .
               \esc_url(
                   'https://developer.yoast.com/duplicate-post/functions-template-tags#duplicate_post_clone_post_link'
               ) .
               '">',
           '</a>'
       ); ?>
      ...
      echo $this->generate_input('duplicate_post_show_notice'); ?>
		...
		<p class="submit">
			<input type="submit" class="button button-primary" value="<?php \esc_html_e(
       'Save changes',
       'duplicate-post'
   ); ?>"/>
		</p>
