...
class Options
{
    ...
    public function get_options_for_tab($tab, $fieldset = '')
    {
        ...
        $options = \array_filter($options, function ($option) use ($tab) {
        ...
        if (!empty($fieldset)) {
            $options = \array_filter($options, function ($option) use (
                $fieldset
            ) {
                return \array_key_exists('fieldset', $option) &&
    ...
    public function get_options()
    {
        return [
            ...
            'duplicate_post_copycomments' => [
                ...
                'value' => 1,
                'description' => \__(
                    'except pingbacks and trackbacks',
                    'duplicate-post'
                ),
            ],
            ...
            'duplicate_post_title_prefix' => [
                ...
                'description' => [
                    \__(
                        'Prefix to be added before the title, e.g. "Copy of" (blank for no prefix)',
                        'duplicate-post'
                    ),
                ],
            ],
            ...
            'duplicate_post_increase_menu_order_by' => [
                ...
                'description' => [
                    \__(
                        'Add this number to the original menu order (blank or zero to retain the value)',
                        'duplicate-post'
                    ),
                ],
            ],
            'duplicate_post_blacklist' => [
                ...
                'description' => [
                    __(
                        'Comma-separated list of meta fields that must not be copied.',
                        'duplicate-post'
                    ),
                    __(
