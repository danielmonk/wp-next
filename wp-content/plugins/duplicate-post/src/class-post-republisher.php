...
class Post_Republisher
{
    ...
    public function register_hooks()
    {
        ...
        foreach ($enabled_post_types as $enabled_post_type) {
             ...
             */
            \add_action("rest_after_insert_{$enabled_post_type}", [
                $this,
                'republish_after_rest_api_request',
            ]);
        }
        ...
        \add_action(
            'wp_insert_post',
            [$this, 'republish_after_post_request'],
            \PHP_INT_MAX,
            2
        );
        ...
        \add_action('before_delete_post', [
            $this,
            'clean_up_when_copy_manually_deleted',
        ]);
    ...
    {
        if (
            !$copy instanceof WP_Post ||
            !$this->permissions_helper->is_rewrite_and_republish_copy($copy)
        ) {
            return;
        }
    ...
    public function clean_up_after_redirect()
    {
        if (
            ...
            if (
                \intval(
                    \get_post_meta($copy_id, '_dp_has_been_republished', true)
                ) === 1
            ) {
    ...
    protected function republish_post_elements($post, $original_post)
    {
        ...
        $post_to_be_rewritten->post_status = $this->determine_post_status(
            $post,
            $original_post
        );
    ...
    protected function republish_post_taxonomies($post)
    {
        ...
        $this->post_duplicator->copy_post_taxonomies(
            $original_post_id,
            $post,
            $copy_taxonomies_options
        );
    ...
    protected function republish_post_meta($post)
    {
        ...
        $this->post_duplicator->copy_post_meta_info(
            $original_post_id,
            $post,
            $copy_meta_options
        );
