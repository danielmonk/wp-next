...
class Permissions_Helper
{
    ...
    {
        $enabled_post_types = \get_option('duplicate_post_types_enabled', [
            'post',
            'page',
        ]);
    ...
    {
        return \intval(
            \get_post_meta($post->ID, '_dp_is_rewrite_republish_copy', true)
        ) === 1;
    }
    ...
    public function has_original_changed(\WP_Post $post)
    {
        ...
        $copy_creation_date_gmt = \get_post_meta(
            $post->ID,
        ...
        if ($original && $copy_creation_date_gmt) {
            if (
                \strtotime($original->post_modified_gmt) >
                \strtotime($copy_creation_date_gmt)
            ) {
                return true;
            }
        }
