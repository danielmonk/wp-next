...
function duplicate_post_admin_init()
{
    ...
    if (intval(get_site_option('duplicate_post_show_notice')) === 1) {
        if (is_multisite()) {
            add_action(
                ...
                'duplicate_post_show_update_notice'
            );
        } else {
            add_action('admin_notices', 'duplicate_post_show_update_notice');
        }
        add_action(
            'wp_ajax_duplicate_post_dismiss_notice',
    ...
    if (intval(get_option('duplicate_post_copyattachments')) === 1) {
        add_action(
            'dp_duplicate_post',
            'duplicate_post_copy_attachments',
            30,
            2
        );
        add_action(
...
function duplicate_post_plugin_upgrade()
{
    ...
    update_option(
        ...
        duplicate_post_get_current_version()
    );
}
...
function duplicate_post_show_update_notice()
{
    ...
    $message =
        ...
        sprintf(
            ...
            __(
                "What's new in Yoast Duplicate Post version %s:",
                'duplicate-post'
            ),
    ...
    $message .=
        __(
            'Now also available in Elementor: the powerful Rewrite & Republish feature. Updating your content has never been easier!',
            'duplicate-post'
        ) . ' ';

    $message .=
        '<a href="https://yoa.st/duplicate-post-4-1">' .
        sprintf(
            /* translators: %s: Yoast Duplicate Post version. */
            __(
                'Read more about what’s new in Yoast Duplicate Post %s!',
                'duplicate-post'
            ),
            DUPLICATE_POST_CURRENT_VERSION
        ) .
    ...
    $sanitized_message = str_replace(
        '%%SIGNUP_FORM%%',
        duplicate_post_newsletter_signup_form(),
        $sanitized_message
    );
    ...
    $sanitized_message . // phpcs:ignore WordPress.Security.EscapeOutput
        '</div></div>';
...
function duplicate_post_copy_post_taxonomies($new_id, $post)
{
    ...
    if (isset($wpdb->terms)) {
        ...
        if (
            ...
            !in_array('post_format', $post_taxonomies, true)
        ) {
            $post_taxonomies[] = 'post_format';
        }

        $taxonomies_blacklist = get_option(
            'duplicate_post_taxonomies_blacklist'
        );
        if ('' === $taxonomies_blacklist) {
            $taxonomies_blacklist = [];
         ...
         * @return array
         */
        $taxonomies_blacklist = apply_filters(
...
function duplicate_post_copy_post_meta_info($new_id, $post)
{
    ...
    $meta_blacklist = apply_filters_deprecated(
        'duplicate_post_blacklist_filter',
        [$meta_blacklist],
        '3.2.5',
...
function duplicate_post_copy_attachments($new_id, $post)
{
    ...
    foreach ($children as $child) {
        ...
        if (
            intval(get_option('duplicate_post_copythumbnail')) === 1 &&
            $old_thumbnail_id === $child->ID
        ) {
...
function duplicate_post_copy_comments($new_id, $post)
{
    ...
    foreach ($comments as $comment) {
        ...
        $parent =
            $comment->comment_parent && $old_id_to_new[$comment->comment_parent]
                ? $old_id_to_new[$comment->comment_parent]
                : 0;
        $commentdata = [
        ...
        if (intval(get_option('duplicate_post_copydate')) === 1) {
            ...
            $commentdata['comment_date_gmt'] = get_gmt_from_date(
...
function duplicate_post_create_duplicate($post, $status = '', $parent_id = '')
{
    ...
    if (
        ...
        wp_die(
            esc_html(
                __(
                    'Copy features for this post type are not enabled in options page',
                    'duplicate-post'
                ) .
                    ': ' .
                    $post->post_type
    ...
    if ('attachment' !== $post->post_type) {
        $prefix = sanitize_text_field(
            get_option('duplicate_post_title_prefix')
        );
        $suffix = sanitize_text_field(
            get_option('duplicate_post_title_suffix')
        );
    ...
    $menu_order =
        intval(get_option('duplicate_post_copymenuorder')) === 1
            ? $post->menu_order
            : 0;
    $increase_menu_order_by = get_option(
        'duplicate_post_increase_menu_order_by'
    );
    if (
        !empty($increase_menu_order_by) &&
        is_numeric($increase_menu_order_by)
    ) {
    ...
    $new_post = [
        ...
        'post_content_filtered' =>
            intval(get_option('duplicate_post_copycontent')) === 1
                ...
                : '',
        'post_excerpt' =>
            intval(get_option('duplicate_post_copyexcerpt')) === 1
                ? $post->post_excerpt
                : '',
        'post_mime_type' => $post->post_mime_type,
        'post_parent' => $new_post_parent,
        'post_password' =>
            intval(get_option('duplicate_post_copypassword')) === 1
                ? $post->post_password
                : '',
...
function duplicate_post_add_plugin_links($links, $file)
{
    if (plugin_basename(dirname(__FILE__) . '/duplicate-post.php') === $file) {
        $links[] =
            '<a href="https://yoast.com/wordpress/plugins/duplicate-post">' .
            esc_html__('Documentation', 'duplicate-post') .
            '</a>';
    }
    return $links;
}
...
<div class="mc-field-group" style="margin-top: 8px;">
	...
	<input type="submit" value="' .
        esc_attr__('Subscribe', 'duplicate-post') .
