...
function duplicate_post_wpml_init()
{
    if (defined('ICL_SITEPRESS_VERSION')) {
        add_action(
            'dp_duplicate_page',
            'duplicate_post_wpml_copy_translations',
            10,
            3
        );
        add_action(
...
function duplicate_wpml_string_packages()
{
    ...
    foreach ($duplicated_posts as $original_post_id => $duplicate_post_id) {
        $original_string_packages = apply_filters(
            'wpml_st_get_post_string_packages',
            false,
            $original_post_id
        ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
        $new_string_packages = apply_filters(
            'wpml_st_get_post_string_packages',
            false,
            $duplicate_post_id
        ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
            ...
            foreach ($original_string_packages as $original_string_package) {
                ...
                foreach ($new_string_packages as $new_string_package) {
                    ...
                    foreach ($new_strings as $new_string) {
                        if (
                            isset(
                                $translated_original_strings[$new_string->name]
                            )
                        ) {
                            foreach (
                                $translated_original_strings[$new_string->name]
                                as $language => $translated_string
                            ) {
                                do_action(
