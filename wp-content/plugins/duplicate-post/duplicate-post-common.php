...
function duplicate_post_is_post_type_enabled($post_type)
{
    ...
    $duplicate_post_types_enabled = apply_filters(
 ...
 * @return string
 */
function duplicate_post_get_clone_post_link(
    $id = 0,
    $context = 'display',
    $draft = true
) {
 ...
 * @param int         $id     Optional. Post ID.
 */
function duplicate_post_clone_post_link(
    $link = null,
    $before = '',
    $after = '',
    $id = 0
     ...
     */
    echo $before .
        apply_filters('duplicate_post_clone_post_link', $link, $post->ID) .
        $after; // phpcs:ignore WordPress.Security.EscapeOutput
}
