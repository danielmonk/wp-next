...
{
    register_block_type('genesis-blocks/gb-newsletter', [
...
function genesis_blocks_render_newsletter_block($attributes)
{
    ...
    if (!empty($_GET['gb-newsletter-submission-message'])) {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- False positive. Only used for displaying a message on AMP redirects.
        echo '<p class="gb-newsletter-submission-message">' .
            esc_html(
                urldecode(
                    sanitize_text_field(
                        wp_unslash($_GET['gb-newsletter-submission-message'])
                    )
                )
            );
        return;
    ...
    $button_bg_color_class = !empty($attributes['buttonBackgroundColor'])
        ? 'has-' . $attributes['buttonBackgroundColor'] . '-background-color'
        : null;
    $button_text_color_class = !empty($attributes['buttonTextColor'])
    ...
    /* Padding styles. */
    if (
        !empty($attributes['containerPadding']) &&
        $attributes['containerPadding'] > 0
    ) {
    ...
    /* Margin styles. */
    if (
        !empty($attributes['containerMarginTop']) &&
        $attributes['containerMarginTop'] > 0
    ) {
        $wrapper_styles .=
            'margin-top:' . $attributes['containerMarginTop'] . 'px;';
    }

    if (
        !empty($attributes['containerMarginBottom']) &&
        $attributes['containerMarginBottom'] > 0
    ) {
        $wrapper_styles .=
            'margin-bottom:' . $attributes['containerMarginBottom'] . 'px;';
    }
    ...
    if (!empty($attributes['textColor'])) {
        $wrapper_class .=
            ' has-text-color has-' . $attributes['textColor'] . '-color';
    }
    ...
    if (!empty($attributes['customButtonTextColor'])) {
        $button_styles_custom .=
            'color:' . $attributes['customButtonTextColor'] . ';';
    }
    ...
    $form =
        '
		<div class="gb-block-newsletter gb-form-styles ' .
        esc_attr($wrapper_class) .
        '" style="' .
        safecss_filter_attr($wrapper_style) .
        '" >
			<form method="post" action-xhr="' .
        esc_url(admin_url('admin-ajax.php')) .
        '">
				<label for="gb-newsletter-email-address-' .
        esc_attr($attributes['instanceId']) .
        '" class="gb-newsletter-email-address-label">' .
        esc_html($attributes['emailInputLabel']) .
        '</label>
				<input type="email" id="gb-newsletter-email-address-' .
        esc_attr($attributes['instanceId']) .
        '" name="gb-newsletter-email-address" class="gb-newsletter-email-address-input" />
				<input class="' .
        esc_attr($button_class) .
        ' gb-newsletter-submit" type="submit" style="' .
        safecss_filter_attr($button_styles) .
        '"  value="' .
        esc_attr($attributes['buttonText']) .
        '"/>
				<input type="hidden" name="gb-newsletter-mailing-list-provider" value="' .
        esc_attr($attributes['mailingListProvider']) .
        '" />
				<input type="hidden" name="gb-newsletter-mailing-list" value="' .
        esc_attr($attributes['mailingList']) .
        '" />
				<input type="hidden" name="gb-newsletter-success-message" value="' .
        esc_attr($attributes['successMessage']) .
        '" />
				<input type="hidden" name="gb-newsletter-double-opt-in" value="' .
        esc_attr($attributes['doubleOptIn']) .
        '" />
				<input type="hidden" name="gb-newsletter-amp-endpoint-request" value="' .
        $amp_endpoint .
        '" />
				<input type="hidden" name="gb-newsletter-form-nonce" value="' .
        wp_create_nonce('gb-newsletter-form-nonce') .
        '" />
			</form>
...
function genesis_blocks_register_newsletter_block_sections_and_fields()
{
    ...
    add_settings_field(
        ...
        [
            'description' => __(
                'Your Mailchimp API key is required for the Newsletter block to communicate with your Mailchimp account.',
                'genesis-blocks'
            ),
            'type' => 'text',
        ]
