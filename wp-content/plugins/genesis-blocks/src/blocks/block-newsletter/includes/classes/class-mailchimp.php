...
final class Mailchimp implements Provider_Interface
{
    ...
    public function __construct($api_key)
    {
        if (empty($api_key)) {
            throw new Mailchimp_API_Error_Exception(
                ...
                sprintf(
                    esc_html__(
                        'An API key is required to use %s.',
                        'genesis-blocks'
                    ),
                    __CLASS__
                )
    ...
    public function subscribe($email, array $args = [])
    {
        ...
        $list_id = !empty($args['list_id'])
            ? sanitize_text_field($args['list_id'])
            : false;
        ...
        if (
            !empty($args['status']) &&
            in_array($args['status'], $valid_statuses, true)
        ) {
            $status = $args['status'];
        }
        ...
        if (empty($list_id) || !in_array($list_id, $list_ids, true)) {
            throw new Mailchimp_API_Error_Exception(
                ...
                sprintf(
                    esc_html__(
                        'An invalid Mailchimp list ID was provided in %s',
                        'genesis-blocks'
                    ),
                    __METHOD__
                )
        ...
        $request_method = sprintf(
            'lists/%s/members/%s',
            $list_id,
            $this->mailchimp->subscriberHash($email)
        );

        $response = $this->mailchimp->put($request_method, [
            'email_address' => $email,
            'status' => $status,
        ]);
        ...
        if (
            ...
            throw new Mailchimp_API_Error_Exception(
                ...
                esc_html(
                    sprintf(
                        __(
                            'There was an error subscribing your email address. Please try again. Error: %s',
                            'genesis-blocks'
                        ),
                        $this->mailchimp->getLastError()
                    )
