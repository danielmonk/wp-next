...
add_action(
    'wp_ajax_genesis_blocks_newsletter_submission',
    __NAMESPACE__ . '\form_submission_listener'
);
add_action(
    'wp_ajax_nopriv_genesis_blocks_newsletter_submission',
    __NAMESPACE__ . '\form_submission_listener'
);
/**
...
function form_submission_listener()
{
    ...
    if (
        !wp_verify_nonce(
            sanitize_text_field(wp_unslash($_POST['gb-newsletter-form-nonce'])),
            'gb-newsletter-form-nonce'
        )
    ) {
        send_processing_response(
            __('Nonce verification failed. Please try again.', 'genesis-blocks')
        );
    }

    if (
        empty($_POST['gb-newsletter-mailing-list-provider']) ||
        empty($_POST['gb-newsletter-mailing-list'])
    ) {
        send_processing_response(
            __('Invalid mailing provider configuration.', 'genesis-blocks')
        );
    }

    if (empty($_POST['gb-newsletter-email-address'])) {
        send_processing_response(
            __('You must provide an email address.', 'genesis-blocks')
        );
    }

    $email = sanitize_email(wp_unslash($_POST['gb-newsletter-email-address']));
    $provider = sanitize_text_field(
        wp_unslash($_POST['gb-newsletter-mailing-list-provider'])
    );
    $list = sanitize_text_field(
        wp_unslash($_POST['gb-newsletter-mailing-list'])
    );
    $default_attributes = genesis_blocks_newsletter_block_attributes();
    $success_message = !empty($_POST['gb-newsletter-success-message'])
        ? sanitize_text_field(
            wp_unslash($_POST['gb-newsletter-success-message'])
        )
        : $default_attributes['successMessage']['default'];
    $subscriber_status = !empty($_POST['gb-newsletter-double-opt-in'])
        ? 'pending'
        : 'subscribed';

    if (!is_email($email)) {
        send_processing_response(
            __('Please provide a valid email address.', 'genesis-blocks')
        );
    }

    $response = process_submission($email, $provider, [
        'list_id' => $list,
...
function send_processing_response($message, $success = true)
{
    ...
    if (
        ...
        !empty($_POST['gb-newsletter-amp-endpoint-request'])
    ) {
        $redirect_url = null;
        ...
        if ($location) {
            $redirect_url = add_query_arg(
                [
                    ...
                    'gb-newsletter-submission-message' => rawurlencode(
                        $message
                    ),
                ],
                $location
...
function process_submission($email, $provider, array $args)
{
    ...
    $list_id = !empty($args['list_id'])
        ? sanitize_text_field(trim((string) $args['list_id']))
        : false;
    ...
    $errors = [
        'invalid_provider' => esc_html__(
            'Invalid newsletter provider.',
            'genesis-blocks'
        ),
        'invalid_email' => esc_html__(
            'Invalid email address.',
            'genesis-blocks'
        ),
        'invalid_list_id' => esc_html__('Invalid list ID.', 'genesis-blocks'),
        'invalid_api_key' => esc_html__(
            'You must enter your email API key in the plugin settings page.',
            'genesis-blocks'
        ),
    ];
...
function mailing_list_providers()
{
    ...
    return [
        'mailchimp' => [
            ...
            'api_key_defined' => !empty(
                get_option('genesis_blocks_mailchimp_api_key')
            ),
        ],
    ];
}
...
function frontend_assets()
{
    wp_register_script(
        ...
        plugins_url(
            '/dist/assets/js/newsletter-block-functions.js',
    ...
    wp_localize_script(
        ...
        [
            ...
            'l10n' => [
                'button_text_processing' => esc_html(
                    genesis_blocks_newsletter_block_attributes()[
                        'buttonTextProcessing'
                    ]['default']
                ),
                'invalid_configuration' => esc_html__(
                    'Invalid configuration. Site owner: Please configure your newsletter provider settings.',
                    'genesis-blocks'
                ),
                'a11y' => [
                    'submission_processing' => esc_html__(
                        'The submission is being processed.',
                        'genesis-blocks'
                    ),
                    'submission_succeeded' => esc_html__(
                        'The submission successfully succeeded.',
                        'genesis-blocks'
                    ),
                    'submission_failed' => esc_html__(
...
function admin_assets()
{
    wp_localize_script(
        ...
        [
            ...
            'plugin_settings_page_url' => esc_url(
                admin_url('admin.php?page=genesis-blocks-settings')
            ),
        ]
    );
}
