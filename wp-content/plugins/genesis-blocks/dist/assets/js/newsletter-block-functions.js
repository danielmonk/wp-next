...
var GenesisBlocksNewsletterSubmission = {
  init() {
    $(".gb-newsletter-submit").on("click", function (event) {
      ...
      button
        .val(genesis_blocks_newsletter_vars.l10n.button_text_processing)
        .prop("disabled", true);
      ...
      if (!email) {
        setTimeout(function () {
          ...
          wp.a11y.speak(
            genesis_blocks_newsletter_vars.l10n.a11y.submission_failed
          );
        }, 400);
        return;
      }
      ...
      if (!provider || !list) {
        form.html(
          '<p class="gb-newsletter-submission-message">' +
            genesis_blocks_newsletter_vars.l10n.invalid_configuration +
            "</p>"
        );
        return;
      }
      ...
      $.ajax({
        ...
        success(response) {
          if (response.success) {
            ...
            wp.a11y.speak(
              genesis_blocks_newsletter_vars.l10n.a11y.submission_succeeded
            );
          }
    ...
    $(".gb-newsletter-email-address-input").on("keyup", function (event) {
      $(".gb-block-newsletter-errors").html("").fadeOut();
    });
  },
};
