	...
	*/
    ...
    public function __construct($api_key, $api_endpoint = null)
    {
        if (!function_exists('curl_init') || !function_exists('curl_setopt')) {
            throw new \Exception(
                "cURL support is required, but can't be found."
            );
        }
    ...
    private function makeRequest(
        ...
        $response = $this->prepareStateForRequest(
            $http_verb,
            $method,
            $url,
            $timeout
        ...
        curl_setopt(
            $ch,
            CURLOPT_USERAGENT,
            'DrewM/MailChimp-API/3.0 (github.com/drewm/mailchimp-api)'
        );
        ...
        $isSuccess = $this->determineSuccess(
            $response,
            $formattedResponse,
            $timeout
        );
    ...
    private function setResponseState($response, $responseContent, $ch)
    {
        if ($responseContent === false) {
            ...
            $response['httpHeaders'] = $this->getHeadersAsArray(
                substr($responseContent, 0, $headerSize)
            );
    ...
    private function determineSuccess($response, $formattedResponse, $timeout)
    {
        ...
        if (
            $timeout > 0 &&
            $response['headers'] &&
            $response['headers']['total_time'] >= $timeout
        ) {
            $this->last_error = sprintf(
                'Request timed out after %f seconds.',
                $response['headers']['total_time']
            );
            return false;
        }
