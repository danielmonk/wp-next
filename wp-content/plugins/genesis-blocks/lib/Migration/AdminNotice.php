...
class AdminNotice
{
    ...
    public function admin_notice()
    {
        ...
        if (
            'genesis-blocks_page_genesis-blocks-migrate' ===
            $current_screen->base
        ) {
            return;
        }
    ...
    private function has_content_to_migrate()
    {
        ...
        if ($has_content === false) {
            ...
            $block_permissions = get_option(
                'genesis_page_builder_block_settings_permissions'
            );
            $has_permissions =
                strpos(wp_json_encode($block_permissions), 'atomic-blocks') !==
                false;

            $has_content =
                (int) ($has_posts || $has_mailchimp_key || $has_permissions);
    ...
    private function print_notice_text(): void
    {
        ...
        'Genesis Blocks Pro: We need to migrate your blocks to give you the latest features!',
        'genesis-blocks'
    ); ?></p>
				<p><a href="<?php echo esc_url(
        $migration_url
    ); ?>" class="button"><?php echo esc_html__(
    'Migrate now',
    'genesis-blocks'
); ?></a></p>
			</div>
			...
			<div class="notice notice-info ab-notice-migration">
				<p><?php echo esc_html__(
        'Welcome to Genesis Blocks! Would you like to migrate your Atomic Blocks content to Genesis Blocks?',
        'genesis-blocks'
    ); ?></p>
				<p><a href="<?php echo esc_url(
        $migration_url
