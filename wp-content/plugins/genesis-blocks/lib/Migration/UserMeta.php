            ...
            foreach ($users as $user) {
                if (isset($user->ID)) {
                    ...
                    if (is_wp_error($migrated_user_meta)) {
                        $error_count++;
                        $errors->add(
                            $migrated_user_meta->get_error_code(),
                            $migrated_user_meta->get_error_message()
                        );
                    } else {
                        $success_count++;
                    }
    ...
    public function migrate_user(int $user_id)
    {
        ...
        if (is_array($user_meta) && count($user_meta) > 0) {
            foreach ($user_meta as $idx => $entry) {
                $user_meta[$idx] = substr_replace(
                    $entry,
                    $this->new_user_meta_namespace,
                    0,
                    2
                );
            }

            $new_user_meta = get_user_meta(
                $user_id,
            ...
            if (
                update_user_meta($user_id, self::GB_USER_META_KEY, $user_meta)
            ) {
                return delete_user_meta($user_id, self::AB_USER_META_KEY);
            } else {
                return new WP_Error(
