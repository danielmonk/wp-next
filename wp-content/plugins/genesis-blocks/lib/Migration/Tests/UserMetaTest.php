...
final class UserMetaTest extends TestCase
{
    ...
    public function test_migrate_success(): void
    {
        ...
        Functions\expect('get_user_meta')
            ->once()
            ->with(1, UserMeta::AB_USER_META_KEY, true)
            ->andReturn($old_user_meta);

        Functions\expect('get_user_meta')
            ->once()
            ->with(1, UserMeta::GB_USER_META_KEY, true)
            ->andReturn('');

        Functions\expect('update_user_meta')
            ->once()
            ->with(1, UserMeta::GB_USER_META_KEY, $new_user_meta)
            ->andReturn(true);

        Functions\expect('delete_user_meta')
            ->once()
            ->with(1, UserMeta::AB_USER_META_KEY)
            ->andReturn(true);
    ...
    public function test_migrate_user_meta_not_updated(): void
    {
        ...
        Functions\expect('get_user_meta')->once();
        ...
        $this->assertEquals(
            'WP_Error',
            get_class($this->instance->migrate_user(1))
        );
    }
    ...
    {
        Mockery::mock('overload:WP_Error')->shouldReceive([
            'add' => true,
        ...
        Functions\expect('get_user_meta')->andReturn(['gb_section_hero_1']);

        Functions\expect('update_user_meta')->andReturn(false);

        $this->assertEquals(
            'WP_Error',
            get_class($this->instance->migrate_all())
        );
    }
}
