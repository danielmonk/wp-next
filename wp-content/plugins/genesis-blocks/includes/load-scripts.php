...
function genesis_blocks_block_assets()
{
    ...
    wp_register_style(
        ...
        [],
        filemtime(
            plugin_dir_path(genesis_blocks_main_plugin_file()) .
    ...
    wp_enqueue_style(
        ...
        plugins_url(
            'dist/assets/fontawesome/css/all' . $postfix . '.css',
            dirname(__FILE__)
        ),
        [],
        filemtime(
            plugin_dir_path(genesis_blocks_main_plugin_file()) .
                'dist/assets/fontawesome/css/all.css'
        )
    );
}
...
function genesis_blocks_editor_assets()
{
    ...
    // Pass in REST URL.
    wp_localize_script('genesis-blocks-block-js', 'genesis_blocks_globals', [
        'rest_url' => esc_url(rest_url()),
        'user_data' => $user_data,
        'pro_activated' => genesis_blocks_is_pro(),
        'is_wpe' => function_exists('is_wpe'),
        'pattern_fallback_image' => plugins_url(
            'dist/assets/images/gb-fallback-image.jpg',
            dirname(__FILE__)
        ),
    ]);
}
