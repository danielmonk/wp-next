...
function register_layout_endpoints()
{
     ...
     */
    register_rest_route(GB_API_NAMESPACE, FAVORITE_LAYOUTS_ROUTE, [
        'methods' => WP_REST_Server::READABLE,
        'callback' => function () {
            return new WP_REST_Response(
                (array) get_user_meta(
                    get_current_user_id(),
                    'genesis_blocks_favorite_layouts',
                    true
                )
            );
        },
     ...
     */
    register_rest_route(GB_API_NAMESPACE, ALL_LAYOUTS_ROUTE, [
        'methods' => WP_REST_Server::READABLE,
        'callback' => function (\WP_REST_Request $request) {
            $layouts = genesis_blocks_get_layouts();
            $sections = genesis_blocks_get_sections();
            $additional_layouts = apply_filters(
                'genesis_blocks_additional_layout_components',
                []
            );
            $all_layouts = array_merge(
                $layouts,
                $sections,
                $additional_layouts
            );
            ...
            if (
                empty($request_params['filter']) ||
                'allowed' !== $request_params['filter']
            ) {
             ...
             */
            $allowed_layouts = (array) apply_filters(
                'genesis_blocks_allowed_layout_components',
                array_keys($all_layouts)
            );
     ...
     */
    register_rest_route(GB_API_NAMESPACE, LAYOUTS_ROUTE, [
        'methods' => WP_REST_Server::READABLE,
        'callback' => function () {
     ...
     */
    register_rest_route(GB_API_NAMESPACE, SINGLE_LAYOUT_ROUTE, [
        'methods' => WP_REST_Server::READABLE,
        'callback' => function ($request) {
            ...
            return new WP_REST_Response(
                esc_html__('Layout not found.', 'genesis-blocks')
            );
        },
        'permission_callback' => function () {
            return current_user_can('edit_posts');
     ...
     */
    register_rest_route(GB_API_NAMESPACE, SECTIONS_ROUTE, [
        'methods' => WP_REST_Server::READABLE,
        'callback' => function () {
