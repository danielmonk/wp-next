...
final class Component_Registry
{
     ...
     * @var array
     */
    private static $required_data_keys = [
        'type',
        'key',
        'name',
        'content',
    ...
    public static function add(array $data): void
    {
        if (
            empty($data['type']) ||
            !in_array($data['type'], self::$supported_component_types, true)
        ) {
            throw new InvalidArgumentException(
                esc_html__(
                    'You must supply a valid component type.',
                    'genesis-blocks'
                )
            );
        }
        ...
        if (empty($data)) {
            throw new InvalidArgumentException(
                __(
                    'You must supply valid layout data to register a layout.',
                    'genesis-blocks'
                )
            );
        }
        ...
        foreach (self::$required_data_keys as $required_key) {
            if (
                !array_key_exists($required_key, $data) ||
                empty($data[$required_key])
            ) {
                /* translators: %s: The missing key that is required to register a component. */
                throw new InvalidArgumentException(
                    sprintf(
                        esc_html__(
                            'You must supply a %s to register a layout.',
                            'genesis-blocks'
                        ),
                        $required_key
                    )
        ...
        switch ($data['type']) {
            case 'layout':
                if (!empty(self::$layouts[$data['key']])) {
                    ...
                    throw new InvalidArgumentException(
                        sprintf(
                            esc_html__(
                                'The %s layout is already registered.',
                                'genesis-blocks'
                            ),
                            $data['key']
                        )
            ...
            case 'section':
                if (!empty(self::$sections[$data['key']])) {
                    ...
                    throw new InvalidArgumentException(
                        sprintf(
                            esc_html__(
                                'The %s section is already registered.',
                                'genesis-blocks'
                            ),
                            $data['key']
                        )
            ...
            default:
                ...
                throw new InvalidArgumentException(
                    sprintf(
                        esc_html__(
                            'You must supply a valid component type in %s.',
                            'genesis-blocks'
                        ),
                        __METHOD__
                    )
    ...
    public static function remove($type, $key)
    {
        if (
            empty($type) ||
            !in_array($type, self::$supported_component_types, true)
        ) {
        ...
        if (empty($key)) {
            ...
            throw new InvalidArgumentException(
                sprintf(
                    esc_html__(
                        'You must supply a valid component key in %s.',
                        'genesis-blocks'
                    ),
                    __METHOD__
                )
            ...
            case 'layout':
                if (empty(self::$layouts[$key])) {
                    ...
                    throw new Exception(
                        sprintf(
                            esc_html__(
                                'The %s layout is not registered.',
                                'genesis-blocks'
                            ),
                            $key
                        )
            ...
            case 'section':
                if (empty(self::$sections[$key])) {
                    ...
                    throw new Exception(
                        sprintf(
                            esc_html__(
                                'The %s section is not registered.',
                                'genesis-blocks'
                            ),
                            $key
                        )
    ...
    public static function get($type, $key)
    {
        if (
            ...
            throw new Exception(
                sprintf(
                    esc_html__(
                        'You must supply a component type in %s.',
                        'genesis-blocks'
                    ),
                    __METHOD__
                )
