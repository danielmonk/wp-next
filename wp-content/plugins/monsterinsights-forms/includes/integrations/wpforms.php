...
class MonsterInsights_Forms_Integration_WPForms
{
    ...
    public function __construct()
    {
        if (
            ...
            (function_exists('wpforms') && !wpforms()->pro)
        ) {
        ...
        add_filter(
            ...
            [$this, 'use_utm_data_in_filter'],
            15,
            2
        );

        // Add UTM data to the export options.
        add_filter('wpforms_pro_admin_entries_export_additional_info_fields', [
            $this,
            'add_utm_to_export_options',
        ]);
        // Add UTM data in the export data.
        add_filter(
            'wpforms_pro_admin_entries_export_ajax_get_additional_info_value',
            [$this, 'add_utm_to_export_data'],
            10,
            3
        );
    }
                ...
                wpforms()->entry_meta->add(
                    [
                        ...
                        'data' => sanitize_text_field(
                            wp_unslash($_REQUEST['utm_source'])
                        ),
                    ],
                    'entry_meta'
                );
            ...
            if (!empty($_REQUEST['utm_medium'])) {
                wpforms()->entry_meta->add(
                    [
                        ...
                        'data' => sanitize_text_field(
                            wp_unslash($_REQUEST['utm_medium'])
                        ),
                    ],
                    'entry_meta'
                );
            ...
            if (!empty($_REQUEST['utm_campaign'])) {
                wpforms()->entry_meta->add(
                    [
                        ...
                        'data' => sanitize_text_field(
                            wp_unslash($_REQUEST['utm_campaign'])
                        ),
                    ],
                    'entry_meta'
                );
            ...
            if (!empty($_REQUEST['utm_content'])) {
                wpforms()->entry_meta->add(
                    [
                        ...
                        'data' => sanitize_text_field(
                            wp_unslash($_REQUEST['utm_content'])
                        ),
                    ],
                    'entry_meta'
                );
    ...
    public function display_utm_data($entry, $form_data, $instance)
    {
        ...
        foreach ($utm_keys as $utm_key) {
            ...
            if (
                empty($utm_single_data) ||
                empty($utm_single_data[0]) ||
                !isset($utm_single_data[0]->data)
            ) {
    ...
    foreach ($utm_data as $utm => $value) {
        echo '<div class="wpforms-entry-notes-single"><strong>' .
            esc_html($this->get_utm_label($utm)) .
            ' (' .
            esc_attr($utm) .
            '):</strong> ' .
            esc_attr($value) .
     ...
     * @return string
     */
    public function process_smart_tags(
        $content,
        $form_data,
        $fields = '',
        $entry_id = ''
    ...
    public function use_utm_data_in_filter($args)
    {
        if (
            ...
            in_array($_GET['search']['field'], $this->utm_tags) &&
            !empty($_GET['search']['term'])
        ) {
            global $wpdb;
            ...
            $value_compare = !empty($_GET['search']['comparison'])
                ? esc_sql($_GET['search']['comparison'])
                : '';
            if (!empty($value_compare)) {
                switch ($value_compare) {
                    ...
                    case 'contains':
                        $where['arg_value'] =
                            "`data` LIKE '%" . esc_sql($search_term) . "%'";
                        break;

                    case 'contains_not':
                        $where['arg_value'] =
                            "`data` NOT LIKE '%" . esc_sql($search_term) . "%'";
                        break;
                }
            }
    ...
    {
        if (
            in_array($col_id, $this->utm_tags, true) &&
    ...
    {
        if (
            empty($formdata['settings']['ajax_submit']) ||
            !$formdata['settings']['ajax_submit']
        ) {
            return;
        }
        ...
        foreach ($this->utm_tags as $utm_tag) {
            if (!empty($_GET[$utm_tag])) {
                echo '<input type="hidden" name="' .
                    $utm_tag .
                    '" value="' .
                    sanitize_text_field(wp_unslash($_GET[$utm_tag])) .
                    '" />';
            }
        }
    }
