...
if (!class_exists('myCRED_WP_Pro_Quiz')):
    final class myCRED_WP_Pro_Quiz
    {
        ...
        public function __construct()
        {
            ...
            add_filter('mycred_setup_hooks', [$this, 'register_hook']);
            add_action('mycred_init', [$this, 'load_textdomain']);
            add_action('mycred_all_references', [$this, 'add_badge_support']);
        ...
        public function plugin_updates()
        {
            ...
            add_filter('plugins_api', [$this, 'plugin_api_call'], 330, 3);
            add_filter('plugin_row_meta', [$this, 'plugin_view_info'], 330, 3);
        }
        ...
        public function register_hook($installed)
        {
            ...
            $installed['wpproquiz'] = [
                ...
                'description' => __(
                    'Awards %_plural% to users who complete quizes.',
                    'mycred_wp_pro_quiz'
                ),
                'callback' => ['myCRED_Hook_WP_Pro_Quiz'],
            ];
        ...
        public function add_badge_support($references)
        {
            ...
            $references['completing_quiz'] = __(
                'Completing Quiz (WP-Pro-Quiz)',
                'mycred_wp_pro_quiz'
            );
            $references['full_completing_quiz'] = __(
                '100% Completion (WP-Pro-Quiz)',
                'mycred_wp_pro_quiz'
            );
...
if (!function_exists('mycred_load_wp_pro_quiz_hook')):
    ...
    {
        if (class_exists('myCRED_Hook_WP_Pro_Quiz')) {
            return;
        }
        ...
        class myCRED_Hook_WP_Pro_Quiz extends myCRED_Hook
        {
            ...
            function run()
            {
                ...
                if ($this->prefs['completed']['creds'] != 0) {
                    add_action('wp_pro_quiz_completed_quiz', [
                        $this,
                        'completed_quiz',
                    ]);
                }
                ...
                if ($this->prefs['fullscrore']['creds'] != 0) {
                    add_action('wp_pro_quiz_completed_quiz_100_percent', [
                        $this,
                        'completed_quiz_full',
                    ]);
                }
            ...
            {
                $prefs = $this->prefs; ?>
<label class="subheader" for="<?php echo $this->field_id([
    'completed' => 'creds',
]); ?>"><?php _e('Completing Quiz', 'mycred_wp_pro_quiz'); ?></label>
<ol>
	<li>
		<div class="h2"><input type="text" name="<?php echo $this->field_name([
      'completed' => 'creds',
  ]); ?>" id="<?php echo $this->field_id([
    'completed' => 'creds',
]); ?>" value="<?php echo esc_attr(
    $prefs['completed']['creds']
); ?>" size="8" /></div>
	</li>
	<li class="empty">&nbsp;</li>
	<li>
		<label for="<?php echo $this->field_id(['completed' => 'log']); ?>"><?php _e(
    'Log template',
    'mycred_wp_pro_quiz'
); ?></label>
		<div class="h2"><input type="text" name="<?php echo $this->field_name([
	...
	<li>
		<?php echo $this->hook_limit_setting(
      $this->field_name(['completed' => 'limit']),
      $this->field_id(['completed' => 'limit']),
      $prefs['completed']['limit']
  ); ?>
	</li>
	<li class="empty">&nbsp;</li>
</ol>
<label class="subheader" for="<?php echo $this->field_id([
    'fullscrore' => 'creds',
]); ?>"><?php _e('100% Completion', 'mycred_wp_pro_quiz'); ?></label>
<ol>
	<li>
		<div class="h2"><input type="text" name="<?php echo $this->field_name([
      'fullscrore' => 'creds',
    ...
    $prefs['fullscrore']['creds']
); ?>" size="8" /></div>
	</li>
	<li class="empty">&nbsp;</li>
	<li>
		<label for="<?php echo $this->field_id(['fullscrore' => 'log']); ?>"><?php _e(
    'Log template',
    'mycred_wp_pro_quiz'
); ?></label>
		<div class="h2"><input type="text" name="<?php echo $this->field_name([
	...
	<li>
		<?php echo $this->hook_limit_setting(
      $this->field_name(['fullscrore' => 'limit']),
      $this->field_id(['fullscrore' => 'limit']),
      $prefs['fullscrore']['limit']
  ); ?>
	</li>
</ol>
<?php
            ...
            function sanitise_preferences($data)
            {
                ...
                if (
                    isset($data['completed']['limit']) &&
                    isset($data['completed']['limit_by'])
                ) {
                    $limit = sanitize_text_field($data['completed']['limit']);
                    if ($limit == '') {
                        $limit = 0;
                    }
                    $data['completed']['limit'] =
                        $limit . '/' . $data['completed']['limit_by'];
                ...
                // These two settings needs to be combined into one string divided by a forward slash.
                if (
                    isset($data['fullscrore']['limit']) &&
                    isset($data['fullscrore']['limit_by'])
                ) {
                    $limit = sanitize_text_field($data['fullscrore']['limit']);
                    if ($limit == '') {
                        $limit = 0;
                    }
                    $data['fullscrore']['limit'] =
                        $limit . '/' . $data['fullscrore']['limit_by'];
