...
function acf_rendered_block(
    ...
    acf_get_store('block-cache')->set(
        $attributes['id'],
        '<div class="acf-block-preview">' . $html . '</div>'
    );
    return $html;
}
...
function acf_enqueue_block_assets()
{
    ...
    wp_enqueue_script(
        'acf-blocks',
        acf_get_url("pro/assets/js/acf-pro-blocks.min.js"),
        ['acf-input', 'wp-blocks'],
        ACF_VERSION,
        true
    );
    ...
    if (
        defined('ACF_EXPERIMENTAL_PRELOAD_BLOCKS') &&
...
function acf_parse_save_blocks_callback($matches)
{
    ...
    $attrs = isset($matches['attrs'])
        ? json_decode($matches['attrs'], true)
        : '';
