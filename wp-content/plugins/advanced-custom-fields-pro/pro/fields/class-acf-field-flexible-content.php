    ...
    class acf_field_flexible_content extends acf_field
    {
        ...
        function initialize()
        {
            ...
            $this->add_action(
                ...
                [$this, 'ajax_layout_title']
            );
            $this->add_action(
                'wp_ajax_nopriv_acf/fields/flexible_content/layout_title',
                [$this, 'ajax_layout_title']
            );

            // filters
            $this->add_filter('acf/prepare_field_for_export', [
                $this,
                'prepare_any_field_for_export',
            ]);
            $this->add_filter(
                'acf/clone_field',
                [$this, 'clone_any_field'],
                10,
                2
            );
            $this->add_filter('acf/validate_field', [
                $this,
                'validate_any_field',
            ]);
            ...
            $this->add_field_filter('acf/prepare_field_for_export', [
                $this,
                'prepare_field_for_export',
            ]);
            $this->add_field_filter('acf/prepare_field_for_import', [
                $this,
        ...
        function input_admin_enqueue_scripts()
        {
            ...
            acf_localize_text([
                ...
                // min / max
                'This field requires at least {min} {label} {identifier}' => __(
                    'This field requires at least {min} {label} {identifier}',
                    'acf'
                ),
                'This field has a limit of {max} {label} {identifier}' => __(
                    'This field has a limit of {max} {label} {identifier}',
                    'acf'
                ),

                // popup badge
                '{available} {label} {identifier} available (max {max})' => __(
                    '{available} {label} {identifier} available (max {max})',
                    'acf'
                ),
                '{required} {label} {identifier} required (min {min})' => __(
                    '{required} {label} {identifier} required (min {min})',
                    'acf'
                ),

                // field settings
                'Flexible Content requires at least 1 layout' => __(
                ...
                if (!empty($sub_fields)) {
                    foreach (array_keys($sub_fields) as $k) {
                        ...
                        if (
                            ...
                            $layout['sub_fields'][] = acf_extract_var(
                                $sub_fields,
                                $k
                            );
                        }
        ...
        function render_field($field)
        {
            ...
            // no value message
            $no_value_message = __(
                'Click the "%s" button below to start creating your layout',
                'acf'
            );
            $no_value_message = apply_filters(
...
<div <?php acf_esc_attr_e($div); ?>>
	...
	<div class="acf-actions">
		<a class="acf-button button button-primary" href="#" data-name="add-layout"><?php echo $field[
      'button_label'
  ]; ?></a>
	</div>
	
	<script type="text-html" class="tmpl-popup"><?php  ?><ul><?php foreach (
    $layouts
    as $layout
):
    $atts = [
        ...
        'data-min' => $layout['min'],
        'data-max' => $layout['max'],
    ]; ?><li><a <?php acf_esc_attr_e($atts); ?>><?php echo $layout[
    'label'
]; ?></a></li><?php
endforeach; ?></ul>
	</script>
...
<div <?php echo acf_esc_attr($div); ?>>
	...
	<div class="acf-fc-layout-handle" title="<?php _e(
     'Drag to reorder',
     'acf'
 ); ?>" data-name="collapse-layout"><?php echo $title; ?></div>
	
	<div class="acf-fc-layout-controls">
		<a class="acf-icon -plus small light acf-js-tooltip" href="#" data-name="add-layout" title="<?php _e(
      'Add layout',
      'acf'
  ); ?>"></a>
		<a class="acf-icon -duplicate small light acf-js-tooltip" href="#" data-name="duplicate-layout" title="<?php _e(
      'Duplicate layout',
      'acf'
  ); ?>"></a>
		<a class="acf-icon -minus small light acf-js-tooltip" href="#" data-name="remove-layout" title="<?php _e(
      'Remove layout',
      'acf'
  ); ?>"></a>
		<a class="acf-icon -collapse small -clear acf-js-tooltip" href="#" data-name="collapse-layout" title="<?php _e(
      'Click to toggle',
      'acf'
  ); ?>"></a>
	</div>
...
<?php if (!empty($sub_fields)): ?>
    ...
    endforeach; ?> 
		...
		<tbody>
			<tr class="acf-row">
	<?php else: ?>
	<div class="acf-fields <?php if (
     $layout['display'] == 'row'
 ): ?>-left<?php endif; ?>">
	<?php endif; ?>
	
		<?php // loop though sub fields
...
<?php
        ...
        function render_field_settings($field)
        {
            ...
            foreach ($field['layouts'] as $layout) {
                ...
                ?><tr class="acf-field acf-field-setting-fc_layout" data-name="fc_layout" data-setting="flexible_content" data-id="<?php echo $layout[
    'key'
]; ?>">
	<td class="acf-label">
		<label><?php _e("Layout", 'acf'); ?></label>
		<ul class="acf-bl acf-fl-actions">
			<li><a class="reorder-layout" href="#" title="<?php _e(
       "Reorder Layout",
       'acf'
   ); ?>"><?php _e("Reorder", 'acf'); ?></a></li>
			<li><a class="delete-layout" href="#" title="<?php _e(
       "Delete Layout",
       'acf'
   ); ?>"><?php _e("Delete", 'acf'); ?></a></li>
			<li><a class="duplicate-layout" href="#" title="<?php _e(
       "Duplicate Layout",
       'acf'
   ); ?>"><?php _e("Duplicate", 'acf'); ?></a></li>
			<li><a class="add-layout" href="#" title="<?php _e(
       "Add New Layout",
       'acf'
   ); ?>"><?php _e("Add New", 'acf'); ?></a></li>
		</ul>
	</td>
	<td class="acf-input">
		<?php acf_hidden_input([
...
<?php
        ...
        function load_value($value, $post_id, $field)
        {
            ...
            foreach ($value as $i => $l) {
                ...
                foreach (array_keys($layout) as $j) {
                    ...
                    $sub_field[
                        'name'
                    ] = "{$field['name']}_{$i}_{$sub_field['name']}";
        ...
        function get_layout_title($field, $layout, $i, $value)
        {
            ...
            $title = apply_filters(
                'acf/fields/flexible_content/layout_title/name=' .
                    $field['_name'],
                $title,
                $field,
                $layout,
            ...
            $title = apply_filters(
                'acf/fields/flexible_content/layout_title/key=' . $field['key'],
                $title,
                $field,
                $layout,
                $i
            ...
            $title =
                '<span class="acf-fc-layout-order">' .
                $order .
                '</span> ' .
                $title;
        ...
        function prepare_field_for_export($field)
        {
            ...
            if (!empty($field['layouts'])) {
                foreach ($field['layouts'] as &$layout) {
                    $layout['sub_fields'] = acf_prepare_fields_for_export(
                        $layout['sub_fields']
                    );
                }
            }
        ...
        function validate_any_field($field)
        {
            ...
            if (isset($field['column_width'])) {
                $field['wrapper']['width'] = acf_extract_var(
                    $field,
                    'column_width'
                );
            }
