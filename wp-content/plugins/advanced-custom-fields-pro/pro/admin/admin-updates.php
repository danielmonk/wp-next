    ...
    class ACF_Admin_Updates
    {
        ...
        function display_wp_error($wp_error)
        {
            ...
            acf_new_admin_notice([
                'text' =>
                    __(
                        '<b>Error</b>. Could not connect to update server',
                        'acf'
                    ) .
        ...
        function load()
        {
            ...
            if (version_compare($info['version'], $version, '>')) {
                ...
                $this->view['changelog'] = $this->get_changelog_changes(
                    $info['changelog'],
                    $info['version']
                );
                $this->view['upgrade_notice'] = $this->get_changelog_changes(
                    $info['upgrade_notice'],
                    $info['version']
                );
                ...
                if ($license) {
                    ...
                    if ($update && !$update['package']) {
                        ...
                        acf_new_admin_notice([
                            'text' => __(
                                '<b>Error</b>. Could not authenticate update package. Please check again or deactivate and reactivate your ACF PRO license.',
                                'acf'
                            ),
                            'type' => 'error',
                        ]);
        ...
        {
            acf_get_view(
                dirname(__FILE__) . '/views/html-settings-updates.php',
                $this->view
            );
        }
    }
