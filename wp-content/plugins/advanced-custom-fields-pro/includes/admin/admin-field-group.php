...
if (!class_exists('acf_admin_field_group')):
    class acf_admin_field_group
    {
        ...
        function __construct()
        {
            ...
            add_action('wp_ajax_acf/field_group/render_location_rule', [
                $this,
                'ajax_render_location_rule',
            ]);
            add_action('wp_ajax_acf/field_group/move_field', [
                $this,
                'ajax_move_field',
            ]);
            ...
            add_filter(
                ...
                [$this, 'use_block_editor_for_post_type'],
                10,
                2
            );
        }
        ...
        function current_screen()
        {
            ...
            // actions
            add_action('acf/input/admin_enqueue_scripts', [
                $this,
        ...
        function admin_enqueue_scripts()
        {
            ...
            // localize text
            acf_localize_text([
                'The string "field_" may not be used at the start of a field name' => __(
                    'The string "field_" may not be used at the start of a field name',
                    'acf'
                ),
                'This field cannot be moved until its changes have been saved' => __(
                    'This field cannot be moved until its changes have been saved',
                    'acf'
                ),
                'Field group title is required' => __(
                    'Field group title is required',
                    'acf'
                ),
                'Move to trash. Are you sure?' => __(
                    'Move to trash. Are you sure?',
                    'acf'
                ),
                'No toggle fields available' => __(
                    'No toggle fields available',
                    'acf'
                ),
        ...
        {
            // vars
            $checked = acf_get_user_setting('show_field_keys')
                ? 'checked="checked"'
                : '';
            ...
            $html .=
                '<label for="acf-field-key-hide"><input id="acf-field-key-hide" type="checkbox" value="1" name="show_field_keys" ' .
                $checked .
                ' /> ' .
                __('Field Keys', 'acf') .
                '</label>';
        ...
        function save_post($post_id, $post)
        {
            ...
            if (!empty($_POST['acf_fields'])) {
                ...
                foreach ($_POST['acf_fields'] as $field) {
                    ...
                    if ($save == 'meta') {
                        $specific = ['menu_order', 'post_parent'];
                    }
        ...
        function ajax_move_field()
        {
            ...
            if ($args['field_id'] && $args['field_group_id']) {
                ...
                $a =
                    '<a href="' .
                    admin_url(
                        "post.php?post={$field_group['ID']}&action=edit"
                    ) .
                    '" target="_blank">' .
                    $field_group['title'] .
                    '</a>';
                ...
                echo '<p>' .
                    sprintf(
                        __(
                            'The %s field can now be found in the %s field group',
                            'acf'
                        ),
                        $field['label'],
                        $a
                    ...
                    '</p>';
                echo '<a href="#" class="button button-primary acf-close-popup">' .
                    __("Close Window", 'acf') .
                    '</a>';
                die();
            }
