    ...
    class ACF_Admin_Field_Groups
    {
        ...
        public function get_current_admin_url($params = '')
        {
            return $this->get_admin_url(
                ($this->view ? '&post_status=' . $this->view : '') . $params
            );
        }
        ...
        public function current_screen()
        {
            ...
            $this->view = isset($_GET['post_status'])
            ...
            $wp_post_statuses['trash'] = acf_extract_var(
                $wp_post_statuses,
                'trash'
            );
            ...
            add_filter(
                'views_edit-acf-field-group',
                [$this, 'admin_table_views'],
                10,
                1
            );
            add_filter(
                'manage_acf-field-group_posts_columns',
                [$this, 'admin_table_columns'],
                10,
                1
            );
            add_action(
                'manage_acf-field-group_posts_custom_column',
                [$this, 'admin_table_columns_html'],
                10,
                2
            );
            add_filter(
            ...
            add_filter(
                'bulk_actions-edit-acf-field-group',
                [$this, 'admin_table_bulk_actions'],
                10,
                1
            );
        ...
        public function admin_enqueue_scripts()
        {
            ...
            // Localize text.
            acf_localize_text([
                'Review local JSON changes' => __(
                    'Review local JSON changes',
                    'acf'
                ),
            ...
            switch ($column_name) {
                ...
                case 'acf-description':
                    if ($field_group['description']) {
                        echo '<span class="acf-description">' .
                            acf_esc_html($field_group['description']) .
                            '</span>';
                    }
                    break;
            ...
            if ($field_group['location']) {
                foreach ($field_group['location'] as $i => $rules) {
                    ...
                    foreach ($rules as $j => $rule) {
                        ...
                        if ($location) {
                            $location_object_type = $location->get_object_type(
                                $rule
                            );
                            $location_object_subtype = $location->get_object_subtype(
                                $rule
                            );
                        }
                    ...
                    if (count($object_subtypes) > 1) {
                        $object_subtypes = call_user_func_array(
                            'array_intersect',
                            $object_subtypes
                        );
                        $object_subtypes = array_values($object_subtypes);
            ...
            if (isset($json[$field_group['key']])) {
                ...
                if (isset($this->sync[$field_group['key']])) {
                    ...
                    if ($field_group['ID']) {
						...
						<span class="sync"><a href="' .
                            esc_url($url) .
                            '">' .
                            __('Sync', 'acf') .
                            '</a> | </span>
						<span class="review"><a href="#" data-event="review-sync" data-id="' .
                            esc_attr($field_group['ID']) .
                            '" data-href="' .
                            esc_url($url) .
                            '">' .
                            __('Review changes', 'acf') .
                            '</a></span>
					</div>';
                    } else {
                        echo '<div class="row-actions">
                ...
                echo '<span class="acf-secondary-text">' .
                    __('Awaiting save', 'acf') .
                    '</span>';
            }
        }
        ...
        public function page_row_actions($actions, $post)
        {
            ...
            $duplicate_action_url = $this->get_admin_url(
                '&acfduplicate=' .
                    $post->ID .
                    '&_wpnonce=' .
                    wp_create_nonce('bulk-posts')
            ...
            $actions['acfduplicate'] =
                '<a href="' .
                esc_url($duplicate_action_url) .
                '" aria-label="' .
                esc_attr__('Duplicate this item', 'acf') .
                '">' .
                __('Duplicate', 'acf') .
                '</a>';
        ...
        public function check_duplicate()
        {
            ...
            if (isset($_GET['acfduplicatecomplete'])) {
                $ids = array_map(
                    'intval',
                    explode(',', $_GET['acfduplicatecomplete'])
                );
            ...
            if (isset($_GET['acfduplicate'])) {
                ...
                isset($_GET['post'], $_GET['action2']) &&
                $_GET['action2'] === 'acfduplicate'
            ) {
            ...
            if ($ids) {
                ...
                wp_redirect(
                    $this->get_admin_url(
                        '&acfduplicatecomplete=' . implode(',', $new_ids)
                    )
                );
                exit();
            }
        ...
        public function check_sync()
        {
            ...
            if (isset($_GET['acfsynccomplete'])) {
                $ids = array_map(
                    'intval',
                    explode(',', $_GET['acfsynccomplete'])
                );

                // Generate text.
                $text = sprintf(
                    _n(
                        'Field group synchronised.',
                        '%s field groups synchronised.',
                        count($ids),
                        'acf'
                    ),
                    count($ids)
            ...
            if ($keys && $this->sync) {
                ...
                foreach ($this->sync as $key => $field_group) {
                    if (
                        $field_group['key'] &&
                        in_array($field_group['key'], $keys)
                    ) {
                        // Import.
                    } elseif (
                        $field_group['ID'] &&
                        in_array($field_group['ID'], $keys)
                    ) {
                        // Import.
                    } else {
                        // Ignore.
                ...
                wp_redirect(
                    $this->get_current_admin_url(
                        '&acfsynccomplete=' . implode(',', $new_ids)
                    )
                );
                exit();
            }
...
<table>
	...
	<?php foreach ($this->sync as $k => $field_group) {
     echo '<tr>';
     foreach ($columns as $column_name => $column_label) {
         ...
         switch ($column_name) {
             // Checkbox.
             case 'cb':
                 echo '<label for="cb-select-' .
                     esc_attr($k) .
                     '" class="screen-reader-text">' .
                     esc_html(
                         sprintf(__('Select %s', 'acf'), $field_group['title'])
                     ) .
                     '</label>';
                 echo '<input id="cb-select-' .
                     esc_attr($k) .
                     '" type="checkbox" value="' .
                     esc_attr($k) .
                     '" name="post[]">';
             ...
             case 'title':
                 ...
                 if (!$field_group['active']) {
                     $post_state =
                         ...
                         $this->get_disabled_post_state() .
                         '</span>';
                 }
                 echo '<strong><span class="row-title">' .
                     esc_html($field_group['title']) .
                     '</span>' .
                     $post_state .
                     '</strong>';
                 echo '<div class="row-actions"><span class="file acf-secondary-text">' .
                     $this->get_human_readable_file_location(
                         $field_group['local_file']
                     ) .
                     '</span></div>';
