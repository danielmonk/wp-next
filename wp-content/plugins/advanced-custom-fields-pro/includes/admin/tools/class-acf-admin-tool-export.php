...
if (!class_exists('ACF_Admin_Tool_Export')):
    class ACF_Admin_Tool_Export extends ACF_Admin_Tool
    {
        ...
        function submit_download()
        {
            ...
            // validate
            if ($json === false) {
                return acf_add_admin_notice(
                    __("No field groups selected", 'acf'),
                    'warning'
                );
            }
        ...
        function load()
        {
            ...
            if ($this->is_active()) {
                ...
                if ($selected) {
                    ...
                    $text = sprintf(
                        _n(
                            'Exported 1 field group.',
                            'Exported %s field groups.',
                            $count,
                            'acf'
                        ),
                        $count
        ...
        function html_field_selection()
        {
            ...
            if ($field_groups) {
                foreach ($field_groups as $field_group) {
                    $choices[$field_group['key']] = esc_html(
                        $field_group['title']
                    );
                }
            }
		...
		<div class="acf-panel acf-panel-selection">
			<h3 class="acf-panel-title"><?php _e(
       'Select Field Groups',
       'acf'
   ); ?> <i class="dashicons dashicons-arrow-right"></i></h3>
...
*/
        ...
        function html_archive()
        {
            ?>
		<p><?php _e(
      'Select the field groups you would like to export and then select your export method. Use the download button to export to a .json file which you can then import to another ACF installation. Use the generate button to export to PHP code which you can place in your theme.',
      'acf'
  ); ?></p>
		...
		<p class="acf-submit">
			<button type="submit" name="action" class="button button-primary" value="download"><?php _e(
       'Export File',
       'acf'
   ); ?></button>
			<button type="submit" name="action" class="button" value="generate"><?php _e(
       'Generate PHP',
       'acf'
   ); ?></button>
		</p>
		...
		<div class="acf-postbox-columns">
			...
			<div class="acf-postbox-side">
				...
				<p class="acf-submit">
					<button type="submit" name="action" class="button button-primary" value="generate"><?php _e(
         'Generate PHP',
         'acf'
     ); ?></button>
				</p>
  ...
  ?></textarea>
		<p class="acf-submit">
			<a class="button" id="acf-export-copy"><?php _e(
       'Copy to clipboard',
       'acf'
   ); ?></a>
		</p>
