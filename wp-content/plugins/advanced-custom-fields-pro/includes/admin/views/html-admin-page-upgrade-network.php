...
<div id="acf-upgrade-wrap" class="wrap">
	...
	<p><?php echo sprintf(
     ...
     '"' . __('Upgrade Sites', 'acf') . '"'
 ); ?></p>
	<p><input type="submit" name="upgrade" value="<?php _e(
     'Upgrade Sites',
     'acf'
 ); ?>" class="button" id="upgrade-sites"></p>
      ...
      foreach ($sites as $i => $site):
          ...
          switch_to_blog($site['blog_id']); ?>
			<tr<?php if ($i % 2 == 0): ?> class="alternate"<?php endif; ?>>
				...
				<td>
					<strong><?php echo get_bloginfo(
         'name'
     ); ?></strong><br /><?php echo home_url(); ?>
				</td>
				<td>
				<?php if (acf_has_upgrade()): ?>
  ...
  ?>
	...
	<p><input type="submit" name="upgrade" value="<?php _e(
     'Upgrade Sites',
     'acf'
 ); ?>" class="button" id="upgrade-sites-2"></p>
	...
	(function($) {
		...
		var upgrader = new acf.Model({
			...
			onClick: function( e, $el ){
				...
				if( !this.$inputs().length ) {
					return alert('<?php _e(
         'Please select at least one site to upgrade.',
         'acf'
     ); ?>');
				}
        ...
        'acf'
			...
			upgrade: function(){
				...
				// show loading
				$row.find('.response').html('<i class="acf-loading"></i></span> <?php printf(
