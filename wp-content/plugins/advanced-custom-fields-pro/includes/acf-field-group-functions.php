 ...
 */
function _acf_apply_unique_field_group_slug(
    $slug,
    $post_ID,
    $post_status,
    $post_type,
...
function acf_flush_field_group_cache($field_group)
{
    ...
    wp_cache_delete(
        acf_cache_key("acf_get_field_group_post:key:{$field_group['key']}"),
        'acf'
    );
 ...
 * @return string.
 */
function _acf_untrash_field_group_post_status(
    $new_status,
    $post_id,
    $previous_status
) {
    return get_post_type($post_id) === 'acf-field-group'
        ? $previous_status
        : $new_status;
}
...
function acf_duplicate_field_group($id = 0, $new_post_id = 0)
{
    ...
    if (!$field_group['ID']) {
        $field_group['ID'] = wp_insert_post([
            'post_title' => $field_group['key'],
        ]);
    }
...
function acf_prepare_field_group_for_export($field_group = [])
{
    ...
    // Prepare fields.
    $field_group['fields'] = acf_prepare_fields_for_export(
        $field_group['fields']
    );
...
function acf_import_field_group($field_group)
{
    ...
    if ($field_group['ID']) {
        ...
        $db_fields = acf_prepare_fields_for_import(
