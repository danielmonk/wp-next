...
if (!class_exists('acf_field_select')):
    class acf_field_select extends acf_field
    {
        ...
        function initialize()
        {
            ...
            // ajax
            add_action('wp_ajax_acf/fields/select/query', [
                $this,
                'ajax_query',
            ]);
            add_action('wp_ajax_nopriv_acf/fields/select/query', [
                $this,
                'ajax_query',
            ]);
        }
            ...
            acf_localize_data([
                'select2L10n' => [
                    'matches_1' => _x(
                        ...
                        'Select2 JS matches_1',
                        'acf'
                    ),
                    'matches_n' => _x(
                        '%d results are available, use up and down arrow keys to navigate.',
                        'Select2 JS matches_n',
                        'acf'
                    ),
                    'matches_0' => _x(
                        'No matches found',
                        'Select2 JS matches_0',
                        'acf'
                    ),
                    'input_too_short_1' => _x(
                        'Please enter 1 or more characters',
                        'Select2 JS input_too_short_1',
                        'acf'
                    ),
                    'input_too_short_n' => _x(
                        'Please enter %d or more characters',
                        'Select2 JS input_too_short_n',
                        'acf'
                    ),
                    'input_too_long_1' => _x(
                        'Please delete 1 character',
                        'Select2 JS input_too_long_1',
                        'acf'
                    ),
                    'input_too_long_n' => _x(
                        'Please delete %d characters',
                        'Select2 JS input_too_long_n',
                        'acf'
                    ),
                    'selection_too_long_1' => _x(
                        'You can only select 1 item',
                        'Select2 JS selection_too_long_1',
                        'acf'
                    ),
                    'selection_too_long_n' => _x(
                        'You can only select %d items',
                        'Select2 JS selection_too_long_n',
                        'acf'
                    ),
                    'load_more' => _x(
                        'Loading more results&hellip;',
                        'Select2 JS load_more',
                        'acf'
                    ),
                    'searching' => _x(
                        'Searching&hellip;',
                        'Select2 JS searching',
                        'acf'
                    ),
                    'load_fail' => _x(
                        'Loading failed',
                        'Select2 JS load_fail',
                        'acf'
                    ),
                ],
            ]);
        ...
        function render_field_settings($field)
        {
            ...
            $field['default_value'] = acf_encode_choices(
                $field['default_value'],
                false
            );
            ...
            acf_render_field_setting($field, [
                ...
                'instructions' =>
                    __('Enter each choice on a new line.', 'acf') .
                    '<br /><br />' .
                    __(
        ...
        function update_field($field)
        {
            ...
            $field['default_value'] = acf_decode_choices(
                $field['default_value'],
                true
            );
        ...
        function format_value($value, $post_id, $field)
        {
            if (is_array($value)) {
                foreach ($value as $i => $val) {
                    $value[$i] = $this->format_value_single(
                        $val,
                        $post_id,
                        $field
                    );
