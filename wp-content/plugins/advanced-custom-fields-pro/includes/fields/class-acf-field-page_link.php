...
if (!class_exists('acf_field_page_link')):
    class acf_field_page_link extends acf_field
    {
        ...
        function initialize()
        {
            ...
            // extra
            add_action('wp_ajax_acf/fields/page_link/query', [
                $this,
                'ajax_query',
            ]);
            add_action('wp_ajax_nopriv_acf/fields/page_link/query', [
                $this,
                'ajax_query',
            ]);
        }
        ...
        function ajax_query()
        {
            ...
            $args = apply_filters(
                'acf/fields/page_link/query/name=' . $field['name'],
                $args,
                $field,
                $options['post_id']
            );
            $args = apply_filters(
                'acf/fields/page_link/query/key=' . $field['key'],
                $args,
                $field,
                $options['post_id']
            );
        ...
        function get_post_title($post, $field, $post_id = 0, $is_search = 0)
        {
            ...
            $title = apply_filters(
                'acf/fields/page_link/result/name=' . $field['_name'],
                $title,
                $post,
                $field,
                $post_id
            ...
            $title = apply_filters(
                'acf/fields/page_link/result/key=' . $field['key'],
                $title,
                $post,
                $field,
                $post_id
                ...
                if (!empty($posts)) {
                    foreach (array_keys($posts) as $i) {
                        ...
                        if (is_object($post)) {
                            ...
                            $field['choices'][
                                $post->ID
                            ] = $this->get_post_title($post, $field);
                        } else {
                            // append to choices
