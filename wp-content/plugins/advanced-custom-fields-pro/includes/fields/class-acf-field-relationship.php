...
if (!class_exists('acf_field_relationship')):
    class acf_field_relationship extends acf_field
    {
        ...
        function initialize()
        {
            ...
            // extra
            add_action('wp_ajax_acf/fields/relationship/query', [
                $this,
                'ajax_query',
            ]);
            add_action('wp_ajax_nopriv_acf/fields/relationship/query', [
                $this,
                'ajax_query',
            ]);
        }
        ...
        function input_admin_enqueue_scripts()
        {
            ...
            acf_localize_text([
                ...
                'Maximum values reached ( {max} values )' => __(
        ...
        function get_ajax_query($options = [])
        {
            ...
            $args = apply_filters(
                'acf/fields/relationship/query/key=' . $field['key'],
                $args,
                $field,
                $options['post_id']
            );
            ...
            foreach (array_keys($groups) as $group_title) {
                ...
                foreach (array_keys($posts) as $post_id) {
                    $posts[$post_id] = $this->get_post_title(
                        $posts[$post_id],
                        $field,
                        $options['post_id']
                    );
                ...
                // append to $data
                foreach (array_keys($posts) as $post_id) {
                    $data['children'][] = $this->get_post_result(
                        $post_id,
                        $posts[$post_id]
                    );
                }
        ...
        function get_post_title($post, $field, $post_id = 0, $is_search = 0)
        {
            ...
            $title = apply_filters(
                'acf/fields/relationship/result/key=' . $field['key'],
                $title,
                $post,
                $field,
                $post_id
        ...
        function render_field($field)
        {
            ...
            if (in_array('taxonomy', $filters)) {
                ...
                if ($taxonomy) {
                    ...
                    $term_choices = acf_get_choices_from_grouped_terms(
                        $terms,
                        'slug'
                    );
                }
                ...
                // append term choices
                $filter_taxonomy_choices =
                    $filter_taxonomy_choices + $term_choices;
            }
