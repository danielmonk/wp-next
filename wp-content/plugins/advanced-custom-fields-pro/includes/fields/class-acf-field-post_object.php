...
if (!class_exists('acf_field_post_object')):
    class acf_field_post_object extends acf_field
    {
        ...
        function initialize()
        {
            ...
            // extra
            add_action('wp_ajax_acf/fields/post_object/query', [
                $this,
                'ajax_query',
            ]);
            add_action('wp_ajax_nopriv_acf/fields/post_object/query', [
                $this,
                'ajax_query',
            ]);
        }
        ...
        function get_ajax_query($options = [])
        {
            ...
            $args = apply_filters(
                'acf/fields/post_object/query/name=' . $field['name'],
                $args,
                $field,
                $options['post_id']
            );
            $args = apply_filters(
                'acf/fields/post_object/query/key=' . $field['key'],
                $args,
                $field,
                $options['post_id']
            );
        ...
        function get_post_title($post, $field, $post_id = 0, $is_search = 0)
        {
            ...
            $title = apply_filters(
                'acf/fields/post_object/result/name=' . $field['_name'],
                $title,
                $post,
                $field,
                $post_id
            ...
            $title = apply_filters(
                'acf/fields/post_object/result/key=' . $field['key'],
                $title,
                $post,
                $field,
                $post_id
        ...
        function render_field($field)
        {
            ...
            if ($posts) {
                foreach (array_keys($posts) as $i) {
                    ...
                    // append to choices
                    $field['choices'][$post->ID] = $this->get_post_title(
                        $post,
                        $field
                    );
                }
