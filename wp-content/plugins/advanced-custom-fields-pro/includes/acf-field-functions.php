...
function acf_render_field_label($field)
{
    ...
    if ($label) {
        echo '<label' .
            ($field['id'] ? ' for="' . esc_attr($field['id']) . '"' : '') .
            '>' .
            acf_esc_html($label) .
            '</label>';
    }
...
function acf_update_field($field, $specific = [])
{
    ...
    if ($field['conditional_logic']) {
        ...
        $field['conditional_logic'] = array_map(
            'array_filter',
 ...
 */
function _acf_apply_unique_field_slug(
    $slug,
    $post_ID,
    $post_status,
    $post_type,
...
function acf_flush_field_cache($field)
{
    ...
    wp_cache_delete(
        acf_cache_key("acf_get_field_post:name:{$field['name']}"),
        'acf'
    );
    wp_cache_delete(
        acf_cache_key("acf_get_field_post:key:{$field['key']}"),
        'acf'
    );

    // Flush cached array of post_ids for this field's parent.
    wp_cache_delete(
        acf_cache_key("acf_get_field_posts:{$field['parent']}"),
        'acf'
    );
}
...
{
    return get_post_type($post_id) === 'acf-field'
        ? $previous_status
        : $new_status;
}
    ...
    if ($field['conditional_logic']) {
        ...
        foreach ($field['conditional_logic'] as $group_i => $group) {
            ...
            foreach ($group as $rule_i => $rule) {
                $field['conditional_logic'][$group_i][$rule_i]['field'] = isset(
                    $keys[$rule['field']]
                )
                    ? $keys[$rule['field']]
                    : $rule['field'];
