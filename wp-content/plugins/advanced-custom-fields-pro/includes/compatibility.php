...
if (!class_exists('ACF_Compatibility')):
    class ACF_Compatibility
    {
        ...
        function __construct()
        {
            ...
            add_filter(
                'acf/validate_field/type=textarea',
                [$this, 'validate_textarea_field'],
                20,
                1
            );
            add_filter(
                'acf/validate_field/type=relationship',
                [$this, 'validate_relationship_field'],
                20,
                1
            );
            add_filter(
                'acf/validate_field/type=post_object',
                [$this, 'validate_relationship_field'],
                20,
                1
            );
            add_filter(
                'acf/validate_field/type=page_link',
                [$this, 'validate_relationship_field'],
                20,
                1
            );
            add_filter(
                'acf/validate_field/type=image',
                [$this, 'validate_image_field'],
                20,
                1
            );
            add_filter(
            ...
            add_filter(
                'acf/validate_field/type=wysiwyg',
                [$this, 'validate_wysiwyg_field'],
                20,
                1
            );
            add_filter(
                'acf/validate_field/type=date_picker',
                [$this, 'validate_date_picker_field'],
                20,
                1
            );
            add_filter(
                'acf/validate_field/type=taxonomy',
                [$this, 'validate_taxonomy_field'],
                20,
                1
            );
            add_filter(
                'acf/validate_field/type=date_time_picker',
                [$this, 'validate_date_time_picker_field'],
                20,
                1
            );
            add_filter(
                'acf/validate_field/type=user',
                [$this, 'validate_user_field'],
                20,
                1
            );
            add_filter(
            ...
            // location
            add_filter(
                'acf/location/validate_rule/type=post_taxonomy',
                [$this, 'validate_post_taxonomy_location_rule'],
                20,
                1
            );
            add_filter(
                'acf/location/validate_rule/type=post_category',
                [$this, 'validate_post_taxonomy_location_rule'],
                20,
                1
            );
        ...
        function validate_field($field)
        {
            ...
            if (isset($field['conditional_logic']['status'])) {
                ...
                if ($field['conditional_logic']['status']) {
                    $field['conditional_logic'] = acf_convert_rules_to_groups(
                        $field['conditional_logic']['rules'],
                        $field['conditional_logic']['allorany']
                    );
                } else {
        ...
        function validate_image_field($field)
        {
            ...
            if (isset($field['save_format'])) {
                $field['return_format'] = acf_extract_var(
                    $field,
                    'save_format'
                );
            }
        ...
        function validate_taxonomy_field($field)
        {
            ...
            if (isset($field['load_save_terms'])) {
                $field['save_terms'] = acf_extract_var(
                    $field,
                    'load_save_terms'
                );
            }
        ...
        function validate_field_group($field_group)
        {
            ...
            if (!$field_group['key']) {
                $version = 4;
                $field_group['key'] = isset($field_group['id'])
                    ? "group_{$field_group['id']}"
                    : uniqid('group_');
            }
            ...
            if (isset($field_group['location']['rules'])) {
                $field_group['location'] = acf_convert_rules_to_groups(
                    $field_group['location']['rules'],
                    $field_group['location']['allorany']
                );
            }
                    ...
                    if ($group) {
                        foreach ($group as $j => $rule) {
                            ...
                            if (isset($replace[$rule['param']])) {
                                $field_group['location'][$i][$j]['param'] =
                                    $replace[$rule['param']];
                            }
                        }
                    }
