...
function acf_maybe_get_sub_field($selectors, $post_id = false, $strict = true)
{
    ...
    for ($j = 0; $j < count($selectors); $j += 2) {
        ...
        // add to name
        $field['name'] =
            $field_name . '_' . ($sub_i - $offset) . '_' . $field['name'];
    }
...
function have_rows($selector, $post_id = false)
{
    ...
    if (!$active_loop) {
        ...
        if ($sub_field) {
            $sub_field_exists = isset(
                $active_loop['value'][$active_loop['i']][$sub_field['key']]
            );
        }
        ...
        if ($post_id != $active_loop['post_id']) {
            ...
            if ($sub_field_exists) {
                ...
                $prev_loop &&
                $prev_loop['selector'] == $selector &&
                $prev_loop['post_id'] == $post_id
            ) {
                acf_remove_loop('active');
                $active_loop = $prev_loop;
    ...
    if ($new_loop) {
        ...
        if ($new_loop === 'parent') {
            ...
            $args[
                'name'
            ] = "{$active_loop['name']}_{$active_loop['i']}_{$sub_field['name']}";
        ...
        if (
            acf_get_field_type_prop($args['field']['type'], 'have_rows') ===
            'single'
        ) {
...
function get_sub_field_object(
    ...
    if ($format_value) {
        // get value for field
        $sub_field['value'] = acf_format_value(
            $sub_field['value'],
            $row['post_id'],
            $sub_field
        );
