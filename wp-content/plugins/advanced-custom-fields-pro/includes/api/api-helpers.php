...
{
    echo '<input type="hidden" name="_acf_nonce" value="' .
...
function acf_get_image_size($s = '')
{
    ...
    // vars
    $data = [
        'width' => isset($_sizes[$s]['width'])
            ? $_sizes[$s]['width']
            : get_option("{$s}_size_w"),
        'height' => isset($_sizes[$s]['height'])
            ? $_sizes[$s]['height']
        ...
        if ($file['width']) {
            if ($min_width && $file['width'] < $min_width) {
                ...
                $errors['min_width'] = sprintf(
                    __('Image width must be at least %dpx.', 'acf'),
                    $min_width
                );
            } elseif ($max_width && $file['width'] > $max_width) {
                // min width
                $errors['max_width'] = sprintf(
                    __('Image width must not exceed %dpx.', 'acf'),
                    $max_width
                );
            }
        }
        ...
        if ($file['height']) {
            if ($min_height && $file['height'] < $min_height) {
                ...
                $errors['min_height'] = sprintf(
                    __('Image height must be at least %dpx.', 'acf'),
                    $min_height
                );
            } elseif ($max_height && $file['height'] > $max_height) {
                // min height
                $errors['max_height'] = sprintf(
                    __('Image height must not exceed %dpx.', 'acf'),
                    $max_height
                );
            }
        }
    ...
    if ($file['size']) {
        ...
        if ($min_size && $file['size'] < acf_get_filesize($min_size)) {
            ...
            $errors['min_size'] = sprintf(
                __('File size must be at least %s.', 'acf'),
                acf_format_filesize($min_size)
            );
        } elseif ($max_size && $file['size'] > acf_get_filesize($max_size)) {
            // min width
            $errors['max_size'] = sprintf(
                __('File size must not exceed %s.', 'acf'),
                acf_format_filesize($max_size)
            );
        }
    }
...
function acf_is_row_collapsed($field_key = '', $row_index = 0)
{
    ...
    if ($collapsed === '') {
        $collapsed = acf_extract_var(
            $_COOKIE,
            "acf_collapsed_{$field_key}",
            ''
        );
