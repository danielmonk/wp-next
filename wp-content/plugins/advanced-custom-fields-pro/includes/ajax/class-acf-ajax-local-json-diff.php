    ...
    class ACF_Ajax_Local_JSON_Diff extends ACF_Ajax
    {
        ...
        function get_response($request)
        {
            ...
            if (!$id) {
                return new WP_Error(
                    'acf_invalid_param',
                    __('Invalid field group parameter(s).', 'acf'),
                    ['status' => 404]
                );
            }
            ...
            if (!$field_group) {
                return new WP_Error(
                    'acf_invalid_id',
                    __('Invalid field group ID.', 'acf'),
                    ['status' => 404]
                );
            }
            $field_group['fields'] = acf_get_fields($field_group);
            $field_group['modified'] = get_post_modified_time(
                'U',
                true,
                $field_group['ID']
            );
            ...
            if (!isset($files[$key])) {
                return new WP_Error(
                    'acf_cannot_compare',
                    __(
                        'Sorry, this field group is unavailable for diff comparison.',
                        'acf'
                    ),
                    ['status' => 404]
                );
		...
		<div class="acf-diff">
			<div class="acf-diff-title">
				<div class="acf-diff-title-left">
					<strong>' .
                __('Original field group', 'acf') .
                '</strong>
					<span>' .
                sprintf(
                    $date_template,
                    wp_date($date_format, $field_group['modified'])
                ) .
                '</span>
				</div>
				<div class="acf-diff-title-right">
					<strong>' .
                __('JSON field group (newer)', 'acf') .
                '</strong>
					<span>' .
                sprintf(
                    $date_template,
                    wp_date($date_format, $local_field_group['modified'])
                ) .
                '</span>
				</div>
			</div>
			<div class="acf-diff-content">
				' .
                wp_text_diff(
                    acf_json_encode($field_group),
                    acf_json_encode($local_field_group)
                ) .
                '
			</div>
		</div>';
