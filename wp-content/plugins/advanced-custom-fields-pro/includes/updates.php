...
if (!class_exists('ACF_Updates')):
    class ACF_Updates
    {
        ...
        function get_plugin_updates($force_check = false)
        {
            ...
            if (!$force_check) {
                ...
                if (is_array($transient)) {
                    $transient_checked = isset($transient['checked'])
                        ? $transient['checked']
                        : [];
                    if (
                        wp_json_encode($checked) !==
                        wp_json_encode($transient_checked)
                    ) {
                        $transient = false;
                    }
                }
        ...
        function modify_plugins_transient($transient)
        {
            ...
            $force_check =
                $this->checked == 0 ? !empty($_GET['force-check']) : false;
