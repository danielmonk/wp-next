...
if (!class_exists('ACF_Assets')):
    class ACF_Assets
    {
        ...
        public function __call($name, $arguments)
        {
            switch ($name) {
                ...
                case 'admin_print_footer_scripts':
                    _doing_it_wrong(
                        __FUNCTION__,
                        'The ACF_Assets class should not be accessed directly.',
                        '5.9.0'
                    );
            }
        }
        ...
        public function register_scripts()
        {
            ...
            wp_register_script(
                'acf',
                acf_get_url('assets/js/acf' . $suffix . '.js'),
                ['jquery'],
                $version
            );
            wp_register_script(
                'acf-input',
                acf_get_url('assets/js/acf-input' . $suffix . '.js'),
                ['jquery', 'jquery-ui-sortable', 'jquery-ui-resizable', 'acf'],
                $version
            );
            wp_register_script(
                'acf-field-group',
                acf_get_url('assets/js/acf-field-group' . $suffix . '.js'),
                ['acf-input'],
                $version
            );
            ...
            wp_register_style(
                'acf-global',
                acf_get_url('assets/css/acf-global.css'),
                [],
                $version
            );
            wp_register_style(
                'acf-input',
                acf_get_url('assets/css/acf-input.css'),
                ['acf-global'],
                $version
            );
            wp_register_style(
                'acf-field-group',
                acf_get_url('assets/css/acf-field-group.css'),
                ['acf-input'],
                $version
            );
        ...
        public function enqueue_scripts()
        {
            ...
            if (wp_script_is('acf-input')) {
                acf_localize_text([
                    // Unload
                    'The changes you made will be lost if you navigate away from this page' => __(
                        'The changes you made will be lost if you navigate away from this page',
                        'acf'
                    ),

                    // Validation
                    'Validation successful' => __(
                        'Validation successful',
                        'acf'
                    ),
                    'Validation failed' => __('Validation failed', 'acf'),
                    '1 field requires attention' => __(
                        '1 field requires attention',
                        'acf'
                    ),
                    '%d fields require attention' => __(
