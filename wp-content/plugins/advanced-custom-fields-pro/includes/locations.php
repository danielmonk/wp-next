...
function acf_register_location_type($class_name)
{
    ...
    if ($store->has($name)) {
        $message = sprintf(
            __('Location type "%s" is already registered.'),
            $name
        );
        _doing_it_wrong(__FUNCTION__, $message, '5.9.0');
        return false;
...
function acf_get_location_rule_types()
{
    ...
    foreach ($location_types as $location_type) {
        ...
        // Append
        $types[$category][$location_type->name] = esc_html(
            $location_type->label
        );
    }
...
function acf_validate_location_rule($rule = [])
{
     ...
     */
    $rule = apply_filters(
        "acf/location/validate_rule/type={$rule['param']}",
        $rule
    );
...
function acf_match_location_rule($rule, $screen, $field_group)
{
    ...
    $result = apply_filters(
        "acf/location/match_rule/type={$rule['param']}",
        $result,
        $rule,
        $screen,
        $field_group
    ...
    $result = apply_filters(
        "acf/location/rule_match/{$rule['param']}",
        $result,
        $rule,
        $screen,
        $field_group
