...
class MonsterInsights_Page_Insights
{
    ...
    private function __construct()
    {
        ...
        // Load the updater.
        add_action('monsterinsights_updater', [$this, 'updater']);

        // Load the plugin.
        add_action('monsterinsights_load_plugins', [$this, 'init'], 99);
        ...
        if (!defined('MONSTERINSIGHTS_PRO_VERSION')) {
            ...
            add_filter(
                ...
                [$this, 'modify_autoupdater_setting_html'],
                11,
                2
            );
        }
    ...
    public static function get_instance()
    {
        if (
            !isset(self::$instance) &&
            !(self::$instance instanceof MonsterInsights_Page_Insights)
        ) {
    ...
    public function init()
    {
        ...
        if (version_compare(MONSTERINSIGHTS_VERSION, '7.3.0', '<')) {
            ...
            add_action('admin_notices', [
                self::$instance,
                'requires_monsterinsights_version',
            ]);
    ...
    {
        // The caching class.
        require_once plugin_dir_path($this->file) .
            'includes/class-monsterinsights-page-insights-cache.php';
        // Load the background data fetcher.
        require_once plugin_dir_path($this->file) .
            'includes/class-monsterinsights-page-insights-background.php';

        // Load the admin interface.
        require_once plugin_dir_path($this->file) .
            'includes/admin/class-monsterinsights-page-insights-admin.php';
        new MonsterInsights_Page_Insights_Admin();

        // Load the report.
        require_once plugin_dir_path($this->file) .
            'includes/admin/reports/class-monsterinsights-report-page-insights.php';
    }
    ...
    {
        require_once plugin_dir_path($this->file) .
            'includes/class-monsterinsights-page-insights-ajax.php';
    ...
    public function modify_autoupdater_setting_html($html, $plugin_file)
    {
        if (
            ...
            !apply_filters(
                "monsterinsights_is_autoupdate_setting_html_filtered_${plugin_file}",
                false
            )
        ) {
            $html = sprintf(
                ...
                __(
                    'Enable the MonsterInsights PRO plugin to manage auto-updates',
    ...
    {
        require_once plugin_dir_path($this->file) .
            'includes/class-monsterinsights-page-insights-install.php';
        ...
        add_action(
            'wpmu_new_blog',
            ['MonsterInsights_Page_Insights_Install', 'install_new_blog'],
            10,
            6
        );
    }
    ...
    public function requires_monsterinsights()
    {
		...
		<div class="error">
			<p><?php esc_html_e(
       'Please install MonsterInsights Pro to use the MonsterInsights Page Insights addon',
       'monsterinsights-page-insights'
   ); ?></p>
		</div>
		<?php
    ...
    public function requires_monsterinsights_version()
    {
		...
		<div class="error">
			<p><?php esc_html_e(
       'Please install or update MonsterInsights Pro with version 7.4.0 or newer to use the MonsterInsights Page Insights addon',
       'monsterinsights-page-insights'
   ); ?></p>
		</div>
		<?php
