...
final class MonsterInsights_Report_Page_Insights extends MonsterInsights_Report
{
    ...
    public function get_data($args = [])
    {
        ...
        if (is_numeric($_REQUEST['post_id'])) {
            ...
            $author_can_view =
                ...
                apply_filters(
                    'monsterinsights_pageinsights_author_can_view',
                    false
                );
            if (
                !$author_can_view &&
                ...
                return [
                    ...
                    'error' => sprintf(
                        esc_html__(
                            'You are not allowed to view reports for this %s.',
                            'monsterinsights-pageinsights'
                        ),
            ...
            if (!current_user_can('edit_others_posts')) {
                return [
                    ...
                    // Translators: %s is the name of the post type.
                    'error' => esc_html__(
                        'You are not allowed to view reports for this page.',
                        'monsterinsights-pageinsights'
                    ),
                    'data' => [],
                ];
        ...
        $check_cache = MonsterInsights_Page_Insights_Cache::get_instance()->get(
            $this->path
        );
        ...
        $api = new MonsterInsights_API_Request(
            'analytics/reports/' . $this->name . '/single',
            $api_options,
            'GET'
        );
        ...
        if (is_wp_error($report_data)) {
            ...
            if (!empty($report_data['data']['needs_more'])) {
                ...
                MonsterInsights_Page_Insights_Cache::get_instance()->set(
                    $report_data['data']['page_path'],
                    $report_data['data']
                );
     ...
     * @return string
     */
    public function get_report_html($data = [])
    {
        ...
        ?>
		<div class="monsterinsights-pageinsights-report-content monsterinsights-pageinsights-interval-<?php echo esc_attr(
      $interval
  ); ?>">
			<?php
   $report_data = [];
   ...
   $report_data = wp_parse_args(
       $report_data,
       self::get_default_metrics_value()
   );
   $labels = self::get_metrics_labels();

   foreach ($report_data as $metric_name => $metric_value) {

       $label = isset($labels[$metric_name])
           ? $labels[$metric_name]
           : $metric_name;
       $metric_value = self::prepare_metric($metric_value, $metric_name);
       ?>
				<div class="monsterinsights-pageinsights-report-box monsterinsights-pageinsights-report-<?php echo esc_attr(
        $metric_name
    ); ?>">
					<span class="monsterinsights-pageinsights-report-metric"><?php echo esc_html(
         $label
     ); ?></span>
					<span class="monsterinsights-pageinsights-report-value"><?php echo esc_html(
         $metric_value
     ); ?></span>
				</div>
				<?php
   ...
   ?>
    ...
    public function get_path_by_post_id($post_id)
    {
        if (!isset($this->path)) {
            ...
            $this->path = apply_filters(
                'monsterinsights_pageinsights_path',
                $path,
                $post_id
            );
        }
    ...
    {
        return apply_filters(
            'monsterinsights_pageinsights_report_metrics_default',
            [
                'bouncerate' => 0,
                'entrances' => 0,
                'pageviews' => 0,
    ...
    {
        return apply_filters(
            'monsterinsights_pageinsights_report_metrics_labels',
            [
                'bouncerate' => esc_html__(
                    'Bounce Rate',
                    'monsterinsights-pageinsights'
                ),
                'entrances' => esc_html__(
                    'Entrances',
                    'monsterinsights-pageinsights'
                ),
                'pageviews' => esc_html__(
                    'Page Views',
                    'monsterinsights-pageinsights'
                ),
                'timeonpage' => esc_html__(
                    'Time on Page',
                    'monsterinsights-pageinsights'
                ),
                'pageloadtime' => esc_html__(
                    'Page Load Time',
                    'monsterinsights-pageinsights'
                ),
    ...
    public static function prepare_metric($value, $name)
    {
        ...
        $value = apply_filters(
            'monsterinsights_pageinsights_prepare_metric',
            $value,
            $name
        );
        ...
        if (!empty($data['data'])) {
            if (isset($data['data']['yesterday'])) {
                foreach ($data['data']['yesterday'] as $key => $value) {
                    $data['data']['yesterday'][$key] = self::prepare_metric(
                        $value,
                        $key
                    );
                }
            ...
            if (isset($data['data']['30days'])) {
                foreach ($data['data']['30days'] as $key => $value) {
                    $data['data']['30days'][$key] = self::prepare_metric(
                        $value,
                        $key
                    );
                }
