...
final class MonsterInsights_Page_Insights_Admin
{
    ...
    public function __construct()
    {
        add_action('admin_init', [
            $this,
            'add_admin_column_for_public_post_types',
        ]);
        ...
        add_action('admin_footer', [$this, 'add_reports_markup']);

        // Clear the cache when the profile is updated.
        add_action('update_option_monsterinsights_site_profile', [
            $this,
            'clear_cache',
        ]);
        add_action('update_site_option_monsterinsights_network_profile', [
            $this,
            'clear_network_cache',
        ]);
    }
    ...
    public function get_post_types()
    {
        if (!isset($this->post_types)) {
            ...
            $this->post_types = apply_filters(
                'monsterinsights_posts_post_types_admin_column',
                $post_types
            );
        }
    ...
    public function add_posts_table_column($columns)
    {
        if (!isset($columns['monsterinsights_reports'])) {
            if (
                ...
                $columns['monsterinsights_reports'] = esc_html__(
                    'Insights',
    ...
    public function custom_column_content($column, $post_id)
    {
        ...
        if ('monsterinsights_reports' === $column) {
            ...
            if (
                ...
                echo '<button class="monsterinsights-reports-loader" type="button" data-post_id="' .
                    esc_attr($post_id) .
                    '" data-title="' .
                    esc_attr($post_title) .
                    '">';
    ...
    public function load_scripts($hook)
    {
        ...
        wp_enqueue_style(
            'monsterinsights_page_insights_styles',
            MONSTERINSIGHTS_PAGE_INSIGHTS_ADDON_PLUGIN_URL .
                'assets/css/page-insights-reports.css',
            [],
            monsterinsights_get_asset_version()
        );

        wp_enqueue_script(
            'monsterinsights_page_insights_script',
            MONSTERINSIGHTS_PAGE_INSIGHTS_ADDON_PLUGIN_URL .
                'assets/js/page-insights-reports.js',
            ['jquery'],
            monsterinsights_get_asset_version(),
            true
        );
        ...
        wp_localize_script(
            'monsterinsights_page_insights_script',
            'monsterinsights_page_insights_admin',
            [
                ...
                'error_default' => esc_html__(
                    'There was an issue loading the report data. Please try again.',
                    'monsterinsights-page-insights'
                ),
            ]
        );
    ...
    public function add_reports_markup()
    {
		...
		<div class="monsterinsights-reports-overlay">
          ...
          MONSTERINSIGHTS_PAGE_INSIGHTS_ADDON_PLUGIN_URL
      ); ?>/assets/img/MonsterInsights-Logo.png" srcset="<?php echo esc_url(
    MONSTERINSIGHTS_PAGE_INSIGHTS_ADDON_PLUGIN_URL
); ?>/assets/img/MonsterInsights-Logo@2x.png 2x" alt="MonsterInsights"/>
					</div>
					<h2 class="monsterinsights-reports-overlay-title"><?php esc_html_e(
         'Page Insights for:',
         'monsterinsights-page-insights'
     ); ?>
          ...
          'monsterinsights-page-insights'
				...
				<div class="monsterinsights-reports-overlay-controls">
					<select id="monsterinsights-report-interval">
						<option value="30days"><?php esc_html_e(
          'Last 30 Days',
          'monsterinsights-page-insights'
      ); ?></option>
						<option value="yesterday"><?php esc_html_e(
          'Yesterday',
          'monsterinsights-page-insights'
      ); ?></option>
					</select>
