function MonsterInsights_Posts_Reports() {
  var self = this,
    ...
    reports_overlay = document.querySelector(
      ".monsterinsights-reports-overlay"
    ),
    $reports_overlay = $(reports_overlay),
    overlay_title = reports_overlay.querySelector(
      ".monsterinsights-reports-overlay-title-text"
    ),
    overlay_content = reports_overlay.querySelector(
      ".monsterinsights-reports-overlay-content"
    ),
    close_button = reports_overlay.querySelector(
      ".monsterinsights-close-overlay"
    ),
    report_interval = reports_overlay.querySelector(
      "#monsterinsights-report-interval"
    ),
    load_element = overlay_content.innerHTML,
    $error_template = $(
      document.getElementById("monsterinsights-pageinsights-error-template")
    ).html();
  ...
  this.set = function (property, value) {
    ...
    if (
      "object" === typeof value &&
      JSON.stringify(this.state[property]) === JSON.stringify(value)
    ) {
      return false;
    }
  ...
  this.add_events = function () {
    posts_table.addEventListener("click", function (event) {
      ...
      if (
        el.classList &&
        el.classList.contains("monsterinsights-reports-loader")
      ) {
  ...
  this.close_overlay = function () {
    ...
    var element = document.querySelector(
      '.monsterinsights-reports-loader[data-post_id="' +
        this.get("post_id") +
        '"]'
    );
    element.focus();
      ...
      .done(function (response) {
        if (response.success && response.data.html) {
          ...
          var text = response.data.message
            ...
            : monsterinsights_page_insights_admin.error_default;
          $(overlay_content).html(
            self.error_markup(
              monsterinsights_page_insights_admin.error_text,
              text,
              false
            )
          );
  ...
  this.error_markup = function (title, text, footer) {
    ...
    error_template
      .find(".monsterinsights-pageinsights-error-title")
      .html(title);
    error_template
      .find(".monsterinsights-pageinsights-error-content")
      .html(text);

    if (footer) {
      error_template
        .find(".monsterinsights-pageinsights-error-footer")
        .html(footer)
        .addClass("visible");
    }
  ...
  this.check_empty_column = function () {
    ...
    var column_data = $(
      ".monsterinsights_reports.column-monsterinsights_reports"
    );
