...
if (!class_exists('myCRED_BP_Group_Leaderboards')):
    final class myCRED_BP_Group_Leaderboards
    {
        ...
        public function __construct()
        {
            ...
            add_action('bp_init', [$this, 'setup_bp']);
            add_filter(
                'mycred_add_finished',
                [$this, 'reset_users_leaderboards'],
                999,
                2
            );
            ...
            add_action('mycred_delete_point_type', [
                $this,
                'delete_cached_leaderboards',
            ]);
            add_action('mycred_bp_leaderboard_remove_type', [
                $this,
                'delete_cached_leaderboards',
            ]);
        }
        ...
        public function define_constants()
        {
            ...
            $this->define(
                'MYCRED_BP_GROUP_LEAD_ROOT',
                plugin_dir_path(__FILE__)
            );
            $this->define(
                'MYCRED_BP_GROUP_LEAD_INCLUDES',
                MYCRED_BP_GROUP_LEAD_ROOT . 'includes/'
            );
        }
        ...
        public function setup_bp()
        {
            ...
            $this->file(
                MYCRED_BP_GROUP_LEAD_INCLUDES .
                    'bpgroup-leaderboard-functions.php'
            );
            $this->file(
                MYCRED_BP_GROUP_LEAD_INCLUDES . 'bpgroup-leaderboard.php'
            );
            ...
            if (is_multisite() && !is_network_admin()) {
                ...
                add_filter('bp_core_admin_tabs', [$this, 'add_settings_tab']);
                add_action('admin_menu', [$this, 'add_settings_page']);
                add_action('network_admin_menu', [$this, 'add_settings_page']);
            } elseif (!is_multisite()) {
                add_action('admin_init', [$this, 'register_settings']);
                add_filter('bp_core_admin_tabs', [$this, 'add_settings_tab']);
                add_action('admin_menu', [$this, 'add_settings_page']);
                add_action('bp_admin_head', [$this, 'admin_head'], 900);
            }

            add_action('groups_join_group', [$this, 'reset_group_leaderboard']);
            add_action('groups_leave_group', [
                $this,
                'reset_group_leaderboard',
            ]);
        }
        ...
        public function reset_users_leaderboards($results, $request)
        {
            ...
            if (
                $results === false ||
                !function_exists('groups_get_user_groups')
            ) {
                return $results;
            }
            ...
            if (!empty($group_memberships['groups'])) {
                foreach ($group_memberships['groups'] as $group_id) {
                    groups_delete_groupmeta(
                        $group_id,
                        'cached_leaderboard' . $request['type']
                    );

                    delete_user_meta(
                        $request['user_id'],
                        '_bp_leaderboard_position' . $group_id
                    );
                }
            }
        ...
        public function reset_group_leaderboard($group_id)
        {
            ...
            $wpdb->delete(
                $wpdb->usermeta,
                ['meta_key' => '_bp_leaderboard_position' . $group_id],
                ['%s']
            );
        }
        ...
        public function delete_cached_leaderboards($point_type)
        {
            ...
            $wpdb->delete(
                ...
                ['meta_key' => 'cached_leaderboard' . $point_type],
                ['%s']
            );
        }
        ...
        {
            register_setting('bp-leaderboards', 'mycred_bp_leaderboards', [
                $this,
                'sanitize_settings',
            ]);
        }
        ...
        public function add_settings_tab($tabs)
        {
            $tabs['99'] = [
                'href' => bp_get_admin_url(
                    add_query_arg(['page' => 'bp-leaderboards'], 'admin.php')
                ),
        ...
        public function add_settings_page()
        {
            add_submenu_page(
                'options-general.php',
                __('Leaderboards', 'bpmycredleaderboard'),
                __('Leaderboards', 'bpmycredleaderboard'),
                'export',
                'bp-leaderboards',
...
<div class="wrap">
	...
	<h1><?php _e(
     'BuddyPress Settings',
     'bpmycredleaderboard'
 ); ?> <a href="https://mycred.me/store/mycred-bp-group-leaderboards/" class="page-title-action" target="_blank"><?php _e(
     'Documentation',
     'bpmycredleaderboard'
 ); ?></a></h1>
	...
	<?php else: ?>
	<h1><?php _e(
     'myCred BP Group Leaderboard Settings',
     'bpmycredleaderboard'
 ); ?> <a href="https://mycred.me/store/mycred-bp-group-leaderboards/" class="page-title-action" target="_blank"><?php _e(
     'Documentation',
     'bpmycredleaderboard'
		...
		<table class="form-table">
			<tbody>
				<tr>
					...
					<td>
						<p id="mycred-point-types"><?php mycred_types_select_from_checkboxes(
          'mycred_bp_leaderboards[types][]',
          'mycred-bp-group-leaderboard',
          $settings['types']
      ); ?></p>
						<p class="description"><?php _e(
          'Select the point type(s) to base the leaderboard on. If more then one type is selected, members will be able to select which point type to show in each group.',
          'bpmycredleaderboard'
      ); ?></p>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="text" name="mycred_bp_leaderboards[size]" id="mycred-bp-group-leaderboard-size" value="<?php echo esc_attr(
          $settings['size']
      ); ?>" />
						<p class="description"><?php printf(
          __(
              'The number of users to show in the leaderboards by default. Maximum %d.',
              'bpmycredleaderboard'
          ),
          MYCRED_BP_GROUP_LEAD_MAX_SIZE
      ); ?></p>
					</td>
				</tr>
				<tr>
         ...
         'bpmycredleaderboard'
					...
					<td>
						<p><label for="mycred-bp-group-leaderboard-current-yes"><input type="radio" name="mycred_bp_leaderboards[current]" id="mycred-bp-group-leaderboard-current-yes"<?php checked(
          $settings['current'],
          1
      ); ?> value="1" /> <?php _e(
     'Yes',
     'bpmycredleaderboard'
 ); ?></label><br /><label for="mycred-bp-group-leaderboard-current-no"><input type="radio" name="mycred_bp_leaderboards[current]" id="mycred-bp-group-leaderboard-current-no"<?php checked(
    $settings['current'],
    0
); ?> value="0" /> <?php _e('No', 'bpmycredleaderboard'); ?></label></p>
						<p class="description"><?php _e(
          'When a member views the leaderboard and they are not in the size set, do you want the user to be appended to the end of the leaderboard with their current position?',
          'bpmycredleaderboard'
      ); ?></p>
					</td>
				</tr>
<?php if (!empty($leaderboard_types)) { ?>
				<tr>
					<th scope="row"><?php _e('Leaderboard Type', 'bpmycredleaderboard'); ?></th>
					<td>
<?php if (!array_key_exists('type', $settings)) {
    $settings['type'] = 'current';
} ?>
						<div id="bp-group-leaderboard-types">
<?php foreach ($leaderboard_types as $type_id => $type_label) { ?>
							<p>
								<label for="bp-leaderboard-type-<?php echo esc_attr(
            $type_id
        ); ?>" style="font-weight: normal;"><input type="radio" name="mycred_bp_leaderboards[type]" class="bp-leaderboard-type" id="bp-leaderboard-type-<?php echo esc_attr(
    $type_id
); ?>"<?php checked($settings['type'], $type_id); ?> value="<?php echo esc_attr(
     $type_id
 ); ?>" /> <?php echo esc_attr($type_label); ?></label>
								<?php do_action(
            "mycred_bp_leaderboard_{$type_id}_prefs_field",
            $settings['type'],
            $settings
        ); ?>
							</p>
...
<?php } ?>
				<tr>
					...
					<td>
						<p><label for="mycred-bp-group-leaderboard-override-no"><input type="radio" name="mycred_bp_leaderboards[override]" id="mycred-bp-group-leaderboard-override-no"<?php checked(
          $settings['override'],
          1
      ); ?> value="1" /> <?php _e(
     'Enforce these default settings for all group leaderboards.',
     'bpmycredleaderboard'
 ); ?></label><br />
						<label for="mycred-bp-group-leaderboard-override-yes"><input type="radio" name="mycred_bp_leaderboards[override]" id="mycred-bp-group-leaderboard-override-yes"<?php checked(
          $settings['override'],
          0
      ); ?> value="0" /> <?php _e(
     'Allow group admins to change leaderboard setups for each group.',
     'bpmycredleaderboard'
 ); ?></label></p>
					</td>
				</tr>
		...
		<table class="form-table">
				<tr>
					...
					<td>
						<input type="text" name="mycred_bp_leaderboards[title]" id="mycred-bp-group-leaderboard-title" style="width: 50%;" value="<?php echo esc_attr(
          $settings['title']
      ); ?>" />
				...
				<tr>
					...
					<td>
						<input type="text" name="mycred_bp_leaderboards[slug]" id="mycred-bp-group-leaderboard-slug" class="code" size="32" value="<?php echo esc_attr(
          $settings['slug']
      ); ?>" />
				...
				<tr>
					...
					<td>
						<input type="text" name="mycred_bp_leaderboards[position]" id="mycred-bp-group-leaderboard-position" size="10" value="<?php echo esc_attr(
          $settings['position']
      ); ?>" />
		...
		<p class="submit">
			<input type="submit" name="submit" class="button-primary" value="<?php esc_attr_e(
       'Save Settings',
       'bpmycredleaderboard'
   ); ?>" />
		</p>
...
<?php
        ...
        public function sanitize_settings($data)
        {
            ...
            $new_data['current'] = isset($data['current'])
                ? absint($data['current'])
                : MYCRED_BP_GROUP_LEAD_MAX_SIZE;
            $new_data['type'] = isset($data['type'])
                ? sanitize_key($data['type'])
                : 'current';
            $new_data['override'] = isset($data['override'])
                ? absint($data['override'])
                : 0;
            ...
            return apply_filters(
                'mycred_bp_leaderboard_save_prefs',
                $new_data,
                $data,
                $settings
            );
