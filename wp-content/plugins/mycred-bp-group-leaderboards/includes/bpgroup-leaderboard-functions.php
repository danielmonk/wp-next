...
if (!function_exists('mycred_get_bp_leaderboard_types')):
    ...
    {
        return apply_filters('mycred_bp_leaderboard_types', [
            'current' => __(
                'Leaderboard Based on Current Balance',
                'bpmycredleaderboard'
            ),
            'total' => __(
...
if (!function_exists('mycred_get_bp_group_setup')):
    function mycred_get_bp_group_setup($group_id = null)
    {
        ...
        if (!$settings['override']) {
            $number = groups_get_groupmeta(
                $group_id,
                'bp_group_leaderboard_size'
            );
            if (
                $number != '' &&
                absint($number) <= MYCRED_BP_GROUP_LEAD_MAX_SIZE
            ) {
                $setup['size'] = absint($number);
            }

            $attach_current = groups_get_groupmeta(
                $group_id,
...
if (!function_exists('mycred_get_bp_groups_leaderboard')):
    function mycred_get_bp_groups_leaderboard(
        $group_id = null,
        ...
        if (!$fresh) {
            $leaderboard = groups_get_groupmeta(
                $group_id,
        ...
        if (!empty($leaderboard) && $append_user !== false) {
            ...
            if ($setup['current'] && !$in_list) {
                $position = mycred_get_users_bp_group_position(
                    $append_user,
                    $group_id,
                    $point_type
                );
                if ($position !== false) {
                    ...
                    $row->balance = mycred_get_users_cred(
                        $append_user,
                        $point_type
                    );
                    $row->current = true;
...
if (!function_exists('mycred_get_users_bp_group_positions')):
    function mycred_get_users_bp_group_positions(
        $user_id = null,
        $group_id = null
    ) {
        $setup = mycred_get_bp_group_setup($group_id);
...
if (!function_exists('mycred_get_users_bp_group_position')):
    function mycred_get_users_bp_group_position(
        $user_id = null,
        $group_id = null,
        $point_type = MYCRED_DEFAULT_TYPE_KEY,
        $find = true
        ...
        $result = false;
        if (
            array_key_exists($point_type, $positions) &&
            $positions[$point_type] !== false
        ) {
            $result = $positions[$point_type];
        }

        if ($result === false && $find) {
            $result = mycred_find_users_bp_group_position(
                $user_id,
                $group_id,
                $point_type
            );
...
if (!function_exists('mycred_find_users_bp_group_position')):
    function mycred_find_users_bp_group_position(
        $user_id = null,
        $group_id = null,
        $point_type = MYCRED_DEFAULT_TYPE_KEY
    ) {
