...
function bp_get_notifications_unread_permalink($user_id = 0)
{
    ...
    $retval = trailingslashit(
        $domain . bp_get_notifications_slug() . '/unread'
    );
...
function bp_get_the_notification_item_id()
{
    ...
    return apply_filters(
        ...
        buddypress()->notifications->query_loop->notification->item_id
    );
}
...
function bp_get_the_notification_component_name()
{
    ...
    return apply_filters(
        ...
        buddypress()->notifications->query_loop->notification->component_name
    );
}
...
function bp_get_the_notification_component_action()
{
    ...
    return apply_filters(
        ...
        buddypress()->notifications->query_loop->notification->component_action
    );
}
...
function bp_get_the_notification_date_notified()
{
    ...
    return apply_filters(
        ...
        buddypress()->notifications->query_loop->notification->date_notified
    );
}
...
function bp_get_the_notification_mark_read_link($user_id = 0)
{
    ...
    $retval = sprintf(
        '<a href="%1$s" class="mark-read primary">%2$s</a>',
...
function bp_get_the_notification_mark_read_url($user_id = 0)
{
    ...
    $url = add_query_arg(
        $args,
        bp_get_notifications_unread_permalink($user_id)
    );
...
function bp_get_the_notification_mark_unread_link($user_id = 0)
{
    ...
    $retval = sprintf(
        '<a href="%1$s" class="mark-unread primary bp-tooltip">%2$s</a>',
...
function bp_get_notifications_pagination_count()
{
    ...
    $to_num = bp_core_number_format(
        $start_num + ($query_loop->pag_num - 1) >
    ...
    if (1 == $query_loop->total_notification_count) {
        ...
        $pag = sprintf(
            _n(
                'Viewing %1$s - %2$s of %3$s notification',
...
function bp_notifications_sort_order_form()
{
	...
	<form action="" method="get" id="notifications-sort-order">
		<label for="notifications-sort-order-list"><?php esc_html_e(
      'Order By:',
      'buddypress'
  ); ?></label>

		<select id="notifications-sort-order-list" name="sort_order" onchange="this.form.submit();">
			<option value="DESC" <?php selected($selected, 'DESC'); ?>><?php _e(
    'Newest First',
    'buddypress'
); ?></option>
			<option value="ASC"  <?php selected($selected, 'ASC'); ?>><?php _e(
    'Oldest First',
    'buddypress'
); ?></option>
		</select>
		...
		<noscript>
			<input id="submit" type="submit" name="form-submit" class="submit" value="<?php esc_attr_e(
       'Go',
       'buddypress'
   ); ?>" />
		</noscript>
...
{
    ?>
	<label class="bp-screen-reader-text" for="notification-select"><?php /* translators: accessibility text */
 _e('Select Bulk Action', 'buddypress'); ?></label>
	...
	<input type="submit" id="notification-bulk-manage" class="button action" value="<?php esc_attr_e(
     'Apply',
     'buddypress'
 ); ?>">
	<?php
