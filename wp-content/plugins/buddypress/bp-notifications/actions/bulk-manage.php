...
function bp_notifications_action_bulk_manage()
{
    ...
    if (
        ...
        !(bp_is_current_action('read') || bp_is_current_action('unread'))
    ) {
        return false;
    }

    // Get the action.
    $action = !empty($_POST['notification_bulk_action'])
        ? $_POST['notification_bulk_action']
        : '';
    $nonce = !empty($_POST['notifications_bulk_nonce'])
    ...
    if (
        !in_array($action, ['delete', 'read', 'unread']) ||
        empty($notifications) ||
        empty($nonce)
    ) {
        return false;
    ...
    switch ($action) {
        ...
        case 'read':
            ...
            bp_core_add_message(
                __('Notifications marked as read', 'buddypress')
            );
            break;
        ...
        case 'unread':
            ...
            bp_core_add_message(
                __('Notifications marked as unread.', 'buddypress')
            );
            break;
    }
