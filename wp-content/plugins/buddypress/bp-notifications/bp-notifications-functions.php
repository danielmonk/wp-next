...
{
    if (
        !bp_notifications_check_notification_access(bp_displayed_user_id(), $id)
    ) {
        return false;
    }

    return BP_Notifications_Notification::delete(['id' => $id]);
}
...
function bp_notifications_get_all_notifications_for_user($user_id = 0)
{
    ...
    $notifications = wp_cache_get(
        'all_for_user_' . $user_id,
        'bp_notifications'
    );
...
function bp_notifications_get_grouped_notifications_for_user($user_id = 0)
{
    ...
    if (false === $notifications) {
        $notifications = BP_Notifications_Notification::get_grouped_notifications_for_user(
            $user_id
        );
        wp_cache_set(
            $user_id,
 ...
 */
function bp_notifications_get_notifications_for_user(
    $user_id,
    $format = 'string'
) {
    $bp = buddypress();

    $notifications = bp_notifications_get_grouped_notifications_for_user(
        $user_id
    );
    ...
    foreach ($notifications as $notification_item) {
        ...
        if (
            ...
            is_callable($bp->{$component_name}->notification_callback)
        ) {
            ...
            if ('object' === $format) {
                ...
                if (is_string($content)) {
                    ...
                    $notification_object->content = isset($content['text'])
                        ? $content['text']
                        : '';
                    $notification_object->href = isset($content['link'])
                        ? $content['link']
                        : '';
                }
            ...
            isset($bp->{$component_name}->format_notification_function) &&
            function_exists(
                $bp->{$component_name}->format_notification_function
            )
        ) {
            $renderable[] = call_user_func(
                $bp->{$component_name}->notification_callback,
                $notification_item->component_action,
                $notification_item->item_id,
                $notification_item->secondary_item_id,
            ...
            if ('object' === $format) {
                ...
                $renderable[] = apply_filters_ref_array(
                    'bp_notifications_get_notifications_for_user',
                    $ref_array
                );
            }
 ...
 */
function bp_notifications_delete_notifications_by_type(
    $user_id,
    $component_name,
    $component_action
) {
 ...
 * @return int|false True on success, false on failure.
 */
function bp_notifications_delete_notifications_by_item_id(
    $user_id,
    $item_id,
    $component_name,
    $component_action,
 ...
 * @return int|false True on success, false on failure.
 */
function bp_notifications_delete_all_notifications_by_type(
    $item_id,
    $component_name,
 ...
 * @return int|false True on success, false on failure.
 */
function bp_notifications_delete_notifications_from_user(
    $user_id,
    $component_name,
    $component_action
) {
 ...
 * @return int|false True on success, false on failure.
 */
function bp_notifications_mark_notifications_by_type(
    $user_id,
    $component_name,
    $component_action,
    $is_new = false
 ...
 * @return int|false True on success, false on failure.
 */
function bp_notifications_mark_all_notifications_by_type(
    $item_id,
    $component_name,
    $component_action = false,
    $secondary_item_id = false,
...
{
    return (bool) BP_Notifications_Notification::check_access(
        $user_id,
        $notification_id
    );
}
...
function bp_notifications_get_registered_components()
{
    ...
    foreach ($active_components as $component) {
        ...
        if (
            'xprofile' == $component &&
            !empty($bp->profile->notification_callback)
        ) {
            $component_names[] = $component;
        }
    }
 ...
 */
function bp_notifications_delete_meta(
    $notification_id,
    $meta_key = '',
    $meta_value = '',
    $delete_all = false
 ...
 */
function bp_notifications_get_meta(
    $notification_id = 0,
    $meta_key = '',
    $single = true
) {
 ...
 */
function bp_notifications_add_meta(
    $notification_id,
    $meta_key,
    $meta_value,
    $unique = false
...
function bp_notifications_personal_data_exporter($email_address, $page)
{
    ...
    foreach ($notifications as $notification) {
        ...
        if (
            isset(buddypress()->{$component_name}->notification_callback) &&
            is_callable(buddypress()->{$component_name}->notification_callback)
        ) {
            $content = call_user_func(
        ...
        $item_data = [
            ...
            [
                ...
                'value' => $notification->is_new
                    ? __('Unread', 'buddypress')
                    : __('Read', 'buddypress'),
            ],
        ];
