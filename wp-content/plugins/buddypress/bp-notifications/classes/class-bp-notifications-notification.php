...
class BP_Notifications_Notification
{
    ...
    public function save()
    {
         ...
         */
        do_action_ref_array('bp_notification_before_save', [&$this]);
        ...
        $data_format = ['%d', '%d', '%d', '%s', '%s', '%s', '%d'];

        // Update.
        if (!empty($this->id)) {
            $result = self::_update($data, ['ID' => $this->id], $data_format, [
                '%d',
            ]);
         ...
         *                                             Passed by reference.
         */
        do_action_ref_array('bp_notification_after_save', [&$this]);
     ...
     */
    protected static function get_where_sql(
        $args = [],
        $select_sql = '',
        $from_sql = '',
        $join_sql = '',
        ...
        if (!empty($args['secondary_item_id'])) {
            $secondary_item_id_in = implode(
                ',',
                wp_parse_id_list($args['secondary_item_id'])
            );
            $where_conditions[
                'secondary_item_id'
            ] = "secondary_item_id IN ({$secondary_item_id_in})";
        }
        ...
        if (!empty($args['is_new']) && 'both' !== $args['is_new']) {
            ...
            isset($args['is_new']) &&
            (0 === $args['is_new'] || false === $args['is_new'])
        ) {
        ...
        if (!empty($args['search_terms'])) {
            ...
            $where_conditions['search_terms'] = $wpdb->prepare(
                "( component_name LIKE %s OR component_action LIKE %s )",
                $search_terms_like,
                $search_terms_like
            );
        }
        ...
        if (!empty($args['date_query'])) {
            $where_conditions['date_query'] = self::get_date_query_sql(
                $args['date_query']
            );
        }
     ...
     */
    protected static function get_order_by_sql($args = [])
    {
        ...
        if (
            !empty($args['sort_order']) &&
            in_array($args['sort_order'], ['ASC', 'DESC'])
        ) {
    ...
    protected static function get_query_clauses($args = [])
    {
        ...
        // The secondary_item_id.
        if (!empty($args['secondary_item_id'])) {
            $where_clauses['data']['secondary_item_id'] = absint(
        ...
        // The component_action.
        if (!empty($args['component_action'])) {
            $where_clauses['data']['component_action'] =
    ...
    public static function get($args = [])
    {
        ...
        foreach ($results as $key => $result) {
            ...
            $results[$key]->secondary_item_id =
                (int) $results[$key]->secondary_item_id;
    ...
    public static function get_meta_query_sql($meta_query = [])
    {
        ...
        $GLOBALS[
            'wpdb'
        ]->notificationmeta = buddypress()->notifications->table_name_meta;
     ...
     * }
     */
    public static function get_current_notifications_for_user($args = [])
    {
     ...
     */
    public static function mark_all_for_user(
        $user_id,
        $is_new = 0,
        $item_id = 0,
        $component_name = '',
     ...
     * @return int|false
     */
    public static function mark_all_from_user(
        $user_id,
        $is_new = 0,
        $component_name = '',
        $component_action = '',
     ...
     * @return int|false
     */
    public static function mark_all_by_type(
        $item_id,
        $is_new = 0,
        $component_name = '',
        $component_action = '',
    ...
    public static function get_grouped_notifications_for_user($user_id)
    {
        ...
        // SELECT.
        $select_sql =
            "SELECT id, user_id, item_id, secondary_item_id, component_name, component_action, date_notified, is_new, COUNT(id) as total_count ";
