...
class BP_REST_Notifications_Endpoint extends WP_REST_Controller
{
    ...
    public function get_items_permissions_check($request)
    {
        $retval = new WP_Error(
            ...
            __(
                'Sorry, you are not allowed to see the notifications.',
                'buddypress'
            ),
        ...
        if (
            (is_user_logged_in() &&
                bp_loggedin_user_id() === $request->get_param('user_id') &&
    ...
    public function get_item_permissions_check($request)
    {
        $retval = new WP_Error(
            'bp_rest_authorization_required',
            __(
                'Sorry, you are not allowed to see the notification.',
                'buddypress'
            ),
            [
                'status' => rest_authorization_required_code(),
        ...
        if (is_user_logged_in()) {
            ...
            if (is_null($notification->item_id)) {
                ...
                $retval = new WP_Error(
                    'bp_rest_authorization_required',
                    __(
                        'Sorry, you cannot view this notification.',
                        'buddypress'
                    ),
                    [
                        'status' => rest_authorization_required_code(),
    ...
    public function update_item($request)
    {
        ...
        if (!(bool) $updated) {
            return new WP_Error(
                ...
                __(
                    'Cannot update the status of this notification.',
                    'buddypress'
                ),
                [
                    'status' => 500,
    ...
    public function delete_item($request)
    {
        ...
        if (
            !BP_Notifications_Notification::delete(['id' => $notification->id])
        ) {
            return new WP_Error(
                'bp_rest_notification_invalid_id',
                __('Invalid notification ID.', 'buddypress'),
    ...
    public function prepare_item_for_response($notification, $request)
    {
        $data = [
            ...
            'date' => bp_rest_prepare_date_response(
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        if (
            !empty($schema['properties']['user_id']) &&
        ...
        if (
            !empty($schema['properties']['item_id']) &&
        ...
        if (
            ...
            $prepared_notification->secondary_item_id =
                $request['secondary_item_id'];
        ...
        if (
            ...
            $prepared_notification->component_name =
                $notification->component_name;
        }

        if (
            !empty($schema['properties']['action']) &&
            isset($request['action'])
        ) {
            $prepared_notification->component_action = $request['action'];
        } elseif (isset($notification->component_action)) {
            $prepared_notification->component_action =
                $notification->component_action;
        }

        if (
            !empty($schema['properties']['is_new']) &&
            isset($request['is_new'])
        ) {
        ...
        if (!empty($schema['properties']['date']) && isset($request['date'])) {
            ...
            $prepared_notification->date_notified =
                $notification->date_notified;
        }
    ...
    protected function prepare_links($notification)
    {
        ...
        $links = [
            ...
            'user' => [
                'href' => rest_url(
                    bp_rest_get_user_url($notification->user_id)
                ),
                'embeddable' => true,
            ],
        ];
    ...
    protected function can_see($notification_id = 0)
    {
        ...
        if (
            ...
            (bool) BP_Notifications_Notification::check_access(
                bp_loggedin_user_id(),
                $notification_id
            )
        ) {
    ...
    {
        $notification_id = is_numeric($request)
            ? $request
            : (int) $request['id'];
    ...
    public function get_endpoint_args_for_item_schema(
        ...
        if (WP_REST_Server::EDITABLE === $method) {
            ...
            $args = array_intersect_key($args, ['is_new' => true]);
    ...
    public function get_collection_params()
    {
        ...
        $params['order_by'] = [
            'description' => __(
                'Name of the field to order according to.',
                'buddypress'
            ),
            'default' => 'id',
            'type' => 'string',
        ...
        $params['component_action'] = [
            'description' => __(
                'Limit result set to notifications associated with a specific component\'s action name.',
                'buddypress'
            ),
            'default' => '',
            'type' => 'string',
        ...
        $params['user_id'] = [
            'description' => __(
                'Limit result set to notifications addressed to a specific user.',
                'buddypress'
            ),
            'default' => bp_loggedin_user_id(),
            'type' => 'integer',
        ...
        $params['secondary_item_id'] = [
            'description' => __(
                'Limit result set to notifications associated with a specific secondary item ID.',
                'buddypress'
            ),
            'default' => 0,
            'type' => 'integer',
