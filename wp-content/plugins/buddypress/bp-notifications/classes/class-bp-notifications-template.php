...
class BP_Notifications_Template
{
    ...
    public function __construct($args = [])
    {
        ...
        $orders = ['ASC', 'DESC'];
        if (
            !empty($_GET['sort_order']) &&
            in_array($_GET['sort_order'], $orders)
        ) {
            $r['sort_order'] = $_GET['sort_order'];
        } else {
            $r['sort_order'] = in_array($r['sort_order'], $orders)
                ? $r['sort_order']
                : 'DESC';
        }
        ...
        $this->notifications = BP_Notifications_Notification::get(
            $this->query_vars
        );
        $this->total_notification_count = BP_Notifications_Notification::get_total_count(
            $this->query_vars
        );
        ...
        if ((int) $this->total_notification_count && (int) $this->pag_num) {
            ...
            $this->pag_links = paginate_links([
                ...
                'format' => '',
                'total' => ceil(
                    (int) $this->total_notification_count / (int) $this->pag_num
                ),
                'current' => $this->pag_page,
                'prev_text' => _x(
                    '&larr;',
                    'Notifications pagination previous text',
                    'buddypress'
                ),
                'next_text' => _x(
                    '&rarr;',
                    'Notifications pagination next text',
                    'buddypress'
                ),
                'mid_size' => 1,
                'add_args' => $add_args,
