...
<script id="tmpl-bp-cover-image-delete" type="text/html">
	<# if ( 'user' === data.object ) { #>
		<p><?php esc_html_e(
      "If you'd like to delete your current cover image, use the delete Cover Image button.",
      'buddypress'
  ); ?></p>
		<button type="button" class="button edit" id="bp-delete-cover-image">
			<?php echo esc_html_x('Delete My Cover Image', 'button', 'buddypress'); ?>
		</button>
	<# } else if ( 'group' === data.object ) { #>
      ...
      'buddypress'
		...
		<button type="button" class="button edit" id="bp-delete-cover-image">
			<?php echo esc_html_x('Delete Group Cover Image', 'button', 'buddypress'); ?>
		</button>
	<# } else { #>
		<?php /**
