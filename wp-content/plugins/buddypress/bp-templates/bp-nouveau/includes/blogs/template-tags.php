 ...
 */
function bp_nouveau_blogs_loop_buttons($args = [])
{
...
function bp_nouveau_get_blogs_buttons($args)
{
    ...
    if (
        isset(bp_nouveau()->blogs->button_args) &&
    ...
    if (
        !isset(bp_nouveau()->blogs->group_buttons) ||
        !is_a(bp_nouveau()->blogs->group_buttons, 'BP_Buttons_Group')
    ) {
        $sort = true;
        bp_nouveau()->blogs->group_buttons = new BP_Buttons_Group(
            $buttons_group
        );
     ...
     * @param string $type    Whether we're displaying a blogs loop or a the blogs single item (in the future!).
     */
    do_action_ref_array('bp_nouveau_return_blogs_buttons', [
        &$return,
        $blog,
        $type,
    ]);
