 ...
 */
function bp_nouveau_hook($pieces = [])
{
    if (!$pieces) {
        return;
    }
...
function bp_nouveau_plugin_hook($suffix = '')
{
    ...
    bp_nouveau_hook(['bp', 'template', $suffix]);
}
...
function bp_nouveau_friend_hook($suffix = '')
{
    ...
    bp_nouveau_hook(['bp', 'friend', $suffix]);
}
...
function bp_nouveau_template_message_classes()
{
    ...
    $classes = ['bp-feedback', 'bp-messages'];
...
function bp_nouveau_has_template_message()
{
    ...
    if (
        empty($bp_nouveau->template_message['message']) &&
        empty($bp_nouveau->user_feedback)
    ) {
        return false;
    }
...
function bp_nouveau_has_dismiss_button()
{
    ...
    if (
        !empty($bp_nouveau->template_message['message']) &&
        true === $bp_nouveau->template_message['dismiss']
    ) {
        return true;
    }
...
function bp_nouveau_get_template_message()
{
    ...
    if (!empty($bp_nouveau->user_feedback['message'])) {
        ...
        return apply_filters(
            ...
            $bp_nouveau->template_message['message'],
...
function bp_nouveau_template_notices()
{
    ...
    if (!empty($bp->template_message)) {
        ...
        $template_message = ['message' => $bp->template_message];
...
function bp_nouveau_user_feedback($feedback_id = '')
{
    ...
    bp_get_template_part(
        ...
        apply_filters(
            ...
            'common/notices/template-notices'
        )
    );
...
function bp_nouveau_pagination($position)
{
    ...
    switch ($pagination_type) {
        ...
        default:
            ...
            list($pag_count, $pag_links, $page_arg) = array_values(
                $pagination_params
            );
            break;
    }
	...
	<div class="<?php echo esc_attr(
     'bp-pagination ' . sanitize_html_class($position)
 ); ?>" data-bp-pagination="<?php echo esc_attr($page_arg); ?>">

		<?php if ($pag_count): ?>
			<div class="<?php echo esc_attr(
       'pag-count ' . sanitize_html_class($position)
   ); ?>">
		...
		<?php if ($pag_links): ?>
			<div class="<?php echo esc_attr(
       'bp-pagination-links ' . sanitize_html_class($position)
   ); ?>">
...
function bp_nouveau_get_loop_classes()
{
    ...
    if (
        !$is_directory &&
        (bp_is_group() && 'members' === bp_current_action())
    ) {
        $component = 'members_group';
    } elseif (
        !$is_directory &&
        (bp_is_user() && 'my-friends' === bp_current_action())
    ) {
    ...
    if (
        !empty($available_components[$component]) &&
        ($is_directory || bp_is_group() || bp_is_user())
    ) {
     ...
     */
    $class_list = (array) apply_filters(
...
function bp_nouveau_loop_is_grid()
{
    ...
    return !empty($bp_nouveau->{$component}->loop_layout) &&
        $bp_nouveau->{$component}->loop_layout > 1;
}
 ...
 */
function bp_nouveau_has_nav($args = [])
{
    $bp_nouveau = bp_nouveau();
    ...
    if (bp_is_directory() || 'directory' === $bp_nouveau->object_nav) {
        ...
        if (
            'group_manage' === $bp_nouveau->object_nav &&
            ...
            bp_nouveau_set_nav_item_order(
                $group_nav,
                bp_nouveau_get_appearance_settings('group_nav_order'),
                $parent_slug
            );
        }
        ...
        if ('secondary' === $n['type']) {
            ...
            if (
                true === (bool) $n['show_for_displayed_user'] &&
                !bp_is_my_profile()
            ) {
                $args = ['show_for_displayed_user' => true];
            }

            // Reorder the user's primary nav according to the customizer setting.
            bp_nouveau_set_nav_item_order(
                $user_nav,
                bp_nouveau_get_appearance_settings('user_nav_order')
            );
    ...
    if (0 === count($bp_nouveau->sorted_nav) || !$bp_nouveau->displayed_nav) {
        unset(
            $bp_nouveau->sorted_nav,
            $bp_nouveau->displayed_nav,
            $bp_nouveau->object_nav
        );
...
function bp_nouveau_nav_item()
{
    ...
    $bp_nouveau->current_nav_item =
        $bp_nouveau->sorted_nav[$bp_nouveau->current_nav_index];
...
function bp_nouveau_get_nav_id()
{
    ...
    if ('directory' === $bp_nouveau->displayed_nav) {
        ...
        'groups' === $bp_nouveau->displayed_nav ||
        'personal' === $bp_nouveau->displayed_nav
    ) {
        $id = sprintf(
            '%1$s-%2$s-li',
            $nav_item->css_id,
...
function bp_nouveau_get_nav_classes()
{
    ...
    if ('directory' === $bp_nouveau->displayed_nav) {
        ...
        $classes = ['bp-' . $bp_nouveau->displayed_nav . '-tab'];
        ...
        if (!empty($nav_item->primary)) {
            ...
            $classes = ['bp-' . $bp_nouveau->displayed_nav . '-admin-tab'];

            // If we are here, it's the member's subnav
        } elseif ('personal' === $bp_nouveau->displayed_nav) {
            $classes = ['bp-' . $bp_nouveau->displayed_nav . '-sub-tab'];
        }

        if ($nav_item->slug === $selected) {
            $classes = array_merge($classes, ['current', 'selected']);
        }
    }
...
function bp_nouveau_get_nav_scope()
{
    ...
    if ('directory' === $bp_nouveau->displayed_nav) {
        $scope = ['data-bp-scope' => $nav_item->slug];
    } elseif (
        'personal' === $bp_nouveau->displayed_nav &&
        !empty($nav_item->secondary)
    ) {
        $scope = ['data-bp-user-scope' => $nav_item->slug];
    } else {
        /**
...
function bp_nouveau_get_nav_link_title()
{
    ...
    if (
        'directory' === $bp_nouveau->displayed_nav &&
        !empty($nav_item->title)
    ) {
...
function bp_nouveau_get_directory_type_navs_class()
{
    ...
    $nav_style = bp_nouveau_get_temporary_setting(
        $customizer_option,
        bp_nouveau_get_appearance_settings($customizer_option)
    );
    $tab_style = '';
     ...
     */
    $nav_wrapper_classes = (array) apply_filters(
...
{
    $classes = ['buddypress-wrap', get_template()];
    $component = bp_current_component();
    $bp_nouveau = bp_nouveau();
    $member_type_class = '';
    ...
    if (bp_is_user()) {
         ...
         */
        $customizer_option = apply_filters(
    ...
    $general_settings = bp_nouveau_get_temporary_setting(
        'avatar_style',
        bp_nouveau_get_appearance_settings('avatar_style')
    );
    if ($general_settings) {
    ...
    $global_alignment = bp_nouveau_get_temporary_setting(
        'global_alignment',
        bp_nouveau_get_appearance_settings('global_alignment')
    );
    if (
        $global_alignment &&
        'alignnone' !== $global_alignment &&
...
{
    $classes = ['main-navs', 'no-ajax', 'bp-navs', 'single-screen-navs'];
    $component = bp_current_component();
    $bp_nouveau = bp_nouveau();
    ...
    if (bp_is_group()) {
        $nav_tabs = (int) bp_nouveau_get_temporary_setting(
            'group_nav_tabs',
            bp_nouveau_get_appearance_settings('group_nav_tabs')
        );
    } elseif (bp_is_user()) {
        $nav_tabs = (int) bp_nouveau_get_temporary_setting(
            'user_nav_tabs',
            bp_nouveau_get_appearance_settings('user_nav_tabs')
        );
    }
    ...
    $customizer_option = bp_is_user()
        ? 'user_nav_display'
        : 'group_nav_display';

    $layout_prefs = (int) bp_nouveau_get_temporary_setting(
        $customizer_option,
        bp_nouveau_get_appearance_settings($customizer_option)
    );
...
{
    $classes = ['bp-navs', 'bp-subnavs', 'no-ajax'];
    ...
    if (
        (bp_is_group() && 'send-invites' === bp_current_action()) ||
        (bp_is_group_create() &&
            'group-invites' === bp_get_groups_current_create_step())
    ) {
...
{
    $classes = ['bp-navs', 'group-create-links', 'no-ajax'];
    $nav_tabs = (int) bp_nouveau_get_temporary_setting(
        'groups_create_tabs',
        bp_nouveau_get_appearance_settings('groups_create_tabs')
    );
 ...
 */
function bp_nouveau_get_search_objects($objects = [])
{
...
function bp_nouveau_search_container_class()
{
    ...
    $class =
        join('-search ', array_map('sanitize_html_class', $objects)) .
        '-search';
...
function bp_nouveau_search_selector_id($suffix = '', $sep = '-')
{
    $id = join(
        $sep,
        array_merge(bp_nouveau_get_search_objects(), (array) $suffix)
    );
    echo esc_attr($id);
}
...
function bp_nouveau_search_form()
{
    ...
    if ('dir' === $objects['primary']) {
         ...
         */
        echo apply_filters(
            "bp_directory_{$objects['secondary']}_search_form",
            $search_form_html
        );
        ...
        if ('members' !== $objects['secondary']) {
             ...
             */
            echo apply_filters(
                "bp_group_{$objects['secondary']}_search_form",
                $search_form_html
            );
        } else {
            /**
...
function bp_nouveau_get_filter_label()
{
    ...
    if (
        'activity' === $component['object'] ||
        'friends' === $component['object']
    ) {
 ...
 * @return string HTML.
 */
function bp_nouveau_get_customizer_link($args = [])
{
    $r = bp_parse_args(
        $args,
        [
    ...
    if (bp_is_user()) {
        ...
        if ('user' === $r['object']) {
            ...
            $group = groups_get_group(['group_id' => $r['item_id']]);
...
function bp_nouveau_signup_form($section = 'account_details')
{
    ...
    foreach ($fields as $name => $attributes) {
        if ('signup_password' === $name) { ?>
			<label for="pass1"><?php esc_html_e(
			...
			<div class="user-pass1-wrap">
				<div class="wp-pwd">
					<div class="password-input-wrapper">
						<input type="password" data-reveal="1" name="signup_password" id="pass1" class="password-entry" size="24" value="" <?php bp_form_field_attributes(
          'password',
          [
              ...
              'aria-describedby' => 'pass-strength-result',
					...
					<div id="pass-strength-result" aria-live="polite"><?php esc_html_e(
         'Strength indicator',
         'buddypress'
     ); ?></div>
				</div>
			...
			<?php } elseif ('signup_password_confirm' === $name) { ?>
			<p class="user-pass2-wrap">
				<label for="pass2"><?php esc_html_e(
        'Confirm new password',
        'buddypress'
    ); ?></label><br />
				<input type="password" name="signup_password_confirm" id="pass2" class="password-entry-confirm" size="24" value="" <?php bp_form_field_attributes(
        'password'
    ); ?> />
			</p>
          ...
          'buddypress'
            ...
            if ($attribute_type) {
                ...
                if (!empty($required)) {
                    $existing_attributes = ['aria-required' => 'true'];
            ...
            if (
                'signup_blog_privacy_public' === $name ||
                'signup_blog_privacy_private' === $name
            ) {
...
function bp_nouveau_signup_privacy_policy_acceptance_section()
{
    ...
    ?>
			<input type="checkbox" name="signup-privacy-policy-accept" id="signup-privacy-policy-accept" required /> <?php printf(
