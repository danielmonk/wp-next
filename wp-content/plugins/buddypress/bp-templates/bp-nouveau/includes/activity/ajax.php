...
function bp_nouveau_ajax_mark_activity_favorite()
{
    ...
    if (bp_activity_add_user_favorite($_POST['id'])) {
        $response = ['content' => __('Remove Favorite', 'buddypress')];
...
function bp_nouveau_ajax_unmark_activity_favorite()
{
    ...
    if (bp_activity_remove_user_favorite($_POST['id'])) {
        $response = ['content' => __('Mark as Favorite', 'buddypress')];
        ...
        if (0 === $fav_count && !bp_is_single_activity()) {
            $response['no_favorite'] =
                '<li><div class="bp-feedback bp-messages info">
				' .
                __(
...
function bp_nouveau_ajax_get_single_activity_content()
{
     ...
     */
    do_action_ref_array('bp_nouveau_get_single_activity_content', [&$activity]);
...
function bp_nouveau_ajax_new_activity_comment()
{
    ...
    // Nonce check!
    if (
        empty($_POST['_wpnonce_new_activity_comment']) ||
        !wp_verify_nonce(
            $_POST['_wpnonce_new_activity_comment'],
            'new_activity_comment'
        )
    ) {
        wp_send_json_error($response);
    ...
    if (!$comment_id) {
        if (
            !empty($bp->activity->errors['new_comment']) &&
            is_wp_error($bp->activity->errors['new_comment'])
        ) {
            $response = [
                'feedback' => sprintf(
                    '<div class="bp-feedback bp-messages error">%s</div>',
...
function bp_nouveau_ajax_get_activity_objects()
{
    ...
    if ('group' === $_POST['type']) {
        ...
        wp_send_json_success(
            array_map('bp_nouveau_prepare_group_for_js', $groups['groups'])
        );
    } else {
        /**
    ...
    if (empty($response)) {
        wp_send_json_error([
            'error' => __('No activities were found.', 'buddypress'),
        ]);
    } else {
        wp_send_json_success($response);
    }
...
function bp_nouveau_ajax_post_update()
{
    ...
    if (
        !is_user_logged_in() ||
        empty($_POST['_wpnonce_post_update']) ||
        !wp_verify_nonce($_POST['_wpnonce_post_update'], 'post_update')
    ) {
        wp_send_json_error();
    }
    ...
    wp_send_json_success([
        'id' => $activity_id,
        'message' =>
            esc_html__('Update posted.', 'buddypress') .
            ' ' .
            sprintf(
                '<a href="%s" class="just-posted">%s</a>',
                esc_url(bp_activity_get_permalink($activity_id)),
                esc_html__('View activity.', 'buddypress')
            ),
        'activity' => $acivity,
...
function bp_nouveau_ajax_spam_activity()
{
    ...
    if (
        ...
        !wp_verify_nonce(
            $_POST['_wpnonce'],
            'bp_activity_akismet_spam_' . $activity->id
        )
    ) {
        wp_send_json_error($response);
    }
    ...
    if (!empty($_POST['is_single'])) {
        ...
        bp_core_add_message(
            __(
                'This activity has been marked as spam and is no longer visible.',
                'buddypress'
            )
        );
    }
