...
{
    return bp_activity_has_content() ||
        (bool) has_action('bp_activity_entry_content');
}
...
function bp_nouveau_get_activity_timestamp()
{
    ...
    return apply_filters(
        ...
        strtotime(bp_get_activity_date_recorded())
    );
}
 ...
 */
function bp_nouveau_activity_entry_buttons($args = [])
{
...
function bp_nouveau_get_activity_entry_buttons($args)
{
    ...
    if ($activity_type === 'activity_comment') {
        ...
        if ('button' === $button_element) {
            $buttons['activity_conversation']['button_attr'][
                'data-bp-url'
    ...
    if (has_filter('bp_get_activity_delete_link')) {
        ...
        if (!empty($attrs[1]) && !empty($attrs[2])) {
            foreach ($attrs[1] as $key_attr => $key_value) {
                $delete_args['link_' . $key_value] = trim(
                    $attrs[2][$key_attr],
                    '"'
                );
            }
    ...
    if (empty($delete_args['link_href'])) {
        $delete_args = [
            ...
            'link_id' => '',
            'link_class' =>
                'button item-button bp-secondary-action bp-tooltip delete-activity confirm',
            'link_rel' => 'nofollow',
    ...
    if (
        ...
        isset(buddypress()->activity->akismet) &&
        bp_activity_user_can_mark_spam()
    ) {
        $buttons['activity_spam'] = [
            'id' => 'activity_spam',
            'position' => 45,
            'component' => 'activity',
            'parent_element' => $parent_element,
            ...
            'button_attr' => [
                'class' =>
                    'bp-secondary-action spam-activity confirm button item-button bp-tooltip',
    ...
    if (
        !isset(bp_nouveau()->activity->entry_buttons) ||
        !is_a(bp_nouveau()->activity->entry_buttons, 'BP_Buttons_Group')
    ) {
        $sort = true;
        bp_nouveau()->activity->entry_buttons = new BP_Buttons_Group(
            $buttons_group
        );
    ...
    if (
        isset($return['activity_spam']) &&
        !in_array($activity_type, BP_Akismet::get_activity_types(), true)
    ) {
     ...
     * @param int   $activity_id The current activity ID.
     */
    do_action_ref_array('bp_nouveau_return_activity_entry_buttons', [
        &$return,
        $activity_id,
    ]);
...
function bp_nouveau_get_activity_comment_action()
{
    ...
    return apply_filters(
        ...
        sprintf(
            __(
                '<a href="%1$s">%2$s</a> replied <a href="%3$s" class="activity-time-since"><time class="time-since" datetime="%4$s" data-bp-timestamp="%5$d">%6$s</time></a>',
                'buddypress'
            ),
 ...
 */
function bp_nouveau_activity_comment_buttons($args = [])
{
...
function bp_nouveau_get_activity_comment_buttons($args)
{
    ...
    // If button element set add nonce link to data-attr attr
    if ('button' === $button_element) {
        $buttons['activity_comment_reply']['button_attr'][
            'data-bp-act-reply-nonce'
        ] = sprintf('#acomment-%s', $activity_comment_id);
        $buttons['activity_comment_delete']['button_attr'][
            'data-bp-act-reply-delete-nonce'
        ] = bp_get_activity_comment_delete_link();
    } else {
        $buttons['activity_comment_reply']['button_attr']['href'] = sprintf(
            '#acomment-%s',
            $activity_comment_id
        );
        $buttons['activity_comment_delete']['button_attr'][
    ...
    if (
        ...
        $buttons['activity_comment_spam']['button_attr'][
            $data_element
        ] = wp_nonce_url(
            bp_get_root_domain() .
                '/' .
    ...
    if (
        !isset(bp_nouveau()->activity->comment_buttons) ||
        !is_a(bp_nouveau()->activity->comment_buttons, 'BP_Buttons_Group')
    ) {
        $sort = true;
        bp_nouveau()->activity->comment_buttons = new BP_Buttons_Group(
            $buttons_group
        );
    ...
    if (
        isset($return['activity_comment_spam']) &&
        (!bp_activity_current_comment() ||
            !in_array(
                bp_activity_current_comment()->type,
     ...
     * @param int   $activity_id         The current activity ID.
     */
    do_action_ref_array('bp_nouveau_return_activity_comment_buttons', [
        &$return,
        $activity_comment_id,
        $activity_id,
    ]);
