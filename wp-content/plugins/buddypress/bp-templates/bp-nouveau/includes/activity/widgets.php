...
class BP_Latest_Activities extends WP_Widget
{
    ...
    public function __construct()
    {
         ...
         */
        $widget_ops = apply_filters('bp_latest_activities', [
            'classname' => 'bp-latest-activities buddypress',
            'description' => __(
                'Display the latest updates of your community having the types of your choice.',
                'buddypress'
            ),
        ...
        parent::__construct(
            false,
            __('(BuddyPress) Latest Activities', 'buddypress'),
            $widget_ops
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        if (!empty($instance['type'])) {
            ...
            $classes = array_map(
                'sanitize_html_class',
                array_merge($type, ['bp-latest-activities'])
            );
         ...
         */
        $bp_nouveau->activity->widget_args = [
            'max' => $max,
            'scope' => 'all',
            'user_id' => 0,
            'object' => false,
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        $instance['type'] = maybe_serialize(['activity_update']);
    ...
    public function form($instance)
    {
       ...
       'title'
   ); ?>"><?php esc_html_e('Title:', 'buddypress'); ?></label>
       ...
       'title'
		...
		<p>
			<label for="<?php echo $this->get_field_id(
       'max'
   ); ?>"><?php _e('Maximum amount to display:', 'buddypress'); ?></label>
			<input type="number" class="widefat" id="<?php echo $this->get_field_id(
       'max'
   ); ?>" name="<?php echo $this->get_field_name('max'); ?>" value="<?php echo intval($max); ?>" step="1" min="1" max="20" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id(
       'type'
   ); ?>"><?php esc_html_e('Type:', 'buddypress'); ?></label>
			<select class="widefat" multiple="multiple" id="<?php echo $this->get_field_id(
       'type'
   ); ?>" name="<?php echo $this->get_field_name('type'); ?>[]">
				<?php foreach (bp_nouveau_get_activity_filters() as $key => $name): ?>
					<option value="<?php echo esc_attr($key); ?>" <?php selected(
    in_array($key, $type, true)
