 ...
 */
function bp_nouveau_messages_enqueue_styles($styles = [])
{
    ...
    return array_merge($styles, [
        'bp-nouveau-messages-at' => [
            'file' =>
                ...
                'bp-activity/css/mentions%1$s%2$s.css',
 ...
 */
function bp_nouveau_messages_register_scripts($scripts = [])
{
    ...
    return array_merge($scripts, [
        'bp-nouveau-messages-at' => [
            ...
            'dependencies' => ['bp-nouveau', 'jquery', 'jquery-atwho'],
            'version' => bp_get_version(),
            'footer' => true,
        ],
        'bp-nouveau-messages' => [
 ...
 */
function bp_nouveau_messages_localize_scripts($params = [])
{
    if (!bp_is_user_messages()) {
        return $params;
    }
    ...
    $params['messages'] = [
        ...
        'doingAction' => [
            'read' => __(
                'Marking messages as read. Please wait.',
                'buddypress'
            ),
            'unread' => __(
                'Marking messages as unread. Please wait.',
                'buddypress'
            ),
        ...
        'rootUrl' => parse_url(
            trailingslashit(
                bp_displayed_user_domain() . bp_get_messages_slug()
            ),
            PHP_URL_PATH
        ),
    ];
    ...
    if (bp_is_active('messages', 'star')) {
        $params['messages'] = array_merge($params['messages'], [
            'strings' => [
...
function bp_nouveau_messages_adjust_admin_nav($admin_nav)
{
    ...
    $user_messages_link = trailingslashit(
...
function bp_nouveau_add_notice_notification_for_user($notifications, $user_id)
{
    ...
    return array_merge($notifications, [$notice_notification]);
}
...
function bp_nouveau_push_sitewide_notices()
{
    ...
    if (
        $notice->id &&
        is_array($closed_notices) &&
        !in_array($notice->id, $closed_notices, true)
    ) {
 ...
 */
function bp_nouveau_messages_mce_buttons($buttons = [])
{
    $remove_buttons = [
        'wp_more',
        'spellchecker',
...
function bp_nouveau_messages_at_on_tinymce_init($settings, $editor_id)
{
    ...
    if ('message_content' === $editor_id) {
        $settings['init_instance_callback'] =
            'window.bp.Nouveau.Messages.tinyMCEinit';
    }
...
function bp_nouveau_get_message_date($date)
{
    ...
    return apply_filters(
        ...
        date_i18n($date_format, $calculated_time, true),
        $calculated_time,
        $date,
        $date_format
    );
 ...
 */
function bp_nouveau_messages_catch_hook_content($hooks = [])
{
    $content = [];
