...
function bp_nouveau_ajax_messages_send_message()
{
    ...
    if (
        empty($_POST['nonce']) ||
        !wp_verify_nonce($_POST['nonce'], 'messages_send_message')
    ) {
    ...
    if (empty($_POST['send_to']) || !is_array($_POST['send_to'])) {
        $response['feedback'] = __(
            'Your message was not sent. Please enter at least one username.',
            'buddypress'
        );
...
function bp_nouveau_ajax_get_user_message_threads()
{
    ...
    if (
        empty($_POST['nonce']) ||
        !wp_verify_nonce($_POST['nonce'], 'bp_nouveau_messages')
    ) {
        wp_send_json_error([
    ...
    if ('starred' === $bp->current_action) {
        add_filter(
            'bp_after_has_message_threads_parse_args',
    ...
    $threads->meta = [
        'total_page' => ceil(
            (int) $messages_template->total_thread_count /
                (int) $messages_template->pag_num
        ),
    ...
    while (bp_message_threads()):
        ...
        $threads->threads[$i] = [
            ...
            'sender_name' => bp_core_get_user_displayname(
                $messages_template->thread->last_sender_id
            ),
            'sender_link' => bp_core_get_userlink(
                $messages_template->thread->last_sender_id,
                false,
                true
            ),
            ...
            'date' =>
                strtotime(bp_get_message_thread_last_post_date_raw()) * 1000,
            'display_date' => bp_nouveau_get_message_date(
        ...
        if (is_array($messages_template->thread->recipients)) {
            foreach ($messages_template->thread->recipients as $recipient) {
                $threads->threads[$i]['recipients'][] = [
                    ...
                    'user_link' => bp_core_get_userlink(
                        $recipient->user_id,
                        false,
                        true
                    ),
        ...
        if (bp_is_active('messages', 'star')) {
            ...
            $threads->threads[$i]['is_starred'] = array_search(
                'unstar',
                $star_link_data
            );
            ...
            if ($threads->threads[$i]['is_starred']) {
                $sm_id =
                    (int) $star_link_data[
                        $threads->threads[$i]['is_starred'] + 1
                    ];
            }

            $threads->threads[$i]['star_nonce'] = wp_create_nonce(
                'bp-messages-star-' . $sm_id
            );
        ...
        if (array_filter($thread_extra_content)) {
            $threads->threads[$i] = array_merge(
                $threads->threads[$i],
                $thread_extra_content
            );
        }
...
function bp_nouveau_ajax_get_thread_messages()
{
    ...
    // Simulate the loop.
    if (!bp_thread_has_messages(['thread_id' => $thread_id])) {
        // Remove the bp_current_action() override.
        $bp->current_action = $reset_action;
    ...
    while (bp_thread_messages()):
        ...
        if (bp_is_active('messages', 'star')) {
            ...
            $thread->messages[$i]['is_starred'] = array_search(
                'unstar',
                explode('/', $star_link)
            );
            $thread->messages[$i]['star_nonce'] = wp_create_nonce(
                'bp-messages-star-' . bp_get_the_thread_message_id()
            );
        }
...
function bp_nouveau_ajax_star_thread_messages()
{
    ...
    if (1 !== count($ids)) {
        ...
        if (
            empty($_POST['star_nonce']) ||
            !wp_verify_nonce($_POST['star_nonce'], 'bp-messages-star-' . $id)
        ) {
            wp_send_json_error($response);
        }
...
function bp_nouveau_ajax_dismiss_sitewide_notice()
{
    ...
    $response = [
        'feedback' =>
            '<div class="bp-feedback error"><span class="bp-icon" aria-hidden="true"></span><p>' .
            __(
    ...
    if (!empty($notice->id)) {
        ...
        bp_update_user_meta(
            $user_id,
            'closed_notices',
            array_map('absint', array_unique($closed_notices))
        );

        wp_send_json_success([
            'feedback' =>
                '<div class="bp-feedback info"><span class="bp-icon" aria-hidden="true"></span><p>' .
                __('Sitewide notice dismissed', 'buddypress') .
                '</p></div>',
            'type' => 'success',
        ]);
    }
