 ...
 */
function bp_nouveau_member_header_buttons($args = [])
{
 ...
 * @param array $args See bp_nouveau_wrapper() for the description of parameters.
 */
function bp_nouveau_members_loop_buttons($args = [])
{
...
function bp_nouveau_get_members_buttons($args)
{
    ...
    if (bp_is_active('friends')) {
        ...
        if ('friendship_request' === $type) {
            ...
            if ('button' === $button_element) {
                $buttons['accept_friendship']['button_attr'][
                    'data-bp-nonce'
                ] = bp_get_friend_accept_request_link();
                $buttons['reject_friendship']['button_attr'][
                    'data-bp-nonce'
            ...
            if (
                ...
                if (
                    ...
                    'awaiting_response' !== $button_args['id']
                ) {
                    $buttons['member_friendship']['button_attr'][
                        'data-bp-nonce'
                    ] = $button_args['link_href'];
                } else {
                    $buttons['member_friendship']['button_element'] = 'a';
                    $buttons['member_friendship']['button_attr']['href'] =
                        $button_args['link_href'];
                }
    ...
    if (
        !isset(bp_nouveau()->members->member_buttons) ||
        !is_a(bp_nouveau()->members->member_buttons, 'BP_Buttons_Group')
    ) {
        $sort = true;
        bp_nouveau()->members->member_buttons = new BP_Buttons_Group(
            $buttons_group
        );
     ...
     * @param string $type    Whether we're displaying a members loop or a user's page
     */
    do_action_ref_array('bp_nouveau_return_members_buttons', [
        &$return,
        $user_id,
        $type,
    ]);
...
function bp_nouveau_get_member_meta()
{
    ...
    if (empty($member->template_meta)) {
         ...
         */
        $member->template_meta = apply_filters(
            'bp_nouveau_get_member_meta',
            $meta,
            $member,
            $is_loop
...
function bp_nouveau_member_has_extra_content()
{
     ...
     *                    False otherwise.
     */
    $members_directory_only = (bool) apply_filters(
        'bp_nouveau_member_extra_content_in_members_directory',
...
function bp_nouveau_member_get_description_edit_link()
{
    ...
    return sprintf(
        '<a href="%1$s">%2$s</a>',
        esc_url($link),
        esc_html__('Edit your bio', 'buddypress')
    );
}
...
function bp_nouveau_wp_profile_fields()
{
    ...
    if (
        isset(
            $bp_nouveau->members->wp_profile[
                $bp_nouveau->members->wp_profile_index
            ]
        )
    ) {
        return true;
...
function bp_nouveau_wp_profile_field()
{
    ...
    $bp_nouveau->members->wp_profile_current =
        $bp_nouveau->members->wp_profile[
            $bp_nouveau->members->wp_profile_index
        ];
