...
function bp_nouveau_members_enqueue_scripts()
{
    ...
    if (
        !bp_is_user_front() ||
        !bp_nouveau_get_appearance_settings('user_front_page')
    ) {
        return;
    }
 ...
 */
function bp_nouveau_members_catch_button_args($button = [])
{
    /*
 ...
 */
function bp_nouveau_member_reset_front_template($templates = [])
{
 ...
 */
function bp_nouveau_member_restrict_user_front_templates($templates = [])
{
    return array_intersect(
        ['members/single/front.php', 'members/single/default-front.php'],
        $templates
    );
}
...
function bp_nouveau_member_locate_template_part($template = '')
{
    ...
    if (!isset($bp_nouveau->members->displayed_user_hierarchy)) {
        $bp_nouveau->members->displayed_user_hierarchy = [
            'members/single/%s-id-' . (int) $displayed_user->id . '.php',
            'members/single/%s-nicename-' .
        ...
        if (bp_get_member_types()) {
            $displayed_user_member_type = bp_get_member_type(
                $displayed_user->id
            );
    ...
    return bp_locate_template(
        apply_filters('bp_nouveau_member_locate_template_part', $templates),
        false,
        true
    );
}
...
function bp_nouveau_members_wp_bio_info()
{
    ...
    return !empty($user_settings['user_front_page']) &&
        !empty($user_settings['user_front_bio']);
}
 ...
 */
function bp_nouveau_member_activity_widget_overrides($args = [])
{
 ...
 */
function bp_nouveau_member_groups_widget_overrides($args = [])
{
    return array_merge($args, [
        'user_id' => bp_displayed_user_id(),
    ]);
 ...
 */
function bp_nouveau_member_members_widget_overrides($args = [])
{
    // Do nothing for the friends widget
    if (
        !empty($args['user_id']) &&
        (int) $args['user_id'] === (int) bp_displayed_user_id()
    ) {
        return $args;
    }
...
function bp_nouveau_get_wp_profile_fields($user = null)
{
     ...
     */
    $contact_methods = (array) apply_filters(
...
{
    add_filter(
        '_bp_nouveau_member_reset_front_template',
