...
class BP_Nouveau_Nav_Customize_Control extends WP_Customize_Control
{
    ...
    public function render_content()
    {
        $id =
            'customize-control-' .
            str_replace('[', '-', str_replace(']', '', $this->id));
        ...
        if ('group' === $this->type) {
            $guide = __(
                'Customizing the Groups navigation order needs you create at least one group first.',
                'buddypress'
            );
            ...
            if ($item_nav) {
                $guide = __(
                    'Drag each possible group navigation items that are listed below into the order you prefer, in some groups some of these navigation items might not be active.',
                    'buddypress'
                );
            }
            ...
            $guide = __(
                'Drag each possible member navigation items that are listed below into the order you prefer.',
                'buddypress'
            );
        }
        ?>
       ...
       $id
   ); ?>" class="ui-sortable" style="margin-top: 0px; height: 500px;" data-bp-type="<?php echo esc_attr(
    $this->type
); ?>">
						...
						<div class="menu-item-bar">
							<div class="menu-item-handle ui-sortable-handle">
								<span class="item-title" aria-hidden="true">
									<span class="menu-item-title"><?php echo esc_html(
             _bp_strip_spans_from_title($item->name)
         ); ?></span>
								</span>
							</div>
						</div>
    ...
    ?>
       ...
       'bp_item_' . $this->type
   ); ?>" type="hidden" value="" data-customize-setting-link="<?php echo esc_attr(
    $setting
); ?>" />
