...
class BP_Nouveau_Group_Invite_Query extends BP_User_Query
{
    ...
    {
        add_action('bp_pre_user_query_construct', [
            $this,
            'build_exclude_args',
        ]);
        add_action('bp_pre_user_query', [$this, 'build_meta_query']);
    }
    ...
    protected function get_group_member_ids()
    {
        ...
        /** LIMIT clause ******************************************************/
        $this->group_member_ids = $wpdb->get_col(
            "{$sql['select']} {$sql['where']} {$sql['orderby']} {$sql['order']} {$sql['limit']}"
        );
    ...
    {
        if (
            isset($this->query_vars['scope']) &&
            'members' === $this->query_vars['scope'] &&
            isset($this->query_vars['meta_query'])
        ) {
            $invites_meta_query = new WP_Meta_Query(
...
class BP_Nouveau_Customizer_Group_Nav extends BP_Core_Nav
{
    ...
    public function __construct($object_id = 0)
    {
        ...
        $group = groups_get_group(['group_id' => $object_id]);
        if (empty($group->id)) {
            return $error;
        }
    ...
    protected function setup_nav()
    {
        $nav_items = [
            'root' => [
                ...
                'default_subnav_slug' => apply_filters(
        ...
        add_filter(
            '_bp_nouveau_group_reset_front_template',
            [$this, 'all_groups_fronts'],
            10,
            1
        );
        ...
        foreach (get_declared_classes() as $class) {
            if (is_subclass_of($class, 'BP_Group_Extension')) {
                ...
                if (
                    !empty($extension->params) &&
                    !array_diff_key($required_params, $extension->params)
                ) {
                    $nav_items[$extension->params['slug']] = [
     ...
     */
    public function all_groups_fronts($templates = [])
    {
        return array_intersect(
            ['groups/single/front.php', 'groups/single/default-front.php'],
            $templates
        );
    }
    ...
    {
        $default_nav = $this->get_secondary([
            'parent_slug' => $this->group->slug,
        ]);
    ...
    public function get_group_nav()
    {
        ...
        return $this->get_secondary(['parent_slug' => $this->group->slug]);
    }
}
...
class BP_Nouveau_Group_Meta
{
    ...
    public function __get($key = '')
    {
        ...
        _doing_it_wrong(
            ...
            sprintf(
                __('Please use %s instead', 'buddypress'),
                'bp_nouveau_the_group_meta( array( \'keys\' => \'' .
                    $key .
                    '\' ) )'
            ),
            '7.0.0'
        ...
        return bp_nouveau_the_group_meta(['keys' => $key, 'echo' => false]);
    }
    ...
    {
        // Backwards compatibility.
        $this->meta = bp_nouveau_the_group_meta(['echo' => false]);
    }
}
