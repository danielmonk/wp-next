...
function bp_nouveau_message_markup_wrapper($message, $type)
{
    ...
    $message =
        '<div class=" ' .
        esc_attr("bp-feedback {$type}") .
        '"><span class="bp-icon" aria-hidden="true"></span><p>' .
        esc_html($message) .
        '</p></div>';
 ...
 */
function bp_nouveau_groups_register_scripts($scripts = [])
{
    ...
    return array_merge($scripts, [
        'bp-nouveau-group-invites' => [
            ...
            'dependencies' => ['bp-nouveau', 'json2', 'wp-backbone'],
            'footer' => true,
        ],
    ]);
}
...
function bp_nouveau_groups_enqueue_scripts()
{
    ...
    if (
        bp_is_group_home() &&
        bp_nouveau_get_appearance_settings('group_front_page')
    ) {
        wp_add_inline_style(
            'bp-nouveau',
            '
    ...
    if (
        bp_is_group_invites() ||
        (bp_is_group_create() && bp_is_group_creation_step('group-invites'))
    ) {
    ...
    if (
        ...
        bp_is_group_admin_screen('manage-members')
    ) {
 ...
 */
function bp_nouveau_groups_localize_scripts($params = [])
{
    if (
        !bp_is_group_invites() &&
        !(bp_is_group_create() && bp_is_group_creation_step('group-invites'))
    ...
    $show_pending =
        bp_group_has_invites(['user_id' => 'any']) && !bp_is_group_create();
    ...
    $params['group_invites'] = [
        ...
        'invites_form_reset' => __(
            'Group invitations cleared. Please use one of the available tabs to select members to invite.',
            'buddypress'
        ),
        'invites_sending' => __(
            'Sending group invitations. Please wait.',
            'buddypress'
        ),
...
function bp_nouveau_prepare_group_potential_invites_for_js($user)
{
    ...
    if (
        !empty($bp->groups->invites_scope) &&
        'invited' === $bp->groups->invites_scope
    ) {
        $response['is_sent'] = (bool) groups_check_user_has_invite(
            $user->ID,
            bp_get_current_group_id()
        );

        $inviter_ids = bp_nouveau_groups_get_inviter_ids(
            $user->ID,
            bp_get_current_group_id()
        );
 ...
 * @since 3.0.0
 */
function bp_nouveau_get_group_potential_invites($args = [])
{
    ...
    $response->meta = ['total_page' => 0, 'current_page' => 0];
    $response->users = [];
    ...
    if (!empty($query->results)) {
        ...
        if (!empty($r['per_page'])) {
            $response->meta = [
                'total_page' => ceil(
                    (int) $query->total_users / (int) $r['per_page']
                ),
                'page' => (int) $r['page'],
            ];
        }
 ...
 * @since 3.0.0
 */
function bp_nouveau_group_invites_create_steps($steps = [])
{
    if (bp_is_active('friends') && isset($steps['group-invites'])) {
        // Simply change the name
        $steps['group-invites']['name'] = _x(
            'Invite',
            'Group invitations menu title',
            'buddypress'
        );
        return $steps;
    }
...
function bp_nouveau_groups_invites_restriction_nav()
{
    ...
    bp_core_new_subnav_item(
        [
            'name' => _x(
                'Group Invites',
                'Group invitations main menu title',
                'buddypress'
            ),
            'slug' => 'invites',
...
function bp_nouveau_groups_invites_restriction_admin_nav($wp_admin_nav)
{
    ...
    $settings_link = trailingslashit(
        bp_loggedin_user_domain() . bp_get_settings_slug()
    );
...
function bp_nouveau_groups_screen_invites_restriction()
{
        ...
        if (bp_is_my_profile() || bp_current_user_can('bp_moderate')) {
            if (empty($_POST['account-group-invites-preferences'])) {
                bp_delete_user_meta(
                    ...
                    '_bp_nouveau_restrict_invites_to_friends'
                );
            } else {
                bp_update_user_meta(
                    bp_displayed_user_id(),
                    '_bp_nouveau_restrict_invites_to_friends',
                    (int) $_POST['account-group-invites-preferences']
                );
            }

            bp_core_add_message(
                __('Group invites preferences saved.', 'buddypress')
            );
        } else {
            bp_core_add_message(
    ...
    bp_core_load_template(
        apply_filters(
            ...
            'members/single/settings/group-invites'
        )
    );
}
...
function bp_nouveau_restrict_rest_group_invite_to_friends($retval, $request)
{
    if (true === $retval && bp_is_active('friends')) {
        ...
        if (
            ...
            'is_friend' !==
                BP_Friends_Friendship::check_is_friend($inviter_id, $user_id)
        ) {
            $retval = new WP_Error(
...
function bp_nouveau_get_groups_directory_nav_items()
{
    ...
    if (is_user_logged_in()) {
        $my_groups_count = bp_get_total_group_count_for_user(
            bp_loggedin_user_id()
        );
        ...
        if (bp_user_can_create_groups()) {
            $nav_items['create'] = [
                ...
                'li_class' => ['no-ajax', 'group-create', 'create-button'],
 ...
 */
function bp_nouveau_groups_catch_button_args($button = [])
{
    /**
...
function bp_nouveau_groups_do_group_boxes()
{
    ...
    return !empty($group_settings['group_front_page']) &&
        !empty($group_settings['group_front_boxes']);
}
...
function bp_nouveau_groups_front_page_description()
{
    ...
    // @todo
    return !empty($group_settings['group_front_page']) &&
        !empty($group_settings['group_front_description']) &&
        bp_is_group_home();
}
 ...
 */
function bp_nouveau_groups_customizer_sections($sections = [])
{
    return array_merge($sections, [
        'bp_nouveau_group_front_page' => [
            'title' => __('Group front page', 'buddypress'),
            'panel' => 'bp_nouveau_panel',
            'priority' => 20,
            'description' => __(
                'Configure the default front page for groups.',
                'buddypress'
            ),
        ],
        'bp_nouveau_group_primary_nav' => [
            ...
            'priority' => 40,
            'description' => __(
                'Customize the navigation menu for groups. See your changes by navigating to a group in the live-preview window.',
                'buddypress'
            ),
        ],
    ]);
 ...
 */
function bp_nouveau_groups_customizer_settings($settings = [])
{
    return array_merge($settings, [
        'bp_nouveau_appearance[group_front_page]' => [
        ...
        'bp_nouveau_appearance[group_front_boxes]' => [
        ...
        'bp_nouveau_appearance[group_front_description]' => [
        ...
        'bp_nouveau_appearance[group_nav_display]' => [
        ...
        'bp_nouveau_appearance[group_nav_tabs]' => [
        ...
        'bp_nouveau_appearance[group_subnav_tabs]' => [
        ...
        'bp_nouveau_appearance[groups_create_tabs]' => [
        ...
        'bp_nouveau_appearance[group_nav_order]' => [
        ...
        'bp_nouveau_appearance[groups_layout]' => [
            'index' => 'groups_layout',
            'capability' => 'bp_moderate',
            'sanitize_callback' => 'absint',
            'transport' => 'refresh',
        ...
        'bp_nouveau_appearance[groups_dir_tabs]' => [
 ...
 */
function bp_nouveau_groups_customizer_controls($controls = [])
{
    return array_merge($controls, [
        'group_front_page' => [
            'label' => __(
                'Enable custom front pages for groups.',
                'buddypress'
            ),
        ...
        'group_front_boxes' => [
            'label' => __(
                'Enable widget region for group homepages. When enabled, the site admin can add widgets to group pages via the Widgets panel.',
                'buddypress'
            ),
            'section' => 'bp_nouveau_group_front_page',
        ...
        'group_nav_order' => [
            ...
            'label' => __(
                'Reorder the primary navigation for a group.',
                'buddypress'
            ),
            'section' => 'bp_nouveau_group_primary_nav',
 ...
 */
function bp_nouveau_group_reset_front_template($templates = [], $group = null)
{
    if (empty($group->id)) {
        return $templates;
    }
    ...
    if (is_customize_preview()) {
        $use_default_front = bp_nouveau_get_temporary_setting(
            'group_front_page',
            $use_default_front
        );
    }
...
function bp_nouveau_group_locate_template_part($template = '')
{
    ...
    if (!isset($bp_nouveau->groups->current_group_hierarchy)) {
        $bp_nouveau->groups->current_group_hierarchy = [
            ...
            'groups/single/%s-slug-' .
            sanitize_file_name($current_group->slug) .
            '.php',
        ];
        ...
        if (bp_groups_get_group_types()) {
            ...
            $bp_nouveau->groups->current_group_hierarchy[] =
                'groups/single/%s-group-type-' .
                sanitize_file_name($current_group_type) .
                '.php';
        }
        ...
        $bp_nouveau->groups->current_group_hierarchy = array_merge(
            $bp_nouveau->groups->current_group_hierarchy,
            [
                'groups/single/%s-status-' .
                sanitize_file_name($current_group->status) .
                '.php',
                'groups/single/%s.php',
            ]
        );
    ...
    return bp_locate_template(
        apply_filters('bp_nouveau_group_locate_template_part', $templates),
        false,
        true
    );
}
 ...
 */
function bp_nouveau_group_activity_widget_overrides($args = [])
{
 ...
 */
function bp_nouveau_group_groups_widget_overrides($args = [])
{
    return array_merge($args, [
 ...
 */
function bp_nouveau_group_members_widget_overrides($args = [])
{
    $group_members = groups_get_group_members(['exclude_admins_mods' => false]);
 ...
 */
function bp_nouveau_rest_group_invites_get_items_permissions_check(
    $retval,
    $request
) {
