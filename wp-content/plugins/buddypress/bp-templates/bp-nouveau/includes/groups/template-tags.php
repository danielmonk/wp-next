...
function bp_nouveau_group_creation_tabs()
{
    ...
    foreach ((array) $bp->groups->group_creation_steps as $slug => $step) {
      ...
      bp_get_groups_current_create_step() === $slug
        ...
        bp_groups_directory_permalink() . 'create/step/' . $slug . '/'
    ); ?>">
...
function bp_nouveau_group_manage_screen()
{
    ...
    if (
        !bp_is_group_admin_screen($screen_id) &&
        !bp_is_group_creation_step($screen_id)
    ) {
        return;
    }
    ...
    if (!$core_screen) {
        ...
        if (!empty($core_screen['nonce'])) {
            if (!$is_group_create) {
                $output = sprintf(
                    '<p><input type="submit" value="%s" id="save" name="save" /></p>',
                    esc_attr__('Save Changes', 'buddypress')
                );
        ...
        if ('crop-image' !== bp_get_avatar_admin_step()) {
            ...
            if (!bp_is_first_group_creation_step()) {
                $creation_step_buttons .= sprintf(
                    ...
                    "location.href='" .
                        esc_js(
                            esc_url_raw(bp_get_group_creation_previous_link())
                        ) .
                        "'"
                );
            }
            ...
            $output = sprintf(
                '<div class="submit" id="previous-next">%s</div>',
                $creation_step_buttons
            );
        }
    ...
    printf(
        ...
        $is_group_create
            ? esc_attr(bp_get_new_group_id())
            : esc_attr(bp_get_group_id())
    );
 ...
 */
function bp_nouveau_group_header_buttons($args = [])
{
    ...
    if (!$args) {
        $args = ['classes' => ['item-buttons']];
    }

    bp_nouveau_wrapper(array_merge($args, ['output' => $output]));
}
 ...
 * @param array $args Optional. See bp_nouveau_wrapper() for the description of parameters.
 */
function bp_nouveau_groups_loop_buttons($args = [])
{
 ...
 * @param array $args Optional. See bp_nouveau_wrapper() for the description of parameters.
 */
function bp_nouveau_groups_invite_buttons($args = [])
{
    if (empty($GLOBALS['groups_template'])) {
        return;
    }
 ...
 * @param array $args Optional. See bp_nouveau_wrapper() for the description of parameters.
 */
function bp_nouveau_groups_request_buttons($args = [])
{
 ...
 * @param array $args Optional. See bp_nouveau_wrapper() for the description of parameters.
 */
function bp_nouveau_groups_manage_members_buttons($args = [])
{
...
function bp_nouveau_get_groups_buttons($args = [])
{
    ...
    if (
        ('loop' === $type || 'invite' === $type) &&
        isset($GLOBALS['groups_template']->group)
    ) {
    ...
    if ('invite' === $type) {
        ...
        if ('button' === $button_element) {
            $buttons['accept_invite']['button_attr']['data-bp-nonce'] = esc_url(
                bp_get_group_accept_invite_link()
            );
        } else {
            $buttons['accept_invite']['button_attr']['href'] = esc_url(
                bp_get_group_accept_invite_link()
            );
        }

        // Setup Reject button attributes
        $buttons['reject_invite'] = [
            'id' => 'reject_invite',
            'position' => 15,
            'component' => 'groups',
            'must_be_logged_in' => true,
        ...
        if ('button' === $button_element) {
            $buttons['reject_invite']['button_attr']['data-bp-nonce'] = esc_url(
                bp_get_group_reject_invite_link()
            );
        } else {
            $buttons['reject_invite']['button_attr']['href'] = esc_url(
                bp_get_group_reject_invite_link()
            );
        }
        ...
        // Setup Accept button attributes
        $buttons['group_membership_accept'] = [
        ...
        // If button element set add nonce link to data-attr attr
        if ('button' === $button_element) {
            $buttons['group_membership_accept']['button_attr'][
                'data-bp-nonce'
            ] = esc_url(bp_get_group_request_accept_link());
        } else {
            $buttons['group_membership_accept']['button_attr'][
                'href'
            ] = esc_url(bp_get_group_request_accept_link());
        ...
        $buttons['group_membership_reject'] = [
        ...
        // If button element set add nonce link to data-attr attr
        if ('button' === $button_element) {
            $buttons['group_membership_reject']['button_attr'][
                'data-bp-nonce'
            ] = esc_url(bp_get_group_request_reject_link());
        } else {
            $buttons['group_membership_reject']['button_attr'][
                'href'
            ] = esc_url(bp_get_group_request_reject_link());
        ...
        'manage_members' === $type &&
        isset($GLOBALS['members_template']->member->user_id)
    ) {
        ...
        if ('button' === $button_element) {
            $buttons['unban_member']['button_attr'][
                'data-bp-nonce'
            ] = bp_get_group_member_unban_link($user_id);
            $buttons['ban_member']['button_attr'][
                'data-bp-nonce'
            ] = bp_get_group_member_ban_link($user_id);
            $buttons['promote_mod']['button_attr'][
                'data-bp-nonce'
            ] = bp_get_group_member_promote_mod_link();
            $buttons['promote_admin']['button_attr'][
                'data-bp-nonce'
            ] = bp_get_group_member_promote_admin_link();
            $buttons['remove_member']['button_attr'][
                'data-bp-nonce'
            ] = bp_get_group_member_remove_link($user_id);
        } else {
            $buttons['unban_member']['button_attr'][
                'href'
            ] = bp_get_group_member_unban_link($user_id);
        ...
        if (
            isset(bp_nouveau()->groups->button_args) &&
            ...
            $buttons['group_membership'] = [
                ...
                'button_attr' => [
                    'id' => !empty($button_args['link_id'])
                        ? $button_args['link_id']
                        : '',
                    'class' => $button_args['link_class'] . ' button',
                    'rel' => !empty($button_args['link_rel'])
                        ? $button_args['link_rel']
                        : '',
                    'title' => '',
                ],
            ];
            ...
            if ('button' === $button_element) {
                $buttons['group_membership']['button_attr']['data-bp-nonce'] =
                    $button_args['link_href'];
            } else {
                // Else this is an anchor so use an 'href' attr.
                $buttons['group_membership']['button_attr']['href'] =
                    $button_args['link_href'];
            }
    ...
    if (
        !isset(bp_nouveau()->groups->group_buttons) ||
        !is_a(bp_nouveau()->groups->group_buttons, 'BP_Buttons_Group')
    ) {
        $sort = true;
        bp_nouveau()->groups->group_buttons = new BP_Buttons_Group(
            $buttons_group
        );
     ...
     */
    do_action_ref_array('bp_nouveau_return_groups_buttons', [
        &$return,
        $group,
        $type,
    ]);
 ...
 * @return string HTML Output.
 */
function bp_nouveau_the_group_meta($args = [])
{
    $r = bp_parse_args(
        $args,
        [
 ...
 */
function bp_nouveau_get_group_meta($keys = [])
{
    $keys = (array) $keys;
    $group = false;
    $group_meta = [];
    ...
    foreach ($keys as $key) {
        switch ($key) {
            ...
            case 'group_type_list':
                $group_meta['group_type_list'] = bp_get_group_type_list(
                    $group->id
                );
                break;
...
function bp_nouveau_group_type_checked($type = null)
{
    ...
    if (bp_is_group_create()) {
        ...
        checked(
            bp_groups_has_group_type(bp_get_current_group_id(), $type->name)
        );
    }
}
