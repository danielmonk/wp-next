...
function bp_nouveau_ajax_querystring($query_string, $object)
{
    ...
    if (
        !empty($post_query['search_terms']) &&
        $object_search_text != $post_query['search_terms'] &&
        'false' != $post_query['search_terms'] &&
        'undefined' != $post_query['search_terms']
    ) {
    ...
    list($filter, $scope, $page, $search_terms, $extras) = array_values(
        $post_query
    );
 ...
 * @return string
 */
function bp_nouveau_ajax_button(
    $output = '',
    $button = null,
    $before = '',
    $after = '',
    ...
    if (!empty($reset_ids[$button->id])) {
        $parse_class = array_map(
            'sanitize_html_class',
            explode(' ', $r['button_attr']['class'])
        );
        if (false === $parse_class) {
            return $output;
        }

        $find_id = array_intersect($parse_class, [
            'pending_friend',
            'is_friend',
            'not_friends',
            'leave-group',
 ...
 * }
 */
function bp_nouveau_wrapper($args = [])
{
    /**
    ...
    $r = bp_parse_args(
        ...
        [
            'container' => 'div',
            'container_id' => '',
            'container_classes' => [$generic_class, $current_component_class],
            'output' => '',
        ],
        'nouveau_wrapper'
    );
    ...
    $r['container_classes'] = array_merge(
        $r['container_classes'],
        $default_classes
    );
    ...
    if (
        empty($r['container']) ||
        !isset($valid_containers[$r['container']]) ||
        empty($r['output'])
    ) {
        return;
    }
    ...
    if (!empty($r['container_classes']) && is_array($r['container_classes'])) {
        $container_classes =
            ...
            join(
                ' ',
                array_map('sanitize_html_class', $r['container_classes'])
            ) .
            '"';
    }
...
function bp_nouveau_register_sidebars()
{
    ...
    if (is_customize_preview()) {
        $default_user_front = bp_nouveau_get_temporary_setting(
            'user_front_page',
            $default_user_front
        );

        if ($is_active_groups) {
            $default_group_front = bp_nouveau_get_temporary_setting(
                'group_front_page',
                $default_group_front
            );
        }
    ...
    if ($default_user_front) {
        $sidebars[] = [
            ...
            'description' => __(
                'Add widgets here to appear in the front page of each member of your community.',
                'buddypress'
            ),
    ...
    if ($default_group_front) {
        $sidebars[] = [
            ...
            'description' => __(
                'Add widgets here to appear in the front page of each group of your community.',
                'buddypress'
            ),
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
 ...
 */
function bp_nouveau_parse_hooked_dir_nav(
    $hook = '',
    $component = '',
    $position = 99
) {
    ...
    $extra_nav_items = array_fill_keys($lis[1], [
        'component' => $component,
        'position' => $position,
    ]);
        ...
        foreach ($as[0] as $ka => $a) {
            ...
            if (!empty($attrs[1])) {
                foreach ($attrs[1] as $katt => $att) {
                    if ('href' === $att) {
                        $extra_nav_items[$lis[1][$ka]]['link'] = trim(
                            $attrs[2][$katt],
                            '"'
                        );
                    } else {
                        $extra_nav_items[$lis[1][$ka]][$att] = trim(
                            $attrs[2][$katt],
                            '"'
                        );
                    }
 ...
 * @return array
 */
function bp_nouveau_parse_hooked_options($hook = '', $filters = [])
{
    if (empty($hook)) {
        return $filters;
    }
...
function bp_nouveau_get_component_filters($context = '', $component = '')
{
    ...
    if ('members' === $component) {
        ...
        $filters = array_merge(
            ['-1' => __('&mdash; Everything &mdash;', 'buddypress')],
            $filters
        );
...
function bp_nouveau_get_temporary_setting($option = '', $retval = false)
{
    ...
    if ('any' === $option) {
        ...
        isset($temporary_setting['bp_nouveau_appearance[' . $option . ']'])
    ) {
...
function bp_nouveau_get_appearance_settings($option = '')
{
    ...
    $settings = bp_parse_args(
        bp_get_option('bp_nouveau_appearance', []),
        $default_args,
 ...
 * @return string
 */
function bp_nouveau_theme_cover_image($params = [])
{
    if (empty($params)) {
        return '';
    }
    ...
    $cover_image = isset($params['cover_image'])
...
function bp_nouveau_get_user_feedback($feedback_id = '')
{
     ...
     */
    $feedback_messages = apply_filters('bp_nouveau_feedback_messages', [
        'registration-disabled' => [
            'type' => 'info',
            'message' => __(
                'Member registration is currently not allowed.',
                'buddypress'
            ),
        ...
        'group-members-none' => [
            'type' => 'info',
            'message' => __(
                'Sorry, there were no group members found.',
                'buddypress'
            ),
        ],
        'group-members-search-none' => [
            'type' => 'info',
            'message' => __(
                'Sorry, there was no member of that name found in this group.',
                'buddypress'
            ),
        ],
        'group-manage-members-none' => [
            'type' => 'info',
        ...
        'group-requests-loading' => [
            'type' => 'loading',
            'message' => __(
                'Loading the members who requested to join the group. Please wait.',
                'buddypress'
            ),
        ],
        ...
        'member-requests-none' => [
            'type' => 'info',
            'message' => __(
                'You have no pending friendship requests.',
                'buddypress'
            ),
        ],
        'member-invites-none' => [
        ...
        'member-notifications-none' => [
            'type' => 'info',
            'message' => __('This member has no notifications.', 'buddypress'),
        ],
        'member-wp-profile-none' => [
            'type' => 'info',
            /* translators: %s: member name */
            'message' => __(
                '%s did not save any profile information yet.',
                'buddypress'
            ),
        ],
        'member-delete-account' => [
            'type' => 'warning',
            'message' => __(
                'Deleting this account will delete all of the content it has created. It will be completely unrecoverable.',
                'buddypress'
            ),
        ],
        ...
        'member-blogs-loading' => [
            'type' => 'loading',
            'message' => __(
                'Loading the member\'s blogs. Please wait.',
                'buddypress'
            ),
        ],
        'member-friends-loading' => [
            'type' => 'loading',
            'message' => __(
                'Loading the member\'s friends. Please wait.',
                'buddypress'
            ),
        ],
        'member-groups-loading' => [
            'type' => 'loading',
            'message' => __(
        ...
        'member-group-invites-all' => [
            'type' => 'info',
            'message' => __(
                'Currently every member of the community can invite you to join their groups. If you are not comfortable with it, you can always restrict group invites to your friends only.',
                'buddypress'
            ),
        ],
        'member-group-invites-friends-only' => [
            'type' => 'info',
            'message' => __(
                'Currently only your friends can invite you to groups. Uncheck the box to allow any member to send invites.',
                'buddypress'
            ),
        ],
    ]);
     ...
     */
    if (
        'completed-confirmation' === $feedback_id &&
        bp_registration_needs_activation()
    ) {
        $feedback_messages['completed-confirmation']['message'] = __(
        ...
        if (bp_is_current_action('unread')) {
            $feedback_messages['member-notifications-none']['message'] = __(
        ...
        'member-wp-profile-none' === $feedback_id &&
        bp_is_user_profile()
    ) {
        $feedback_messages['member-wp-profile-none']['message'] = sprintf(
            $feedback_messages['member-wp-profile-none']['message'],
            bp_get_displayed_user_fullname()
        );
    } elseif ('member-delete-account' === $feedback_id && bp_is_my_profile()) {
        $feedback_messages['member-delete-account']['message'] = __(
            'Deleting your account will delete all of the content you have created. It will be completely irrecoverable.',
            'buddypress'
        );
    } elseif (
        'member-activity-loading' === $feedback_id &&
        bp_is_my_profile()
    ) {
        $feedback_messages['member-activity-loading']['message'] = __(
            'Loading your updates. Please wait.',
            'buddypress'
        );
    } elseif ('member-blogs-loading' === $feedback_id && bp_is_my_profile()) {
        $feedback_messages['member-blogs-loading']['message'] = __(
            'Loading your blogs. Please wait.',
            'buddypress'
        );
    } elseif ('member-friends-loading' === $feedback_id && bp_is_my_profile()) {
        $feedback_messages['member-friends-loading']['message'] = __(
            'Loading your friends. Please wait.',
            'buddypress'
        );
    } elseif ('member-groups-loading' === $feedback_id && bp_is_my_profile()) {
        $feedback_messages['member-groups-loading']['message'] = __(
...
function bp_nouveau_get_signup_fields($section = '')
{
     ...
     */
    $fields = apply_filters('bp_nouveau_get_signup_fields', [
        'account_details' => [
            'signup_username' => [
...
function bp_nouveau_get_submit_button($action = '')
{
     ...
     * @return array|false
     */
    $actions = apply_filters('bp_nouveau_get_submit_button', [
        'register' => [
 ...
 */
function bp_nouveau_set_nav_item_order(
    $nav = null,
    $order = [],
    $parent_slug = ''
) {
    ...
    foreach ($order as $slug) {
        ...
        if ((int) $item_nav->position !== (int) $position) {
            $nav->edit_nav(['position' => $position], $slug, $parent_slug);
        }
    }
