...
function bp_nouveau_ajax_addremove_friend()
{
    ...
    if (
        isset($_POST['action']) &&
        $_POST['action'] !== 'friends_accept_friendship' &&
        $_POST['action'] !== 'friends_reject_friendship'
    ) {
    ...
    if (
        !empty($_POST['action']) &&
        'friends_accept_friendship' === $_POST['action']
    ) {
        ...
        !empty($_POST['action']) &&
        'friends_reject_friendship' === $_POST['action']
    ) {
        ...
        if (!friends_remove_friend(bp_loggedin_user_id(), $friend_id)) {
            ...
            if (!$is_user) {
                $response = [
                    'contents' => bp_get_add_friend_button($friend_id),
                ];
            } else {
                $response = [
                    'feedback' => sprintf(
        ...
        if (friends_withdraw_friendship(bp_loggedin_user_id(), $friend_id)) {
            ...
            $response['feedback'] = sprintf(
                '<div class="bp-feedback error">%s</div>',
                esc_html__(
                    'Friendship request could not be cancelled.',
                    'buddypress'
                )
            );
