...
class BP_Nouveau extends BP_Theme_Compat
{
    ...
    protected function setup_globals()
    {
        ...
        foreach (
            $bp->theme_compat->packages['nouveau']
            as $property => $value
        ) {
            $this->{$property} = $value;
        }
    ...
    protected function includes()
    {
        ...
        foreach (bp_core_get_packaged_component_ids() as $component) {
            $component_loader =
                trailingslashit($this->includes_dir) .
                $component .
                '/loader.php';
         ...
         */
        do_action_ref_array('bp_nouveau_includes', [&$this]);
    }
    ...
    protected function setup_support()
    {
        ...
        bp_set_theme_compat_feature($this->id, [
            'name' => 'cover_image',
            'settings' => [
    ...
    protected function setup_actions()
    {
        ...
        add_action(
            'bp_actions',
            [$this, 'neutralize_core_template_notices'],
            6
        );

        // Scripts.
        add_action('bp_enqueue_scripts', [$this, 'register_scripts'], 2); // Register theme JS.
        ...
        add_action('bp_screens', [$this, 'setup_directory_nav'], 15);
    ...
    public function enqueue_styles()
    {
         ...
         */
        $styles = apply_filters('bp_nouveau_enqueue_styles', [
            'bp-nouveau' => [
        ...
        $theme = get_template();
        $companion_stylesheet = bp_locate_template_asset(
            sprintf('css/%1$s%2$s.css', $theme, $min)
        );
        $companion_handle = 'bp-' . $theme;

        if (
            !is_rtl() &&
            isset($companion_stylesheet['uri']) &&
        ...
        if (is_rtl()) {
            $rtl_companion_stylesheet = bp_locate_template_asset(
                sprintf('css/%1$s-rtl%2$s.css', $theme, $min)
            );
    ...
    public function register_scripts()
    {
         ...
         */
        $scripts = apply_filters('bp_nouveau_register_scripts', [
            'bp-nouveau' => [
        ...
        if (bp_is_active('settings') || bp_get_signup_allowed()) {
             ...
             * @deprecated 5.0.0
             */
            $scripts['bp-nouveau-password-verify'] = [
                'file' => 'js/password-verify%s.js',
                'dependencies' => ['bp-nouveau', 'password-strength-meter'],
                'footer' => true,
            ];
        }
    ...
    public function localize_scripts()
    {
         ...
         */
        $supported_objects = (array) apply_filters(
        ...
        if (bp_is_active('groups')) {
            $supported_objects = array_merge($supported_objects, [
                'group_members',
                'group_requests',
            ]);
        }
        ...
        if (is_customize_preview()) {
            $params['customizer_settings'] = bp_nouveau_get_temporary_setting(
                'any'
            );
        }
        ...
        wp_localize_script(
            'bp-nouveau',
            'BP_Nouveau',
            apply_filters('bp_core_get_js_strings', $params)
        );
    }
    ...
    public function setup_directory_nav()
    {
        ...
        foreach ($nav_items as $nav_item) {
            if (
                empty($nav_item['component']) ||
                $nav_item['component'] !== bp_current_component()
            ) {
                continue;
            }
    ...
    public function customizer_set_uri($path)
    {
        ...
        if (false === strpos($uri['path'], 'customize.php')) {
            ...
            $vars = bp_parse_args($uri['query'], [], 'customizer_set_uri');
