      ...
      void 0 !== bp.Nouveau.Activity &&
        ...
        (bp.Nouveau.Activity.postForm = {
          ...
          postFormView: function () {
            if (t("#bp-nouveau-activity-form").length) {
              var e = new bp.Views.PostForm();
              this.views.add({ id: "post_form", view: e }),
                e.inject("#bp-nouveau-activity-form");
            }
          },
        }),
        void 0 === bp.View &&
          (bp.View = bp.Backbone.View.extend({
            inject: function (e) {
              this.render(), t(e).html(this.el), this.views.ready();
            },
            prepare: function () {
              return !_.isUndefined(this.model) &&
                _.isFunction(this.model.toJSON)
                ? this.model.toJSON()
                : {};
            },
          })),
        (bp.Models.Activity = Backbone.Model.extend({
          defaults: { user_id: 0, item_id: 0, object: "", content: "" },
        })),
        (bp.Models.ActivityObject = Backbone.Model.extend({
          defaults: { id: 0, name: "", avatar_url: "", object_type: "group" },
        })),
        ...
        (bp.Views.activityFeedback = bp.View.extend({
          ...
          initialize: function () {
            (this.model = new Backbone.Model()),
              this.options.value &&
                this.model.set("message", this.options.value, { silent: !0 }),
              (this.type = "info"),
              _.isUndefined(this.options.type) ||
        ...
        (bp.Views.WhatsNew = bp.View.extend({
          ...
          adjustContent: function () {
            this.$el.css({ resize: "none", height: "50px" });
            var e = bp.Nouveau.getLinkParams(null, "r") || null;
            _.isNull(e) ||
              (this.$el.text("@" + _.escape(e) + " "), this.$el.focus());
          },
          resetContent: function (e) {
            _.isUndefined(e) || this.$el.val(e.get("content"));
          },
        })),
          ...
          change: function () {
            ...
            e &&
              this.model.set({
                ...
                placeholder: e.autocomplete_placeholder,
              });
          },
        })),
        (bp.Views.Item = bp.View.extend({
          ...
          initialize: function () {
            this.model.get("selected") && (this.el.className += " selected");
          },
          events: { click: "setObject" },
          setObject: function (e) {
            e.preventDefault(),
              !0 === this.model.get("selected")
                ? this.model.clear()
        ...
        (bp.Views.AutoComplete = bp.View.extend({
          ...
          events: { keyup: "autoComplete" },
          initialize: function () {
            var e = new bp.Views.ActivityInput({
              type: "text",
              id: "activity-autocomplete",
              placeholder: this.options.placeholder || "",
            }).render();
            this.$el.prepend(t("<li></li>").html(e.$el)),
          ...
          addItemView: function (e) {
            this.views.add(new bp.Views.Item({ model: e }));
          },
          autoComplete: function () {
            var e = t("#activity-autocomplete").val();
          ...
          itemFetched: function (e) {
            e.length || this.cleanView();
          },
          cleanView: function () {
        ...
        (bp.Views.FormAvatar = bp.View.extend({
          ...
          initialize: function () {
            (this.model = new Backbone.Model(
              ...
              this.model.has("avatar_url") &&
                this.model.set("display_avatar", !0);
          },
        })),
        (bp.Views.FormContent = bp.View.extend({
          ...
          initialize: function () {
            this.$el.html(t("<div></div>").prop("id", "whats-new-textarea")),
              this.views.set(
                "#whats-new-textarea",
        ...
        (bp.Views.FormTarget = bp.View.extend({
          ...
          initialize: function () {
            var e = new bp.Views.WhatsNewPostIn({
              filters: BP_Nouveau.activity.params.objects,
            });
            this.views.add(e),
              e.model.on("change", this.attachAutocomplete, this),
          ...
          attachAutocomplete: function (e) {
            0 !== bp.Nouveau.Activity.postForm.ActivityObjects.models.length &&
              bp.Nouveau.Activity.postForm.ActivityObjects.reset(),
              _.each(this.views._views[""], function (e) {
                _.isUndefined(e.collection) || e.remove();
              }),
              "profile" !== e.get("selected")
                ? (this.views.add(
                  ...
                  this.model.set("object", e.get("selected")))
                : this.model.set({ object: "user", item_id: 0 }),
              this.updateDisplay();
          },
          postIn: function (e) {
            if (_.isUndefined(e.get("id")))
              return (
            ...
            this.model.set("item_id", e.get("id")),
              this.views.set(
                "#whats-new-post-in-box-items",
                new bp.Views.Item({ model: e })
              );
          },
          updateDisplay: function () {
            "user" !== this.model.get("object")
              ? this.$el.removeClass()
              : this.$el.hasClass("in-profile") ||
        ...
        (bp.Views.FormButtons = bp.View.extend({
          ...
          initialize: function () {
            this.views.add(
              new bp.View({ tagName: "ul", id: "whats-new-buttons" })
            ),
              _.each(
          ...
          addItemView: function (e) {
            this.views.add(
              "#whats-new-buttons",
              new bp.Views.FormButton({ model: e })
            );
          },
          isActive: function (e) {
            _.each(this.views._views[""], function (e, t) {
              0 !== t && e.remove();
            }),
              !0 === e.get("active")
                ? (_.each(
                    ...
                    function (t) {
                      t.model.get("id") !== e.get("id") &&
                        (t.model.set("active", !1, { silent: !0 }),
                        t.$el.removeClass("active"),
                        this.collection.trigger(
                          "reset:" + t.model.get("id"),
                          this.model
                        ));
                    },
                    this
                  ),
                  this.collection.trigger("display:" + e.get("id"), this))
                : this.collection.trigger("reset:" + e.get("id"), this.model);
          },
        })),
        ...
        (bp.Views.PostForm = bp.View.extend({
          ...
          id: "whats-new-form",
          attributes: { name: "whats-new-form", method: "post" },
          events: {
          ...
          initialize: function () {
            ...
            !0 === this.options.backcompat.before_post_form &&
              e.unshift(new bp.Views.BeforeFormInputs()),
              (this.resetModel = this.model.clone()),
              this.views.set(e),
              this.model.on("change:errors", this.displayFeedback, this);
          },
          displayFull: function (e) {
            var i = !0 === this.options.backcompat.before_post_form ? 3 : 2;
            this.cleanFeedback(),
              i === this.views._views[""].length &&
                (t(e.target).css({ resize: "vertical", height: "auto" }),
                this.$el.addClass("activity-form-expanded"),
                !0 === this.options.backcompat.post_form_options
                  ? this.views.add(
                      new bp.Views.FormOptions({ model: this.model })
                    )
                  : this.views.add(new bp.View({ id: "whats-new-options" })),
                _.isUndefined(BP_Nouveau.activity.params.buttons) ||
          ...
          resetForm: function () {
            var e = this.options.backcompat.before_post_form ? 2 : 1;
            _.each(this.views._views[""], function (t, i) {
              i > e && t.remove();
            }),
              t("#whats-new").css({ resize: "none", height: "50px" }),
          ...
          cleanFeedback: function () {
            _.each(this.views._views[""], function (e) {
              "message" === e.$el.prop("id") && e.remove();
            });
          },
          displayFeedback: function (e) {
            _.isUndefined(this.model.get("errors"))
              ? this.cleanFeedback()
              : this.views.add(new bp.Views.activityFeedback(e.get("errors")));
          },
          postUpdate: function (e) {
            var i = this,
              s = {};
            if (e) {
              if ("keydown" === e.type && (13 !== e.keyCode || !e.ctrlKey))
                return e;
              e.preventDefault();
            }
            _.each(this.$el.serializeArray(), function (e) {
              (e.name = e.name.replace("[]", "")),
                "whats-new" === e.name
                  ? i.model.set("content", e.value)
                  : -1 ===
                      _.indexOf(
                        ["aw-whats-new-submit", "whats-new-post-in"],
                        e.name
                      ) &&
                    (_.isUndefined(s[e.name])
                      ? (s[e.name] = e.value)
                      : (_.isArray(s[e.name]) || (s[e.name] = [s[e.name]]),
                        s[e.name].push(e.value)));
            }),
              this.model.set(s, { silent: !0 });
            var a = {
              _wpnonce_post_update: BP_Nouveau.activity.params.post_nonce,
            };
            t("#_bp_as_nonce").val() &&
              (a._bp_as_nonce = t("#_bp_as_nonce").val()),
              bp.ajax
                .post("post_update", _.extend(a, this.model.attributes))
                .done(function (e) {
                  var s = bp.Nouveau.getStorage("bp-activity"),
                    a = t(
                      '[data-bp-search="activity"] input[type="search"]'
                    ).val(),
                    o = {},
                    n = !1;
                  a && ((a = new RegExp(a, "im")), (o = e.activity.match(a))),
                    (a && !o) ||
                      (n =
                        !s.filter ||
                        0 === parseInt(s.filter, 10) ||
                        "activity_update" === s.filter),
                    n &&
                      e.is_directory &&
                      (n =
                        ("all" === s.scope &&
                          ("user" === i.model.get("object") ||
                            !1 === e.is_private)) ||
                        i.model.get("object") + "s" === s.scope),
                    i.resetForm(),
                    n
                      ? t("#activity-" + e.id).length ||
                        (t("#activity-stream ul.activity-list").length ||
                          t("#activity-stream").html(
                            t("<ul></ul>").addClass(
                ...
                .fail(function (e) {
                  i.model.set("errors", { type: "error", value: e.message });
                });
          },
        })),
