...
(function (exports, $) {
  ...
  bp.Nouveau.Activity = {
    ...
    setupGlobals: function () {
      ...
      this.mentions_count =
        Number(
          $(bp.Nouveau.objectNavParent + ' [data-bp-scope="mentions"]')
            .find("a span")
            .html()
        ) || 0;
    ...
    addListeners: function () {
      ...
      // Inject Activities
      $('#buddypress [data-bp-list="activity"]').on(
        "click",
    ...
    heartbeatSend: function (event, data) {
      this.heartbeat_data.first_recorded =
        $("#buddypress [data-bp-list] [data-bp-activity-id]")
          .first()
          .data("bp-timestamp") || 0;

      if (
        0 === this.heartbeat_data.last_recorded ||
        this.heartbeat_data.first_recorded > this.heartbeat_data.last_recorded
      ) {
      ...
      if ($("#buddypress .dir-search input[type=search]").length) {
        data.bp_activity_last_recorded_search_terms = $(
    ...
    heartbeatTick: function (event, data) {
      var newest_activities_count,
        newest_activities,
        objects = bp.Nouveau.objects,
        scope = bp.Nouveau.getStorage("bp-activity", "scope"),
        self = this;
      ...
      this.heartbeat_data.newest =
        $.trim(data.bp_activity_newest_activities.activities) +
        this.heartbeat_data.newest;
      this.heartbeat_data.last_recorded = Number(
        data.bp_activity_newest_activities.last_recorded
      );

      // Parse activities
      newest_activities = $(this.heartbeat_data.newest).filter(
        ".activity-item"
      );
      ...
      if ("all" === scope) {
        $.each(newest_activities, function (a, activity) {
          ...
          $.each(objects, function (o, object) {
            if (
              -1 !== $.inArray("bp-my-" + object, activity.get(0).classList)
            ) {
              if (undefined === self.heartbeat_data.highlights[object]) {
                self.heartbeat_data.highlights[object] = [
                  activity.data("bp-activity-id"),
                ];
              } else if (
                -1 ===
                $.inArray(
                ...
                self.heartbeat_data.highlights[object].push(
                  activity.data("bp-activity-id")
                );
              }
            }
        ...
        this.heartbeat_data.newest = this.heartbeat_data.newest.replace(
          regexp,
          ""
        );
         ...
         */
        $(bp.Nouveau.objectNavParent + ' [data-bp-scope="all"]')
          .find("a span")
          .html(newest_activities_count);
        ...
        $.each(newest_activities, function (a, activity) {
          self.heartbeat_data.highlights[scope].push(
            $(activity).data("bp-activity-id")
          );
        });
      }
      ...
      $.each(objects, function (o, object) {
        if (
          undefined !== self.heartbeat_data.highlights[object] &&
          self.heartbeat_data.highlights[object].length
        ) {
          var count = 0;
          ...
          $(bp.Nouveau.objectNavParent + ' [data-bp-scope="' + object + '"]')
            .find("a span")
            .html(
              Number(self.heartbeat_data.highlights[object].length) + count
            );
        }
      });
      ...
      $(document).prop(
        "title",
        "(" +
          newest_activities_count +
          ") " +
          this.heartbeat_data.document_title
      );

      // Update the Load Newest li if it already exists.
      if (
        $('#buddypress [data-bp-list="activity"] li')
          .first()
        ...
        $('#buddypress [data-bp-list="activity"] .load-newest a').html(
          newest_link.replace(/([0-9]+)/, newest_activities_count)
        );

        // Otherwise add it
      } else {
        $('#buddypress [data-bp-list="activity"] ul.activity-list').prepend(
       ...
       */
      $('#buddypress [data-bp-list="activity"]').trigger(
    ...
    injectActivities: function (event) {
      ...
      if ($(event.currentTarget).hasClass("load-newest")) {
        ...
        $.each(activities, function (a, activity) {
          if (
            "LI" === activity.nodeName &&
            $(activity).hasClass("just-posted")
          ) {
        ...
        $(bp.Nouveau.objectNavParent + ' [data-bp-scope="' + scope + '"]')
          .find("a span")
          .html("");
        ...
        setTimeout(function () {
          $(event.delegateTarget)
            ...
            .removeClass("newest_" + scope + "_activity");
        }, 3000);
    ...
    hideComments: function (event) {
      ...
      comments.each(function (c, comment) {
        ...
        comment_count =
          $(
            "#acomment-comment-" +
              activity_item.data("bp-activity-id") +
              " span.comment-count"
          ).html() || " ";
              ...
              if ("undefined" !== typeof activity_id) {
                ...
                $(item).before(
                  '<li class="show-all"><button class="text-button" type="button" data-bp-show-comments-id="#activity-' +
                    ...
                    BP_Nouveau.show_x_comments.replace("%d", comment_count) +
                    "</button></li>"
                );
              }
            }
        ...
        if (
          $(comment_parents).children(".bp-hidden").length ===
            $(comment_parents).children("li").length - 1 &&
          $(comment_parents).find("li.show-all").length
        ) {
    ...
    scopeLoaded: function (event, data) {
      ...
      if (
        "mentions" === data.scope &&
        undefined !== data.response.new_mentions
      ) {
        $.each(data.response.new_mentions, function (i, id) {
          $("#buddypress #activity-stream")
            .find('[data-bp-activity-id="' + id + '"]')
        ...
        undefined !== this.heartbeat_data.highlights[data.scope] &&
        this.heartbeat_data.highlights[data.scope].length
      ) {
        $.each(this.heartbeat_data.highlights[data.scope], function (i, id) {
          if (
            $("#buddypress #activity-stream").find(
              '[data-bp-activity-id="' + id + '"]'
            ).length
          ) {
            $("#buddypress #activity-stream")
              .find('[data-bp-activity-id="' + id + '"]')
              .addClass("newest_" + data.scope + "_activity");
          }
        });
      }
      ...
      setTimeout(function () {
        $("#buddypress #activity-stream .activity-item").removeClass(
          "newest_" + data.scope + "_activity"
        );
      }, 3000);
    },
    ...
    activityActions: function (event) {
      ...
      if (target.hasClass("fav") || target.hasClass("unfav")) {
        ...
        parent
          .ajax(
            { action: "activity_mark_" + type, id: activity_id },
            "activity"
          )
          .done(function (response) {
            ...
            if ("fav" === type) {
              if (undefined !== response.data.directory_tab) {
                if (
                  !$(parent.objectNavParent + ' [data-bp-scope="favorites"]')
                    .length
                ) {
                  $(parent.objectNavParent + ' [data-bp-scope="all"]').after(
              ...
              var favoriteScope =
                $('[data-bp-user-scope="favorites"]').hasClass("selected") ||
                $(
                  parent.objectNavParent + ' [data-bp-scope="favorites"]'
                ).hasClass("selected");
              ...
              if (undefined !== response.data.no_favorite) {
                ...
                if (
                  $(parent.objectNavParent + ' [data-bp-scope="all"]').length &&
                  $(parent.objectNavParent + ' [data-bp-scope="all"]').hasClass(
                    "selected"
                  )
                ) {
                  $(
      ...
      if (
        target.hasClass("delete-activity") ||
        target.hasClass("acomment-delete") ||
        target.hasClass("spam-activity") ||
        target.hasClass("spam-activity-comment")
      ) {
        var activity_comment_li = target.closest(
            "[data-bp-activity-comment-id]"
          ),
          activity_comment_id = activity_comment_li.data(
            "bp-activity-comment-id"
          ),
          li_parent,
          comment_count_span,
          comment_count,
        ...
        if (
          undefined !== BP_Nouveau.confirm &&
          false === window.confirm(BP_Nouveau.confirm)
        ) {
          return false;
        }
        ...
        parent.ajax(ajaxData, "activity").done(function (response) {
          ...
          if (false === response.success) {
            ...
            if (activity_comment_id) {
              ...
              comment_count_span = activity_item.find(
                ".acomment-reply span.comment-count"
              );
              comment_count = Number(
                comment_count_span.html() - deleted_comments_count
              );
            ...
            if (
              !activity_comment_id &&
              activity_item.data("bp-timestamp") ===
                parent.Activity.heartbeat_data.last_recorded
            ) {
      ...
      if (
        ...
        if (item_id === activity_id) {
          $(
            '[data-bp-activity-id="' + item_id + '"] .activity-comments'
          ).append(form);
          form.addClass("root");
      ...
      if ("ac_form_submit" === target.prop("name")) {
        ...
        comment_data = {
          ...
          _wpnonce_new_activity_comment: $(
            "#_wpnonce_new_activity_comment" + "_" + activity_id
          ).val(),
          comment_id: item_id,
          form_id: activity_id,
        ...
        if ($("#_bp_as_nonce_" + activity_id).val()) {
          comment_data["_bp_as_nonce_" + activity_id] = $(
            "#_bp_as_nonce_" + activity_id
          ).val();
        }
        ...
        parent.ajax(comment_data, "activity").done(function (response) {
          ...
          if (false === response.success) {
            ...
            form.fadeOut(200, function () {
              ...
              activity_comments
                .children("ul")
                .append($(the_comment).hide().fadeIn(200));
            ...
            comment_count =
              Number(
                $(activity_item).find("a span.comment-count").html() || 0
              ) + 1;
