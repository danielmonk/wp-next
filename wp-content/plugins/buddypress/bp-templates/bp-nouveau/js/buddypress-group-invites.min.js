  ...
  (function (e, i) {
    "undefined" != typeof BP_Nouveau &&
      ...
      (bp.Nouveau.GroupInvites = {
          ...
          (this.nav = new bp.Views.invitesNav({ collection: this.navItems })),
            _.each(
              ...
              function (i, t) {
                _.isObject(i) &&
                  ((e = 0),
                  0 === t && ((this.scope = i.id), (e = 1)),
                  this.navItems.add({
                    id: i.id,
                    name: i.caption,
                    href: i.href || "#members-list",
                    active: e,
                    hide: _.isUndefined(i.hide) ? 0 : i.hide,
                  }));
              },
              this
            ),
        ...
        setupLoops: function (e) {
          ...
          (e = e || this.scope),
            ...
            e !== this.scope &&
              this.displayFeedback(BP_Nouveau.group_invites.loading, "loading"),
            (this.scope = e),
            (i = new bp.Views.inviteUsers({
              collection: this.users,
              scope: e,
            })),
            this.views.add({ id: "users", view: i }),
            i.inject(".bp-invites-content"),
        ...
        displayFilters: function (e) {
          ...
          (this.filters = new Backbone.Model({
            ...
            (i = new bp.Views.inviteFilters({ model: this.filters, users: e })),
            this.views.add({ id: "filters", view: i }),
            i.inject(".bp-invites-filters");
        },
        ...
        displayFeedback: function (e, i) {
          var t;
          this.removeFeedback(),
            e &&
              ((t = new bp.Views.Feedback({ value: e, type: i || "info" })),
              this.views.add({ id: "feedback", view: t }),
              t.inject(".bp-invites-feedback"));
        },
        addInvite: function (e) {
          if (!0 === e.get("selected")) this.invites.add(e);
          else {
            var i = this.invites.get(e.get("id"));
            !0 === i.get("selected") && this.invites.remove(i);
          }
        },
        manageInvite: function (e) {
          var i = this.users.get(e.get("id"));
          i && i.set("selected", !1),
            this.invites.remove(e),
            this.invites.length || this.invites.reset();
        },
        invitesNav: function () {
        ...
        loadConfirmView: function () {
          ...
          var e = new bp.Views.invitesEditor({ collection: this.invites });
          this.views.add({ id: "invites", view: e }),
            e.inject(".bp-invites-content");
        },
      }),
      (bp.Models.User = Backbone.Model.extend({
        defaults: { id: 0, avatar: "", name: "", selected: !1 },
      })),
      (bp.Collections.Users = Backbone.Collection.extend({
        ...
        sync: function (e, i, t) {
          return (
            ...
            (t.context = this),
            (t.data = t.data || {}),
            (t.data.nonce = BP_Nouveau.nonces.groups),
            this.options.group_id && (t.data.group_id = this.options.group_id),
            "read" === e
        ...
        parse: function (e) {
          return (
            _.isArray(e.users) || (e.users = [e.users]),
            _.each(e.users, function (i, t) {
              _.isNull(i) ||
                ((e.users[t].id = i.id),
                (e.users[t].avatar = i.avatar),
                (e.users[t].name = i.name));
            }),
      ...
      (bp.Nouveau.GroupInvites.View = bp.Backbone.View.extend({
        inject: function (e) {
          this.render(), i(e).html(this.el), this.views.ready();
        },
        prepare: function () {
      ...
      (bp.Views.invitesNav = bp.Nouveau.GroupInvites.View.extend({
        tagName: "ul",
        className: "subnav",
        events: { "click .bp-invites-nav-item": "toggleView" },
        initialize: function () {
        ...
        outputNav: function (e) {
          1 !== e.get("hide") &&
            this.views.add(new bp.Views.invitesNavItem({ model: e }));
        },
        showHideNavItem: function (e) {
          var i = null;
          _.each(this.views._views[""], function (t) {
            1 === t.model.get("hide") && t.remove(),
              e.get("id") === t.model.get("id") && (i = !0);
          }),
            _.isBoolean(i) ||
              (e.set("invites_count", bp.Nouveau.GroupInvites.invites.length),
              this.outputNav(e));
        },
              ...
              e.id === s
                ? (e.set("active", 1),
                  "invites" === e.id
                    ...
                    : this.trigger("bp-invites:loops", e.id))
                : 1 !== e.get("hide") && e.set("active", 0);
            },
            this
          );
        },
        confirmQuit: function () {
          if (
            ...
            return i('[data-nav="invites"]').focus(), !1;
        },
      })),
      (bp.Views.invitesNavItem = bp.Nouveau.GroupInvites.View.extend({
        ...
        initialize: function () {
          1 === this.model.get("active") && (this.el.className += " current"),
            "invites" === this.model.get("id") &&
              (this.el.className += " dynamic"),
            "invited" === this.model.get("id") &&
            ...
            bp.Nouveau.GroupInvites.invites.on(
              "remove",
              this.updateCount,
              this
            );
        ...
        updateCount: function (e, t) {
          if ("invites" === this.model.get("id")) {
            var s = _.isUndefined(t)
              ? this.model.get("invites_count")
              : t.models.length;
            i(this.el).find("span").length
              ? i(this.el).find("span").html(s)
              : i(this.el)
                  .find("a")
                  .append(i('<span class="count"></span>').html(s));
          }
        },
        toggleClass: function (e) {
          0 === e.get("active")
            ? i(this.el).removeClass("current")
            : i(this.el).addClass("current");
        },
      })),
      ...
      (bp.Views.inviteFilters = bp.Nouveau.GroupInvites.View.extend({
        ...
        addPaginatation: function (e) {
          _.each(this.views._views[""], function (e) {
            ...
            1 !== e.options.total_page &&
              this.views.add(
                new bp.Views.Pagination({
                  model: new Backbone.Model(e.options),
                })
              );
        },
        ...
        usersFilterError: function (e, i) {
          bp.Nouveau.GroupInvites.displayFeedback(i.feedback, "error");
        },
          ...
          e.preventDefault(),
            i(e.target).val()
              ? i(e.target)
                  ...
                  .find("[type=submit]")
                  .addClass("bp-show")
              : i(e.target).closest("form").submit();
        },
        setSearchTerms: function (e) {
          e.preventDefault(),
            this.model.set({
              search_terms: i(e.target).find("input[type=search]").val() || "",
              page: 1,
            });
        },
        nextPage: function (e) {
      ...
      (bp.Views.inviteUsers = bp.Nouveau.GroupInvites.View.extend({
        ...
        usersFetched: function (e, i) {
          bp.Nouveau.GroupInvites.displayFeedback(i.feedback, "help");
        },
        usersFetchError: function (e, i) {
          var t = i.type || "help";
          bp.Nouveau.GroupInvites.displayFeedback(i.feedback, t);
        },
        ...
        addUser: function (e) {
          this.views.add(new bp.Views.inviteUser({ model: e }));
        },
      })),
      (bp.Views.inviteUser = bp.Nouveau.GroupInvites.View.extend({
        ...
        initialize: function () {
          bp.Nouveau.GroupInvites.invites.get(this.model.get("id")) &&
            this.model.set("selected", !0, { silent: !0 });
        },
        render: function () {
          this.model.get("selected")
            ? (this.el.className = "selected")
            : (this.el.className = ""),
            bp.Nouveau.GroupInvites.View.prototype.render.apply(
              this,
              arguments
            );
        },
        ...
        inviteRemoved: function (e) {
          ...
          i.length &&
            ...
            bp.Nouveau.GroupInvites.removeFeedback(),
            !1 === e.has_invites &&
              (bp.Nouveau.GroupInvites.displayFeedback(e.feedback, "success"),
              bp.Nouveau.GroupInvites.navItems
                .get("invited")
                .set({ active: 0, hide: 1 })));
        },
        uninviteError: function (e) {
          bp.Nouveau.GroupInvites.displayFeedback(e.feedback, "error");
        },
      })),
      (bp.Views.invitesEditor = bp.Nouveau.GroupInvites.View.extend({
          ...
          e.preventDefault(),
            ...
            this.collection.sync(
              ...
              _.pluck(this.collection.models, "id"),
              {
                ...
                data: { message: i(this.el).find("textarea").val() },
              }
            );
        },
        invitesSent: function (e) {
          this.collection.reset(),
            bp.Nouveau.GroupInvites.displayFeedback(e.feedback, "success"),
        ...
        invitesError: function (e) {
          var t = e.type || "help";
          i(this.el).removeClass("bp-hide"),
            bp.Nouveau.GroupInvites.displayFeedback(e.feedback, t),
            _.isUndefined(e.users) ||
              (1 ===
                bp.Nouveau.GroupInvites.navItems.get("invited").get("hide") &&
                e.users.length < this.collection.length &&
                bp.Nouveau.GroupInvites.navItems
                  .get("invited")
                  .set({ active: 0, hide: 0 }),
              _.each(
        ...
        clearForm: function (e) {
          e.preventDefault(), this.collection.reset();
        },
        cleanViews: function () {
          _.each(this.views._views[""], function (e) {
            e.remove();
        ...
        initialize: function () {
          this.cleanContent(),
            _.each(
              this.collection.models,
              function (e) {
                this.views.add(new bp.Views.selectedUser({ model: e }));
              },
              this
            );
        },
      ...
      (bp.Views.selectedUser = bp.Nouveau.GroupInvites.View.extend({
        ...
        initialize: function () {
          this.model.on("change:selected", this.removeView, this),
            this.model.get("uninviteTooltip") ||
              this.model.set(
                "uninviteTooltip",
        ...
        removeSelection: function (e) {
          e.preventDefault(), this.model.set("selected", !1);
        },
        removeView: function (e) {
          !1 === e.get("selected") && this.remove();
        },
      })),
