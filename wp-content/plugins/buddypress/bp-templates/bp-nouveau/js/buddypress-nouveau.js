...
(function (exports, $) {
  ...
  bp.Nouveau = {
    ...
    ajax: function (post_data, object) {
      ...
      var postData = $.extend(
        {},
        bp.Nouveau.getStorage("bp-" + object),
    ...
    inject: function (selector, content, method) {
      ...
      if (
        "undefined" !== typeof bp_mentions ||
        "undefined" !== typeof bp.mentions
      ) {
    ...
    objectRequest: function (data) {
      ...
      if (data.search_terms) {
        data.search_terms = data.search_terms
          .replace(/</g, "&lt;")
          .replace(/>/g, "&gt;");
      }
      ...
      if (
        "friends" === data.object ||
        "friend_requests" === data.object ||
        "group_members" === data.object
      ) {
      ...
      return this.ajax(postdata, data.object).done(function (response) {
        ...
        $(
          self.objectNavParent + ' [data-bp-scope="' + data.scope + '"]'
        ).removeClass("loading");
        ...
        if ("reset" !== data.method) {
          ...
          $(data.target).trigger(
            "bp_ajax_" + data.method,
            $.extend(data, { response: response.data })
          );
        } else {
    ...
    initObjects: function () {
      ...
      $.each(this.objects, function (o, object) {
        ...
        if ($('#buddypress [data-bp-filter="' + object + '"]').length) {
          if (
            "-1" !== $('#buddypress [data-bp-filter="' + object + '"]').val() &&
            "0" !== $('#buddypress [data-bp-filter="' + object + '"]').val()
          ) {
        ...
        if (
          $(this.objectNavParent + ' [data-bp-object="' + object + '"]').length
        ) {
        ...
        if (null !== self.querystring) {
          ...
          if (search_terms) {
            ...
            $(
              '#buddypress [data-bp-search="' + object + '"] input[type=search]'
            ).val(search_terms);
          }
        }
    ...
    addListeners: function () {
      ...
      // Refreshing.
      $(this.objectNavParent + " .bp-navs").on(
        "click",
        "a",
        this,
        this.scopeQuery
      ...
      // Searching.
      $("#buddypress [data-bp-search]").on(
        "submit",
        "form",
        this,
        this.searchQuery
      );
      $("#buddypress [data-bp-search] form").on(
        "search",
        "input[type=search]",
        this.resetSearch
      );
      ...
      // Buttons.
      $("#buddypress [data-bp-list], #buddypress #item-header").on(
        "click",
        "[data-bp-btn-action]",
        this,
        this.buttonAction
    ...
    toggleDisabledInput: function (event) {
      var target = $(event.currentTarget),
        disabledControl = "#" + target.data("bp-disable-input"),
    ...
    scopeQuery: function (event) {
      ...
      if (
        target.hasClass("no-ajax") ||
        $(event.currentTarget).hasClass("no-ajax") ||
        !target.attr("data-bp-scope")
      ) {
        return event;
      }
    ...
    filterQuery: function (event) {
      ...
      if ($(self.objectNavParent + " [data-bp-object].selected").length) {
        scope = $(self.objectNavParent + " [data-bp-object].selected").data(
          "bp-scope"
        );
      }
    ...
    searchQuery: function (event) {
      ...
      if (
        $(event.delegateTarget).hasClass("no-ajax") ||
        undefined === $(event.delegateTarget).data("bp-search")
      ) {
        return event;
      }
      ...
      filter = $("#buddypress")
        ...
        .first()
        .val();
      search_terms = $(event.delegateTarget)
      ...
      if ($(self.objectNavParent + " [data-bp-object]").length) {
        scope = $(
          self.objectNavParent + ' [data-bp-object="' + object + '"].selected'
        ).data("bp-scope");
      }
        ...
        .done(function (response) {
          if (false === response.success) {
            ...
            if ("groups" === object) {
              ...
              if (
                undefined !== response.data.is_group &&
            ...
            // Update count.
            if (
              $(self.objectNavParent + ' [data-bp-scope="personal"]').length
            ) {
              var personal_count =
                Number(
                  $(
                    self.objectNavParent + ' [data-bp-scope="personal"] span'
                  ).html()
                ) || 0;
              ...
              $(self.objectNavParent + ' [data-bp-scope="personal"] span').html(
                personal_count
              );
            }
    ...
    paginateAction: function (event) {
      ...
      pagArg =
        navLink.closest("[data-bp-pagination]").data("bp-pagination") || null;
