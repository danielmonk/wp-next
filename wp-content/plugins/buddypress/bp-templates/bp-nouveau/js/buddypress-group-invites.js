...
(function (exports, $) {
  ...
  bp.Nouveau.GroupInvites = {
    ...
    setupLoops: function (scope) {
      ...
      users = new bp.Views.inviteUsers({
    ...
    displayFilters: function (collection) {
      ...
      filters_view = new bp.Views.inviteFilters({
        model: this.filters,
        users: collection,
      });
  ...
  bp.Views.invitesNav = bp.Nouveau.GroupInvites.View.extend({
    ...
    confirmQuit: function () {
      if (
        bp.Nouveau.GroupInvites.invites &&
        bp.Nouveau.GroupInvites.invites.length
      ) {
  ...
  bp.Views.invitesNavItem = bp.Nouveau.GroupInvites.View.extend({
    ...
    updateCount: function (user, invite) {
      ...
      var span_count = _.isUndefined(invite)
        ? this.model.get("invites_count")
        : invite.models.length;
      ...
      if ($(this.el).find("span").length) {
        ...
        $(this.el)
          .find("a")
          .append($('<span class="count"></span>').html(span_count));
      }
    },
  ...
  bp.Views.inviteFilters = bp.Nouveau.GroupInvites.View.extend({
    ...
    filterUsers: function () {
      bp.Nouveau.GroupInvites.displayFeedback(
        BP_Nouveau.group_invites.loading,
        "loading"
      );
  ...
  bp.Views.inviteUser = bp.Nouveau.GroupInvites.View.extend({
    ...
    inviteRemoved: function (response) {
      ...
      if (false === response.has_invites) {
        ...
        bp.Nouveau.GroupInvites.navItems
          .get("invited")
          .set({ active: 0, hide: 1 });
      }
    },
  ...
  bp.Views.invitesEditor = bp.Nouveau.GroupInvites.View.extend({
    ...
    initialize: function () {
      this.views.add(
        new bp.Views.selectedUsers({ collection: this.collection })
      );
    ...
    sendInvites: function (event) {
      ...
      bp.Nouveau.GroupInvites.displayFeedback(
        BP_Nouveau.group_invites.invites_sending,
        "info"
      );
    ...
    invitesSent: function (response) {
      ...
      if (
        1 === bp.Nouveau.GroupInvites.navItems.get("invited").get("hide") &&
        !BP_Nouveau.group_invites.is_group_create
      ) {
        bp.Nouveau.GroupInvites.navItems
          .get("invited")
          .set({ active: 0, hide: 0 });
    ...
    invitesError: function (response) {
      ...
      if (!_.isUndefined(response.users)) {
        ...
        if (
          1 === bp.Nouveau.GroupInvites.navItems.get("invited").get("hide") &&
          response.users.length < this.collection.length
        ) {
          bp.Nouveau.GroupInvites.navItems
            .get("invited")
            .set({ active: 0, hide: 0 });
    ...
    cleanViews: function () {
      ...
      bp.Nouveau.GroupInvites.displayFeedback(
        BP_Nouveau.group_invites.invites_form_reset,
        "success"
      );
    },
  });
  ...
  bp.Views.selectedUser = bp.Nouveau.GroupInvites.View.extend({
    ...
    initialize: function () {
      ...
      if (!this.model.get("uninviteTooltip")) {
        this.model.set(
          "uninviteTooltip",
          BP_Nouveau.group_invites.removeUserInvite.replace(
            "%s",
            this.model.get("name")
          ),
          { silent: true }
