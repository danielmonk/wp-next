...
(function (wp, $) {
  ...
  wp.customize.bind("ready", function () {
      ...
      $(groupFrontPage.selector).on(
        ...
        function (event) {
          var checked = $(event.currentTarget).prop("checked"),
            controller = $(event.delegateTarget).prop("id");
    ...
    if ("undefined" !== typeof userFrontPage) {
      $(userFrontPage.selector).on(
        ...
        function (event) {
          ...
          _.each(
            wp.customize.section("bp_nouveau_user_front_page").controls(),
            function (control) {
              if (control.selector !== "#" + controller) {
                if (true === checked) {
                  $(control.selector).show();
    ...
    $(
      "ul#customize-control-group_nav_order, ul#customize-control-user_nav_order"
    )
      .sortable({
        cursor: "move",
        axis: "y",
        ...
        update: function () {
          ...
          $(this)
            .find("[data-bp-nav]")
            .each(function (s, slug) {
              order.push($(slug).data("bp-nav"));
            });

          if (order.length) {
            $("#bp_item_" + $(this).data("bp-type"))
              .val(order.join())
              .trigger("change");
          }
        },
