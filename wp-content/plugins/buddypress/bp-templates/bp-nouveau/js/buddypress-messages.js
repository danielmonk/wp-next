...
(function (exports, $) {
  ...
  bp.Nouveau.Messages = {
    ...
    setupNav: function () {
      ...
      $("#subnav a").on("click", function (event) {
        ...
        if ("compose" === view_id) {
          ...
          if (
            self.box !== view_id ||
            !_.isUndefined(self.views.get("compose"))
          ) {
            self.clearViews();
    ...
    threadsView: function () {
      ...
      // Create the loop view.
      var threads_list = new bp.Views.userThreads({
    ...
    displayFilters: function (collection) {
      ...
      filters_view = new bp.Views.messageFilters({
        model: this.filters,
        threads: collection,
      });
    ...
    singleView: function (thread) {
      ...
      var single_thread = new bp.Views.userMessages({
  ...
  bp.Models.Thread = Backbone.Model.extend({
    ...
    updateReadState: function (options) {
      options = options || {};
      options.data = _.extend(_.pick(this.attributes, ["id", "message_id"]), {
  ...
  bp.Collections.Threads = Backbone.Collection.extend({
    ...
    parse: function (resp) {
      ...
      if (!_.isUndefined(resp.extraContent)) {
        _.extend(
          this.options,
          _.pick(resp.extraContent, ["beforeLoop", "afterLoop"])
        );
      }
    ...
    resetFields: function (model) {
      ...
      _.each(model.previousAttributes(), function (value, input) {
        if ("message_content" === input) {
          // tinyMce.
          if (
            undefined !== tinyMCE.activeEditor &&
      ...
      $(this.el).trigger(
        "message:reset",
        _.pick(model.previousAttributes(), "meta")
      );
    },
    ...
    sendMessage: function (event) {
      ...
      _.each(
        ...
        function (pair) {
          ...
          if (
            -1 ===
            _.indexOf(["send_to", "subject", "message_content"], pair.name)
          ) {
            ...
            if ("send_to" === pair.name) {
              var usernames = pair.value.match(
                /(^|[^@\w\-])@([a-zA-Z0-9_\-]{1,50})\b/g
              );
      ...
      this.model
        ...
        .fail(function (response) {
          if (response.feedback) {
            bp.Nouveau.Messages.displayFeedback(
              response.feedback,
              response.type
            );
          }
  ...
  bp.Views.userThreads = bp.Nouveau.Messages.View.extend({
    ...
    threadsFetched: function () {
      ...
      if (this.collection.options.afterLoop) {
        this.views.add(
          new bp.Views.Hook({
            extraContent: this.collection.options.afterLoop,
            className: "after-messages-loop",
          }),
          { at: 1 }
        );
      }
      ...
      if (this.collection.options.beforeLoop) {
        this.views.add(
          new bp.Views.Hook({
            extraContent: this.collection.options.beforeLoop,
            className: "before-messages-loop",
          }),
          { at: 0 }
        );
      }
    ...
    addThread: function (thread) {
      ...
      this.views.add(
        "#message-threads",
        new bp.Views.userThread({ model: thread })
      );
    },
    ...
    changePreview: function (event) {
      ...
      if (target.closest(".thread-item").hasClass("selected")) {
        ...
        this.setActiveThread(
          target.closest(".thread-content").data("thread-id")
        );
  ...
  bp.Views.userThread = bp.Nouveau.Messages.View.extend({
    ...
    initialize: function () {
      ...
      if ("sentbox" === bp.Nouveau.Messages.box) {
        var recipientsCount = this.model.get("recipients").length,
          toOthers = "";

        if (2 === recipientsCount) {
          toOthers = BP_Nouveau.messages.toOthers.one;
        } else if (2 < recipientsCount) {
          toOthers = BP_Nouveau.messages.toOthers.more.replace(
            "%d",
            Number(recipientsCount - 1)
          );
        }
    ...
    bulkSelect: function (model) {
      if ($("#bp-message-thread-" + model.get("id")).length) {
        $("#bp-message-thread-" + model.get("id")).prop(
          "checked",
          model.get("checked")
        );
      }
    ...
    singleSelect: function (event) {
      ...
      if (hasChecked) {
        $("#user-messages-bulk-actions")
          .closest(".bulk-actions-wrap")
          .removeClass("bp-hide");

        // Inform the user about how to use the bulk actions.
        bp.Nouveau.Messages.displayFeedback(
          BP_Nouveau.messages.howtoBulk,
          "info"
        );
      } else {
        $("#user-messages-bulk-actions")
  ...
  bp.Views.previewThread = bp.Nouveau.Messages.View.extend({
    ...
    doAction: function (event) {
      ...
      if ("view" === action) {
        bp.Nouveau.Messages.router.navigate("view/" + mid + "/", {
          trigger: true,
        });
      ...
      this.collection
        ...
        .done(function (response) {
          ...
          if (
            "delete" === action ||
            ("starred" === self.collection.options.box && "unstar" === action)
          ) {
            ...
            self.collection.fetch({
              data: _.pick(self.collection.options, [
                "box",
                "search_terms",
                "page",
              ]),
  ...
  bp.Views.messageFilters = bp.Nouveau.Messages.View.extend({
    ...
    addPaginatation: function (collection) {
      ...
      this.views.add(
        new bp.Views.Pagination({
          model: new Backbone.Model(collection.options),
        })
      );

      this.views.add(
        ".user-messages-bulk-actions",
        new bp.Views.BulkActions({
    ...
    filterThreads: function () {
      ...
      _.extend(
        this.options.threads.options,
        _.pick(this.model.attributes, ["box", "search_terms"])
      );
  ...
  bp.Views.userMessagesHeader = bp.Nouveau.Messages.View.extend({
    ...
    doAction: function (event) {
      ...
      if ("star" === action || "unstar" === action) {
        ...
        $(event.currentTarget)
          .parent()
          .find('[data-bp-action="' + opposite[action] + '"]')
          .removeClass("bp-hide");
      }
      ...
      bp.Nouveau.Messages.threads
        .doAction(action, this.model.get("id"), options)
        .done(function (response) {
          // Remove all views
          if ("delete" === action) {
  ...
  bp.Views.userMessages = bp.Nouveau.Messages.View.extend({
    ...
    messagesFetched: function (collection, response) {
      ...
      this.views.add(
        ...
        new bp.Views.userMessagesHeader({ model: this.options.thread })
      );
    },
    ...
    addMessage: function (message) {
      this.views.add(
        ...
        new bp.Views.userMessagesEntry({ model: message })
      );
    },
    ...
    sendReply: function (event) {
      ...
      this.collection.sync(
        "create",
        _.pick(this.reply.attributes, ["thread_id", "content"]),
        {
