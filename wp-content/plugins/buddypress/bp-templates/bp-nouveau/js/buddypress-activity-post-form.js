...
(function (exports, $) {
  ...
  bp.Views.FormAvatar = bp.View.extend({
    ...
    initialize: function () {
      this.model = new Backbone.Model(
        _.pick(BP_Nouveau.activity.params, [
          "user_id",
          "avatar_url",
          "avatar_width",
          "avatar_height",
  ...
  bp.Views.FormTarget = bp.View.extend({
    ...
    initialize: function () {
      var select = new bp.Views.WhatsNewPostIn({
        filters: BP_Nouveau.activity.params.objects,
      });
      this.views.add(select);

      select.model.on("change", this.attachAutocomplete, this);
      bp.Nouveau.Activity.postForm.ActivityObjects.on(
        "change:selected",
        this.postIn,
        this
      );
    ...
    postIn: function (model) {
      if (_.isUndefined(model.get("id"))) {
        ...
        this.attachAutocomplete(
          new Backbone.Model({ selected: this.model.get("object") })
        );
        return;
      }
  ...
  bp.Views.FormButtons = bp.View.extend({
    ...
    addItemView: function (button) {
      this.views.add(
        "#whats-new-buttons",
        new bp.Views.FormButton({ model: button })
      );
    },
  ...
  bp.Views.PostForm = bp.View.extend({
    ...
    initialize: function () {
      this.model = new bp.Models.Activity(
        _.pick(BP_Nouveau.activity.params, ["user_id", "item_id", "object"])
      );
    ...
    displayFull: function (event) {
      var numStaticViews =
        true === this.options.backcompat.before_post_form ? 3 : 2;
      ...
      if (!_.isUndefined(BP_Nouveau.activity.params.buttons)) {
        // Global
        bp.Nouveau.Activity.postForm.buttons.set(
          BP_Nouveau.activity.params.buttons
        );
        this.views.add(
          "#whats-new-options",
      ...
      if (
        ...
        1 < _.keys(BP_Nouveau.activity.params.objects).length
      ) {
        this.views.add(
          "#whats-new-options",
          new bp.Views.FormTarget({ model: this.model })
        );
      }

      this.views.add(
        "#whats-new-options",
        new bp.Views.FormSubmit({ model: this.model })
      );
    },

    resetForm: function () {
      var self = this,
        indexStaticViews = self.options.backcompat.before_post_form ? 2 : 1;
    ...
    postUpdate: function (event) {
      ...
      _.each(this.$el.serializeArray(), function (pair) {
        ...
        if ("whats-new" === pair.name) {
          ...
          -1 ===
          _.indexOf(["aw-whats-new-submit", "whats-new-post-in"], pair.name)
        ) {
      ...
      bp.ajax
        .post("post_update", _.extend(data, this.model.attributes))
        .done(function (response) {
          var store = bp.Nouveau.getStorage("bp-activity"),
            searchTerms = $(
              '[data-bp-search="activity"] input[type="search"]'
            ).val(),
            matches = {},
            toPrepend = false;
          ...
          if (!searchTerms || matches) {
            toPrepend =
              !store.filter ||
              0 === parseInt(store.filter, 10) ||
              "activity_update" === store.filter;
          }
          ...
          if (toPrepend && response.is_directory) {
            toPrepend =
              ("all" === store.scope &&
                ("user" === self.model.get("object") ||
                  false === response.is_private)) ||
              self.model.get("object") + "s" === store.scope;
          }
          ...
          if (!toPrepend) {
            ...
            if (!$("#activity-stream ul.activity-list").length) {
              $("#activity-stream").html(
                $("<ul></ul>").addClass("activity-list item-list bp-list")
              );
            }
