...
<?php if (!bp_group_has_requested_membership()): ?>
	<h2 class="bp-screen-reader-text"><?php esc_html_e(
	...
	<p>
      ...
      sprintf(
          ...
          __(
              'You are requesting to become a member of the group "%s".',
              'buddypress'
          ),
          bp_get_group_name()
      )
	...
	<form action="<?php bp_group_form_action(
     'request-membership'
 ); ?>" method="post" name="request-membership-form" id="request-membership-form" class="standard-form">
		<label for="group-request-membership-comments"><?php esc_html_e(
      'Comments (optional)',
      'buddypress'
  ); ?></label>
  ...
  do_action('bp_group_request_membership_content'); ?>
		...
		<p><input type="submit" name="group-request-send" id="group-request-send" value="<?php echo esc_attr_x(
      'Send Request',
      'button',
      'buddypress'
  ); ?>" />
