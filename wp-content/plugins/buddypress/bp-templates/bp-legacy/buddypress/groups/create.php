...
?>
			<?php if (bp_is_group_creation_step('group-details')): ?>
				...
				<h2 class="bp-screen-reader-text"><?php /* translators: accessibility text */
    _e('Group Details', 'buddypress'); ?></h2>
    ...
    do_action('bp_before_group_details_creation_step'); ?>
				...
				<div>
					<label for="group-name"><?php _e(
         'Group Name (required)',
         'buddypress'
     ); ?></label>
					<input type="text" name="group-name" id="group-name" aria-required="true" value="<?php echo esc_attr(
         bp_get_new_group_name()
     ); ?>" />
				</div>

				<div>
					<label for="group-desc"><?php _e(
...
?>
			<?php if (bp_is_group_creation_step('group-settings')): ?>
				...
				<h2 class="bp-screen-reader-text"><?php /* translators: accessibility text */
    _e('Group Settings', 'buddypress'); ?></h2>
    ...
    do_action('bp_before_group_settings_creation_step'); ?>
				...
				<fieldset class="group-create-privacy">
					...
					<div class="radio">
						...
						<label for="group-status-public"><input type="radio" name="group-status" id="group-status-public" value="public"<?php if (
          'public' == bp_get_new_group_status() ||
          !bp_get_new_group_status()
      ) { ?> checked="checked"<?php } ?> aria-describedby="public-group-description" /> <?php _e(
     ...
     'buddypress'
           ...
           'buddypress'
						...
						<label for="group-status-private"><input type="radio" name="group-status" id="group-status-private" value="private"<?php if (
          'private' == bp_get_new_group_status()
      ) { ?> checked="checked"<?php } ?> aria-describedby="private-group-description" /> <?php _e(
     ...
     'buddypress'
           ...
           'buddypress'
						...
						<label for="group-status-hidden"><input type="radio" name="group-status" id="group-status-hidden" value="hidden"<?php if (
          'hidden' == bp_get_new_group_status()
      ) { ?> checked="checked"<?php } ?> aria-describedby="hidden-group-description" /> <?php _e(
        ...
        $group_types = bp_groups_get_group_types(
            ...
            'objects'
					...
					<fieldset class="group-create-types">
						...
						<p><?php _e(
          'Select the types this group should be a part of.',
          'buddypress'
      ); ?></p>

						<?php foreach ($group_types as $type): ?>
							<div class="checkbox">
								<label for="<?php printf(
            'group-type-%s',
            $type->name
        ); ?>"><input type="checkbox" name="group-types[]" id="<?php printf(
    'group-type-%s',
    $type->name
); ?>" value="<?php echo esc_attr($type->name); ?>" <?php checked(
    true,
    !empty($type->create_screen_checked)
); ?> /> <?php echo esc_html($type->labels['name']); ?>
									<?php if (!empty($type->description)) {
             /* translators: Group type description shown when creating a group. */
             printf(
                 __('&ndash; %s', 'buddypress'),
                 '<span class="bp-group-type-desc">' .
         ...
         'buddypress'
					...
					<div class="radio">
						...
						<label for="group-invite-status-members"><input type="radio" name="group-invite-status" id="group-invite-status-members" value="members"<?php bp_group_show_invite_status_setting(
          'members'
      ); ?> /> <?php _e('All group members', 'buddypress'); ?></label>

						<label for="group-invite-status-mods"><input type="radio" name="group-invite-status" id="group-invite-status-mods" value="mods"<?php bp_group_show_invite_status_setting(
          'mods'
      ); ?> /> <?php _e('Group admins and mods only', 'buddypress'); ?></label>

						<label for="group-invite-status-admins"><input type="radio" name="group-invite-status" id="group-invite-status-admins" value="admins"<?php bp_group_show_invite_status_setting(
          'admins'
...
?>
    ...
    do_action('bp_before_group_avatar_creation_step'); ?>
          ...
          'buddypress'
						...
						<p>
							<label for="file" class="bp-screen-reader-text"><?php /* translators: accessibility text */
       _e('Select an image', 'buddypress'); ?></label>
							<input type="file" name="file" id="file" />
							<input type="submit" name="upload" id="upload" value="<?php esc_attr_e(
           'Upload Image',
           'buddypress'
       ); ?>" />
     ...
     bp_avatar_get_templates(); ?>
				...
				<?php if ('crop-image' == bp_get_avatar_admin_step()): ?>
					...
					<img src="<?php bp_avatar_to_crop(); ?>" id="avatar-to-crop" class="avatar" alt="<?php esc_attr_e(
    'Profile photo to crop',
    'buddypress'
); ?>" />

					<div id="avatar-crop-pane">
						<img src="<?php bp_avatar_to_crop(); ?>" id="avatar-crop-preview" class="avatar" alt="<?php esc_attr_e(
    'Profile photo preview',
    'buddypress'
); ?>" />
					</div>

					<input type="submit" name="avatar-crop-submit" id="avatar-crop-submit" value="<?php esc_attr_e(
         'Crop Image',
         'buddypress'
     ); ?>" />
    ...
    do_action('bp_after_group_avatar_creation_step'); ?>
			...
			<?php
/* Group creation step 4: Cover image */
?>
			<?php if (bp_is_group_creation_step('group-cover-image')): ?>

				<h2 class="bp-screen-reader-text"><?php /* translators: accessibility text */
    _e('Cover Image', 'buddypress'); ?></h2>
    ...
    do_action('bp_after_group_cover_image_creation_step'); ?>
			...
			<?php
/* Group creation step 5: Invite friends to group */
?>
			<?php if (bp_is_group_creation_step('group-invites')): ?>

				<h2 class="bp-screen-reader-text"><?php /* translators: accessibility text */
    _e('Group Invites', 'buddypress'); ?></h2>
        ...
        bp_get_total_friend_count(bp_loggedin_user_id())
					...
					<div class="left-menu">
						...
						<div id="invite-list">
							...
							<?php wp_nonce_field(
           'groups_invite_uninvite_user',
           '_wpnonce_invite_uninvite_user'
       ); ?>
						</div>
        ...
        ?>
           ...
           bp_group_the_invite(); ?>
								...
								<li id="<?php bp_group_invite_item_id(); ?>">
									...
									<div class="action">
										<a class="remove" href="<?php bp_group_invite_user_remove_invite_url(); ?>" id="<?php bp_group_invite_item_id(); ?>"><?php _e(
    'Remove Invite',
    'buddypress'
); ?></a>
									</div>
