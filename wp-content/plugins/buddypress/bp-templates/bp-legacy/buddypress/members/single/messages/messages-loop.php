...
<?php if (bp_has_message_threads(bp_ajax_querystring('messages'))): ?>
 ...
 _e('Starred messages', 'buddypress'); ?></h2>
 ...
 do_action('bp_before_member_messages_threads'); ?>
	...
	<form action="<?php echo bp_displayed_user_domain() .
     bp_get_messages_slug() .
     '/' .
     bp_current_action(); ?>/bulk-manage/" method="post" id="messages-bulk-management">

		<table id="message-threads" class="messages-notices">

			<thead>
				<tr>
					<th scope="col" class="thread-checkbox bulk-select-all"><input id="select-all-messages" type="checkbox"><label class="bp-screen-reader-text" for="select-all-messages"><?php /* translators: accessibility text */
     _e('Select all', 'buddypress'); ?></label></th>
     ...
     do_action('bp_messages_inbox_list_header'); ?>
					...
					<?php if (bp_is_active('messages', 'star')): ?>
						<th scope="col" class="thread-star"><span class="message-action-star"><span class="icon"></span> <span class="screen-reader-text"><?php /* translators: accessibility text */
      _e('Star', 'buddypress'); ?></span></span></th>
					<?php endif; ?>

					<th scope="col" class="thread-options"><?php _e(
         'Actions',
         'buddypress'
     ); ?></th>
				</tr>
...
if (bp_message_thread_has_unread()): ?> unread<?php else: ?> read<?php endif;
?>">
						<td class="bulk-select-check">
							<label for="bp-message-thread-<?php bp_message_thread_id(); ?>"><input type="checkbox" name="message_ids[]" id="bp-message-thread-<?php bp_message_thread_id(); ?>" class="message-check" value="<?php bp_message_thread_id(); ?>" /><span class="bp-screen-reader-text"><?php /* translators: accessibility text */
_e('Select this message', 'buddypress'); ?></span></label>
						</td>

						<?php if ('sentbox' != bp_current_action()): ?>
							<td class="thread-from">
								<?php bp_message_thread_avatar(['width' => 25, 'height' => 25]); ?>
								<span class="from"><?php _e(
            'From:',
            'buddypress'
        ); ?></span> <?php bp_message_thread_from(); ?>
						...
						<?php else: ?>
							<td class="thread-from">
								<?php bp_message_thread_avatar(['width' => 25, 'height' => 25]); ?>
								<span class="to"><?php _e(
            'To:',
            'buddypress'
        ); ?></span> <?php bp_message_thread_to(); ?>
								<?php bp_message_thread_total_and_unread_count(); ?>
								<span class="activity"><?php bp_message_thread_last_post_date(); ?></span>
							</td>
						<?php endif; ?>
       ...
       ); ?>" class="bp-tooltip" data-bp-tooltip="<?php esc_attr_e(
    "View Message",
    'buddypress'
); ?>" aria-label="<?php esc_attr_e(
    "View Message",
    'buddypress'
); ?>"><?php bp_message_thread_subject(); ?></a></p>
            ...
            'thread_id' => bp_get_message_thread_id(),
						...
						<td class="thread-options">
							<?php if (bp_message_thread_has_unread()): ?>
								<a class="read" href="<?php bp_the_message_thread_mark_read_url(
            bp_displayed_user_id()
        ); ?>"><?php _e('Read', 'buddypress'); ?></a>
							<?php else: ?>
								<a class="unread" href="<?php bp_the_message_thread_mark_unread_url(
            bp_displayed_user_id()
        ); ?>"><?php _e('Unread', 'buddypress'); ?></a>
							<?php endif; ?>
							 |
							<a class="delete" href="<?php bp_message_thread_delete_link(
           bp_displayed_user_id()
       ); ?>"><?php _e('Delete', 'buddypress'); ?></a>
