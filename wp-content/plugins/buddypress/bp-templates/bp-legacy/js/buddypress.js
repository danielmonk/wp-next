...
jq(document).ready(function () {
  ...
  jq("#aw-whats-new-submit").on("click", function () {
    ...
    jq.each(form.serializeArray(), function (key, input) {
      ...
      if (
        "_" !== input.name.substr(0, 1) &&
        "whats-new" !== input.name.substr(0, 9)
      ) {
      ...
      jq.post(
        ...
        function (response) {
          ...
          if ("fav" === type) {
            ...
            jq(".item-list-tabs ul #activity-favorites span").html(
              Number(jq(".item-list-tabs ul #activity-favorites span").html()) -
                1
            );
    ...
    if (target.attr("name") === "ac_form_submit") {
      ...
      ajaxdata = {
        ...
        _wpnonce_new_activity_comment: jq(
          "#_wpnonce_new_activity_comment" + "_" + form_id[2]
        ).val(),
      ...
      jq.post(ajaxurl, ajaxdata, function (response) {
        ...
        if (response[0] + response[1] === "-1") {
          ...
          new_count =
            Number(
              jq("#activity-" + form_id[2] + " a.acomment-reply span").html()
            ) + 1;
          jq("#activity-" + form_id[2] + " a.acomment-reply span").html(
            new_count
          );
      ...
      jq.post(
        ...
        function (response) {
          ...
          if (response[0] + response[1] === "-1") {
            ...
            var children = jq("#" + comment_li.attr("id") + " ul").children(
                "li"
              ),
              child_count = 0,
              count_span,
            ...
            count_span = jq(
              "#" +
                comment_li.parents("#activity-stream > li").attr("id") +
            ...
            if (0 === new_count) {
              jq(comment_li.parents("#activity-stream > li")).removeClass(
                "has-comments"
              );
            }
          }
      ...
      jq.post(
        ...
        function (response) {
          ...
          if (response[0] + response[1] === "-1") {
            ...
            jq("#" + parent_li.attr("id") + " a.acomment-reply span").html(
              jq("#" + parent_li.attr("id") + " a.acomment-reply span").html() -
                (1 + child_count)
            );
          }
  ...
  jq("#buddypress").on("click", function (event) {
    ...
    if (
      ...
      if (jq("div.dir-search input").length) {
        ...
        if (
          !search_terms.val() &&
          bp_get_querystring(search_terms.attr("name"))
        ) {
  ...
  jq("#groups-dir-list").on("click", ".group-button a", function () {
    ...
    // AJAX request
    if (
      thelink.hasClass("leave-group") &&
      false === confirm(BP_DTheme.leave_group_confirm)
    ) {
      return false;
    }
  ...
  jq("#message-threads td.bulk-select-check :checkbox").on(
    ...
    function () {
      ...
      if (
        BP_PM_Star.star_counter > 0 &&
        parseInt(BP_PM_Star.unstar_counter, 10) === 0
      ) {
      ...
      if (
        BP_PM_Star.unstar_counter > 0 &&
        parseInt(BP_PM_Star.star_counter, 10) === 0
      ) {
  ...
  if (
    ...
    typeof BP_DTheme.pulse !== "undefined"
  ) {
  ...
  jq(document).on("heartbeat-send.buddypress", function (e, data) {
    ...
    if (
      0 === activity_last_recorded ||
      Number(first_item_recorded) > activity_last_recorded
    ) {
  ...
  jq(document).on("heartbeat-tick", function (e, data) {
    ...
    newest_activities =
      data.bp_activity_newest_activities.activities + newest_activities;
...
function bp_get_directory_preference(directoryType, pref) {
  ...
  if (BP_DTheme.store_filter_settings) {
    directoryPreferences[directoryType][pref] = jq.cookie(
...
function bp_init_activity() {
  ...
  if (undefined !== scope && jq(".activity-type-tabs").length) {
    ...
    jq("#activity-" + scope + ", .item-list-tabs li.current").addClass(
      "selected"
    );
  }
}
...
function bp_init_objects(objects) {
  jq(objects).each(function (i) {
    ...
    if (
      undefined !== filter &&
      jq("#" + objects[i] + "-order-select select").length
    ) {
      jq(
        "#" + objects[i] + '-order-select select option[value="' + filter + '"]'
      ).prop("selected", true);
    }
    ...
    if (undefined !== scope && jq("div." + objects[i]).length) {
      ...
      jq("#" + objects[i] + "-" + scope + ", #object-nav li.current").addClass(
        "selected"
      );
    }
  });
...
function bp_filter_request(
  ...
  jq("#" + object + "-" + scope + ", #object-nav li.current").addClass(
    "selected"
  );
  jq(".item-list-tabs li.selected").addClass("loading");
  ...
  var cookies = {};
  cookies["bp-" + object + "-filter"] = bp_get_directory_preference(
    object,
    "filter"
  );
  cookies["bp-" + object + "-scope"] = bp_get_directory_preference(
    object,
    "scope"
  );
