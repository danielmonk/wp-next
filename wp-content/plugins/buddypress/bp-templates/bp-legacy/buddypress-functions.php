    ...
    class BP_Legacy extends BP_Theme_Compat
    {
        ...
        protected function setup_actions()
        {
            ...
            add_filter(
                ...
                [$this, 'theme_compat_page_templates'],
                10,
                1
            );
            ...
            add_action('bp_enqueue_scripts', [$this, 'enqueue_styles']); // Enqueue theme CSS
            add_action('bp_enqueue_scripts', [$this, 'enqueue_scripts']); // Enqueue theme JS
            add_filter('bp_enqueue_scripts', [$this, 'localize_scripts']); // Enqueue theme script localization

            /** Body no-js Class **************************************************/

            add_filter('body_class', [$this, 'add_nojs_body_class'], 20, 1);
            ...
            if (
                ...
                add_action('wp_footer', [$this, 'sitewide_notices'], 9999);
            }
             ...
             */
            do_action_ref_array('bp_theme_compat_actions', [&$this]);
        }
        ...
        public function enqueue_styles()
        {
            ...
            // RTL.
            if (is_rtl()) {
                $rtl = $this->locate_asset_in_stack(
                    "buddypress-rtl{$min}.css",
                    'css'
                );

                if (isset($rtl['location'], $rtl['handle'])) {
                    $rtl['handle'] = str_replace(
                        '-css',
                        '-css-rtl',
                        $rtl['handle']
            ...
            $theme = $this->locate_asset_in_stack(
                get_template() . "{$min}.css",
                'css'
            );
            ...
            if (is_rtl()) {
                $theme_rtl = $this->locate_asset_in_stack(
                    get_template() . "-rtl{$min}.css",
                    'css'
                );
        ...
        public function enqueue_scripts()
        {
             ...
             */
            $store_filter_settings = apply_filters(
                'bp_legacy_store_filter_settings',
                is_user_logged_in()
            );
             ...
             */
            $params = apply_filters('bp_core_get_js_strings', [
                // Strings for display.
                'accepted' => __('Accepted', 'buddypress'),
                'close' => __('Close', 'buddypress'),
                'comments' => __('comments', 'buddypress'),
                'leave_group_confirm' => __(
                ...
                'show_all_comments' => __(
                    'Show all comments for this thread',
                    'buddypress'
                ),
            ...
            if (
                bp_is_register_page() ||
                (function_exists('bp_is_user_settings_general') &&
                    bp_is_user_settings_general())
            ) {
                // Locate the Register Page JS file.
                $asset = $this->locate_asset_in_stack(
                    "password-verify{$min}.js",
                    'js',
                    'bp-legacy-password-verify'
                );

                $dependencies = array_merge(bp_core_get_js_dependencies(), [
            ...
            if (bp_is_active('messages', 'star') && bp_is_user_messages()) {
                wp_localize_script($asset['handle'], 'BP_PM_Star', [
                    'strings' => [
                        ...
                        'title_unstar_thread' => __(
                            'Remove all starred messages in this thread',
                            'buddypress'
                        ),
                        'title_star_thread' => __(
                            'Star the first message in this thread',
                            'buddypress'
                        ),
                    ],
        ...
        private function locate_asset_in_stack(
            ...
            if (
                defined('BP_SOURCE_SUBDIRECTORY') &&
                BP_SOURCE_SUBDIRECTORY === 'src'
            ) {
            ...
            $subdirs = ['buddypress/' . $type, 'community/' . $type, $type];
            ...
            foreach ($locations as $location) {
                foreach ($subdirs as $subdir) {
                    if (
                        file_exists(
                            trailingslashit($location['dir']) .
                                trailingslashit($subdir) .
                                $location['file']
                        )
                    ) {
                        $retval['location'] =
                            trailingslashit($location['uri']) .
                            trailingslashit($subdir) .
                            $location['file'];
                        $retval['handle'] = $script_handle
                            ? $script_handle
                            : "{$location['type']}-{$type}";
        ...
        public function sitewide_notices()
        {
            ...
            $class = did_action('admin_bar_menu')
                ? 'admin-bar-on'
                : 'admin-bar-off';
         ...
         * @return array
         */
        public function theme_compat_page_templates($templates = [])
        {
            /**
...
function bp_legacy_theme_ajax_querystring($query_string, $object)
{
    ...
    if (
        !empty($_BP_COOKIE['bp-' . $object . '-filter']) &&
        '-1' != $_BP_COOKIE['bp-' . $object . '-filter']
    ) {
        ...
        if (bp_is_active('activity')) {
            ...
            $action_filter = explode(
                ',',
                $_BP_COOKIE['bp-' . $object . '-filter']
            );
    ...
    if (!empty($_BP_COOKIE['bp-' . $object . '-scope'])) {
        ...
        if (
            'all' != $_BP_COOKIE['bp-' . $object . '-scope'] &&
            !bp_displayed_user_id() &&
            !bp_is_single_item()
        ) {
            $qs[] =
                'scope=' . urlencode($_BP_COOKIE['bp-' . $object . '-scope']);
        }
    }
...
function bp_legacy_theme_object_template_loader()
{
    ...
    if (
        ...
        'groups/single/members' === $_POST['template']
    ) {
    ...
    $template_path = bp_locate_template([$template_part], false);

    $template_path = apply_filters(
        'bp_legacy_object_template_path',
        $template_path
    );
...
function bp_legacy_theme_activity_template_loader()
{
    ...
    switch ($scope) {
        ...
        case 'mentions':
            ...
            if (
                isset($_POST['_wpnonce_activity_filter']) &&
                wp_verify_nonce(
                    wp_unslash($_POST['_wpnonce_activity_filter']),
                    'activity_filter'
                )
            ) {
...
function bp_legacy_theme_post_update()
{
    ...
    if (empty($_POST['content'])) {
        exit(
            '-1<div id="message" class="error bp-ajax-message"><p>' .
    ...
    $last_recorded = !empty($_POST['since'])
        ? date('Y-m-d H:i:s', intval($_POST['since']))
        : 0;
    if ($last_recorded) {
        $activity_args = ['since' => $last_recorded];
...
function bp_legacy_theme_new_activity_comment()
{
    ...
    bp_has_activities(
        'display_comments=stream&hide_spam=false&show_hidden=true&include=' .
            $comment_id
    );
    ...
    if (isset($activities_template->activities[0])) {
        ...
        $activities_template->activity->id =
            $activities_template->activities[0]->item_id;
        $activities_template->activity->current_comment =
            $activities_template->activities[0];

        // Because the whole tree has not been loaded, we manually
        // determine depth.
        $depth = 1;
        $parent_id =
            (int) $activities_template->activities[0]->secondary_item_id;
        while (
            $parent_id !== (int) $activities_template->activities[0]->item_id
        ) {
            $depth++;
...
function bp_legacy_theme_mark_activity_favorite()
{
    ...
    if (
        !wp_verify_nonce($nonce, 'mark_favorite') &&
        !wp_verify_nonce($nonce, 'unmark_favorite')
    ) {
        return;
    }
...
function bp_legacy_theme_get_single_activity_content()
{
    ...
    $activity = !empty($activity_array['activities'][0])
        ? $activity_array['activities'][0]
        : false;
     ...
     */
    do_action_ref_array('bp_legacy_theme_get_single_activity_content', [
        &$activity,
    ]);
...
function bp_legacy_theme_ajax_invite_user()
{
    ...
    if ('invite' == $_POST['friend_action']) {
        ...
        echo '<span class="activity">' .
            esc_attr($user->last_active) .
            '</span>';
        echo '<div class="action">
				<a class="button remove" href="' .
            wp_nonce_url($uninvite_url, 'groups_invite_uninvite_user') .
            '" id="uid-' .
            esc_attr($user->id) .
            '">' .
        ...
        if ('is_pending' == $user_status) {
            ...
            echo '<p class="description">' .
                sprintf(
                    __(
                        '%s has previously requested to join this group. Sending an invitation will automatically add the member to the group.',
                        'buddypress'
                    ),
                    $user->user_link
                ) .
...
function bp_legacy_theme_ajax_addremove_friend()
{
    ...
    if (
        ...
        if (!friends_remove_friend(bp_loggedin_user_id(), $friend_id)) {
            ...
            echo '<a id="friend-' .
                esc_attr($friend_id) .
                '" class="friendship-button not_friends add" rel="add" href="' .
                wp_nonce_url(
                    bp_loggedin_user_domain() .
                        bp_get_friends_slug() .
                        '/add-friend/' .
        ...
        if (!friends_add_friend(bp_loggedin_user_id(), $friend_id)) {
            ...
            echo '<a id="friend-' .
                esc_attr($friend_id) .
                '" class="remove friendship-button pending_friend requested" rel="remove" href="' .
                wp_nonce_url(
                    bp_loggedin_user_domain() .
                        bp_get_friends_slug() .
                        '/requests/cancel/' .
                ...
                '" class="requested">' .
                __('Cancel Friendship Request', 'buddypress') .
                '</a>';
        }
...
function bp_legacy_theme_ajax_accept_friendship()
{
    ...
    if (!friends_accept_friendship((int) $_POST['id'])) {
        echo "-1<div id='message' class='error'><p>" .
            __(
...
function bp_legacy_theme_ajax_joinleave_group()
{
    ...
    switch ($request_type) {
        case 'join_group':
            if (
                !bp_current_user_can('groups_join_group', [
                    'group_id' => $group->id,
                ])
            ) {
            ...
            if (!groups_join_group($group->id)) {
                ...
                echo '<a id="group-' .
                    esc_attr($group->id) .
                    '" class="group-button leave-group" rel="leave" href="' .
                    wp_nonce_url(
                        bp_get_group_permalink($group) . 'leave-group',
                        'groups_leave_group'
                    ) .
                    '">' .
                    __('Leave Group', 'buddypress') .
                    '</a>';
            }
            break;
        ...
        case 'accept_invite':
            if (
                !bp_current_user_can('groups_request_membership', [
                    'group_id' => $group->id,
                ])
            ) {
        ...
        case 'request_membership':
            ...
            if (
                ...
                echo '<a id="group-' .
                    esc_attr($group->id) .
                    '" class="group-button disabled pending membership-requested" rel="membership-requested" href="' .
                    bp_get_group_permalink($group) .
                    '">' .
                    __('Request Sent', 'buddypress') .
                    '</a>';
            }
            break;
        ...
        case 'leave_group':
            ...
            if (!groups_leave_group($group->id)) {
                ...
                echo '<a id="group-' .
                    esc_attr($group->id) .
                    '" class="group-button join-group" rel="join" href="' .
                    wp_nonce_url(
                        bp_get_group_permalink($group) . 'join',
                        'groups_join_group'
                    ) .
                    '">' .
                    __('Join Group', 'buddypress') .
                    '</a>';
            } else {
                echo '<a id="group-' .
                    esc_attr($group->id) .
                    '" class="group-button request-membership" rel="join" href="' .
                    wp_nonce_url(
                        bp_get_group_permalink($group) . 'request-membership',
                        'groups_request_membership'
                    ) .
                    '">' .
                    __('Request Membership', 'buddypress') .
                    '</a>';
            }
            break;
    }
...
function bp_legacy_theme_ajax_messages_send_reply()
{
    ...
    if (
        ...
        (!messages_is_valid_thread($thread_id) ||
            !messages_check_thread_access($thread_id))
    ) {
        echo "-1<div id='message' class='error'><p>" .
            __(
    ...
    if (!empty($result)) {
        ...
        bp_thread_has_messages(['thread_id' => $thread_id]);
        ...
        $thread_template->current_message = key(
            $thread_template->thread->messages
        );
...
function bp_legacy_theme_ajax_messages_autocomplete_results()
{
    ...
    $limit = isset($_GET['limit'])
        ? absint($_GET['limit'])
        : (int) apply_filters('bp_autocomplete_max_results', 10);
...
{
    if (
        false === bp_is_active('messages', 'star') ||
    ...
    // Check nonce.
    check_ajax_referer(
        'bp-messages-star-' . (int) $_POST['message_id'],
        'nonce'
    );
 ...
 */
function bp_legacy_theme_cover_image($params = [])
{
    if (empty($params)) {
        return;
    }
