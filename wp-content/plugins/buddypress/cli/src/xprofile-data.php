...
class XProfile_Data extends BuddyPressCommand
{
            ...
            foreach ($data as $field_name => $field_data) {
                ...
                $formatted_data[] = [
                    ...
                    'value' => wp_json_encode(
                        maybe_unserialize($field_data['field_data'])
                    ),
                ];
            }

            $format_args = $assoc_args;
            $format_args['fields'] = ['field_id', 'field_name', 'value'];
    ...
    public function delete($args, $assoc_args)
    {
        ...
        if (
            !isset($assoc_args['field-id']) &&
            !isset($assoc_args['delete-all'])
        ) {
            WP_CLI::error(
        ...
        if (isset($assoc_args['delete-all'])) {
            WP_CLI::confirm(
                sprintf(
                    'Are you sure you want to delete all XProfile data for the user %s (#%d)?',
                    $user->user_login,
                    $user->ID
                ),
                $assoc_args
            ...
            if (
                xprofile_delete_field_data($assoc_args['field-id'], $user->ID)
            ) {
                WP_CLI::success('XProfile data removed.');
            } else {
