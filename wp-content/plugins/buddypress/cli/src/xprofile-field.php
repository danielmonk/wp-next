...
class XProfile_Field extends BuddyPressCommand
{
    ...
    public function create($args, $assoc_args)
    {
        ...
        if (
            !in_array(
                $assoc_args['type'],
                (array) buddypress()->profile->field_types,
                true
            )
        ) {
    ...
    public function delete($args, $assoc_args)
    {
        ...
        parent::_delete($args, $assoc_args, function ($field_id) use (
            $delete_data
        ) {
            $field = new \BP_XProfile_Field($field_id);
            $name = $field->name;
            ...
            if ($field->delete($delete_data)) {
                return [
                    'success',
                    sprintf('Deleted XProfile field "%s" (ID %d).', $name, $id),
                ];
            } else {
                return [
                    'error',
                    sprintf(
                        'Failed deleting XProfile field (ID %d).',
                        $field_id
                    ),
                ];
            }
