...
class Scaffold extends Scaffold_Command
{
    ...
    public function plugin($args, $assoc_args)
    {
        ...
        if (!is_dir($target_dir)) {
            WP_CLI::error(
                "Invalid plugin slug specified. No such target directory '{$target_dir}'."
            );
        }
        ...
        foreach ($to_copy as $file => $dir) {
            ...
            // Prompt it.
            $should_write_file = $this->prompt_if_files_will_be_overwritten(
                $file_name,
                $prompt
            );
            ...
            if ('install-bp-tests.sh' === $file) {
                if (!$wp_filesystem->chmod("$dir/$file", 0755)) {
                    WP_CLI::warning(
                        "Couldn't mark 'install-bp-tests.sh' as executable."
                    );
                }
            }
        }
    ...
    {
        $parent_dir = dirname(
            self::canonicalize_path(str_replace('\\', '/', $target_dir))
        );

        if (str_replace('\\', '/', WP_PLUGIN_DIR) !== $parent_dir) {
            return sprintf(
                'The target directory \'%1$s\' is not in \'%2$s\'.',
                $target_dir,
                WP_PLUGIN_DIR
            );
        }
