...
class Components extends BuddyPressCommand
{
    ...
    public function activate($args)
    {
        ...
        if (!$this->component_exists($component)) {
            WP_CLI::error(
                sprintf('%s is not a valid component.', ucfirst($component))
            );
        }
        ...
        if (bp_is_active($component)) {
            WP_CLI::error(
                sprintf(
                    'The %s component is already active.',
                    ucfirst($component)
                )
            );
        }
        ...
        WP_CLI::success(
            sprintf('The %s component has been activated.', ucfirst($component))
        );
    }
    ...
    public function deactivate($args)
    {
        ...
        if (!bp_is_active($component)) {
            WP_CLI::error(
                sprintf('The %s component is not active.', ucfirst($component))
            );
        }
        ...
        WP_CLI::success(
            sprintf(
                'The %s component has been deactivated.',
                ucfirst($component)
            )
        );
    }
    ...
    public function list_($args, $assoc_args)
    {
        ...
        if ('optional' !== $type && isset($components['core'])) {
            if (!isset($active_components['core'])) {
                $active_components = array_merge($active_components, [
            ...
            case 'all':
                ...
                foreach ($components as $component_key => $component) {
                    ...
                    $current_components[] = [
                        'number' => $index,
                        'id' => $component_key,
                        'status' => $this->verify_component_status(
                            $component_key
                        ),
            ...
            case 'active':
                ...
                foreach (array_keys($active_components) as $component_key) {
                    ...
                    $current_components[] = [
                        ...
                        'title' => esc_html(
                            $components[$component_key]['title']
                        ),
                        'description' => html_entity_decode(
                            $components[$component_key]['description']
                        ),
                    ];
                }
                break;
