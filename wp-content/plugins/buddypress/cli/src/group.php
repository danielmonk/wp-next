...
class Group extends BuddyPressCommand
{
    ...
    public function create($args, $assoc_args)
    {
        ...
        if (empty($r['description'])) {
            $r['description'] = sprintf(
                'Description for group "%s"',
                $r['name']
            );
        }
        ...
        for ($i = 0; $i < $assoc_args['count']; $i++) {
            $this->create(
                ...
                [
                    ...
                    'status' => $this->random_group_status(
                        $assoc_args['status']
                    ),
    ...
    {
        WP_CLI::confirm(
            'Are you sure you want to delete this group and its metadata?',
            $assoc_args
        );
    ...
    {
        parent::_update($args, $assoc_args, function ($group_id, $fields = []) {
