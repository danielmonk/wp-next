...
class Activity extends BuddyPressCommand
{
    ...
    public function create($args, $assoc_args)
    {
        ...
        if (WP_CLI\Utils\get_flag_value($assoc_args, 'porcelain')) {
            ...
            WP_CLI::success(
                sprintf('Successfully created new activity item (ID #%d)', $id)
            );
        }
    }
    ...
    public function list_($args, $assoc_args)
    {
        ...
        if (
            isset($assoc_args['user-id']) &&
            is_numeric($assoc_args['user-id'])
        ) {
    ...
    public function generate($args, $assoc_args)
    {
        ...
        if (
            (bool) $assoc_args['skip-activity-comments'] &&
    ...
    public function post_update($args, $assoc_args)
    {
        ...
        if (WP_CLI\Utils\get_flag_value($assoc_args, 'porcelain')) {
            ...
            WP_CLI::success(
                sprintf(
                    'Successfully updated with a new activity item (ID #%d)',
                    $id
                )
            );
        }
    ...
    public function comment($args, $assoc_args)
    {
        ...
        $user = $this->get_user_id_from_identifier($assoc_args['user-id']);
        $skip_notification = WP_CLI\Utils\get_flag_value(
            $assoc_args,
            'skip-notification'
        );
        ...
        if (WP_CLI\Utils\get_flag_value($assoc_args, 'porcelain')) {
            ...
            WP_CLI::success(
                sprintf(
                    'Successfully added a new activity comment (ID #%d)',
                    $id
                )
            );
        }
    ...
    public function delete_comment($args, $assoc_args)
    {
        ...
        WP_CLI::confirm(
            'Are you sure you want to delete this activity comment?',
            $assoc_args
        );

        // Delete Comment. True if deleted.
        if (
            bp_activity_delete_comment($activity_id, $assoc_args['comment-id'])
        ) {
        ...
        switch ($r['type']) {
            case 'activity_update':
                ...
                if ('groups' === $r['component']) {
                    ...
                    $r['action'] = sprintf(
                        ...
                        bp_core_get_userlink($r['user-id']),
                        '<a href="' .
                            bp_get_group_permalink($group_obj) .
                            '">' .
                            esc_attr($group_obj->name) .
                            '</a>'
                    );
                } else {
            ...
            case 'activity_comment':
                ...
                $parent_item = $wpdb->get_row(
                    "SELECT * FROM {$bp->activity->table_name} ORDER BY RAND() LIMIT 1"
            ...
            case 'new_blog_comment':
                ...
                if (is_multisite()) {
                    $r['item-id'] = $wpdb->get_var(
                        "SELECT blog_id FROM {$wpdb->blogs} ORDER BY RAND() LIMIT 1"
                    ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                } else {
                    $r['item-id'] = 1;
                }
                ...
                if (
                    ...
                    $comment_info = $wpdb->get_results(
                        "SELECT comment_id, comment_post_id FROM {$wpdb->comments} ORDER BY RAND() LIMIT 1"
                    ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                ...
                if ('new_blog' === $r['type']) {
                    ...
                    if (!$r['action']) {
                        $r['action'] = sprintf(
                            '%s created the site %s',
                            bp_core_get_userlink($r['user-id']),
                            '<a href="' .
                                get_home_url($r['item-id']) .
                                '">' .
                                esc_attr(
                                    get_blog_option($r['item-id'], 'blogname')
                                ) .
                                '</a>'
                        );
                    }
                    ...
                    if ('' === $r['primary-link']) {
                        $r['primary-link'] = add_query_arg(
                            'p',
                            $post->ID,
                            trailingslashit(get_home_url($r['item-id']))
                        );
                    }
            ...
            case 'friendship_created':
                ...
                $r['action'] = sprintf(
                    '%1$s and %2$s are now friends',
                    bp_core_get_userlink($r['user-id']),
                    bp_core_get_userlink($r['item-id'])
                );
            ...
            case 'updated_profile':
                ...
                if ('new_avatar' === $r['type']) {
                    $r['action'] = sprintf(
                        '%s changed their profile picture',
                        $userlink
                    );
