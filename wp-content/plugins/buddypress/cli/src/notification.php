...
class Notification extends BuddyPressCommand
{
    ...
    public function create($args, $assoc_args)
    {
        ...
        if (WP_CLI\Utils\get_flag_value($assoc_args, 'porcelain')) {
            ...
            WP_CLI::success(
                sprintf('Successfully created new notification (ID #%d)', $id)
            );
        }
    }
    ...
    public function delete($args, $assoc_args)
    {
        ...
        if (count($notifications) > 1) {
            WP_CLI::confirm(
                'Are you sure want to delete these notifications?',
                $assoc_args
            );
        } else {
            WP_CLI::confirm(
        ...
        parent::_delete($notifications, $assoc_args, function (
            $notification_id
        ) {
            if (
                ...
                return [
                    'success',
                    sprintf('Deleted notification %d.', $notification_id),
                ];
            } else {
                return [
                    'error',
                    sprintf(
                        'Could not delete notification %d.',
                        $notification_id
                    ),
                ];
            }
