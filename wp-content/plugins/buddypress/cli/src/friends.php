    ...
    public function create($args, $assoc_args)
    {
        ...
        if (WP_CLI\Utils\get_flag_value($assoc_args, 'porcelain')) {
            ...
            if ($force) {
                ...
                WP_CLI::success(
                    'Friendship successfully created but not accepted.'
                );
            }
        }
    }
    ...
    public function accept($args, $assoc_args)
    {
        parent::_update($args, $assoc_args, function ($friendship_id) {
            if (friends_accept_friendship((int) $friendship_id)) {
                ...
                return [
                    'error',
                    'There was a problem accepting the friendship.',
                ];
            }
        });
    }
    ...
    public function reject($args, $assoc_args)
    {
        parent::_update($args, $assoc_args, function ($friendship_id) {
            if (friends_reject_friendship((int) $friendship_id)) {
                ...
                return [
                    'error',
                    'There was a problem rejecting the friendship.',
                ];
            }
        });
    }
    ...
    public function generate($args, $assoc_args)
    {
        ...
        for ($i = 0; $i < $assoc_args['count']; $i++) {
            if (isset($assoc_args['initiator'])) {
                $user = $this->get_user_id_from_identifier(
            ...
            if (isset($assoc_args['friend'])) {
                $user_2 = $this->get_user_id_from_identifier(
                    $assoc_args['friend']
                );
                $friend = $user_2->ID;
            } else {
            ...
            $this->create([$member, $friend], ['silent', 'force-accept']);
