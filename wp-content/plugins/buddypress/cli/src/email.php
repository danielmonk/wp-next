...
class Email extends BuddyPressCommand
{
    ...
    public function create($args, $assoc_args)
    {
        ...
        if (0 !== $term && null !== $term) {
            ...
            WP_CLI::error(
                sprintf('Email type %s already exists.', $assoc_args['type'])
            );
        }

        if (
            !empty($args[0]) &&
            \method_exists($this, 'read_from_file_or_stdin')
        ) {
        ...
        $email = [
            ...
            'post_excerpt' => !empty($assoc_args['plain-text-content'])
                ? $assoc_args['plain-text-content']
                : '',
        ];
        ...
        if (!is_wp_error($post_id)) {
            ...
            if (
                !is_wp_error($tt_ids) &&
                !empty($assoc_args['type-description'])
            ) {
                $term = get_term_by(
                    'term_taxonomy_id',
                    (int) $tt_ids[0],
            ...
            WP_CLI::success(
                sprintf('Email post created for type %s.', $assoc_args['type'])
            );
        } else {
            if (true === $switched) {
                restore_current_blog();
            ...
            WP_CLI::error(
                "There was a problem creating the email post for type '{$assoc_args['type']}' - " .
    ...
    public function get_post($args, $assoc_args)
    {
        ...
        if (is_wp_error($email)) {
            WP_CLI::error(
                sprintf('Email post for type %s does not exist.', $args[0])
            );
        }
        ...
        $this->get_formatter($assoc_args)->display_item(
            $email->get_post_object()
        );
    }
    ...
    {
        WP_CLI::confirm(
            'Are you sure you want to reinstall BuddyPress emails?',
            $assoc_args
        );
