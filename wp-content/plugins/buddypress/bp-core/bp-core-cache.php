...
{
    foreach (
        ['add_option', 'add_site_option', 'update_option', 'update_site_option']
        as $action
    ) {
 ...
 */
function bp_update_meta_cache($args = [])
{
    global $wpdb;

    $defaults = [
        'object_ids' => [], // Comma-separated list or array of item ids.
    ...
    if (!empty($uncached_ids)) {
        ...
        $meta_list = $wpdb->get_results(
            esc_sql(
                "SELECT {$object_column}, meta_key, meta_value FROM {$meta_table} WHERE {$object_column} IN ({$id_list})"
            ),
            ARRAY_A
        );
