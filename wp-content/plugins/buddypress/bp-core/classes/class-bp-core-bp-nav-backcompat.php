...
class BP_Core_BP_Nav_BackCompat implements ArrayAccess
{
     ...
     */
    public function __construct($backcompat_nav = [])
    {
    ...
    public function offsetSet($offset, $value)
    {
        _doing_it_wrong(
            'bp_nav',
            __(
                'The bp_nav and bp_options_nav globals should not be used directly and are deprecated. Please use the BuddyPress nav functions instead.',
                'buddypress'
            ),
            '2.6.0'
        );
        ...
        if ($offset !== null) {
            ...
            if (isset($args['parent_slug'])) {
                $this->get_component_nav($args['parent_slug'])->edit_nav(
                    $args,
                    $args['slug'],
                    $args['parent_slug']
                );
    ...
    public function get_component($offset = '')
    {
        if (!isset($this->component)) {
            if (
                bp_is_active('groups') &&
                $offset === bp_get_current_group_slug()
            ) {
    ...
    protected function get_nav($offset)
    {
        ...
        $primary_nav = $component_nav->get_primary(['slug' => $offset], false);
