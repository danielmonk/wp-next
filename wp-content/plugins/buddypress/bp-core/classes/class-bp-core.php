...
class BP_Core extends BP_Component
{
    ...
    private function bootstrap()
    {
         ...
         */
        $bp->required_components = apply_filters('bp_required_components', [
            'members',
        ]);
        ...
        if ($active_components = bp_get_option('bp-active-components')) {
            ...
            $bp->active_components = apply_filters(
             ...
             */
            $bp->deactivated_components = apply_filters(
            ...
            $active_components = array_fill_keys(
                array_diff(
                    ...
                    array_values($bp->deactivated_components)
                ),
                '1'
            );
    ...
    public function setup_globals($args = [])
    {
        ...
        $bp->loggedin_user->id = isset($current_user->ID)
            ? $current_user->ID
            : 0;
         ...
         */
        $bp->grav_default->user = apply_filters(
            'bp_user_gravatar_default',
            $bp->site_options['avatar_default']
        );
         ...
         */
        $bp->grav_default->group = apply_filters(
         ...
         */
        $bp->grav_default->blog = apply_filters(
