...
class BP_Invitation
{
    ...
    public function save()
    {
        ...
        $data = [
            ...
            'type' => $this->type,
            'content' => wp_kses(wp_unslash($this->content), []),
         ...
         */
        do_action_ref_array('bp_invitation_before_save', [&$this]);
         ...
         */
        do_action_ref_array('bp_invitation_after_save', [&$this]);
    ...
    public function populate()
    {
        ...
        if (false === $invitation) {
            $invitation = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT * FROM {$invites_table_name} WHERE id = %d",
                    $this->id
                )
            );
    ...
    protected static function get_where_sql($args = [])
    {
        ...
        if (!empty($args['inviter_id']) || 0 === $args['inviter_id']) {
            $inviter_id_in = implode(
                ',',
                wp_parse_id_list($args['inviter_id'])
            );
            $where_conditions[
                'inviter_id'
            ] = "inviter_id IN ({$inviter_id_in})";
        }
        ...
        if (!empty($args['invitee_email'])) {
            ...
            $where_conditions[
                'invitee_email'
            ] = "invitee_email IN ({$invitee_email_in})";
        }
    ...
    public static function get($args = [])
    {
         ...
         */
        $paged_invites_sql = apply_filters(
        ...
        if (
            in_array(
                $r['fields'],
                ['ids', 'item_ids', 'user_ids', 'inviter_ids'],
                true
            )
        ) {
        ...
        $uncached_ids = bp_get_non_cached_ids(
            $paged_invite_ids,
            'bp_invitations'
        );
        if ($uncached_ids) {
            $ids_sql = implode(',', array_map('intval', $uncached_ids));
            $data_objects = $wpdb->get_results(
                "SELECT i.* FROM {$invites_table_name} i WHERE i.id IN ({$ids_sql})"
            );
