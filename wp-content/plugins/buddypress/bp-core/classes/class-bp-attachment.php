...
abstract class BP_Attachment
{
    ...
    public function set_upload_dir()
    {
        ...
        if (!empty($this->base_dir)) {
            $this->upload_path =
                trailingslashit($this->upload_path) . $this->base_dir;
     ...
     */
    public function set_upload_error_strings($param = [])
    {
        /**
        ...
        $upload_errors = [
            ...
            2 => sprintf(
                ...
                size_format($this->original_max_filesize)
            ),
            3 => __(
    ...
    public function upload($file, $upload_dir_filter = '', $time = null)
    {
        ...
        if (empty($upload_dir_filter) && !empty($this->base_dir)) {
            $upload_dir_filter = [$this, 'upload_dir_filter'];
        }
        ...
        if (!empty($upload_dir_filter)) {
            add_filter(
                ...
                $upload_dir_filter,
                10,
                $this->upload_dir_filter_args
            );
        }

        // Helper for utf-8 filenames.
        add_filter('sanitize_file_name', [$this, 'sanitize_utf8_filename']);

        // Upload the attachment.
        $this->attachment = wp_handle_upload(
            $file[$this->file_input],
            $overrides,
            $time
        );

        remove_filter('sanitize_file_name', [$this, 'sanitize_utf8_filename']);
    ...
    public function sanitize_utf8_filename($retval)
    {
        ...
        if (
            ...
            $retval = transliterator_transliterate(
    ...
    public function validate_upload($file = [])
    {
        ...
        if (
            !empty($this->original_max_filesize) &&
    ...
    public function crop($args = [])
    {
        ...
        if (empty($r['original_file']) || !file_exists($r['original_file'])) {
            $wp_error->add(
                'crop_error',
                __(
                    'Cropping the file failed: missing source file.',
                    'buddypress'
                )
            );
            return $wp_error;
        ...
        $path_error = __(
            'Cropping the file failed: the file path is not allowed.',
            'buddypress'
        );
        ...
        if (
            !empty($r['dst_file']) &&
            false === strpos($r['dst_file'], $this->upload_path)
        ) {
            $wp_error->add('crop_error', $path_error);
            return $wp_error;
        }
     ...
     */
    public static function edit_image($attachment_type, $args = [])
    {
