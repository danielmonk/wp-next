...
class BP_Embed extends WP_Embed
{
    ...
    public function __construct()
    {
        ...
        if (bp_use_embed_in_activity_replies()) {
            add_filter('bp_get_activity_content', [&$this, 'autoembed'], 8);
            add_filter('bp_get_activity_content', [&$this, 'run_shortcode'], 7);
        }
         ...
         */
        do_action_ref_array('bp_core_setup_oembed', [&$this]);
    }
    ...
    public function shortcode($attr, $url = '')
    {
         ...
         */
        $attr['discover'] =
            apply_filters('bp_embed_oembed_discover', $default_discovery) &&
            $unfiltered_html;
        ...
        if (!$attr['discover']) {
            foreach (
                (array) $oembed_obj->providers
                as $provider_matchmask => $provider
            ) {
                $regex = ($is_regex = $provider[1])
                    ? $provider_matchmask
                    : '#' .
                        str_replace(
                            '___wildcard___',
