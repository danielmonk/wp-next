...
class BP_REST_Components_Endpoint extends WP_REST_Controller
{
        ...
        switch ($args['status']) {
            ...
            case 'active':
                foreach (array_keys($active_components) as $component) {
                    $current_components[] = $this->get_component_info(
                        $component
                    );
                }
                break;
        ...
        if ('activate' === $request['action']) {
            if (bp_is_active($component)) {
                return new WP_Error(
                    ...
                    __(
                        'Sorry, this component is already active.',
                        'buddypress'
                    ),
                    [
                        'status' => 400,
            ...
            if (
                array_key_exists($component, bp_core_get_components('required'))
            ) {
                return new WP_Error(
                    'bp_rest_required_component',
                    __(
                        'Sorry, you cannot deactivate a required component.',
                        'buddypress'
                    ),
                    [
                        'status' => 400,
    ...
    public function get_collection_params()
    {
        ...
        $params['type'] = [
            ...
            'default' => 'all',
            'type' => 'string',
            'enum' => ['all', 'optional', 'retired', 'required'],
            'sanitize_callback' => 'sanitize_key',
            'validate_callback' => 'rest_validate_request_arg',
        ];
