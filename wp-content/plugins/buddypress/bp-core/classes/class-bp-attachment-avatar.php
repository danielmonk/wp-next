...
class BP_Attachment_Avatar extends BP_Attachment
{
    ...
    public function __construct()
    {
        ...
        parent::__construct([
            ...
            'upload_error_strings' => [
                ...
                9 => sprintf(
                    _x(
                        'That photo is too big. Please upload one smaller than %s',
                        'profile photo upload error',
                        'buddypress'
                    ),
                    size_format(bp_core_avatar_original_max_filesize())
                ),

                /* translators: %s: comma separated list of file types allowed for the profile photo */
                10 => sprintf(
                    _nx(
                        'Please upload only this file type: %s.',
                        'Please upload only these file types: %s.',
                        count($allowed_types),
                        'profile photo upload error',
                        'buddypress'
                    ),
                    self::get_avatar_types($allowed_types)
                ),
            ],
        ]);
    }
     ...
     */
    public static function get_avatar_types($allowed_types = [])
    {
    ...
    public function validate_upload($file = [])
    {
        ...
        if (!bp_core_check_avatar_size(['file' => $file])) {
            $file['error'] = 9;

            // File is of invalid type.
        } elseif (!bp_core_check_avatar_type(['file' => $file])) {
            $file['error'] = 10;
        }
    ...
    public static function shrink($file = '', $ui_available_width = 0)
    {
        ...
        if (
            !empty($ui_available_width) &&
            $ui_available_width < $original_max_width
        ) {
            /**
        ...
        if (
            isset($avatar_data['width']) &&
            $avatar_data['width'] > $original_max_width
        ) {
            $edit_args = [
        ...
        if (
            isset($avatar_data['meta']['orientation']) &&
            isset($angles[$avatar_data['meta']['orientation']])
        ) {
    ...
    public static function is_too_small($file = '')
    {
        ...
        if (
            (isset($uploaded_image[0]) && $uploaded_image[0] < $full_width) ||
            $uploaded_image[1] < $full_height
        ) {
            return true;
        }
    ...
    public function crop($args = [])
    {
         ...
         */
        if (
            !empty($existing_avatar) &&
            $existing_avatar !== $this->url . $relative_path
        ) {
    ...
    public function script_data()
    {
        ...
        if (!empty($user_id)) {
            ...
            if (bp_avatar_use_webcam()) {
                $js_scripts = ['bp-webcam'];
            }

            $script_data['bp_params'] = [
                'object' => 'user',
                'item_id' => $user_id,
            ...
            $script_data['feedback_messages'] = [
                ...
                4 => __(
                    'Your profile photo was deleted successfully!',
                    'buddypress'
                ),
            ];
            ...
            $script_data['feedback_messages'] = [
                ...
                4 => __(
                    'The group profile photo was deleted successfully!',
                    'buddypress'
                ),
            ];
        } else {
             ...
             */
            $script_data['bp_params'] = apply_filters(
        ...
        $script_data['extra_css'] = ['bp-avatar'];
