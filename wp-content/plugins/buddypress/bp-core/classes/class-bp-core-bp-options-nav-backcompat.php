...
class BP_Core_BP_Options_Nav_BackCompat extends BP_Core_BP_Nav_BackCompat
{
    ...
    public function offsetGet($offset)
    {
        _doing_it_wrong(
            'bp_nav',
            __(
                'These globals should not be used directly and are deprecated. Please use the BuddyPress nav functions instead.',
                'buddypress'
            ),
            '2.6.0'
        );
        ...
        if (empty($this->backcompat_nav[$offset])) {
            ...
            if ($nav) {
                $subnavs = $this->get_component_nav($offset)->get_secondary([
                    'parent_slug' => $offset,
                ]);
                $subnav_keyed = [];
                if ($subnavs) {
    ...
    public function offsetUnset($offset)
    {
        ...
        $this->get_component_nav($offset)->delete_nav(
            $offset,
    ...
    public function get_nav($offset)
    {
        ...
        if (!$nav) {
            ...
            $secondary_nav = $component_nav->get_secondary(
                ['slug' => $offset],
                false
            );
