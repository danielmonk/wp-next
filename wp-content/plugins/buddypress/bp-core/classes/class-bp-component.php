...
if (!class_exists('BP_Component')):
    ...
    class BP_Component
    {
         ...
         * }
         */
        public function start($id = '', $name = '', $path = '', $params = [])
        {
            ...
            if (!empty($params)) {
                ...
                if (!empty($params['adminbar_myaccount_order'])) {
                    $this->adminbar_myaccount_order =
                        (int) $params['adminbar_myaccount_order'];
                }
                ...
                if (!empty($params['search_query_arg'])) {
                    $this->search_query_arg = sanitize_title(
                        $params['search_query_arg']
                    );
                }
        ...
        public function setup_globals($args = [])
        {
            ...
            $default_root_slug = isset(buddypress()->pages->{$this->id}->slug)
                ? buddypress()->pages->{$this->id}->slug
                : '';
             ...
             */
            $this->has_directory = apply_filters(
             ...
             */
            $this->directory_title = apply_filters(
             ...
             */
            $this->search_string = apply_filters(
             ...
             */
            $this->notification_callback = apply_filters(
                'bp_' . $this->id . '_notification_callback',
                $r['notification_callback']
            );
        ...
        {
            // Setup globals.
            add_action('bp_setup_globals', [$this, 'setup_globals'], 10);

            // Set up canonical stack.
            add_action(
                'bp_setup_canonical_stack',
                [$this, 'setup_canonical_stack'],
                10
            );
            ...
            add_action('bp_include', [$this, 'includes'], 8);

            // Load files conditionally, based on certain pages.
            add_action('bp_late_include', [$this, 'late_includes']);
            ...
            add_action('bp_setup_title', [$this, 'setup_title'], 10);
            ...
            add_action('bp_add_rewrite_tags', [$this, 'add_rewrite_tags'], 10);
            ...
            add_action('bp_add_permastructs', [$this, 'add_permastructs'], 10);

            // Allow components to parse the main query.
            add_action('bp_parse_query', [$this, 'parse_query'], 10);
            ...
            if (bp_rest_in_buddypress() && bp_rest_api_is_available()) {
                add_action('bp_rest_api_init', [$this, 'rest_api_init'], 10);
            }

            // Register BP Blocks.
            if (bp_support_blocks()) {
                add_action('bp_blocks_init', [$this, 'blocks_init'], 10);
            }
         ...
         * @param array $tables Table names to register.
         */
        public function register_meta_tables($tables = [])
        {
            global $wpdb;
