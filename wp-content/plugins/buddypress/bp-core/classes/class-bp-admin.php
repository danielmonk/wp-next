...
if (!class_exists('BP_Admin')):
    ...
    class BP_Admin
    {
        ...
        private function setup_globals()
        {
            ...
            $this->admin_dir = trailingslashit(
                $bp->plugin_dir . 'bp-core/admin'
            ); // Admin path.
            $this->admin_url = trailingslashit(
                $bp->plugin_url . 'bp-core/admin'
            ); // Admin url.
            ...
            // Main settings page.
            $this->settings_page = bp_core_do_network_admin()
                ? 'settings.php'
                : 'options-general.php';
        ...
        private function setup_actions()
        {
            ...
            add_action('admin_menu', [$this, 'site_admin_menus'], 5);
            add_action(bp_core_admin_hook(), [$this, 'admin_menus'], 5);
            ...
            add_action('admin_bar_menu', [$this, 'admin_bar_about_link'], 100);
            ...
            // Emails.
            add_filter(
                'manage_' . bp_get_email_post_type() . '_posts_columns',
                [$this, 'emails_register_situation_column']
            );
            add_action(
                'manage_' . bp_get_email_post_type() . '_posts_custom_column',
                [$this, 'emails_display_situation_column_data'],
                10,
                2
            );

            // Privacy Policy.
            add_action('bp_admin_init', [$this, 'add_privacy_policy_content']);

            // BuddyPress Hello.
            add_action('admin_footer', [$this, 'about_screen']);
        ...
        public function register_admin_settings()
        {
            ...
            if ((bool) bp_get_option('_bp_force_buddybar', false)) {
                ...
                if (
                    !function_exists('bp_admin_setting_callback_force_buddybar')
                ) {
                    require buddypress()->plugin_dir .
            ...
            add_settings_field(
                ...
                __('Account Deletion', 'buddypress'),
            ...
            add_settings_field(
                ...
                __('Template Pack', 'buddypress'),
                'bp_admin_setting_callback_theme_package_id',
                'buddypress',
                'bp_main',
                ['label_for' => '_bp_theme_package_id']
            );
            register_setting(
                'buddypress',
                '_bp_theme_package_id',
            ...
            add_settings_section(
                'bp_members',
                _x('Members Settings', 'BuddyPress setting tab', 'buddypress'),
                'bp_admin_setting_callback_members_section',
                'buddypress'
            );
            ...
            add_settings_field(
                ...
                __('Profile Photo Uploads', 'buddypress'),
            ...
            if (bp_is_active('members', 'cover_image')) {
                add_settings_field(
                    ...
                    __('Cover Image Uploads', 'buddypress'),
            ...
            if (bp_is_active('groups')) {
                ...
                add_settings_field(
                    ...
                    __('Group Photo Uploads', 'buddypress'),
                ...
                if (bp_is_active('groups', 'cover_image')) {
                    add_settings_field(
                        ...
                        __('Group Cover Image Uploads', 'buddypress'),
            ...
            if (bp_is_active('activity')) {
                ...
                add_settings_section(
                    'bp_activity',
                    __('Activity Settings', 'buddypress'),
                    'bp_admin_setting_callback_activity_section',
                    'buddypress'
                );
                ...
                add_settings_field(
                    ...
                    __('Activity auto-refresh', 'buddypress'),
                ...
                if (
                    ...
                    defined('AKISMET_VERSION')
                ) {
                    add_settings_field(
                        '_bp_enable_akismet',
                        __('Akismet', 'buddypress'),
        ...
        public function admin_bar_about_link($wp_admin_bar)
        {
            ...
            $wp_admin_bar->add_node([
                ...
                'title' => esc_html_x(
                    'Hello, BuddyPress!',
                    'Colloquial alternative to "learn about BuddyPress"',
                    'buddypress'
                ),
        ...
        public function modify_plugin_action_links($links, $file)
        {
            ...
            return array_merge($links, [
                'settings' =>
                    ...
                    '">' .
                    esc_html__('Settings', 'buddypress') .
                    '</a>',
                'about' =>
                    '<a href="' .
                    esc_url(bp_get_admin_url('?hello=buddypress')) .
                    '">' .
                    esc_html_x(
                        'Hello, BuddyPress!',
                        'Colloquial alternative to "learn about BuddyPress"',
        ...
        public function enqueue_scripts()
        {
            ...
            // BuddyPress Hello.
            if (
                0 === strpos(get_current_screen()->id, 'dashboard') &&
                !empty($_GET['hello']) &&
                $_GET['hello'] === 'buddypress'
            ) {
                wp_enqueue_style('bp-hello-css');
                wp_enqueue_script('bp-hello-js');
                wp_localize_script('bp-hello-js', 'bpHelloStrings', [
                    'pageNotFound' => __(
        ...
        public function add_privacy_policy_content()
        {
            ...
            $suggested_text =
                ...
                esc_html__('Suggested text:', 'buddypress') .
                ' </strong>';
            $content = '';
            ...
            if (bp_is_active('xprofile')) {
                $content .=
                    '<h3>' . esc_html__('Profile Data', 'buddypress') . '</h3>';
                $content .=
                    '<p class="privacy-policy-tutorial">' .
                    esc_html__(
            ...
            if (bp_is_active('activity')) {
                $content .=
                    '<h3>' . esc_html__('Activity', 'buddypress') . '</h3>';
                $content .=
                    '<p class="privacy-policy-tutorial">' .
                    esc_html__(
            ...
            if (bp_is_active('messages')) {
                $content .=
                    '<h3>' . esc_html__('Messages', 'buddypress') . '</h3>';
                $content .=
                    '<p class="privacy-policy-tutorial">' .
                    esc_html__(
		...
		<div id="bp-hello-container">
          ...
          $version
				...
				<div id="plugin-information-tabs">
					<a name="whats-new" href="#whats-new" class="current"><?php esc_html_e(
         'What\'s new?',
         'buddypress'
     ); ?></a>
					<a name="changelog" href="#changelog" class="dynamic" data-slug="<?php echo esc_attr(
         $version_slug
     ); ?>" data-endpoint="https://codex.buddypress.org/wp-json/wp/v2/pages"><?php esc_html_e(
    'Changelog',
    'buddypress'
); ?></a>
					<a name="get-involved" href="#get-involved" class="dynamic" data-slug="participate-and-contribute" data-endpoint="https://codex.buddypress.org/wp-json/wp/v2/pages"><?php esc_html_e(
         'Get involved',
         'buddypress'
     ); ?></a>
				</div>
				...
				<div class="bp-hello-content">
					...
					<div id="top-features">
						<h2><?php esc_html_e(
          'Manage Member Types and Group Types right from your WordPress Dashboard.',
          'buddypress'
      ); ?></h2>
						<figure class="bp-hello-aligncenter">
							<img src="<?php echo esc_url(
           buddypress()->plugin_url . 'bp-core/images/bp-types-illustration.png'
       ); ?>" alt="<?php esc_attr_e(
    ...
    'buddypress'
						...
						<p>
							<?php esc_html_e(
           'Playing with BP Types just became much easier! The Member Types and Group Types were primarily introduced in BuddyPress as features for advanced users, just like the WordPress Custom Post Type feature.',
           'buddypress'
       ); ?>
							<?php printf(
           /* translators: %s is the placeholder for the link to the BP Types Admin developer note. */
           esc_html__(
               'Thanks to the two new %s, adding, editing and deleting Member & Group Types has never been so easy!',
               'buddypress'
           ),
           sprintf(
               '<a href="%1$s">%2$s</a>',
               esc_url(
                   'https://bpdevel.wordpress.com/2020/09/21/bp-types-admin-ui/'
               ),
               esc_html__('WordPress Administration Screens', 'buddypress')
           )
       ); ?>
							<?php esc_html_e(
           'Now you can set up BP Types using custom code or by simply using the Administration interfaces.',
           'buddypress'
       ); ?>
						</p>
           ...
           /* translators: %s is the placeholder for the link to the BP Types Admin developer note. */
           esc_html__(
               'As we were pretty hot on the subject, we also improved Member Types assignment. You can now %s to users from their extended profile in the WordPress Dashboard.',
               'buddypress'
           ),
           sprintf(
               '<a href="%1$s">%2$s</a>',
               esc_url(
                   'https://bpdevel.wordpress.com/2020/10/26/assigning-multiple-member-types-to-a-user/'
               ),
               esc_html__('add more than one member type', 'buddypress')
						...
						<h2><?php esc_html_e(
          'The BP Blocks category now includes 5 blocks!',
          'buddypress'
      ); ?></h2>
						<figure class="bp-hello-aligncenter">
							<img src="<?php echo esc_url(
           buddypress()->plugin_url . 'bp-core/images/bp-new-blocks.png'
       ); ?>" alt="<?php esc_attr_e(
    ...
    'buddypress'
						...
						<p>
							<?php esc_html_e(
           '3 new BP Blocks are now available via your WordPress Editor.',
           'buddypress'
       ); ?>
							<?php esc_html_e(
           'From the BuddyPress blocks category of the WordPress Block Inserter, you can pick a BP Block to feature a list of members, a list of groups or embed a public BuddyPress Activity into your post or page.',
           'buddypress'
       ); ?>
							<?php printf(
           ...
           sprintf(
               '<a href="%1$s">%2$s</a>',
               esc_url(
                   'https://bpdevel.wordpress.com/2020/10/14/three-new-blocks-to-expect-in-buddypress-7-0-0/'
               ),
               esc_html__('development note', 'buddypress')
						...
						<h2><?php esc_html_e(
          'Get the best of the BP REST API improvements!',
          'buddypress'
      ); ?></h2>
           ...
           esc_html__(
               'The %s has been updated according to the latest improvements we’ve brought to the BuddyPress REST API.',
               'buddypress'
           ),
           sprintf(
               '<a href="%1$s">%2$s</a>',
               ...
               esc_html__('Developer documentation', 'buddypress')
						...
						<p>
							<?php esc_html_e(
           'To name two: get the groups the logged in user is a member of, and create a blog when BuddyPress is activated on a network of WordPress sites.',
           'buddypress'
       ); ?>
							<?php printf(
           /* translators: %s is the placeholder for the link to the BP REST API developer note. */
           esc_html__(
               'Read this %s to learn about all the others.',
               'buddypress'
           ),
           sprintf(
               '<a href="%1$s">%2$s</a>',
               esc_url(
                   'https://bpdevel.wordpress.com/2020/11/14/buddypress-rest-api-whats-new-in-7-0-0/'
               ),
               esc_html__('development note', 'buddypress')
           )
       ); ?>
          ...
          'buddypress'
						...
						<p>
							<?php esc_html_e(
           'You love the latest default WordPress Theme, so do we!',
           'buddypress'
       ); ?>
							<?php esc_html_e(
           'It’s important for us to make sure the BP Nouveau template pack looks great in the default themes included in the WordPress package.',
           'buddypress'
       ); ?>
							<?php esc_html_e(
           'This is the first of the many improvements we are bringing to our default Template Pack.',
           'buddypress'
       ); ?>
						</p>
						<figure class="bp-hello-aligncenter">
							<img src="<?php echo esc_url(
           buddypress()->plugin_url . 'bp-core/images/bp-nouveau-2021.png'
       ); ?>" alt="<?php esc_attr_e(
    'Screenshot of the BuddyPress Members directory',
    'buddypress'
); ?>" />
						</figure>
						...
						<p>
							<?php esc_html_e(
           'WP-CLI is the command-line interface for WordPress. You can update plugins, configure multisite installs, and much more, all without using a web browser.',
           'buddypress'
       ); ?>
							<?php esc_html_e(
           'In 7.0.0, you will be able to use new BuddyPress CLI commands to manage BuddyPress Group Meta, BuddyPress Activity Meta, activate or deactivate the BuddyPress signup feature and create BuddyPress-specific testing code for plugins.',
           'buddypress'
       ); ?>
						</p>
						<p>
           ...
           sprintf(
               '<a href="%1$s">%2$s</a>',
               esc_url(
                   'https://bpdevel.wordpress.com/2020/10/09/wp-cli-buddypress-2-0/'
               ),
               esc_html__('developer note', 'buddypress')
						...
						<p>
							<?php esc_html_e(
           '7.0.0 includes more than 65 changes to improve your BuddyPress experience as users, and as contributors to our project; click on the “Changelog” tab to discover them all!',
           'buddypress'
       ); ?>
						</p>
						<figure class="bp-hello-aligncenter">
          ...
          _x('Your feedback', 'screen heading', 'buddypress')
      ); ?></h2>
						<p>
							<?php esc_html_e(
           'How are you using BuddyPress? Receiving your feedback and suggestions for future versions of BuddyPress genuinely motivates and encourages our contributors.',
           'buddypress'
       ); ?>
							<?php printf(
          ...
          _n(
              'Built with %1$s by <a href="%2$s">%3$d volunteer</a>.',
              'Built with %1$s by <a href="%2$s">%3$d volunteers</a>.',
              55,
              'buddypress'
          ),
      ...
      'buddypress'
		...
		<h2 class="nav-tab-wrapper"><?php bp_core_admin_tabs(
      esc_html__('Credits', 'buddypress')
  ); ?></h2>

			<p class="about-description"><?php esc_html_e(
       'Meet the contributors behind BuddyPress:',
       'buddypress'
   ); ?></p>

			<h3 class="wp-people-group"><?php esc_html_e(
       'Project Leaders',
       'buddypress'
   ); ?></h3>
       ...
       'buddypress'
			...
			<ul class="wp-people-group " id="wp-people-group-core-team">
				...
				<li class="wp-person" id="wp-person-henry-wright">
					...
					Henry Wright</a>
					<span class="title"><?php esc_html_e(
         'Community Support',
         'buddypress'
     ); ?></span>
				</li>
        ...
        self::display_version()
			...
			<h3 class="wp-people-group"><?php esc_html_e(
       'With our thanks to these Open Source projects',
       'buddypress'
   ); ?></h3>
			<p class="wp-credits-list">
       ...
       'buddypress'
         ...
         * @return array
         */
        public function emails_register_situation_column($columns = [])
        {
            $situation = [
                'situation' => _x(
                    'Situations',
         ...
         */
        public function emails_display_situation_column_data(
            $column = '',
            $post_id = 0
        ) {
            if ('situation' !== $column) {
            ...
            $situations = wp_list_pluck(
                get_the_terms($post_id, bp_get_email_tax_type()),
                'description'
            );
        ...
        public static function display_version()
        {
            ...
            if ('' === $display) {
                ...
                $display =
                    false !== $pre ? substr($version, 0, $pre) : $version;
            }
         ...
         */
        public function emails_admin_menu_order($custom_menus = [])
        {
            array_push(
                $custom_menus,
                'edit.php?post_type=' . bp_get_email_post_type()
        ...
        public function admin_register_styles()
        {
             ...
             */
            $styles = apply_filters('bp_core_admin_register_styles', [
                // Legacy.
                'bp-admin-common-css' => [
                    'file' => $common_css,
                    'dependencies' => [],
                ...
                'bp-hello-css' => [
                    ...
                    'dependencies' => ['bp-admin-common-css', 'thickbox'],
                ],
            ]);
        ...
        public function admin_register_scripts()
        {
             ...
             */
            $scripts = apply_filters('bp_core_admin_register_scripts', [
                // 2.5
                'bp-customizer-controls' => [
                ...
                'bp-hello-js' => [
                    ...
                    'dependencies' => ['thickbox', 'bp-api-request'],
                    'footer' => true,
                ],
            ]);
