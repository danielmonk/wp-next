...
if (!class_exists('BP_Admin_Types')):
    ...
    class BP_Admin_Types
    {
        ...
        {
            // Actions.
            add_action('admin_head-edit-tags.php', [$this, 'screen_head']);
            add_action('admin_head-term.php', [$this, 'screen_head']);
            add_action('bp_admin_enqueue_scripts', [$this, 'screen_scripts']);
            add_action(
            ...
            // Filters
            add_filter('bp_core_admin_register_scripts', [
                $this,
                'register_scripts',
            ]);
            add_filter(
                "manage_{$this->screen_id}_columns",
                [$this, 'column_headers'],
                10,
                1
            );
            ...
            add_filter(
                "{$this->taxonomy}_row_actions",
                [$this, 'row_actions'],
                10,
                2
            );
            add_filter(
                "bulk_actions-{$this->screen_id}",
            ...
            if ('add-tag' === $action) {
                ...
                if (is_wp_error($result)) {
                    $referer = add_query_arg(
                        array_merge($result->get_error_data(), [
                            'error' => 1,
                        ]),
                        $referer
                    );
        ...
        public function screen_head()
        {
            ...
            if ('members' === $this->taxonomies[$this->taxonomy]['component']) {
                ...
                $parent_file =
                    'bp-' . $this->taxonomies[$this->taxonomy]['component'];
            }
        }
        ...
        public function add_form_fields($taxonomy = '', $type = null)
        {
            ...
            $type_id_desc = __(
                'Enter a lower-case string without spaces or special characters (used internally to identify the type).',
                'buddypress'
            );
            ...
            if (
                isset($labels->bp_type_id_description) &&
                $labels->bp_type_id_description
            ) {
            ...
            foreach ($metafields as $meta_key => $meta_schema) {
                if (!isset($labels->{$meta_key}) || !$labels->{$meta_key}) {
                    _doing_it_wrong(
                        __METHOD__,
                        __(
                            'Type metadata labels need to be set into the labels argument when registering your taxonomy using the meta key as the label’s key.',
                            'buddypress'
                        ) .
                            ' ' .
                            sprintf(
                                ...
                                __(
                                    'As a result, the form elements for the "%s" meta key cannot be displayed',
                                    'buddypress'
                                ),
                                $meta_key
                            ),
                ...
                if ('string' === $meta_schema['type']) {
                    if (isset($type->name)) {
                        ...
                        if (
                            in_array($type_key, ['name', 'singular_name'], true)
                        ) {
        ...
        public function column_headers($column_headers = [])
        {
            ...
            unset(
                ...
                $column_headers['posts']
            );

            $column_headers['plural_name'] = __('Name', 'buddypress');
            $column_headers['counts'] = _x(
                'Count',
                'Number/count of types',
                'buddypress'
            );
        ...
        public function column_contents(
            ...
            if (
                ('plural_name' !== $column_name && 'counts' !== $column_name) ||
                !$type_id
            ) {
            ...
            if ('plural_name' === $column_name) {
                $type_plural_name = get_term_meta(
                    $type_id,
                    'bp_type_name',
                    true
                );
                ...
                if (!$type_plural_name) {
                    ...
                    $metadata = apply_filters(
                        "{$this->taxonomy}_set_registered_by_code_metada",
                        [],
                        $type_name
                    );
         ...
         */
        public function row_actions($actions = [], $type = null)
        {
             ...
             * @since 7.0.0
             */
            $registered_by_code_types = apply_filters(
                "{$type->taxonomy}_registered_by_code",
                []
            );
