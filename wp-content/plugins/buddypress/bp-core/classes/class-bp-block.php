    ...
    public function __construct($args)
    {
        if (!did_action('bp_blocks_init')) {
            _doing_it_wrong(
                __METHOD__,
                esc_html__(
                    'BP Blocks needs to be registered hooking `bp_blocks_init`',
                    'buddypress'
                ),
                '6.0.0'
            );
        ...
        $wp_args = array_intersect_key($args, [
            'name' => '',
            'render_callback' => '',
            'attributes' => '',
            'editor_script' => '',
        ...
        if (
            !isset($wp_args['name']) ||
            !$wp_args['name'] ||
            !isset($wp_args['editor_script']) ||
            !$wp_args['editor_script']
        ) {
            $this->block = new WP_Error(
                'missing_parameters',
                __(
                    'The `name` or `editor_script` required keys are missing.',
                    'buddypress'
                )
            );
        } else {
            // Get specific BP Blocks arguments.
            $bp_args = array_intersect_key($args, [
                'editor_script_url' => '',
                'editor_script_deps' => [],
                'script_url' => '',
                'script_deps' => [],
            ...
            foreach (['editor_script', 'script'] as $script_handle_key) {
                if (
                    !isset($wp_args[$script_handle_key]) ||
                ...
                if (
                    !isset($bp_args[$script_handle_key . '_url']) ||
                ...
                $deps = [];
                if (
                    isset($bp_args[$script_handle_key . '_deps']) &&
                    is_array($bp_args[$script_handle_key . '_deps'])
                ) {
            ...
            if (
                !isset($this->registered_scripts['editor_script']) ||
                !$this->registered_scripts['editor_script']
            ) {
                $this->block = new WP_Error(
                    'script_registration_error',
                    __(
                ...
                foreach (['editor_style', 'style'] as $style_handle_key) {
                    if (
                        !isset($wp_args[$style_handle_key]) ||
                    ...
                    if (
                        !isset($bp_args[$style_handle_key . '_url']) ||
                    ...
                    if ($min) {
                        $minified_css = str_replace(
                            '.css',
                            $min . '.css',
                            $bp_args[$style_handle_key . '_url']
                        );
                        $css_file_path = str_replace(
                            content_url(),
                            WP_CONTENT_DIR,
                            $minified_css
                        );
                    ...
                    $deps = [];
                    if (
                        isset($bp_args[$style_handle_key . '_deps']) &&
                        is_array($bp_args[$style_handle_key . '_deps'])
                    ) {
                ...
                if ($this->block->editor_script) {
                     ...
                     */
                    $translation_dir = apply_filters(
                        'bp_block_translation_dir',
                        null,
                        $this->block->editor_script,
                        $name
                    );
                     ...
                     * @param string $name          The block's name.
                     */
                    $translation_domain = apply_filters(
                        'bp_block_translation_domain',
                        'buddypress',
                        $this->block->editor_script,
                        $name
                    );
                    ...
                    $translated = wp_set_script_translations(
                        $this->block->editor_script,
                        $translation_domain,
                        $translation_dir
                    );
