...
abstract class BP_Core_oEmbed_Extension
{
    ...
    {
        return $this->is_page()
            ? $this->validate_url_to_item_id($this->set_permalink())
            : buddypress()->{$this->slug_endpoint}->embedid_in_progress;
    }
    ...
    protected function setup_hooks()
    {
        ...
        add_action('bp_embed_content', [$this, 'inject_content']);

        add_filter('embed_template', [$this, 'setup_template_parts']);
        add_filter('post_embed_url', [$this, 'filter_embed_url']);
        add_filter('embed_html', [$this, 'filter_embed_html']);
        add_filter('oembed_discovery_links', [
            $this,
            'add_oembed_discovery_links',
        ]);
        add_filter(
            'rest_pre_serve_request',
            [$this, 'oembed_xml_request'],
            20,
            4
        );
    }
    ...
    public function register_route()
    {
        ...
        register_rest_route('oembed/1.0', "/embed/{$this->slug_endpoint}", [
            [
    ...
    public function setup_template_parts($template)
    {
        ...
        add_action(
            'get_template_part_embed',
            [$this, 'content_buffer_start'],
            -999,
            2
        );
        add_action('get_footer', [$this, 'content_buffer_end'], -999);
    ...
    public function add_oembed_discovery_links($retval)
    {
        ...
        add_filter('rest_url', [$this, 'filter_rest_url']);

        $retval =
            '<link rel="alternate" type="application/json+oembed" href="' .
            esc_url(get_oembed_endpoint_url($permalink)) .
            '" />' .
            "\n";
        ...
        if (class_exists('SimpleXMLElement')) {
            $retval .=
                ...
                esc_url(get_oembed_endpoint_url($permalink, 'xml')) .
                '" />' .
                "\n";
        }

        remove_filter('rest_url', [$this, 'filter_rest_url']);
    ...
    public function get_item($request)
    {
        ...
        if (!empty($item_id)) {
            ...
            if (
                !isset(buddypress()->{$this->slug_endpoint}) ||
                !buddypress()->{$this->slug_endpoint}
            ) {
            ...
            if (!empty($custom_args)) {
                buddypress()->{$this->slug_endpoint}->embedargs_in_progress = [];
            ...
            $data = $this->get_oembed_response_data(
                $item,
                $request['maxwidth']
            );
        }
    ...
    public function filter_embed_url($retval)
    {
        if (
            false ===
                isset(
                    buddypress()->{$this->slug_endpoint}->embedurl_in_progress
                ) &&
            !$this->is_page()
        ) {
            return $retval;
    ...
    public function filter_embed_html($retval)
    {
        ...
        $item_id = $this->is_page()
            ? $this->validate_url_to_item_id($url)
            : buddypress()->{$this->slug_endpoint}->embedid_in_progress;
        ...
        if (!empty($custom_title)) {
            ...
            $retval = substr_replace(
                $retval,
                esc_attr($custom_title),
                $title_pos,
                $title_end_pos - $title_pos
            );
        }
