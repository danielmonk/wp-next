...
class BP_Media_Extractor
{
    ...
    public function extract(
        ...
        if (self::LINKS & $what_to_extract) {
            $media = array_merge_recursive(
                $media,
                $this->extract_links($richtext, $plaintext, $extra_args)
            );
        }
        ...
        if (self::MENTIONS & $what_to_extract) {
            $media = array_merge_recursive(
                $media,
                $this->extract_mentions($richtext, $plaintext, $extra_args)
            );
        }
        ...
        if (self::IMAGES & $what_to_extract) {
            $media = array_merge_recursive(
                $media,
                $this->extract_images($richtext, $plaintext, $extra_args)
            );
        }
        ...
        if (self::SHORTCODES & $what_to_extract) {
            $media = array_merge_recursive(
                $media,
                $this->extract_shortcodes($richtext, $plaintext, $extra_args)
            );
        }
        ...
        if (self::EMBEDS & $what_to_extract) {
            $media = array_merge_recursive(
                $media,
                $this->extract_embeds($richtext, $plaintext, $extra_args)
            );
        }
        ...
        if (self::AUDIO & $what_to_extract) {
            $media = array_merge_recursive(
                $media,
                $this->extract_audio($richtext, $plaintext, $extra_args)
            );
        }
        ...
        if (self::VIDEOS & $what_to_extract) {
            $media = array_merge_recursive(
                $media,
                $this->extract_video($richtext, $plaintext, $extra_args)
            );
        }
     ...
     * }
     */
    protected function extract_links($richtext, $plaintext, $extra_args = [])
    {
        $data = ['has' => ['links' => 0], 'links' => []];
     ...
     * }
     */
    protected function extract_mentions($richtext, $plaintext, $extra_args = [])
    {
        $data = ['has' => ['mentions' => 0], 'mentions' => []];
        $mentions = [];
        ...
        foreach ($mentions as $user_id => $mention_name) {
            $mention = ['name' => strtolower($mention_name)];
    ...
    {
        $media = ['has' => ['images' => 0], 'images' => []];

        $featured_image = $this->extract_images_from_featured_images(
            $richtext,
            $plaintext,
            $extra_args
        );
        $galleries = $this->extract_images_from_galleries(
            $richtext,
            $plaintext,
            $extra_args
        );
     ...
     * }
     */
    protected function extract_shortcodes(
        $richtext,
        $plaintext,
        $extra_args = []
    ) {
        $data = ['has' => ['shortcodes' => 0], 'shortcodes' => []];
     ...
     * }
     */
    protected function extract_embeds($richtext, $plaintext, $extra_args = [])
    {
        $data = ['has' => ['embeds' => 0], 'embeds' => []];
        $embeds = [];
     ...
     * }
     */
    protected function extract_audio($richtext, $plaintext, $extra_args = [])
    {
        $data = ['has' => ['audio' => 0], 'audio' => []];
        ...
        // [audio]
        $audios = wp_list_filter($audios['shortcodes'], ['type' => 'audio']);
        foreach ($audios as $audio) {
            ...
            $path = untrailingslashit(
                parse_url($audio['attributes'][$src_param], PHP_URL_PATH)
            );
            ...
            foreach ($audio_types as $extension) {
                ...
                $data['audio'][] = [
                    'original' =>
                        '[audio src="' .
                        esc_url_raw($audio['attributes'][$src_param]) .
                        '"]',
                    'source' => 'shortcodes',
        ...
        foreach ($audio_types as $extension) {
            ...
            foreach ($links['links'] as $link) {
                ...
                $data['audio'][] = [
                    'original' =>
                        '[audio src="' . esc_url_raw($link['url']) . '"]', // Build an audio shortcode.
                    'source' => 'html',
     ...
     * }
     */
    protected function extract_video($richtext, $plaintext, $extra_args = [])
    {
        $data = ['has' => ['videos' => 0], 'videos' => []];
        $videos = $this->extract_shortcodes($richtext, $plaintext, $extra_args);

        $video_types = wp_get_video_extensions();

        // [video]
        $videos = wp_list_filter($videos['shortcodes'], ['type' => 'video']);
        foreach ($videos as $video) {
            ...
            $path = untrailingslashit(
                parse_url($video['attributes'][$src_param], PHP_URL_PATH)
            );
            ...
            foreach ($video_types as $extension) {
                ...
                if (
                    !$path ||
                    (substr($path, -4) !== $extension &&
                        substr($path, -5) !== $extension)
                ) {
                    continue;
                }
     ...
     * @return array
     */
    protected function extract_images_from_galleries(
        $richtext,
        $plaintext,
        $extra_args = []
    ) {
        if (
            !isset($extra_args['post']) ||
            !is_a($extra_args['post'], 'WP_Post')
        ) {
            $post = new WP_Post((object) ['post_content' => $richtext]);
        } else {
        ...
        if (!empty($galleries)) {
            ...
            if (isset($extra_args['width'])) {
                ...
                if (
                    !isset($extra_args['height']) &&
                    ctype_digit($extra_args['width'])
                ) {
                    $extra_args['height'] = round(
                        ($extra_args['width'] / 4) * 3
                    );
                }

                if (ctype_digit($extra_args['width'])) {
                    $image_size = [$extra_args['width'], $extra_args['height']];
                } else {
     ...
     * @return array
     */
    protected function extract_images_from_featured_images(
        $richtext,
        $plaintext,
        $extra_args
    ) {
    ...
    {
        $plaintext = strip_shortcodes(
            html_entity_decode(strip_tags($richtext))
        );
