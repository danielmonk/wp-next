...
class BP_User_Query
{
     ...
     * @var string
     */
    protected $no_results = ['join' => '', 'where' => '0 = 1'];
    ...
    public function __construct($query = null)
    {
        ...
        if (!empty($this->query_vars_raw)) {
            $this->query_vars = wp_parse_args($this->query_vars_raw, [
                'type' => 'newest',
                'per_page' => 0,
                'page' => 1,
                'user_id' => 0,
             ...
             */
            do_action_ref_array('bp_pre_user_query_construct', [&$this]);

            // Get user ids.
            // If the user_ids param is present, we skip the query.
            if (false !== $this->query_vars['user_ids']) {
                $this->user_ids = wp_parse_id_list(
                    $this->query_vars['user_ids']
                );
            } else {
    ...
    public function prepare_user_ids_query()
    {
        ...
        switch ($type) {
            ...
            case 'online':
                ...
                $sql[
                    'select'
                ] = "SELECT u.{$this->uid_name} as id FROM {$this->uid_table} u";
                $sql['where'][] = $wpdb->prepare(
                    "u.component = %s AND u.type = 'last_activity'",
                 ...
                 */
                $sql['where'][] = $wpdb->prepare(
                    "u.date_recorded >= DATE_SUB( UTC_TIMESTAMP(), INTERVAL %d MINUTE )",
                    apply_filters('bp_user_query_online_interval', 15)
                );
            ...
            case 'alphabetical':
                ...
                $sql['where'][] =
                    "u.{$this->uid_name} IN ( SELECT ID FROM {$wpdb->users} WHERE " .
        ...
        if (false !== $search_terms) {
            $search_terms = bp_esc_like(
                wp_kses_normalize_entities($search_terms)
            );
            ...
            $match_in_clause = empty($matched_user_ids)
                ? 'NULL'
                : implode(',', $matched_user_ids);
            $sql['where'][
                'search'
            ] = "u.{$this->uid_name} IN ({$match_in_clause})";
        }
        ...
        if (!empty($member_type__not_in)) {
            $member_type_clause = $this->get_sql_clause_for_member_types(
                $member_type__not_in,
                'NOT IN'
            );
        ...
        if (false !== $meta_key) {
            ...
            if (!empty($found_user_ids)) {
                $sql['where'][] =
                    "u.{$this->uid_name} IN (" .
                    implode(',', wp_parse_id_list($found_user_ids)) .
                    ")";
            } else {
                $sql['where'][] = '1 = 0';
            }
        ...
        if (!empty($per_page) && !empty($page)) {
            $sql['limit'] = $wpdb->prepare(
                "LIMIT %d, %d",
                intval(($page - 1) * $per_page),
                intval($per_page)
            );
        } else {
            $sql['limit'] = '';
         ...
         */
        $sql = apply_filters_ref_array('bp_user_query_uid_clauses', [
            $sql,
            &$this,
        ]);
        ...
        $this->uid_clauses['where'] = !empty($sql['where'])
         ...
         */
        do_action_ref_array('bp_pre_user_query', [&$this]);
    }
    ...
    public function do_user_ids_query()
    {
        ...
        if ('sql_calc_found_rows' == $this->query_vars['count_total']) {
            $this->uid_clauses['select'] = str_replace(
                'SELECT',
                'SELECT SQL_CALC_FOUND_ROWS',
                $this->uid_clauses['select']
            );
        }

        // Get the specific user ids.
        $this->user_ids = $wpdb->get_col(
            "{$this->uid_clauses['select']} {$this->uid_clauses['where']} {$this->uid_clauses['orderby']} {$this->uid_clauses['order']} {$this->uid_clauses['limit']}"
        );
        ...
        if ('sql_calc_found_rows' == $this->query_vars['count_total']) {
            ...
            $count_select = preg_replace(
                '/^SELECT.*?FROM (\S+) u/',
                "SELECT COUNT(u.{$this->uid_name}) FROM $1 u",
                $this->uid_clauses['select']
            );
            ...
            $this->total_users = $wpdb->get_var(
                apply_filters(
                    'bp_found_user_query',
                    "{$count_select} {$this->uid_clauses['where']}",
                    $this
                )
            );
        }
    ...
    public function populate_extras()
    {
         ...
         */
        do_action_ref_array('bp_user_query_populate_extras', [
            $this,
            $user_ids_sql,
        ]);
        ...
        foreach ($user_metas as $user_meta) {
            ...
            if (isset($this->results[$user_meta->user_id])) {
                $this->results[$user_meta->user_id]->{$key} =
        ...
        if (!empty($this->query_vars['meta_key'])) {
            ...
            $metas = $wpdb->get_results(
                "{$meta_sql['select']} {$meta_sql['from']} {$meta_sql['where']}"
            );

            if (!empty($metas)) {
                foreach ($metas as $meta) {
                    if (isset($this->results[$meta->user_id])) {
                        $this->results[$meta->user_id]->meta_key =
                            $meta->meta_key;

                        if (!empty($meta->meta_value)) {
                            $this->results[$meta->user_id]->meta_value =
                                $meta->meta_value;
                        }
                    }
                }
