    ...
    function start_el(&$output, $item, $depth = 0, $args = [], $id = 0)
    {
        ...
        $_nav_menu_placeholder =
            0 > $_nav_menu_placeholder
                ? intval($_nav_menu_placeholder) - 1
                : -1;
        $possible_object_id =
            isset($item->post_type) && 'nav_menu_item' == $item->post_type
                ? $item->object_id
                : $_nav_menu_placeholder;
        $possible_db_id =
            !empty($item->ID) && 0 < $possible_object_id ? (int) $item->ID : 0;
        ...
        if (
            !in_array(
                ['bp-menu', 'bp-' . $item->post_excerpt . '-nav'],
                $item->classes
            )
        ) {
        ...
        $output .=
            ...
            '" />';
        $output .=
            '<input type="hidden" class="menu-item-attr_title" name="menu-item[' .
            $possible_object_id .
            '][menu-item-attr_title]" value="' .
