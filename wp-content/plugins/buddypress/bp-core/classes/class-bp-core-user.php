...
class BP_Core_User
{
    ...
    public function populate()
    {
        ...
        if (!empty($this->profile_data)) {
            ...
            $this->user_url = bp_core_get_user_domain(
                $this->id,
                $this->profile_data['user_nicename'],
                $this->profile_data['user_login']
            );
            $this->fullname = esc_attr(
                $this->profile_data[$full_name_field_name]['field_data']
            );
        ...
        $this->last_active = bp_core_get_last_activity(
            bp_get_user_last_activity($this->id),
            _x('Active %s', 'last time the user was active', 'buddypress')
        );
    }
    ...
    public function populate_extras()
    {
        if (bp_is_active('friends')) {
            $this->total_friends = BP_Friends_Friendship::total_friend_count(
                $this->id
            );
        }

        if (bp_is_active('groups')) {
            $this->total_groups = BP_Groups_Member::total_group_count(
                $this->id
            );
    ...
    public static function get_users(
        ...
        $sql['select_main'] =
            "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.display_name, u.user_email";
        ...
        $sql[
            'from'
        ] = "FROM {$wpdb->users} u LEFT JOIN {$wpdb->usermeta} um ON um.user_id = u.ID";

        // We search against xprofile fields, so we must join the table.
        if ($search_terms && bp_is_active('xprofile')) {
            $sql[
                'join_profiledata_search'
            ] = "LEFT JOIN {$bp->profile->table_name_data} spd ON u.ID = spd.user_id";
        }
        ...
        if ($meta_key) {
            $sql[
                'join_meta'
            ] = "LEFT JOIN {$wpdb->usermeta} umm ON umm.user_id = u.ID";
        }

        $sql['where'] = 'WHERE ' . bp_core_get_status_sql('u.');

        if ('active' == $type || 'online' == $type || 'newest' == $type) {
            $sql['where_active'] = $wpdb->prepare(
                "AND um.meta_key = %s",
        ...
        if ('popular' == $type) {
            $sql['where_popular'] = $wpdb->prepare(
                "AND um.meta_key = %s",
                bp_get_user_meta_key('total_friend_count')
            );
        }

        if ('online' == $type) {
            $sql['where_online'] =
                "AND DATE_ADD( um.meta_value, INTERVAL 5 MINUTE ) >= UTC_TIMESTAMP()";
        }
        ...
        if (!empty($search_terms) && bp_is_active('xprofile')) {
            ...
            $sql['where_searchterms'] = $wpdb->prepare(
                "AND spd.value LIKE %s",
                $search_terms_like
            );
        }

        if (!empty($meta_key)) {
            $sql['where_meta'] = $wpdb->prepare(
                " AND umm.meta_key = %s",
                $meta_key
            );
        ...
        if (!empty($populate_extras)) {
            ...
            $paged_users = BP_Core_User::get_user_extras(
                $paged_users,
                $user_ids,
                $type
            );
        ...
        return ['users' => $paged_users, 'total' => $total_users];
    }
    ...
    public static function get_users_by_letter(
        ...
        $total_users_sql = apply_filters(
            ...
            $wpdb->prepare(
                "SELECT COUNT(DISTINCT u.ID) FROM {$wpdb->users} u LEFT JOIN {$bp->profile->table_name_data} pd ON u.ID = pd.user_id LEFT JOIN {$bp->profile->table_name_fields} pf ON pd.field_id = pf.id WHERE {$status_sql} AND pf.name = %s {$exclude_sql} AND pd.value LIKE %s ORDER BY pd.value ASC",
        ...
        $paged_users_sql = apply_filters(
            ...
            $wpdb->prepare(
                "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.user_email FROM {$wpdb->users} u LEFT JOIN {$bp->profile->table_name_data} pd ON u.ID = pd.user_id LEFT JOIN {$bp->profile->table_name_fields} pf ON pd.field_id = pf.id WHERE {$status_sql} AND pf.name = %s {$exclude_sql} AND pd.value LIKE %s ORDER BY pd.value ASC{$pag_sql}",
                bp_xprofile_fullname_field_name(),
                $letter_like
            )
        );
     ...
     */
    public static function get_specific_users(
        $user_ids,
        $limit = null,
        $page = 1,
        $populate_extras = true
        ...
        $paged_users_sql = apply_filters(
            'bp_core_get_specific_users_count_sql',
            "SELECT ID as id, user_registered, user_nicename, user_login, user_email FROM {$wpdb->users} WHERE {$status_sql} AND ID IN ({$user_ids}) {$pag_sql}"
        );
    ...
    public static function search_users(
        ...
        $total_users_sql = apply_filters(
            ...
            $wpdb->prepare(
                "SELECT COUNT(DISTINCT u.ID) as id FROM {$wpdb->users} u LEFT JOIN {$bp->profile->table_name_data} pd ON u.ID = pd.user_id WHERE {$status_sql} AND pd.value LIKE %s ORDER BY pd.value ASC",
                $search_terms_like
            ),
            $search_terms
        );
        ...
        $paged_users_sql = apply_filters(
            ...
            $wpdb->prepare(
                "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.user_email FROM {$wpdb->users} u LEFT JOIN {$bp->profile->table_name_data} pd ON u.ID = pd.user_id WHERE {$status_sql} AND pd.value LIKE %s ORDER BY pd.value ASC{$pag_sql}",
                $search_terms_like
            ),
            $search_terms,
            $pag_sql
     ...
     */
    public static function get_user_extras(
        &$paged_users,
        &$user_ids,
        $type = false
    ) {
        ...
        if (bp_is_active('xprofile') && 'alphabetical' != $type) {
            $names = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT pd.user_id as id, pd.value as fullname FROM {$bp->profile->table_name_fields} pf, {$bp->profile->table_name_data} pd WHERE pf.id = pd.field_id AND pf.name = %s AND pd.user_id IN ( {$user_ids} )",
                    bp_xprofile_fullname_field_name()
                )
            );
        ...
        if ('popular' != $type) {
            $friend_count = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT user_id as id, meta_value as total_friend_count FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )",
                    bp_get_user_meta_key('total_friend_count')
                )
            );
            for ($i = 0, $count = count($paged_users); $i < $count; ++$i) {
                foreach ((array) $friend_count as $fcount) {
                    if ($fcount->id == $paged_users[$i]->id) {
                        $paged_users[$i]->total_friend_count =
                            (int) $fcount->total_friend_count;
                    }
                }
            }
        }
        ...
        if (bp_is_active('friends')) {
            ...
            for ($i = 0, $count = count($paged_users); $i < $count; ++$i) {
                foreach ((array) $friend_status as $status) {
                    if (
                        $status->initiator_user_id == $paged_users[$i]->id ||
                        $status->friend_user_id == $paged_users[$i]->id
                    ) {
                ...
                foreach ((array) $user_activity as $activity) {
                    if (
                        ...
                        (int) $activity['user_id'] ===
                            (int) $paged_users[$i]->id
                    ) {
                        $paged_users[$i]->last_activity =
        ...
        $user_update = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT user_id as id, meta_value as latest_update FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )",
    ...
    public static function get_last_activity($user_id)
    {
        ...
        $uncached_user_ids = bp_get_non_cached_ids(
            $user_ids,
            'bp_last_activity'
        );
        if (!empty($uncached_user_ids)) {
            ...
            $last_activities = $wpdb->get_results(
                $wpdb->prepare(
        ...
        foreach ($user_ids as $user_id) {
            ...
            if (isset($retval['user_id'])) {
                $retval[$user_id]['user_id'] =
                    (int) $retval[$user_id]['user_id'];
            }
            if (isset($retval['activity_id'])) {
                $retval[$user_id]['activity_id'] =
                    (int) $retval[$user_id]['activity_id'];
            }
        }
