...
class BP_Email_Recipient extends BP_Email_Participant
{
    ...
    public function __construct($email_or_user, $name = '')
    {
        ...
        if (
            is_int($email_or_user) ||
            (is_string($email_or_user) && is_email($email_or_user)) ||
            is_object($email_or_user)
        ) {
            ...
            if (!is_array($email_or_user)) {
                $email_or_user = [$email_or_user => $name];
            }
        ...
        if ($this->user_object) {
            ...
            $wp_name = wp_specialchars_decode(
                bp_core_get_user_displayname($this->user_object->ID),
                ENT_QUOTES
            );
