...
class BP_Email
{
    ...
    public function __construct($email_type)
    {
        ...
        $from_name = wp_specialchars_decode(
    ...
    public function get($property_name, $transform = 'raw')
    {
        ...
        if ($property_name === 'content') {
            ...
            if (
                !in_array(
                    $property_name,
                    ['content_html', 'content_plaintext'],
                    true
                )
            ) {
        ...
        switch ($transform) {
            ...
            case 'add-content':
                $retval = str_replace(
                    '{{{content}}}',
                    wpautop($this->get_content('replace-tokens')),
                    $retval
                );
            // Fall through.

            case 'replace-tokens':
                $retval = bp_core_replace_tokens_in_text(
                    $retval,
                    $this->get_tokens('raw')
                );
            // Fall through.
    ...
    public function get_preheader()
    {
        ...
        if ($post) {
            ...
            $preheader = sanitize_text_field(
                get_post_meta($post->ID, 'bp_email_preheader', true)
            );
    ...
    {
        $bcc = $operation !== 'replace' ? $this->bcc : [];
    ...
    public function set_content_html($content)
    {
         ...
         * @param BP_Email $this Current instance of the email type class.
         */
        $this->content_html = apply_filters(
    ...
    public function set_content_plaintext($content)
    {
         ...
         * @param BP_Email $this Current instance of the email type class.
         */
        $this->content_plaintext = apply_filters(
    ...
    {
        if (!in_array($content_type, ['html', 'plaintext'], true)) {
    ...
    public function set_post_object(WP_Post $post)
    {
         ...
         * @param BP_Email $this Current instance of the email type class.
         */
        $this->post_object = apply_filters(
        ...
        if (is_a($this->post_object, 'WP_Post')) {
            ...
            bp_locate_template(
                bp_email_get_template($this->post_object),
                true,
                false
            );
