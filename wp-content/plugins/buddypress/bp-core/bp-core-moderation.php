...
function bp_core_check_for_flood($user_id = 0)
{
    ...
    if (
        isset($last_posted) &&
        time() < $last_posted + $throttle_time &&
        !current_user_can('throttle')
    ) {
        return false;
    }
 ...
 */
function bp_core_check_for_moderation(
    $user_id = 0,
    $title = '',
    $content = '',
    $error_type = 'bool'
    ...
    if (!empty($max_links)) {
        ...
        if ($num_links >= $max_links) {
            if ('bool' === $error_type) {
                ...
                return new WP_Error(
                    'bp_moderation_too_many_links',
                    __('You have posted too many links', 'buddypress')
                );
            }
        }
    }
            ...
            foreach ($_post as $post_data) {
                ...
                if (preg_match($pattern, $post_data)) {
                    if ('bool' === $error_type) {
                        ...
                        return new WP_Error(
                            'bp_moderation_word_match',
                            _x(
                                'You have posted an inappropriate word.',
                                'Comment moderation',
                                'buddypress'
                            )
                        );
 ...
 * @return bool|WP_Error True if test is passed, false if fail.
 */
function bp_core_check_for_disallowed_keys(
    $user_id = 0,
    $title = '',
    $content = '',
    $error_type = 'bool'
    ...
    if (
        apply_filters_deprecated(
            ...
            [false, $user_id, $title, $content],
            '7.0.0',
            'bp_bypass_check_for_disallowed_keys'
        )
    ) {
        return true;
    }
        ...
        foreach ($_post as $post_data) {
            ...
            if (preg_match($pattern, $post_data)) {
                if ('bool' === $error_type) {
                    ...
                    return new WP_Error(
                        'bp_moderation_disallowed_key_match',
                        _x(
                            'You have posted an inappropriate word.',
                            'Comment disallowed key',
                            'buddypress'
                        )
                    );
                }
