...
(function () {
  ...
  bp.WebCam = {
    ...
    setView: function (view) {
      ...
      var cameraView = new bp.Views.WebCamAvatar({
        model: new Backbone.Model({ user_media: false }),
      });
  ...
  bp.Views.WebCamAvatar = bp.View.extend({
    ...
    captureStream: function (event) {
      ...
      if (
        this.model.get("h") > this.options.video.el.videoHeight ||
        this.model.get("w") > this.options.video.el.videoWidth
      ) {
        bp.WebCam.displayWarning("videoerror");
        return;
      }
      ...
      if (!bp.WebCam.params.flipped) {
        this.options.canvas.el
          .getContext("2d")
          .translate(this.model.get("w"), 0);
