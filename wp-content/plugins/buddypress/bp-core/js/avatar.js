...
(function (exports, $) {
  ...
  bp.Avatar = {
    ...
    removeLegacyUI: function () {
      ...
      if ($("#avatar-upload-form").length) {
        ...
        $("#bp_members_user_admin_avatar a.bp-members-avatar-user-admin").length
      ) {
        $(
          "#bp_members_user_admin_avatar a.bp-members-avatar-user-admin"
        ).remove();
      }
    },
    ...
    uploadProgress: function () {
      ...
      var avatarStatus = new bp.Views.uploaderStatus({
        collection: bp.Uploader.filesQueue,
      });
      ...
      bp.ajax
        ...
        .done(function (response) {
          var avatarStatus = new bp.Views.AvatarStatus({
            value:
              BP_Uploader.strings.feedback_messages[response.feedback_code],
            type: "success",
          });
          ...
          $(
            "." + avatar.get("object") + "-" + response.item_id + "-avatar"
          ).each(function () {
           ...
           */
          self.Attachment.set(
            _.extend(_.pick(avatar.attributes, ["object", "item_id"]), {
              url: response.avatar,
              action: "uploaded",
            })
          );
    ...
    deleteAvatar: function (model) {
      ...
      bp.ajax
        ...
        .done(function (response) {
          ...
          // Update each avatars of the page.
          $(
            "." + model.get("object") + "-" + response.item_id + "-avatar"
          ).each(function () {
            $(this).prop("src", response.avatar);
          });
           ...
           */
          self.Attachment.set(
            _.extend(_.pick(model.attributes, ["object", "item_id"]), {
              url: response.avatar,
              action: "deleted",
            })
          );
  ...
  bp.Views.Avatars = bp.View.extend({
    ...
    addItemView: function (item) {
      ...
      if (
        !_.isUndefined(BP_Uploader.settings.crop.full_h) &&
        !_.isUndefined(BP_Uploader.settings.crop.full_w)
      ) {
  ...
  bp.Views.Avatar = bp.View.extend({
    ...
    initCropper: function () {
      ...
      if (
        !_.isUndefined(this.options.full_h) &&
        !_.isUndefined(this.options.full_w)
      ) {
