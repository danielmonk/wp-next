...
(function (exports, $) {
  ...
  bp.Uploader.uploader = function () {
    ...
    if (
      !isIE &&
      "flash" === plupload.predictRuntime(this.params.defaults) &&
      (!this.params.defaults.required_features ||
        !this.params.defaults.required_features.hasOwnProperty(
          "send_binary_string"
        ))
    ) {
      this.params.defaults.required_features =
    ...
    this.uploader.bind("FilesAdded", function (uploader, files) {
      ...
      if (!uploader.settings.multi_selection && files.length > 1) {
        ...
        $(self).trigger(
          "bp-uploader-warning",
          self.strings.unique_file_warning
        );
        return;
      }
    ...
    this.uploader.bind("Error", function (uploader, pluploadError) {
      var message = self.strings.default_error,
        ...
        errors = {
          ...
          FILE_SIZE_ERROR: self.strings.file_exceeds_size_limit.replace(
            "%s",
            $("<span />").text(pluploadError.file.name).html()
          ),
        };
  ...
  bp.Views.uploaderStatus = bp.View.extend({
    ...
    progress: function (model) {
      if (!_.isUndefined(model.get("percent"))) {
        $("#" + model.get("id") + " .bp-progress .bp-bar").css(
          "width",
          model.get("percent") + "%"
        );
      }
    ...
    feedback: function (model) {
      if (
        !_.isUndefined(model.get("message")) &&
        !_.isUndefined(model.get("file"))
      ) {
        $("#" + model.get("file").id)
