...
function bp_core_set_avatar_constants()
{
    ...
    if (!defined('BP_AVATAR_ORIGINAL_MAX_FILESIZE')) {
        define(
            ...
            bp_attachments_get_max_upload_file_size('avatar')
        );
    }
...
function bp_core_fetch_avatar($args = '')
{
    ...
    if (empty($params['avatar_dir'])) {
         ...
         * @param array  $params Array of parameters for the request.
         */
        $params['avatar_dir'] = apply_filters(
            'bp_core_avatar_dir',
            $params['avatar_dir'],
            $params['object'],
            $params
    ...
    if (
        false !== strpos($params['alt'], '%s') ||
        false !== strpos($params['alt'], '%1$s')
    ) {
        switch ($params['object']) {
            case 'blog':
            ...
            case 'group':
                $item_name = bp_get_group_name(
                    groups_get_group($params['item_id'])
                );
                break;
    ...
    // Merge classes.
    $avatar_classes = array_merge($avatar_classes, [
     ...
     */
    $avatar_folder_url = apply_filters(
        'bp_core_avatar_folder_url',
        $avatar_loc->url . $avatar_loc->dir . $params['item_id'],
        $params['item_id'],
        $params['object'],
        $params['avatar_dir']
    );
    ...
    $avatar_folder_dir = apply_filters(
        ...
        $avatar_loc->path . $avatar_loc->dir . $params['item_id'],
        $params['item_id'],
        $params['object'],
        $params['avatar_dir']
    );
    ...
    $legacy_user_avatar_name =
        'full' == $params['type'] ? '-avatar2' : '-avatar1';
    $legacy_group_avatar_name =
        'full' == $params['type'] ? '-groupavatar-full' : '-groupavatar-thumb';
        ...
        if ($av_dir = opendir($avatar_folder_dir)) {
            ...
            if (0 < count($avatar_files)) {
                ...
                if (!isset($avatar_url)) {
                    foreach ($avatar_files as $key => $value) {
                        if (
                            strpos($value, $legacy_user_avatar_name) !== false
                        ) {
                            $avatar_url =
                                $avatar_folder_url . '/' . $avatar_files[$key];
                        }
                    ...
                    if (!isset($avatar_url)) {
                        foreach ($avatar_files as $key => $value) {
                            if (
                                strpos($value, $legacy_group_avatar_name) !==
                                false
                            ) {
                                $avatar_url =
                                    $avatar_folder_url .
    ...
    if (
        ...
        $gravatar = esc_url(
            add_query_arg(rawurlencode_deep(array_filter($url_args)), $gravatar)
        );
...
function bp_core_delete_existing_avatar($args = '')
{
    ...
    if ($av_dir = opendir($avatar_folder_dir)) {
        while (false !== ($avatar_file = readdir($av_dir))) {
            if (
                (preg_match("/-bpfull/", $avatar_file) ||
                    preg_match("/-bpthumb/", $avatar_file)) &&
                '.' != $avatar_file &&
                '..' != $avatar_file
            ) {
...
function bp_core_avatar_handle_upload($file, $upload_dir_filter)
{
    ...
    $bp->avatar_admin->original = $avatar_attachment->upload(
        $file,
        $upload_dir_filter
    );
    ...
    if (!empty($bp->avatar_admin->original['error'])) {
        ...
        bp_core_add_message(
            sprintf(
                ...
                $bp->avatar_admin->original['error']
            ),
            'error'
        );
        return false;
    ...
    if (empty($bp->avatar_admin->resized)) {
        ...
        $bp->avatar_admin->image->dir = str_replace(
            $upload_path,
            '',
            $bp->avatar_admin->original['file']
        );
    ...
    if (is_wp_error($bp->avatar_admin->image->dir)) {
        ...
        bp_core_add_message(
            sprintf(
                __('Upload failed! Error was: %s', 'buddypress'),
                $bp->avatar_admin->image->dir->get_error_message()
            ),
            'error'
        );
        return false;
    ...
    $bp->avatar_admin->image->url =
        bp_core_avatar_url() . $bp->avatar_admin->image->dir;
...
function bp_avatar_ajax_upload()
{
    ...
    if (isset($bp_params['ui_available_width'])) {
        $bp->avatar_admin->ui_available_width =
            (int) $bp_params['ui_available_width'];
    }

    // Upload the avatar.
    $avatar = bp_core_avatar_handle_upload(
        $_FILES,
        $bp_params['upload_dir_filter']
    );
...
function bp_avatar_handle_capture($data = '', $item_id = 0)
{
    ...
    $original_file =
        $avatar_folder_dir . '/webcam-capture-' . $item_id . '.png';

    if (file_put_contents($original_file, $data)) {
        $avatar_to_crop = str_replace(
...
function bp_core_avatar_handle_crop($args = '')
{
    ...
    if (
        empty($cropped['full']) ||
        empty($cropped['thumb']) ||
        is_wp_error($cropped['full']) ||
        is_wp_error($cropped['thumb'])
    ) {
        return false;
    }
...
function bp_avatar_ajax_set()
{
    ...
    $avatar_data = wp_parse_args($_POST, [
        'crop_w' => bp_core_avatar_full_width(),
        'crop_h' => bp_core_avatar_full_height(),
        'crop_x' => 0,
        'crop_y' => 0,
    ...
    if (
        empty($avatar_data['object']) ||
        empty($avatar_data['item_id']) ||
        empty($avatar_data['original_file'])
    ) {
        wp_send_json_error();
    }
    ...
    if (
        !empty($avatar_data['type']) &&
        'camera' === $avatar_data['type'] &&
        'user' === $avatar_data['object']
    ) {
        $webcam_avatar = false;

        if (!empty($avatar_data['original_file'])) {
            $webcam_avatar = str_replace(
                ['data:image/png;base64,', ' '],
                ['', '+'],
                $avatar_data['original_file']
            );
            $webcam_avatar = base64_decode($webcam_avatar);
        }

        if (
            !bp_avatar_handle_capture($webcam_avatar, $avatar_data['item_id'])
        ) {
            wp_send_json_error([
                'feedback_code' => 1,
            ]);
    ...
    if (bp_core_avatar_handle_crop($r)) {
        ...
        if ('user' === $avatar_data['object']) {
            ...
            do_action_deprecated(
                'xprofile_avatar_uploaded',
                [(int) $avatar_data['item_id'], $avatar_data['type'], $r],
                '6.0.0',
                'bp_members_avatar_uploaded'
            );
...
{
    return bp_attachments_check_filetype(
        $file['file']['tmp_name'],
        $file['file']['name'],
...
{
    $bp = buddypress();
    $dim = isset($bp->avatar->{$type}->{$h_or_w})
        ? (int) $bp->avatar->{$type}->{$h_or_w}
        : false;
...
function bp_core_avatar_thumb_width()
{
    ...
    return apply_filters(
        ...
        bp_core_avatar_dimension('thumb', 'width')
    );
}
...
function bp_core_avatar_thumb_height()
{
    ...
    return apply_filters(
        ...
        bp_core_avatar_dimension('thumb', 'height')
    );
}
...
function bp_core_avatar_full_width()
{
    ...
    return apply_filters(
        'bp_core_avatar_full_width',
        bp_core_avatar_dimension('full', 'width')
    );
}
...
function bp_core_avatar_full_height()
{
    ...
    return apply_filters(
        'bp_core_avatar_full_height',
        bp_core_avatar_dimension('full', 'height')
    );
}
...
function bp_core_avatar_original_max_filesize()
{
    ...
    return apply_filters(
        ...
        (int) buddypress()->avatar->original_max_filesize
    );
}
...
function bp_core_avatar_default($type = 'gravatar', $params = [])
{
    ...
    if (defined('BP_AVATAR_DEFAULT')) {
        ...
        $avatar =
            buddypress()->plugin_url . "bp-core/images/mystery-man{$size}.jpg";
        ...
        $avatar =
            '//www.gravatar.com/avatar/00000000000000000000000000000000?d=mm&amp;s=' .
            $size;
    }
 ...
 */
function bp_core_avatar_default_thumb($type = 'gravatar', $params = [])
{
    // Local override.
    if (defined('BP_AVATAR_DEFAULT_THUMB')) {
        ...
        $avatar =
            buddypress()->plugin_url . 'bp-core/images/mystery-man-50.jpg';
...
function bp_avatar_is_front_edit()
{
    ...
    if (
        ...
        'crop-image' !== bp_get_avatar_admin_step()
    ) {
