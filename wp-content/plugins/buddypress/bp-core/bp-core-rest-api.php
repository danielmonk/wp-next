...
{
    $is_src =
        defined('BP_SOURCE_SUBDIRECTORY') && BP_SOURCE_SUBDIRECTORY === 'src';
...
function bp_rest_api_is_available()
{
    ...
    return apply_filters(
        ...
        bp_is_running_wp('4.7.0') && bp_rest_in_buddypress()
...
function bp_rest_api_register_request_script()
{
    ...
    wp_register_script(
        'bp-api-request',
        sprintf(
            '%1$sbp-core/js/bp-api-request%2$s.js',
    ...
    wp_localize_script('bp-api-request', 'bpApiSettings', [
 ...
 */
function bp_rest_response_add_total_headers(
 ...
 */
function bp_rest_register_field(
    $component_id,
    $attribute,
    $args = [],
    $object_type = ''
    ...
    if ('xprofile' === $component_id) {
        if (!in_array($object_type, ['data', 'field', 'group'], true)) {
            return $registered_fields;
        }
