 ...
 */
function bp_attachments_cover_image_upload_dir($args = [])
{
    ...
    // Set the subdir.
    $subdir =
        '/' . $r['object_directory'] . '/' . $r['object_id'] . '/cover-image';
 ...
 */
function bp_attachments_get_allowed_mimes($type = '', $allowed_types = [])
{
    if (empty($allowed_types)) {
        $allowed_types = bp_attachments_get_allowed_types($type);
    }

    $validate_mimes = wp_match_mime_types(
        join(',', $allowed_types),
        wp_get_mime_types()
    );
 ...
 */
function bp_attachments_create_item_type($type = 'avatar', $args = [])
{
    ...
    if ('avatar' === $type) {
        ...
        if (is_callable($r['component'] . '_avatar_upload_dir')) {
            ...
            if ('members' === $r['component']) {
                $dir_args = [false, $r['item_id']];
            }

            $attachment_data = call_user_func_array(
                $r['component'] . '_avatar_upload_dir',
                $dir_args
            );
        }
        ...
        // Set Subdir.
        $attachment_data['subdir'] =
            $object_subdir . '/' . $r['item_id'] . '/cover-image';

        // Set Path.
        $attachment_data['path'] =
            trailingslashit($attachment_data['basedir']) .
            $attachment_data['subdir'];
    }

    if (
        !isset($attachment_data['path']) ||
        !isset($attachment_data['subdir'])
    ) {
        return false;
    }
    ...
    $image_file_name = wp_unique_filename(
        $attachment_data['path'],
        basename($r['image'])
    );
    ...
    if ('avatar' === $type) {
        $created = bp_core_avatar_handle_crop([
            ...
            'original_file' =>
                trailingslashit($attachment_data['subdir']) . $image_file_name,
            'crop_w' => $r['crop_w'],
            'crop_h' => $r['crop_h'],
            'crop_x' => $r['crop_x'],
            'crop_y' => $r['crop_y'],
 ...
 */
function bp_attachments_get_attachment($data = 'url', $args = [])
{
    // Default value.
    ...
    $type_dir =
        trailingslashit($bp_attachments_uploads_dir['basedir']) . $type_subdir;
    ...
    if (!empty($r['file'])) {
        ...
        if ($att_dir = opendir($type_dir)) {
            while (false !== ($attachment_file = readdir($att_dir))) {
                ...
                if (
                    false !== strpos($attachment_file, $r['type']) &&
                    empty($file)
                ) {
 ...
 */
function bp_attachments_delete_file($args = [])
{
...
function bp_attachments_get_plupload_l10n()
{
     ...
     */
    return apply_filters('bp_attachments_get_plupload_l10n', [
        ...
        'unique_file_warning' => __(
            'Make sure to upload a unique file',
            'buddypress'
        ),
...
function bp_attachments_enqueue_scripts($class = '')
{
    ...
    $ui_args = array_intersect_key($args, [
        'file_data_name' => true,
        'browse_button' => true,
        'container' => true,
        'drop_element' => true,
    ...
    if ('bp_avatar_upload' === $defaults['multipart_params']['action']) {
        ...
        if (bp_avatar_use_webcam() && 'user' === $object) {
            ...
            $strings['camera_warnings'] = [
                'requesting' => __(
                    'Please allow us to access to your camera.',
                    'buddypress'
                ),
                'loading' => __(
                    'Please wait as we access your camera.',
                    'buddypress'
                ),
                'loaded' => __(
                    'Camera loaded. Click on the "Capture" button to take your photo.',
                    'buddypress'
                ),
                'noaccess' => __(
                    'It looks like you do not have a webcam or we were unable to get permission to use your webcam. Please upload a photo instead.',
                    'buddypress'
                ),
                'errormsg' => __(
                    'Your browser is not supported. Please upload a photo instead.',
                    'buddypress'
                ),
                'videoerror' => __(
         ...
         */
        $settings['nav'] = bp_sort_by_key(
            apply_filters('bp_attachments_avatar_nav', $avatar_nav, $object),
            'order',
            'num'
        );
        ...
        'bp_cover_image_upload' === $defaults['multipart_params']['action']
    ) {
        ...
        if ('group' === $object) {
            ...
            $cover_component = apply_filters(
                'bp_attachments_cover_image_ui_component',
                $cover_component
            );
        }
        ...
        $strings['cover_image_warnings'] = apply_filters(
            ...
            [
                'dimensions' => sprintf(
                    ...
                    __(
                        'For better results, make sure to upload an image that is larger than %1$spx wide, and %2$spx tall.',
                        'buddypress'
                    ),
    ...
    wp_localize_script('bp-plupload', 'BP_Uploader', [
        'strings' => $strings,
        'settings' => $settings,
    ]);
 ...
 * @return bool
 */
function bp_attachments_current_user_can($capability, $args = [])
{
    $can = false;
        ...
        if (isset($args['item_id']) && isset($args['object'])) {
            ...
            if (bp_is_active('groups') && 'group' === $args['object']) {
                if (bp_is_group_create()) {
                    $can =
                        (bool) groups_is_user_creator(
                            bp_loggedin_user_id(),
                            $args['item_id']
                        ) || bp_current_user_can('bp_moderate');
                } else {
                    $can =
                        (bool) groups_is_user_admin(
                            bp_loggedin_user_id(),
                            $args['item_id']
                        ) || bp_current_user_can('bp_moderate');
                }
                ...
                $can =
                    bp_loggedin_user_id() === (int) $args['item_id'] ||
                    bp_current_user_can('bp_moderate');
            }
            /**
...
function bp_attachments_get_cover_image_settings($component = 'members')
{
    ...
    // Set default args.
    $default_args = wp_parse_args($args, [
        'components' => [],
        'width' => 1300,
        'height' => 225,
        'callback' => '',
    ...
    if ('members' === $component) {
        ...
        $settings = apply_filters_deprecated(
            'bp_after_xprofile_cover_image_settings_parse_args',
            [$settings],
            '6.0.0',
            'bp_after_members_cover_image_settings_parse_args'
        );
    }

    if (
        empty($settings['components']) ||
        empty($settings['callback']) ||
        empty($settings['theme_handle'])
    ) {
        return false;
    }
...
function bp_attachments_cover_image_is_edit()
{
    ...
    if (
        (bp_is_group_admin_page() &&
            'group-cover-image' == bp_get_group_current_admin_tab()) ||
...
function bp_attachments_get_user_has_cover_image($user_id = 0)
{
    ...
    $cover_src = bp_attachments_get_attachment('url', [
        'item_id' => $user_id,
    ]);
 ...
 */
function bp_attachments_cover_image_generate_file(
    $args = [],
    $cover_image_class = null
) {
    // Bail if an argument is missing.
    if (
        empty($args['file']) ||
        empty($args['component']) ||
        empty($args['cover_image_dir'])
    ) {
        return false;
    }
    ...
    if (
        false === $dimensions ||
        $args['file'] !==
            $args['cover_image_dir'] . '/' . wp_basename($args['file'])
    ) {
        return false;
    }
    ...
    if (empty($cover_image)) {
        ...
        if (
            $cover_image['width'] < $dimensions['width'] ||
            $cover_image['height'] < $dimensions['height']
        ) {
            $is_too_small = true;
        }
    }
    ...
    if ($att_dir = opendir($args['cover_image_dir'])) {
        while (false !== ($attachment_file = readdir($att_dir))) {
            ...
            if (
                2 < strlen($attachment_file) &&
                0 !== strpos($attachment_file, '.') &&
                $cover_basename !== $attachment_file
            ) {
...
function bp_attachments_cover_image_ajax_upload()
{
    ...
    if ('user' === $bp_params['object']) {
        $object_data = ['dir' => 'members', 'component' => 'members'];
        ...
        $object_data = ['dir' => 'groups', 'component' => 'groups'];
        ...
        $object_data = apply_filters(
            'bp_attachments_cover_image_object_dir',
            [],
            $bp_params['object']
        );
    }
    ...
    if (!empty($needs_reset)) {
        if (!empty($needs_reset['component'])) {
            $bp->{$needs_reset['component']}->{$needs_reset['key']} =
                $needs_reset['value'];
        } else {
    ...
    $cover_subdir =
        $object_data['dir'] . '/' . $bp_params['item_id'] . '/cover-image';
    $cover_dir =
        trailingslashit($bp_attachments_uploads_dir['basedir']) . $cover_subdir;
    ...
    if ('members' === $component) {
        ...
        do_action_deprecated(
            ...
            [$item_id, $name, $cover_url, $feedback_code],
            '6.0.0',
    ...
    bp_attachments_json_response(true, $is_html4, [
        'name' => $name,
        'url' => $cover_url,
...
function bp_attachments_cover_image_ajax_delete()
{
    ...
    if (
        empty($_POST['object']) ||
        empty($_POST['item_id']) ||
        (!ctype_digit($_POST['item_id']) && !is_int($_POST['item_id']))
    ) {
        wp_send_json_error();
    }
