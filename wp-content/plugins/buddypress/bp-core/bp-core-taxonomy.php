...
function bp_register_default_taxonomies()
{
    ...
    foreach ($taxonomies as $taxonomy_name => $taxonomy_params) {
        if (
            !isset($taxonomy_params['object']) ||
            !isset($taxonomy_params['args'])
        ) {
            continue;
        }
 ...
 * @return array
 */
function bp_get_object_terms($object_ids, $taxonomies, $args = [])
{
 ...
 */
function bp_get_objects_in_term($term_ids, $taxonomies, $args = [])
{
 ...
 *                        WP_Error otherwise.
 */
function bp_insert_term($term, $taxonomy = '', $args = [])
{
    if (!taxonomy_exists($taxonomy)) {
        return new WP_Error(
            'invalid_taxonomy',
            __('Invalid taxonomy.', 'buddypress')
        );
    }
...
function bp_get_terms($args = [])
{
     ...
     */
    return apply_filters('bp_get_terms', $terms, $args);
}
...
function bp_delete_term($term_id = 0, $taxonomy = '')
{
    if (!$term_id || !$taxonomy) {
        return new WP_Error(
            'missing_arguments',
            __(
                'Sorry, the term ID and the taxonomy are required arguments.',
                'buddypress'
            )
        );
    }
    ...
    if (0 === $deleted) {
        return new WP_Error(
            'default_term',
            __('Sorry, the default term cannot be deleted.', 'buddypress')
        );
    }
