...
function bp_core_install_extended_profiles()
{
    ...
    if (!bp_get_option('bp-xprofile-base-group-name')) {
        bp_update_option(
            ...
            _x('General', 'First field-group name', 'buddypress')
        );
    }
    ...
    if (!bp_get_option('bp-xprofile-fullname-field-name')) {
        bp_update_option(
            ...
            _x('Display Name', 'Display name field', 'buddypress')
        );
    }
    ...
    if (
        ...
        $insert_sql[] =
            "INSERT INTO {$bp_prefix}bp_xprofile_groups ( name, description, can_delete ) VALUES ( " .
            $wpdb->prepare(
                '%s',
                stripslashes(bp_get_option('bp-xprofile-base-group-name'))
            ) .
            ", '', 0 );";
    }
    ...
    if (
        ...
        $insert_sql[] =
            "INSERT INTO {$bp_prefix}bp_xprofile_fields ( group_id, parent_id, type, name, description, is_required, can_delete ) VALUES ( 1, 0, 'textbox', " .
            $wpdb->prepare(
                '%s',
                stripslashes(bp_get_option('bp-xprofile-fullname-field-name'))
            ) .
            ", '', 1, 0 );";
        ...
        if (
            !$wpdb->get_var(
                "SELECT id FROM {$bp_prefix}bp_xprofile_meta WHERE id = 1"
            )
        ) {
...
function bp_core_install_emails()
{
    ...
    foreach ($emails as $id => $email) {
        ...
        if (
            !is_multisite() &&
            isset($email['args']) &&
            !empty($email['args']['multisite'])
        ) {
            continue;
        }

        $post_id = wp_insert_post(
            bp_parse_args($email, $defaults, 'install_email_' . $id)
        );
        if (!$post_id) {
            continue;
        }
        ...
        foreach ($tt_ids as $tt_id) {
            ...
            wp_update_term((int) $term->term_id, bp_get_email_tax_type(), [
