...
function bp_core_admin_tools()
{
	...
	<div class="wrap">
      ...
      'buddypress'
		...
		<p><?php esc_html_e(
      'BuddyPress keeps track of various relationships between members, groups, and activity items.',
      'buddypress'
  ); ?></p>
		<p><?php esc_html_e(
      'Occasionally these relationships become out of sync, most often after an import, update, or migration.',
      'buddypress'
  ); ?></p>
		<p><?php esc_html_e(
				...
				<?php foreach (bp_admin_repair_list() as $item): ?>
					<p>
						<label for="<?php echo esc_attr(str_replace('_', '-', $item[0])); ?>">
							<input type="checkbox" class="checkbox" name="<?php echo esc_attr($item[0]) .
           '" id="' .
           esc_attr(
               str_replace('_', '-', $item[0])
           ); ?>" value="1" /> <?php echo esc_html($item[1]); ?>
						</label>
					</p>
				<?php endforeach; ?>

				<p class="submit">
					<input class="button-primary" type="submit" name="bp-tools-submit" value="<?php esc_attr_e(
         'Repair Items',
         'buddypress'
     ); ?>" />
...
function bp_admin_repair_handler()
{
    ...
    $capability = bp_core_do_network_admin()
    ...
    if (bp_is_active('blogs')) {
        ...
        if (is_multisite() && bp_is_active('blogs', 'site-icon')) {
            $repair_list[91] = [
                'bp-blog-site-icons',
                __(
                    'Repair site tracking site icons/blog avatars synchronization.',
                    'buddypress'
                ),
...
function bp_admin_repair_friend_count()
{
    ...
    $statement = __(
        'Counting the number of friends for each user&hellip; %s',
        'buddypress'
    );
    ...
    $total_users = $wpdb->get_row("SELECT count(ID) as c FROM {$wpdb->users}")
        ->c;
    ...
    return [0, sprintf($statement, __('Complete!', 'buddypress'))];
}
...
function bp_admin_repair_blog_site_icons()
{
    /* translators: %s: the result of the action performed by the repair tool */
    $statement = __(
        'Repairing site icons/blog avatars synchronization&hellip; %s',
        'buddypress'
    );

    if (!is_multisite()) {
        return [0, sprintf($statement, __('Failed!', 'buddypress'))];
    }
    ...
    if (bp_is_active('blogs', 'site-icon')) {
        ...
        foreach ($blog_ids as $blog_id) {
            ...
            foreach ($sizes as $size) {
                $site_icon = bp_blogs_get_site_icon_url(
                    $blog_id,
                    $size['size']
                );
                if (!$site_icon) {
...
function bp_admin_repair_last_activity()
{
    /* translators: %s: the result of the action performed by the repair tool */
    $statement = __(
        'Determining last activity dates for each user&hellip; %s',
        'buddypress'
    );
...
function bp_admin_invitations_table()
{
    ...
    require_once buddypress()->plugin_dir .
    ...
    if (bp_is_active('groups')) {
        ...
        /* translators: %s: the result of the action performed by the repair tool */
        $migrate_statement = __(
            'Migrating group invitations&hellip; %s',
            'buddypress'
        );
        ...
        $records = $wpdb->get_results(
            "SELECT id FROM {$bp->groups->table_name_members} WHERE is_confirmed = 0 AND is_banned = 0"
        );
        if (empty($records)) {
...
function bp_admin_tools_feedback($message, $class = false)
{
    ...
    add_action(
        bp_core_do_network_admin() ? 'network_admin_notices' : 'admin_notices',
        $lambda
    );
...
function bp_core_admin_available_tools_intro()
{
	...
	<div class="card tool-box">
		...
		<p>
			<?php esc_html_e(
       'BuddyPress keeps track of various relationships between users, groups, and activity items. Occasionally these relationships become out of sync, most often after an import, update, or migration.',
       'buddypress'
   ); ?>
			<?php printf(
       ...
       esc_html_x(
           ...
           'buddypress tools intro',
           'buddypress'
       ),
       '<a href="' .
           esc_url($url) .
           '">' .
           esc_html__('BuddyPress Tools', 'buddypress') .
           '</a>'
   ); ?>
		</p>
	</div>
...
function bp_admin_reinstall_emails()
{
    ...
    $email_types = get_terms(bp_get_email_tax_type(), [
        'fields' => 'ids',
        'hide_empty' => false,
    ...
    return [0, __('Emails have been successfully reinstalled.', 'buddypress')];
}
 ...
 */
function bp_core_admin_debug_information($debug_info = [])
{
    global $wp_settings_fields;
    $active_components = array_intersect_key(
    ...
    foreach ($wp_settings_fields['buddypress'] as $section => $settings) {
        ...
        foreach ($settings as $bp_setting) {
            ...
            if (
                !isset($bp_setting['id']) ||
                '_bp_theme_package_id' === $bp_setting['id']
            ) {
                continue;
            }
            ...
            if ('0' === $bp_setting_value || '1' === $bp_setting_value) {
                if (
                    ($reverse && '0' === $bp_setting_value) ||
                    (!$reverse && '1' === $bp_setting_value)
                ) {
        ...
        'fields' => array_merge(
            [
                ...
                'active_components' => [
                    ...
                    'value' => implode(
                        ', ',
                        wp_list_pluck($active_components, 'title')
                    ),
                ],
                'template_pack' => [
