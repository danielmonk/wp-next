...
function bp_core_admin_get_type_default_meta_values($type_taxonomy)
{
    ...
    foreach ($metadata as $meta_key => $meta_value) {
        if (in_array($meta_value, ['boolean', 'integer'], true)) {
            $metadata[$meta_key] = 0;
        } else {
            $metadata[$meta_key] = '';
        }
 ...
 */
function bp_core_admin_insert_type($args = [])
{
    $default_args = [
        'taxonomy' => '',
        'bp_type_id' => '',
    ...
    $args = bp_parse_args($args, $default_args, 'admin_insert_type');
     ...
     */
    $type_exists = apply_filters(
        "{$type_taxonomy}_check_existing_type",
        false,
        $type_id
    );
    ...
    $type_term_id = bp_insert_term($type_id, $type_taxonomy, [
        'slug' => $type_id,
        'metas' => $metas,
    ]);
 ...
 */
function bp_core_admin_update_type($args = [])
{
    $default_args = [
        'taxonomy' => '',
        'type_term_id' => 0,
    ...
    $args = array_map('wp_unslash', $args);
    $args = bp_parse_args($args, $default_args, 'admin_update_type');
 ...
 * @return boolean|WP_Error True on success. A WP_Error object otherwise.
 */
function bp_core_admin_delete_type($args = [])
{
    $default_args = [
        'taxonomy' => '',
        'type_term_id' => 0,
    ...
    $args = array_map('wp_unslash', $args);
    $args = bp_parse_args($args, $default_args, 'admin_delete_type');
    ...
    $registered_by_code_types = apply_filters(
        "{$type_taxonomy}_registered_by_code",
        []
    );
    ...
    if (isset($registered_by_code_types[$type_term->name])) {
        return new WP_Error(
            ...
            __(
                'This type is registered using code, deactivate the plugin or remove the custom code before trying to delete it again.',
                'buddypress'
            ),
            [
                'message' => 7,
    ...
    if (true !== $deleted) {
        return new WP_Error(
            'type_not_deleted',
            __(
                'There was an error while trying to delete this type.',
                'buddypress'
            ),
            [
                'message' => 8,
