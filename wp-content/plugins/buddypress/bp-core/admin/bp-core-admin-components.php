      ...
      __('Components', 'buddypress')
		...
		<form action="" method="post" id="bp-admin-component-form">
			...
			<p class="submit clear">
				<input class="button-primary" type="submit" name="bp-admin-component-submit" id="bp-admin-component-submit" value="<?php esc_attr_e(
        'Save Settings',
        'buddypress'
    ); ?>"/>
			</p>
...
function bp_core_admin_components_options()
{
     ...
     */
    $active_components = apply_filters(
        'bp_active_components',
        bp_get_option('bp-active-components')
    );
    ...
    if (empty($active_components)) {
        ...
        if (!empty($deactivated_components)) {
            ...
            $trimmed = [];
            foreach (
                array_keys((array) $deactivated_components)
                as $component
            ) {
                $trimmed[] = str_replace(
                    '.php',
                    '',
                    str_replace('bp-', '', $component)
                );
            }
    ...
    $inactive_components = array_diff(
        array_keys($all_components),
        array_keys($active_components)
    );
    ...
    $component_views = [
        ...
        [
            ...
            'view' => sprintf(
                ...
                _n(
                    'Retired <span class="count">(%s)</span>',
                    'Retired <span class="count">(%s)</span>',
                    count($retired_components),
                    'buddypress'
                ),
	...
	<h3 class="screen-reader-text"><?php /* translators: accessibility text */
 esc_html_e('Filter components list', 'buddypress'); ?></h3>
	...
	<ul class="subsubsub">
        ...
        add_query_arg(
            ['page' => 'bp-components', 'action' => $component_view['action']],
            bp_get_admin_url($page)
        )
    ); ?>" <?php if (
    $action === $component_view['action']
): ?>class="current"<?php endif; ?>>
					<?php echo wp_kses($component_view['view'], ['span' => ['class' => true]]); ?>
	...
	<h3 class="screen-reader-text"><?php /* translators: accessibility text */
 esc_html_e('Components list', 'buddypress'); ?></h3>

	<table class="wp-list-table widefat plugins">
		<thead>
			<tr>
				<td id="cb" class="manage-column column-cb check-column"><input id="cb-select-all-1" type="checkbox" <?php checked(
        empty($inactive_components)
    ); ?>>
					<label class="screen-reader-text" for="cb-select-all-1"><?php /* translators: accessibility text */
     _e(
         'Enable or disable all optional components in bulk',
         'buddypress'
     ); ?></label></td>
				<th scope="col" id="name" class="manage-column column-title column-primary"><?php _e(
        'Component',
        'buddypress'
    ); ?></th>
				<th scope="col" id="description" class="manage-column column-description"><?php _e(
        'Description',
        'buddypress'
    ); ?></th>
			</tr>
		...
		<tbody id="the-list">
			...
			<?php if (!empty($current_components)): ?>
				...
				<?php foreach ($current_components as $name => $labels): ?>
					...
					<?php if (!in_array($name, ['core', 'members'])):
         $class = isset($active_components[esc_attr($name)])
             ? 'active'
             : 'inactive';
     else:
         $class = 'active';
					...
					<tr id="<?php echo esc_attr($name); ?>" class="<?php echo esc_attr($name) .
    ' ' .
    esc_attr($class); ?>">
    ...
    isset($active_components[esc_attr($name)])
...
printf(__('Select %s', 'buddypress'), esc_html($labels['title'])); ?></label>
			...
			<?php else: ?>
				...
				<tr class="no-items">
					<td class="colspanchange" colspan="3"><?php _e(
         'No components found.',
         'buddypress'
     ); ?></td>
				</tr>
		...
		<tfoot>
			<tr>
				<td class="manage-column column-cb check-column"><input id="cb-select-all-2" type="checkbox" <?php checked(
        empty($inactive_components)
    ); ?>>
					<label class="screen-reader-text" for="cb-select-all-2"><?php /* translators: accessibility text */
     _e(
         'Enable or disable all optional components in bulk',
         'buddypress'
     ); ?></label></td>
				<th class="manage-column column-title column-primary"><?php _e(
        'Component',
        'buddypress'
    ); ?></th>
				<th class="manage-column column-description"><?php _e(
        'Description',
        'buddypress'
    ); ?></th>
			</tr>
...
function bp_core_admin_components_settings_handler()
{
    ...
    if (isset($_POST['bp_components'])) {
        ...
        require_once $bp->plugin_dir .
            '/bp-core/admin/bp-core-admin-schema.php';

        $submitted = stripslashes_deep($_POST['bp_components']);
        $bp->active_components = bp_core_admin_get_active_components_from_submitted_settings(
            $submitted
        );
    ...
    $base_url = bp_get_admin_url(
        add_query_arg(
            ['page' => 'bp-components', 'updated' => 'true'],
            'admin.php'
        )
    );
...
function bp_core_admin_get_active_components_from_submitted_settings($submitted)
{
    ...
    if (
        isset($_GET['action']) &&
        in_array($_GET['action'], ['active', 'inactive', 'retired'])
    ) {
