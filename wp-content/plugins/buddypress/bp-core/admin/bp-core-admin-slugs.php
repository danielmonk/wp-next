      ...
      esc_html__('Pages', 'buddypress')
		...
		<form action="" method="post" id="bp-admin-page-form">
			...
			<p class="submit clear">
				<input class="button-primary" type="submit" name="bp-admin-pages-submit" id="bp-admin-pages-submit" value="<?php esc_attr_e(
        'Save Settings',
        'buddypress'
    ); ?>"/>
			</p>
...
function bp_core_admin_get_directory_pages()
{
    ...
    if (is_array($bp->loaded_components)) {
        foreach ($bp->loaded_components as $component_slug => $component_id) {
            ...
            if (
                isset($bp->{$component_id}) &&
                !empty($bp->{$component_id}->has_directory)
            ) {
                // The component->name property was introduced in BP 1.5, so we must provide a fallback.
                $directory_pages[$component_id] = !empty(
                    $bp->{$component_id}->name
                )
                    ? $bp->{$component_id}->name
                    : ucwords($component_id);
            }
        }
    }
...
function bp_core_admin_slugs_options()
{
		...
		<table class="form-table">
			<tbody>
				...
				<?php foreach ($directory_pages as $name => $label): ?>
					...
					<tr valign="top">
						<th scope="row">
							<label for="bp_pages[<?php echo esc_attr($name); ?>]"><?php echo esc_html(
    $label
); ?></label>
						</th>
           ...
           'selected' => !empty($existing_pages[$name])
               ? $existing_pages[$name]
               : false,
       ]); ?>

							<?php if (
           !empty($existing_pages[$name]) &&
           get_post($existing_pages[$name])
       ): ?>

								<a href="<?php echo esc_url(
            get_permalink($existing_pages[$name])
        ); ?>" class="button-secondary" target="_bp">
									<?php _e(
             'View',
             'buddypress'
      ...
      ?>else: ?>
			<?php if (is_multisite()): ?>
				<p>
					<?php printf(
         __(
             'Registration is currently disabled.  Before associating a page is allowed, please enable registration by selecting either the "User accounts may be registered" or "Both sites and user accounts can be registered" option on <a href="%s">this page</a>.',
             'buddypress'
         ),
         network_admin_url('settings.php')
			...
			<?php else: ?>
				<p>
					<?php printf(
         __(
             'Registration is currently disabled.  Before associating a page is allowed, please enable registration by clicking on the "Anyone can register" checkbox on <a href="%s">this page</a>.',
             'buddypress'
         ),
    ...
    $label
           ...
           get_post($existing_pages[$name])
            ...
            $existing_pages[$name]
        ); ?>" class="button-secondary" target="_bp"><?php _e(
    'View',
    'buddypress'
); ?></a>
...
function bp_core_admin_slugs_setup_handler()
{
    if (isset($_POST['bp-admin-pages-submit'])) {
        ...
        $base_url = bp_get_admin_url(
            add_query_arg(
                ['page' => 'bp-page-settings', 'updated' => 'true'],
                'admin.php'
            )
        );
