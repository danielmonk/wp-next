...
(function ($, bp) {
  ...
  var bpHelloOpenModal = function () {
    ...
    $("#TB_ajaxContent").prop(
      "style",
      "height: 100%; width: auto; padding: 0; border: none;"
    );

    var tabbables = $(":tabbable", "#TB_ajaxContent"),
    ...
    $("#TB_window").on("keydown", function (event) {
      if (
        9 === event.keyCode &&
        !event.shiftKey &&
        $(lastTabbable).prop("classList").value ===
          $(event.target).prop("classList").value
      ) {
      ...
      if (
        9 === event.keyCode &&
        event.shiftKey &&
        "TB_closeWindowButton" === $(event.target).prop("id")
      ) {
        event.preventDefault();
