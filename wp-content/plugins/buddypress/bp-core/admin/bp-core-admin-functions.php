...
function bp_core_modify_admin_menu_highlight()
{
    ...
    if (!in_array($plugin_page, ['bp-activity', 'bp-general-settings'])) {
        $submenu_file = 'bp-components';
    }

    // Network Admin > Tools.
    if (in_array($plugin_page, ['bp-tools', 'available-tools'])) {
...
function bp_core_admin_backpat_page()
{
	...
	<div class="wrap">
      ...
      'Why have all my BuddyPress menus disappeared?',
      'buddypress'
  ); ?></h1>
...
function bp_core_print_admin_notices()
{
    ...
    foreach ($notice_types as $type) {
        $notices = wp_list_filter(buddypress()->admin->notices, [
            'type' => $type,
        ]);
        printf(
            '<div id="message" class="fade %s notice is-dismissible">',
...
function bp_core_activation_notice()
{
    ...
    if (empty($wp_rewrite->permalink_structure)) {
        bp_core_add_admin_notice(
            sprintf(
                ...
                __(
                    '<strong>BuddyPress is almost ready</strong>. You must <a href="%s">update your permalink structure</a> to something other than the default for it to work.',
                    'buddypress'
                ),
    ...
    foreach (array_keys($bp->loaded_components) as $component_id) {
        if (!empty($bp->{$component_id}->has_directory)) {
            $wp_page_components[] = [
                'id' => $component_id,
                'name' => isset($bp->{$component_id}->name)
                    ? $bp->{$component_id}->name
                    : ucwords($bp->{$component_id}->id),
            ];
        }
    }
    ...
    if (!empty($orphaned_components)) {
        $admin_url = bp_get_admin_url(
            add_query_arg(['page' => 'bp-page-settings'], 'admin.php')
        );
        $notice = sprintf(
...
function bp_do_activation_redirect()
{
    ...
    wp_safe_redirect(
        add_query_arg($query_args, bp_get_admin_url('?hello=buddypress'))
    );
}
...
function bp_core_admin_tabs($active_tab = '')
{
    ...
    $tabs = apply_filters(
        'bp_core_admin_tabs',
        bp_core_get_admin_tabs($active_tab)
    );
    ...
    $tabs = [
        '0' => [
            'href' => bp_get_admin_url(
                add_query_arg(['page' => 'bp-components'], 'admin.php')
            ),
            'name' => __('Components', 'buddypress'),
        ],
        '2' => [
            'href' => bp_get_admin_url(
                add_query_arg(['page' => 'bp-settings'], 'admin.php')
            ),
        ...
        '3' => [
            'href' => bp_get_admin_url(
                add_query_arg(['page' => 'bp-credits'], 'admin.php')
            ),
    ...
    switch ($screen->id) {
        ...
        case 'settings_page_bp-components':
            ...
            $screen->add_help_tab([
                ...
                'content' => bp_core_add_contextual_help_content(
                    'bp-comp-overview'
                ),
            ]);
            ...
            $screen->set_help_sidebar(
                '<p><strong>' .
                    ...
                    '<p>' .
                    __(
                        '<a href="https://codex.buddypress.org/getting-started/configure-components/">Managing Components</a>',
                        'buddypress'
                    ) .
                    '</p>' .
                    '<p>' .
        ...
        case 'settings_page_bp-page-settings':
            ...
            $screen->set_help_sidebar(
                '<p><strong>' .
                    ...
                    '<p>' .
                    __(
                        '<a href="https://codex.buddypress.org/getting-started/configure-components/#settings-buddypress-pages">Managing Pages</a>',
                        'buddypress'
                    ) .
                    '</p>' .
                    '<p>' .
        ...
        case 'settings_page_bp-settings':
            ...
            $screen->set_help_sidebar(
                '<p><strong>' .
                    ...
                    '<p>' .
                    __(
                        '<a href="https://codex.buddypress.org/getting-started/configure-components/#settings-buddypress-settings">Managing Settings</a>',
                        'buddypress'
                    ) .
                    '</p>' .
                    '<p>' .
        ...
        case 'users_page_bp-profile-setup':
            ...
            $screen->set_help_sidebar(
                '<p><strong>' .
                    ...
                    '<p>' .
                    __(
                        '<a href="https://codex.buddypress.org/administrator-guide/extended-profiles/">Managing Profile Fields</a>',
                        'buddypress'
                    ) .
                    '</p>' .
                    '<p>' .
...
add_action(
    'load-settings_page_bp-page-settings',
...
function bp_core_add_contextual_help_content($tab = '')
{
    switch ($tab) {
        ...
        case 'bp-page-overview':
            $retval = __(
                'BuddyPress Components use WordPress Pages for their root directory/archive pages. You can change the page associations for each active component by using the form below.',
                'buddypress'
            );
            break;
        ...
        case 'bp-settings-overview':
            $retval = __(
                'Extra configuration settings are provided and activated. You can selectively enable or disable any setting by using the form on this screen.',
                'buddypress'
            );
            break;
        ...
        case 'bp-profile-overview':
            $retval =
                __(
                    'Your users will distinguish themselves through their profile page. Create relevant profile fields that will show on each users profile.',
                    'buddypress'
                ) .
                '<br /><br />' .
                __(
 ...
 */
function bp_admin_menu_order($menu_order = [])
{
    // Bail if user cannot see admin pages.
     ...
     */
    $custom_menus = (array) apply_filters('bp_admin_menu_order', []);
...
function bp_admin_wp_nav_menu_meta_box()
{
    ...
    add_meta_box(
        ...
        __('BuddyPress Member', 'buddypress'),
...
{
    if (
        isset($wp_query->query['orderby'], $wp_query->query['order']) &&
        'post_date' === $wp_query->query['orderby'] &&
        'DESC' === $wp_query->query['order']
    ) {
        return bp_nav_menu_get_loggedin_pages();
    } elseif (
        isset($wp_query->query['nopaging']) &&
        true === $wp_query->query['nopaging']
    ) {
        return array_merge(
 ...
 */
function bp_admin_do_wp_nav_menu_meta_box($object = '', $box = [])
{
    global $nav_menu_selected_id;
    ...
    register_post_type('bp_nav_menu_item', [
        'label' => 'BuddyPress',
        'labels' => [
            'search_items' => __(
                'Search BuddyPress member menu items',
                'buddypress'
            ),
    ...
    $all_bp_items = $get_bp_items->query(['nopaging' => true]);
    ...
    $all_bp_tabs = sprintf(
        ...
        walk_nav_menu_tree(
            array_map('wp_setup_nav_menu_item', $all_bp_items),
            0,
    ...
    if (
        ...
        in_array($_REQUEST[$tab_name], array_keys($tabs), true)
    ) {
    ...
    if ($nav_menu_selected_id) {
        $tab_urls['all'] = esc_url(
            add_query_arg($tab_name, 'all', remove_query_arg($removed_args))
        );
        $tab_urls['logged-in'] = esc_url(
            add_query_arg(
                $tab_name,
                'logged-in',
    ...
    preg_match(
        '/\<ul\sid=\"posttype-bp_nav_menu_item-tabs\"[^>]*>(.*?)\<\/ul\>\<!-- \.posttype-tabs --\>/s',
        $output,
        $tabs_nav
    );
    ...
    echo preg_replace(
        '/\<div\sclass=\".*\"\sid=\"tabs-panel-posttype-bp_nav_menu_item-search\"[^>]*>(.*?)\<\/div\>/s',
        $all_bp_tabs,
        $output
    );
}
...
{
    if (
        get_current_screen()->post_type !== bp_get_email_post_type() ||
        get_locale() === 'en_US'
    ) {
        return;
    }
    ...
    bp_core_add_admin_notice(
        sprintf(
            ...
            __(
                'Are these emails not written in your site\'s language? Go to <a href="%s">BuddyPress Tools and try the "reinstall emails"</a> tool.',
                'buddypress'
            ),
            esc_url(
                add_query_arg('page', 'bp-tools', bp_get_admin_url($admin_page))
            )
        ),
        'updated'
    );
...
function bp_admin_email_add_codex_notice()
{
    ...
    bp_core_add_admin_notice(
        sprintf(
            ...
            __(
                'Phrases wrapped in braces <code>{{ }}</code> are email tokens. <a href="%s">Learn about tokens on the BuddyPress Codex</a>.',
                'buddypress'
            ),
...
function bp_email_tax_type_metabox($post, $box)
{
   ...
   // Allows for an empty term set to be sent. 0 is an invalid Term ID and will be ignored by empty() checks.
    ?>
		...
		<p><?php esc_html_e(
      'Choose when this email will be sent.',
      'buddypress'
  ); ?></p>
	</div>
	<?php
...
function bp_email_custom_metaboxes()
{
    ...
    add_meta_box(
        'postexcerpt',
        __('Plain text email content', 'buddypress'),
...
add_action(
    'add_meta_boxes_' . bp_get_email_post_type(),
...
function bp_email_plaintext_metabox($post)
{
 ...
 _e('Plain text email content', 'buddypress'); ?>
	</label><textarea rows="5" cols="40" name="excerpt" id="excerpt"><?php echo $post->post_excerpt;// textarea_escaped
    ?></textarea>

	<p><?php _e(
     'Most email clients support HTML email. However, some people prefer to receive plain text email. Enter a plain text alternative version of your email here.',
     'buddypress'
 ); ?></p>
    ...
    if (
        ...
        if (bp_is_user_spammer($user_id)) {
            ...
            $actions['ham'] = sprintf(
                '<a href="%1$s">%2$s</a>',
                esc_url($unspam_link),
                esc_html__('Not Spam', 'buddypress')
            );
...
function bp_core_admin_user_manage_spammers()
{
    ...
    if (!empty($action) && in_array($action, ['spam', 'ham'])) {
        check_admin_referer('bp-spam-user');

        $user_id = !empty($_REQUEST['user'])
    ...
    // Display feedback.
    if (!empty($updated) && in_array($updated, ['marked-spam', 'marked-ham'])) {
...
function bp_core_admin_notice_dismiss_callback()
{
    ...
    if (
        !wp_verify_nonce($_POST['nonce'], 'bp-dismissible-notice-' . $notice_id)
    ) {
        wp_send_json_error();
    }
 ...
 */
function bp_block_category($categories = [], $post = null)
{
     ...
     */
    $post_types = apply_filters('bp_block_category_post_types', [
        'post',
        'page',
    ]);
