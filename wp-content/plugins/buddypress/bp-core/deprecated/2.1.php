...
function bp_adminbar_blogs_menu()
{
    ...
    $blogs = wp_cache_get(
        'bp_blogs_of_user_' . bp_loggedin_user_id() . '_inc_hidden',
        'bp'
    );
    if (empty($blogs)) {
    ...
    if (is_array($blogs['blogs']) && (int) $blogs['count']) {
        echo '<li id="bp-adminbar-blogs-menu"><a href="' .
            trailingslashit(bp_loggedin_user_domain() . bp_get_blogs_slug()) .
            '">';
...
function bp_admin_setting_callback_force_buddybar()
{
	...
	<input id="_bp_force_buddybar" name="_bp_force_buddybar" type="checkbox" value="1" <?php checked(
     !bp_force_buddybar(true)
 ); ?> />
	<label for="_bp_force_buddybar"><?php _e(
     'Switch to WordPress Toolbar',
     'buddypress'
 ); ?></label>
...
function bp_core_admin_bar()
{
    ...
    if (
        (int) bp_get_option('hide-loggedout-adminbar') &&
        !is_user_logged_in()
    ) {
        return false;
    }
...
function bp_adminbar_logo()
{
    echo '<a href="' .
        bp_get_root_domain() .
        '" id="admin-bar-logo">' .
        get_blog_option(bp_get_root_blog_id(), 'blogname') .
        '</a>';
}
...
function bp_adminbar_login_menu()
{
    ...
    if (bp_get_signup_allowed()) {
        echo '<li class="bp-signup no-arrow"><a href="' .
            bp_get_signup_page() .
            '">' .
...
function bp_adminbar_account_menu()
{
    ...
    foreach ((array) $bp->bp_nav as $nav_item) {
        ...
        if (
            isset($bp->bp_options_nav[$nav_item['slug']]) &&
            is_array($bp->bp_options_nav[$nav_item['slug']])
        ) {
            echo '<ul>';
            $sub_counter = 0;

            foreach (
                (array) $bp->bp_options_nav[$nav_item['slug']]
                as $subnav_item
            ) {
    ...
    echo '<li' .
        $alt .
        '><a id="bp-admin-logout" class="logout" href="' .
...
function bp_adminbar_thisblog_menu()
{
    if (current_user_can('edit_posts')) {
        echo '<li id="bp-adminbar-thisblog-menu"><a href="' .
            admin_url() .
            '">';
...
function bp_adminbar_random_menu()
{
	...
	<li class="align-right" id="bp-adminbar-visitrandom-menu">
		...
		<ul class="random-list">
			<li><a href="<?php bp_members_directory_permalink(); ?>?random-member" rel="nofollow"><?php _e(
    'Random Member',
    'buddypress'
); ?></a></li>
			...
			<?php if (bp_is_active('groups')): ?>
				...
				<li class="alt"><a href="<?php bp_groups_directory_permalink(); ?>?random-group"  rel="nofollow"><?php _e(
    'Random Group',
    'buddypress'
); ?></a></li>
			...
			<?php if (is_multisite() && bp_is_active('blogs')): ?>
				...
				<li><a href="<?php bp_blogs_directory_permalink(); ?>?random-blog"  rel="nofollow"><?php _e(
    'Random Site',
    'buddypress'
); ?></a></li>
...
function bp_core_load_buddybar_css()
{
    ...
    if (file_exists(get_stylesheet_directory() . '/_inc/css/adminbar.css')) {
        ...
        $stylesheet =
            buddypress()->plugin_url . "bp-core/css/buddybar{$min}.css";
    }

    wp_enqueue_style(
        'bp-admin-bar',
        apply_filters('bp_core_buddybar_rtl_css', $stylesheet),
        [],
        bp_get_version()
    );
...
function bp_groups_adminbar_admin_menu()
{
	...
	<li id="bp-adminbar-adminoptions-menu">
		<a href="<?php bp_groups_action_link('admin'); ?>"><?php _e(
    'Admin Options',
    'buddypress'
); ?></a>

		<ul>
			<li><a href="<?php bp_groups_action_link('admin/edit-details'); ?>"><?php _e(
    'Edit Details',
    'buddypress'
); ?></a></li>

			<li><a href="<?php bp_groups_action_link('admin/group-settings'); ?>"><?php _e(
    'Group Settings',
    'buddypress'
); ?></a></li>

			<?php if (
       !(int) bp_get_option('bp-disable-avatar-uploads') &&
       $bp->avatar->show_avatars
   ): ?>

				<li><a href="<?php bp_groups_action_link('admin/group-avatar'); ?>"><?php _e(
    'Group Profile Photo',
    'buddypress'
); ?></a></li>
			...
			<?php if (bp_is_active('friends')): ?>
				...
				<li><a href="<?php bp_groups_action_link('send-invites'); ?>"><?php _e(
    'Manage Invitations',
    'buddypress'
); ?></a></li>
			...
			<li><a href="<?php bp_groups_action_link('admin/manage-members'); ?>"><?php _e(
    'Manage Members',
    'buddypress'
); ?></a></li>
			...
			<?php if ($bp->groups->current_group->status == 'private'): ?>
				...
				<li><a href="<?php bp_groups_action_link(
        'admin/membership-requests'
    ); ?>"><?php _e('Membership Requests', 'buddypress'); ?></a></li>

			<?php endif; ?>

			<li><a class="confirm" href="<?php echo wp_nonce_url(
       bp_get_group_permalink($bp->groups->current_group) .
           'admin/delete-group/',
       'groups_delete_group'
   ); ?>&amp;delete-group-button=1&amp;delete-group-understand=1"><?php _e(
    "Delete Group",
    'buddypress'
); ?></a></li>
...
function bp_adminbar_authors_menu()
{
    ...
    $authors = $wpdb->get_results(
        "SELECT user_id, user_login, user_nicename, display_name, user_email, meta_value as caps FROM $wpdb->users u, $wpdb->usermeta um WHERE u.ID = um.user_id AND meta_key = '{$blog_prefix}capabilities' ORDER BY um.user_id"
    );
...
function bp_members_adminbar_admin_menu()
{
	...
	<li id="bp-adminbar-adminoptions-menu">
      ...
      printf(
          __("Edit %s's Profile", 'buddypress'),
          esc_attr(bp_get_displayed_user_fullname())
      ); ?>
					</a>
				</li>
			...
			<li>
				<a href="<?php bp_members_component_link('profile', 'change-avatar'); ?>">
					<?php /* translators: %s: member name */
     printf(
         _x("Edit %s's Profile Photo", 'deprecated string', 'buddypress'),
         esc_attr(bp_get_displayed_user_fullname())
     ); ?>
				</a>
			</li>

			<li><a href="<?php bp_members_component_link(
       'settings',
       'capabilities'
   ); ?>"><?php _e('User Capabilities', 'buddypress'); ?></a></li>

			<li>
				<a href="<?php bp_members_component_link('settings', 'delete-account'); ?>">
					<?php /* translators: %s: member name */
     printf(
         _x("Delete %s's Account", 'deprecated string', 'buddypress'),
         esc_attr(bp_get_displayed_user_fullname())
     ); ?>
				</a>
			</li>
...
function bp_notifications_buddybar_menu()
{
    ...
    echo '<li id="bp-adminbar-notifications-menu"><a href="' .
        esc_url(bp_loggedin_user_domain()) .
        '">';
    _e('Notifications', 'buddypress');

    $notification_count = bp_notifications_get_unread_notification_count(
        bp_loggedin_user_id()
    );
    ...
    if (!empty($notifications)) {
		...
		<li><a href="<?php echo esc_url(bp_loggedin_user_domain()); ?>"><?php _e(
