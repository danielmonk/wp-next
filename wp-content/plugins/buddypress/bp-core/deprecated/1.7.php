...
function xprofile_get_profile()
{
    ...
    bp_locate_template(['profile/profile-loop.php'], true);
}
...
{
    _deprecated_function(__FUNCTION__, '1.7');
    bp_locate_template(['profile/profile-header.php'], true);
}
...
{
    _deprecated_function(__FUNCTION__, '1.7');
    bp_locate_template(['plugin-sidebar.php'], true);
}
...
function bp_update_db_stuff()
{
    ...
    if (
        $wpdb->get_var(
            "SHOW TABLES LIKE '%{$bp_prefix}bp_activity_user_activity_cached%'"
        )
    ) {
        $wpdb->query(
            "RENAME TABLE {$bp_prefix}bp_activity_user_activity_cached TO {$bp->activity->table_name}"
        );
    }
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '%{$bp->activity->table_name}%'")) {
        if (
            $wpdb->get_var(
                "SHOW COLUMNS FROM {$bp->activity->table_name} LIKE 'component_action'"
            )
        ) {
            $wpdb->query(
                "ALTER TABLE {$bp->activity->table_name} CHANGE component_action type varchar(75) NOT NULL"
            );
        }

        if (
            $wpdb->get_var(
                "SHOW COLUMNS FROM {$bp->activity->table_name} LIKE 'component_name'"
            )
        ) {
            $wpdb->query(
                "ALTER TABLE {$bp->activity->table_name} CHANGE component_name component varchar(75) NOT NULL"
            );
        }
    }
    ...
    if (
        $wpdb->get_var("SHOW TABLES LIKE '%{$bp_prefix}bp_messages_threads%'")
    ) {
