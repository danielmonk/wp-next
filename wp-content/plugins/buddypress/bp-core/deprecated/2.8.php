...
function bp_core_admin_php52_plugin_row($file, $plugin_data)
{
    ...
    echo '<tr class="plugin-update-tr' .
        ...
        '" class="plugin-update colspanchange"><div class="update-message inline notice notice-error notice-alt">';

    printf(
        $p,
        esc_html__(
            'A BuddyPress update is available, but your system is not compatible.',
            'buddypress'
        ) .
            ' ' .
            sprintf(
                __(
                    'See <a href="%s">the Codex guide</a> for more information.',
                    'buddypress'
                ),
...
function bp_core_admin_php53_admin_notice()
{
	...
	<div id="message" class="error notice is-dismissible bp-is-dismissible" data-noticeid="<?php echo esc_attr(
     $notice_id
 ); ?>">
		<p><strong><?php esc_html_e(
      'Your site is not ready for BuddyPress 2.8.',
      'buddypress'
  ); ?></strong></p>
		<p>
			<?php /* translators: %s: the site's PHP version number */
   printf(
       esc_html_x(
       ...
       esc_html(phpversion())
			...
			&nbsp;
			<?php /* translators: %s: the url to a codex page */
   printf(
       _x(
           'See <a href="%s">the Codex guide</a> for more information.',
           'deprecated string',
       ...
       'https://codex.buddypress.org/getting-started/buddypress-2-8-will-require-php-5-3/'
		...
		<?php wp_nonce_field(
      "bp-dismissible-notice-$notice_id",
      "bp-dismissible-nonce-$notice_id"
  ); ?>
	</div>
	<?php
}
