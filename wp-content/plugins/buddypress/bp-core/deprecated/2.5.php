...
function bp_core_email_from_name_filter()
{
    ...
    return apply_filters(
        ...
        bp_get_option('blogname', 'WordPress')
    );
}
...
function bp_core_deprecated_email_filters($value, $property, $transform, $email)
{
    ...
    if ($email_type === 'activity-comment') {
        ...
        if ($property === 'to') {
            ...
            $value = apply_filters(
                ...
                $value,
                $tokens['initiator.name'],
                $tokens['initiator.url'],
                $tokens['friend-requests.url'],
                ''
            );
        }
        ...
        if ($property === 'to') {
             ...
             */
            $value = apply_filters_ref_array(
                'groups_notification_group_updated_subject',
                [$value, &$tokens['group']]
            );
        } elseif ($property === 'content') {
            /**
            ...
            $value = apply_filters_ref_array(
                ...
                [$value, &$tokens['group'], $tokens['group.url'], '']
            );
        }
        ...
        if ($property === 'to') {
             ...
             */
            $value = apply_filters_ref_array(
                'groups_notification_new_membership_request_subject',
                [$value, &$tokens['group']]
            );
        } elseif ($property === 'content') {
            /**
            ...
            $value = apply_filters_ref_array(
                ...
                [
                    $value,
                    &$tokens['group'],
                    $tokens['requesting-user.name'],
                    $tokens['profile.url'],
                    $tokens['group-requests.url'],
                    '',
                ]
            );
        }
        ...
        if ($property === 'to') {
             ...
             */
            $value = apply_filters_ref_array(
                'groups_notification_membership_request_completed_subject',
                [$value, &$tokens['group']]
            );
        } elseif ($property === 'content') {
            /**
    ...
    if ($property === 'to' && $to_changed) {
        ...
        array_unshift(
            $original_value,
            new BP_Email_Recipient($value, $recipient_name)
        );
...
function bp_core_deprecated_email_actions($delivery_status, $email)
{
    ...
    if ($email_type === 'activity-comment') {
        ...
        do_action(
            ...
            $tokens['original_activity.user_id'],
            $email_subject,
            $email_content,
            $tokens['comment.id'],
            $tokens['commenter.id'],
