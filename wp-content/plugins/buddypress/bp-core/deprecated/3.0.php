...
function bp_is_group_forums_active($default = false)
{
    _deprecated_function(
        __FUNCTION__,
        '3.0',
        'groups_get_group( $id )->enable_forum'
    );

    $is_active = function_exists('bbp_is_group_forums_active')
        ? bbp_is_group_forums_active($default)
        : $default;
...
function bp_core_action_set_spammer_status($user_id = 0)
{
    ...
    if (
        bp_is_current_component('admin') &&
        in_array(bp_current_action(), ['mark-spammer', 'unmark-spammer'])
    ) {
        // Check the nonce.
...
function bp_core_action_delete_user()
{
    ...
    if (
        bp_is_current_component('admin') &&
        bp_is_current_action('delete-user')
    ) {
        // Check the nonce.
