...
function bp_core_filter_comments($comments, $post_id)
{
    ...
    if (
        !($userdata = $wpdb->get_results(
            "SELECT ID as user_id, user_login, user_nicename FROM {$wpdb->users} WHERE ID IN ({$user_ids})"
        ))
    ) {
        return $comments;
    }
    ...
    foreach ((array) $userdata as $user) {
        $users[$user->user_id] = bp_core_get_user_domain(
            $user->user_id,
            $user->user_nicename,
            $user->user_login
        );
...
function bp_core_login_redirect($redirect_to, $redirect_to_raw, $user)
{
    ...
    if (
        false === strpos(wp_get_referer(), 'wp-login.php') &&
        false === strpos(wp_get_referer(), 'activate') &&
        empty($_REQUEST['nr'])
    ) {
...
function bp_core_filter_user_welcome_email($welcome_email)
{
    ...
    if (
        ...
        buddypress()->members->admin->signups_page != get_current_screen()->id
    ) {
 ...
 */
function bp_core_filter_blog_welcome_email(
    $welcome_email,
    $blog_id,
    $user_id,
    $password
 ...
 */
function bp_core_activation_signup_blog_notification(
    $domain,
    $path,
    $title,
    $user,
    ...
    $args = [
        'tokens' => [
            'activate-site.url' => esc_url(
                bp_get_activation_page() . '?key=' . urlencode($key)
            ),
            'domain' => $domain,
            'key_blog' => $key,
            'path' => $path,
            'user-site.url' => esc_url(
                set_url_scheme("http://{$domain}{$path}")
            ),
            'title' => $title,
    ...
    if ($signups && bp_is_active('xprofile')) {
        ...
        if (isset($signup->meta['field_' . bp_xprofile_fullname_field_id()])) {
            $salutation =
                $signup->meta['field_' . bp_xprofile_fullname_field_id()];
        }
    }

    bp_send_email(
        'core-user-registration-with-blog',
        [[$user_email => $salutation]],
        $args
    );
 ...
 */
function bp_core_activation_signup_user_notification(
    $user,
    $user_email,
    $key,
    $meta
    ...
    if (is_admin()) {
        ...
        if (in_array(get_current_screen()->id, ['user', 'user-network'])) {
            ...
            buddypress()->members->admin->signups_page ==
    ...
    if (
        ...
        isset($meta['field_' . bp_xprofile_fullname_field_id()])
    ) {
...
function bp_modify_page_title(
    ...
    $title_tag_compatibility =
        (bool) (!empty($_wp_theme_features['title-tag']) ||
            ($blogname && strstr($title, $blogname)));
    ...
    if (true === $title_tag_compatibility) {
        ...
        if (
            ...
            $bp_title_parts['page'] = sprintf(
                __('Page %s', 'buddypress'),
                max($paged, $page)
            );
        }
    }
 ...
 */
function bp_modify_document_title_parts($title = [])
{
    // Get the BuddyPress title parts.
...
function bp_setup_nav_menu_item($menu_item)
{
    if (is_admin()) {
        if ('bp_nav_menu_item' === $menu_item->object) {
            ...
            if (
                !in_array(
                    ['bp-menu', 'bp-' . $menu_item->post_excerpt . '-nav'],
                    $menu_item->classes
                )
            ) {
                $menu_item->classes[] = 'bp-menu';
                $menu_item->classes[] =
                    'bp-' . $menu_item->post_excerpt . '-nav';
            }
        }
 ...
 */
function bp_customizer_nav_menus_get_items(
    $items = [],
    $type = '',
    $object = '',
    $page = 0
 ...
 */
function bp_customizer_nav_menus_set_item_types($item_types = [])
{
    $item_types = array_merge($item_types, [
        'bp_loggedin_nav' => [
            'title' => _x(
                'BuddyPress (logged-in)',
                'customizer menu section title',
                'buddypress'
            ),
            'type' => 'bp_nav',
...
function bp_core_add_loading_lazy_attribute($content = '')
{
    ...
    if (
        false !== strpos($content, 'data-secret=') &&
        false !== strpos($content, $find_pos_absolute)
    ) {
 ...
 */
function bp_maybe_load_mentions_scripts_for_blog_content(
    $load_mentions,
    $mentions_enabled
) {
    if (!$mentions_enabled) {
    ...
    if (
        $load_mentions ||
        (bp_is_blog_page() && is_singular() && comments_open())
    ) {
        return true;
    }
...
function _bp_core_inject_bp_widget_css_class($params)
{
    ...
    if (
        0 !==
        strpos(
            $wp_registered_widgets[$widget_id]['callback'][0]->id_base,
            'bp_'
        )
    ) {
        return $params;
...
function bp_email_add_link_color_to_template($value, $property_name, $transform)
{
    ...
    $replacement =
        'style="color: ' . esc_attr($settings['link_text_color']) . ';';
...
function bp_email_set_default_tokens(
    ...
    $tokens['site.description'] = wp_specialchars_decode(
        bp_get_option('blogdescription'),
        ENT_QUOTES
    );
...
function bp_core_render_email_template($template)
{
    ...
    $template = str_replace(
        '<head>',
        '<head><title>' .
            esc_html_x('BuddyPress Emails', 'screen heading', 'buddypress') .
            '</title>',
        $template
    );
    echo str_replace(
        '{{{content}}}',
        wpautop(get_post()->post_content),
        $template
    );
 ...
 */
function bp_core_components_subdirectory_reserved_names($names = [])
{
