 ...
 */
function bp_parse_args($args, $defaults = [], $filter_key = '')
{
...
function bp_core_get_directory_page_ids($status = 'active')
{
    ...
    foreach ($page_ids as $component_name => $page_id) {
        ...
        if (in_array($component_name, ['register', 'activate'], true)) {
            continue;
        }
...
function bp_core_get_directory_pages()
{
    ...
    if (false === $pages) {
        ...
        if (!empty($page_ids)) {
            ...
            $posts_table_name = bp_is_multiblog_mode()
                ? $wpdb->posts
            ...
            foreach ((array) $page_ids as $component_id => $page_id) {
                foreach ((array) $page_names as $page_name) {
                    if ($page_name->ID == $page_id) {
                        if (
                            !isset($pages->{$component_id}) ||
                            !is_object($pages->{$component_id})
                        ) {
                        ...
                        $pages->{$component_id}->slug = implode(
                            '/',
                            array_reverse((array) $slug)
                        );
                    }
...
function bp_core_add_page_mappings($components, $existing = 'keep')
{
    ...
    foreach (array_keys($components) as $component_name) {
        if (
            !isset($pages[$component_name]) &&
            isset($page_titles[$component_name])
        ) {
    ...
    if (bp_get_signup_allowed()) {
        foreach (['register', 'activate'] as $slug) {
            if (!isset($pages[$slug])) {
...
function bp_core_get_directory_page_default_titles()
{
    $page_default_titles = [
        'activity' => _x(
            'Activity',
            'Page title for the Activity directory.',
            'buddypress'
        ),
        'groups' => _x(
            'Groups',
            'Page title for the Groups directory.',
            'buddypress'
        ),
        'blogs' => _x(
            'Sites',
            'Page title for the Sites directory.',
            'buddypress'
        ),
        'members' => _x(
            'Members',
            'Page title for the Members directory.',
            'buddypress'
        ),
        'activate' => _x(
            'Activate',
            'Page title for the user activation screen.',
            'buddypress'
        ),
        'register' => _x(
            'Register',
            'Page title for the user registration screen.',
            'buddypress'
        ),
    ];
...
function bp_core_get_component_search_query_arg($component = null)
{
    ...
    if (isset(buddypress()->{$component}->search_query_arg)) {
        $query_arg = sanitize_title(
            buddypress()->{$component}->search_query_arg
        );
    }
...
function bp_do_register_theme_directory()
{
    ...
    foreach (
        array_values((array) $GLOBALS['wp_theme_directories'])
        as $directory
    ) {
...
function bp_core_time_since($older_date, $newer_date = false)
{
     ...
     */
    $unknown_text = apply_filters(
        'bp_core_time_since_unknown_text',
        __('sometime', 'buddypress')
    );
     ...
     * @param string $value String representing the time since the older date.
     */
    $right_now_text = apply_filters(
        'bp_core_time_since_right_now_text',
        __('right now', 'buddypress')
    );
    ...
    $newer_date = !$newer_date
        ? bp_core_current_time(true, 'timestamp')
        : $newer_date;
        ...
        if (!isset($chunks[$i])) {
            ...
            switch ($seconds) {
                case YEAR_IN_SECONDS:
                    ...
                    $output = sprintf(
                        _n('%s year', '%s years', $count, 'buddypress'),
                        $count
                    );
                    break;
                case 30 * DAY_IN_SECONDS:
                    ...
                    $output = sprintf(
                        _n('%s month', '%s months', $count, 'buddypress'),
                        $count
                    );
                    break;
                case WEEK_IN_SECONDS:
                    ...
                    $output = sprintf(
                        _n('%s week', '%s weeks', $count, 'buddypress'),
                        $count
                    );
                    break;
                case DAY_IN_SECONDS:
                    ...
                    $output = sprintf(
                        _n('%s day', '%s days', $count, 'buddypress'),
                        $count
                    );
                    break;
                case HOUR_IN_SECONDS:
                    ...
                    $output = sprintf(
                        _n('%s hour', '%s hours', $count, 'buddypress'),
                        $count
                    );
                    break;
                case MINUTE_IN_SECONDS:
                    ...
                    $output = sprintf(
                        _n('%s minute', '%s minutes', $count, 'buddypress'),
                        $count
                    );
                    break;
                default:
                    ...
                    $output = sprintf(
                        _n('%s second', '%s seconds', $count, 'buddypress'),
                        $count
                    );
            }
            ...
            if ($i + 2 < $j) {
                ...
                if (0 != $count2) {
                    $output .=
                        _x(',', 'Separator in time since', 'buddypress') . ' ';
                    ...
                    switch ($seconds2) {
                        ...
                        case DAY_IN_SECONDS:
                            /* translators: %s: the number of days. */
                            $output .= sprintf(
                                _n('%s day', '%s days', $count2, 'buddypress'),
                                $count2
                            );
                            break;
                        case HOUR_IN_SECONDS:
...
function bp_core_record_activity()
{
    ...
    if (
        empty($activity) ||
        $current_time >= strtotime('+5 minutes', $activity)
    ) {
...
function bp_use_embed_in_activity()
{
    ...
    return apply_filters(
        ...
        !defined('BP_EMBED_DISABLE_ACTIVITY') || !BP_EMBED_DISABLE_ACTIVITY
    );
}
...
function bp_use_embed_in_activity_replies()
{
    ...
    return apply_filters(
        ...
        !defined('BP_EMBED_DISABLE_ACTIVITY_REPLIES') ||
...
function bp_use_embed_in_private_messages()
{
    ...
    return apply_filters(
        ...
        !defined('BP_EMBED_DISABLE_PRIVATE_MESSAGES') ||
...
function bp_update_is_directory($is_directory = false, $component = '')
{
     ...
     */
    buddypress()->is_directory = apply_filters(
...
function bp_update_is_item_admin($is_item_admin = false, $component = '')
{
     ...
     * @param string $component     Component name. Default: the current component.
     */
    buddypress()->is_item_admin = apply_filters(
...
function bp_update_is_item_mod($is_item_mod = false, $component = '')
{
     ...
     * @param string $component   Component name. Default: the current component.
     */
    buddypress()->is_item_mod = apply_filters(
...
function bp_verify_nonce_request($action = '', $query_arg = '_wpnonce')
{
    ...
    $home_url = trim(
        strtolower(
            $parsed_home['scheme'] . '://' . $parsed_host . $parsed_home['path']
        ),
        '/'
    );
    ...
    if (
        ...
        false === strpos($_SERVER['HTTP_HOST'], ':')
    ) {
    ...
    $requested_url = strtolower(
        $scheme . $request_host . $_SERVER['REQUEST_URI']
    );
...
function bp_core_action_search_site($slug = '')
{
    ...
    $search_which = !empty($_POST['search-which'])
        ? $_POST['search-which']
        : '';
    $query_string = '/?s=';
    ...
    bp_core_redirect(
        apply_filters(
            'bp_core_search_site',
            home_url($slug . $query_string . urlencode($search_terms)),
            $search_terms
        )
    );
}
...
function bp_core_get_components($type = 'all')
{
    $required_components = [
        'core' => [
            ...
            'description' => __(
                'It&#8216;s what makes <del>time travel</del> BuddyPress possible!',
                'buddypress'
            ),
        ],
        'members' => [
            ...
            'description' => __(
                'Everything in a BuddyPress community revolves around its members.',
                'buddypress'
            ),
        ],
    ];
    ...
    if (is_multisite()) {
        $optional_components['blogs']['description'] = __(
...
{
    $args = bp_parse_args($args, [], 'get_suggestions');
...
function bp_get_email_post_type_labels()
{
     ...
     */
    return apply_filters('bp_get_email_post_type_labels', [
        'add_new' => _x('Add New', 'email post type label', 'buddypress'),
        'add_new_item' => _x(
            'Add a New Email',
            'email post type label',
            'buddypress'
        ),
        ...
        'singular_name' => _x(
            'Email',
            'email post type singular name',
            'buddypress'
        ),
        'uploaded_to_this_item' => _x(
            'Uploaded to this email',
            'email post type label',
            'buddypress'
        ),
...
function bp_get_email_post_type_supports()
{
     ...
     */
    return apply_filters('bp_get_email_post_type_supports', [
        'custom-fields',
        'editor',
        'excerpt',
        'revisions',
...
function bp_get_taxonomy_common_labels()
{
    return [
        ...
        'bp_type_singular_name' => _x(
            'Singular name',
            'BP Type singular name label',
            'buddypress'
        ),
        'bp_type_has_directory' => _x(
            'Has Directory View',
            'BP Type has directory checkbox label',
            'buddypress'
        ),
        'bp_type_directory_slug' => _x(
            'Custom type directory slug',
            'BP Type slug label',
            'buddypress'
        ),
    ];
...
function bp_get_email_tax_type()
{
    ...
    return apply_filters(
        ...
        buddypress()->email_taxonomy_type
    );
}
...
function bp_get_email_tax_type_labels()
{
     ...
     * @param array $value Associative array (name => label).
     */
    return apply_filters('bp_get_email_tax_type_labels', [
        'add_new_item' => _x(
            'New Email Situation',
            'email type taxonomy label',
            'buddypress'
        ),
        'all_items' => _x(
            'All Email Situations',
            'email type taxonomy label',
            'buddypress'
        ),
        'edit_item' => _x(
            'Edit Email Situations',
            'email type taxonomy label',
            'buddypress'
        ),
        'items_list' => _x(
        ...
        'singular_name' => _x(
            'Email',
            'email type taxonomy singular name',
            'buddypress'
        ),
        'update_item' => _x(
...
function bp_get_email_tax_type_args()
{
    ...
    return apply_filters(
        ...
        array_merge(
            [
                'description' => _x(
                    'BuddyPress email types',
                    'email type taxonomy description',
                    'buddypress'
                ),
 ...
 */
function bp_get_type_metadata_schema(
    ...
    $schema = [
        'bp_type_singular_name' => [
            'description' => __(
                'The name of this type in singular form. ',
                'buddypress'
            ),
            'type' => 'string',
            'single' => true,
        ...
        'bp_type_has_directory' => [
            'description' => __(
                'Make a list matching this type available on the directory.',
                'buddypress'
            ),
            'type' => 'boolean',
            'single' => true,
 ...
 */
function bp_update_type_metadata($type_id = 0, $taxonomy = '', $type_metas = [])
{
 ...
 */
function bp_get_taxonomy_types($taxonomy = '', $types = [])
{
    if (!$taxonomy) {
        return $types;
    }
    ...
    if (!$db_types) {
        ...
        $type_metadata = array_keys(
            get_registered_meta_keys('term', $taxonomy)
        );
        ...
        foreach ($terms as $term) {
            ...
            if ($type_metadata) {
                ...
                if (
                    !empty($db_types[$type_name]->has_directory) &&
                    empty($db_types[$type_name]->directory_slug)
                ) {
    ...
    if (is_array($db_types)) {
        foreach ($db_types as $db_type_name => $db_type) {
            ...
            if (
                isset($types[$db_type_name]) &&
                isset($types[$db_type_name]->code) &&
                $types[$db_type_name]->code
            ) {
                // Merge Labels.
                if ($db_type->labels) {
                    foreach ($db_type->labels as $key_label => $value_label) {
                        if ('' !== $value_label) {
                            $types[$db_type_name]->labels[
                                $key_label
                            ] = $value_label;
                        }
                    }
                ...
                foreach (
                    get_object_vars($types[$db_type_name])
                    as $key_prop => $value_prop
                ) {
                    ...
                    if (
                        isset($db_type->{$key_prop}) &&
                        '' !== $db_type->{$key_prop}
                    ) {
                        $types[$db_type_name]->{$key_prop} =
                            $db_type->{$key_prop};
                    }
                }
...
function bp_get_email($email_type)
{
    ...
    if (!$post) {
        ...
        return new WP_Error('missing_email', __FUNCTION__, [
            $email_type,
            $args,
        ]);
    }
...
function bp_send_email($email_type, $to, $args = [])
{
    ...
    if (is_null($is_default_wpmail)) {
        try {
            ...
            $is_default_wpmail =
                substr($mirror->getFileName(), -strlen('pluggable.php')) ===
                'pluggable.php';
        } catch (Exception $e) {
    ...
    if (
        'settings-verify-email-change' === $email_type &&
        isset($args['tokens']['displayname'])
    ) {
     ...
     * }
     */
    do_action_ref_array('bp_send_email', [&$email, $email_type, $to, $args]);
...
function bp_email_get_appearance_settings()
{
    $footer_text = [
        sprintf(
            ...
            _x(
                '&copy; %1$s %2$s',
                'copyright text for email footers',
                'buddypress'
            ),
            date_i18n('Y'),
    ...
    $options = bp_parse_args(
        bp_get_option('bp_email_options', []),
        $default_args,
...
function bp_email_get_schema()
{
     ...
     */
    return (array) apply_filters('bp_email_get_schema', [
        'activity-comment' => [
        ...
        'friends-request-accepted' => [
            ...
            /* translators: do not remove {} brackets or translate its contents. */
            'post_content' => __(
                "<a href=\"{{{friendship.url}}}\">{{friend.name}}</a> accepted your friend request.",
                'buddypress'
            ),
            /* translators: do not remove {} brackets or translate its contents. */
            'post_excerpt' => __(
...
function bp_email_get_type_schema($field = 'description')
{
    $activity_comment = [
        ...
        'unsubscribe' => [
            ...
            'message' => __(
                'You will no longer receive emails when someone replies to an update or comment you posted.',
                'buddypress'
            ),
        ],
    ];
    ...
    $core_user_registration_with_blog = [
        'description' => __(
    ...
    $friends_request_accepted = [
        ...
        'unsubscribe' => [
            ...
            'message' => __(
                'You will no longer receive emails when someone accepts your friendship request.',
                'buddypress'
            ),
        ],
    ];
    ...
    $messages_unread = [
        ...
        'unsubscribe' => [
            ...
            'message' => __(
                'You will no longer receive emails when someone sends you a message.',
                'buddypress'
            ),
        ],
    ];

    $settings_verify_email_change = [
        'description' => __(
    ...
    $groups_membership_request_accepted = [
        'description' => __(
    ...
    $groups_membership_request_rejected = [
        'description' => __(
...
function bp_email_unsubscribe_handler()
{
    ...
    if (
        ...
        !$raw_email_type ||
        !$raw_hash ||
        !array_key_exists($raw_email_type, $emails)
    ) {
        ...
        $result_msg = __('Something has gone wrong.', 'buddypress');
        $unsub_msg = __(
            'Please go to your notifications settings to unsubscribe from emails.',
            'buddypress'
        );
        ...
        $unsub_msg = __(
            'You can change this or any other email notification preferences in your email settings.',
            'buddypress'
        );
    }
...
function bp_email_get_unsubscribe_link($args)
{
    ...
    if (
        empty($args['notification_type']) ||
        !array_key_exists($args['notification_type'], $emails)
    ) {
        return wp_login_url();
    }

    $email_type = $args['notification_type'];
    $redirect_to = !empty($args['redirect_to'])
        ? $args['redirect_to']
        : site_url();
