...
function bp_core_register_common_scripts()
{
    ...
    if (
        file_exists(
            buddypress()->core->path .
                "bp-core/js/vendor/moment-js/locale/{$locale}{$min}.js"
        )
    ) {
        $moment_locale_url =
            $url . "vendor/moment-js/locale/{$locale}{$min}.js";
    ...
    $scripts = [
        ...
        'jquery-atwho' => [
            ...
            'dependencies' => ['jquery', 'jquery-caret'],
            'footer' => true,
        ],
        ...
        'bp-plupload' => [
            ...
            'dependencies' => ['plupload', 'jquery', 'json2', 'wp-backbone'],
            'footer' => true,
        ],
        'bp-avatar' => [
    ...
    if (isset($moment_locale_url)) {
        $scripts['bp-moment-locale'] = [
            'file' => esc_url($moment_locale_url),
            'dependencies' => ['bp-moment'],
            'footer' => true,
        ];
    }
...
function bp_core_register_common_styles()
{
     ...
     */
    $admin_bar_file = apply_filters(
        'bp_core_admin_bar_css',
        "{$url}admin-bar{$min}.css"
    );
     ...
     */
    $styles = apply_filters('bp_core_register_common_styles', [
        'bp-admin-bar' => [
            'file' => $admin_bar_file,
            'dependencies' => ['admin-bar'],
        ],
        'bp-avatar' => [
...
function bp_core_confirmation_js()
{
    ...
    wp_localize_script('bp-confirm', 'BP_Confirm', [
    ...
    $image = apply_filters(
        ...
        getimagesize(
            bp_core_avatar_upload_path() .
                buddypress()->avatar_admin->image->dir
        )
    );
    if (empty($image)) {
        return;
...
function bp_core_get_js_dependencies()
{
     ...
     */
    return apply_filters('bp_core_get_js_dependencies', [
        'jquery',
        'bp-confirm',
        'bp-widget-members',
        'bp-jquery-query',
...
function bp_add_cover_image_inline_css($return = false)
{
    ...
    if (bp_is_user()) {
        ...
        $cover_image_object = apply_filters(
            'bp_current_cover_image_object_inline_css',
            []
        );
    }
    ...
    if (
        empty($cover_image_object['component']) ||
        empty($cover_image_object['object']) ||
        !bp_is_active($cover_image_object['component'], 'cover_image')
    ) {
        return;
    }

    // Get the settings of the cover image feature for the current component.
    $params = bp_attachments_get_cover_image_settings(
        $cover_image_object['component']
    );
    ...
    if (is_callable($params['callback'])) {
        ...
        $cover_image = bp_attachments_get_attachment('url', [
        ...
        $inline_css = call_user_func_array($params['callback'], [
            [
...
function bp_core_moment_js_config()
{
    ...
    $moment_locale = substr(
        $moment_locale->src,
        strpos($moment_locale->src, '/moment-js/locale/') + 18
    );
