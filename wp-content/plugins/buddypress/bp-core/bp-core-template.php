...
function bp_get_options_nav($parent_slug = '')
{
    ...
    $component_index = !empty($bp->displayed_user)
        ...
        : bp_get_root_slug(bp_current_component());
    ...
    if (!bp_is_single_item()) {
        ...
        $secondary_nav_items = $bp->members->nav->get_secondary([
        ...
        if (!isset($bp->{$single_item_component}->nav)) {
            $secondary_nav_items = $bp->members->nav->get_secondary([
                'parent_slug' => $current_item,
            ]);
        } else {
            $secondary_nav_items = $bp->{$single_item_component}->nav->get_secondary(
                ['parent_slug' => $current_item]
            );
        }
    ...
    foreach ($secondary_nav_items as $subnav_item) {
        ...
        echo apply_filters(
            ...
            '<li id="' .
                esc_attr($subnav_item->css_id . '-' . $list_type . '-li') .
                '" ' .
                $selected .
                '><a id="' .
                esc_attr($subnav_item->css_id) .
                '" href="' .
                esc_url($subnav_item->link) .
                '">' .
                $subnav_item->name .
                '</a></li>',
            $subnav_item,
...
function bp_get_directory_title($component = '')
{
    ...
    if (
        isset(buddypress()->{$component}->directory_title) &&
...
{
    ?>
	<img id="avatar-to-crop" class="avatar" src="<?php echo esc_url(
     buddypress()->avatar_admin->image
 ); ?>" />
<?php
}
...
function bp_search_form_action()
{
    ...
    return apply_filters(
        ...
        trailingslashit(bp_get_root_domain() . '/' . bp_get_search_slug())
    );
}
...
function bp_search_form_type_select()
{
    ...
    $selection_box =
        '<label for="search-which" class="accessibly-hidden">' .
        _x('Search these:', 'search form', 'buddypress') .
        '</label>';
    $selection_box .=
 ...
 */
function bp_form_field_attributes($name = '', $attributes = [])
{
 ...
 * @return string
 */
function bp_get_form_field_attributes($name = '', $attributes = [])
{
    $retval = '';
 ...
 */
function bp_create_excerpt($text, $length = 225, $options = [])
{
    ...
    if (!empty($r['html'])) {
        ...
        foreach ($tags as $tag) {
            ...
            if (
                !preg_match(
                    '/img|br|input|hr|area|base|basefont|col|frame|isindex|link|meta|param/s',
                    $tag[2]
                )
            ) {
...
function bp_account_was_activated()
{
    ...
    $activation_complete =
        !empty($bp->activation_complete) ||
        (bp_is_current_component('activate') && !empty($_GET['activated']));
 ...
 */
function bp_get_email_subject($args = [])
{
    $r = bp_parse_args(
        $args,
        [
    ...
    $subject =
        ...
        wp_specialchars_decode(
            bp_get_option('blogname', $r['default']),
            ENT_QUOTES
        ) .
        $r['after'] .
        ' ' .
...
function bp_get_root_slug($component = '')
{
    ...
    if (!empty($bp->active_components[$component])) {
        ...
        $component_name =
            '1' == $bp->active_components[$component]
                ? $component
...
function bp_is_current_component($component = '')
{
    ...
    if (!empty($bp->current_component)) {
        ...
        if ($bp->current_component == $component) {
            ...
            isset($bp->{$component}->root_slug) &&
            $bp->{$component}->root_slug == $bp->current_component
        ) {
            $is_current_component = true;

            // Because slugs can differ from root_slugs, we should check them too.
        } elseif (
            isset($bp->{$component}->slug) &&
            $bp->{$component}->slug == $bp->current_component
        ) {
            $is_current_component = true;
            ...
            foreach ($bp->active_components as $id) {
                ...
                if (
                    empty($bp->{$id}->root_slug) ||
                    $bp->{$id}->root_slug != $bp->current_component
                ) {
                    continue;
                }
...
function bp_is_action_variable($action_variable = '', $position = false)
{
    ...
    if (false !== $position) {
        ...
        if ($action_variable) {
            $is_action_variable =
                $action_variable == bp_action_variable($position);
        } else {
        ...
        $is_action_variable = in_array(
            $action_variable,
            (array) bp_action_variables()
        );
    }
...
function bp_is_component_front_page($component = '')
{
    ...
    return (bool) apply_filters(
        ...
        $bp->pages->{$component}->id == $page_on_front,
        $component
    );
}
...
function bp_is_active($component = '', $feature = '')
{
    ...
    if (
        isset(buddypress()->active_components[$component]) ||
        in_array($component, buddypress()->required_components, true)
    ) {
        $retval = true;
        ...
        if (!empty($feature)) {
            ...
            if ('xprofile' === $component) {
                ...
                if ('cover_image' === $feature && 'profile' === $component) {
                    _doing_it_wrong(
                        'bp_is_active( \'profile\', \'cover_image\' )',
                        esc_html__(
                            'The cover image is a Members component feature, please use bp_is_active( \'members\', \'cover_image\' ) instead.',
                            'buddypress'
                        ),
                        '6.0.0'
                    );
                    ...
                    if (
                        !isset($members_component->features) ||
                        false ===
                            in_array(
                                $feature,
                                $members_component->features,
                                true
                            )
                    ) {
                        $retval = false;
            ...
            $component_features = isset(buddypress()->{$component}->features)
                ? buddypress()->{$component}->features
                : [];

            if (
                empty($component_features) ||
                false === in_array($feature, $component_features, true)
            ) {
                $retval = false;
            }
...
{
    return (bool) (bp_is_activity_component() &&
...
{
    return (bool) (bp_is_profile_component() ||
...
{
    return (bool) (bp_is_profile_component() &&
        bp_is_current_action('change-avatar'));
}
...
{
    return (bool) (bp_is_profile_component() &&
        bp_is_current_action('change-cover-image'));
}
...
{
    return (bool) (bp_is_user_blogs() &&
        bp_is_current_action('recent-comments'));
}
...
{
    return (bool) (bp_is_user_settings() &&
...
{
    return (bool) (bp_is_user_settings() &&
        bp_is_current_action('delete-account'));
}
...
function bp_is_groups_directory()
{
    if (
        ...
        (!bp_current_action() ||
            (bp_action_variable() &&
                bp_is_current_action(bp_get_groups_group_type_base())))
    ) {
        return true;
    }
...
function bp_is_group_home()
{
    if (
        bp_is_single_item() &&
        bp_is_groups_component() &&
        (!bp_current_action() || bp_is_current_action('home'))
    ) {
        return true;
    }
...
{
    return (bool) (bp_is_single_item() &&
        bp_is_groups_component() &&
        bp_is_current_action('admin'));
}
...
{
    return (bool) (bp_is_groups_component() &&
        bp_is_current_action('send-invites'));
}
...
function bp_is_group_leave()
{
    return (bool) (bp_is_groups_component() &&
        bp_is_single_item() &&
        bp_is_current_action('leave-group'));
}
...
{
    $bp = buddypress();
    return (bool) bp_is_group_home() &&
        !empty($bp->groups->current_group->front_template);
}
...
function bp_is_messages_inbox()
{
    if (
        bp_is_user_messages() &&
        (!bp_current_action() || bp_is_current_action('inbox'))
    ) {
        return true;
    }
...
{
    return (bool) (bp_is_current_component($component) &&
        true === call_user_func($callback));
}
...
function bp_get_title_parts($seplocation = 'right')
{
    ...
    if (!empty($displayed_user_name) && !is_404()) {
        ...
        if (!empty($bp->members->nav)) {
            $primary_nav_item = (array) $bp->members->nav->get_primary(
                ['slug' => $component_id],
                false
            );
        ...
        if (
            !empty($secondary_nav_item->name) &&
            !empty($bp->canonical_stack['action'])
        ) {
        ...
        if (
            bp_is_current_component($bp->default_component) &&
            bp_get_requested_url() === bp_displayed_user_domain()
        ) {
            ...
            $bp_title_parts = array_merge(
                $bp_title_parts,
                array_map('strip_tags', [$displayed_user_name, $component_name])
            );
        ...
        $bp_title_parts = [$bp->bp_options_title, $single_item_subnav];
        ...
        $bp_title_parts = [
            _x('Directory', 'component directory title', 'buddypress'),
        ];

        if (!empty($current_component)) {
            $bp_title_parts = [bp_get_directory_title($current_component)];
        }

        // Sign up page.
    } elseif (bp_is_register_page()) {
        $bp_title_parts = [__('Create an Account', 'buddypress')];

        // Activation page.
    } elseif (bp_is_activation_page()) {
        $bp_title_parts = [__('Activate Your Account', 'buddypress')];

        // Group creation page.
    } elseif (bp_is_group_create()) {
        $bp_title_parts = [__('Create a Group', 'buddypress')];

        // Blog creation page.
    } elseif (bp_is_create_blog()) {
        $bp_title_parts = [__('Create a Site', 'buddypress')];
    }
...
function bp_get_the_body_class($wp_classes = [], $custom_classes = false)
{
    ...
    if (!bp_is_directory()) {
        ...
        if (
            ...
            (bp_is_groups_directory() && bp_get_current_group_directory_type())
        ) {
            $bp_classes[] = 'type';
        }
    }
    ...
    if (bp_is_group()) {
        ...
        if (
            $group_types = bp_groups_get_group_type(
    ...
    $classes = array_unique(
        array_merge((array) $bp_classes, (array) $wp_classes)
    );
 ...
 * @return array
 */
function bp_get_the_post_class($wp_classes = [])
{
...
function bp_get_nav_menu_items($component = 'members')
{
    ...
    foreach ($bp->{$component}->nav->get_item_nav() as $nav_menu) {
        ...
        $link = bp_loggedin_user_domain()
            ...
            : trailingslashit(bp_displayed_user_domain() . $nav_menu->link);
        ...
        if (!empty($nav_menu->children)) {
            ...
            foreach ($nav_menu->children as $sub_menu) {
                ...
                if (
                    bp_is_current_action($sub_menu->slug) &&
                    bp_is_current_component($nav_menu->slug)
                ) {
 ...
 */
function bp_email_the_salutation($settings = [])
{
 ...
 */
function bp_email_get_salutation($settings = [])
{
