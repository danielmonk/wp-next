...
function bp_setup_theme_compat($theme = '')
{
    ...
    if (
        !isset($bp->theme_compat->packages[$theme]) ||
        !is_a($bp->theme_compat->packages[$theme], 'BP_Theme_Compat')
    ) {
        $theme = 'legacy';
    }
...
function bp_get_theme_compat_id()
{
    ...
    return apply_filters(
        ...
        buddypress()->theme_compat->theme->id
    );
}
...
function bp_get_theme_compat_version()
{
    ...
    return apply_filters(
        ...
        buddypress()->theme_compat->theme->version
    );
}
...
function bp_get_theme_compat_dir()
{
    ...
    return apply_filters(
        ...
        buddypress()->theme_compat->theme->dir
    );
}
...
function bp_get_theme_compat_url()
{
    ...
    return apply_filters(
        ...
        buddypress()->theme_compat->theme->url
    );
}
 ...
 */
function bp_set_theme_compat_templates($templates = [])
{
 ...
 */
function bp_set_theme_compat_feature($theme_id, $feature = [])
{
    ...
    if (isset($features[$feature['name']]->components)) {
        ...
        foreach (
            (array) $features[$feature['name']]->components
            as $component
        ) {
...
function bp_register_theme_compat_default_features()
{
    ...
    bp_set_theme_compat_feature('legacy', [
        'name' => 'cover_image',
        'settings' => [
 ...
 *                        Default: true.
 */
function bp_register_theme_package($theme = [], $override = true)
{
 ...
 */
function bp_theme_compat_reset_post($args = [])
{
...
function bp_template_include_theme_compat($template = '')
{
    ...
    if (
        isset(buddypress()->theme_compat->found_template) &&
...
{
    return (bool) (!bp_is_template_included() &&
        in_the_loop() &&
...
function bp_remove_all_filters($tag, $priority = false)
{
    ...
    if (isset($wp_filter[$tag])) {
        ...
        if (!empty($priority) && isset($wp_filter[$tag][$priority])) {
            ...
            $bp->filters->wp_filter[$tag][$priority] =
...
function bp_restore_all_filters($tag, $priority = false)
{
    ...
    if (isset($bp->filters->wp_filter[$tag])) {
        ...
        if (
            !empty($priority) &&
            isset($bp->filters->wp_filter[$tag][$priority])
        ) {
            // Store filters in a backup.
            $wp_filter[$tag][$priority] =
                $bp->filters->wp_filter[$tag][$priority];
