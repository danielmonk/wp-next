...
function bp_core_set_uri_globals()
{
    ...
    if (
        ...
        strpos($_SERVER['REQUEST_URI'], 'wp-load.php')
    ) {
    ...
    if (
        ...
        (bp_is_multiblog_mode() || 1 != bp_get_root_blog_id())
    ) {
        // Blow chunks.
    ...
    if (
        isset($_POST['search-terms']) &&
        !empty($bp_uri[0]) &&
        bp_get_search_slug() == $bp_uri[0]
    ) {
        $matches[] = 1;
        $match = new stdClass();
        $match->key = 'search';
        ...
        if ('members' == $match->key) {
            ...
            if ($after_member_slug) {
                ...
                if ($root_profile instanceof WP_User) {
                    ...
                    $bp->displayed_user->id = (int) bp_core_get_userid(
                        urldecode($after_member_slug)
                    );
                } else {
            ...
            if (
                ...
                $after_member_slug === bp_get_members_member_type_base() &&
                !empty($bp_uri[$uri_offset + 2])
            ) {
            ...
            if (bp_displayed_user_id()) {
                if (isset($bp_uri[$uri_offset + 2])) {
                    $bp_uri = array_merge(
                        [],
                        array_slice($bp_uri, $uri_offset + 2)
                    );
    ...
    $current_action = isset($bp_uri[$uri_offset + 1])
        ? $bp_uri[$uri_offset + 1]
        : '';
    ...
    // Reset the keys by merging with an empty array.
    $bp->action_variables = array_merge([], $bp->action_variables);
}
...
function bp_core_load_template($templates)
{
     ...
     */
    $located_template = apply_filters(
        'bp_located_template',
        $template,
        $filtered_templates
    );
...
function bp_core_no_access_wp_login_error($errors)
{
    ...
    $message = apply_filters(
        'bp_wp_login_error',
        __('You must log in to access the page you requested.', 'buddypress'),
    ...
    if (!bp_is_blog_page() && apply_filters('bp_do_redirect_canonical', true)) {
        ...
        if ($canonical_url !== $req_url_clean) {
            ...
            if (isset($bp->template_message)) {
                ...
                $message_type = isset($bp->template_message_type)
 ...
 */
function bp_get_canonical_url($args = [])
{
    ...
    if (
        ...
        $front_page_component = array_search(
            $page_on_front,
        ...
        if (
            ...
            $bp->canonical_stack['canonical_url'] = trailingslashit(
                bp_get_root_domain()
            );
             ...
             */
            $bp->canonical_stack['canonical_url'] = apply_filters(
    ...
    if (empty($bp->canonical_stack['canonical_url'])) {
        ...
        if (isset($bp->canonical_stack['component'])) {
            $url_stack[0] = trailingslashit(
                $url_stack[0] . $bp->canonical_stack['component']
            );
        }

        if (isset($bp->canonical_stack['action'])) {
            $url_stack[0] = trailingslashit(
                $url_stack[0] . $bp->canonical_stack['action']
            );
        }
...
function bp_get_requested_url()
{
    ...
    if (empty($bp->canonical_stack['requested_url'])) {
        $bp->canonical_stack['requested_url'] = is_ssl()
            ? 'https://'
            : 'http://';
