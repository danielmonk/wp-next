...
function bp_is_activation($basename = '')
{
    ...
    if (
        empty($action) ||
        !in_array($action, ['activate', 'activate-selected'])
    ) {
        return false;
    }
...
function bp_is_deactivation($basename = '')
{
    ...
    if (
        empty($action) ||
        !in_array($action, ['deactivate', 'deactivate-selected'])
    ) {
        return false;
    }
...
function bp_version_updater()
{
     ...
     */
    $default_components = apply_filters('bp_new_install_default_components', [
        'activity' => 1,
        'members' => 1,
        'settings' => 1,
        'xprofile' => 1,
...
function bp_pre_schema_upgrade()
{
    ...
    if ($raw_db_version < 9695) {
        ...
        $tables = [
            $bp_prefix . 'bp_activity_meta' => ['meta_key'],
            $bp_prefix . 'bp_groups_groupmeta' => ['meta_key'],
            $bp_prefix . 'bp_messages_meta' => ['meta_key'],
            $bp_prefix . 'bp_notifications_meta' => ['meta_key'],
            $bp_prefix . 'bp_user_blogs_blogmeta' => ['meta_key'],
            $bp_prefix . 'bp_xprofile_meta' => ['meta_key'],
        ];
...
function bp_update_to_2_7()
{
    bp_add_option(
        ...
        base64_encode(wp_generate_password(64, true, true))
    );
...
function bp_update_to_5_0()
{
    ...
    $field_id = $wpdb->get_var(
        $wpdb->prepare(
            "SELECT id FROM {$bp_prefix}bp_xprofile_fields WHERE name = %s",
            addslashes(bp_get_option('bp-xprofile-fullname-field-name'))
        )
    );
...
function bp_migrate_directory_page_titles()
{
    ...
    foreach ($bp_pages as $component => $page_id) {
        ...
        if (
            isset($default_titles[$component]) &&
            $default_titles[$component] !== $page->post_title
        ) {
            continue;
        }
...
function bp_core_maybe_install_signups()
{
    ...
    if (true === $table_exists) {
        ...
        $column_exists = $wpdb->query(
            "SHOW COLUMNS FROM {$signups_table} LIKE 'signup_id'"
        );
...
function bp_deactivation()
{
    ...
    if (in_array('bp-default', [get_template(), get_stylesheet()])) {
        switch_theme(WP_DEFAULT_THEME, WP_DEFAULT_THEME);
        update_option(
            'template_root',
            get_raw_theme_root(WP_DEFAULT_THEME, true)
        );
        update_option(
            'stylesheet_root',
            get_raw_theme_root(WP_DEFAULT_THEME, true)
