...
function bp_set_current_user_default_role()
{
    ...
    buddypress()->current_user->set_role(
        bp_get_option('default_role', 'subscriber')
    );
}
 ...
 */
function bp_current_user_can($capability, $args = [])
{
 ...
 * @return bool True if the user has the cap for the given parameters.
 */
function bp_user_can($user_id, $capability, $args = [])
{
    ...
    $retval = call_user_func_array('user_can', [$user_id, $capability, $args]);
...
function _bp_roles_init(WP_Roles $wp_roles)
{
    ...
    $filtered_list = wp_list_filter($caps_list, ['manage_options' => true]);
    ...
    if ($filtered_list) {
        ...
        foreach ($roles_list as $role) {
            ...
            if (isset($wp_roles->role_objects[$role])) {
                $wp_roles->role_objects[$role]->capabilities[
                    'bp_moderate'
                ] = true;
            }
        }
    ...
    if (bp_is_network_activated()) {
        foreach ($roles_list as $role) {
            unset(
                $wp_roles->roles[$role]['capabilities']['bp_moderate'],
                $wp_roles->role_objects[$role]->capabilities['bp_moderate']
            );
        }
    }
}
