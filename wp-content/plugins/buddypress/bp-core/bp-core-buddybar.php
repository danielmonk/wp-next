...
function bp_core_register_nav_screen_function($args = '')
{
    ...
    if (bp_is_current_component($r['slug']) || bp_is_current_item($r['slug'])) {
        ...
        if (
            !empty($r['default_subnav_slug']) &&
            bp_is_current_action($r['default_subnav_slug']) &&
            ...
            if (!empty($r['default_subnav_slug'])) {
                 ...
                 * @param array  $r     Parsed arguments for the nav item.
                 */
                $bp->current_action = apply_filters(
                    'bp_default_component_subnav',
                    $r['default_subnav_slug'],
                    $r
                );
            }
        }
    ...
    if (bp_is_current_component($parent_nav->slug)) {
        ...
        if (false !== $component_uri_key) {
            if (!empty($bp->unfiltered_uri[$component_uri_key + 1])) {
                $unfiltered_action =
                    $bp->unfiltered_uri[$component_uri_key + 1];
            }
        }
...
function bp_core_new_subnav_item($args, $component = null)
{
    ...
    if (
        ...
        if (
            ...
            (0 === strpos($args['parent_slug'], $group_slug) &&
                !buddypress()->members->nav->get_primary(
...
function bp_core_create_subnav_link($args = '', $component = 'members')
{
    ...
    if (empty($r['link'])) {
        ...
        $parent_nav = $bp->{$component}->nav->get_primary(
            ['slug' => $r['parent_slug']],
            false
        );
        ...
        if ($parent_nav) {
            ...
            if (
                !empty($parent_nav_item->default_subnav_slug) &&
                $r['slug'] === $parent_nav_item->default_subnav_slug
            ) {
 ...
 * @return null|false Returns false on failure.
 */
function bp_core_register_subnav_screen_function(
    $args = '',
    $component = 'members'
) {
    $bp = buddypress();
    ...
    if (
        !bp_is_current_component($r['parent_slug']) &&
        !bp_is_current_item($r['parent_slug'])
    ) {
        return;
    }
    ...
    if (
        (bp_current_action() && bp_is_current_action($r['slug'])) ||
        (bp_is_user() &&
            !bp_current_action() &&
            !empty($parent_nav->screen_function) &&
            $r['screen_function'] == $parent_nav->screen_function)
    ) {
        // If this is for site admins only and the user is not one, don't create the subnav item.
        if (
            !empty($r['site_admin_only']) &&
        ...
        if (
            !empty($hooked['status']) &&
            'failure' === $hooked['status'] &&
            isset($hooked['redirect_args'])
        ) {
...
function bp_core_maybe_hook_new_subnav_screen_function(
    ...
    if (
        !empty($subnav_item['site_admin_only']) &&
        !bp_current_user_can('bp_moderate')
    ) {
    ...
    if (!empty($subnav_item['user_has_access']) && !$site_admin_restricted) {
        ...
        if (is_user_logged_in()) {
            ...
            if (!empty($subnav_item['no_access_url'])) {
                ...
                $parent_nav_default = $bp->{$component}->nav->get_primary(
                    ['slug' => $bp->default_component],
                    false
                );
                ...
                if (
                    bp_is_my_profile() ||
                    (isset($parent_nav_default_item) &&
                        $parent_nav_default_item->show_for_displayed_user)
                ) {
                    $message = __(
                        'You do not have access to that page.',
                        'buddypress'
...
function bp_nav_item_has_subnav($nav_item = '', $component = 'members')
{
    ...
    $has_subnav = (bool) $bp->{$component}->nav->get_secondary(
        ['parent_slug' => $nav_item],
        false
    );
...
function bp_core_remove_nav_item($slug, $component = null)
{
    ...
    if (!$component && bp_is_active('groups') && isset($bp->groups->nav)) {
        if ($bp->groups->nav->get_primary(['slug' => $slug])) {
            $component = 'groups';
        }
    }
...
function bp_core_remove_subnav_item($parent_slug, $slug, $component = null)
{
    ...
    // Backward compatibility for removing group nav items using the group slug as `$parent_slug`.
    if (!$component && bp_is_active('groups') && isset($bp->groups->nav)) {
        if ($bp->groups->nav->get_primary(['slug' => $parent_slug])) {
            $component = 'groups';
        }
    }
...
function bp_core_reset_subnav_items($parent_slug, $component = 'members')
{
    ...
    $subnav_items = $bp->{$component}->nav->get_secondary(
