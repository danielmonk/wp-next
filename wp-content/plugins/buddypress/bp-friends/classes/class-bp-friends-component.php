...
class BP_Friends_Component extends BP_Component
{
    ...
    {
        $includes = ['cache', 'filters', 'template', 'functions', 'widgets'];
    ...
    public function setup_admin_bar($wp_admin_nav = [])
    {
        ...
        if (is_user_logged_in()) {
            ...
            $friends_link = trailingslashit(
                bp_loggedin_user_domain() . bp_get_friends_slug()
            );

            // Pending friend requests.
            $count = count(
                friends_get_friendship_request_user_ids(bp_loggedin_user_id())
            );
            if (!empty($count)) {
                $title = sprintf(
                ...
                $pending = sprintf(
                    ...
                    _x(
                        'Pending Requests %s',
                        'My Account Friends menu sub nav',
                        'buddypress'
                    ),
                    '<span class="count">' .
                        bp_core_number_format($count) .
    ...
    {
        parent::rest_api_init(['BP_REST_Friends_Endpoint']);
    }
}
