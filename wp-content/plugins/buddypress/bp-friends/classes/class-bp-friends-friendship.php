...
class BP_Friends_Friendship
{
    ...
    public function save()
    {
        ...
        $this->friend_user_id = apply_filters(
        ...
        $this->is_limited = apply_filters(
            'friends_friendship_is_limited_before_save',
            $this->is_limited,
            $this->id
        );
        $this->date_created = apply_filters(
         ...
         */
        do_action_ref_array('friends_friendship_before_save', [&$this]);

        // Update.
        if (!empty($this->id)) {
            $result = $wpdb->query(
                $wpdb->prepare(
                    "UPDATE {$bp->friends->table_name} SET initiator_user_id = %d, friend_user_id = %d, is_confirmed = %d, is_limited = %d, date_created = %s WHERE id = %d",
                    $this->initiator_user_id,
                    $this->friend_user_id,
                    $this->is_confirmed,
                    $this->is_limited,
            ...
            $result = $wpdb->query(
                $wpdb->prepare(
                    "INSERT INTO {$bp->friends->table_name} ( initiator_user_id, friend_user_id, is_confirmed, is_limited, date_created ) VALUES ( %d, %d, %d, %d, %s )",
                    $this->initiator_user_id,
                    $this->friend_user_id,
                    $this->is_confirmed,
                    $this->is_limited,
         ...
         * @param BP_Friends_Friendship $value Current friendship request object.
         */
        do_action_ref_array('friends_friendship_after_save', [&$this]);
    ...
    public function delete()
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$bp->friends->table_name} WHERE id = %d",
                $this->id
            )
        );
    }
     ...
     */
    public static function get_friendships(
        $user_id,
        $args = [],
        $operator = 'AND'
    ) {
        ...
        $uncached_friendship_ids = bp_get_non_cached_ids(
            $friendship_ids,
            'bp_friends_friendships'
        );
        if (!empty($uncached_friendship_ids)) {
            $uncached_friendships = self::get_friendships_by_id(
                $uncached_friendship_ids
            );

            foreach ($uncached_friendships as $uncached_friendship) {
                wp_cache_set(
                    $uncached_friendship->id,
                    $uncached_friendship,
                    'bp_friends_friendships'
                );
            }
        ...
        $friendships = [];
        $int_keys = ['id', 'initiator_user_id', 'friend_user_id'];
        $bool_keys = ['is_confirmed', 'is_limited'];
        foreach ($friendship_ids as $friendship_id) {
            ...
            if ('OR' == $operator || 'NOT' == $operator) {
                ...
                foreach ($filters as $filter_name => $filter_value) {
                    if (
                        isset($friendship->{$filter_name}) &&
                        $filter_value == $friendship->{$filter_name}
                    ) {
                        $matched++;
                    }
                }
    ...
    public static function get_friendship_ids_for_user($user_id)
    {
        ...
        $friendship_ids = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT id FROM {$bp->friends->table_name} WHERE (initiator_user_id = %d OR friend_user_id = %d) ORDER BY date_created DESC",
                $user_id,
                $user_id
            )
        );
     ...
     */
    public static function get_friend_user_ids(
        $user_id,
        $friend_requests_only = false,
        $assoc_arr = false
    ) {
        global $wpdb;
    ...
    public static function total_friend_count($user_id = 0)
    {
        ...
        if (
            !$count &&
            !bp_get_user_meta($user_id, 'total_friend_count', true)
        ) {
            return 0;
        }
    ...
    public static function search_friends(
        ...
        if (
            !($friend_ids = BP_Friends_Friendship::get_friend_user_ids(
                $user_id
            ))
        ) {
            return false;
        ...
        if (bp_is_active('xprofile')) {
            $sql = $wpdb->prepare(
                "SELECT DISTINCT user_id FROM {$bp->profile->table_name_data} WHERE user_id IN ({$fids}) AND value LIKE %s {$pag_sql}",
                $search_terms_like
            );
            $total_sql = $wpdb->prepare(
                "SELECT COUNT(DISTINCT user_id) FROM {$bp->profile->table_name_data} WHERE user_id IN ({$fids}) AND value LIKE %s",
                $search_terms_like
            );
        } else {
            $sql = $wpdb->prepare(
                "SELECT DISTINCT user_id FROM {$wpdb->usermeta} WHERE user_id IN ({$fids}) AND meta_key = 'nickname' AND meta_value LIKE %s {$pag_sql}",
                $search_terms_like
            );
            $total_sql = $wpdb->prepare(
                "SELECT COUNT(DISTINCT user_id) FROM {$wpdb->usermeta} WHERE user_id IN ({$fids}) AND meta_key = 'nickname' AND meta_value LIKE %s",
                $search_terms_like
            );
        }
     ...
     */
    public static function check_is_friend(
        $initiator_userid,
        ...
        return bp_core_get_incremented_cache(
            $initiator_userid . ':' . $possible_friend_userid,
            'bp_friends'
        );
    }
     ...
     * @return null
     */
    public static function update_bp_friends_cache(
        $user_id,
        $possible_friend_ids
    ) {
        global $wpdb;
        ...
        $sql = $wpdb->prepare(
            "SELECT initiator_user_id, friend_user_id, is_confirmed FROM {$bp->friends->table_name} WHERE (initiator_user_id = %d AND friend_user_id IN ({$friend_ids_sql}) ) OR (initiator_user_id IN ({$friend_ids_sql}) AND friend_user_id = %d )",
            $user_id,
            $user_id
        );
        ...
        foreach ($not_friends as $not_friend_id) {
            bp_core_set_incremented_cache(
                $user_id . ':' . $not_friend_id,
                'bp_friends',
                'not_friends'
            );
            bp_core_set_incremented_cache(
                $not_friend_id . ':' . $user_id,
                'bp_friends',
                'not_friends'
            );
        }
    ...
    public static function get_bulk_last_active($user_ids)
    {
        ...
        usort($last_activities, function ($a, $b) {
            ...
            return strtotime($a['date_recorded']) <
                strtotime($b['date_recorded'])
                ? 1
                : -1;
        });
    ...
    public static function accept($friendship_id)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$bp->friends->table_name} SET is_confirmed = 1, date_created = %s WHERE id = %d AND friend_user_id = %d",
    ...
    public static function withdraw($friendship_id)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$bp->friends->table_name} WHERE id = %d AND initiator_user_id = %d",
                $friendship_id,
                bp_loggedin_user_id()
            )
        );
    ...
    public static function reject($friendship_id)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$bp->friends->table_name} WHERE id = %d AND friend_user_id = %d",
                $friendship_id,
                bp_loggedin_user_id()
            )
        );
    ...
    public static function search_users(
        ...
        if (bp_is_active('xprofile')) {
            $sql = $wpdb->prepare(
                "SELECT DISTINCT d.user_id as id FROM {$bp->profile->table_name_data} d, {$users_table} u WHERE d.user_id = u.id AND d.value LIKE %s ORDER BY d.value DESC {$pag_sql}",
                $search_terms_like
            );
        } else {
            $sql = $wpdb->prepare(
                "SELECT DISTINCT user_id as id FROM {$usermeta_table} WHERE meta_value LIKE %s ORDER BY d.value DESC {$pag_sql}",
                $search_terms_like
            );
        }
    ...
    public static function search_users_count($filter)
    {
        ...
        if (bp_is_active('xprofile')) {
            $sql = $wpdb->prepare(
                "SELECT COUNT(DISTINCT d.user_id) FROM {$bp->profile->table_name_data} d, {$users_table} u WHERE d.user_id = u.id AND d.value LIKE %s",
                $search_terms_like
            );
        } else {
            $sql = $wpdb->prepare(
                "SELECT COUNT(DISTINCT user_id) FROM {$usermeta_table} WHERE meta_value LIKE %s",
                $search_terms_like
            );
        }
    ...
    public static function get_random_friends($user_id, $total_friends = 5)
    {
        ...
        for ($i = 0, $count = count($results); $i < $count; ++$i) {
            $fids[] =
                $results[$i]->friend_user_id == $user_id
                    ? $results[$i]->initiator_user_id
                    : $results[$i]->friend_user_id;
        }
    ...
    public static function get_invitable_friend_count($user_id, $group_id)
    {
        ...
        for ($i = 0, $count = count($friend_ids); $i < $count; ++$i) {
            ...
            if (
                groups_check_user_has_invite((int) $friend_ids[$i], $group_id)
            ) {
                continue;
            }
            ...
            if (
                false === $is_group_admin &&
                groups_is_user_banned((int) $friend_ids[$i], $group_id)
            ) {
                continue;
            }
    ...
    public static function get_friendships_by_id($friendship_ids)
    {
        ...
        return $wpdb->get_results(
            "SELECT * FROM {$bp->friends->table_name} WHERE id IN ({$friendship_ids})"
        );
    }
    ...
    public static function delete_all_for_user($user_id)
    {
        ...
        if ($friendship_ids) {
            $friendship_ids_sql = implode(
                ',',
                wp_parse_id_list($friendship_ids)
            );
            $wpdb->query(
                "DELETE FROM {$bp->friends->table_name} WHERE id IN ({$friendship_ids_sql})"
            );
        }
        ...
        if (bp_is_active('notifications')) {
            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM {$bp->notifications->table_name} WHERE component_name = 'friends' AND ( component_action = 'friendship_request' OR component_action = 'friendship_accepted' ) AND item_id = %d",
                    $user_id
                )
            );
        }
