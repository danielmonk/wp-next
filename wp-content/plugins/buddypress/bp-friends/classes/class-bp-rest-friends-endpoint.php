...
class BP_REST_Friends_Endpoint extends WP_REST_Controller
{
        ...
        register_rest_route(
            ...
            [
                'args' => [
                    'id' => [
                        'description' => __(
                            'Numeric identifier of a user ID.',
                            'buddypress'
                        ),
                        'type' => 'integer',
                    ],
    ...
    public function get_items($request)
    {
        ...
        if (!$user instanceof WP_User) {
            return new WP_Error(
                ...
                __(
                    'There was a problem confirming if user is valid.',
                    'buddypress'
                ),
                [
                    'status' => 404,
    ...
    public function create_item($request)
    {
        ...
        if (
            'not_friends' !==
            friends_check_friendship_status($initiator_id->ID, $friend_id->ID)
        ) {
            return new WP_Error(
                'bp_rest_friends_create_item_failed',
                __(
        ...
        if (!friends_add_friend($initiator_id->ID, $friend_id->ID, $force)) {
            return new WP_Error(
                'bp_rest_friends_create_item_failed',
                __(
                    'There was an error trying to create the friendship.',
                    'buddypress'
                ),
                [
                    'status' => 500,
    ...
    public function delete_item($request)
    {
        ...
        if (true === $request->get_param('force')) {
            ...
            if (bp_loggedin_user_id() === $friendship->initiator_user_id) {
                $deleted = friends_withdraw_friendship(
                    $friendship->initiator_user_id,
                    $friendship->friend_user_id
                );
            } else {
    ...
    protected function prepare_links($friendship)
    {
        ...
        $links = [
            ...
            'initiator' => [
                'href' => rest_url(
                    bp_rest_get_user_url($friendship->initiator_user_id)
                ),
                'embeddable' => true,
            ],
            'friend' => [
                'href' => rest_url(
                    bp_rest_get_user_url($friendship->friend_user_id)
                ),
                'embeddable' => true,
            ],
        ];
    ...
    public function get_endpoint_args_for_item_schema(
        ...
        if (WP_REST_Server::EDITABLE === $method) {
            ...
            $args['force'] = [
                'description' => __(
                    'Whether to force the friendship agreement.',
                    'buddypress'
                ),
                'default' => false,
                'type' => 'boolean',
            ...
            $args['force'] = [
                'description' => __(
                    'Whether to force friendship removal.',
                    'buddypress'
                ),
                'default' => false,
                'type' => 'boolean',
        ...
        $schema = [
            ...
            'properties' => [
                'id' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'Unique numeric identifier of the friendship.',
                        'buddypress'
                    ),
                    'type' => 'integer',
                ],
                ...
                'is_confirmed' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'Whether the friendship been confirmed/accepted.',
                        'buddypress'
                    ),
                    'readonly' => true,
                    'type' => 'boolean',
    ...
    public function get_collection_params()
    {
        ...
        $params['user_id'] = [
            'description' => __(
                'ID of the member whose friendships are being retrieved.',
                'buddypress'
            ),
        ...
        $params['is_confirmed'] = [
            'description' => __(
                'Wether the friendship has been accepted.',
                'buddypress'
            ),
            'default' => 0,
            'type' => 'integer',
