...
class BP_Core_Friends_Widget extends WP_Widget
{
    ...
    function __construct()
    {
        ...
        parent::__construct(
            false,
            $name = _x('(BuddyPress) Friends', 'widget name', 'buddypress'),
            $widget_ops
        );
    ...
    function widget($args, $instance)
    {
        ...
        $link = trailingslashit(
            bp_displayed_user_domain() . bp_get_friends_slug()
        );
        $instance['title'] = sprintf(
            __("%s's Friends", 'buddypress'),
		...
		<?php if (bp_has_members($members_args)): ?>
			<div class="item-options" id="friends-list-options">
				<a href="<?php bp_members_directory_permalink(); ?>" id="newest-friends" <?php if (
    $instance['friend_default'] == 'newest'
): ?>class="selected"<?php endif; ?>><?php _e('Newest', 'buddypress'); ?></a>
				| <a href="<?php bp_members_directory_permalink(); ?>" id="recently-active-friends" <?php if (
    $instance['friend_default'] == 'active'
): ?>class="selected"<?php endif; ?>><?php _e('Active', 'buddypress'); ?></a>
				| <a href="<?php bp_members_directory_permalink(); ?>" id="popular-friends" <?php if (
    $instance['friend_default'] == 'popular'
						...
						<div class="item">
							...
							<div class="item-meta">
								<?php if ('newest' == $instance['friend_default']): ?>
									<span class="activity" data-livestamp="<?php bp_core_iso8601_date(
             bp_get_member_registered(['relative' => false])
         ); ?>"><?php bp_member_registered(); ?></span>
								<?php elseif ('active' == $instance['friend_default']): ?>
									<span class="activity" data-livestamp="<?php bp_core_iso8601_date(
             bp_get_member_last_active(['relative' => false])
         ); ?>"><?php bp_member_last_active(); ?></span>
								<?php else: ?>
    ...
    endwhile; ?>
			...
			<input type="hidden" name="friends_widget_max" id="friends_widget_max" value="<?php echo absint(
       $instance['max_friends']
   ); ?>" />
  ...
  $members_template = $old_members_template;
    ...
    function update($new_instance, $old_instance)
    {
        ...
        $instance['friend_default'] = sanitize_text_field(
            $new_instance['friend_default']
        );
    ...
    function form($instance)
    {
      ...
      'link_title'
  ); ?>"><input type="checkbox" name="<?php echo $this->get_field_name('link_title'); ?>" id="<?php echo $this->get_field_id('link_title'); ?>" value="1" <?php checked($link_title); ?> /> <?php _e('Link widget title to Members directory', 'buddypress'); ?></label></p>

		<p><label for="<?php echo $this->get_field_id(
      'max_friends'
  ); ?>"><?php _e('Max friends to show:', 'buddypress'); ?> <input class="widefat" id="<?php echo $this->get_field_id('max_friends'); ?>" name="<?php echo $this->get_field_name('max_friends'); ?>" type="text" value="<?php echo absint($max_friends); ?>" style="width: 30%" /></label></p>

		<p>
			<label for="<?php echo $this->get_field_id(
       'friend_default'
   ); ?>"><?php _e('Default friends to show:', 'buddypress'); ?></label>
			<select name="<?php echo $this->get_field_name(
       'friend_default'
   ); ?>" id="<?php echo $this->get_field_id('friend_default'); ?>">
