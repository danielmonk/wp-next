...
{
    if (
        !($friend_ids = wp_cache_get(
            'friends_friend_ids_' . bp_displayed_user_id(),
            'bp'
        ))
    ) {
   ...
   bp_word_or_name(
       __("My Friends", 'buddypress'),
       __("%s's Friends", 'buddypress')
   ); ?>
			&nbsp;
			(<?php echo BP_Friends_Friendship::total_friend_count(
       bp_displayed_user_id()
   ); ?>)
			&nbsp;
			<span>
		...
		<?php } else { ?>
			...
			<div id="message" class="info">
				<p>
					<?php /* translators: %s: member name */
     bp_word_or_name(
         __("You haven't added any friend connections yet.", 'buddypress'),
         __("%s hasn't created any friend connections yet.", 'buddypress')
     ); ?>
				</p>
			</div>
...
function bp_friends_random_members($total_members = 5)
{
	...
	<?php } else { ?>
		...
		<div id="message" class="info">
			<p><?php _e(
       "There aren't enough site members to show a random sample just yet.",
       'buddypress'
   ); ?></p>
		</div>
...
function bp_get_member_total_friend_count()
{
    ...
    $total_friend_count =
        (int) $members_template->member->total_friend_count; /**
     ...
     */
    return apply_filters(
        'bp_get_member_total_friend_count' /* translators: %d: total friend count */,
        sprintf(
            _n('%d friend', '%d friends', $total_friend_count, 'buddypress'),
            number_format_i18n($total_friend_count)
        )
    );
} /**
...
function bp_get_potential_friend_id($user_id = 0)
{
    ...
    if (empty($user_id) && isset($friends_template->friendship->friend)) {
        ...
        empty($user_id) &&
        !isset($friends_template->friendship->friend)
    ) {
...
function bp_is_friend($user_id = 0)
{
    ...
    return apply_filters(
        'bp_is_friend',
        friends_check_friendship_status(bp_loggedin_user_id(), $user_id),
        $user_id
    );
}
/**
 ...
 */
function bp_get_add_friend_button(
    $potential_friend_id = 0,
    $friend_status = false
) {
...
function bp_get_total_friend_count($user_id = 0)
{
     ...
     */ return apply_filters(
        ...
        friends_get_total_friend_count($user_id),
        $user_id
    );
} /**
...
function bp_friend_get_total_requests_count($user_id = 0)
{
    ...
    return apply_filters(
        ...
        count(BP_Friends_Friendship::get_friend_user_ids($user_id, true)),
        $user_id
    );
} /** Stats **********************************************************************/ /**
...
function bp_friends_get_profile_stats($args = '')
{
    ...
    if (empty($r['output'])) {
        ...
        if (!empty($r['user_id'])) {
            ...
            if (empty($r['friends'])) {
                $r['friends'] = absint(
                    friends_get_total_friend_count($r['user_id'])
                );
            } // If friends exist, show some formatted output.
            $r['output'] =
                $r['before']; /* translators: %d: total friend count */
            $r['output'] .= sprintf(
                _n('%d friend', '%d friends', $r['friends'], 'buddypress'),
                '<strong>' . number_format_i18n($r['friends']) . '</strong>'
            );
            $r['output'] .= $r['after'];
        }
    } /**
