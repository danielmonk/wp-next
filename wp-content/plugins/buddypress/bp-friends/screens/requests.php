...
{
    if (
        bp_is_action_variable('accept', 0) &&
        is_numeric(bp_action_variable(1))
    ) {
        // Check the nonce.
        ...
        if (friends_accept_friendship(bp_action_variable(1))) {
            ...
            bp_core_add_message(
                __('Friendship could not be accepted', 'buddypress'),
                'error'
            );
        }
        ...
        bp_is_action_variable('reject', 0) &&
        is_numeric(bp_action_variable(1))
    ) {
        // Check the nonce.
        ...
        if (friends_reject_friendship(bp_action_variable(1))) {
            ...
            bp_core_add_message(
                __('Friendship could not be rejected', 'buddypress'),
                'error'
            );
        }
        ...
        bp_is_action_variable('cancel', 0) &&
        is_numeric(bp_action_variable(1))
    ) {
        // Check the nonce.
        ...
        if (
            ...
            bp_core_add_message(
                __('Friendship request withdrawn', 'buddypress')
            );
        } else {
            bp_core_add_message(
                __('Friendship request could not be withdrawn', 'buddypress'),
                'error'
            );
        }
