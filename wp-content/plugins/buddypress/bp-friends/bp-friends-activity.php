...
function friends_register_activity_actions()
{
    ...
    bp_activity_set_action(
        $bp->friends->id,
        'friends_register_activity_action',
        __('New friendship created', 'buddypress')
    );
 ...
 */
function bp_friends_format_activity_action_friendship_accepted(
    $action,
    $activity
) {
    ...
    $action = sprintf(
        esc_html__('%1$s and %2$s are now friends', 'buddypress'),
        $initiator_link,
        $friend_link
    );
    ...
    if (has_filter('friends_activity_friendship_accepted_action')) {
        ...
        $action = apply_filters(
            'friends_activity_friendsip_accepted_action',
            $action,
            $friendship
        );
    }
 ...
 * @return string $action Formatted activity action.
 */
function bp_friends_format_activity_action_friendship_created(
    $action,
    $activity
) {
    $initiator_link = bp_core_get_userlink($activity->user_id);
...
function bp_friends_filter_activity_just_me_scope($retval = [], $filter = [])
{
    ...
    $action = isset($filter['filter']['action'])
