...
function friends_accept_friendship($friendship_id)
{
    ...
    if (
        empty($friendship->is_confirmed) &&
        BP_Friends_Friendship::accept($friendship_id)
    ) {
...
function friends_reject_friendship($friendship_id)
{
    ...
    if (
        empty($friendship->is_confirmed) &&
        BP_Friends_Friendship::reject($friendship_id)
    ) {
        /**
         ...
         */
        do_action_ref_array('friends_friendship_rejected', [
            $friendship_id,
            &$friendship,
        ]);
        return true;
...
function friends_withdraw_friendship($initiator_userid, $friend_userid)
{
    ...
    if (
        empty($friendship->is_confirmed) &&
        BP_Friends_Friendship::withdraw($friendship_id)
    ) {
        // @deprecated Since 1.9
        do_action_ref_array('friends_friendship_whithdrawn', [
            $friendship_id,
            &$friendship,
        ]);
         ...
         */
        do_action_ref_array('friends_friendship_withdrawn', [
            $friendship_id,
            &$friendship,
        ]);
...
function friends_check_friendship($user_id, $possible_friend_id)
{
    if (
        'is_friend' ==
        BP_Friends_Friendship::check_is_friend($user_id, $possible_friend_id)
    ) {
        return true;
    }
...
function friends_check_friendship_status($user_id, $possible_friend_id)
{
    ...
    return BP_Friends_Friendship::check_is_friend(
        $user_id,
        $possible_friend_id
    );
}
...
{
    return BP_Friends_Friendship::get_friendship_id(
        $initiator_user_id,
        $friend_user_id
    );
}
 ...
 * @return array
 */
function friends_get_friend_user_ids(
    $user_id,
    $friend_requests_only = false,
    $assoc_arr = false
) {
    return BP_Friends_Friendship::get_friend_user_ids(
        $user_id,
...
function friends_search_friends(
    ...
    return BP_Friends_Friendship::search_friends(
        $search_terms,
        $user_id,
        $pag_num,
        $pag_page
 ...
 * @return array See {@link BP_Core_User::get_users()}.
 */
function friends_get_alphabetically(
    $user_id,
    $per_page = 0,
    $page = 0,
    $filter = ''
...
function friends_get_friends_invite_list($user_id = 0, $group_id = 0)
{
     ...
     */
    $args = apply_filters('bp_friends_pre_get_invite_list', [
        'user_id' => $user_id,
        'type' => 'alphabetical',
        'per_page' => 0,
    ]);
    ...
    if (bp_has_members($args)) {
        ...
        while (bp_members()):
            ...
            if (
                false === $user_is_admin &&
                groups_is_user_banned($friend_user_id, $group_id)
            ) {
                continue;
            }
...
{
    return BP_Friends_Friendship::get_invitable_friend_count(
        $user_id,
        $group_id
    );
}
 ...
 */
function friends_search_users(
    $search_terms,
    $user_id,
    $pag_num = 0,
    $pag_page = 0
) {
    $user_ids = BP_Friends_Friendship::search_users(
        $search_terms,
        $user_id,
        $pag_num,
        $pag_page
 ...
 *                                  will --.
 */
function friends_update_friend_totals(
    $initiator_user_id,
    $friend_user_id,
    $status = 'add'
) {
...
function bp_friends_prime_mentions_results()
{
    ...
    wp_localize_script('bp-mentions', 'BP_Suggestions', [
        'friends' => $results,
    ]);
}
add_action(
 ...
 */
function friends_notification_new_request(
    $friendship_id,
    $initiator_id,
    $friend_id
) {
 ...
 * @param int $friend_id     ID of the request recipient.
 */
function friends_notification_accepted_request(
    $friendship_id,
    $initiator_id,
    $friend_id
) {
...
function bp_friends_personal_data_exporter($email_address, $page)
{
    ...
    $friendships = BP_Friends_Friendship::get_friendships($user->ID, [
        'is_confirmed' => true,
        'page' => $page,
        'per_page' => $number,
    ]);
