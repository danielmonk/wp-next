...
function bp_core_get_user_domain(
    ...
    $after_domain = bp_core_enable_root_profiles()
        ? $username
        : bp_get_members_root_slug() . '/' . $username;
...
function bp_core_get_username(
    ...
    if (!$user_nicename && !$user_login) {
        ...
        $username = bp_is_username_compatibility_mode()
            ? $user_login
            : $user_nicename;
    }
...
function bp_members_get_user_nicename($user_id)
{
    ...
    return apply_filters(
        ...
        get_the_author_meta('nicename', $user_id)
    );
}
...
function bp_core_get_userlink($user_id, $no_anchor = false, $just_link = false)
{
    ...
    return apply_filters(
        ...
        '<a href="' . esc_url($url) . '">' . $display_name . '</a>',
        $user_id
    );
}
...
function bp_core_get_userlink_by_username($username)
{
    ...
    return apply_filters(
        ...
        bp_core_get_userlink($user_id, false, false, true)
    );
}
...
function bp_core_get_active_member_count()
{
    ...
    if (false === $count) {
        ...
        $exclude_users_sql = !empty($exclude_users)
            ? "AND user_id NOT IN (" .
                implode(',', wp_parse_id_list($exclude_users)) .
                ")"
            : '';
 ...
 */
function bp_core_process_spammer_status(
    $user_id,
    $status,
    $do_wp_cleanup = true
) {
    ...
    if ($do_wp_cleanup) {
        ...
        if (is_multisite()) {
            ...
            foreach ($meta as $key => $val) {
                ...
                $site_id = str_replace(
                    [$wpdb->base_prefix, '_capabilities'],
                    '',
                    $key
                );
                ...
                if (
                    empty($counts['avail_roles']['administrator']) ||
                    $counts['avail_roles']['administrator'] > 1
                ) {
                    continue;
                }
...
function bp_is_user_spammer($user_id = 0)
{
    ...
    switch ($user_id) {
        case bp_loggedin_user_id():
            $user = !empty($bp->loggedin_user->userdata)
                ? $bp->loggedin_user->userdata
                : false;
            break;

        case bp_displayed_user_id():
            $user = !empty($bp->displayed_user->userdata)
        ...
        case bp_get_member_user_id():
            ...
            $user =
                isset($members_template) && isset($members_template->member)
 ...
 * @return string|null
 */
function _bp_get_user_meta_last_activity_warning(
    $retval,
    $object_id,
    $meta_key,
    $single
 ...
 */
function _bp_update_user_meta_last_activity_warning(
    $meta_id,
    $object_id,
    $meta_key,
    $meta_value
    ...
    if ('last_activity' === $meta_key) {
        _doing_it_wrong(
            'update_user_meta( $user_id, \'last_activity\' )',
            __(
...
function bp_last_activity_migrate()
{
    ...
    $wpdb->query(
        $wpdb->prepare(
            "DELETE FROM {$bp->members->table_name_last_activity} WHERE component = %s AND type = 'last_activity'",
            $bp->members->id
        )
    );
...
function bp_core_boot_spammer($user)
{
    ...
    if (
        is_a($user, 'WP_User') &&
        ((is_multisite() && (int) $user->spam) || 1 == $user->user_status)
    ) {
        return new WP_Error(
            'invalid_username',
            __(
...
function bp_core_get_illegal_names($value = '', $oldvalue = '')
{
     ...
     */
    $filtered_illegal_names = apply_filters(
     ...
     */
    $wp_filtered_illegal_names = apply_filters('illegal_user_logins', []);

    // First merge BuddyPress illegal names.
    $bp_merged_names = array_merge(
        (array) $filtered_illegal_names,
        (array) $db_illegal_names
    );

    // Then merge WordPress and BuddyPress illegal names.
    $merged_names = array_merge(
        (array) $wp_filtered_illegal_names,
        (array) $bp_merged_names
    );
...
add_filter(
    'pre_update_site_option_illegal_names',
 ...
 */
function bp_core_add_validation_error_messages(
    WP_Error $errors,
    $validation_results
) {
    if (!empty($validation_results['invalid'])) {
        $errors->add(
            'user_email',
            __('Please check your email address.', 'buddypress')
        );
    }
    ...
    if (!empty($validation_results['domain_banned'])) {
        $errors->add(
            'user_email',
            __('Sorry, that email address is not allowed!', 'buddypress')
        );
    }
    ...
    if (!empty($validation_results['in_use'])) {
        $errors->add(
            'user_email',
            __('Sorry, that email address is already used!', 'buddypress')
        );
    }
}
...
function bp_core_validate_user_signup($user_name, $user_email)
{
    ...
    if (function_exists('wpmu_validate_user_signup')) {
        ...
        if (empty($user_name)) {
            $errors->add(
                'user_name',
                __('Please enter a username', 'buddypress')
            );
        }
        ...
        if (in_array($user_name, (array) $illegal_names)) {
            $errors->add(
                'user_name',
                __('That username is not allowed', 'buddypress')
            );
        }
        ...
        if (strlen($user_name) < 4) {
            $errors->add(
                'user_name',
                __('Username must be at least 4 characters', 'buddypress')
            );
        }
        ...
        if ($match[0] == $user_name) {
            $errors->add(
                'user_name',
                __('Sorry, usernames must have letters too!', 'buddypress')
            );
        }
        ...
        $signup =
            isset($signups['signups']) && !empty($signups['signups'][0])
                ? $signups['signups'][0]
                : false;
        ...
        if (username_exists($user_name) || !empty($signup)) {
            $errors->add(
                'user_name',
                __('Sorry, that username already exists!', 'buddypress')
            );
        }
 ...
 */
function bp_members_validate_user_password(
    $pass,
    $confirm_pass,
    $userdata = null
) {
    ...
    if (!$pass || !$confirm_pass) {
        $errors->add(
            ...
            __('Please make sure you enter your password twice', 'buddypress')
        );
    }

    if ($pass && $confirm_pass && $pass !== $confirm_pass) {
        $errors->add(
            'mismatching_user_password',
            __('The passwords you entered do not match.', 'buddypress')
        );
    }
...
function bp_core_validate_blog_signup($blog_url, $blog_title)
{
    ...
    return apply_filters(
        ...
        wpmu_validate_blog_signup($blog_url, $blog_title)
    );
}
...
function bp_core_signup_user(
    ...
    if (is_multisite()) {
         ...
         */
        if (
            !defined('BP_SIGNUPS_SKIP_USER_CREATION') ||
        ...
        if (
            ...
            if (
                ...
                isset($usermeta['field_' . bp_xprofile_fullname_field_id()])
            ) {
                $salutation =
                    $usermeta['field_' . bp_xprofile_fullname_field_id()];
            }
...
function bp_core_activate_signup($key)
{
    ...
    if (is_multisite()) {
        ...
        if ($signup->active) {
            if (empty($signup->domain)) {
                return new WP_Error(
                    'already_active',
                    __('The user is already active.', 'buddypress'),
                    $signup
                );
            } else {
                return new WP_Error(
                    'already_active',
                    __('The site is already active.', 'buddypress'),
                    $signup
                );
            }
        }
        ...
        if (!$user_id) {
            return new WP_Error(
                'create_user',
                __('Could not create user', 'buddypress'),
                $signup
            );
        }
        ...
        if (isset($user_already_exists)) {
            return new WP_Error(
                'user_already_exists',
                __('That username is already activated.', 'buddypress'),
                $signup
            );
        }
        ...
        $user = [
            'user_id' => $user_id,
            'password' => isset($signup->meta['password'])
    ...
    if (bp_is_active('xprofile')) {
        if (!empty($user['meta']['profile_field_ids'])) {
            $profile_field_ids = explode(
                ',',
                $user['meta']['profile_field_ids']
            );

            foreach ((array) $profile_field_ids as $field_id) {
                $current_field = isset($user['meta']["field_{$field_id}"])
                    ? $user['meta']["field_{$field_id}"]
                    : false;
...
function bp_members_add_role_after_activation($user_id)
{
    ...
    if (
        is_multisite() &&
        !is_user_member_of_blog($user_id, bp_get_root_blog_id())
    ) {
...
function bp_members_migrate_signups()
{
    ...
    if (!empty($status_2_ids)) {
        ...
        foreach ($ak_data as $ak_datum) {
            $activation_keys[intval($ak_datum->user_id)] =
                $ak_datum->meta_value;
        }
    ...
    foreach ($signups as $signup) {
        ...
        $user_login = preg_replace(
            '/\s+/',
            '',
            sanitize_user($signup->user_login, true)
        );
...
function bp_core_signup_avatar_upload_dir()
{
    ...
    $newbdir = $path;
    $newurl =
        bp_core_avatar_url() . '/' . $directory . '/' . $bp->signup->avatar_dir;
    $newburl = $newurl;
     ...
     */
    return apply_filters('bp_core_signup_avatar_upload_dir', [
        'path' => $path,
        'url' => $newurl,
        'subdir' => $newsubdir,
        'basedir' => $newbdir,
 ...
 */
function bp_core_signup_send_validation_email(
    $user_id,
    $user_email,
    $key,
    $salutation = ''
    ...
    $args = [
        'tokens' => [
            'activate.url' => esc_url(
                trailingslashit(bp_get_activation_page()) . "{$key}/"
            ),
            'key' => $key,
...
function bp_core_signup_disable_inactive(
    ...
    $signup = BP_Signup::get(['user_login' => sanitize_user($user_login)]);
...
function bp_members_login_resend_activation_email()
{
    ...
    $resend = BP_Signup::resend([$signup_id]);
...
function bp_core_wpsignup_redirect()
{
    ...
    if (!empty($_SERVER['SCRIPT_NAME'])) {
        ...
        if (
            'wp-signup.php' === basename($script_name_path) ||
            ('wp-login.php' === basename($script_name_path) &&
                !empty($_GET['action']) &&
    ...
    if (isset($_POST['stage']) && $_POST['stage'] === 'gimmeanotherblog') {
        ...
        $is_site_creation =
            false !== strpos($referer_path, 'wp-admin/my-sites.php');
    }

    if ($is_site_creation) {
        if (bp_is_active('blogs')) {
            $redirect_to = trailingslashit(
                bp_get_blogs_directory_permalink() . 'create'
            );
        } else {
...
function bp_stop_live_spammer()
{
    ...
    if (
        ...
        false !== strpos($GLOBALS['pagenow'], 'wp-login.php')
    ) {
        $is_login = true;
    } elseif (
        isset($_SERVER['SCRIPT_NAME']) &&
        false !== strpos($_SERVER['SCRIPT_NAME'], 'wp-login.php')
    ) {
        $is_login = true;
    }
...
function bp_get_member_type_tax_labels()
{
     ...
     */
    return apply_filters('bp_get_member_type_tax_labels', [
        // General labels
        'name' => _x('Member Types', 'Member type taxonomy name', 'buddypress'),
        'singular_name' => _x(
            'Member Type',
            'Member type taxonomy singular name',
            'buddypress'
        ),
        'search_items' => _x(
            'Search Member Types',
            'Member type taxonomy search items label',
            'buddypress'
        ),
        'popular_items' => _x(
            'Popular Member Types',
            'Member type taxonomy popular items label',
            'buddypress'
        ),
        'all_items' => _x(
            'All Member Types',
            'Member type taxonomy all items label',
            'buddypress'
        ),
        'parent_item' => _x(
            'Parent Member Type',
            'Member type taxonomy parent item label',
            'buddypress'
        ),
        ...
        'edit_item' => _x(
            'Edit Member Type',
            'Member type taxonomy edit item label',
            'buddypress'
        ),
        'view_item' => _x(
            'View Member Type',
            'Member type taxonomy view item label',
            'buddypress'
        ),
        'update_item' => _x(
            'Update Member Type',
            'Member type taxonomy update item label',
            'buddypress'
        ),
        'add_new_item' => _x(
            'Add New Member Type',
            'Member type taxonomy add new item label',
            'buddypress'
        ),
        'new_item_name' => _x(
            'New Member Type Name',
            'Member type taxonomy new item name label',
            'buddypress'
        ),
        'separate_items_with_commas' => _x(
            'Separate member types with commas',
            'Member type taxonomy separate items with commas label',
            'buddypress'
        ),
        'add_or_remove_items' => _x(
            'Add or remove member types',
            'Member type taxonomy add or remove items label',
            'buddypress'
        ),
        'choose_from_most_used' => _x(
            'Choose from the most used meber types',
            'Member type taxonomy choose from most used label',
            'buddypress'
        ),
        'not_found' => _x(
            'No member types found.',
            'Member type taxonomy not found label',
            'buddypress'
        ),
        'no_terms' => _x(
            'No member types',
            'Member type taxonomy no terms label',
            'buddypress'
        ),
        'items_list_navigation' => _x(
            'Member Types list navigation',
            'Member type taxonomy items list navigation label',
            'buddypress'
        ),
        'items_list' => _x(
            'Member Types list',
            'Member type taxonomy items list label',
            'buddypress'
        ),
        ...
        'most_used' => _x(
            'Most Used',
            'Member type taxonomy most used items label',
            'buddypress'
        ),
        'back_to_items' => _x(
            '&larr; Back to Member Types',
            'Member type taxonomy back to items label',
            'buddypress'
        ),
        ...
        'bp_type_id_label' => _x(
            'Member Type ID',
            'BP Member type ID label',
            'buddypress'
        ),
        'bp_type_id_description' => _x(
            'Enter a lower-case string without spaces or special characters (used internally to identify the member type).',
            'BP Member type ID description',
            'buddypress'
        ),
        'bp_type_show_in_list' => _x(
            'Show on Member',
            'BP Member type show in list',
            'buddypress'
        ),
    ]);
}
        ...
        array_merge(
            [
                'description' => _x(
                    ...
                    'Member type taxonomy description',
                    'buddypress'
                ),
                'labels' => array_merge(
 ...
 */
function bp_get_member_type_metadata_schema($schema = [], $taxonomy = '')
{
    if (bp_get_member_type_tax_name() === $taxonomy) {
        // Directory.
        if (isset($schema['bp_type_has_directory']['description'])) {
            $schema['bp_type_has_directory']['description'] = __(
                'Make a list of members matching this type available on the members directory.',
                'buddypress'
            );
        }
        ...
        if (isset($schema['bp_type_directory_slug']['description'])) {
            $schema['bp_type_directory_slug']['description'] = __(
                'Enter if you want the type slug to be different from its ID.',
                'buddypress'
            );
        }

        // List.
        $schema['bp_type_show_in_list'] = [
            'description' => __(
                'Show where member types may be listed, like in the member header.',
                'buddypress'
            ),
            'type' => 'boolean',
            'single' => true,
...
function bp_register_member_type_metadata()
{
    ...
    foreach (
        bp_get_type_metadata_schema(false, $type_taxonomy)
        as $meta_key => $meta_args
    ) {
 ...
 */
function bp_register_member_type($member_type, $args = [])
{
    $bp = buddypress();

    if (isset($bp->members->types[$member_type])) {
        return new WP_Error(
            'bp_member_type_exists',
            __('Member type already exists.', 'buddypress'),
            $member_type
        );
    }
     ...
     */
    $illegal_names = apply_filters('bp_member_type_illegal_names', [
        'any',
        'null',
        '_none',
    ]);
    if (in_array($member_type, $illegal_names, true)) {
        return new WP_Error(
            'bp_member_type_illegal_name',
            __(
                'You may not register a member type with this name.',
                'buddypress'
            ),
            $member_type
        );
 ...
 */
function bp_get_member_types($args = [], $output = 'names', $operator = 'and')
{
...
add_filter(
    bp_get_member_type_tax_name() . '_registered_by_code',
 ...
 */
function bp_set_registered_by_code_member_type_metadata(
    $metadata = [],
    $type = ''
) {
    ...
    foreach (get_object_vars($member_type) as $object_key => $object_value) {
        ...
        } elseif (!in_array($object_key, ['name', 'code', 'db_id'], true)) {
...
add_filter(
    bp_get_member_type_tax_name() . '_set_registered_by_code_metada',
...
{
    $all_types = bp_get_member_types([], 'objects');
    $unsaved_types = wp_filter_object_list(
        $all_types,
        ['db_id' => 0],
        'and',
        'name'
    ...
    if ($unsaved_types) {
        foreach ($unsaved_types as $type_name) {
            bp_insert_term($type_name, bp_get_member_type_tax_name(), [
                'slug' => $type_name,
            ]);
        }
    }
...
add_action(
    bp_get_member_type_tax_name() . '_add_form',
...
function bp_set_member_type($user_id, $member_type, $append = false)
{
    ...
    if (!empty($member_type)) {
        ...
        $valid_types = array_filter(
            array_map('bp_get_member_type_object', $member_types)
        );
...
function bp_remove_member_type($user_id, $member_type)
{
    ...
    $deleted = bp_remove_object_terms(
        $user_id,
        $member_type,
        bp_get_member_type_tax_name()
    );
...
function bp_members_avatar_upload_dir($directory = 'avatars', $user_id = 0)
{
     ...
     */
    return apply_filters('bp_members_avatar_upload_dir', [
        'path' => $path,
        'url' => $newurl,
        'subdir' => $newsubdir,
        'basedir' => $newbdir,
