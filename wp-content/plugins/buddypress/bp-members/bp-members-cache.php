...
{
    $uncached_member_ids = bp_get_non_cached_ids(
        $bp_user_query->user_ids,
    ...
    foreach ($member_types as $member_type) {
        if (!isset($keyed_member_types[$member_type->object_id])) {
            $keyed_member_types[$member_type->object_id] = [];
        }
    ...
    foreach (
        array_diff($uncached_member_ids, $cached_member_ids)
        as $no_type_id
    ) {
