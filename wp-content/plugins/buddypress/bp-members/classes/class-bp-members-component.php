...
class BP_Members_Component extends BP_Component
{
    ...
    public function late_includes()
    {
        ...
        if (bp_is_user()) {
            ...
            if (
                ...
                bp_is_action_variable('delete-avatar', 0)
            ) {
            ...
            if (
                ...
                in_array(
                    bp_current_action(),
                    ['change-avatar', 'change-cover-image'],
                    true
                )
            ) {
                require $this->path .
    ...
    public function setup_globals($args = [])
    {
        ...
        $args = [
            ...
            'root_slug' => isset($bp->pages->members->slug)
                ? $bp->pages->members->slug
                : BP_MEMBERS_SLUG,
            'has_directory' => true,
            'directory_title' => isset($bp->pages->members->title)
                ? $bp->pages->members->title
                : $default_directory_title,
            'search_string' => __('Search Members...', 'buddypress'),
            'global_tables' => [
                'table_name_invitations' =>
                    bp_core_get_table_prefix() . 'bp_invitations',
                'table_name_last_activity' =>
        ...
        $bp->loggedin_user->userdata = bp_core_get_core_userdata(
            bp_loggedin_user_id()
        );

        // Fetch the full name for the logged in user.
        $bp->loggedin_user->fullname = isset(
        ...
        $bp->displayed_user->fullname = isset(
        ...
        $bp->displayed_user->domain = bp_core_get_user_domain(
            bp_displayed_user_id()
        );
    ...
    public function setup_canonical_stack()
    {
        ...
        if (bp_displayed_user_has_front_template()) {
            ...
            $bp->default_component =
                'xprofile' === $bp->profile->id ? 'profile' : $bp->profile->id;
        }
        ...
        if (bp_displayed_user_id()) {
            ...
            if (
                bp_is_user_spammer(bp_displayed_user_id()) &&
    ...
    public function setup_nav($main_nav = [], $sub_nav = [])
    {
        ...
        if (!bp_displayed_user_has_front_template() && $is_xprofile_active) {
            add_action('bp_xprofile_setup_nav', [$this, 'setup_xprofile_nav']);
            return;
        }
        ...
        if (!$is_xprofile_active) {
            $this->main_nav = [
                'name' => _x(
                    'Profile',
                    'Member profile main navigation',
                    'buddypress'
                ),
                'slug' => $slug,
                'position' => 20,
        ...
        if (bp_displayed_user_has_front_template()) {
            ...
            $front_subnav['parent_url'] = trailingslashit(
                $user_domain . 'front'
            );
             ...
             */
            if (!$is_xprofile_active) {
                add_action('bp_members_setup_nav', [
                    $this,
                    'setup_profile_nav',
                ]);
            }
            ...
            if (!$is_xprofile_active) {
                $sub_nav = array_merge(
                    $sub_nav,
                    $this->get_avatar_cover_image_subnavs()
                );
            }
        }
    ...
    public function get_avatar_cover_image_admin_navs($admin_bar_menu_id = '')
    {
        ...
        $profile_link = trailingslashit(
            bp_loggedin_user_domain() . bp_get_profile_slug()
        );
        ...
        if (bp_displayed_user_use_cover_image_header()) {
            $wp_admin_nav[] = [
                'parent' => 'my-account-' . $admin_bar_menu_id,
                'id' =>
                    'my-account-' . $admin_bar_menu_id . '-change-cover-image',
                'title' => _x(
                    'Change Cover Image',
                    'My Account Profile sub nav',
                    'buddypress'
    ...
    public function setup_admin_bar($wp_admin_nav = [])
    {
        ...
        if (is_user_logged_in()) {
            ...
            if (!bp_is_active('xprofile')) {
                ...
                $wp_admin_nav = array_merge(
                    $wp_admin_nav,
                    $this->get_avatar_cover_image_admin_navs()
                );
                ...
                add_filter(
                    'bp_xprofile_admin_nav',
                    [$this, 'setup_xprofile_admin_nav'],
                    2
                );
            }
        }
    ...
    {
        $items = $this->get_avatar_cover_image_admin_navs(
    ...
    {
        // Global groups.
        wp_cache_add_global_groups(['bp_last_activity', 'bp_member_type']);
            ...
            'bp/members' => [
                ...
                'attributes' => [
                    ...
                    'extraData' => [
                        'type' => 'string',
                        'default' => 'none',
                        'enum' => ['last_activity', 'latest_update', 'none'],
                    ],
                    'layoutPreference' => [
                        'type' => 'string',
                        'default' => 'list',
