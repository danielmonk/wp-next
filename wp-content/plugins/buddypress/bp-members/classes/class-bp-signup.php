...
class BP_Signup
{
    ...
    public function populate()
    {
        ...
        $this->user_name = !empty($this->meta['field_1'])
    ...
    public static function get($args = [])
    {
        ...
        if (
            !in_array(
                $r['orderby'],
                ['login', 'email', 'registered', 'activated'],
                true
            )
        ) {
            $r['orderby'] = 'signup_id';
        ...
        if (empty($r['include'])) {
            ...
            if (!empty($r['usersearch'])) {
                ...
                $sql['where'][] = $wpdb->prepare(
                    "( user_login LIKE %s OR user_email LIKE %s OR meta LIKE %s )",
                    $search_terms_like,
                    $search_terms_like,
                    $search_terms_like
                );
         ...
         */
        $paged_signups = $wpdb->get_results(
            apply_filters(
        ...
        if (empty($paged_signups)) {
            return ['signups' => false, 'total' => false];
        }
        ...
        if ('ids' === $r['fields']) {
            ...
            foreach ((array) $paged_signups as $key => $signup) {
                ...
                $signup->meta = !empty($signup->meta)
                    ? maybe_unserialize($signup->meta)
                    : false;
        ...
        return ['signups' => $paged_signups, 'total' => $total_signups];
    }
     ...
     * @return int User id.
     */
    public static function add_backcompat(
        $user_login = '',
        $user_password = '',
        $user_email = '',
        $usermeta = []
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$wpdb->users} SET user_status = 2 WHERE ID = %d",
                $user_id
            )
        );
    ...
    public static function count_signups()
    {
        ...
        $signups_table = buddypress()->members->table_name_signups;
        $count_signups = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*) AS total FROM {$signups_table} WHERE active = %d",
                0
            )
        );
     ...
     * @return array
     */
    public static function resend($signup_ids = [])
    {
        ...
        foreach ($signups as $signup) {
            ...
            if (is_multisite()) {
                ...
                if (
                    !empty($user_id) &&
                    2 != self::check_user_status($user_id)
                ) {
                    // Status is not 2, so user's account has been activated.
                    $result['errors'][$signup->signup_id] = [
                        $signup->user_login,
                        esc_html__(
                    ...
                    if (
                        ...
                        $salutation =
                            $meta['field_' . bp_xprofile_fullname_field_id()];
                    }
     ...
     * @return array
     */
    public static function activate($signup_ids = [])
    {
        if (empty($signup_ids) || !is_array($signup_ids)) {
            return false;
        }
     ...
     * @return array
     */
    public static function delete($signup_ids = [])
    {
        global $wpdb;
        ...
        foreach ($signups as $signup) {
            ...
            if (
                !empty($user_id) &&
                $signup->activation_key ===
                    bp_get_user_meta($user_id, 'activation_key', true)
            ) {
                if (2 != self::check_user_status($user_id)) {
                    // Status is not 2, so user's account has been activated.
                    $result['errors'][$signup->signup_id] = [
