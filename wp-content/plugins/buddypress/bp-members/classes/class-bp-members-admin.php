...
if (!class_exists('BP_Members_Admin')):
    ...
    class BP_Members_Admin
    {
        ...
        private function setup_globals()
        {
            ...
            // Paths and URLs
            $this->admin_dir = trailingslashit(
                $bp->plugin_dir . 'bp-members/admin'
            ); // Admin path.
            $this->admin_url = trailingslashit(
                $bp->plugin_url . 'bp-members/admin'
            ); // Admin URL.
            ...
            $this->edit_profile_args = ['page' => 'bp-profile-edit'];
            ...
            $this->users_screen = bp_core_do_network_admin()
                ? 'users-network'
                : 'users';

            // Specific config: BuddyPress is not network activated.
            $this->subsite_activated =
                (bool) is_multisite() && !bp_is_network_activated();
        ...
        private function setup_actions()
        {
            ...
            add_action('bp_admin_enqueue_scripts', [$this, 'enqueue_scripts']);

            // Add some page specific output to the <head>.
            add_action('bp_admin_head', [$this, 'admin_head'], 999);

            // Add menu item to all users menu.
            add_action('admin_menu', [$this, 'admin_menus'], 5);
            add_action('network_admin_menu', [$this, 'admin_menus'], 5);
            add_action('user_admin_menu', [$this, 'user_profile_menu'], 5);

            // Create the Profile Navigation (Profile/Extended Profile).
            add_action('edit_user_profile', [$this, 'profile_nav'], 99, 1);
            add_action('show_user_profile', [$this, 'profile_nav'], 99, 1);

            // Editing users of a specific site.
            add_action("admin_head-site-users.php", [
                $this,
                'profile_admin_head',
            ]);
            ...
            add_action('bp_members_admin_load', [
                $this,
            ...
            if (is_admin()) {
                ...
                if (!is_multisite()) {
                    add_action(
                        'pre_user_query',
                        [$this, 'remove_signups_from_user_query'],
                        10,
                        1
                    );
                }
                ...
                if (current_user_can($this->capability)) {
                    ...
                    if (
                        ...
                        false === strpos($user_screen, '-network')
                    ) {
                        $user_screen .= '-network';
                    }
                    ...
                    add_filter(
                        "views_{$user_screen}",
                        [$this, 'signup_filter_view'],
                        10,
                        1
                    );
                    add_filter(
                        'set-screen-option',
                        [$this, 'signup_screen_options'],
                        10,
                        3
                    );
                }
                ...
                add_action(
                    'update_site_option_registration',
                    [$this, 'multisite_registration_on'],
                    10,
                    2
                );
                add_action(
                    'update_option_users_can_register',
                    [$this, 'single_site_registration_on'],
                    10,
                    2
                );
            }
            ...
            if (is_admin() && bp_get_member_types()) {
                ...
                add_action('restrict_manage_users', [
                    $this,
                ...
                switch ($_REQUEST['error']) {
                    ...
                    case 'ham':
                        $notice = [
                            'class' => 'error',
                            'message' => __(
                                'User could not be removed as spammer.',
                                'buddypress'
                            ),
                        ];
                        break;
                    case 'spam':
                        $notice = [
                            'class' => 'error',
                            'message' => __(
                                'User could not be marked as spammer.',
                                'buddypress'
                            ),
                        ];
                        break;
        ...
        public function user_profile_menu()
        {
            ...
            foreach ($hooks as $key => $hook) {
                add_action("load-{$hook}", [$this, $key . '_admin_load']);
            }

            // Add the profile_admin_head method to proper admin_head actions.
            add_action("admin_head-{$this->user_page}", [
                $this,
                'profile_admin_head',
            ]);
            add_action("admin_head-profile.php", [$this, 'profile_admin_head']);
        }
        ...
        public function admin_menus()
        {
            ...
            if (
                (!bp_is_network_activated() && !is_network_admin()) ||
                (is_network_admin() && bp_is_network_activated())
            ) {
                // Manage signups.
            ...
            // Setup the screen ID's.
            $this->screen_id = [$edit_page, $this->user_page, $profile_page];
            ...
            foreach ($page_head as $head) {
                add_action("admin_head-{$head}", [$this, 'profile_admin_head']);
            }
        }
        ...
        public function profile_admin_head()
        {
            ...
            if (
                ...
                $this->is_self_profile =
                    (bool) ($this->get_user_id() === $this->current_user_id);
            }
        ...
        public function enqueue_scripts()
        {
            ...
            wp_enqueue_style('bp-members-css', $css, [], bp_get_version());
   ...
   if (current_user_can('edit_user', $user->ID)): ?>
				...
				<a class="nav-tab<?php echo esc_attr($wp_active); ?>" href="<?php echo esc_url(
    $wordpress_url
); ?>"><?php _e('Profile', 'buddypress'); ?></a>

			<?php endif; ?>

			<a class="nav-tab<?php echo esc_attr(
       $bp_active
   ); ?>" href="<?php echo esc_url($community_url); ?>"><?php _e('Extended Profile', 'buddypress'); ?></a>
		</h2>
        ...
        public function user_admin_load()
        {
            ...
            $redirect_to = remove_query_arg(
                ['action', 'error', 'updated', 'spam', 'ham', 'delete_avatar'],
                $_SERVER['REQUEST_URI']
            );
            $doaction = !empty($_REQUEST['action'])
                ? $_REQUEST['action']
                : false;
             ...
             */
            do_action_ref_array('bp_members_admin_load', [
                $doaction,
                $_REQUEST,
            ]);
            ...
            if (!in_array($doaction, $allowed_actions)) {
                ...
                get_current_screen()->set_help_sidebar(
                    '<p><strong>' .
                        ...
                        '<p>' .
                        __(
                            '<a href="https://codex.buddypress.org/administrator-guide/extended-profiles/">Managing Profiles</a>',
                            'buddypress'
                        ) .
                        '</p>' .
                        '<p>' .
                ...
                add_meta_box(
                    ...
                    _x(
                        'Status',
                        'members user-admin edit screen',
                        'buddypress'
                    ),
                    [$this, 'user_admin_status_metabox'],
                    get_current_screen()->id,
                    'side',
                    'core'
                );
                ...
                if (buddypress()->avatar->show_avatars) {
                    ...
                    add_meta_box(
                        ...
                        [$this, 'user_admin_avatar_metabox'],
                        $screen_id,
                        'side',
                        'low'
                    );
                ...
                if (!empty($member_types)) {
                    add_meta_box(
                        ...
                        [$this, 'user_admin_member_type_metabox'],
                        $screen_id,
                        'side',
                        'core'
                    );
                ...
                in_array($doaction, ['spam', 'ham']) &&
                empty($this->is_self_profile)
            ) {
                ...
                if (bp_core_delete_existing_avatar(['item_id' => $user_id])) {
                    $redirect_to = add_query_arg(
                        'updated',
                        'avatar',
                        $redirect_to
                 ...
                 */
                do_action_ref_array('bp_members_admin_update_user', [
                    $doaction,
                    $user_id,
                    $_REQUEST,
                    $this->redirect,
        ...
        public function user_admin()
        {
			...
			<div <?php if (
       'updated' === $notice['class']
   ): ?>id="message" <?php endif; ?>class="<?php echo esc_attr(
    $notice['class']
				...
				<?php if (!empty($wp_http_referer) && 'updated' === $notice['class']): ?>
					...
					<p><a href="<?php echo esc_url($wp_http_referer); ?>"><?php esc_html_e(
    '&larr; Back to Users',
    'buddypress'
); ?></a></p>
      ...
      get_current_screen()->get_columns()
        ...
        public function user_admin_status_metabox($user = null)
        {
			...
			<p class="not-activated"><?php esc_html_e(
     ...
     if (
         (empty($this->is_self_profile) &&
             !in_array($user->user_login, get_super_admins()) &&
             empty($this->subsite_activated)) ||
         (!empty($this->subsite_activated) &&
             current_user_can('manage_network_users'))
     ): ?>

						<div class="misc-pub-section" id="comment-status-radio">
							<label class="approved"><input type="radio" name="user_status" value="ham" <?php checked(
           $is_spammer,
           false
       ); ?>><?php esc_html_e('Active', 'buddypress'); ?></label><br />
							<label class="spam"><input type="radio" name="user_status" value="spam" <?php checked(
           $is_spammer,
           true
       ); ?>><?php esc_html_e('Spammer', 'buddypress'); ?></label>
						</div>
     ...
     ?>
      ...
      ?>
						<span id="timestamp">
							<?php /* translators: %s: registration date */
       printf(
           __('Registered on: %s', 'buddypress'),
           '<strong>' . $date . '</strong>'
       ); ?>
						</span>
					</div>
			...
			<div id="major-publishing-actions">
				...
				<div id="publishing-action">
					<a class="button bp-view-profile" href="<?php echo esc_url(
         bp_core_get_user_domain($user->ID)
     ); ?>" target="_blank"><?php esc_html_e('View Profile', 'buddypress'); ?></a>
					<?php submit_button(
         esc_html__('Update Profile', 'buddypress'),
         'primary',
         'save',
         false
     ); ?>
   ...
   printf(
    ...
    printf(
        __('Last active: %1$s', 'buddypress'),
        '<strong>' . $date . '</strong>'
    ); ?>
			</li>

			<?php // Loading other stats only if user has activated their account.
   ...
   if (bp_get_user_has_avatar($user->ID)):
       ...
       ?>
        ...
        $delete_link
    ); ?>" class="bp-members-avatar-user-admin"><?php esc_html_e(
   ...
   if (!bp_core_get_root_option('bp-disable-avatar-uploads')): ?>
				<a href="#TB_inline?width=800px&height=400px&inlineId=bp-members-avatar-editor" class="thickbox bp-members-avatar-user-edit"><?php esc_html_e(
        'Edit Profile Photo',
        'buddypress'
    ); ?></a>
   ...
   esc_html_e('Select member type', 'buddypress'); ?>
		...
		<ul class="categorychecklist form-no-clear">
          ...
          $type->name
      ); ?>" name="bp-members-profile-member-type[]" type="checkbox" <?php checked(
    true,
    in_array($type->name, $current_type)
); ?>>
						<?php echo esc_html($type->labels['singular_name']); ?>
					</label>
				</li>
			<?php endforeach; ?>
			<input type="hidden" value="<?php echo intval(
       $types_count
   ); ?>" name="bp-members-profile-member-types-count" />
		</ul>

		<?php wp_nonce_field(
      'bp-member-type-change-' . $user->ID,
      'bp-member-type-nonce'
  );
        }
        ...
        {
            if (
                !isset($_POST['bp-member-type-nonce']) ||
                !isset($_POST['bp-members-profile-member-types-count'])
            ) {
                return;
            }
            ...
            if (isset($_POST['bp-members-profile-member-type'])) {
                ...
                $member_type = wp_parse_slug_list(
                    wp_unslash($_POST['bp-members-profile-member-type'])
                );
                $member_type = array_filter($member_type);
            } elseif (
                0 !== intval($_POST['bp-members-profile-member-types-count'])
            ) {
                $member_type = false;
            }
        ...
        public function row_actions($actions = '', $user = null)
        {
            ...
            if (
                ...
                $edit_profile_link = sprintf(
                    '<a href="%1$s">%2$s</a>',
                    esc_url($edit_profile),
                    esc_html__('Extended', 'buddypress')
                );
                ...
                if (!empty($actions['edit'])) {
                    ...
                    $new_edit_actions = ['edit-profile' => $edit_profile_link];
                }
        ...
        public function add_edit_profile_url_filter()
        {
            add_filter(
                ...
                [$this, 'filter_adminbar_profile_link'],
                10,
                3
            );
        }
         ...
         * @return string
         */
        public function filter_adminbar_profile_link(
            $profile_link = '',
            $url = '',
            $user_id = 0
        ) {
         ...
         */
        public function signup_screen_options(
            $value = 0,
            $option = '',
            $new_value = 0
        ) {
            if (
                'users_page_bp_signups_network_per_page' != $option &&
                'users_page_bp_signups_per_page' != $option
            ) {
                return $value;
            }
        ...
        public function remove_signups_from_user_query($query = null)
        {
            ...
            if (
                ...
                $this->users_page !== $current_screen->id
            ) {
                return;
            }
         ...
         */
        public function signup_filter_view($views = [])
        {
            global $role;
            ...
            $text = sprintf(
                _x('Pending %s', 'signup users', 'buddypress'),
                '<span class="count">(' .
                    number_format_i18n($signups) .
                    ')</span>'
            );
        ...
        public function signups_admin_load()
        {
            ...
            $allowed_actions = apply_filters(
                ...
                ['do_delete', 'do_activate', 'do_resend']
            );

            // Prepare the display of the Community Profile screen.
            if (!in_array($doaction, $allowed_actions) || -1 == $doaction) {
                if (is_network_admin()) {
                    $bp_members_signup_list_table = self::get_list_table_class(
                ...
                add_screen_option('per_page', [
                    'label' => _x(
                        'Pending Accounts',
                        'Pending Accounts per page (screen options)',
                        'buddypress'
                    ),
                ]);
                ...
                get_current_screen()->add_help_tab([
                    ...
                    'content' =>
                        '<p>' .
                        __(
                            'This is the administration screen for pending accounts on your site.',
                            'buddypress'
                        ) .
                        '</p>' .
                        '<p>' .
                        __(
                            'From the screen options, you can customize the displayed columns and the pagination of this screen.',
                            'buddypress'
                        ) .
                        '</p>' .
                        '<p>' .
                ...
                get_current_screen()->add_help_tab([
                    ...
                    'content' =>
                        '<p>' .
                        __(
                            'Hovering over a row in the pending accounts list will display action links that allow you to manage pending accounts. You can perform the following actions:',
                            'buddypress'
                        ) .
                        '</p>' .
                        '<ul><li>' .
                        __(
                            '"Email" takes you to the confirmation screen before being able to send the activation link to the desired pending account. You can only send the activation email once per day.',
                            'buddypress'
                        ) .
                        '</li>' .
                        '<li>' .
                ...
                get_current_screen()->set_screen_reader_content([
                    ...
                    /* translators: accessibility text */
                    'heading_pagination' => __(
                        'Pending users list navigation',
                        'buddypress'
                    ),
                    /* translators: accessibility text */
                ...
                if ('do_resend' == $doaction) {
                    ...
                    if (empty($activated)) {
                        ...
                        $query_arg = ['updated' => 'activated'];

                        if (!empty($activated['activated'])) {
                            $query_arg['activated'] = count(
                                $activated['activated']
                            );
                        }

                        if (!empty($activated['errors'])) {
                            $query_arg['notactivated'] = count(
                                $activated['errors']
                            );
                            set_transient(
                                '_bp_admin_signups_errors',
        ...
        public function signups_admin()
        {
            ...
            if (!empty($notice)):
				...
				<div id="message" class="<?php echo esc_attr(
        $notice['class']
    ); ?> notice is-dismissible">
			...
			<?php endif; ?>
				...
				<?php if (
        !empty($_REQUEST['notactivated']) ||
        !empty($_REQUEST['notdeleted']) ||
        !empty($_REQUEST['notsent'])
    ): ?>
   ...
   ?>
			...
			<form id="bp-signups-search-form" action="<?php echo esc_url(
       $search_form_url
   ); ?>">
				<input type="hidden" name="page" value="<?php echo esc_attr(
        $plugin_page
    ); ?>" />
				<?php $bp_members_signup_list_table->search_box(
        __('Search Pending Users', 'buddypress'),
        'bp-signups'
    ); ?>
			</form>

			<form id="bp-signups-form" action="<?php echo esc_url(
       $form_url
   ); ?>" method="post">
	...
	<?php
        ...
        public function signups_admin_manage($action = '')
        {
            ...
            $action_url = wp_nonce_url(
                add_query_arg(array_merge($url_args, $action_args), $base_url),
                'signups_' . $action
            );
       ...
       if (!empty($signup->meta['profile_field_ids'])) {
           $profile_field_ids = array_flip(
               explode(',', $signup->meta['profile_field_ids'])
           );
       ...
       ?>
				...
				<li>
					...
					<?php if ('activate' == $action): ?>
						<table class="wp-list-table widefat fixed striped">
							<tbody>
								<tr>
									<td class="column-fields"><?php esc_html_e(
             'Display Name',
             'buddypress'
         ); ?></td>
             ...
             $field_value = isset($signup->meta["field_{$pid}"])
                 ? $signup->meta["field_{$pid}"]
                 : ''; ?>
										<tr>
      ...
      do_action('bp_activate_signup_confirmation_after_details', $signup); ?>
					...
					<?php if ('resend' == $action): ?>
						...
						<p class="description">
							<?php /* translators: %s: notification date */
       printf(esc_html__('Last notified: %s', 'buddypress'), $last_notified); ?>

							<?php if (!empty($signup->recently_sent)): ?>

								<span class="attention wp-ui-text-notification"> <?php esc_html_e(
   ...
   endforeach; ?>
       ...
       $action_url
   ); ?>"><?php esc_html_e('Confirm', 'buddypress'); ?></a>
       ...
       $cancel_url
        ...
        public function users_table_output_type_change_select($which = 'top')
        {
            ...
            if (
                version_compare(bp_get_major_wp_version(), '4.6', '<') &&
                $displayed
            ) {
                return;
            }
            ...
            $id_name =
                'bottom' === $which ? 'bp_change_type2' : 'bp_change_type';
		...
		<select name="<?php echo $id_name; ?>" id="<?php echo $id_name; ?>" style="display:inline-block;float:none;">
			<option value=""><?php _e(
       'Change member type to&hellip;',
       'buddypress'
   ); ?></option>

			<?php foreach ($types as $type): ?>

				<option value="<?php echo esc_attr($type->name); ?>"><?php echo esc_html(
    $type->labels['singular_name']
); ?></option>

			<?php endforeach; ?>

			<option value="remove_member_type"><?php _e(
       'No Member Type',
       'buddypress'
   ); ?></option>
  ...
  wp_nonce_field(
      ...
      'bp-bulk-users-change-type-nonce'
  );
  submit_button(
      __('Change', 'buddypress'),
      'button',
      'bp_change_member_type',
      false
  );
        }
        ...
        public function users_table_process_bulk_type_change()
        {
            ...
            if (
                'remove_member_type' != $new_type &&
                null === bp_get_member_type_object($new_type)
            ) {
                $error = true;
            } else {
            ...
            if ($error) {
                $redirect = add_query_arg(
                    ['updated' => 'member-type-change-error'],
                    wp_get_referer()
                );
            } else {
                $redirect = add_query_arg(
                    ['updated' => 'member-type-change-success'],
                    wp_get_referer()
                );
            }
        ...
        {
            $updated = isset($_REQUEST['updated'])
                ? $_REQUEST['updated']
                : false;
            ...
            if (
                ...
                in_array(
                    $updated,
                    ['member-type-change-error', 'member-type-change-success'],
                    true
                )
            ) {
                if ('member-type-change-error' === $updated) {
                    ...
                    $notice = __(
                        'Member type was changed successfully.',
                        'buddypress'
                    );
                    $type = 'updated';
                }
         ...
         */
        public function users_table_add_type_column($columns = [])
        {
            $columns[bp_get_member_type_tax_name()] = _x(
                'Member Type',
                'Label for the WP users table member type column',
                'buddypress'
            );
         ...
         */
        public function users_table_populate_type_cell(
            $retval = '',
            $column_name = '',
            $user_id = 0
        ) {
            ...
            // Build the Output.
            if ($member_type) {
                $member_types = array_filter(
                    array_map('bp_get_member_type_object', $member_type)
                );
                if (!$member_types) {
                    return $retval;
                }
                ...
                foreach ($member_types as $type) {
                    $url = add_query_arg([
                        'bp-member-type' => urlencode($type->name),
                    ]);
                    $type_links[] = sprintf(
                        '<a href="%1$s">%2$s</a>',
                        esc_url($url),
        ...
        public function users_table_filter_by_type($query)
        {
            ...
            if (
                ...
                $user_ids = bp_get_objects_in_term(
                    $type->term_id,
                    bp_get_member_type_tax_name()
                );
        ...
        protected function format_xprofile_field_for_display($value)
        {
            if (is_array($value)) {
                $value = array_map(
                    [$this, 'format_xprofile_field_for_display'],
                    $value
                );
                $value = implode(', ', $value);
            } else {
