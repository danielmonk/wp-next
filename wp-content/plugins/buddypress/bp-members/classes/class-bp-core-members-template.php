...
class BP_Core_Members_Template
{
    ...
    public function __construct($args)
    {
        ...
        $args['page_number'] = isset($args['page_number'])
            ? $args['page_number']
            : $args['page'];
        ...
        if (!empty($_REQUEST['letter'])) {
            $this->members = BP_Core_User::get_users_by_letter(
                $_REQUEST['letter'],
                $this->pag_num,
                $this->pag_page,
                $r['populate_extras'],
        ...
        if ((int) $this->total_member_count && (int) $this->pag_num) {
            ...
            $this->pag_links = paginate_links([
                ...
                'format' => '',
                'total' => ceil(
                    (int) $this->total_member_count / (int) $this->pag_num
                ),
                'current' => (int) $this->pag_page,
                'prev_text' => _x(
                    '&larr;',
                    'Member pagination previous text',
                    'buddypress'
                ),
                'next_text' => _x(
                    '&rarr;',
                    'Member pagination next text',
                    'buddypress'
                ),
                'mid_size' => 1,
                'add_args' => $add_args,
