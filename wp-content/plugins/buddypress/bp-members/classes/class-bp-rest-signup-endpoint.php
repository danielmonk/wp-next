...
class BP_REST_Signup_Endpoint extends WP_REST_Controller
{
        ...
        register_rest_route(
            ...
            [
                'args' => [
                    'id' => [
                        'description' => __(
                            'Identifier for the signup. Can be a signup ID, an email address, or an activation key.',
                            'buddypress'
                        ),
                        'type' => 'string',
                    ],
        ...
        register_rest_route(
            ...
            [
                ...
                [
                    ...
                    'permission_callback' => [
                        $this,
                        'activate_item_permissions_check',
                    ],
                    'args' => [
                        'context' => $this->get_context_param([
                            'default' => 'edit',
    ...
    public function create_item($request)
    {
        ...
        $signup_validation = bp_core_validate_user_signup(
            $request['user_login'],
            $request['user_email']
        );
        if (
            is_wp_error($signup_validation['errors']) &&
            $signup_validation['errors']->get_error_messages()
        ) {
        ...
        if (is_multisite()) {
            ...
            if ($this->is_blog_signup_allowed()) {
                ...
                if ($site_title && $site_name) {
                    ...
                    $blog_signup_validation = bp_core_validate_blog_signup(
                        $site_name,
                        $site_title
                    );
                    if (
                        is_wp_error($blog_signup_validation['errors']) &&
                        $blog_signup_validation['errors']->get_error_messages()
                    ) {
                        // Return the first error.
                        return new WP_Error(
                            'bp_rest_blog_signup_validation_failed',
                            $blog_signup_validation[
                                'errors'
                            ]->get_error_message(),
                            [
                                'status' => 500,
                    ...
                    if (in_array($site_language, $languages, true)) {
                        $language = wp_unslash(
                            sanitize_text_field($site_language)
                        );
        ...
        if (is_multisite()) {
            ...
            $activation_key = substr(
                md5(time() . wp_rand() . $wp_key_suffix),
                0,
                16
            );
         ...
         */
        $meta = apply_filters(
            'bp_rest_signup_create_item_meta',
            $meta,
            $request
        );
        ...
        $signup_update = $this->update_additional_fields_for_object(
            $signup,
            $request
        );
    ...
    public function delete_item($request)
    {
        ...
        $deleted = BP_Signup::delete([$signup->id]);
    ...
    public function prepare_item_for_response($signup, $request)
    {
        ...
        if ('edit' === $context) {
            ...
            $data['date_sent'] = bp_rest_prepare_date_response(
                $signup->date_sent
            );
            ...
            if (
                ...
                $data['site_public'] = isset($signup->meta['public'])
                    ? (bool) $signup->meta['public']
                    : true;
                $data['site_language'] = isset($signup->meta['WPLANG'])
                    ? $signup->meta['WPLANG']
                    : get_locale();
            }
    ...
    {
        $active_signup = get_network_option(
            get_main_network_id(),
            'registration'
        );
    ...
    public function get_endpoint_args_for_item_schema(
        ...
        if (WP_REST_Server::CREATABLE === $method) {
            ...
            $args['password'] = [
                'description' => __(
                    'Password for the new user (never included).',
                    'buddypress'
                ),
                'type' => 'string',
    ...
    public function get_item_schema()
    {
        $schema = [
            ...
            'properties' => [
                ...
                'date_sent' => [
                    'context' => ['edit'],
                    'description' => __(
                        'The date the activation email was sent to the user, in the site\'s timezone.',
                        'buddypress'
                    ),
                    'type' => 'string',
                    'readonly' => true,
                ...
                'count_sent' => [
                    'description' => __(
                        'The number of times the activation email was sent to the user.',
                        'buddypress'
                    ),
                    'type' => 'integer',
                    'context' => ['edit'],
        ...
        if (is_multisite() && $this->is_blog_signup_allowed()) {
            ...
            $schema['properties']['site_public'] = [
                'context' => ['edit'],
                'description' => __(
                    'Search engine visibility of the new user\'s  site.',
                    'buddypress'
                ),
                'type' => 'boolean',
                'default' => true,
            ...
            $schema['properties']['site_language'] = [
                'context' => ['edit'],
                'description' => __(
                    'Language to use for the new user\'s  site.',
                    'buddypress'
                ),
                'type' => 'string',
                'default' => get_locale(),
