...
class BP_REST_Members_Endpoint extends WP_REST_Users_Controller
{
        ...
        register_rest_route(
            ...
            [
                ...
                [
                    ...
                    'args' => [
                        'context' => parent::get_context_param([
                            'default' => 'view',
                        ]),
                        'populate_extras' => [
                            'description' => __(
                                'Whether to fetch extra BP data about the returned member.',
                                'buddypress'
                            ),
                            'context' => ['view', 'edit'],
                            'default' => false,
                ...
                [
                    ...
                    'args' => [
                        ...
                        'reassign' => [
                            'type' => 'integer',
                            'description' => __(
                                'Reassign the deleted member\'s posts and links to this user ID.',
                                'buddypress'
                            ),
                            'required' => true,
    ...
    public function get_item_permissions_check($request)
    {
        ...
        if (!$user instanceof WP_User) {
            ...
            if (
                ...
                bp_current_user_can('list_users')
            ) {
                $retval = true;
            } else {
                $retval = new WP_Error(
                    'bp_rest_authorization_required',
                    __(
                        'Sorry, you are not allowed to view members with the edit context.',
                        'buddypress'
                    ),
                    [
                        'status' => rest_authorization_required_code(),
    ...
    public function delete_current_item_permissions_check($request)
    {
        return new WP_Error(
            ...
            sprintf(
                __('\'%s\' Transport Method not implemented.', 'buddypress'),
                $request->get_method()
            ),
            [
                'status' => 405,
    ...
    public function prepare_item_for_response($user, $request)
    {
        ...
        if (
            strpos($request->get_route(), 'members/me') !== false &&
            get_current_user_id() === $user->ID
        ) {
    ...
    public function user_data($user, $context, $request)
    {
        ...
        if ($request->get_param('populate_extras')) {
            ...
            if (get_current_user_id() === $user->ID) {
                ...
                $data['last_activity']['timediff'] = bp_core_time_since(
                    $right_now
                );
                $data['last_activity']['date'] = bp_rest_prepare_date_response(
                    $right_now
                );
            ...
            if (bp_is_active('activity')) {
                ...
                if (isset($user->latest_update) && $user->latest_update) {
                    ...
                    if (
                        isset($activity_data['id'], $activity_data['content'])
                    ) {
                        $data['latest_update']['id'] = $activity_data['id'];
                        $data['latest_update']['raw'] =
                            $activity_data['content'];
                        $data['latest_update']['rendered'] = apply_filters(
            ...
            if (
                ...
                isset($user->total_friend_count) &&
                $user->total_friend_count
            ) {
        ...
        if (bp_is_active('friends') && get_current_user_id() !== $user->ID) {
            $friendship_status = friends_check_friendship_status(
                get_current_user_id(),
                $user->ID
            );
        ...
        if ('edit' === $context && current_user_can('list_users')) {
            $data['registered_date'] = bp_rest_prepare_date_response(
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        if (
            !isset($prepared_user->user_login) &&
            isset($request['user_login'])
        ) {
        ...
        if ($types) {
            $request->set_param(
                'member_type',
                bp_rest_sanitize_member_types($types)
            );
        }
        ...
        if (bp_is_active('xprofile')) {
            ...
            foreach ($groups as $group) {
                ...
                foreach ($group->fields as $item) {
                    $data['groups'][$group->id]['fields'][$item->id] = [
                        'name' => $item->name,
                        'value' => [
                            'raw' => $item->data->value,
                            'unserialized' => $fields_endpoint->get_profile_field_unserialized_value(
                                $item->data->value
                            ),
                            'rendered' => $fields_endpoint->get_profile_field_rendered_value(
                                $item->data->value,
                                $item
                            ),
                        ],
    ...
    public function get_endpoint_args_for_item_schema(
        ...
        $member_type_args = [
            'description' => __(
                'Assign a member type to a member, use a comma separated list of member types to assign more than one.',
                'buddypress'
            ),
            'type' => 'string',
    ...
    public function get_item_schema()
    {
        $schema = [
            ...
            'properties' => [
                ...
                'friendship_status' => [
                    'description' => __(
                        'Friendship relation with, current, logged in user.',
                        'buddypress'
                    ),
                    'type' => 'bool',
                    'context' => ['view', 'edit'],
                ...
                'friendship_status_slug' => [
                    'description' => __(
                        'Slug of the friendship status with current logged in user.',
                        'buddypress'
                    ),
                    'enum' => [
                        'is_friend',
                ...
                'last_activity' => [
                    'description' => __(
                        'Last date the member was active on the site.',
                        'buddypress'
                    ),
                    'type' => 'object',
                    'properties' => [
                ...
                'latest_update' => [
                    'description' => __(
                        'The content of the latest activity posted by the member.',
                        'buddypress'
                    ),
                    'type' => 'object',
                    'properties' => [
    ...
    {
        $params = array_intersect_key(parent::get_collection_params(), [
            'context' => true,
            'page' => true,
            'per_page' => true,
            'search' => true,
        ...
        $params['user_id'] = [
            'description' => __(
                'Limit results to friends of a user.',
                'buddypress'
            ),
            'default' => 0,
            'type' => 'integer',
        ...
        $params['user_ids'] = [
            'description' => __(
                'Pass IDs of users to limit result set.',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['populate_extras'] = [
            'description' => __(
                'Whether to fetch extra BP data about the returned members.',
                'buddypress'
            ),
            'default' => false,
            'type' => 'boolean',
        ...
        $params['member_type'] = [
            'description' => __(
                'Limit results set to certain type(s).',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['xprofile'] = [
            'description' => __(
                'Limit results set to a certain XProfile field.',
                'buddypress'
            ),
            'default' => '',
            'type' => 'string',
