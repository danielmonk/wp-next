...
function bp_core_screen_signup()
{
    ...
    if (is_user_logged_in()) {
         ...
         */
        bp_core_redirect(
            apply_filters('bp_loggedin_register_page_redirect_to', $redirect_to)
        );
    ...
    if (!bp_get_signup_allowed()) {
        ...
        bp_verify_nonce_request('bp_new_signup')
    ) {
        /**
        ...
        if (!empty($account_details['errors']->errors['user_name'])) {
            $bp->signup->errors['signup_username'] =
                $account_details['errors']->errors['user_name'][0];
        }

        if (!empty($account_details['errors']->errors['user_email'])) {
            $bp->signup->errors['signup_email'] =
                $account_details['errors']->errors['user_email'][0];
        }
        ...
        if (isset($_POST['signup_password_confirm'])) {
            $signup_pass_confirm = wp_unslash(
                $_POST['signup_password_confirm']
            );
        }

        // Check the account password for problems.
        $account_password = bp_members_validate_user_password(
            $signup_pass,
            $signup_pass_confirm
        );
        ...
        if (
            ...
            !empty($_POST['signup-privacy-policy-check']) &&
            empty($_POST['signup-privacy-policy-accept'])
        ) {
            $bp->signup->errors['signup_privacy_policy'] = __(
                'You must indicate that you have read and agreed to the Privacy Policy.',
                'buddypress'
            );
        }
        ...
        if (bp_is_active('xprofile')) {
            ...
            if (
                isset($_POST['signup_profile_field_ids']) &&
                ...
                foreach ((array) $profile_field_ids as $field_id) {
                    ...
                    if (isset($_POST['field_' . $field_id])) {
                        if (is_array($_POST['field_' . $field_id])) {
                            $_POST['field_' . $field_id] = array_map(
                                'trim',
                                $_POST['field_' . $field_id]
                            );
                        } else {
                            $_POST['field_' . $field_id] = trim(
                                $_POST['field_' . $field_id]
                            );
                        }
                    }
                    ...
                    if (
                        xprofile_check_is_required_field($field_id) &&
                        empty($_POST['field_' . $field_id]) &&
                        !bp_current_user_can('bp_moderate')
                    ) {
                        $bp->signup->errors['field_' . $field_id] = __(
        ...
        if (isset($_POST['signup_with_blog'])) {
            ...
            if ('blog' == $active_signup || 'all' == $active_signup) {
                $blog_details = bp_core_validate_blog_signup(
                ...
                if (!empty($blog_details['errors']->errors['blogname'])) {
                    $bp->signup->errors['signup_blog_url'] =
                        $blog_details['errors']->errors['blogname'][0];
                }

                if (!empty($blog_details['errors']->errors['blog_title'])) {
                    $bp->signup->errors['signup_blog_title'] =
                        $blog_details['errors']->errors['blog_title'][0];
                }
            }
        }
        ...
        if (!empty($bp->signup->errors)) {
            foreach (
                (array) $bp->signup->errors
                as $fieldname => $error_message
            ) {
                /**
                 ...
                 */
                add_action('bp_' . $fieldname . '_errors', function () use (
                    $error_message
                ) {
                    echo apply_filters(
            ...
            if ('none' != $active_signup) {
                ...
                if (bp_is_active('xprofile')) {
                    ...
                    foreach ((array) $profile_field_ids as $field_id) {
                        ...
                        if (!empty($_POST['field_' . $field_id])) {
                            $usermeta['field_' . $field_id] =
                                $_POST['field_' . $field_id];
                        }

                        if (
                            !empty($_POST['field_' . $field_id . '_visibility'])
                        ) {
                            $usermeta['field_' . $field_id . '_visibility'] =
                                $_POST['field_' . $field_id . '_visibility'];
                        }
                    }
                ...
                $usermeta['password'] = wp_hash_password(
