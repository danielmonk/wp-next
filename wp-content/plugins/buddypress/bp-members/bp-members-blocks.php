 ...
 */
function bp_members_editor_settings($bp_editor_settings = [])
{
    $bp = buddypress();

    return array_merge($bp_editor_settings, [
        'members' => [
            'isMentionEnabled' =>
                bp_is_active('activity') && bp_activity_do_mentions(),
 ...
 */
function bp_members_render_member_block($attributes = [])
{
    $bp = buddypress();

    $block_args = wp_parse_args($attributes, [
        'itemID' => 0,
        'avatarSize' => 'full',
    ...
    $container_classes = ['bp-block-member'];
    ...
    if (
        ...
        in_array($block_args['avatarSize'], ['thumb', 'full'], true)
    ) {
    ...
    if ($avatar) {
        $avatar_container = sprintf(
            ...
            sprintf(
                esc_html__('Profile photo of %s', 'buddypress'),
                $display_name
            )
        );
    }
    ...
    return apply_filters(
        'bp_members_render_member_block_output',
        $output,
        $params
    );
}
...
function bp_members_render_members_block($attributes = [])
{
    ...
    $container_classes = sprintf(
        'bp-block-members avatar-%s',
        $block_args['avatarSize']
    );
    if ('grid' === $block_args['layoutPreference']) {
        $container_classes .= sprintf(
            ' is-grid columns-%d',
            (int) $block_args['columns']
        );
    }
    ...
    foreach ($members as $member) {
        ...
        if (
            'list' === $block_args['layoutPreference'] &&
            'latest_update' === $block_args['extraData'] &&
            isset($member->latest_update) &&
        ...
        if (
            ...
            $output .= sprintf(
                ...
                /* translators: %s: member name */
                sprintf(
                    esc_attr__('Profile photo of %s', 'buddypress'),
                    $member->display_name
                ),
                esc_url(
            ...
            if ('last_activity' === $block_args['extraData']) {
                $output .= sprintf(
                    ...
                    sprintf(
                        ...
                        bp_core_time_since($member->last_activity)
                    )
                );
            }
        }
     ...
     */
    return apply_filters(
        'bp_members_render_members_block_output',
        $output,
        $block_args,
        $members
    );
