...
function bp_get_members_member_type_base()
{
    ...
    return apply_filters(
        ...
        _x('type', 'member type URL base', 'buddypress')
    );
}
        ...
        if (1 == $members_template->total_member_count) {
            ...
            $pag = sprintf(
                _n(
                    ...
                    'Viewing %1$s - %2$s of %3$s active members',
                    $members_template->total_member_count,
                    'buddypress'
                ),
                $from_num,
        ...
        if (1 == $members_template->total_member_count) {
            ...
            $pag = sprintf(
                _n(
                    ...
                    'Viewing %1$s - %2$s of %3$s members with friends',
                    $members_template->total_member_count,
                    'buddypress'
                ),
                $from_num,
        ...
        if (1 == $members_template->total_member_count) {
            ...
            $pag = sprintf(
                _n(
                    ...
                    'Viewing %1$s - %2$s of %3$s online members',
                    $members_template->total_member_count,
                    'buddypress'
                ),
                $from_num,
 ...
 */
function bp_member_class($classes = [])
{
 ...
 */
function bp_get_member_class($classes = [])
{
    global $members_template;
...
function bp_member_is_loggedin_user()
{
    ...
    return apply_filters(
        ...
        bp_loggedin_user_id() == $members_template->member->id ? true : false
    );
}
...
function bp_get_member_name()
{
    ...
    if (empty($members_template->member->fullname)) {
        ...
        $name_stack = ['display_name', 'user_nicename', 'user_login'];
        ...
        foreach ($name_stack as $source) {
            if (!empty($members_template->member->{$source})) {
                ...
                $members_template->member->fullname =
                    $members_template->member->{$source};
                break;
            }
        }
    }
    ...
    if (isset($members_template->member->last_activity)) {
        ...
        $last_activity = !empty($r['active_format'])
            ? bp_core_get_last_activity(
                $members_template->member->last_activity,
                $r['active_format']
            )
...
function bp_get_member_latest_update($args = '')
{
    ...
    if (
        ...
        empty($members_template->member->latest_update) ||
     ...
     */
    $update_content = apply_filters(
        'bp_get_activity_latest_update_excerpt',
        trim(strip_tags(bp_create_excerpt($update['content'], $length))),
        $r
    );
    ...
    $update_content = sprintf(
        _x(
            '- &quot;%s&quot;',
            'member latest update in member directory',
            'buddypress'
        ),
        $update_content
    );
    ...
    if ($view_link && $update_content != $update['content']) {
        ...
        $update_content .=
            '<span class="activity-read-more"><a href="' .
            bp_activity_get_permalink($update['id']) .
            '" rel="nofollow">' .
            $view .
            '</a></span>';
    }
...
function bp_get_member_profile_data($args = '')
{
    ...
    if (
        ...
        $profile_data = BP_XProfile_ProfileData::get_all_for_user(
            $r['user_id']
        );
    }
    ...
    if (
        !empty($members_template->member) &&
        empty($members_template->member->profile_data)
    ) {
    ...
    if (
        !empty($profile_data) &&
        !empty($profile_data[$r['field']]['field_type']) &&
        !empty($profile_data[$r['field']]['field_data'])
    ) {
        $data = xprofile_format_profile_field(
            $profile_data[$r['field']]['field_type'],
            $profile_data[$r['field']]['field_data']
        );
    }
 ...
 */
function bp_member_registered($args = [])
{
 ...
 * @return string
 */
function bp_get_member_registered($args = [])
{
    global $members_template;
    ...
    $registered = esc_attr(
        bp_core_get_last_activity(
            $members_template->member->user_registered,
            _x(
                'registered %s',
...
function bp_member_random_profile_data()
{
    ...
    if (bp_is_active('xprofile')) { ?>
		<?php $random_data = xprofile_get_random_profile_data(
      $members_template->member->id,
      true
  ); ?>
...
function bp_member_hidden_fields()
{
    ...
    if (isset($_REQUEST['members_search'])) {
        echo '<input type="hidden" id="search_terms" value="' .
            esc_attr($_REQUEST['members_search']) .
            '" name="search_terms" />';
    }
}
...
function bp_directory_members_search_form()
{
    ...
    $search_form_html =
		...
		<label for="members_search"><input type="text" name="' .
        esc_attr($query_arg) .
        '" id="members_search" placeholder="' .
        esc_attr($search_value) .
        '" /></label>
		<input type="submit" id="members_search_submit" name="members_search_submit" value="' .
...
function bp_get_total_site_member_count()
{
    ...
    return apply_filters(
        ...
        bp_core_number_format(bp_core_get_total_member_count())
    );
}
...
function bp_get_loggedin_user_nav()
{
    ...
    foreach ((array) $bp->members->nav->get_primary() as $nav_item) {
        ...
        if (
            !bp_is_directory() &&
            !empty($bp->active_components[bp_current_component()]) &&
            $bp->active_components[bp_current_component()] == $nav_item->css_id
        ) {
        ...
        echo apply_filters_ref_array(
            'bp_get_loggedin_user_nav_' . $nav_item->css_id,
            [
                '<li id="li-nav-' .
                $nav_item->css_id .
                '" ' .
...
function bp_displayed_user_get_front_template($displayed_user = null)
{
    ...
    $template_names = [
        ...
        'members/single/front-nicename-' .
        sanitize_file_name($displayed_user->userdata->user_nicename) .
        '.php',
    ];
    ...
    if (bp_get_member_types()) {
        ...
        $template_names[] =
            'members/single/front-member-type-' .
            sanitize_file_name($displayed_user_member_type) .
            '.php';
    }
    ...
    return bp_locate_template(
        apply_filters('bp_displayed_user_get_front_template', $template_names),
        false,
        true
    );
}
...
function bp_get_displayed_user_nav()
{
    ...
    foreach ($bp->members->nav->get_primary() as $user_nav_item) {
        if (
            empty($user_nav_item->show_for_displayed_user) &&
            !bp_is_my_profile()
        ) {
            continue;
        }
        ...
        if (bp_loggedin_user_domain()) {
            ...
            $link = trailingslashit(
                bp_displayed_user_domain() . $user_nav_item->link
            );
        }
...
{
    return (bool) bp_is_active('members', 'cover_image') &&
...
function bp_displayed_user_domain()
{
    ...
    return apply_filters(
        'bp_displayed_user_domain',
        isset($bp->displayed_user->domain) ? $bp->displayed_user->domain : ''
    );
}
...
function bp_loggedin_user_domain()
{
    ...
    return apply_filters(
        'bp_loggedin_user_domain',
        isset($bp->loggedin_user->domain) ? $bp->loggedin_user->domain : ''
    );
}
...
function bp_get_displayed_user_fullname()
{
    ...
    return apply_filters(
        ...
        isset($bp->displayed_user->fullname)
...
function bp_get_loggedin_user_fullname()
{
    ...
    return apply_filters(
        'bp_get_loggedin_user_fullname',
        isset($bp->loggedin_user->fullname) ? $bp->loggedin_user->fullname : ''
    );
}
...
function bp_get_current_member_type_message()
{
    ...
    $message = sprintf(
        __('Viewing members of the type: %s', 'buddypress'),
...
function bp_get_member_type_directory_link($member_type = '')
{
    ...
    if (
        isset($member_type_object->labels['singular_name']) &&
 ...
 */
function bp_member_type_list($user_id = 0, $r = [])
{
 ...
 * @return string
 */
function bp_get_member_type_list($user_id = 0, $r = [])
{
    if (empty($user_id)) {
        $user_id = bp_displayed_user_id();
    }
    ...
    if ($types) {
        ...
        if (false === $r['show_all']) {
            $types = array_intersect(
                bp_get_member_types(['show_in_list' => true]),
                $types
            );
            if (empty($types)) {
                return $retval;
        ...
        $list = implode(
            ', ',
            array_map('bp_get_member_type_directory_link', $types)
        );
...
function bp_get_activation_page()
{
    if (bp_has_custom_activation_page()) {
        $page = trailingslashit(
            bp_get_root_domain() . '/' . bp_get_activate_slug()
        );
    } else {
...
function bp_get_signup_allowed()
{
    ...
    return apply_filters(
        ...
        (bool) bp_get_option('users_can_register')
    );
}
...
function bp_members_activity_feed()
{
	...
	<link rel="alternate" type="application/rss+xml" title="<?php bloginfo(
     'name'
 ); ?> | <?php bp_displayed_user_fullname(); ?> | <?php _e(
     'Activity RSS Feed',
     'buddypress'
 ...
 */
function bp_members_component_link(
    $component,
    $action = '',
    $query_args = '',
    $nonce = false
    ...
    echo esc_url(
        bp_get_members_component_link($component, $action, $query_args, $nonce)
    );
}
/**
 ...
 * @return string
 */
function bp_get_members_component_link(
    $component,
    $action = '',
    $query_args = '',
    $nonce = false
