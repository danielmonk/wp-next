...
add_action('bp_init', ['BP_Members_Admin', 'register_members_admin']);
...
function bp_members_type_admin_menu()
{
    ...
    if (
        ...
        $member_type_admin_url = add_query_arg(
            'taxonomy',
...
add_filter(
    bp_get_member_type_tax_name() . '_check_existing_type',
 ...
 */
function bp_members_type_admin_updated_messages($messages = [])
{
    ...
    $messages[$type_taxonomy] = [
        ...
        3 => __(
            'Sorry, there was an error and the Member type wasn’t added.',
            'buddypress'
        ),
        ...
        6 => __(
            'Sorry, the Member type was not deleted: it does not exist.',
            'buddypress'
        ),
        7 => __(
            'Sorry, This Member type is registered using code, deactivate the plugin or remove the custom code before trying to delete it again.',
            'buddypress'
        ),
        8 => __(
            'Sorry, there was an error while trying to delete this Member type.',
            'buddypress'
        ),
        9 => __('Member type successfully deleted.', 'buddypress'),
        10 => __(
            'Member type could not be updated due to missing required information.',
            'buddypress'
        ),
    ];
