...
function bp_get_message_thread_subject()
{
    ...
    return apply_filters(
        ...
        $messages_template->thread->last_message_subject
    );
}
...
function bp_get_message_thread_excerpt()
{
    ...
    return apply_filters(
        ...
        strip_tags(
            bp_create_excerpt(
                $messages_template->thread->last_message_content,
                75
            )
        )
    );
...
function bp_get_message_thread_from()
{
    ...
    return apply_filters(
        ...
        bp_core_get_userlink($messages_template->thread->last_sender_id)
    );
}
...
function bp_get_message_thread_to()
{
    ...
    return apply_filters(
        ...
        BP_Messages_Thread::get_recipient_links(
            $messages_template->thread->recipients
        )
    );
}
...
function bp_get_message_thread_view_link($thread_id = 0, $user_id = null)
{
    ...
    return apply_filters(
        ...
        trailingslashit(
            $domain . bp_get_messages_slug() . '/view/' . $thread_id
        ),
        $thread_id,
        $user_id
    );
...
function bp_get_the_message_thread_mark_unread_url($user_id = null)
{
    ...
    // Base unread URL.
    $url = trailingslashit(
        $domain . bp_get_messages_slug() . '/' . bp_current_action() . '/unread'
    );
...
function bp_get_the_message_thread_mark_read_url($user_id = null)
{
    ...
    // Base read URL.
    $url = trailingslashit(
        $domain . bp_get_messages_slug() . '/' . bp_current_action() . '/read'
    );
...
function bp_message_thread_has_unread()
{
    ...
    $retval = !empty($messages_template->thread->unread_count) ? true : false;
...
function bp_get_message_thread_total_and_unread_count($thread_id = false)
{
    ...
    return sprintf(
        ...
        sprintf(
            _n('%d unread', '%d unread', $unread, 'buddypress'),
...
function bp_get_message_thread_last_post_date_raw()
{
    ...
    return apply_filters(
        ...
        $messages_template->thread->last_message_date
    );
}
...
function bp_get_message_thread_avatar($args = '')
{
    ...
    $fullname = bp_core_get_user_displayname(
        $messages_template->thread->last_sender_id
    );
...
function bp_get_total_unread_messages_count($user_id = 0)
{
    ...
    return apply_filters(
        ...
        BP_Messages_Thread::get_inbox_count($user_id)
    );
}
...
function bp_messages_pagination_count()
{
    ...
    $start_num =
        intval(
            ($messages_template->pag_page - 1) * $messages_template->pag_num
        ) + 1;
    $from_num = bp_core_number_format($start_num);
    $to_num = bp_core_number_format(
        $start_num + ($messages_template->pag_num - 1) >
        $messages_template->total_thread_count
            ? $messages_template->total_thread_count
            : $start_num + ($messages_template->pag_num - 1)
    );
    ...
    if (1 == $messages_template->total_thread_count) {
        ...
        $message = sprintf(
            _n(
                ...
                'Viewing %1$s - %2$s of %3$s messages',
                $messages_template->total_thread_count,
                'buddypress'
            ),
            $from_num,
...
function bp_message_search_form()
{
    ...
    $search_submitted = !empty($_REQUEST['s'])
        ? stripslashes($_REQUEST['s'])
        : $default_search_value;
    $search_placeholder =
        $search_submitted === $default_search_value
            ? ' placeholder="' . esc_attr($search_submitted) . '"'
            : '';
    $search_value =
        $search_submitted !== $default_search_value
            ? ' value="' . esc_attr($search_submitted) . '"'
            : '';
	...
	<form action="" method="get" id="search-message-form">
		<label for="messages_search" class="bp-screen-reader-text"><?php /* translators: accessibility text */
  esc_html_e('Search Messages', 'buddypress'); ?></label>
		<input type="text" name="s" id="messages_search"<?php echo $search_placeholder .
      $search_value; ?> />
		<input type="submit" class="button" id="messages_search_submit" name="messages_search_submit" value="<?php esc_html_e(
      'Search',
      'buddypress'
  ); ?>" />
	</form>
...
function bp_messages_options()
{
	...
	<label for="message-type-select" class="bp-screen-reader-text"><?php /* translators: accessibility text */
 _e('Select:', 'buddypress'); ?></label>
	<select name="message-type-select" id="message-type-select">
		<option value=""><?php _e('Select', 'buddypress'); ?></option>
		<option value="read"><?php _ex(
      'Read',
      'Message dropdown filter',
      'buddypress'
  ); ?></option>
		<option value="unread"><?php _ex(
      'Unread',
      'Message dropdown filter',
      'buddypress'
  ); ?></option>
		<option value="all"><?php _ex(
      'All',
      'Message dropdown filter',
      'buddypress'
  ); ?></option>
	...
	<?php if (
     !bp_is_current_action('sentbox') &&
     !bp_is_current_action('notices')
 ): ?>

		<a href="#" id="mark_as_read"><?php _ex(
      'Mark as Read',
      'Message management markup',
      'buddypress'
  ); ?></a> &nbsp;
		<a href="#" id="mark_as_unread"><?php _ex(
      'Mark as Unread',
      'Message management markup',
      'buddypress'
  ); ?></a> &nbsp;
		...
		<?php wp_nonce_field(
      'bp_messages_mark_messages_read',
      'mark-messages-read-nonce',
      false
  ); ?>
		<?php wp_nonce_field(
      'bp_messages_mark_messages_unread',
      'mark-messages-unread-nonce',
      false
  ); ?>

	<?php endif; ?>

	<a href="#" id="delete_<?php echo bp_current_action(); ?>_messages"><?php _e(
    'Delete Selected',
    'buddypress'
); ?></a> &nbsp;
	<?php wp_nonce_field(
     'bp_messages_delete_selected',
     'delete-selected-nonce',
     false
 ); ?>
<?php
} /**
...
{
    ?>
	<label class="bp-screen-reader-text" for="messages-select"><?php _e(
     'Select Bulk Action',
     'buddypress'
 ); ?></label>
	<select name="messages_bulk_action" id="messages-select">
		<option value="" selected="selected"><?php _e(
      'Bulk Actions',
      'buddypress'
  ); ?></option>
	...
	<input type="submit" id="messages-bulk-manage" class="button action" value="<?php esc_attr_e(
     'Apply',
...
{
    global $messages_template;
    $retval = !empty($messages_template->thread->is_active) ? true : false; /**
...
function bp_get_message_notice_post_date()
{
    ...
    return apply_filters(
        ...
        bp_format_time(strtotime($messages_template->thread->date_sent))
    );
} /**
...
function bp_message_get_notices()
{
    ...
    $closed_notices = bp_get_user_meta(
        bp_loggedin_user_id(),
        'closed_notices',
        true
    );
    ...
    if (is_array($closed_notices)) {
        if (!in_array($notice->id, $closed_notices) && $notice->id) { ?>
			<div id="message" class="info notice" rel="n-<?php echo esc_attr(
       $notice->id
   ); ?>">
				<strong><?php bp_message_notice_subject($notice); ?></strong>
				<button type="button" id="close-notice" class="bp-tooltip" data-bp-tooltip="<?php esc_attr_e(
        'Dismiss this notice',
        'buddypress'
    ); ?>"><span class="bp-screen-reader-text"><?php _e(
    'Dismiss this notice',
    'buddypress'
); ?></span> <span aria-hidden="true">&Chi;</span></button>
...
function bp_get_send_message_button($args = '')
{
    ...
    ]); // Note: 'bp_get_send_message_button' is a legacy filter. Use // 'bp_get_send_message_button_args' instead. See #4536.
    return apply_filters(
...
function bp_get_message_loading_image_src()
{
     ...
     */ return apply_filters(
        ...
        buddypress()->messages->image_base . '/ajax-loader.gif'
    );
} /**
...
{
    // Sanitized in bp-messages-filters.php.
    $recipients = isset($_GET['r']) ? $_GET['r'] : ''; /**
...
function bp_get_the_thread_id()
{
    ...
    return apply_filters(
        ...
        $thread_template->thread->thread_id
    );
}
/**
...
function bp_get_the_thread_subject()
{
    ...
    return apply_filters(
        ...
        $thread_template->thread->last_message_subject
    );
} /**
...
function bp_get_the_thread_recipients()
{
    if (5 <= bp_get_thread_recipients_count()) {
        /* translators: %s: number of message recipients */ $recipients = sprintf(
            __('%s recipients', 'buddypress'),
            number_format_i18n(bp_get_thread_recipients_count())
        );
    } else {
...
{
    global $thread_template;
    $classes = []; // Zebra-striping.
    $classes[] = bp_get_the_thread_message_alt_class(); // ID of the sender.
    $classes[] = 'sent-by-' . intval($thread_template->message->sender_id); // Whether the sender is the same as the logged-in user.
...
function bp_get_the_thread_message_sender_avatar_thumb($args = '')
{
    ...
    return apply_filters(
        ...
        bp_core_fetch_avatar([
            ...
            'width' => $r['width'],
            'height' => $r['height'],
            'alt' => bp_core_get_user_displayname(
                $thread_template->message->sender_id
            ),
        ]),
        $r
...
function bp_get_the_thread_message_sender_link()
{
    ...
    return apply_filters(
        ...
        bp_core_get_userlink($thread_template->message->sender_id, false, true)
    );
} /**
...
{
    global $thread_template;
    $display_name = bp_core_get_user_displayname(
        $thread_template->message->sender_id
    );
...
function bp_get_the_thread_message_time_since()
{
     ...
     */ return apply_filters(
        ...
        sprintf /* translators: %s: last activity timestamp (e.g. "active 1 hour ago") */(
...
function bp_get_the_thread_message_date_sent()
{
    ...
    return apply_filters(
        ...
        strtotime($thread_template->message->date_sent)
    );
} /**
...
{
    global $thread_template;
    $content = $thread_template->message->message; // If user was deleted, mark content as deleted.
    if (
        false ===
        bp_core_get_core_userdata(bp_get_the_thread_message_sender_id())
    ) {
