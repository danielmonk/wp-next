...
function messages_new_message($args = '')
{
    ...
    $r = bp_parse_args(
        ...
        [
            ...
            'recipients' => [], // Can be an array of usernames, user_ids or mixed.
            'subject' => false,
            'content' => false,
    ...
    if (empty($r['sender_id']) || empty($r['content'])) {
        if ('wp_error' === $r['error_type']) {
            if (empty($r['sender_id'])) {
                ...
                $feedback = __(
                    'Your message was not sent. Please use a valid sender.',
                    'buddypress'
                );
            } else {
    ...
    if (!empty($r['thread_id'])) {
        ...
        if (
            isset($message->recipients[$r['sender_id']]) &&
        ...
        if (empty($r['recipients'])) {
            if ('wp_error' === $r['error_type']) {
                return new WP_Error(
                    'message_empty_recipients',
                    __(
                        'Message could not be sent. Please enter a recipient.',
                        'buddypress'
                    )
                );
            } else {
        ...
        if ('wp_error' === $r['error_type']) {
            if (is_wp_error($send)) {
                ...
                return new WP_Error(
                    ...
                    __('Message was not sent. Please try again.', 'buddypress')
                );
            }
        }
     ...
     */
    do_action_ref_array('messages_message_sent', [&$message]);
...
function messages_send_notice($subject, $message)
{
    if (
         ...
         * @param string $message Content of the notice.
         */
        do_action_ref_array('messages_send_notice', [$subject, $message]);
 ...
 * @return mixed
 */
function bp_messages_update_meta(
    $message_id,
    $meta_key,
    $meta_value,
    $prev_value = ''
 ...
 * }
 */
function messages_notification_new_message($raw_args = [])
{
    ...
    foreach ($recipients as $recipient) {
        ...
        bp_send_email('messages-unread', $ud, [
            'tokens' => [
...
function bp_messages_personal_data_exporter($email_address, $page)
{
    ...
    foreach ($user_threads['threads'] as $thread) {
        ...
        $thread_link = bp_get_message_thread_view_link(
            $thread->thread_id,
            $user->ID
        );
