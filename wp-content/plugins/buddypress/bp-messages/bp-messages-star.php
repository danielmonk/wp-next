...
function bp_get_messages_starred_slug()
{
    ...
    return sanitize_title(
        apply_filters('bp_get_messages_starred_slug', 'starred')
    );
}
...
function bp_messages_is_message_starred($mid = 0, $user_id = 0)
{
    ...
    $starred = array_flip(
        (array) bp_messages_get_meta($mid, 'starred_by_user', false)
    );
 ...
 */
function bp_the_message_star_action_link($args = [])
{
 ...
 * @return string
 */
function bp_get_the_message_star_action_link($args = [])
{
    // Default user ID.
    $user_id = bp_displayed_user_id()
    ...
    if ((int) $r['thread_id'] > 0) {
        ...
        if (bp_get_message_thread_id() == $r['thread_id']) {
            ...
            $mids = wp_list_pluck(
                $GLOBALS['messages_template']->thread->messages,
                'id'
            );
     ...
     */
    $retval = esc_url(
        apply_filters('bp_get_the_message_star_action_urlonly', $retval, $r)
    );
    ...
    return apply_filters(
        ...
        '<a data-bp-tooltip="' .
            esc_attr($title) .
            '" class="bp-tooltip message-action-' .
            esc_attr($action) .
            '" data-star-status="' .
            esc_attr($action) .
            '" data-star-nonce="' .
            ...
            $bulk_attr .
            ' data-message-id="' .
            esc_attr((int) $message_id) .
            '" href="' .
            $retval .
            '" role="button" aria-pressed="false"><span class="icon"></span> <span class="bp-screen-reader-text">' .
            $r['text_' . $action] .
            '</span></a>',
        $r
    );
 ...
 * @return bool
 */
function bp_messages_star_set_action($args = [])
{
    ...
    $is_starred = bp_messages_is_message_starred(
        $r['message_id'],
        $r['user_id']
    );
    ...
    if ('star' == $r['action']) {
        ...
        if (false === $r['bulk']) {
            ...
            foreach ($mids as $mid) {
                if (
                    true === bp_messages_is_message_starred($mid, $r['user_id'])
                ) {
                    bp_messages_delete_meta(
                        $mid,
                        'starred_by_user',
 ...
 * @return array
 */
function bp_messages_star_message_css_class($retval = [])
{
    if (
        true === bp_messages_is_message_starred(bp_get_the_thread_message_id())
    ) {
        $status = 'starred';
    } else {
        $status = 'not-starred';
 ...
 */
function bp_messages_filter_starred_message_threads($r = [])
{
    $r['box'] = 'starred';
    $r['meta_query'] = [
        [
