...
class BP_Messages_Notices_Admin
{
    ...
    public function admin_menu()
    {
        ...
        add_action('load-' . $this->screen_id, [$this, 'admin_load']);
    }
    ...
    public function admin_load()
    {
        ...
        if (!empty($_POST['bp_notice']['send'])) {
            ...
            $notice = wp_parse_args($_POST['bp_notice'], [
                'subject' => '',
                'content' => '',
            ]);
        ...
        if (!empty($_GET['notice_id']) && !empty($_GET['notice_action'])) {
            ...
            check_admin_referer(
                'messages-' . $_GET['notice_action'] . '-notice-' . $notice_id
            );
        ...
        $this->list_table = new BP_Messages_Notices_List_Table([
    ...
    public function admin_index()
    {
		...
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php echo esc_html_x(
       'Site Notices',
       ...
       'buddypress'
			...
			<p class="bp-notice-about"><?php esc_html_e(
       'Manage notices shown at front end of your site to all logged-in users.',
       'buddypress'
   ); ?></p>

			<div class="bp-new-notice-panel">

				<h2 class="bp-new-notice"><?php esc_html_e(
        'Add New Notice',
        'buddypress'
    ); ?></h2>

				<form action="<?php echo esc_url(
        wp_nonce_url($this->url, 'new-notice', 'ns-nonce')
    ); ?>" method="post">

					<div>
						<label for="bp_notice_subject"><?php esc_html_e(
          'Subject',
          'buddypress'
      ); ?></label>
						<input type="text" class="bp-panel-input" id="bp_notice_subject" name="bp_notice[subject]"/>

						<label for="bp_notice_content"><?php esc_html_e(
          'Content',
          'buddypress'
      ); ?></label>
         ...
         'Publish Notice',
         'buddypress'
     ); ?>" name="bp_notice[send]" class="button button-primary save alignleft">
			...
			<?php if (isset($_GET['success']) || isset($_GET['error'])): ?>
				...
				<div id="message" class="<?php echo isset($_GET['success'])
        ? 'updated'
        : 'error'; ?> notice is-dismissible">
          ...
          if ('create' === $_GET['success']) {
			...
			<h2 class="bp-notices-list"><?php esc_html_e(
       'Notices List',
       'buddypress'
   ); ?></h2>
