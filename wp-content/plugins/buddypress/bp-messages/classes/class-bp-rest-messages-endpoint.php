...
class BP_REST_Messages_Endpoint extends WP_REST_Controller
{
    ...
    public function register_routes()
    {
        ...
        register_rest_route($this->namespace, $thread_endpoint, [
            [
                'methods' => WP_REST_Server::READABLE,
        ...
        if (bp_is_active('messages', 'star')) {
            ...
            register_rest_route($this->namespace, $starred_endpoint, [
                'args' => [
                    'id' => [
                        'description' => __(
                            'ID of one of the message of the Thread.',
                            'buddypress'
                        ),
                        'type' => 'integer',
                    ],
    ...
    public function get_items($request)
    {
        ...
        $response = bp_rest_response_add_total_headers(
            $response,
            $messages_box->total_thread_count,
            $args['per_page']
        );
    ...
    public function get_items_permissions_check($request)
    {
        ...
        if (is_user_logged_in()) {
            ...
            if (!$user instanceof WP_User) {
                ...
                (int) bp_loggedin_user_id() === $user->ID ||
    ...
    public function get_item_permissions_check($request)
    {
        ...
        if (is_user_logged_in()) {
            ...
            if (empty($thread->thread_id)) {
                ...
                bp_current_user_can('bp_moderate') ||
                messages_check_thread_access($thread->thread_id)
            ) {
                $retval = true;
            } else {
                $retval = $error;
    ...
    public function create_item($request)
    {
        ...
        if (false === $thread_id) {
            return new WP_Error(
                ...
                __(
                    'There was an error trying to create the message.',
                    'buddypress'
                ),
                [
                    'status' => 500,
        ...
        $last_message = wp_list_filter($thread->messages, [
    ...
    public function create_item_permissions_check($request)
    {
        ...
        if (!is_user_logged_in()) {
            $retval = new WP_Error(
                'bp_rest_authorization_required',
                __(
                    'Sorry, you need to be logged in to create a message.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
    ...
    public function update_item($request)
    {
        ...
        $updated_message = wp_list_filter($thread->messages, [
            'id' => $message_id,
        ]);
        ...
        $can_edit_item_meta = apply_filters(
            ...
            bp_loggedin_user_id() === $updated_message->sender_id ||
                bp_current_user_can('bp_moderate'),
            $updated_message,
            $request
        );
    ...
    public function delete_item($request)
    {
        ...
        $recipient_ids = wp_parse_id_list(
            wp_list_pluck($thread->recipients, 'user_id')
        );

        // Delete a thread.
        if (
            !in_array($user_id, $recipient_ids, true) ||
            !messages_delete_thread($thread->thread_id, $user_id)
        ) {
            return new WP_Error(
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        if (
            !empty($schema['properties']['sender_id']) &&
        ...
        if (
            !empty($schema['properties']['message']) &&
        ...
        if (
            !empty($schema['properties']['subject']) &&
        ...
        if (
            !empty($schema['properties']['recipients']) &&
            !empty($request['recipients'])
        ) {
            $prepared_thread->recipients = $request['recipients'];
        } elseif (!empty($thread->recipients)) {
            $prepared_thread->recipients = wp_parse_id_list(
                wp_list_pluck($thread->recipients, 'user_id')
            );
        }
    ...
    public function prepare_message_for_response($message, $request)
    {
        $data = [
            ...
            'subject' => [
                ...
                'rendered' => apply_filters(
                    ...
                    wp_staticize_emoji($message->subject)
                ),
            ],
            'message' => [
                'raw' => $message->message,
                'rendered' => apply_filters(
                    ...
                    wp_staticize_emoji($message->message)
                ),
            ],
        ...
        if (bp_is_active('messages', 'star')) {
            ...
            $data['is_starred'] = bp_messages_is_message_starred(
                $data['id'],
                $user_id
            );
        }
    ...
    public function prepare_item_for_response($thread, $request)
    {
        ...
        if (isset($thread->last_message_content)) {
            $excerpt = wp_strip_all_tags(
                bp_create_excerpt($thread->last_message_content, 75)
            );
        }
        ...
        $data = [
            ...
            'subject' => [
                ...
                'rendered' => apply_filters(
                    'bp_get_message_thread_subject',
                    wp_staticize_emoji($thread->last_message_subject)
                ),
            ],
            'excerpt' => [
                'raw' => $excerpt,
            ...
            'message' => [
                ...
                'rendered' => apply_filters(
                    ...
                    wp_staticize_emoji($thread->last_message_content)
                ),
            ],
            'date' => bp_rest_prepare_date_response($thread->last_message_date),
            'unread_count' => !empty($thread->unread_count)
                ? $thread->unread_count
                : 0,
        ...
        foreach ($thread->messages as $message) {
            $data['messages'][] = $this->prepare_message_for_response(
                $message,
                $request
            );
        }
        ...
        foreach ($thread->recipients as $recipient) {
            $data['recipients'][
                $recipient->user_id
            ] = $this->prepare_recipient_for_response($recipient, $request);
        }

        // Pluck starred message ids.
        $data['starred_message_ids'] = array_keys(
            array_filter(wp_list_pluck($data['messages'], 'is_starred', 'id'))
        );
    ...
    public function get_endpoint_args_for_item_schema(
        ...
        if (WP_REST_Server::CREATABLE === $method) {
            ...
            $args['recipients']['items'] = ['type' => 'integer'];
            $args['recipients']['sanitize_callback'] = 'wp_parse_id_list';
            $args['recipients']['validate_callback'] =
                'rest_validate_request_arg';
            $args['recipients']['description'] = __(
        ...
        $schema = [
            ...
            'properties' => [
                ...
                'subject' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'Title of the latest message of the Thread.',
                        'buddypress'
                    ),
                    'type' => 'object',
                    'arg_options' => [
                    ...
                    'properties' => [
                        ...
                        'rendered' => [
                            'description' => __(
                                'Title of the latest message of the Thread, transformed for display.',
                                'buddypress'
                            ),
                            'type' => 'string',
                            'context' => ['view', 'edit'],
                ...
                'excerpt' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'Summary of the latest message of the Thread.',
                        'buddypress'
                    ),
                    'type' => 'object',
                    'readonly' => true,
                ...
                'message' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'Content of the latest message of the Thread.',
                        'buddypress'
                    ),
                    'type' => 'object',
                    'required' => true,
                ...
                'date' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        "The date the latest message of the Thread, in the site's timezone.",
                        'buddypress'
                    ),
                    'readonly' => true,
                    'type' => 'string',
                ...
                'sender_ids' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'The list of user IDs for all messages in the Thread.',
                        'buddypress'
                    ),
                    'readonly' => true,
                    'type' => 'array',
