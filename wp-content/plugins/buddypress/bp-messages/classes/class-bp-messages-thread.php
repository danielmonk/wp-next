...
class BP_Messages_Thread
{
     ...
     */
    public function __construct($thread_id = false, $order = 'ASC', $args = [])
    {
        if ($thread_id) {
     ...
     */
    public function populate($thread_id = 0, $order = 'ASC', $args = [])
    {
        ...
        $this->last_message_date =
            $this->messages[$last_message_index]->date_sent;
        $this->last_sender_id = $this->messages[$last_message_index]->sender_id;
        $this->last_message_subject =
            $this->messages[$last_message_index]->subject;
        $this->last_message_content =
            $this->messages[$last_message_index]->message;
        ...
        if (isset($this->recipients[$r['user_id']])) {
            $this->unread_count =
                $this->recipients[$r['user_id']]->unread_count;
        }
    ...
    public static function delete($thread_id = 0, $user_id = 0)
    {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$bp->messages->table_name_recipients} SET is_deleted = 1 WHERE thread_id = %d AND user_id = %d",
                $thread_id,
                $user_id
            )
        );
        ...
        if (empty($recipients)) {
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM {$bp->messages->table_name_messages} WHERE thread_id = %d",
                    $thread_id
                )
            );
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM {$bp->messages->table_name_recipients} WHERE thread_id = %d",
                    $thread_id
                )
            );
        }
     ...
     */
    public static function get_current_threads_for_user($args = [])
    {
        global $wpdb;
        ...
        switch ($r['box']) {
            case 'sentbox':
                $user_id_sql =
                    'AND ' . $wpdb->prepare('m.sender_id = %d', $r['user_id']);
                $sender_sql = 'AND m.sender_id = r.user_id';
                break;

            case 'inbox':
                $user_id_sql =
                    'AND ' . $wpdb->prepare('r.user_id = %d', $r['user_id']);
                $sender_sql = 'AND r.sender_only = 0';
                break;

            default:
                // Omit user-deleted threads from all other custom message boxes.
                $deleted_sql = $wpdb->prepare(
                    '( r.user_id = %d AND r.is_deleted = 0 )',
                    $r['user_id']
                );
                break;
        }
        ...
        $sql[
            'from'
        ] = "FROM {$bp->messages->table_name_recipients} r INNER JOIN {$bp->messages->table_name_messages} m ON m.thread_id = r.thread_id {$meta_query_sql['join']}";
        $sql[
            'where'
        ] = "WHERE {$deleted_sql} {$user_id_sql} {$sender_sql} {$type_sql} {$search_sql} {$meta_query_sql['where']}";
        $sql[
            'misc'
        ] = "GROUP BY m.thread_id ORDER BY date_sent DESC {$pag_sql}";
        ...
        foreach ((array) $sorted_threads as $thread_id => $date_sent) {
            $threads[] = new BP_Messages_Thread($thread_id, 'ASC', [
         ...
         * }
         */
        return apply_filters('bp_messages_thread_current_threads', [
            'threads' => &$threads,
    ...
    public static function mark_as_read($thread_id = 0)
    {
        ...
        $retval = $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$bp->messages->table_name_recipients} SET unread_count = 0 WHERE user_id = %d AND thread_id = %d",
                $user_id,
                $thread_id
            )
        );
    ...
    public static function mark_as_unread($thread_id = 0)
    {
        ...
        $retval = $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$bp->messages->table_name_recipients} SET unread_count = 1 WHERE user_id = %d AND thread_id = %d",
                $user_id,
                $thread_id
            )
        );
     ...
     */
    public static function get_total_threads_for_user(
        $user_id,
        $box = 'inbox',
        $type = 'all'
    ) {
    ...
    public static function check_access($thread_id, $user_id = 0)
    {
        ...
        if (
            ...
            0 == $recipients[$user_id]->is_deleted
        ) {
        ...
        foreach ((array) $threads as $thread) {
            ...
            if (!empty($message_ids)) {
                ...
                if (
                    !$wpdb->query(
                        $wpdb->prepare(
                            "UPDATE {$bp->messages->table_name_messages} SET thread_id = %d WHERE id IN ({$message_ids})",
                            $thread->id
                        )
                    )
                ) {
