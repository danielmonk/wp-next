...
class BP_Messages_Notice
{
    ...
    public function save()
    {
         ...
         */
        do_action_ref_array('messages_notice_before_save', [&$this]);

        if (empty($this->id)) {
            $sql = $wpdb->prepare(
                "INSERT INTO {$bp->messages->table_name_notices} (subject, message, date_sent, is_active) VALUES (%s, %s, %s, %d)",
                $this->subject,
                $this->message,
                $this->date_sent,
                $this->is_active
            ...
            $sql = $wpdb->prepare(
                "UPDATE {$bp->messages->table_name_notices} SET subject = %s, message = %s, is_active = %d WHERE id = %d",
                $this->subject,
                $this->message,
                $this->is_active,
                $this->id
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$bp->messages->table_name_notices} SET is_active = 0 WHERE id != %d",
                $id
            )
        );
         ...
         */
        do_action_ref_array('messages_notice_after_save', [&$this]);
    ...
    public function delete()
    {
        ...
        $bp = buddypress();
        $sql = $wpdb->prepare(
            "DELETE FROM {$bp->messages->table_name_notices} WHERE id = %d",
            $this->id
        );
     ...
     */
    public static function get_notices($args = [])
    {
        global $wpdb;
