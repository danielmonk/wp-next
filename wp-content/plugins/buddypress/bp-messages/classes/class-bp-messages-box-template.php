...
class BP_Messages_Box_Template
{
    ...
    public function __construct($args = [])
    {
        ...
        if ('notices' === $this->box) {
            ...
            $this->threads = isset($threads['threads'])
                ? $threads['threads']
                : [];
            $this->total_thread_count = isset($threads['total'])
                ? $threads['total']
                : 0;
        }
        ...
        if (!$this->threads) {
            ...
            if (
                empty($r['max']) ||
                (int) $r['max'] >= (int) $total_notice_count
            ) {
                if ('notices' === $this->box) {
        ...
        if ((int) $this->total_thread_count && (int) $this->pag_num) {
            ...
            $this->pag_links = paginate_links([
                ...
                'format' => '',
                'total' => ceil(
                    (int) $this->total_thread_count / (int) $this->pag_num
                ),
                'current' => $this->pag_page,
                'prev_text' => _x(
                    '&larr;',
                    'Message pagination previous text',
                    'buddypress'
                ),
                'next_text' => _x(
                    '&rarr;',
                    'Message pagination next text',
                    'buddypress'
                ),
                'mid_size' => 1,
                'add_args' => $add_args,
    ...
    public function the_message_thread()
    {
        ...
        if (!bp_is_current_action('notices')) {
            ...
            $this->thread->messages = array_reverse(
                (array) $this->thread->messages
            );
            ...
            $this->thread->last_message_id =
                $this->thread->messages[$last_message_index]->id;
            $this->thread->last_message_date =
                $this->thread->messages[$last_message_index]->date_sent;
            $this->thread->last_sender_id =
                $this->thread->messages[$last_message_index]->sender_id;
            $this->thread->last_message_subject =
                $this->thread->messages[$last_message_index]->subject;
            $this->thread->last_message_content =
                $this->thread->messages[$last_message_index]->message;
        }
