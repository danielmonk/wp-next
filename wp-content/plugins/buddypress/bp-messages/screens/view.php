...
function messages_screen_conversation()
{
    ...
    if (empty($thread_id) || !messages_is_valid_thread($thread_id)) {
        ...
        bp_core_redirect(
            trailingslashit(bp_displayed_user_domain() . bp_get_messages_slug())
        );
    }

    // No access.
    if (
        (!messages_check_thread_access($thread_id) || !bp_is_my_profile()) &&
    ...
    $nav_name = sprintf(
        __('Messages <span class="%1$s">%2$s</span>', 'buddypress'),
        esc_attr($class),
