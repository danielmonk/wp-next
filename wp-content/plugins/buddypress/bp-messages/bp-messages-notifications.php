...
function messages_format_notifications(
    ...
    $link = trailingslashit(
        bp_loggedin_user_domain() . bp_get_messages_slug() . '/inbox'
    );
    ...
    if ('new_message' === $action) {
        if ($total_items > 1) {
            ...
            $text = sprintf(
                __('You have %d new messages', 'buddypress'),
                $total_items
            );
        } else {
            ...
            if (!empty($secondary_item_id)) {
                ...
                $text = sprintf(
                    __('%s sent you a new private message', 'buddypress'),
                ...
                $text = sprintf(
                    _n(
                        'You have %s new private message',
                        'You have %s new private messages',
                        $total_items,
                        'buddypress'
                    ),
...
function bp_messages_screen_conversation_mark_notifications()
{
    ...
    $message_ids = array_intersect(
        $unread_message_ids,
        wp_list_pluck($thread_template->thread->messages, 'id')
    );
	...
	<table class="notification-settings" id="messages-notification-settings">
		...
		<tbody>
			<tr id="messages-notification-settings-new-message">
				...
				<td class="yes"><input type="radio" name="notifications[notification_messages_new_message]" id="notification-messages-new-messages-yes" value="yes" <?php checked(
        $new_messages,
        'yes',
        true
    ); ?>/><label for="notification-messages-new-messages-yes" class="bp-screen-reader-text"><?php /* translators: accessibility text */
_e('Yes, send email', 'buddypress'); ?></label></td>
				<td class="no"><input type="radio" name="notifications[notification_messages_new_message]" id="notification-messages-new-messages-no" value="no" <?php checked(
        $new_messages,
        'no',
        true
    ); ?>/><label for="notification-messages-new-messages-no" class="bp-screen-reader-text"><?php /* translators: accessibility text */
