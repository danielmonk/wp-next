...
function bp_blogs_register_post_tracking_args($params = null, $post_type = 0)
{
     ...
     */
    $post_types = apply_filters('bp_blogs_record_post_post_types', ['post']);
    ...
    $params->contexts = ['activity', 'member'];
    $params->position = 5;
    ...
    if (post_type_supports($post_type, 'comments')) {
        ...
        if (isset($comment_post_types_array[$post_type])) {
            ...
            $params->comments_tracking->admin_filter = __(
                'New post comment posted',
                'buddypress'
            );
            $params->comments_tracking->format_callback =
                'bp_blogs_format_activity_action_new_blog_comment';
            $params->comments_tracking->front_filter = __(
                'Comments',
                'buddypress'
            );
            $params->comments_tracking->contexts = ['activity', 'member'];
...
function bp_blogs_format_activity_action_new_blog($action, $activity)
{
    ...
    if (has_filter('bp_blogs_activity_created_blog_action')) {
        $user_blog = BP_Blogs_Blog::get_user_blog(
            $activity->user_id,
            $activity->item_id
        );
        if ($user_blog) {
        ...
        if (isset($recorded_blog)) {
            $action = apply_filters(
                ...
                $recorded_blog,
                $blog_name,
                bp_blogs_get_blogmeta($activity->item_id, 'description')
            );
        }
    }
...
function bp_blogs_format_activity_action_new_blog_comment($action, $activity)
{
    ...
    if (is_multisite()) {
        $action = sprintf(
            /* translators: 1: the activity user link. 2: the post link. 3: the blog link. */
            esc_html_x(
                '%1$s commented on the post, %2$s, on the site %3$s',
                '`new_blog_comment` activity action',
                'buddypress'
            ),
            $user_link,
            $post_link,
    ...
    if (has_filter('bp_blogs_activity_new_comment_action')) {
        ...
        if (!empty($comment) && !is_wp_error($comment)) {
            $action = apply_filters(
                ...
                $action,
                $comment,
                $post_url . '#' . $activity->secondary_item_id
            );
        }
    }
...
function bp_blogs_comments_open($activity)
{
    ...
    if ('' === $days_old) {
        ...
        if (
            'closed' ==
            bp_activity_get_meta($activity->id, 'post_comment_status')
        ) {
            return false;
        }
 ...
 */
function bp_blogs_record_activity_on_site_creation(
    $recorded_blog,
    $is_private,
    $is_recorded,
    $no_activity
...
function bp_blogs_sync_add_from_activity_comment(
    ...
    $post_type = bp_activity_post_type_get_tracking_arg(
        $parent_activity->type,
        'post_type'
    );
    $comment_parent = 0;
    ...
    if (!empty($params['parent_id']) && !empty($post_type)) {
        $comment_parent = bp_activity_get_meta(
            $params['parent_id'],
            "bp_blogs_{$post_type}_comment_id"
        );
    }
    ...
    $args = [
        ...
        'comment_author_url' => bp_core_get_user_domain(
            $params['user_id'],
            $user->user_nicename,
            $user->user_login
        ),
 ...
 */
function bp_blogs_sync_activity_edit_to_post_comment(
    BP_Activity_Activity $activity
) {
    ...
    $post_comment_id = bp_activity_get_meta(
        $activity->id,
        "bp_blogs_{$post_type}_comment_id"
    );
 ...
 */
function bp_blogs_remove_activity_meta_for_trashed_comments(
    $post_id = 0,
    $comments = []
) {
    if (!empty($comments)) {
...
function bp_blogs_new_blog_comment_query_backpat($args)
{
    ...
    $activity_ids = $wpdb->get_col(
        $wpdb->prepare(
            "SELECT activity_id FROM {$bp->activity->table_name_meta} WHERE meta_key = %s",
            "bp_blogs_{$post_type}_comment_id"
        )
    );
...
function bp_blogs_setup_activity_loop_globals($activity)
{
    ...
    if (
        !bp_activity_type_supports($activity->type, 'post-type-comment-reply')
    ) {
        return;
    }
    ...
    $thread_depth = bp_blogs_get_blogmeta(
        $activity->item_id,
        'thread_comments_depth'
    );
    $moderation = bp_blogs_get_blogmeta(
        $activity->item_id,
        'comment_moderation'
    );
    ...
    if (!isset(buddypress()->blogs->allow_comments)) {
        buddypress()->blogs->allow_comments = [];
    }
    if (!isset(buddypress()->blogs->thread_depth)) {
        buddypress()->blogs->thread_depth = [];
    }
    if (!isset(buddypress()->blogs->comment_moderation)) {
        buddypress()->blogs->comment_moderation = [];
    }
...
function bp_blogs_disable_activity_commenting($retval)
{
    ...
    if (
        ...
        !isset($activities_template->in_the_loop)
    ) {
        return $retval;
    }
    ...
    if (bp_activity_type_supports($type, 'post-type-comment-tracking')) {
        ...
        if (bp_activity_type_supports($type, 'post-type-comment-reply')) {
            ...
            if (
                !isset(
                    buddypress()->blogs->allow_comments[bp_get_activity_id()]
                ) ||
                !buddypress()->blogs->allow_comments[bp_get_activity_id()]
            ) {
                $retval = false;
            ...
            if (
                ...
                buddypress()->blogs->comment_moderation[bp_get_activity_id()]
            ) {
                $retval = false;
            }
...
function bp_blogs_can_comment_reply($retval, $comment)
{
    ...
    if (isset(buddypress()->blogs->thread_depth[$comment->item_id])) {
        if (
            ...
            buddypress()->blogs->thread_depth[$comment->item_id]
        ) {
            $retval = false;
        }
    }
    ...
    // If comments need moderation, disable activity commenting.
    if (
        isset(buddypress()->blogs->comment_moderation[$comment->item_id]) &&
        buddypress()->blogs->comment_moderation[$comment->item_id]
    ) {
        $retval = false;
    }
...
function bp_blogs_activity_comment_permalink($retval = '')
{
    ...
    if (
        false !== $item_id &&
        isset(buddypress()->blogs->allow_comments[$item_id])
    ) {
...
function bp_blogs_activity_comment_single_action($retval, $activity)
{
    ...
    $blog_comment_id = bp_activity_get_meta(
        $activity->id,
        "bp_blogs_{$post_type}_comment_id"
    );
    ...
    if (!empty($blog_comment_id)) {
        ...
        $comment_action_id = bp_activity_post_type_get_tracking_arg(
            $parent_activity->type,
            'comment_action_id'
        );
        ...
        if (!empty($comment_action_id)) {
            ...
            $format_callback = bp_activity_post_type_get_tracking_arg(
                $comment_action_id,
                'format_callback'
            );
            ...
            if (is_callable($format_callback)) {
                $retval = call_user_func_array($format_callback, ['', $object]);
            }
        }
    }
