...
class BP_Blogs_Blog
{
    ...
    public function save()
    {
         ...
         */
        do_action_ref_array('bp_blogs_blog_before_save', [&$this]);
        ...
        if ($this->id) {
            // Update.
            $sql = $wpdb->prepare(
                "UPDATE {$bp->blogs->table_name} SET user_id = %d, blog_id = %d WHERE id = %d",
                $this->user_id,
                $this->blog_id,
                $this->id
            );
            ...
            // Save.
            $sql = $wpdb->prepare(
                "INSERT INTO {$bp->blogs->table_name} ( user_id, blog_id ) VALUES ( %d, %d )",
                $this->user_id,
                $this->blog_id
            );
        }
         ...
         * @param BP_Blogs_Blog $this Current instance of the blog item being saved. Passed by reference.
         */
        do_action_ref_array('bp_blogs_blog_after_save', [&$this]);
    ...
    public static function get(
        ...
        $user_sql = !empty($user_id)
            ? $wpdb->prepare(" AND b.user_id = %d", $user_id)
            : '';
        ...
        if (!empty($search_terms)) {
            ...
            $search_terms_sql = $wpdb->prepare(
                'AND (bm_name.meta_value LIKE %s OR bm_description.meta_value LIKE %s)',
                $search_terms_like,
                $search_terms_like
            );
        } else {
        ...
        foreach ((array) $paged_blogs as $key => $data) {
            ...
            $paged_blogs[$key]->admin_user_id =
                (int) $paged_blogs[$key]->admin_user_id;
        }
        ...
        return ['blogs' => $paged_blogs, 'total' => $total_blogs];
    }
    ...
    public static function delete_blog_for_all($blog_id)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$bp->blogs->table_name} WHERE blog_id = %d",
                $blog_id
            )
        );
    }
    ...
    public static function delete_blog_for_user($blog_id, $user_id = null)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$bp->blogs->table_name} WHERE user_id = %d AND blog_id = %d",
                $user_id,
                $blog_id
            )
        );
    ...
    public static function delete_blogs_for_user($user_id = null)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$bp->blogs->table_name} WHERE user_id = %d",
                $user_id
            )
        );
    }
     ...
     */
    public static function get_blogs_for_user(
        $user_id = 0,
        $show_hidden = false
    ) {
        global $wpdb;
    ...
    public static function total_blog_count_for_user($user_id = null)
    {
        ...
        if (
            ...
            return $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(DISTINCT b.blog_id) FROM {$bp->blogs->table_name} b LEFT JOIN {$wpdb->base_prefix}blogs wb ON b.blog_id = wb.blog_id WHERE wb.deleted = 0 AND wb.spam = 0 AND wb.mature = 0 AND wb.archived = '0' AND user_id = %d",
                    $user_id
                )
            );
        } else {
            return $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(DISTINCT b.blog_id) FROM {$bp->blogs->table_name} b LEFT JOIN {$wpdb->base_prefix}blogs wb ON b.blog_id = wb.blog_id WHERE wb.public = 1 AND wb.deleted = 0 AND wb.spam = 0 AND wb.mature = 0 AND wb.archived = '0' AND user_id = %d",
                    $user_id
                )
            );
        }
    ...
    public static function search_blogs($filter, $limit = null, $page = null)
    {
        ...
        $paged_blogs = $wpdb->get_results(
            "SELECT DISTINCT bm.blog_id FROM {$bp->blogs->table_name_blogmeta} bm LEFT JOIN {$wpdb->base_prefix}blogs wb ON bm.blog_id = wb.blog_id WHERE ( ( bm.meta_key = 'name' OR bm.meta_key = 'description' ) AND {$search_terms_sql} ) {$hidden_sql} AND wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 ORDER BY meta_value ASC{$pag_sql}"
        );
        $total_blogs = $wpdb->get_var(
            "SELECT COUNT(DISTINCT bm.blog_id) FROM {$bp->blogs->table_name_blogmeta} bm LEFT JOIN {$wpdb->base_prefix}blogs wb ON bm.blog_id = wb.blog_id WHERE ( ( bm.meta_key = 'name' OR bm.meta_key = 'description' ) AND {$search_terms_sql} ) {$hidden_sql} AND wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 ORDER BY meta_value ASC"
        );
        ...
        return ['blogs' => $paged_blogs, 'total' => (int) $total_blogs];
    }
    ...
    public static function get_all($limit = null, $page = null)
    {
        ...
        $hidden_sql = !bp_current_user_can('bp_moderate')
            ? "AND wb.public = 1"
            : '';
        $pag_sql =
            $limit && $page
        ...
        $paged_blogs = $wpdb->get_results(
            "SELECT DISTINCT b.blog_id FROM {$bp->blogs->table_name} b LEFT JOIN {$wpdb->base_prefix}blogs wb ON b.blog_id = wb.blog_id WHERE wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 {$hidden_sql} {$pag_sql}"
        );
        $total_blogs = $wpdb->get_var(
            "SELECT COUNT(DISTINCT b.blog_id) FROM {$bp->blogs->table_name} b LEFT JOIN {$wpdb->base_prefix}blogs wb ON b.blog_id = wb.blog_id WHERE wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 {$hidden_sql}"
        );
    ...
    public static function get_by_letter($letter, $limit = null, $page = null)
    {
        ...
        $paged_blogs = $wpdb->get_results(
            "SELECT DISTINCT bm.blog_id FROM {$bp->blogs->table_name_blogmeta} bm LEFT JOIN {$wpdb->base_prefix}blogs wb ON bm.blog_id = wb.blog_id WHERE bm.meta_key = 'name' AND {$letter_sql} {$hidden_sql} AND wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 ORDER BY bm.meta_value ASC{$pag_sql}"
        );
        $total_blogs = $wpdb->get_var(
            "SELECT COUNT(DISTINCT bm.blog_id) FROM {$bp->blogs->table_name_blogmeta} bm LEFT JOIN {$wpdb->base_prefix}blogs wb ON bm.blog_id = wb.blog_id WHERE bm.meta_key = 'name' AND {$letter_sql} {$hidden_sql} AND wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 ORDER BY bm.meta_value ASC"
        );
     ...
     */
    public static function get_blog_extras(
        &$paged_blogs,
        &$blog_ids,
        $type = false
    ) {
        ...
        for ($i = 0, $count = count($paged_blogs); $i < $count; ++$i) {
            ...
            $paged_blogs[$i]->latest_post = $wpdb->get_row(
            ...
            if (
                !empty($paged_blogs[$i]->latest_post) &&
                has_post_thumbnail($paged_blogs[$i]->latest_post->ID)
            ) {
                // Grab 4 sizes of the image. Thumbnail.
                $image = wp_get_attachment_image_src(
                    get_post_thumbnail_id($paged_blogs[$i]->latest_post->ID),
                    'thumbnail',
                    false
                );
                if (!empty($image)) {
        ...
        $blog_descs = $wpdb->get_results(
            "SELECT blog_id, meta_value as description FROM {$bp->blogs->table_name_blogmeta} WHERE meta_key = 'description' AND blog_id IN ( {$blog_ids} )"
        );
