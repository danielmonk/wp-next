...
function bp_get_blogs_directory_permalink()
{
    ...
    return apply_filters(
        ...
        trailingslashit(bp_get_root_domain() . '/' . bp_get_blogs_root_slug())
    );
}
...
function bp_has_blogs($args = '')
{
    ...
    // Get the blogs.
    $blogs_template = new BP_Blogs_Template(
        $r['type'],
        $r['page'],
        $r['per_page'],
        $r['max'],
...
function bp_get_blogs_pagination_count()
{
    ...
    $start_num =
        intval(($blogs_template->pag_page - 1) * $blogs_template->pag_num) + 1;
    $from_num = bp_core_number_format($start_num);
    $to_num = bp_core_number_format(
        $start_num + ($blogs_template->pag_num - 1) >
        $blogs_template->total_blog_count
            ? $blogs_template->total_blog_count
            : $start_num + ($blogs_template->pag_num - 1)
    );
    ...
    if (1 == $blogs_template->total_blog_count) {
        ...
        $message = sprintf(
            _n(
                'Viewing %1$s - %2$s of %3$s site',
                'Viewing %1$s - %2$s of %3$s sites',
                $blogs_template->total_blog_count,
                'buddypress'
            ),
            $from_num,
...
function bp_get_blog_avatar($args = '')
{
    ...
    if (!$blogs_template && isset($args['blog_id']) && $args['blog_id']) {
        ...
        $alt_attribute = sprintf(
            __('Site icon for %s', 'buddypress'),
            bp_get_blog_name()
        );
    }
...
function bp_get_blog_permalink()
{
    ...
    if (empty($blogs_template->blog->domain)) {
        ...
        $permalink =
            $protocol .
            $blogs_template->blog->domain .
            $blogs_template->blog->path;
    }
...
function bp_get_blog_description()
{
    ...
    return apply_filters(
        ...
        $blogs_template->blog->description
    );
}
 ...
 */
function bp_blog_class($classes = [])
{
 ...
 */
function bp_get_blog_class($classes = [])
{
    global $blogs_template;
 ...
 */
function bp_blog_last_active($args = [])
{
 ...
 */
function bp_get_blog_last_active($args = [])
{
    global $blogs_template;
    ...
    if (isset($blogs_template->blog->last_activity)) {
        ...
        $last_activity = !empty($r['active_format'])
            ? bp_core_get_last_activity(
                $blogs_template->blog->last_activity,
                $r['active_format']
            )
 ...
 */
function bp_blog_latest_post($args = [])
{
 ...
 */
function bp_get_blog_latest_post($args = [])
{
    global $blogs_template;
    ...
    if (!empty($retval)) {
        if (!empty($r['latest_format'])) {
            ...
            $retval = sprintf(
                ...
                '<a href="' .
                    $blogs_template->blog->latest_post->guid .
                    '">' .
...
function bp_get_blog_latest_post_title()
{
    ...
    if (
        ...
        !empty($blogs_template->blog->latest_post->post_title)
    ) {
...
function bp_get_blog_latest_post_permalink()
{
    ...
    if (
        ...
        $retval = add_query_arg(
            'p',
            $blogs_template->blog->latest_post->ID,
...
function bp_get_blog_latest_post_content()
{
    ...
    if (
        !empty($blogs_template->blog->latest_post) &&
        !empty($blogs_template->blog->latest_post->post_content)
    ) {
...
function bp_get_blog_latest_post_featured_image($size = 'thumbnail')
{
    ...
    if (
        !empty($blogs_template->blog->latest_post) &&
        !empty($blogs_template->blog->latest_post->images[$size])
    ) {
...
function bp_blog_hidden_fields()
{
    ...
    if (isset($_REQUEST['blogs_search'])) {
        echo '<input type="hidden" id="search_terms" value="' .
            esc_attr($_REQUEST['blogs_search']) .
            '" name="search_terms" />';
    }
}
...
function bp_get_total_blog_count_for_user($user_id = 0)
{
    ...
    return apply_filters(
        ...
        bp_blogs_total_blogs_for_user($user_id),
        $user_id
    );
}
    ...
    if (false === $blog_id || is_wp_error($blog_id)) {
        ...
        if ($errors->get_error_code()) {
            if (
                ...
                printf('<p class="error">%s</p>', $errors->get_error_message());
            }
        }
        ...
        printf(
            ...
            esc_html__(
                'There is no limit to the number of sites that you can have, so create to your heart’s content, but blog responsibly!',
                'buddypress'
            )
        );
        ?>
...
function bp_blogs_signup_blog($blogname = '', $blog_title = '', $errors = '')
{
  ...
  if (!is_subdomain_install()) {
      printf(
          '<label for="blogname">%s</label>',
          esc_html__('Site Name:', 'buddypress')
      );
  } else {
      printf(
          '<label for="blogname">%s</label>',
          esc_html__('Site Domain:', 'buddypress')
      );
  }
  ...
  if (is_wp_error($errors) && $errors->get_error_message('blogname')) {
      printf(
          ...
          $errors->get_error_message('blogname')
      );
  }
  ?>
	</p>
    ...
    ?>
	<p>
		<label for="blog_title"><?php esc_html_e(
      'Site Title:',
      'buddypress'
  ); ?></label>
		<input name="blog_title" type="text" id="blog_title" value="<?php echo esc_html(
      $blog_title
  ); ?>" />

		<?php if (is_wp_error($errors) && $errors->get_error_message('blog_title')) {
      printf(
          '<div class="error">%s</div>',
	...
	<fieldset class="create-site">
		...
		<legend class="label"><?php esc_html_e(
      ...
      'buddypress'
		...
		<p>
			<label class="checkbox" for="blog_public_on">
				<input type="radio" id="blog_public_on" name="blog_public" value="1" <?php checked(
        !isset($_POST['blog_public']) || 1 === (int) $_POST['blog_public']
    ); ?> />
		...
		<p>
			<label class="checkbox" for="blog_public_off">
				<input type="radio" id="blog_public_off" name="blog_public" value="0" <?php checked(
        isset($_POST['blog_public']) && 0 === (int) $_POST['blog_public']
    ); ?> />
...
function bp_blogs_validate_blog_signup()
{
    ...
    if (
        ...
        return new WP_Error(
            'bp_blogs_doing_it_wrong',
            __(
                'Sorry, we cannot create the site. Please try again later.',
                'buddypress'
            )
        );
    }
 ...
 */
function bp_blogs_confirm_blog_signup(
    $domain,
    $path,
    $blog_title,
    $user_name,
    ...
    ?>
	<p class="success"><?php esc_html_e(
...
function bp_blogs_blog_tabs()
{
	...
	<ul class="content-header-nav">
		<li<?php if (
      bp_is_current_action('my-blogs') ||
      !bp_current_action()
  ): ?> class="current"<?php endif; ?>>
			<a href="<?php echo trailingslashit(
       bp_displayed_user_domain() . bp_get_blogs_slug() . '/my-blogs'
   ); ?>">
				<?php /* translators: %s: the User Display Name */
    printf(__("%s's Sites", 'buddypress'), bp_get_displayed_user_fullname()); ?>
			</a>
		</li>
		<li<?php if (
      bp_is_current_action('recent-posts')
  ): ?> class="current"<?php endif; ?>>
			<a href="<?php echo trailingslashit(
       bp_displayed_user_domain() . bp_get_blogs_slug() . '/recent-posts'
   ); ?>">
				<?php /* translators: %s: the User Display Name */
    printf(
        __("%s's Recent Posts", 'buddypress'),
      ...
      bp_is_current_action('recent-comments')
			...
			<a href="<?php echo trailingslashit(
       bp_displayed_user_domain() . bp_get_blogs_slug() . '/recent-comments'
   ); ?>">
				<?php /* translators: %s: the User Display Name */
    printf(
        __("%s's Recent Comments", 'buddypress'),
        bp_get_displayed_user_fullname()
    ); ?>
			</a>
		</li>
...
function bp_directory_blogs_search_form()
{
    ...
    $search_form_html =
		...
		<label for="blogs_search"><input type="text" name="' .
        esc_attr($query_arg) .
        '" id="blogs_search" placeholder="' .
...
function bp_get_blog_create_button()
{
    ...
    return bp_get_button(
        apply_filters('bp_get_blog_create_button', $button_args)
    );
}
...
function bp_blogs_get_profile_stats($args = '')
{
    ...
    if (is_multisite() && empty($r['output'])) {
        ...
        if (!empty($r['user_id'])) {
            ...
            if (empty($r['blogs'])) {
                $r['blogs'] = absint(
                    bp_blogs_total_blogs_for_user($r['user_id'])
                );
            }
            ...
            $r['output'] .= sprintf(
                _n('%s site', '%s sites', $r['blogs'], 'buddypress'),
                '<strong>' . $r['blogs'] . '</strong>'
            );
            $r['output'] .= $r['after'];
        }
    }
