 ...
 * @return bool
 */
function bp_blogs_record_existing_blogs($args = [])
{
    global $wpdb;
    ...
    if (is_multisite()) {
        ...
        $sql['select'] = $wpdb->prepare(
            "SELECT blog_id, last_updated FROM {$wpdb->base_prefix}blogs WHERE mature = 0 AND spam = 0 AND deleted = 0 AND site_id = %d",
            $r['site_id']
        );

        // Omit root blog if large network.
        if (bp_is_large_install()) {
            $sql['omit_root_blog'] = $wpdb->prepare(
                "AND blog_id != %d",
                bp_get_root_blog_id()
            );
        }
    ...
    foreach ((array) $blogs as $blog) {
        ...
        if (
            !empty($blog->last_updated) &&
            false !== strtotime($blog->last_updated)
        ) {
...
function bp_blogs_is_blog_recordable($blog_id, $user_id = 0)
{
     ...
     */
    $recordable_globally = apply_filters(
    ...
    if (!empty($user_id)) {
         ...
         */
        $recordable_for_user = apply_filters(
...
function bp_blogs_is_blog_trackable($blog_id, $user_id = 0)
{
     ...
     * @param int  $blog_id Current blog ID.
     */
    $trackable_globally = apply_filters(
        'bp_blogs_is_blog_trackable',
        bp_blogs_is_blog_recordable($blog_id, $user_id),
        $blog_id
    );
    ...
    if (!empty($user_id)) {
         ...
         * @param int  $user_id Current user ID.
         */
        $trackable_for_user = apply_filters(
...
function bp_blogs_record_blog($blog_id, $user_id, $no_activity = false)
{
    ...
    $close_old_posts = get_blog_option(
        $blog_id,
    ...
    $is_private =
        !empty($_POST['blog_public']) && (int) $_POST['blog_public']
            ? false
            : true;
     ...
     */
    do_action_ref_array('bp_blogs_new_blog', [
        &$recorded_blog,
        $is_private,
        $is_recorded,
        $no_activity,
 ...
 * @param string $newvalue Value to change meta to.
 */
function bp_blogs_update_option_close_comments_for_old_posts(
    $oldvalue,
    $newvalue
) {
    global $wpdb;
...
add_action(
    'update_option_close_comments_for_old_posts',
...
add_action(
    'update_option_close_comments_days_old',
...
add_action(
    'update_option_thread_comments_depth',
...
add_action(
    'update_option_comment_moderation',
...
function bp_blogs_update_option_site_icon($old_value, $new_value)
{
    ...
    if (0 === $new_value) {
        ...
        bp_blogs_update_blogmeta(
            $blog_id,
            'site_icon_url_thumb',
            bp_blogs_get_site_icon_url($blog_id, bp_core_avatar_thumb_width())
        );
        bp_blogs_update_blogmeta(
            $blog_id,
            'site_icon_url_full',
            bp_blogs_get_site_icon_url($blog_id, bp_core_avatar_full_width())
        );
    }
}
add_action(
 ...
 */
function bp_blogs_update_post_activity_meta(
    $post,
    $activity,
    ...
    if ($post->post_title !== $existing_title) {
        ...
        if (!empty($activity_post_object->comments_tracking->action_id)) {
            ...
            if (!empty($comments)) {
                ...
                $args['filter'] = [
                    'object' =>
                        $activity_post_object->comments_tracking->component_id,
                    'action' =>
                ...
                if (!empty($activities['activities'])) {
                    $activity_ids = (array) wp_list_pluck(
                ...
                $activities = bp_activity_get($args);
                if (!empty($activities['activities'])) {
                    $activity_ids = array_merge(
                        $activity_ids,
                        (array) wp_list_pluck($activities['activities'], 'id')
                    );
                }
 ...
 */
function bp_blogs_comment_sync_activity_comment(
    &$activity_id,
    $comment = null,
    $activity_args = [],
    ...
    if (
        empty($activity_args) ||
        empty($comment->post->ID) ||
        empty($activity_post_object->comment_action_id)
    ) {
        return false;
    }
    ...
    if (
        !empty($activity_id) &&
        !empty($activity_args['item_id']) &&
        'new_blog_comment' === $activity_post_object->comment_action_id
    ) {
    ...
    if (!bp_disable_blogforum_comments()) {
        if (!empty($_REQUEST['action'])) {
            $existing_activity_id = get_comment_meta(
                $comment->comment_ID,
        ...
        if (
            isset($activity_post_object->action_id) &&
            isset($activity_post_object->component_id)
        ) {
            ...
            if (empty($parent_activity_id)) {
                $parent_activity_id = bp_activity_post_type_publish(
                    $comment->post->ID,
                    $comment->post
                );
            }
        ...
        if (!empty($parent_activity_id)) {
            ...
            if (!empty($comment->comment_parent)) {
                $comment_parent = get_comment_meta(
        ...
        if (!empty($activity_args)) {
            ...
            if (empty($activity_args['id'])) {
                ...
                if (
                    'new_blog_comment' ===
                    $activity_post_object->comment_action_id
                ) {
                    bp_activity_update_meta(
                        $activity_id,
                        'post_title',
...
function bp_blogs_add_user_to_blog($user_id, $role = false, $blog_id = 0)
{
    ...
    if (empty($role)) {
        ...
        $user_roles = array_keys(
            (array) bp_get_user_meta($user_id, $key, true)
        );
...
function bp_blogs_get_allowed_roles()
{
     ...
     */
    return apply_filters('bp_blogs_get_allowed_roles', [
        'contributor',
        'author',
        'editor',
        'administrator',
 ...
 */
function bp_blogs_post_type_remove_comment(
    $deleted,
    $comment_id,
    $activity_post_object,
    $activity_type = ''
 ...
 */
function bp_blogs_remove_associated_blog_comments(
    $activity_ids = [],
    $force_delete = true
) {
    // Query args.
 ...
 */
function bp_blogs_delete_blogmeta(
    $blog_id,
    $meta_key = false,
    $meta_value = false,
    $delete_all = false
 ...
 */
function bp_blogs_add_blogmeta(
    $blog_id,
    $meta_key,
    $meta_value,
    $unique = false
...
function bp_blogs_get_signup_form_submitted_vars()
{
    ...
    return array_map(
        'wp_unslash',
        array_intersect_key($submitted_vars, $exprected_vars)
    );
}
