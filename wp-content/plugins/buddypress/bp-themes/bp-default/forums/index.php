	...
	<div id="content">
		<div class="padder">
			...
			<form action="" method="post" id="forums-search-form" class="dir-form">
				...
				<h3><?php
    _e('Forums Directory', 'buddypress');
    if (
        is_user_logged_in()
    ): ?> &nbsp;<a class="button show-hide-new" href="#new-topic" id="new-topic-button"><?php _e(
     'New Topic',
     'buddypress'
 ); ?></a><?php endif;
    ?></h3>
			...
			<form action="" method="post" id="forums-directory-form" class="dir-form">
				...
				<div class="item-list-tabs" role="navigation">
					<ul>
						<li class="selected" id="forums-all"><a href="<?php echo trailingslashit(
          bp_get_root_domain() . '/' . bp_get_forums_root_slug()
      ); ?>"><?php printf(
    __('All Topics <span>%s</span>', 'buddypress'),
    bp_get_forum_topic_count()
); ?></a></li>

						<?php if (
          is_user_logged_in() &&
          bp_get_forum_topic_count_for_user(bp_loggedin_user_id())
      ): ?>

							<li id="forums-personal"><a href="<?php echo trailingslashit(
           bp_loggedin_user_domain() . bp_get_forums_slug() . '/topics'
       ); ?>"><?php printf(
    __('My Topics <span>%s</span>', 'buddypress'),
    bp_get_forum_topic_count_for_user(bp_loggedin_user_id())
); ?></a></li>
				...
				<div id="forums-dir-list" class="forums dir-list" role="main">
					...
					<?php locate_template(['forums/forums-loop.php'], true); ?>
                 ...
                 '&type=alphabetical&max=100&per_page=100'
						...
						<form action="" method="post" id="forum-topic-form" class="standard-form">
							...
							<select id="topic_group_id" name="topic_group_id">
								...
								<option value=""><?php /* translators: no option picked in select box */ _e(
            '----',
            'buddypress'
        ); ?></option>
        ...
        endwhile; ?>
							...
							<div class="submit">
								<input type="submit" name="submit_topic" id="submit" value="<?php esc_attr_e(
            'Post Topic',
            'buddypress'
        ); ?>" />
								<input type="button" name="submit_topic_cancel" id="submit_topic_cancel" value="<?php esc_attr_e(
            'Cancel',
            'buddypress'
        ); ?>" />
							</div>
           ...
           site_url(bp_get_groups_root_slug() . '/create/')
       ); ?></p>
