...
if (!function_exists('bp_dtheme_enqueue_scripts')):
    ...
    function bp_dtheme_enqueue_scripts()
    {
        ...
        if (
            is_active_widget(false, false, 'bp_core_members_widget') &&
            !is_admin() &&
            !is_network_admin()
        ) {
...
if (!function_exists('bp_dtheme_enqueue_styles')):
    ...
    function bp_dtheme_enqueue_styles()
    {
        ...
        wp_register_style(
            'bp-default-main',
            get_template_directory_uri() . '/_inc/css/default.css',
            [],
            bp_get_version()
        );
        ...
        if (is_rtl()) {
            wp_enqueue_style(
                'bp-default-main-rtl',
                get_template_directory_uri() . '/_inc/css/default-rtl.css',
                ['bp-default-main'],
                bp_get_version()
            );
        }
        ...
        if (current_theme_supports('bp-default-responsive')) {
            wp_enqueue_style(
                ...
                get_template_directory_uri() . '/_inc/css/responsive.css',
                ['bp-default-main'],
                bp_get_version()
            );
            ...
            if (is_rtl()) {
                wp_enqueue_style(
                    'bp-default-responsive-rtl',
                    get_template_directory_uri() .
                        '/_inc/css/responsive-rtl.css',
                    ['bp-default-responsive'],
                    bp_get_version()
                );
            }
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		...
		<div class="comment-content">
			<div class="comment-meta">
				<p>
					<?php /* translators: 1: comment author url, 2: comment author name, 3: comment permalink, 4: comment date/timestamp*/
     printf(
         __(
             '<a href="%1$s" rel="nofollow">%2$s</a> said on <a href="%3$s"><span class="time-since">%4$s</span></a>',
             'buddypress'
         ),
         ...
         get_comment_date()
          ...
          'max_depth' => $args['max_depth'],
					...
					<?php if (current_user_can('edit_comment', $comment->comment_ID)): ?>
						<?php printf(
          '<a class="button comment-edit-link bp-secondary-action" href="%1$s" title="%2$s">%3$s</a> ',
          get_edit_comment_link($comment->comment_ID),
          esc_attr__('Edit comment', 'buddypress'),
          __('Edit', 'buddypress')
      ); ?>
					<?php endif; ?>
...
if (!function_exists('bp_dtheme_show_notice')):
	...
	<div id="message" class="updated fade">
		<p><?php printf(
      __(
          'Theme activated! This theme contains <a href="%s">custom header image</a> support and <a href="%s">sidebar widgets</a>.',
          'buddypress'
      ),
...
if (!function_exists('bp_dtheme_comment_form')):
    ...
    function bp_dtheme_comment_form($default_labels)
    {
        ...
        $new_labels = [
            'comment_field' =>
                '<p class="form-textarea"><textarea name="comment" id="comment" cols="60" rows="10" aria-required="true"></textarea></p>',
            ...
            'must_log_in' =>
                ...
                sprintf(
                    __(
                        'You must be <a href="%1$s">logged in</a> to post a comment.',
                        'buddypress'
                    ),
        ...
        return apply_filters(
            ...
            array_merge($default_labels, $new_labels)
        );
    }
...
if (!function_exists('bp_dtheme_content_nav')):
    ...
    function bp_dtheme_content_nav($nav_id)
    {
       ...
       __('&larr; Previous Entries', 'buddypress')
   ); ?></div>
			<div class="alignright"><?php previous_posts_link(
       __('Next Entries &rarr;', 'buddypress')
   ); ?></div>
		</div><!-- #<?php echo $nav_id; ?> -->
