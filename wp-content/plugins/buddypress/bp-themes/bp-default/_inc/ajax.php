...
function bp_dtheme_object_template_loader()
{
    ...
    locate_template(["$object/$object-loop.php"], true);
    exit();
}
...
{
    locate_template(['members/single/messages/messages-loop.php'], true);
    exit();
}
...
function bp_dtheme_activity_template_loader()
{
    ...
    ob_start();
    locate_template(['activity/activity-loop.php'], true);
...
function bp_dtheme_post_update()
{
    ...
    if (bp_has_activities('include=' . $activity_id)) {
        while (bp_activities()) {
            bp_the_activity();
            locate_template(['activity/entry.php'], true);
        }
    }
...
function bp_dtheme_get_single_activity_content()
{
    ...
    do_action_ref_array('bp_dtheme_get_single_activity_content', [&$activity]);
    ...
    if (!groups_is_user_member(bp_loggedin_user_id(), $group->id)) {
        if ('public' == $group->status) {
            ...
            if (!groups_join_group($group->id)) {
                ...
                echo '<a id="group-' .
                    esc_attr($group->id) .
                    '" class="leave-group" rel="leave" title="' .
            ...
            if (
                groups_check_user_has_invite(bp_loggedin_user_id(), $group->id)
            ) {
                ...
                if (
                    ...
                    echo '<a id="group-' .
                        esc_attr($group->id) .
                        '" class="membership-requested" rel="membership-requested" title="' .
                        __('Membership Requested', 'buddypress') .
                        '" href="' .
        ...
        if (!groups_leave_group($group->id)) {
            ...
            echo '<a id="group-' .
                esc_attr($group->id) .
                '" class="join-group" rel="join" title="' .
            ...
            echo '<a id="group-' .
                esc_attr($group->id) .
                '" class="request-membership" rel="join" title="' .
...
function bp_dtheme_ajax_messages_send_reply()
{
    ...
    if ($result) { ?>
		<div class="message-box new-message">
			<div class="message-metadata">
				...
				<strong><a href="<?php echo bp_loggedin_user_domain(); ?>"><?php bp_loggedin_user_fullname(); ?></a> <span class="activity"><?php printf(
    __('Sent %s', 'buddypress'),
    bp_core_time_since(bp_core_current_time())
); ?></span></strong>
	...
	<?php } else {echo "-1<div id='message' class='error'><p>" .
            __(
...
function bp_dtheme_ajax_message_markunread()
{
    ...
    if (!$nonce_check || !isset($_POST['thread_ids'])) {
        echo "-1<div id='message' class='error'><p>" .
            __(
                'There was a problem marking messages as unread.',
                'buddypress'
            ) .
            '</p></div>';
    } else {
...
function bp_dtheme_ajax_messages_delete()
{
    ...
    if (!$nonce_check || !isset($_POST['thread_ids'])) {
        echo "-1<div id='message' class='error'><p>" .
            __('There was a problem deleting messages.', 'buddypress') .
            '</p></div>';
    } else {
        $thread_ids = explode(',', $_POST['thread_ids']);
