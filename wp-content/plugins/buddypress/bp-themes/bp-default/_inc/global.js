...
jq(document).ready(function () {
    ...
    jq.post(
      ...
      function (response) {
        ...
        if (response[0] + response[1] == "-1") {
          ...
          jq("form#" + form.attr("id") + " div.error")
            .hide()
            .fadeIn(200);
        } else {
          ...
          if (0 != jq("#latest-update").length) {
            var l = jq(
              "ul#activity-stream li.new-update .activity-content .activity-inner p"
            ).html();
            var v = jq(
              "ul#activity-stream li.new-update .activity-content .activity-header p a.view"
            ).attr("href");
            ...
            u +=
              '<a href="' + v + '" rel="nofollow">' + BP_DTheme.view + "</a>";
  ...
  jq("div.activity-type-tabs").click(function (event) {
    ...
    else if (event.target.nodeName != "A") return false;
  ...
  jq("#activity-filter-select select").change(function () {
    ...
    if (!selected_tab.length) var scope = null;
    else
      var scope = selected_tab
        .attr("id")
        .substr(9, selected_tab.attr("id").length);
      ...
      jq.post(
        ...
        function (response) {
          ...
          target.fadeOut(100, function () {
            ...
            jq(this).attr(
              "title",
              "fav" == type ? BP_DTheme.remove_fav : BP_DTheme.mark_as_fav
            );
            jq(this).fadeIn(100);
          });
          ...
          if ("fav" == type) {
            if (!jq(".item-list-tabs li#activity-favorites").length)
              jq(".item-list-tabs ul li#activity-mentions").before(
            ...
            jq(".item-list-tabs ul li#activity-favorites span").html(
              Number(
                jq(".item-list-tabs ul li#activity-favorites span").html()
              ) + 1
            );
            ...
            if (
              ...
              if (
                jq(".item-list-tabs ul li#activity-favorites").hasClass(
                  "selected"
                )
              )
          ...
          if (
            "activity-favorites" == jq(".item-list-tabs li.selected").attr("id")
          )
  ...
  jq("div.activity").on("click", ".activity-read-more a", function (event) {
    ...
    var inner_class =
      type == "acomment" ? "acomment-content" : "activity-inner";
  ...
  if (jq(".activity-comments").length) bp_dtheme_hide_comments();
  ...
  jq("div.activity").click(function (event) {
    ...
    if (
      ...
      if (target.parent().hasClass("acomment-reply")) target = target.parent();
      ...
      form.children("div").each(function () {
        if (jq(this).hasClass("error")) jq(this).hide();
      });
    ...
    if (target.attr("name") == "ac_form_submit") {
      ...
      var ajaxdata = {
        ...
        _wpnonce_new_activity_comment: jq(
          "input#_wpnonce_new_activity_comment"
        ).val(),
      ...
      jq.post(ajaxurl, ajaxdata, function (response) {
        ...
        if (response[0] + response[1] == "-1") {
          form.append(
            jq(response.substr(2, response.length)).hide().fadeIn(200)
          );
        } else {
          ...
          form.fadeOut(200, function () {
            ...
            activity_comments
              .children("ul")
              .append(jq(the_comment).hide().fadeIn(200));
          ...
          jq("li#activity-" + form_id[2] + " a.acomment-reply span").html(
            Number(
              jq("li#activity-" + form_id[2] + " a.acomment-reply span").html()
            ) + 1
          );
          ...
          if (show_all_a) {
            var new_count = jq(
              "li#activity-" + form_id[2] + " a.acomment-reply span"
            ).html();
      ...
      jq.post(
        ...
        function (response) {
          ...
          if (response[0] + response[1] == "-1") {
            ...
            var children = jq("li#" + comment_li.attr("id") + " ul").children(
              "li"
            );
            var child_count = 0;
            jq(children).each(function () {
              if (!jq(this).is(":hidden")) child_count++;
            });
            ...
            var count_span = jq(
              "li#" +
                comment_li.parents("ul#activity-stream > li").attr("id") +
            ...
            if (show_all_a) {
              show_all_a.html(
                BP_DTheme.show_x_comments.replace("%d", new_count)
              );
            }

            /* If that was the last comment for the item, remove the has-comments class to clean up the styling */
            if (0 == new_count) {
              jq(comment_li.parents("ul#activity-stream > li")).removeClass(
                "has-comments"
              );
            }
          }
    ...
    if (target.hasClass("spam-activity-comment")) {
      ...
      comment_li
        ...
        .append(comment_li.parents(".activity-comments").children("form"));
      ...
      jq.post(
        ...
        function (response) {
          ...
          if (response[0] + response[1] == "-1") {
            ...
            jq("li#" + parent_li.attr("id") + " a.acomment-reply span").html(
              jq(
                "li#" + parent_li.attr("id") + " a.acomment-reply span"
              ).html() -
                (1 + child_count)
            );
          }
  ...
  jq("div#content").click(function (event) {
    ...
    if (target.hasClass("button")) return true;

    if (
      target.parent().parent().hasClass("pagination") &&
      !target.parent().parent().hasClass("no-ajax")
    ) {
      if (target.hasClass("dots") || target.hasClass("current")) return false;

      if (jq(".item-list-tabs li.selected").length)
        var el = jq(".item-list-tabs li.selected");
      else var el = jq("li.filter select");
      ...
      if (jq(target).hasClass("next"))
        ...
        var page_number = Number(jq(".pagination span.current").html()) - 1;
      else var page_number = Number(jq(target).html());
  ...
  jq("a.show-hide-new").click(function () {
    if (!jq("#new-topic-post").length) return false;
  ...
  jq("div#invite-list input").click(function () {
    ...
    if (jq(this).prop("checked") == true) var friend_action = "invite";
    else var friend_action = "uninvite";
  ...
  jq("input#send_reply_button").click(function () {
    ...
    checkboxes.each(function (i) {
      if (jq(this).is(":checked")) {
        if (jq("tr#m-" + jq(this).attr("value")).hasClass(currentClass)) {
          ...
          var thread_count = jq(
            "tr#m-" + jq(this).attr("value") + " td span.unread-count"
          ).html();

          jq("tr#m-" + jq(this).attr("value") + " td span.unread-count").html(
            unreadCount
          );
          jq("tr#m-" + jq(this).attr("value") + " td span.unread-count").css(
            "display",
            unreadCountDisplay
          );
  ...
  jq("body.messages #item-body div.messages").on(
    ...
    function () {
      if (
        ...
        jq.inArray(
          this.id,
          Array("delete_sentbox_messages", "delete_inbox_messages")
        )
      ) {
        return;
      }
        ...
        function (response) {
          if (response[0] + response[1] == "-1") {
            ...
            jq("#message-threads").before(
              '<div id="message" class="updated"><p>' + response + "</p></div>"
            );
          ...
          jq("#delete_inbox_messages, #delete_sentbox_messages").removeClass(
            "loading"
          );
        }
      );
...
function bp_init_activity() {
  ...
  if (
    null != jq.cookie("bp-activity-filter") &&
  ...
  /* Activity Tab Set */
  if (
    null != jq.cookie("bp-activity-scope") &&
...
function bp_init_objects(objects) {
  jq(objects).each(function (i) {
    if (
      null != jq.cookie("bp-" + objects[i] + "-filter") &&
      jq("li#" + objects[i] + "-order-select select").length
    )
      jq(
        "li#" +
          objects[i] +
          '-order-select select option[value="' +
          jq.cookie("bp-" + objects[i] + "-filter") +
          '"]'
      ).prop("selected", true);

    if (
      null != jq.cookie("bp-" + objects[i] + "-scope") &&
      jq("div." + objects[i]).length
    ) {
      ...
      jq(
        ".item-list-tabs li#" +
          objects[i] +
          "-" +
          jq.cookie("bp-" + objects[i] + "-scope") +
          ", div.item-list-tabs#object-nav li.current"
      ).addClass("selected");
    }
  });
}
...
function bp_filter_request(
  ...
  jq(
    ".item-list-tabs li#" +
      ...
      "-" +
      scope +
      ", .item-list-tabs#object-nav li.current"
  ).addClass("selected");
  jq(".item-list-tabs li.selected").addClass("loading");
  jq('.item-list-tabs select option[value="' + filter + '"]').prop(
    "selected",
    true
  );
...
function bp_activity_request(scope, filter) {
  ...
  /* Set the correct selected nav and filter */
  jq("li#activity-" + scope + ", .item-list-tabs li.current").addClass(
    "selected"
  );
  jq(
    "#object-nav.item-list-tabs li.selected, div.activity-type-tabs li.selected"
  ).addClass("loading");
  jq('#activity-filter-select select option[value="' + filter + '"]').prop(
    "selected",
    true
  );
