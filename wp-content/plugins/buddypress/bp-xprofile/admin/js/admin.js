...
function add_option(forWhat) {
  ...
  if (forWhat === "checkbox" || forWhat === "multiselectbox") {
    ...
    isDefault.setAttribute(
      "name",
      "isDefault_" + forWhat + "_option[" + theId + "]"
    );
  } else {
  ...
  toDelete.setAttribute(
    "href",
    'javascript:hide("' + forWhat + "_div" + theId + '")'
  );
...
function show_options(forWhat) {
  ...
  for (
    var i = 0;
    i < XProfileAdmin.do_settings_section_field_types.length;
    i++
  ) {
    document.getElementById(
      XProfileAdmin.do_settings_section_field_types[i]
    ).style.display = "none";
  }
...
function titleHint(id) {
  ...
  var title = jQuery("#" + id),
    titleprompt = jQuery("#" + id + "-prompt-text");
      ...
      update: function () {
        jQuery.post(
          ajaxurl,
          {
            ...
            cookie: encodeURIComponent(document.cookie),
            _wpnonce_reorder_groups: jQuery(
              "input#_wpnonce_reorder_groups"
            ).val(),
      ...
      update: function () {
        jQuery.post(
          ajaxurl,
          {
            ...
            cookie: encodeURIComponent(document.cookie),
            _wpnonce_reorder_fields: jQuery(
              "input#_wpnonce_reorder_fields"
            ).val(),
  ...
  $date_format.click(function (e) {
    switch (e.target.value) {
      ...
      default:
        ...
        $date_format_sample.html(
          jQuery(e.target).siblings(".date-format-label").html()
        );
        break;
    }
  });
    ...
    $tab_items = jQuery("ul:first li", $tabs).droppable({
      ...
      drop: function (ev, ui) {
        var $item = jQuery(this), // The tab
          $list = jQuery($item.find("a").attr("href")).find(
            ".connectedSortable"
          ); // The tab body
