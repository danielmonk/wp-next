...
function xprofile_admin($message = '', $type = 'error')
{
    ...
    // Field ID.
    $field_id = !empty($_GET['field_id']) ? intval($_GET['field_id']) : false;
    ...
    if (in_array($mode, $allowed_modes, true)) {
        ...
        if (false !== $group_id) {
            ...
            if ('add_field' == $mode) {
                ...
                in_array($mode, ['delete_group', 'do_delete_group'], true)
            ) {
            ...
            false !== $field_id &&
            in_array($mode, ['delete_field', 'do_delete_field'], true)
        ) {
            ...
            !empty($option_id) &&
            in_array($mode, ['delete_option', 'do_delete_option'], true)
        ) {
...
function xprofile_admin_screen($message = '', $type = 'error')
{
	...
	<div class="wrap">
		<h1 class="wp-heading-inline"><?php _ex(
      'Profile Fields',
      'Settings page header',
      'buddypress'
  ); ?></h1>

			<a id="add_group" class="page-title-action" href="<?php echo esc_url(
       $add_group_url
   ); ?>"><?php _e('Add New Field Group', 'buddypress'); ?></a>
   ...
   ?>
         ...
         foreach ($groups as $group): ?>
						...
						<li id="group_<?php echo esc_attr($group->id); ?>">
							<a href="#tabs-<?php echo esc_attr($group->id); ?>" class="ui-tab">
								<?php /** This filter is documented in bp-xprofile/bp-xprofile-template.php */
        echo esc_html(
        ...
        foreach ($groups as $group):
            ...
            ?>
					...
					<noscript>
						<h3><?php /** This filter is documented in bp-xprofile/bp-xprofile-template.php */
      echo esc_html(
          apply_filters('bp_get_the_profile_group_name', $group->name)
      ); ?></h3>
					</noscript>
            ...
            $add_field_url
        ); ?>"><?php _e('Add New Field', 'buddypress'); ?></a>
								<a class="button edit" href="<?php echo esc_url($edit_group_url); ?>"><?php _ex(
    'Edit Group',
    ...
    'buddypress'
								...
								<?php if ($group->can_delete): ?>
									...
									<div class="delete-button">
										<a class="confirm submitdelete deletion ajax-option-delete" href="<?php echo esc_url(
              $delete_group_url
          ); ?>"><?php _ex(
    'Delete Group',
    ...
    'buddypress'
        ...
        do_action('xprofile_admin_group_action', $group); ?>
						...
						<?php if (!empty($group->description)): ?>
							...
							<p><?php /** This filter is documented in bp-xprofile/bp-xprofile-template.php */
       echo esc_html(
           apply_filters(
               ...
               $group->description
						...
						<fieldset id="<?php echo esc_attr(
          $group->id
      ); ?>" class="connectedSortable field-group" aria-live="polite" aria-atomic="true" aria-relevant="all">
							<legend class="screen-reader-text"><?php /** This filter is documented in bp-xprofile/bp-xprofile-template.php */
       /* translators: accessibility text */
       printf(
           esc_html__('Fields for "%s" Group', 'buddypress'),
           apply_filters('bp_get_the_profile_group_name', $group->name)
       ); ?></legend>
           ...
           foreach ($group->fields as $field) {
           ...
           } // end for // !$group->fields
       else:
            ?>

								<p class="nodrag nofields"><?php _e(
            ...
            ?>
						...
						<?php if (empty($group->can_delete)): ?>
							...
							<p><?php esc_html_e(
           '* Fields in this group appear on the signup page.',
           'buddypress'
       ); ?></p>
        ...
        endforeach;
         ...
         ?>
					...
					<div id="message" class="error notice is-dismissible"><p><?php _ex(
         'You have no groups.',
         'You have no profile fields groups.',
         'buddypress'
     ); ?></p></div>
					<p><a href="<?php echo esc_url($add_group_url); ?>"><?php _ex(
    'Add New Group',
...
function xprofile_admin_delete_group_screen($group_id)
{
	...
	<div class="wrap">
      ...
      'buddypress'
		...
		<p><?php esc_html_e(
      'You are about to delete the following field group:',
      'buddypress'
  ); ?></p>
      ...
      wp_nonce_url(
          add_query_arg(
              ['mode' => 'do_delete_group', 'group_id' => $group_id],
              $base_url
          ),
          'bp_xprofile_delete_group'
      )
  ); ?>"><?php esc_html_e('Delete Permanently', 'buddypress'); ?></a>
		<a class="button" href="<?php echo esc_attr(
      $base_url
...
function xprofile_admin_manage_field($group_id, $field_id = null)
{
    ...
    if (isset($_POST['saveField'])) {
        ...
        if (BP_XProfile_Field::admin_validate()) {
            ...
            $field->field_order = $wpdb->get_var(
                $wpdb->prepare(
            ...
            if (
                !is_numeric($field->field_order) ||
                is_wp_error($field->field_order)
            ) {
                $field->field_order = (int) $wpdb->get_var(
                    $wpdb->prepare(
            ...
            if (empty($field_id)) {
                ...
                if (isset($_POST['has-member-types'])) {
                    ...
                    if (isset($_POST['member-types'])) {
                        $member_types = stripslashes_deep(
                            $_POST['member-types']
                        );
                    }
                ...
                if (
                    !empty($_POST['default-visibility']) &&
                    in_array(
                        $_POST['default-visibility'],
                        wp_list_pluck(bp_xprofile_get_visibility_levels(), 'id')
                    )
                ) {
                ...
                if (
                    !empty($_POST['allow-custom-visibility']) &&
                    in_array($_POST['allow-custom-visibility'], [
                        'allowed',
                        'disabled',
                    ])
                ) {
                ...
                if (
                    isset($_POST['do_autolink']) &&
                    'on' === wp_unslash($_POST['do_autolink'])
                ) {
                    bp_xprofile_update_field_meta(
                        $field_id,
                        'do_autolink',
                ...
                if ($field->type_obj->do_settings_section()) {
                    $settings = isset($_POST['field-settings'])
                        ? wp_unslash($_POST['field-settings'])
                        : [];
...
function xprofile_admin_delete_field(
    ...
    check_admin_referer(
        'bp_xprofile_delete_field-' . $field_id,
    ...
    $field_type =
        'field' == $field_type
            ? __('field', 'buddypress')
            : __('option', 'buddypress');

    // Display the field/option delete confirmation screen.
    if (in_array($mode, ['delete_field', 'delete_option'])) {
        ...
        if (!$field->delete((bool) $delete_data)) {
            ...
            /* translators: %s: the field type */
            $message = sprintf(
                __('The %s was deleted successfully!', 'buddypress'),
                $field_type
            );
            $type = 'success';
...
function xprofile_admin_delete_field_screen($field_id, $field_type)
{
    ...
    $base_url = remove_query_arg(
        ['mode', 'field_id', 'bp_xprofile_delete_field'],
        $_SERVER['REQUEST_URI']
    );
    ?>
	...
	<div class="wrap">
      ...
      wp_nonce_url(
          add_query_arg(
              ['mode' => 'do_delete_field', 'field_id' => $field_id],
              $base_url
          ),
          'bp_xprofile_delete_field-' . $field_id,
          'bp_xprofile_delete_field'
...
add_action(
    'wp_ajax_xprofile_reorder_groups',
...
function xprofile_admin_field($admin_field, $admin_group, $class = '')
{
	...
	<fieldset id="draggable_field_<?php echo esc_attr(
     $field->id
 ); ?>" class="sortable<?php
echo ' ' . $field->type;
if (!empty($class)) {
...
?>">
    ...
    do_action('xprofile_admin_field_name_legend', $field); ?>
		...
		<div class="field-wrapper">
			...
			<?php if (in_array($field->type, array_keys(bp_xprofile_get_field_types()))) {
       ...
       do_action('xprofile_admin_field', $field, 1);
			...
			<div class="actions">
				<a class="button edit" href="<?php echo esc_url(
        $field_edit_url
    ); ?>"><?php _ex('Edit', 'Edit field link', 'buddypress'); ?></a>

				<?php if ($field->can_delete): ?>

					<div class="delete-button">
						<a class="confirm submit-delete deletion" href="<?php echo esc_url(
          wp_nonce_url(
              $field_delete_url,
              'bp_xprofile_delete_field-' . $field->id,
              'bp_xprofile_delete_field'
          )
      ); ?>"><?php _ex('Delete', 'Delete field link', 'buddypress'); ?></a>
					</div>
...
function bp_xprofile_admin_form_field_types($select_field_type)
{
    ...
    foreach (bp_xprofile_get_field_types() as $field_name => $field_class) {
        ...
        if (isset($categories[$the_category])) {
            $categories[$the_category][] = [$field_name, $field_type_obj];
        } else {
            $categories[$the_category] = [[$field_name, $field_type_obj]];
        }
    }
    ...
    foreach ($categories as $category => $fields) {
        ...
        foreach ($fields as $field_type_obj) {
            ...
            printf(
                ...
                esc_attr($field_name),
                selected($select_field_type, $field_name, false),
                esc_html($field_type_obj->name)
            );
        }
