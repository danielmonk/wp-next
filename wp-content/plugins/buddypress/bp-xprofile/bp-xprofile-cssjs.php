...
function xprofile_add_admin_css()
{
    if (
        !empty($_GET['page']) &&
        strpos($_GET['page'], 'bp-profile-setup') !== false
    ) {
        $min = bp_core_get_minified_asset_suffix();

        wp_enqueue_style(
            'xprofile-admin-css',
            buddypress()->plugin_url . "bp-xprofile/admin/css/admin{$min}.css",
            [],
            bp_get_version()
        );
...
function xprofile_add_admin_js()
{
    if (
        ...
        wp_enqueue_script(
            'xprofile-admin-js',
            buddypress()->plugin_url . "bp-xprofile/admin/js/admin{$min}.js",
        ...
        foreach (
            ...
            as $field_type => $field_type_class
        ) {
        ...
        if (!empty($_GET['field_id'])) {
            ...
            $strings['do_autolink'] = bp_xprofile_get_meta(
                $field_id,
                'field',
                'do_autolink'
            );
