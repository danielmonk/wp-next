    ...
    switch ($cap) {
        case 'bp_xprofile_change_field_visibility':
            ...
            if (!empty($args[0])) {
                ...
                isset($GLOBALS['profile_template']) &&
                $GLOBALS['profile_template']->in_the_loop
            ) {
 ...
 * @return bool
 */
function bp_xprofile_grant_bp_xprofile_change_field_visibility_for_logged_out_users(
    $user_can,
    $user_id,
    $capability
) {
    if (
        'bp_xprofile_change_field_visibility' === $capability &&
        0 === $user_id
    ) {
        $field_id = bp_get_the_profile_field_id();
        if (
