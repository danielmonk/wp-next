...
function xprofile_filter_kses($content, $data_obj = null, $field_id = null)
{
    ...
    if ($field_id && bp_xprofile_is_richtext_enabled_for_field($field_id)) {
        ...
        if (bp_current_user_can('unfiltered_html')) {
            $richtext_tags['span'] = ['style' => 1];
            $richtext_tags['p'] = ['style' => 1];
        }
     ...
     */
    $xprofile_allowedtags = apply_filters(
 ...
 * @param int    $field_id Field ID.
 */
function xprofile_sanitize_data_value_before_display_from_get_field_data(
    $value,
    $field_id
) {
...
function xprofile_sanitize_data_value_before_save(
    ...
    if (!is_array($field_value)) {
        ...
        $filtered_field_value = wp_rel_nofollow(
            force_balance_tags($kses_field_value)
        );
         ...
         */
        $filtered_field_value = apply_filters(
        ...
        foreach ((array) $field_value as $value) {
            ...
            $filtered_value = wp_rel_nofollow(
                force_balance_tags($kses_field_value)
            );
 ...
 * @return mixed
 */
function xprofile_filter_format_field_value_by_type(
    $field_value,
    $field_type = '',
    $field_id = ''
) {
 ...
 * @return mixed
 */
function xprofile_filter_pre_validate_value_by_field_type(
    $value,
    $field,
    $field_type_obj
) {
    if (method_exists($field_type_obj, 'pre_validate_filter')) {
        $value = call_user_func(
            [$field_type_obj, 'pre_validate_filter'],
            $value,
            $field->id
        );
    }
...
function bp_xprofile_escape_field_data($value, $field_type, $field_id)
{
    if (bp_xprofile_is_richtext_enabled_for_field($field_id)) {
        ...
        $data_obj = null;
        if (bp_is_user()) {
            $data_obj = new BP_XProfile_ProfileData(
                $field_id,
...
function xprofile_filter_link_profile_data(
    ...
    if (strpos($field_value, ',') !== false) {
        ...
        array_walk($values, function (&$value, $key) use ($field_type, $field) {
            $value = bp_xprofile_escape_field_data(
                $value,
                $field_type,
                $field->id
            );
 ...
 */
function bp_xprofile_filter_user_query_populate_extras(
    BP_User_Query $user_query,
    $user_ids_sql = ''
) {
...
function bp_xprofile_filter_meta_query($q)
{
    ...
    switch ($first_word_matches[0]) {
        ...
        case 'SELECT':
            $q = str_replace(
                [$matches[0], 'SELECT object_id', 'WHERE '],
                [
                    'object_id',
...
function bp_xprofile_register_personal_data_exporter($exporters)
{
    $exporters['buddypress-xprofile'] = [
        ...
        'exporter_bp_friendly_name' => _x(
            ...
            'BuddyPress Extended Profile data exporter friendly name',
            'buddypress'
        ),
    ];
