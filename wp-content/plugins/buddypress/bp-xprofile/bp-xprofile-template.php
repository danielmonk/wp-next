...
function bp_get_field_css_class($class = false)
{
    ...
    $css_classes[] =
        'visibility-' . esc_attr(bp_get_the_profile_field_visibility_level());
     ...
     */
    $css_classes = apply_filters_ref_array('bp_field_css_classes', [
        &$css_classes,
    ]);
...
function bp_field_has_data()
{
    ...
    return apply_filters(
        'bp_field_has_data',
        $profile_template->field_has_data,
        $profile_template,
        $profile_template->field,
        $profile_template->field->id
    );
}
...
function bp_field_has_public_data()
{
    ...
    return apply_filters(
        ...
        !empty($profile_template->field_has_data),
        $profile_template,
        $profile_template->field,
        $profile_template->field->id
    );
...
function bp_get_the_profile_field_visibility_level()
{
    ...
    if (
        bp_is_register_page() &&
        !empty($_POST['field_' . $field->id . '_visibility'])
    ) {
        $retval = esc_attr($_POST['field_' . $field->id . '_visibility']);
    } else {
        $retval = !empty($field->visibility_level)
...
function bp_get_the_profile_field_visibility_level_label()
{
    ...
    if (
        ...
        $level = !empty($field->visibility_level)
            ? $field->visibility_level
            : 'public';
    }
...
function bp_get_profile_last_updated()
{
    ...
    if (!empty($last_updated)) {
        ...
        return apply_filters(
            ...
            sprintf(
                __('Profile updated %s', 'buddypress'),
                bp_core_time_since(strtotime($last_updated))
            )
        );
    }
...
function bp_edit_profile_button()
{
    bp_button([
        ...
        'link_href' => trailingslashit(
            bp_displayed_user_domain() . bp_get_profile_slug() . '/edit'
        ),
        'link_class' => 'edit',
...
function bp_profile_get_visibility_radio_buttons($args = '')
{
   ...
   if (bp_current_user_can('bp_xprofile_change_field_visibility')): ?>
         ...
         'see-field_' . $r['field_id'] . '_' . $level['id']
     ); ?>">
						<input type="radio" id="<?php echo esc_attr(
          'see-field_' . $r['field_id'] . '_' . $level['id']
      ); ?>" name="<?php echo esc_attr(
    'field_' . $r['field_id'] . '_visibility'
); ?>" value="<?php echo esc_attr($level['id']); ?>" <?php checked(
    $level['id'],
    bp_get_the_profile_field_visibility_level()
); ?> />
						<span class="field-visibility-text"><?php echo esc_html(
          $level['label']
      ); ?></span>
					</label>
...
function bp_profile_get_settings_visibility_select($args = '')
{
   ...
   if (bp_current_user_can('bp_xprofile_change_field_visibility')): ?>
			...
			<?php echo $r['before_controls']; ?>
				...
				<label for="<?php echo esc_attr(
        'field_' . $r['field_id']
    ); ?>_visibility" class="<?php echo esc_attr(
    $r['label_class']
); ?>"><?php /* translators: accessibility text */
_e('Select visibility', 'buddypress'); ?></label>
				<select class="<?php echo esc_attr($r['class']); ?>" name="<?php echo esc_attr(
    'field_' . $r['field_id']
); ?>_visibility" id="<?php echo esc_attr(
    'field_' . $r['field_id']
); ?>_visibility">
    ...
    $level['id'],
    bp_get_the_profile_field_visibility_level()
); ?>><?php echo esc_html($level['label']); ?></option>
			...
			<?php else: ?>
				...
				<<?php echo esc_html($r['notoggle_tag']); ?> class="<?php echo esc_attr(
     $r['notoggle_class']
 ); ?>"><?php bp_the_profile_field_visibility_level_label(); ?></<?php echo esc_html(
    $r['notoggle_tag']
); ?>>
