 ...
 * @return array
 */
function bp_xprofile_get_non_cached_field_ids($user_id = 0, $field_ids = [])
{
    $uncached_fields = [];
 ...
 * @return bool
 */
function bp_xprofile_update_meta_cache($object_ids = [])
{
    global $wpdb;
    ...
    $uncached_object_ids = ['group', 'field', 'data'];
    ...
    foreach ($uncached_object_ids as $object_type) {
        ...
        $uncached_object_ids[$object_type] = bp_get_non_cached_ids(
            $object_type_ids,
            $cache_groups[$object_type]
        );
    ...
    foreach ($uncached_object_ids as $otype => $oids) {
        ...
        $where_conditions[] = $wpdb->prepare(
            "( object_type = %s AND object_id IN ({$oids_sql}) )",
            $otype
        );
    }
    ...
    foreach ($meta_list as $meta) {
        ...
        if (
            !isset($cache[$otype][$oid][$okey]) ||
            !is_array($cache[$otype][$oid][$okey])
        ) {
            $cache[$otype][$oid][$okey] = [];
        }
...
{
    wp_cache_delete(
        "{$data_obj->user_id}:{$data_obj->field_id}",
        'bp_xprofile_data'
    );
}
add_action(
...
add_action(
    'update_option_bp-xprofile-fullname-field-name',
