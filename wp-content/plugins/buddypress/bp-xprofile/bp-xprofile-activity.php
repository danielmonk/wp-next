...
function bp_xprofile_format_activity_action_new_avatar($action, $activity)
{
    ...
    $action = sprintf(
        esc_html__('%s changed their profile picture', 'buddypress'),
        $userlink
    );
...
function bp_xprofile_format_activity_action_updated_profile($action, $activity)
{
    ...
    $profile_link = trailingslashit(
        bp_core_get_user_domain($activity->user_id) . bp_get_profile_slug()
    );

    /* translators: %s: user profile link */
    $action = sprintf(
        esc_html__("%s's profile was updated", 'buddypress'),
        '<a href="' .
...
function xprofile_register_activity_action($key, $value)
{
    ...
    return apply_filters(
        ...
        bp_activity_set_action(buddypress()->profile->id, $key, $value),
        $key,
        $value
    );
}
...
function bp_xprofile_updated_profile_activity(
    ...
    foreach ($new_values as $field_id => $new_value) {
        $old_value = isset($old_values[$field_id])
            ? $old_values[$field_id]
            : '';

        // Don't register changes to private fields.
        if (
            empty($new_value['visibility']) ||
            'public' !== $new_value['visibility']
        ) {
            continue;
        }
...
function xprofile_activity_filter_options()
{
	...
	<option value="updated_profile"><?php _e(
     'Profile Updates',
     'buddypress'
 ); ?></option>
