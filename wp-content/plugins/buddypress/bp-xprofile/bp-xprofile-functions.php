 ...
 * @return array $groups
 */
function bp_xprofile_get_groups($args = [])
{
 ...
 * @return boolean
 */
function xprofile_update_field_group_position(
    $field_group_id = 0,
    $position = 0
) {
...
function xprofile_get_field_data($field, $user_id = 0, $multi_format = 'array')
{
    ...
    $values = maybe_unserialize(
        BP_XProfile_ProfileData::get_value_byid($field_id, $user_id)
    );
...
function xprofile_set_field_data($field, $user_id, $value, $is_required = false)
{
    ...
    if (
        ...
        !is_integer($value) &&
        $value !== '0' &&
        (empty($value) || (!is_array($value) && !strlen(trim($value))))
    ) {
        return false;
    }
    ...
    if (
        ...
        !is_integer($value) &&
        $value !== '0' &&
        $field_type_obj->accepts_null_value
    ) {
        $value = [];
    }
    ...
    if ($field_type_obj->supports_options) {
        $field_type_obj->set_allowed_values(
            wp_list_pluck($field->get_children(), 'name')
        );
    }
...
function xprofile_set_field_visibility_level(
    ...
    $current_visibility_levels = bp_get_user_meta(
        $user_id,
...
function xprofile_get_field_visibility_level($field_id = 0, $user_id = 0)
{
    ...
    $current_levels = bp_get_user_meta(
        $user_id,
        'bp_xprofile_visibility_levels',
        true
    );
    $current_level = isset($current_levels[$field_id])
        ? $current_levels[$field_id]
        : '';

    // Use the user's stored level, unless custom visibility is disabled.
    $field = xprofile_get_field($field_id, null, false);
    if (
        isset($field->allow_custom_visibility) &&
        'disabled' === $field->allow_custom_visibility
    ) {
...
{
    $field_data = BP_XProfile_ProfileData::get_random(
        $user_id,
        $exclude_fullname
    );
    ...
    $field_data[0]->value = xprofile_format_profile_field(
        $field_data[0]->type,
        $field_data[0]->value
    );
...
{
    return BP_XProfile_Field::update_position(
        $field_id,
        $position,
        $field_group_id
    );
...
function xprofile_override_user_fullnames()
{
    ...
    if (bp_loggedin_user_id()) {
        buddypress()->loggedin_user->fullname = bp_core_get_user_displayname(
            bp_loggedin_user_id()
        );
    }

    if (bp_displayed_user_id()) {
        buddypress()->displayed_user->fullname = bp_core_get_user_displayname(
...
function bp_xprofile_bp_user_query_search($sql, BP_User_Query $query)
{
    ...
    $search_terms_clean = bp_esc_like(
        wp_kses_normalize_entities($query->query_vars['search_terms'])
    );
 ...
 * @return bool True on success, false on failure.
 */
function bp_xprofile_delete_meta(
    $object_id,
    $object_type,
    $meta_key = false,
    $meta_value = false,
    ...
    if (!in_array($object_type, ['group', 'field', 'data'])) {
        return false;
    }
 ...
 */
function bp_xprofile_update_meta(
    $object_id,
    $object_type,
    $meta_key,
    $meta_value,
 ...
 * @return bool|int
 */
function bp_xprofile_update_fieldgroup_meta(
    $field_group_id,
    $meta_key,
    $meta_value
) {
    return bp_xprofile_update_meta(
        $field_group_id,
        'group',
        $meta_key,
        $meta_value
 ...
 * @return bool|int
 */
function bp_xprofile_update_fielddata_meta(
    $field_data_id,
    $meta_key,
    $meta_value
) {
    ...
    if (false === $id) {
        ...
        $id = $wpdb->get_var(
            $wpdb->prepare(
                ...
                addslashes(bp_xprofile_fullname_field_name())
            )
        );
 ...
 */
function bp_xprofile_get_hidden_fields_for_user(
    $displayed_user_id = 0,
    $current_user_id = 0
) {
    ...
    $hidden_levels = bp_xprofile_get_hidden_field_types_for_user(
        $displayed_user_id,
        $current_user_id
    );
    $hidden_fields = bp_xprofile_get_fields_by_visibility_levels(
        $displayed_user_id,
        $hidden_levels
    );
 ...
 */
function bp_xprofile_get_hidden_field_types_for_user(
    $displayed_user_id = 0,
    $current_user_id = 0
) {
    ...
    if (!empty($current_user_id)) {
        ...
        if (
            $displayed_user_id == $current_user_id ||
 ...
 */
function bp_xprofile_get_fields_by_visibility_levels($user_id, $levels = [])
{
    if (!is_array($levels)) {
        $levels = (array) $levels;
    }

    $user_visibility_levels = bp_get_user_meta(
        $user_id,
        'bp_xprofile_visibility_levels',
        true
    );
    ...
    $field_ids = [];
    foreach (
        (array) $user_visibility_levels
        as $field_id => $field_visibility
    ) {
...
function bp_xprofile_maybe_format_datebox_post_data($field_id)
{
    if (!isset($_POST['field_' . $field_id])) {
        if (
            !empty($_POST['field_' . $field_id . '_day']) &&
