...
class BP_XProfile_ProfileData
{
    ...
    public function exists()
    {
         ...
         */
        return apply_filters_ref_array('xprofile_data_exists', [
            (bool) $retval,
            $this,
        ]);
    }
    ...
    public function is_valid_field()
    {
         ...
         * @param BP_XProfile_ProfileData $this   Instance of the current BP_XProfile_ProfileData class.
         */
        return apply_filters_ref_array('xprofile_data_is_valid_field', [
            (bool) $retval,
            $this,
        ]);
    }
    ...
    public function save()
    {
         ...
         * @param int $data_id      The field data ID.
         */
        $this->last_updated = apply_filters(
         ...
         * @param BP_XProfile_ProfileData $this Current instance of the profile data being saved.
         */
        do_action_ref_array('xprofile_data_before_save', [$this]);

        if ($this->is_valid_field()) {
            if ($this->exists() && strlen(trim($this->value))) {
                $result = $wpdb->query(
                    $wpdb->prepare(
                        "UPDATE {$bp->profile->table_name_data} SET value = %s, last_updated = %s WHERE user_id = %d AND field_id = %d",
                        $this->value,
                        $this->last_updated,
                        $this->user_id,
                        $this->field_id
                ...
                $result = $wpdb->query(
                    $wpdb->prepare(
                        "INSERT INTO {$bp->profile->table_name_data} (user_id, field_id, value, last_updated) VALUES (%d, %d, %s, %s)",
                        $this->user_id,
                        $this->field_id,
                        $this->value,
                        $this->last_updated
             ...
             * @param BP_XProfile_ProfileData $this Current instance of the profile data being saved.
             */
            do_action_ref_array('xprofile_data_after_save', [$this]);
    ...
    public function delete()
    {
         ...
         */
        do_action_ref_array('xprofile_data_before_delete', [$this]);

        $deleted = $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$bp->profile->table_name_data} WHERE field_id = %d AND user_id = %d",
                $this->field_id,
                $this->user_id
            )
        );
         ...
         * @param BP_XProfile_ProfileData $this Current instance of the profile data being deleted.
         */
        do_action_ref_array('xprofile_data_after_delete', [$this]);
    ...
    public static function get_data_for_user($user_id, $field_ids)
    {
        ...
        $uncached_field_ids = bp_xprofile_get_non_cached_field_ids(
            $user_id,
            $field_ids
        );
        ...
        if (!empty($uncached_field_ids)) {
            $bp = buddypress();
            $uncached_field_ids_sql = implode(
                ',',
                wp_parse_id_list($uncached_field_ids)
            );
            $uncached_data = $wpdb->get_results(
                $wpdb->prepare(
    ...
    public static function get_value_byfieldname($fields, $user_id = null)
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT d.value, f.name FROM {$bp->profile->table_name_data} d, {$bp->profile->table_name_fields} f WHERE d.field_id = f.id AND d.user_id = %d AND f.parent_id = 0 $field_sql",
            $user_id
        );
    ...
    public static function delete_data_for_user($user_id)
    {
        ...
        $field_ids = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT field_id FROM {$bp->profile->table_name_data} WHERE user_id = %d",
                $user_id
            )
        );
    ...
    public static function get_random($user_id, $exclude_fullname)
    {
        ...
        return $wpdb->get_results(
            $wpdb->prepare(
                "SELECT pf.type, pf.name, pd.value FROM {$bp->profile->table_name_data} pd INNER JOIN {$bp->profile->table_name_fields} pf ON pd.field_id = pf.id AND pd.user_id = %d {$exclude_sql} ORDER BY RAND() LIMIT 1",
                $user_id
            )
        );
    }
