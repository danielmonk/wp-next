...
class BP_XProfile_Meta_Query extends WP_Meta_Query
{
    ...
    public function parse_query_vars($qv)
    {
        ...
        if (
            isset($qv['object_type']) &&
            '' !== $qv['object_type'] &&
            (!is_array($qv['object_type']) || $qv['object_type'])
        ) {
    ...
    public function get_sql(
         ...
         * }
         */
        return apply_filters_ref_array('bp_xprofile_get_meta_sql', [
            $sql,
            $this->queries,
            $type,
            $primary_table,
    ...
    public function get_sql_for_clause(
        ...
        if (array_key_exists('object', $clause)) {
            ...
            if (in_array($meta_compare, ['IN', 'NOT IN'])) {
            ...
            switch ($meta_compare) {
                ...
                case 'NOT IN':
                    $meta_compare_string =
                        '(' .
                        substr(str_repeat(',%s', count($object_type)), 1) .
                        ')';
                    $object_where = $wpdb->prepare(
                        $meta_compare_string,
                        $object_type
                    );
                    break;
            ...
            if (!empty($object_where)) {
                $sql_chunks[
                    'where'
                ][] = "{$alias}.object_type {$meta_compare} {$object_where}";
            }
        }
