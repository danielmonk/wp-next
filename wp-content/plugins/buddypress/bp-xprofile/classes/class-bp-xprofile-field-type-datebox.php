...
class BP_XProfile_Field_Type_Datebox extends BP_XProfile_Field_Type
{
    ...
    do_action(bp_get_the_profile_field_errors_action()); ?>
				...
				<label for="<?php bp_the_profile_field_input_name(); ?>_day" class="xprofile-field-label"><?php esc_html_e('Day', 'buddypress'); ?></label>
         ...
         'user_id' => $user_id,
				...
				<label for="<?php bp_the_profile_field_input_name(); ?>_month" class="xprofile-field-label"><?php esc_html_e('Month', 'buddypress'); ?></label>
         ...
         'user_id' => $user_id,
				...
				<label for="<?php bp_the_profile_field_input_name(); ?>_year" class="xprofile-field-label"><?php esc_html_e('Year', 'buddypress'); ?></label>
     ...
     */
    public function edit_field_options_html(array $args = [])
    {
        $date = BP_XProfile_ProfileData::get_value_byid(
            $this->field_obj->id,
            $args['user_id']
        );
        $day = 0;
        ...
        if (!empty($_POST['field_' . $this->field_obj->id . '_month'])) {
            if (
                in_array(
                    $_POST['field_' . $this->field_obj->id . '_month'],
                    $eng_months
                )
            ) {
        ...
        switch ($args['type']) {
            case 'day':
                $html = sprintf(
                    ...
                    selected($day, 0, false),
                    /* translators: no option picked in select box */ __(
                        '----',
                        'buddypress'
                    )
                );
        ...
        foreach (array_keys($existing_settings) as $setting) {
            switch ($setting) {
                ...
                case 'range_relative_end':
                    ...
                    if (
                        isset($settings[$op_key]) &&
                        'past' === $settings[$op_key]
                    ) {
     ...
     */
    public function admin_new_field_html(
        BP_XProfile_Field $current_field,
        $control_type = ''
    ) {
...
<div id="<?php echo esc_attr(
    $type
); ?>" class="postbox bp-options-box" style="<?php echo esc_attr(
    $class
); ?> margin-top: 15px;">
					...
					<?php foreach ($this->get_date_formats() as $format): ?>
						<div class="bp-date-format-option">
							<label for="date-format-<?php echo esc_attr($format); ?>">
								<input type="radio" name="field-settings[date_format]" id="date-format-<?php echo esc_attr(
            $format
    ...
    $settings['date_format']
					...
					<div class="bp-date-format-option">
						<label for="date-format-elapsed">
							<input type="radio" name="field-settings[date_format]" id="date-format-elapsed" <?php checked(
           'elapsed',
           $settings['date_format']
       ); ?> value="elapsed" aria-describedby="date-format-elapsed-setting" />
							<span class="date-format-label" id="date-format-elapsed-setting"><?php esc_html_e(
           'Time elapsed',
           'buddypress'
       ); ?></span> <?php _e(
    '<code>4 years ago</code>, <code>4 years from now</code>',
    'buddypress'
); ?>
						</label>
					</div>
					...
					<div class="bp-date-format-option">
						<label for="date-format-custom">
							<input type="radio" name="field-settings[date_format]" id="date-format-custom" <?php checked(
           'custom',
           $settings['date_format']
       ); ?> value="custom" />
							<span class="date-format-label"><?php esc_html_e(
           'Custom:',
           'buddypress'
       ); ?></span>
						</label>
						<label for="date-format-custom-value" class="screen-reader-text"><?php esc_html_e(
          'Enter custom time format',
          'buddypress'
      ); ?></label>
						<input type="text" name="field-settings[date_format_custom]" id="date-format-custom-value" class="date-format-custom-value" value="<?php echo esc_attr(
          $settings['date_format_custom']
      ); ?>" aria-describedby="date-format-custom-example" /> <span class="screen-reader-text"><?php esc_html_e(
    'Example:',
    'buddypress'
); ?></span><span class="date-format-custom-example" id="date-format-custom-sample"><?php if (
    $settings['date_format_custom']
):
    echo esc_html(date_i18n($settings['date_format_custom']));
endif; ?></span><span class="spinner" id="date-format-custom-spinner" aria-hidden="true"></span>

						<p><a href="https://codex.wordpress.org/Formatting_Date_and_Time"><?php esc_html_e(
		...
		<tr>
					...
					<div class="bp-date-format-option">
						<div class="bp-date-range-type-label">
							<label for="range_type_absolute">
								<input type="radio" name="field-settings[range_type]" id="range_type_absolute" value="absolute" <?php checked(
            'absolute',
            $settings['range_type']
        ); ?> />
						...
						<div class="bp-date-range-type-values">
							<label for="field-settings[range_absolute_start]" aria-label="Year"><?php esc_html_e(
           'Start:',
           'buddypress'
       ); ?></label>
							<?php printf(
           '<input class="date-range-numeric" type="text" name="field-settings[range_absolute_start]" id="field-settings[range_absolute_start]" value="%s" />',
           esc_attr($settings['range_absolute_start'])
       ); ?>
							<label for="field-settings[range_absolute_end]" aria-label="Year"><?php esc_html_e(
           'End:',
           'buddypress'
       ); ?></label>
							<?php printf(
           '<input class="date-range-numeric" type="text" name="field-settings[range_absolute_end]" id="field-settings[range_absolute_end]" value="%s" />',
           esc_attr($settings['range_absolute_end'])
       ); ?>
						</div>
					</div>
					...
					<div class="bp-date-format-option">
						<div class="bp-date-range-type-label">
							<label for="range_type_relative">
								<input type="radio" name="field-settings[range_type]" id="range_type_relative" value="relative" <?php checked(
            'relative',
            $settings['range_type']
        ); ?> />
						...
						<div class="bp-date-range-type-values">
							<label for="field-settings[range_relative_start]"><?php esc_html_e(
           'Start:',
           'buddypress'
       ); ?></label>
							<?php printf(
           ...
           esc_attr(abs($settings['range_relative_start']))
							...
							<label class="screen-reader-text" for="field-settings[range_relative_start_type]"><?php esc_html_e(
           'Select range',
           'buddypress'
       ); ?></label>
							<?php printf(
           ...
           esc_attr__('years from now', 'buddypress')
							...
							<label for="field-settings[range_relative_end]"><?php esc_html_e(
           'End:',
           'buddypress'
       ); ?></label>
							<?php printf(
           ...
           esc_attr(abs($settings['range_relative_end']))
							...
							<label class="screen-reader-text" for="field-settings[range_relative_end_type]"><?php esc_html_e(
           'Select range',
           'buddypress'
       ); ?></label>
							<?php printf(
