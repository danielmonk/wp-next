...
class BP_XProfile_Group
{
    ...
    public function save()
    {
         ...
         */
        do_action_ref_array('xprofile_group_before_save', [&$this]);
        ...
        if (!empty($this->id)) {
            $sql = $wpdb->prepare(
                "UPDATE {$bp->profile->table_name_groups} SET name = %s, description = %s WHERE id = %d",
                $this->name,
                $this->description,
                $this->id
            );
            ...
            $sql = $wpdb->prepare(
                "INSERT INTO {$bp->profile->table_name_groups} (name, description, can_delete) VALUES (%s, %s, 1)",
                $this->name,
                $this->description
            );
        }
         ...
         * @param BP_XProfile_Group $this Current instance of the group being saved. Passed by reference.
         */
        do_action_ref_array('xprofile_group_after_save', [&$this]);
    ...
    public function delete()
    {
         ...
         */
        do_action_ref_array('xprofile_group_before_delete', [&$this]);
         ...
         * @param BP_XProfile_Group $this Current instance of the group being deleted. Passed by reference.
         */
        do_action_ref_array('xprofile_group_after_delete', [&$this]);
    ...
    public static function get($args = [])
    {
        ...
        $uncached_field_ids = bp_get_non_cached_ids(
            $field_ids,
            'bp_xprofile_fields'
        );
        if (!empty($uncached_field_ids)) {
            $_uncached_field_ids = implode(
                ',',
                array_map('intval', $uncached_field_ids)
            );
            $uncached_fields = $wpdb->get_results(
                "SELECT * FROM {$bp->profile->table_name_fields} WHERE id IN ({$_uncached_field_ids})"
            );
        ...
        if (!empty($r['fetch_field_data'])) {
            ...
            if (!empty($field_ids) && !empty($r['user_id'])) {
                $field_data = BP_XProfile_ProfileData::get_data_for_user(
                    $r['user_id'],
                    $field_ids
                );
            }
            ...
            if (
                ...
                foreach ((array) $field_data as $data) {
                    ...
                    if (
                        (!empty($maybe_value) || '0' == $maybe_value) &&
                        false !==
                            ($key = array_search($data->field_id, $field_ids))
                    ) {
     ...
     * @return array
     */
    public static function get_group_ids($args = [])
    {
        global $wpdb;
     ...
     * @return array
     */
    public static function get_group_field_ids($group_ids, $args = [])
    {
        global $wpdb;
        ...
        if (bp_get_member_types()) {
            if ($r['user_id'] || false !== $r['member_type']) {
                ...
                $member_types_fields = BP_XProfile_Field::get_fields_for_member_type(
                    $member_types
                );
        ...
        if (!empty($include_field_ids)) {
            $include_field_ids_cs = implode(
                ',',
                array_map('intval', $include_field_ids)
            );
    ...
    protected static function get_group_data($group_ids)
    {
        ...
        if (!empty($uncached_gids)) {
            ...
            $queried_gdata = $wpdb->get_results(
                "SELECT * FROM {$table_name} WHERE id IN ({$uncached_gids_sql}) ORDER BY FIELD( id, {$uncached_gids_sql} )"
            );
    ...
    public static function admin_validate()
    {
        ...
        if (empty($_POST['group_name'])) {
            $message = __(
                'Please make sure you give the group a name.',
                'buddypress'
            );
            return false;
        } else {
    ...
    public static function update_position($field_group_id, $position)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$bp->profile->table_name_groups} SET group_order = %d WHERE id = %d",
                $position,
                $field_group_id
            )
        );
     ...
     */
    public static function fetch_visibility_level($user_id = 0, $fields = [])
    {
        // Get the user's visibility level preferences.
        $visibility_levels = bp_get_user_meta(
            $user_id,
        ...
        foreach ((array) $fields as $key => $field) {
            ...
            $visibility = bp_xprofile_get_meta(
                $field->id,
                'field',
    ...
    public static function fetch_default_visibility_levels()
    {
        ...
        $default_visibility_levels = wp_cache_get(
            ...
            foreach ($levels as $level) {
                switch ($level->meta_key) {
                    case 'default_visibility':
                        $default_visibility_levels[$level->object_id][
                            'default'
                        ] = $level->meta_value;
                        break;
    ...
    public function render_admin_form()
    {
		...
		<div class="wrap">
			...
			<form id="bp-xprofile-add-field-group" action="<?php echo esc_url(
       $action
   ); ?>" method="post">
				<div id="poststuff">
     ...
     get_current_screen()->get_columns()
         ...
         : '2'; ?>">
						<div id="post-body-content">
							<div id="titlediv">
								<div class="titlewrap">
									<label id="title-prompt-text" for="title"><?php esc_html_e(
             'Field Group Name (required)',
             'buddypress'
         ); ?></label>
									<input type="text" name="group_name" id="title" value="<?php echo esc_attr(
             $this->name
         ); ?>" autocomplete="off" />
								</div>
							</div>
							<div class="postbox">
								...
								<div class="inside">
									<label for="group_description" class="screen-reader-text"><?php /* translators: accessibility text */
         esc_html_e('Add description', 'buddypress'); ?></label>
									<textarea name="group_description" id="group_description" rows="8" cols="60"><?php echo esc_textarea(
             $this->description
         ); ?></textarea>
								</div>
							</div>
       ...
       do_action('xprofile_group_before_submitbox', $this); ?>
           ...
           ?>
											...
											<input type="hidden" name="group_order" id="group_order" value="<?php echo esc_attr(
               $this->group_order
           ); ?>" />
											<div id="publishing-action">
												<input type="submit" name="save_group" value="<?php echo esc_attr(
                $button
