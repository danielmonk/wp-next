...
class BP_XProfile_Field
{
    ...
    public function delete($delete_data = false)
    {
         ...
         */
        do_action_ref_array('xprofile_field_before_delete', [
            &$this,
            $delete_data,
        ]);
        ...
        $bp = buddypress();
        $sql = $wpdb->prepare(
            "DELETE FROM {$bp->profile->table_name_fields} WHERE id = %d OR parent_id = %d",
            $this->id,
            $this->id
        );
         ...
         * @param bool              $delete_data Whether or not to delete data.
         */
        do_action_ref_array('xprofile_field_after_delete', [
            &$this,
            $delete_data,
        ]);
    ...
    public function save()
    {
        ...
        $this->is_required = apply_filters(
            'xprofile_field_is_required_before_save',
            $this->is_required,
            $this->id
        );
        $this->order_by = apply_filters(
        ...
        $this->option_order = apply_filters(
            'xprofile_field_option_order_before_save',
            $this->option_order,
            $this->id
        );
        $this->can_delete = apply_filters(
            'xprofile_field_can_delete_before_save',
            $this->can_delete,
            $this->id
        );
        $this->type_obj = bp_xprofile_create_field_type($this->type);
         ...
         */
        do_action_ref_array('xprofile_field_before_save', [$this]);

        $is_new_field = is_null($this->id);

        if (!$is_new_field) {
            $sql = $wpdb->prepare(
                "UPDATE {$bp->profile->table_name_fields} SET group_id = %d, parent_id = 0, type = %s, name = %s, description = %s, is_required = %d, order_by = %s, field_order = %d, option_order = %d, can_delete = %d, is_default_option = %d WHERE id = %d",
                $this->group_id,
                $this->type,
                $this->name,
                $this->description,
        ...
        if ($wpdb->query($sql) !== null) {
            ...
            if ($this->type_obj->supports_options) {
                ...
                $post_option = !empty($_POST["{$this->type}_option"])
                    ? $_POST["{$this->type}_option"]
                    : '';
                $post_default = !empty($_POST["isDefault_{$this->type}_option"])
                    ? $_POST["isDefault_{$this->type}_option"]
                    : '';
             ...
             * @param BP_XProfile_Field $this Current instance of the field being saved.
             */
            do_action_ref_array('xprofile_field_after_save', [$this]);
    ...
    public function delete_children()
    {
        ...
        $bp = buddypress();
        $sql = $wpdb->prepare(
            "DELETE FROM {$bp->profile->table_name_fields} WHERE parent_id = %d",
            $this->id
        );
    ...
    public function get_member_types()
    {
        ...
        $raw_types = bp_xprofile_get_meta(
            $this->id,
            'field',
            'member_type',
            false
        ...
        if (!in_array('_none', $raw_types)) {
            ...
            foreach ($raw_types as $raw_type) {
                ...
                if (
                    'null' === $raw_type ||
                    isset($registered_types[$raw_type])
                ) {
                    $types[] = $raw_type;
                }
            }
         ...
         */
        $this->member_types = apply_filters(
    ...
    public function set_member_types($member_types, $append = false)
    {
        ...
        foreach ($member_types as $member_type) {
            ...
            if (
                'null' === $member_type ||
                bp_get_member_type_object($member_type)
            ) {
                $types[] = $member_type;
            }
        }
    ...
    public function get_member_type_label()
    {
        ...
        if (!empty($member_types)) {
            ...
            $label = sprintf(
                __('(Member types: %s)', 'buddypress'),
                implode(', ', array_map('esc_html', $member_type_labels))
            );
        } else {
            $label =
                '<span class="member-type-none-notice">' .
                __('(Unavailable to all members)', 'buddypress') .
                '</span>';
        }
    ...
    public function get_default_visibility()
    {
        if (!isset($this->default_visibility)) {
            $this->default_visibility = bp_xprofile_get_meta(
                $this->id,
                'field',
                'default_visibility'
            );
    ...
    public function get_allow_custom_visibility()
    {
        if (!isset($this->allow_custom_visibility)) {
            $allow_custom_visibility = bp_xprofile_get_meta(
                $this->id,
                'field',
    ...
    public function get_do_autolink()
    {
        if (!isset($this->do_autolink)) {
            $do_autolink = bp_xprofile_get_meta(
                $this->id,
                'field',
                'do_autolink'
            );
    ...
    public static function delete_for_group($group_id = 0)
    {
        ...
        $bp = buddypress();
        $sql = $wpdb->prepare(
            "DELETE FROM {$bp->profile->table_name_fields} WHERE group_id = %d",
            $group_id
        );
    ...
    public static function get_id_from_name($field_name = '')
    {
        ...
        $id = bp_core_get_incremented_cache(
            $field_name,
            'bp_xprofile_fields_by_name'
        );
        if (false === $id) {
            $sql = $wpdb->prepare(
    ...
    public static function update_position(
        ...
        $sql = $wpdb->prepare(
            "UPDATE {$table_name} SET field_order = %d, group_id = %d WHERE id = %d",
            $position,
            $field_group_id,
            $field_id
        );
        ...
        if (!empty($parent) && !is_wp_error($parent)) {
            ...
            $sql = $wpdb->prepare(
                "UPDATE {$table_name} SET group_id = %d WHERE parent_id = %d",
                $field_group_id,
                $field_id
            );
            $wpdb->query($sql);
    ...
    public static function get_fields_for_member_type($member_types)
    {
        ...
        if (false === $mt_meta) {
            $mt_meta = $wpdb->get_results(
                "SELECT object_id, meta_value FROM {$bp->profile->table_name_meta} WHERE meta_key = 'member_type' AND object_type = 'field'"
            );
        ...
        if (!in_array('_none', $member_types)) {
            if (!empty($all_recorded_field_ids)) {
                $all_recorded_field_ids_sql = implode(
                    ',',
                    array_map('absint', $all_recorded_field_ids)
                );
                $unrestricted_field_ids = $wpdb->get_col(
                    "SELECT id FROM {$bp->profile->table_name_fields} WHERE id NOT IN ({$all_recorded_field_ids_sql})"
                );
            } else {
                $unrestricted_field_ids = $wpdb->get_col(
    ...
    public static function admin_validate()
    {
        ...
        if (!isset($_POST['title']) || '' === $_POST['title']) {
            $message = esc_html__(
                'Profile fields must have a name.',
                'buddypress'
            );
            return false;
        }
        ...
        if (!isset($_POST['required'])) {
            $message = esc_html__(
                'Profile field requirement is missing.',
                'buddypress'
            );
            return false;
        }
        ...
        if (
            !in_array(
                $_POST['fieldtype'],
                array_keys(bp_xprofile_get_field_types()),
                true
            )
        ) {
            ...
            $message = sprintf(
                esc_html__(
                    'The profile field type %s is not registered.',
                    'buddypress'
                ),
                '<code>' . esc_attr($_POST['fieldtype']) . '</code>'
            );
            return false;
        }
        ...
        if (true === $field_type->supports_options) {
            ...
            if (
                ...
                $message = esc_html__(
                    'These field options are invalid.',
                    'buddypress'
                );
                return false;
            }
            ...
            if (0 === $field_count) {
                /* translators: %s: field type name */
                $message = sprintf(
                    esc_html__('%s require at least one option.', 'buddypress'),
                    $field_type->name
                );
                return false;
            }
    ...
    public function render_admin_form($message = '')
    {
		...
		<div class="wrap">
			...
			<form id="bp-xprofile-add-field" action="<?php echo esc_url(
       $action
   ); ?>" method="post">
				<div id="poststuff">
     ...
     get_current_screen()->get_columns()
         ...
         : '2'; ?>">
						<div id="post-body-content">
							...
							<?php // Output the name & description fields.
    ...
    private function submit_metabox($button_text = '')
    {
      ...
      do_action('xprofile_field_submitbox_start', $this); ?>
						...
						<input type="hidden" name="field_order" id="field_order" value="<?php echo esc_attr(
          $this->field_order
      ); ?>" />
						...
						<?php if (!empty($button_text)): ?>
							...
							<div id="publishing-action">
								<input type="submit" name="saveField" value="<?php echo esc_attr(
            $button_text
            ...
            $delete_url
        ); ?>" class="submitdelete deletion"><?php esc_html_e(
    'Delete',
    'buddypress'
); ?></a>
							<?php endif; ?>
           ...
           $cancel_url
       ); ?>" class="deletion"><?php esc_html_e('Cancel', 'buddypress'); ?></a></div>
						</div>
  ...
  do_action('xprofile_field_after_submitbox', $this);
    ...
    private function name_and_description()
    {
		...
		<div id="titlediv">
			<div class="titlewrap">
				<label id="title-prompt-text" for="title"><?php echo esc_html_x(
        'Name (required)',
        'XProfile admin edit field',
        'buddypress'
    ); ?></label>
				<input type="text" name="title" id="title" value="<?php echo esc_attr(
        $this->name
    ); ?>" autocomplete="off" />
			</div>
		</div>
		...
		<div class="postbox">
			<h2><?php echo esc_html_x(
       'Description',
       'XProfile admin edit field',
       'buddypress'
   ); ?></h2>
			<div class="inside">
				<label for="description" class="screen-reader-text"><?php /* translators: accessibility text */
    esc_html_e('Add description', 'buddypress'); ?></label>
				<textarea name="description" id="description" rows="8" cols="60"><?php echo esc_textarea(
        $this->description
    ); ?></textarea>
			</div>
		</div>
    ...
    private function member_type_metabox()
    {
        ...
        if (!($member_types = bp_get_member_types([], 'objects'))) {
            return;
        }
				...
				<ul>
					...
					<li>
						<label for="member-type-<?php echo $member_type->labels['name']; ?>">
							<input name="member-types[]" id="member-type-<?php echo $member_type->labels[
           'name'
       ]; ?>" class="member-type-selector" type="checkbox" value="<?php echo $member_type->name; ?>" <?php checked(
    in_array($member_type->name, $field_member_types)
); ?>/>
					...
					<li>
						<label for="member-type-none">
							<input name="member-types[]" id="member-type-none" class="member-type-selector" type="checkbox" value="null" <?php checked(
           in_array('null', $field_member_types)
       ); ?>/>
				...
				<p class="description member-type-none-notice<?php if (
        !empty($field_member_types)
    ): ?> hide<?php endif; ?>"><?php _e(
    'Unavailable to all members.',
    'buddypress'
); ?></p>
			</div>
    ...
    private function visibility_metabox()
    {
		...
		<div class="postbox">
			<h2><label for="default-visibility"><?php esc_html_e(
       'Visibility',
       'buddypress'
   ); ?></label></h2>
			<div class="inside">
				<div>
					<select name="default-visibility" id="default-visibility">
						...
						<?php foreach (bp_xprofile_get_visibility_levels() as $level): ?>
							...
							<option value="<?php echo esc_attr($level['id']); ?>" <?php selected(
    $this->get_default_visibility(),
    $level['id']
); ?>>
				...
				<div>
					<ul>
						<li>
							<input type="radio" id="allow-custom-visibility-allowed" name="allow-custom-visibility" value="allowed" <?php checked(
           $this->get_allow_custom_visibility(),
           'allowed'
       ); ?> />
							<label for="allow-custom-visibility-allowed"><?php esc_html_e(
           'Allow members to override',
           'buddypress'
       ); ?></label>
						</li>
						<li>
							<input type="radio" id="allow-custom-visibility-disabled" name="allow-custom-visibility" value="disabled" <?php checked(
           $this->get_allow_custom_visibility(),
           'disabled'
       ); ?> />
							<label for="allow-custom-visibility-disabled"><?php esc_html_e(
    ...
    private function required_metabox()
    {
		...
		<div class="postbox">
			<h2><label for="required"><?php esc_html_e(
       'Requirement',
       'buddypress'
   ); ?></label></h2>
			<div class="inside">
				<select name="required" id="required">
					<option value="0"<?php selected($this->is_required, '0'); ?>><?php esc_html_e(
    'Not Required',
    'buddypress'
); ?></option>
					<option value="1"<?php selected($this->is_required, '1'); ?>><?php esc_html_e(
    'Required',
    'buddypress'
); ?></option>
				</select>
    ...
    private function autolink_metabox()
    {
        ...
        'On user profiles, link this field to a search of the Members directory, using the field value as a search term.',
        'buddypress'
    ); ?></p>

				<p>
					<label for="do-autolink" class="screen-reader-text"><?php /* translators: accessibility text */
     esc_html_e('Autolink status for this field', 'buddypress'); ?></label>
					<select name="do_autolink" id="do-autolink">
						<option value="on" <?php selected(
          $this->get_do_autolink()
      ); ?>><?php esc_html_e('Enabled', 'buddypress'); ?></option>
						<option value="" <?php selected(
          $this->get_do_autolink(),
          false
      ); ?>><?php esc_html_e('Disabled', 'buddypress'); ?></option>
					</select>
				</p>
			</div>
		</div>
    ...
    private function type_metabox()
    {
		...
		<div class="postbox">
			<h2><label for="fieldtype"><?php esc_html_e(
       'Type',
       'buddypress'
   ); ?></label></h2>
