...
class BP_XProfile_Query
{
    ...
    public function get_sql_for_clause(&$clause, $parent_query)
    {
        ...
        if (false === $alias) {
            ...
            if ('NOT EXISTS' === $field_compare) {
                ...
                $join .= $i ? " AS $alias" : '';
                $join .= $wpdb->prepare(
                    " ON ($this->primary_table.$this->primary_id_column = $alias.user_id AND $alias.field_id = %d )",
                    $clause['field']
                );
        ...
        if (array_key_exists('field', $clause)) {
            ...
            if (!is_numeric($clause['field'])) {
                $clause['field'] = xprofile_get_field_id_from_name(
                    $clause['field']
                );
            }
        ...
        if (array_key_exists('value', $clause)) {
            ...
            $field_type = $this->get_cast_for_type(
                isset($clause['type']) ? $clause['type'] : ''
            );

            if (
                in_array($field_compare, [
                    'IN',
                    'NOT IN',
                    'BETWEEN',
                    'NOT BETWEEN',
            ...
            switch ($field_compare) {
                case 'IN':
                case 'NOT IN':
                    $field_compare_string =
                        '(' .
                        substr(str_repeat(',%s', count($field_value)), 1) .
                        ')';
                    $where = $wpdb->prepare(
                        $field_compare_string,
                        $field_value
            ...
            if ($where) {
                $sql_chunks[
                    'where'
                ][] = "CAST($alias.value AS {$field_type}) {$field_compare} {$where}";
            }
        }
    ...
    protected function find_compatible_table_alias($clause, $parent_query)
    {
        ...
        foreach ($parent_query as $sibling) {
            ...
            if (
                in_array($clause_compare, $compatible_compares) &&
                in_array($sibling_compare, $compatible_compares)
            ) {
                $alias = $sibling['alias'];
                break;
            }
