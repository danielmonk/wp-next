...
class BP_REST_XProfile_Fields_Endpoint extends WP_REST_Controller
{
            ...
            [
                ...
                [
                    ...
                    'args' => [
                        'user_id' => [
                            'description' => __(
                                'Required if you want to load a specific user\'s data.',
                                'buddypress'
                            ),
                            'default' => 0,
                            'type' => 'integer',
                        ...
                        'fetch_field_data' => [
                            'description' => __(
                                'Whether to fetch data for the field. Requires a $user_id.',
                                'buddypress'
                            ),
                            'default' => false,
                            'type' => 'boolean',
                ...
                [
                    ...
                    'args' => [
                        'delete_data' => [
                            'description' => __(
                                'Required if you want to delete users data for the field.',
                                'buddypress'
                            ),
                            'default' => false,
                            'type' => 'boolean',
    ...
    public function get_item($request)
    {
        ...
        if (!empty($request->get_param('user_id'))) {
            ...
            if (in_array($field->id, $hidden_user_fields, true)) {
                ...
                $field->data->value = BP_XProfile_ProfileData::get_value_byid(
                    $field->id,
                    $request->get_param('user_id')
                );
            }
     ...
     * @param WP_REST_Request $request  The request sent to the API.
     */
    public function set_additional_field_properties(
        $field_id,
        ...
        if (isset($schema['allow_custom_visibility'])) {
            $allow_custom_visibility =
                $schema['allow_custom_visibility']['default'];
    ...
    public function create_item_permissions_check($request)
    {
        $retval = new WP_Error(
            ...
            __(
                'Sorry, you are not allowed to create a XProfile field.',
                'buddypress'
            ),
            [
    ...
    public function update_item($request)
    {
        ...
        $args = [
            'field_id' => $field->id,
            'field_group_id' => is_null($request['group_id'])
                ? $field->group_id
                : $request['group_id'],
            'parent_id' => is_null($request['parent_id'])
                ? $field->parent_id
                : $request['parent_id'],
            ...
            'description' => is_null($request['description'])
                ? $field->description
                : $request['description'],
            'is_required' => is_null($request['required'])
                ? $field->is_required
                : $request['required'],
            'can_delete' => $request['can_delete'], // Set to true by default.
            'order_by' => is_null($request['order_by'])
                ? $field->order_by
                : $request['order_by'],
            'is_default_option' => is_null($request['is_default_option'])
                ? $field->is_default_option
                : $request['is_default_option'],
            'option_order' => is_null($request['option_order'])
                ? $field->option_order
                : $request['option_order'],
            'field_order' => is_null($request['field_order'])
                ? $field->field_order
                : $request['field_order'],
        ];
    ...
    public function delete_item_permissions_check($request)
    {
        $retval = new WP_Error(
            'bp_rest_authorization_required',
            __(
                'Sorry, you are not allowed to delete this field.',
                'buddypress'
            ),
            [
                'status' => rest_authorization_required_code(),
    ...
    public function assemble_response_data($field, $request)
    {
        ...
        if (
            0 === $data['parent_id'] &&
            true ===
                wp_validate_boolean($request->get_param('fetch_field_data')) &&
            ...
            $data['data']['value'] = [
                ...
                'unserialized' => $this->get_profile_field_unserialized_value(
                    $field->data->value
                ),
                'rendered' => $this->get_profile_field_rendered_value(
                    $field->data->value,
                    $field
                ),
            ];
        ...
        if (method_exists($field, 'get_children')) {
            $data['options'] = array_map(function ($item) use ($request) {
     ...
     */
    public function get_profile_field_rendered_value(
        $value = '',
        $profile_field = null
    ) {
        if (empty($value)) {
    ...
    public function get_endpoint_args_for_item_schema(
        ...
        if (
            ...
            $edit_args['default_visibility']['description'] = __(
                'Default visibility for the profile field.',
                'buddypress'
            );
            ...
            // Add specific properties to the edit context.
            $edit_args['allow_custom_visibility'] = [
                'context' => ['edit'],
                'description' => __(
            ...
            $edit_args['do_autolink'] = [
                'context' => ['edit'],
                'description' => __(
                    'Autolink status for this profile field',
                    'buddypress'
                ),
                'default' => 'off',
                'type' => 'string',
        ...
        $schema = [
            ...
            'properties' => [
                ...
                'group_id' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'The ID of the group the field is part of.',
                        'buddypress'
                    ),
                    'type' => 'integer',
                ],
                ...
                'type' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'The type for the profile field.',
                        'buddypress'
                    ),
                    'type' => 'string',
                ...
                'description' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'The description of the profile field.',
                        'buddypress'
                    ),
                    'type' => 'object',
                    'arg_options' => [
                    ...
                    'properties' => [
                        'raw' => [
                            'description' => __(
                                'Content for the profile field, as it exists in the database.',
                                'buddypress'
                            ),
                            'type' => 'string',
                            'context' => ['edit'],
                        ...
                        'rendered' => [
                            'description' => __(
                                'HTML content for the profile field, transformed for display.',
                                'buddypress'
                            ),
                            'type' => 'string',
                            'context' => ['view', 'edit'],
                ...
                'is_required' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'Whether the profile field must have a value.',
                        'buddypress'
                    ),
                    'type' => 'boolean',
                ],
                ...
                'field_order' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'The order of the profile field into the group of fields.',
                        'buddypress'
                    ),
                    'type' => 'integer',
                ],
                'option_order' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'The order of the option into the profile field list of options',
                        'buddypress'
                    ),
                    'type' => 'integer',
                ],
                'order_by' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'The way profile field\'s options are ordered.',
                        'buddypress'
                    ),
                    'default' => 'asc',
                    'type' => 'string',
                ...
                'data' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'The saved value for this profile field.',
                        'buddypress'
                    ),
                    'type' => 'object',
                    'readonly' => true,
    ...
    public function get_collection_params()
    {
        ...
        $params['hide_empty_groups'] = [
            'description' => __(
                'Whether to hide profile groups of fields that do not have any profile fields or not.',
                'buddypress'
            ),
            'default' => false,
            'type' => 'boolean',
        ...
        $params['fetch_visibility_level'] = [
            'description' => __(
                'Whether to fetch the visibility level for each field.',
                'buddypress'
            ),
            'default' => false,
            'type' => 'boolean',
        ...
        $params['exclude_groups'] = [
            'description' => __(
                'Ensure result set excludes specific profile field groups.',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['exclude_fields'] = [
            'description' => __(
                'Ensure result set excludes specific profile fields.',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
