...
abstract class BP_XProfile_Field_Type
{
     ...
     */
    protected $validation_allowed_values = [];
    ...
    public function set_allowed_values($values)
    {
        foreach ((array) $values as $value) {
             ...
             * @param BP_XProfile_Field_Type $this   Current instance of the BP_XProfile_Field_Type class.
             */
            $this->validation_allowed_values[] = apply_filters_deprecated(
             ...
             * @param BP_XProfile_Field_Type $this   Current instance of the BP_XProfile_Field_Type class.
             */
            $this->validation_allowed_values[] = apply_filters(
                'bp_xprofile_field_type_set_allowed_values',
                $value,
                $values,
                $this
    ...
    public function is_valid($values)
    {
        ...
        if (
            true === $validated &&
            !empty($values) &&
            !empty($this->validation_allowed_values)
        ) {
            foreach ((array) $values as $value) {
     ...
     * @return void
     */
    abstract public function edit_field_html(array $raw_properties = []);
     ...
     * @return void
     */
    abstract public function admin_field_html(array $raw_properties = []);
    ...
    $class
       ...
       'buddypress'
			...
			<div class="inside" aria-live="polite" aria-atomic="true" aria-relevant="all">
				<p>
					<label for="sort_order_<?php echo esc_attr($type); ?>"><?php esc_html_e(
    'Sort Order:',
    'buddypress'
); ?></label>
					<select name="sort_order_<?php echo esc_attr(
         $type
     ); ?>" id="sort_order_<?php echo esc_attr($type); ?>" >
						<option value="custom" <?php selected(
          'custom',
          $current_field->order_by
      ); ?>><?php esc_html_e('Custom', 'buddypress'); ?></option>
						<option value="asc"    <?php selected(
          'asc',
          $current_field->order_by
      ); ?>><?php esc_html_e('Ascending', 'buddypress'); ?></option>
						<option value="desc"   <?php selected(
          'desc',
          $current_field->order_by
      ); ?>><?php esc_html_e('Descending', 'buddypress'); ?></option>
					</select>
				</p>
    ...
    if (empty($options)) {
        ...
        while (isset($_POST[$type . '_option'][$i])) {
            ...
            if (
                $current_type_obj->supports_options &&
                ...
                $is_default_option =
                    (bool) $_POST["isDefault_{$type}_option"][$i];
            } else {
            ...
            $options[] = (object) [
                ...
                'name' => sanitize_text_field(
                    stripslashes($_POST[$type . '_option'][$i])
                ),
            ];
...
esc_html_e('Add an option', 'buddypress'); ?></label>
							<input type="text" name="<?php echo esc_attr(
           "{$type}_option[{$j}]"
       ); ?>" id="<?php echo esc_attr(
    "{$type}_option{$j}"
); ?>" value="<?php echo esc_attr(stripslashes($options[$i]->name)); ?>" />
							<label for="<?php echo esc_attr("{$type}_option{$default_name}"); ?>">
								<input type="<?php echo esc_attr($control_type); ?>" id="<?php echo esc_attr(
    "{$type}_option{$default_name}"
); ?>" name="<?php echo esc_attr(
    "isDefault_{$type}_option{$default_name}"
); ?>" <?php checked(
    $options[$i]->is_default_option,
    true
); ?> value="<?php echo esc_attr($j); ?>" />
								<?php _e('Default Value', 'buddypress'); ?>
							</label>

							<?php if (1 !== $j): ?>
								<div class ="delete-button">
									<a href='javascript:hide("<?php echo esc_attr(
             "{$type}_div{$j}"
         ); ?>")' class="delete"><?php esc_html_e(
    'Delete',
    'buddypress'
); ?></a>
								</div>
    ...
    "{$type}_option_number"
); ?>" value="<?php echo esc_attr($j + 1); ?>" />
				<?php
    }
    ?>

				<div id="<?php echo esc_attr("{$type}_more"); ?>"></div>
				<p><a href="javascript:add_option('<?php echo esc_js(
        $type
    ); ?>')"><?php esc_html_e('Add Another Option', 'buddypress'); ?></a></p>
     ...
     * @return string
     */
    protected function get_edit_field_html_elements(array $properties = [])
    {
        $r = bp_parse_args($properties, [
         ...
         */
        $r = (array) apply_filters(
            'bp_xprofile_field_edit_html_elements',
            $r,
            get_class($this)
        );

        return bp_get_form_field_attributes(
            sanitize_key(bp_get_the_profile_field_name()),
            $r
        );
    }
}
