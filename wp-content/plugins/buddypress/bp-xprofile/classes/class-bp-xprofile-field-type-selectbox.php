...
class BP_XProfile_Field_Type_Selectbox extends BP_XProfile_Field_Type
{
  ...
  do_action(bp_get_the_profile_field_errors_action()); ?>
    ...
    public function edit_field_options_html(array $args = [])
    {
        ...
        $html =
            '<option value="">' .
            /* translators: no option picked in select box */ esc_html__(
                '----',
                'buddypress'
            ) .
            '</option>';
        ...
        for ($k = 0, $count = count($options); $k < $count; ++$k) {
            ...
            foreach ($option_values as $i => $option_value) {
                if (
                    ...
                    $_POST['field_' . $this->field_obj->id] != $option_value
                ) {
                    if (!empty($_POST['field_' . $this->field_obj->id])) {
                        $option_values[$i] = sanitize_text_field(
                            $_POST['field_' . $this->field_obj->id]
                        );
                    }
                }
