...
if (!class_exists('BP_XProfile_User_Admin')):
    ...
    class BP_XProfile_User_Admin
    {
        ...
        public function register_metaboxes(
            ...
            if (
                ...
                bp_has_profile($profile_args)
            ) {
                // Loop through field groups and add a metabox for each one.
                while (bp_profile_groups()):
                    bp_the_profile_group();
                    add_meta_box(
                        'bp_xprofile_user_admin_fields_' .
                            sanitize_key(bp_get_the_profile_group_slug()),
                        esc_html(bp_get_the_profile_group_name()),
                        [$this, 'user_admin_profile_metaboxes'],
                        $screen_id,
                        'normal',
                        'core',
                ...
                add_meta_box(
                    ...
                    _x(
                        ...
                        'xprofile user-admin edit screen',
                        'buddypress'
                    ),
                    [$this, 'user_admin_spammer_metabox'],
                    $screen_id,
                    'normal',
                    'core'
                );
            ...
            if (isset($_POST['field_ids'])) {
                ...
                foreach ((array) $posted_field_ids as $field_id) {
                    ...
                    $old_values[$field_id] = [
                        ...
                        'visibility' => xprofile_get_field_visibility_level(
                            $field_id,
                            $user_id
                        ),
                    ];
         ...
         */
        public function user_admin_profile_metaboxes($user = null, $args = [])
        {
            // Bail if no user ID.
            if (empty($user->ID)) {
                return;
			...
			<input type="hidden" name="field_ids[]" id="<?php echo esc_attr(
       'field_ids_' . bp_get_the_profile_group_slug()
    ...
    bp_get_the_profile_group_field_ids()
     ...
     $field_type->edit_field_html(['user_id' => $r['user_id']]);
     ...
     $can_change_visibility = bp_current_user_can(
         'bp_xprofile_change_field_visibility'
     );
     ?>

					<p class="field-visibility-settings-<?php echo $can_change_visibility
         ? 'toggle'
         : 'notoggle'; ?>" id="field-visibility-settings-toggle-<?php bp_the_profile_field_id(); ?>"><span id="<?php bp_the_profile_field_input_name(); ?>-2">
          ...
          '<span class="current-visibility-level">' .
              ...
              '</span>'
						...
						<?php if ($can_change_visibility): ?>
							...
							<button type="button" class="button visibility-toggle-link" aria-describedby="<?php bp_the_profile_field_input_name(); ?>-2" aria-expanded="false"><?php esc_html_e(
    'Change',
    'buddypress'
); ?></button>
					...
					<?php if ($can_change_visibility): ?>
						...
						<div class="field-visibility-settings" id="field-visibility-settings-<?php bp_the_profile_field_id(); ?>">
							...
							<button type="button" class="button field-visibility-settings-close"><?php esc_html_e(
           'Close',
           'buddypress'
       ); ?></button>
						</div>
