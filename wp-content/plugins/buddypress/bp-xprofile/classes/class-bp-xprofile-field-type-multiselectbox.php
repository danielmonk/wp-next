...
class BP_XProfile_Field_Type_Multiselectbox extends BP_XProfile_Field_Type
{
    ...
    public function edit_field_html(array $raw_properties = [])
    {
		...
		<?php /** This action is documented in bp-xprofile/bp-xprofile-classes */
  do_action(bp_get_the_profile_field_errors_action()); ?>

		<select <?php echo $this->get_edit_field_html_elements(
      $r
       ...
       'user_id' => $user_id,
		...
		<?php if (!bp_get_the_profile_field_is_required()): ?>
			...
			<a class="clear-value" href="javascript:clear( '<?php echo esc_js(
    ...
    public function edit_field_options_html(array $args = [])
    {
        ...
        if (
            ...
            !empty($_POST['field_' . $this->field_obj->id])
        ) {
            $original_option_values = sanitize_text_field(
                $_POST['field_' . $this->field_obj->id]
            );
        }

        $option_values = $original_option_values
            ? (array) $original_option_values
            : [];
        for ($k = 0, $count = count($options); $k < $count; ++$k) {
            ...
            foreach ($option_values as $i => $option_value) {
                if (
                    isset($_POST['field_' . $this->field_obj->id]) &&
                    $_POST['field_' . $this->field_obj->id][$i] != $option_value
                ) {
                    if (!empty($_POST['field_' . $this->field_obj->id][$i])) {
                        $option_values[] = sanitize_text_field(
                            $_POST['field_' . $this->field_obj->id][$i]
                        );
                    }
                }
            }
            ...
            $allowed_options = xprofile_sanitize_data_value_before_save(
                $options[$k]->name,
                false,
                false
            );
            ...
            if (
                ...
                empty($option_values) &&
                !empty($options[$k]->is_default_option)
            ) {
                $selected = ' selected="selected"';
            }
    ...
    public function admin_field_html(array $raw_properties = [])
    {
		...
		<label for="<?php bp_the_profile_field_input_name(); ?>" class="screen-reader-text"><?php /* translators: accessibility text */
esc_html_e('Select', 'buddypress'); ?></label>
