 ...
 */
function bp_settings_sanitize_notification_settings($settings = [])
{
    $sanitized_settings = [];
...
function bp_settings_personal_data_exporter($email_address, $page)
{
    ...
    if (bp_is_active('friends')) {
        $notification_settings[] = [
            'name' => __(
                'Receive email when a member invites you to join a group?',
                'buddypress'
            ),
    ...
    if (bp_is_active('groups')) {
        $notification_settings[] = [
            'name' => __(
                'Receive email when group information is updated?',
                'buddypress'
            ),
        ...
        $notification_settings[] = [
            'name' => __(
                'Receive email when you are promoted to a group administrator or moderator?',
                'buddypress'
            ),
        ...
        $notification_settings[] = [
            'name' => __(
                'Receive email when a member requests to join a private group for which you are an admin?',
                'buddypress'
            ),
        ...
        $notification_settings[] = [
            'name' => __(
                'Receive email when your request to join a group has been approved or denied?',
                'buddypress'
            ),
    ...
    foreach ($notification_settings as $notification_setting) {
        $user_notification_setting = bp_get_user_meta(
            $user->ID,
    ...
    if (function_exists('bp_nouveau_groups_get_group_invites_setting')) {
        $user_settings[] = [
            ...
            'value' => bp_nouveau_groups_get_group_invites_setting()
                ? $yes
                : $no,
        ];
    }
...
function bp_settings_get_personal_data_request($user_id = 0)
{
    ...
    if (!empty($query->post)) {
        ...
        $user_request = bp_is_running_wp('4.9.6')
            ? 'wp_get_user_request'
 ...
 */
function bp_settings_get_personal_data_confirmation_date(
...
function bp_settings_data_exporter_items()
{
    ...
    $custom_friendly_names = apply_filters(
        ...
        [
            'wordpress-comments' => _x(
                'Comments',
                'WP Comments data exporter friendly name',
                'buddypress'
            ),
            'wordpress-media' => _x(
                'Media',
                'WP Media data exporter friendly name',
                'buddypress'
            ),
            'wordpress-user' => _x(
	...
	<?php foreach ($exporters as $exporter => $data):
      ...
      */
     $item = apply_filters(
         'bp_settings_data_exporter_name',
         esc_html($friendly_name),
         $exporter
     );
     ?>
