...
function bp_settings_action_general()
{
    ...
    $feedback = []; // array of strings for feedback.
    ...
    if (
        ...
        if (!empty($_POST['email'])) {
            ...
            if ($old_user_email != $user_email) {
                ...
                if (true !== $email_checks) {
                    ...
                    if (
                        isset($email_checks['domain_banned']) ||
                        isset($email_checks['domain_not_allowed'])
                    ) {
                        $email_error = 'blocked';
                    }
                ...
                if (false === $email_error) {
                    ...
                    $args = [
                        'tokens' => [
                            'displayname' => bp_core_get_user_displayname(
                                bp_displayed_user_id()
                            ),
        ...
        if (!empty($_POST['pass1']) && !empty($_POST['pass2'])) {
            ...
            $pass_error = bp_members_validate_user_password(
                $pass,
                $pass_confirm,
                $update_user
            );
            ...
            $pass_error = new WP_Error(
                'empty_user_password',
                __('One of the password fields was empty.', 'buddypress')
            );
        }
        ...
        $pass_error = new WP_Error(
            ...
            __('Your current password is invalid.', 'buddypress')
        );
    }
    ...
    switch ($email_error) {
        case 'invalid':
            $feedback['email_invalid'] = __(
                'That email address is invalid. Check the formatting and try again.',
                'buddypress'
            );
            break;
        case 'blocked':
            $feedback['email_blocked'] = __(
                'That email address is currently unavailable for use.',
                'buddypress'
            );
            break;
        case 'taken':
            $feedback['email_taken'] = __(
                'That email address is already taken.',
                'buddypress'
            );
            break;
        case 'empty':
            $feedback['email_empty'] = __(
                'Email address cannot be empty.',
                'buddypress'
            );
            break;
    ...
    if (is_wp_error($pass_error) && $pass_error->get_error_message()) {
        $feedback[
            $pass_error->get_error_code()
        ] = $pass_error->get_error_message();
    }
    ...
    if (
        ...
        (false === $email_error || false === $pass_error) &&
        (false === $pass_changed || false === $email_changed)
    ) {
        if (bp_is_my_profile()) {
            $feedback['nochange'] = __(
...
function bp_settings_verify_email_change()
{
    ...
    if (isset($_GET['verify_email_change'])) {
        $pending_email = bp_get_user_meta(
            bp_displayed_user_id(),
            'pending_email_change',
            true
        );
        ...
        if (
            !hash_equals(
                urldecode($_GET['verify_email_change']),
                $pending_email['hash']
            )
        ) {
            return;
        ...
        if ($email_changed) {
            ...
            bp_core_add_message(
                __(
                    'You have successfully verified your new email address.',
                    'buddypress'
                )
            );
        } else {
        ...
        if ($nonce_check) {
            ...
            bp_core_add_message(
                __(
                    'You have successfully dismissed your pending email change.',
                    'buddypress'
                )
            );
        }
