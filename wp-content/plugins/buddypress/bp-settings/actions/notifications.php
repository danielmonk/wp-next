...
function bp_settings_action_notifications()
{
    ...
    bp_settings_update_notification_settings(
        ...
        (array) $_POST['notifications']
    );

    // Switch feedback for super admins.
    if (bp_is_my_profile()) {
        bp_core_add_message(
            __('Your notification settings have been saved.', 'buddypress'),
            'success'
        );
    } else {
        bp_core_add_message(
            __(
                "This user's notification settings have been saved.",
                'buddypress'
            ),
            'success'
        );
    ...
    bp_core_redirect(
        bp_displayed_user_domain() . bp_get_settings_slug() . '/notifications/'
    );
}
