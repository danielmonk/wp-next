...
class BuddyPress
{
    ...
    private function constants()
    {
        ...
        if (!defined('BP_ROOT_BLOG')) {
            ...
            if (is_multisite()) {
                ...
                if (
                    !defined('BP_ENABLE_MULTIBLOG') ||
                    (defined('BP_ENABLE_MULTIBLOG') &&
                        (int) constant('BP_ENABLE_MULTIBLOG') === 0)
                ) {
                    ...
                    // Basename.
                    $basename =
                        basename(constant('BP_PLUGIN_DIR')) . '/bp-loader.php';
    ...
    private function setup_globals()
    {
         ...
         */
        $this->root_blog_id = (int) apply_filters(
            'bp_get_root_blog_id',
            BP_ROOT_BLOG
        );
        ...
        $this->plugin_dir = trailingslashit(
            constant('BP_PLUGIN_DIR') . constant('BP_SOURCE_SUBDIRECTORY')
        );
        $this->plugin_url = trailingslashit(
            constant('BP_PLUGIN_URL') . constant('BP_SOURCE_SUBDIRECTORY')
        );
         ...
         */
        $this->email_post_type = apply_filters(
            'bp_email_post_type',
            'bp-email'
        );
         ...
         */
        $this->email_taxonomy_type = apply_filters(
            'bp_email_tax_type',
            'bp-email-type'
        );
    }
    ...
    public function autoload($class)
    {
        ...
        if ('bp-rest-attachments' === $class) {
            $path =
                dirname(__FILE__) .
                "/bp-{$component}/classes/trait-attachments.php";
        } else {
            $path =
                dirname(__FILE__) .
                "/bp-{$component}/classes/class-{$class}.php";
        }
         ...
         */
        if (
            !in_array($component, ['core', 'members'], true) &&
    ...
    private function setup_actions()
    {
        ...
        foreach ($actions as $class_action) {
            if (method_exists($this, $class_action)) {
                add_action('bp_' . $class_action, [$this, $class_action], 5);
            }
        }
         ...
         */
        do_action_ref_array('bp_after_setup_actions', [&$this]);
    }
    ...
    private function versions()
    {
        ...
        $versions = [];
        $versions['1.6-single'] = get_blog_option(
            $this->root_blog_id,
            '_bp_db_version'
        );
        ...
        if (!empty($versions['1.6-single'])) {
            ...
            $versions['1.5-single'] = get_blog_option(
                $this->root_blog_id,
                'bp-db-version'
            );
            ...
            $this->db_version_raw = (int) !empty($versions)
                ? (int) max($versions)
                : 0;
        }
    }
