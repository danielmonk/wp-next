...
{
    return sprintf(
        '<a href="%1$s" onclick="top.location.href=\'%1$s\'"',
        $matches[1]
    );
}
...
function bp_activity_get_embed_excerpt($content = '')
{
    if (
        empty($content) &&
        !empty($GLOBALS['activities_template']->in_the_loop)
    ) {
    ...
    $content = bp_activity_truncate_entry($content, [
        'html' => false,
    ...
    return apply_filters(
        ...
        $content,
        $GLOBALS['activities_template']->activity->content
    );
}
...
function bp_activity_embed_media()
{
    ...
    if (
        isset($_GET['hide_media']) &&
        true == wp_validate_boolean($_GET['hide_media'])
    ) {
        /**
    ...
    $media = bp_core_extract_media_from_content(
        $GLOBALS['activities_template']->activity->content,
        'embeds'
    );
    ...
    if (isset($media['embeds']) && true === $allow_media) {
        ...
        if ('' === $oembed) {
            ...
            $oembed = $o->fetch(
                $o->get_provider($url, $oembed_args),
                $url,
                $oembed_args
            );
...
EOD;
                ...
                $play_icon = sprintf(
                    '<a rel="nofollow" class="play-btn" href="%1$s" onclick="top.location.href=\'%1$s\'">%2$s</a>',
                    esc_url($url),
                    $play_icon
                );
            }
            ...
            $thumb_width =
                ...
                'photo' !== $oembed->type &&
                (int) $oembed->thumbnail_width < 550
                    ? (int) $oembed->thumbnail_width
                    : $width;
            ...
            // Set up thumb.
            $content = sprintf(
                '<div class="thumb" style="max-width:%1$spx">%2$s<a href="%3$s" rel="nofollow" onclick="top.location.href=\'%3$s\'"><img loading="lazy" src="%4$s" alt="" /></a></div>',
                $thumb_width,
                $play_icon,
                esc_url($url),
                esc_url($thumbnail)
            ...
            if (isset($oembed->title)) {
                $caption .= sprintf(
                    '<p class="caption-title"><strong>%s</strong></p>',
                    apply_filters('single_post_title', $oembed->title)
                );
            }
            ...
            if (isset($oembed->description)) {
                $caption .= sprintf(
                    '<div class="caption-description">%s</div>',
                    apply_filters(
                        'bp_activity_get_embed_excerpt',
                        $oembed->description
                    )
            ...
            if (true === isset($anchor_text)) {
                $caption .= sprintf(
                    '<a rel="nofollow" href="%1$s" onclick="top.location.href=\'%1$s\'">%2$s</a>',
                    esc_url($url),
                    apply_filters('the_title', $anchor_text)
                );
            }

            // Set up caption.
            if ('' !== $caption) {
                $css_class = isset($oembed->provider_name)
                    ? sprintf(
                        ' provider-%s',
                        sanitize_html_class(strtolower($oembed->provider_name))
                    )
                    : '';
                $caption = sprintf(
                    '<div class="caption%1$s" style="width:%2$s">%3$s</div>',
                    $css_class,
                    $thumb_width > $float_width
                        ? 100 . '%'
                        : round(
                                (($width - (int) $thumb_width) / $width) * 100
                            ) . '%',
                    $caption
                );
        ...
        $content = buddypress()->embed->run_shortcode(
            $GLOBALS['activities_template']->activity->content
        );
        ...
        if (
            isset($media['videos']) &&
            'shortcodes' === $media['videos'][0]['source']
        ) {
            printf(
            ...
            isset($media['audio']) &&
            'shortcodes' === $media['audio'][0]['source']
        ) {
            printf(
