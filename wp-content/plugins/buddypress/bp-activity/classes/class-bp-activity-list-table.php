...
class BP_Activity_List_Table extends WP_List_Table
{
    ...
    function prepare_items()
    {
        ...
        if (
            !empty($_REQUEST['activity_status']) &&
        ...
        if (!empty($_REQUEST['activity_type'])) {
            $filter = ['action' => $_REQUEST['activity_type']];
             ...
             * @param array $filter
             */
            $has_filter_query = apply_filters(
        ...
        foreach ($activities['activities'] as $activity_item) {
            ...
            $this->activity_user_id[$activity_item->id] =
  ...
  _e('Activities list', 'buddypress'); ?></h2>
    ...
    function get_views()
    {
		...
		<h2 class="screen-reader-text"><?php /* translators: accessibility text */
  esc_html_e('Filter activities list', 'buddypress'); ?></h2>
    ...
    echo 'current';
        ...
        add_query_arg(['activity_status' => 'spam'], $url_base)
    ); ?>" class="<?php if ('spam' === $this->view) {
    echo 'current';
} ?>">
					<?php printf(
         /* translators: %s is the placeholder for the count html tag `<span class="count"/>` */
   ...
   do_action('bp_activity_list_table_get_views', $url_base, $this->view); ?>
    ...
    public function get_sortable_columns()
    {
         ...
         */
        return apply_filters('bp_activity_list_table_get_sortable_columns', []);
    }
    ...
    function extra_tablenav($which)
    {
        ...
        $selected = !empty($_REQUEST['activity_type'])
		...
		<div class="alignleft actions">
			<label for="activity-type" class="screen-reader-text"><?php /* translators: accessibility text */
   _e('Filter by activity type', 'buddypress'); ?></label>
			<select name="activity_type" id="activity-type">
				<option value="" <?php selected(
        !$selected
    ); ?>><?php _e('View all actions', 'buddypress'); ?></option>
     ...
     if (
         isset($actions->activity_update) &&
         'bp_groups_format_activity_action_group_activity_update' ===
             $actions->activity_update['format_callback']
     ) {
     ...
     ?>
					...
					<optgroup label="<?php echo esc_html($component_name); ?>">
						...
						<?php foreach ($actions as $action_key => $action_values): ?>
							...
							<?php // Skip the incorrectly named pre-1.6 action.
       if ('friends_register_activity_action' !== $action_key): ?>

								<option value="<?php echo esc_attr($action_key); ?>" <?php selected(
    $action_key,
    $selected
); ?>><?php echo esc_html($action_values['value']); ?></option>
			...
			<?php submit_button(__('Filter', 'buddypress'), 'secondary', false, false, [
    ...
    function column_cb($item)
    {
        ...
        printf(
            '<label class="screen-reader-text" for="aid-%1$d">' .
                __('Select activity item %1$d', 'buddypress') .
                '</label><input type="checkbox" name="aid[]" value="%1$d" id="aid-%1$d" />',
            $item['id']
        );
    }
    ...
    function column_comment($item)
    {
        ...
        $base_url = bp_get_admin_url(
            'admin.php?page=bp-activity&amp;aid=' . $item['id']
        );
        $spam_nonce = esc_html(
            '_wpnonce=' . wp_create_nonce('spam-activity_' . $item['id'])
        );
        ...
        if ('spam' != $item_status) {
            if ($this->can_comment($item)) {
                $actions['reply'] = sprintf(
                    '<a href="#" class="reply hide-if-no-js">%s</a>',
                    esc_html__('Reply', 'buddypress')
                );
            } else {
                $actions['reply'] = sprintf(
                    '<span class="form-input-tip">%s</span>',
                    esc_html__('Replies disabled', 'buddypress')
                );
            }
        ...
        $actions['delete'] = sprintf(
            '<a href="%1$s">%2$s</a>',
            esc_url($delete_url),
            esc_html__('Delete Permanently', 'buddypress')
        );
        ...
        if (!empty($item['content'])) {
            ...
            /** This filter is documented in bp-activity/bp-activity-template.php */
            $content = apply_filters_ref_array('bp_get_activity_action', [
                $item['action'],
                &$activity,
                $r,
            ]);
  ...
  if (!$item['is_spam']) {
      $activity_permalink =
          '<a href="' .
          bp_activity_get_permalink($item['id'], (object) $item) .
  ...
  if (
      ...
      $comment_count = !empty($item['children'])
          ? bp_activity_recurse_comment_count((object) $item)
          : 0;
      $root_activity_url = bp_get_admin_url(
          'admin.php?page=bp-activity&amp;aid=' . $item['id']
      );
      ...
      if ($comment_count) {
          printf(
              '<a href="%1$s" class="post-com-count post-com-count-approved"><span class="comment-count comment-count-approved">%2$s</span></a>',
              esc_url($root_activity_url),
      ...
      echo '<strong>' .
          get_avatar($this->get_activity_user_id($item['item_id']), '32') .
          ' ' .
    ...
    {
        $can_comment = bp_activity_type_supports(
            $item['type'],
            'comment-reply'
        );
        ...
        if (!$this->disable_blogforum_comments && bp_is_active('blogs')) {
            ...
            if (
                isset($parent_activity->type) &&
                bp_activity_post_type_get_tracking_arg(
                    $parent_activity->type,
                    'post_type'
                )
            ) {
