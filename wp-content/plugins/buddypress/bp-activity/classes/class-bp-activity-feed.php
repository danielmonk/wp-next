...
class BP_Activity_Feed
{
    ...
    public function __construct($args = [])
    {
         ...
         */
        do_action_ref_array('bp_activity_feed_prefetch', [&$this]);
        ...
        if (empty($this->id)) {
            _doing_it_wrong(
                'BP_Activity_Feed',
                __("RSS feed 'id' must be defined", 'buddypress'),
                'BP 1.8'
            );
            return false;
        }
         ...
         * @param BP_Activity_Feed $this Current instance of activity feed. Passed by reference.
         */
        do_action_ref_array('bp_activity_feed_postfetch', [&$this]);
    ...
    protected function setup_properties()
    {
        ...
        $this->activity_args = wp_parse_args($this->activity_args, [
            'max' => $this->max,
            'per_page' => $this->max,
    ...
    {
        add_action('bp_activity_feed_rss_attributes', [
            $this,
            'backpat_rss_attributes',
        ]);
        add_action('bp_activity_feed_channel_elements', [
            $this,
            'backpat_channel_elements',
        ]);
        add_action('bp_activity_feed_item_elements', [
            $this,
    ...
    protected function http_headers()
    {
        ...
        if (
            $client_last_modified && $client_etag
                ? $client_modified_timestamp >= $modified_timestamp &&
                    $client_etag == $etag
                : $client_modified_timestamp >= $modified_timestamp ||
                    $client_etag == $etag
        ) {
