...
class BP_Activity_Template
{
    ...
    public function __construct($args)
    {
        ...
        $this->disable_blogforum_replies = (bool) bp_core_get_root_option(
            'bp-disable-blogforum-comments'
        );

        // Get an array of the logged in user's favorite activities.
        $this->my_favs = bp_get_user_meta(
            bp_loggedin_user_id(),
        ...
        if (!empty($parent_ids)) {
            $activity_parents = bp_activity_get_specific([
                'activity_ids' => $parent_ids,
            ]);
        }
        ...
        if ((int) $this->total_activity_count && (int) $this->pag_num) {
            $this->pag_links = paginate_links([
                ...
                'format' => '',
                'total' => ceil(
                    (int) $this->total_activity_count / (int) $this->pag_num
                ),
                'current' => (int) $this->pag_page,
                'prev_text' => _x(
                    '&larr;',
                    'Activity pagination previous text',
                    'buddypress'
                ),
                'next_text' => _x(
                    '&rarr;',
                    'Activity pagination next text',
                    'buddypress'
                ),
                'mid_size' => 1,
                'add_args' => [],
