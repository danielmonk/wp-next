...
class BP_Activity_Query extends BP_Recursive_Query
{
    ...
    protected function get_sql_for_clause($clause, $parent_query)
    {
        ...
        $column = isset($clause['column'])
            ? $this->validate_column($clause['column'])
            : '';
        ...
        if (isset($clause['value'])) {
            if (
                in_array($compare, ['IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN'])
            ) {
                if (!is_array($value)) {
            ...
            if (
                !empty($column) &&
                true === in_array($column, ['hide_sitewide', 'is_spam'])
            ) {
                ...
                switch ($compare) {
                    ...
                    case 'IN':
                    case 'NOT IN':
                        $in_sql = BP_Activity_Activity::get_in_operator_sql(
                            "{$alias}{$column}",
                            $value
                        );
