...
class BP_Activity_oEmbed_Extension extends BP_Core_oEmbed_Extension
{
    ...
    protected function custom_hooks()
    {
        add_action(
            'oembed_dataparse',
            [$this, 'use_custom_iframe_sandbox_attribute'],
            20,
            3
        );
        add_action('embed_content_meta', [$this, 'embed_comments_button'], 5);
        add_action(
            'get_template_part_assets/embeds/header',
            [$this, 'on_activity_header'],
            10,
            2
        );

        add_filter('bp_activity_embed_html', [$this, 'modify_iframe']);
    }
    ...
    {
        $activity = new BP_Activity_Activity($item_id);
        $mentionname = bp_activity_do_mentions()
        ...
        $blockquote = sprintf(
            ...
            '- ' .
                ...
                $mentionname,
            '<a href="' .
                esc_url(bp_activity_get_permalink($item_id)) .
                '">' .
                $date .
                '</a>'
        );
    ...
    public function use_custom_iframe_sandbox_attribute($result, $data, $url)
    {
        ...
        $sandbox = substr(
            $data->html,
            $sandbox_pos,
            strpos($data->html, '"', $sandbox_pos) - $sandbox_pos
        );
    ...
    public function embed_comments_button()
    {
        ...
        add_filter('the_permalink', [$this, 'filter_embed_url']);
