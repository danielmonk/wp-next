...
class BP_Activity_Activity
{
    ...
    public function save()
    {
        ...
        $this->id = apply_filters_ref_array('bp_activity_id_before_save', [
            $this->id,
            &$this,
        ]);
        $this->item_id = apply_filters_ref_array(
            'bp_activity_item_id_before_save',
            [$this->item_id, &$this]
        );
        $this->secondary_item_id = apply_filters_ref_array(
            'bp_activity_secondary_item_id_before_save',
            [$this->secondary_item_id, &$this]
        );
        $this->user_id = apply_filters_ref_array(
            'bp_activity_user_id_before_save',
            [$this->user_id, &$this]
        );
        $this->primary_link = apply_filters_ref_array(
            'bp_activity_primary_link_before_save',
            [$this->primary_link, &$this]
        );
        $this->component = apply_filters_ref_array(
            'bp_activity_component_before_save',
            [$this->component, &$this]
        );
        $this->type = apply_filters_ref_array('bp_activity_type_before_save', [
            $this->type,
            &$this,
        ]);
        $this->action = apply_filters_ref_array(
        ...
        $this->date_recorded = apply_filters_ref_array(
            'bp_activity_date_recorded_before_save',
            [$this->date_recorded, &$this]
        );
        $this->hide_sitewide = apply_filters_ref_array(
            'bp_activity_hide_sitewide_before_save',
            [$this->hide_sitewide, &$this]
        );
        $this->mptt_left = apply_filters_ref_array(
            'bp_activity_mptt_left_before_save',
            [$this->mptt_left, &$this]
        );
        $this->mptt_right = apply_filters_ref_array(
            'bp_activity_mptt_right_before_save',
            [$this->mptt_right, &$this]
        );
        $this->is_spam = apply_filters_ref_array(
         ...
         */
        do_action_ref_array('bp_activity_before_save', [&$this]);

        if (
            'wp_error' === $this->error_type &&
            $this->errors->get_error_code()
        ) {
            return $this->errors;
        }
        ...
        if (empty($this->component) || empty($this->type)) {
            if ('bool' === $this->error_type) {
                ...
                if (empty($this->component)) {
                    $this->errors->add(
                        'bp_activity_missing_component',
                        __(
                            'You need to define a component parameter to insert activity.',
                            'buddypress'
                        )
                    );
                } else {
                    $this->errors->add(
                        'bp_activity_missing_type',
                        __(
                            'You need to define a type parameter to insert activity.',
                            'buddypress'
                        )
                    );
                }
         ...
         */
        $type_requires_content = (bool) apply_filters(
        ...
        if (!empty($this->id)) {
            $q = $wpdb->prepare(
                "UPDATE {$bp->activity->table_name} SET user_id = %d, component = %s, type = %s, action = %s, content = %s, primary_link = %s, date_recorded = %s, item_id = %d, secondary_item_id = %d, hide_sitewide = %d, is_spam = %d WHERE id = %d",
                $this->user_id,
                $this->component,
                $this->type,
                $this->action,
            ...
            $q = $wpdb->prepare(
                "INSERT INTO {$bp->activity->table_name} ( user_id, component, type, action, content, primary_link, date_recorded, item_id, secondary_item_id, hide_sitewide, is_spam ) VALUES ( %d, %s, %s, %s, %s, %s, %s, %d, %d, %d, %d )",
                $this->user_id,
                $this->component,
                $this->type,
                $this->action,
         ...
         */
        do_action_ref_array('bp_activity_after_save', [&$this]);
    ...
    public static function get($args = [])
    {
        ...
        if (
            $r['filter'] &&
            ($filter_sql = BP_Activity_Activity::get_filter_sql($r['filter']))
        ) {
        ...
        if ($r['search_terms']) {
            ...
            $where_conditions['search_sql'] = $wpdb->prepare(
                'a.content LIKE %s',
                $search_terms_like
            );
         ...
         */
        if (
            apply_filters('bp_use_legacy_activity_query', false, __METHOD__, $r)
        ) {
            ...
            if (!empty($page) && !empty($per_page)) {
                ...
                $activity_sql = apply_filters(
                    'bp_activity_get_user_join_filter',
                    "{$select_sql} {$from_sql} {$join_sql} {$where_sql} ORDER BY a.date_recorded {$sort}, a.id {$sort} {$pag_sql}",
                    $select_sql,
                    $from_sql,
                    $where_sql,
                    $sort,
                ...
                $activity_sql = apply_filters(
                    'bp_activity_get_user_join_filter',
                    "{$select_sql} {$from_sql} {$join_sql} {$where_sql} ORDER BY a.date_recorded {$sort}, a.id {$sort}",
                    $select_sql,
                    $from_sql,
                    $where_sql,
                    $sort,
            ...
            foreach ((array) $activities as $i => $ac) {
                ...
                $activities[$i]->secondary_item_id =
                    (int) $ac->secondary_item_id;
             ...
             */
            $activity_ids_sql = apply_filters(
            ...
            $cached = bp_core_get_incremented_cache(
                $activity_ids_sql,
                $cache_group
            );
            if (false === $cached) {
            ...
            $retval['has_more_items'] =
                !empty($per_page) && count($activity_ids) > $per_page;
        ...
        if ('ids' !== $r['fields']) {
            ...
            if ($activities && $r['display_comments']) {
                $activities = BP_Activity_Activity::append_comments(
                    $activities,
                    $r['spam']
                );
            }
            ...
            $activities = BP_Activity_Activity::generate_action_strings(
                $activities
            );
        }
        ...
        if (!empty($r['count_total'])) {
             ...
             */
            $total_activities_sql = apply_filters(
                'bp_activity_total_activities_sql',
                "SELECT count(DISTINCT a.id) FROM {$bp->activity->table_name} a {$join_sql} {$where_sql}",
                $where_sql,
                $sort
            );
            $cached = bp_core_get_incremented_cache(
     ...
     * @return array
     */
    protected static function get_activity_data($activity_ids = [])
    {
        global $wpdb;
        ...
        if (!empty($uncached_ids)) {
            ...
            $queried_adata = $wpdb->get_results(
        ...
        foreach ($activities as $a_index => $a_item) {
            ...
            $a_user = isset($user_query->results[$a_user_id])
                ? $user_query->results[$a_user_id]
                : '';
                ...
                if (!empty($fullnames)) {
                    foreach ((array) $activities as $i => $activity) {
                        if (!empty($fullnames[$activity->user_id])) {
                            $activities[$i]->user_fullname =
                                $fullnames[$activity->user_id];
                        }
                    }
                }
            }
     ...
     */
    public static function get_meta_query_sql($meta_query = [])
    {
        global $wpdb;
     ...
     */
    public static function get_scope_query_sql($scope = false, $r = [])
    {
    ...
    public static function get_specific(
        ...
        return BP_Activity_Activity::get(
            $max,
            $page,
            $per_page,
            $sort,
    ...
    public static function delete($args = [])
    {
        ...
        $activities = $wpdb->get_results(
            "SELECT * FROM {$bp->activity->table_name} {$where_sql}"
        );
         ...
         */
        do_action_ref_array('bp_activity_before_delete', [$activities, $r]);

        // Attempt to delete activities from the database.
        $deleted = $wpdb->query(
            "DELETE FROM {$bp->activity->table_name} {$where_sql}"
        );
         ...
         * @param array $r          Array of parsed arguments.
         */
        do_action_ref_array('bp_activity_after_delete', [$activities, $r]);
        ...
        if (!empty($activity_ids)) {
            ...
            foreach ($activity_ids as $activity_id) {
                ...
                $comments = BP_Activity_Activity::delete([
            ...
            if (!empty($comment_ids)) {
                $activity_ids = array_unique(
                    array_merge($activity_ids, $comment_ids)
                );
            }
        }
     ...
     */
    public static function delete_activity_item_comments(
        $activity_ids = [],
        $delete_meta = true
    ) {
        global $wpdb;
        ...
        if ($delete_meta) {
            ...
            $activity_comment_ids = $wpdb->get_col(
        ...
        return $wpdb->query(
            "DELETE FROM {$bp->activity->table_name} WHERE type = 'activity_comment' AND item_id IN ({$activity_ids})"
        );
    }
     ...
     * @return bool True on success.
     */
    public static function delete_activity_meta_entries($activity_ids = [])
    {
    ...
    public static function append_comments($activities, $spam = 'ham_only')
    {
        ...
        foreach ((array) $activities as $activity) {
            $top_level_parent_id =
                'activity_comment' == $activity->type ? $activity->item_id : 0;
            $activity_comments[
                $activity->id
    ...
    public static function get_activity_comments(
        ...
        if ('none' === $comments) {
            ...
            if (bp_is_active('xprofile')) {
                ...
                $fullname_where =
                    "AND pd.user_id = a.user_id AND pd.field_id = 1";
            ...
            if (
                ...
                $sql = apply_filters(
                    ...
                    $wpdb->prepare(
                        "SELECT a.*, u.user_email, u.user_nicename, u.user_login, u.display_name{$fullname_select} FROM {$bp->activity->table_name} a, {$wpdb->users} u{$fullname_from} WHERE u.ID = a.user_id {$fullname_where} AND a.type = 'activity_comment' {$spam_sql} AND a.item_id = %d AND a.mptt_left > %d AND a.mptt_left < %d ORDER BY a.date_recorded ASC",
                        $top_level_parent_id,
                        $left,
                        $right
                    ),
            ...
            foreach ((array) $descendants as $d) {
                ...
                if (isset($ref[$d->secondary_item_id])) {
                    ...
                    $ref[$d->id] =
                        &$ref[$d->secondary_item_id]->children[$d->id];
    ...
    public static function rebuild_activity_comment_tree($parent_id, $left = 1)
    {
        ...
        if (1 === $left) {
            $wpdb->query(
                $wpdb->prepare(
                    "UPDATE {$bp->activity->table_name} SET mptt_left = %d, mptt_right = %d WHERE id = %d",
                    $left,
                    $right,
                    $parent_id
                )
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "UPDATE {$bp->activity->table_name} SET mptt_left = %d, mptt_right = %d WHERE type = 'activity_comment' AND id = %d",
                    $left,
                    $right,
                    $parent_id
                )
    ...
    public static function get_recorded_components($skip_last_activity = true)
    {
        ...
        if (true === $skip_last_activity) {
            $components = $wpdb->get_col(
                "SELECT DISTINCT component FROM {$bp->activity->table_name} WHERE action != '' AND action != 'last_activity' ORDER BY component ASC"
            );
        } else {
            $components = $wpdb->get_col(
                "SELECT DISTINCT component FROM {$bp->activity->table_name} ORDER BY component ASC"
            );
        }
    ...
    {
        $activities = bp_activity_get_sitewide(['max' => $limit]);
        $activity_feed = [];
        ...
        for ($i = 0, $count = count($activities); $i < $count; ++$i) {
            ...
            $activity_feed[$i]['description'] = @sprintf(
    ...
    public static function get_in_operator_sql($field, $items)
    {
        ...
        foreach ($items as $item) {
            ...
            $items_prepared[] = is_numeric($item)
                ? $wpdb->prepare('%d', $item)
                : $wpdb->prepare('%s', $item);
        }
    ...
    public static function get_filter_sql($filter_array)
    {
        ...
        if (!empty($filter_array['user_id'])) {
            $user_sql = BP_Activity_Activity::get_in_operator_sql(
                'a.user_id',
                $filter_array['user_id']
            );
            if (!empty($user_sql)) {
                $filter_sql[] = $user_sql;
            }
        ...
        if (!empty($filter_array['action'])) {
            $action_sql = BP_Activity_Activity::get_in_operator_sql(
                'a.type',
                $filter_array['action']
            );
            if (!empty($action_sql)) {
        ...
        if (!empty($filter_array['secondary_id'])) {
            $sid_sql = BP_Activity_Activity::get_in_operator_sql(
                'a.secondary_item_id',
                $filter_array['secondary_id']
            );
            if (!empty($sid_sql)) {
                $filter_sql[] = $sid_sql;
            }
    ...
    public static function total_favorite_count($user_id)
    {
        ...
        $favorite_activity_entries = bp_get_user_meta(
            $user_id,
    ...
    public static function hide_all_for_user($user_id)
    {
        ...
        return $wpdb->get_var(
            $wpdb->prepare(
                "UPDATE {$bp->activity->table_name} SET hide_sitewide = 1 WHERE user_id = %d",
                $user_id
            )
        );
    }
