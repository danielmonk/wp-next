...
class BP_Akismet
{
    ...
    protected function setup_actions()
    {
        ...
        add_action('bp_after_activity_post_form', [
            $this,
        ...
        add_action('bp_activity_comment_options', [
            $this,
            'add_activity_comment_spam_button',
        ]);

        // Check activity for spam.
        add_action('bp_activity_before_save', [$this, 'check_activity'], 4, 1);

        // Tidy up member's latest (activity) update.
        add_action(
            'bp_activity_posted_update',
            [$this, 'check_member_activity_update'],
            1,
            3
        );

        // Hooks to extend Activity core spam/ham functions for Akismet.
        add_action('bp_activity_mark_as_spam', [$this, 'mark_as_spam'], 10, 2);
        add_action('bp_activity_mark_as_ham', [$this, 'mark_as_ham'], 10, 2);

        // Hook into the Activity wp-admin screen.
        add_action(
            'bp_activity_admin_comment_row_actions',
            [$this, 'comment_row_action'],
            10,
            2
        );
        add_action('bp_activity_admin_load', [$this, 'add_history_metabox']);
    }
    ...
    {
        $akismet_result = bp_activity_get_meta(
            $activity['id'],
            '_bp_akismet_result'
        );
        $user_result = bp_activity_get_meta(
            $activity['id'],
            '_bp_akismet_user_result'
        );
        $desc = '';
        ...
        if (!$user_result || $user_result == $akismet_result) {
            ...
            if ('true' == $user_result) {
                ...
                $desc = sprintf(
                    __('Flagged as spam by %s', 'buddypress'),
                    $who
                );
            } else {
                /* translators: %s: the name of the user */
            ...
            foreach ($actions as $k => $item) {
                ...
                if ($k == 'edit') {
                    $b['history'] =
                        ...
                        esc_url(
                            bp_get_admin_url(
                                'admin.php?page=bp-activity&amp;action=edit&aid=' .
                                    $activity['id']
                            )
                        ) .
                        '#bp_activity_history"> ' .
                        __('History', 'buddypress') .
                        '</a>';
                }
            }
        ...
        if ($desc) {
            echo '<span class="akismet-status"><a href="' .
                esc_url(
                    bp_get_admin_url(
                        'admin.php?page=bp-activity&amp;action=edit&aid=' .
                            $activity['id']
     ...
     */
    public function check_member_activity_update(
        $content,
        $user_id,
        $activity_id
    ) {
        ...
        if (
            ...
            $activity_id != $this->last_activity->id ||
            'false' == $this->last_activity->akismet_submission['bp_as_result']
        ) {
            return;
        }
    ...
    public function add_activity_spam_button()
    {
        ...
        // By default, only handle activity updates and activity comments.
        if (
            !in_array(bp_get_activity_type(), BP_Akismet::get_activity_types())
        ) {
            return;
        }
        ...
        bp_button([
            ...
            'link_class' =>
                'bp-secondary-action spam-activity confirm button item-button',
            'link_href' => wp_nonce_url(
                bp_get_root_domain() .
                    ...
                    bp_get_activity_id() .
                    '/',
                'bp_activity_akismet_spam_' . bp_get_activity_id()
            ),
    ...
    public function add_activity_comment_spam_button()
    {
        ...
        if (
            empty($current_comment) ||
            !in_array($current_comment->type, BP_Akismet::get_activity_types())
        ) {
            return;
        }
        ...
        bp_button([
            ...
            'link_href' => wp_nonce_url(
                bp_get_root_domain() .
                    ...
                    '/?cid=' .
                    bp_get_activity_comment_id(),
                'bp_activity_akismet_spam_' . bp_get_activity_comment_id()
            ),
            'link_text' => __('Spam', 'buddypress'),
            'wrapper' => false,
        ]);
    ...
    public static function get_activity_types()
    {
         ...
         */
        return apply_filters('bp_akismet_get_activity_types', [
            'activity_comment',
            'activity_update',
        ]);
    }
    ...
    public function mark_as_ham($activity, $source)
    {
        ...
        if (
            'true' ==
                bp_activity_get_meta($activity->id, '_bp_akismet_result') &&
            !bp_activity_get_meta($activity->id, '_bp_akismet_user_result')
        ) {
            $activity->content = bp_activity_at_name_filter(
                $activity->content,
                $activity->id
            );
        }
    ...
    public static function build_akismet_data_package($activity)
    {
        ...
        $activity_data['comment_author_url'] = bp_core_get_userlink(
            $userdata->ID,
            false,
            true
        );
        ...
        $activity_data['permalink'] = bp_activity_get_permalink(
            $activity->id,
            $activity
        );
        ...
        if (!empty($_POST['_bp_as_nonce'])) {
            $activity_data['akismet_comment_nonce'] = wp_verify_nonce(
                $_POST['_bp_as_nonce'],
         ...
         */ elseif (
            ...
            $activity_data['akismet_comment_nonce'] = wp_verify_nonce(
                $_POST["_bp_as_nonce_{$activity->secondary_item_id}"],
                "_bp_as_nonce_{$userdata->ID}_{$activity->secondary_item_id}"
            )
                ? 'passed'
                : 'failed';
        }
    ...
    public function check_activity($activity)
    {
        ...
        $activity_data = $this->send_akismet_request(
            $activity_data,
            'check',
            'spam'
        );
        ...
        if ('true' == $activity_data['bp_as_result']) {
             ...
             */
            do_action_ref_array('bp_activity_akismet_spam_caught', [
                &$activity,
                $activity_data,
            ]);
        ...
        add_action(
            ...
            [$this, 'update_activity_akismet_meta'],
            1,
            1
        );
    }
    ...
    public function update_activity_akismet_meta($activity)
    {
        ...
        if (
            empty($this->last_activity) ||
            (!empty($this->last_activity) &&
                $activity->id != $this->last_activity->id)
        ) {
            return;
        }
        ...
        // Spam.
        if (
            'true' == $this->last_activity->akismet_submission['bp_as_result']
        ) {
            bp_activity_update_meta(
                $activity->id,
                '_bp_akismet_result',
            ...
            $this->update_activity_history(
                $activity->id,
                __('Akismet caught this item as spam', 'buddypress'),
                'check-spam'
            );
            ...
            $this->update_activity_history(
                $activity->id,
                __('Akismet cleared this item', 'buddypress'),
                'check-ham'
            );
            ...
            $this->update_activity_history(
                ...
                sprintf(
                    ...
                    __(
                        'Akismet was unable to check this item (response: %s), will automatically retry again later.',
                        'buddypress'
                    ),
     ...
     */
    public function send_akismet_request(
        $activity_data,
        $check = 'check',
        $spam = 'spam'
    ) {
        ...
        // Send to Akismet.
        add_filter('akismet_ua', [$this, 'buddypress_ua']);
        $response = Akismet::http_post($query_string, $path);
        remove_filter('akismet_ua', [$this, 'buddypress_ua']);

        // Save response data.
        $activity_data['bp_as_result'] = $response[1];
        if (isset($response[0]['x-akismet-pro-tip'])) {
            $activity_data['akismet_pro_tip'] =
    ...
    function add_history_metabox($screen_action)
    {
        ...
        add_meta_box(
            'bp_activity_history',
            __('Activity History', 'buddypress'),
    ...
    function history_metabox($item)
    {
        ...
        printf(
            _x(
                '%1$s &mdash; %2$s',
                'x hours ago - akismet cleared this item',
                'buddypress'
            ),
            '<span>' . bp_core_time_since($history[2]) . '</span>',
            esc_html($history[1])
        );
        echo '</div></div>';
    }
     ...
     */
    public function update_activity_history(
        $activity_id = 0,
        $message = '',
        $event = ''
    ) {
