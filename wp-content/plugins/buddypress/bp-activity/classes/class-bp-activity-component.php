...
class BP_Activity_Component extends BP_Component
{
    ...
    public function includes($includes = [])
    {
        ...
        if (
            ...
            (!empty($akismet_key) || defined('WPCOM_API_KEY')) &&
            apply_filters('bp_activity_use_akismet', bp_is_akismet_active())
        ) {
            $includes[] = 'akismet';
        }
    ...
    public function setup_globals($args = [])
    {
        ...
        $args = [
            ...
            'root_slug' => isset($bp->pages->activity->slug)
                ? $bp->pages->activity->slug
                : BP_ACTIVITY_SLUG,
            'has_directory' => true,
            'directory_title' => isset($bp->pages->activity->title)
                ? $bp->pages->activity->title
    ...
    public function setup_admin_bar($wp_admin_nav = [])
    {
        ...
        if (is_user_logged_in()) {
            ...
            $activity_link = trailingslashit(
                bp_loggedin_user_domain() . bp_get_activity_slug()
            );
            ...
            if (bp_is_active('friends')) {
                $wp_admin_nav[] = [
                    ...
                    'href' => trailingslashit(
                        $activity_link . bp_get_friends_slug()
                    ),
                    'position' => 40,
                ];
            }
            ...
            if (bp_is_active('groups')) {
                $wp_admin_nav[] = [
                    ...
                    'href' => trailingslashit(
                        $activity_link . bp_get_groups_slug()
                    ),
                    'position' => 50,
                ];
            }
    ...
    {
        parent::rest_api_init(['BP_REST_Activity_Endpoint']);
    }
