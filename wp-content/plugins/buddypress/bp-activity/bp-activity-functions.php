...
{
    return isset(buddypress()->pages->activity->id) &&
        buddypress()->pages->activity->id;
}
...
function bp_activity_maybe_load_mentions_scripts()
{
    ...
    $load_mentions =
        $mentions_enabled && (bp_is_activity_component() || is_admin());
 ...
 * @return bool
 */
function bp_activity_update_mention_count_for_user(
    $user_id,
    $activity_id,
    $action = 'add'
) {
    ...
    $new_mention_count = (int) bp_get_user_meta(
        $user_id,
...
function bp_activity_set_action(
    ...
    if (
        !isset($bp->activity->actions->{$component_id}) ||
        !is_object($bp->activity->actions->{$component_id})
    ) {
    ...
    foreach ($action_array as $key_ordered) {
        $bp->activity->actions->{$component_id}->{$key_ordered[
            'key'
        ]} = $key_ordered;
    }
 ...
 * @return bool
 */
function bp_activity_set_post_type_tracking_args($post_type = '', $args = [])
{
    global $wp_post_types;

    if (
        empty($wp_post_types[$post_type]) ||
        !post_type_supports($post_type, 'buddypress-activity') ||
        !is_array($args)
    ) {
        return false;
    }
    ...
    foreach ($activity_labels as $label_type) {
        if (!empty($args[$label_type])) {
            $wp_post_types[$post_type]->labels->{$label_type} =
                $args[$label_type];
...
function bp_activity_get_post_type_tracking_args($post_type)
{
    ...
    if (!empty($post_type_object->labels->bp_activity_admin_filter)) {
        $post_type_activity->admin_filter =
            $post_type_object->labels->bp_activity_admin_filter;

        // Fall back to a generic name.
    } else {
        $post_type_activity->admin_filter = _x(
            'New item published',
            'Post Type generic activity post admin filter',
            'buddypress'
        );
    }
    ...
    if (!empty($post_type_object->labels->bp_activity_front_filter)) {
        $post_type_activity->front_filter =
            $post_type_object->labels->bp_activity_front_filter;
    }
    ...
    if (!empty($post_type_object->labels->bp_activity_new_post)) {
        $post_type_activity->new_post_type_action =
            $post_type_object->labels->bp_activity_new_post;
    }
    ...
    if (!empty($post_type_object->labels->bp_activity_new_post_ms)) {
        $post_type_activity->new_post_type_action_ms =
            $post_type_object->labels->bp_activity_new_post_ms;
    }
    ...
    if (
        ...
        !empty($post_type_activity->comment_action_id)
    ) {
        ...
        $post_type_activity->comments_tracking->component_id =
            $post_type_activity->component_id;
        $post_type_activity->comments_tracking->action_id =
            $post_type_activity->comment_action_id;

        // Try to get the comments admin filter from the post type labels.
        if (
            !empty($post_type_object->labels->bp_activity_comments_admin_filter)
        ) {
            $post_type_activity->comments_tracking->admin_filter =
                $post_type_object->labels->bp_activity_comments_admin_filter;

            // Fall back to a generic name.
        } else {
            $post_type_activity->comments_tracking->admin_filter = _x(
                'New item comment posted',
                'Post Type generic comments activity admin filter',
                'buddypress'
            );
        }

        $post_type_activity->comments_tracking->format_callback =
            $post_type_activity->comment_format_callback;

        // Check for the comments front filter in the post type labels.
        if (
            !empty($post_type_object->labels->bp_activity_comments_front_filter)
        ) {
            $post_type_activity->comments_tracking->front_filter =
                $post_type_object->labels->bp_activity_comments_front_filter;

            // Fall back to a generic name.
        } else {
            $post_type_activity->comments_tracking->front_filter = _x(
                'Item comments',
                'Post Type generic comments activity front filter',
                'buddypress'
            );
        }

        $post_type_activity->comments_tracking->contexts =
            $post_type_activity->contexts;
        $post_type_activity->comments_tracking->position =
            (int) $post_type_activity->position + 1;
        ...
        if (!empty($post_type_object->labels->bp_activity_new_comment)) {
            $post_type_activity->comments_tracking->new_post_type_comment_action =
                $post_type_object->labels->bp_activity_new_comment;
        }
        ...
        if (!empty($post_type_object->labels->bp_activity_new_comment_ms)) {
            $post_type_activity->comments_tracking->new_post_type_comment_action_ms =
                $post_type_object->labels->bp_activity_new_comment_ms;
        }
    }
    ...
    foreach ($post_types as $post_type) {
        ...
        if (!empty($track_post_type)) {
            ...
            if (!empty($track_post_type->comments_tracking->action_id)) {
                ...
                $post_types_tracking_args[
                    $track_post_type->comments_tracking->action_id
                ] = $track_post_type->comments_tracking;
...
function bp_activity_type_supports($activity_type = '', $feature = '')
{
    ...
    switch ($feature) {
        ...
        case 'post-type-comment-tracking':
            ...
            if (
                !empty($bp->activity->track[$activity_type]->comments_tracking)
            ) {
                $retval = true;
            }
            break;
        ...
        case 'post-type-comment-reply':
            ...
            if (
                !empty(
                    $bp->activity->track[$activity_type]->comments_tracking
                ) &&
                !empty($bp->activity->track[$activity_type]->comment_action_id)
            ) {
                $retval = true;
            }
            break;
        ...
        case 'comment-reply':
            ...
            if (!empty($bp->activity->track[$activity_type])) {
                if (
                    isset(
                        $bp->activity->track[$activity_type]->activity_comment
                    )
                ) {
                    $retval =
                        $bp->activity->track[$activity_type]->activity_comment;
                ...
                if (
                    ...
                    $retval =
                        $bp->activity->track[$activity_type]
                            ->comments_tracking &&
                ...
                'new_forum_topic' === $activity_type ||
                'new_forum_post' === $activity_type
            ) {
...
function bp_activity_get_actions()
{
    ...
    if (!empty($bp->activity->track)) {
        foreach ($bp->activity->track as $post_type) {
            if (
                isset(
                    $bp->activity->actions->{$post_type->component_id}
                        ->{$post_type->action_id}
                )
            ) {
                continue;
...
function bp_activity_get_actions_for_context($context = '')
{
    ...
    foreach (bp_activity_get_actions() as $component_actions) {
        foreach ($component_actions as $component_action) {
            if (
                in_array($context, (array) $component_action['context'], true)
            ) {
...
function bp_activity_check_exists_by_content($content)
{
    ...
    return apply_filters(
        ...
        BP_Activity_Activity::check_exists_by_content($content)
    );
}
 ...
 * @return bool True on success, false on failure.
 */
function bp_activity_delete_meta(
    $activity_id,
    $meta_key = '',
    $meta_value = '',
    $delete_all = false
    ...
    if (empty($meta_key)) {
        ...
        $keys = !empty($all_meta) ? array_keys($all_meta) : [];
...
function bp_activity_spam_all_user_data($user_id = 0)
{
    ...
    $wpdb->query(
        $wpdb->prepare(
            "UPDATE {$bp->activity->table_name} SET is_spam = 1 WHERE user_id = %d",
            $user_id
        )
    );
...
function bp_activity_ham_all_user_data($user_id = 0)
{
    ...
    $wpdb->query(
        $wpdb->prepare(
            "UPDATE {$bp->activity->table_name} SET is_spam = 0 WHERE user_id = %d",
            $user_id
        )
    );
...
function bp_activity_register_activity_actions()
{
    ...
    bp_activity_set_action(
        ...
        ['activity', 'group', 'member', 'member_groups']
    );
...
function bp_activity_generate_action_string($activity)
{
    ...
    if (
        empty(
            $actions->{$activity->component}->{$activity->type}[
                'format_callback'
            ]
        )
    ) {
...
function bp_activity_format_activity_action_custom_post_type_post(
    ...
    if (
        empty($activity->type) ||
        empty($bp->activity->track[$activity->type])
    ) {
        return $action;
    }
    ...
    $post_link =
        ...
        esc_html_x(
            'item',
            'Default text for the post type name',
            'buddypress'
        ) .
        '</a>';
    ...
    if (is_multisite()) {
        $blog_link =
            ...
            esc_url($blog_url) .
            '">' .
            esc_html(get_blog_option($activity->item_id, 'blogname')) .
            '</a>';

        if (
            !empty(
                $bp->activity->track[$activity->type]->new_post_type_action_ms
            )
        ) {
            $action = sprintf(
                $bp->activity->track[$activity->type]->new_post_type_action_ms,
        ...
        if (
            !empty($bp->activity->track[$activity->type]->new_post_type_action)
        ) {
            $action = sprintf(
                $bp->activity->track[$activity->type]->new_post_type_action,
                $user_link,
                $post_url
            );
        } else {
...
function bp_activity_format_activity_action_custom_post_type_comment(
    ...
    if (is_multisite()) {
        $blog_link =
            '<a href="' .
            esc_url(get_home_url($activity->item_id)) .
            '">' .
        ...
        if (
            !empty(
                $bp->activity->track[$activity->type]
            ...
            $action = sprintf(
                esc_html_x(
                    '%1$s commented on the %2$s, on the site %3$s',
                    'Activity Custom Post Type comment action',
                    'buddypress'
                ),
                $user_link,
                $post_link,
        ...
        if (
            ...
            $action = sprintf(
                esc_html_x(
                    '%1$s commented on the %2$s',
                    'Activity Custom Post Type post comment action',
                    'buddypress'
                ),
                $user_link,
                $post_link
...
function bp_activity_get($args = '')
{
     ...
     */
    return apply_filters_ref_array('bp_activity_get', [&$activity, &$r]);
}
...
function bp_activity_get_specific($args = '')
{
    ...
    return apply_filters(
        ...
        BP_Activity_Activity::get($get_args),
        $args,
        $get_args
    );
}
...
function bp_activity_post_update($args = '')
{
     ...
     */
    $add_primary_link = apply_filters(
     ...
     */
    $activity_content = apply_filters(
    ...
    bp_update_user_meta(bp_loggedin_user_id(), 'bp_latest_update', [
        'id' => $activity_id,
...
function bp_activity_post_type_publish($post_id = 0, $post = null, $user_id = 0)
{
    ...
    $activity_post_object = bp_activity_get_post_type_tracking_args(
        $post->post_type
    );
    ...
    if (
        ...
        empty($activity_post_object->action_id)
    ) {
        return;
    }
    ...
    if ('blogs' == $activity_post_object->component_id) {
        ...
        $activity_primary_link = apply_filters(
    ...
    if (!empty($activity_args['content'])) {
        // Create the excerpt.
        $activity_summary = bp_activity_create_summary(
        ...
        if ('blogs' == $activity_post_object->component_id) {
            $activity_args['content'] = apply_filters(
    ...
    $action_args = array_merge($activity_args, [
    ...
    $activity_args[
        'action'
    ] = call_user_func_array($activity_post_object->format_callback, [
        '',
        (object) $action_args,
    ]);
    ...
    if (empty($activity_args['action'])) {
        ...
        if ('blogs' == $activity_post_object->component_id) {
            $activity_args['action'] = apply_filters(
                'bp_blogs_record_activity_action',
                $activity_args['action']
            );
        }
    }
 ...
 * @return null|WP_Error|bool|int The ID of the activity on success. False on error.
 */
function bp_activity_post_type_comment(
    $comment_id = 0,
    $is_approved = true,
    $activity_post_object = null
) {
     ...
     */
    $is_post_status_not_allowed = (bool) apply_filters(
        'bp_activity_post_type_is_post_status_allowed',
        'publish' !== $post_type_comment->post->post_status ||
            !empty($post_type_comment->post->post_password)
    );
    ...
    if ('blogs' == $activity_comment_object->component_id) {
        $activity_content = apply_filters_ref_array(
        ...
        $activity_primary_link = apply_filters_ref_array(
            'bp_blogs_activity_new_comment_primary_link',
            [$comment_link, &$post_type_comment]
        );
    } else {
    ...
    if (bp_disable_blogforum_comments()) {
        ...
        $activity_args[
            'action'
        ] = call_user_func_array($activity_comment_object->format_callback, [
            '',
            (object) $action_args,
        ]);
     ...
     * @param object     $activity_post_object the post type tracking args object.
     */
    do_action_ref_array('bp_activity_post_type_comment', [
        &$activity_id,
        $post_type_comment,
        $activity_args,
        $activity_post_object,
 ...
 */
function bp_activity_post_type_remove_comment(
    $comment_id = 0,
    $activity_post_object = null
) {
    if (empty($activity_post_object)) {
...
function bp_activity_new_comment($args = '')
{
    ...
    if (empty($activity->date_recorded)) {
        $error = new WP_Error(
            'missing_activity',
            __('The item you were replying to no longer exists.', 'buddypress')
        );
...
function bp_activity_delete($args = '')
{
    ...
    if (!empty($latest_update)) {
        if (
            in_array((int) $latest_update['id'], (array) $activity_ids_deleted)
        ) {
            bp_delete_user_meta($user_id, 'bp_latest_update');
        }
    }
...
function bp_activity_delete_comment($activity_id, $comment_id)
{
     ...
     */
    if (
        !apply_filters_ref_array('bp_activity_delete_comment_pre', [
            true,
            $activity_id,
            $comment_id,
            &$deleted,
    ...
    if (
        !bp_activity_delete(['id' => $comment_id, 'type' => 'activity_comment'])
    ) {
        return false;
    } else {
        $deleted = true;
...
function bp_activity_get_permalink($activity_id, $activity_obj = false)
{
    ...
    if (!empty($bp->activity->track)) {
        $use_primary_links = array_merge(
            $use_primary_links,
            array_keys($bp->activity->track)
        );
    }
     ...
     */
    return apply_filters_ref_array('bp_activity_get_permalink', [
        $link,
        &$activity_obj,
    ]);
}
    ...
    if (
        ...
        if ($group) {
            ...
            if (bp_loggedin_user_id() === $user_id) {
                ...
                if (
                    !groups_is_user_member($user_id, $activity->item_id) ||
 ...
 *                         single thumb.
 */
function bp_activity_thumbnail_content_images(
    $content,
    $link = false,
    $args = false
) {
    ...
    if (!empty($matches) && !empty($matches[0])) {
        ...
        preg_match(
            '/<img.*?(height\=[\'|"]{0,1}.*?[\'|"]{0,1})[\s|>]{1}/i',
            $matches[0][0],
            $height
        );
        preg_match(
            '/<img.*?(width\=[\'|"]{0,1}.*?[\'|"]{0,1})[\s|>]{1}/i',
            $matches[0][0],
            $width
        );
        ...
        if (!empty($src)) {
            ...
            if (isset($width[1])) {
                $width = substr(
                    substr(str_replace('width=', '', $width[1]), 0, -1),
                    1
                );
            }
            ...
            if (isset($height[1])) {
                $height = substr(
                    substr(str_replace('height=', '', $height[1]), 0, -1),
                    1
                );
            }
...
{
    $args = [
        'width' => isset($GLOBALS['content_width'])
    ...
    $has_feat_image =
        !empty($media['has']['featured_images']) &&
        $media['has']['featured_images'];
    $has_galleries =
        !empty($media['has']['galleries']) && $media['has']['galleries'];
    ...
    if ($use_media_type) {
        if ($use_media_type === 'images') {
            $extracted_media = wp_list_filter($media[$use_media_type], [
                'source' => $image_source,
            ]);
    ...
    $summary = bp_create_excerpt(html_entity_decode($content), 225, [
        'html' => false,
    ...
    if ($use_media_type === 'embeds') {
        ...
        $extracted_media_url = isset($extracted_media['url'])
            ? $extracted_media['url']
            : '';
        $summary .= sprintf(' <img src="%s">', esc_url($extracted_media_url));
    } elseif (in_array($use_media_type, ['audio', 'videos'], true)) {
...
function bp_activity_mark_as_spam(&$activity, $source = 'by_a_person')
{
    ...
    if ('by_a_person' == $source && !empty($bp->activity->akismet)) {
        remove_action(
            ...
            [$bp->activity->akismet, 'check_activity'],
            4
        );
        ...
        add_action(
            ...
            [$bp->activity->akismet, 'update_activity_spam_meta'],
            1,
            1
        );
    }
...
function bp_activity_mark_as_ham(&$activity, $source = 'by_a_person')
{
    ...
    if ('by_a_person' == $source && !empty($bp->activity->akismet)) {
        ...
        add_action(
            'bp_activity_after_save',
            [$bp->activity->akismet, 'update_activity_ham_meta'],
            1,
            1
        );
    }
...
{
    $notifications = BP_Core_Notification::get_all_for_user(
        $receiver_user_id,
        'all'
    );
    ...
    if (
        ...
        $args = [
            'tokens' => [
                ...
                'unsubscribe' => esc_url(
                    bp_email_get_unsubscribe_link($unsubscribe_args)
                ),
            ],
        ];
 ...
 */
function bp_activity_new_comment_notification(
    $comment_id = 0,
    $commenter_id = 0,
    $params = []
) {
    ...
    if (
        ...
        $params['activity_id'] == $params['parent_id']
    ) {
        return;
    }
    ...
    if (
        $parent_comment->user_id != $commenter_id &&
        $original_activity->user_id != $parent_comment->user_id
    ) {
        // Send an email if the user hasn't opted-out.
        if (
            'no' !=
...
function bp_embed_activity_save_cache($cache, $cachekey, $id)
{
    ...
    if (true === isset(buddypress()->activity->oembed_response)) {
        ...
        bp_activity_update_meta(
            $id,
            $cachekey,
            buddypress()->activity->oembed_response
        );
    }
}
 ...
 */
function bp_activity_catch_transition_post_type_status(
    $new_status,
    $old_status,
    $post
) {
 ...
 */
function bp_activity_transition_post_type_comment_status(
    $new_status,
    $old_status,
    $comment
) {
    ...
    if (in_array($new_status, ['delete', 'hold'])) {
        return bp_activity_post_type_remove_comment(
            $comment->comment_ID,
            $activity_post_object
        );

        // These clauses handle trash, spam, and un-spams.
    } elseif (in_array($new_status, ['trash', 'spam', 'unapproved'])) {
    ...
    $comment_akismet_history = function ($activity_types) use (
...
function bp_activity_personal_data_exporter($email_address, $page)
{
    ...
    foreach ($activities['activities'] as $activity) {
        if (
            !empty(
                $activity_actions->{$activity->component}->{$activity->type}[
                    'format_callback'
                ]
            )
        ) {
