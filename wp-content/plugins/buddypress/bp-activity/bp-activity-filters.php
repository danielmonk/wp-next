...
{
    $types = ['activity_comment', 'activity_update'];
...
function bp_activity_check_moderation_keys($activity)
{
    ...
    if (
        !in_array($activity->type, bp_activity_get_moderated_activity_types())
    ) {
        return;
    }
    ...
    $moderate = bp_core_check_for_moderation(
        $activity->user_id,
        '',
        $activity->content,
        'wp_error'
...
function bp_activity_check_disallowed_keys($activity)
{
    ...
    $disallowed = bp_core_check_for_disallowed_keys(
        $activity->user_id,
        '',
        $activity->content,
        'wp_error'
...
function bp_activity_filter_kses($content)
{
     ...
     */
    $activity_allowedtags = apply_filters(
...
function bp_activity_at_name_filter($content, $activity_id = 0)
{
    ...
    foreach ((array) $usernames as $user_id => $username) {
        $content = preg_replace(
            '/(@' . $username . '\b)/',
            "<a class='bp-suggestions-mention' href='" .
                bp_core_get_user_domain($user_id) .
                "' rel='nofollow'>@$username</a>",
            $content
        );
    }
 ...
 */
function bp_activity_truncate_entry($text, $args = [])
{
    ...
    if (
        empty($args['force_truncate']) &&
        (!$maybe_truncate_text || bp_is_single_activity())
    ) {
        return $text;
    }
     ...
     */
    $append_text = apply_filters(
        'bp_activity_excerpt_append_text',
        __('[Read more]', 'buddypress')
    );

    $excerpt_length = bp_activity_get_excerpt_length();

    $args = wp_parse_args($args, ['ending' => __('&hellip;', 'buddypress')]);
    ...
    if (strlen($excerpt) < strlen(strip_shortcodes($text))) {
        $id = !empty($activities_template->activity->current_comment->id)
            ? 'acomment-read-more-' .
                $activities_template->activity->current_comment->id
            : 'activity-read-more-' . bp_get_activity_id();
 ...
 */
function bp_activity_get_js_dependencies($js_handles = [])
{
...
function bp_activity_newest_class($classes = '')
{
    ...
    if (
        !empty($bp->activity->last_recorded) &&
        $bp->activity->last_recorded == bp_get_activity_date_recorded()
    ) {
        $classes .= ' new-update';
    }
 ...
 */
function bp_activity_heartbeat_last_recorded($response = [], $data = [])
{
    ...
    $activity_latest_args = bp_parse_args(
        ...
        ['since' => date('Y-m-d H:i:s', $data['bp_activity_last_recorded'])],
        'activity_latest_args'
    );

    if (
        !empty($data['bp_activity_last_recorded_search_terms']) &&
        empty($activity_latest_args['search_terms'])
    ) {
        $activity_latest_args['search_terms'] = addslashes(
 ...
 */
function bp_activity_heartbeat_strings($strings = [])
{
    if (!bp_activity_do_heartbeat()) {
        return $strings;
    }
     ...
     */
    $heartbeat_settings = apply_filters('heartbeat_settings', []);
    if (!empty($heartbeat_settings['interval'])) {
        // 'Fast' is 5.
        $global_pulse = is_numeric($heartbeat_settings['interval'])
            ? absint($heartbeat_settings['interval'])
            : 5;
    }
...
add_filter(
    'bp_activity_set_just-me_scope_args',
...
add_filter(
    'bp_activity_set_favorites_scope_args',
...
add_filter(
    'bp_activity_set_mentions_scope_args',
...
{
    $exporters['buddypress-activity'] = [
        'exporter_friendly_name' => __(
            'BuddyPress Activity Data',
            'buddypress'
        ),
        'callback' => 'bp_activity_personal_data_exporter',
        'exporter_bp_friendly_name' => _x(
            'Activity Data',
            'BuddyPress Activity data exporter friendly name',
            'buddypress'
        ),
    ];
