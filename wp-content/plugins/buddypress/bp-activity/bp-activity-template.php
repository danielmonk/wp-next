...
function bp_get_activity_directory_permalink()
{
    ...
    return apply_filters(
        ...
        trailingslashit(
            bp_get_root_domain() . '/' . bp_get_activity_root_slug()
        )
    );
}
...
function bp_has_activities($args = '')
{
    ...
    if (bp_is_group()) {
        ...
        $show_hidden =
            (bool) (groups_is_user_member(bp_loggedin_user_id(), $primary_id) ||
    ...
    if (in_array($r['display_comments'], [0, '0', 'none', 'false'], true)) {
    ...
    if (
        isset($_GET['afilter']) &&
        apply_filters('bp_activity_enable_afilter_support', false)
    ) {
        $r['filter'] = [
            'object' => $_GET['afilter'],
        ];
...
function bp_get_activity_load_more_link()
{
    ...
    $link = add_query_arg(
        $activities_template->pag_arg,
        $activities_template->pag_page + 1,
        $url
    );
...
function bp_get_activity_pagination_count()
{
    ...
    $start_num =
        intval(
            ($activities_template->pag_page - 1) * $activities_template->pag_num
        ) + 1;
    $from_num = bp_core_number_format($start_num);
    $to_num = bp_core_number_format(
        $start_num + ($activities_template->pag_num - 1) >
...
function bp_activity_has_more_items()
{
    ...
    if (!empty($activities_template->has_more_items)) {
        ...
        if (!empty($activities_template->pag_page)) {
            $remaining_pages = floor(
                ($activities_template->total_activity_count - 1) /
                    ($activities_template->pag_num *
                        $activities_template->pag_page)
            );
        }
...
function bp_get_activity_count()
{
    ...
    return apply_filters(
        ...
        (int) $activities_template->activity_count
    );
}
...
function bp_get_activity_item_id()
{
    ...
    return apply_filters(
        ...
        $activities_template->activity->item_id
    );
}
...
function bp_get_activity_object_name()
{
    ...
    return apply_filters(
        ...
        $activities_template->activity->component
    );
}
...
function bp_get_activity_type()
{
    ...
    return apply_filters(
        ...
        $activities_template->activity->type
    );
}
...
function bp_get_activity_user_id()
{
    ...
    return apply_filters(
        ...
        $activities_template->activity->user_id
    );
}
...
function bp_get_activity_user_link()
{
    ...
    if (
        empty($activities_template->activity->user_id) ||
        empty($activities_template->activity->user_nicename) ||
        empty($activities_template->activity->user_login)
    ) {
        $link = $activities_template->activity->primary_link;
    } else {
        $link = bp_core_get_user_domain(
            $activities_template->activity->user_id,
            $activities_template->activity->user_nicename,
            $activities_template->activity->user_login
        );
    }
...
function bp_get_activity_avatar($args = '')
{
    ...
    $dn_default = isset($current_activity_item->display_name)
        ? $current_activity_item->display_name
        : '';
    ...
    if (!isset($height) && !isset($width)) {
        // Backpat.
        if (
            isset($bp->avatar->full->height) ||
            isset($bp->avatar->thumb->height)
        ) {
            $height =
                'full' == $type
                    ? $bp->avatar->full->height
                    : $bp->avatar->thumb->height;
        } else {
            $height = 20;
        }
        ...
        // Backpat.
        if (
            isset($bp->avatar->full->width) ||
            isset($bp->avatar->thumb->width)
        ) {
            $width =
                'full' == $type
                    ? $bp->avatar->full->width
    ...
    if (
        ...
        empty($user_id) &&
        empty($email) &&
        isset($current_activity_item->user_email)
    ) {
...
function bp_get_activity_secondary_avatar($args = '')
{
    ...
    switch ($activities_template->activity->component) {
        ...
        case 'blogs':
            ...
            if (empty($alt)) {
                ...
                $alt = sprintf(
                    __(
                        'Profile picture of the author of the site %s',
                        'buddypress'
                    ),
                    get_blog_option($item_id, 'blogname')
                );
            }
     ...
     */
    $object = apply_filters(
        'bp_get_activity_secondary_avatar_object_' .
            $activities_template->activity->component,
        $object
    );
 ...
 */
function bp_activity_action($args = [])
{
 ...
 */
function bp_get_activity_action($args = [])
{
    global $activities_template;
     ...
     */
    $action = apply_filters_ref_array('bp_get_activity_action_pre_meta', [
     ...
     */
    return apply_filters_ref_array('bp_get_activity_action', [
        $action,
        &$activities_template->activity,
        $r,
    ]);
...
function bp_get_activity_content_body()
{
    ...
    if (
        empty($activities_template->activity->action) &&
        !empty($activities_template->activity->content)
    ) {
        $activities_template->activity->content = bp_insert_activity_meta(
     ...
     */
    return apply_filters_ref_array('bp_get_activity_content_body', [
        $activities_template->activity->content,
        &$activities_template->activity,
    ]);
}
...
function bp_insert_activity_meta($content = '')
{
    ...
    $date_recorded = bp_core_time_since(
        $activities_template->activity->date_recorded
    );
     ...
     */
    $time_since = apply_filters_ref_array('bp_activity_time_since', [
        $time_since,
        &$activities_template->activity,
    ]);
    ...
    if (!bp_is_single_activity()) {
        ...
        $activity_permalink = bp_activity_get_permalink(
            $activities_template->activity->id,
            $activities_template->activity
        );
         ...
         */
        $new_content = apply_filters_ref_array('bp_activity_permalink', [
            $activity_meta,
            &$activities_template->activity,
        ]);
    } else {
...
function bp_activity_user_can_delete($activity = false)
{
    ...
    if (is_user_logged_in()) {
        ...
        if (
            isset($activity->user_id) &&
            $activity->user_id === bp_loggedin_user_id()
        ) {
            $can_delete = true;
        }
...
function bp_get_activity_parent_content($args = '')
{
    ...
    if (
        empty($activities_template->activity) ||
    ...
    if (!empty($activities_template->activity_parents[$parent_id]->content)) {
        $content .=
            ' ' . $activities_template->activity_parents[$parent_id]->content;
    }
...
function bp_get_activity_is_favorite()
{
    ...
    return (bool) apply_filters(
        ...
        in_array(
            $activities_template->activity->id,
            (array) $activities_template->my_favs
        )
    );
}
...
function bp_get_activity_comment_user_id()
{
    ...
    $user_id = isset($activities_template->activity->current_comment->user_id)
        ? $activities_template->activity->current_comment->user_id
        : false;
...
function bp_get_activity_comment_name()
{
    ...
    if (isset($activities_template->activity->current_comment->user_fullname)) {
        $name = apply_filters(
            'bp_acomment_name',
            $activities_template->activity->current_comment->user_fullname,
            $activities_template->activity->current_comment
...
function bp_get_activity_comment_date_recorded()
{
    ...
    return apply_filters(
        ...
        bp_core_time_since(bp_get_activity_comment_date_recorded_raw())
    );
}
    ...
    if (!empty($deprecated)) {
        _deprecated_argument(
            ...
            sprintf(
                ...
                __(
                    '%1$s no longer accepts arguments. See the inline documentation at %2$s for more details.',
                    'buddypress'
                ),
                __FUNCTION__,
                __FILE__
...
function bp_activity_recurse_comment_count($comment, $count = 0)
{
    ...
    if (!empty($comment->children)) {
        foreach ((array) $comment->children as $comment) {
            $new_count++;
            $new_count = bp_activity_recurse_comment_count(
                $comment,
                $new_count
            );
        }
...
function bp_activity_get_comment_depth($comment = 0)
{
    ...
    if (
        isset($GLOBALS['activities_template']->activity->current_comment->depth)
    ) {
        $depth =
            $GLOBALS['activities_template']->activity->current_comment->depth;
        ...
        if (
            is_object($comment) &&
            isset($comment->type) &&
            'activity_comment' === $comment->type
        ) {
            // Fetch the entire root comment tree... ugh.
            $comments = BP_Activity_Activity::get_activity_comments(
                $comment->item_id,
                1,
...
function bp_get_activity_comment_form_nojs_display()
{
    ...
    if (
        isset($_GET['ac']) &&
        $_GET['ac'] === $activities_template->activity->id . '/'
    ) {
...
function bp_get_activity_comment_form_action()
{
    ...
    return apply_filters(
        ...
        home_url(bp_get_activity_root_slug() . '/reply/')
    );
}
...
function bp_get_activity_thread_permalink()
{
    ...
    $link = bp_activity_get_permalink(
        $activities_template->activity->id,
        $activities_template->activity
    );
...
function bp_get_activity_delete_link()
{
    ...
    $link =
        '<a href="' .
        esc_url($url) .
        '" class="button item-button bp-secondary-action ' .
        $class .
        ' confirm" rel="nofollow">' .
...
function bp_get_activity_filter_links($args = false)
{
    ...
    foreach ((array) $components as $component) {
        ...
        switch ($r['style']) {
            case 'list':
                $tag = 'li';
                $before =
                    '<li id="afilter-' . $component . '"' . $selected . '>';
                $after = '</li>';
                break;
            case 'paragraph':
                $tag = 'p';
                $before =
                    '<p id="afilter-' . $component . '"' . $selected . '>';
                $after = '</p>';
                break;
            case 'span':
                $tag = 'span';
                $before =
                    '<span id="afilter-' . $component . '"' . $selected . '>';
                $after = '</span>';
                break;
        }
    ...
    if (isset($_GET['afilter'])) {
        $component_links[] =
            '<' .
            $tag .
            ' id="afilter-clear"><a href="' .
            esc_url($link) .
            '">' .
            __('Clear Filter', 'buddypress') .
            '</a></' .
            $tag .
            '>';
    }
...
function bp_activity_can_comment_reply($comment = false)
{
    ...
    if (!empty($comment)) {
        ...
        if (get_option('thread_comments')) {
            $can_comment =
                (bool) ($comment_depth < get_option('thread_comments_depth'));
...
function bp_get_send_public_message_link()
{
    ...
    if (!is_user_logged_in() || !bp_is_user() || bp_is_my_profile()) {
        ...
        $args = ['r' => bp_get_displayed_user_mentionname()];
 ...
 * @return array
 */
function bp_activity_recurse_comments_activity_ids(
    $activity = [],
    $activity_ids = []
) {
    ...
    if (!empty($activity->children)) {
        foreach ($activity->children as $child) {
            ...
            if (!empty($child->children)) {
                $activity_ids = bp_activity_recurse_comments_activity_ids(
                    $child,
                    $activity_ids
                );
            }
...
function bp_get_send_public_message_button($args = '')
{
    ...
    return bp_get_button(
        apply_filters('bp_get_send_public_message_button', $r)
    );
}
...
function bp_get_activity_post_form_action()
{
    ...
    return apply_filters(
        ...
        home_url(bp_get_activity_root_slug() . '/post/')
    );
}
 ...
 */
function bp_activity_comments_user_avatars($args = [])
{
    $r = bp_parse_args($args, [
        'height' => false,
        'html' => true,
...
function bp_activity_get_comments_user_ids()
{
    ...
    $user_ids = !empty($activities_template->activity->children)
        ? (array) bp_activity_recurse_comments_user_ids(
            $activities_template->activity->children
        )
        : [];
        ...
        foreach ($comments as $comment) {
            ...
            if (!empty($comment->children)) {
                $user_ids = array_merge(
                    $user_ids,
                    bp_activity_recurse_comments_user_ids($comment->children)
                );
            }
        }
    }
...
function bp_get_displayed_user_mentionname()
{
    ...
    return apply_filters(
        ...
        bp_activity_get_user_mentionname(bp_displayed_user_id())
    );
}
...
function bp_activity_types_list($output = 'select', $args = '')
{
    ...
    foreach ($activities as $type => $description) {
        ...
        $checked = checked(
            true,
            in_array($type, (array) $args['selected']),
            false
        );
        $selected = selected(
            true,
        ...
        switch ($output) {
            ...
            case 'checkbox':
                printf(
                    '<label style="" for="%1$s[]">%2$s<input type="checkbox" id="%1$s[]" name="%1$s[]" value="%3$s" %4$s/></label>',
                    esc_attr($args['checkbox_name']),
                    esc_html($description),
                    esc_attr($args['checkbox_name']),
                    esc_attr($args['checkbox_name']),
                    esc_attr($type),
