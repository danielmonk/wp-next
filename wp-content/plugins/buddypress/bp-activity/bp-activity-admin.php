 ...
 */
function bp_activity_admin_menu_order($custom_menus = [])
{
...
function bp_activity_admin_reply()
{
    ...
    $parent_id = !empty($_REQUEST['parent_id'])
        ? (int) $_REQUEST['parent_id']
        : 0;
...
{
    if (
        'toplevel_page_bp_activity_per_page' != $option &&
        'toplevel_page_bp_activity_network_per_page' != $option
    ) {
        return $value;
    }
...
function bp_activity_admin_edit_hidden_metaboxes($hidden, $screen)
{
    if (
        empty($screen->id) ||
        ('toplevel_page_bp-activity' !== $screen->id &&
            'toplevel_page_bp-activity-network' !== $screen->id)
    ) {
        return $hidden;
    }

    // Hide the primary link meta box by default.
    $hidden = array_merge((array) $hidden, [
        'bp_activity_itemids',
        'bp_activity_link',
        'bp_activity_type',
        'bp_activity_userid',
...
function bp_activity_admin_load()
{
    ...
    if ('edit' == $doaction && !empty($_GET['aid'])) {
        ...
        get_current_screen()->add_help_tab([
            ...
            'content' =>
                '<p>' .
                __(
                    'You edit activities made on your site similar to the way you edit a comment. This is useful if you need to change which page the activity links to, or when you notice that the author has made a typographical error.',
                    'buddypress'
                ) .
                '</p>' .
                '<p>' .
                ...
                '<p>' .
                __(
                    'You can also moderate the activity from this screen using the Status box, where you can also change the timestamp of the activity.',
                    'buddypress'
                ) .
                '</p>',
        ]);
        ...
        get_current_screen()->add_help_tab([
            ...
            'content' =>
                '<p>' .
                __(
                    '<strong>Primary Item/Secondary Item</strong> - These identify the object that created the activity. For example, the fields could reference a comment left on a specific site. Some types of activity may only use one, or none, of these fields.',
                    'buddypress'
                ) .
                '</p>' .
                '<p>' .
                __(
                    '<strong>Link</strong> - Used by some types of activity (blog posts and comments) to store a link back to the original content.',
                    'buddypress'
                ) .
                '</p>' .
                '<p>' .
                __(
                    '<strong>Type</strong> - Each distinct kind of activity has its own type. For example, <code>created_group</code> is used when a group is created and <code>joined_group</code> is used when a user joins a group.',
                    'buddypress'
                ) .
                '</p>' .
                '<p>' .
                __(
                    'For information about when and how BuddyPress uses all of these settings, see the Managing Activity link in the panel to the side.',
                    'buddypress'
                ) .
                '</p>',
        ]);
        ...
        get_current_screen()->set_help_sidebar(
            '<p><strong>' .
                ...
                '<p>' .
                __(
                    '<a href="https://codex.buddypress.org/administrator-guide/activity-stream-management-panels/">Managing Activity</a>',
                    'buddypress'
                ) .
                '</p>' .
                '<p>' .
        ...
        add_meta_box(
            'submitdiv',
            _x('Status', 'activity admin edit screen', 'buddypress'),
        ...
        add_meta_box(
            'bp_activity_link',
            _x('Link', 'activity admin edit screen', 'buddypress'),
        ...
        add_meta_box(
            'bp_activity_type',
            _x('Type', 'activity admin edit screen', 'buddypress'),
        ...
        add_meta_box(
            'bp_activity_userid',
            _x('Author ID', 'activity admin edit screen', 'buddypress'),
        ...
        add_screen_option('per_page', [
            'label' => _x(
                'Activity',
                'Activity items per page (screen options)',
                'buddypress'
            ),
        ]);
        ...
        get_current_screen()->add_help_tab([
            ...
            'content' =>
                '<p>' .
                __(
                    'You can manage activities made on your site similar to the way you manage comments and other content. This screen is customizable in the same ways as other management screens, and you can act on activities using the on-hover action links or the Bulk Actions.',
                    'buddypress'
                ) .
                '</p>' .
                '<p>' .
    ...
    wp_enqueue_script(
        ...
        $bp->plugin_url . "bp-activity/admin/js/admin{$min}.js",
        ['jquery', 'wp-ajax-response'],
        bp_get_version(),
        true
    );
    wp_localize_script('bp_activity_admin_js', 'bp_activity_admin_vars', [
        'page' => get_current_screen()->id,
    ]);
    wp_enqueue_style(
        'bp_activity_admin_css',
        $bp->plugin_url . "bp-activity/admin/css/admin{$min}.css",
        [],
        bp_get_version()
    );
    ...
    if (
        !empty($doaction) &&
        !in_array($doaction, ['-1', 'edit', 'save', 'delete', 'bulk_delete'])
    ) {
        // Build redirection URL.
        $redirect_to = remove_query_arg(
            ['aid', 'deleted', 'error', 'spammed', 'unspammed'],
            wp_get_referer()
        );
        $redirect_to = add_query_arg(
            'paged',
            $bp_activity_list_table->get_pagenum(),
            $redirect_to
        );
            ...
            switch ($doaction) {
                case 'do_delete':
                    if ('activity_comment' === $activity->type) {
                        $delete_result = bp_activity_delete_comment(
                            $activity->item_id,
                            $activity->id
                        );
                    } else {
                        $delete_result = bp_activity_delete([
                            'id' => $activity->id,
                        ]);
                    }
        ...
        do_action(
            ...
            [$spammed, $unspammed, $deleted, $errors],
            $redirect_to,
            $activity_ids
        );
        ...
        if (!empty($errors)) {
            $redirect_to = add_query_arg(
                'error',
                implode(',', array_map('absint', $errors)),
                $redirect_to
            );
        }
         ...
         */
        wp_redirect(
            apply_filters('bp_activity_admin_action_redirect', $redirect_to)
        );
        exit();
        ...
        // Build redirection URL.
        $redirect_to = remove_query_arg(
            ['action', 'aid', 'deleted', 'error', 'spammed', 'unspammed'],
            $_SERVER['REQUEST_URI']
        );
        ...
        if (!empty($_POST['activity_status'])) {
            ...
            $new_spam_status =
                'spam' == $_POST['activity_status'] ? true : false;
        }
         ...
         */
        do_action_ref_array('bp_activity_admin_edit_after', [
            &$activity,
            $error,
        ]);
         ...
         */
        wp_redirect(
            apply_filters('bp_activity_admin_edit_redirect', $redirect_to)
        );
        exit();
...
function bp_activity_admin()
{
    ...
    if ('edit' === $doaction && !empty($_GET['aid'])) {
        ...
        in_array($doaction, ['bulk_delete', 'delete']) &&
        !empty($_GET['aid'])
    ) {
...
function bp_activity_admin_delete()
{
    ...
    $base_url = remove_query_arg(
        ['action', 'action2', 'paged', 's', '_wpnonce', 'aid'],
        $_SERVER['REQUEST_URI']
    );
    ?>
	...
	<div class="wrap">
      ...
      'buddypress'
		...
		<p><?php esc_html_e(
      'You are about to delete the following activities:',
      'buddypress'
  ); ?></p>
   ...
   if (isset($actions[$activity->type])) {
       ...
       $activity_type = sprintf(
           __('Unregistered action - %s', 'buddypress'),
           $activity->type
       );
   }
   ...
   ?>
      ...
      wp_nonce_url(
          add_query_arg(
              ['action' => 'do_delete', 'aid' => implode(',', $aids)],
              $base_url
          ),
          'bp-activities-delete'
      )
  ); ?>"><?php esc_html_e('Delete Permanently', 'buddypress'); ?></a>
		<a class="button" href="<?php echo esc_attr($base_url); ?>"><?php esc_html_e(
    'Cancel',
    'buddypress'
); ?></a>
	</div>
...
function bp_activity_admin_edit()
{
    ...
    $form_url = remove_query_arg(
        ['action', 'deleted', 'error', 'spammed', 'unspammed'],
        $_SERVER['REQUEST_URI']
    );
     ...
     */
    do_action_ref_array('bp_activity_admin_edit', [&$activity]);
    ?>

	<div class="wrap">
		<h1 class="wp-heading-inline">
			<?php /* translators: %s: the activity ID */
   printf(
       __('Editing Activity (ID #%s)', 'buddypress'),
       number_format_i18n((int) $_REQUEST['aid'])
   ); ?>
		</h1>
						...
						<div id="post-body-content">
							<div id="postdiv">
								<div id="bp_activity_action" class="postbox">
									...
									<div class="inside">
										<label for="bp-activities-action" class="screen-reader-text"><?php /* translators: accessibility text */
          _e('Edit activity action', 'buddypress'); ?></label>
										<?php wp_editor(stripslashes($activity->action), 'bp-activities-action', [
              'media_buttons' => false,
              'textarea_rows' => 7,
              'teeny' => true,
              'quicktags' => [
                  'buttons' =>
                      'strong,em,link,block,del,ins,img,code,spell,close',
								...
								<div id="bp_activity_content" class="postbox">
									...
									<div class="inside">
										<label for="bp-activities-content" class="screen-reader-text"><?php /* translators: accessibility text */
          _e('Edit activity content', 'buddypress'); ?></label>
										<?php wp_editor(stripslashes($activity->content), 'bp-activities-content', [
              'media_buttons' => false,
              'teeny' => true,
              'quicktags' => [
                  'buttons' =>
...
function bp_activity_admin_edit_metabox_status($item)
{
	...
	<div class="submitbox" id="submitcomment">
		...
		<div id="minor-publishing">
			<div id="minor-publishing-actions">
				<div id="preview-action">
					<a class="button preview" href="<?php echo esc_attr(
         bp_activity_get_permalink($item->id, $item)
     ); ?>" target="_blank"><?php _e('View Activity', 'buddypress'); ?></a>
				</div>
			...
			<div id="misc-publishing-actions">
				<div class="misc-pub-section" id="comment-status-radio">
					<label class="approved" for="activity-status-approved"><input type="radio" name="activity_status" id="activity-status-approved" value="ham" <?php checked(
         $item->is_spam,
         0
     ); ?>><?php _e('Approved', 'buddypress'); ?></label><br />
					<label class="spam" for="activity-status-spam"><input type="radio" name="activity_status" id="activity-status-spam" value="spam" <?php checked(
         $item->is_spam,
         1
     ); ?>><?php _e('Spam', 'buddypress'); ?></label>
				</div>
     ...
     ?>
					<span id="timestamp">
						<?php /* translators: %s: the date the activity was submitted on */
      printf(
          __('Submitted on: %s', 'buddypress'),
          '<strong>' . $date . '</strong>'
      ); ?>
					</span>&nbsp;<a href="#edit_timestamp" class="edit-timestamp hide-if-no-js" tabindex='4'><?php _e(
         'Edit',
         'buddypress'
     ); ?></a>
...
function bp_activity_admin_edit_metabox_link($item)
{
	...
	<label class="screen-reader-text" for="bp-activities-link"><?php /* translators: accessibility text */
 _e('Link', 'buddypress'); ?></label>
	<input type="url" name="bp-activities-link" id="bp-activities-link" value="<?php echo esc_url(
     $item->primary_link
 ); ?>" aria-describedby="bp-activities-link-description" />
	<p id="bp-activities-link-description"><?php _e(
     'Activity generated by posts and comments uses the link field for a permalink back to the content item.',
     'buddypress'
 ); ?></p>
...
function bp_activity_admin_edit_metabox_userid($item)
{
	...
	<label class="screen-reader-text" for="bp-activities-userid"><?php /* translators: accessibility text */
 _e('Author ID', 'buddypress'); ?></label>
	<input type="number" name="bp-activities-userid" id="bp-activities-userid" value="<?php echo esc_attr(
     $item->user_id
 ); ?>" min="1" />
...
function bp_activity_admin_get_activity_actions()
{
    ...
    foreach (bp_activity_get_actions() as $action) {
        ...
        for ($i = 0, $i_count = count($action); $i < $i_count; $i++) {
             ...
             */
            if (
                'friends_register_activity_action' === $action[$i]['key'] ||
                'bp_groups_format_activity_action_group_activity_update' ===
                    $action[$i]['format_callback']
            ) {
                continue;
            }
...
function bp_activity_admin_edit_metabox_type($item)
{
	...
	<label for="bp-activities-type" class="screen-reader-text"><?php /* translators: accessibility text */
 esc_html_e('Select activity type', 'buddypress'); ?></label>
...
function bp_activity_admin_edit_metabox_itemids($item)
{
	...
	<label for="bp-activities-primaryid"><?php _e(
     'Primary Item ID',
     'buddypress'
 ); ?></label>
	<input type="number" name="bp-activities-primaryid" id="bp-activities-primaryid" value="<?php echo esc_attr(
     $item->item_id
 ); ?>" min="0" />
	<br />

	<label for="bp-activities-secondaryid"><?php _e(
     'Secondary Item ID',
     'buddypress'
 ); ?></label>
	<input type="number" name="bp-activities-secondaryid" id="bp-activities-secondaryid" value="<?php echo esc_attr(
     $item->secondary_item_id
 ); ?>" min="0" />

	<p><?php _e(
     'These identify the object that created this activity. For example, the fields could reference a pair of site and comment IDs.',
     'buddypress'
 ); ?></p>
...
function bp_activity_admin_index()
{
    ...
    if (
        !empty($_REQUEST['deleted']) ||
        !empty($_REQUEST['spammed']) ||
        ...
        $spammed = !empty($_REQUEST['spammed'])
            ? (int) $_REQUEST['spammed']
            : 0;
        $unspammed = !empty($_REQUEST['unspammed'])
            ? (int) $_REQUEST['unspammed']
            : 0;
        $updated = !empty($_REQUEST['updated'])
            ? (int) $_REQUEST['updated']
            : 0;
        ...
        if ($deleted > 0) {
            ...
            $messages[] = sprintf(
                _n(
                    '%s activity item has been permanently deleted.',
                    '%s activity items have been permanently deleted.',
                    $deleted,
                    'buddypress'
                ),
        ...
        if (!empty($errors)) {
            if (1 == count($errors)) {
                ...
                $messages[] = sprintf(
                    __(
                        'An error occurred when trying to update activity ID #%s.',
                        'buddypress'
                    ),
                    number_format_i18n($errors[0])
                );
            } else {
                $error_msg = __(
                    'Errors occurred when trying to update these activity items:',
                    'buddypress'
                );
        ...
        if ($spammed > 0) {
            ...
            $messages[] = sprintf(
                _n(
                    '%s activity item has been successfully spammed.',
                    '%s activity items have been successfully spammed.',
                    $spammed,
                    'buddypress'
                ),
                number_format_i18n($spammed)
            );
        }
        ...
        if ($unspammed > 0) {
            ...
            $messages[] = sprintf(
                ...
                number_format_i18n($unspammed)
            );
        }

        if ($updated > 0) {
            $messages[] = __(
                'The activity item has been updated successfully.',
                'buddypress'
            );
        }
    }
	...
	<div class="wrap">
		<h1 class="wp-heading-inline">
			<?php if (!empty($_REQUEST['aid'])): ?>
				<?php /* translators: %s: the activity ID */
    printf(
        __('Activity related to ID #%s', 'buddypress'),
        number_format_i18n((int) $_REQUEST['aid'])
    ); ?>
			<?php else: ?>
			...
			<?php if (!empty($_REQUEST['s'])): ?>
				<span class="subtitle">
					<?php /* translators: %s: the activity search terms */
     printf(
    ...
    ?>
		...
		<form id="bp-activities-form" action="" method="get">
			<?php $bp_activity_list_table->search_box(
       __('Search all Activity', 'buddypress'),
       'bp-activity'
   ); ?>
			...
			<tr id="bp-activities-container" style="display: none;">
				<td colspan="4">
					<form method="get" action="">
						...
						<label for="bp-activities" class="screen-reader-text"><?php /* translators: accessibility text */
      _e('Reply', 'buddypress'); ?></label>
						<?php wp_editor('', 'bp-activities', [
          'dfw' => false,
          'media_buttons' => false,
          'quicktags' => [
          ...
          'tinymce' => false,
						...
						<p id="bp-replysubmit" class="submit">
							<a href="#" class="cancel button-secondary alignleft"><?php _e(
           'Cancel',
           'buddypress'
       ); ?></a>
							<a href="#" class="save button-primary alignright"><?php _e(
           'Reply',
           'buddypress'
       ); ?></a>
