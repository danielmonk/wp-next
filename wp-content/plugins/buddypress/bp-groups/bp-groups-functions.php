...
function groups_create_group($args = '')
{
    ...
    if (!empty($group_id)) {
        ...
        $enable_forum = !is_null($enable_forum)
            ? $enable_forum
            : $group->enable_forum;
        $date_created = !is_null($date_created)
            ? $date_created
            : $group->date_created;
 ...
 */
function groups_edit_base_group_details($args = [])
{
 ...
 * @return bool True on success, false on failure.
 */
function groups_edit_group_settings(
    $group_id,
    $enable_forum,
    $status,
    $invite_status = false,
...
function groups_leave_group($group_id, $user_id = 0)
{
    ...
    if (count(groups_get_group_admins($group_id)) < 2) {
        if (groups_is_user_admin($user_id, $group_id)) {
            bp_core_add_message(
                __(
                    'As the only admin, you cannot leave the group.',
                    'buddypress'
                ),
                'error'
            );
 ...
 */
function groups_get_group_members($args = [])
{
    $function_args = func_get_args();
    ...
    if (
        ...
        $retval = BP_Groups_Member::get_all_for_group(
            $r['group_id'],
            $r['per_page'],
            $r['page'],
...
function groups_get_groups($args = '')
{
    $defaults = [
        ...
        'slug' => [], // Find a group or groups by slug.
        'search_terms' => false, // Limit to groups that match these search terms.
        'search_columns' => [], // Select which columns to search.
        ...
        'status' => [], // Array or comma-separated list of group statuses to limit results to.
     ...
     *                                Passed by reference.
     */
    return apply_filters_ref_array('groups_get_groups', [&$groups, &$r]);
}
 ...
 */
function bp_get_user_groups($user_id, $args = [])
{
    $r = bp_parse_args(
        $args,
        [
    ...
    if (false === $membership_ids) {
        $membership_ids = BP_Groups_Member::get_membership_ids_for_user(
            $user_id
        );
        wp_cache_set(
            $user_id,
    ...
    $uncached_membership_ids = bp_get_non_cached_ids(
        $membership_ids,
        'bp_groups_memberships'
    );
    if (!empty($uncached_membership_ids)) {
        $uncached_memberships = BP_Groups_Member::get_memberships_by_id(
            $uncached_membership_ids
        );

        foreach ($uncached_memberships as $uncached_membership) {
            wp_cache_set(
                $uncached_membership->id,
                $uncached_membership,
                'bp_groups_memberships'
            );
        }
    ...
    if (true !== $r['is_confirmed'] || false !== $r['invite_sent']) {
        ...
        $uncached_invitation_ids = bp_get_non_cached_ids(
            $invitation_ids,
        ...
        if ($uncached_invitation_ids) {
            ...
            foreach ($uncached_invitations as $uncached_invitation) {
                ...
                $invitation->date_modified =
                    $uncached_invitation->date_modified;
    ...
    $filters = wp_array_slice_assoc($r, [
        'is_confirmed',
        'is_banned',
        'is_admin',
        'is_mod',
    ...
    $groups = [];
    $int_keys = ['id', 'group_id', 'user_id', 'inviter_id'];
    $bool_keys = [
        'is_admin',
        'is_mod',
        'is_confirmed',
    ...
    foreach ($invitation_ids as $invitation_id) {
        ...
        foreach ($filters as $filter_name => $filter_value) {
            if (
                !isset($invitation->{$filter_name}) ||
                $filter_value != $invitation->{$filter_name}
            ) {
                continue 2;
            }
        }
    ...
    if ('group_id' === $r['orderby']) {
        ksort($groups);
    } elseif (in_array($r['orderby'], ['id', 'date_modified'])) {
...
function groups_avatar_upload_dir($group_id = 0)
{
     ...
     */
    return apply_filters('groups_avatar_upload_dir', [
        'path' => $path,
        'url' => $newurl,
        'subdir' => $newsubdir,
        'basedir' => $newbdir,
 ...
 * }
 */
function groups_get_invites_for_user(
    $user_id = 0,
    $limit = false,
    $page = false,
    $exclude = false
 ...
 */
function groups_get_invites($args = [])
{
    $invites_class = new BP_Groups_Invitation_Manager();
 ...
 * @return bool True on success, false on failure.
 */
function groups_accept_membership_request(
    $membership_id,
    $user_id = 0,
    $group_id = 0
) {
 ...
 * @return bool True on success, false on failure.
 */
function groups_reject_membership_request(
    $membership_id,
    $user_id = 0,
    $group_id = 0
) {
 ...
 */
function groups_delete_membership_request(
    $membership_id,
    $user_id = 0,
    $group_id = 0
) {
 ...
 */
function groups_get_requests($args = [])
{
    $invites_class = new BP_Groups_Invitation_Manager();
 ...
 * @return bool True on success, false on failure.
 */
function groups_delete_groupmeta(
    $group_id,
    $meta_key = false,
    $meta_value = false,
    $delete_all = false
...
function bp_get_group_type_tax_labels()
{
     ...
     */
    return apply_filters('bp_get_group_type_tax_labels', [
        // General labels
        'name' => _x('Group Types', 'Group type taxonomy name', 'buddypress'),
        'singular_name' => _x(
            'Group Type',
            'Group type taxonomy singular name',
            'buddypress'
        ),
        'search_items' => _x(
            'Search Group Types',
            'Group type taxonomy search items label',
            'buddypress'
        ),
        'popular_items' => _x(
            'Popular Group Types',
            'Group type taxonomy popular items label',
            'buddypress'
        ),
        'all_items' => _x(
            'All Group Types',
            'Group type taxonomy all items label',
            'buddypress'
        ),
        'parent_item' => _x(
            'Parent Group Type',
        ...
        'edit_item' => _x(
            'Edit Group Type',
            'Group type taxonomy edit item label',
            'buddypress'
        ),
        'view_item' => _x(
            'View Group Type',
            'Group type taxonomy view item label',
            'buddypress'
        ),
        'update_item' => _x(
            'Update Group Type',
            'Group type taxonomy update item label',
            'buddypress'
        ),
        'add_new_item' => _x(
            'Add New Group Type',
            'Group type taxonomy add new item label',
            'buddypress'
        ),
        'new_item_name' => _x(
            'New Group Type Name',
            'Group type taxonomy new item name label',
            'buddypress'
        ),
        'separate_items_with_commas' => _x(
            'Separate group types with commas',
            'Group type taxonomy separate items with commas label',
            'buddypress'
        ),
        'add_or_remove_items' => _x(
            'Add or remove group types',
            'Group type taxonomy add or remove items label',
            'buddypress'
        ),
        ...
        'bp_type_id_description' => _x(
            'Lower-case string, no spaces or special characters. Used to identify the group type.',
            'BP Group type ID description',
            'buddypress'
        ),
        'bp_type_show_in_create_screen' => _x(
 ...
 */
function bp_groups_register_group_type_taxonomy($taxonomies = [])
{
 ...
 */
function bp_get_group_type_metadata_schema($schema = [], $taxonomy = '')
{
    if (bp_get_group_type_tax_name() === $taxonomy) {
        ...
        if (isset($schema['bp_type_has_directory']['description'])) {
            $schema['bp_type_has_directory']['description'] = __(
                'Make a list of groups matching this type available on the groups directory.',
                'buddypress'
            );
        }
        ...
        // Create
        $schema['bp_type_show_in_create_screen'] = [
            'description' => __(
                'Show during group creation, and when a group admin is on the group&rsquo;s settings page.',
                'buddypress'
            ),
            'type' => 'boolean',
            'single' => true,
        ...
        $schema['bp_type_show_in_list'] = [
            'description' => __(
                'Show where group types may be listed, like in the group header.',
                'buddypress'
            ),
            'type' => 'boolean',
            'single' => true,
 ...
 */
function bp_groups_register_group_type($group_type, $args = [])
{
    $bp = buddypress();

    if (isset($bp->groups->types[$group_type])) {
        return new WP_Error(
            'bp_group_type_exists',
            __('Group type already exists.', 'buddypress'),
            $group_type
        );
    }
     ...
     */
    $illegal_names = apply_filters('bp_group_type_illegal_names', [
        'any',
        'null',
        '_none',
    ]);
    if (in_array($group_type, $illegal_names, true)) {
        return new WP_Error(
            'bp_group_type_illegal_name',
            __(
                'You may not register a group type with this name.',
                'buddypress'
            ),
            $group_type
        );
 ...
 */
function bp_groups_get_group_types(
    $args = [],
    $output = 'names',
    $operator = 'and'
) {
...
add_filter(
    bp_get_group_type_tax_name() . '_registered_by_code',
 ...
 */
function bp_set_registered_by_code_group_type_metadata(
    $metadata = [],
    $type = ''
) {
...
add_filter(
    bp_get_group_type_tax_name() . '_set_registered_by_code_metada',
...
{
    $all_types = bp_groups_get_group_types([], 'objects');
    ...
    if ($unsaved_types) {
        foreach ($unsaved_types as $type_name) {
            bp_insert_term($type_name, bp_get_group_type_tax_name(), [
                'slug' => $type_name,
            ]);
        }
    }
...
add_action(
    bp_get_group_type_tax_name() . '_add_form',
...
function bp_groups_memberships_personal_data_exporter($email_address, $page)
{
    ...
    $memberships = BP_Groups_Member::get_user_memberships($user->ID, [
        'type' => 'membership',
        'page' => $page,
        'per_page' => $number,
    ]);
 ...
 * @return array An array of personal data.
 */
function bp_groups_pending_requests_personal_data_exporter(
    $email_address,
    $page
) {
    $number = 20;
 ...
 * @return array An array of personal data.
 */
function bp_groups_pending_received_invitations_personal_data_exporter(
    $email_address,
    $page
) {
    $number = 20;
...
function bp_groups_migrate_invitations()
{
    ...
    $records = $wpdb->get_results(
        "SELECT id, group_id, user_id, inviter_id, date_modified, comments, invite_sent FROM {$bp->groups->table_name_members} WHERE is_confirmed = 0 AND is_banned = 0"
    );
    if (empty($records)) {
        return;
    }
    ...
    if ($ids_to_delete) {
        $wpdb->query(
            "DELETE FROM {$bp->groups->table_name_members} WHERE ID IN ($ids_to_delete)"
        );
    }
}
