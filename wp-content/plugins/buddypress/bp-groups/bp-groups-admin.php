...
{
    wp_safe_redirect(
        add_query_arg('page', 'bp-groups', network_admin_url('admin.php'))
    );
    exit();
}
...
function bp_groups_admin_types_menu()
{
    ...
    if (
        ...
        $group_type_admin_url = add_query_arg(
            'taxonomy',
            'bp_group_type',
            get_admin_url(bp_get_root_blog_id(), 'edit-tags.php')
        );
        add_submenu_page(
            'bp-groups',
        ...
        add_submenu_page(
            ...
            basename(
                add_query_arg(
                    'taxonomy',
                    'bp_group_type',
                    bp_get_admin_url('edit-tags.php')
                )
            )
        );
    }
 ...
 */
function bp_groups_admin_menu_order($custom_menus = [])
{
...
function bp_groups_admin_load()
{
    ...
    if ('do_delete' == $doaction && !empty($_GET['gid'])) {
        ...
        get_current_screen()->set_help_sidebar(
            '<p><strong>' .
                ...
                '</strong></p>' .
                '<p><a href="https://buddypress.org/support">' .
        ...
        add_meta_box(
            'submitdiv',
            _x('Save', 'group admin edit screen', 'buddypress'),
        ...
        add_meta_box(
            'bp_group_settings',
            _x('Settings', 'group admin edit screen', 'buddypress'),
        ...
        add_meta_box(
            'bp_group_add_members',
            _x('Add New Members', 'group admin edit screen', 'buddypress'),
        ...
        add_meta_box(
            'bp_group_members',
            _x('Manage Members', 'group admin edit screen', 'buddypress'),
        ...
        add_screen_option('per_page', [
            'label' => _x(
                'Groups',
                'Groups per page (screen options)',
                'buddypress'
            ),
        ]);
        ...
        get_current_screen()->add_help_tab([
            ...
            'content' =>
                '<p>' .
                __(
                    'You can manage groups much like you can manage comments and other content. This screen is customizable in the same ways as other management screens, and you can act on groups by using the on-hover action links or the Bulk Actions.',
                    'buddypress'
                ) .
                '</p>',
        ]);
        ...
        get_current_screen()->add_help_tab([
            ...
            'content' =>
                '<p>' .
                __(
                    'Clicking "Visit" will take you to the group&#8217;s public page. Use this link to see what the group looks like on the front end of your site.',
                    'buddypress'
                ) .
                '</p>' .
                '<p>' .
                __(
                    'Clicking "Edit" will take you to a Dashboard panel where you can manage various details about the group, such as its name and description, its members, and other settings.',
                    'buddypress'
                ) .
                '</p>' .
                '<p>' .
                __(
                    'If you click "Delete" under a specific group, or select a number of groups and then choose Delete from the Bulk Actions menu, you will be led to a page where you&#8217;ll be asked to confirm the permanent deletion of the group(s).',
                    'buddypress'
                ) .
                '</p>',
        ]);
        ...
        get_current_screen()->set_help_sidebar(
            '<p><strong>' .
                ...
                '<p>' .
                __(
                    '<a href="https://buddypress.org/support/">Support Forums</a>',
                    'buddypress'
                ) .
                '</p>'
        );
    ...
    wp_enqueue_script(
        'bp_groups_admin_js',
        $bp->plugin_url . "bp-groups/admin/js/admin{$min}.js",
        ['jquery', 'wp-ajax-response', 'jquery-ui-autocomplete'],
        bp_get_version(),
        true
    );
    wp_localize_script('bp_groups_admin_js', 'BP_Group_Admin', [
        'add_member_placeholder' => __(
            'Start typing a username to add a new member.',
            'buddypress'
        ),
        'warn_on_leave' => __(
            'If you leave this page, you will lose any unsaved changes you have made to the group.',
            'buddypress'
        ),
    ]);
    wp_enqueue_style(
        'bp_groups_admin_css',
        $bp->plugin_url . "bp-groups/admin/css/admin{$min}.css",
        [],
        bp_get_version()
    );
    ...
    if ($doaction && 'save' == $doaction) {
        ...
        // Store errors.
        $error = 0;
        $success_new = $error_new = $success_modified = $error_modified = [];
         ...
         */
        $allowed_status = apply_filters('groups_allowed_status', [
            'public',
            'private',
            'hidden',
        ]);
        $status = in_array($_POST['group-status'], (array) $allowed_status)
            ? $_POST['group-status']
            : 'public';
         ...
         */
        $allowed_invite_status = apply_filters('groups_allowed_invite_status', [
            'members',
            'mods',
            'admins',
        ]);
        $invite_status = in_array(
            $_POST['group-invite-status'],
            (array) $allowed_invite_status
        )
            ? $_POST['group-invite-status']
            : 'members';
        ...
        if (!empty($_POST['bp-groups-new-members'])) {
            $user_names = array_merge(
                $user_names,
                explode(',', $_POST['bp-groups-new-members'])
            );
        }
        ...
        if (
            !empty($_POST['bp-groups-role']) &&
            !empty($_POST['bp-groups-existing-role'])
        ) {
            ...
            if (!$admin_count) {
                ...
                foreach (
                    (array) $_POST['bp-groups-role']
                    as $user_id => $new_role
                ) {
                    $user_id = (int) $user_id;

                    $existing_role = isset(
                        $_POST['bp-groups-existing-role'][$user_id]
                    )
                        ? $_POST['bp-groups-existing-role'][$user_id]
                        : '';
                        ...
                        switch ($new_role) {
                            ...
                            case 'member':
                                if (
                                    ...
                                    $result = groups_demote_member(
                                        $user_id,
                                        $group_id
                                    );
                                } elseif ('banned' == $existing_role) {
                                    $result = groups_unban_member(
                                        $user_id,
                                        $group_id
                                    );
                                }
        ...
        if (!empty($success_new)) {
            $success_new = implode(
                ',',
                array_filter($success_new, 'urlencode')
            );
            $redirect_to = add_query_arg(
                'success_new',
                $success_new,
        ...
        if (!empty($success_modified)) {
            $success_modified = implode(
                ',',
                array_filter($success_modified, 'urlencode')
            );
            $redirect_to = add_query_arg(
                'success_modified',
                $success_modified,
        ...
        if (!empty($error_modified)) {
            $error_modified = implode(
                ',',
                array_filter($error_modified, 'urlencode')
            );
            $redirect_to = add_query_arg(
                'error_modified',
                $error_modified,
         ...
         */
        wp_redirect(
            apply_filters('bp_group_admin_edit_redirect', $redirect_to)
        );
        exit();
...
{
    if (
        'toplevel_page_bp_groups_per_page' != $option &&
        'toplevel_page_bp_groups_network_per_page' != $option
    ) {
        return $value;
    }
...
function bp_groups_admin_edit()
{
    ...
    if (
        !empty($_REQUEST['no_admins']) ||
        !empty($_REQUEST['error']) ||
        !empty($_REQUEST['updated']) ||
        !empty($_REQUEST['error_new']) ||
        !empty($_REQUEST['success_new']) ||
        !empty($_REQUEST['error_modified']) ||
        !empty($_REQUEST['success_modified'])
    ) {
        ...
        $error_new = !empty($_REQUEST['error_new'])
            ? explode(',', $_REQUEST['error_new'])
            : [];
        $success_new = !empty($_REQUEST['success_new'])
            ? explode(',', $_REQUEST['success_new'])
            : [];
        $error_modified = !empty($_REQUEST['error_modified'])
            ? explode(',', $_REQUEST['error_modified'])
            : [];
        $success_modified = !empty($_REQUEST['success_modified'])
            ? explode(',', $_REQUEST['success_modified'])
            : [];

        if (!empty($no_admins)) {
            $messages[] = __(
                'You cannot remove all administrators from a group.',
                'buddypress'
            );
        }
        ...
        if (!empty($errors)) {
            if ($errors < 0) {
                $messages[] = __(
                    'Group name, slug, and description are all required fields.',
                    'buddypress'
                );
            } else {
                $messages[] = __(
        ...
        if (!empty($error_new)) {
            ...
            $messages[] = sprintf(
                __(
                    'The following users could not be added to the group: %s',
                    'buddypress'
                ),
                '<em>' . esc_html(implode(', ', $error_new)) . '</em>'
            );
        }
        ...
        if (!empty($success_new)) {
            ...
            $messages[] = sprintf(
                __(
                    'The following users were successfully added to the group: %s',
                    'buddypress'
                ),
                '<em>' . esc_html(implode(', ', $success_new)) . '</em>'
            );
        }

        if (!empty($error_modified)) {
            $error_modified = bp_groups_admin_get_usernames_from_ids(
                $error_modified
            );
            /* translators: %s: comma separated list of usernames */
            $messages[] = sprintf(
                __(
                    'An error occurred when trying to modify the following members: %s',
                    'buddypress'
                ),
                '<em>' . esc_html(implode(', ', $error_modified)) . '</em>'
            );
        }

        if (!empty($success_modified)) {
            $success_modified = bp_groups_admin_get_usernames_from_ids(
                $success_modified
            );
            /* translators: %s: comma separated list of usernames */
            $messages[] = sprintf(
                __(
                    'The following members were successfully modified: %s',
                    'buddypress'
                ),
                '<em>' . esc_html(implode(', ', $success_modified)) . '</em>'
            );
        }
    }
     ...
     */
    do_action_ref_array('bp_groups_admin_edit', [&$group]);
    ?>
	...
	<div class="wrap">
		...
		<?php if (is_user_logged_in() && bp_user_can_create_groups()): ?>
			<a class="page-title-action" href="<?php echo trailingslashit(
       bp_get_groups_directory_permalink() . 'create'
   ); ?>"><?php _e('Add New', 'buddypress'); ?></a>
		<?php endif; ?>
    ...
    ?>
		<?php if (!empty($messages)): ?>
			<div id="moderated" class="<?php echo $is_error
       ? 'error'
       : 'updated'; ?> notice is-dismissible"><p><?php echo implode(
     "</p><p>",
     $messages
 ); ?></p></div>
		<?php endif; ?>
						...
						<div id="post-body-content">
							<div id="postdiv">
								<div id="bp_groups_name" class="postbox">
									...
									<div class="inside">
										<label for="bp-groups-name" class="screen-reader-text"><?php /* translators: accessibility text */
          _e('Group Name', 'buddypress'); ?></label>
										<input type="text" name="bp-groups-name" id="bp-groups-name" value="<?php echo esc_attr(
              stripslashes($group_name)
          ); ?>" />
										<div id="bp-groups-permalink-box">
											<strong><?php esc_html_e('Permalink:', 'buddypress'); ?></strong>
											<span id="bp-groups-permalink">
												<?php bp_groups_directory_permalink(); ?> <input type="text" id="bp-groups-slug" name="bp-groups-slug" value="<?php bp_group_slug(
     $group
 ); ?>" autocomplete="off"> /
											</span>
											<a href="<?php echo bp_group_permalink(
               $group
           ); ?>" class="button button-small" id="bp-groups-visit-group"><?php esc_html_e(
    'Visit Group',
    'buddypress'
); ?></a>
										</div>

										<label for="bp-groups-description" class="screen-reader-text"><?php /* translators: accessibility text */
          _e('Group Description', 'buddypress'); ?></label>
										<?php wp_editor(stripslashes($group->description), 'bp-groups-description', [
              'media_buttons' => false,
              'teeny' => true,
              'textarea_rows' => 5,
              'quicktags' => [
                  'buttons' =>
                      'strong,em,link,block,del,ins,img,code,spell,close',
              ],
          ]); ?>
									</div>
								</div>
...
function bp_groups_admin_delete()
{
    ...
    $base_url = remove_query_arg(
        ['action', 'action2', 'paged', 's', '_wpnonce', 'gid'],
        $_SERVER['REQUEST_URI']
    );
    ?>
	...
	<div class="wrap">
      ...
      wp_nonce_url(
          add_query_arg(
              ['action' => 'do_delete', 'gid' => implode(',', $gids)],
              $base_url
          ),
          'bp-groups-delete'
      )
  ); ?>"><?php _e('Delete Permanently', 'buddypress'); ?></a>
		<a class="button" href="<?php echo esc_attr($base_url); ?>"><?php _e(
    'Cancel',
    'buddypress'
); ?></a>
	</div>
...
function bp_groups_admin_index()
{
    ...
    if (!empty($_REQUEST['deleted'])) {
        $deleted = !empty($_REQUEST['deleted'])
            ? (int) $_REQUEST['deleted']
            : 0;
	...
	<div class="wrap">
       ...
       __('Search results for &#8220;%s&#8221;', 'buddypress'),
    ...
    ?>
		<?php if (!empty($messages)): ?>
			<div id="moderated" class="<?php echo !empty($_REQUEST['error'])
       ? 'error'
       : 'updated'; ?> notice is-dismissible"><p><?php echo implode(
     "<br/>\n",
     $messages
    ...
    ?>
		...
		<form id="bp-groups-form" action="" method="get">
			<?php $bp_groups_list_table->search_box(
       __('Search all Groups', 'buddypress'),
       'bp-groups'
   ); ?>
...
function bp_groups_admin_edit_metabox_settings($item)
{
	...
	<?php if (bp_is_active('forums')): ?>
		<div class="bp-groups-settings-section" id="bp-groups-settings-section-forum">
			<label for="group-show-forum"><input type="checkbox" name="group-show-forum" id="group-show-forum" <?php checked(
       $item->enable_forum
   ); ?> /> <?php _e('Enable discussion forum', 'buddypress'); ?></label>
		</div>
	<?php endif; ?>
	...
	<div class="bp-groups-settings-section" id="bp-groups-settings-section-status">
		<fieldset>
			...
			<label for="bp-group-status-public"><input type="radio" name="group-status" id="bp-group-status-public" value="public" <?php checked(
       $item->status,
       'public'
   ); ?> /><?php _e('Public', 'buddypress'); ?></label>
			<label for="bp-group-status-private"><input type="radio" name="group-status" id="bp-group-status-private" value="private" <?php checked(
       $item->status,
       'private'
   ); ?> /><?php _e('Private', 'buddypress'); ?></label>
			<label for="bp-group-status-hidden"><input type="radio" name="group-status" id="bp-group-status-hidden" value="hidden" <?php checked(
       $item->status,
       'hidden'
   ); ?> /><?php _e('Hidden', 'buddypress'); ?></label>
		</fieldset>
	</div>
	...
	<div class="bp-groups-settings-section" id="bp-groups-settings-section-invite-status">
       ...
       'buddypress'
			...
			<label for="bp-group-invite-status-members"><input type="radio" name="group-invite-status" id="bp-group-invite-status-members" value="members" <?php checked(
       $invite_status,
       'members'
   ); ?> /><?php _e('All group members', 'buddypress'); ?></label>
			<label for="bp-group-invite-status-mods"><input type="radio" name="group-invite-status" id="bp-group-invite-status-mods" value="mods" <?php checked(
       $invite_status,
       'mods'
   ); ?> /><?php _e('Group admins and mods only', 'buddypress'); ?></label>
			<label for="bp-group-invite-status-admins"><input type="radio" name="group-invite-status" id="bp-group-invite-status-admins" value="admins" <?php checked(
       $invite_status,
       'admins'
   ); ?> /><?php _e('Group admins only', 'buddypress'); ?></label>
		</fieldset>
	</div>
...
function bp_groups_admin_edit_metabox_add_new_members($item)
{
    if (bp_is_large_install()) {
        $class = '';
        $notice = __(
            'Enter a comma-separated list of user logins.',
            'buddypress'
        );
    } else {
	...
	<label for="bp-groups-new-members" class="screen-reader-text"><?php /* translators: accessibility text */
 _e('Add new members', 'buddypress'); ?></label>
	<input name="bp-groups-new-members" type="text" id="bp-groups-new-members" class="<?php echo esc_attr(
     $class
 ); ?>" placeholder="" />
	<?php if ($notice): ?>
...
function bp_groups_admin_edit_metabox_members($item)
{
    ...
    if (
         ...
         */
        echo '<script type="text/javascript">var group_id = "' .
            esc_js($item->id) .
            '";</script>';
        return;
    }
    ...
    foreach ($members as $type => &$member_type_users) {
        ...
        $current_type_page = isset($_GET[$page_qs_key])
            ? absint($_GET[$page_qs_key])
            : 1;
        ...
        $pagination[$type] = bp_groups_admin_create_pagination_links(
            $member_type_query,
            $type
        );
    }
    ...
    foreach ($members as $member_type => $type_users): ?>
		...
		<div class="bp-groups-member-type" id="bp-groups-member-type-<?php echo esc_attr(
      $member_type
  ); ?>">
   ...
   endswitch; ?></h3>
		...
		<?php if (!empty($type_users)): ?>
			...
			<table class="widefat bp-group-members">
				<thead>
					<tr>
						<th scope="col" class="uid-column"><?php _ex(
          'ID',
          'Group member user_id in group admin',
          'buddypress'
      ); ?></th>
						<th scope="col" class="uname-column"><?php _ex(
          'Name',
          'Group member name in group admin',
          'buddypress'
      ); ?></th>
						<th scope="col" class="urole-column"><?php _ex(
          'Group Role',
          'Group member role in group admin',
          'buddypress'
      ); ?></th>
					</tr>
				</thead>
				...
				<?php foreach ($type_users as $type_user): ?>
					<tr>
						...
						<td class="uname-column">
							<a style="float: left;" href="<?php echo bp_core_get_user_domain(
           $type_user->ID
    ...
    'height' => '32',
							...
							<span style="margin: 8px; float: left;"><?php echo bp_core_get_userlink(
           $type_user->ID
       ); ?></span>
						</td>

						<td class="urole-column">
							<label for="bp-groups-role-<?php echo esc_attr(
           $type_user->ID
       ); ?>" class="screen-reader-text"><?php /* translators: accessibility text */
_e('Select group role for member', 'buddypress'); ?></label>
							<select class="bp-groups-role" id="bp-groups-role-<?php echo esc_attr(
           $type_user->ID
       ); ?>" name="bp-groups-role[<?php echo esc_attr($type_user->ID); ?>]">
								<optgroup label="<?php esc_attr_e('Roles', 'buddypress'); ?>">
									<option class="admin"  value="admin"  <?php selected(
             'admin',
             $member_type
         ); ?>><?php esc_html_e('Administrator', 'buddypress'); ?></option>
									<option class="mod"    value="mod"    <?php selected(
             'mod',
             $member_type
         ); ?>><?php esc_html_e('Moderator', 'buddypress'); ?></option>
									<option class="member" value="member" <?php selected(
             'member',
             $member_type
         ); ?>><?php esc_html_e('Member', 'buddypress'); ?></option>
									<?php if ('banned' === $member_type): ?>
									<option class="banned" value="banned" <?php selected(
             'banned',
             $member_type
         ); ?>><?php esc_html_e('Banned', 'buddypress'); ?></option>
									<?php endif; ?>
								</optgroup>
								<optgroup label="<?php esc_attr_e('Actions', 'buddypress'); ?>">
									<option class="remove" value="remove"><?php esc_html_e(
             'Remove',
             'buddypress'
         ); ?></option>
									<?php if ('banned' !== $member_type): ?>
										<option class="banned" value="banned"><?php esc_html_e(
              'Ban',
              'buddypress'
          ); ?></option>
									<?php endif; ?>
        ...
        /**
							 ...
							 */
        ?>
							<input type="hidden" name="bp-groups-existing-role[<?php echo esc_attr(
           $type_user->ID
		...
		<?php else: ?>
			...
			<p class="bp-groups-no-members description"><?php esc_html_e(
       'No members of this type',
       'buddypress'
   ); ?></p>
...
function bp_groups_admin_edit_metabox_status($item)
{
	...
	<div id="submitcomment" class="submitbox">
        ...
        wp_nonce_url(
            add_query_arg('action', 'delete', $base_url),
            'bp-groups-delete'
        )
    ); ?>"><?php _e('Delete Group', 'buddypress'); ?></a>
			</div>
...
function bp_groups_admin_edit_metabox_group_type(BP_Groups_Group $group = null)
{
    ...
    $types = bp_groups_get_group_types([], 'objects');
    $current_types = (array) bp_groups_get_group_type($group->id, false);
    $backend_only = bp_groups_get_group_types([
        'show_in_create_screen' => false,
    ]);
    ?>

	<label for="bp-groups-group-type" class="screen-reader-text"><?php /* translators: accessibility text */
 esc_html_e('Select group type', 'buddypress'); ?></label>

	<ul class="categorychecklist form-no-clear">
		<?php foreach ($types as $type): ?>
			<li>
				<label class="selectit"><input value="<?php echo esc_attr(
        $type->name
    ); ?>" name="bp-groups-group-type[]" type="checkbox" <?php checked(
    true,
    in_array($type->name, $current_types)
); ?>>
					<?php
     ...
     if (in_array($type->name, $backend_only)) {
         printf(
             ...
             esc_html__('(Not available on the front end)', 'buddypress')
         );
     }
     ?>
...
function bp_groups_process_group_type_update($group_id)
{
    ...
    $group_types = !empty($_POST['bp-groups-group-type'])
        ? wp_unslash($_POST['bp-groups-group-type'])
        : [];
 ...
 */
function bp_groups_admin_create_pagination_links(
    BP_Group_Member_Query $query,
    $member_type
) {
    $pagination = '';

    if (!in_array($member_type, ['admin', 'mod', 'member', 'banned'])) {
        return $pagination;
    }

    // The key used to paginate this member type in the $_GET global.
    $qs_key = $member_type . '_page';
    $url_base = remove_query_arg(
        [$qs_key, 'updated', 'success_modified'],
        $_SERVER['REQUEST_URI']
    );
    ...
    $current_page_end =
        $page * $per_page > intval($query->total_users)
            ? $query->total_users
            : $page * $per_page;
    ...
    if (1 == $query->total_users) {
        ...
        $viewing_text = sprintf(
            ...
            _nx(
                'Viewing %1$s - %2$s of %3$s member',
                'Viewing %1$s - %2$s of %3$s members',
                $query->total_users,
    ...
    $pagination .=
        '<span class="bp-group-admin-pagination-viewing">' .
        $viewing_text .
        '</span>';
    $pagination .=
        '<span class="bp-group-admin-pagination-links">' .
        $pag_links .
        '</span>';
...
{
    $usernames = [];
    $users = new WP_User_Query(['blog_id' => 0, 'include' => $user_ids]);
...
add_action(
    'wp_ajax_bp_group_admin_member_autocomplete',
...
function bp_groups_admin_process_group_type_bulk_changes($doaction)
{
    ...
    if (
        empty($_REQUEST['gid']) ||
        (empty($_REQUEST['bp_change_type']) &&
            empty($_REQUEST['bp_change_type2'])) ||
    ...
    check_admin_referer(
        ...
        'bp-bulk-groups-change-type-nonce'
    );
    ...
    if (
        'remove_group_type' !== $new_type &&
        null === bp_groups_get_group_type_object($new_type)
    ) {
        $error = true;
    } else {
        ...
        foreach ((array) $_REQUEST['gid'] as $group_id) {
            ...
            if ($current_types && 'remove_group_type' === $new_type) {
                ...
                !$current_types ||
                1 !== count($current_types) ||
                $new_type !== $current_types[0]
            ) {
                $group_types = [$new_type];
            }
    ...
    if ($error) {
        $redirect = add_query_arg(
            ['updated' => 'group-type-change-error'],
            wp_get_referer()
        );
    } else {
        $redirect = add_query_arg(
            ['updated' => 'group-type-change-success'],
            wp_get_referer()
        );
    }
...
function bp_groups_admin_groups_type_change_notice()
{
    ...
    if (
        ...
        in_array(
            $updated,
            ['group-type-change-error', 'group-type-change-success'],
            true
        )
    ) {
...
add_filter(
    bp_get_group_type_tax_name() . '_check_existing_type',
...
function bp_groups_type_admin_updated_messages($messages = [])
{
    ...
    $messages[$type_taxonomy] = [
        ...
        6 => __(
            'Sorry, the Group type was not deleted: it does not exist.',
            'buddypress'
        ),
        7 => __(
            'Sorry, This Group type is registered using code, deactivate the plugin or remove the custom code before trying to delete it again.',
            'buddypress'
        ),
        8 => __(
            'Sorry, there was an error while trying to delete this Group type.',
            'buddypress'
        ),
        9 => __('Group type successfully deleted.', 'buddypress'),
        10 => __(
            'Group type could not be updated due to missing required information.',
            'buddypress'
        ),
    ];
