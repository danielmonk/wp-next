 ...
 */
function bp_groups_editor_settings($bp_editor_settings = [])
{
    $bp = buddypress();
 ...
 */
function bp_groups_render_group_block($attributes = [])
{
    $bp = buddypress();
    ...
    $container_classes = ['bp-block-group'];
    ...
    if ($avatar) {
        $avatar_container = sprintf(
            ...
            sprintf(
                esc_html__('Group Profile photo of %s', 'buddypress'),
                $group_name
            )
        );
    }
    ...
    return apply_filters(
        'bp_groups_render_group_block_output',
        $output,
        $group,
        $params
    );
 ...
 * @return string           HTML output.
 */
function bp_groups_render_groups_block($attributes = [])
{
    $bp = buddypress();
    ...
    foreach ($groups as $group) {
        ...
        if (
            ...
            'description' === $block_args['extraInfo'] &&
        ...
        if (
            ...
            $output .= sprintf(
                ...
                /* Translators: %s is the group's name. */
                sprintf(
                    esc_attr__('Group Profile photo of %s', 'buddypress'),
                    $group->display_name
                ),
                esc_url(
        ...
        if ($has_description && $group->description) {
            ...
            $output .= sprintf(
                ...
                sprintf(
                    ...
                    bp_get_group_last_active($group)
                )
            );
     ...
     */
    return apply_filters(
        'bp_groups_render_groups_block_output',
        $output,
        $block_args,
        $groups
    );
