...
function bp_get_groups_group_type_base()
{
    ...
    return apply_filters(
        ...
        _x('type', 'group type URL base', 'buddypress')
    );
}
...
function bp_get_group_type_directory_link($group_type = '')
{
    ...
    if (
        isset($group_type_object->labels['singular_name']) &&
 ...
 */
function bp_group_type_list($group_id = 0, $r = [])
{
 ...
 * @return string
 */
function bp_get_group_type_list($group_id = 0, $r = [])
{
    if (empty($group_id)) {
    ...
    if ($types = bp_groups_get_group_type($group_id, false)) {
        ...
        if (false === $r['show_all']) {
            $types = array_intersect(
                bp_groups_get_group_types(['show_in_list' => true]),
                $types
            );
            if (empty($types)) {
                return $retval;
        ...
        $list = implode(
            ', ',
            array_map('bp_get_group_type_directory_link', $types)
        );
...
function bp_has_groups($args = '')
{
    ...
    $r = bp_parse_args(
        ...
        [
            ...
            'update_admin_cache' =>
                bp_is_groups_directory() || bp_is_user_groups(),
        ],
        'has_groups'
    );
...
function bp_group_is_visible($group = null)
{
    ...
    return bp_current_user_can('groups_access_group', [
        'group_id' => $group->id,
    ]);
}
 ...
 */
function bp_group_class($classes = [])
{
...
function bp_get_group_avatar($args = '')
{
    ...
    if (empty($avatar)) {
        $avatar =
            '<img src="' .
            esc_url($groups_template->group->avatar_thumb) .
            '" class="avatar" alt="' .
            esc_attr($groups_template->group->name) .
            '" />';
    }
...
{
    return (bool) bp_is_active('groups', 'cover_image') &&
 ...
 */
function bp_group_last_active($group = false, $args = [])
{
 ...
 * @return string
 */
function bp_get_group_last_active($group = false, $args = [])
{
    global $groups_template;
...
function bp_get_group_admin_permalink($group = false)
{
    ...
    return apply_filters(
        ...
        trailingslashit(bp_get_group_permalink($group) . 'admin'),
        $group
    );
}
...
function bp_get_group_description_excerpt($group = false, $length = 225)
{
    ...
    return apply_filters(
        ...
        bp_create_excerpt($group->description, $length),
        $group
    );
}
 ...
 */
function bp_group_date_created($group = false, $args = [])
{
 ...
 * @return string
 */
function bp_get_group_date_created($group = false, $args = [])
{
    global $groups_template;
    ...
    return apply_filters(
        ...
        bp_core_time_since($group->date_created),
        $group
    );
}
...
function bp_get_group_creator_permalink($group = false)
{
    ...
    return apply_filters(
        ...
        bp_core_get_user_domain($group->creator_id),
        $group
    );
}
 ...
 * }
 */
function bp_group_creator_avatar($group = false, $args = [])
{
 ...
 * @return string
 */
function bp_get_group_creator_avatar($group = false, $args = [])
{
    global $groups_template;
    ...
    if (!empty($group->admins)) { ?>
		<ul id="group-admins">
			<?php foreach ((array) $group->admins as $admin) { ?>
				<li>
					<a href="<?php echo bp_core_get_user_domain(
         $admin->user_id,
         $admin->user_nicename,
         $admin->user_login
     ); ?>" class="bp-tooltip" data-bp-tooltip="<?php printf(
    '%s',
...
function bp_group_list_mods($group = false)
{
    ...
    bp_core_get_user_displayname($mod->user_id)
          ...
          'alt' => sprintf /* translators: %s: member name */(
              __('Profile picture of %s', 'buddypress'),
              bp_core_get_user_displayname($mod->user_id)
          ),
      ]); ?>
...
function bp_get_group_all_members_permalink($group = false)
{
    ...
    return apply_filters(
        ...
        trailingslashit(bp_get_group_permalink($group) . 'members'),
        $group
    );
} /**
...
function bp_group_search_form()
{
    ...
    $search_form_html =
        '<form action="' .
        $action .
        '" id="group-search-form" method="post">
		<label for="' .
        $name .
        '" id="' .
        $name .
...
function bp_group_is_activity_permalink()
{
    if (
        ...
        !bp_is_current_action(bp_get_activity_slug())
    ) {
        return false;
    }
    return true;
...
function bp_get_groups_pagination_count()
{
    global $groups_template;
    $start_num =
        intval(($groups_template->pag_page - 1) * $groups_template->pag_num) +
        1;
    $from_num = bp_core_number_format($start_num);
    $to_num = bp_core_number_format(
        $start_num + ($groups_template->pag_num - 1) >
    ...
    if (1 == $groups_template->total_group_count) {
        ...
        /* translators: 1: group from number. 2: group to number. 3: total groups. */ $message = sprintf(
            _n(
                'Viewing %1$s - %2$s of %3$s group',
                'Viewing %1$s - %2$s of %3$s groups',
                $groups_template->total_group_count,
                'buddypress'
            ),
            $from_num,
 ...
 */ function bp_groups_auto_join()
{
     ...
     */ return apply_filters(
        'bp_groups_auto_join',
        (bool) buddypress()->groups->auto_join
    );
} /**
...
function bp_get_group_forum_permalink($group = false)
{
    ...
    return apply_filters(
        ...
        trailingslashit(bp_get_group_permalink($group) . 'forum'),
        $group
    );
} /**
...
function bp_group_get_invite_status($group_id = false)
{
    ...
    $invite_status = groups_get_groupmeta($group_id, 'invite_status'); // Backward compatibility. When 'invite_status' is not set, fall back to a default value.
    if (!$invite_status) {
        $invite_status = apply_filters(
...
function bp_groups_user_can_send_invites($group_id = 0, $user_id = 0)
{
    ...
    $invite_status = false; // If $user_id isn't specified, we check against the logged-in user.
    if (!$user_id) {
        $user_id = bp_loggedin_user_id();
    ...
    if ($user_id) {
        $can_send_invites = bp_user_can($user_id, 'groups_send_invitation', [
            'group_id' => $group_id,
        ]);
    } /**
...
function bp_group_admin_memberlist($admin_list = false, $group = false)
{
    ...
    _deprecated_function(
        __FUNCTION__,
        '1.5',
        'No longer used. See /bp-default/groups/single/admin.php'
    );
    if (empty($group)) {
        $group = &$groups_template->group;
    }
    if ($admins = groups_get_group_admins($group->id)): ?>
		...
		<ul id="admins-list" class="item-list<?php if (
      !empty($admin_list)
        ...
        'alt' => sprintf /* translators: %s: member name */(
            ...
            bp_core_get_user_displayname($admin->user_id)
				...
				<h5>
					...
					<span class="small">
						<a class="button confirm admin-demote-to-member" href="<?php bp_group_member_demote_link(
          $admin->user_id
			 ...
			 /* translators: %s: member name */ /* translators: %s: human time diff */<?php
       /* translators: %s: member name */
       ...
       ?>else: ?>
        ...
        'alt' => sprintf(
            ...
            bp_core_get_user_displayname($admin->user_id)
				...
				<span class="activity">
					<?php echo bp_core_get_last_activity(
         strtotime($admin->date_modified),
...
function bp_group_mod_memberlist($admin_list = false, $group = false)
{
    ...
    if ($group_mods = groups_get_group_mods($group->id)) { ?>
		...
		<ul id="mods-list" class="item-list<?php if (
      $admin_list
  ) { ?> single-line<?php } ?>">
       ...
       ?>echo bp_core_fetch_avatar([
        ...
        'alt' => sprintf /* translators: %s: member name */(
          ...
          'user_id' => $mod->user_id,
      ]); ?>" class="button confirm mod-promote-to-admin"><?php _e(
    'Promote to Admin',
    'buddypress'
); ?></a>
						<a class="button confirm mod-demote-to-member" href="<?php bp_group_member_demote_link(
          $mod->user_id
      ); ?>"><?php _e('Demote to Member', 'buddypress'); ?></a>
					</span>
				</h5>
        ...
        'alt' => sprintf /* translators: %s: member name */(
            ...
            bp_core_get_user_displayname($mod->user_id)
				...
				<span class="activity">
					 /* translators: %s: human time diff */<?php
       /* translators: %s: human time diff */
       ?>echo bp_core_get_last_activity(
         strtotime($mod->date_modified),
         __('joined %s', 'buddypress')
     ); ?>
				</span>
        ...
        wp_nonce_url(
            trailingslashit(
                bp_get_group_permalink($group) .
                    'admin/manage-members/promote/mod/' .
                    $user_id
            ),
...
function bp_get_group_member_promote_admin_link($args = '')
{
    ...
    $r = bp_parse_args(
        $args,
        [
            'user_id' => !empty($members_template->member->user_id)
    ...
    return apply_filters(
        ...
        wp_nonce_url(
            trailingslashit(
                bp_get_group_permalink($group) .
                    'admin/manage-members/promote/admin/' .
                    $user_id
            ),
            'groups_promote_member'
        )
...
function bp_group_admin_tabs_backcompat(
    ...
    if (!empty($admin_tabs_backcompat)) {
        _doing_it_wrong(
            ...
            __(
                'This action should not be used directly. Please use the BuddyPress Group Extension API to generate Manage tabs.',
                'buddypress'
            ),
            '2.2.0'
        );
...
function bp_get_group_form_action($page, $group = false)
{
    ...
    return apply_filters(
        ...
        trailingslashit(bp_get_group_permalink($group) . $page),
        $group
    );
} /**
...
function bp_get_group_admin_form_action($page = false, $group = false)
{
    ...
    return apply_filters(
        ...
        trailingslashit(bp_get_group_permalink($group) . 'admin/' . $page),
        $group
    );
} /**
...
function bp_group_has_requested_membership($group = false)
{
    ...
    if (
        groups_check_for_membership_request(bp_loggedin_user_id(), $group->id)
    ) {
        return true;
    }
    return false;
...
{
    global $groups_template; // Site admins always have access.
    if (bp_current_user_can('bp_moderate')) {
        return true;
    }
    if (empty($group)) {
...
function bp_group_is_user_banned($group = false, $user_id = 0)
{
    ...
    if (bp_current_user_can('bp_moderate')) {
        return false;
    } // Check groups loop first // @see BP_Groups_Group::get_group_extras().
    if (
        !empty($groups_template->in_the_loop) &&
        isset($groups_template->group->is_banned)
    ) {
        $retval = $groups_template->group->is_banned; // Not in loop.
    } else {
...
function bp_get_group_leave_confirm_link($group = false)
{
    ...
    return apply_filters(
        ...
        wp_nonce_url(
            trailingslashit(bp_get_group_permalink($group) . 'leave-group/yes'),
            'groups_leave_group'
        ),
        $group
    );
...
function bp_get_group_send_invite_form_action($group = false)
{
    ...
    return apply_filters(
        ...
        trailingslashit(bp_get_group_permalink($group) . 'send-invites/send'),
        $group
    );
} /**
...
function bp_has_friends_to_invite($group = false)
{
    ...
    if (
        !friends_check_user_has_friends(bp_loggedin_user_id()) ||
        !friends_count_invitable_friends(bp_loggedin_user_id(), $group->id)
    ) {
        return false;
    }
    return true;
...
{
    global $groups_template; // Set group to current loop group if none passed.
    if (empty($group)) {
        $group = &$groups_template->group;
    ...
    if (!empty($group->is_member)) {
        ...
        if (
            1 == count($group_admins) &&
            bp_loggedin_user_id() === (int) $group_admins[0]->user_id
        ) {
            return false;
        }
        ...
        $button = [
            ...
            'link_href' => wp_nonce_url(
                trailingslashit(bp_get_group_permalink($group) . 'leave-group'),
                'groups_leave_group'
            ),
        ...
        switch ($group->status) {
            ...
            case 'public':
                $button = [
                    ...
                    'link_href' => wp_nonce_url(
                        trailingslashit(
                            bp_get_group_permalink($group) . 'join'
                        ),
                        'groups_join_group'
                    ),
                ...
                break;
            case 'private': // Member has outstanding invitation - // show an "Accept Invitation" button.
                if ($group->is_invited) {
                    $button = [
                        'id' => 'accept_invite',
                        'component' => 'groups',
                        ...
                        'wrapper_class' => 'group-button ' . $group->status,
                        'wrapper_id' => 'groupbutton-' . $group->id,
                        'link_href' => add_query_arg(
                            'redirect_to',
                            bp_get_group_permalink($group),
                            bp_get_group_accept_invite_link($group)
                        ),
                        'link_text' => __('Accept Invitation', 'buddypress'),
                        'link_class' => 'group-button accept-invite',
                    ]; // Member has requested membership but request is pending - // show a "Request Sent" button.
    ...
    return bp_get_button(
        apply_filters('bp_get_group_join_button', $button, $group)
    );
} /**
...
function bp_get_group_create_button()
{
     ...
     */
    return bp_get_button(
        apply_filters('bp_get_group_create_button', $button_args)
    );
} /**
...
{
    // Get the create a group button.
    $create_group_button = bp_get_group_create_button(); // Make sure the button is available.
...
{
    global $groups_template; // Group not passed so look for loop.
    if (empty($group)) {
        $group = &$groups_template->group;
    } // Group status is not set (maybe outside of group loop?).
    if (empty($group->status)) {
        $message = __('This group is not currently accessible.', 'buddypress'); // Group has a status.
    } else {
        switch ($group->status) {
            // Private group.
            case 'private':
                if (!bp_group_has_requested_membership($group)) {
                    ...
                    $message = __(
                        'This is a private group. Your membership request is awaiting approval from the group administrator.',
                        'buddypress'
                    );
                }
                break; // Hidden group.
...
function bp_group_hidden_fields()
{
    ...
    if (isset($_REQUEST[$query_arg])) {
        echo '<input type="hidden" id="search_terms" value="' .
            esc_attr($_REQUEST[$query_arg]) .
            '" name="search_terms" />';
    }
    if (isset($_REQUEST['letter'])) {
        echo '<input type="hidden" id="selected_letter" value="' .
            esc_attr($_REQUEST['letter']) .
            '" name="selected_letter" />';
    }
    if (isset($_REQUEST['groups_search'])) {
        echo '<input type="hidden" id="search_terms" value="' .
            esc_attr($_REQUEST['groups_search']) .
            '" name="search_terms" />';
    }
} /**
...
function bp_get_group_member_is_friend()
{
    ...
    if (!isset($members_template->member->is_friend)) {
        ...
        $friend_status =
            0 == $members_template->member->is_friend ? 'pending' : 'is_friend';
    } /**
...
function bp_get_group_member_is_banned()
{
    ...
    return apply_filters(
        ...
        $members_template->member->is_banned
    );
} /**
 * @since 1.2.6
 */
 ...
 * @return string|null
 */
function bp_group_member_joined_since($args = [])
{
 ...
 * @return string
 */
function bp_get_group_member_joined_since($args = [])
{
    global $members_template;
    $r = bp_parse_args(
        $args,
    ...
    ); // We do not want relative time, so return now. // @todo Should the 'bp_get_group_member_joined_since' filter be applied here?
    if (!$r['relative']) {
    ...
    return apply_filters(
        ...
        bp_core_get_last_activity(
            $members_template->member
                ->date_modified /* translators: %s: human time diff */,
            __('joined %s', 'buddypress')
        )
    );
} /**
...
function bp_get_group_member_pagination_count()
{
    global $members_template;
    $start_num =
        intval(($members_template->pag_page - 1) * $members_template->pag_num) +
        1;
    $from_num = bp_core_number_format($start_num);
    $to_num = bp_core_number_format(
        $start_num + ($members_template->pag_num - 1) >
    ...
    if (1 == $members_template->total_member_count) {
        ...
        /* translators: 1: group member from number. 2: group member to number. 3: total group members. */ $message = sprintf(
            _nx(
...
function bp_groups_get_front_template($group = null)
{
    ...
    if (bp_groups_get_group_types()) {
        ...
        $template_names[] =
            'groups/single/front-group-type-' .
            sanitize_file_name($group_type) .
            '.php';
    }
    $template_names = array_merge($template_names, [
        'groups/single/front-status-' .
        sanitize_file_name($group->status) .
        '.php',
    ...
    return bp_locate_template(
        apply_filters('bp_groups_get_front_template', $template_names, $group),
        false,
        true
    );
} /**
...
{
    ?>
	<div class="item-list-tabs" id="subnav" aria-label="<?php esc_attr_e(
	...
	<h2 class="bp-screen-reader-text"> /* translators: accessibility text */<?php
    /* translators: accessibility text */
    ?>_e('Members', 'buddypress'); ?></h2>
...
function bp_groups_members_filter()
{
	...
	<li id="group_members-order-select" class="last filter">
		<label for="group_members-order-by"><?php _e(
      'Order By:',
      'buddypress'
  ); ?></label>
		<select id="group_members-order-by">
			...
			<?php if (bp_is_active('activity')): ?>
				<option value="group_activity"><?php _e(
        'Group Activity',
        'buddypress'
    ); ?></option>
			<?php endif; ?>
...
function bp_user_can_create_groups()
{
    ...
    $restricted = (int) bp_get_option('bp_restrict_group_creation', 0); // Allow by default.
    $can_create = true; // Are regular users restricted?
    if ($restricted) {
        $can_create = false;
    } /**
...
function bp_group_creation_tabs()
{
    ...
    foreach ((array) $bp->groups->group_creation_steps as $slug => $step) {
		...
		<li<?php if (
      bp_get_groups_current_create_step() == $slug
  ): ?> class="current"<?php endif; ?>><?php
if (
    $is_enabled
): ?><a href="<?php bp_groups_directory_permalink(); ?>create/step/<?php echo $slug; ?>/"><?php else: ?><span><?php endif;
echo $counter;
?>. <?php
echo $step['name'];
if ($is_enabled): ?></a><?php else: ?></span><?php endif;
?></li><?php $counter++;
    }
    unset($is_enabled); /**
...
function bp_group_creation_stage_title()
{
    ...
    echo apply_filters(
        ...
        '<span>&mdash; ' .
            $bp->groups->group_creation_steps[
...
function bp_is_group_creation_step($step_slug)
{
    ...
    $bp = buddypress(); // If this the first step, we can just accept and return true.
    $keys = array_keys($bp->groups->group_creation_steps);
...
{
    $bp = buddypress(); // If this is the first group creation step, return true.
    $keys = array_keys($bp->groups->group_creation_steps);
    if (array_shift($keys) == $step_slug) {
        return true;
    }
    reset($bp->groups->group_creation_steps);
    $previous_steps = []; // Get previous steps.
...
{
    $bp = buddypress();
    $steps = array_keys($bp->groups->group_creation_steps); // Loop through steps.
    ...
    $url = trailingslashit(
        $group_directory . $create_step . $previous_step
    ); /**
 ...
 */
function bp_new_group_invite_friend_list($args = [])
{
...
function bp_get_new_group_invite_friend_list($args = [])
{
    ...
    $items = []; // Build list markup parent elements.
    $before = '';
    if (!empty($r['before'])) {
        $before = $r['before'];
    }
    ...
    if (!empty($friends)) {
        ...
        for ($i = 0, $count = count($friends); $i < $count; ++$i) {
            ...
            $items[] =
                ...
                $r['separator'] .
                '><label for="f-' .
                esc_attr($friends[$i]['id']) .
                '"><input' .
                checked($checked, true, false) .
                ' type="checkbox" name="friends[]" id="f-' .
                esc_attr($friends[$i]['id']) .
                '" value="' .
                esc_attr($friends[$i]['id']) .
                '" /> ' .
                esc_html($friends[$i]['full_name']) .
                '</label></' .
                $r['separator'] .
                '>';
        }
     ...
     */
    $invitable_friends = apply_filters(
 ...
 */ function bp_groups_header_tabs()
{
	...
	<li<?php if (
     ...
     bp_is_action_variable('recently-active', 0)
 ): ?> class="current"<?php endif; ?>><a href="<?php echo trailingslashit(
    $user_groups . '/my-groups/recently-active'
); ?>"><?php _e('Recently Active', 'buddypress'); ?></a></li>
	<li<?php if (
     bp_is_action_variable('recently-joined', 0)
 ): ?> class="current"<?php endif; ?>><a href="<?php echo trailingslashit(
    $user_groups . '/my-groups/recently-joined'
); ?>"><?php _e('Recently Joined', 'buddypress'); ?></a></li>
	<li<?php if (
     bp_is_action_variable('most-popular', 0)
 ): ?> class="current"<?php endif; ?>><a href="<?php echo trailingslashit(
    $user_groups . '/my-groups/most-popular'
); ?>"><?php _e('Most Popular', 'buddypress'); ?></a></li>
	<li<?php if (
     bp_is_action_variable('admin-of', 0)
 ): ?> class="current"<?php endif; ?>><a href="<?php echo trailingslashit(
    $user_groups . '/my-groups/admin-of'
); ?>"><?php _e('Administrator Of', 'buddypress'); ?></a></li>
	<li<?php if (
     bp_is_action_variable('mod-of', 0)
 ): ?> class="current"<?php endif; ?>><a href="<?php echo trailingslashit(
    $user_groups . '/my-groups/mod-of'
); ?>"><?php _e('Moderator Of', 'buddypress'); ?></a></li>
	<li<?php if (
     bp_is_action_variable('alphabetically')
 ): ?> class="current"<?php endif; ?>><a href="<?php echo trailingslashit(
    $user_groups . '/my-groups/alphabetically'
); ?>"><?php _e(
    'Alphabetically',
...
<?php do_action('groups_header_tabs');
} /**
...
{
    $type_object = bp_groups_get_group_type_object(
        bp_get_current_group_directory_type()
    ); /* translators: %s: group type singular name */
    $message = sprintf(
        __('Viewing groups of the type: %s', 'buddypress'),
        '<strong>' . $type_object->labels['singular_name'] . '</strong>'
    ); /**
...
function bp_group_has_membership_requests($args = '')
{
    ...
    return apply_filters(
        ...
        $requests_template->has_requests(),
        $requests_template
    );
} /**
 * @since 1.0.0
...
function bp_get_group_request_reject_link()
{
    ...
    $link = add_query_arg(
        ...
        trailingslashit(bp_get_group_permalink(groups_get_current_group())) .
...
function bp_get_group_request_user_link()
{
    ...
    return apply_filters(
        ...
        bp_core_get_userlink($requests_template->request->user_id)
    );
} /**
 * @since 1.0.0
 */
function bp_group_request_time_since_requested()
{
     ...
     */
    echo apply_filters(
        'bp_group_request_time_since_requested' /* translators: %s: human time diff */,
        sprintf(
            __('requested %s', 'buddypress'),
            bp_core_time_since($requests_template->request->date_modified)
        )
    );
} /**
 * @since 1.0.0
...
function bp_get_group_requests_pagination_count()
{
    ...
    $start_num =
        intval(
            ($requests_template->pag_page - 1) * $requests_template->pag_num
        ) + 1;
    $from_num = bp_core_number_format($start_num);
    $to_num = bp_core_number_format(
        $start_num + ($requests_template->pag_num - 1) >
        $requests_template->total_request_count
            ? $requests_template->total_request_count
            : $start_num + ($requests_template->pag_num - 1)
    );
    ...
    if (1 == $requests_template->total_request_count) {
        ...
        /* translators: 1: group request from number. 2: group request to number. 3: total group requests. */ $message = sprintf(
            _n(
                'Viewing %1$s - %2$s of %3$s request',
                'Viewing %1$s - %2$s of %3$s requests',
                $requests_template->total_request_count,
                'buddypress'
            ),
            $from_num,
 ...
 */ function bp_group_has_invites($args = '')
{
    ...
    if (empty($r['group_id'])) {
        if (groups_get_current_group()) {
            ...
            isset(buddypress()->groups->new_group_id) &&
            buddypress()->groups->new_group_id
        ) {
...
function bp_get_group_invite_item_id()
{
    ...
    return apply_filters(
        ...
        'uid-' . $invites_template->invite->user->id
    );
} /**
 * @since 1.1.0
 */
...
function bp_get_group_invite_user_avatar()
{
    ...
    return apply_filters(
        ...
        $invites_template->invite->user->avatar_thumb
    );
} /**
 * @since 1.1.0
 */
...
function bp_get_group_invite_user_link()
{
    ...
    return apply_filters(
        ...
        bp_core_get_userlink($invites_template->invite->user->id)
    );
} /**
 * @since 1.1.0
 */
...
function bp_get_group_invite_user_last_active()
{
    ...
    return apply_filters(
        ...
        $invites_template->invite->user->last_active
    );
} /**
 * @since 1.1.0
 */
...
function bp_get_group_invite_user_remove_invite_url()
{
    ...
    if (bp_is_current_action('create')) {
        $uninvite_url =
            bp_get_groups_directory_permalink() .
            'create/step/group-invites/?user_id=' .
            $user_id;
    } else {
        $uninvite_url = trailingslashit(
...
function bp_get_group_invite_pagination_count()
{
    global $invites_template;
    $start_num =
        intval(($invites_template->pag_page - 1) * $invites_template->pag_num) +
        1;
    $from_num = bp_core_number_format($start_num);
    $to_num = bp_core_number_format(
        $start_num + ($invites_template->pag_num - 1) >
        $invites_template->total_invite_count
            ? $invites_template->total_invite_count
            : $start_num + ($invites_template->pag_num - 1)
    );
    ...
    if (1 == $invites_template->total_invite_count) {
        ...
        /* translators: 1: group invite from number. 2: group invite to number. 3: total group invites. */ $message = sprintf(
            _n(
                'Viewing %1$s - %2$s of %3$s invitation',
                'Viewing %1$s - %2$s of %3$s invitations',
                $invites_template->total_invite_count,
                'buddypress'
            ),
            $from_num,
...
function bp_groups_activity_feed()
{
	...
	<link rel="alternate" type="application/rss+xml" title="<?php bloginfo(
     'name'
 ); ?> | <?php echo esc_attr(bp_get_current_group_name()); ?> | <?php _e(
...
{
    $current_group = groups_get_current_group();
    $current_group_id = isset($current_group->id)
        ? (int) $current_group->id
        : 0; /**
...
{
    $current_group = groups_get_current_group();
    $current_group_slug = isset($current_group->slug)
        ? $current_group->slug
        : ''; /**
...
{
    $current_group = groups_get_current_group();
    $current_group_desc = isset($current_group->description)
 ...
 * @param bool $nonce
 * @return string|null
 */ function bp_groups_action_link(
    $action = '',
    $query_args = '',
    $nonce = false
) {
 ...
 * @return string
 */
function bp_get_groups_action_link(
    $action = '',
    $query_args = '',
    $nonce = false
) {
...
function bp_groups_get_profile_stats($args = '')
{
    ...
    if (empty($r['output'])) {
        ...
        if (!empty($r['user_id'])) {
            ...
            if (empty($r['groups'])) {
                $r['groups'] = absint(
                    bp_get_total_group_count_for_user($r['user_id'])
                );
            } // If groups exist, show some formatted output
            $r['output'] = $r['before']; /* translators: %s: number of groups */
            $r['output'] .= sprintf(
                _n('%s group', '%s groups', $r['groups'], 'buddypress'),
...
{
    return file_exists(
        bp_locate_template('common/js-templates/group-members/index.php')
    );
}
