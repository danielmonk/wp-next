...
function bp_groups_format_activity_action_created_group($action, $activity)
{
    ...
    $action = sprintf(
        esc_html__('%1$s created the group %2$s', 'buddypress'),
        $user_link,
        $group_link
    );
...
function bp_groups_format_activity_action_joined_group($action, $activity)
{
    ...
    /* translators: 1: the user link. 2: the group link. */
    $action = sprintf(
        esc_html__('%1$s joined the group %2$s', 'buddypress'),
        $user_link,
        $group_link
    );
    ...
    if (has_filter('groups_activity_accepted_invite_action')) {
        $action = apply_filters_ref_array(
            'groups_activity_accepted_invite_action',
            [$action, $activity->user_id, &$group]
        );
    }
...
function bp_groups_format_activity_action_group_details_updated(
     ...
     */
    $changed = groups_get_groupmeta(
        $activity->item_id,
        'updated_details_' . $activity->date_recorded
    );
    ...
    if (empty($changed)) {
        /* translators: 1: the user link. 2: the group link. */
        $action = sprintf(
            esc_html__('%1$s updated details for the group %2$s', 'buddypress'),
            $user_link,
            $group_link
        );
        ...
        !empty($changed['name']['old']) &&
        !empty($changed['name']['new'])
    ) {
        ...
        $action = sprintf(
            ...
            $user_link,
            $group_link,
            esc_html($changed['name']['old']),
            esc_html($changed['name']['new'])
        );

        // Description only.
    } elseif (
        !empty($changed['description']['old']) &&
        !empty($changed['description']['new'])
    ) {
        ...
        $action = sprintf(
            ...
            $user_link,
            $group_link,
            esc_html($changed['description']['old']),
            esc_html($changed['description']['new'])
        );
    } elseif (
        !empty($changed['slug']['old']) &&
        !empty($changed['slug']['new'])
    ) {
        /* translators: 1: the user link. 2: the group link. */
        $action = sprintf(
            esc_html__(
                '%1$s changed the permalink of the group %2$s.',
                'buddypress'
            ),
            $user_link,
            $group_link
 ...
 */
function bp_groups_format_activity_action_group_activity_update(
    $action,
    $activity
) {
    $user_link = bp_core_get_userlink($activity->user_id);
...
function groups_post_update($args = '')
{
     ...
     */
    $content_filtered = apply_filters(
...
function bp_groups_filter_activity_can_comment($retval, $activity = null)
{
    ...
    if (
        ...
        (!groups_is_user_member(bp_loggedin_user_id(), $group_id) ||
            groups_is_user_banned(bp_loggedin_user_id(), $group_id))
    ) {
        $retval = false;
    }
 ...
 */
function bp_groups_group_details_updated_add_activity(
    $group_id,
    $old_group,
    $notify_members
) {
    ...
    if (
        !isset($old_group->name) ||
        !isset($old_group->slug) ||
        !isset($old_group->description)
    ) {
        return false;
    }
