...
function groups_ajax_widget_groups_list()
{
    ...
    if (bp_has_groups($groups_args)): ?>
      ...
      bp_the_group(); ?>
           ...
           bp_get_group_date_created(0, ['relative' => false])
       ); ?>">
        ...
        printf(__('created %s', 'buddypress'), bp_get_group_date_created()); ?>
          ...
          ?>elseif ('popular-groups' === $_POST['filter']): ?>
						...
						<?php else: ?>
							<span class="activity" data-livestamp="<?php bp_core_iso8601_date(
           bp_get_group_last_active(0, ['relative' => false])
       ); ?>">
								<?php printf(
            _x('Active %s', 'last time the group was active', 'buddypress'),
...
add_action(
    'wp_ajax_nopriv_widget_groups_list',
