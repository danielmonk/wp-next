...
(function (wp, bp, $) {
  // Bail if not set
  if (
    typeof bpGroupManageMembersSettings === "undefined" ||
    !bp.isRestEnabled
  ) {
    return;
  }
      ...
      if ("read" === method) {
        ...
        options.success = function (data, textStatus, request) {
          if (!_.isUndefined(request)) {
            ...
            self.totalGroupMembers = parseInt(
  ...
  bp.Views.GroupRolesDropDown = bp.View.extend({
    tagName: "select",
    filters: _.extend(
      { all: { name: bpGroupManageMembersSettings.strings.allMembers } },
      bpGroupManageMembersSettings.roles
    ),
      ...
      this.$el.html(
        _.chain(this.filters)
          .map(function (filter, value) {
            ...
            if (
              this.options.currentRole &&
              value === this.options.currentRole
            ) {
              return {
  ...
  bp.Views.GroupMembersSearch = bp.View.extend({
    ...
    searchMember: function (event) {
      ...
      var searchTerms = $("#manage-members-search").val(),
        queryArgs = _.extend(this.collection.options.data, {
          search: searchTerms,
          page: 1,
        });
  ...
  bp.Views.GroupsMembersPagination = bp.View.extend({
    ...
    setPagination: function (collection) {
      var attributes = _.pick(collection, [
        "currentPage",
        "totalGroupMembers",
        "totalPages",
      ]);
  ...
  bp.Views.GroupMembersListRow = bp.View.extend({
    ...
    initialize: function () {
      ...
      _.each(bpGroupManageMembersSettings.roles, function (props) {
        if (_.isMatch(self.model.attributes, _.pick(props, roleProps))) {
          self.model.set("role", _.pick(props, ["id", "name"]), {
            silent: true,
          });
        }
      });
    ...
    renderEditForm: function () {
      ...
      this.views.set("#edit-group-member-" + userId, [
        new bp.Views.GroupsMembersLabel({
          value: userId,
          attributes: { for: "group-member" + userId + "-role" },
        }),
        new bp.Views.GroupRolesDropDown({
          id: "group-member" + userId + "-role",
          omits: ["all", "banned"],
    ...
    doMemberAction: function (event) {
      ...
      if ("edit" === action) {
        ...
        var newRole = "ban" === action ? "banned" : "member",
          roleObject = this.getRoleObject(newRole);
        ...
        this.views.set(
          "#edit-group-member-" + this.model.get("id"),
          new bp.Views.GroupMemberUpdatingInfo({ value: action }).render()
        );
        ...
        this.model.save(roleObject, {
          ...
          error: function (model, response) {
            self.views.set(
              "#edit-group-member-" + model.get("id"),
              new bp.Views.GroupMemberErrorInfo({
    ...
    editMemberRole: function (event) {
      ...
      this.views.set(
        "#edit-group-member-" + this.model.get("id"),
        new bp.Views.GroupMemberUpdatingInfo().render()
      );

      if (
        "admin" === currentRole ||
        ("mod" === currentRole && "member" === newRole)
      ) {
        roleAction = "demote";
      }
      ...
      this.model.save(roleObject, {
        ...
        success: function (model) {
          if (
            self.model.collection.currentRole &&
            newRole !== self.model.collection.currentRole
          ) {
            self.model.collection.remove(model);
            return self.clearRow();
          } else {
  ...
  bp.Views.GroupMembersListTable = bp.View.extend({
    ...
    initialize: function () {
      ...
      if (preloaded.body && preloaded.body.length > 0) {
        ...
        if (preloaded.headers && preloaded.headers["X-WP-TotalPages"]) {
          this.collection.totalPages = parseInt(
            preloaded.headers["X-WP-TotalPages"],
            10
          );
        }

        if (preloaded.headers && preloaded.headers["X-WP-Total"]) {
          this.collection.totalGroupMembers = parseInt(
            preloaded.headers["X-WP-Total"],
            10
          );
        }
    ...
    addListTableRows: function (collection) {
      if (this.views._views) {
        var noMembersRow = _.findWhere(this.views._views[""], {
          id: "bp-no-group-members",
        });
      ...
      if (!collection.length) {
        this.views.add(
          new bp.Views.GroupMembersNoMatches({ id: "bp-no-group-members" })
        );
      } else {
        _.each(
          collection.models,
  ...
  bp.Views.GroupMembersUI = bp.View.extend({
    ...
    initialize: function () {
      ...
      this.views.set("#group-roles-filter", [
        new bp.Views.GroupsMembersLabel({
          attributes: { for: "group-members-role-filter" },
        }),
        new bp.Views.GroupRolesDropDown({
          id: "group-members-role-filter",
      ...
      this.views.set(
        ...
        new bp.Views.GroupMembersSearch({
      ...
      this.views.set(
        ...
        new bp.Views.GroupsMembersPagination({ collection: groupMembers })
      );
  ...
  bp.manageGroupMembersUI = new bp.Views.GroupMembersUI({
    el: "#group-manage-members-ui",
  }).render();
