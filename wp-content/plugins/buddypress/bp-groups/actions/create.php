...
function groups_action_create_group()
{
    ...
    if (isset($_COOKIE['bp_completed_create_steps']) && !isset($reset_steps)) {
        $bp->groups->completed_create_steps = json_decode(
            base64_decode(stripslashes($_COOKIE['bp_completed_create_steps']))
        );
    }
    ...
    if (bp_get_new_group_id()) {
        ...
        if (
            ...
            bp_core_add_message(
                __(
                    'Only the group creator may continue editing this group.',
                    'buddypress'
                ),
                'error'
            );
    ...
    if (isset($_POST['save'])) {
        ...
        if ('group-details' == bp_get_groups_current_create_step()) {
            if (
                ...
                empty($_POST['group-desc']) ||
                !strlen(trim($_POST['group-name'])) ||
                !strlen(trim($_POST['group-desc']))
            ) {
                bp_core_add_message(
                    __(
            ...
            if (
                !($bp->groups->new_group_id = groups_create_group([
                    ...
                    'slug' => groups_check_slug(
                        sanitize_title(esc_attr($_POST['group-name']))
                    ),
        ...
        if ('group-settings' == bp_get_groups_current_create_step()) {
             ...
             *                     'mods', and 'admins'.
             */
            $allowed_invite_status = apply_filters(
                'groups_allowed_invite_status',
                ['members', 'mods', 'admins']
            );
            $invite_status =
                !empty($_POST['group-invite-status']) &&
                in_array(
        ...
        if ('group-invites' === bp_get_groups_current_create_step()) {
            ...
            groups_send_invites(['group_id' => $bp->groups->new_group_id]);
        }
         ...
         */
        do_action(
            'groups_create_group_step_save_' .
                bp_get_groups_current_create_step()
        );
         ...
         */
        $completed_create_steps = isset($bp->groups->completed_create_steps)
            ? $bp->groups->completed_create_steps
            : [];
        if (
            !in_array(
                bp_get_groups_current_create_step(),
                $completed_create_steps
            )
        ) {
        ...
        setcookie(
            'bp_completed_create_steps',
            base64_encode(json_encode($bp->groups->completed_create_steps)),
            time() + 60 * 60 * 24,
            COOKIEPATH,
            COOKIE_DOMAIN,
            is_ssl()
        ...
        $keys = array_keys($bp->groups->group_creation_steps);
        if (
            count($bp->groups->completed_create_steps) == count($keys) &&
            bp_get_groups_current_create_step() == array_pop($keys)
        ) {
            unset($bp->groups->current_create_step);
            unset($bp->groups->completed_create_steps);
    ...
    if (
        'group-avatar' == bp_get_groups_current_create_step() &&
        isset($_POST['upload'])
    ) {
        ...
        if (isset($_POST['avatar-crop-submit']) && isset($_POST['upload'])) {
            ...
            if (!bp_core_avatar_handle_crop($args)) {
                bp_core_add_message(
                    __(
                        'There was an error saving the group profile photo, please try uploading again.',
                        'buddypress'
                    ),
                    'error'
                );
                ...
                bp_core_add_message(
                    __(
                        'The group profile photo was uploaded successfully.',
                        'buddypress'
                    )
                );
            }
...
function groups_action_sort_creation_steps()
{
    ...
    foreach ((array) $bp->groups->group_creation_steps as $slug => $step) {
        ...
        $temp[$step['position']] = ['name' => $step['name'], 'slug' => $slug];
    }
    ...
    foreach ((array) $temp as $position => $step) {
        $bp->groups->group_creation_steps[$step['slug']] = [
            'name' => $step['name'],
            'position' => $position,
        ];
    }
