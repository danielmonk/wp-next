...
function groups_action_leave_group()
{
    ...
    if (
        groups_is_user_member(bp_loggedin_user_id(), bp_get_current_group_id())
    ) {
        $bp = buddypress();
        ...
        if (
            ...
            $group_admins[0]->user_id == bp_loggedin_user_id()
        ) {
            bp_core_add_message(
            ...
            bp_core_add_message(
                __('There was an error leaving the group.', 'buddypress'),
                'error'
            );
        } else {
            bp_core_add_message(
                __('You successfully left the group.', 'buddypress')
            );
        }
