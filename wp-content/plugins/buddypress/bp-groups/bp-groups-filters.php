...
function bp_groups_filter_kses($content = '')
{
    ...
    $allowed_tags['a']['class'] = [];
    $allowed_tags['img'] = [];
    $allowed_tags['img']['src'] = [];
    $allowed_tags['img']['alt'] = [];
    $allowed_tags['img']['width'] = [];
    $allowed_tags['img']['height'] = [];
    $allowed_tags['img']['class'] = [];
    $allowed_tags['img']['id'] = [];
    $allowed_tags['code'] = [];
 ...
 */
function bp_groups_maybe_load_mentions_scripts(
    $load_mentions,
    $mentions_enabled
) {
    if (!$mentions_enabled) {
 ...
 * @return bool
 */
function bp_groups_disable_at_mention_notification_for_non_public_groups(
    $send,
    $usernames,
    $user_id,
    ...
    if (
        ...
        !bp_user_can($user_id, 'groups_access_group', [
 ...
 * @return bool
 */
function bp_groups_user_can_filter(
    $retval,
    $user_id,
    $capability,
    $site_id,
    ...
    switch ($capability) {
        ...
        case 'groups_request_membership':
            ...
            $group = groups_get_group($group_id);
            if (
                'private' === bp_get_group_status($group) &&
                !groups_is_user_member($user_id, $group->id) &&
        ...
        case 'groups_see_group':
            ...
            if ($user_id === bp_loggedin_user_id()) {
                ...
                'hidden' !== bp_get_group_status($group) ||
                groups_is_user_member($user_id, $group->id)
            ) {
                $retval = true;
            }
...
{
    $exporters['buddypress-groups-memberships'] = [
        ...
        'exporter_bp_friendly_name' => _x(
            'Group Memberships',
            'BuddyPress Group Memberships data exporter friendly name',
            'buddypress'
        ),
    ];

    $exporters['buddypress-groups-pending-requests'] = [
        'exporter_friendly_name' => __(
        ...
        'exporter_bp_friendly_name' => _x(
            ...
            'BuddyPress Pending Group Membership Requests data exporter friendly name',
            'buddypress'
        ),
    ];

    $exporters['buddypress-groups-pending-received-invitations'] = [
        'exporter_friendly_name' => __(
        ...
        'exporter_bp_friendly_name' => _x(
            ...
            'BuddyPress Pending Group Invitations data exporter friendly name',
            'buddypress'
        ),
    ];

    $exporters['buddypress-groups-pending-sent-invitations'] = [
        'exporter_friendly_name' => __(
