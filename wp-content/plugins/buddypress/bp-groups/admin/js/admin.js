...
(function ($) {
  function add_member_to_list(e, ui) {
    $("#bp-groups-new-members-list").append(
      '<li data-login="' +
        ui.item.value +
        '"><a href="#" class="bp-groups-remove-new-member">x</a> ' +
        ui.item.label +
        "</li>"
    );
  }
  ...
  $(document).ready(function () {
    ...
    $(".bp-suggest-user").autocomplete({
      ...
      minLength: 2,
      position:
        "undefined" !== typeof isRtl && isRtl
          ? { my: "right top", at: "right bottom", offset: "0, -1" }
          : { offset: "0, -1" },
      open: function () {
        $(this).addClass("open");
      },
      ...
      select: function (event, ui) {
        add_member_to_list(event, ui);
      },
    });

    /* Replace noscript placeholder */
    $("#bp-groups-new-members.bp-suggest-user").prop(
      "placeholder",
      BP_Group_Admin.add_member_placeholder
    );
