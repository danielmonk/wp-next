...
function groups_screen_group_invite()
{
    ...
    if (bp_is_action_variable('send', 0)) {
        if (
            !check_admin_referer('groups_send_invites', '_wpnonce_send_invites')
        ) {
            return false;
        }
        ...
        // Send the invites.
        groups_send_invites(['group_id' => $bp->groups->current_group->id]);
...
function groups_remove_group_invite()
{
    ...
    if (
        !bp_is_action_variable('remove', 0) ||
    ...
    if (!bp_groups_user_can_send_invites($group_id)) {
        $message = __(
            'You are not allowed to send or remove invites',
            'buddypress'
        );
        $error = 'error';
