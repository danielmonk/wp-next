...
function groups_screen_group_request_membership()
{
    ...
    if (isset($_POST['group-request-send'])) {
        ...
        if (
            isset($_POST['group-request-membership-comments']) &&
        ...
        if (!groups_send_membership_request($request_args)) {
            ...
            bp_core_add_message(
                __(
                    'Your membership request was sent to the group administrator successfully. You will be notified when the group administrator responds to your request.',
                    'buddypress'
                )
            );
        }
