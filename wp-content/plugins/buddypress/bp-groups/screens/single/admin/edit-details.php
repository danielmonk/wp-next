...
function groups_screen_group_admin_edit_details()
{
    ...
    if (bp_is_item_admin()) {
        ...
        if (isset($_POST['save'])) {
            ...
            $group_notify_members = isset($_POST['group-notify-members'])
                ? (int) $_POST['group-notify-members']
                : 0;
            ...
            if (empty($_POST['group-name']) || empty($_POST['group-desc'])) {
                bp_core_add_message(
                    __(
                        'Groups must have a name and a description. Please try again.',
                        'buddypress'
                    ),
                    'error'
                );
                ...
                bp_core_add_message(
                    __('Group details were successfully updated.', 'buddypress')
                );
            }
