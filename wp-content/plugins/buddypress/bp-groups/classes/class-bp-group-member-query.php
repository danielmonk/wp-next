...
class BP_Group_Member_Query extends BP_User_Query
{
    ...
    public function setup_hooks()
    {
        ...
        // Set the sort order.
        add_action('bp_pre_user_query', [$this, 'set_orderby']);

        // Set up our populate_extras method.
        add_action(
            'bp_user_query_populate_extras',
            [$this, 'populate_group_member_extras'],
            10,
            2
        );
    }
    ...
    protected function get_group_member_ids()
    {
        ...
        $roles = !empty($this->query_vars['group_role'])
        ...
        $allowed_roles = ['admin', 'mod', 'member', 'banned'];
        ...
        $sql['order'] =
            'first_joined' === $this->query_vars['type'] ? 'ASC' : 'DESC';

        $group_member_ids = $wpdb->get_col(
            "{$sql['select']} {$sql['where']} {$sql['orderby']} {$sql['order']}"
        );
        ...
        if (
            !$is_confirmed ||
            !is_null($this->query_vars['invite_sent']) ||
            !is_null($this->query_vars['inviter_id'])
        ) {
            $invite_args = [
            ...
            if (!is_null($this->query_vars['invite_sent'])) {
                $invite_args['invite_sent'] = !empty(
                    $this->query_vars['invite_sent']
                )
                    ? 'sent'
                    : 'draft';
            }
        ...
        $this->group_member_ids = array_merge(
            $group_member_ids,
            $invited_member_ids
        );
         ...
         */
        $this->group_member_ids = apply_filters(
    ...
    public function set_orderby($query)
    {
        ...
        if (
            in_array($query->query_vars['type'], [
                'last_joined',
                'first_joined',
                'group_activity',
            ])
            ...
            if ('group_activity' == $query->query_vars['type']) {
                $gm_ids = $this->get_gm_ids_ordered_by_activity(
                    $query,
                    $gm_ids
                );
            }
            ...
            $gm_ids = array_merge(['u.id'], wp_parse_id_list($gm_ids));
            $gm_ids_sql = implode(',', $gm_ids);

            $query->uid_clauses['orderby'] =
                "ORDER BY FIELD(" . $gm_ids_sql . ")";
        }

        // Prevent this filter from running on future BP_User_Query
        // instances on the same page.
        remove_action('bp_pre_user_query', [$this, 'set_orderby']);
    }
    ...
    public function populate_group_member_extras($query, $user_ids_sql)
    {
        ...
        foreach ((array) $extras as $extra) {
            if (isset($this->results[$extra->user_id])) {
                ...
                $this->results[$extra->user_id]->user_id =
                    (int) $extra->user_id;
                $this->results[$extra->user_id]->is_admin =
                    (int) $extra->is_admin;
                $this->results[$extra->user_id]->is_mod = (int) $extra->is_mod;
                $this->results[$extra->user_id]->is_banned =
                    (int) $extra->is_banned;
                $this->results[$extra->user_id]->date_modified =
                    $extra->date_modified;
                $this->results[$extra->user_id]->user_title =
                    $extra->user_title;
                $this->results[$extra->user_id]->comments = $extra->comments;
                $this->results[$extra->user_id]->invite_sent =
                    (int) $extra->invite_sent;
                $this->results[$extra->user_id]->inviter_id =
                    (int) $extra->inviter_id;
                $this->results[$extra->user_id]->is_confirmed =
                    (int) $extra->is_confirmed;
                $this->results[$extra->user_id]->membership_id =
                    (int) $extra->id;
            }
        }
        ...
        foreach ($invites as $invite) {
            if (isset($this->results[$invite->user_id])) {
                ...
                $this->results[$invite->user_id]->invite_sent =
                    (int) $invite->invite_sent;
                $this->results[$invite->user_id]->inviter_id =
                    $invite->inviter_id;

                // Backfill properties that are not being set above.
                if (!isset($this->results[$invite->user_id]->user_id)) {
                    $this->results[$invite->user_id]->user_id =
                        $invite->user_id;
                }
                ...
                if (!isset($this->results[$invite->user_id]->date_modified)) {
                    $this->results[$invite->user_id]->date_modified =
     ...
     * @return array
     */
    public function get_gm_ids_ordered_by_activity($query, $gm_ids = [])
    {
        global $wpdb;
        ...
        $group_user_ids = $wpdb->get_results(
            "{$sql['select']} {$sql['where']} {$sql['groupby']} {$sql['orderby']} {$sql['order']}"
        );
