...
class BP_REST_Group_Membership_Request_Endpoint extends WP_REST_Controller
{
    ...
    public function get_items_permissions_check($request)
    {
        ...
        if (!$user_id) {
            $retval = new WP_Error(
                'bp_rest_authorization_required',
                __(
                    'Sorry, you need to be logged in to view membership requests.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
            ...
            bp_current_user_can('bp_moderate') ||
            ($request['group_id'] &&
                groups_is_user_admin($user_id, $request['group_id'])) ||
            $user_id_arg === $user_id
        ) {
            $retval = true;
        } else {
            $retval = new WP_Error(
                ...
                __(
                    'Sorry, you are not allowed to view membership requests.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
    ...
    {
        $group_request = $this->fetch_single_membership_request(
            $request['request_id']
        );
        $retval = $this->prepare_response_for_collection(
            $this->prepare_item_for_response($group_request, $request)
    ...
    public function get_item_permissions_check($request)
    {
        ...
        if (!$user_id) {
            $retval = new WP_Error(
                'bp_rest_authorization_required',
                __(
                    'Sorry, you need to be logged in to get a membership.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
            ...
            $retval = new WP_Error(
                ...
                __(
                    'Sorry, you are not allowed to view a membership request.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
    ...
    public function create_item($request)
    {
        ...
        if (!$request_id) {
            return new WP_Error(
                ...
                __(
                    'Could not send membership request to this group.',
                    'buddypress'
                ),
                [
                    'status' => 500,
         ...
         */
        do_action(
            'bp_rest_group_membership_requests_create_item',
            $user,
            $invite,
            $group,
            $response,
    ...
    public function create_item_permissions_check($request)
    {
        ...
        if (!is_user_logged_in()) {
            $retval = new WP_Error(
                'bp_rest_authorization_required',
                __(
                    'Sorry, you need to be logged in to create a membership request.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
            ...
            $retval = new WP_Error(
                'bp_rest_group_membership_requests_cannot_create_item',
                __(
                    'User may not extend requests on behalf of another user.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
         ...
         */
        return apply_filters(
            'bp_rest_group_membership_requests_create_item_permissions_check',
            $retval,
            $request
        );
    }
    ...
    public function update_item($request)
    {
        ...
        $success = groups_accept_membership_request(
            false,
            $group_request->user_id,
            $group_request->item_id
        );
        if (!$success) {
            return new WP_Error(
                'bp_rest_group_member_request_cannot_update_item',
                __(
                    'There was an error accepting the membership request.',
                    'buddypress'
                ),
                [
                    'status' => 500,
    ...
    public function update_item_permissions_check($request)
    {
        ...
        if (!$user_id) {
            ...
            $retval = new WP_Error(
                'bp_rest_group_member_request_cannot_update_item',
                __(
                    'User is not allowed to approve membership requests to this group.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
    ...
    public function delete_item($request)
    {
        ...
        if (bp_loggedin_user_id() === $group_request->user_id) {
            $success = groups_delete_membership_request(
                false,
                $group_request->user_id,
                $group_request->item_id
            );
            ...
            $success = groups_reject_membership_request(
                false,
                $group_request->user_id,
                $group_request->item_id
            );
        ...
        if (!$success) {
            return new WP_Error(
                ...
                __(
                    'There was an error rejecting the membership request.',
                    'buddypress'
                ),
                [
                    'status' => 500,
         ...
         */
        do_action(
            'bp_rest_group_membership_requests_delete_item',
            $user,
            $group,
            $response,
            $request
    ...
    public function delete_item_permissions_check($request)
    {
        ...
        if (!$user_id) {
            $retval = new WP_Error(
                'bp_rest_authorization_required',
                __(
                    'Sorry, you need to be logged in to delete a request.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
            ...
            $retval = new WP_Error(
                'bp_rest_group_membership_requests_cannot_delete_item',
                __(
                    'User is not allowed to delete this membership request.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
         ...
         */
        return apply_filters(
            'bp_rest_group_membership_requests_delete_item_permissions_check',
            $retval,
            $request
        );
    }
    ...
    {
        $group_requests = groups_get_requests(['id' => $request_id]);
        if ($group_requests) {
    ...
    public function get_endpoint_args_for_item_schema(
         ...
         */
        return apply_filters(
            "bp_rest_group_membership_requests_{$key}_query_arguments",
            $args,
            $method
        );
    }
    ...
    public function get_item_schema()
    {
        ...
        $schema['properties']['user_id']['description'] = __(
            'The ID of the user who requested a Group membership.',
            'buddypress'
        );
        $schema['properties']['group_id']['description'] = __(
        ...
        unset(
            $schema['properties']['invite_sent'],
            $schema['properties']['inviter_id']
        );
    ...
    public function get_collection_params()
    {
        ...
        // Adapt some item schema property descriptions to this endpoint.
        $params['user_id']['description'] = __(
            'Return only Membership requests made by a specific user.',
            'buddypress'
        );
        $params['group_id']['description'] = __(
            'The ID of the group the user requested a membership for.',
            'buddypress'
        );
         ...
         */
        return apply_filters(
            'bp_rest_group_membership_requests_collection_params',
            $params
        );
    }
}
