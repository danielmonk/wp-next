...
class BP_Groups_List_Table extends WP_List_Table
{
    ...
    public function __construct()
    {
        ...
        if (bp_groups_get_group_types()) {
            ...
            add_filter('bp_groups_list_table_get_columns', [
                $this,
                'add_type_column',
            ]);
            add_filter(
                'bp_groups_admin_get_group_custom_column',
                [$this, 'column_content_group_type'],
                10,
                3
            );
            // Add the bulk change select.
            add_action('bp_groups_list_table_after_bulk_actions', [
                $this,
    ...
    public function prepare_items()
    {
        ...
        if (
            isset($_GET['group_status']) &&
            in_array($_GET['group_status'], ['public', 'private', 'hidden'])
        ) {
        ...
        if ('all' != $this->view && isset($this->group_type_ids[$this->view])) {
            $include = !empty($this->group_type_ids[$this->view])
                ? $this->group_type_ids[$this->view]
                : [0];
        }
        ...
        $group_type = false;
        if (
            isset($_GET['bp-group-type']) &&
            null !== bp_groups_get_group_type_object($_GET['bp-group-type'])
        ) {
        ...
        if ($include_id) {
            $groups = [(array) groups_get_group($include_id)];
        } else {
            $groups_args = [
                'include' => $include,
                'per_page' => $per_page,
        ...
        $this->set_pagination_args([
            ...
            'total_pages' => ceil(
                $groups_template->total_group_count / $per_page
            ),
        ]);

        // Set the Total number of groups.
        if ('all' === $this->view) {
            $this->group_counts['all'] =
                (int) $groups_template->total_group_count;
    ...
    public function display()
    {
		...
		<h2 class="screen-reader-text"><?php /* translators: accessibility text */
  _e('Groups list', 'buddypress'); ?></h2>
    ...
    public function get_views()
    {
		...
		<h2 class="screen-reader-text"><?php /* translators: accessibility text */
  _e('Filter groups list', 'buddypress'); ?></h2>

		<ul class="subsubsub">
			<li class="all">
				<a href="<?php echo esc_url($url_base); ?>" class="<?php if (
    'all' === $this->view
) {
    echo 'current';
} ?>">
        ...
        add_query_arg('group_status', 'public', $url_base)
    ); ?>" class="<?php if ('public' === $this->view) {
    echo 'current';
} ?>">
					<?php printf(
         ...
         sprintf(
			...
			<li class="private">
				<a href="<?php echo esc_url(
        add_query_arg('group_status', 'private', $url_base)
    ); ?>" class="<?php if ('private' === $this->view) {
    echo 'current';
} ?>">
					<?php printf(
         /* translators: %s is the placeholder for the count html `<span class="count"/>` */
         ...
         sprintf(
			...
			<li class="hidden">
				<a href="<?php echo esc_url(
        add_query_arg('group_status', 'hidden', $url_base)
   ...
   do_action('bp_groups_list_table_get_views', $url_base, $this->view); ?>
    ...
    public function get_bulk_actions()
    {
         ...
         */
        return apply_filters('bp_groups_list_table_get_bulk_actions', [
    ...
    public function get_columns()
    {
         ...
         */
        return apply_filters('bp_groups_list_table_get_columns', [
            'cb' => '<input name type="checkbox" />',
            'comment' => _x(
                'Name',
                'Groups admin Group Name column header',
                'buddypress'
            ),
            'description' => _x(
                'Description',
                'Groups admin Group Description column header',
                'buddypress'
            ),
            'status' => _x(
                'Status',
                'Groups admin Privacy Status column header',
                'buddypress'
            ),
            'members' => _x(
                'Members',
            ...
            'last_active' => _x(
                'Last Active',
                'Groups admin Last Active column header',
                'buddypress'
            ),
        ]);
    }
    ...
    public function get_sortable_columns()
    {
         ...
         */
        return apply_filters('bp_groups_list_table_get_sortable_columns', [
            'gid' => ['gid', false],
            'comment' => ['name', false],
    ...
    public function column_cb($item = [])
    {
        ...
        printf(
            '<label class="screen-reader-text" for="gid-%1$d">' .
                __('Select group %1$d', 'buddypress') .
                '</label><input type="checkbox" name="gid[]" value="%1$d" id="gid-%1$d" />',
            $item['id']
        );
    }
     ...
     * @param array $item A singular item (one full row).
     */
    public function column_gid($item = [])
    {
    ...
    public function column_comment($item = [])
    {
        ...
        $base_url = bp_get_admin_url(
            'admin.php?page=bp-groups&amp;gid=' . $item['id']
        );
         ...
         */
        $group_name = apply_filters_ref_array('bp_get_group_name', [
            $item['name'],
            $item,
        ]);
    ...
    public function column_description($item = [])
    {
         ...
         * @param array  $item  The current group item in the loop.
         */
        echo apply_filters_ref_array('bp_get_group_description', [
            $item['description'],
            $item,
        ]);
    }
    ...
    public function column_status($item = [])
    {
         ...
         */
        echo apply_filters_ref_array('bp_groups_admin_get_group_status', [
            $status_desc,
            $item,
        ]);
    }
    ...
    public function column_members($item = [])
    {
         ...
         * @parma array $item  The current group item in the loop.
         */
        echo apply_filters_ref_array('bp_groups_admin_get_group_member_count', [
            (int) $count,
            $item,
        ]);
    }
    ...
    public function column_last_active($item = [])
    {
         ...
         * @parma array  $item        The current group item in the loop.
         */
        echo apply_filters_ref_array('bp_groups_admin_get_group_last_active', [
            $last_active,
            $item,
        ]);
    }
    ...
    {
        $columns['bp_group_type'] = _x(
            'Group Type',
            'Label for the WP groups table group type column',
            'buddypress'
        );
     ...
     * @return string
     */
    public function column_content_group_type(
        $retval = '',
        $column_name = '',
        $item = []
    ) {
        ...
        add_filter(
            ...
            [$this, 'group_type_permalink_use_admin_filter'],
            10,
            2
        );
        $retval = bp_get_group_type_list($item['id'], [
            'parent_element' => '',
            'label_element' => '',
            'label' => '',
            'show_all' => true,
         ...
         * @parma array  $item   The current group item in the loop.
         */
        echo apply_filters_ref_array('bp_groups_admin_get_group_type_column', [
            $retval,
            $item,
        ]);
    }
    ...
    {
        return add_query_arg(['bp-group-type' => urlencode($type->name)]);
    }
    ...
    $type->labels['singular_name']
				...
				<option value="remove_group_type"><?php _e(
        'No Group Type',
        'buddypress'
    ); ?></option>
