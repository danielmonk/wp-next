...
class BP_Groups_Member
{
    ...
    public function save()
    {
         ...
         */
        do_action_ref_array('groups_member_before_save', [&$this]);
        ...
        if (!empty($this->id)) {
            $sql = $wpdb->prepare(
                "UPDATE {$bp->groups->table_name_members} SET inviter_id = %d, is_admin = %d, is_mod = %d, is_banned = %d, user_title = %s, date_modified = %s, is_confirmed = %d, comments = %s, invite_sent = %d WHERE id = %d",
                $this->inviter_id,
                $this->is_admin,
                $this->is_mod,
                $this->is_banned,
            ...
            $sql = $wpdb->prepare(
                "INSERT INTO {$bp->groups->table_name_members} ( user_id, group_id, inviter_id, is_admin, is_mod, is_banned, user_title, date_modified, is_confirmed, comments, invite_sent ) VALUES ( %d, %d, %d, %d, %d, %d, %s, %s, %d, %s, %d )",
                $this->user_id,
                $this->group_id,
                $this->inviter_id,
                $this->is_admin,
         ...
         */
        do_action_ref_array('groups_member_after_save', [&$this]);
    ...
    public function remove()
    {
         ...
         */
        do_action_ref_array('groups_member_before_remove', [$this]);

        $bp = buddypress();
        $sql = $wpdb->prepare(
            "DELETE FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d",
            $this->user_id,
            $this->group_id
        );
         ...
         * @param BP_Groups_Member $this Current group membership object.
         */
        do_action_ref_array('groups_member_after_remove', [$this]);
    ...
    {
        return bp_update_user_meta(
            $user_id,
            'total_group_count',
            (int) self::total_group_count($user_id)
        );
    }
    ...
    {
        return groups_update_groupmeta(
            $group_id,
            'total_member_count',
            (int) BP_Groups_Group::get_total_member_count($group_id)
        );
    }
    ...
    public static function get_group_ids(
        ...
        if ($user_id != bp_loggedin_user_id()) {
            $group_sql = $wpdb->prepare(
                "SELECT DISTINCT m.group_id FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE g.status != 'hidden' AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0{$pag_sql}",
                $user_id
            );
            $total_groups = $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE g.status != 'hidden' AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0",
                    $user_id
                )
            );
        } else {
            $group_sql = $wpdb->prepare(
                "SELECT DISTINCT group_id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND is_confirmed = 1 AND is_banned = 0{$pag_sql}",
                $user_id
            );
            $total_groups = $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(DISTINCT group_id) FROM {$bp->groups->table_name_members} WHERE user_id = %d AND is_confirmed = 1 AND is_banned = 0",
                    $user_id
                )
            );
        }
        ...
        return ['groups' => $groups, 'total' => (int) $total_groups];
    }
     ...
     * }
     */
    public static function get_recently_joined(
        $user_id,
        $limit = false,
        $page = false,
        $filter = false
        ...
        $total_groups = $wpdb->get_var(
            "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id{$hidden_sql}{$filter_sql} AND {$user_id_sql} AND m.is_banned = 0 AND m.is_confirmed = 1 ORDER BY m.date_modified DESC"
        );

        return ['groups' => $paged_groups, 'total' => $total_groups];
    }
     ...
     * }
     */
    public static function get_is_admin_of(
        $user_id,
        $limit = false,
        $page = false,
        $filter = false
        ...
        $total_groups = $wpdb->get_var(
            "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id{$hidden_sql}{$filter_sql} AND {$user_id_sql} AND m.is_confirmed = 1 AND m.is_banned = 0 AND m.is_admin = 1 ORDER BY date_modified ASC"
        );
     ...
     * }
     */
    public static function get_is_mod_of(
        $user_id,
        $limit = false,
        $page = false,
        $filter = false
        ...
        $total_groups = $wpdb->get_var(
            "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id{$hidden_sql}{$filter_sql} AND {$user_id_sql} AND m.is_confirmed = 1 AND m.is_banned = 0 AND m.is_mod = 1 ORDER BY date_modified ASC"
        );
     ...
     * }
     */
    public static function get_is_banned_of(
        $user_id,
        $limit = false,
        $page = false,
        $filter = false
        ...
        $total_groups = $wpdb->get_var(
            "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id{$hidden_sql}{$filter_sql} AND {$user_id_sql} AND m.is_banned = 1 ORDER BY date_modified ASC"
        );
    ...
    public static function total_group_count($user_id = 0)
    {
        ...
        if (
            ...
            return (int) $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id AND g.status != 'hidden' AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0",
                    $user_id
                )
            );
        } else {
            return (int) $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0",
                    $user_id
                )
            );
        }
     ...
     * @return array
     */
    public static function get_user_memberships($user_id, $args = [])
    {
        global $wpdb;
        ...
        $memberships = $wpdb->get_results(
            "{$sql['select']} {$sql['from']} WHERE {$sql['where']} {$sql['limits']}"
        );
     ...
     */
    public static function delete_invite(
        $user_id,
        $group_id,
        $inviter_id = false
    ) {
    ...
    public static function get_random_groups($user_id = 0, $total_groups = 5)
    {
        ...
        if (bp_is_my_profile()) {
            return array_map(
                ...
                $wpdb->get_col(
                    $wpdb->prepare(
                        "SELECT DISTINCT group_id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND is_confirmed = 1 AND is_banned = 0 ORDER BY rand() LIMIT %d",
                        $user_id,
                        $total_groups
                    )
                )
            ...
            return array_map(
                ...
                $wpdb->get_col(
                    $wpdb->prepare(
                        "SELECT DISTINCT m.group_id FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id AND g.status != 'hidden' AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0 ORDER BY rand() LIMIT %d",
                        $user_id,
                        $total_groups
                    )
                )
    ...
    public static function get_group_administrator_ids($group_id)
    {
        ...
        if (false === $group_admins) {
            self::prime_group_admins_mods_cache([$group_id]);
            $group_admins = wp_cache_get($group_id, 'bp_group_admins');
        }
        ...
        if (false === $group_admins) {
            ...
            foreach ((array) $group_admins as $key => $data) {
                $group_admins[$key]->user_id =
                    (int) $group_admins[$key]->user_id;
            }
        }
    ...
    public static function prime_group_admins_mods_cache($group_ids)
    {
        ...
        if ($uncached) {
            ...
            $group_admin_mods = $wpdb->get_results(
                "SELECT user_id, group_id, date_modified, is_admin, is_mod FROM {$bp->groups->table_name_members} WHERE group_id IN ({$uncached_sql}) AND ( is_admin = 1 OR is_mod = 1 ) AND is_banned = 0"
            );
            ...
            foreach ($uncached as $group_id) {
                $group_admins = isset($admins[$group_id])
                    ? $admins[$group_id]
                    : [];
                wp_cache_set($group_id, $group_admins, 'bp_group_admins');

                $group_mods = isset($mods[$group_id]) ? $mods[$group_id] : [];
    ...
    public static function get_memberships_by_id($membership_ids)
    {
        ...
        return $wpdb->get_results(
            "SELECT * FROM {$bp->groups->table_name_members} WHERE id IN ({$membership_ids})"
        );
    }
     ...
     * @return false|array
     */
    public static function get_all_for_group(
        $group_id,
        $limit = false,
        $page = false,
        ...
        if (bp_is_active('xprofile')) {
            ...
            $members = $wpdb->get_results(
                apply_filters(
                    ...
                    $wpdb->prepare(
                        "SELECT m.user_id, m.date_modified, m.is_banned, u.user_login, u.user_nicename, u.user_email, pd.value as display_name FROM {$bp->groups->table_name_members} m, {$wpdb->users} u, {$bp->profile->table_name_data} pd WHERE u.ID = m.user_id AND u.ID = pd.user_id AND pd.field_id = 1 AND group_id = %d AND is_confirmed = 1 {$banned_sql} {$exclude_admins_sql} {$exclude_sql} ORDER BY m.date_modified DESC {$pag_sql}",
                        $group_id
                    )
                )
            );
            ...
            $members = $wpdb->get_results(
                apply_filters(
                    'bp_group_members_user_join_filter',
                    $wpdb->prepare(
                        "SELECT m.user_id, m.date_modified, m.is_banned, u.user_login, u.user_nicename, u.user_email, u.display_name FROM {$bp->groups->table_name_members} m, {$wpdb->users} u WHERE u.ID = m.user_id AND group_id = %d AND is_confirmed = 1 {$banned_sql} {$exclude_admins_sql} {$exclude_sql} ORDER BY m.date_modified DESC {$pag_sql}",
                        $group_id
                    )
                )
            );
        ...
        if (empty($pag_sql)) {
             ...
             */
            $total_member_count = $wpdb->get_var(
                apply_filters(
        ...
        if (bp_is_active('friends')) {
            $friend_status = $wpdb->get_results(
                $wpdb->prepare(
            ...
            for ($i = 0, $count = count($members); $i < $count; ++$i) {
                foreach ((array) $friend_status as $status) {
                    if (
                        $status->initiator_user_id == $members[$i]->user_id ||
                        $status->friend_user_id == $members[$i]->user_id
                    ) {
        ...
        return ['members' => $members, 'count' => $total_member_count];
    }
    ...
    public static function get_membership_ids_for_user($user_id)
    {
        ...
        $group_ids = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT id FROM {$bp->groups->table_name_members} WHERE user_id = %d ORDER BY id ASC",
                $user_id
            )
        );
    ...
    public static function delete_all($group_id)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$bp->groups->table_name_members} WHERE group_id = %d",
                $group_id
            )
        );
    }
