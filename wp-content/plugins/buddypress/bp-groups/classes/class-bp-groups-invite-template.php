...
class BP_Groups_Invite_Template
{
    ...
    public function the_invite()
    {
        ...
        if (bp_is_active('xprofile')) {
            $this->invite->user->profile_data = BP_XProfile_ProfileData::get_all_for_user(
                $user_id
            );
        }
        ...
        $this->invite->user->user_url = bp_core_get_user_domain(
            $user_id,
            $this->invite->user->user_nicename,
            $this->invite->user->user_login
        );
        $this->invite->user->user_link = "<a href='{$this->invite->user->user_url}'>{$this->invite->user->fullname}</a>";

        /* translators: %s: last activity timestamp (e.g. "Active 1 hour ago") */
        $this->invite->user->last_active = bp_core_get_last_activity(
            $this->invite->user->last_activity,
        ...
        if (bp_is_active('friends')) {
            $this->invite->user->total_friends = BP_Friends_Friendship::total_friend_count(
                $user_id
            );
        }
