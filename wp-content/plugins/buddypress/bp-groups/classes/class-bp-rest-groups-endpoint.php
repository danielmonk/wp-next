...
class BP_REST_Groups_Endpoint extends WP_REST_Controller
{
    ...
    public function register_routes()
    {
            ...
            [
                ...
                [
                    ...
                    'args' => [
                        ...
                        'populate_extras' => [
                            'description' => __(
                                'Whether to fetch extra BP data about the returned group.',
                                'buddypress'
                            ),
                            'context' => ['view', 'edit'],
                            'default' => false,
        ...
        register_rest_route($this->namespace, '/' . $this->rest_base . '/me', [
            'args' => [
                'max' => [
                    'description' => __(
                        'The maximum amount of groups the user is member of to return. Defaults to all groups.',
                        'buddypress'
                    ),
                    'type' => 'integer',
                    'default' => 0,
    ...
    public function get_items($request)
    {
        ...
        if (
            isset($request['show_hidden']) &&
            true === (bool) $request['show_hidden'] &&
            !$this->can_see_hidden_groups($request)
        ) {
    ...
    public function get_current_user_groups_permissions_check($request)
    {
        $retval = new WP_Error(
            'bp_rest_authorization_required',
            __(
                'Sorry, you need to be logged in to view your groups.',
                'buddypress'
            ),
            [
                'status' => rest_authorization_required_code(),
    ...
    public function prepare_item_for_response($item, $request)
    {
        $data = [
            ...
            'date_created' => bp_rest_prepare_date_response(
                $item->date_created
            ),
            'description' => [
        ...
        if (
            'edit' === $context ||
            true === $request->get_param('populate_extras')
        ) {
            $data['total_member_count'] = (int) $item->total_member_count;
            $data['last_activity'] = bp_rest_prepare_date_response(
                $item->last_activity
            );
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        // Group Creator ID.
        if (
            !empty($schema['properties']['creator_id']) &&
            isset($request['creator_id'])
        ) {
        ...
        if (
            ...
            (isset($group->slug) &&
                isset($prepared_group->slug) &&
                $group->slug !== $prepared_group->slug)
        ) {
            ...
            $prepared_group->slug = groups_check_slug(
                sanitize_title(esc_attr($prepared_group->slug))
            );
        }
        ...
        // Group Forum Enabled.
        if (
            !empty($schema['properties']['enable_forum']) &&
            isset($request['enable_forum'])
        ) {
        ...
        // Group Parent ID.
        if (
            !empty($schema['properties']['parent_id']) &&
        ...
        // Remove group type(s).
        if (
            isset($prepared_group->group_id) &&
            isset($request['remove_types'])
        ) {
            array_map(function ($type) use ($prepared_group) {
        ...
        if (
            isset($prepared_group->group_id) &&
            isset($request['append_types'])
        ) {
            bp_groups_set_group_type(
                $prepared_group->group_id,
                $request['append_types'],
                true
            );
        }
    ...
    public function get_endpoint_args_for_item_schema(
        ...
        if (
            ...
            $args['types'] = [
                'description' => __(
                    'Assign one or more type to a group. To assign more than one type, use a comma separated list of types.',
                    'buddypress'
                ),
                'type' => 'string',
            ...
            if (WP_REST_Server::EDITABLE === $method) {
                ...
                $args['append_types'] = [
                    'description' => __(
                        'Append one or more type to a group. To append more than one type, use a comma separated list of types.',
                        'buddypress'
                    ),
                    'type' => 'string',
                    'enum' => bp_groups_get_group_types(),
                ...
                $args['remove_types'] = [
                    'description' => __(
                        'Remove one or more type of a group. To remove more than one type, use a comma separated list of types.',
                        'buddypress'
                    ),
                    'type' => 'string',
                    'enum' => bp_groups_get_group_types(),
        ...
        $schema = [
            ...
            'properties' => [
                ...
                'enable_forum' => [
                    'context' => ['view', 'edit'],
                    'description' => __(
                        'Whether the Group has a forum enabled or not.',
                        'buddypress'
                    ),
                    'type' => 'boolean',
                ],
    ...
    public function get_collection_params()
    {
        ...
        $params['orderby'] = [
            'description' => __(
                'Order Groups by which attribute.',
                'buddypress'
            ),
        ...
        $params['status'] = [
            'description' => __(
                'Group statuses to limit results to.',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['parent_id'] = [
            'description' => __(
                'Get Groups that are children of the specified Group(s) IDs.',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['meta'] = [
            'description' => __(
                'Get Groups based on their meta data information.',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['include'] = [
            'description' => __(
                'Ensure result set includes Groups with specific IDs.',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['exclude'] = [
            'description' => __(
                'Ensure result set excludes Groups with specific IDs',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['group_type'] = [
            'description' => __(
                'Limit results set to a certain Group type.',
                'buddypress'
            ),
            'default' => '',
            'type' => 'string',
        ...
        $params['show_hidden'] = [
            'description' => __(
                'Whether results should include hidden Groups.',
                'buddypress'
            ),
            'default' => false,
            'type' => 'boolean',
        ...
        $params['populate_extras'] = [
            'description' => __(
                'Whether to fetch extra BP data about the returned groups.',
                'buddypress'
            ),
            'default' => false,
            'type' => 'boolean',
