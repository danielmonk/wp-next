...
class BP_Groups_Group_Members_Template
{
    ...
    public function __construct($args = [])
    {
        ...
        if (
            empty($current_group) ||
            ($current_group && $current_group->id !== bp_get_current_group_id())
        ) {
        ...
        $base_url = trailingslashit(
            bp_get_group_permalink($current_group) . bp_current_action()
        );
        ...
        $this->pag_links = paginate_links([
            'base' => add_query_arg([$this->pag_arg => '%#%'], $base_url),
            'format' => '',
            'total' => !empty($this->pag_num)
                ? ceil($this->total_member_count / $this->pag_num)
