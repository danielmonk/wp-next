...
class BP_Groups_Group
{
    ...
    public function populate()
    {
        ...
        if (false === $group) {
            $group = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT g.* FROM {$bp->groups->table_name} g WHERE g.id = %d",
                    $this->id
                )
            );
    ...
    public function save()
    {
        ...
        $this->parent_id = apply_filters(
            'groups_group_parent_id_before_save',
            $this->parent_id,
            $this->id
        );
        $this->enable_forum = apply_filters(
         ...
         */
        do_action_ref_array('groups_group_before_save', [&$this]);
         ...
         */
        do_action_ref_array('groups_group_after_save', [&$this]);
    ...
    public function delete()
    {
        ...
        $wpdb->query(
            "UPDATE {$wpdb->usermeta} SET meta_value = meta_value - 1 WHERE meta_key = 'total_group_count' AND user_id IN ( {$user_id_str} )"
        );
         ...
         */
        do_action_ref_array('bp_groups_delete_group', [&$this, $user_ids]);
        ...
        if (
            !$wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM {$bp->groups->table_name} WHERE id = %d",
                    $this->id
                )
            )
        ) {
    ...
    public function __get($key)
    {
        switch ($key) {
            ...
            case 'is_invited':
                return groups_check_user_has_invite(
                    bp_loggedin_user_id(),
                    $this->id
                );

            case 'is_pending':
                return groups_check_for_membership_request(
                    bp_loggedin_user_id(),
                    $this->id
                );
    ...
    protected function get_is_member()
    {
        ...
        $this->is_member = groups_is_user_member(
            bp_loggedin_user_id(),
            $this->id
        );
        return $this->is_member;
    ...
    protected function get_user_has_access()
    {
        ...
        if ('private' === $this->status || 'hidden' === $this->status) {
            ...
            if (
                (is_user_logged_in() && $this->get_is_member()) ||
     ...
     * }
     */
    public static function filter_user_groups(
        $filter,
        $user_id = 0,
        $order = false,
        $limit = null,
        ...
        return ['groups' => $paged_groups, 'total' => $groups['total']];
    }
     ...
     * }
     */
    public static function get_membership_requests(
        $group_id,
        $limit = null,
        $page = null
    ) {
    ...
    public static function get($args = [])
    {
        ...
        if (!empty($r['user_id'])) {
            $sql[
                'from'
            ] .= " JOIN {$bp->groups->table_name_members} m ON ( g.id = m.group_id )";
        }
        ...
        if ($search) {
            ...
            foreach ($search_columns as $search_column) {
                $searches[] = $wpdb->prepare(
                    "$search_column LIKE %s",
                    $wildcarded
                );
            }

            $where_conditions['search'] =
                '(' . implode(' OR ', $searches) . ')';
        }
        ...
        if (!empty($r['user_id'])) {
            $where_conditions['user'] = $wpdb->prepare(
                "m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0",
                $r['user_id']
            );
        }
        ...
        if ('total_member_count' === $orderby) {
            $sql[
                'from'
            ] .= " JOIN {$bp->groups->table_name_groupmeta} gm_total_member_count ON ( g.id = gm_total_member_count.group_id )";
            $where_conditions['total_member_count'] =
                "gm_total_member_count.meta_key = 'total_member_count'";
        } elseif ('last_activity' === $orderby) {
            $sql[
                'from'
            ] .= " JOIN {$bp->groups->table_name_groupmeta} gm_last_activity on ( g.id = gm_last_activity.group_id )";
            $where_conditions['last_activity'] =
                "gm_last_activity.meta_key = 'last_activity'";
        }
        ...
        $orderby = apply_filters(
            ...
            self::convert_orderby_to_order_by_term($orderby),
            $orderby,
            $r['type']
        );
        ...
        if ('ids' === $r['fields']) {
            ...
            $uncached_group_ids = bp_get_non_cached_ids(
                $paged_group_ids,
                'bp_groups'
            );
            if ($uncached_group_ids) {
                $group_ids_sql = implode(
                    ',',
                    array_map('intval', $uncached_group_ids)
                );
                $group_data_objects = $wpdb->get_results(
                    "SELECT g.* FROM {$bp->groups->table_name} g WHERE g.id IN ({$group_ids_sql})"
                );
                foreach ($group_data_objects as $group_data_object) {
                    wp_cache_set(
                        $group_data_object->id,
                        $group_data_object,
                        'bp_groups'
                    );
                }
            ...
            $int_props = ['id', 'creator_id', 'enable_forum'];

            // Integer casting.
            foreach ($paged_groups as $key => $g) {
                foreach ($int_props as $int_prop) {
                    $paged_groups[$key]->{$int_prop} =
                        (int) $paged_groups[$key]->{$int_prop};
                }
            }
        }
     ...
     */
    protected static function get_meta_query_sql($meta_query = [])
    {
        global $wpdb;
    ...
    protected static function convert_orderby_to_order_by_term($orderby)
    {
        ...
        switch ($orderby) {
            ...
            case 'total_member_count':
                $order_by_term =
                    'CONVERT(gm_total_member_count.meta_value, SIGNED)';
                break;
            ...
            case 'meta_id':
                $order_by_term =
                    buddypress()->groups->table_name_groupmeta . '.id';
                break;
        }
     ...
     * }
     */
    public static function get_by_letter(
        $letter,
        $limit = null,
        $page = null,
     ...
     */
    public static function get_group_extras(
        &$paged_groups,
        &$group_ids,
        $type = false
    ) {
        ...
        foreach ($paged_groups as &$group) {
            ...
            $group->is_pending = groups_is_user_pending($user_id, $group->id)
                ? 1
                : 0;
            $group->is_banned = (bool) groups_is_user_banned(
                $user_id,
                $group->id
            );
        }
     ...
     */
    protected static function get_sql_clause_for_group_types(
        $group_types,
        $operator
    ) {
        global $wpdb;
