...
class BP_Groups_Component extends BP_Component
{
            ...
            if (bp_is_group()) {
                ...
                if (bp_is_item_admin() && is_user_logged_in()) {
                    ...
                    if (
                        ...
                        require $this->path .
                            'bp-groups/screens/single/admin/' .
                            bp_get_group_current_admin_tab() .
                            '.php';
                    }
                }
    ...
    public function setup_globals($args = [])
    {
        ...
        $args = [
            'slug' => BP_GROUPS_SLUG,
            'root_slug' => isset($bp->pages->groups->slug)
                ? $bp->pages->groups->slug
                : BP_GROUPS_SLUG,
            'has_directory' => true,
            'directory_title' => isset($bp->pages->groups->title)
                ? $bp->pages->groups->title
                : $default_directory_title,
        ...
        if (
            ...
            (($group_id = BP_Groups_Group::group_exists(bp_current_action())) ||
                ($group_id = BP_Groups_Group::get_id_by_previous_slug(
                    bp_current_action()
                )))
        ) {
             ...
             */
            $current_group_class = apply_filters(
            ...
            if ($current_group_class == 'BP_Groups_Group') {
                 ...
                 */
                $this->current_group = apply_filters(
                    'bp_groups_current_group_object',
                    new $current_group_class($group_id)
                );
            }
            ...
            $this->current_group->front_template = bp_groups_get_front_template(
                $this->current_group
            );
         ...
         */
        $this->forbidden_names = apply_filters('groups_forbidden_names', [
            'my-groups',
            'create',
            'invites',
            'send-invites',
        ...
        if (
            bp_is_groups_component() &&
            empty($this->current_group) &&
            empty($this->current_directory_type) &&
            bp_current_action() &&
            !in_array(bp_current_action(), $this->forbidden_names)
        ) {
            bp_do_404();
            return;
        }
        ...
        if (!$disabled_avatar_uploads && $bp->avatar->show_avatars) {
            $this->group_creation_steps['group-avatar'] = [
        ...
        if (bp_group_use_cover_image_header()) {
            $this->group_creation_steps['group-cover-image'] = [
        ...
        if (bp_is_active('friends')) {
            $this->group_creation_steps['group-invites'] = [
         ...
         */
        $this->valid_status = apply_filters('groups_valid_status', [
            'public',
            'private',
            'hidden',
        ]);
        ...
        // Auto join group when non group member performs group activity.
        $this->auto_join =
            defined('BP_DISABLE_AUTO_GROUP_JOIN') && BP_DISABLE_AUTO_GROUP_JOIN
                ? false
                : true;
    }
    ...
    public function setup_canonical_stack()
    {
         ...
         *                      else 'home'.
         */
        $this->default_extension = apply_filters(
            'bp_groups_default_extension',
            defined('BP_GROUPS_DEFAULT_EXTENSION')
                ? BP_GROUPS_DEFAULT_EXTENSION
                : 'home'
        );
        ...
        if (
            'members' === $this->default_extension &&
            !bp_is_active('activity') &&
            !$this->current_group->front_template
        ) {
        ...
        $bp->canonical_stack['base_url'] = bp_get_group_permalink(
            $this->current_group
        );
        ...
        if (
            ...
            (bp_is_current_action('activity') ||
                (!bp_is_active('activity') && bp_is_current_action('members')))
        ) {
        ...
        // action variables.
        if (
            bp_is_current_action($this->default_extension) &&
            empty($bp->action_variables)
        ) {
    ...
    public function setup_nav($main_nav = [], $sub_nav = [])
    {
        ...
        if (bp_is_user()) {
            $class =
                0 === groups_total_groups_for_user(bp_displayed_user_id())
                    ? 'no-count'
                    : 'count';
            ...
            $nav_name = _x(
                'Groups',
                'Group screen nav without counter',
                'buddypress'
            );
        }
            ...
            if (
                ...
                if (
                    ...
                    $sub_nav[] = [
                        ...
                        'position' => 11,
                        'user_has_access' =>
                            $this->current_group->user_has_access,
            ...
            if (bp_is_my_profile() && !bp_is_single_item()) {
                ...
                if (empty($bp->bp_options_avatar)) {
                    $bp->bp_options_avatar =
                        ...
                        esc_url(bp_core_avatar_default_thumb()) .
                        '" alt="' .
                        esc_attr__('No Group Profile Photo', 'buddypress') .
                        '" class="avatar" />';
                }
            }
        }
     ...
     *                           description.
     */
    public function rest_api_init($controllers = [])
    {
        $controllers = [
     ...
     *                      description.
     */
    public function blocks_init($blocks = [])
    {
        parent::blocks_init([
            'bp/group' => [
                'name' => 'bp/group',
                'editor_script' => 'bp-group-block',
                'editor_script_url' => plugins_url(
                    'js/blocks/group.js',
                    dirname(__FILE__)
                ),
                'editor_script_deps' => [
            ...
            'bp/groups' => [
                ...
                'attributes' => [
                    ...
                    'extraInfo' => [
                        'type' => 'string',
                        'default' => 'none',
                        'enum' => ['description', 'popular', 'active', 'none'],
                    ],
                    'layoutPreference' => [
                        'type' => 'string',
                        'default' => 'list',
