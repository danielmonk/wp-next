...
class BP_Group_Extension
{
     ...
     * @var array
     */
    protected $legacy_properties_converted = [];
    ...
    public function init($args = [])
    {
        ...
        // Parse with defaults.
        $this->params = $this->parse_args_r($args, [
            'slug' => $this->slug,
            'name' => $this->name,
    ...
    public static function get_group_id()
    {
        ...
        if (
            empty($group_id) &&
            is_admin() &&
            (isset($_GET['page']) && 'bp-groups' === $_GET['page']) &&
            !empty($_GET['gid'])
        ) {
    ...
    protected function get_default_screens()
    {
        ...
        foreach ($screens as $context => &$screen) {
            ...
            $screen['screen_callback'] = $this->get_screen_callback(
                $context,
                'screen'
            );
            $screen['screen_save_callback'] = $this->get_screen_callback(
                $context,
                'screen_save'
            );
        }
    ...
    protected function setup_access_settings()
    {
        ...
        if (empty($this->params['show_tab'])) {
            if (false === $this->params['enable_nav_item']) {
                ...
                isset($this->params_raw['enable_nav_item']) ||
                isset($this->params_raw['visibility'])
            ) {
    ...
    protected function user_meets_access_condition($access_condition)
    {
        switch ($access_condition) {
            case 'admin':
                $meets_condition = groups_is_user_admin(
                    bp_loggedin_user_id(),
                    $this->group_id
                );
                break;
            ...
            case 'mod':
                $meets_condition = groups_is_user_mod(
                    bp_loggedin_user_id(),
                    $this->group_id
                );
                break;
            ...
            case 'member':
                $meets_condition = groups_is_user_member(
                    bp_loggedin_user_id(),
                    $this->group_id
                );
                break;
    ...
    protected function setup_display_hooks()
    {
        ...
        if (
            ...
            !buddypress()->groups->current_group->user_has_access
        ) {
            return;
        }
        ...
        if ($user_can_see_nav_item) {
            ...
            bp_core_create_subnav_link(
                [
                    ...
                    'screen_function' => [&$this, '_display_hook'],
        ...
        if ($user_can_visit) {
            ...
            if (bp_is_current_action($this->slug)) {
                add_filter(
                    ...
                    [$this, 'group_access_protection'],
                    10,
                    2
                );

                $extension_name = $this->name;
                add_action('bp_template_content_header', function () use (
                    $extension_name
                ) {
                    echo esc_attr($extension_name);
                });
                add_action('bp_template_title', function () use (
                    $extension_name
                ) {
                    echo esc_attr($extension_name);
                });
        ...
        if (bp_is_group_home()) {
            add_action($this->display_hook, [&$this, 'widget_display']);
        }
    }
    ...
    {
        add_action('bp_template_content', [&$this, 'call_display']);
        ...
        bp_core_load_template(
            apply_filters('bp_core_template_plugin', $this->template_file)
        );
    }
    ...
    public function group_access_protection($user_can_visit, &$no_access_args)
    {
        ...
        if (!$user_can_visit && is_user_logged_in()) {
            ...
            $no_access_args['message'] = __(
                'You do not have access to this content.',
                'buddypress'
            );
            $no_access_args['root'] =
                bp_get_group_permalink($current_group) . 'home/';
    ...
    protected function setup_create_hooks()
    {
        ...
        buddypress()->groups->group_creation_steps[$screen['slug']] = [
            'name' => $screen['name'],
            'slug' => $screen['slug'],
        ...
        add_action('groups_custom_create_steps', [
            $this,
            'maybe_create_screen',
        ]);
        add_action('groups_create_group_step_save_' . $screen['slug'], [
            $this,
    ...
    public function maybe_create_screen()
    {
        ...
        call_user_func(
            $this->screens['create']['screen_callback'],
            $this->group_id
        );
        ...
        wp_nonce_field(
            'groups_create_save_' . bp_get_groups_current_create_step(),
            '_wpnonce',
            false
        );
    }
    ...
    public function maybe_create_screen_save()
    {
        ...
        call_user_func(
            $this->screens['create']['screen_save_callback'],
            $this->group_id
        );
    }
    ...
    protected function setup_edit_hooks()
    {
        ...
        $admin_link = trailingslashit(
            bp_get_group_permalink($current_group) . 'admin'
        );
        ...
        if (
            ...
            bp_is_action_variable($screen['slug'], 0)
        ) {
            $this->call_edit_screen_save($this->group_id);

            add_action('groups_custom_edit_steps', [
                &$this,
                'call_edit_screen',
            ]);
            ...
            // loading.
            if ('' !== bp_locate_template(['groups/single/home.php'], false)) {
                ...
                add_action('bp_template_content', [&$this, 'call_edit_screen']);
    ...
    public function call_edit_screen()
    {
        ob_start();
        call_user_func(
            $this->screens['edit']['screen_callback'],
            $this->group_id
        );
    ...
    public function call_edit_screen_save()
    {
        ...
        add_filter('wp_redirect', [$this, 'detect_post_save_redirect']);

        // Call the extension's save routine.
        call_user_func(
            $this->screens['edit']['screen_save_callback'],
            $this->group_id
        );

        // Clean up detection filters.
        remove_filter('wp_redirect', [$this, 'detect_post_save_redirect']);
    ...
    protected function setup_admin_hooks()
    {
        ...
        add_action('bp_groups_admin_meta_box_content_' . $this->slug, [
            $this,
            'call_admin_screen',
        ]);
    ...
    {
        call_user_func(
            $this->screens['admin']['screen_callback'],
            $this->group_id
        );
    ...
    public function call_admin_screen_save()
    {
        ...
        call_user_func(
            $this->screens['admin']['screen_save_callback'],
            $this->group_id
        );
    }
    ...
    public function _meta_box_display_callback()
    {
        ...
        $callback = function () use ($extension_slug, $group_id) {
            do_action(
                'bp_groups_admin_meta_box_content_' . $extension_slug,
                $group_id
            );
        };
    ...
    {
        wp_nonce_field(
            'bp_group_extension_' . $this->slug . '_' . $context,
            '_bp_group_' . $context . '_nonce_' . $this->slug
        );
    }
    ...
    public function is_screen_enabled($context = '')
    {
        ...
        if (isset($this->screens[$context])) {
            $enabled =
                $this->screens[$context]['enabled'] &&
                is_callable($this->screens[$context]['screen_callback']);
        }
    ...
    public function get_screen_callback($context = '', $type = 'screen')
    {
        ...
        if (empty($callback)) {
            ...
            $rfallback_method = $this->class_reflection->getMethod(
                $fallback_method
            );
            if (
                isset($rfallback_method->class) &&
                $this->class_name === $rfallback_method->class
            ) {
                $callback = [$this, $fallback_method];
            }
        }
    ...
    protected function setup_legacy_properties()
    {
        ...
        foreach ($properties as $property) {
            switch ($property) {
                case 'enable_create_step':
                    $lp['enable_create_step'] =
                        $params['screens']['create']['enabled'];
                    break;

                case 'enable_edit_item':
                    $lp['enable_edit_item'] =
                        $params['screens']['edit']['enabled'];
                    break;

                case 'enable_admin_item':
                    $lp['enable_admin_item'] =
                        $params['screens']['admin']['enabled'];
                    break;

                case 'create_step_position':
                    $lp['create_step_position'] =
                        $params['screens']['create']['position'];
                    break;
                ...
                case 'admin_metabox_context':
                    $lp['admin_metabox_context'] =
                        $params['screens']['admin']['metabox_context'];
                    break;

                case 'admin_metabox_priority':
                    $lp['admin_metabox_priority'] =
                        $params['screens']['admin']['metabox_priority'];
                    break;
