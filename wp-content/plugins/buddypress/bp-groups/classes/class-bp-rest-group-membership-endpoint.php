...
class BP_REST_Group_Membership_Endpoint extends WP_REST_Controller
{
    ...
    public function register_routes()
    {
        register_rest_route(
            ...
            [
                ...
                [
                    ...
                    'args' => $this->get_endpoint_args_for_method(
        ...
        register_rest_route(
            ...
            '/' .
                $this->rest_base .
                '/(?P<group_id>[\d]+)/members/(?P<user_id>[\d]+)',
            [
                'args' => [
                    'group_id' => [
                        'description' => __(
                    ...
                    'user_id' => [
                        'description' => __(
                            'A unique numeric ID for the Group Member.',
                            'buddypress'
                        ),
                        'type' => 'integer',
                    ],
    ...
    public function get_items($request)
    {
         ...
         */
        $args = apply_filters(
            'bp_rest_group_members_get_items_query_args',
            $args,
            $request
        );
         ...
         */
        do_action(
            'bp_rest_group_members_get_items',
            $members,
            $group,
            $response,
            $request
    ...
    public function get_items_permissions_check($request)
    {
        ...
        if (empty($group->id)) {
            ...
            'public' === $group->status ||
            groups_is_user_member(bp_loggedin_user_id(), $group->id)
        ) {
            $retval = true;
        }
        ...
        return apply_filters(
            'bp_rest_group_members_get_items_permissions_check',
            $retval,
            $request
        );
    }
    ...
    public function create_item($request)
    {
         ...
         */
        do_action(
            'bp_rest_group_members_create_item',
            $user,
            $group_member,
            $group,
            $response,
    ...
    public function create_item_permissions_check($request)
    {
        ...
        if (!is_user_logged_in()) {
            $retval = new WP_Error(
                'bp_rest_authorization_required',
                __(
                    'Sorry, you need to be logged in to join a group.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
         ...
         */
        return apply_filters(
            'bp_rest_group_members_create_item_permissions_check',
            $retval,
            $request
        );
    }
    ...
    public function update_item($request)
    {
        ...
        if ('promote' === $action) {
            ...
            if (!$group_member->$action()) {
                $messages = [
                    'demote' => __(
                        'Could not demote member from the group.',
                        'buddypress'
                    ),
                    'ban' => __(
                        'Could not ban member from the group.',
                        'buddypress'
                    ),
                    'unban' => __(
                        'Could not unban member from the group.',
                        'buddypress'
                    ),
                ];
         ...
         */
        do_action(
            'bp_rest_group_members_update_item',
            $user,
            $group_member,
            $group,
            $response,
    ...
    public function update_item_permissions_check($request)
    {
        ...
        if (!is_user_logged_in()) {
            $retval = new WP_Error(
                'bp_rest_authorization_required',
                __(
                    'Sorry, you need to be logged in to make an update.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
            ...
            if (!$user instanceof WP_User) {
                ...
                if (!$group instanceof BP_Groups_Group) {
                    ...
                    if (groups_is_user_admin($loggedin_user_id, $group->id)) {
                        ...
                        $messages = [
                            'ban' => __(
                                'Sorry, you are not allowed to ban this group member.',
                                'buddypress'
                            ),
                            'unban' => __(
                                'Sorry, you are not allowed to unban this group member.',
                                'buddypress'
                            ),
                            'promote' => __(
                                'Sorry, you are not allowed to promote this group member.',
                                'buddypress'
                            ),
                            'demote' => __(
                                'Sorry, you are not allowed to demote this group member.',
                                'buddypress'
                            ),
                        ];
         ...
         */
        return apply_filters(
            'bp_rest_group_members_update_item_permissions_check',
            $retval,
            $request
        );
    }
    ...
    public function delete_item($request)
    {
         ...
         */
        do_action(
            'bp_rest_group_members_delete_item',
            $user,
            $member,
            $group,
            $response,
    ...
    public function delete_item_permissions_check($request)
    {
        ...
        if (!is_user_logged_in()) {
            $retval = new WP_Error(
                'bp_rest_authorization_required',
                __(
                    'Sorry, you need to be logged in to view a group membership.',
                    'buddypress'
                ),
                [
                    'status' => rest_authorization_required_code(),
            ...
            if (!$user instanceof WP_User) {
                ...
                if (!$group instanceof BP_Groups_Group) {
                    ...
                    bp_current_user_can('bp_moderate') ||
                    ($user->ID !== $loggedin_user_id &&
                        groups_is_user_admin($loggedin_user_id, $group->id))
                ) {
                    $retval = true;
                } elseif (
                    $user->ID === $loggedin_user_id &&
                    !groups_is_user_banned($user->ID, $group->id)
                ) {
                    $group_admins = groups_get_group_admins($group->id);
                    ...
                    if (
                        in_array(
                            $loggedin_user_id,
                            wp_list_pluck($group_admins, 'user_id'),
                            true
                        )
                    ) {
                        if (1 !== count($group_admins)) {
         ...
         */
        return apply_filters(
            'bp_rest_group_members_delete_item_permissions_check',
            $retval,
            $request
        );
    }
    ...
    public function prepare_item_for_response($group_member, $request)
    {
        ...
        $member_data = $this->members_endpoint->user_data(
            $user,
            $context,
            $request
        );
         ...
         */
        return apply_filters(
            'bp_rest_group_members_prepare_value',
            $response,
            $request,
            $group_member
        );
    ...
    protected function prepare_links($user)
    {
         ...
         */
        return apply_filters(
            'bp_rest_group_members_prepare_links',
            $links,
            $user
        );
    }
     ...
     */
    public function get_endpoint_args_for_method(
        ...
        if (
            ...
            $group_roles = array_diff(array_keys(bp_groups_get_group_roles()), [
                'banned',
            ]);

            $args['role'] = [
                'description' => __(
                    'Group role to assign the user to.',
                    'buddypress'
                ),
                'default' => 'member',
                'type' => 'string',
            ...
            if (WP_REST_Server::CREATABLE === $method) {
                $key = 'create_item';
                $schema = $this->get_item_schema();
                $args['user_id'] = array_merge($schema['properties']['id'], [
                    'description' => __(
                        'A unique numeric ID for the Member to add to the Group.',
                        'buddypress'
                    ),
            ...
            if (WP_REST_Server::EDITABLE === $method) {
                ...
                $args['action'] = [
                    'description' => __(
                        'Action used to update a group member.',
                        'buddypress'
                    ),
                    'default' => 'promote',
                    'type' => 'string',
                    'enum' => ['promote', 'demote', 'ban', 'unban'],
                    'sanitize_callback' => 'sanitize_key',
                    'validate_callback' => 'rest_validate_request_arg',
                ];
            }
         ...
         */
        return apply_filters(
            "bp_rest_group_members_{$key}_query_arguments",
            $args,
            $method
        );
    }
    ...
    public function get_item_schema()
    {
        ...
        $schema['properties']['is_mod'] = [
            'context' => ['view', 'edit'],
            'description' => __(
                'Whether the member is a group moderator.',
                'buddypress'
            ),
            'type' => 'boolean',
        ];

        $schema['properties']['is_banned'] = [
            'context' => ['view', 'edit'],
            'description' => __(
                'Whether the member has been banned from the group.',
                'buddypress'
            ),
            'type' => 'boolean',
        ];

        $schema['properties']['is_admin'] = [
            'context' => ['view', 'edit'],
            'description' => __(
                'Whether the member is a group administrator.',
                'buddypress'
            ),
            'type' => 'boolean',
        ];

        $schema['properties']['is_confirmed'] = [
            'context' => ['view', 'edit'],
            'description' => __(
                'Whether the membership of this user has been confirmed.',
                'buddypress'
            ),
            'type' => 'boolean',
        ];

        $schema['properties']['date_modified'] = [
            'context' => ['view', 'edit'],
            'description' => __(
    ...
    public function get_collection_params()
    {
        ...
        $statuses = ['last_joined', 'first_joined', 'alphabetical'];
        ...
        $params['exclude'] = [
            'description' => __(
                'Ensure result set excludes specific member IDs.',
                'buddypress'
            ),
            'default' => [],
            'type' => 'array',
        ...
        $params['exclude_admins'] = [
            'description' => __(
                'Whether results should exclude group admins and mods.',
                'buddypress'
            ),
            'default' => true,
            'type' => 'boolean',
        ...
        $params['exclude_banned'] = [
            'description' => __(
                'Whether results should exclude banned group members.',
                'buddypress'
            ),
            'default' => true,
            'type' => 'boolean',
         ...
         */
        return apply_filters(
            'bp_rest_group_members_collection_params',
            $params
        );
    }
}
