...
if (!class_exists('myCRED_Hook')):
    abstract class myCRED_Hook
    {
        ...
        {
            wp_die(
                'function myCRED_Hook::run() must be over-ridden in a sub-class.'
            );
        }
         ...
         * @version 1.3
         */
        public function over_hook_limit(
            $instance = '',
            $reference = '',
            $user_id = null,
            $ref_id = null
        ...
        public function hook_limit_setting($name = '', $id = '', $selected = '')
        {
            ...
            $output =
                '<div class="h2"><input type="' .
                $hide .
                '" size="8" class="mini" name="' .
                $name .
                '" id="' .
                $id .
                '" value="' .
         ...
         * @version 1.3
         */
        public function impose_limits_dropdown(
            $pref_id = '',
            $use_select = true
        ) {
            $settings = '';
            ...
            $limits = apply_filters(
                'mycred_hook_impose_limits',
                $limits,
                $this
            );

            echo '<select name="' .
                $this->field_name($pref_id) .
                '" id="' .
                $this->field_id($pref_id) .
                '" class="form-control">';
         ...
         * @version 1.0
         */
        public function available_template_tags($available = [], $custom = '')
        {
         ...
         * @version 1.1.1
         */
        public function is_over_daily_limit(
            $ref = '',
            $user_id = 0,
            $max = 0,
            $ref_id = null
            ...
            // DB Query
            $total = $this->limit_query(
                $ref,
                $user_id,
                strtotime('today midnight', $this->now),
                $this->now,
                $ref_id
            );
        ...
        public function include_post_type($post_type)
        {
            ...
            if (
                in_array(
                    $post_type,
                    apply_filters('mycred_post_type_excludes', $excludes)
                )
            ) {
                return false;
            }
        ...
        public function limit_query(
            ...
            // DB Query
            $total = $wpdb->get_var(
                "SELECT COUNT(*) FROM {$mycred_log_table} WHERE {$wheres};"
            );
            if ($total === null) {
                $total = 0;
            }
