    ...
    abstract class myCRED_Module
    {
            ...
            // Request to register settings
                ...
                if (!empty($this->option_id)) {
                    ...
                    if (is_array($this->option_id)) {
                        ...
                        foreach (
                            $this->option_id
                            as $option_id => $option_name
                        ) {
                            $settings = mycred_get_option($option_name, false);

                            if (
                                $settings === false &&
                                array_key_exists($option_id, $defaults)
                            ) {
                                $this->$module[$option_name] =
                                    $this->default_prefs[$option_id];
                            } else {
                            ...
                            if (
                                ...
                                $this->$module[$option_name] = wp_parse_args(
                                    $this->$module[$option_name],
                                    $this->default_prefs[$option_id]
                                );
                            }
        ...
        public function load()
        {
            if (!empty($this->screen_id) && !empty($this->labels['menu'])) {
                ...
                add_action('admin_init', [$this, 'set_entries_per_page']);
            }
            ...
            add_action('mycred_widgets_init', [$this, 'module_widgets_init']);
            add_action(
        ...
        public function add_menu()
        {
            ...
            if (
                $this->module_id != 'myCRED_Log_Module' &&
            ...
            if (!empty($this->labels) && !empty($this->screen_id)) {
                // Menu Slug
                $menu_slug = !$this->is_main_type
                    ? MYCRED_SLUG . '_' . $this->mycred_type
                    : MYCRED_SLUG;
        ...
        {
            if (empty($this->option_id) || $this->register === false) {
                return;
            }

            register_setting($this->settings_name, $this->option_id, [
                $this,
                'sanitize_settings',
            ]);
        }
...
h4.ui-accordion-header.ui-state-active:before { content: "<?php _e(
    'click to close',
    'mycred'
); ?>"; }
h4.ui-accordion-header:before { content: "<?php _e(
    'click to open',
    'mycred'
); ?>"; }
</style>
<?php
         ...
         * @version 1.0
         */
        public function available_template_tags($available = [])
        {
        ...
        {
            $variables = ['page' => MYCRED_SLUG . '-settings'];
            if (!empty($module)) {
