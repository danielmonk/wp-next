<?php
if (!defined('myCRED_STATS_VERSION')) {
    exit();
}
...
if (!function_exists('mycred_get_color_sets')):
    ...
    {
        $color_sets = apply_filters('mycred_stats_color_sets', [
            0 => [
                'positive' => 'rgba(204,175,11,1)',
                'negative' => 'rgba(51,80,244,1)',
            ],
            1 => [
                'positive' => 'rgba(221,130,59,1)',
                'negative' => 'rgba(34,125,196,1)',
            ],
            2 => [
                'positive' => 'rgba(207,73,68,1)',
                'negative' => 'rgba(48,182,187,1)',
            ],
            3 => [
                'positive' => 'rgba(180,60,56,1)',
                'negative' => 'rgba(75,195,199,1)',
            ],
            4 => [
                'positive' => 'rgba(34,34,34,1)',
                'negative' => 'rgba(221,221,221,1)',
            ],
        ]);
        ...
        if (MYCRED_STATS_COLOR_TYPE == 'hex') {
            foreach ($color_sets as $row => $setup) {
                $color_sets[$row]['positive'] = mycred_rgb_to_hex(
                    $color_sets[$row]['positive']
                );
                $color_sets[$row]['negative'] = mycred_rgb_to_hex(
                    $color_sets[$row]['negative']
                );
            }
        }
        ...
        elseif (MYCRED_STATS_COLOR_TYPE == 'rgb') {
            foreach ($color_sets as $row => $setup) {
                ...
                $check_negative = explode(
                    ',',
                    str_replace(['rgba(', 'rgb(', ')'], '', $setup['negative'])
                );

                // In a perfect world, colors are always provided in proper RBGA format.
                if (count($check_negative) == 4) {
                    $setup['negative'] = str_replace(
                        'rgba(',
                        'rgb(',
                        $setup['negative']
                    );
...
if (!function_exists('mycred_get_type_color')):
    function mycred_get_type_color($point_type = null)
    {
        ...
        $saved = (array) mycred_get_option('mycred-point-colors', []);
        ...
        foreach ($point_types as $type_id => $label) {
            ...
            if (
                !empty($saved) &&
                array_key_exists($type_id, $saved) &&
                is_array($saved[$type_id])
            ) {
                $value = $saved[$type_id];
            ...
            foreach ($value as $state => $color_value) {
                if (
                    $color_value == '' &&
                    array_key_exists($row, $color_set) &&
                    array_key_exists($state, $color_set[$row])
                ) {
...
if (!function_exists('mycred_create_chart')):
    function mycred_create_chart($args = [])
    {
        global $mycred_chart;
    ...
    function mycred_get_stat_dates($instance = 'x_dates', $value = 0)
    {
        ...
        switch ($instance) {
            case 'days':
                ...
                $start = date(
                    'U',
                    strtotime('-' . $from . ' days midnight', $now)
                );
            ...
            case 'weeks':
                ...
                $start = date(
                    'U',
                    strtotime('-' . $from . ' weeks midnight', $now)
                );
            ...
            case 'months':
                ...
                $start = date(
                    'U',
                    strtotime('-' . $from . ' months midnight', $now)
                );
            ...
            case 'years':
                ...
                $start = date(
                    'U',
                    strtotime('-' . $from . ' years midnight', $now)
                );
...
if (!function_exists('mycred_maybe_clear_user_stats_data')):
    function mycred_maybe_clear_user_stats_data($user_id = false)
    {
        ...
        $now = current_time('timestamp');
        $last_clear = mycred_get_user_meta(
            $user_id,
            'mycred-last-clear-stats',
            '',
            true
        );
        if ($last_clear == '') {
        ...
        return apply_filters(
            'mycred_maybe_clear_user_stats_data',
            $clear_data,
            $last_clear,
            $settings
        );
...
if (!function_exists('mycred_delete_stats_data')):
    ...
    {
        if (!mycred_maybe_clear_stats_data() && !$force) {
            return;
        }
...
if (!function_exists('mycred_delete_user_stats_data')):
    ...
    {
        if (!mycred_maybe_clear_user_stats_data($user_id) && !$force) {
            return;
        }
    ...
    function mycred_get_circulation_data()
    {
        ...
        if ($cache === false) {
            ...
            $data = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT SUM( meta_value ) AS value, meta_key AS label, 'point' AS type FROM {$wpdb->usermeta} WHERE meta_key IN ( %s" .
                        str_repeat(', %s', count($meta_keys) - 1) .
                        ") GROUP BY meta_key ORDER BY value DESC;",
                    $meta_keys
                )
            );
...
if (!function_exists('mycred_get_gains_vs_losses_data')):
    function mycred_get_gains_vs_losses_data($point_type = '')
    {
        ...
        $cache = mycred_get_option(
            MYCRED_SLUG . '-stats-gains-vs-losses' . $point_type,
            false
        );
        ...
        if ($cache === false) {
            ...
            $query = $wpdb->get_row(
                "SELECT SUM( CASE WHEN creds > 0 THEN creds END) as gains, SUM( CASE WHEN creds < 0 THEN creds END) as losses FROM {$mycred_log_table} {$where};"
            );

            $row = new StdClass();
            $row->value = $type_object->number(
                isset($query->gains) && $query->gains !== null
                    ? $query->gains
                    : 0
            );
            ...
            $row = new StdClass();
            $row->value = $type_object->number(
                isset($query->losses) && $query->losses !== null
                    ? abs($query->losses)
                    : 0
            );
            ...
            mycred_update_option(
                MYCRED_SLUG . '-stats-gains-vs-losses',
                $cache
            );
        }
...
if (!function_exists('mycred_get_top_balances_data')):
    function mycred_get_top_balances_data(
        ...
        $stats_key =
            MYCRED_SLUG .
            '-stats-' .
            md5('balances' . $point_type . $number . $order);
        ...
        if ($cache === false) {
            ...
            $data = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT meta_value AS value, user_id AS label, 'user' AS type FROM {$wpdb->usermeta} WHERE meta_key = %s ORDER BY meta_value+0 {$order} {$limit}",
...
if (!function_exists('mycred_get_top_instances_data')):
    function mycred_get_top_instances_data(
        $point_type = MYCRED_DEFAULT_TYPE_KEY,
        $number = 10,
        $order = 'DESC'
    ) {
        $stats_key =
            MYCRED_SLUG .
            '-stats-' .
            md5('instances' . $point_type . $number . $order);
        $cache = mycred_get_option($stats_key, false);
        ...
        if ($cache === false) {
            ...
            $data = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT SUM( creds ) AS value, ref AS label, 'reference' AS type FROM {$mycred_log_table} WHERE ctype = %s GROUP BY ref ORDER BY value {$order} {$limit}",
                    $point_type
                )
            );
...
if (!function_exists('mycred_get_history_data')):
    function mycred_get_history_data(
        $point_type = MYCRED_DEFAULT_TYPE_KEY,
        $period = 'days',
        $number = 10,
        $order = 'DESC'
        ...
        $stats_key =
            MYCRED_SLUG .
            '-stats-' .
            md5($point_type . $period . $number . $order);
        $cache = mycred_get_option($stats_key, false);
            ...
            if (!empty($periods)) {
                ...
                foreach ($periods as $row => $setup) {
                    $selects[] = $wpdb->prepare(
                        'AVG( CASE WHEN time BETWEEN %d AND %d THEN creds END) AS period%d',
                        $setup['from'],
                        $setup['until'],
                        $row
                    );
                ...
                $query = $wpdb->get_row(
                    "SELECT {$select} FROM {$mycred_log_table} WHERE {$where} {$limit};"
                );
                ...
                if ($query !== null) {
                    foreach ($periods as $row => $setup) {
                        ...
                        $value =
                            isset($query->$value_key) &&
                            $query->$value_key !== null
                                ? $query->$value_key
                                : 0;
...
if (!function_exists('mycred_get_users_history_data')):
    function mycred_get_users_history_data(
        $user_id = 0,
        $point_type = MYCRED_DEFAULT_TYPE_KEY,
        $period = 'days',
        $number = 10,
        ...
        if (empty($cache)) {
            ...
            if (!empty($periods)) {
                ...
                foreach ($periods as $row => $setup) {
                    $selects[] = $wpdb->prepare(
                        'SUM( CASE WHEN time BETWEEN %d AND %d THEN creds END) AS period%d',
                        $setup['from'],
                        $setup['until'],
                        $row
                    );
...
if (!function_exists('mycred_get_ref_history_data')):
    function mycred_get_ref_history_data(
        $reference = '',
        $point_type = MYCRED_DEFAULT_TYPE_KEY,
        $period = 'days',
        $number = 10,
        ...
        $stats_key =
            MYCRED_SLUG .
            '-stats-' .
            md5($reference . $point_type . $period . $number . $order);
        $cache = mycred_get_option($stats_key, false);
