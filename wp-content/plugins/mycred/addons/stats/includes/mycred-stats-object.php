...
if (!class_exists('myCRED_Chart')):
    class myCRED_Chart extends myCRED_Object
    {
         ...
         * @version 1.0
         */
        public function apply_defaults($args = [])
        {
            $defaults = [
                'type' => 'pie',
                'color' => '',
        ...
        public function generate_element_id()
        {
            ...
            return apply_filters(
                'mycred_chart_generate_element_id',
                MYCRED_SLUG . '-chart' . count($mycred_charts),
                $this
            );
        }
        ...
        public function get_canvas_styling()
        {
            ...
            return apply_filters(
                'mycred_chart_get_canvas_styling',
                $css,
                $this
            );
        }
        ...
        public function get_reference_label($reference = '')
        {
            ...
            return apply_filters(
                'mycred_chart_get_reference_label',
                $label,
                $reference,
                $this
            );
         ...
         * @version 1.0
         */
        public function generate_canvas($type = '', $input = [])
        {
            global $mycred_charts;
            ...
            if (!empty($input)) {
                ...
                foreach ($input as $set_id => $input_set) {
                    ...
                    foreach ($input_set as $row_id => $row) {
                        $value = $row->value;
                        $label = $row->label;
                        $color = array_key_exists($row_id, $this->args['color'])
                            ? $this->args['color'][$row_id]
                            : '#222';
                        ...
                        $dataset->backgroundColor[] =
                            $type != 'radar'
                                ...
                                : (MYCRED_STATS_COLOR_TYPE == 'hex'
                                    ? mycred_hex_to_rgb($color, true, '0.6')
                                    : $color);

                        if (in_array($type, ['line', 'bar', 'radar'])) {
                            $dataset->borderColor =
                                is_array($dataset->borderColor) &&
                                empty($dataset->borderColor)
                                    ? $color
                                    : $dataset->borderColor;
            ...
            $output = apply_filters(
                'mycred_chart_generate_canvas_' . $type,
                $output,
                $input,
                $this
            );
         ...
         * @version 1.0
         */
        public function pie_chart($input = [])
        {
         ...
         * @version 1.0
         */
        public function doughnut_chart($input = [])
        {
         ...
         * @version 1.0
         */
        public function line_chart($input = [])
        {
         ...
         * @version 1.0
         */
        public function bar_chart($input = [])
        {
         ...
         * @version 1.0
         */
        public function polar_chart($input = [])
        {
         ...
         * @version 1.0
         */
        public function radar_chart($input = [])
        {
