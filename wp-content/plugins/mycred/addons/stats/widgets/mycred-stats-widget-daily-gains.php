...
if (!class_exists('myCRED_Stats_Widget_Daily_Gains')):
    class myCRED_Stats_Widget_Daily_Gains extends myCRED_Statistics_Widget
    {
        ...
        function __construct($widget_id = '', $args = [])
        {
            ...
            $this->dates = mycred_get_stat_dates(
                'x_dates',
                $this->args['span']
            );
        }
        ...
        function get_data()
        {
            ...
            return $wpdb->get_results(
				...
				{$ctype_limit}
				GROUP BY ref ORDER BY total DESC LIMIT 0,%d;",
                    strtotime(
                        '-' . $this->args['span'] . ' days midnight',
                        $this->now
                    ),
                    $this->now,
                    $this->args['number']
        ...
        function get_gains()
        {
            ...
            if (count($point_types) > 0) {
                foreach ($point_types as $type_id => $label) {
                    ...
                    $count = $wpdb->get_var(
                        "SELECT COUNT( * ) FROM {$this->core->log_table} WHERE ctype = '{$type_id}';"
                    );
                    if ($count === null) {
                        $count = $num;
                    }
        ...
        {
            $label = __(
                'Most common ways your users have gained points during this period.',
                'mycred'
            );
            if ($this->args['ctypes'] != 'all') {
                $label = $this->core->template_tags_general(
                    __(
                        'Most common ways your users have gained %_plural% during this period.',
                        'mycred'
                    )
                );
            }
...
<?php if (!empty($ten_day_gain)) {
    ...
    foreach ($ten_day_gain as $item) {
        ...
        $url = add_query_arg(
            ['ref' => $item->ref, 'page' => $page_id],
            $base_url
        );
        ?>
<li>
	<strong style="color:<?php echo $this->colors[$item->type][
     'positive'
 ]; ?>;"><?php echo $label; ?></strong>
	<span class="view"><a href="<?php echo esc_url($url); ?>"><?php _e(
    'View',
    'mycred'
); ?></a></span>
	<ul>
...
<?php
    ...
    echo '</ol></div><div class="last-ten-days-chart"><canvas id="daily-gain-' .
        $this->id .
        '-chart"></canvas><div id="' .
        $this->id .
        '-legend" class="mycred-chart-legend clear clearfix"></div></div>';
} else {
    echo '<div class="mycred-empty-widget"><p>' .
        __('No data found', 'mycred') .
        '</p></div>';
} ?>
	<div class="clear clearfix"></div>
</div>
