...
if (!class_exists('myCRED_Stats_Widget_Circulation')):
    class myCRED_Stats_Widget_Circulation extends myCRED_Statistics_Widget
    {
         ...
         * Constructor
         */
        function __construct($widget_id = '', $args = [])
        {
            if ($widget_id == '') {
        ...
        function get_data()
        {
            ...
            foreach ($point_types as $type_id => $label) {
                ...
                $totals[] =
                    ...
                    $this->colors[$type_id]['positive'] .
                    '">' .
                    $mycred->format_creds($total) .
                    '</span>';
                $all = $all + $total;
                $series[] =
                    "{ value: " .
                ...
                foreach ($this->dates as $date) {
                    ...
                    if (!empty($query)) {
                        foreach ($query as $entry) {
                            if ($entry > 0) {
                                $series[$type_id][$date['key']]['gains'] =
                                    $series[$type_id][$date['key']]['gains'] +
                                    $entry;
                            } else {
                                $series[$type_id][$date['key']]['loses'] =
                                    $series[$type_id][$date['key']]['loses'] +
                                    $entry;
                            }
                        }
        ...
        function get_total($ctype = '', $positive = true)
        {
            ...
            if ($positive) {
                $total = $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT SUM( creds ) FROM {$this->core->log_table} WHERE ctype = %s AND creds > 0;",
                        $ctype
                    )
                );
            } else {
        ...
        function front_widget()
        {
            if ($this->args['ctypes'] == 'all') {
                ...
                $label = sprintf(
                    __('Total amount of %s in circulation', 'mycred'),
                    $this->ctypes[$this->args['ctypes']]
                );
            }
...
<div id="mycred-stats-overview" class="row">
	...
	<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
		<h1><?php echo $label; ?>: <?php echo $this->format_number(
    $circulation['total']
); ?></h1>
<?php if ($this->args['ctypes'] == 'all'): ?>
		<ul id="total-per-point-type"><li><?php echo implode(
      '</li><li>',
      $circulation['totals']
  ); ?></li></ul>
<?php
    ...
    $circulation['series'][] =
        ...
        $color .
        "', label: '" .
        esc_attr__('Total loses (%)', 'mycred') .
        "' }";

    //
    ?>
		<ul id="total-per-point-type"><li><?php printf(
      __('Total Gained: %s', 'mycred'),
      '<span style="color:' .
          $gain_color .
          '">' .
          $mycred->format_creds($gains) .
          '</span>'
  ); ?></li><li><?php printf(
    __('Total Spent: %s', 'mycred'),
    '<span style="color:' .
        $lose_color .
        '">' .
        $mycred->format_creds($loses) .
        '</span>'
); ?></li></ul>
<?php endif; ?>
		<div class="table-responsive mycred-stats-table">
			<table class="table table-condensed" cellpadding="0" cellspacing="0">
				...
				<tbody>
<?php foreach ($gains_loses as $type_id => $dates) {
    $mycred = mycred($type_id); ?>
...
$base_url = add_query_arg(['page' => $page_id], admin_url('admin.php'));
...
$base_url = add_query_arg(['page' => $page_id], admin_url('admin.php'));
foreach ($dates as $key => $item) {
    $url = add_query_arg(['show' => $key], $base_url);
    echo '<td class="cell"><a href="' .
        esc_url($url) .
        '">' .
        $mycred->format_number($item['gains']) .
        '</a></td>';
    $url = add_query_arg(
        ['show' => $key, 'num' => 0, 'compare' => urlencode('<')],
        $base_url
    );
    echo '<td class="cell"><a href="' .
        esc_url($url) .
        '">' .
        $mycred->format_number($item['loses']) .
        '</a></td>';
}
