...
if (!function_exists('mycred_render_email_subscriptions')):
    function mycred_render_email_subscriptions($atts = [], $content = '')
    {
        extract(
            shortcode_atts(
                [
        ...
        $unsubscriptions = mycred_get_user_meta(
            $user_id,
        ...
        if (
            isset($_REQUEST['do']) &&
            $_REQUEST['do'] == 'mycred-unsubscribe' &&
            wp_verify_nonce(
                $_REQUEST['token'],
                'update-mycred-email-subscriptions'
            )
        ) {
            if (
                isset($_POST['mycred_email_unsubscribe']) &&
                !empty($_POST['mycred_email_unsubscribe'])
            ) {
        ...
        $email_notices = $wpdb->get_results(
			...
			WHERE notices.post_type = 'mycred_email_notice' 
                ...
                '%s:9:"recipient";s:4:"user";%',
                '%s:9:"recipient";s:4:"both";%'
            )
        );
...
<form action="<?php echo esc_url(
	...
	<table class="table">
       ...
       $settings = mycred_get_email_settings($notice->ID); ?>
			...
			<?php if ($settings['recipient'] == 'admin') {
       continue;
   } ?>

			<tr>
				<td class="check"><input type="checkbox" name="mycred_email_unsubscribe[]"<?php if (
        in_array($notice->ID, $unsubscriptions)
    ) {
   ...
   endforeach; ?>
	...
	<input type="submit" class="btn btn-primary button button-primary pull-right" value="<?php _e(
     'Save Changes',
     'mycred'
 ); ?>" />
</form>
