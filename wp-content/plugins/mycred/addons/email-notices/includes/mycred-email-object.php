...
if (!class_exists('myCRED_Email')):
    class myCRED_Email extends myCRED_Object
    {
        ...
        function __construct($notice_id = null)
        {
            ...
            if (mycred_get_post_type($notice_id) != MYCRED_EMAIL_KEY) {
                return;
            }
        ...
        protected function populate($notice_id = null)
        {
            ...
            $this->point_types = (array) mycred_get_post_meta(
                $this->post_id,
                'mycred_email_ctype',
                true
            );
            if (empty($this->point_types)) {
                $this->point_types = [MYCRED_DEFAULT_TYPE_KEY];
            }
            ...
            if ($settings['senders_name'] == '') {
                $settings['senders_name'] = $this->emailnotices['from']['name'];
            }
            if ($settings['senders_email'] == '') {
                $settings['senders_email'] =
                    $this->emailnotices['from']['email'];
            }
            ...
            $this->trigger = mycred_get_post_meta(
                $this->post_id,
                'mycred_email_instance',
                true
            );
            $this->last_run = mycred_get_post_meta(
                $this->post_id,
                'mycred_email_last_run',
                true
            );
        }
         ...
         * @version 1.0
         */
        public function save_settings($setup = [])
        {
        ...
        public function schedule(
            ...
            if (empty($event) || !array_key_exists('user_id', $event)) {
                return false;
            }
         ...
         * @version 1.0
         */
        public function send($event = [], $point_type = MYCRED_DEFAULT_TYPE_KEY)
        {
            if (empty($event) || !array_key_exists('user_id', $event)) {
                return false;
            }
        ...
        public function update_last_run($timestamp = null)
        {
            ...
            $timestamp = apply_filters(
                'mycred_email_update_last_run',
                absint($timestamp),
                $this
            );

            if ($timestamp > 0) {
                $result = mycred_update_post_meta(
                    $this->post_id,
                    'mycred_email_last_run',
                    $timestamp
                );
        ...
        public function get_email_styling()
        {
            ...
            return apply_filters(
                'mycred_email_notice_get_styling',
                $style,
                $this
            );
        }
        ...
        public function get_recipients($user = '', $admin = '')
        {
            ...
            return apply_filters(
                'mycred_email_notice_get_recipients',
                $emails,
                $user,
                $admin,
                $this
        ...
        public function get_headers()
        {
            ...
            if (
                $this->settings['senders_name'] != '' &&
                $this->settings['senders_email'] != ''
            ) {
                $headers[] =
                    'From: ' .
                    $this->settings['senders_name'] .
            ...
            return apply_filters(
                'mycred_email_notice_get_headers',
                $headers,
                $this
            );
        }
        ...
        public function get_subject(
            ...
            if (!empty($subject)) {
                ...
                $subject = $mycred->template_tags_user(
                    $subject,
                    $event['user_id']
                );

                if (
                    array_key_exists('data', $event) &&
                    is_array($event['data']) &&
                    !empty($event['data']) &&
                    array_key_exists('ref_type', $event['data']) &&
                    $event['data']['ref_type'] == 'post'
                ) {
                    $subject = $mycred->template_tags_post(
                        $subject,
                        $event['ref_id']
                    );
                }
            ...
            return apply_filters(
                'mycred_email_notice_get_subject',
                $subject,
                $this
            );
        }
        ...
        public function get_body(
            ...
            if (!empty($content)) {
                ...
                if ($event['ref'] === 'transfer') {
                    $content = $mycred->template_tags_user(
                        $content,
                        $event['ref_id']
                    );
                    $content = mycred_transfer_render_message(
                        $content,
                        $event['data']
                    );
                ...
                if (
                    ...
                    $content = $mycred->template_tags_post(
                        $content,
                        $event['ref_id']
                    );
                }
                ...
                $content = str_replace(
                    '%amount%',
                    $mycred->format_creds($event['amount']),
                    $content
                );
                $content = str_replace(
                    '%new_balance%',
                    $mycred->format_creds($event['new']),
                    $content
                );
                $content = str_replace(
                    '%old_balance%',
                    $mycred->format_creds($event['old']),
                    $content
                );
        ...
        public function get_message(
            ...
            return apply_filters(
                'mycred_email_notice_get_message',
                $message,
                $this
            );
        }
