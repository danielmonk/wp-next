...
if (!function_exists('mycred_get_email_notice')):
    function mycred_get_email_notice($notice_id = false)
    {
        ...
        if ($notice_id === false || absint($notice_id) === 0) {
            return false;
        }
...
if (!function_exists('mycred_user_wants_email')):
    function mycred_user_wants_email($user_id = false, $notice_id = false)
    {
        ...
        if (
            isset($account->email_block) &&
            !empty($account->email_block) &&
            in_array($notice_id, $account->email_block)
        ) {
            $wants_email = false;
        } elseif (!isset($account->email_block)) {
            $unsubscriptions = (array) mycred_get_user_meta(
                $user_id,
            ...
            if (
                !empty($unsubscriptions) &&
                in_array($notice_id, $unsubscriptions)
            ) {
                $wants_email = false;
            }
        }
        ...
        return apply_filters(
            'mycred_email_notice_user_wants',
            $wants_email,
            $user_id,
            $notice_id
        );
...
if (!function_exists('mycred_get_email_instances')):
    function mycred_get_email_instances($none = true)
    {
        ...
        if (class_exists('myCRED_Badge_Module')) {
            ...
            $instances['badge_level'] = __(
                'user gains a new badge level',
                'mycred'
            );
        }

        if (class_exists('myCRED_Ranks_Module')) {
            $instances['rank_up'] = __(
                'user is promoted to a higher rank',
                'mycred'
            );
            $instances['rank_down'] = __(
                'user is demoted to a lower rank',
                'mycred'
            );
        }
...
if (!function_exists('mycred_get_email_triggers')):
    function mycred_get_email_triggers(
        ...
        $setup = (array) mycred_get_option(
            'mycred-email-triggers-' . $point_type,
            $defaults
        );
...
if (!function_exists('mycred_add_email_trigger')):
    function mycred_add_email_trigger(
        $event_type = '',
        $instance = '',
        $notice_id = false,
        $point_type = MYCRED_DEFAULT_TYPE_KEY
        ...
        if (empty($event_type) || empty($instance) || $notice_id === 0) {
            return false;
        }
        ...
        if (array_key_exists($event_type, $triggers)) {
            if (!array_key_exists($instance, $triggers[$event_type])) {
                ...
                $triggers[$event_type][$instance] = [];
                $triggers[$event_type][$instance][] = $notice_id;
            } else {
                if (
                    empty($triggers[$event_type]) ||
                    !in_array($notice_id, $triggers[$event_type][$instance])
                ) {
                    $triggers[$event_type][$instance][] = $notice_id;
                }
            }
...
if (!function_exists('mycred_delete_email_trigger')):
    function mycred_delete_email_trigger(
        $notice_id = false,
        $point_type = MYCRED_DEFAULT_TYPE_KEY
    ) {
        $notice_id = absint($notice_id);
...
if (!function_exists('mycred_get_triggered_emails')):
    function mycred_get_triggered_emails($mycred_event = [], $new_balance = 0)
    {
        extract(
            shortcode_atts(
                [
        ...
        if (!empty($triggers)) {
            ...
            if (
                ...
                $ref == 'transfer' &&
                floatval($amount) > 0 &&
                !empty($triggers['generic']['transfer_in'])
            ) {
            ...
            if (
                ...
                $ref == 'transfer' &&
                floatval($amount) < 0 &&
                !empty($triggers['generic']['transfer_out'])
            ) {
            ...
            if (
                !empty($triggers['specific']) &&
                array_key_exists($ref, $triggers['specific']) &&
                !empty($triggers['specific'][$ref])
            ) {
...
if (!function_exists('mycred_get_event_emails')):
    function mycred_get_event_emails(
        $point_type = MYCRED_DEFAULT_TYPE_KEY,
        $event_type = '',
        $instance = ''
    ) {
        ...
        if (array_key_exists($event_type, $triggers)) {
            if (
                array_key_exists($instance, $triggers[$event_type]) &&
                !empty($triggers[$event_type][$instance])
            ) {
...
if (!function_exists('mycred_send_new_email')):
    function mycred_send_new_email(
        $notice_id = false,
        $event = [],
        $point_type = MYCRED_DEFAULT_TYPE_KEY
    ) {
        if ($notice_id === false || get_post_status($notice_id) !== 'publish') {
            return false;
        }
    ...
    function mycred_email_notice_cron_job()
    {
        ...
        $pending = $wpdb->get_results(
            $wpdb->prepare(
                ...
                mycred_get_meta_key('mycred_scheduled_email_notices')
            )
        );
        ...
        if ($pending) {
            foreach ($pending as $pending_notice) {
                ...
                mycred_delete_user_meta(
                    $pending_notice->user_id,
                    'mycred_scheduled_email_notices',
                    '',
                    $notice
                );
            }
...
if (!function_exists('mycred_render_email_subscriptions')):
    function mycred_get_email_settings($post_id)
    {
        ...
        $settings = (array) mycred_get_post_meta(
            $post_id,
