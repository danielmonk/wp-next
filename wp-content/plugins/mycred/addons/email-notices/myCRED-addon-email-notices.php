...
if (!class_exists('myCRED_Email_Notice_Module')):
    class myCRED_Email_Notice_Module extends myCRED_Module
    {
        ...
        public function module_init()
        {
            ...
            add_filter('mycred_add_finished', [$this, 'email_check'], 80, 3);
            ...
            add_action(
                ...
                'mycred_email_notice_cron_job'
            );

            add_shortcode(
                MYCRED_SLUG . '_email_subscriptions',
                'mycred_render_email_subscriptions'
            );
        ...
        public function module_admin_init()
        {
            ...
            add_filter('user_can_richedit', [$this, 'disable_richedit']);
            add_filter('default_content', [$this, 'default_content']);

            add_filter(
                'manage_' . MYCRED_EMAIL_KEY . '_posts_columns',
                [$this, 'adjust_column_headers'],
                50
            );
            add_action(
                'manage_' . MYCRED_EMAIL_KEY . '_posts_custom_column',
                [$this, 'adjust_column_content'],
                10,
                2
            );
            add_action(
                'save_post_' . MYCRED_EMAIL_KEY,
        ...
        {
            // Schedule Cron
            if (!isset($this->emailnotices['send'])) {
                return;
            }

            if (
                $this->emailnotices['send'] == 'hourly' &&
                wp_next_scheduled('mycred_send_email_notices') === false
            ) {
                wp_schedule_event(
                    time(),
                    'hourly',
        ...
        protected function register_email_notices()
        {
            $labels = [
                ...
                'not_found_in_trash' => __(
                    'No email notifications found in Trash',
                    'mycred'
                ),
                'parent_item_colon' => '',
            ...
            register_post_type(
                MYCRED_EMAIL_KEY,
                apply_filters('mycred_register_emailnotices', $args)
            );
        }
        ...
        public function populate_current_account()
        {
            ...
            $mycred_current_account->email_block = (array) mycred_get_user_meta(
                $mycred_current_account->user_id,
                'mycred_email_unsubscriptions',
                '',
                true
            );
        }
        ...
        public function populate_account()
        {
            ...
            $mycred_account->email_block = (array) mycred_get_user_meta(
        ...
        public function parent_file($parent = '')
        {
            ...
            if (
                isset($_GET['post']) &&
                mycred_get_post_type($_GET['post']) == MYCRED_EMAIL_KEY &&
                isset($_GET['action']) &&
                $_GET['action'] == 'edit'
            ) {
                return MYCRED_SLUG;
            ...
            if (
                ...
                $_GET['post_type'] == MYCRED_EMAIL_KEY
            ) {
                return MYCRED_SLUG;
            }
        ...
        public function adjust_column_content($column_name, $post_id)
        {
            ...
            if ($column_name == 'mycred-email-status') {
                if (
                    $email->post->post_status != 'publish' &&
                    $email->post->post_status != 'future'
                ) {
                    ...
                    echo '<p>' .
                        sprintf(
                            ...
                            date(
                                ...
                                strtotime($email->post->post_date)
                            )
                        ) .
                        '</p>';
                } else {
            ...
            elseif ($column_name == 'mycred-email-reference') {
                ...
                if ($trigger == '') {
                    ...
                    $description[] = sprintf(
                        '<strong>%s:</strong> %s',
                        __('Sent at custom events', 'mycred'),
                        str_replace(',', ', ', $trigger)
                    );
                }
        ...
        public function enqueue_scripts()
        {
            ...
            // Commonly used
            if (
                $screen->id == 'edit-' . MYCRED_EMAIL_KEY ||
                $screen->id == MYCRED_EMAIL_KEY
            ) {
            ...
            if ($screen->id == MYCRED_EMAIL_KEY) {
                ...
                wp_enqueue_script(
                    'mycred-edit-email',
                    plugins_url('assets/js/edit-email.js', myCRED_EMAIL),
                    ['jquery'],
                    myCRED_EMAIL_VERSION,
                    true
                );
        ...
        public function admin_header()
        {
            ...
            if (
                $screen->id == MYCRED_EMAIL_KEY &&
                $this->emailnotices['use_html'] === false
            ) {
        ...
        public function disable_richedit($default)
        {
            ...
            if (
                ...
                $post->post_type == MYCRED_EMAIL_KEY &&
                $this->emailnotices['use_html'] === false
            ) {
                return false;
            }
        ...
        public function default_content($content)
        {
            ...
            if (
                $post_type == MYCRED_EMAIL_KEY &&
                !empty($this->emailnotices['content'])
            ) {
        ...
        public function metabox_email_setup($post)
        {
            ...
            $uses_generic =
                $trigger == '' || array_key_exists($trigger, $instances)
                    ? true
                    : false;
            $uses_specific =
                !$uses_generic && array_key_exists($trigger, $references)
                    ? true
                    : false;
...
foreach ($instances as $instance => $event) {
    ...
    echo '>... ' . esc_html($event) . '</option>';
			...
			<div id="reference-selection" style="display: <?php if (
       $uses_specific ||
       $uses_custom
   ) {
       echo 'block';
...
foreach ($references as $ref_id => $ref_description) {
    ...
    if ($uses_specific && $trigger == $ref_id) {
        echo ' selected="selected"';
    } elseif ($ref_id == 'mycred_custom' && $uses_custom) {
        echo ' selected="selected"';
    }
...
?>
				...
				<div id="custom-reference-selection" style="display: <?php if ($uses_custom) {
        echo 'block';
    } else {
        echo 'none';
    } ?>;">
					<div class="form-group">
						<label for="mycred-email-custom-ref"><?php _e(
          'Custom Reference',
          'mycred'
      ); ?></label>
						<input type="text" name="mycred_email[custom_reference]" placeholder="<?php _e(
          'required',
          'mycred'
      ); ?>" id="mycred-email-custom-ref" class="form-control" value="<?php echo esc_attr($trigger); ?>" />
					</div>
					<p class="description" style="line-height: 16px;"><?php _e(
         'This can be either a single reference or a comma separated list of references.',
         'mycred'
     ); ?></p>
				</div>
			</div>
...
<?php if (count($this->point_types) > 1) {
    mycred_types_select_from_checkboxes(
        'mycred_email[ctype][]',
        'mycred-email-ctype',
        $email->point_types
    );
...
<?php
			...
			<div class="form-group" style="margin-bottom: 0;">
				<label for="mycred-email-recipient-user"><?php _e(
        'Recipient:',
        'mycred'
    ); ?></label>
				<div class="inline-radio">
					<label for="mycred-email-recipient-user"><input type="radio" name="mycred_email[recipient]" id="mycred-email-recipient-user" value="user" <?php checked(
         $email->settings['recipient'],
         'user'
     ); ?> /> <?php _e('User', 'mycred'); ?></label>
				</div>
				<div class="inline-radio">
					<label for="mycred-email-recipient-admin"><input type="radio" name="mycred_email[recipient]" id="mycred-email-recipient-admin" value="admin" <?php checked(
         $email->settings['recipient'],
         'admin'
     ); ?> /> <?php _e('Administrator', 'mycred'); ?></label>
				</div>
				<div class="inline-radio">
					<label for="mycred-email-recipient-both"><input type="radio" name="mycred_email[recipient]" id="mycred-email-recipient-both" value="both" <?php checked(
         $email->settings['recipient'],
         'both'
     ); ?> /> <?php _e('Both', 'mycred'); ?></label>
				</div>
			</div>
		</div>
	</div>
...
<div class="form">
	<div class="row">
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="mycred-email-senders-name"><?php _e(
        'Senders Name:',
        'mycred'
    ); ?></label>
				<input type="text" name="mycred_email[senders_name]" id="mycred-email-senders-name" class="form-control" value="<?php echo esc_attr(
        $email->settings['senders_name']
    ); ?>" />
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="mycred-email-senders-email"><?php _e(
        'Senders Email:',
        'mycred'
    ); ?></label>
				<input type="text" name="mycred_email[senders_email]" id="mycred-email-senders-email" class="form-control" value="<?php echo esc_attr(
        $email->settings['senders_email']
    ); ?>" />
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="mycred-email-reply-to"><?php _e(
        'Reply-To Email:',
        'mycred'
    ); ?></label>
				<input type="text" name="mycred_email[reply_to]" id="mycred-email-reply-to" class="form-control" value="<?php echo esc_attr(
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				...
				<textarea name="mycred_email[styling]" class="form-control code" rows="10" cols="30" id="mycred-email-styling"><?php echo esc_html(
        $email->get_email_styling()
    ); ?></textarea>
			</div>
		</div>
	</div>
...
<div class="row">
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		...
		<div class="row">
			...
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div><?php _e(
        'The amount of points gained or lost in this instance',
        'mycred'
    ); ?></div>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<div><?php printf(
        __('You can also use %s.', 'mycred'),
        '<a href="http://codex.mycred.me/category/template-tags/temp-user/" target="_blank">' .
            __('user related template tags', 'mycred') .
            '</a>'
    ); ?></div>
			</div>
		</div>
...
<?php
        ...
        public function save_email_notice($post_id, $post = null)
        {
            ...
            // Generic
            if (
                $_POST['mycred_email']['instance'] != '' &&
                $_POST['mycred_email']['instance'] != 'custom'
            ) {
            ...
            elseif ($_POST['mycred_email']['instance'] != '') {
                ...
                // Based on custom reference
                    ...
                    $reference_list = [];
                    $custom_reference = explode(
                        ',',
                        $_POST['mycred_email']['custom_reference']
                    );
                    ...
                    foreach ($custom_reference as $reference_id) {
                        ...
                        if (
                            !empty($reference_id) &&
                            !array_key_exists($reference_id, $instances)
                        ) {
            ...
            if (!empty($_POST['mycred_email']['recipient'])) {
                $settings['recipient'] = sanitize_text_field(
                    $_POST['mycred_email']['recipient']
                );
            }

            if (!empty($_POST['mycred_email']['senders_name'])) {
                $settings['senders_name'] = sanitize_text_field(
                    $_POST['mycred_email']['senders_name']
                );
            }

            if (!empty($_POST['mycred_email']['senders_email'])) {
                $settings['senders_email'] = sanitize_text_field(
                    $_POST['mycred_email']['senders_email']
                );
            }

            if (!empty($_POST['mycred_email']['reply_to'])) {
                $settings['reply_to'] = sanitize_text_field(
                    $_POST['mycred_email']['reply_to']
                );
            }
            ...
            // Point Types
            if (
                array_key_exists('ctype', $_POST['mycred_email']) &&
                !empty($_POST['mycred_email']['ctype'])
            ) {
                $checked_types = isset($_POST['mycred_email']['ctype'])
                    ? $_POST['mycred_email']['ctype']
                    : [];
                if (!empty($checked_types)) {
                    foreach ($checked_types as $type_key) {
                        $type_key = sanitize_key($type_key);
                        if (
                            mycred_point_type_exists($type_key) &&
                            !in_array($type_key, $point_types)
                        ) {
            ...
            if ($email->emailnotices['use_html'] === false) {
                return;
            }
            ...
            if (!empty($_POST['mycred_email']['styling'])) {
                mycred_update_post_meta(
                    $post_id,
                    'mycred_email_styling',
                    wp_kses_post($_POST['mycred_email']['styling'])
                );
            }
        }
        ...
        public function badge_check($user_id, $badge_id, $level_reached)
        {
            ...
            foreach ($mycred_types as $type_id => $label) {
                $emails = mycred_get_event_emails(
                    $type_id,
                    'generic',
                    $instance
                );
        ...
        {
            $this->emailnotices = mycred_apply_defaults(
                $this->default_prefs,
                $this->emailnotices
            ); ?>
...
<div class="body" style="display:none;">
	...
	<div class="row">
		<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
         ...
         'use_html' => 'no',
     ]); ?>"><input type="radio" name="<?php echo $this->field_name('use_html'); ?>" id="<?php echo $this->field_id(['use_html' => 'no']); ?>" <?php checked($this->emailnotices['use_html'], 0); ?> value="0" /> <?php _e('Plain Text', 'mycred'); ?></label>
				</div>
				<div class="radio">
					<label for="<?php echo $this->field_id([
         'use_html' => 'yes',
     ]); ?>"><input type="radio" name="<?php echo $this->field_name('use_html'); ?>" id="<?php echo $this->field_id(['use_html' => 'yes']); ?>" <?php checked($this->emailnotices['use_html'], 1); ?> value="1" /> HTML</label>
				</div>
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
			...
			<div class="form-group">
				...
				<p class="form-control-static"><?php _e(
        'WordPress Cron is disabled. Emails will be sent immediately.',
        'mycred'
    ); ?></p>
				<?php else: ?>
				<div class="radio">
    ...
    'send'
); ?>" id="<?php echo $this->field_id('send'); ?>" <?php checked(
    $this->emailnotices['send'],
    ''
); ?> value="" /> <?php _e('Send emails immediately', 'mycred'); ?></label>
				</div>
				<div class="radio">
					<label for="<?php echo $this->field_id(
         'send'
     ); ?>-hourly"><input type="radio" name="<?php echo $this->field_name(
    'send'
); ?>" id="<?php echo $this->field_id('send'); ?>-hourly" <?php checked(
    $this->emailnotices['send'],
    'hourly'
); ?> value="hourly" /> <?php _e(
     'Send emails once an hour',
     'mycred'
 ); ?></label>
				</div>
				<div class="radio">
					<label for="<?php echo $this->field_id(
         'send'
     ); ?>-daily"><input type="radio" name="<?php echo $this->field_name(
    'send'
); ?>" id="<?php echo $this->field_id('send'); ?>-daily" <?php checked(
    $this->emailnotices['send'],
    'daily'
); ?> value="daily" /> <?php _e('Send emails once a day', 'mycred'); ?></label>
				</div>
				<?php endif; ?>
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			...
			<div class="form-group">
				<div class="checkbox">
					<label for="<?php echo $this->field_id([
         'filter' => 'subject',
     ]); ?>"><input type="checkbox" name="<?php echo $this->field_name(['filter' => 'subject']); ?>" id="<?php echo $this->field_id(['filter' => 'subject']); ?>" <?php checked($this->emailnotices['filter']['subject'], 1); ?> value="1" /> <?php _e('Filter Email Subjects', 'mycred'); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id([
         'filter' => 'content',
     ]); ?>"><input type="checkbox" name="<?php echo $this->field_name(['filter' => 'content']); ?>" id="<?php echo $this->field_id(['filter' => 'content']); ?>" <?php checked($this->emailnotices['filter']['content'], 1); ?> value="1" /> <?php _e('Filter Email Body', 'mycred'); ?></label>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<div class="checkbox">
					<label for="<?php echo $this->field_id(
         'override'
     ); ?>"><input type="checkbox" name="<?php echo $this->field_name('override'); ?>" id="<?php echo $this->field_id('override'); ?>" <?php checked($this->emailnotices['override'], 1); ?> value="1" /> <?php _e('SMTP Debug. Enable if you are experiencing issues with wp_mail() or if you use a SMTP plugin for emails.', 'mycred'); ?></label>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id([
        'from' => 'name',
    ]); ?>"><?php _e('Senders Name:', 'mycred'); ?></label>
				<input type="text" name="<?php echo $this->field_name([
        'from' => 'name',
    ]); ?>" id="<?php echo $this->field_id(['from' => 'name']); ?>" value="<?php echo esc_attr($this->emailnotices['from']['name']); ?>" class="form-control" />
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id([
        'from' => 'email',
    ]); ?>"><?php _e('Senders Email:', 'mycred'); ?></label>
				<input type="text" name="<?php echo $this->field_name([
        'from' => 'email',
    ]); ?>" id="<?php echo $this->field_id(['from' => 'email']); ?>" value="<?php echo esc_attr($this->emailnotices['from']['email']); ?>" class="form-control" />
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id([
        'from' => 'reply_to',
    ]); ?>"><?php _e('Reply-To:', 'mycred'); ?></label>
				<input type="text" name="<?php echo $this->field_name([
        'from' => 'reply_to',
    ]); ?>" id="<?php echo $this->field_id(['from' => 'reply_to']); ?>" value="<?php echo esc_attr($this->emailnotices['from']['reply_to']); ?>" class="form-control" />
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id(
        'content'
    ); ?>"><?php _e('Default Email Content', 'mycred'); ?></label>
				<textarea rows="10" cols="50" name="<?php echo $this->field_name(
        'content'
    ); ?>" id="<?php echo $this->field_id('content'); ?>" class="form-control"><?php echo esc_attr($this->emailnotices['content']); ?></textarea>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id(
        'styling'
    ); ?>"><?php _e('Default CSS Styling', 'mycred'); ?></label>
				<textarea rows="10" cols="50" name="<?php echo $this->field_name(
        'styling'
    ); ?>" id="<?php echo $this->field_id('styling'); ?>" class="form-control"><?php echo esc_attr($this->emailnotices['styling']); ?></textarea>
				<p><span class="description"><?php _e(
        'Default email CSS styling. Note that if you intend to send HTML emails, you should use inline CSS styling for best results.',
        'mycred'
    ); ?></span></p>
			</div>
		</div>
...
<?php
        ...
        {
            $new_data['emailnotices']['use_html'] = isset(
                $data['emailnotices']['use_html']
            )
                ? absint($data['emailnotices']['use_html'])
                : 0;

            $new_data['emailnotices']['filter']['subject'] = isset(
                $data['emailnotices']['filter']['subject']
            )
                ? 1
                : 0;
            $new_data['emailnotices']['filter']['content'] = isset(
                $data['emailnotices']['filter']['content']
            )
                ? 1
                : 0;

            $new_data['emailnotices']['from']['name'] = sanitize_text_field(
                $data['emailnotices']['from']['name']
            );
            $new_data['emailnotices']['from']['email'] = sanitize_text_field(
                $data['emailnotices']['from']['email']
            );
            $new_data['emailnotices']['from']['reply_to'] = sanitize_text_field(
                $data['emailnotices']['from']['reply_to']
            );

            $new_data['emailnotices']['content'] = wp_kses_post(
                $data['emailnotices']['content']
            );
            $new_data['emailnotices']['styling'] = sanitize_textarea_field(
                $data['emailnotices']['styling']
            );

            $new_data['emailnotices']['send'] = sanitize_text_field(
                $data['emailnotices']['send']
            );
            $new_data['emailnotices']['override'] = isset(
                $data['emailnotices']['override']
            )
                ? 1
                : 0;
