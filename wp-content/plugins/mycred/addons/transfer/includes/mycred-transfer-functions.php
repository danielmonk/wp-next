...
if (!function_exists('mycred_new_transfer')):
    function mycred_new_transfer($request = [], $post = [])
    {
...
if (!function_exists('mycred_user_can_transfer')):
    function mycred_user_can_transfer(
        ...
        // Get Transfer Max
        $max = apply_filters(
            'mycred_transfer_limit',
            $mycred->number($settings['limit']['amount']),
            $user_id,
            $amount,
            $settings,
            $reference
...
if (!function_exists('mycred_transfer_render_message')):
    function mycred_transfer_render_message($original = '', $data = [])
    {
        ...
        $message = apply_filters(
            'mycred_transfer_default_message',
            $original,
            $data
        );
...
if (!function_exists('mycred_get_users_transfer_history')):
    function mycred_get_users_transfer_history(
        $user_id,
        $type = MYCRED_DEFAULT_TYPE_KEY,
        $key = null
    ) {
        ...
        return mycred_apply_defaults(
            $default,
            mycred_get_user_meta($user_id, $key, '', true)
        );
    }
endif;
...
if (!function_exists('mycred_update_users_transfer_history')):
    function mycred_update_users_transfer_history(
        $user_id,
        $history,
        $type = MYCRED_DEFAULT_TYPE_KEY,
        $key = null
