...
if (!class_exists('myCRED_Transfer')):
    class myCRED_Transfer extends myCRED_Object
    {
        ...
        protected function populate()
        {
            ...
            if ($settings === false) {
                $settings = [
                    ...
                    'errors' => [
                        'low' => __(
                            'You do not have enough %plural% to send.',
                            'mycred'
                        ),
                        'over' => __(
         ...
         * @version 1.0
         */
        public function generate_new_transfer_id(
            $sender_id = 0,
            $recipient_id = 0
        ) {
        ...
        public function get_transfer($transfer_id = false)
        {
            ...
            elseif ($transfer_id !== false) {
                ...
                $log_entries = $wpdb->get_results(
                    $wpdb->prepare(
                        "SELECT * FROM {$mycred_log_table} WHERE data LIKE %s ORDER BY creds ASC LIMIT 2;",
                        $transfer_sql
                    )
                );
            }
            ...
            if (!empty($log_entries) && count($log_entries) == 2) {
                ...
                $transfer->status =
                    $log_entries[0]->ref_id == 0 && $log_entries[1]->ref_id == 0
                        ? 'refunded'
                        : 'completed';
                ...
                $transfer->point_type = new myCRED_Point_Type(
                    $log_entries[0]->ctype
                );
                ...
                $transfer->message =
                    array_key_exists('message', $transfer->data) &&
                    !empty($transfer->data['message'])
        ...
        {
            return apply_filters('mycred_transfer_statuses', [
                '' => __('New', ''),
         ...
         * @version 1.0
         */
        public function new_instance($request = [])
        {
            ...
            if (!empty($usable_types)) {
                ...
                $balances = mycred_is_current_account($this->sender_id)
                ...
                foreach ($requested_types as $type_id) {
                    ...
                    if (
                        !mycred_point_type_exists($type_id) ||
                        !in_array($type_id, $usable_types) ||
                        !in_array($type_id, $this->settings['types'])
                    ) {
                        continue;
                    }
                    ...
                    $balance =
                        $balances !== false && $balances[$type_id] !== false
                            ? $balances[$type_id]
                            : new myCRED_Balance($this->sender_id, $type_id);
                    ...
                    $balance->lowest = apply_filters(
                        'mycred_transfer_acc_limit',
                        $balance->point_type->number(0),
                        $type_id,
                        $this->sender_id,
                        $this->reference
                    );
            ...
            if (empty($this->transferable_types)) {
                $this->errors['excluded'] =
                    'You do not have access to this point type.';
            ...
            if (!$this->user_can_transfer_minimum()) {
                $this->errors['minimum'] = __(
                    'You do not have enough points to make a transfer.',
                    'mycred'
                );
            ...
            if ($this->user_is_over_limit()) {
                $this->errors['limit'] =
                    'You have reached your transfer limit.';
        ...
        public function user_can_transfer_minimum()
        {
            ...
            if (!empty($this->transferable_types)) {
                ...
                foreach ($this->transferable_types as $type_id) {
                    ...
                    $balance->minimum =
                        $this->minimum === false
                            ? $type->get_lowest_value()
                            : $type->number($this->minimum);
                    $balance->usable = true;
            ...
            if ($this->settings['limit']['limit'] != 'none') {
                ...
                if ($limit->period == 'weekly') {
                    ...
                    $limit->from = mktime(
                        ...
                        0,
                        date('n', $this->now),
                        date('j', $this->now) - date('n', $this->now) + 1
                    );
                }
                ...
                elseif ($limit->period == 'monthly') {
                    ...
                    $limit->from = strtotime(
                        date('Y-m-01') . ' midnight',
                        $this->now
                    );
                }
            }
         ...
         * @version 1.0
         */
        public function get_transfer_limit_desc(
            $point_type = MYCRED_DEFAULT_TYPE_KEY
        ) {
            $description = __('No limit', 'mycred');
            if (
                !empty($this->limits) &&
                array_key_exists($point_type, $this->limits)
            ) {
                ...
                $remaining =
                    $limit->amount - $this->balances[$point_type]->total;
                $description = str_replace(
                    '%left%',
                    $type_object->format($remaining),
                    $description
                );
         ...
         * @version 1.0
         */
        public function is_valid_transfer_request($request = [], $posted = [])
        {
            $this->valid_request = false;
            $this->request = shortcode_atts(
                apply_filters(
                    'mycred_new_transfer_request',
                    [
                        'token' => null,
                        'recipient_id' => null,
                        'user_id' => 'current',
            ...
            if (
                !wp_verify_nonce(
                    $this->request['token'],
                    'mycred-new-transfer-' . $this->request['reference']
                )
            ) {
                return 'error_1';
            }
            ...
            if (
                !mycred_point_type_exists($this->request['ctype']) ||
                !in_array($this->request['ctype'], $this->settings['types'])
            ) {
                return 'error_10';
            }
            ...
            $sender_id = get_current_user_id();
            if (
                $this->request['user_id'] != 'current' &&
                absint($this->request['user_id']) > 0 &&
                absint($this->request['user_id']) != $user_id
            ) {
            ...
            // Transfer recipient
            $recipient_id = mycred_get_transfer_recipient(
                sanitize_text_field($this->request['recipient_id'])
            );
            ...
            if ($transfer_limit !== false) {
                // Format the limit amount
                $transfer_limit->amount = $mycred->number(
            ...
            $this->transfer_id = $this->generate_new_transfer_id(
                $this->sender_id,
                $this->recipient_id
            );
            $this->data = apply_filters(
                'mycred_transfer_data',
                [
                    'ref_type' => 'user',
            ...
            $this->valid_request = apply_filters(
                'mycred_is_valid_transfer_request',
                true,
                $this
            );
        ...
        public function total_sent(
            ...
            if ($user_id === false || $period_start <= 0 || $period_end <= 0) {
                return 0;
            }
        ...
        public function new_transfer()
        {
            ...
            $result = false;
            $mycred = mycred($this->point_type);
            $this->transfer_charge = apply_filters(
                'mycred_transfer_charge',
                $this->transfer_amount,
                $this
            );
            $attempt_check =
                $this->balances[$this->point_type] - $this->transfer_charge;
            ...
            if (
                ...
                if (
                    ...
                    $this->transfer_payout = apply_filters(
                        'mycred_transfer_payout',
                        $this->transfer_amount,
                        $this
                    );
                    ...
                    $result = [
                        ...
                        'zero' =>
                            $attempt_check <= $mycred->zero() ? true : false,
                    ];
                }
            }
        ...
        public function refund()
        {
            ...
            if (in_array($transfer->status, ['', 'refunded'])) {
                return false;
            }
         ...
         * @version 1.0
         */
        public function get_transfer_form($args = [])
        {
            ...
            ob_start();
            ?>
<div class="mycred-transfer-cred-wrapper"<?php if ($this->reference != '') {
    echo ' id="transfer-form-' . esc_attr($this->reference) . '"';
} ?>>
	<form class="form mycred-transfer mycred-transfer-form" id="mycred-transfer-form-<?php echo esc_attr(
     $this->reference
 ); ?>" method="post" data-ref="<?php echo esc_attr($this->reference); ?>" action="">
		...
		<div class="row">
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<input type="hidden" name="mycred_new_transfer[token]" value="<?php echo wp_create_nonce(
        'mycred-new-transfer-' . $this->reference
    ); ?>" />
				<input type="hidden" name="mycred_new_transfer[reference]" value="<?php echo esc_attr(
        $this->reference
    ); ?>" />
				<input type="submit" class="mycred-submit-transfer<?php echo ' ' .
        esc_attr(
            $this->args['button_class']
        ); ?>" value="<?php echo esc_attr($this->args['button']); ?>" />
			</div>
...
return do_shortcode(
    apply_filters('mycred_transfer_render', $output, $this->args, $this)
);
        }
        ...
        public function get_transfer_recipient_field($return = false)
        {
            ...
            if ($this->args['placeholder'] == '') {
                ...
                $placeholder = sprintf(
                    apply_filters(
                        'mycred_transfer_to_placeholder',
                        __('recipients %s', 'mycred'),
                        $this->settings,
                        $this->args
                    ),
                    $placeholder
            ...
            if ($this->args['recipient_label'] != '') {
                $field .=
                    '<label class="recipient-label">' .
                    esc_html($this->args['recipient_label']) .
                    '</label>';
            }
            ...
            if (count($recipients) < 1) {
                $field .=
                    '<input type="text" name="mycred_new_transfer[recipient_id]" value="" aria-required="true" class="mycred-autofill form-control" data-form="' .
                    esc_attr($this->reference) .
                    '" placeholder="' .
                    $recipient_placeholder .
                    '" />';
            }
            ...
            elseif (count($recipients) == 1) {
                ...
                $field .=
                    ...
                    $value .
                    '</span><input type="hidden" name="mycred_new_transfer[recipient_id]" value="' .
                    esc_attr($fist_user->ID) .
                    '"  />';
            }
            ...
            elseif (count($recipients) > 1) {
                $field .=
                    '<select name="mycred_new_transfer[recipient_id]" class="form-control">';
        ...
        public function get_transfer_amount_field($return = false)
        {
            ...
            if ($this->args['amount_label'] != '') {
                $field .=
                    ...
                    esc_html($this->args['amount_label']) .
                    '</label>';
            }

            // User needs to nominate the amount
            if (
                !is_array($this->transfer_amount) &&
                $this->transfer_amount == 0
            ) {
                $field .=
                    '<input type="text" name="mycred_new_transfer[amount]" placeholder="' .
                    esc_html($this->args['amount_placeholder']) .
                    '" class="form-control" value="" />';
            }
            // Multiple amounts to pick from
            elseif (
                is_array($this->transfer_amount) &&
                count($this->transfer_amount) > 1
            ) {
                $field .=
                    '<select name="mycred_new_transfer[amount]" class="form-control">';
            ...
            // An amount has been nominated
                ...
                $field .=
                    '<input type="hidden" name="mycred_new_transfer[amount]" value="' .
                    esc_attr($this->transfer_amount) .
                    '" />';
                $field .=
                    '<span class="form-control-static" id="mycred-transfer-form-amount-field">' .
                    esc_attr($this->transfer_amount) .
                    '</span>';
            }
        ...
        {
            $field =
                '<input type="hidden" name="mycred_new_transfer[ctype]" value="' .
                esc_attr($this->transferable_types[0]) .
                '" />';

            if (count($this->transferable_types) > 1) {
                $field = '<div class="form-group select-point-type-wrapper">';

                if ($this->args['balance_label'] != '') {
                    $field .=
                        '<label>' .
                        esc_html($this->args['balance_label']) .
                        '</label>';
                }
                $field .=
        ...
        public function get_transfer_message_field($return = false)
        {
            ...
            if (
                (bool) $this->args['show_message'] &&
                $this->settings['message'] > 0
            ) {
                $field = '<div class="form-group message-wrapper">';
                if ($this->args['message_label'] != '') {
                    $field .=
                        '<label>' .
                        esc_html($this->args['message_label']) .
                        '</label>';
                }

                $field .=
                    '<input type="text" class="form-control" id="mycred-transfer-form-message-field" name="message" value="" placeholder="' .
                    esc_html($this->args['message_placeholder']) .
                    '"  />';
        ...
        public function get_transfer_extra_fields($return = false)
        {
            ...
            $extras = [];
            if (
                (bool) $this->args['show_balance'] &&
                !empty($this->settings['templates']['balance'])
            ) {
                foreach ($this->balances as $type_id => $balance) {
                    ...
                    $template = str_replace(
                        '%balance%',
                        $balance->point_type->format($balance->current),
                        $template
                    );
                    $template = str_replace(
                        '%singular%',
                        $balance->point_type->singular,
                        $template
                    );
                    $template = str_replace(
                        '%plural%',
            ...
            // Show Limits
            if (
                (bool) $this->args['show_limit'] &&
                !empty($this->settings['templates']['limit']) &&
                $this->settings['limit']['limit'] != 'none' &&
                count($this->balances) == 1
            ) {
                foreach ($this->balances as $type_id => $balance) {
