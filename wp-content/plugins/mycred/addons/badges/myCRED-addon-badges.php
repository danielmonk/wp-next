...
if (!class_exists('myCRED_Badge_Module')):
    class myCRED_Badge_Module extends myCRED_Module
    {
        ...
        {
            add_filter('mycred_add_finished', [$this, 'add_finished'], 30, 3);
            add_action("wp_ajax_mycred_switch_all_to_open_badge", [
                $this,
                "mycred_switch_all_to_open_badge",
            ]);
            add_action("wp_ajax_nopriv_mycred_switch_all_to_open_badge", [
                $this,
                "mycred_switch_all_to_open_badge",
            ]);
        }
        ...
        public function module_init()
        {
            ...
            add_action('mycred_set_current_account', [
                $this,
                'populate_current_account',
            ]);
            add_action('mycred_get_account', [$this, 'populate_account']);

            add_shortcode(
                MYCRED_SLUG . '_my_badges',
                'mycred_render_my_badges'
            );
            add_shortcode(MYCRED_SLUG . '_badges', 'mycred_render_badges');

            add_shortcode(
                MYCRED_SLUG . '_badges_list',
                'mycred_render_badges_list'
            );

            add_shortcode(
                MYCRED_SLUG . '_badge_evidence',
                'mycred_render_badge_evidence'
            );

            // Insert into bbPress
            if (class_exists('bbPress')) {
                if (
                    $this->badges['bbpress'] == 'profile' ||
                    $this->badges['bbpress'] == 'both'
                ) {
                    add_action('bbp_template_after_user_profile', [
                        $this,
                        'insert_into_bbpress_profile',
                    ]);
                }

                if (
                    $this->badges['bbpress'] == 'reply' ||
                    $this->badges['bbpress'] == 'both'
                ) {
                    add_action('bbp_theme_after_reply_author_details', [
                        $this,
                        'insert_into_bbpress_reply',
                    ]);
                }
            }
            ...
            if (class_exists('BuddyPress')) {
                // Insert into header
                if (
                    $this->badges['buddypress'] == 'header' ||
                    $this->badges['buddypress'] == 'both'
                ) {
                    add_action('bp_before_member_header_meta', [
                        $this,
                        'insert_into_buddypress',
                    ]);
                }

                // Insert into profile
                if (
                    $this->badges['buddypress'] == 'profile' ||
                    $this->badges['buddypress'] == 'both'
                ) {
                    add_action('bp_after_profile_loop_content', [
                        $this,
                        'insert_into_buddypress',
                    ]);
                }
            ...
            add_filter(
                'the_content',
                [$this, 'mycred_badge_page_template'],
                10,
                1
            );
        ...
        public function enqueue_front_scripts()
        {
            wp_enqueue_script(
                'mycred-badge-front',
                plugins_url('assets/js/front.js', myCRED_BADGE),
                '',
        ...
        public function module_admin_init()
        {
            ...
            add_filter('submenu_file', [$this, 'subparent_file'], 10, 2);
            add_action(
            ...
            add_action('post_submitbox_start', [$this, 'publishing_actions']);

            add_action('wp_ajax_mycred-assign-badge', [
                $this,
                'action_assign_badge',
            ]);
            add_action('wp_ajax_mycred-remove-connections', [
                $this,
                'action_remove_connections',
            ]);

            add_action(
                'mycred_user_edit_after_balances',
                [$this, 'badge_user_screen'],
                10
            );

            add_action(
                'personal_options_update',
                [$this, 'save_manual_badges'],
                10
            );
            add_action(
            ...
            add_action('mycred_delete_point_type', [
                $this,
                'delete_point_type',
            ]);
            add_action('before_delete_post', [$this, 'delete_badge']);

            add_filter('manage_' . MYCRED_BADGE_KEY . '_posts_columns', [
                $this,
                'adjust_column_headers',
            ]);
            add_action(
                'manage_' . MYCRED_BADGE_KEY . '_posts_custom_column',
                [$this, 'adjust_column_content'],
                10,
                2
            );
            add_action(
                'save_post_' . MYCRED_BADGE_KEY,
                [$this, 'save_badge'],
                10,
                2
            );
        ...
        public function register_badges()
        {
            ...
            $args = [
                ...
                'taxonomies' => [MYCRED_BADGE_CATEGORY],
            ];

            register_post_type(
                MYCRED_BADGE_KEY,
                apply_filters('mycred_register_badge', $args)
            );
        }
        ...
        public function register_badge_category()
        {
            ...
            register_taxonomy(MYCRED_BADGE_CATEGORY, MYCRED_BADGE_KEY, $args);
        }
        ...
        public function populate_current_account()
        {
            ...
            $earned = [];
            $users_badges = mycred_get_users_badges(
                $mycred_current_account->user_id,
                true
            );
        ...
        {
            if (
                !mycred_point_type_exists($point_type) ||
            ...
            if (!$mycred->user_is_point_editor()) {
                return;
            }
        ...
        {
            if (get_post_status($post_id) != MYCRED_BADGE_KEY) {
                return $post_id;
            }
            ...
            $wpdb->delete(
                $wpdb->usermeta,
                ['meta_key' => MYCRED_BADGE_KEY . $post_id],
                ['%s']
            );
        }
        ...
        public function parent_file($parent = '')
        {
            ...
            if (
                ...
                $_GET['post_type'] == MYCRED_BADGE_KEY
            ) {
                return MYCRED_SLUG;
            } elseif (
                $pagenow == 'post.php' &&
                isset($_GET['post']) &&
                mycred_get_post_type($_GET['post']) == MYCRED_BADGE_KEY
            ) {
                return MYCRED_SLUG;
            } elseif (
                ...
                isset($_GET['taxonomy']) &&
                $_GET['post_type'] == MYCRED_BADGE_KEY &&
                $_GET['taxonomy'] == MYCRED_BADGE_CATEGORY
            ) {
                $submenu_file =
                    'edit-tags.php?post_type=' .
                    MYCRED_BADGE_KEY .
        ...
        public function enqueue_scripts()
        {
            ...
            if ($screen->id == MYCRED_BADGE_KEY) {
                ...
                wp_register_script(
                    ...
                    ['jquery', 'mycred-mustache'],
                    myCRED_BADGE_VERSION . '.1'
                );

                wp_localize_script('mycred-edit-badge', 'myCREDBadge', [
                    ...
                    'levellabel' => esc_js(
                        sprintf('%s {{level}}', __('Level', 'mycred'))
                    ),
                    'uploadtitle' => esc_js(
                        esc_attr__('Badge Image', 'mycred')
                    ),
                    'uploadbutton' => esc_js(
                        esc_attr__('Use as Badge', 'mycred')
                    ),
                    'compareAND' => esc_js(
                        _x(
                            'AND',
                            'Comparison of badge requirements. A AND B',
                            'mycred'
                        )
                    ),
                    'compareOR' => esc_js(
                        _x(
                            'OR',
                            'Comparison of badge requirements. A OR B',
                            'mycred'
                        )
                    ),
                ]);
                ...
                add_filter(
                    'postbox_classes_' .
                        MYCRED_BADGE_KEY .
                        '_mycred-badge-setup',
                    [$this, 'metabox_classes']
                );
                add_filter(
                    'postbox_classes_' .
                        MYCRED_BADGE_KEY .
                        '_mycred-badge-default',
                    [$this, 'metabox_classes']
                );
                add_filter(
                    'postbox_classes_' .
                        MYCRED_BADGE_KEY .
                        '_mycred-badge-rewards',
                    [$this, 'metabox_classes']
                );
...
.mycred-badge-requirement-list.open_badge { border: 0px; }
        ...
        public function add_metaboxes()
        {
            ...
            add_meta_box(
                ...
                [$this, 'metabox_congratulation_msg'],
                MYCRED_BADGE_KEY,
                'normal',
                'low'
            );
            ...
            if ($setup['attachment_id'] > 0) {
                ...
                if (strlen($_image) > 5) {
                    $image =
                        '<img src="' .
                        $_image .
                        '" alt="Badge level image" /><input type="hidden" name="mycred_badge[levels][' .
                        $level .
                        '][attachment_id]" value="' .
                        $setup['attachment_id'] .
                        '" /><input type="hidden" name="mycred_badge[levels][' .
                        $level .
                        '][image_url]" value="" />';
                }
            } else {
                if (strlen($setup['image_url']) > 5) {
                    $image =
                        ...
                        '" alt="Badge level image" /><input type="hidden" name="mycred_badge[levels][' .
                        $level .
                        '][attachment_id]" value="0" /><input type="hidden" name="mycred_badge[levels][' .
                        $level .
                        '][image_url]" value="' .
                        $setup['image_url'] .
                        '" />';
        ...
        public function publishing_actions()
        {
            ...
            if (
                ...
                $post->post_type != MYCRED_BADGE_KEY
            ) {
                return;
            }

            $manual_badge =
                (int) mycred_get_post_meta($post->ID, 'manual_badge', true) == 1
                    ? true
                    : false;
            ?>
            <div id="mycred-badge-actions" class="seperate-bottom">
                ...
                <input type="hidden" name="mycred-badge-edit" value="<?php echo wp_create_nonce(
                    'edit-mycred-badge'
                ); ?>" />
                <input type="button" id="mycred-assign-badge-connections"<?php if (
                    $manual_badge ||
     ...
     'Assign Badge',
     'mycred'
 ); ?>" class="button button-secondary mycred-badge-action-button" data-action="mycred-assign-badge" data-token="<?php echo wp_create_nonce(
    'mycred-assign-badge'
); ?>" />
                <input type="button" id="mycred-remove-badge-connections"<?php if (
                    $post->post_status != 'publish'
                ) {
                    echo ' disabled="disabled"';
                } ?> value="<?php _e(
     'Remove Connections',
     'mycred'
 ); ?>" class="button button-secondary mycred-badge-action-button" data-action="mycred-remove-connections" data-token="<?php echo wp_create_nonce(
    'mycred-remove-badge-connection'
); ?>" />
                    ...
                    jQuery(function($) {
                        ...
                        $( 'input.mycred-badge-action-button' ).click(function(){
                            ...
                            $.ajax({
                                ...
                                url : ajaxurl,
                                beforeSend : function() {
                                    button.attr( 'value', '<?php echo esc_js(
                                        esc_attr__('Processing...', 'mycred')
                                    ); ?>' );
            ...
            <div id="mycred-manual-badge" class="seperate-bottom">
                <label for="mycred-badge-is-manual"><input type="checkbox" name="mycred_badge[manual]" id="mycred-badge-is-manual"<?php if (
                    $manual_badge
                ) {
     ...
     'mycred'
        ...
        {
            $default_image = $di = mycred_get_post_meta(
                $post->ID,
                'main_image',
                true
            );
            ...
            if (is_numeric($di) && strpos('://', $di) === false) {
                $attachment = $di;
                $default_image =
                    '<img src="' . wp_get_attachment_url($di) . '" alt="" />';
            }
            ?>
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="default-image text-center seperate-bottom">
                        <div class="default-image-wrapper image-wrapper<?php if (
                            $default_image == ''
                        ) {
                            echo ' empty dashicons';
                        } ?>">
                            <?php echo $default_image; ?>
                            <input type="hidden" name="mycred_badge[main_image]" id="badge-main-image-id" value="<?php if (
                                $attachment
                            ) {
                                echo esc_attr($di);
                            } ?>" />
                            <input type="hidden" name="mycred_badge[main_image_url]" id="badge-main-image-url" value="<?php if (
                                $di != '' &&
                                strpos('://', $di) !== false
                            ) {
                        ...
                        <div class="level-image-actions">
                            <button type="button" class="button button-secondary" id="badges-change-default-image" data-do="<?php if (
                                $default_image == ''
                            ) {
                                echo 'set';
                            } else {
    ...
    _e('Change Image', 'mycred');
} ?></button>
                            <button type="button" class="button button-secondary <?php echo !$attachment
                                ? 'hidden'
                                : ''; ?>" id="badges-remove-default-image"><?php _e('Remove Image', 'mycred'); ?></button>
                        </div>
                    </div>
                    <span class="description"><?php _e(
                        'Optional image to show when a user has not earned this badge.',
                        'mycred'
                    ); ?></span>
                </div>
            </div>
        ...
        {
            $open_badge =
                mycred_get_post_meta($post->ID, 'open_badge', true) == 1
                    ? true
                    : false; ?>
            <div id="mycred-open-badge" class="seperate-bottom">
                <label for="mycred-badge-is-open-badge"><input type="checkbox" name="mycred_badge[open_badge]" id="mycred-badge-is-open-badge"<?php if (
                    $open_badge
                ) {
                    echo ' checked="checked"';
                } ?> value="1" /> <?php _e('This badge is Open Badge.', 'mycred'); ?></label>
            </div>
            <span class="description"><?php _e(
                'Multi level badge settings will be disable when switched to open badge.',
                'mycred'
            ); ?></span>
            <?php
        }
        ...
        {
            $congratulation_msg = mycred_get_post_meta(
                $post->ID,
                'congratulation_msg',
                true
            ); ?>
            <style>#mycred-badge-congratulation-msg{display:block;margin:12px 0 0;width:100%;}</style>
            <textarea name="mycred_badge[congratulation_msg]" placeholder="You have earned this Badge!" id="mycred-badge-congratulation-msg"><?php echo empty(
                $congratulation_msg
            )
                ? ''
                : $congratulation_msg; ?></textarea>
            <?php
        }
        ...
        public function metabox_badge_setup($post)
        {
            ...
            <div id="badge-levels">
                ...
                foreach ($badge->levels as $level => $setup) {
                    ...
                    $add_level =
                        '<button type="button" class="button button-seconary button-small top-right-corner" id="badges-add-new-level">' .
                        esc_js(__('Add Level', 'mycred')) .
                        '</button>';
                    $remove_level =
                        '<button type="button" class="button button-seconary button-small top-right-corner remove-badge-level" data-level="' .
                        $level .
                        '">' .
                        esc_js(__('Remove Level', 'mycred')) .
                        '</button>';
                    ...
                    $js_level = str_replace(
                        '{{levelimagebutton}}',
                        esc_js(__('Set Image', 'mycred')),
                        $js_level
                    );
                    $js_level = str_replace(
                        '{{levelplaceholder}}',
                        esc_js(__('Level', 'mycred')) . ' {{levelone}}',
                        $js_level
                    );
                    ...
                    $template = str_replace(
                        ...
                        $level_image === false
                            ? esc_js(__('Set Image', 'mycred'))
                            : esc_js(__('Change Image', 'mycred')),
                        $template
                    );

                    $template = str_replace(
                        '{{levelplaceholder}}',
                        esc_js(sprintf(__('Level %d', 'mycred'), $level + 1)),
                        $template
                    );
                    $template = str_replace(
                        '{{levellabel}}',
                    ...
                    $template = str_replace(
                        '{{adnselected}}',
                        $setup['compare'] === 'AND' ? 'selected' : '',
                        $template
                    );
                    $template = str_replace(
                        '{{orselected}}',
                        $setup['compare'] === 'OR' ? 'selected' : '',
                        $template
                    );

                    $template = str_replace(
                        '{{badge_compare_andor}}',
                        isset($setup['compare']) && !empty($setup['compare'])
                            ? $setup['compare']
                            : 'AND',
                        $template
                    );
                    ...
                    foreach ($setup['requires'] as $req_level => $reqsetup) {
                        ...
                        if ($reqsetup['type'] == '') {
                            $point_type_options .= ' selected="selected"';
                        }
                        $point_type_options .=
                            '>' .
                            esc_js(__('Select Point Type', 'mycred')) .
                            '</option>';
                        foreach ($point_types as $type_id => $type_label) {
                            $point_type_options .=
                                '<option value="' . esc_attr($type_id) . '"';
                            if ($reqsetup['type'] == $type_id) {
                                $point_type_options .= ' selected="selected"';
                            }
                            $point_type_options .=
                        ...
                        $point_type_options = str_replace(
                            'selected="selected"',
                            '',
                            $point_type_options
                        );
                        $js_requirement = str_replace(
                            '{{pointtypes}}',
                            $point_type_options,
                            $js_requirement
                        );
                        ...
                        if ($reqsetup['reference'] == '') {
                            $reference_options .= ' selected="selected"';
                        }
                        $reference_options .=
                            '>' .
                            esc_js(__('Select Reference', 'mycred')) .
                            '</option>';
                        foreach ($references as $ref_id => $ref_label) {
                            $reference_options .=
                                '<option value="' . esc_attr($ref_id) . '"';
                            if ($reqsetup['reference'] == $ref_id) {
                                $reference_options .= ' selected="selected"';
                            }
                            $reference_options .=
                                '>' . esc_html($ref_label) . '</option>';
                        }
                        ...
                        $requirement_specific = apply_filters(
                            'mycred_badge_requirement_specific_template',
                            '',
                            $req_level,
                            $reqsetup,
                            $badge,
                        ...
                        if ($reqsetup['by'] == '') {
                            $by_options .= ' selected="selected"';
                        }
                        $by_options .=
                            '>' . __('Select', 'mycred') . '</option>';
                        foreach ($sums as $sum_id => $sum_label) {
                            $by_options .= '<option value="' . $sum_id . '"';
                            if ($reqsetup['by'] == $sum_id) {
                                $by_options .= ' selected="selected"';
                            }
                        ...
                        $requirement_button =
                            '<button type="button" class="button button-primary form-control remove-requirement" data-req="{{reqlevel}}">-</button>';
                        $js_requirement = str_replace(
                            '{{reqbutton}}',
                            $requirement_button,
                            $js_requirement
                        ...
                        $requirement_button =
                            '<button type="button" class="button button-primary form-control remove-requirement" data-req="' .
                            $req_level .
                            '">-</button>';
                        if ($req_level == 0) {
                            $requirement_button =
                                '<button type="button" class="button button-secondary form-control" id="badges-add-new-requirement">+</button>';
                        }
                    ...
                    $rewards = str_replace(
                        '{{logplaceholder}}',
                        esc_js(__('Log template', 'mycred')),
                        $rewards
                    );
                    $rewards = str_replace(
                        '{{logtemplate}}',
                        esc_js($setup['reward']['log']),
                        $rewards
                    );
                    $rewards = str_replace(
                        '{{rewardamount}}',
        ...
        public function save_badge($post_id, $post = null)
        {
            if (
                $post === null ||
                !$this->core->user_is_point_editor() ||
                !isset($_POST['mycred_badge'])
            ) {
                return $post_id;
            }
            ...
            // Else we are using a URL (old setup)
                ...
                $image = sanitize_text_field(
                    $_POST['mycred_badge']['main_image_url']
                );
            }
            ...
            if (isset($_POST['mycred_badge']['congratulation_msg'])) {
                $congratulation_msg = sanitize_text_field(
                    $_POST['mycred_badge']['congratulation_msg']
                );
            }
            ...
            if (!empty($_POST['mycred_badge']['levels'])) {
                ...
                foreach (
                    $_POST['mycred_badge']['levels']
                    as $level_id => $level_setup
                ) {
                    $level = [];

                    if (array_key_exists('attachment_id', $level_setup)) {
                        $level['attachment_id'] = absint(
                            $level_setup['attachment_id']
                        );
                        $level['image_url'] = array_key_exists(
                            'image_url',
                            $level_setup
                        )
                            ? sanitize_text_field($level_setup['image_url'])
                            : '';
                    }
                    ...
                    $level['label'] = sanitize_text_field(
                        $level_setup['label']
                    );

                    if (array_key_exists('compare', $level_setup)) {
                        $level['compare'] =
                            $level_setup['compare'] == 'AND' ? 'AND' : 'OR';
                    } else {
                        $level['compare'] = array_key_exists(
                            'compare',
                            $badge_levels[0]
                        )
                            ? $badge_levels[0]['compare']
                            : 'AND';
                    }
                    ...
                    if (array_key_exists('requires', $level_setup)) {
                        ...
                        $row = 0;
                        foreach (
                            $level_setup['requires']
                            as $requirement_id => $requirement_setup
                        ) {
                            $requirement = [];
                            $requirement['type'] = array_key_exists(
                                'type',
                                $requirement_setup
                            )
                                ? sanitize_key($requirement_setup['type'])
                                : '';
                            $requirement['reference'] = array_key_exists(
                                'reference',
                                $requirement_setup
                            )
                                ? sanitize_key($requirement_setup['reference'])
                                : '';
                            $requirement['amount'] = array_key_exists(
                                'amount',
                                $requirement_setup
                            )
                                ? sanitize_text_field(
                                    $requirement_setup['amount']
                                )
                                : '';
                            $requirement['by'] = array_key_exists(
                                'by',
                                $requirement_setup
                            )
                                ? sanitize_key($requirement_setup['by'])
                                : '';
                            $requirement['specific'] = array_key_exists(
                                'specific',
                                $requirement_setup
                            )
                                ? sanitize_text_field(
                                    $requirement_setup['specific']
                                )
                                : '';
                        ...
                        foreach (
                            ...
                            $completed_requirements[$requirement_id] =
                                $badge_requirements[$requirement_id];
                            $completed_requirements[$requirement_id]['amount'] =
                                $requirement_setup['amount'];
                        }
                    ...
                    $reward = ['type' => '', 'log' => '', 'amount' => ''];

                    if (array_key_exists('reward', $level_setup)) {
                        $reward['type'] = sanitize_key(
                            $level_setup['reward']['type']
                        );
                        $reward['log'] = sanitize_text_field(
                            $level_setup['reward']['log']
                        );
                        ...
                        $reward['amount'] = $mycred->number(
                            $level_setup['reward']['amount']
                        );
                    }
        ...
        public function after_general_settings($mycred = null)
        {
            ...
            ?>
            <h4><span class="dashicons dashicons-admin-plugins static"></span><?php _e(
                'Badges',
                'mycred'
            ); ?></h4>
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                        ...
                        <div class="form-group">
                            <div class="checkbox" style="padding-top: 4px;">
                                <label for="<?php echo $this->field_id(
                                    ...
                                    <input type="checkbox" name="<?php echo $this->field_name(
                                        'show_level_description'
                                    ); ?>" id="<?php echo $this->field_id('show_level_description'); ?>" <?php checked($settings['show_level_description'], 1); ?> value="1"><?php _e('Show Level Description', 'mycred'); ?>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="checkbox" style="padding-top: 4px;">
                                <label for="<?php echo $this->field_id(
                                    'show_congo_text'
                                ); ?>"><input type="checkbox" name="<?php echo $this->field_name('show_congo_text'); ?>" id="<?php echo $this->field_id('show_congo_text'); ?>" <?php checked($settings['show_congo_text'], 1); ?> value="1"> <?php _e('Show Congratulation Text', 'mycred'); ?></label>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="checkbox" style="padding-top: 4px;">
                                ...
                                ); ?>"><input type="checkbox" name="<?php echo $this->field_name('show_steps_to_achieve'); ?>" id="<?php echo $this->field_id('show_steps_to_achieve'); ?>" <?php checked($settings['show_steps_to_achieve'], 1); ?> value="1"> <?php _e('Show Steps to Achieve', 'mycred'); ?></label>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                        ...
                        <div class="form-group">
                            <div class="checkbox" style="padding-top: 4px;">
                                <label for="<?php echo $this->field_id(
                                    'show_levels'
                                ); ?>"><input type="checkbox" name="<?php echo $this->field_name('show_levels'); ?>" id="<?php echo $this->field_id('show_levels'); ?>" <?php checked($settings['show_levels'], 1); ?> value="1"> <?php _e('Show Levels', 'mycred'); ?></label>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="checkbox" style="padding-top: 4px;">
                                <label for="<?php echo $this->field_id(
                                    'show_level_points'
                                ); ?>"><input type="checkbox" name="<?php echo $this->field_name('show_level_points'); ?>" id="<?php echo $this->field_id('show_level_points'); ?>" <?php checked($settings['show_level_points'], 1); ?> value="1"> <?php _e('Show Level Reward', 'mycred'); ?></label>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="checkbox" style="padding-top: 4px;">
                                <label for="<?php echo $this->field_id(
                                    'show_earners'
                                ); ?>"><input type="checkbox" name="<?php echo $this->field_name('show_earners'); ?>" id="<?php echo $this->field_id('show_earners'); ?>" <?php checked($settings['show_earners'], 1); ?> value="1"> <?php _e('Show Earners', 'mycred'); ?></label>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                        <div class="form-group">
                            <div class="checkbox">
                                <label for="<?php echo $this->field_id(
                                    'open_badge'
                                ); ?>"><input type="checkbox" name="<?php echo $this->field_name('open_badge'); ?>" id="<?php echo $this->field_id('open_badge'); ?>" <?php checked($settings['open_badge'], 1); ?> value="1" > <?php _e('Enable Open Badge.', 'mycred'); ?></label>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                ...
                <div class="row">
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                        <div class="form-group">
                            ...
                            <?php if ($buddypress): ?>
                            <select name="<?php echo $this->field_name(
                                'buddypress'
    ...
    'buddypress'
                                ...
                                $buddypress_options = [
                                    ...
                                    'header' => __(
                                        'Include in Profile Header',
                                        'mycred'
                                    ),
                                    'profile' => __(
                                        'Include under the "Profile" tab',
                                        'mycred'
                                    ),
                                    'both' => __(
                                        'Include under the "Profile" tab and Profile Header',
                                        'mycred'
                                    ),
                                ];
                                ...
                                foreach (
                                    ...
                                    if (
                                        isset($settings['buddypress']) &&
                                        $settings['buddypress'] == $location
                                    ) {
    ...
    'show_all_bp'
); ?>" id="<?php echo $this->field_id('show_all_bp'); ?>" <?php checked(
    $settings['show_all_bp'],
    1
); ?> value="1" /> <?php _e(
     'Show all badges, including badges users have not yet earned.',
     'mycred'
 ); ?></label>
                            </div>
                            <?php else: ?>
                                <input type="hidden" name="<?php echo $this->field_name(
                                    'buddypress'
                                ); ?>" id="<?php echo $this->field_id(
    'buddypress'
); ?>" value="" />
                                <p><span class="description"><?php _e(
                                    'Not installed',
                                    'mycred'
                                ); ?></span></p>
                            <?php endif; ?>
    ...
    'bbpress'
                                ...
                                $bbpress_options = [
                                    ...
                                    'reply' => __(
                                        'Include in Forum Replies',
                                        'mycred'
                                    ),
                                    'both' => __(
                                        'Include in Profile and Forum Replies',
                                        'mycred'
                                    ),
                                ];
                                ...
                                foreach (
                                    ...
                                    echo '<option value="' . $location . '"';
                                    if (
                                        isset($settings['bbpress']) &&
                                        $settings['bbpress'] == $location
                                    ) {
                                        echo ' selected="selected"';
                                    }
                                    echo '>' . $description . '</option>';
    ...
    'show_all_bb'
); ?>" id="<?php echo $this->field_id('show_all_bb'); ?>" <?php checked(
    $settings['show_all_bb'],
    1
); ?> value="1" /> <?php _e(
     'Show all badges, including badges users have not yet earned.',
     'mycred'
    ...
    'bbpress'
                ...
                <h3 style="margin-bottom: 0;"><?php _e(
        ...
        {
            $new_data['badges']['show_all_bp'] = isset(
                $data['badges']['show_all_bp']
            )
                ? $data['badges']['show_all_bp']
                : 0;
            $new_data['badges']['show_all_bb'] = isset(
                $data['badges']['show_all_bb']
            )
                ? $data['badges']['show_all_bb']
                : 0;

            $new_data['badges']['buddypress'] = isset(
                $data['badges']['buddypress']
            )
                ? sanitize_text_field($data['badges']['buddypress'])
                : '';
            $new_data['badges']['bbpress'] = isset($data['badges']['bbpress'])
                ? sanitize_text_field($data['badges']['bbpress'])
                : '';

            $new_data['badges']['open_badge'] = isset(
                $data['badges']['open_badge']
            )
                ? intval($data['badges']['open_badge'])
                : 0;

            //Specific Badge Page Setup @since 2.1
            $new_data['badges']['show_level_description'] = isset(
                $data['badges']['show_level_description']
            )
                ? intval($data['badges']['show_level_description'])
                : 0;
            $new_data['badges']['show_congo_text'] = isset(
                $data['badges']['show_congo_text']
            )
                ? intval($data['badges']['show_congo_text'])
                : 0;
            $new_data['badges']['show_levels'] = isset(
                $data['badges']['show_levels']
            )
                ? intval($data['badges']['show_levels'])
                : 0;
            $new_data['badges']['show_level_points'] = isset(
                $data['badges']['show_level_points']
            )
                ? intval($data['badges']['show_level_points'])
                : 0;
            $new_data['badges']['show_steps_to_achieve'] = isset(
                $data['badges']['show_steps_to_achieve']
            )
                ? intval($data['badges']['show_steps_to_achieve'])
                : 0;
            $new_data['badges']['show_earners'] = isset(
                $data['badges']['show_earners']
            )
                ? intval($data['badges']['show_earners'])
                : 0;
            $new_data['badges']['open_badge_evidence_page'] = isset(
                $data['badges']['open_badge_evidence_page']
            )
                ? intval($data['badges']['open_badge_evidence_page'])
                : 0;
        ...
        public function badge_user_screen($user)
        {
            ...
            <style type="text/css">
                ...
                .badge-image-wrap .badge-image.empty { content: "<?php _e(
                    'No image set',
                    'mycred'
                ); ?>"; }
                        ...
                        <fieldset id="mycred-badge-list" class="badge-list">
                            ...
                            <?php if (!empty($all_badges)) {
                                foreach ($all_badges as $badge_id) {
                                    ...
                                    $level_select =
                                        '<input type="hidden" name="mycred_badge_manual[badges][' .
                                        $badge_id .
                                        '][level]" value="0" /><select disabled="disabled"><option>Level 1</option></select>';
                                    if (count($badge->levels) > 1) {
                                        $level_select =
                                            '<select name="mycred_badge_manual[badges][' .
                                            $badge_id .
                                            '][level]">';
                                    ...
                                    ?>
                                    <div class="badge-wrapper color-option<?php if (
                                        $earned === 1
                                    ) {
                                        echo ' selected';
                                    } ?>" id="mycred-badge<?php echo $badge_id; ?>-wrapper">
                                        <label for="mycred-badge<?php echo $badge_id; ?>"><input type="checkbox" name="mycred_badge_manual[badges][<?php echo $badge_id; ?>][has]" class="toggle-badge" id="mycred-badge<?php echo $badge_id; ?>" <?php checked(
    $earned,
    1
); ?> value="1" /> <?php _e('Earned', 'mycred'); ?></label>
                                        <div class="badge-image-wrap">

                                            <div class="badge-image<?php if (
                                                $badge_image == ''
                                            ) {
                                                echo ' empty';
                                            } ?>"><?php echo $badge_image; ?></div>
                        ...
                        <input type="hidden" name="mycred_badge_manual[token]" value="<?php echo wp_create_nonce(
                            'mycred-manual-badges' . $user_id
                        ); ?>" />
                    </td>
                </tr>
            </table>
        ...
        public function save_manual_badges($user_id)
        {
            ...
            if (isset($_POST['mycred_badge_manual']['token'])) {
                if (
                    wp_verify_nonce(
                        $_POST['mycred_badge_manual']['token'],
                        'mycred-manual-badges' . $user_id
                    )
                ) {
                    ...
                    if (!empty($_POST['mycred_badge_manual']['badges'])) {
                        foreach (
                            $_POST['mycred_badge_manual']['badges']
                            as $badge_id => $data
                        ) {
                            $badge = mycred_get_badge($badge_id);
                            ...
                            // Give badge
                            if (
                                !array_key_exists($badge_id, $users_badges) &&
                                isset($data['has']) &&
                                $data['has'] == 1
                            ) {
                                $level = 0;
                            ...
                            elseif (
                                array_key_exists($badge_id, $users_badges) &&
                                isset($data['level']) &&
                                $data['level'] != $users_badges[$badge_id]
                            ) {
        ...
        public function action_assign_badge()
        {
            ...
            if ($results > 0) {
                wp_send_json_success(
                    sprintf(
                        __(
                            'A total of %d users have received this badge.',
                            'mycred'
                        ),
                        $results
                    )
            ...
            wp_send_json_error(
                __('No users has yet earned this badge.', 'mycred')
            );
        }
        ...
        public function action_remove_connections()
        {
            ...
            if ($results == 0) {
                wp_send_json_success(
                    __('No connections where removed.', 'mycred')
                );
            }

            wp_send_json_success(
                sprintf(__('%s connections where removed.', 'mycred'), $results)
            );
        }
        ...
        public function insert_into_bbpress_profile()
        {
            ...
            if (
                isset($this->badges['show_all_bb']) &&
                $this->badges['show_all_bb'] == 1
            ) {
                echo mycred_render_my_badges([
                    'show' => 'all',
        ...
        public function insert_into_buddypress()
        {
            ...
            if (
                isset($this->badges['show_all_bp']) &&
                $this->badges['show_all_bp'] == 1
            ) {
                echo mycred_render_my_badges([
                    'show' => 'all',
                    'width' => MYCRED_BADGE_WIDTH,
        ...
        public function mycred_open_badge_init()
        {
            ...
            add_action(
                'mycred_after_badge_assign',
                [$mycred_Open_Badge, 'bake_users_image'],
                10,
                2
            );
            add_action('rest_api_init', [
                $mycred_Open_Badge,
                'register_open_badge_routes',
            ]);
        }
        ...
        public function mycred_badge_page_template($content)
        {
            ...
            if ($post->post_type == MYCRED_BADGE_KEY && is_single()) {
                ...
                if (
                    is_array($mycred->core) &&
                    array_key_exists('badges', $mycred->core)
                ) {
                    $show_level_description =
                        $mycred->core["badges"]["show_level_description"] == '1'
                            ? true
                            : false;
                    $show_congo_text =
                        $mycred->core["badges"]["show_congo_text"] == '1'
                            ? true
                            : false;
                    $show_levels =
                        $mycred->core["badges"]["show_levels"] == '1'
                            ? true
                            : false;
                    $show_level_points =
                        $mycred->core["badges"]["show_level_points"] == '1'
                            ? true
                            : false;
                    $show_steps_to_achieve =
                        $mycred->core["badges"]["show_steps_to_achieve"] == '1'
                            ? true
                            : false;
                    $show_earners =
                        $mycred->core["badges"]["show_earners"] == '1'
                            ? true
                            : false;
                }
                ...
                if ($show_congo_text && $has_earned) {
                    if (!empty($badge_object->congratulation_msg)) {
                        $content .=
                            '<div class="mycred-badge-congratulation-msg">' .
                            $badge_object->congratulation_msg .
                            '</div>';
                    }
                }
                ...
                if (
                    $show_levels ||
                    ($badge_object->open_badge && $show_steps_to_achieve)
                ) {
                    if (!$badge_object->open_badge) {
                        $content .= "<h3>" . __("Levels", "mycred") . "</h3>";
                    } else {
                        $content .=
                            "<h3>" . __("Requirement", "mycred") . "</h3>";
                    }
                    ...
                    foreach ($levels as $id => $level) {
                        ...
                        if (!$badge_object->open_badge) {
                            if ($level_image_url != '') {
                                $content .=
                                    '<img src="' .
                                    $level_image_url .
                                    '" class="mycred-level-image mycred-float-left" />';
                            }
                        ...
                        if ($show_steps_to_achieve) {
                            $content .= mycred_badge_level_req_check(
                                $badge_id,
                                $id
                            );
                        }
                        ...
                        if ($show_level_points && $reward != 0) {
                            ...
                            $content .=
                                '<div class="mycred-level-reward">' .
                                __('Reward:', 'mycred') .
                                ' ' .
                                $reward_type->format_creds($reward) .
                                '</div>';
                        }
                ...
                if ($show_earners) {
                    $users_have_badge = mycred_get_users_has_earned_badge(
                        $badge_id
                    );
                    ...
                    if ($earned_count > 0) {
                        ...
                        foreach ($users_have_badge as $user) {
                            ...
                            $content .=
                                '<div class="mycred-badge-earner-grid mycred-float-left">';
                            $content .=
                                '<div><img src="' .
                                $user_profile .
                                '" /></div>';
        ...
        public function flush_rewrite_rules_for_badges()
        {
            ...
            if (
                $pagenow == 'admin.php' &&
                isset($_GET['page']) &&
                $_GET['page'] == 'mycred-addons' &&
                isset($_GET['badges']) &&
                $_GET['badges'] == 'activate'
            ) {
                flush_rewrite_rules();
            }
        }
