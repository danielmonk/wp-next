jQuery(function ($) {
  ...
  var mycred_get_badge_requirement_template = function (badgelevel) {
    ...
    $("#mycred-badge-level0 .level-requirements .row-narrow").each(function (
      index
    ) {
      // Row
      ...
      if (required_reference_specific[0] != undefined) {
        ...
        if (required_reference_specific[0].tagName === "INPUT") {
          ...
          var required_reference_specific = $(this).find(
            "select.specific option:selected"
          );
          if (required_reference === undefined || required_reference == "")
            required_reference_specific = "-";
          else required_reference_specific = required_reference_specific.text();
        }
      } else {
        required_reference_specific = "";
      }
      ...
      // Amount
      var required_amount = $(this)
        .find(".form-inline input.form-control")
        .val();
      ...
      if (required_type === undefined || required_type == "")
        required_type = "-";
      else required_type = required_type.text();

      // Comparison
      var compare_to_show = myCREDBadge.compareOR;
      if (UsedtoCompare == "AND") compare_to_show = myCREDBadge.compareAND;

      var totalrequirements = $(
        "#mycred-badge-level0 .level-requirements .row-narrow"
      ).length;
      if (
        (totalrequirements > 1 && index + 1 == totalrequirements) ||
        (totalrequirements == 1 && index == 0)
      )
        compare_to_show = "";
  ...
  $(document).ready(function () {
    TotalBadgeLevels = $("#mycred-badge-setup #badge-levels .badge-level")
      .length;
    TotalRequirements = $("#mycred-badge-setup .level-requirements .row-narrow")
      .length;

    console.log("Total levels detected: " + TotalBadgeLevels);

    if (TotalBadgeLevels > 1) {
      $("#mycred-badge-setup #badge-levels .badge-level").each(function (
        index
      ) {
        ...
        if (subrequirementrow !== undefined) {
          subrequirementrow
            .empty()
            .text($("#mycred-badge-setup input.specific").val());
        }
      });
    }
    ...
    RequirementCompare.click(function (e) {
      ...
      var numberoflevels = $("#mycred-badge-setup #badge-levels .badge-level")
        .length;
      if (numberoflevels == 1) return;

      var newrequirementlevel = $(
        "#mycred-badge-level0 .level-requirements .row-narrow"
      ).length;
      ...
      $(
        "#mycred-badge-setup #badge-levels .level-requirements .level-compare p"
      ).each(function (index) {
        $(this).fadeOut(function () {
          ...
          if (newrequirementlevel > 1 && reqrowid + 1 == newrequirementlevel)
            $(this).empty().fadeIn();
          else $(this).empty().text(compare_to_show).fadeIn();
        });
      });
    });
    ...
    AddNewRequirementButton.click(function (e) {
      ...
      $("#mycred-badge-setup #mycred-badge-level0 .level-requirements").append(
        template
      );
      ...
      if (totallevels.length > 1) {
        ...
        totallevels.each(function (index) {
          ...
          if (currentlevel > 0) {
            ...
            $(
              "#mycred-badge-setup #mycred-badge-level" +
                currentlevel +
                " .level-requirements"
            ).append(reqtemplate);
    ...
    $("#mycred-badge-setup").on("change", "select.point-type", function (e) {
      ...
      $("#mycred-badge-setup #badge-levels .badge-level").each(function (
        ...
        var subrequirementrow = $(this).find(
          "#level" +
            badgelevel +
            "requirement" +
            requirementrow +
    ...
    AddNewLevelButton.click(function (e) {
      ...
      TotalBadgeLevels = $("#mycred-badge-setup #badge-levels .badge-level")
        .length;
      TotalRequirements = $(
        "#mycred-badge-setup .level-requirements .row-narrow"
      ).length;
    ...
    $("#mycred-badge-setup").on(
      ...
      function (e) {
        ...
        LevelImageSelector.on("select", function () {
          attachment = LevelImageSelector.state()
            .get("selection")
            .first()
            .toJSON();
          if (attachment.url != "") {
            $(
              "#mycred-badge-level" + currentlevel + " .level-image-wrapper"
            ).fadeOut(function () {
              $("#mycred-badge-level" + currentlevel + " .level-image-wrapper")
                .empty()
                .removeClass("empty dashicons")
                .html(
                  '<img src="' +
                    attachment.url +
    ...
    $("#mycred-badge-setup").on(
      "click",
      "button.remove-badge-level",
      function (e) {
        var leveltoremove = $(this).data("level");
        if ($("#mycred-badge-level" + leveltoremove) === undefined)
          return false;
        ...
        $("#mycred-badge-level" + leveltoremove)
          .slideUp()
          .remove();
        TotalBadgeLevels--;
      }
    ...
    ChangeDefaultImageButton.click(function (e) {
      ...
      DefaultImageSelector.on("select", function () {
        attachment = DefaultImageSelector.state()
          .get("selection")
          .first()
          .toJSON();
        if (attachment.url != "") {
          $("#mycred-badge-default .default-image-wrapper").fadeOut(
            function () {
              $("#mycred-badge-default .default-image-wrapper")
                .empty()
                .removeClass("empty dashicons")
                .html(
                  '<img src="' +
                    attachment.url +
                    '" alt="Badge default image" /><input type="hidden" name="mycred_badge[main_image]" value="' +
                    attachment.id +
                    '" /><input type="hidden" name="mycred_badge[main_image_url]" value="" />'
                )
                .fadeIn();
              button.text(myCREDBadge.changeimage);
