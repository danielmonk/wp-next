...
if (!function_exists('mycred_get_badge')):
    function mycred_get_badge($badge_id = null, $level = null)
    {
        if (
            absint($badge_id) === 0 ||
            mycred_get_post_type($badge_id) != MYCRED_BADGE_KEY
        ) {
            return false;
        }
    ...
    function mycred_get_badge_references(
        ...
        if (!is_array($references) || empty($references) || $force) {
            ...
            $references = $wpdb->get_results(
                "SELECT post_id, meta_value FROM {$wpdb->postmeta} WHERE meta_key = 'badge_requirements';"
            );
            if (!empty($references)) {
                foreach ($references as $entry) {
                    ...
                    if (
                        !array_key_exists('type', $requirement) ||
                        $requirement['type'] != $point_type ||
                    ...
                    if (
                        !array_key_exists($requirement['reference'], $new_list)
                    ) {
                        $new_list[$requirement['reference']] = [];
                    }
            ...
            $references = $wpdb->get_results(
                "SELECT post_id, meta_value FROM {$table} WHERE meta_key = 'badge_prefs';"
            );
            if (!empty($references)) {
                foreach ($references as $entry) {
                    ...
                    foreach ($levels as $level => $setup) {
                        ...
                        foreach (
                            $setup['requires']
                            as $requirement_row => $requirement
                        ) {
                            if (
                                $requirement['type'] != $point_type ||
                                $requirement['reference'] == ''
...
if (!function_exists('mycred_get_badge_levels')):
    function mycred_get_badge_levels($badge_id)
    {
        ...
        if (!is_array($setup) || empty($setup)) {
            // Backwards comp.
            $old_setup = mycred_get_post_meta(
                $badge_id,
                'badge_requirements',
                true
            );
            ...
            if (is_array($old_setup) && !empty($old_setup)) {
                ...
                foreach ($old_setup as $level => $requirements) {
                    $level_image = mycred_get_post_meta(
                        $badge_id,
                        'level_image' . $level,
                        true
                    );
    ...
    function mycred_display_badge_requirements($badge_id = null)
    {
        ...
        if (empty($levels)) {
            ...
            foreach (
                ...
                $base_requirements[$requirement_row] = [
            ...
            foreach ($levels as $level => $setup) {
                ...
                if (!$badge->open_badge) {
                    $level_label =
                        '<strong>' .
                        sprintf(__('Level %s', 'mycred'), $level + 1) .
                        ':</strong>';
                ...
                foreach (
                    ...
                    if ($requirement['by'] == 'count') {
                        ...
                        $rendered_row = sprintf(
                            _x(
                                '%s %s for "%s"',
                                '"Gained/Lost" "x points" for "reference"',
                                'mycred'
                            ),
                            $requirement['amount'] < 0
                                ? __('Lost', 'mycred')
                                : __('Gained', 'mycred'),
                            $mycred->format_creds($requirement['amount']),
                            $requirement['ref']
                        );
                    }
                ...
                $output[] =
                    $level_label .
                    '<ul class="mycred-badge-requirement-list ' .
                    ($badge->open_badge ? 'open_badge' : '') .
                    '"><li>' .
                    implode('</li><li>', $level_req) .
                    '</li></ul>';
            }

            if (
                (int) mycred_get_post_meta($badge_id, 'manual_badge', true) ===
                1
            ) {
                $output[] =
                    '<strong><small><em>' .
                    __('This badge is manually awarded.', 'mycred') .
                    '</em></small></strong>';
            }
        ...
        return apply_filters(
            'mycred_badge_display_requirements',
            $reply,
            $badge_id
        );
    }
...
if (!function_exists('mycred_ref_has_badge')):
    function mycred_ref_has_badge(
        ...
        if (
            !empty($badge_references) &&
            array_key_exists($reference, $badge_references)
        ) {
...
if (!function_exists('mycred_check_if_user_gets_badge')):
    function mycred_check_if_user_gets_badge(
        $user_id = null,
        $badge_ids = [],
        $depreciated = [],
        $save = true
...
if (!function_exists('mycred_user_has_badge')):
    function mycred_user_has_badge(
        $user_id = 0,
        $badge_id = null,
        $level_id = 0
    ) {
...
if (!function_exists('mycred_get_users_badge_level')):
    function mycred_get_users_badge_level($user_id = 0, $badge_id = null)
    {
        ...
        if (
            ...
            !empty($mycred_current_account->badges) &&
            array_key_exists($badge_id, $mycred_current_account->badges)
        ) {
...
if (!function_exists('mycred_get_users_badges')):
    function mycred_get_users_badges($user_id = null, $force = false)
    {
        ...
        $badge_ids = mycred_get_user_meta(
            $user_id,
            MYCRED_BADGE_KEY . '_ids',
            '',
            true
        );
        if (!isset($badge_ids) || $badge_ids == '' || $force) {
            ...
            $query = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->usermeta} WHERE user_id = %d AND meta_key LIKE %s",
                    $user_id,
                    mycred_get_meta_key(MYCRED_BADGE_KEY) . '%'
                )
            );

            if (!empty($query)) {
                foreach ($query as $badge) {
                    $badge_id = str_replace(
                        MYCRED_BADGE_KEY,
                        '',
                        $badge->meta_key
                    );
...
if (!function_exists('mycred_get_badge_ids')):
    function mycred_get_badge_ids()
    {
        ...
        if ($badge_ids !== false && is_array($badge_ids)) {
            return $badge_ids;
        }
...
if (!function_exists('mycred_get_evidence_page_id')):
    function mycred_get_evidence_page_id()
    {
        ...
        if (isset($badges['open_badge']) && $badges['open_badge'] == '1') {
            ...
            $evidence_page_refrence = mycred_get_option(
                'open_badge_evidence_page',
                0
            );

            if (
                !empty($badges['open_badge_evidence_page']) ||
                !empty($evidence_page_refrence)
            ) {
