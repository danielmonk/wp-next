...
if (!class_exists('mycred_Open_Badge')):
    class mycred_Open_Badge
    {
        ...
        {
            register_rest_route('open-badge', '/assertion/', [
                'methods' => 'GET',
                'callback' => [$this, 'get_assertion_data'],
                'permission_callback' => '__return_true',
            ]);

            register_rest_route('open-badge', '/info/', [
                'methods' => 'GET',
                'callback' => [$this, 'get_badge_info'],
                'permission_callback' => '__return_true',
            ]);

            register_rest_route('open-badge', '/issuer/', [
                'methods' => 'GET',
                'callback' => [$this, 'get_issuer_data'],
                'permission_callback' => '__return_true',
            ]);
        }
        ...
        function bake_users_image($user_id, $badge_id)
        {
            ...
            if (
                !file_exists($user_badge_directory) &&
                !is_dir($user_badge_directory)
            ) {
            ...
            $identity =
                'sha256$' .
                hash('sha256', $user_info->user_email . $this->salt);

            $json = $this->get_image_json_data(
                $user_id,
                $badge_id,
                $identity,
                $badge->title,
            ...
            $this->bake_image(
                ...
                $json,
                $filename,
                $user_badge_directory
            );

            $user_image_url =
                trailingslashit($baseurl . $folderName) . $user_id;

            $json['user_image'] = trailingslashit($user_image_url) . $filename;

            mycred_update_user_meta(
                $user_id,
                "open-badge-{$badge_id}-{$user_id}",
                '',
                $json
            );
        }
                ...
                if ($user_info && $badge) {
                    ...
                    if ($badge->user_has_badge($user_id)) {
                        $identity_id =
                            ...
                            hash(
                                'sha256',
                                $user_info->user_email . $this->salt
                            );

                        $issuedOn = mycred_get_user_meta(
                            $user_id,
                        ...
                        $assertion_data = [
                            ...
                            'evidence' => $this->get_evidence_url(
                                $user_id,
                                $badge_id
                            ),
                        ];
        ...
        public function get_image_json_data(
            $user_id,
            $badge_id,
            $identity,
            $badge_title,
        ...
        public function bake_image(
            ...
            $png =
                substr($png, 0, $length - 12) .
                $final .
                substr($png, $length - 12, 12);
        ...
        {
            $assertion_url = add_query_arg(
                'uid',
                $user_id,
                $this->get_endpoint_url('assertion')
            );
        ...
        {
            $badge_info_url = add_query_arg(
                'bid',
                $badge_id,
                $this->get_endpoint_url('info')
            );
        ...
        public function get_evidence_url($user_id, $badge_id)
        {
            ...
            $evidence_url = add_query_arg(
                'uid',
                $user_id,
                mycred_get_permalink($evidence_page_id)
            );
