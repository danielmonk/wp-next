...
if (!function_exists('mycred_render_badges')):
    function mycred_render_badges($atts, $template = '')
    {
        ...
        if ($template == '') {
            $template =
                '<div class="the-badge row"><div class="col-xs-12"><h3 class="badge-title">%badge_title%</h3><div class="badge-requirements">%requirements%</div><div class="users-with-badge">%count%</div><div class="badge-images">%default_image% %main_image%</div></div></div>';
        }
        ...
        if (!empty($all_badges)) {
            foreach ($all_badges as $badge_id) {
                ...
                $row = str_replace(
                    '%requirements%',
                    mycred_display_badge_requirements($badge_id),
                    $row
                );
    ...
    function mycred_render_badges_list($atts = '')
    {
        ...
        if ($achievement_tabs == 1) {
            ...
            foreach ($categories as $category) {
                ...
                if ($badges_count > 0) {
                    $badges_list_tabs[$category_id] =
                        '<li data-id="' .
                        $category_id .
                        '" ' .
                        ($counter == 1 ? 'class="active"' : '') .
                        '>';
                    $badges_list_tabs[$category_id] .=
                        $category_name .
                        '<span class="mycred-badge-count">' .
                        $badges_count .
                        '</span>';
                    ...
                    $badges_list_panels[$category_id] =
                        '<div data-id="' .
                        $category_id .
                        '" class="mycred-badges-list-panel ' .
                        ($counter == 1 ? 'active' : '') .
                        '">';
                    ...
                    foreach ($category_badges as $badge) {
                        ...
                        $has_earned = $badge_object->user_has_badge($user_id)
                            ? 'earned'
                            : 'not-earned';

                        $badges_list_panels[$category_id] .=
                            '<div class="mycred-badges-list-item ' .
                            $has_earned .
                            '" data-url="' .
                            mycred_get_permalink($badge_id) .
                            '">';
            ...
            if ($category_count > 0) { ?>
                ...
                <div class="mycred-badges-list">
                    ...
                    <div class="mycred-badges-list-panels">
                        <?php foreach ($badges_list_panels as $id => $element) {
                            echo $element;
                        } ?>
                    </div>
                </div>
            <?php } else {echo 'First Create Achievements Containing Badges';} ?>
        ...
        <?php
            ...
            while ($query->have_posts()):
                ...
                ?>
                <div class="mycred-badges-list-item <?php echo $has_earned; ?>" data-url="<?php echo mycred_get_permalink(
    $badge_id
); ?>">
                    <?php if ($image_url): ?>
                    ...
                    <div class="mycred-left">
                        ...
                        <?php if ($category_count > 0) {
                            foreach ($categories as $category) {
                                if ($category != '') {
                                    echo '<sup class="mycred-sup-category">' .
                                        $category .
                                        '</sup>';
                                }
                            }
    ...
    function mycred_render_badge_evidence($atts = '')
    {
        ...
        if (isset($_GET['uid']) && isset($_GET['bid'])) {
            ...
            if ($user_info && $badge && $badge->open_badge) {
                $issued_on = mycred_get_user_meta(
                    $user_id,
                ...
                $content =
                    '<div class="mycred-evidence-page">
                                <div class="mycred-left">
                                    <img src="' .
                    $badge->get_earned_image($user_id) .
                    '" alt="">
                                </div>
                                <div class="mycred-left intro">
                                    <h4 class="mycred-remove-margin">' .
                    $badge->title .
                    '</h4>
