    ...
    class myCRED_Badge extends myCRED_Object
    {
        ...
        function __construct($object = null, $level_id = null)
        {
            ...
            if (
                ...
                $object->post_type == MYCRED_BADGE_KEY
            ) {
                $this->post_id = $object->ID;
            } elseif (
                is_numeric($object) &&
                mycred_get_post_type($object) == MYCRED_BADGE_KEY
            ) {
        ...
        protected function populate($object = null, $level_id = null)
        {
            ...
            $this->title = isset($object->post_title)
                ? $object->post_title
                : mycred_get_the_title($this->post_id);
            $this->earnedby = isset($object->earnedby)
                ? $object->earnedby
                : $this->get_user_count($level_id);
            $this->levels = isset($object->levels)
                ? $object->levels
                : mycred_get_badge_levels($this->post_id);
            ...
            if (!empty($this->levels)) {
                ...
                if (!empty($this->levels[0]['requires'])) {
                    foreach (
                        $this->levels[0]['requires']
                        as $requirement_id => $req
                    ) {
                        if (
                            $req['reference'] != '' &&
                            !in_array($req['reference'], $this->references)
                        ) {
                        ...
                        if (
                            $req['type'] != '' &&
                            !in_array($req['type'], $this->point_types)
                        ) {
            ...
            if (
                absint(
                    mycred_get_post_meta($this->post_id, 'manual_badge', true)
                ) === 1
            ) {
                $this->manual = true;
            }
            ...
            if (
                absint(
                    mycred_get_post_meta($this->post_id, 'open_badge', true)
                ) === 1
            ) {
            ...
            $this->congratulation_msg = mycred_get_post_meta(
                $this->post_id,
                'congratulation_msg',
                true
            );
        ...
        public function get_user_count($level_id = null)
        {
            ...
            $count = mycred_get_post_meta(
                $this->post_id,
                'total-users-with-badge',
                true
            );
            if ($count == '' || $level_id !== null) {
                ...
                $level_filter =
                    $level_id !== null && is_numeric($level_id)
                        ? $wpdb->prepare("AND meta_value = %s", $level_id)
                        : '';

                $count = $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT COUNT( DISTINCT user_id ) FROM {$wpdb->usermeta} WHERE meta_key = %s {$level_filter};",
        ...
        public function get_level($level_id = false)
        {
            if (
                $level_id === false ||
                empty($this->levels) ||
                !array_key_exists($level_id, $this->levels)
            ) {
                return false;
            }
        ...
        public function get_users_current_level($user_id = false)
        {
            ...
            if (
                mycred_is_current_account($user_id) &&
                isset($mycred_current_account->badge_ids) &&
                !empty($mycred_current_account->badge_ids) &&
                array_key_exists(
                    $this->post_id,
                    $mycred_current_account->badge_ids
                )
            ) {
                return absint(
                    $mycred_current_account->badge_ids[$this->post_id]
                );
            }

            $current_level = mycred_get_user_meta(
                $user_id,
                $this->user_meta_key,
                '',
                true
            );
            $current_level = !empty($current_level)
                ? absint($current_level)
                : false;
        ...
        {
            if ($user_id === false || empty($this->levels)) {
                return false;
            }
            ...
            if (!empty($base_requirements)) {
                foreach (
                    ...
                    $query = $wpdb->get_var(
                        $wpdb->prepare(
                            "SELECT {$having} FROM {$mycred_log_table} WHERE ctype = %s AND ref = %s AND user_id = %d;",
                            $requirement['type'],
        ...
        public function assign($user_id = false, $level_id = 0)
        {
            ...
            if ($this->user_has_badge($user_id)) {
                ...
                if ($this->current_level == $level_id) {
                    return true;
                }
        ...
        public function payout_reward($previous_level = -1, $new_level = 0)
        {
            ...
            if ($previous_level < 0 && $new_level == 0) {
                ...
                if (
                    ...
                    $exec = apply_filters(
                        'customize_mycred_badge_condition',
                        true,
                        $this->post_id,
                        $this->user_id,
                        $reward['type']
        ...
        {
            if ($this->post_id === false || empty($this->levels)) {
                return false;
            }
            ...
            if (!empty($this->levels[0]['requires'])) {
                ...
                if (count($this->levels[0]['requires']) == 1) {
                    ...
                    $results = $wpdb->get_results(
                        $wpdb->prepare(
                            "SELECT DISTINCT user_id, {$having} as total FROM {$mycred_log_table} WHERE ctype = %s AND ref = %s GROUP BY user_id HAVING {$having} >= %f;",
                            $requirement['type'],
                            $requirement['reference'],
                            $requirement['amount']
                        )
                        ...
                        if (!empty($level_setup['requires'])) {
                            foreach (
                                ...
                                $level_user_ids[
                                    ...
                                    $wpdb->prepare(
                                        "SELECT DISTINCT user_id FROM {$mycred_log_table} WHERE ctype = %s AND ref = %s GROUP BY user_id HAVING {$having} >= %f;",
                                        $requirement['type'],
                                        $requirement['reference'],
                                        $requirement['amount']
                                    )
        ...
        public function divest_all()
        {
            ...
            $count = $wpdb->delete(
                $wpdb->usermeta,
                ['meta_key' => mycred_get_meta_key($this->user_meta_key)],
                ['%s']
            );
        ...
        public function get_image($image = null)
        {
            ...
            $image_width =
                $this->image_width !== false
                    ? ' width="' . esc_attr($this->image_width) . '"'
                    : '';
            $image_height =
                $this->image_height !== false
                    ? ' height="' . esc_attr($this->image_height) . '"'
                    : '';
        ...
        public function get_image_url($image = null)
        {
            ...
            if ($image === 'main') {
                $image_identification = mycred_get_post_meta(
                    $this->post_id,
                    'main_image',
                    true
                );
                ...
                $image !== null &&
                is_numeric($image) &&
                isset($this->levels[$image]['attachment_id'])
            ) {
                $image_identification = $this->levels[$image]['image_url'];
                if ($this->levels[$image]['attachment_id'] > 0) {
                    $image_identification =
                        $this->levels[$image]['attachment_id'];
                }
            }

            if (
                $image_identification === false ||
                strlen($image_identification) == 0
            ) {
                return false;
            }
            ...
            if (
                is_numeric($image_identification) &&
                strpos('://', (string) $image_identification) === false
            ) {
                $image_url = mycred_get_attachment_url($image_identification);
            }

            return apply_filters(
                'mycred_badge_image_url',
                $image_url,
                $image,
                $this
            );
        ...
        public function get_earned_image($user_id)
        {
            ...
            if ($this->open_badge && $this->user_has_badge($user_id)) {
                ...
                $folderName = apply_filters(
                    'mycred_open_badge_folder',
                    'open_badges'
                );
                ...
                $image_url =
                    trailingslashit($baseurl . $folderName) . $filename;
            }
