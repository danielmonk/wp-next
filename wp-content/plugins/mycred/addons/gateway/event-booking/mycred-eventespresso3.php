...
if (!class_exists('myCRED_Espresso_Gateway')):
    class myCRED_Espresso_Gateway
    {
        ...
        function __construct()
        {
            $defaults = [
                ...
                'messages' => [
                    ...
                    'insolvent' =>
                        'Unfortunately you do not have enough %plural% to pay for this event.',
                    'visitors' =>
                        'Payments using %_plural% is only available for registered members.',
                ],
            ];
        ...
        {
            add_action('init', [$this, 'gateway_setup']);
        }
        ...
        public function gateway_active($id = 'mycred')
        {
            ...
            if (!isset($active_gateways) || !is_array($active_gateways)) {
                $active_gateways = get_option(
                    'event_espresso_active_gateways',
                    []
                );
            }
        ...
        public function gateway_setup()
        {
            ...
            if (
                isset($_REQUEST['mycred-gateway-action']) &&
                isset($_REQUEST['mycred-gateway-token'])
            ) {
                $this->update_settings();
            }

            add_filter('action_hook_espresso_gateway_formal_name', [
                $this,
                'formal_name',
            ]);
            ...
            if ($this->gateway_active()) {
                ...
                add_action(
                    ...
                    'espresso_display_onsite_payment_header'
                );
                add_action(
                    'action_hook_espresso_display_onsite_payment_footer',
                    'espresso_display_onsite_payment_footer'
                );
            }
        }
        ...
        public function is_payment()
        {
            if (
                ...
                $_REQUEST['payment_type'] == 'mycred' &&
                (isset($_REQUEST['token']) &&
                    wp_verify_nonce($_REQUEST['token'], 'pay-with-mycred'))
            ) {
                return true;
            }
        ...
        {
            $gateway_payment_types['mycred'] =
                $this->prefs['labels']['payment'];
        ...
        public function process_payment($payment_data)
        {
            ...
            if (isset($event_meta['mycred_no_sale'])) {
                return;
            }
            ...
            $payment_data['txn_details'] = $this->core->template_tags_general(
                $entry
            );

            // Let others play
            do_action(
                'mycred_espresso_processed',
                $payment_data,
                $this->prefs,
                $this->core
            );
...
<div id="mycred-payment-option-dv" class="payment-option-dv">
	...
	<?php if ($member && $solvent): ?>
		...
		<p><?php echo $this->core->template_tags_general(
      $this->prefs['messages']['solvent']
  ); ?></p>

		<?php endif; ?>

		<div class="event-display-boxes">
			<h4 id="mycred_title" class="payment_type_title section-heading"><?php echo $this
       ->prefs['labels']['payment']; ?></h4>
			<table style="width:100%;">
				...
				<tr>
					...
					<td class="amount"><?php echo $this->core->format_creds(
         $after_purchase
     ); ?></td>
				</tr>
			</table>
       ...
       $finalize_link
   ); ?>" class="button button-large button-primary" style="float:right;"><?php echo $this
    ->prefs['labels']['button']; ?></a></p>
		</div>
       ...
       ->prefs['labels']['payment']; ?></h4>
			<table style="width:100%;">
				...
				<tr class="after-purchase">
					<td class="info"><?php _e('Balance After Purchase', 'mycred'); ?></td>
					<td class="amount" style="color:red;"><?php echo $this->core->format_creds(
         $after_purchase
     ); ?></td>
				</tr>
			</table>
...
<?php
        ...
        public function gateway_settings_page()
        {
            ...
            if (!empty($_REQUEST['activate_mycred_payment'])) {
                $active_gateways['mycred'] =
                    myCRED_GATE_CART_DIR . 'mycred-eventespresso3.php';
                update_option(
                    'event_espresso_active_gateways',
                    $active_gateways
                );
            ...
            $activate_url = admin_url(
                'admin.php?page=payment_gateways&activate_mycred_payment=true'
            );
            ...
            $deactivate_url = admin_url(
                'admin.php?page=payment_gateways&deactivate_mycred_payment=true'
            );
            $deactivate_text = sprintf(
                __('Deactivate %s', 'mycred'),
                $this->label
            );
...
if (array_key_exists('mycred', $active_gateways)) {
    echo '<li id="deactivate_check" style="width:30%;" onclick="location.href=\'' .
        $deactivate_url .
        '\';" class="red_alert pointer"><strong>' .
        $deactivate_text .
        '</strong></li>';
    $this->gateway_settings();
} else {
    echo '<li id="activate_check" style="width:30%;" onclick="location.href=\'' .
        $activate_url .
        '\';" class="green_alert pointer"><strong>' .
        $activate_text .
        '</strong></li>';
}
echo '</ul>';?>
...
<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>#mycred-gate">
	...
	<table width="99%" border="0" cellspacing="5" cellpadding="5">
		<tr>
			<td valign="top">
				...
				<ul>
					<li>
						<label for="mycred-prefs-gateway-labels"><?php _e(
          'Gateway Title',
          'mycred'
      ); ?></label>
						<input type="text" name="mycred_prefs[labels][gateway]" id="mycred-prefs-gateway-labels" size="30" value="<?php echo $this
          ->prefs['labels']['gateway']; ?>" /><br />
						<span class="description"><?php _e(
          'Title to show on Payment page',
          'mycred'
      ); ?>.</span>
					</li>
					<li>
						<label for="mycred-prefs-payment-labels"><?php _e(
          'Payment Type',
          'mycred'
      ); ?></label>
						<input type="text" name="mycred_prefs[labels][payment]" id="mycred-prefs-payment-labels" size="30" value="<?php echo $this
          ->prefs['labels']['payment']; ?>" /><br />
						<span class="description"><?php _e(
          'Title to show on receipts and logs',
          'mycred'
      ); ?>.</span>
					</li>
					<li>
						<label for="mycred-prefs-button-labels"><?php _e(
          'Button Label',
          'mycred'
      ); ?></label>
						<input type="text" name="mycred_prefs[labels][button]" id="mycred-prefs-button-labels" size="30" value="<?php echo $this
          ->prefs['labels']['button']; ?>" /><br />
				...
				<ul>
					<li>
						<label for="mycred-prefs-payment-type"><?php _e(
          'Point Type',
          'mycred'
      ); ?></label>
						<?php mycred_types_select_from_dropdown(
          'mycred_prefs[type]',
          'mycred-prefs-payment-type',
          $this->prefs['type']
      ); ?>
				...
				<ul>
					<li id="mycred-event-exchange-box">
						<label for="mycred-prefs-price-x-rate"><?php _e(
          'Exchange Rate',
          'mycred'
      ); ?></label>
						<input type="text" name="mycred_prefs[rate]" id="mycred-prefs-price-x-rate" size="30" value="<?php echo $this
          ->prefs['rate']; ?>" /><br />
					...
					<li>
						...
						<ol>
							<li><?php _e(
           'You can disable purchases using this gateway by adding a custom Event Meta: <code>mycred_no_sale</code>',
           'mycred'
       ); ?></li>
					...
					<li id="mycred-event-profit-sharing">
						<label for="mycred-prefs-profit-share"><?php _e(
          'Profit Sharing',
          'mycred'
      ); ?></label>
						<input type="text" name="mycred_prefs[share]" id="mycred-prefs-profit-share" size="5" value="<?php echo $this
          ->prefs['share']; ?>" /> %<br />
						<span class="description"><?php _e(
          ...
          'mycred'
				...
				<ul>
					<li>
						...
						<input type="text" name="mycred_prefs[log]" id="mycred-prefs-log" size="30" value="<?php echo $this
          ->prefs['log']; ?>" /><br />
          ...
          'general',
			...
			<td valign="top">
				...
				<ul>
					<li>
						<label for="mycred-prefs-message-solvent"><?php _e(
          'Solvent users',
          'mycred'
      ); ?></label>
						<textarea name="mycred_prefs[messages][solvent]" id="mycred-prefs-message-solvent" style="width: 90%; max-width: 90%; min-height: 90px;"><?php echo stripslashes(
          $this->prefs['messages']['solvent']
      ); ?></textarea><br />
						<span class="description"><?php _e(
          'Message to show users on the payment page before they are charged. Leave empty to hide.',
          'mycred'
    ...
    'general',
					...
					<li>
						<label for="mycred-prefs-message-insolvent"><?php _e(
          'Insolvent users',
          'mycred'
      ); ?></label>
						<textarea name="mycred_prefs[messages][insolvent]" id="mycred-prefs-message-solvent" style="width: 90%; max-width: 90%; min-height: 90px;"><?php echo stripslashes(
          $this->prefs['messages']['insolvent']
      ); ?></textarea><br />
						<span class="description"><?php _e(
          'Message to show users who do not have enough points to pay.',
          'mycred'
      ); ?><br /><?php echo $this->core->available_template_tags([
    'general',
]); ?></span>
          ...
          'mycred'
						...
						<textarea name="mycred_prefs[messages][visitors]" id="mycred-prefs-message-visitors" style="width: 90%; max-width: 90%; min-height: 90px;"><?php echo stripslashes(
          $this->prefs['messages']['visitors']
      ); ?></textarea><br />
						<span class="description"><?php _e(
          'Message to show visitors (users not logged in) on the payment page.',
          'mycred'
      ); ?><br /><?php echo $this->core->available_template_tags([
    'general',
]); ?></span>
	...
	<input type="hidden" name="mycred-gateway-token" value="<?php echo wp_create_nonce(
     'mycred-espresso-update'
 ); ?>" />
	<p><input class="button-primary" type="submit" name="Submit" value="<?php _e(
     'Update Settings',
     'mycred'
 ); ?>" /></p>
</form>
<?php
        ...
        {
            // Apply Whitelabeling
            $this->label = mycred_label(); // Security
            if (
                !wp_verify_nonce(
                    $_REQUEST['mycred-gateway-token'],
                    'mycred-espresso-update'
                )
            ) {
                return;
            ...
            $new_settings['labels']['gateway'] = strip_tags(
                $post['labels']['gateway'],
                '<strong><em><span>'
            );
            $new_settings['labels']['payment'] = strip_tags(
                $post['labels']['payment'],
                '<strong><em><span>'
            );
            ...
            $new_settings['type'] = sanitize_text_field($post['type']); // Exchange Rate
            $new_settings['rate'] = sanitize_text_field($post['rate']); // Profit Share
            $new_settings['share'] = abs($post['share']); // Log
            $new_settings['log'] = sanitize_text_field($post['log']); // Messages
            $new_settings['messages']['solvent'] = sanitize_text_field(
                stripslashes($post['messages']['solvent'])
            );
            $new_settings['messages']['insolvent'] = sanitize_text_field(
                stripslashes($post['messages']['insolvent'])
            );
            $new_settings['messages']['visitors'] = sanitize_text_field(
                stripslashes($post['messages']['visitors'])
            ); // Let others play
            $new_settings = apply_filters(
                'mycred_espresso_save_pref',
                $new_settings
            ); // Save new settings
            $current = $this->prefs;
            $this->prefs = mycred_apply_defaults($current, $new_settings);
            update_option('mycred_espresso_gateway_prefs', $this->prefs); // Flag update
            $this->update = true;
        }
    }
endif;
