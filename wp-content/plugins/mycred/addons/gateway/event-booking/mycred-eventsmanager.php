...
if (!class_exists('myCRED_Events_Manager_Gateway') && defined('EM_VERSION')):
    class myCRED_Events_Manager_Gateway
    {
        ...
        {
            // Settings
            add_action('em_options_page_footer_bookings', [
                $this,
                'settings_page',
            ]);
            add_action('em_options_save', [$this, 'save_settings']);
            ...
            add_action('em_my_bookings_booking_loop', [$this, 'payment_box']);
            add_action('em_template_my_bookings_footer', [
                $this,
                'insert_scripting',
            ]);

            // Ajax Payments
            add_action('wp_ajax_mycred-pay-em-booking', [
                $this,
                'process_payment',
            ]);
        ...
        public function process_payment()
        {
            ...
            elseif (!$this->has_paid($booking)) {
                ...
                if (
                    get_option('dbem_bookings_approval') == 1 &&
                    apply_filters(
                ...
                if ($this->prefs['share'] != 0) {
                    $event_post = mycred_get_post(
                        (int) $booking->event->post_id
                    );
...
<?php
        ...
        public function add_pay_button($cancel_link = '', $EM_Booking)
        {
            ...
            if ($this->can_pay($EM_Booking) && !$this->has_paid($EM_Booking)) {
                ...
                $cancel_link .=
                    '<a href="javascript:void(0)" class="mycred-show-pay" data-booking="' .
                    $EM_Booking->booking_id .
                    '">' .
                    $this->prefs['labels']['link'] .
                    '</a>';
...
<tr id="mycred-payment-<?php echo $EM_Booking->booking_id; ?>" style="display: none;">
	<td colspan="5">
		<h5><?php echo $this->core->template_tags_general(
      $this->prefs['labels']['header']
  ); ?></h5>
        ...
        $balance - $price
			...
			<tr>
				<td colspan="2" class="action" style="text-align: right;">
					...
					<input type="hidden" name="mycred-booking-<?php echo $EM_Booking->booking_id; ?>-token" value="<?php echo wp_create_nonce(
    'mycred-pay-booking'
); ?>" />
					<input type="button" class="button button-primary button-medium mycred-pay" value="<?php echo $this
         ->prefs['labels']['button']; ?>" />
				</td>
			</tr>
		</table>
...
<div class="postbox" id="em-opt-mycred">
	...
	<h3><span><?php echo sprintf(
     __('%s Payments', 'mycred'),
     $this->label
 ); ?></span></h3>
	<div class="inside">
     ...
     __(
         ...
         $this->prefs['type']
			...
			<tr>
				...
				<td>
					<input name="mycred_gateway[refund]" type="text" id="mycred-gateway-log-refund" value="<?php echo esc_attr(
         $this->prefs['refund']
     ); ?>" size="5" /> %<br />
					<span class="description"><?php _e(
         'The percentage of the paid amount to refund if a booking gets cancelled. Use zero for no refunds. No refunds are given to "Rejected" bookings.',
         'mycred'
     ); ?></span>
				</td>
			</tr>
			<tr>
				...
				<td>
					<input name="mycred_gateway[share]" type="text" id="mycred-gateway-profit-sharing" value="<?php echo esc_attr(
         $this->prefs['share']
     ); ?>" size="5" /> %<br />
					<span class="description"><?php _e(
		...
		<table class="form-table" id="mycred-exchange-rate" style="<?php echo $box; ?>">
			<tr>
				...
				<td>
					<input name="mycred_gateway[rate]" type="text" id="mycred-gateway-rate" size="6" value="<?php echo esc_attr(
         $this->prefs['rate']
     ); ?>" /><br />
		...
		<table class="form-table">
			<tr>
				...
				<td>
					<input name="mycred_gateway[log][purchase]" type="text" id="mycred-gateway-log-purchase" style="width: 95%;" value="<?php echo esc_attr(
         $this->prefs['log']['purchase']
     ); ?>" size="45" /><br />
         ...
         'post',
			...
			<tr>
				<th scope="row"><?php _e('Refunds', 'mycred'); ?></th>
				<td>
					<input name="mycred_gateway[log][refund]" type="text" id="mycred-gateway-log-refund" style="width: 95%;" value="<?php echo esc_attr(
         $this->prefs['log']['refund']
     ); ?>" size="45" /><br />
					<span class="description"><?php echo $this->core->available_template_tags([
         'general',
		...
		<table class="form-table">
			<tr valign="top">
				...
				<td>
					<input name="mycred_gateway[labels][link]" type="text" id="mycred-gateway-labels-link" style="width: 95%" value="<?php echo esc_attr(
         $this->prefs['labels']['link']
     ); ?>" size="45" /><br />
					<span class="description"><?php _e(
         ...
         'mycred'
			...
			<tr valign="top">
				...
				<td>
					<input name="mycred_gateway[labels][header]" type="text" id="mycred-gateway-labels-header" style="width: 95%" value="<?php echo esc_attr(
         $this->prefs['labels']['header']
     ); ?>" size="45" /><br />
					<span class="description"><?php _e(
         ...
         'mycred'
			...
			<tr valign="top">
				...
				<td>
					<input name="mycred_gateway[labels][button]" type="text" id="mycred-gateway-labels-button" style="width: 95%" value="<?php echo esc_attr(
         $this->prefs['labels']['button']
     ); ?>" size="45" /><br />
					<span class="description"><?php _e(
		...
		<table class='form-table'>
			<tr valign="top">
				...
				<td>
					<input type="text" name="mycred_gateway[messages][success]" id="mycred-gateway-messages-success" style="width: 95%;" value="<?php echo esc_attr(
    ...
    'general',
			...
			<tr valign="top">
				...
				<td>
					<input type="text" name="mycred_gateway[messages][error]" id="mycred-gateway-messages-error" style="width: 95%;" value="<?php echo esc_attr(
...
<?php
        ...
        public function save_settings()
        {
            ...
            // Logs
            $new_settings['log']['purchase'] = sanitize_text_field(
                stripslashes($data['log']['purchase'])
            );
            $new_settings['log']['refund'] = sanitize_text_field(
