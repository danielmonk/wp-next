    ...
    class EM_Gateway_myCRED extends EM_Gateway
    {
        ...
        function __construct()
        {
            ...
            $defaults = [
                ...
                'log' => [
                    'purchase' => __(
                        'Payment for tickets to %link_with_title%',
                        'mycred'
                    ),
                    'refund' => __(
                        'Ticket refund for %link_with_title%',
                        'mycred'
                    ),
                ],
                'refund' => 0,
                'labels' => [
                    'header' => __(
                        'Pay using your %_plural% balance',
                        'mycred'
                    ),
                ...
                'messages' => [
                    ...
                    'error' => __(
                        "I'm sorry but you can not pay for these tickets using %_plural%",
                        'mycred'
                    ),
                ],
            ];
            ...
            // Currency
            add_filter('em_get_currencies', [$this, 'add_currency']);
            ...
            add_filter(
                'em_booking_form_tickets_cols',
                [$this, 'ticket_columns'],
                10,
                2
            );
            add_action(
                'em_booking_form_tickets_col_mycred',
                [$this, 'ticket_col'],
                10,
                2
            );
            add_filter(
                'em_bookings_table_cols_col_action',
                [$this, 'bookings_table_actions'],
                10,
                2
            );
            ...
            if ($this->prefs['refund'] != 0) {
                add_filter('em_booking_set_status', [$this, 'refunds'], 10, 2);
            }
        }
        ...
        public function add_currency($currencies)
        {
            ...
            if (empty($this->core->before) && !empty($this->core->after)) {
                ...
                !empty($this->core->before) &&
                empty($this->core->after)
            ) {
...
<td class="em-bookings-ticket-table-points"><?php echo $this->core->format_creds(
    $price
); ?></td>
<?php
        }
        ...
        function booking_form_button()
        {
            ...
            if (
                $this->core->get_users_cred($user_id, $this->mycred_type) <=
                $this->core->format_number(0)
            ) {
                return;
            }

            $button = get_option(
                'em_' . $this->gateway . '_button',
                $this->title
            );
            ob_start();
...
<?php }
        ...
        function booking_add($EM_Event, $EM_Booking, $post_validation = false)
        {
            ...
            if ($post_validation && empty($EM_Booking->booking_id)) {
                if (
                    get_option('dbem_multiple_bookings') &&
                    get_class($EM_Booking) == 'EM_Multiple_Booking'
                ) {
                    add_filter(
                        'em_multiple_booking_save',
                ...
                if ($EM_Booking->get_price() > 0) {
                    ...
                    if ($this->core->exclude_user($EM_Booking->person->ID)) {
                        $EM_Booking->add_error(
                            __('You can not pay using this gateway.', 'mycred')
                        );
                        $ok = false;
                    }
                    ...
                    elseif (!$this->has_paid($EM_Booking)) {
                        // Price
                        $price = $this->core->number(
                            $EM_Booking->booking_price
                        );
                        if (!$this->single_currency()) {
                        ...
                        $this->record_transaction(
                            $EM_Booking,
                            $EM_Booking->get_price(false, false, true),
                            get_option('dbem_bookings_currency'),
                            current_time('mysql'),
                            $transaction_id,
                            'Completed',
                            ''
                    ...
                    // Error in payment - delete booking
                        ...
                        if (
                            !is_user_logged_in() &&
                            get_option('dbem_bookings_anonymous') &&
                            !get_option('dbem_bookings_registration_disable') &&
                            !empty($EM_Booking->person_id)
                        ) {
                            $EM_Person = $EM_Booking->get_person();
                            if (
                                strtotime($EM_Person->data->user_registered) >=
                                $this->registered_timer
                            ) {
                                if (is_multisite()) {
            ...
            if (
                ...
                if ($this->has_paid($EM_Booking)) {
                    ...
                    if ($this->single_currency()) {
                        ...
                        $price = $this->core->number(
                            $this->prefs['rate'] * $EM_Booking->booking_price
                        );
                    }
                    ...
                    $this->core->add_creds(
                        ...
                        $EM_Booking->event->post_id,
                        ['ref_type' => 'post', 'bid' => (int) $booking_id],
                        $this->mycred_type
                    );
                }
            }
        ...
        function bookings_table_actions($actions, $EM_Booking)
        {
            if (
                ...
                $this->uses_gateway($EM_Booking)
            ) {
                return [
                    'reject' =>
                        '<a class="em-bookings-reject" href="' .
                        em_add_get_params($url, [
                        ...
                        '</a>',
                    'delete' =>
                        '<span class="trash"><a class="em-bookings-delete" href="' .
                        em_add_get_params($url, [
        ...
        public function can_pay($EM_Booking)
        {
            ...
            $balance = $this->core->get_users_cred(
                $EM_Booking->person->ID,
                $this->mycred_type
            );
            if ($balance <= $this->core->zero()) {
                return false;
            }

            $price = $this->core->number($EM_Booking->booking_price);
            if ($price == $this->core->zero()) {
                return true;
            }
            if (!$this->single_currency()) {
                $exchange_rate = $this->prefs['rate'];
            ...
            if ($balance - $price < $this->core->zero()) {
                return false;
            }
        ...
        {
            global $page, $action;
            $gateway_link = admin_url(
                'edit.php?post_type=' .
                    EM_POST_TYPE_EVENT .
                    '&page=events-manager-options#bookings'
            );
...
<table class="form-table">
	...
	<tr>
		...
		<td>
			<input type="radio" name="mycred_gateway[setup]" id="mycred-gateway-setup-off" value="off"<?php checked(
       $this->prefs['setup'],
       'off'
   ); ?> /> <label for="mycred-gateway-setup-off"><?php echo $this->core->template_tags_general(
     __('Disabled - Users CAN NOT pay for tickets using %plural%.', 'mycred')
 ); ?></label><br />
			<input type="radio" name="mycred_gateway[setup]" id="mycred-gateway-setup-single" value="single"<?php checked(
       $this->prefs['setup'],
       'single'
   ); ?> /> <label for="mycred-gateway-setup-single"><?php echo $this->core->template_tags_general(
     __('Single - Users can ONLY pay for tickets using %plural%.', 'mycred')
 ); ?></label><br />
			<input type="radio" name="mycred_gateway[setup]" id="mycred-gateway-setup-multi" value="multi"<?php checked(
       $this->prefs['setup'],
       'multi'
   ); ?> /> <label for="mycred-gateway-setup-multi"><?php echo $this->core->template_tags_general(
     __(
         'Multi - Users can pay for tickets using other gateways or %plural%.',
         'mycred'
     )
 ); ?></label>
		</td>
	...
	<tr>
		...
		<td>
			<input name="mycred_gateway[refund]" type="text" id="mycred-gateway-log-refund" value="<?php echo $this
       ->prefs['refund']; ?>" size="5" /> %<br />
			<span class="description"><?php _e(
       'The percentage of the paid amount to refund if a user cancels their booking. Use zero for no refunds. No refunds are given to "Rejected" bookings!',
       'mycred'
   ); ?></span>
		</td>
	</tr>
	<tr>
		...
		<td>
			<input name="mycred_gateway[share]" type="text" id="mycred-gateway-profit-sharing" value="<?php echo $this
       ->prefs['share']; ?>" size="5" /> %<br />
			<span class="description"><?php _e(
       'Option to share sales with the product owner. Use zero to disable.',
       'mycred'
   ); ?></span>
		</td>
	</tr>
...
<table class="form-table" id="mycred-exchange-rate" style="<?php echo $box; ?>">
	<tr>
		...
		<td>
			<input name="mycred_gateway[rate]" type="text" id="mycred-gateway-rate" size="6" value="<?php echo $this
       ->prefs['rate']; ?>" /><br />
...
<table class="form-table">
	<tr>
		...
		<td>
			<input name="mycred_gateway[log][purchase]" type="text" id="mycred-gateway-log-purchase" style="width: 95%;" value="<?php echo $this
       ->prefs['log']['purchase']; ?>" size="45" /><br />
			<span class="description"><?php echo $this->core->available_template_tags([
       'general',
       'post',
   ]); ?>></span>
		</td>
	...
	<tr>
		<th scope="row"><?php _e('Refunds', 'mycred'); ?></th>
		<td>
			<input name="mycred_gateway[log][refund]" type="text" id="mycred-gateway-log-refund" style="width: 95%;" value="<?php echo $this
       ->prefs['log']['refund']; ?>" size="45" /><br />
			<span class="description"><?php echo $this->core->available_template_tags([
       'general',
       'post',
   ]); ?></span>
...
<table class="form-table">
	<tr valign="top">
		...
		<td>
			<input name="mycred_gateway[labels][link]" type="text" id="mycred-gateway-labels-link" style="width: 95%" value="<?php echo $this
       ->prefs['labels']['link']; ?>" size="45" /><br />
			<span class="description"><?php _e(
       'The payment link shows / hides the payment form under "My Bookings". No HTML allowed.',
       'mycred'
   ); ?></span>
		</td>
	</tr>
	<tr valign="top">
		...
		<td>
			<input name="mycred_gateway[labels][header]" type="text" id="mycred-gateway-labels-header" style="width: 95%" value="<?php echo $this
       ->prefs['labels']['header']; ?>" size="45" /><br />
			<span class="description"><?php _e(
       'Shown on top of the payment form. No HTML allowed.',
       'mycred'
   ); ?></span>
		</td>
	</tr>
	<tr valign="top">
		...
		<td>
			<input name="mycred_gateway[labels][button]" type="text" id="mycred-gateway-labels-button" style="width: 95%" value="<?php echo $this
       ->prefs['labels']['button']; ?>" size="45" /><br />
			<span class="description"><?php _e(
       'The button label for payments. No HTML allowed!',
       'mycred'
   ); ?></span>
		</td>
	</tr>
...
<table class='form-table'>
	<tr valign="top">
		...
		<td>
			<input type="text" name="mycred_gateway[messages][success]" id="mycred-gateway-messages-success" style="width: 95%;" value="<?php echo stripslashes(
       $this->prefs['messages']['success']
   ); ?>" /><br />
			<span class="description"><?php _e(
       'No HTML allowed!',
       'mycred'
   ); ?><br /><?php echo $this->core->available_template_tags([
    'general',
]); ?></span>
		</td>
	</tr>
	<tr valign="top">
		...
		<td>
			<input type="text" name="mycred_gateway[messages][error]" id="mycred-gateway-messages-error" style="width: 95%;" value="<?php echo stripslashes(
       $this->prefs['messages']['error']
   ); ?>" /><br />
			<span class="description"><?php _e(
       'No HTML allowed!',
...
<?php do_action('mycred_em_after_settings', $this);
        ...
        function update()
        {
            parent::update();
            if (
                !isset($_POST['mycred_gateway']) ||
                !is_array($_POST['mycred_gateway'])
            ) {
                return;
            }
            ...
            // Logs
            $new_settings['log']['purchase'] = trim(
                stripslashes($data['log']['purchase'])
            );
            $new_settings['log']['refund'] = trim(
                stripslashes($data['log']['refund'])
            );
            ...
            if ($new_settings['setup'] == 'single') {
                update_option(
                    ...
                    $this->core->format['separators']['decimal']
                );
                update_option(
                    'dbem_bookings_currency_thousands_sep',
                    $this->core->format['separators']['thousand']
                );
            ...
            // Labels
            $new_settings['labels']['link'] = sanitize_text_field(
                stripslashes($data['labels']['link'])
            );
            $new_settings['labels']['header'] = sanitize_text_field(
                stripslashes($data['labels']['header'])
            );
            $new_settings['labels']['button'] = sanitize_text_field(
                stripslashes($data['labels']['button'])
            );

            // Messages
            $new_settings['messages']['success'] = sanitize_text_field(
                stripslashes($data['messages']['success'])
            );
            $new_settings['messages']['error'] = sanitize_text_field(
                stripslashes($data['messages']['error'])
            );
