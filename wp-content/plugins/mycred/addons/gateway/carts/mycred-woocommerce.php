...
if (!function_exists('mycred_init_woo_gateway')):
    function mycred_init_woo_gateway()
    {
        if (
            ...
            class_exists('WC_Gateway_myCRED')
        ) {
            return;
        }
        ...
        class WC_Gateway_myCRED extends WC_Payment_Gateway
        {
            ...
            public function __construct()
            {
                ...
                $this->method_description = __(
                    'Let users pay using points.',
                    'mycred'
                );

                $gateway_suppots = ['products', 'refunds'];

                $this->supports = apply_filters(
                    'mycred_woocommerce_gateway_supports',
                    $gateway_suppots
                );
                ...
                $this->log_template_refund = $this->get_option(
                    'log_template_refund'
                );
                $this->profit_sharing_refund_log = $this->get_option(
                    'profit_sharing_refund_log'
                );
                ...
                $this->profit_sharing_percent = $this->get_option(
                    'profit_sharing_percent'
                );
                $this->profit_sharing_log = $this->get_option(
                    'profit_sharing_log'
                );
                ...
                add_action('woocommerce_thankyou_mycred', [
                    $this,
                    'thankyou_page',
                ]);
            }
            ...
            function init_form_fields()
            {
                ...
                $fields['enabled'] = [
                    ...
                    'type' => 'checkbox',
                    'label' => sprintf(
                        __('Enable %s Payment', 'mycred'),
                        $this->method_title
                    ),
                    'default' => 'no',
                    'description' => __(
                        'Users who are not logged in or excluded from using myCRED will not have access to this gateway!',
                        'mycred'
                    ),
                ];
                $fields['title'] = [
                    ...
                    'type' => 'text',
                    'description' => __(
                        'Title to show for this payment option.',
                        'mycred'
                    ),
                ...
                $fields['description'] = [
                    ...
                    'type' => 'textarea',
                    'default' => $this->mycred->template_tags_general(
                        'Deduct the amount from your %_plural% balance.'
                    ),
                ];
                $fields['log_template'] = [
                    'title' => __('Log Template', 'mycred'),
                    'type' => 'text',
                    'description' => $this->mycred->available_template_tags(
                        ['general'],
                        '%order_id%, %order_link%'
                    ),
                    'default' => 'Payment for Order: #%order_id%',
                ];
                $fields['log_template_refund'] = [
                    ...
                    'default' =>
                        'Payment refund for order #%order_id% Reason: %reason%',
                ];
                ...
                if ($this->use_exchange()) {
                    ...
                    $fields['point_type'] = [
                        ...
                        'type' => 'select',
                        'label' => __(
                            'Select the point type users can use to pay.',
                            'mycred'
                        ),
                    ...
                    $exchange_desc = $this->mycred->template_tags_general(
                        __(
                    ...
                    $fields['show_total'] = [
                        ...
                        'type' => 'select',
                        'label' => $this->mycred->template_tags_general(
                            __('Show the final price in %_plural% .', 'mycred')
                        ),
                        'options' => [
                            ...
                            'all' => __(
                                'Show in Cart and on Checkout Page',
                                'mycred'
                            ),
                        ],
                        'default' => '',
                    ...
                    $fields['total_label'] = [
                        ...
                        'type' => 'text',
                        'default' => $this->mycred->template_tags_general(
                            __('Order Total in %_plural%', 'mycred')
                        ),
                        'desc_tip' => true,
                    ];
                } else {
                ...
                $fields['balance_format'] = [
                    'title' => __('Balance Label', 'mycred'),
                    'type' => 'text',
                    'description' => __(
                        'The label to use when presenting a user their balance on the checkout / cart pages. Leave empty to hide.',
                        'mycred'
                    ),
                    'default' => 'Your Balance',
                    'desc_tip' => false,
                ];
                $fields['profit_sharing_percent'] = [
                ...
                $fields['profit_sharing_log'] = [
                    'title' => __('Log Template', 'mycred'),
                    'type' => 'text',
                    'description' =>
                        __('Log entry template for profit sharing.', 'mycred') .
                        ' ' .
                        $this->mycred->available_template_tags([
                            'general',
                            'post',
                        ]),
                    'default' => 'Sale of %post_title%',
                ];
                $fields['profit_sharing_refund_log'] = [
                    'title' => __('Refund Log Template', 'mycred'),
                    'type' => 'text',
                    'description' =>
                        __(
                            'Log entry template for refunds of profit shares.',
                            'mycred'
                        ) .
                        ' ' .
                        $this->mycred->available_template_tags([
...
<tr valign="top">
	<th scope="row" class="titledesc">
      ...
      $field_key
  ); ?>"><?php echo wp_kses_post($data['title']); ?></label>
		<?php echo $this->get_tooltip_html($data); ?>
	</th>
	<td class="forminp">
		<?php if ($data['type'] == 'currency'):
      $mycred = mycred($currency); ?>
    ...
    $field_key
       ...
       ?>
		<fieldset>
			<legend class="screen-reader-text"><span><?php echo wp_kses_post(
       $data['title']
   ); ?></span></legend>
			<input class="input-text regular-input <?php echo esc_attr(
       $data['class']
   ); ?>" type="<?php echo esc_attr(
    $data['type']
); ?>" name="<?php echo esc_attr($field_key); ?>" id="<?php echo esc_attr(
    $field_key
); ?>" style="<?php echo esc_attr($data['css']); ?>" value="<?php echo esc_attr(
    $this->get_option($key)
...
<?php
            ...
            function process_payment($order_id)
            {
                ...
                if (!is_user_logged_in()) {
                    wc_add_notice(
                        $this->mycred->template_tags_general(
                            __(
                                'You must be logged in to pay with %_plural%',
                                'mycred'
                            )
                        ),
                        'error'
                ...
                if ($this->mycred->exclude_user($user_id)) {
                    wc_add_notice(
                        $this->mycred->template_tags_general(
                            __(
                                'You can not use this gateway. Please try a different payment option.',
                                'mycred'
                            )
                        ),
                        'error'
                ...
                $cost = $order_total;
                if ($this->use_exchange()) {
                    $cost = $this->mycred->number(
                        $order_total / $this->exchange_rate
                    );
                }
                ...
                // Check funds
                if (
                    $this->mycred->get_users_balance(
                        $user_id,
                        $this->mycred_type
                    ) < $cost
                ) {
                    $message = apply_filters(
                        'mycred_woo_error_insufficient_funds',
                        __('Insufficient funds.', 'mycred')
                    );
                ...
                if ($this->profit_sharing_percent > 0) {
                    ...
                    foreach ($items as $item) {
                        ...
                        $percentage = apply_filters(
                            'mycred_woo_profit_share',
                            $this->profit_sharing_percent,
                            $order,
                            $product,
                            $this
                        );
            ...
            public function process_refund(
                ...
                $order_total = version_compare(
                    ...
                    ? $order->get_total()
                    : $order->order_total;
                $user_id = version_compare($woocommerce->version, '3.0', '>=')
                    ? $order->get_user_id()
                    : $order->user_id;
                ...
                $refund = $amount;
                if ($this->use_exchange()) {
                    $refund = $this->mycred->number(
                        $refund / $this->exchange_rate
                    );
                }
                ...
                $order->add_order_note(
                    sprintf(
                        _x(
                            'Refunded %s',
                            '%s = Point amount formatted',
                            'mycred'
                        ),
                        $this->mycred->format_creds($refund)
                    )
                );
                ...
                if ($this->profit_sharing_percent > 0) {
                    ...
                    foreach ($items as $item) {
                        ...
                        // Calculate Share
                        $percentage = apply_filters(
                            'mycred_woo_profit_share_refund',
                            $this->profit_sharing_percent,
                            $order,
                            $product,
                            $this
                        ...
                        $this->mycred->add_creds(
                            ...
                            $product->ID,
                            ['ref_type' => 'post', 'order_id' => $order_id],
                            $this->mycred_type
                        );
                    }
                }
            ...
            {
                echo apply_filters(
                    'mycred_woo_thank_you_message',
                    '<p>' .
                        __(
                            'Your account has successfully been charged.',
                            'mycred'
                        ) .
                        '</p>'
                );
...
if (!function_exists('mycred_woo_available_gateways')):
    function mycred_woo_available_gateways($gateways)
    {
        ...
        // Easy override
        if (
            defined('SHOW_MYCRED_IN_WOOCOMMERCE') &&
            SHOW_MYCRED_IN_WOOCOMMERCE
        ) {
            return $gateways;
        }
        ...
        if (!mycred_point_type_exists($currency) && $currency != 'MYC') {
            $cost = $mycred->number(
                $woocommerce->cart->total /
                    $gateways['mycred']->get_option('exchange_rate')
            );
        }
...
add_filter(
    'mycred_parse_log_entry_woocommerce_payment',
    'mycred_woo_log_entry_payment',
    90,
    2
);
...
add_filter(
    'mycred_parse_log_entry_woocommerce_refund',
    'mycred_woo_log_entry_refunds',
    90,
    2
);
...
add_filter(
    'mycred_parse_log_entry_store_sale_refund',
    'mycred_woo_log_entry_profit_refund',
    90,
    2
);
...
if (!function_exists('mycred_woo_parse_email')):
    ...
    {
        if (
            $email['request']['ref'] == 'woocommerce_payment' &&
...
if (!function_exists('mycred_woo_after_order_total')):
    function mycred_woo_after_order_total()
    {
        ...
        $balance_label = $available_gateways['mycred']->get_option(
            'balance_format'
        );
        ...
        if (
            ...
            if (!mycred_point_type_exists($currency) && $currency != 'MYC') {
                ...
                $cost = $mycred->number(
                    $woocommerce->cart->total /
                        $available_gateways['mycred']->get_option(
                            'exchange_rate'
                        )
                );
                $cost = apply_filters(
                ...
                ?>
<tr class="total">
	<th><strong><?php echo $mycred->template_tags_general(
     $available_gateways['mycred']->get_option('total_label')
 ); ?></strong></th>
	<td>
		<div class="current-balance order-total-in-points">
			<strong class="<?php if ($balance < $cost) {
       echo 'mycred-low-funds';
   } else {
       echo 'mycred-funds';
   } ?>"<?php if ($balance < $cost) {
    echo ' style="color:red;"';
} ?>><?php echo $mycred->format_creds($cost); ?></strong> 
		</div>
	</td>
</tr>
<?php
