...
if (!class_exists('buyCRED_Pending_Payments')):
    class buyCRED_Pending_Payments extends myCRED_Module
    {
        ...
        public function module_init()
        {
            ...
            add_shortcode(
                MYCRED_SLUG . '_buy_pending',
                'mycred_render_pending_purchases'
            );

            add_action('mycred_pre_process_buycred', [
                $this,
                'intercept_cancellations',
            ]);
        ...
        public function intercept_cancellations()
        {
            ...
            if (
                isset($_REQUEST['buycred-cancel']) &&
                isset($_REQUEST['_token']) &&
                wp_verify_nonce(
                    $_REQUEST['_token'],
                    'buycred-cancel-pending-payment'
                )
            ) {
                // Get pending payment object
                $pending_payment_id = sanitize_text_field(
                    $_REQUEST['buycred-cancel']
                );
                ...
                // Redirect
                wp_safe_redirect(
                    remove_query_arg(['buycred-cancel', '_token'])
                );
                exit();
            }
        }
        ...
        public function module_admin_init()
        {
            ...
            add_filter('manage_' . MYCRED_BUY_KEY . '_posts_columns', [
                $this,
                'adjust_column_headers',
            ]);
            add_action(
                'manage_' . MYCRED_BUY_KEY . '_posts_custom_column',
                [$this, 'adjust_column_content'],
                10,
                2
            );
            add_filter('bulk_actions-edit-' . MYCRED_BUY_KEY, [
                $this,
                'bulk_actions',
            ]);
            add_action(
                'save_post_' . MYCRED_BUY_KEY,
            ...
            if (
                isset($_GET['credit']) &&
                isset($_GET['token']) &&
                wp_verify_nonce($_GET['token'], 'buycred-payout-pending')
            ) {
                $pending_id = absint($_GET['credit']);

                if ($this->core->user_is_point_editor()) {
                    $url = remove_query_arg(['credit', 'token']);

                    if (buycred_complete_pending_payment($pending_id)) {
                        $url = add_query_arg(['credited' => 1], $url);
                    } else {
                        $url = add_query_arg(['credited' => 0], $url);
                    }
        ...
        protected function register_pending_payments()
        {
            ...
            register_post_type(
                MYCRED_BUY_KEY,
                apply_filters('mycred_setup_pending_payment', $args)
            );
        }
        ...
        public function parent_file($parent = '')
        {
            ...
            if (
                isset($_GET['post']) &&
                mycred_get_post_type($_GET['post']) == MYCRED_BUY_KEY &&
                isset($_GET['action']) &&
                $_GET['action'] == 'edit'
            ) {
                return MYCRED_SLUG;
        ...
        public function edit_pending_payment_style()
        {
            ...
            add_filter(
                'postbox_classes_buycred_payment_buycred-pending-comments',
                [$this, 'metabox_classes']
            );?>
...
<div class="form">
	...
	<div class="row">
		<div class="col-md-3 col-sm-6">
			<div class="form-group">
				<label for="buycred-pending-payment-point_type"><?php _e(
        'Point Type',
        'mycred'
    ); ?></label>
<?php // Just one point type is set for sale. No need for a dropdown menu
...
<?php foreach ($this->core->buy_creds['types'] as $point_type) {
    echo '<option value="' . $point_type . '"';
    if ($pending_payment->point_type == $point_type) {
        echo ' selected="selected"';
    }
    echo '>' .
        mycred_get_point_type_name($pending_payment->point_type, false) .
        '</option>';
} ?>
				</select>
<?php } ?>
...
<?php foreach (
    ...
    echo '>' . $info['title'] . '</option>';
		...
		<div class="col-md-2 col-sm-6">
			<div class="form-group">
				<label for="buycred-pending-payment-amount"><?php _e(
        'Amount',
        'mycred'
    ); ?></label>
				<input type="text" name="buycred_pending_payment[amount]" id="buycred-pending-payment-amount" class="form-control" value="<?php echo $mycred->number(
		...
		<div class="col-md-2 col-sm-6">
			<div class="form-group">
				...
				<input type="text" name="buycred_pending_payment[cost]" id="buycred-pending-payment-cost" class="form-control" value="<?php echo esc_attr(
		...
		<div class="col-md-2 col-sm-6">
			<div class="form-group">
				<label for="buycred-pending-payment-currency"><?php _e(
        'Currency',
        'mycred'
    ); ?></label>
				<input type="text" name="buycred_pending_payment[currency]" id="buycred-pending-payment-currency" class="form-control" value="<?php echo esc_attr(
...
<?php
        ...
        public function save_pending_payment($post_id, $post)
        {
            if (
                ...
                !isset($_POST['buycred_pending_payment'])
            ) {
                return;
            }
            ...
            if ($changed) {
                ...
                $this->add_comment(
                    $post_id,
                    sprintf(
                        __('Pending payment updated by %s', 'mycred'),
                        $user->user_login
                    )
                );
            }
