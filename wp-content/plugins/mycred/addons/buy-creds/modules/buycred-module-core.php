...
if (!class_exists('myCRED_buyCRED_Module')):
    class myCRED_buyCRED_Module extends myCRED_Module
    {
        ...
        {
            add_filter(
                'mycred_parse_log_entry',
                [$this, 'render_gift_tags'],
                10,
                2
            );

            add_action('mycred_init', [$this, 'module_init'], $this->menu_pos);
            add_action('wp_loaded', [$this, 'module_run']);

            add_action('mycred_register_assets', [$this, 'register_assets']);
            add_action('mycred_front_enqueue_footer', [
                $this,
                'enqueue_footer',
            ]);
            ...
            add_action('mycred_add_menu', [$this, 'add_menu'], $this->menu_pos);
            add_action(
                'mycred_add_menu',
                [$this, 'add_to_menu'],
                $this->menu_pos + 1
        ...
        public function module_init()
        {
            ...
            add_shortcode(
                MYCRED_SLUG . '_buy_form',
                'mycred_render_buy_form_points'
            );
        ...
        public function register_assets()
        {
            wp_register_style(
                'buycred-checkout',
                plugins_url('assets/css/checkout.css', MYCRED_PURCHASE),
                [],
                MYCRED_PURCHASE_VERSION,
                'all'
            );
            wp_register_script(
                'buycred-checkout',
                plugins_url('assets/js/checkout.js', MYCRED_PURCHASE),
                ['jquery'],
                MYCRED_PURCHASE_VERSION,
                'all'
            );
        ...
        public function module_run()
        {
            ...
            if (empty($buycred_instance->active)) {
                return;
            }
            ...
            foreach ($buycred_instance->active as $id => $data) {
                if ($data['external'] === true) {
                    $this->call(
                        'returning',
                        $buycred_instance->active[$id]['callback']
                    );
                }
            }
            ...
            if (
                $buycred_instance->gateway_id !== false &&
                array_key_exists(
                    $buycred_instance->gateway_id,
                    $buycred_instance->active
                )
            ) {
                // Construct Gateway
                $buycred_instance->gateway = buycred_gateway(
                    $buycred_instance->gateway_id
                );
                ...
                if (isset($_REQUEST['mycred_call'])) {
                    ...
                    do_action(
                        'mycred_buycred_process',
                        $buycred_instance->gateway_id,
                        $this->gateway_prefs
                    );
                    do_action(
                        "mycred_buycred_process_{$gateway_id}",
                        $this->gateway_prefs
                    );
                }

                add_action('template_redirect', [$this, 'process_new_request']);
                add_filter('template_include', [$this, 'checkout_page']);
            }
        }
        ...
        public function process_new_request()
        {
            ...
            if ($buycred_instance->checkout) {
                ...
                if ($buycred_instance->gateway->valid_request()) {
                    ...
                    do_action(
                        "mycred_buycred_buy_{$buycred_instance->gateway_id}",
                        $this->gateway_prefs
                    );
                } else {
                        ...
                        if ($buycred_instance->is_ajax) {
                            die(
                                json_encode([
                                    'validationFail' => true,
                                    'errors' =>
                                        $buycred_instance->gateway->errors,
                                ])
                            );
                        }
                    }
        ...
        public function checkout_page($template)
        {
            ...
            if ($buycred_instance->checkout) {
                ...
                $override = mycred_locate_template(
                    'buycred-checkout.php',
                    MYCRED_SLUG,
                    MYCRED_BUYCRED_TEMPLATES_DIR
                );
                if (!$override) {
                    $template = $override;
                }
        ...
        public function enqueue_footer()
        {
            ...
            if ($buycred_sale) {
                ...
                wp_localize_script(
                    ...
                    'buyCRED',
                    apply_filters(
                        'mycred_buycred_checkout_js',
                        [
                            'ajaxurl' => home_url('/'),
                            'token' => wp_create_nonce('mycred-buy-creds'),
                            'checkout' => $settings['checkout'],
                            'redirecting' => esc_js(
                                esc_attr__('Redirecting', 'mycred')
                            ),
	...
	<div class="checkout-inside">
        ...
        public function module_admin_init()
        {
            add_action(
                ...
                [$this, 'exchange_rates_user_screen'],
                30
            );

            add_action(
                'personal_options_update',
                [$this, 'save_manual_exchange_rates'],
                30
            );
            add_action(
...
<?php foreach ($this->point_types as $type_id => $label) {
			...
			<div class="row">
				<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
					...
					<div class="form-group">
						<label for="buycred-type-<?php echo esc_attr(
          $type_id
      ); ?>-enabled"><?php echo $mycred->plural(); ?></label>
						<div class="checkbox" style="padding-top: 4px;">
							<label for="buycred-type-<?php echo esc_attr(
           $type_id
       ); ?>-enabled"><input type="checkbox" name="mycred_pref_core[buy_creds][types][<?php echo esc_attr(
    $type_id
); ?>][enabled]" id="buycred-type-<?php echo esc_attr(
    $type_id
); ?>-enabled"<?php if (in_array($type_id, $settings['types'])) {
    echo ' checked="checked"';
} ?> value="<?php echo esc_attr($type_id); ?>" /> <?php _e(
    'Enable',
    'mycred'
); ?></label>
						</div>
				...
				<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
					...
					<div class="form-group">
						<label for="buycred-type-<?php echo esc_attr($type_id); ?>-min"><?php _e(
    'Minimum Amount',
    'mycred'
); ?></label>
						<input type="text" name="mycred_pref_core[buy_creds][types][<?php echo esc_attr(
          $type_id
      ); ?>][min]" id="buycred-type-<?php echo esc_attr(
    $type_id
); ?>-min" class="form-control" placeholder="<?php echo $mycred->get_lowest_value(); ?>" value="<?php echo esc_attr(
    $sale_setup['min']
); ?>" />
					</div>
				...
				<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
					...
					<div class="form-group">
						<label for="buycred-type-<?php echo esc_attr($type_id); ?>-max"><?php _e(
    'Maximum',
    'mycred'
); ?></label>
						<div class="row">
							<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
								<input type="text" name="mycred_pref_core[buy_creds][types][<?php echo esc_attr(
            $type_id
        ); ?>][max]" id="buycred-type-<?php echo esc_attr(
    $type_id
); ?>-max" class="form-control" placeholder="<?php _e(
    'No limit',
    'mycred'
); ?>" value="<?php echo esc_attr($sale_setup['max']); ?>" />
							</div>
							<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
								<?php mycred_purchase_limit_dropdown(
            'mycred_pref_core[buy_creds][types][' . $type_id . '][time]',
            'buycred-type-' . $type_id . '-time',
            $sale_setup['time']
        ); ?>
							</div>
						</div>
...
<?php
         ...
         'custom_log'
     ); ?>"><input type="checkbox" name="<?php echo $this->field_name('custom_log'); ?>" id="<?php echo $this->field_id('custom_log'); ?>"<?php checked($settings['custom_log'], 1); ?> value="1" /> <?php echo $this->core->template_tags_general(__('Create a dedicated log for purchases.', 'mycred')); ?></label>
				</div>
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
           ...
           'assets/images/checkout-full.png',
           MYCRED_PURCHASE
       ); ?>" alt="" style="max-width: 100%; height: auto;" />
							<input type="radio" name="<?php echo $this->field_name(
           'checkout'
       ); ?>"<?php checked($settings['checkout'], 'page'); ?> id="<?php echo $this->field_id('checkout-full'); ?>" value="page" /> Full Page
						</label>
					</div>
					<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 text-center">
						<label for="<?php echo $this->field_id('checkout-popup'); ?>">
							<img src="<?php echo plugins_url(
           'assets/images/checkout-popup.png',
           MYCRED_PURCHASE
       ); ?>" alt="" style="max-width: 100%; height: auto;" />
							<input type="radio" name="<?php echo $this->field_name(
           'checkout'
       ); ?>"<?php checked($settings['checkout'], 'popup'); ?> id="<?php echo $this->field_id('checkout-popup'); ?>" value="popup" /> Popup
						</label>
					</div>
				</div>
	...
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			...
			<div class="form-group">
				<p style="margin-top: 0;"><span class="description"><?php _e(
        'Where should users be redirected to upon successfully completing a purchase. You can nominate a specific URL or a page.',
        'mycred'
    ); ?></span></p>
			</div>
			<div class="form-group">
				<label for="<?php echo $this->field_id([
        'thankyou' => 'page',
    ]); ?>"><?php _e('Redirect to Page', 'mycred'); ?></label>
<?php
// Thank you page dropdown
$thankyou_args = [
    'name' => $this->field_name(['thankyou' => 'page']),
    'id' => $this->field_id(['thankyou' => 'page']) . '-id',
...
?>
			...
			<div class="form-group">
				<label for="<?php echo $this->field_id([
        'thankyou' => 'custom',
    ]); ?>"><?php _e('Redirect to URL', 'mycred'); ?></label>
				<input type="text" name="<?php echo $this->field_name([
        'thankyou' => 'custom',
    ]); ?>" id="<?php echo $this->field_id(['thankyou' => 'custom']); ?>" placeholder="https://" class="form-control" value="<?php echo esc_attr($settings['thankyou']['custom']); ?>" />
			</div>
			<?php if ($uses_buddypress): ?>
			<p style="margin-top: 0;"><span class="description"><?php _e(
       'You can use %profile% for the base URL of the users profile.',
       'mycred'
   ); ?></span></p>
			<?php endif; ?>
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			...
			<div class="form-group">
				<p style="margin-top: 0;"><span class="description"><?php _e(
        'Where should users be redirected to if they cancel a transaction. You can nominate a specific URL or a page.',
        'mycred'
    ); ?></span></p>
			</div>
			<div class="form-group">
...
// Thank you page dropdown
$thankyou_args = [
    'name' => $this->field_name(['cancelled' => 'page']),
    'id' => $this->field_id(['cancelled' => 'page']) . '-id',
...
?>
        ...
        'cancelled' => 'custom',
				...
				<input type="text" name="<?php echo $this->field_name([
        'cancelled' => 'custom',
    ]); ?>" id="<?php echo $this->field_id(['cancelled' => 'custom']); ?>" placeholder="https://" class="form-control" value="<?php echo esc_attr($settings['cancelled']['custom']); ?>" />
			</div>
			<?php if ($uses_buddypress): ?>
			<p style="margin-top: 0;"><span class="description"><?php _e(
       'You can use %profile% for the base URL of the users profile.',
	...
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			...
			<div class="form-group">
				<label for="<?php echo $this->field_id(
        'login'
    ); ?>"><?php _e('Login Message', 'mycred'); ?></label>
				<input type="text" name="<?php echo $this->field_name(
        'login'
    ); ?>" id="<?php echo $this->field_id('login'); ?>" class="form-control" value="<?php echo esc_attr($settings['login']); ?>" />
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			...
			<div class="form-group">
				<label for="<?php echo $this->field_id(
        'log'
    ); ?>"><?php _e('Log Template', 'mycred'); ?></label>
				<input type="text" name="<?php echo $this->field_name(
        'log'
    ); ?>" id="<?php echo $this->field_id('log'); ?>" class="form-control" placeholder="<?php _e('Required', 'mycred'); ?>" value="<?php echo esc_attr($settings['log']); ?>" />
				<p><span class="description"><?php echo $this->core->available_template_tags(
        ['general'],
        '%gateway%'
    ); ?></span></p>
			</div>
	...
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			...
			<div class="form-group">
				<div class="checkbox">
					<label for="<?php echo $this->field_id([
         'gifting' => 'members',
     ]); ?>"><input type="checkbox" name="<?php echo $this->field_name(['gifting' => 'members']); ?>" id="<?php echo $this->field_id(['gifting' => 'members']); ?>"<?php checked($settings['gifting']['members'], 1); ?> value="1" /> <?php echo $this->core->template_tags_general(__('Allow users to buy %_plural% for other users.', 'mycred')); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id([
         'gifting' => 'authors',
     ]); ?>"><input type="checkbox" name="<?php echo $this->field_name(['gifting' => 'authors']); ?>" id="<?php echo $this->field_id(['gifting' => 'authors']); ?>"<?php checked($settings['gifting']['authors'], 1); ?> value="1" /> <?php echo $this->core->template_tags_general(__('Allow users to buy %_plural% for content authors.', 'mycred')); ?></label>
				</div>
			</div>
        ...
        'gifting' => 'log',
				...
				<input type="text" name="<?php echo $this->field_name([
        'gifting' => 'log',
    ]); ?>" id="<?php echo $this->field_id('log'); ?>" class="form-control" placeholder="<?php _e('Required', 'mycred'); ?>" value="<?php echo esc_attr($settings['gifting']['log']); ?>" />
				<p><span class="description"><?php echo $this->core->available_template_tags([
        'general',
        'user',
    ]); ?></span></p>
...
$this->option_id = 'mycred_pref_buycreds';
        ...
        public function sanitize_extra_settings($new_data, $data, $core)
        {
            ...
            if (
                isset($data['buy_creds']['types']) &&
                !empty($data['buy_creds']['types'])
            ) {
                foreach ($data['buy_creds']['types'] as $type_id => $setup) {
                    $type_id = sanitize_key($type_id);
                    if (!mycred_point_type_exists($type_id)) {
                        continue;
                    }
                    ...
                    $settings = shortcode_atts(
                        mycred_get_buycred_sale_setup($type_id),
                        $settings
                    );

                    mycred_update_option(
                        'buycred-setup-' . $type_id,
                        $settings
                    );
                }
            }
            ...
            $new_data['buy_creds']['checkout'] = sanitize_key(
                $data['buy_creds']['checkout']
            );
            $new_data['buy_creds']['log'] = sanitize_text_field(
                $data['buy_creds']['log']
            );
            $new_data['buy_creds']['login'] = wp_kses_post(
                $data['buy_creds']['login']
            );

            $new_data['buy_creds']['thankyou']['page'] = absint(
                $data['buy_creds']['thankyou']['page']
            );
            $new_data['buy_creds']['thankyou']['custom'] = sanitize_text_field(
                $data['buy_creds']['thankyou']['custom']
            );
            $new_data['buy_creds']['thankyou']['use'] =
                $new_data['buy_creds']['thankyou']['custom'] != ''
                    ? 'custom'
                    : 'page';

            $new_data['buy_creds']['cancelled']['page'] = absint(
                $data['buy_creds']['cancelled']['page']
            );
            $new_data['buy_creds']['cancelled']['custom'] = sanitize_text_field(
                $data['buy_creds']['cancelled']['custom']
            );
            $new_data['buy_creds']['cancelled']['use'] =
                $new_data['buy_creds']['cancelled']['custom'] != ''
                    ? 'custom'
                    : 'page';

            $new_data['buy_creds']['custom_log'] = !isset(
                $data['buy_creds']['custom_log']
            )
                ? 0
                : 1;

            $new_data['buy_creds']['gifting']['members'] = !isset(
                $data['buy_creds']['gifting']['members']
            )
                ? 0
                : 1;
            $new_data['buy_creds']['gifting']['authors'] = !isset(
                $data['buy_creds']['gifting']['authors']
            )
                ? 0
                : 1;
            $new_data['buy_creds']['gifting']['log'] = sanitize_text_field(
                $data['buy_creds']['gifting']['log']
            );
        ...
        {
            if (substr($log->ref, 0, 15) != 'buy_creds_with_') {
                return $content;
            }
            return $this->core->template_tags_user(
                $content,
                absint($log->ref_id)
            );
        }
        ...
        public function add_to_menu()
        {
            ...
            if (
                isset($this->core->buy_creds['custom_log']) &&
                $this->core->buy_creds['custom_log']
            ) {
                $pages = [];
                $point_types =
                    isset($this->core->buy_creds['types']) &&
                    !empty($this->core->buy_creds['types'])
                        ? $this->core->buy_creds['types']
...
<?php if (!empty($installed)) {
    foreach ($installed as $key => $data) {
        ...
        $has_documentation =
            array_key_exists('documentation', $data) &&
            !empty($data['documentation'])
                ? esc_url_raw($data['documentation'])
                : false;
        $has_test_mode = array_key_exists('sandbox', $data)
            ? (bool) $data['sandbox']
            : false;
        $sandbox_mode =
            array_key_exists($key, $this->gateway_prefs) &&
            array_key_exists('sandbox', $this->gateway_prefs[$key]) &&
            $this->gateway_prefs[$key]['sandbox'] === 1
                ? true
                : false;
   ...
   ?>"></span><?php echo $this->core->template_tags_general(
    $data['title']
); ?></h4>
			<div class="body" style="display: none;">
				...
				<div class="row">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="form-group">
							<div>&nbsp;</div>
							<label for="buycred-gateway-<?php echo $key; ?>"><input type="checkbox" name="mycred_pref_buycreds[active][]" id="buycred-gateway-<?php echo $key; ?>" value="<?php echo $key; ?>"<?php if (
    $this->is_active($key)
) {
    echo ' checked="checked"';
					...
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						...
						<div class="form-group">
							<div>&nbsp;</div>
							<label for="buycred-gateway-<?php echo $key; ?>-sandbox"><input type="checkbox" name="mycred_pref_buycreds[gateway_prefs][<?php echo $key; ?>][sandbox]" id="buycred-gateway-<?php echo $key; ?>-sandbox" value="<?php echo $key; ?>"<?php if (
    $sandbox_mode
) {
    echo ' checked="checked"';
					...
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12" style="text-align: right;">
						...
						<div class="form-group">
							<div>&nbsp;</div>
							<a href="<?php echo $has_documentation; ?>" target="_blank"><?php _e(
    'Documentation',
    'mycred'
); ?></a>
						</div>
...
<?php
		...
		<p><?php submit_button(
      __('Update Settings', 'mycred'),
      'primary large',
      'submit',
      false
  ); ?> <?php if (
     MYCRED_SHOW_PREMIUM_ADDONS
 ): ?><a href="https://mycred.me/product-category/buycred-gateways/" class="button button-secondary button-large" target="_blank">More Gateways</a><?php endif; ?></p>
...
<?php
        ...
        public function sanitize_settings($data)
        {
            ...
            foreach ($installed as $gateway_id => $gateway) {
                ...
                $submitted_data =
                    !empty($data['gateway_prefs']) &&
                    array_key_exists($gateway_id, $data['gateway_prefs'])
                        ? $data['gateway_prefs'][$gateway_id]
                        : false;

                // No need to do anything if we have no data
                if ($submitted_data !== false) {
                    $data['gateway_prefs'][$gateway_id] = $this->call(
                        'sanitise_preferences',
                        $installed[$gateway_id]['callback'],
                        $submitted_data
                    );
                }
            }
        ...
        public function screen_options()
        {
            ...
            $meta_key =
                'mycred_payments_' .
                str_replace(MYCRED_SLUG . '-purchases-', '', $_GET['page']);
        ...
        public function purchase_log_page()
        {
            ...
            $filter_url = admin_url(
                'admin.php?page=' . MYCRED_SLUG . '-purchases-' . $point_type
            );

            $per_page = mycred_get_user_meta(
                $this->current_user_id,
                'mycred_payments_' . $point_type,
                '',
                true
            );
            ...
            $log->headers = apply_filters('mycred_buycred_log_columns', [
...
?>
		...
		<table class="wp-list-table widefat fixed striped users mycred-table" cellspacing="0">
			<thead>
				<tr>
<?php foreach ($log->headers as $col_id => $col_title) {
    echo '<th scope="col" id="' .
        str_replace('column-', '', $col_id) .
        '" class="manage-column ' .
        $col_id .
        '">' .
        $col_title .
...
if ($log->have_entries()) {
    ...
    foreach ($log->results as $log_entry) {
        ...
        if (
            !array_key_exists(str_replace('_', '-', $gateway_name), $installed)
        ) {
        ...
        foreach ($log->headers as $column_id => $column_name) {
            ...
            switch ($column_id) {
                // Used gateway
                case 'column-gateway':
                    $gateway = str_replace(['-', '_'], ' ', $gateway_name);
                ...
                case 'column-payed':
                    ...
                    if (
                        is_array($data) &&
                        array_key_exists('sales_data', $data)
                    ) {
                    ...
                    if ($cost === 'n/a') {
                        ...
                        $rendered_cost = apply_filters(
                            'mycred_buycred_display_cost',
                            $cost . ' ' . $currency,
                            $sales_data,
                            $log_entry,
                            $gateway_name
                        ...
                        $rendered_cost = apply_filters(
                            'mycred_buycred_display_cost_' . $gateway_name,
                            $rendered_cost,
                            $sales_data,
                            $log_entry
                        );
                ...
                default:
                    ...
                    do_action(
                        "mycred_payment_log_{$column_id}_{$type}",
                        $log_entry
                    );
...
<?php
        ...
        public function get_sales_data_from_log_data($log_data = '')
        {
            ...
            if (
                is_array($log_data) &&
                array_key_exists('sales_data', $log_data)
            ) {
        ...
        public function exchange_rates_user_screen($user)
        {
            ...
            foreach ($installed as $gateway_id => $prefs) {
                ...
                if (
                    !$this->is_active($gateway_id) ||
                    !array_key_exists($gateway_id, $this->gateway_prefs) ||
                    !$prefs['custom_rate']
                ) {
                    continue;
                }
                ...
                // Need a currency
                if (
                    array_key_exists('currency', $gateway_prefs) &&
                    $gateway_prefs['currency'] == ''
                ) {
                    continue;
                }
                ...
                $setup = [
                    'name' => $prefs['title'],
                    'currency' => $gateway_prefs['currency'],
                    'types' => [],
                ];
                ...
                foreach ($mycred_types as $point_type_key => $label) {
                    ...
                    if (
                        in_array($point_type_key, $point_types_for_sale) &&
                        array_key_exists(
                            $point_type_key,
                            $gateway_prefs['exchange']
                        )
                    ) {
                        $row['enabled'] = true;
                        ...
                        if (!$mycred->exclude_user($user->ID)) {
                            $row['excluded'] = false;
                            $row['default'] =
                                $gateway_prefs['exchange'][$point_type_key];

                            $row['before'] = $mycred->format_creds(1) . ' = ';

                            $saved_overrides = (array) mycred_get_user_meta(
                                $user->ID,
                                'mycred_buycred_rates_' . $point_type_key,
                                '',
                                true
                            );

                            if (
                                !empty($saved_overrides) &&
                                array_key_exists($gateway_id, $saved_overrides)
                            ) {
                                $row['override'] = true;
            ...
            ?>
<p class="mycred-p"><?php _e(
    'Users exchange rate when buying points.',
    'mycred'
); ?></p>
<table class="form-table mycred-inline-table">
<?php foreach ($available_options as $gateway_id => $setup): ?>
	<tr>
...
<?php }
    ...
    elseif ($data['excluded']) { ?>
					<div class="mycred-wrapper buycred-wrapper excluded-option color-option">
						<div><?php printf(
          _x('Buying %s', 'Buying Points', 'mycred'),
          $data['name']
      ); ?></div>
						<div class="balance-row">
							<div class="balance-view"><?php _e('Excluded', 'mycred'); ?></div>
							<div class="balance-desc"><em><?php printf(
           _x('User can not buy %s', 'Points Name', 'mycred'),
           $data['name']
       ); ?></em></div>
						</div>
					</div>
<?php }
    ...
    // Eligeble user
          ...
          $data['name']
						...
						<div class="balance-row">
							<div class="balance-view"><?php echo $data[
           'before'
       ]; ?><input type="text" name="mycred_adjust_users_buyrates[<?php echo $type_id; ?>][<?php echo $gateway_id; ?>]" placeholder="<?php echo $data[
    'default'
]; ?>" value="<?php if ($data['override']) {
    echo esc_attr($data['custom']);
...
<?php
        ...
        public function save_manual_exchange_rates($user_id)
        {
            ...
            if (
                isset($_POST['mycred_adjust_users_buyrates']) &&
                is_array($_POST['mycred_adjust_users_buyrates']) &&
                !empty($_POST['mycred_adjust_users_buyrates'])
            ) {
                foreach (
                    $_POST['mycred_adjust_users_buyrates']
                    as $ctype => $gateway
                ) {
