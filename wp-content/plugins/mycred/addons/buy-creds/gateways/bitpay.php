...
if (!class_exists('WC_Gateway_Bitpay')) {
    ...
    if (
        true === file_exists($autoloader_param) &&
...
if (!class_exists('myCRED_Bitpay')):
    class myCRED_Bitpay extends myCRED_Payment_Gateway
    {
        ...
        public function __construct($gateway_prefs)
        {
            ...
            parent::__construct(
                [
                    'id' => 'bitpay',
                    'label' => 'Bitpay',
                    'gateway_logo_url' => plugins_url(
                        'assets/images/bitpay.png',
                        MYCRED_PURCHASE
                    ),
                    'defaults' => [
                        'sandbox' => 0,
            ...
            $this->is_ready = false;
            if (
                isset($this->prefs['api_public']) &&
                !empty($this->prefs['api_public']) &&
                isset($this->prefs['api_secret']) &&
                !empty($this->prefs['api_secret'])
            ) {
                $this->is_ready = true;
            }
        }
                ...
                if (
                    ...
                    try {
                        ...
                        $client->setPrivateKey(
                            buycred_bitpay_decrypt($this->prefs['api_secret'])
                        );
                        $client->setPublicKey(
                            buycred_bitpay_decrypt($this->prefs['api_public'])
                        );
                        $client->setToken(
                            buycred_bitpay_decrypt($this->prefs['api_token'])
                        );
                    ...
                    if (empty($new_call)) {
                        ...
                        $pending_post_id = buycred_get_pending_payment_id(
                            $transaction_id
                        );
                        $pending_payment = $this->get_pending_payment(
                            $pending_post_id
                        );
                        ...
                        if ($pending_payment !== false) {
                            ...
                            if (
                                ...
                                $new_call[] = __(
                                    'Failed to credit users account.',
                                    'mycred'
                                );
                            }
        ...
        {
            add_action('wp_ajax_buycred-bitpay-pairing', [$this, 'ajax_pair']);
        }
        ...
        public function ajax_pair()
        {
            ...
            wp_send_json_success([
                'api_secret' =>
                    ...
                    $this->field_name('api_secret') .
                    '" value="' .
                    buycred_bitpay_encrypt($key) .
                    '" />',
                'api_public' =>
                    '<input type="hidden" name="' .
                    $this->field_name('api_public') .
                    '" value="' .
                    buycred_bitpay_encrypt($pub) .
                    '" />',
                'api_sign' =>
                    '<input type="hidden" name="' .
                    $this->field_name('api_sign') .
                    '" value="' .
                    (string) $sin .
                    '" />',
                'api_token' =>
                    '<input type="hidden" name="' .
                    $this->field_name('api_token') .
                    '" value="' .
                    buycred_bitpay_encrypt($token) .
                    '" />',
                'label' =>
                    '<input type="hidden" name="' .
                    $this->field_name('api_label') .
                    '" value="' .
                    $label .
        ...
        public function prep_sale($new_transaction = false)
        {
            ...
            try {
                ...
                $invoice->setFullNotifications(
                    $this->prefs['notifications'] ? true : false
                );
...
jQuery(function($){
	...
	$( '#bitpay-pair-network' ).change(function(){
		...
		<div class="form-group">
			...
			<input type="hidden" class="reset-api" name="<?php echo $this->field_name(
       'api_secret'
   ); ?>" value="<?php echo esc_attr($prefs['api_secret']); ?>" />
			<input type="hidden" class="reset-api" name="<?php echo $this->field_name(
       'api_public'
   ); ?>" value="<?php echo esc_attr($prefs['api_public']); ?>" />
			<input type="hidden" class="reset-api" name="<?php echo $this->field_name(
       'api_sign'
   ); ?>" value="<?php echo esc_attr($prefs['api_sign']); ?>" />
			<input type="hidden" class="reset-api" name="<?php echo $this->field_name(
       'api_token'
   ); ?>" value="<?php echo esc_attr($prefs['api_token']); ?>" />
			<input type="hidden" class="reset-api" name="<?php echo $this->field_name(
       'api_label'
   ); ?>" value="<?php echo esc_attr($prefs['api_label']); ?>" />
		</div>
<script type="text/javascript">
jQuery(function($){
	...
	$( '#bitpay-cancel-pair' ).click(function(e){
      ...
      esc_attr(__('Are you sure you want to do this?', 'mycred'))
  ); ?>' ) ) {

			$( 'input.reset-api' ).val( '' );
			$(this).before().html( '<?php echo esc_js(
       __('Removed - Remember to save your changes.', 'mycred')
   ); ?>' );
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('item_name'); ?>"><?php _e(
    'Item Name',
    'mycred'
); ?></label>
    ...
    $prefs['item_name']
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('logo_url'); ?>"><?php _e(
    'Logo URL',
    'mycred'
); ?></label>
			<input type="text" name="<?php echo $this->field_name(
    ...
    $prefs['logo_url']
		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id('speed'); ?>"><?php _e(
    'Transaction Speed',
    'mycred'
); ?></label>
					<select name="<?php echo $this->field_name(
         'speed'
     ); ?>" id="<?php echo $this->field_id('speed'); ?>" class="form-control">
<?php
$options = [
    'high' => __('High', 'mycred'),
...
?>
			...
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id('notifications'); ?>"><?php _e(
    'Full Notifications',
    'mycred'
); ?></label>
					<select name="<?php echo $this->field_name(
...
?>
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('currency'); ?>"><?php _e(
    'Currency',
    'mycred'
); ?></label>
			<input type="text" name="<?php echo $this->field_name(
    ...
    'currency'
); ?>" value="<?php echo $prefs[
    'currency'
]; ?>" class="form-control" maxlength="3" placeholder="<?php _e(
    'Currency Code',
    'mycred'
); ?>" />
...
<?php
        ...
        public function sanitise_preferences($data)
        {
            ...
            if (array_key_exists('api_secret', $data)) {
                $new_data['api_secret'] = sanitize_text_field(
                    $data['api_secret']
                );
                $new_data['api_public'] = sanitize_text_field(
                    $data['api_public']
                );
                $new_data['api_sign'] = sanitize_text_field($data['api_sign']);
                $new_data['api_token'] = sanitize_text_field(
                    $data['api_token']
                );
                $new_data['api_label'] = sanitize_text_field(
                    $data['api_label']
                );
            } else {
                $new_data['api_secret'] = '';
            ...
            $new_data['notifications'] = sanitize_text_field(
                $data['notifications']
            );
...
if (!function_exists('buycred_bitpay_encrypt')):
    function buycred_bitpay_encrypt($data)
    {
        if (false === isset($data) || true === empty($data)) {
            throw new \Exception(
                'The Bitpay payment plugin was called to encrypt data but no data was passed!'
            );
        }
        ...
        if (
            ...
            if (false === isset($fingerprint) || true === empty($fingerprint)) {
                throw new \Exception(
                    'The Bitpay payment plugin was called to encrypt data but could not generate a fingerprint parameter!'
                );
            }

            $encrypted = $openssl_ext->encrypt(
            ...
            if (true === empty($encrypted)) {
                throw new \Exception(
                    'The Bitpay payment plugin was called to serialize an encrypted object and failed!'
                );
            }
...
if (!function_exists('buycred_bitpay_decrypt')):
    function buycred_bitpay_decrypt($encrypted)
    {
        if (false === isset($encrypted) || true === empty($encrypted)) {
            throw new \Exception(
                'The Bitpay payment plugin was called to decrypt data but no data was passed!'
            );
        }
        $openssl_ext = new \Bitpay\Crypto\OpenSSLExtension();
        ...
        if (
            ...
            if (false === isset($fingerprint) || true === empty($fingerprint)) {
                throw new \Exception(
                    'The Bitpay payment plugin was called to decrypt data but could not generate a fingerprint parameter!'
                );
            }
            ...
            if (false === base64_decode($decrypted, true)) {
                $error_string =
                    '    [Warning] In bitpay_decrypt: data appears to have already been decrypted. Strict base64 check failed.';
            } else {
            ...
            if (true === empty($decrypted)) {
                throw new \Exception(
                    'The Bitpay payment plugin was called to unserialize a decrypted object and failed! The decrypt function was called with "' .
                        $encrypted .
                        '"'
                );
            }
