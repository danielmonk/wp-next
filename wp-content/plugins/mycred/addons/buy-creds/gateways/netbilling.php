...
if (!class_exists('myCRED_NETbilling')):
    class myCRED_NETbilling extends myCRED_Payment_Gateway
    {
        ...
        public function IPN_is_valid_call()
        {
            ...
            if (
                $account[0] != $this->prefs['account'] ||
                $account[1] != $this->prefs['site_tag']
            ) {
                $result = false;
            }
            ...
            // Crypto Check
            $crypto_check = md5(
                $this->prefs['cryptokey'] .
                    $_REQUEST['Ecom_Cost_Total'] .
                    $_REQUEST['Ecom_Receipt_Description']
            );
            if (
                $crypto_check != $_REQUEST['Ecom_Ezic_Security_HashValue_MD5']
            ) {
                $result = false;
            }
        ...
        public function process()
        {
            // Required fields
            if (
                isset($_REQUEST['Ecom_UserData_salesdata']) &&
                isset($_REQUEST['Ecom_Ezic_Response_TransactionID']) &&
                isset($_REQUEST['Ecom_Cost_Total'])
            ) {
                // Get Pending Payment
                $pending_post_id = sanitize_key(
                    $_REQUEST['Ecom_UserData_salesdata']
                );
                ...
                if ($pending_payment !== false) {
                    ...
                    if ($this->IPN_is_valid_call()) {
                        ...
                        if ($_REQUEST['Ecom_Ezic_Response_StatusCode'] != 1) {
                            $new_call[] = sprintf(
                                __(
                                    'Payment not completed. Received: %s',
                                    'mycred'
                                ),
                                $_REQUEST['Ecom_Ezic_Response_StatusCode']
                            );
                            $errors = true;
                        }
        ...
        {
            if (
                isset($_REQUEST['Ecom_Ezic_AccountAndSitetag']) &&
                isset($_REQUEST['Ecom_UserData_salesdata'])
            ) {
                $this->process();
            }
        ...
        public function prep_sale($new_transaction = false)
        {
            ...
            $this->redirect_fields = [
                'Ecom_Ezic_AccountAndSitetag' =>
                    $this->prefs['account'] . ':' . $this->prefs['site_tag'],
                ...
                'Ecom_Ezic_Fulfillment_GiveUpURL' => $this->get_cancelled(
                    $this->transaction_id
                ),
                'Ecom_Ezic_Security_HashValue_MD5' => md5(
                    $this->prefs['cryptokey'] . $this->cost . $item_name
                ),
                'Ecom_Ezic_Security_HashFields' =>
                    'Ecom_Cost_Total Ecom_Receipt_Description',
            ];
        }
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('account'); ?>"><?php _e(
    'Account ID',
    'mycred'
); ?></label>
    ...
    $prefs['account']
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('site_tag'); ?>"><?php _e(
    'Site Tag',
    'mycred'
); ?></label>
			<input type="text" name="<?php echo $this->field_name(
    ...
    $prefs['site_tag']
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('cryptokey'); ?>"><?php _e(
    'Order Integrity Key',
    'mycred'
); ?></label>
			<input type="password" name="<?php echo $this->field_name(
       'cryptokey'
   ); ?>" id="<?php echo $this->field_id(
    'cryptokey'
); ?>" value="<?php echo esc_attr(
    $prefs['cryptokey']
       ...
       'Found under Step 12 on the Fraud Defense page.',
       'mycred'
   ); ?></span></p>
		</div>
		<div class="form-group">
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		...
		<div class="form-group">
			...
			<input type="text" readonly="readonly" class="form-control" name="<?php echo $this->field_name(
       'currency'
   ); ?>" value="USD" />
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<p><?php _e(
       'For this gateway to work, you must login to your NETbilling account and edit your site. Under "Default payment form settings" make sure the Postback CGI URL is set to the above address and "Return method" is set to POST.',
       'mycred'
   ); ?></p>
		</div>
	</div>
