    ...
    class myCRED_Skrill extends myCRED_Payment_Gateway
    {
        ...
        public function IPN_is_valid_call()
        {
            ...
            $check =
                ...
                strtoupper(md5($this->prefs['word'])) .
                $_POST['mb_amount'] .
                $_POST['mb_currency'] .
                $_POST['status'];
                ...
                if ($pending_payment !== false) {
                    ...
                    if ($this->IPN_is_valid_call()) {
                        ...
                        if ($_POST['currency'] != $pending_payment->currency) {
                            $new_call[] = sprintf(
                                __(
                                    'Currency mismatch. Expected: %s Received: %s',
                                    'mycred'
                                ),
        ...
        public function prep_sale($new_transaction = false)
        {
            ...
            if (
                isset($this->prefs['account_title']) &&
                !empty($this->prefs['account_title'])
            ) {
                $redirect_fields[
                    'recipient_description'
                ] = $this->core->template_tags_general(
                    $this->prefs['account_title']
                );
            }

            if (
                isset($this->prefs['account_logo']) &&
                !empty($this->prefs['account_logo'])
            ) {
                $redirect_fields['logo_url'] = $this->prefs['account_logo'];
            }

            if (
                isset($this->prefs['confirmation_note']) &&
                !empty($this->prefs['confirmation_note'])
            ) {
                $redirect_fields[
                    'confirmation_note'
                ] = $this->core->template_tags_general(
                    $this->prefs['confirmation_note']
                );
            }

            // If we want an email receipt for purchases
            if (
                isset($this->prefs['email_receipt']) &&
                !empty($this->prefs['email_receipt'])
            ) {
            ...
            if ($this->gifting) {
                ...
                $redirect_fields['detail2_description'] = __(
                    'Recipient',
                    'mycred'
                );
...
<div class="row">
    ...
    $prefs['account']
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('word'); ?>"><?php _e(
    'Secret Word',
    'mycred'
); ?></label>
			<input type="text" name="<?php echo $this->field_name(
    ...
    'word'
); ?>" value="<?php echo esc_attr($prefs['word']); ?>" class="form-control" />
		</div>
		<div class="form-group">
    ...
    'email_receipt'
); ?>" value="1"<?php checked($prefs['email_receipt'], 1); ?> /> <?php _e(
    ...
    'mycred'
       ...
       'currency',
       'mycred-gateway-skrill-currency'
   ); ?>
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="<?php echo $this->field_id('account_title'); ?>"><?php _e(
    'Title',
    'mycred'
); ?></label>
    ...
    $prefs['account_title']
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('confirmation_note'); ?>"><?php _e(
    'Confirmation Note',
    'mycred'
); ?></label>
			<p><span class="description"><?php _e(
       'Optional text to show user once a transaction has been successfully completed. This text is shown by Skrill.',
       'mycred'
   ); ?></span></p>
    ...
    'confirmation_note'
); ?>" class="form-control"><?php echo esc_html(
    $prefs['confirmation_note']
); ?></textarea>
		</div>
	</div>
</div>
<?php remove_filter('mycred_dropdown_currencies', [$this, 'skrill_currencies']);
        }
        ...
        public function sanitise_preferences($data)
        {
            ...
            $new_data['confirmation_note'] = substr(
