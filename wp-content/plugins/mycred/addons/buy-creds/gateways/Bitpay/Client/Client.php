...
class Client implements ClientInterface
{
    ...
    public function createInvoice(InvoiceInterface $invoice)
    {
        ...
        $body = [
            ...
            'buyerName' => trim(
                sprintf('%s %s', $buyer->getFirstName(), $buyer->getLastName())
            ),
        ...
        $error_message = false;
        $error_message = !empty($body['error'])
            ? $body['error']
            : $error_message;
        $error_message = !empty($body['errors'])
            ? $body['errors']
            : $error_message;
        $error_message = is_array($error_message)
            ? implode("\n", $error_message)
            : $error_message;
    ...
    public function createPayout(PayoutInterface $payout)
    {
        ...
        // Optional
        foreach (
            ['reference', 'notificationURL', 'notificationEmail']
            as $value
        ) {
    ...
    public function getPayouts($status = null)
    {
        ...
        array_walk($body['data'], function ($value, $key) use (&$payouts) {
            ...
            array_walk($value['instructions'], function ($value, $key) use (
                &$payout
            ) {
                ...
                array_walk($value['transactions'], function ($value, $key) use (
                    &$instruction
                ) {
    ...
    public function getPayout($payoutId)
    {
        ...
        $request->setMethod(Request::METHOD_GET);
        $request->setPath(
            sprintf('payouts/%s?token=%s', $payoutId, $this->token->getToken())
        );
        $this->addIdentityHeader($request);
        $this->addSignatureHeader($request);
        ...
        array_walk($data['instructions'], function ($value, $key) use (
            &$payout
        ) {
            $instruction = new \Bitpay\PayoutInstruction();
            $instruction
    ...
    {
        if (
            isset($payload['pairingCode']) &&
            1 !== preg_match('/^[a-zA-Z0-9]{7}$/', $payload['pairingCode'])
        ) {
        ...
        if (isset($body['error'])) {
            throw new \Bitpay\Client\BitpayException(
                $this->response->getStatusCode() . ": " . $body['error']
            );
        }
        ...
        $token
            ...
            ->setCreatedAt(
                $createdAt->setTimestamp(floor($tkn['dateCreated'] / 1000))
            );
        ...
        if (isset($tkn['pairingCode'])) {
            ...
            $token->setPairingExpiration(
                $pairingExpiration->setTimestamp(
                    floor($tkn['pairingExpiration'] / 1000)
                )
            );
        }
    ...
    public function getInvoice($invoiceId)
    {
        ...
        $invoice
            ...
            ->setUrl($data['url'])
            ->setPosData(
                array_key_exists('posData', $data) ? $data['posData'] : ''
            )
            ->setStatus($data['status'])
            ->setBtcPrice($data['btcPrice'])
            ->setPrice($data['price'])
            ->setCurrency(new \Bitpay\Currency($data['currency']))
            ->setOrderId(
                array_key_exists('orderId', $data) ? $data['orderId'] : ''
            )
            ->setInvoiceTime($data['invoiceTime'])
            ->setExpirationTime($data['expirationTime'])
            ->setCurrentTime($data['currentTime'])
            ...
            ->setRefundAddresses(
                array_key_exists('refundAddresses', $data)
                    ? $data['refundAddresses']
                    : ''
            );
    ...
    protected function checkPriceAndCurrency($price, $currency)
    {
        ...
        if (
            ($decimalPrecision > 2 && $currency != 'BTC') ||
            $decimalPrecision > 6
        ) {
