...
class Util
{
    ...
    public static function encodeHex($dec)
    {
        if (!is_string($dec) && !ctype_digit($dec)) {
            throw new \Exception(
                sprintf(
                    'Argument is expected to be a string of decimal numbers. You passed in "%s"',
                    gettype($dec)
                )
            );
        }
    ...
    public static function decodeHex($hex)
    {
        if (
            !is_string($hex) ||
            (!ctype_xdigit($hex) && '0x' != substr($hex, 0, 2))
        ) {
    ...
    public static function doubleAndAdd(
        $hex,
        PointInterface $point,
        CurveParameterInterface $parameters = null
    ) {
        if (null === $parameters) {
    ...
    public static function decToBin($dec)
    {
        ...
        while (Math::cmp($dec, '0') > 0) {
            ...
            if (Math::cmp($prevDec, $dec) < 1) {
                throw new \Exception(
                    'Math library has unexpected behavior, please report the following information to support@bitpay.com. Math Engine is: ' .
                        Math::getEngineName() .
                        '. PHP Version is: ' .
                        phpversion() .
                        '.'
     ...
     */
    public static function pointDouble(
        PointInterface $point,
        CurveParameterInterface $parameters = null
    ) {
        if ($point->isInfinity()) {
        ...
        try {
            $m = Math::add(
                Math::mul(3, Math::mul($point->getX(), $point->getX())),
                $a
            );
            ...
            throw new \Exception(
                'Error in Util::pointDouble(): ' . $e->getMessage()
            );
        }
    ...
    public static function pointAdd(PointInterface $P, PointInterface $Q)
    {
        ...
        try {
            ...
            $R['x'] = Math::mod(
                Math::sub(Math::sub(Math::mul($s, $s), $P->getX()), $Q->getX()),
                $p
            );
            $R['y'] = Math::mod(
                Math::add(
                    Math::sub(0, $P->getY()),
                    Math::mul($s, Math::sub($P->getX(), $R['x']))
                ),
                $p
            );
            ...
            throw new \Exception(
                'Error in Util::pointAdd(): ' . $e->getMessage()
            );
        }
    ...
    public static function checkRequirements()
    {
        ...
        if (PHP_VERSION_ID < 50400) {
            $requirements['PHP'] =
                'Your PHP version, ' .
                PHP_VERSION .
                ', is too low. PHP version >= 5.4 is required.';
        } else {
        ...
        if (!extension_loaded('mcrypt')) {
            $requirements['Mcrypt'] =
                'The Mcrypt PHP extension could not be found.';
        } else {
        ...
        if (!extension_loaded('openssl')) {
            $requirements['OpenSSL'] =
                'The OpenSSL PHP extension could not be found.';
        } else {
        ...
        if (!extension_loaded('json')) {
            $requirements['JSON'] =
                'The JSON PHP extension could not be found.';
        } else {
        ...
        if (!extension_loaded('curl')) {
            $requirements['cURL'] =
                'The cURL PHP extension could not be found.';
        } else {
            $requirements['cURL'] = true;
            $curl_version = curl_version();
            $ssl_supported = $curl_version['features'] & CURL_VERSION_SSL;
            if (!$ssl_supported) {
                $requirements['cURL.SSL'] =
                    'The cURL PHP extension does not have SSL support.';
            } else {
        ...
        if (!extension_loaded('bcmath') && !extension_loaded('gmp')) {
            $requirements['Math'] =
                'Either the BC Math or GMP PHP extension is required.  Neither could be found.';
        } else {
