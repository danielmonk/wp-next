    ...
    final public function generateKeypair(
        ...
        try {
            ...
            if (function_exists('openssl_pkey_new')) {
                ...
                if (!$resource) {
                    throw new \Exception(
                        'Error in generateOpenSSLKeypair: Could not create new OpenSSL resource.'
                    );

                    /* with the openssl extension, you also have it's own errors returned */
                    while ($msg = openssl_error_string()) {
                        throw new \Exception(
                            'Error in generateOpenSSLKeypair: OpenSSL reported error: ' .
                                $msg
                        );
                    }
                ...
                if (openssl_pkey_export($resource, $keypair['pri'])) {
                    ...
                    throw new \Exception(
                        'Error in generateOpenSSLKeypair: Private key could not be determined from OpenSSL key resource.'
                    );
                ...
                throw new \Exception(
                    'Error in generateOpenSSLKeypair: OpenSSL PHP extension missing. Cannot continue.'
                );
        ...
        try {
            if (function_exists('openssl_pkey_new')) {
                ...
                if (!empty($text) && is_string($text)) {
                    ...
                    $text_num =
                        $bit_check - strlen($text_num[count($text_num) - 1]);
                ...
                throw new \Exception(
                    'Error in OpenSSL encrypt: OpenSSL PHP extension missing. openssl_encrypt function not found.'
                );
