...
class PrivateKey extends Key
{
    ...
    public function generate()
    {
        ...
        do {
            ...
            Math::cmp('0x' . $this->hex, '1') <= 0 ||
            Math::cmp('0x' . $this->hex, '0x' . Secp256k1::N) >= 0
        );
    ...
    public function pemDecode($pem_data)
    {
        ...
        if (strlen($decoded) < 230) {
            throw new \Exception(
                'Invalid or corrupt secp256k1 key provided. Cannot decode the supplied PEM data.'
            );
        }
    ...
    public function pemEncode($keypair)
    {
        if (
            is_array($keypair) &&
            (strlen($keypair[0]) < 64 || strlen($keypair[1]) < 128)
        ) {
            throw new \Exception(
                'Invalid or corrupt secp256k1 keypair provided. Cannot decode the supplied PEM data.'
            );
        }
        ...
        if (strlen($dec) < 230) {
            throw new \Exception(
                'Invalid or corrupt secp256k1 keypair provided. Cannot encode the supplied data.'
            );
        }
        ...
        $byte =
            $beg_ec_text .
            "\r\n" .
            chunk_split(base64_encode(strrev($byte)), 64) .
            $end_ec_text;
