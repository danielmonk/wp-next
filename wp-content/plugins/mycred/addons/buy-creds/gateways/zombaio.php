...
if (!class_exists('myCRED_Zombaio')):
    class myCRED_Zombaio extends myCRED_Payment_Gateway
    {
        ...
        public function returning()
        {
            // ZOA Validation
            if (
                isset($_REQUEST['wp_zombaio_ips']) ||
                (isset($_REQUEST['ZombaioGWPass']) &&
                    isset($_GET['username']) &&
                    substr($_GET['username'], 0, 4) == 'Test')
            ) {
                if (!headers_sent()) {
        ...
        {
            if (
                isset($_GET['wp_zombaio_ips']) &&
                $_GET['wp_zombaio_ips'] == 1
            ) {
                if (isset($_GET['csv']) && $_GET['csv'] == 1) {
                    echo '<textarea style="width: 270px;" rows="10" readonly="readonly">' .
                        implode(',', $this->get_zombaio_ips()) .
                        '</textarea>';
                    exit();
                }
                ...
                echo '<ul>';
                foreach ($ips as $ip) {
                    echo '<li><input type="text" readonly="readonly" value="' .
                        $ip .
                        '" size="15" /></li>';
                }
                echo '</ul>';
        ...
        public function IPN_is_valid_call()
        {
            ...
            if (
                $result === true &&
                $_GET['SiteID'] != $this->prefs['site_id']
            ) {
                $result = false;
            }
                ...
                if ($pending_payment !== false) {
                    ...
                    if ($this->IPN_is_valid_call()) {
                        ...
                        if (!$this->transaction_id_is_unique($transaction_id)) {
                            $new_call[] = sprintf(
                                __(
                                    'Duplicate transaction. Received: %s',
                                    'mycred'
                                ),
                                $transaction_id
                            );
                        ...
                        if ($this->sandbox_mode && $transaction_id != '0000') {
                            $new_call[] = sprintf(
                                __(
                                    'Live transaction while debug mode is enabled! Received: %s',
                                    'mycred'
                                ),
                                $transaction_id
                            );
                        ...
                        if ($errors === false) {
                            ...
                            if (is_numeric($amount) && $amount > 0) {
                                ...
                                $pending_payment->amount = $mycred->number(
                                    $amount
                                );
        ...
        public function prep_sale($new_transaction = false)
        {
            ...
            $this->redirect_to =
                'https://secure.zombaio.com/?' .
                $this->prefs['site_id'] .
                '.' .
                $this->prefs['pricing_id'] .
                '.' .
                $this->prefs['lang'];
            ...
            if ($this->prefs['dynamic']) {
                ...
                $redirect_fields['DynAmount_Hash'] = md5(
                    $this->prefs['gwpass'] . $this->cost
                );
            }
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('site_id'); ?>"><?php _e(
    'Site ID',
    'mycred'
); ?></label>
    ...
    $prefs['site_id']
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('gwpass'); ?>"><?php _e(
    'GW Password',
    'mycred'
); ?></label>
			<input type="text" name="<?php echo $this->field_name(
    ...
    'gwpass'
); ?>" value="<?php echo esc_attr($prefs['gwpass']); ?>" class="form-control" />
		</div>
		<div class="form-group">
    ...
    $prefs['logo_url']
		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id('bypass_ipn'); ?>"><?php _e(
    'IP Verification',
    'mycred'
); ?></label>
...
?>
			...
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id('lang'); ?>"><?php _e(
    'Language',
    'mycred'
); ?></label>
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('pricing_id'); ?>"><?php _e(
    'Pricing ID',
    'mycred'
); ?></label>
			<input type="text" name="<?php echo $this->field_name(
    ...
    'dynamic'
); ?>" id="<?php echo $this->field_id('dynamic'); ?>"<?php checked(
    $prefs['dynamic'],
    1
); ?> value="1" /> <?php _e(
     'This pricing ID is a "Dynamic Credits Purchase" in Zombaio.',
     'mycred'
 ); ?></label>
			</div>
		</div>
		<div id="zombaio-dynamic-wrapper" style="display: <?php if (
      $prefs['dynamic'] == 1
  ) {
      echo 'block';
  } else {
    ...
    'mycred'
				...
				<?php $this->currencies_dropdown(
        'currency',
        'mycred-gateway-zombaio-currency'
    ); ?>
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		...
		<code style="padding: 12px;display:block;"><?php echo get_bloginfo(
      'url'
  ); ?></code>
		<p><?php _e(
      'For this gateway to work, login to ZOA and set the Postback URL to the above address and click validate.',
      'mycred'
  ); ?></p>
	</div>
</div>
...
<?php
        ...
        public function sanitise_preferences($data)
        {
            ...
            $new_data['bypass_ipn'] = array_key_exists('bypass_ipn', $data)
                ? 1
                : 0;
