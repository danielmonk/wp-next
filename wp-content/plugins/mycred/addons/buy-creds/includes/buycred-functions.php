...
if (!function_exists('mycred_get_buycred_gateways')):
    function mycred_get_buycred_gateways()
    {
        ...
        // PayPal Standard
        $installed['paypal-standard'] = [
            'title' => 'PayPal Payments Standard',
            'documentation' =>
                'http://codex.mycred.me/chapter-iii/buycred/payment-gateways/paypal/',
            'callback' => ['myCRED_PayPal_Standard'],
        ...
        $installed['bitpay'] = [
            ...
            'documentation' =>
                'http://codex.mycred.me/chapter-iii/buycred/payment-gateways/bitpay/',
            'callback' => ['myCRED_Bitpay'],
            'icon' => 'dashicons-admin-generic',
            'sandbox' => true,
            'external' => true,
            'custom_rate' => true,
        ...
        // NetBilling
        $installed['netbilling'] = [
            'title' => 'NETBilling',
            'callback' => ['myCRED_NETbilling'],
            'documentation' =>
                'http://codex.mycred.me/chapter-iii/buycred/payment-gateways/netbilling/',
            'icon' => 'dashicons-admin-generic',
            'sandbox' => true,
            'external' => true,
            'custom_rate' => true,
        ...
        $installed['skrill'] = [
            ...
            'callback' => ['myCRED_Skrill'],
            'documentation' =>
                'http://codex.mycred.me/chapter-iii/buycred/payment-gateways/skrill/',
            'icon' => 'dashicons-admin-generic',
            'sandbox' => true,
            'external' => true,
            'custom_rate' => true,
        ...
        $installed['bank'] = [
            ...
            'documentation' =>
                'http://codex.mycred.me/chapter-iii/buycred/payment-gateways/bank-transfers/',
            'callback' => ['myCRED_Bank_Transfer'],
            'icon' => 'dashicons-admin-generic',
            'sandbox' => false,
            'external' => false,
            'custom_rate' => true,
...
if (!function_exists('mycred_get_buycred_sale_setup')):
    function mycred_get_buycred_sale_setup(
...
if (!function_exists('mycred_purchase_limit_dropdown')):
    function mycred_purchase_limit_dropdown(
        $name = '',
        $id = '',
        $selected = ''
    ) {
        $options = apply_filters('mycred_buycred_limit_dropdown', [
            '' => __('No limit', 'mycred'),
            'day' => __('/ Day', 'mycred'),
            'week' => __('/ Week', 'mycred'),
...
if (!function_exists('mycred_user_can_buycred')):
    function mycred_user_can_buycred(
        $user_id = 0,
        $point_type = MYCRED_DEFAULT_TYPE_KEY
    ) {
        $can_buy = false;
        ...
        if ($user_id === 0 || !in_array($point_type, $settings['types'])) {
            return $can_buy;
        }
        ...
        $minimum = $mycred->number(
            $setup['min'] == '' ? $mycred->get_lowest_value() : $setup['min']
        );
        ...
        if (
            ...
            $total = mycred_get_users_total_purchase(
                $user_id,
                $point_type,
                $setup['time']
            );
...
if (!function_exists('mycred_buycred_gateway_is_usable')):
    function mycred_buycred_gateway_is_usable($gateway_id)
    {
        ...
        $usable = false;
        if (
            isset($buycred_instance->active) &&
            array_key_exists($gateway_id, $buycred_instance->active)
        ) {
            $usable = true;
        }
...
if (!function_exists('mycred_get_users_total_purchase')):
    function mycred_get_users_total_purchase(
        ...
        $user_id = absint($user_id);
        if ($user_id === 0 || !mycred_point_type_exists($point_type)) {
            return $total;
        }
        ...
        return apply_filters(
            'mycred_get_users_total_purchase',
            $total,
            $user_id,
            $point_type,
            $completed,
...
if (!function_exists('buycred_get_pending_payment_id')):
    function buycred_get_pending_payment_id($payment_id = null)
    {
        ...
        if (!is_numeric($payment_id)) {
            $post = mycred_get_page_by_title(
                strtoupper($payment_id),
                OBJECT,
                'buycred_payment'
            );
            if ($post === null) {
...
if (!function_exists('buycred_get_pending_payment')):
    function buycred_get_pending_payment($payment_id = null)
    {
        ...
        if (is_array($payment_id)) {
            ...
            $pending_payment->point_type = mycred_get_post_meta(
                $payment_id,
                'point_type',
                true
            );
            $pending_payment->amount = mycred_get_post_meta(
                $payment_id,
                'amount',
                true
            );
            $pending_payment->cost = mycred_get_post_meta(
                $payment_id,
                'cost',
                true
            );
            $pending_payment->currency = mycred_get_post_meta(
                $payment_id,
                'currency',
                true
            );
            $pending_payment->buyer_id = mycred_get_post_meta(
                $payment_id,
                'from',
                true
            );
            $pending_payment->recipient_id = mycred_get_post_meta(
                $payment_id,
                'to',
                true
            );
            $pending_payment->gateway_id = mycred_get_post_meta(
                $payment_id,
                'gateway',
                true
            );
            ...
            $pending_payment->cancel_url = buycred_get_cancel_transaction_url(
                $pending_payment->public_id
            );
...
if (!function_exists('buycred_add_pending_comment')):
    function buycred_add_pending_comment(
        $payment_id = null,
        $comment = null,
        $time = null
    ) {
        ...
        $author_url = sprintf(
            'buyCRED: %s %s',
            __('Unknown Gateway', 'mycred'),
            $gateway
        );
        $author_email = apply_filters(
            'mycred_buycred_comment_email',
            'buycred-service@mycred.me'
        );
...
if (!function_exists('buycred_get_cancel_transaction_url')):
    function buycred_get_cancel_transaction_url($transaction_id = null)
    {
        ...
        if ($settings['cancelled']['use'] == 'page') {
            if (
                !empty($settings['cancelled']['page']) &&
                $settings['cancelled']['page'] > 0
            ) {
        ...
        // Custom URL
        elseif (
            $settings['cancelled']['use'] != 'page' &&
            $settings['cancelled']['custom'] != ''
        ) {
        ...
        // Override
        if (
            isset($_REQUEST['return_to']) &&
            esc_url_raw($_REQUEST['return_to']) != ''
        ) {
        ...
        if ($transaction_id !== null) {
            $url = add_query_arg(
                [
                    'buycred-cancel' => $transaction_id,
                    '_token' => wp_create_nonce(
    ...
    function buycred_get_users_pending_payments(
        ...
        if (!is_array($pending)) {
            ...
            $saved = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM {$table} posts WHERE posts.post_type = 'buycred_payment' AND posts.post_author = %d AND posts.post_status = 'publish';",
                    $user_id
                )
            );
            ...
            if (!empty($saved)) {
                foreach ($saved as $entry) {
                    ...
                    $pending[$point_type][] = buycred_get_pending_payment(
                        (int) $entry->ID
                    );
                }
            } else {
                if ($point_type == '') {
                    $pending[MYCRED_DEFAULT_TYPE_KEY] = [];
                } else {
                    $pending[$point_type] = [];
                }
            }
        ...
        if ($point_type != '' && mycred_point_type_exists($point_type)) {
            if (
                !is_array($pending) ||
                !array_key_exists($point_type, $pending)
            ) {
                return false;
            }
...
if (!function_exists('buycred_gateway')):
    function buycred_gateway($gateway_id = null)
    {
        ...
        if (array_key_exists($gateway_id, $installed)) {
            ...
            // Construct Gateway
            $buycred_gateway = new $class(
                $mycred_modules['solo']['buycred']->gateway_prefs
            );
        }
