...
if (!function_exists('mycred_render_buy_points')):
    function mycred_render_buy_points($atts = [], $button_label = '')
    {
        ...
        // Catch errors
        if (empty($installed)) {
            return 'No gateways installed.';
        } elseif (!empty($gateway) && !array_key_exists($gateway, $installed)) {
            return 'Gateway does not exist.';
        } elseif (empty($buycred->active)) {
            return 'No active gateways found.';
        } elseif (!empty($gateway) && !$buycred->is_active($gateway)) {
            return 'The selected gateway is not active.';
        }
        ...
        $button_label = $mycred->template_tags_user(
            $button_label,
            $recipient_id
        );
        ...
        return '<a href="' .
            esc_url(add_query_arg($args, $current_url)) .
            '" data-gateway="' .
            $gateway .
            '" class="' .
            implode(' ', $classes) .
            '" title="' .
            esc_attr(strip_tags($button_label)) .
            '">' .
            do_shortcode($button_label) .
            '</a>';
    }
endif;
...
if (!function_exists('mycred_render_buy_form_points')):
    function mycred_render_buy_form_points($atts = [], $content = '')
    {
        $settings = mycred_get_buycred_settings();
        ...
        $buyer_id = get_current_user_id();
        $recipient_id = $buyer_id;
        $classes = ['myCRED-buy-form'];
        $amounts = [];
        $gifting = false;

        // Make sure we have a gateway we can use
        if (
            (!empty($gateway) && !mycred_buycred_gateway_is_usable($gateway)) ||
            (empty($gateway) && empty($buycred_instance->active))
        ) {
        ...
        if (!empty($gateway)) {
            $gateway_name = explode(
                ' ',
                $buycred_instance->active[$gateway]['title']
            );
        ...
        if (!empty($buycred_instance->gateway->errors)) {
            foreach ($buycred_instance->gateway->errors as $error) {
                echo '<div class="alert alert-warnng"><p>' .
                    $error .
                    '</p></div>';
            }
        }
...
<div class="row">
	<div class="col-xs-12">
		<form method="post" class="form<?php if ($inline == 1) {
      echo '-inline';
  } ?> <?php echo implode(' ', $classes); ?>" action="">
			<input type="hidden" name="token" value="<?php echo wp_create_nonce(
       'mycred-buy-creds'
   ); ?>" />
			<input type="hidden" name="ctype" value="<?php echo esc_attr($ctype); ?>" />
			<?php if (isset($e_rate) && !empty($e_rate)) {
       $e_rate = base64_encode($e_rate); ?>
			<input type="hidden" name="er_random" value="<?php echo esc_attr(
       $e_rate
   ); ?>" />
			<?php
   } ?>			
...
if (count($amounts) == 0) { ?>
				<input type="text" name="amount" class="form-control" placeholder="<?php echo $mycred->format_creds(
        $minimum
    ); ?>" min="<?php echo $minimum; ?>" value="" />
<?php }

// One amount - this is the amount a user must buy
elseif (count($amounts) == 1) { ?>
				<p class="form-control-static"><?php echo $mycred->format_creds(
        $amounts[0]
    ); ?></p>
				<input type="hidden" name="amount" value="<?php echo esc_attr(
        $amounts[0]
    ); ?>" />
<?php }
...
if ($gifting) {
    $user = get_userdata($recipient_id); ?>
				<div class="form-group">
					...
					<input type="hidden" name="<?php if ($gift_to == 'author') {
         echo 'post_id';
     } else {
         echo 'gift_to';
     } ?>" value="<?php echo absint($recipient_id); ?>" />
				</div>
<?php
}
...
<?php
$active_gateways = apply_filters(
    'mycred_buycred_sort_gateways',
...
else {// If no gateway is set, use the first active gateway
    if (empty($gateway) && count($buycred_instance->active) > 0) {
        $gateway = array_keys($buycred_instance->active)[0];
    } ?>
				<input type="hidden" name="mycred_buy" value="<?php echo esc_attr(
        $gateway
    ); ?>" />
<?php }
?>
...
if (!function_exists('mycred_render_pending_purchases')):
    function mycred_render_pending_purchases($atts = [], $content = '')
    {
        // Must be logged in
        if (!is_user_logged_in()) {
            return $content;
	...
	<table class="table">
		<thead>
			<tr>
				...
				<?php if ($ctype == ''): ?><th class="column-ctype"><?php _e(
    'Point Type',
    'mycred'
); ?></th><?php endif; ?>
...
<?php if (!empty($pending)) {
    ...
    if ($ctype == '') {
        foreach ($pending as $point_type => $entries) {
			...
			<tr>
				<td class="column-transaction-id"><?php echo esc_attr(
        $entry->public_id
    ); ?></td>
				<td class="column-gateway"><?php echo $buycred->adjust_column_content(
        'gateway',
        $entry->payment_id
    ); ?></td>
				<td class="column-amount"><?php echo $buycred->adjust_column_content(
        'amount',
        $entry->payment_id
    ); ?></td>
				<td class="column-cost"><?php echo $buycred->adjust_column_content(
        'cost',
        $entry->payment_id
    ); ?></td>
				<td class="column-ctype"><?php echo mycred_get_point_type_name(
        $entry->point_type,
        false
    ); ?></td>
         ...
         $entry->pay_now_url
     ); ?>"><?php echo $pay_now; ?></a> &bull; <a href="<?php echo esc_url(
    $entry->cancel_url
); ?>"><?php echo $cancel; ?></a>
				</td>
			</tr>
...
<?php }
     ...
     ?>
			<tr>
				<td colspan="<?php if ($ctype == '') {
        echo '6';
    } else {
        echo '5';
    } ?>"><?php _e('No pending payments found', 'mycred'); ?></td>
			</tr>
<?php
} ?>
		</tbody>
