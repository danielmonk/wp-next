...
if (!class_exists('myCRED_buyCRED_Reward_Hook')):
    class myCRED_buyCRED_Reward_Hook extends myCRED_Hook
    {
        ...
        public function assign_buycred_reward($result, $request, $mycred)
        {
            ...
            if (!$result || strpos($ref, 'buy_creds_with') === false) {
                return $result;
            }
        ...
        public function sanitise_preferences($data)
        {
            ...
            foreach ($data as $data_key => $data_value) {
                foreach ($data_value as $key => $value) {
                    if ($data_key == 'creds') {
                        $new_data[$data_key][$key] = !empty($value)
                            ? floatval($value)
                            : 10;
                    } elseif ($data_key == 'log') {
                        $new_data[$data_key][$key] = !empty($value)
                            ? sanitize_text_field($value)
                            : 'Reward for Buying %plural%.';
                    } elseif ($data_key == 'min') {
                        $new_data[$data_key][$key] = !empty($value)
                            ? floatval($value)
                            : 1;
        ...
        public function buycred_reward_setting($data)
        {
            ...
            $this->mycred_type,
            'creds'
        ); ?>" value="<?php echo $this->core->number(
    $hook['creds']
); ?>" class="form-control buycred-reward-creds" />
							</div>
						</div>
						<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
							<div class="form-group">
    ...
    $hook['log']
); ?>" class="form-control buycred-reward-log" />
								<span class="description"><?php echo $this->available_template_tags([
            'general',
        ]); ?></span>
							</div>
						</div>
    ...
    $hook['min']
); ?>" class="form-control buycred-reward-min" />
							</div>
						</div>
						<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
							<div class="form-group">
    ...
    $hook['max']
); ?>" class="form-control buycred-reward-max" />
							</div>
						</div>
					</div>
					<div class="row">
