...
if (!class_exists('myCRED_Payment_Gateway')):
    abstract class myCRED_Payment_Gateway
    {
         ...
         * Construct
         */
        public function __construct($args = [], $gateway_prefs = null)
        {
            ...
            if (
                is_array($gateway_prefs) &&
                array_key_exists($this->id, $gateway_prefs)
            ) {
                $gateway_settings = $gateway_prefs[$this->id];
            } elseif (
                is_object($gateway_prefs) &&
                array_key_exists($this->id, $gateway_prefs->gateway_prefs)
            ) {
            ...
            // Sandbox Mode
            $this->sandbox_mode = isset($this->prefs['sandbox'])
                ? (bool) $this->prefs['sandbox']
                : false;

            // Decode Log Entries
            add_filter(
                'mycred_prep_template_tags',
                [$this, 'decode_log_entries'],
                10,
                2
            );
            add_filter(
                'mycred_parse_log_entry_buy_creds_with_' . $this->id,
                [$this, 'log_entry'],
                10,
                2
            );
        ...
        public function valid_request()
        {
            ...
            if (
                ...
                !wp_verify_nonce($_REQUEST['token'], 'mycred-buy-creds')
            ) {
                return false;
            }
            ...
            $this->buycred_limit = mycred_get_buycred_sale_setup(
                $this->point_type
            );

            $this->transaction_id = isset($_REQUEST['revisit'])
                ? strtoupper(sanitize_text_field($_REQUEST['revisit']))
                : false;
            $this->post_id =
                $this->transaction_id !== false
                    ? buycred_get_pending_payment_id($this->transaction_id)
                    : false;
            ...
            $this->currency = isset($this->prefs['currency'])
                ? $this->prefs['currency']
                : '';
            $this->maximum = -1;
            ...
            if ($this->core->exclude_user($this->buyer_id)) {
                $valid = false;
                $this->errors[] = __(
                    'Buyer is excluded from this point type.',
                    'mycred'
                );
            } elseif ($this->core->exclude_user($this->recipient_id)) {
                $valid = false;
                $this->errors[] = __(
                    'Recipient is excluded from this point type. ',
                    'mycred'
                );
                ...
                !empty($this->buycred_limit['max']) &&
                $this->amount > floatval($this->buycred_limit['max'])
            ) {
                $valid = false;
                $this->errors[] = apply_filters(
                    'buycred_max_amount_error',
                    sprintf(
                        __('The amount must be less than %d.', 'mycred'),
                        $this->buycred_limit['max']
                    ),
                    $this->buycred_limit['max'],
                    $this
                );
        ...
        {
            $exceeds = false;
            $remaining = mycred_user_can_buycred(
                $this->buyer_id,
                $this->point_type
            );
        ...
        public function checkout_header()
        {
            ...
            if ($this->sandbox_mode) {
                $content .=
                    '<div class="checkout-header"><div class="warning">' .
                    esc_js(esc_attr(__('Test Mode', 'mycred'))) .
                    '</div></div>';
            }

            $content .=
                '<div class="checkout-body padded' .
                (!$this->sandbox_mode ? ' no-header' : '') .
                '">';

            return apply_filters(
                'mycred_buycred_checkout_header',
                $content,
                $this
            );
        }
        ...
        public function checkout_footer($button_label = '')
        {
            ...
            if (!empty($this->redirect_fields)) {
                $fields = apply_filters(
                    'mycred_buycred_redirect_fields',
            ...
            $button =
                '<button type="button" id="checkout-action-button" data-act="submit" data-value="" class="btn btn-default">' .
                esc_js($button_label) .
                '</button>';
            ...
            if (!empty($this->toggle_id)) {
                $button =
                    '<button type="button" id="checkout-action-button" data-act="toggle" data-value="' .
                ...
                $button =
                    '<button type="button" id="checkout-action-button" data-act="redirect" data-value="' .
                    $this->redirect_to .
                    '" class="btn btn-default ' .
                    $this->id .
                    '">' .
            ...
            $button = apply_filters(
                'mycred_buycred_checkout_button',
                $button,
                $this
            );
            ...
            if ($button != '') {
                $content .=
                    '<div class="checkout-footer">' . $button . '</div>';
            }

            return apply_filters(
                'mycred_buycred_checkout_footer',
                $content,
                $this
            );
        }
        ...
        {
            if ($title === '') {
                if (isset($this->prefs['title'])) {
                    $title = $this->prefs['title'];
                } elseif (isset($this->prefs['label'])) {
                    $title = $this->prefs['label'];
                }
            }
            ...
            if (isset($this->prefs['logo']) && !empty($this->prefs['logo'])) {
                ...
                isset($this->prefs['logo_url']) &&
                !empty($this->prefs['logo_url'])
            ) {
                $content =
                    '<img src="' . $this->prefs['logo_url'] . '" alt="" />';
            } elseif (
                isset($this->gateway_logo_url) &&
                !empty($this->gateway_logo_url)
            ) {
                $content =
                    '<img src="' . $this->gateway_logo_url . '" alt="" />';
            } elseif ($title !== false) {
                $content =
                    '<h2 class="gateway-title">' . esc_html($title) . '</h2>';
            } else {
                $content = '';
            }

            return apply_filters(
                'mycred_buycred_checkout_logo',
                $content,
                $this
            );
        }
        ...
        public function checkout_order()
        {
            ...
            if ($this->gifting) {
                $table_rows[] =
                    ...
                    esc_js(esc_attr(__('Recipient', 'mycred'))) .
                    ':</strong> ' .
                    esc_html(get_userdata($this->recipient_id)->display_name) .
                    '</td></tr>';
            }

            $table_rows[] =
                '<tr class="total"><td class="item right">' .
                esc_js(esc_attr(__('Cost', 'mycred'))) .
                '</td><td class="cost right">' .
                sprintf('%s %s', $this->cost, $this->prefs['currency']) .
                '</td></tr>';

            $table_rows = apply_filters(
                'mycred_buycred_order_table_rows',
                $table_rows,
                $this
            );
...
<table class="table" cellspacing="0" cellpadding="0">
	<thead>
		<tr>
			<th class="item">' .
                    esc_js(esc_attr(__('Item', 'mycred'))) .
                    '</td>
			<th class="cost right">' .
                    esc_js(esc_attr(__('Amount', 'mycred'))) .
                    '</td>
		</tr>
	</thead>
	<tbody>
        ...
        {
            $content =
                '<h2><span class="text-mutted">' .
                esc_js(esc_attr(__('Transaction ID', 'mycred'))) .
                '</span>' .
                esc_attr($this->transaction_id) .
                '</h2>';

            return apply_filters(
                'mycred_buycred_checkout_txtid',
                $content,
                $this
            );
        }
        ...
        {
            $content =
                '<hr /><div class="cancel"><a href="' .
                $this->get_cancelled($this->transaction_id) .
                '">' .
...
<table>
	<tr>
		<td style="min-width: 100px;"><div class="form-control-static">1 ' .
                    esc_html($mycred->singular()) .
                    '</div></td>
		<td style="width: 10px;"><div class="form-control-static">=</div></td>
		<td><input type="text" name="' .
                    $this->field_name(['exchange' => $type_id]) .
                    '" id="' .
                    $this->field_id(['exchange' => $type_id]) .
                    '" value="' .
                    esc_attr($this->prefs['exchange'][$type_id]) .
                    '" size="8" /> ';

                if (isset($this->prefs['currency'])) {
                    $content .=
                        '<span class="mycred-gateway-' .
                        $this->id .
                        '-currency">' .
                        ($this->prefs['currency'] == ''
                            ? __('Select currency', 'mycred')
                            : esc_attr($this->prefs['currency'])) .
                        '</span>';
                } else {
            ...
            echo apply_filters(
                'mycred_buycred_exchange_rate_field',
                $content,
                $default,
                $this
            );
        ...
        public function add_pending_payment($data)
        {
            ...
            $check = mycred_get_page_by_title(
                $post_title,
                ARRAY_A,
                'buycred_payment'
            );
            if (
                $check === null ||
                (isset($check['post_status']) &&
                    $check['post_status'] == 'trash')
            ) {
                ...
                if ($post_id !== null && !is_wp_error($post_id)) {
                    ...
                    $log_entry = $this->first_comment(
                        sprintf(
                            _x(
                                'Received new request to purchase %s.',
                                '%s is replaced with the point amount and name.',
                                'mycred'
                            ),
                            $mycred->format_number($amount) .
                                ' ' .
                                $mycred->plural()
                        )
                    );
                    $log_entry = apply_filters(
                        'mycred_new_buycred_request_comment_' . $this->id,
                        $log_entry,
                        $data
                    );
        ...
        public function get_recipient_id()
        {
            ...
            return apply_filters(
                'mycred_get_buycred_recipient_id',
                $recipient_id,
                $this
            );
        }
        ...
        public function get_cost(
            ...
            if (
                isset($_REQUEST['er_random']) &&
                !empty($_REQUEST['er_random'])
            ) {
            ...
            if (array_key_exists($point_type, $this->prefs['exchange'])) {
                ...
                $override = mycred_get_user_meta(
                    $this->current_user_id,
                    'mycred_buycred_rates_' . $point_type,
                    '',
                    true
                );
        ...
        public function get_thankyou()
        {
            ...
            if ($this->buycred['thankyou']['use'] == 'page') {
                if (!empty($this->buycred['thankyou']['page'])) {
                    $url = mycred_get_permalink(
                        $this->buycred['thankyou']['page']
                    );
                }
            }
        ...
        public function get_entry($recipient_id = false, $buyer_id = false)
        {
            ...
            if ($recipient_id != $buyer_id) {
                if (
                    $this->buycred['gifting']['members'] == 1 ||
                    $this->buycred['gifting']['authors'] == 1
                ) {
        ...
        public function field_name($field = '')
        {
            ...
            return 'mycred_pref_buycreds[gateway_prefs][' .
                $this->id .
                ']' .
                $field;
        }
        ...
        public function field_id($field = '')
        {
            ...
            return 'mycred-gateway-prefs-' .
                str_replace('_', '-', $this->id) .
                '-' .
                $field;
        }
        ...
        public function start_log()
        {
            ...
            $this->new_log_entry(
                sprintf('Gateway identified itself as "%s"', $this->id)
            );
        ...
        {
            update_option('mycred_buycred_last_call', [
                'gateway' => $this->id,
                'date' => time(),
                'outcome' => $outcome,
                'id' => $id,
...
<head>
	...
	<?php if ($reload) {
     echo '<meta http-equiv="refresh" content="2;url=' . $reload . '" />';
 } ?>
...
<body class="mycred-metabox">
	<div class="row">
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 text-right">
			...
			<p><a href="<?php echo $this->get_cancelled(
       $this->transaction_id
   ); ?>" id="return-where-we-came-from"><?php _e('Cancel', 'mycred'); ?></a></p>
		</div>
	</div>
<?php do_action('mycred_buycred_page_top', $title, $reload, $this->id);
        }
...
<?php
        ...
        public function get_billing_address_form($country_dropdown = false)
        {
            ...
            // Base
            $user_details = [
                'first_name' => isset($_POST['billing']['first_name'])
                    ? $_POST['billing']['first_name']
                    : $user->first_name,
                'last_name' => isset($_POST['billing']['last_name'])
                    ? $_POST['billing']['last_name']
                    : $user->last_name,
                'address1' => isset($_POST['billing']['address1'])
                    ? $_POST['billing']['address1']
                    : $user->address1,
                'address2' => isset($_POST['billing']['address2'])
                    ? $_POST['billing']['address2']
                    : $user->address2,
                'city' => isset($_POST['billing']['city'])
                    ? $_POST['billing']['city']
                    : $user->city,
                'postcode' => isset($_POST['billing']['postcode'])
                    ? $_POST['billing']['postcode']
                    : $user->postcode,
                'state' => isset($_POST['billing']['state'])
                    ? $_POST['billing']['state']
                    : $user->state,
                'country' => isset($_POST['billing']['country'])
            ...
            if (!isset($_POST['billing']['address1'])) {
                if (class_exists('WC_Customer')) {
                    $user_details['first_name'] = get_user_meta(
                        $user->ID,
                        'billing_first_name',
                        true
                    );
                    $user_details['last_name'] = get_user_meta(
                        $user->ID,
                        'billing_last_name',
                        true
                    );
                    $user_details['address1'] = get_user_meta(
                        $user->ID,
                        'billing_address_1',
                        true
                    );
                    $user_details['address2'] = get_user_meta(
                        $user->ID,
                        'billing_address_2',
                        true
                    );
                    $user_details['city'] = get_user_meta(
                        $user->ID,
                        'billing_city',
                        true
                    );
                    $user_details['postcode'] = get_user_meta(
                        $user->ID,
                        'billing_postcode',
                        true
                    );
                    $user_details['state'] = get_user_meta(
                        $user->ID,
                        'billing_state',
                        true
                    );
                ...
                elseif (class_exists('MarketPress')) {
                    ...
                    if (is_array($meta)) {
                        $user_details['address1'] = isset($meta['address1'])
                            ? $meta['address1']
                            : '';
                        $user_details['address2'] = isset($meta['address2'])
                            ? $meta['address2']
                            : '';
                        $user_details['city'] = isset($meta['city'])
                            ? $meta['city']
                            : '';
                        $user_details['postcode'] = isset($meta['zip'])
                            ? $meta['zip']
                            : '';
                        $user_details['state'] = isset($meta['state'])
                            ? $meta['state']
                            : '';
                        $user_details['country'] = isset($meta['country'])
                            ? $meta['country']
                            : '';
                    }
                }
            ...
            // Let others play
            $user_details = apply_filters(
                'mycred_buycred_user_details',
                $user_details,
                $this
            );
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="form-group">
			...
			<input type="text" name="billing[first_name]" id="billing-first-name" value="<?php echo $user_details[
       'first_name'
   ]; ?>" class="form-control<?php if (
    array_key_exists('first_name', $this->errors)
) {
    echo ' error';
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="form-group">
			...
			<input type="text" name="billing[last_name]" id="billing-last-name" value="<?php echo $user_details[
       'last_name'
   ]; ?>" class="form-control<?php if (
    array_key_exists('last_name', $this->errors)
) {
    echo ' error';
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			...
			<input type="text" name="billing[address1]" id="billing-address1" value="<?php echo $user_details[
       'address1'
   ]; ?>" class="form-control<?php if (
    array_key_exists('address1', $this->errors)
) {
    echo ' error';
} ?>" <?php if (in_array('address1', $required_fields)) {
    echo $required;
} else {
    echo $optional;
} ?> autocomplete="off"  />
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			...
			<input type="text" name="billing[address2]" id="billing-address2" value="<?php echo $user_details[
       'address2'
   ]; ?>" class="form-control<?php if (
    array_key_exists('address2', $this->errors)
) {
    echo ' error';
} ?>" <?php if (in_array('address2', $required_fields)) {
    echo $required;
} else {
    echo $optional;
} ?> autocomplete="off"  />
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="form-group">
			...
			<input type="text" name="billing[city]" id="billing-city" value="<?php echo $user_details[
       'city'
   ]; ?>" class="form-control<?php if (
    array_key_exists('city', $this->errors)
) {
    echo ' error';
} ?>" <?php if (in_array('city', $required_fields)) {
    echo $required;
} else {
    echo $optional;
} ?> autocomplete="off"  />
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="form-group">
			...
			<input type="text" name="billing[zip]" id="billing-zip" value="<?php echo $user_details[
       'postcode'
   ]; ?>" class="form-control<?php if (array_key_exists('zip', $this->errors)) {
    echo ' error';
} ?>" <?php if (in_array('zip', $required_fields)) {
    echo $required;
} else {
    echo $optional;
} ?> autocomplete="off"  />
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			...
			<input type="text" name="billing[state]" id="billing-state" value="<?php echo $user_details[
       'state'
   ]; ?>" class="form-control<?php if (
    array_key_exists('state', $this->errors)
) {
    echo ' error';
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			...
			<?php else: ?>
				...
				<input type="text" name="billing[country]" id="billing-country" value="<?php echo $user_details[
        'country'
    ]; ?>" class="form-control<?php if (
    array_key_exists('country', $this->errors)
) {
    echo ' error';
...
<?php
         ...
         * @version 1.0
         */
        public function get_page_redirect($hidden_fields = [], $location = '')
        {
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
       ...
       MYCRED_PURCHASE
			...
			<noscript><input type="submit" name="submit-form" value="<?php printf(
       __('Continue to %s', 'mycred'),
       $this->label
   ); ?>" /></noscript>
...
<script type="text/javascript"><?php if ($this->sandbox_mode) {
    echo '//';
} ?>setTimeout( "document.mycred_<?php echo $id; ?>_request.submit()",2000 );</script>
<?php
        }
        public function form_with_redirect(
            $hidden_fields = [],
            $location = '',
            $logo_url = '',
            $custom_html = '',
        ...
        public function create_unique_transaction_id()
        {
            ...
            do {
                ...
                $query = $wpdb->get_row(
                    $wpdb->prepare(
                        "SELECT * FROM {$mycred_log_table} WHERE ref LIKE %s AND data LIKE %s;",
                        'buy_creds_with_%',
                        "%:\"" . $id . "\";%"
                    )
                );
        ...
        public function verify_token($user_id, $nonce)
        {
            ...
            if (
                substr(
                    wp_hash($i . 'mycred-buy-' . $this->id . $uid, 'nonce'),
                    -12,
                    10
                ) == $nonce
            ) {
            ...
            if (
                substr(
                    wp_hash($i - 1 . 'mycred-buy-' . $this->id . $uid, 'nonce'),
                    -12,
                    10
                ) === $nonce
            ) {
        ...
        public function currencies_dropdown($name = '', $js = '')
        {
            ...
            $currencies = apply_filters(
                'mycred_dropdown_currencies',
                $currencies,
                $this->id
            );
            $currencies = apply_filters(
                'mycred_dropdown_currencies_' . $this->id,
                $currencies
            );
            ...
            foreach ($currencies as $code => $cname) {
                ...
                if (
                    isset($this->prefs[$name]) &&
                    $this->prefs[$name] == $code
                ) {
         ...
         * @version 1.0
         */
        public function IPN_has_required_fields(
            $required_fields = [],
            $method = 'REQUEST'
        ) {
            $missing = 0;
         ...
         * @version 1.1
         */
        public function IPN_is_valid_sale(
            $sales_data_key = '',
            $cost_key = '',
            $transactionid_key = '',
            $method = ''
            ...
            if (
                $result === true &&
                isset($this->prefs['currency']) &&
                $this->prefs['currency'] != $pending_payment['currency']
            ) {
                $result = false;
            }
            ...
            if (
                $result === true &&
                !$this->transaction_id_is_unique($transaction_id)
            ) {
                $result = false;
            }

            $result = apply_filters(
                'mycred_buycred_valid_sale',
                $result,
                $sales_data_key,
                $cost_key,
                $transactionid_key,
        ...
        public function complete_payment(
            ...
            $reference =
                'buy_creds_with_' . str_replace([' ', '-'], '_', $this->id);
            $sales_data = [
            ...
            return apply_filters(
                'mycred_buycred_complete_payment',
                $reply,
                $transaction_id,
                $this
            );
