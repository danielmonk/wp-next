    ...
    class myCRED_Coupon extends myCRED_Object
    {
        ...
        function __construct($coupon_id = null)
        {
            ...
            if (!is_numeric($coupon_id)) {
                ...
                if (
                    $coupon !== null &&
                    $coupon->post_type == MYCRED_COUPON_KEY
                ) {
                    $coupon_id = $coupon->ID;
        ...
        protected function populate($coupon_id = null, $code = '')
        {
            ...
            $this->coupon_code =
                $code == '' ? mycred_get_the_title($this->post_id) : $code;
            $this->code = $this->coupon_code;

            $this->value = mycred_get_coupon_value($this->post_id);
            $this->point_type = mycred_get_post_meta(
                $this->post_id,
                'type',
                true
            );
            ...
            if ($this->expires !== false) {
                $this->expires_unix =
                    strtotime($this->expires . ' midnight') +
                    (DAY_IN_SECONDS - 1);
        ...
        public function get_usage_count()
        {
            ...
            if ($count == '') {
                ...
                $count = $wpdb->get_var(
					...
					WHERE ref = 'coupon' AND ref_id = %d AND ctype = %s;",
                        $this->post_id,
                        $this->point_type
                    )
                );
        ...
        {
            if ($this->post_id === false || $user_id === false) {
                return 'invalid';
            }
            ...
            // Get User max
            if ($can_use === true) {
                $user_count = mycred_get_users_coupon_count(
                    $this->code,
                    $user_id
                );
            ...
            $users_balance = $mycred->get_users_balance(
                $user_id,
                $this->point_type
            );
            ...
            if ($can_use === true) {
                ...
                if ($this->requires_min_type != $this->point_type) {
                    ...
                    $users_balance = $mycred_min->get_users_balance(
                        $user_id,
                        $this->requires_min_type
                    );
                }

                if (
                    $mycred->number($this->requires_min['value']) >
                        $mycred->zero() &&
                    $users_balance <
                        $mycred->number($this->requires_min['value'])
                ) {
                ...
                if ($can_use === true) {
                    if ($this->requires_max_type != $this->point_type) {
                        ...
                        $users_balance = $mycred_max->get_users_balance(
                            $user_id,
                            $this->requires_max_type
                        );
                    }

                    if (
                        $mycred->number($this->requires_max['value']) >
                            $mycred->zero() &&
                        $users_balance >=
                            $mycred->number($this->requires_max['value'])
                    ) {
