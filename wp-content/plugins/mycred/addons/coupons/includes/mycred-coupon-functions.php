...
if (!function_exists('mycred_get_coupon')):
    function mycred_get_coupon($coupon_id = null)
    {
        ...
        if (
            ...
            ($coupon_id === $mycred_coupon->post_id ||
                $coupon_id === $mycred_coupon->code)
        ) {
...
if (!function_exists('mycred_get_coupon_value')):
    function mycred_get_coupon_value($post_id = 0)
    {
        return apply_filters(
            'mycred_coupon_value',
            mycred_get_post_meta($post_id, 'value', true),
            $post_id
        );
    }
endif;
...
if (!function_exists('mycred_get_coupon_user_max')):
    function mycred_get_coupon_user_max($post_id = 0)
    {
        return (int) apply_filters(
            'mycred_coupon_user_max',
            mycred_get_post_meta($post_id, 'user', true),
            $post_id
        );
    }
endif;
...
if (!function_exists('mycred_get_coupon_global_max')):
    ...
    {
        return (int) apply_filters(
            'mycred_coupon_global_max',
            mycred_get_post_meta($post_id, 'global', true),
            $post_id
        );
    }
endif;
...
if (!function_exists('mycred_create_new_coupon')):
    function mycred_create_new_coupon($data = [])
    {
...
if (!function_exists('mycred_get_unique_coupon_code')):
    function mycred_get_unique_coupon_code()
    {
        ...
        do {
            ...
            $query = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT * FROM {$table} WHERE post_title = %s AND post_type = %s;",
                    $id,
                    MYCRED_COUPON_KEY
                )
            );
...
if (!function_exists('mycred_get_coupon_post')):
    function mycred_get_coupon_post($code = '')
    {
        ...
        return apply_filters(
            'mycred_get_coupon_by_code',
            mycred_get_page_by_title(
                strtoupper($code),
                'OBJECT',
                MYCRED_COUPON_KEY
            ),
...
if (!function_exists('mycred_coupon_was_successfully_used')):
    ...
    {
        $results = true;
        $error_codes = apply_filters('mycred_coupon_error_codes', [
            'invalid',
            'expired',
            'user_limit',
            'min',
...
if (!function_exists('mycred_get_coupon_error_message')):
    function mycred_get_coupon_error_message($code = '', $coupon = null)
    {
        ...
        if ($code == 'min' && is_object($coupon)) {
            ...
            $message = str_replace(
                ['%min%', '%amount%'],
                $mycred->format_creds($coupon->requires_min['value']),
                $message
            );
            ...
            $message = str_replace(
                ['%max%', '%amount%'],
                $mycred->format_creds($coupon->requires_max['value']),
                $message
            );
        }

        return apply_filters(
            'mycred_coupon_error_message',
            $message,
            $code,
            $coupon
        );
