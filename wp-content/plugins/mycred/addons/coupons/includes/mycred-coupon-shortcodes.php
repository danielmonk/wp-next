...
if (!function_exists('mycred_render_shortcode_load_coupon')):
    function mycred_render_shortcode_load_coupon($atts, $content = null)
    {
        ...
        // On submits
        if (
            isset($_POST['mycred_coupon_load']['token']) &&
            wp_verify_nonce(
                $_POST['mycred_coupon_load']['token'],
                'mycred-load-coupon' . $user_id
            )
        ) {
            $coupon_code = sanitize_text_field(
                $_POST['mycred_coupon_load']['couponkey']
            );
            ...
            if (isset($coupon_post->ID)) {
                ...
                if (
                    isset($coupon->point_type) &&
                    $coupon->point_type != $mycred->cred_id
                ) {
                ...
                // Success!
                    ...
                    $updated_coupon_value = apply_filters(
                        'mycred_show_custom_coupon_value',
                        $updated_coupon_value
                    );
                    $coupon_settings = mycred_get_addon_settings(
                        'coupons',
                        $coupon->point_type
                    );
                    $message = $mycred->template_tags_amount(
                        $coupon_settings['success'],
                        $updated_coupon_value
                    ); // without filter
                    $message = str_replace(
                        '%amount%',
                        $mycred->format_creds($updated_coupon_value),
                        $message
                    );
                    $output .=
	...
	<form action="" method="post" class="form-inline">
		<div class="form-group">
            ...
            $label .
            '
			<input type="text" name="mycred_coupon_load[couponkey]" placeholder="' .
            esc_attr($placeholder) .
            '" id="mycred-coupon-couponkey" class="form-control" value="" />
		</div>
		<div class="form-group">
			<input type="hidden" name="mycred_coupon_load[token]" value="' .
            wp_create_nonce('mycred-load-coupon' . $user_id) .
            '" />
