...
if (!class_exists('myCRED_Banking_Service_Central')):
    class myCRED_Banking_Service_Central extends myCRED_Service
    {
        ...
        public function run()
        {
            add_filter('mycred_add', [$this, 'mycred_add'], 1, 3);
        ...
        public function mycred_add($reply, $request, $mycred)
        {
            ...
            if ($this->mycred_type != $mycred->cred_id || $reply === false) {
                return $reply;
            }
            ...
            if (
                in_array(
                    ...
                    apply_filters(
                        ...
                        ['interest', 'recurring_payout', 'transfer'],
                        $this
                    )
                )
            ) {
            ...
            $bank_balance = $mycred->get_users_balance(
                $this->prefs['bank_id'],
                $this->mycred_type
            );
            ...
            // User is to gain points
                ...
                if ($bank_balance <= $mycred->zero()) {
                    return false;
                }

                // Second we check if the bank is solvent
                if ($bank_balance - $amount < $mycred->zero()) {
                    return false;
                }
...
<div class="row">
		...
		<div class="row">
			<div class="col-sm-4">
				<div class="form-group">
					<label for="<?php echo $this->field_id('bank_id'); ?>"><?php _e(
    'Select Central Deposit Account',
    'mycred'
); ?></label>
    ...
    'bank_id'
); ?>" class="form-control mycred_bank_id_select2" style="width:100%;">
        ...
        'The ID of the user representing the central bank.',
        'mycred'
    ); ?></span></p>
			</div>
			<div class="col-sm-8">
    ...
    'ignore_manual'
); ?>" value="1"<?php checked($this->prefs['ignore_manual'], 1); ?>> <?php _e(
...
<?php do_action('mycred_banking_central', $this);
        ...
        function sanitise_preferences($post)
        {
            ...
            $new_settings['ignore_manual'] = isset($post['ignore_manual'])
                ? absint($post['ignore_manual'])
                : 0;

            return apply_filters(
                'mycred_banking_save_central',
                $new_settings,
                $this
            );
        }
        ...
        public function get_bank_accounts()
        {
            ...
            echo json_encode(['users' => $users, 'more' => !empty($users)]);
            die();
        }
    }
endif;
