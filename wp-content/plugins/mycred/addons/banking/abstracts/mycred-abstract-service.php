...
if (!class_exists('myCRED_Service')):
    abstract class myCRED_Service
    {
        ...
        function __construct(
            ...
            if (
                $service_prefs !== null &&
                array_key_exists($this->id, $service_prefs)
            ) {
        ...
        {
            wp_die(
                'function myCRED_Service::run() must be over-ridden in a sub-class.'
            );
        }
        ...
        public function get_excluded_user_ids()
        {
            ...
            if (strlen($this->prefs['exclude_ids']) > 0) {
                foreach (
                    explode(',', $this->prefs['exclude_ids'])
                    as $user_id
                ) {
            ...
            if (!empty($this->prefs['exclude_roles'])) {
                ...
                if (!empty($user_ids)) {
                    foreach ($user_ids as $user_id) {
                        if (in_array($user_id, $exclude_ids)) {
                            continue;
                        }
            ...
            if ($this->core->exclude['list'] != '') {
                foreach (
                    explode(',', $this->core->exclude['list'])
                    as $user_id
                ) {
                    $user_id = absint(preg_replace("/[^0-9]/", "", $user_id));
                    if ($user_id === 0 || in_array($user_id, $exclude_ids)) {
                        continue;
                    }
                    $exclude_ids[] = $user_id;
                }
            ...
            if (
                $this->prefs['min_balance'] != '' &&
                $this->core->number($this->prefs['min_balance']) >
                    $this->core->zero()
            ) {
                $format = '%d';
                ...
                $user_ids = $wpdb->get_col(
                    $wpdb->prepare(
                        "SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key = %s AND meta_value < {$format};",
                        $this->mycred_type,
                        $this->prefs['min_balance']
                    )
                );
        ...
        {
            $date =
                $date !== false && strlen($date) > 0 ? date('H:i', $date) : '';
         ...
         * @version 1.0
         */
        public function timeframe_dropdown(
            $pref_id = '',
            $use_select = true,
            $hourly = true
        ) {
