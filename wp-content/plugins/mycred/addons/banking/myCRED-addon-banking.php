...
if (!class_exists('myCRED_Banking_Module')):
    class myCRED_Banking_Module extends myCRED_Module
    {
        ...
        public function module_init()
        {
            ...
            add_action('wp_ajax_run-mycred-bank-service', [
                $this,
                'ajax_handler',
            ]);
        }
        ...
        public function module_admin_init()
        {
            // User Override
            add_action(
                'mycred_user_edit_after_' . $this->mycred_type,
                [$this, 'banking_user_screen'],
                20
            );
        }
        ...
        public function call($call, $callback, $return = null)
        {
            ...
            if (is_array($callback) && class_exists($callback[0])) {
                ...
                if (in_array($call, $methods)) {
                    $new = new $class(
                        isset($this->service_prefs) ? $this->service_prefs : [],
                        $this->mycred_type
                    );
        ...
        public function get($save = false)
        {
            ...
            $services['central'] = [
                ...
                'description' => __(
                    'Instead of creating %_plural% out of thin-air, all payouts are made from a nominated "Central Deposit" account. Any %_plural% a user spends or loses are deposited back into this account. If the central deposit runs out of %_plural%, no %_plural% will be paid out.',
                    'mycred'
                ),
                'cron' => false,
                'icon' => 'dashicons-admin-site',
                'callback' => ['myCRED_Banking_Service_Central'],
            ];
        ...
        {
            $banking_icons = plugins_url(
                'assets/images/gateway-icons.png',
                myCRED_THIS
            );
            ...
            wp_register_script(
                ...
                plugins_url('assets/js/central-deposit-admin.js', myCRED_BANK),
                ['jquery', 'mycred-select2-script'],
                myCRED_VERSION
            );
   ...
   ?>"></span><?php echo $this->core->template_tags_general(
    $data['title']
        ...
        $this->core->template_tags_general($data['description'])
				...
				<label class="subheader" for="mycred-bank-service-<?php echo $key; ?>"><?php _e(
    'Enable',
    'mycred'
); ?></label>
				<ol>
					<li>
						<input type="checkbox" name="<?php echo $this->option_id; ?>[active][]" id="mycred-bank-service-<?php echo $key; ?>" value="<?php echo $key; ?>"<?php if (
    $this->is_active($key)
) {
...
<?php
        ...
        public function sanitize_settings($post)
        {
            ...
            if (!empty($installed)) {
                foreach ($installed as $key => $data) {
                    if (
                        isset($data['callback']) &&
                        isset($post['service_prefs'][$key])
                    ) {
                        // Old settings
                        $old_settings = $post['service_prefs'][$key];

                        // New settings
                        $new_settings = $this->call(
                        ...
                        if (
                            in_array($key, (array) $this->active) &&
                            !in_array($key, $new_post['active'])
                        ) {
                            $this->call(
                                'deactivate',
                                $data['callback'],
                                $new_post['service_prefs'][$key]
                            );
                        }
        ...
        public function ajax_handler()
        {
            ...
            if (
                ...
                wp_verify_nonce(
                    $_REQUEST['_token'],
                    'run-mycred-bank-task' . $this->mycred_type
                )
            ) {
...
if (!function_exists('mycred_load_banking_addon')):
    function mycred_load_banking_addon($modules, $point_types)
    {
        foreach ($point_types as $type => $title) {
            $modules['type'][$type]['banking'] = new myCRED_Banking_Module(
                $type
            );
