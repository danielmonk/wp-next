...
if (!class_exists('myCRED_Ranks_Module')):
    class myCRED_Ranks_Module extends myCRED_Module
    {
        ...
        {
            add_action('mycred_pre_init', [$this, 'module_pre_init']);
            add_action('mycred_init', [$this, 'module_init']);
            add_action(
                'mycred_admin_init',
                [$this, 'module_admin_init'],
                $this->menu_pos
            );
        }
        ...
        {
            add_filter('mycred_has_tags', [$this, 'user_tags']);
            add_filter('mycred_parse_tags_user', [$this, 'parse_rank'], 10, 3);
            add_filter('mycred_post_type_excludes', [$this, 'exclude_ranks']);
            add_filter(
                'mycred_add_finished',
                [$this, 'balance_adjustment'],
                20,
                3
            );
            add_action('mycred_zero_balances', [$this, 'zero_balance_action']);
            add_action('mycred_finish_without_log_entry', [
                $this,
                'balance_adjustment_without_log',
            ]);
        }
        ...
        public function module_init()
        {
            ...
            add_action('pre_get_posts', [$this, 'adjust_wp_query'], 20);
            add_action(
            ...
            if (class_exists('bbPress')) {
                add_action('bbp_theme_after_reply_author_details', [
                    $this,
                    'insert_rank_bb_reply',
                ]);
                add_action('bbp_template_after_user_profile', [
                    $this,
                    'insert_rank_bb_profile',
                ]);
            }
            ...
            add_shortcode(
                ...
                'mycred_render_users_of_rank'
            );
            add_shortcode(
                MYCRED_SLUG . '_users_of_all_ranks',
                'mycred_render_users_of_all_ranks'
            );
            add_shortcode(
                MYCRED_SLUG . '_list_ranks',
                'mycred_render_rank_list'
            );

            // Admin Management items
            add_action('wp_ajax_mycred-calc-totals', [
                $this,
                'calculate_totals',
            ]);
        }
        ...
        public function module_admin_init()
        {
            ...
            add_filter('admin_url', [$this, 'replace_add_new_rank_url'], 10, 3);
            ...
            add_action('wp_ajax_mycred-action-delete-ranks', [
                $this,
                'action_delete_ranks',
            ]);
            add_action('wp_ajax_mycred-action-assign-ranks', [
                $this,
                'action_assign_ranks',
            ]);

            add_filter(
                'mycred_users_balance_column',
                [$this, 'custom_user_column_content'],
                10,
                3
            );

            add_action(
                'mycred_user_edit_after_balance',
                [$this, 'show_rank_in_user_editor'],
                40,
                3
            );
            add_action(
                'personal_options_update',
                [$this, 'save_manual_rank'],
                50
            );
            add_action(
            ...
            add_filter(
                'manage_' . MYCRED_RANK_KEY . '_posts_columns',
                [$this, 'adjust_column_headers'],
                50
            );
            add_action(
                'manage_' . MYCRED_RANK_KEY . '_posts_custom_column',
                [$this, 'adjust_column_content'],
                10,
                2
            );
            add_action(
                'save_post_' . MYCRED_RANK_KEY,
                [$this, 'save_rank'],
                10,
                2
            );
        ...
        public function is_manual_mode($type_id)
        {
            ...
            if (
                !empty($setting['rank']['base']) &&
                $setting['rank']['base'] == 'manual'
            ) {
        ...
        {
            add_action('mycred_management_prefs', [$this, 'rank_management']);
            add_action('mycred_after_core_prefs', [
                $this,
                'after_general_settings',
            ]);
            add_filter(
                'mycred_save_core_prefs',
                [$this, 'sanitize_extra_settings'],
                90,
                3
            );
            ...
            if (count($this->point_types) > 1) {
                ...
                foreach ($this->point_types as $type_id => $label) {
                    add_action(
                        'mycred_management_prefs' . $type_id,
                        [$this, 'rank_management'],
                        $priority
                    );

                    add_action(
                        'mycred_after_core_prefs' . $type_id,
                        [$this, 'after_general_settings'],
                        $priority
                    );
                    add_filter(
                        'mycred_save_core_prefs' . $type_id,
                        [$this, 'sanitize_extra_settings'],
                        $priority,
                        3
                    );
        ...
        public function register_ranks()
        {
            if (
                isset($_GET['ctype']) &&
                array_key_exists($_GET['ctype'], $this->point_types) &&
                isset($_GET['post_type']) &&
                $_GET['post_type'] == MYCRED_RANK_KEY
            ) {
                $name = sprintf(
                    __('Ranks for %s', 'mycred'),
                    $this->point_types[$_GET['ctype']]
                );
            } else {
            ...
            if (
                isset($this->rank['support']['content']) &&
                $this->rank['support']['content']
            ) {
                $supports[] = 'editor';
            }
            if (
                isset($this->rank['support']['excerpt']) &&
                $this->rank['support']['excerpt']
            ) {
                $supports[] = 'excerpt';
            }
            if (
                isset($this->rank['support']['comments']) &&
                $this->rank['support']['comments']
            ) {
                $supports[] = 'comments';
            }
            if (
                isset($this->rank['support']['page-attributes']) &&
                $this->rank['support']['page-attributes']
            ) {
                $supports[] = 'page-attributes';
            }
            if (
                isset($this->rank['support']['custom-fields']) &&
                $this->rank['support']['custom-fields']
            ) {
            ...
            if ($this->rank['public'] && !empty($this->rank['slug'])) {
                $args['rewrite'] = ['slug' => $this->rank['slug']];
            }

            register_post_type(
                MYCRED_RANK_KEY,
                apply_filters('mycred_register_ranks', $args, $this)
            );
        }
        ...
        public function populate_current_account()
        {
            ...
            if (!empty($mycred_current_account->balance)) {
                foreach (
                    $mycred_current_account->balance
                    as $type_id => $balance
                ) {
                    if ($balance === false) {
                        continue;
        ...
        public function replace_add_new_rank_url($url, $path, $blog_id)
        {
            ...
            if ($path == 'post-new.php?post_type=' . MYCRED_RANK_KEY) {
                if (isset($_GET['ctype'])) {
                    return get_site_url($blog_id, 'wp-admin/', 'admin') .
                    ...
                    $post->post_type == MYCRED_RANK_KEY &&
                    mycred_get_post_meta($post->ID, 'ctype', true) != ''
                ) {
                    return get_site_url($blog_id, 'wp-admin/', 'admin') .
                        'post-new.php?post_type=' .
                        MYCRED_RANK_KEY .
        ...
        public function parent_file($parent = '')
        {
            ...
            if (
                ...
                if (
                    isset($_GET['ctype']) &&
                    sanitize_key($_GET['ctype']) != MYCRED_DEFAULT_TYPE_KEY
                ) {
            ...
            elseif (
                $pagenow == 'post.php' &&
                isset($_GET['post']) &&
                mycred_get_post_type($_GET['post']) == MYCRED_RANK_KEY
            ) {
                if (
                    isset($_GET['ctype']) &&
                    $_GET['ctype'] != MYCRED_DEFAULT_TYPE_KEY
                ) {
                    return MYCRED_SLUG . '_' . sanitize_key($_GET['ctype']);
                }

                $point_type = mycred_get_post_meta(
                    $_GET['post'],
                    'ctype',
                    true
                );
        ...
        public function calculate_totals()
        {
            ...
            if (
                isset($_POST['ctype']) &&
                mycred_point_type_exists(sanitize_key($_POST['ctype']))
            ) {
            ...
            $users = $wpdb->get_col(
				...
				FROM {$wpdb->usermeta} 
				WHERE meta_key = %s",
                    mycred_get_meta_key($point_type)
                )
            );
            ...
            if (!empty($users)) {
                ...
                foreach ($users as $user_id) {
                    $total = mycred_calculate_users_total(
                        $user_id,
                        $point_type
                    );
            ...
            wp_send_json(
                sprintf(
                    __('Completed - Total of %d users effected', 'mycred'),
                    $count
                )
            );
        }
        ...
        public function post_status_change($new_status, $old_status, $post)
        {
            ...
            if (
                ...
                wp_cache_delete(
                    'ranks-published-count-' . $point_type,
                    MYCRED_SLUG
                );
                ...
                foreach ($this->point_types as $type_id => $label) {
                    ...
                    if ($rank_id === false) {
                        ...
                        if ($type_id == MYCRED_DEFAULT_TYPE_KEY) {
                            $content = str_replace(
                                '%rank%',
                                mycred_get_the_title($rank_id),
                                $content
                            );
                            $content = str_replace(
                                '%rank_logo%',
                                mycred_get_rank_logo($rank_id),
                                $content
                            );
                        } else {
                            $content = str_replace(
        ...
        public function insert_rank_header()
        {
            ...
            foreach ($mycred_types as $type_id) {
                ...
                // No settings
                if (!isset($mycred->rank['bb_location'])) {
                    continue;
                }

                // Not shown
                if (
                    !in_array($mycred->rank['bb_location'], ['top', 'both']) ||
                    $mycred->rank['bb_template'] == ''
                ) {
                    continue;
                }
                ...
                $template = str_replace(
                    '%rank_title%',
                    mycred_get_the_title($users_rank),
                    $template
                );
                $template = str_replace(
                    '%rank_logo%',
                    mycred_get_rank_logo($users_rank, 'full'),
                    $template
                );

                $template = $mycred->template_tags_general($template);
                $template =
                    '<div class="mycred-my-rank ' .
                    $type_id .
                    '">' .
                    $template .
                    '</div>';
        ...
        public function insert_rank_profile()
        {
            ...
            foreach ($mycred_types as $type_id) {
                ...
                // Not shown
                if (
                    !in_array($mycred->rank['bb_location'], [
                        'profile_tab',
                        'both',
                    ]) ||
                    $mycred->rank['bb_template'] == ''
                ...
                $template = str_replace(
                    '%rank_logo%',
                    mycred_get_rank_logo($users_rank),
                    $template
                );
...
<?php
        ...
        public function insert_rank_bb_reply()
        {
            ...
            foreach ($mycred_types as $type_id) {
                ...
                // Not shown
                if (
                    !in_array($mycred->rank['bp_location'], [
                        'reply',
                        'both',
                    ]) ||
                    $mycred->rank['bp_template'] == ''
                ) {
                    continue;
                }
        ...
        public function custom_user_column_content($balance, $user_id, $type)
        {
            ...
            if ($rank !== false) {
                ...
                $balance .=
                    '<small style="display:block;">' .
                    sprintf('<strong>%s:</strong> -', __('Rank', 'mycred')) .
                    '</small>';
            }
        ...
        public function show_rank_in_user_editor($point_type, $user, $data)
        {
            ...
            if (!mycred_have_ranks($point_type)) {
                echo '<div class="balance-desc current-rank"><em>' .
                    __('No ranks exists.', 'mycred') .
                    '</em></div>';
                return;
            }
            ...
            if (
                $this->is_manual_mode($point_type) &&
                mycred_is_admin(null, $point_type)
            ) {
                $ranks = mycred_get_ranks('publish', '-1', 'DESC', $point_type);
                echo '<div class="balance-desc current-rank"><select name="rank-' .
                    $point_type .
                    '" id="mycred-rank">';
                ...
                foreach ($ranks as $rank) {
                    ...
                    if (
                        !empty($users_rank) &&
                        $users_rank->post_id == $rank->post_id
                    ) {
                        echo ' selected="selected"';
                    }
                ...
                echo '<div class="balance-desc current-rank">' .
                    sprintf(
                        '<strong>%s:</strong> %s',
                        __('Rank', 'mycred'),
                        $rank_title
        ...
        public function save_manual_rank($user_id)
        {
            ...
            foreach ($point_types as $type_key => $label) {
                if ($this->is_manual_mode($type_key)) {
                    if (
                        isset($_POST['rank-' . $type_key]) &&
                        mycred_is_admin(null, $type_key)
                    ) {
                        // Get users current rank for comparison
                        $users_rank = mycred_get_users_rank(
                            $user_id,
                            $type_key
                        );
        ...
        public function enqueue_scripts()
        {
            ...
            if (strpos('edit-' . MYCRED_RANK_KEY, $screen->id) > -1) {
                ...
                if (
                    ...
                    wp_localize_script('mycred-rank-tweaks', 'myCRED_Ranks', [
            ...
            if (strpos(MYCRED_RANK_KEY, $screen->id) > -1) {
                ...
                add_filter(
                    'postbox_classes_' . MYCRED_RANK_KEY . '_mycred-rank-setup',
...
<?php
            ...
            if (in_array(substr($screen->id, -9), ['_settings', '-settings'])) {
                wp_localize_script('mycred-rank-management', 'myCRED_Ranks', [
                    'ajaxurl' => admin_url('admin-ajax.php'),
                    'token' => wp_create_nonce(
                        'mycred-management-actions-roles'
                    ),
                    'working' => esc_attr__('Processing...', 'mycred'),
                    'confirm_del' => esc_attr__(
                        'Warning! All ranks will be deleted! This can not be undone!',
                        'mycred'
                    ),
                    'confirm_assign' => esc_attr__(
                        'Are you sure you want to re-assign user ranks?',
                        'mycred'
                    ),
                ]);
        ...
        public function adjust_wp_query($query)
        {
            ...
            if (!is_admin()) {
                ...
                if (
                    !is_post_type_archive(MYCRED_RANK_KEY) ||
                    !$query->is_main_query()
                ) {
                    return;
                }
            ...
            // Admin Queries
                ...
                if (
                    ...
                    $query->query['post_type'] != MYCRED_RANK_KEY
                ) {
                    return;
                }
			...
			<div class="row">
				<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
					<div class="form-group">
						<label for="mycred-rank-min"><?php _e(
          'Minimum Balance Requirement',
          'mycred'
      ); ?></label>
						<input type="text" name="mycred_rank[mycred_rank_min]" id="mycred-rank-min" class="form-control" value="<?php echo esc_attr(
          $rank->minimum
      ); ?>" />
					</div>
				</div>
				<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
					<div class="form-group">
						<label for="mycred-rank-max"><?php _e(
          'Maximum Balance Requirement',
          'mycred'
      ); ?></label>
						<input type="text" name="mycred_rank[mycred_rank_max]" id="mycred-rank-max" class="form-control" value="<?php echo esc_attr(
          $rank->maximum
      ); ?>" />
					</div>
				</div>
			...
			<div class="row">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<div class="form-group">
						...
						<?php mycred_types_select_from_dropdown(
          'mycred_rank[ctype]',
          'mycred-rank-point-type',
          $type,
          false,
          '  class="form-control"'
      ); ?>
...
if (!empty($all_ranks)) {
    ...
    foreach ($all_ranks as $rank) {
        if ($rank->minimum == '') {
            $rank->minimum = __('Not Set', 'mycred');
        }
        if ($rank->maximum == '') {
            $rank->maximum = __('Not Set', 'mycred');
        }
...
<div class="body" style="display:none;">
	...
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			...
			<div class="form-group">
				<div class="checkbox">
					<label><input type="checkbox" value="1" checked="checked" disabled="disabled" /> <?php _e(
         'Title',
         'mycred'
     ); ?></label>
				</div>
				<div class="checkbox">
					<label><input type="checkbox" value="1" checked="checked" disabled="disabled" /> <?php echo $mycred->core->template_tags_general(
         __('%plural% requirement', 'mycred')
     ); ?></label>
				</div>
				<div class="checkbox">
					<label><input type="checkbox" value="1" checked="checked" disabled="disabled" /> <?php _e(
    ...
    $prefs['support']['content'],
    1
); ?> value="1" /> <?php _e('Content', 'mycred'); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id([
         'support' => 'excerpt',
    ...
    $prefs['support']['excerpt'],
    1
); ?> value="1" /> <?php _e('Excerpt', 'mycred'); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id([
         'support' => 'comments',
    ...
    $prefs['support']['comments'],
    1
); ?> value="1" /> <?php _e('Comments', 'mycred'); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id([
    ...
    $prefs['support']['page-attributes'],
    1
); ?> value="1" /> <?php _e('Page Attributes', 'mycred'); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id([
    ...
    $prefs['support']['custom-fields'],
    1
); ?> value="1" /> <?php _e('Custom Fields', 'mycred'); ?></label>
				</div>
			</div>
		</div>
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
    ...
    'public'
); ?>" id="<?php echo $this->field_id('public'); ?>" <?php checked(
    $prefs['public'],
    1
); ?> value="1" /> <?php _e('Make Ranks Public', 'mycred'); ?></label>
				</div>
			</div>
			<div class="form-group">
				<label class="subheader" for="<?php echo $this->field_id('slug'); ?>"><?php _e(
    'Rank SLUG',
    'mycred'
); ?></label>
				<input type="text" name="<?php echo $this->field_name(
        'slug'
    ); ?>" id="<?php echo $this->field_id(
    'slug'
); ?>" value="<?php echo esc_attr($prefs['slug']); ?>" class="form-control" />
				<p><span class="description"><?php _e(
        'If you have selected to make Ranks public, you can select what rank archive URL slug you want to use. Ignored if Ranks are not set to be public.',
        'mycred'
    ); ?></span></p>
			</div>
			<div class="form-group">
				<label for="<?php echo $this->field_id('order'); ?>"><?php _e(
    'Display Order',
    'mycred'
); ?></label>
				<select name="<?php echo $this->field_name(
        'order'
    ); ?>" id="<?php echo $this->field_id('order'); ?>" class="form-control">
<?php
...
?>
				...
				<p><span class="description"><?php _e(
        'Option to set in which order Ranks should be shown on the archive page.',
        'mycred'
    ); ?></span></p>
			</div>
		</div>
		...
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<div class="radio">
					<label for="<?php echo $this->field_id([
         'base' => 'manual',
     ]); ?>"><input type="radio" name="<?php echo $this->field_name('base'); ?>" id="<?php echo $this->field_id(['base' => 'manual']); ?>"<?php checked($prefs['base'], 'manual'); ?> value="manual" /> <?php _e('Manual Mode', 'mycred'); ?></label>
				</div>
				<p><span class="description"><?php _e(
        'Ranks are assigned manually for each user.',
        'mycred'
    ); ?></span></p>
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<div class="radio">
					<label for="<?php echo $this->field_id([
         'base' => 'current',
     ]); ?>"><input type="radio" name="<?php echo $this->field_name('base'); ?>" id="<?php echo $this->field_id(['base' => 'current']); ?>"<?php checked($prefs['base'], 'current'); ?> value="current" /> <?php _e('Based on Current Balances', 'mycred'); ?></label>
				</div>
				<p><span class="description"><?php _e(
        'Users can be promoted or demoted depending on where their balance fits in amongst your ranks.',
        'mycred'
    ); ?></span></p>
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<div class="radio">
					<label for="<?php echo $this->field_id([
         'base' => 'total',
     ]); ?>"><input type="radio" name="<?php echo $this->field_name('base'); ?>" id="<?php echo $this->field_id(['base' => 'total']); ?>"<?php checked($prefs['base'], 'total'); ?> value="total" /> <?php _e('Based on Total Balances', 'mycred'); ?></label>
				</div>
				<p><span class="description"><?php _e(
        'Users can only be promoted and gain higher ranks even if their balance decreases.',
        'mycred'
    ); ?></span></p>
			</div>
		</div>
	...
	<div class="row" id="mycred-rank-based-on-wrapper" style="<?php echo $box; ?>">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<p><span class="description"><?php _e(
       'Use this button to calculate or recalculate each individual users total balance if you think your users total balances are incorrect, or if you switch from Ranks being based on users current balance to total balance.',
       'mycred'
   ); ?></span></p>
			<p><input type="button" name="mycred-update-totals" data-type="<?php echo $mycred->mycred_type; ?>" id="mycred-update-totals" value="<?php _e('Calculate Totals', 'mycred'); ?>" class="button button-large button-<?php if (
    $prefs['base'] == 'current'
) {
    echo 'secondary';
} else {
    echo 'primary';
} ?>"<?php if ($prefs['base'] == 'current') {
...
?>
			...
			<div class="form-group">
				<label for="<?php echo $this->field_id('bb_template'); ?>"><?php _e(
    'Template',
    'mycred'
); ?></label>
				<textarea name="<?php echo $this->field_name(
        'bb_template'
    ); ?>" id="<?php echo $this->field_id(
    'bb_template'
); ?>" rows="5" cols="50" class="form-control"><?php echo esc_attr(
    $prefs['bb_template']
); ?></textarea>
				<p><span class="description"><?php _e(
        'Template to use when showing a users Rank in BuddyPress. Use %rank_title% for the title and %rank_logo% to show the rank logo. HTML is allowed.',
        'mycred'
    ); ?></span></p>
				<?php else: ?>
...
?>
			...
			<div class="form-group">
				<label for="<?php echo $this->field_id('bp_template'); ?>"><?php _e(
    'Template',
    'mycred'
); ?></label>
				<textarea name="<?php echo $this->field_name(
...
jQuery(function($){
	...
	var mycred_calc = function( button, pointtype ) {
		...
		$.ajax({
			...
			beforeSend : function() {
				button.attr( 'disabled', 'disabled' ).removeClass( 'button-primary' ).addClass( 'button-seconday' ).val( '<?php echo esc_js(
...
<?php
        ...
        {
            $new_data['rank']['support']['content'] = isset(
                $data['rank']['support']['content']
            )
                ? true
                : false;
            $new_data['rank']['support']['excerpt'] = isset(
                $data['rank']['support']['excerpt']
            )
                ? true
                : false;
            $new_data['rank']['support']['comments'] = isset(
                $data['rank']['support']['comments']
            )
                ? true
                : false;
            $new_data['rank']['support']['page-attributes'] = isset(
                $data['rank']['support']['page-attributes']
            )
                ? true
                : false;
            $new_data['rank']['support']['custom-fields'] = isset(
                $data['rank']['support']['custom-fields']
            )
                ? true
                : false;

            $new_data['rank']['base'] = sanitize_key($data['rank']['base']);
            $new_data['rank']['public'] = isset($data['rank']['public'])
                ? true
                : false;
            $new_data['rank']['slug'] = isset($data['rank']['slug'])
                ? sanitize_title($data['rank']['slug'])
                : '';
            $new_data['rank']['order'] = isset($data['rank']['order'])
                ? sanitize_text_field($data['rank']['order'])
                : '';

            $new_data['rank']['bb_location'] = sanitize_text_field(
                $data['rank']['bb_location']
            );
            $new_data['rank']['bb_template'] = wp_kses_post(
                $data['rank']['bb_template']
            );
            $new_data['rank']['bp_location'] = sanitize_text_field(
                $data['rank']['bp_location']
            );
            $new_data['rank']['bp_template'] = wp_kses_post(
                $data['rank']['bp_template']
            );
        ...
        public function rank_management($mycred)
        {
            ...
            if (
                $this->core->is_multisite &&
                $GLOBALS['blog_id'] > 1 &&
                !$this->core->use_master_template
            ) {
...
<ol id="myCRED-rank-actions" class="inline">
	...
	<li>
		...
		<div class="h2"><input type="button" id="mycred-manage-action-reset-ranks" data-type="<?php echo $mycred->mycred_type; ?>" value="<?php _e('Remove All Ranks', 'mycred'); ?>" class="button button-large large <?php if (
    $reset_block
) {
    echo '" disabled="disabled';
} else {
    ...
    !$this->is_manual_mode($mycred->mycred_type)
): ?> <input type="button" id="mycred-manage-action-assign-ranks" data-type="<?php echo $mycred->mycred_type; ?>" value="<?php _e(
    'Assign Ranks to Users',
    'mycred'
); ?>" class="button button-large large <?php if ($reset_block) {
    echo '" disabled="disabled';
} ?>" /></div><?php endif; ?>
	</li>
</ol>
<?php
        ...
        public function action_delete_ranks()
        {
            ...
            wp_send_json(['status' => 'OK', 'rows' => $rows]);
        }
        ...
        public function action_assign_ranks()
        {
            ...
            wp_send_json(['status' => 'OK', 'rows' => $adjustments]);
        }
    }
endif;
