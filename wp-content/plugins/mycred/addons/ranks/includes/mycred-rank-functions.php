    ...
    function mycred_get_published_ranks_count($point_type = null)
    {
        ...
        if ($point_type === null) {
            ...
            if ($count === false) {
                ...
                if (
                    $point_type !== null &&
                    mycred_point_type_exists(sanitize_key($point_type))
                ) {
                ...
                $count = $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT COUNT(*) FROM {$posts} ranks {$type_filter} WHERE ranks.post_type = %s AND ranks.post_status = 'publish';",
                        MYCRED_RANK_KEY
                    )
                );
                if ($count === null) {
        ...
        return apply_filters(
            'mycred_get_published_ranks_count',
            $count,
            $point_type
        );
    }
...
if (!function_exists('mycred_get_rank_object_id')):
    function mycred_get_rank_object_id($identifier = null)
    {
        ...
        if (
            absint($identifier) !== 0 &&
            mycred_get_post_type(absint($identifier)) == MYCRED_RANK_KEY
        ) {
            $rank_id = absint($identifier);
        } else {
            $rank = mycred_get_page_by_title(
                $identifier,
                OBJECT,
                MYCRED_RANK_KEY
            );
            if (
                isset($rank->post_type) &&
...
if (!function_exists('mycred_get_users_rank_id')):
    function mycred_get_users_rank_id(
        ...
        if (
            isset($account_object->ranks) &&
            $account_object->balance[$point_type] !== false &&
            $account_object->balance[$point_type]->rank !== false
        ) {
            return $account_object->balance[$point_type]->rank->post_id;
        }

        $rank_id = mycred_get_user_meta(
            $user_id,
            MYCRED_RANK_KEY,
...
if (!function_exists('mycred_save_users_rank')):
    function mycred_save_users_rank(
        $user_id = null,
        $rank_id = null,
        $point_type = MYCRED_DEFAULT_TYPE_KEY
    ) {
        if ($user_id === null || $rank_id === null) {
            return false;
        }
...
if (!function_exists('mycred_find_users_rank')):
    function mycred_find_users_rank(
        ...
        if ($balance_object === false) {
            return false;
        }

        if (isset($account_object->ranks)) {
            $current_rank_id =
                $balance_object->rank !== false
                    ? $balance_object->rank->post_id
                    : false;
            $current_rank =
                $balance_object->rank !== false ? $balance_object->rank : false;
        } else {
            $current_rank_id = mycred_get_user_meta(
                $user_id,
                MYCRED_RANK_KEY,
                $point_type != MYCRED_DEFAULT_TYPE_KEY ? $point_type : '',
                true
            ...
            $current_rank =
                $current_rank_id != ''
                    ? mycred_get_rank($current_rank_id)
                    : false;
        }
        ...
        if (
            $current_rank !== false &&
            $balance >= $current_rank->minimum &&
            $balance <= $current_rank->maximum
        ) {
            return false;
        }
        ...
        $balance_format = isset($balance_object->point_type->sql_format)
            ? $balance_object->point_type->sql_format
            : '%d';
        ...
        if ($act === true && isset($results->rank_id)) {
            // Demotions
            if (
                $results->current_id !== false &&
                $current_rank !== false &&
                $current_rank->maximum > $results->maximum
            ) {
                do_action(
...
if (!function_exists('mycred_assign_ranks')):
    function mycred_assign_ranks($point_type = MYCRED_DEFAULT_TYPE_KEY)
    {
        ...
        $balance_format = isset($type_object->sql_format)
            ? $type_object->sql_format
            : '%d';
            ...
            foreach ($ranks as $rank) {
                $count += $wpdb->query(
					...
					WHERE ranks.meta_key = %s 
						AND balance.meta_value BETWEEN {$balance_format} AND {$balance_format};",
                        $balance_key,
                        $rank->post_id,
                        $rank_key,
                        $rank->minimum,
    ...
    function mycred_get_ranks(
        ...
        if ($ranks === false) {
            ...
            $rank_ids = $wpdb->get_col(
				...
				ORDER BY min.meta_value+0 {$order} {$limit};",
                    MYCRED_RANK_KEY,
                    $status,
                    $point_type
                )
...
if (!function_exists('mycred_get_users_of_rank')):
    function mycred_get_users_of_rank(
        ...
        $rank_id = mycred_get_rank_object_id($rank_identifier);
        if ($rank_id === false || !mycred_point_type_exists($point_type)) {
            return false;
        }
        ...
        if ($users === false) {
            ...
            $rank_meta_key = mycred_get_meta_key(
                MYCRED_RANK_KEY,
                $point_type != MYCRED_DEFAULT_TYPE_KEY ? $point_type : ''
            );
            $balance_key = mycred_get_meta_key($point_type);
            ...
            $users = $wpdb->get_results(
				...
				ORDER BY creds.meta_value+0 DESC LIMIT 25;",
                    $rank_meta_key,
                    $balance_key,
                    $rank_id
                )
