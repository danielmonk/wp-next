...
if (!class_exists('myCRED_Rank')):
    class myCRED_Rank extends myCRED_Object
    {
        ...
        public function __construct($rank_id = null)
        {
            ...
            if (mycred_get_post_type($rank_id) != MYCRED_RANK_KEY) {
                return;
            }
        ...
        protected function populate($rank_id = null)
        {
            ...
            $this->maximum = mycred_get_post_meta(
                $this->post_id,
                'mycred_rank_max',
                true
            );
        ...
        public function user_has_rank($user_id = false)
        {
            ...
            $users_rank = mycred_get_user_meta(
                $user_id,
                MYCRED_RANK_KEY,
                $post_type != MYCRED_DEFAULT_TYPE_KEY ? $post_type : '',
                true
            );
        ...
        public function assign($user_id = false)
        {
            ...
            if ($this->user_has_rank($user_id)) {
                return true;
            }

            $value = apply_filters(
                'mycred_rank_user_value',
                $this->post_id,
                $user_id,
                $this
            );

            return mycred_update_user_meta(
                $user_id,
                MYCRED_RANK_KEY,
        ...
        public function divest($user_id = false)
        {
            ...
            if ($users_rank != '') {
                $results = mycred_delete_user_meta(
                    $user_id,
                    MYCRED_RANK_KEY,
                    $post_type != MYCRED_DEFAULT_TYPE_KEY ? $post_type : ''
                );
