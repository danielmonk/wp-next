...
if (!function_exists('mycred_render_my_rank')):
    function mycred_render_my_rank($atts, $content = '')
    {
        ...
        if (empty($account_object->balance[$ctype]->rank)) {
            return;
        }
        ...
        if (!empty($show)) {
            $content =
                '<div class="mycred-my-rank">' . implode(' ', $show) . '</div>';
        }
...
if (!function_exists('mycred_render_users_of_rank')):
    function mycred_render_users_of_rank($atts, $row_template = null)
    {
        ...
        if ($row_template === null || empty($row_template)) {
            $row_template =
                '<p class="user-row">%user_profile_link% with %balance% %_plural%</p>';
        }
        ...
        if (!empty($users)) {
            ...
            if ($wrap != 'table' && !empty($wrap)) {
                $output .=
                    '<' . $wrap . ' class="mycred-users-of-rank-wrapper">';
            }

            // Loop
            foreach ($users as $user) {
                $output .= $mycred->template_tags_user(
                    $row_template,
                    false,
                    $user
                );
...
if (!function_exists('mycred_render_rank_list')):
    function mycred_render_rank_list($atts, $row_template = null)
    {
        ...
        if (!empty($all_ranks)) {
            ...
            if ($row_template === null || empty($row_template)) {
                $row_template =
                    '<p>%rank% <span class="min">%min%</span> - <span class="max">%max%</span></p>';
            }
            ...
            foreach ($all_ranks as $rank) {
                ...
                $row = str_replace(
                    '%rank_logo%',
                    mycred_get_rank_logo($rank->post_id),
                    $row
                );
                $row = str_replace(
                    '%min%',
                    $mycred->format_creds($rank->minimum),
                    $row
                );
                $row = str_replace(
                    '%max%',
                    $mycred->format_creds($rank->maximum),
                    $row
                );
