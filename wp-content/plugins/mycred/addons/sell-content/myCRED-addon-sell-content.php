...
if (!class_exists('myCRED_Sell_Content_Module')):
    class myCRED_Sell_Content_Module extends myCRED_Module
    {
        ...
        {
            parent::__construct('myCRED_Sell_Content_Module', [
                ...
                'defaults' => [
                    ...
                    'templates' => [
                        'members' =>
                            '<div class="text-center"><h3>Premium Content</h3><p>Buy access to this content.</p><p>%buy_button%</p></div>',
                        'visitors' =>
                            '<div class="text-center"><h3>Premium Content</h3><p>Login to buy access to this content.</p></div>',
                        'cantafford' =>
                            '<div class="text-center"><h3>Premium Content</h3><p>Buy access to this content.</p><p><strong>Insufficient Funds</strong></p></div>',
                    ],
                ],
                'add_to_core' => true,
            ]);
            ...
            if (!is_array($this->sell_content['type'])) {
                $this->sell_content['type'] = [$this->sell_content['type']];
            }
        }
        ...
        public function module_init()
        {
            ...
            add_filter(
                'mycred_get_email_events',
                [$this, 'email_notice_instance'],
                10,
                2
            );
            add_filter(
            ...
            // Register shortcodes
            add_shortcode(
                MYCRED_SLUG . '_sell_this',
                'mycred_render_sell_this'
            );
            add_shortcode(
                MYCRED_SLUG . '_sell_this_ajax',
                'mycred_render_sell_this_ajax'
            );
            add_shortcode(
                MYCRED_SLUG . '_sales_history',
                'mycred_render_sell_history'
            );
            add_shortcode(
                MYCRED_SLUG . '_content_sale_count',
                'mycred_render_sell_count'
            );
            add_shortcode(
                MYCRED_SLUG . '_content_buyer_count',
                'mycred_render_sell_buyer_count'
            );
            add_shortcode(
                MYCRED_SLUG . '_content_buyer_avatars',
                'mycred_render_sell_buyer_avatars'
            );
            ...
            add_action('bbp_template_redirect', [$this, 'bbp_content'], 10);
        }
        ...
        public function module_admin_init()
        {
            ...
            if (!empty($post_types)) {
                foreach ($post_types as $type) {
                    add_action("add_meta_boxes_{$type}", [
                        $this,
                        'add_metabox',
                    ]);
                    add_action("save_post_{$type}", [$this, 'save_metabox']);
                }
            }
            ...
            add_action(
                ...
                [$this, 'sell_content_user_screen'],
                50
            );

            add_action(
                'personal_options_update',
                [$this, 'save_manual_profit_share'],
                50
            );
            add_action(
        ...
        public function enqueue_footer()
        {
            ...
            if ($mycred_sell_this === true) {
                ...
                wp_localize_script('mycred-sell-this', 'myCREDBuyContent', [
                    'ajaxurl' => esc_url(
                        isset($post->ID)
                            ? mycred_get_permalink($post->ID)
                            : home_url('/')
                    ),
        ...
        public function template_redirect()
        {
            ...
            add_filter('the_content', [$this, 'the_content'], $this->priority);
        }
        ...
        public function maybe_buy_content()
        {
            if (is_user_logged_in() && !mycred_is_admin()) {
                if (
                    isset($_POST['action']) &&
                    $_POST['action'] == 'mycred-buy-content' &&
                    isset($_POST['postid']) &&
                    isset($_POST['token']) &&
                    wp_verify_nonce($_POST['token'], 'mycred-buy-this-content')
                ) {
                    ...
                    if (
                        !array_key_exists($point_type, $mycred_types) ||
                        mycred_force_singular_session(
                            $this->current_user_id,
                            'mycred-last-content-purchase'
                        ) ||
                        !in_array($point_type, $buying_cred)
                    ) {
                        wp_send_json('ERROR');
                    }
                    ...
                    if (
                        mycred_post_is_for_sale($post_id) &&
                        ...
                        $purchase = mycred_sell_content_new_purchase(
                            $post,
                            $this->current_user_id,
                            $point_type
                        );
                        ...
                        if ($purchase === true) {
                            preg_match(
                                '/\[mycred_sell_this[^\]]*](.*)\[\/mycred_sell_this[^\]]*]/uis',
                                $post->post_content,
                                $match
                            );
        ...
        public function the_content($content)
        {
            ...
            if (mycred_post_is_for_sale($post_id)) {
                ...
                if (is_user_logged_in()) {
                    ...
                    if (
                        !mycred_is_admin() &&
                        $post->post_author != $this->current_user_id
                    ) {
                        ...
                        if (
                            ...
                            // Get Payment Options
                            $payment_options = mycred_sell_content_payment_buttons(
                                $this->current_user_id,
                                $post_id
                            );
                            ...
                            if ($payment_options !== false) {
                                $content =
                                    $this->sell_content['templates']['members'];
                                $content = str_replace(
                                    '%buy_button%',
                                    $payment_options,
                                    $content
                                );
                                $content = mycred_sell_content_template(
                                    $content,
                                    $post,
                                    'mycred-sell-entire-content',
                                    'mycred-sell-unpaid'
                                );
                            }
        ...
        public function mycred_bbp_sell_forum_actions()
        {
            add_action('bbp_template_before_single_forum', [
                $this,
                'bbp_template_before_single',
            ]);
            add_action('bbp_template_before_single_topic', [
                $this,
                'bbp_template_before_single',
            ]);
            add_filter(
                'bbp_no_breadcrumb',
                [$this, 'bbp_remove_breadcrumb'],
                10
            );
            add_filter('bbp_is_single_topic', [$this, 'bbp_is_topic'], 10);
            add_filter(
                'bbp_get_forum_subscribe_link',
                [$this, 'bbp_remove_subscribe_link'],
                10,
                3
            );
            add_filter(
            ...
            add_filter(
                ...
                [$this, 'bbp_remove_templates'],
                10,
                3
            );
            add_filter(
                'bbp_get_single_forum_description',
                [$this, 'bbp_get_single_description'],
                10,
                3
            );
            add_filter(
        ...
        public function bbp_is_topic($post_id = 0)
        {
            ...
            if (
                !empty($post_id) &&
                bbp_get_topic_post_type() === get_post_type($post_id)
            ) {
                $retval = true;
            }
        ...
        public function sell_content_user_screen($user)
        {
            ...
            foreach ($mycred_types as $point_type_key => $label) {
                ...
                if (
                    !empty($this->sell_content['type']) &&
                    in_array($point_type_key, $this->sell_content['type'])
                ) {
                    $setup['enabled'] = true;
                    ...
                    if (!$mycred->exclude_user($user->ID)) {
                        ...
                        $settings = mycred_get_option(
                            'mycred_sell_this_' . $point_type_key
                        );

                        $setup['default'] = $settings['profit_share'];

                        $users_share = mycred_get_user_meta(
                            $user->ID,
                            'mycred_sell_content_share_' . $point_type_key,
                            '',
                            true
                        );
...
<p class="mycred-p"><?php _e(
    'Users profit share when their content is purchased.',
    'mycred'
); ?></p>
...
<?php }
    ...
    elseif ($data['excluded']) { ?>
         ...
         $data['name']
          ...
          _x('User can not pay using %s', 'Points Name', 'mycred'),
          $data['name']
      ); ?></em></div>
					</div>
				</div>
<?php }
    ...
    // Eligeble user
				...
				<div class="mycred-wrapper buycred-wrapper color-option selected">
					<div><?php printf(
         _x('%s Profit Share', 'Buying Points', 'mycred'),
         $data['name']
     ); ?></div>
					<div class="balance-row">
						<div class="balance-view"><input type="text" size="8" name="mycred_sell_this[<?php echo $point_type; ?>]" class="half" placeholder="<?php echo esc_attr(
    $data['default']
...
<?php
        ...
        function save_manual_profit_share($user_id)
        {
            ...
            if (
                isset($_POST['mycred_sell_this']) &&
                !empty($_POST['mycred_sell_this'])
            ) {
                foreach ($_POST['mycred_sell_this'] as $point_type => $share) {
                    ...
                    mycred_delete_user_meta(
                        $user_id,
                        'mycred_sell_content_share_' . $point_type
                    );
...
<div class="body" style="display:none;">
	...
	<p><?php _e(
     'Which post type(s) content field do you want to sell access to?',
     'mycred'
 ); ?></p>
...
<?php if (!empty($post_types)) {
    ...
    'post_types' => $post_type,
]); ?>"<?php echo $selected; ?> id="<?php echo $this->field_id([
     'post_types' => $post_type,
 ]); ?>" class="mycred-check-count" data-type="<?php echo $post_type; ?>" value="<?php echo $post_type; ?>" /> <?php echo esc_attr(
    $post_type_label
); ?></label>
			</div>
		</div>
		<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
			<div id="<?php echo $this->field_id([
       'post_types' => $post_type,
   ]); ?>-wrap" style="display: <?php echo $show_options; ?>;">
				<div class="row">
					<div class="col-lg-5 col-md-5 col-sm-6 col-xs-12">
						<div class="form-group">
							<select name="<?php echo $this->field_name([
           'filters' => $post_type,
       ]); ?>[by]" class="form-control toggle-filter-menu" data-type="<?php echo $post_type; ?>">
<?php
$settings = ['by' => 'all', 'list' => ''];
...
if (!empty($options)) {
    foreach ($options as $value => $option) {
        ...
        if ($value == $settings['by']) {
            echo ' selected="selected"';
        }
        if ($option['data'] != '') {
            echo ' data-place="' . $option['data'] . '"';
        }
...
?>
					...
					<div class="col-lg-7 col-md-7 col-sm-6 col-xs-12">
						<div id="post-type-filter-<?php echo $post_type; ?>" style="display: <?php if (
    !in_array($settings['by'], ['all', 'manual'])
) {
    echo 'block';
} else {
    echo 'none';
            ...
            'filters' => $post_type,
        ]); ?>[list]" value="<?php echo esc_attr(
    $settings['list']
); ?>" placeholder="<?php if (array_key_exists($settings['by'], $options)) {
    echo esc_attr($options[$settings['by']]['data']);
} ?>" class="form-control" />
							</div>
						</div>
					</div>
...
<?php
	...
	<p><?php _e(
     'Which point type(s) can be used as payment for accessing content?',
     'mycred'
 ); ?></p>
	<div class="row">
...
<?php if (!empty($point_types)) {
    ...
    'type' => $point_type,
     ...
     'type' => $point_type,
 ]); ?>" class="mycred-check-count" data-type="<?php echo $point_type; ?>" value="<?php echo $point_type; ?>" /> <?php echo esc_attr(
    $point_type_label
); ?></label>
			</div>
<?php
...
<?php if (!empty($point_types)) {
    foreach ($point_types as $point_type => $point_type_label) {
        ...
        $type_setup = wp_parse_args($type_setup, [
            'status' => 'disabled',
            'price' => 0,
            'expire' => 0,
            'profit_share' => 0,
        ...
        $expiration_label = apply_filters(
	...
	<div id="mycred-sell-<?php echo $point_type; ?>-wrap" style="display: <?php echo $selected; ?>;">
		...
		<div class="row">
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id([
         'post_type_setup' => $point_type . '-status',
     ]); ?>"><?php _e('Default Status', 'mycred'); ?></label>
					<select name="<?php echo $this->field_name([
         'post_type_setup' => $point_type,
     ]); ?>[status]" id="<?php echo $this->field_id([
    'post_type_setup' => $point_type . '-status',
]); ?>" class="form-control">
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id([
         'post_type_setup' => $point_type . '-price',
     ]); ?>"><?php _e('Default Price', 'mycred'); ?></label>
					<input type="text" name="<?php echo $this->field_name([
         'post_type_setup' => $point_type,
     ]); ?>[price]" id="<?php echo $this->field_id([
    'post_type_setup' => $point_type . '-price',
]); ?>" class="form-control" value="<?php echo esc_attr(
    $type_setup['price']
); ?>" />
				</div>
			</div>
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id([
         'post_type_setup' => $point_type . '-expire',
     ]); ?>"><?php _e('Expiration', 'mycred'); ?></label>
					<input type="text" name="<?php echo $this->field_name([
         'post_type_setup' => $point_type,
     ]); ?>[expire]" id="<?php echo $this->field_id([
    'post_type_setup' => $point_type . '-expire',
]); ?>" class="form-control" value="<?php echo esc_attr(
    $type_setup['expire']
); ?>" />
					<p><span class="description"><?php printf(
         __(
             'Option to automatically expire purchases after certain number of %s. Use zero to disable.',
             'mycred'
         ),
         $expiration_label
     ); ?></span></p>
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id([
         'post_type_setup' => $point_type . '-profit-share',
     ]); ?>"><?php _e('Profit Share', 'mycred'); ?></label>
					<input type="text" name="<?php echo $this->field_name([
         'post_type_setup' => $point_type,
     ]); ?>[profit_share]" id="<?php echo $this->field_id([
    'post_type_setup' => $point_type . '-profit-share',
]); ?>" class="form-control" value="<?php echo esc_attr(
    $type_setup['profit_share']
); ?>" />
					<p><span class="description"><?php printf(
         __(
             'Option to pay a percentage of each sale with the content author.',
             'mycred'
         ),
         $expiration_label
     ); ?></span></p>
		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id([
         'post_type_setup' => $point_type . '-button',
     ]); ?>"><?php _e('Button Label', 'mycred'); ?></label>
					<input type="text" name="<?php echo $this->field_name([
         'post_type_setup' => $point_type,
     ]); ?>[button_label]" id="<?php echo $this->field_id([
    'post_type_setup' => $point_type . '-button',
]); ?>" class="form-control" placeholder="<?php _e(
    'Required',
    'mycred'
); ?>" value="<?php echo esc_attr($type_setup['button_label']); ?>" />
         ...
         '%price%'
			...
			<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id([
         'post_type_setup' => $point_type . '-css',
     ]); ?>"><?php _e('Button CSS Classes', 'mycred'); ?></label>
					<input type="text" name="<?php echo $this->field_name([
         'post_type_setup' => $point_type,
     ]); ?>[button_classes]" id="<?php echo $this->field_id([
    'post_type_setup' => $point_type . '-css',
]); ?>" class="form-control" value="<?php echo esc_attr(
    $type_setup['button_classes']
); ?>" />
				</div>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id([
         'post_type_setup' => $point_type . '-log-pay',
     ]); ?>"><?php _e('Payment log entry template', 'mycred'); ?></label>
					<input type="text" name="<?php echo $this->field_name([
         'post_type_setup' => $point_type,
     ]); ?>[log_payment]" id="<?php echo $this->field_id([
    'post_type_setup' => $point_type . '-log-pay',
]); ?>" class="form-control" placeholder="<?php _e(
    'Required',
    'mycred'
); ?>" value="<?php echo esc_attr($type_setup['log_payment']); ?>" />
					<p><span class="description"><?php echo $this->core->available_template_tags([
         'general',
         'post',
     ]); ?></span></p>
			...
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id([
         'post_type_setup' => $point_type . '-log-share',
     ]); ?>"><?php _e(
    'Profit Share payout log entry template',
    'mycred'
); ?></label>
					<input type="text" name="<?php echo $this->field_name([
         'post_type_setup' => $point_type,
     ]); ?>[log_sale]" id="<?php echo $this->field_id([
    'post_type_setup' => $point_type . '-log-share',
]); ?>" class="form-control" placeholder="<?php _e(
    'Required',
    'mycred'
); ?>" value="<?php echo esc_attr($type_setup['log_sale']); ?>" />
					<p><span class="description"><?php echo $this->core->available_template_tags([
         'general',
         'post',
     ]); ?></span></p>
...
<?php
	...
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<div class="checkbox">
				<label for="<?php echo $this->field_id(
        'reload'
    ); ?>"><input type="checkbox" name="<?php echo $this->field_name('reload'); ?>" id="<?php echo $this->field_id('reload'); ?>" <?php checked($this->sell_content['reload'], 1); ?> value="1" /> <?php _e('Reload page after successful payments.', 'mycred'); ?></label>
			</div>
		</div>
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
        ...
        'working'
				...
				<input type="text" name="<?php echo $this->field_name(
        'working'
    ); ?>" id="<?php echo $this->field_id('working'); ?>" class="form-control" placeholder="<?php _e('Required', 'mycred'); ?>" value="<?php echo esc_attr($this->sell_content['working']); ?>" />
				<p><span class="description"><?php _e(
        'Option to show a custom button label while the payment is being processed. HTML is allowed.',
        'mycred'
    ); ?></span></p>
			</div>
		</div>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<p><span class="description"><?php _e(
       'The content will be replaced with this template when viewed by a user that has not paid for the content but can afford to pay.',
       'mycred'
   ); ?></span></p>
<?php
wp_editor(
    $this->sell_content['templates']['members'],
    $this->field_id(['templates' => 'members']),
    [
        'textarea_name' => $this->field_name(['templates' => 'members']),
        'textarea_rows' => 10,
    ]
);

echo '<p>' .
    $this->core->available_template_tags(['post'], '%buy_button%') .
    '</p>';
?>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<p><span class="description"><?php _e(
       'The content will be replaced with this template when viewed by a user that has not paid for the content and can not afford to pay.',
       'mycred'
   ); ?></span></p>
<?php
wp_editor(
    $this->sell_content['templates']['cantafford'],
    $this->field_id(['templates' => 'cantafford']),
    [
        'textarea_name' => $this->field_name(['templates' => 'cantafford']),
        'textarea_rows' => 10,
    ]
);

echo '<p>' . $this->core->available_template_tags(['post']) . '</p>';
?>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<p><span class="description"><?php _e(
       'The content will be replaced with this template when viewed by someone who is not logged in on your website.',
       'mycred'
   ); ?></span></p>
<?php
wp_editor(
    $this->sell_content['templates']['visitors'],
    $this->field_id(['templates' => 'visitors']),
    [
        'textarea_name' => $this->field_name(['templates' => 'visitors']),
        'textarea_rows' => 10,
    ]
);
...
<?php
        ...
        public function sanitize_extra_settings($new_data, $data, $general)
        {
            ...
            $post_types = [];
            if (
                array_key_exists('post_types', $settings) &&
            ...
            $filters = [];
            if (
                array_key_exists('filters', $settings) &&
                is_array($settings['filters']) &&
                        ...
                        if ($by !== 'all' && $by !== 'manual') {
                            ...
                            if ($list != '') {
                                ...
                                foreach (explode(',', $list) as $object_slug) {
                                    $object_slug = sanitize_text_field(
                                        $object_slug
                                    );
            ...
            if (array_key_exists('post_type_setup', $settings)) {
                foreach (
                    $settings['post_type_setup']
                    as $point_type => $setup
                ) {
                    $new = wp_parse_args($setup, [
                        'status' => 'disabled',
                        'price' => 0,
                        'expire' => 0,
                        'profit_share' => 0,
                    ...
                    mycred_update_option(
                        'mycred_sell_this_' . $point_type,
                        $new
                    );
                }
            }

            $new_data['sell_content']['reload'] = isset($settings['reload'])
                ? absint($settings['reload'])
                : 0;
            $new_data['sell_content']['working'] = wp_kses_post(
                $settings['working']
            );

            // Templates
            $new_data['sell_content']['templates']['members'] = wp_kses_post(
                $settings['templates']['members']
            );
            $new_data['sell_content']['templates']['visitors'] = wp_kses_post(
                $settings['templates']['visitors']
            );
            $new_data['sell_content']['templates']['cantafford'] = wp_kses_post(
                $settings['templates']['cantafford']
            );
        ...
        public function scripts_and_styles()
        {
            ...
            if (
                in_array(
                    $screen->id,
                    explode(',', $this->sell_content['post_types'])
                )
            ) {
        ...
        public function add_metabox($post)
        {
            ...
            if (
                empty($settings['filters'][$post->post_type]) ||
                $settings['filters'][$post->post_type]['by'] !== 'manual'
            ) {
                return;
            }
        ...
        public function metabox($post)
        {
            ...
            $is_author =
                $post->post_author == $this->current_user_id ? true : false;
            ?>
<style type="text/css">
...
<div id="mycred-sell-content-types" class="container-fluid">
	<input type="hidden" name="mycred-sell-this-setup-token" value="<?php echo wp_create_nonce(
     'mycred-sell-this-content'
 ); ?>" />
<?php if (!empty($settings['type'])) {
    foreach ($settings['type'] as $point_type) {
        ...
        $sale_setup = (array) mycred_get_post_meta(
            $post->ID,
            'myCRED_sell_content' . $suffix
        );
		...
		<div class="row row-narrow">
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<div class="form-group slim">
					<label for="mycred-sell-this-<?php echo $point_type; ?>-status" class="slim"><input type="checkbox" name="mycred_sell_this[<?php echo $point_type; ?>][status]" id="mycred-sell-this-<?php echo $point_type; ?>-status"<?php if (
    $sale_setup['status'] === 'enabled'
) {
    echo ' checked="checked"';
} ?> value="enabled" class="toggle-setup" data-type="<?php echo $point_type; ?>" /> <?php printf(
    __('Sell using %s', 'Point types name', 'mycred'),
    $mycred->plural()
); ?></label>
				</div>
			</div>
		...
		<div class="cover">
			<div class="row row-narrow padded-row mycred-sell-setup-container" id="mycred-sell-content-<?php echo $point_type; ?>-wrap" style="display: <?php if (
    $sale_setup['status'] === 'enabled'
) {
    echo 'block';
} else {
    echo 'none';
				...
				<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
					<div class="form-group slim">
						<label for="mycred-sell-this-<?php echo $point_type; ?>-price"><?php _e(
    'Price',
    'mycred'
); ?></label>
						<input type="text" name="mycred_sell_this[<?php echo $point_type; ?>][price]" id="mycred-sell-this-<?php echo $point_type; ?>-price" class="form-control" value="<?php echo esc_attr(
    $sale_setup['price']
); ?>" />
					</div>
				</div>
				<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
					<div class="form-group slim">
						<label for="mycred-sell-this-<?php echo $point_type; ?>-expire"><?php _e(
    'Expiration',
    'mycred'
); ?></label>
						<input type="text" name="mycred_sell_this[<?php echo $point_type; ?>][expire]" id="mycred-sell-this-<?php echo $point_type; ?>-expire" class="form-control" value="<?php echo absint(
    $sale_setup['expire']
); ?>" />
					</div>
				</div>
...
<?php
        ...
        public function save_metabox($post_id)
        {
            ...
            if (
                !isset($_POST['mycred_sell_this']) ||
                !is_array($_POST['mycred_sell_this']) ||
                empty($_POST['mycred_sell_this'])
            ) {
                return;
            }
            ...
            // Verify nonce
            if (
                isset($_POST['mycred-sell-this-setup-token']) &&
                wp_verify_nonce(
                    $_POST['mycred-sell-this-setup-token'],
                    'mycred-sell-this-content'
                )
            ) {
                $settings = mycred_sell_content_settings();
                ...
                if (!empty($settings['type'])) {
                    foreach ($settings['type'] as $point_type) {
                        ...
                        if (strlen($submission['price']) > 0) {
                            $new_setup['price'] = $mycred->number(
                                sanitize_text_field($submission['price'])
                            );
                        }

                        // If not empty and different from the general setup, save<
                        if (strlen($submission['expire']) > 0) {
                            $new_setup['expire'] = absint(
                                sanitize_text_field($submission['expire'])
                            );
                        }
        ...
        public function email_notices($data)
        {
            if ($data['request']['ref'] == 'buy_content') {
                ...
                $data['message'] = $this->core->template_tags_post(
                    $message,
                    $data['request']['ref_id']
                );
            }
