    ...
    function mycred_sell_content_post_types()
    {
        ...
        if (!empty($post_types)) {
            foreach ($post_types as $post_type => $setup) {
                ...
                if (
                    in_array($post_type, $mycred_post_types) ||
...
if (!function_exists('mycred_sell_content_post_id')):
    function mycred_sell_content_post_id()
    {
        ...
        if (function_exists('bbpress')) {
            ...
            if (
                !empty($bbp->topic_query->in_the_loop) &&
                isset($bbp->topic_query->post->ID)
            ) {
            ...
            elseif (
                !empty($bbp->search_query->in_the_loop) &&
                isset($bbp->search_query->post->ID) &&
                bbp_is_topic($bbp->search_query->post->ID)
            ) {
            ...
            elseif (
                (bbp_is_single_topic() || bbp_is_topic_edit()) &&
                !empty($bbp->current_topic_id)
            ) {
            ...
            if ($bbp_topic_id === false) {
                ...
                if (
                    !empty($bbp->reply_query->in_the_loop) &&
                    isset($bbp->reply_query->post->ID)
                ) {
                ...
                elseif (
                    !empty($bbp->search_query->in_the_loop) &&
                    isset($bbp->search_query->post->ID) &&
                    bbp_is_reply($bbp->search_query->post->ID)
                ) {
                ...
                elseif (
                    (bbp_is_single_reply() || bbp_is_reply_edit()) &&
                    !empty($bbp->current_reply_id)
                ) {
...
if (!function_exists('mycred_post_is_for_sale')):
    function mycred_post_is_for_sale($post = null)
    {
        ...
        if (
            mycred_post_type_for_sale($post->post_type) &&
            array_key_exists($post->post_type, $settings['filters'])
        ) {
            $filter = $settings['filters'][$post->post_type]['by'];
            $list = explode(
                ',',
                $settings['filters'][$post->post_type]['list']
            );
            ...
            if ($filter === 'manual') {
                ...
                foreach ($point_types as $type_id) {
                    ...
                    if (
                        array_key_exists('status', $sale_setup) &&
...
if (!function_exists('mycred_user_paid_for_content')):
    function mycred_user_paid_for_content(
        ...
        $expires = mycred_sell_content_get_expiration_length(
            $post_id,
            $point_type
        );
        ...
        if ($expires == 0) {
            ...
            if (
                !empty($account->point_types) &&
                in_array($point_type, $account->point_types) &&
                isset($account->balance[$point_type]->history)
            ) {
                $data = $account->balance[$point_type]->history->get('data');
                if (
                    array_key_exists('buy_content', $data) &&
                    !empty($data['buy_content']->reference_ids) &&
                    in_array($post_id, $data['buy_content']->reference_ids)
                ) {
                    $has_paid = true;
                }
            }
        ...
        if (!$has_paid) {
            $last_payment = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT * FROM {$mycred_log_table} WHERE user_id = %d AND ref = 'buy_content' AND ref_id = %d ORDER BY time DESC LIMIT 1;",
                    $user_id,
                    $post_id
                )
            );
...
if (!function_exists('mycred_content_purchase_has_expired')):
    function mycred_content_purchase_has_expired($payment = null)
    {
        ...
        $length = mycred_sell_content_get_expiration_length(
            $payment->ref_id,
            $payment->ctype
        );
        ...
        return apply_filters(
            'mycred_sell_content_sale_expired',
            $has_expired,
            $payment->user_id,
            $payment->ref_id,
            $payment->time,
...
if (!function_exists('mycred_sell_content_get_expiration_length')):
    function mycred_sell_content_get_expiration_length(
        ...
        if ($filter === 'manual') {
            ...
            if (
                !empty($sale_setup) &&
                array_key_exists('expire', $sale_setup) &&
                $sale_setup['expire'] > 0
            ) {
        ...
        // Else we need to check the point type setup in our add-on settings.
            ...
            $point_type_setup = (array) mycred_get_option(
                'mycred_sell_this_' . $type
            );
            if (
                !empty($point_type_setup) &&
                array_key_exists('expire', $point_type_setup) &&
                $point_type_setup['expire'] > 0
            ) {
    ...
    function mycred_sell_content_payment_buttons(
        ...
        if (!empty($settings['type'])) {
            ...
            foreach ($settings['type'] as $point_type) {
                ...
                $price = mycred_get_content_price(
                    $post_id,
                    $point_type,
                    $user_id
                );
                ...
                if ($settings['filters'][$post->post_type]['by'] == 'manual') {
                    $suffix =
                        $point_type != MYCRED_DEFAULT_TYPE_KEY
                            ? '_' . $point_type
                            : '';
                    $manual_setup = (array) mycred_get_post_meta(
                        $post_id,
                        'myCRED_sell_content' . $suffix,
                        true
                    );
                    if (
                        !empty($manual_setup) &&
                        array_key_exists('status', $manual_setup)
                    ) {
                ...
                if (
                    $mycred->get_users_balance($user_id, $point_type) < $price
                ) {
                    continue;
                }

                $button_label = str_replace(
                    '%price%',
                    $mycred->format_creds($price),
                    $setup['button_label']
                );

                $button =
                    '<button type="button" class="mycred-buy-this-content-button ' .
                    $setup['button_classes'] .
                    '" data-pid="' .
                    $post_id .
                    '" data-type="' .
                    $point_type .
...
if (!function_exists('mycred_sell_content_template')):
    function mycred_sell_content_template(
        $template = '',
        $post = null,
        $type = 'mycred-sell-partial-content',
        $status = 'visitor'
    ) {
        if (!is_object($post) || strlen($template) === 0) {
            return $template;
        }
        ...
        $template = str_replace(['%price%', '%expires%'], '', $template);

        $template = str_replace(
            '%post_title%',
            mycred_get_the_title($post->ID),
            $template
        );
        $template = str_replace(
            '%post_type%',
        ...
        $wrapper_classes = apply_filters(
            'mycred_sell_template_class',
            $wrapper_classes,
            $post
        );

        return '<div id="mycred-buy-content' .
            $post->ID .
            '" class="' .
...
if (!function_exists('mycred_sell_content_new_purchase')):
    function mycred_sell_content_new_purchase(
        ...
        $result = apply_filters(
            'mycred_before_content_purchase',
            false,
            $post->ID,
            $user_id,
            $point_type
        ...
        if ($result === false) {
            ...
            if (
                ...
                $mycred->exclude_user($user_id)
            ) {
                $result = sprintf(
                    _x('You can not pay using %s', 'Point type name', 'mycred'),
                    $mycred->plural()
                );
            } else {
                ...
                elseif ($price > 0) {
                    ...
                    if ($setup['profit_share'] > 0) {
                        ...
                        $percentage = mycred_get_authors_profit_share(
                            $post->post_author,
                            $point_type,
                            $setup['profit_share']
                        );
...
if (!function_exists('mycred_get_content_price')):
    function mycred_get_content_price(
        ...
        if (
            array_key_exists($post_type, $settings['filters']) &&
            $settings['filters'][$post_type]['by'] === 'manual'
        ) {
            $suffix = '_' . $point_type;
...
if (!function_exists('mycred_get_users_purchased_content')):
    function mycred_get_users_purchased_content(
        $user_id = null,
        $number = 25,
        $order = 'DESC',
        $point_type = null
        ...
        $sql = apply_filters(
            'mycred_get_users_purchased_content',
            "SELECT * FROM {$mycred_log_table} log INNER JOIN {$wpdb->posts} posts ON ( log.ref_id = posts.ID ) {$wheres} ORDER BY time {$order} {$limit};",
            $user_id,
            $number,
            $order,
            $point_type
...
if (!function_exists('mycred_get_posts_buyers')):
    function mycred_get_posts_buyers(
        $post_id = null,
        $number = 25,
        $point_type = null
    ) {
        ...
        return $wpdb->get_col(
            "SELECT user_id FROM {$mycred_log_table} {$wheres} ORDER BY time DESC {$limit};"
        );
    }
endif;
...
if (!function_exists('mycred_get_content_buyers_count')):
    function mycred_get_content_buyers_count($post_id = null)
    {
        ...
        if (strlen($count) == 0) {
            ...
            $count = $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT( DISTINCT user_id ) FROM {$mycred_log_table} WHERE ref = 'buy_content' AND ref_id = %d;",
                    $post_id
                )
            );
            if ($count === null) {
...
if (!function_exists('mycred_get_authors_profit_share')):
    function mycred_get_authors_profit_share(
        $user_id = null,
        $point_type = null,
        $default = false
    ) {
        ...
        return apply_filters(
            'mycred_sell_content_profit_share',
            $users_share,
            $user_id,
            $point_type,
            $default
...
if (!function_exists('mycred_get_post_type_options')):
    function mycred_get_post_type_options($post_type)
    {
        ...
        $options['all'] = [
            'label' => sprintf(
                _x('All %s', 'all post type name', 'mycred'),
                $type->labels->name
            ),
            'data' => '',
        ];
        $options['manual'] = [
            'label' => sprintf(
                _x('%s I manually select', 'all post type name', 'mycred'),
                $type->labels->name
            ),
            'data' => '',
        ];
        $options['exclude'] = [
            'label' => sprintf(
                _x('All %s except', '%s = post type name', 'mycred'),
                $type->labels->name
            ),
            'data' => sprintf(
                _x(
                    'Comma separated list of %s IDs to exclude',
                    '%s = post type name',
                    'mycred'
                ),
        ...
        $options['include'] = [
            'label' => sprintf(
                _x('Only %s', '%s = post type name', 'mycred'),
                $type->labels->name
            ),
            'data' => sprintf(
                _x(
                    'Comma separated list of %s IDs',
                    '%s = post type name',
                    'mycred'
                ),
                $type->labels->singular_name
            ...
            foreach ($taxonomies as $taxonomy_id => $term) {
                ...
                if ($term->hierarchical) {
                    $options[$taxonomy_id] = [
                        ...
                        'data' => sprintf(
                            _x(
                                'Comma separated list of %s slugs',
                                '%s = taxonomy name',
                                'mycred'
                            ),
                            $term->labels->singular_name
                        ),
                    ];
                    $options['_' . $taxonomy_id] = [
                        'label' => sprintf(
                            _x(
                                'Only %s not in %s',
                                'e.g. Only "Posts" not in "Categories"',
                                'mycred'
                            ),
                            $type->labels->name,
                            $term->labels->name
