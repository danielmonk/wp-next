...
if (!function_exists('mycred_render_sell_this')):
    function mycred_render_sell_this($atts, $content = '')
    {
            ...
            if (!$is_admin && !$is_owner) {
                ...
                if (!mycred_user_paid_for_content($user_id, $post_id)) {
                    ...
                    if ($payment_options !== false) {
                        $content =
                            $mycred_modules['solo']['content']->sell_content[
                                'templates'
                            ]['members'];
                        $content = str_replace(
                            '%buy_button%',
                ...
                if ($is_admin || $is_owner) {
                    return '<p>' .
                        sprintf(
                            ...
                            '<a href="' . get_edit_post_link($post_id)
                        ) .
                        '">' .
                        __('Edit', 'mycred') .
                        '</a></p>';
        ...
        // Visitors
            ...
            $content =
                $mycred_modules['solo']['content']->sell_content['templates'][
                    'visitors'
                ];
            $content = mycred_sell_content_template(
                $content,
...
if (!function_exists('mycred_render_sell_this_ajax')):
    function mycred_render_sell_this_ajax($atts, $content = '')
    {
        _doing_it_wrong(
            ...
            'The mycred_sell_this_ajax shortcode has been depreciated and will be removed in version 1.8.',
            '1.7'
        );
...
if (!function_exists('mycred_render_sell_count')):
    function mycred_render_sell_count($atts, $content = '')
    {
        ...
        if ($wrapper != '') {
            $content .=
                '<' . $wrapper . ' class="mycred-sell-this-sales-count">';
        }
...
if (!function_exists('mycred_render_sell_buyer_count')):
    function mycred_render_sell_buyer_count($atts, $content = '')
    {
        ...
        if ($wrapper != '') {
            $content .=
                '<' . $wrapper . ' class="mycred-sell-this-author-count">';
        }
...
if (!function_exists('mycred_render_sell_history')):
    function mycred_render_sell_history($atts, $content = '')
    {
        ...
        $purchases = mycred_get_users_purchased_content(
            $user_id,
            $number,
            $order,
            $ctype
...
<?php if (!empty($purchases)) {
    foreach ($purchases as $entry) {
        ...
        $expirares_in = mycred_sell_content_get_expiration_length(
            $entry->ref_id,
            $entry->ctype
        );
        ...
        foreach ($columns as $column_id => $column_label) {
            if ($column_id == 'col-date') {
                echo '<td class="' .
                    $column_id .
                    '">' .
                    date($date_format, $entry->time) .
                    '</td>';
            } elseif ($column_id == 'col-title') {
                echo '<td class="' .
                    $column_id .
                    '"><a href="' .
                    mycred_get_permalink($entry->ref_id) .
                    '">' .
                    mycred_get_the_title($entry->ref_id) .
                    '</a></td>';
            } elseif ($column_id == 'col-amount') {
                echo '<td class="' .
                    $column_id .
                    '">' .
                    $mycred->format_creds(abs($entry->creds)) .
                    '</td>';
            } elseif ($column_id == 'col-expires') {
                $expires = __('Never', 'mycred');
                if ($expirares_in > 0) {
                    $expires = sprintf(
                        _x('Purchase expires in %s', 'e.g. 10 hours', 'mycred'),
                        $expirares_in . ' ' . $expiration
                    );
                }
    ...
    echo '<tr><td class="no-results" colspan="' .
        count($columns) .
        '">' .
        $nothing .
        '</td></tr>';
        ...
        if (!empty($buyers)) {
            foreach ($buyers as $buyer_id) {
                ...
                $avatar = apply_filters(
                    'mycred_sell_content_buyer_avatar',
                    $avatar,
                    $buyer_id,
                    $post_id
                );
