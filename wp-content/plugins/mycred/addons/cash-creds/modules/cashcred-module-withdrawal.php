...
if (!class_exists('cashCRED_Pending_Payments')):
    class cashCRED_Pending_Payments extends myCRED_Module
    {
        ...
        public function module_admin_init()
        {
            ...
            add_filter('manage_' . MYCRED_CASHCRED_KEY . '_posts_columns', [
                $this,
                'adjust_column_headers',
            ]);
            add_action(
                'manage_' . MYCRED_CASHCRED_KEY . '_posts_custom_column',
                [$this, 'adjust_column_content'],
                10,
                2
            );
            add_filter('bulk_actions-edit-' . MYCRED_CASHCRED_KEY, [
                $this,
                'bulk_actions',
            ]);
            add_action(
                'save_post_' . MYCRED_CASHCRED_KEY,
                [$this, 'save_pending_payment'],
                10,
                2
            );
            ...
            add_action('restrict_manage_posts', [
                $this,
                'cashcred_filter_html',
            ]);
            add_filter('parse_query', [$this, 'cashcred_filter_query']);

            add_action('admin_enqueue_scripts', [
                $this,
                'cashcred_admin_assets',
            ]);
        }
        ...
        public function cashcred_admin_assets()
        {
            ...
            if ($post_type == MYCRED_CASHCRED_KEY) {
                wp_register_style(
                    'cashcred-admin',
                    plugins_url('assets/css/admin-style.css', MYCRED_CASHCRED),
                    [],
                ...
                wp_register_script(
                    'cashcred-admin-script',
                    plugins_url('assets/js/admin-script.js', MYCRED_CASHCRED),
                    ['jquery'],
                    MYCRED_CASHCRED_VERSION,
                    'all'
                );
    ...
    ?>
				<select name="gateway" id="gateway">
					...
					<?php foreach (
         $mycred_modules['solo']['cashcred']->get()
         as $gateway_id => $info
     ) { ?> 
					<option value="<?php echo esc_attr($gateway_id); ?>" <?php selected(
    $gateway_id,
    $current_selected
); ?>>
    ...
    $current_user
        ...
        protected function register_cashcred_payments()
        {
            ...
            register_post_type(
                MYCRED_CASHCRED_KEY,
                apply_filters('mycred_setup_cashcred_payment', $args)
            );
        }
        ...
        public function admin_notices()
        {
            if (
                ...
                $_GET['post_type'] == MYCRED_CASHCRED_KEY &&
                isset($_GET['credited'])
            ) {
                if ($_GET['credited'] == 1) {
                    echo '<div id="message" class="updated notice is-dismissible"><p>' .
                        __(
                            'Pending payment successfully credited to account.',
                            'mycred'
                        ) .
                    ...
                    echo '<div id="message" class="error notice is-dismissible"><p>' .
                        __(
                            'Failed to credit the pending payment to account.',
                            'mycred'
                        ) .
                        '</p><button type="button" class="notice-dismiss"></button></div>';
                }
        ...
        public function parent_file($parent = '')
        {
            ...
            if (
                isset($_GET['post']) &&
                mycred_get_post_type($_GET['post']) == MYCRED_CASHCRED_KEY &&
                isset($_GET['action']) &&
                $_GET['action'] == 'edit'
            ) {
                return MYCRED_SLUG;
        ...
        public function adjust_column_content($column_name, $post_id)
        {
            ...
            switch ($column_name) {
                case 'User':
                    $from = (int) check_site_get_post_meta(
                        $post_id,
                        'from',
                        true
                    );
                    ...
                    break;
                case 'Points':
                    $type = check_site_get_post_meta(
                        $post_id,
                        'point_type',
                        true
                    );
                    $points = check_site_get_post_meta(
                        $post_id,
                        'points',
                        true
                    );
                ...
                case 'cost':
                    ...
                    $currency = check_site_get_post_meta(
                        $post_id,
                        'currency',
                        true
                    );
                ...
                case 'amount':
                    ...
                    break;
                case 'gateway':
                    $gateway = check_site_get_post_meta(
                        $post_id,
                        'gateway',
                        true
                    );
                ...
                case 'ctype':
                    ...
                    break;
                case 'status':
                    $status = check_site_get_post_meta(
                        $post_id,
                        'status',
                        true
                    );
                    echo "<div class='cashcred_bages'><span class='cashcred_" .
                        $status .
                        "'>" .
                        $status .
                        "</span></div>";
        ...
        public function edit_pending_payment_style()
        {
            ...
            add_filter(
                'postbox_classes_buycred_payment_buycred-pending-payment',
                [$this, 'metabox_classes']
            );
            add_filter('postbox_classes_buycred_payment_cashcred-comments', [
                $this,
                'metabox_classes',
            ]);
        }
        ...
        public function add_metaboxes()
        {
            ...
            add_meta_box(
                'cashcred-comments',
                __('History', 'mycred'),
                [$this, 'metabox_cashcred_comments'],
                MYCRED_CASHCRED_KEY,
                'normal',
                'default'
            );
            ...
            if (
                isset($mycred_pref_cashcreds["debugging"]) &&
                $mycred_pref_cashcreds["debugging"] == 'enable'
            ) {
                add_meta_box(
                    'cashcred-developer-log',
                    __('Debugging Log', 'mycred'),
                    [$this, 'cashcred_developer_log'],
                    MYCRED_CASHCRED_KEY,
                    'normal',
                    'default'
                );
        ...
        public function payment_gateway_detail()
        {
            ...
            foreach (
                ...
                if (
                    !$mycred_modules['solo']['cashcred']->is_active($gateway_id)
                ) {
                    continue;
                }

                $MyCred_payment_setting_call = new $info['callback'][0](
                    $gateway_id
                );
                $MyCred_payment_setting_call->cashcred_payment_settings(
                    $gateway_id
                );
            }
        }
        ...
        public function cashcred_payment_pay()
        {
            ...
            $cashcred_user_settings = get_user_meta(
                $user_id,
          ...
          if ($get_payment_settings->gateway_id == $gateway_id) {
			...
			<div class="row">
				...
				<div class="col-md-4 col-sm-12">
					<div class="form-group"><?php echo $get_payment_settings->currency .
         " " .
         $get_payment_settings->points * $get_payment_settings->cost; ?></div>
				</div>
			</div>
   ...
   if (
       $status == 'Approved' ||
       $this->is_paid_request($user_id, get_the_ID())
   ) {
       $disabled = 'disabled';
   }
   ?>
			<input type="hidden" name="cashcred_create_nonce" value="<?php echo wp_create_nonce(
       'cashcred_create_nonce'
   ); ?>">
        ...
        public function cashcred_user_info()
        {
			...
			<div class="row">
				...
				<div class="col-md-4 col-sm-12">
					<div class="form-group"> <?php echo get_post_meta(
         get_the_ID(),
         'user_ip',
         true
     ); ?></div>
			...
			<div class="row">
				...
				<div class="col-md-4 col-sm-12">
					<div class="form-group" id="user_total_cashcred"><?php if (
         get_user_meta($user_id, 'cashcred_total', true)
     ) {
         echo $get_payment_settings->currency .
             ' ' .
             get_user_meta($user_id, 'cashcred_total', true);
     } else {
         echo 0;
...
<div class="submitbox mycred-metabox" id="submitpost">
		...
		<div id="minor-publishing-actions">
			...
			<div id="cashcred_post_ststus">	 
				...
				<select name="status">
					...
					<option value="Cancelled" <?php echo $status == "Cancelled"
         ? "selected"
         : ""; ?>>
	...
	<div id="major-publishing-actions">
		...
		<div id="publishing-action">
			...
			<input type="submit" id="publish" class="button button-primary primary button-large" value="<?php _e(
       'Save Changes',
       'mycred'
   ); ?>" />
...
<div class="form">
	<div class="row">
		<div class="col-md-2 col-sm-6">
			<div class="form-group">
				<label for="cashcred-pending-payment-point_type"><?php _e(
        'Point Type',
        'mycred'
    ); ?></label>
				...
				<?php foreach (mycred_get_types() as $key => $point_type) {
		...
		<div class="col-md-2 col-sm-6">
			<div class="form-group">
				<label for="cashcred-pending-payment-gateway"><?php _e(
        'Gateway',
        'mycred'
    ); ?></label>
...
<?php foreach (
    ...
    echo '>' . $info['title'] . '</option>';
		...
		<div class="col-md-2 col-sm-6">
			<div class="form-group">
				<label for="cashcred-pending-payment-points"><?php _e(
        'Points',
        'mycred'
    ); ?></label>
		...
		<div class="col-md-2 col-sm-6">
			<div class="form-group">
				<label for="cashcred-pending-payment-cost"><?php _e(
        'Cost',
        'mycred'
    ); ?></label>
		...
		<div class="col-md-2 col-sm-6">
			<div class="form-group">
				<label for="cashcred-pending-payment-currency"><?php _e(
        'Currency',
        'mycred'
    ); ?></label>
...
<?php
        ...
        public function metabox_cashcred_comments($post)
        {
            ...
            if (empty($comments)) {
                ...
                $c->comment_content = __(
                    'Withdrawal request created.',
                    'mycred'
                );

                $event = $this->add_comment(
                    $post->ID,
                    $c->comment_content,
                    $c->comment_date
                );
            ...
            foreach ($comments as $comment) {
                $comment_date = isset($comment->comment_date_gmt)
        ...
        public function save_pending_payment($post_id, $post)
        {
            if (
                ...
                !isset($_POST['cashcred_pending_payment'])
            ) {
                return;
            }
            ...
            $user_settings = mycred_get_user_meta(
                $_POST['user_id'],
            ...
            foreach ($pending_payment as $meta_key => $meta_value) {
                ...
                $old_value = check_site_get_post_meta(
                    $post_id,
                    $meta_key,
                    true
                );
            ...
            if (
                serialize($user_settings) != serialize($updated_user_settings)
            ) {
                $this->add_comment(
                    $post_id,
                    sprintf(
                        __('User\'s detail updated by %s', 'mycred'),
                        $user->user_login
                    )
                );
            }
            ...
            if ($old_status != $new_status) {
                $this->add_comment(
                    ...
                    sprintf(
                        __(
                            'Status changed from %s to %s updated by %s',
                            'mycred'
                        ),
                        $old_status,
                        $new_status,
