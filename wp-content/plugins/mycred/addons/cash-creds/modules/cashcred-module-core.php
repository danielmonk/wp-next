    ...
    class myCRED_cashCRED_Module extends myCRED_Module
    {
        ...
        public function load()
        {
            ...
            add_action('pre_get_comments', [
                $this,
                'hide_cashcred_transactions',
            ]);

            add_action(
                'wp_ajax_cashcred_pay_now',
                [$this, 'cashcred_pay_now'],
                10,
                2
            );
            add_action(
                'wp_ajax_nopriv_cashcred_pay_now',
                [$this, 'cashcred_pay_now'],
                10,
                2
            );
        ...
        public function module_run()
        {
            ...
            if (empty($cashcred_instance->active)) {
                return;
            }
            ...
            foreach ($cashcred_instance->active as $id => $data) {
                if ($data['external'] === true) {
                    $this->call(
                        'returning',
                        $cashcred_instance->active[$id]['callback']
                    );
                }
            }
            ...
            if (
                ...
                array_key_exists(
                    $cashcred_instance->gateway_id,
                    $cashcred_instance->active
                ) &&
                $this->can_withdraw_request()
            ) {
                $this->process_new_withdraw_request(
                    $cashcred_instance->gateway_id
                );
            }
        }
        ...
        public function cashcred_pay_now($post_id = false, $auto = false)
        {
            ...
            if (
                $cashcred_pay_method !== false &&
                array_key_exists(
                    $cashcred_pay_method,
                    $cashcred_instance->active
                )
            ) {
                $cashcred_instance->gateway = cashcred_gateway(
                    $cashcred_pay_method
                );

                $cashcred_prefs = mycred_get_option(
                    'mycred_pref_cashcreds',
                    false
                );

                do_action(
                    'mycred_cashcred_process',
                    $cashcred_pay_method,
                    $cashcred_prefs
                );
                do_action(
                    "mycred_cashcred_process_{$cashcred_pay_method}",
                    $cashcred_prefs
                );

                $payment_response = $cashcred_instance->gateway->process(
                    $post_id
                );

                if ($payment_response['status'] == true) {
                    $history_comments = $this->cashcred_update_payment_status(
                        $post_id,
                        $auto
                    );
                    $payment_response['cashcred_total'] =
                        $history_comments['cashcred_total'];
        ...
        {
            $counter = (int) mycred_get_post_meta(
                $this->post_ID,
                'cashcred_log_counter',
                true
            );
            $orderdesc = $counter;
            ...
            for ($log = 1; $log <= $counter; $log++) {
                ...
                $payment_log = mycred_get_post_meta(
                    $this->post_ID,
                    'cashcred_log_' . $orderdesc,
                    true
                );

                $logdata .= "<pre>";
                $logdata .=
                    "<b>Date Time: </b>" . $payment_log['datetime'] . "<br>";
                $logdata .=
                    "<b>Payment Gateway: </b>" .
                    $payment_log['payment_gateway'] .
                    "<br>";
                $logdata .= print_r(
                    json_decode($payment_log["response"]),
                    true
                );
                $logdata .= "</pre>";
        ...
        public function response($status, $message, $is_auto)
        {
            $response = [
                ...
                'total' => isset($message['cashcred_total'])
                    ? $message['cashcred_total']
                    : '',
                'comments' => isset($message['history_comments'])
                    ? $message['history_comments']
                    : '',
        ...
        public function cashcred_update_payment_status($post_id, $manual)
        {
            $time = get_post_meta(
                $post_id,
                'cashcred_payment_transfer_date',
                true
            );
            ...
            $author_email = apply_filters(
                'mycred_cashcred_comment_email',
                'cashcred-service@mycred.me'
            );
            $get_payment_settings = cashcred_get_payment_settings($post_id);
            $currency = $get_payment_settings->currency;
            $amount =
                $get_payment_settings->points * $get_payment_settings->cost;
            $amount_set = $currency . ' ' . $amount;
            $comment =
                'Withdrawal payment request processed by %s with the amount %s.';
            ...
            $response_send['cashcred_total'] =
                $currency . ' ' . $cashcred_total;
        ...
        public function register_assets()
        {
            wp_register_style(
                'cashcred-withdraw',
                plugins_url('assets/css/withdraw.css', MYCRED_CASHCRED),
                [],
                MYCRED_CASHCRED_VERSION,
                'all'
            );
            wp_register_script(
                'cashcred-withdraw',
                plugins_url('assets/js/withdraw.js', MYCRED_CASHCRED),
                ['jquery'],
                MYCRED_CASHCRED_VERSION,
                'all'
            );
        ...
        public function save_user_payment_methods()
        {
            ...
            foreach ($_POST['cashcred_user_settings'] as $type_id => $value) {
                ...
                foreach ($value as $field_id => $field_value) {
                    $payment_method_data[$field_id] = sanitize_text_field(
                        $field_value
                    );
                }
        ...
        public function process_new_withdraw_request($gateway_id)
        {
            ...
            $cashcred_pay_method = sanitize_text_field(
                $_POST['cashcred_pay_method']
            );
            ...
            $currency =
                $mycred_pref_cashcreds['gateway_prefs'][$gateway_id][
                    'currency'
                ];
            $cost =
                $mycred_pref_cashcreds['gateway_prefs'][$gateway_id][
            ...
            $user_balance = mycred_get_users_balance(
                get_current_user_id(),
                $point_type
            );
            ...
            if ($post_id !== null && !is_wp_error($post_id)) {
                ...
                if (
                    isset(
                        $mycred_pref_cashcreds['gateway_prefs'][
                            $cashcred_pay_method
                        ]["allow_auto_withdrawal"]
                    ) &&
                    $mycred_pref_cashcreds['gateway_prefs'][
                        $cashcred_pay_method
                    ]["allow_auto_withdrawal"] == "yes"
                ) {
                    $cashcred_auto_payment = $this->cashcred_pay_now(
                        $post_id,
                        true
                    );

                    $this->notification_message(
                        $cashcred_auto_payment['message'],
                        $requested_url
                    );
                }
        ...
        public function can_withdraw_request()
        {
            ...
            if (
                ...
                wp_verify_nonce(
                    $_POST['cashcred_withdraw_wpnonce'],
                    'cashCred-withdraw-request'
                )
            ) {
                $response = true;
            }
        ...
        public function can_save_settings()
        {
            ...
            if (
                isset($_POST['cashcred_save_settings']) &&
                !empty($_POST['cashcred_user_settings']) &&
                !empty($_POST['cashcred_settings_wpnonce']) &&
                wp_verify_nonce(
                    $_POST['cashcred_settings_wpnonce'],
                    'cashCred-payment-settings'
                )
            ) {
                $response = true;
            }
        ...
        public function process_new_request()
        {
            ...
            if (
                $cashcred_instance->checkout === false &&
        ...
        public function enqueue_footer()
        {
            ...
            if ($cashcred_withdraw) {
                ...
                $cashcred_user_settings = mycred_get_user_meta(
                    get_current_user_id(),
                    'cashcred_user_settings',
                    '',
                    true
                ...
                foreach (
                    $cashcred_instance->active
                    as $active_gateway_key => $active_gateway_value
                ) {
                    $currency_value =
                        $mycred_pref_cashcreds['gateway_prefs'][
                            $active_gateway_key
                    ...
                    if (
                        !empty($cashcred_user_settings) &&
                        is_array($cashcred_user_settings) &&
                        array_key_exists(
                            $active_gateway_key,
                            $cashcred_user_settings
                        )
                        ...
                        foreach (
                            $cashcred_user_settings[$active_gateway_key]
                            as $field
                        ) {
                            if (empty($field)) {
   ...
   ?>"></span><?php echo $this->core->template_tags_general(
				...
				<div class="row">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="form-group">
							<div>&nbsp;</div>
							<label for="buycred-gateway-<?php echo $key; ?>"><input type="checkbox" name="mycred_pref_cashcreds[active][]" id="cashcred-gateway-<?php echo $key; ?>" value="<?php echo $key; ?>"<?php if (
    $this->is_active($key)
) {
    echo ' checked="checked"';
					...
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						...
						<div class="form-group">
							<div>&nbsp;</div>
							<label for="buycred-gateway-<?php echo $key; ?>-sandbox"><input type="checkbox" name="mycred_pref_cashcreds[gateway_prefs][<?php echo $key; ?>][sandbox]" id="cashcred-gateway-<?php echo $key; ?>-sandbox" value="<?php echo $key; ?>"<?php if (
    $sandbox_mode
) {
    echo ' checked="checked"';
...
<?php
			...
			<div class="body" style="display:none;">
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
						...
						<div class="form-group">
							<label for="mycred_pref_core_cashcreds_debugging"><?php _e(
           'Payment Debugging Log',
           'mycred'
       ); ?></label>
							<select class="form-control" name="mycred_pref_core[cashcreds][debugging]" id="mycred_pref_core_cashcreds_debugging">
								<option value="disable" <?php echo $cashcred_prefs['debugging'] != 'enable'
            ? 'selected="selected"'
            : ''; ?>>Disabled</option>
								<option value="enable" <?php echo $cashcred_prefs['debugging'] == 'enable'
            ? 'selected="selected"'
            : ''; ?>>Enable</option>
							</select>
							<p><span class="description"><?php _e(
           'Payment Debugging log for developers.',
           'mycred'
       ); ?></span></p>
						</div>
        ...
        {
            $new_data['cashcreds']['debugging'] = sanitize_text_field(
                $data['cashcreds']['debugging']
            );
