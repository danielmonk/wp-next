    ...
    class myCRED_cashcred_Bank_Transfer extends myCRED_Cash_Payment_Gateway
    {
        ...
        public function process($post = false)
        {
            ...
            return [
                'status' => true,
                'message' =>
                    'Amount transfer successfully to your bank account',
                'date' => $time,
            ];
        }
        ...
        {
            add_filter('mycred_setup_gateways', [$this, 'relable_gateway']);
        }
        ...
        {
            if (
                !empty($this->prefs['title']) &&
                $this->prefs['title'] != $installed['bank']['title']
            ) {
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id('minimum_amount'); ?>"><?php _e(
    ...
    'minimum_amount'
); ?>"  min="1" value="<?php echo esc_attr(
    $prefs['minimum_amount']
); ?>" class="form-control" />
		</div>
		 
		<div class="form-group">
			<label for="<?php echo $this->field_id('maximum_amount'); ?>"><?php _e(
    'Maximum Points Withdrawal',
    'mycred'
); ?></label>
			<input type="number" name="<?php echo $this->field_name(
       'maximum_amount'
    ...
    $prefs['enable_additional_notes'],
    1
); ?> /> <?php _e('Enable Additional Notes', 'mycred'); ?></label>
		</div>
...
<div class="row" id="additional_notes_show">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="cashcredbanktransferaccount"><?php _e(
       'Additional notes',
       'mycred'
   ); ?></label>
			<?php wp_editor($prefs['additional_notes'], 'cashcredbanktransferaccount', [
       'textarea_name' => $this->field_name('additional_notes'),
       'textarea_rows' => 10,
   ]); ?>
		</div>
	</div>
...
<?php
        ...
        public function cashcred_payment_settings($data)
        {
            ...
            $additional_notes_on =
                $mycred_pref_cashcreds["gateway_prefs"]["bank"][
                    "enable_additional_notes"
                ];
            $additional_notes =
                $mycred_pref_cashcreds["gateway_prefs"]["bank"][
        ...
        public function form_fields()
        {
            ...
            return apply_filters(
                'mycred_cashcred_bank_transfer_fields',
                $gateway_fields
            );
        }
        ...
        public function sanitise_preferences($data)
        {
            ...
            $new_data['additional_notes'] = sanitize_text_field(
                $data['additional_notes']
            );
            $new_data['enable_additional_notes'] = sanitize_text_field(
                $data['enable_additional_notes']
            );
            $new_data['minimum_amount'] = sanitize_text_field(
                $data['minimum_amount']
            );
            $new_data['maximum_amount'] = sanitize_text_field(
                $data['maximum_amount']
            );
            $new_data['additional_notes'] = wp_kses_post(
                $data['additional_notes']
            );
