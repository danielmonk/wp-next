...
if (!function_exists('mycred_get_cashcred_gateways')):
    function mycred_get_cashcred_gateways()
    {
        ...
        $installed['bank'] = [
            ...
            'callback' => ['myCRED_cashcred_Bank_Transfer'],
...
if (!function_exists('cashcred_display_message')):
    ...
    {
        $cashcred_notice = mycred_get_user_meta(
            get_current_user_id(),
            'cashcred_notice',
            '',
            true
        ...
        if (!empty($cashcred_notice)) { ?> 
			...
			<?php if (!isset($_POST['cashcred_withdraw_wpnonce'])) {
        ...
        if (empty($gateways)) {
            ...
            foreach ($gateways as $gateway_id) {
                if (array_key_exists($gateway_id, $cashcred_instance->active)) {
                    $gateways_list[$gateway_id] =
                        $cashcred_instance->active[$gateway_id];
                }
            }
        }
...
if (!function_exists('cashcred_get_payment_settings')):
    function cashcred_get_payment_settings($payment_id = null)
    {
        ...
        if (is_array($payment_id)) {
            ...
            $pending_payment->point_type = check_site_get_post_meta(
                $payment_id,
                'point_type',
                true
            );
            $pending_payment->points = check_site_get_post_meta(
                $payment_id,
                'points',
                true
            );
            $pending_payment->cost = check_site_get_post_meta(
                $payment_id,
                'cost',
                true
            );
            $pending_payment->currency = check_site_get_post_meta(
                $payment_id,
                'currency',
                true
            );
            $pending_payment->gateway_id = check_site_get_post_meta(
                $payment_id,
                'gateway',
                true
            );
...
if (!function_exists('cashcred_add_comment')):
    function cashcred_add_comment(
        $payment_id = null,
        $comment = null,
        $time = null
    ) {
        if (!MYCRED_CASHCRED_PENDING_COMMENTS) {
            return true;
        }
...
if (!function_exists('cashcred_gateway')):
    function cashcred_gateway($gateway_id = null)
    {
        ...
        if (array_key_exists($gateway_id, $installed)) {
            ...
            // Construct Gateway
            $cashcred_gateway = new $class(
                $mycred_modules['solo']['cashcred']->gateway_prefs
            );
        }
...
if (!class_exists('CashCred_Gateway_Fields')):
    class CashCred_Gateway_Fields
    {
        ...
        private function populate()
        {
            ...
            foreach (
                $this->gateway_fields
                as $gateway_field_id => $gateway_field_data
            ) {
                $field_value = '';

                if (
                    !empty(
                        $user_payment_details[$this->gateway_name][
                            $gateway_field_id
                        ]
                    )
                ) {
        ...
        public function generate_form()
        {
         ...
         $gateway_field_id
     ); ?>" class="<?php echo $gateway_field_data[
    'classes'
]; ?>" placeholder="<?php echo $gateway_field_data[
    'placeholder'
]; ?>" value="<?php echo $this->{$gateway_field_id}; ?>">
				</div>
...
if (!function_exists('check_site_add_post_meta')) {
     ...
     * @return mixed
     */
    function check_site_add_post_meta(
        $post_id,
        $meta_key,
        $meta_value,
        $unique = false
