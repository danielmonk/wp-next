...
if (!function_exists('mycred_render_cashcred')):
    function mycred_render_cashcred($atts = [], $content = '')
    {
        extract(
            shortcode_atts(
                [
        ...
        $button_label = $point_types[
            current(array_keys($point_types))
        ]->template_tags_general($button);
...
<div id="cashcred" class="container">
	<ul class="cashcred-nav-tabs">
		<li id="tab1" class="active"><a href="javascript:void(0);"><?php _e(
      'Withdraw Request',
      'mycred'
  ); ?></a></li>
		<li id="tab2"><a href="javascript:void(0);"><?php _e(
      'Approved Requests',
      'mycred'
  ); ?></a></li>
		<li id="tab3"><a href="javascript:void(0);"><?php _e(
      'Cancelled Requests',
      'mycred'
  ); ?></a></li>
		<li id="tab4"><a href="javascript:void(0);"><?php _e(
      'Payment Settings',
      'mycred'
  ); ?></a></li>
	</ul>
	<div id="cashcred_tab_content">
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<?php echo get_post_meta($post->ID, 'currency', true) .
           " " .
           get_post_meta($post->ID, 'points', true) *
               get_post_meta($post->ID, 'cost', true); ?>
						</td>
						<td><?php echo mycred_get_types()[
          get_post_meta($post->ID, 'point_type', true)
      ]; ?></td>
						<td>
					 		<?php
        ...
        ?>
			...
			<?php } else {$mycred_cashcred_gateway_notice = apply_filters(
           'mycred_cashcred_gateway_notice',
       ...
       'mycred'
        ...
        'cashcred_withdraw_wpnonce'
				...
				<?php if (count($point_types) > 1) { ?>
					<div class="form-group"> 
						...
						<select id="cashcred_point_type" name="cashcred_point_type" class="form-control">
							<?php foreach ($point_types as $point_type_id => $point_type_obj) {
           echo '<option value="' .
               $point_type_id .
               '">' .
               esc_html($point_type_obj->plural()) .
               '</option>';
       } ?>
						</select>
					</div>
				...
				<?php if (count($gateways) > 1) { ?>
					<div class="form-group"> 
						...
						<select id="cashcred_pay_method" name="cashcred_pay_method" class="form-control">
							<?php foreach ($gateways as $gateway_id => $gateway_data) {
           echo '<option value="' .
               esc_attr($gateway_id) .
               '">' .
               esc_html($gateway_data['title']) .
               '</option>';
       } ?>
						</select>
					</div>
         ...
         current(array_keys($gateways))
     ); ?>" />
				<?php } ?>

			<div class="form-group">  
				<label><?php echo sprintf(
        __('Withdraw %s value', 'mycred'),
        $point_types[current(array_keys($point_types))]->plural()
    ); ?></label>
				<?php $amount = !empty($amount) ? floatval($amount) : 0; ?> 
				<input type="number" id="withdraw_points" name="points" class="form-control" placeholder="0" value="<?php echo !empty(
        $amount
    )
        ? $amount
        : 0; ?>" required />
		...
		<div id="tab2c" class="cashcred-tab">
			...
			<?php $posts = cashcred_get_withdraw_requests('Approved'); ?>		
			<table>
				<thead>
					<tr>
						<th><span class="nobr">ID</span></th>
       ...
       ?>
		...
		<div id="tab3c" class="cashcred-tab">
			...
			<?php $posts = cashcred_get_withdraw_requests('Cancelled'); ?>
			<table>
				<thead>
					<tr>
						<th><span class="nobr">ID</span></th>
			...
			<form action="" method="POST">
				<?php if (count($gateways) > 1): ?>
					<select class="form-control" name="cashcred_save_settings" id="cashcred_save_settings">
						<?php foreach ($gateways as $key => $active_gateways_value) {
          echo '<option value="' .
              $key .
              '"> ' .
              $active_gateways_value['title'] .
              ' </option>';
      } ?>
					</select>
				<?php else: ?>
    ...
    foreach ($gateways as $key => $active_gateways_value) {
        $MyCred_payment_setting_call = new $active_gateways_value[
            'callback'
        ][0]($key);
